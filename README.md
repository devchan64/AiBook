...
이 저장소는 단순한 자료 모음이 아니라, `기초 복구 -> 머신러닝 -> 딥러닝 -> LLM과 생성형 AI -> 프로젝트`로 이어지는 재학습 경로를 책처럼 읽을 수 있게 만드는 것을 목표로 합니다. 특히 `AI`, `머신러닝(machine learning)`, `딥러닝(deep learning)`, `생성형 AI(generative AI)`, `LLM(large language model)`이 서로 어떻게 연결되는지 다시 설명할 수 있는 상태로 돌아가는 것을 목표로 합니다.
