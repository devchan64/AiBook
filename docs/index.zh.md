...
在同一个 Part 内，重要概念的详细说明会尽量只放在一个代表 Section 中。后续 Section 只保留当前语境所需的最小连接。如果反复出现的核心术语再次变得不稳定，可以回到 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary/) 查看。词汇表中的 `Core Section` 和 `Appears In` 列表，可以帮助区分某个概念第一次被完整解释的位置，以及它后来在哪些地方再次出现。
...
- 韩语：当前持续写作与审校的主版本
- 英语：已开放介绍页、目录页、Part 1 全部、Part 2 全部、Part 3 全部，以及 Part 4 的部分内容
- 简体中文：已开放介绍页、目录页、Part 1 全部、Part 2 全部、Part 3 全部，以及 Part 4 的部分内容

因此，现阶段英文版和简体中文版都已经不再只是占位页面，而是已经开放了 Part 1 到 Part 3 的完整内容；与韩语主版本相比，目前仍在继续补充 Part 4 及后续 Part。
...
- [Concept Glossary (English)](/AiBook/en/reference/concept-glossary/)
