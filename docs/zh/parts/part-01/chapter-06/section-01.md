...
规则并没有从 AI 中消失。它们在政策、安全、权限、流程和校验上依然很强。但现实问题经常带着信息不完整、观测不稳定、例外增多和候选项过多这些条件。
...
6.2 会顺着这条线继续，把 `probability`、`uncertainty` 和 `stochastic` 这些表达分开。
...
当你需要重新解释“规则明明是必要的，但现实问题为什么还会同时需要别的方法”时，就可以先想起这一节。

- 政策和流程都很清晰，但输入会波动，或者关键信息缺失，导致规则无法闭合时
- 例外不断增加，规则优先级和冲突管理越来越复杂时
- 需要把“为什么规则之后会接着出现概率、搜索、启发式和学习”连成一条线说明时

这时先把当前问题分成 `信息不足`、`部分可观测`、`噪声`、`例外` 和 `候选项爆炸` 里的哪一种或哪几种，再看为什么规则不够、还需要什么补充方法，会更清楚。
