...
`uncertainty`、`probability` 和 `stochastic` 是互相关联的，但不是同一个词。

> uncertainty 是“还不知道”的状态。  
> probability 是把这种状态写成数字的语言。  
> stochastic 是过程或结果本身带有概率性波动的性质。

必须先抓住这个区分，后面在读启发式、概率模型、机器学习和 LLM 生成设置时，才不会把不同层次都压扁成一句模糊的话。
