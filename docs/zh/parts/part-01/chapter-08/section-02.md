...
这一节不会深入 representation learning。后面的章节会把它和深度学习、embedding 更直接地连起来。这里只先保留：无监督学习可以和“通过改变数据表示来让结构更可见”这一件事相关。
...
无监督学习是在没有人提供标签的情况下，从数据中寻找 `structure`、`cluster` 和 `representation` 的方式。

> unsupervised learning = 不依赖人预先给出的标签，  
> 而是在数据内部寻找结构的学习方式

没有标签，不代表没有目标。目标只是从“拟合标签”转向了“发现数据内部隐藏的分组、轴与形式”。也正因为如此，无监督学习结果更适合被当成发现的起点，而不是可以直接拿来当业务真理的终点。
...
当你开始觉得“没有标签”这四个字已经足够解释无监督学习时，就该先把这一节的视角拿出来。

- 当你需要把监督学习与无监督学习的差别，从“有没有标准答案”改写成“它们在问什么问题”时
- 当你想把 cluster 编号或降维坐标直接读成业务意义或真相时
- 当你需要整理 clustering、dimensionality reduction 和 representation 各自承担什么角色时

这时先分开 `什么算相似`、`要找什么结构`、`结果由谁来解释`。这样更容易把无监督学习读成结构发现问题，而不是一种模糊的整理技巧。
