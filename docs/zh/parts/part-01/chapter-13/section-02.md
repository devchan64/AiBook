...
相似度搜索是在向量空间中寻找接近候选的过程。但接近候选并不等于正确答案。
...
只要抓住这个视角，就能在 P1-13.3 理解 RAG 时避免把检索过程夸大。RAG 会把搜索出的候选放进 LLM 输入，但搜索结果本身的质量与审查责任仍然重要。

## 什么时候要先想起这个视角

当你准备把搜索结果直接当成答案，或者开始混淆相似度搜索与字符串搜索时，就应该先把本节的视角拿出来。

- 当需要重新解释“问题与文档为什么算接近”时
- 当需要说明为什么 top-k 会一次取回多个候选时
- 当需要再次确认“搜索结果只是相关候选，不保证答案正确”时

这时，先分开 `比较向量`、`挑选接近候选`、`再次审查候选` 三件事，就能更准确地说明 RAG 前面的检索阶段。
