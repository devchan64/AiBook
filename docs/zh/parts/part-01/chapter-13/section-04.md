...
向量搜索实现，是把“寻找接近向量”这个数学想法，变成能在真实服务里高效运行的结构。

> 向量少时，可以全部比较。  
> 向量多时，就需要索引。  
> 近似搜索是在速度与质量之间做折中。  
> 基于图的搜索会沿着接近连接缩小候选范围。  
> 向量数据库会同时处理存储、搜索、过滤与运维。

只要抓住这个视角，在 P1-14 看 AI 服务架构时，就更容易把 RAG 理解成由数据存储、检索、模型输入组装与运维约束共同组成的系统，而不是单一功能。
...
当你开始把向量搜索只理解成一句“找接近的东西”，需要重新解释为什么实际服务里必须引入索引和近似搜索时，就应该先回到本节。

- 当需要说明为什么数据一大，全量比较(brute-force search)就会变成负担时
- 当需要说明近似最近邻(approximate nearest neighbor, ANN)不是“低质量搜索”，而是速度与质量的折中时
- 当需要把向量数据库(vector database)理解成包含存储、过滤与运维的系统，而不只是单一算法时

这时，先拆开 `全量比较`、`索引`、`近似搜索`、`运维平衡` 四个层面，就更容易把搜索实现解释成服务结构上的直觉，而不是产品名列表。
