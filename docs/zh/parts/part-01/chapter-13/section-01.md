...
把文本表示成向量，意思就是把文本转换成可计算的位置。利用这个位置，就可以比较文本之间的关系，并找出与问题接近的文档候选。
...
下一节会接着看：这些向量实际是怎样被比较的。

读完本节后，至少应该能够重新说明下面三种区分：

| 需要保留的核心区分 | 为什么重要 |
| --- | --- |
| 嵌入是把文本变成可计算向量表示的方法。 | 这能让你在阅读向量检索与 RAG 时，不会忘记“为什么先需要数字表示”。 |
| 接近的向量更可能代表相似的使用语境，而不等于同一句话。 | 这能避免把向量空间夸大成意义本身，从而让后面的相似度搜索说明更准确。 |
| 提示词负责给出任务条件，嵌入负责让检索与比较成为可能。 | 这能在阅读 Part 1 后半服务流程时，分清两者的不同角色。 |
...
当你想解释外部资料检索或 RAG，却跳过了“为什么要先把文本变成向量”这一步时，就该先回到这一节。

- 需要把问题和文档拿来比较，但不能只靠原始字符串时
- 把提示词(prompt)和嵌入(embedding)混成同一种东西时
- 需要重新说明为什么句子、段落、文档会按不同单位被向量化时

这时可以先拆成 `把文本变成向量`、`比较这些向量`、`提示词负责给任务条件`。这样就更容易说明：为什么在检索开始之前，先需要做表示转换。
