...
- 具体机器学习算法的内部结构
- `feature`、`representation`、`parameter` 的详细计算
- 完整服务架构的设计
...
| 概念 | 这里先固定的意思 | 为什么现在需要 |
| --- | --- | --- |
| [model](/AiBook/en/reference/concept-glossary.md#model) | 接收输入并计算输出的核心计算部件 | 为了把 AI 里的 `model` 和现实整体区分开 |
| [system](/AiBook/en/reference/concept-glossary.md#system) | 把模型、规则和人工复核包在一起的整体结构 | 为了避免把整个服务误当成一个模型 |
| [input](/AiBook/en/reference/concept-glossary.md#input) | 系统接收到的信息 | 为了看模型到底拿什么做计算材料 |
| [output](/AiBook/en/reference/concept-glossary.md#output) | 系统产生的结果 | 为了看模型到底算出什么 |
...
模型不是现实整体。模型是为了某个目的而缩减出来的可计算表示，用来处理现实问题的一部分。
...
> 它是用什么数据训练出来的？  
> 它刻意排除了什么？  
> 它的输出会在真实 system 里怎样被使用？

下一节会把这些问题重新整理到 `input`、`output` 和 `data` 上。
...
当你听到 `model` 这个词，需要先把它从“现实整体”重新拆回“计算部件”时，就该先想起这一节的视角。

- 当你开始把模型表现直接等同于整个业务问题被解决时
- 当你开始把 LLM 或分类模型说成和整个服务差不多时
- 当你需要分别解释模型做什么、而政策、复核和执行流程又做什么时

这时先回到这个基准：`model 是为了目的缩减出来的可计算表示`，`system 是围绕这个 model 展开的更大流程`。然后再去看模型接收什么输入、产生什么输出，以及它根本不负责什么，角色边界就会清楚起来。
