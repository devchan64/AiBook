...
- `feature`、`representation` 与 `parameter` 的细部差别
- 学习算法与损失函数计算
- 完整的数据流水线与服务运营结构
...
现实问题不会自动变成模型问题。要让它变成模型问题，必须先定义输入、输出和数据。

输入是模型能观察到的东西，输出是交给模型的任务，数据则是把这种输入和输出连接起来的一组案例。即使面对同一个现实问题，只要输入和输出定义不同，所需数据也会不同。
...
只有先把这三件事整理好，才能进入下一步，继续谈特征、表征和参数。

4.3 会更仔细地看：这些输入是如何在模型内部连接到 `feature`、`representation` 和 `parameter` 的。
...
当你在看模型性能之前，更需要先回头问 `我们到底放进了什么、想得到什么、又收集了哪些案例` 时，就该把这一节的视角拿出来。

- 当同一个业务目标下，不同团队却在想象不同输入信息或不同输出形式时
- 当模型总出错，而真正的问题看起来像是少了订单状态、上下文等输入信息时
- 当数据明明很多，却仍要解释为什么它学不会你真正想要的任务时

这时，就先把 `input`、`output`、`data`、`example` 和 `label` 再分开一次。再往下判断：当前问题主要更像输入定义问题、输出定义问题，还是数据定义问题。这样会更快缩小原因。
