...
理解生成式 AI 时，把它想成“直接拿出完成答案”是有风险的。更安全的比喻是：
...
但这个直觉不能抹平不同模型之间的差异。文本生成适合先用“下一个 token”来理解，语音生成也可以用顺序样本生成来说明。图像生成，特别是 diffusion 系列，则需要单独理解成从噪声中逐步恢复。

这里先记住下面三行就够了：

| 现在先记住这个 | 之后回来看这里 |
| --- | --- |
| 文本生成先理解成 `不断接续下一个 token 的过程`。 | Part 6 的 P6-1.1、P6-1.2、P6-5.1 |
...
当生成式 AI 让人觉得它像是整块取出完整答案，需要重新说明其内部的重复生成过程时，就可以先想起本节。

- 当需要重新建立文本生成里 `下一 token(next token)` 直觉为什么重要时
- 当需要区分语音、图像生成虽然都带有重复生成的共同点，但单位和方式不同时时
- 当需要解释即使是同一个 prompt，不同模型和生成方式内部流程也会不同时

此时可以先分开问三个问题：`下一个输出单位是什么`、`这个单位如何在条件之下被选择或恢复`、`前一步输出如何继续成为下一步的条件`。这样更容易把生成过程理解成计算性的构造，而不是魔法式取出。
