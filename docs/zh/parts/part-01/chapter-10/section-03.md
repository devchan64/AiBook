...
本书是借助 AI 工具制作的。所以不会把生成结果原样相信，而是按照下面的流程处理：
...
因此，本节内容也直接连接到这个仓库的写作原则。AI 可以很快地产出草稿，但这本书的可信度不是来自起草速度，而是来自审查流程。

## 本节应记住的视角

生成式 AI 的结果可以很自然，但自然度并不能代替事实性、依据、安全性与权利审查。
...
当生成结果因为看起来自然而让人想把事实性、依据、安全与权利问题往后放时，就可以先想起本节的视角。

- 当需要再次强调“句子写得好”不等于事实和依据也已成立时
- 当需要把生成结果的风险从单纯的错误扩大到信息完整性、隐私、安全、版权与过度信任时
- 当需要在 AI 草稿进入公开文档或高风险场景前，明确写出人工审查流程时

此时可以先把 `自然度`、`准确性`、`依据`、`使用场景`、`权利与责任` 分开来看。这样就能减少因为结果看起来合理而跳过审查的错误。
