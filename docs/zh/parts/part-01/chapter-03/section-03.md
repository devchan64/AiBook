...
这里的重点，是整理出 `规则在外面，表征在里面` 这个区分。输入、输出、数据、特征和参数的更系统整理，会在紧接着的 Part 1 Chapter 4 再回来。
...
显式规则是人能够直接阅读的判断标准。它在解释和控制方面很强，但随着例外增加、表达方式多样化，维护会越来越困难。

学到的表征，是模型内部把输入变成可计算形式后的结果。它对复杂模式和相似性很强，但不是一份人可直接阅读的规则清单。

这个差别会直接连到后面的章节。Part 1 Chapter 4 会处理把现实问题转成输入、输出、数据和特征的过程，而 Part 4 与 Part 5 会继续深入模型是如何调整参数与表征的。
...
当问题已经不再只是“选规则还是选模型”，而是“输入该以什么形式被处理”时，就应该把这一节的视角重新拿出来。

- 当同样意思的句子不断换着说法出现，导致单词规则开始失效时
- 当 feature、representation、vector、activation 和 parameter 都开始像同一种内部术语时
- 当你需要解释为什么政策应固定在规则里，而语义相似性应交给模型时

这时，可以重新按 `规则在外，表征在内` 的标准来划分。先分清这是人必须直接控制的标准，还是应该让系统从数据中学习相似性的部分。这样系统设计和解释都会更容易。
