...
在 Part 1 中，分类、预测、推荐、生成等问题类型的基准区分，就固定在这一节。后面再出现这些词时，只会保留当前问题所需的最小连接；如果需要重新拆分问题类型本身，就回到这一节，以及 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md)。
...
不同问题类型的算法与学习结构，会在 Part 4、Part 5 与 Part 6 重新回收。这里先固定一条基准线：`看清放进去什么、拿出来什么。`
...
| 概念 | 这里先固定的意思 | 为什么现在需要 |
| --- | --- | --- |
| [recognition](/AiBook/en/reference/concept-glossary.md#recognition) | 从输入中读出对象或状态的问题 | 为了先区分比分类更宽的“读取”问题 |
| [classification](/AiBook/en/reference/concept-glossary.md#classification) | 从预先给定的类别中选出一个的问题 | 为了固定一个“猜什么”很清楚的代表问题 |
| [prediction](/AiBook/en/reference/concept-glossary.md#prediction) | 根据当前信息估计未来值或结果的问题 | 为了区分“输出值或可能性”的问题与分类 |
| [recommendation](/AiBook/en/reference/concept-glossary.md#recommendation) | 从多个候选中挑出接下来值得看的内容 | 为了看出 AI 不只是在生成答案 |
| [generation](/AiBook/en/reference/concept-glossary.md#generation) | 生成新文本、图像、音频、代码的问题 | 为了固定为什么输出性质不同 |
| [input](/AiBook/en/reference/concept-glossary.md#input) | 系统接收的信息 | 为了从“放进去什么”开始阅读问题定义 |
| [output](/AiBook/en/reference/concept-glossary.md#output) | 系统产生的结果 | 为了按“出来什么”区分问题类型 |
| [goal](/AiBook/en/reference/concept-glossary.md#goal) | 判断什么样的输出算好的标准 | 为了看到同样数据会因问题变化而变成别的问题 |
...
`input`、`output`、`goal`、`impact` 在上一节已经出现过，作为阅读 AI 系统的基本语言。这里则把它们重新接到问题类型上。输入是系统接收的信息，输出是系统产生的结果，目标是界定“什么样的结果算好”的标准，而 `impact` 则表示这个结果最后会碰到人的判断或环境变化的哪个位置。如果这些词再次不稳定，就回到上一节和 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md)；在这一节里，它们主要是帮助我们把问题读成“放进去什么、拿出来什么”的结构。
...
- AI 问题应先看清输入和输出。
- 同样的数据，只要问题一变，就会变成完全不同的问题。
- 真实服务常会把检索、排序、生成等多个问题类型放进同一条流程。

此阶段至少要留下的一句话是：`同样的数据，只要问题改变，就会变成不同的 AI 问题。`

进一步压缩成三条，就是：

| 先留下的基准 | 为什么这条先重要 |
| --- | --- |
| AI 问题先按 `input -> output` 来读 | 因为即使技术名字不同，也能先把共同结构重新抓住 |
| 同样的数据会因问题变化而变成预测、推荐或生成 | 因为在导论阶段，问题定义比算法名字更先被固定 |
| 真实服务会把搜索、排序、生成等多个问题绑成一条流程 | 因为这样后面的 Part 里看到一个服务内部同时出现多种结构时，就不会太意外 |
...
当下面这些情况出现、你一时说不清“这到底是哪一种问题”时，就先回到这一节：

- 同一份客户数据，需要分别被解释成预测、推荐、聚类与生成
- 一个搜索框背后，其实可能同时包含搜索、排序和生成
- 在谈模型名字之前，得先理清“放进去什么、拿出来什么”

这时就先把问题写成四格：输入、输出、目标、影响，再在上面叠加分类、预测、推荐、生成、控制等问题类型。
