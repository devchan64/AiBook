...
这一节的目标，并不是用一句完美的话把 AI 一次性定义完，而是整理这个词的范围，让后面关于规则式系统、机器学习、深度学习、生成式 AI 和 LLM 的内容都能放在同一张地图上阅读。

在 Part 1 中，`AI` 是最外层、最宽的类别。后面的 Section 再次出现这个词时，只会保留当前问题所需的最小连接；如果需要重新拆分这个范围，就回到这一节，以及 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md)。
...
这一节也是几个核心术语第一次被同时引入的入口。下面这些概念先只固定最短含义；如果之后需要更完整的定义，再回到对应词条。由于同一个韩语词在映射到英语时可能会分成不同概念，因此这里仍然以词汇表中的英韩对照为准。

| 概念 | 这里先固定的意思 | 为什么现在需要 |
| --- | --- | --- |
| [AI](/AiBook/en/reference/concept-glossary.md#ai-artificial-intelligence) | 最宽的外层类别 | 为了把后面出现的多种方法放在同一张地图上 |
| [machine learning](/AiBook/en/reference/concept-glossary.md#machine-learning) | 从数据中学习模式的方法 | 为了区分规则式方法和学习式方法 |
| [deep learning](/AiBook/en/reference/concept-glossary.md#deep-learning) | 用神经网络学习复杂表征的一条技术主线 | 为了固定机器学习内部的重要扩展方向 |
| [generative AI](/AiBook/en/reference/concept-glossary.md#aigenerative-ai) | 生成文本、图像、音频等结果的一条技术流 | 为了避免把最近的 AI 经验理解得过窄 |
| [LLM](/AiBook/en/reference/concept-glossary.md#llm) | 以语言数据为中心的大规模语言模型 | 为了区分生成式 AI 和 AI 整体 |
| [system](/AiBook/en/reference/concept-glossary.md#system) | 接收输入并产出结果的实际实现结构 | 为了区分“领域”与“实现” |
| [input](/AiBook/en/reference/concept-glossary.md#input) | 系统接收的信息 | 为了阅读系统依据什么做判断 |
| [output](/AiBook/en/reference/concept-glossary.md#output) | 系统产生的结果 | 为了阅读分类、推荐、生成等差异 |
| [goal](/AiBook/en/reference/concept-glossary.md#goal) | 为什么要得到某种输出的目的 | 为了看到同样的输出也可能服务于不同目的 |
| [prediction](/AiBook/en/reference/concept-glossary.md#prediction) | 根据当前信息估计结果 | 为了避免和生成、推荐、分类混成一类 |
| [recommendation](/AiBook/en/reference/concept-glossary.md#recommendation) | 在多个候选中挑出接下来值得看的内容 | 为了看出 AI 不只是在生成答案 |
| [rule-based system](/AiBook/en/reference/concept-glossary.md#rule-based-system) | 由人写规则并导出结论的系统 | 为了避免把 AI 缩窄成只有最近的学习模型 |
...
这一节最先应该留下的基准，是 `AI 是宽广领域`，而 `AI 系统是具有输入、目标与输出的结构`。这里之所以把 `影响(impact)` 也放进来，是因为同一种输出在真实环境中可能导致完全不同的后果。如果术语再次变得不稳定，就回到 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md) 对照看。

例如，若把“在线商店的商品推荐”压缩成一行：AI 是包含这类推荐问题的宽广领域；`recommendation system` 是实际实现的系统；输入是点击记录与购买记录；输出是推荐商品列表；目标是挑出用户下一步可能愿意看的商品；`impact` 是某些商品被更频繁地曝光，用户的选择路径也会随之变化。即使后面换成别的案例，也最好先把它拆成这六格，而不是直接去背术语。
...
- AI 不是某一个产品名，而是一大类接收输入并产出判断或输出的系统。
- 如果只用最近的生成式 AI 使用经验来定义 AI，就很容易忽略规则式系统、搜索、推荐和预测等其他层次。
- 更稳妥的读法，是先拆开输入、目标、输出与影响。

这一节至少要留下的一句话是：`AI 不是一个产品，而是一大类具有输入、目标和输出的系统。` 之后在 Part 1 里再次遇到 `AI` 这个词时，就先把这一句当作基准线；若需要核对术语，再回到 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md)。
...
当下面这些情形让你开始不确定“这算不算 AI”或“为什么它也会被叫作 AI”时，就先回到这一节：

- 一个靠规则表工作的系统，因为不是机器学习而让你觉得“不像 AI”
- 只接触过聊天机器人，于是整个 AI 看起来都像问答服务
- 推荐、审批辅助、预测、搜索系统同时存在时，不知道它们为什么还能归在一个更大的类别里

这时就回到 `AI 是最外层、最宽的类别` 这条基准线上，再重新问：输入、目标、输出和影响是怎样被组织起来的。
