...
这个 Part 不会在每一章里用完全相同的密度重复所有概念。比如 `model`、`input`、`output`、`feature`、`representation`、`parameter` 这些核心词，会先在前面的 Section 里把位置固定下来，后面的 Section 则只重新接回当前问题场景所需的部分。所以当同一个词再次出现时，读者应区分：这次是在补充新定义，还是只是把同一个词放进了不同语境。
