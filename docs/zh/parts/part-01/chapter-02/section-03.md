...
在 Part 1 中，从规则式方法转向学习式方法的历史脉络，以及 `data`、`feature`、`representation` 与 `parameter` 的基本连接，固定在这里。`AI / machine learning / deep learning / generative AI / LLM` 的基本关系已在 1.3 先建立；这里再把它们接回到一条历史脉络中。如果这些术语边界之后再次摇晃，就回到这一节和共享的 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md)。
...
更细的学习结构会在 Part 3、Part 4、Part 5 与 Part 6 再次回收。这里先集中整理 `为什么说明中心会转向学习` 这个大历史问题。
...
| 概念 | 这里先固定的意思 | 为什么现在需要 |
| --- | --- | --- |
| [data](/AiBook/en/reference/concept-glossary.md#data) | 学习与判断的材料 | 为了看见模型不是从手写规则，而是从什么材料里学习标准 |
| [feature](/AiBook/en/reference/concept-glossary.md#feature) | 先由人整理或选出的线索 | 为了理解早期机器学习依赖怎样的输入表示 |
| [representation](/AiBook/en/reference/concept-glossary.md#representation) | 模型内部处理数据的形式 | 为了看出深度学习为何常被解释为表征学习 |
| [parameter](/AiBook/en/reference/concept-glossary.md#parameter) | 在学习中被调整的内部值 | 为了固定“学习到底改变了什么” |
| [machine learning](/AiBook/en/reference/concept-glossary.md#machine-learning) | 从数据中提高性能的学习式方法 | 为了理解为什么中心会从写规则转向训练模型 |
| [deep learning](/AiBook/en/reference/concept-glossary.md#deep-learning) | 以神经网络为基础、强烈依赖表征学习的方法 | 为了理解从特征设计转向学习表征 |
| [generative AI](/AiBook/en/reference/concept-glossary.md#aigenerative-ai) | 生成新内容的模型与服务类别 | 为了看清最近这条技术流延伸到了哪里 |
...
机器学习、深度学习与生成式 AI 名字不同，但可以被读成一条连接的脉络。在人发现“把所有规则都直接写出来”越来越困难的问题上，数据式学习变得重要；深度学习把这种学习从特征设计扩展到学习表征本身；生成式 AI 则更强地显示出，学到的这些表征也可以被用来生成新内容。

但这并不意味着新的方法完全取代了旧方法。规则、搜索、知识表示、概率推理、数据挖掘、机器学习、深度学习与生成式 AI 会一起混在真实系统里。把 AI 历史读成“某个新技术把旧技术擦掉”的历史并不安全；更好的读法是：解题工具一层层增加起来的历史。
...
当规则式方法、机器学习、深度学习与生成式 AI 开始看起来像简单的替代关系时，就应该重新拿出这一节的脉络。

- 当你面对复杂输入问题，需要整理为什么说明中心会从写规则转向从数据中学习时
- 当你想一次性解释 feature、representation、model 与 parameter 分别在哪一层变得重要时
- 当你需要把 LLM 放到更早的 AI 脉络之上，而不是把它讲成完全断开的新事物时

这时，可以重新用 `规则 -> 数据学习 -> 表征学习 -> 生成` 这条累积脉络来整理。然后先把当前案例区分成：是 `人直接写标准的问题`、`必须从数据中学习模式的问题`，还是 `需要生成新内容的问题`。这样说明中心会稳定得多。
