...
在 Part 1 中，`search` 与 `probabilistic reasoning` 的基本区分固定在这一节。`Knowledge representation` 已在 2.1 做过初步介绍，这里只在比较它与搜索和概率推理之间的角色时重新连接；如果这些边界之后又变模糊，就回到这一节与共享的 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md)。
...
| 概念 | 这里先固定的意思 | 为什么现在需要 |
| --- | --- | --- |
| [search](/AiBook/en/reference/concept-glossary.md#search) | 沿着可能状态与行动寻找目标的方法 | 为了看见候选很多时为什么“顺序”会成为问题 |
| [knowledge representation](/AiBook/en/reference/concept-glossary.md#knowledge-representation) | 用来书写事实、关系与约束的形式 | 为了把系统“应该知道什么”的问题单独分出来 |
| [probabilistic reasoning](/AiBook/en/reference/concept-glossary.md#probabilistic-reasoning) | 在不完整信息下处理“多大程度上可信”的方式 | 为了避免把所有判断压成简单的真或假 |
...
当搜索、知识表示与概率推理在同一个问题里混在一起，导致你不知道该先解释什么时，就应该重新拿出这一节的视角。

- 当机器人、路径、派单或游戏案例里，`找候选` 与 `写世界` 没有明显分开时
- 当传感器误差或不完整信息使得单靠规则解释显得不够时
- 当你需要解释搜索、知识图谱与概率预测今天在现代 AI 服务里还承担什么角色时

这时，把 `在找什么`、`要先写下什么` 与 `什么是不确定的` 分开写出来，就会重新看到：search、knowledge representation 与 probabilistic reasoning 不是竞争概念，而是回答不同问题的工具。
