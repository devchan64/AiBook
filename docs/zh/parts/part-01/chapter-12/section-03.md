...
> 提示词可以引导模型输出，  
> 但并不会自动保证事实性(factuality)、依据性(evidence)、安全性(safety)、一致性(consistency)。

在 Part 1 中，本节先建立 `提示词的限制(limit)`、`评估(evaluation)`、`事实性(factuality)`、`依据性(evidence)`、`时效性(recency)`、`幻觉(hallucination)`、`可复现性(reproducibility)` 的入门区分。12.1 讲的是提示词指定了什么，12.2 讲的是指示、上下文与示例，而这里会再拆开一个更重要的区分：
...
这里也不会继续堆砌提示词技巧。更窄的焦点是：

> 为什么提示词本身还不够，  
> 以及为什么后面会需要 RAG、工具使用、代理、harness 这类结构？
...
提示词是整理输出条件的工具；`评估` 则是单独的一项工作，用来检查结果是否符合目的、是否有依据、是否避免制造额外风险。
...
> 12.1 提示词指定了什么  
> 12.2 指示、上下文、示例如何分别发挥作用  
> 12.3 提示词无法保证什么，以及还必须评估什么
...
当人开始觉得“只要再改一下提示词，问题就全都能解决”，需要重新把评估与审查建立成独立工作时，就可以先想起本节。

- 当输出看起来合理，但仍然需要分别检查事实性(factuality)、依据性(evidence)、时效性(recency)时
- 当提示词修改与模型本身限制开始被混为一谈时
- 当需要解释为什么后面会需要 RAG、工具使用、代理、harness 这些结构时

此时，可以先分开看：`输入条件`、`输出评估`、`依据核查`、`可复现性记录`。这样就能同时保留两点：
