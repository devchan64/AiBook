...
提示词并不是“和模型说话的巧妙方式”，而是一种把任务条件安排进模型输入的方法。
...
> 提示词用约束缩小范围。  
> 提示词指定输出格式。  
> 提示词让我们观察输入条件变化如何影响输出。  
> 提示词不会改变模型权重。  
> 提示词不能替代事实核查。

只有先抓住这个视角，12.2 才能更具体地拆开指示、上下文与示例；而在 12.3 讨论提示词的局限与评估标准时，也才能避免夸大提示词的作用。

读完本节后，至少应能重新说出下面三条：

| 需要留下的核心区分 | 为什么这个区分重要 |
| --- | --- |
| 提示词不是重新训练模型的过程，而是把当前回答条件写进输入。 | 只有先区分提示词与学习，后面关于 prompting、in-context learning、fine-tuning 的说明才不容易混在一起。 |
...
当人们把提示词理解成“会说话的技巧”，导致输入设计与学习之间的差别开始变模糊时，就可以先想起本节。

- 当需要重新强调提示词是输入设计，而不是学习过程时
- 当需要说明为什么同一个模型会因为不同输入条件而给出不同结果时
- 当需要在后续进一步拆分指示、上下文、示例之前先建立共同框架时

此时，可以先把 `任务是什么`、`背景是什么`、`模式是什么`、`边界是什么` 分开来看。这样更容易把提示词理解成条件安排，而不是神秘技巧。
