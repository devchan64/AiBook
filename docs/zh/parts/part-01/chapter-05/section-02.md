...
`Inference` 是运行训练好的模型，并为新输入生成输出的过程。

> training 会改变内部值。  
> inference 会使用这些已改变的值。

在传统机器学习里，`inference` 可能看起来像“生成 prediction 的过程”；在 LLM 里，它可能看起来像“生成 response 的过程”。但两者共享同一个更大的结构：训练好的模型被应用到新输入上。

所以，在阅读 AI 系统时，把下面三个时刻分开会很有帮助：

> 模型被训练的时刻  
> 模型通过 inference 被运行的时刻  
> 模型输出被用进更大服务决策的时刻

只要不把这三件事混在一起，像“AI 是不是一边回答一边学习？”、“模型输出是不是已经等于业务决定？”、“inference 和 reasoning 是不是一个词？”这种混淆就会少很多。
...
当你开始把模型给出答案的那一刻，和“思考”“判断”甚至“整个服务处理流程”混成一团时，就该把这一节的视角重新拿出来。

- 当你感觉模型每回答一次，似乎就立刻又学会了一点时
- 当你把 `prediction`、`response generation`、`post-processing` 都看成 inference 本身时
- 当你需要解释模型输出和业务系统最终决定是在哪里分开的时

这时，就先回到下面这个基准：

> inference 是把训练好的模型应用到新输入上的执行

然后再去看：你现在讨论的是模型内部生成输出的阶段，还是把输出接到政策与业务流程上的服务阶段。这样结构就会清楚很多。
