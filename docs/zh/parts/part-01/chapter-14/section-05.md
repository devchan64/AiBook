...
如果这个视角固定下来，那么下一节要讨论的 `AI 服务在现实中遇到的约束` 也会更容易被现实地理解。trace、log 和 evaluation 虽然都能提高质量，但同时也会带来成本、延迟和运维负担。
...
- 当模型已经给出答案，就开始觉得整个服务也已经完成验证时
- 当你需要解释为什么 log、trace 和 evaluation 会被放进单独执行环境中处理时
- 当服务里还没有放入变更之后检查 regression 的装置时

这时先重新拆成 `把执行包起来`、`把步骤记下来`、`做重复比较` 三件事。这样就不会把偶然成功和可重复稳定混成一件事。
