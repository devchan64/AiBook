...
RAG 与工具使用都涉及模型外资源，但它们并不是同一种结构。

> RAG 让模型去读外部资料。  
> 工具使用调用外部系统功能。  
> RAG 用来补强回答依据。  
> 工具使用可能通向查询、计算、保存、修改、发送等动作。  
> 两者都必须放在应用和服务器的权限、校验与执行流程里处理。

只要这个区分站稳，下一节里的 `agent` 就会更容易被安全地理解。Agent 可能会把 RAG 与工具使用串成多个步骤，但在那之前，必须先把每个构件的角色与责任分清。

## 什么时候要先想起这个视角

- 当搜索与工具调用都看起来像一种模糊的“外部连接”，差异开始变模糊时
- 当有人想把 RAG 与工具使用解释成同一种结构时
- 当你想在进入 agent 之前，先把阅读责任与执行责任分开时

这时，就先分清：`是在读资料吗？`、`是在执行外部功能吗？`、`是否需要事前审批与校验？`。这样能避免把 RAG 与工具使用压扁成一种模糊的“在用模型外东西”的概念。
