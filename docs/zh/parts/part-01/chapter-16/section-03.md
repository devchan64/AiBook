...
AI 项目的核心，并不只是“把一个大工具装上去”，而是把一个小问题变成可验证的产出物。

> 从问题出发，而不是从技术标签出发。  
> 先写成功标准。  
> 评估不只看回答质量，也要看依据与运行条件。  
> 失败记录会成为下一轮改进材料。  
> 小项目是验证理解的方式。
...
- 当大家只说“做个项目吧”，但还没有明确验证问题时
- 当结果正要被模糊地凭感觉评价，而不是根据成功标准时
- 当失败不应被抹掉，而应被转化成下一轮需求时

这时，先拆开 `中心问题`、`成功标准`、`评估方式`、`失败记录`，会更容易把项目重新缩回成可验证的学习单位，而不是一个过大的技术口号。
