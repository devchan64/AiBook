...
版权审查的关键，不是只看“有没有写来源”。真正的问题是：用了什么材料、为了什么目的、用了多少、以什么方式使用。
...
> 生成式 AI 训练数据争议仍在继续。  
> AI 输出也必须检查与既有作品的相似性。  
> 公开发布稿件应比私人学习笔记采用更严格标准。

如果说版权问题是“如何处理别人的表达”，那么下一节就会转向平行的问题：
...
- 当版权审查被简化成“只要写出处就行”时
- 当书稿中的引用与 AI 训练数据争议被当成同一个问题时
- 当需要重新确认公开发布稿件中哪些材料使用应先暂停时

这时，先拆开 `表达保护`、`引用审查`、`训练数据争议`、`材料使用暂停标准`，会更容易避免把版权问题缩成单一的“标注来源”问题。
