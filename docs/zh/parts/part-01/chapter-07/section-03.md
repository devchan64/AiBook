...
启发式和概率模型都可能出现在信息不完整、问题复杂的情境里，但它们不是同一个东西。

> 启发式在减少搜索。  
> 概率模型在用数字表达不确定性。  
> 运营规则则把输出转成行动。

只要维持这个区分，就更容易冷静地阅读 AI 系统：哪一部分是经验规则，哪一部分是概率建模，哪一部分属于政策与责任。
...
当分数、概率、阈值和运营规则开始看起来像一个混在一起的数字包时，先把这一节的区分拿出来。

- 当你看到一个模型分数，就想立刻把它读成概率或最终判断时
- 当你需要解释 `threshold` 到底属于模型内部概念，还是服务运营标准时
- 当你需要整理启发式、概率模型、校准和政策标准在同一系统里的不同角色时
