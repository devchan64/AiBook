...
经典找路可以被表示成图上的路径搜索。到了自动驾驶这样的真实系统里，额外的车道、速度、障碍物与控制约束会让系统更倾向于把“找整体路线”和“决定此刻轨迹”分成不同层来处理。

`waypoint` 的作用，是把整体路线表达成较容易处理的参考点序列。但车辆并不是机械地复述 waypoint，而是把它们当作参考，再结合当前环境实时选择短时 `trajectory`。

把这一节压成一句话就是：

> 自动驾驶路径规划，是把找路问题拆成更细表示与多个规划层级的一个典型案例。
