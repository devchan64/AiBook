...
在 Part 2 里，我们重新恢复了阅读数学、Python、数组、表格和图形的基础。但能够重新读懂计算工具，并不等于立刻就能把 AI 问题建好。真正面对源数据时，首先撞上的问题，往往不是 `该用什么模型`，而是 `什么才算一条数据样本`。在这本书的整体结构里，Part 2 和 Part 3 一起构成基础恢复区段，而 Part 3 负责的是 `数据科学问题结构的恢复`。

Part 3 的代表案例不是围绕某个具体设备名，而是围绕一个更一般的结构来说明的。这里有一次自动执行的动作，这次动作会留下控制参数时间序列和动作过程中的传感器时间序列；之后又会把多次动作重新聚成近期区段，与基准线做比较。在这种结构里，一个时刻的测量值可以被看成一个样本，一次完整动作也可以被看成一个样本，甚至多个动作组成的近期区段也可以被看成一个样本。怎么选，会直接改变后面形成的数据集、比较方式、还能解释的问题，以及实际行动流程会分成几条支路。即使是同一份源数据，只要重组方式、保留列、比较对象和要接入的行动流程结构不同，AI 问题就会完全不同。
...
| 运营输出 | 供人阅读或交给下一阶段的结果结构 | 警告、复核候选、目标标签候选 |
...
在 Part 3 里，`数据建模` 这个大定义本身先在 3.1 固定，工作顺序先在 3.2 固定。后面的 Section 不再重复同一个词的长定义，而只保留当前问题需要的最小连接。[样本(sample)](/AiBook/en/reference/concept-glossary.md#glossary-sample)、[特征(feature)](/AiBook/en/reference/concept-glossary.md#glossary-feature)、[基准线(baseline)](/AiBook/en/reference/concept-glossary.md#glossary-baseline)、[比较报告(comparison report)](/AiBook/en/reference/concept-glossary.md#glossary-comparison-report)、[目标(target)](/AiBook/en/reference/concept-glossary.md#glossary-target) 需要时可以回到英语版概念词典再确认。

Part 3 会先固定数据建模想要达成什么，以及它按什么顺序推进。接着再确认，为什么存储好的记录不能直接读成数据集，为什么要先决定一行和一样本分别代表什么，再把原始日志重组为可比较的表。然后才会设计特征和中间表示，区分哪些列用于标识、比较、还是目标候选。接下来再建立近期区段与基准线的比较结构，并在小样本和重复性不稳定面前，给解释划出边界。最后才把应当保留为比较报告的问题，与应当提升为预测任务的问题分开，并把输入/结果边界和时间边界一起收口。
...
- 因为复核候选和诊断确认、基准线比较和绝对值判断，经常会被混在一起。
...
相对地，具体机器学习算法的细节学习方式、train/validation/test 切分的细致流程，以及复杂时间序列深度学习结构本身，都不是这里的中心。

这个范围限制的原因很简单。Part 3 的责任，是先把 `什么数据应该被做成什么结构` 说清楚。

## 与前一 Part 的连接

如果 Part 2 恢复的是计算与表达工具，那么 Part 3 决定的就是：用这些工具 `把什么表达成什么结构`。表、数组、汇总统计和可视化，也是在这里才真正开始作为 `问题表达工具` 使用。

## 读完 Part 3 之后应该留下的理解

应该留下这样的感觉：数据集不是一个现成表，而是一个被设计出来的比较结构；机器学习也只有放在这种结构之上，才会被真正读懂。样本结构、特征、目标候选和时间边界必须先被整理好，这样后面的学习解释也才能一直保持 `到底在预测什么`、`又在用什么输入` 的清晰状态。
...
- Google for Developers, `Machine Learning Glossary`. 它提供 sample、feature、label、label leakage 等核心术语的角色区分，因此支持 Part 3 必须先固定样本结构和输入/结果边界，而不是先讨论模型名字的解释。 [https://developers.google.com/machine-learning/glossary](https://developers.google.com/machine-learning/glossary){: target="_blank" rel="noopener noreferrer" } / 确认日期: 2026-07-08
