...
`原始日志 -> 汇总表 -> 聚合表`，是把同一条时间序列重新表达成适合不同问题的表结构的顺序。只要这个顺序能被看见，后面的 [baseline](/AiBook/en/reference/concept-glossary.md#glossary-baseline) 比较和 [intermediate representation](/AiBook/en/reference/concept-glossary.md#glossary-intermediate-representation) 设计究竟接在什么层级上，也会更清楚。
