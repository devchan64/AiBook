...
- MAE 展示平均偏了多少，RMSE 则是对大误差更敏感的一种平均误差。
- R² 是显示 model 比平均预测多解释了多少的 summary value。
- 如果只看一个 metric，解释就容易摇晃，所以 baseline、平均误差、大误差和 residual pattern 必须一起看。
- linear regression 的局限通常会出现在 `问题无法只用一条直线来概括` 的场景里。

这一节的关键，不是去背更多 regression metric 名字，而是固定住：regression evaluation 到底要一起看到什么程度。

| 必须一起看的内容 | 本节先问的问题 | 后面会再接到哪里 |
| --- | --- | --- |
| baseline error | 这条直线真的比简单平均预测更好吗？ | P4-8 baseline 比较 |
| 平均误差与大失败区间 | 整体上错多少？又是在哪些地方明显错得更大？ | P4-6 regression metric |
| 代表性错误案例 | model 会在什么输入条件下反复用同一种方式失败？ | P4-18 feature engineering、后续回归模型比较 |

## 简短检查

- 你有没有在确认超过 baseline 之前，就因为一个误差数字而下结论？
- 你有没有把平均误差和大失败区分开来读？
- 当大误差区间出现时，你有没有避免立刻把原因定死，而是重新检查缺失 feature 或 nonlinear 可能性？

## 什么时候先想起这个视角

- 当你要检查自己是不是在超过 baseline 之前，就先被一个误差数字带着下结论时，想起这一节。
- 当你需要区分平均误差与大失败，并且把 residual 和代表性错误案例一起读时，回到这一节。
- 当你看见一个漂亮数字，却仍需要在定因之前重新检查缺失 feature 或 nonlinear 可能性时，把这一节当作基准。

## 理解检查

- 你能解释 residual 的符号是什么意思吗？
- 你能把 MAE 和 RMSE 的差别解释成 `对大误差的敏感度差异` 吗？
- 你有没有把 R² 理解成“相对于平均预测的解释力”，而不是单纯分数？
- 你能举出一两个直线假设失效的场景吗？
- 你能解释为什么即使数字好看，也不应该立刻过度相信吗？

## 和下一节的连接

走过 linear regression 之后，读者就准备好从 `用直线预测连续值的模型` 转到 `把直线读成分类边界的模型`。下一章 P4-11 的 logistic regression，会最直接地展示这个连接。

- linear regression：连续值预测
- logistic regression：用于分类的概率输出与边界解读

也就是说，Chapter 10 展示的是 `直线在回归里怎么用`，而 Chapter 11 展示的是 `这种线性思考到了分类里会怎样变化`。
