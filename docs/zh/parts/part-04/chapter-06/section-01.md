...
这一节说明 `评价指标(metric)`、`准确率(accuracy)`、`精确率(precision)`、`召回率(recall)`、`F1` 的基本作用。下一节会沿着这个抓手继续判断当前语境，而 `到底把哪一类错误当成更重要` 的标准，会通过本节和 [概念词汇表](../../../reference/concept-glossary.md) 再次接回。
