...
这一节不会重新长篇重复 SVM 的基本定义。`寻找大 margin boundary` 这个核心直觉，会通过 P4-13.1 和 [概念词汇表](../../../reference/concept-glossary.md) 再接回来。这里专注的是：为什么要引入改变表示空间的想法。
...
- P4-12.2 看过 `表示方式和计算 기준 一变，同样的数据也会被读成不同的结构`
...
- 它更在意点 주변的局部性(locality) 和基于距离的相似度
...
XOR 是交互特征的 대표性例子。相对地，当想到 RBF 时，通常更适合想象 `中心和外圈` 这样的圆形结构。
...
团队先试了 linear model 和 linear SVM，但 boundary 总是显得不自然。如果只看每个轴，正常和不良样本会混在一起；一条直线很难把这种对角线交错的模式切干净。如果一直只把问题读成 `还有没有更好的直线？`，解释就会 계속卡住。
...
3. 再看显式 feature expansion 或更简单的非线性 기준
