...
这一节解释 `policy-based reinforcement learning`、`policy gradient`、`actor-critic` 的基本含义。后面的 Section 会在这个把手上继续当前语境里的判断，而“直接调整行动方式”的强化学习基本感觉，会再次通过这一节和[概念词汇表](../../../reference/concept-glossary.md)连回来。
...
这个练习是在 policy 表达层面回收 Part 4 的目标。问题不只是`要做什么`，还包括`要让什么行动更常出现`。评估则是通过`reward 的变化，实际上怎样改变了行动分布`来读。如果跑完例子之后目标还是不够具体，通常不是因为 policy 这个词太抽象，而是因为缺少前后对比`概率分布变化场景`的机会。
...
- 策略型强化学习，是一种直接调整 policy 本身以提高 expected reward 的做法。
- 如果价值型强化学习擅长做行动记分板，那么策略型强化学习就更自然地直接处理行动概率与行动方式。
- policy gradient 会把 policy parameter 往高 reward 行动倾向更强的方向修改。
- REINFORCE 是最能展示策略型强化学习基本哲学的代表性入门算法。
- actor-critic 是把 actor 与 critic 角色拆开，让 policy 调整和评价信号一起工作的结构。
...
- 能说明为什么在连续行动问题里，直接处理 policy 本身会更自然吗？
- 能解释 policy gradient 调的是行动倾向，而不是记分板吗？
- 能区分 actor-critic 里的 critic 不是代替 policy 选行动，而是提供评价信号吗？

## 什么时候应先想到这个视角？

- 当连续行动或大状态空间让直接处理行动倾向比保留记分板更自然时，就先想到策略型视角。
- 当解释 policy gradient 时只剩抽象优化语言，就回到`直接调整 policy，让好行动更常出现`这句话。
- 当 actor 和 critic 的角色混在一起时，就重新把提出执行的一边和给评价信号的一边拆开来看。
