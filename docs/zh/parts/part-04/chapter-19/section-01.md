...
这一节解释 `value-based reinforcement learning`、`state value`、`action value`、`Q-value` 的基本含义。后面的 Section 会在这个把手上继续当前语境里的判断，而“把行动的长期好坏读成分数”这一基本感觉，会再次通过这一节和[概念词汇表](../../../reference/concept-glossary.md)连回来。
