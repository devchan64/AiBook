...
Part 2 重新补回了阅读公式、Python、数组、表格、图表和执行环境的基础。现在在 Part 4，我们要整理这些工具究竟是为了什么而使用，也就是 `从数据中学习规则` 这句话到底是什么意思。
...
- 为什么 feature selection、preprocessing、baseline、tuning 要比模型名字更先检查？
...
- 能说明 feature selection、preprocessing、model selection、baseline、tuning 的作用。
...
接下来处理阅读学习的共同基础。这里包括数据拆分、验证、过拟合、泛化、评估指标、特征选择、预处理、模型选择、基线、调参。这一段是 Part 4 的中心，因为无论后面遇到什么算法，读者最终都要重新追问：数据是怎么拆开的、模型学到了什么、分数该怎么读、现在看起来不错的结果到底能不能信。

评估指标的范围也会在这条流程里明确下来。Part 4 正文里优先处理的是那些能够先抓住 `面对这种问题应该先问什么` 的指标，比如 accuracy、precision、recall、F1、MAE、RMSE、R-squared。像 ROC、PR、log loss、calibration、reliability、silhouette 这类会让分数解释更细致的项目，会放到 P4-6.4 的补充学习里做入门说明，而 threshold 与 calibration 会在 P4-15.3 再次回收。
...
Part 2 里补回的数学与软件工具，会直接用在 Part 4。NumPy 数组用来装输入 `X` 和标签 `y`，Pandas DataFrame 用来读取 dataset，mean、variance、error 会再次出现在阅读评估指标时。derivative、gradient、loss 会继续连到后半部分以及 Part 5 的深度学习，Matplotlib 则会用于查看数据分布、决策边界、loss 曲线和聚类结构。

换句话说，如果 Part 2 补回的是计算语言，那么 Part 4 展示的就是这套计算语言在机器学习语境里到底帮助做什么判断。

从 Part 2 进入 Part 4 时，手里只要还留着下面这些基准，就已经足够开始阅读。

| 从 Part 2 带来的基准 | 到 Part 4 后立刻变成的阅读问题 |
| --- | --- |
| 能从数组和表里读出 `input X`、`label y` 和 shape | 哪些列该当作输入特征，哪个值才是目标？ |
| 能用小例子确认 mean、error 和图表 | 别人说分数变好了，到底在现实里意味着什么？ |
| 知道 Colab、本地执行、Git 记录之间的差别 | 如果想把同一实验重新跑一遍并作比较，必须留下什么记录？ |

从 Part 3 进入时，还要把这样一种感觉带过来：真正重要的不是 `把数据直接塞进模型`，而是先固定 `什么才算一条案例`。

| 从 Part 3 接过来的基准 | 在 Part 4 里立刻继续的问题 |
| --- | --- |
| 一行 row 已经被整理成一条动作记录或一条案例 | 在这条案例里，什么该当作输入特征，什么该当作目标候选？ |
| 控制记录、传感器记录、识别列已经彼此分开 | 哪些列不应该直接进入模型输入？ |
| 摘要特征和待检查标签候选已经分开保存 | 现在的问题是 classification 还是 regression，标签定义够不够一致？ |
| baseline 比较和下一步检查问题已经一起记录 | 即使分数看起来更好，也应该拿什么来判断它是否真有意义？ |

## 与后续 Parts 的连接

完成 Part 4 之后，读者就能把深度学习看成 `用更大的表示能力和计算资源来处理机器学习共同问题的流程`，而不是 `一个新的流行模型`。

Part 4 会接出下面这些问题。

- 神经网络为什么擅长 representation learning？
- 参数一多起来，为什么 GPU 和并行处理就会变重要？
- 梯度下降、loss、generalization 这些问题在深度学习里会怎样重新出现？

因此，Part 4 不是学习深度学习以前时代的 Part，而是为阅读深度学习和生成式 AI 建立共同基础的 Part。
...
- gradient、loss、optimizer 在神经网络里会怎样延伸成更大的计算结构？
...
到了下一 Part，在 Part 4 建立的共同问题会扩展到神经网络语境里。

问题定义、数据拆分、loss、优化、generalization 这些内容会原样保留下来，并在其上再加上 hidden layer、representation learning、并行计算、Transformer 等结构问题。
...
- 能区分 feature selection、preprocessing、baseline、tuning 的作用。
...
- Google, `Machine Learning Glossary`, 包含 `machine learning`、`supervised learning`、`unsupervised learning`、`reinforcement learning` 等条目，确认日期：2026-07-10. [https://developers.google.com/machine-learning/glossary](https://developers.google.com/machine-learning/glossary){: target="_blank" rel="noopener noreferrer" }
- scikit-learn developers, `User Guide`, scikit-learn documentation, 确认日期：2026-07-10. [https://scikit-learn.org/stable/user_guide.html](https://scikit-learn.org/stable/user_guide.html){: target="_blank" rel="noopener noreferrer" }
