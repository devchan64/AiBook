...
这里解释 `函数(function)`、`参数(parameter)`、`实参(argument)`、`返回值(return value)` 之间的基本区分。`值(value)`、`变量(variable)`、`循环(loop)` 的代表性说明仍然放在 P2-8.1、P2-8.4 和[概念词汇表](../../../reference/concept-glossary.md)里，而这里集中在把输入-处理-输出契约读成一个可复用的小单元。
...
- 如果值和多个值的组合已经在循环里被处理过了，现在就给那段处理起名字并复用它。
- 函数会让 `输入 -> 处理 -> 输出` 的边界变得更清楚。
- `print` 是展示出来，而 `return` 是把结果交给下一步计算。
- 小函数会成为后面阅读库函数与 API 函数的标准。

| 这一节现在先抓住什么 | 紧接着会连到的问题 | 后面再次出现的位置 |
| --- | --- | --- |
| 函数会把重复处理变成“有名字的可复用单元” | 它会继续连到：在后面的补充学习和库 API 里，应该怎样读输入输出契约。 | 它会在所有后续 Python 例子、数据预处理、模型调用代码里反复出现。 |
| 函数把输入、处理、输出的边界分开 | 这样就更容易明白为什么要分别区分 `parameter`、`argument`、`return`。 | 它会在后面的模型函数、评估函数、工具函数设计里持续重要。 |
| `print` 和 `return` 的角色不同 | 它会继续连到：怎样区分“给人看的输出”和“交给下一步计算的结果”。 | 它会在后面的调试、可复用代码编写、库函数阅读里再次出现。 |
...
这里不深入讨论 lambda、decorator、class method、generator、type hint、exception handling。看起来像附着在值上的方法调用形式，会在 P2-8.6 补充学习里再次回收。
...
函数不是为了让代码看起来更漂亮的装饰。它是给重复处理起名字，并让输入和输出更清楚的工具。

数学函数帮助我们理解输入和输出的关系。Python 函数则让我们能在真实代码里执行并复用这种关系。

更一般地说，函数是把复杂任务切成小块的边界。如果“处理一条数据的函数”、“对多条数据重复的代码”、“评估结果的函数”彼此分开，那么后面的项目代码就会更容易阅读。

如果重新接回前几节的流程，我们是从处理 `值和变量` 开始，通过 `列表和字典` 收集多个值，再通过 `循环` 把同样的处理应用多次，而现在则是把这段处理本身整理成一个有名字的单元，也就是 `函数`。

这一节的核心是：

- 函数会给处理起名字。
- 参数(parameter)是函数接收值时使用的名字。
- 实参(argument)是调用函数时实际放进去的值。
- `return` 会把结果返回，供下一步计算使用。
- 因为 Python 函数是对象，所以它们可以被放进变量里，也能作为实参传给其他函数。
- 方法(method)可以被读成附着在对象上的函数形式。
- 小函数会让重复数据处理的输入和输出更清楚。
...
- 当同样的处理代码已经在多个单元或多行里重复出现，需要拆成小复用单元时，先想起它。
- 当需要重新连回“数学函数、Python 函数、库函数都在延续同一种输入-处理-输出契约”时，先想起它。
- 当开始混淆 `print` 和 `return`，需要再次分开“显示出来的输出”和“交给下一步计算的结果”时，先想起它。

## 来源与参考资料

- Python Software Foundation, [More Control Flow Tools: Defining Functions](https://docs.python.org/3/tutorial/controlflow.html#defining-functions){: target="_blank" rel="noopener noreferrer" }, Python 3.14.6 documentation, 确认日期: 2026-06-24.
- Python Software Foundation, [More Control Flow Tools: Default Argument Values](https://docs.python.org/3/tutorial/controlflow.html#default-argument-values){: target="_blank" rel="noopener noreferrer" }, Python 3.14.6 documentation, 确认日期: 2026-06-24.
- Python Software Foundation, [Function definitions](https://docs.python.org/3/reference/compound_stmts.html#function-definitions){: target="_blank" rel="noopener noreferrer" }, Python 3.14.6 documentation, 确认日期: 2026-06-24.
