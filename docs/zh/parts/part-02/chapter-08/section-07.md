...
本节不会深入讨论 Python 内存管理实现、引用计数、`copy` 模块的内部行为，以及用户自定义对象的复制规则。这些实现细节先放在本书当前正文范围之外，这里只恢复一种判断感觉：`原对象和副本什么时候会一起变化？`
...
- assignment 可能不同于 copy。
- 浅拷贝只重新创建外层，内部嵌套值仍可能共享。
- 深拷贝连内部也会重新创建。
- 如果数据处理中需要保留原对象，就应先确认 `我现在改动的是否仍是同一个对象`。

本补充学习只要恢复到下面这个程度，就可以重新接回正文主线。

| 这里要恢复的点 | 要回到的正文问题 | 接着阅读的位置 |
| --- | --- | --- |
| `a = b` 并不自动表示复制 | 我现在改动的值是同一个对象，还是新对象？ | 列表与赋值见 P2-8.2，字典见 P2-8.3 |
| 浅拷贝和深拷贝在什么时候分开 | 为了保留原对象而做预处理或中间值时，第一步该检查什么？ | 后续 Pandas 与数据预处理示例见 Part 2 Chapter 12 与 Part 3 正文 |

也就是说，只要你能读懂“一边改动后，用另一个名字看到的值会不会一起变化”，这里就已经足够，接着回到正文主线会更安全。

## 简短检查

- 能说明两个名字可以同时指向同一个列表吗？
- 能用嵌套列表示例说明浅拷贝与深拷贝的差异吗？
- 能说明为什么数据预处理中要谨慎对待复制问题吗？

## 什么时候应先想起这个视角？

- 当你以为保留了原对象，但实验列表也一起变化，需要重新区分 reference 和 copy 时。
- 当你需要用嵌套列表示例重新确认浅拷贝与深拷贝差异时。
- 当你在预处理中需要先检查当前改动对象是否真的已和原对象分离时。

## 来源与参考资料

- Python Software Foundation, [The Python Tutorial - More on Lists](https://docs.python.org/3/tutorial/datastructures.html){: target="_blank" rel="noopener noreferrer" }, 确认日期: 2026-06-29.
- Python Software Foundation, [Standard Library - `copy`](https://docs.python.org/3/library/copy.html){: target="_blank" rel="noopener noreferrer" }, 确认日期: 2026-06-29.
