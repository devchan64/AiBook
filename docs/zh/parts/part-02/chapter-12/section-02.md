...
本节说明 `Series`、过滤(filtering)、聚合(aggregation)、`groupby`、`loc`、`iloc` 的基本区分。`DataFrame` 本身的代表性说明放在 P2-12.1 与[概念词汇表](../../../reference/concept-glossary.md)中，这里关注的是：从那张表里读什么、留下什么、概括什么。
...
| 本章当前的抓手 | 下一章会进一步看的内容 | 在 Part 3 再次出现的位置 |
| --- | --- | --- |
| 决定从表里留下哪些行与列，以及用哪些总结值把它缩小的标准 | 在 Matplotlib 中，应该用什么图形来检查这张表及其总结？ | 选择数据集、拆分列、构造类别总结、再准备模型输入的时候 |

- 选择一列时，通常读成 `Series`；选择多列时，通常读成 `DataFrame`。
- `loc` 按标签选，`iloc` 按位置选。
- 条件过滤会只保留结果为 `True` 的行。
- 聚合是把很多行变成少量总结结果的过程。
- `groupby` 是先把同类归在一起，再进行总结的方法。
...
- 当你需要决定表里哪些行与列要留下、并且按什么标准分组与总结时，先想起选择、过滤、聚合这个视角。
- 当你需要整理 `loc`、`iloc`、布尔过滤、`groupby` 各自在回答什么问题时，回到本节。
- 当你需要从“看原始整张表”转到“抽出需要的部分并变成可分析形态”时，再重新确认这里。
