...
本节说明 `DataFrame`、`row`、`column`、`index` 的基本区分。前一章的 NumPy 更强调这些值怎样以 vector 与 matrix 的形状进入计算；这里则转向它们该怎样被读成“案例与变量组成的表”。下一章会再从这张表走向图表，去看表里不容易直接看到的变化与关系。后续继续进入选择、聚合与数据集准备时，也可以配合 [概念词汇表](../../../reference/concept-glossary.md) 一起作为回返点。

## 本节范围

本节不深入讲 DataFrame 的复杂操作。`loc`、`iloc`、filtering、sorting、aggregation、missing value handling 留到下一节。

这里回答以下问题。

- DataFrame 与 NumPy array 有什么不同？
- DataFrame 的 row、column、index 分别指什么？
- 为什么表格式数据通常比起数组，更自然地用 DataFrame 来读？
- 在学习用数据集里，一行与一列应怎样理解？
- 第一次拿到一个 DataFrame 时，应该先看什么？
...
- DataFrame 是带标签的二维表格式数据结构。
- 建立“一行读成案例，一列读成变量”的习惯很重要。
- index 可以是简单编号，也可以是有意义的行标签。
- DataFrame 能自然容纳不同类型的列，这非常贴近真实数据。
- NumPy 常更适合数组计算，而 DataFrame 常更适合检查表结构与列含义。
...
- 当数据不该被读成裸露的数值数组，而该被读成带标签的表时，先想起 DataFrame 视角。
- 当你需要解释一行到底表示什么、一列又装着什么属性时，回到本节。
- 当你想整理“为什么新打开的数据集应先看 `shape`、`columns`、`index`、`dtypes`、`head()`”时，也回到这里。
