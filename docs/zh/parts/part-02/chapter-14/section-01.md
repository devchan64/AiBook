...
本节说明 `Git`、`版本控制（version control）`、`提交（commit）`、`暂存区（staging area）`、`仓库（repository）` 的基本区分。如果说 Chapter 11 到 13 是通过数组、表格、图表来读取和确认数据的阶段，那么这里的问题就变成：要如何把这些计算与确认过程留下来，作为一组组变更。Git 不是一个突然冒出来的工具，而应该被读成：能让你在 Part 3 再次解释 baseline 比较、预处理修改、metric 解读的记录工具。后面继续到分支与部署时，也请把[概念词汇表](../../../reference/concept-glossary.md)一起当作基准点。
...
- Git 不是保存文件的工具，而是管理变更历史的工具。
- 提交（commit）是一组有意义的变更。
- `git status` 用来确认当前状态，`git add` 用来挑选本次记录要包含的变更。
- `git commit` 把选好的变更留进历史，`git log` 则让你读这段历史。
- 在学习文档项目中，Git 让你追踪正文、代码、图片、调查笔记之间的连接。
...
- 当你需要解释“保存文件”和“留下工作历史”为什么不同的时候，先想起 Git 的视角。
- 当你要判断改了什么、为什么改、以及该按什么单位归成记录时，回到本节的变更历史感觉。
- 当你想在同时处理文档、代码和图表的学习项目里留下可复现记录时，再次查看本节。
