...
1. 微分(derivative)是相对于一个输入的瞬时变化率。
2. 偏微分(partial derivative)是在多个输入里，只改变其中一个时的变化率。
3. 梯度(gradient)是把多个偏微分收集起来的向量。
4. 在 AI 学习里，梯度是告诉我们损失会沿着各个参数方向怎样变化的信息。

如果微分只停留在数学公式上，就很难连到 AI 学习。但如果把它读成 `把某个值稍微改一点，结果会怎样变化？` 这个问题，它就会自然地接到梯度与学习的语言。
...
- 当 `derivative`、`partial derivative`、`gradient` 同时出现在一个段落里，而你开始混淆它们各自指什么时，就先想起它。
- 当脑中只剩单变量微分的记忆，于是多变量损失函数与参数更新的说明突然变得很远时，就先想起它。
- 当你需要把梯度重新读成不是一组简单数字，而是一组按方向组织的变化率向量时，就先想起它。
