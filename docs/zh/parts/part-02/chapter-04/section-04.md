...
- 梯度下降(gradient descent)的重复更新公式
- 各种优化器(optimizer)的类型
- 反向传播(backpropagation)的计算步骤
- 神经网络中逐层的微分计算
- 真实深度学习框架里的自动微分(automatic differentiation)
...
学习是通过调整参数来减小损失的过程。损失是告诉我们当前模型有多不好的数字，而微分是在说明：当某一个参数稍微改变时，损失会怎么变化。梯度则是把多个参数的变化信息收在一起的向量，反向传播是在复杂模型里高效算出这个梯度的过程。

微分在 AI 学习里不是一种数学装饰。只要模型想自己朝着更好的方向移动，就必须读懂：在当前数值附近，损失会怎样变化。那种读法，就是微分与梯度。
...
- 当你已经理解损失函数，却还没真正明白为什么学习说明会立刻接上微分与梯度时，就先想起它。
- 当你需要分别解释 `错了多少？` 和 `该往哪边改？` 这两个问题时，就先想起它。
- 当你把反向传播当成整个学习过程本身，需要重新分开它们的角色时，就先想起它。
