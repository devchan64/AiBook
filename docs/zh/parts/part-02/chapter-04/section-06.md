...
- 复合函数是 `前一个输出进入后一个输入的计算管线`。
- 链式法则是在读 `前段变化会有多少传到后段结果` 的规则。
- 只有把 `x -> y` 和 `y -> z` 分开看，再连起来，才能读懂 `x -> z`。
- 反向传播并不是链式法则本身，而是把这条规则应用到多层结构上去算梯度的过程。
- 后面再看到 `backpropagation` 时，先在脑中画出“损失的变化穿过多个阶段传回去”的图景。
