...
- 与其把微分先读成背公式的过程，不如先把它读成 `什么变化会带来什么结果差异？` 这个问题。
- 切线斜率、瞬时变化率、瞬时速度看起来像不同说法，但它们其实都围绕着同一个问题。

## 通往下一节的流程

在 P2-4.2 中，我们会把这个问题再收窄一些，去看变化率与斜率。
...
这里我们做的是恢复记忆；下一节则会把这个记忆整理成可以计算的形式。
...
- 一听到微分，你能说出自己先想到的是切线斜率、瞬时变化率、还是瞬时速度。
- 当公式先浮现出来时，你能重新追问这个公式在处理什么变化意义。
- 你能把距离、速度、加速度的关系解释成变化率的例子。
- 你能说明点、线、面、体积的关系和累积直觉相连。
- 你能把微分重新看成不是公式背诵，而是 `小变化会怎样影响输出？` 这个问题。

## 什么时候要先想起这个视角

- 当切线、微分系数、瞬时速度这些词虽然能想起来，但它们到底被什么问题串在一起已经模糊时，就先想起它。
- 当你在阅读损失或梯度说明之前，需要先恢复 `数值到底变化了多少？` 这个问题本身时，就先想起它。
- 当你虽然记得公式，但解释一到“为什么要让两点越来越近”“为什么要看切线斜率”就断掉时，就先想起它。
