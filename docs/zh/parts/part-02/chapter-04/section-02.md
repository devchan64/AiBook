...
本节处理变化率(rate of change)、平均变化率(average rate of change)、斜率(slope)的直觉。导数(derivative)、梯度(gradient)、梯度下降(gradient descent)、反向传播(backpropagation)会在下一节和后面的章节里处理。

这里首先要解决的问题是：`为什么不能只看输出变化了多少，还要同时看输入变化了多少，那个变化才有意义？`
...
2. 为什么斜率是变化率的视觉表达？
3. 输入变化与输出变化是怎样连起来的？
4. 为什么微分从斜率开始，最后会接到学习？

| 术语 | 很短的含义 | 本节中的作用 |
| --- | --- | --- |
| 变化率 | 相对于输入变化的输出变化 | 本节的中心问题 |
| 斜率 | 在图像上读取变化率的值 | 视觉解释的基准 |
| 平均变化率 | 整个区间的变化率 | 比较直线与曲线的出发点 |
| 瞬时变化率 | 某一点附近的变化率 | 通向微分的桥 |
| 曲线 | 变化率会随着区间不同而不同的函数形状 | 说明为什么需要微分的对象 |

本节之后的流程也很简单。

- 在 `P2-4.3` 中，我们会把这个变化率问题更直接地读成导数、偏导数、梯度的记号。
- 从 `P2-6.1` 到 `P2-6.3`，我们会再次用变化率的语言去读损失能够减少多少、参数该往哪边改。
- 在 Part 4 中，同一个问题会重复出现在 `gradient`、`update`、`optimizer` 的语言里。
...
![直线斜率示例](../../../assets/part-02/chapter-04/linear-slope-constant-en.svg)
...
![曲线斜率示例](../../../assets/part-02/chapter-04/curve-slope-changing-en.svg)
...
- 当你看表格或图时已经能看出数值大小，但 `输入变了多少时，输出变了多少？` 这个问题还没有被分开时，就先想起它。
- 当你需要一次性说明直线的固定斜率与曲线的区间变化率差别时，就先想起它。
- 当你在学微分之前，需要重新接起“为什么要从平均变化率收窄到瞬时变化率”时，就先想起它。
