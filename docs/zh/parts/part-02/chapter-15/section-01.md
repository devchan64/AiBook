...
- 把公式翻译成代码时，先分开符号、数据形状和计算步骤。
- sigma 通常可以翻译成重复计算或数组计算。
- 先用循环确认含义，再用 NumPy 缩短表达。
- 只有同时检查中间值和最终数字，才能真正理解计算。
- 图表是帮助解释公式结果的辅助工具。
- 本节是在练习把前面章节里分别见过的变量、循环、数组、图表，重新绑成一个步骤。
...
- 当你需要把公式改写成代码步骤，而不是只把它背下来时，先想起 formula-to-code 的视角。
- 当你需要说明每个符号如何对应代码变量，或比较循环计算与数组计算时，回到本节。
- 当你需要通过检查中间值而不是只看最后数字来验证计算时，再次确认本节。
