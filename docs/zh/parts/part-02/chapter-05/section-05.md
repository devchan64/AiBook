...
这里不会深入讨论公式证明、关于 p-value 的细节争论、统计检验流程的完整实务应用，或中心极限定理的严格数学展开。这里只回收最基本的阅读标准，让你在读文档时不至于停住。
...
之所以不会在均值和方差处结束，是因为光靠 `中心` 和 `扩散` 还不足以回答很多问题。
...
也就是说，这些概念全都是让我们不要把数字读得过于武断的统计工具。
...
- 当只靠均值和方差已经无法顺利读懂实验结果表或统计文档里剩下的术语时，先想起这个视角。
- 当标准差、相关系数、标准误、置信区间一起出现在一张表里，需要重新拆分它们的角色时，先想起这个视角。
- 当你需要区分“看起来好像有差异”和“这个差异到底该信到什么程度”时，先想起这个视角。

## 来源与参考资料

- OpenStax, [Introductory Statistics 2e](https://openstax.org/details/books/introductory-statistics-2e){: target="_blank" rel="noopener noreferrer" }, 确认日期: 2026-06-29.
- NIST/SEMATECH, [e-Handbook of Statistical Methods](https://www.itl.nist.gov/div898/handbook/){: target="_blank" rel="noopener noreferrer" }, 确认日期: 2026-06-29.
