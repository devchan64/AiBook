...
- 为什么只看均值还不够？
- 方差是怎样说明扩散的？
- 在 AI 里，分布、均值、方差还会在哪里再次出现？

| 术语 | 很短的含义 | 本节中的作用 |
| --- | --- | --- |
| 分布 | 值摆放出来的整体形状 | 最先要看的大图景 |
| 均值 | 把很多值压成一个中心的值 | 代表值的起点 |
| 方差 | 显示值在均值周围散开程度的值 | 揭露“只看均值会藏住什么”的工具 |
| 中心 | 数据聚集的代表位置 | 均值和中位数背后的问题 |
| 扩散 | 它们离中心散开得有多远 | 会接到方差和标准差的问题 |

本节之后的流程也很简单。

- 在 `P2-5.3` 中，我们会看这些摘要值如果不是来自整体，而是来自样本时，会出现哪些限制。
- 在 `P2-5.5` 中，我们会继续进入像标准差和相关这样的工具，更细地读取扩散和关系。
- 在 Part 3 与 Part 6 中，同样的感觉会在数据分布、分布偏移、平均损失解释里重新出现。
...
![一起读取分布、均值和方差的图](../../../assets/part-02/chapter-05/distribution-mean-variance-summary-en.svg)
...
![均值相同但方差不同的两组数据](../../../assets/part-02/chapter-05/same-mean-different-variance-en.svg)
...
- 均值是把中心压成一个数字的值。
- 方差会把均值周围的扩散单独显示出来。
- 即使均值一样，只要方差不同，数据的性格也可能不同。
- 在后面的 Part 里看到 `mean`、`distribution`、`variance` 时，应该先问：`现在是在看中心、在看扩散，还是在看整体形状？`

分布、均值、方差，是读取一组数据时最基础的三个问题。分布问的是值摆成了什么形状，均值问的是中心该放在哪里，方差问的是这些值围绕这个中心散得有多开。

如果只看均值，就可能漏掉数据之间的重要差异。要更安全地读懂一组数据的性格，就必须把分布、均值、方差放在一起看。

在 AI 里，这种视角更重要。因为模型不是在一个值上找规律，而是在一整组数据里找模式。
...
- 当你已经看到了一个平均数字，却忽略了数据到底集中在哪里、又散得多开时，就先想起它。
- 当你把数据分布和概率分布读成同一个东西，需要重新区分“实际观测值”和“数学模型”时，就先想起它。
- 当 `mean loss`、`distribution shift`、`dataset statistics` 这些表达一起出现，需要把它们都放回同一张统计地图里时，就先想起它。
