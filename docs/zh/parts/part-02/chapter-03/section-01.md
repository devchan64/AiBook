...
标量、向量、矩阵是线性代数的术语，但在 AI 入门里，首先要把它们理解成“阅读数据形状的语言”。标量是一个数字，向量是有顺序的数字列表，矩阵是带有行和列的数字数组。

AI 模型通过把数据转成数字数组来计算。因此，学习线性代数不只是学习数学符号，也是为了读懂模型会用什么形状接收和处理数据。

如果把本节的流程压缩成一句话，就是：AI 会把现实对象先压缩成一个数字，再用多个数字表示，再把多个表示收集起来，然后计算这些表示之间的位置和关系。
...
- 当一个数字、一个值列表、一个行列结构开始混在一起，看上去像同一类数据时
- 当你在读 `X` 的 shape、样本数、特征数时，shape 直觉总是断掉
- 当你需要重新连回“为什么 embedding、特征向量、batch 输入会用同一种线性代数语言来解释”时
