...
如何在个人电脑上安装 Python、如何管理虚拟环境，会在 `P2-7.1`、`P2-7.6`、`P2-7.7`、`P2-7.8` 里再次出现。这里先固定 Colab 与本地 PC 的执行位置差异。需要快速回看术语时，也可以一起查看[概念词汇表](../../../reference/concept-glossary.md)。
...
为了给下一节的 NumPy 实践做准备，这里只处理：怎样区分 Colab 与本地 PC 的执行位置，以及在需要安装 NumPy 时，不同环境里命令写法如何不同。

这里不处理 Colab 的文件管理、Google Drive 集成、GPU/TPU 运行时、笔记本共享权限、长时间运行限制。也不深入处理本地 Python 安装、虚拟环境(virtual environment)、包管理的详细步骤。关于本地安装、终端、虚拟环境流程，会在 `P2-7.1`、`P2-7.6`、`P2-7.7`、`P2-7.8` 里重新整理。
...
- “在本地 PC 上运行”到底是什么意思？
- 代码单元是怎样运行的？
- `%pip install numpy` 应该写在哪里？
- 个人电脑终端命令与 Colab 命令有什么不同？
