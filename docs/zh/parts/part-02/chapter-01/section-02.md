...
公式、代码和数据并不是彼此竞争的关系。它们是在不同位置说明同一个计算。公式更接近计算蓝图，代码更接近执行步骤，数据既是计算材料又是验证对象，而结果则必须重新放回问题语境里解释。

如果带着这个视角去读 Part 2，数学和软件就不会再分家。数学通过代码被确认，代码通过数据被验证，而结果又会重新回到公式和问题定义中被解释。
...
- 当公式、代码、数据开始看起来像互相分开的不同主题时
- 当你能读示例代码，却仍然感觉不出它到底在检查什么计算结构时
- 当你需要通过一个像平均值这样的小计算重新抓回整个模型计算的基本框架时
