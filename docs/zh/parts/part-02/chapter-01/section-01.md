...
2. 比较：计算预测和参考值之间的差、向量之间的距离、概率的大小。
3. 调整：寻找让参数变化后损失下降的方向。

只要记住这三件事，Part 2 里的数学主题就不会再显得彼此割裂。sigma 表达重复计算，向量和矩阵表达数据形状，导数寻找调整方向，概率与统计处理不确定数据。

## 简短检查

- 你能说明：在 AI 里，数学可以被当成阅读计算结构的语言，而不主要是证明吗？
- 你能把公式（formula）、数据（data）、代码（code）、输出（output）连接起来看吗？
- 你能说明向量（vector）、矩阵（matrix）、张量（tensor）是在表达数据的可计算形状吗？
- 你能把学习（learning）解释成“为了降低损失而调整参数”的过程吗？
- 你能说明为什么 Part 2 要重新处理线性代数、导数、概率与统计、优化吗？

## 什么时候应先想起这个视角

- 当你又把数学只看成考试科目，而没有把它看成 AI 识读工具时
- 当你开始失去“为什么损失、概率、向量、优化会出现在同一个 Part 里”的连接时
- 当读不懂公式开始同时阻塞模型、数据和学习解释时
