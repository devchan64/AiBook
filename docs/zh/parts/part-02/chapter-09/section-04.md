...
- 最好先把传统数据结构名称读成 `问题的语言`，再去看实现。
- 数组、栈、队列连接的是线性结构问题，树和图连接的是关系结构问题。
- 如果你已经学过 Python 列表和字典，本节是在它们后面补上更一般的直觉。
- 在 AI 实践里，顺序、基于 key 的查找和关系追踪都会不断回来。

对于数据结构，问题先于名称。

- 如果顺序重要，就想到线性结构。
- 如果“放进去和拿出来”的规则重要，就想到栈和队列。
- 如果层级重要，就想到树。
- 如果关系重要，就想到图。
- 如果按 key 查找重要，就想到字典和哈希表。

真正理解数据结构，并不意味着把每种实现都背下来。它意味着你能同时看到数据的形状与常做的操作，并选出适合这个目的的表达方式。

本补充学习整理的是下面这条标准。

| 这里要恢复的内容 | 要回去的正文问题 | 继续阅读的位置 |
| --- | --- | --- |
| 顺序、key 查找、关系追踪是不同问题这一点 | 当前数据首先产生了什么结构问题？ | 数据结构的大图景见 P2-9.1，数组和表的直觉见 Part 2 Chapter 11 与 12 |
| 把数组、栈、队列、树、图这些名称和问题场景连起来的最小直觉 | 为什么同样的数据会在 AI 实践里被读成不同结构名称？ | 之后 NumPy、Pandas、搜索与关系说明会出现在 Part 3 之后的正文 |

也就是说，如果你能区分 `这是顺序问题吗？`、`这是 key 查找问题吗？`、`这是关系追踪问题吗？`，就已经抓住了本补充学习的核心。
...
- 当你需要把 Python 便利语法背后的传统数据结构直觉单独整理出来时，回到这个补充节。
- 当你需要区分数组、链表、栈、队列、树、图、哈希结构分别是为了解决什么问题而出现时，回到这里。
- 当你想快速恢复各类结构的角色和选择标准，而不是深入挖实现细节时，本节就是基准线。
