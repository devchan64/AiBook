...
这里说明 `data structure`、`abstract data type`、`linear structure`、`non-linear structure` 这几个基本区分。即使后面的章节会再把数组、表、树、图分开来看，为什么要先把数据结构读成一个问题，仍然要回到本节的标准。后面这些结构名称再次出现时，也可以一起查看[概念词汇表](../../../reference/concept-glossary.md)。
