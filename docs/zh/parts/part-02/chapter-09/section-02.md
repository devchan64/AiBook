...
- 数组、表、树、图是回答不同问题的结构。
- 数组先看位置和轴，表先看行和列，树先看层级，图先看连接。
- 同样的数据会因为你问的问题不同，而读成不同结构。
- 以后再次遇到 NumPy 和 Pandas 时，比起工具名称，更应该先想：`现在这是数组问题，还是表问题？`

数组、表、树、图，是为了不同问题而存在的数据视角。
...
在 AI 实践里，这四种直觉会不断混在一起出现。因此，与其从一开始死记实现方式，不如先看当前数据到底要求你回答什么问题。
...
- 当你需要决定把数据看成数字数组、行列表格、层级结构还是关系网络时，先想起这里的比较。
- 当你需要说明“即使面对同一个对象，只要问题变了，合适的结构也会变”时，本节的四种直觉就是标准。
- 当你需要在 AI 实践里，把 token、embedding、dataset、文档结构、知识连接分开读成不同结构直觉时，再回来确认这里。
