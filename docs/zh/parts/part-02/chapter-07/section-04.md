...
本节说明虚拟环境和包的概念。不会深入讨论不同操作系统下的虚拟环境激活命令、Python 安装方法、PATH 设置或权限问题。这些环境检查顺序会在 P2-7.9 补充学习中重新整理。关于保存依赖列表和记录版本的详细说明，会在 P2-7.5 中再次处理。
...
1. 我现在在使用哪个 Python 环境？
2. 需要的包是否安装在那个环境里？
3. 安装命令和 Python 代码执行看到的是不是同一个环境？

这些问题看起来简单，但很重要。很多错误的起点并不是代码语法，而是“安装的地方和运行的地方不一样”。
...
- 当 Python 能运行，但提示缺少 NumPy 这类包，需要一起看执行空间和包准备状态时，就先想起这个视角。
- 当 `pip install` 明明做过，但 `import` 却失败，需要重新抓住“安装和使用是不同阶段”这一点时，就先想起这个视角。
- 当你把 Colab runtime 和本地 `.venv` 误当成同一个空间，需要重新分清到底装到了哪个环境里时，就先想起这个视角。
