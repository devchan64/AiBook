...
1. 这段代码依赖哪些外部包？
2. 那些包安装在哪个 Python 环境里？
3. 是否留下了以后还能重新搭出同样环境的记录？

代码并不会只靠文字就自己运行。把代码所依赖的环境一起留下，才是可复现性的出发点。
...
- 当一个月后重新打开的笔记本突然不能用，需要先恢复执行条件而不是先怪代码时，就想起这个视角。
- 当你需要解释为什么“在我的电脑上能跑”并不够，因此必须重新抓住依赖与可复现性的视角时，就想起这个视角。
- 当你需要从“重新搭环境”的角度重新串起 `requirements.txt`、版本固定、`pip freeze` 为什么必要时，就想起这个视角。
