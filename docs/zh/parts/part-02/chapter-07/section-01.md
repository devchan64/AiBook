...
这里不会一步步讲解 Python 安装方法。Python 安装会交给 P2-7.7 的补充学习。按操作系统划分的安装画面、权限设置、IDE 设置、包冲突处理，也不会在这里深入展开。终端命令语法、解释器运行方式、虚拟环境创建步骤、依赖记录方式的详细说明，则分别会在 P2-7.2、P2-7.3、P2-7.4、P2-7.5 再次回收。
...
只要能回答这四个问题，遇到错误信息时，慌张就会少一些。这并不表示你能立刻解决所有错误，但至少能先把它们分成 `是代码问题、运行时问题，还是包问题。`

读完这一节之后，你至少应该能重新说出下面这个边界。

| 现在这一章的抓手 | 紧接着下一章会继续看的东西 | 这里还不做的事 |
| --- | --- | --- |
| 代码在哪里执行、由谁读取 | 值、变量、数据结构要用什么句子来写 | 笔记本单元顺序、结果记录、共享文档整理 |
...
- 当 Colab 能跑、本地 PC 却跑不起来时，需要先把它分成“执行位置问题”而不是“代码问题”时，先想起这个视角。
- 当终端、解释器、虚拟环境、包这些词一下子全出现，需要重新区分每个词属于哪一层时，先想起这个视角。
- 当你需要重新抓住：Chapter 7 之后关于终端、解释器、虚拟环境的章节，都是在细分同一张执行环境地图时，先想起这个视角。
