...
- 这个命令是否可以在当前文件夹中执行？
- 路径是否和我的电脑匹配？
- 是否包含了与文件删除或管理员权限相关的命令，例如 `sudo`、`Remove-Item`、`rm`、`del`？
- 前面是否包含 `$`、`>`、`PS>` 这样的提示符号？
...
终端不是一张要背诵的命令清单，而是用来确认执行位置的工具。

在实践前，只记住下面这个顺序，也能减少很多错误。
...
这个补充学习是一个临时恢复操作系统步骤的地方。确认下面这些标准后，再回到正文主线。

| 这里解决的问题 | 回去后要接续的正文问题 | 接下来继续读哪里 |
| --- | --- | --- |
| 在 Windows、macOS、Linux 中要从哪里打开终端，又要如何确认当前位置？ | 代码实际上是在哪里运行，又是以哪个文件夹为基准移动的？ | 执行环境的大图景在 P2-7.1，工作文件夹和命令输入位置在 P2-7.2 |

只要做到 `能打开终端`、`能确认当前位置`、`能移动到实践文件夹`，就可以接回正文。如果下一个卡点变成要不要安装 Python，那时再转到 P2-7.7。
...
- 这个命令是以哪个 shell 为基准？
- 这个命令想找到哪个文件或程序？
- 这个命令是在我的电脑上执行，还是在 Colab runtime 中执行？
...
人原本的直觉通常是“既然文档里这样写，直接贴进去就行”。但实际上，不同操作系统里的终端应用、默认 shell、路径表示、复制粘贴方式都会略有不同。
...
- 能说明在 macOS Terminal 中可以用 `pwd`、`ls`、`cd` 来确认位置和移动。
...
- 当你复制了同一条命令，但它在 Windows 和 macOS 中看起来不同，因此需要先把操作系统差异分开看时，就想起这个视角。
- 当你在正文实践之前，就卡在“终端从哪里打开、当前位置怎么确认”这一步，需要先回看操作系统最低限度步骤时，就想起这个视角。
- 当复制粘贴或 `Ctrl + C` 的行为与普通应用不同，需要单独确认终端快捷键时，就想起这个视角。
