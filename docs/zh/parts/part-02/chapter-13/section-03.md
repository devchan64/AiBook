...
本节说明比较与保存流程中的基本区分，包括 `savefig`、图例（legend）、准确率（accuracy）。`plot`、`Figure`、`Axes` 的代表性说明放在 P2-13.1，基础图表选择标准放在 P2-13.2 和[概念词汇表](../../../reference/concept-glossary.md)；这里重点讲“怎样把多个图表一起比较，并把结果留成文件”。
...
- 单位不同的值应拆开看，单位相同的值可以放在同一坐标轴上比较。
- `savefig()` 是把图表保存成图像文件的方法。
- 想让保存的图表成为可复现记录，代码与数据条件也必须一起留下。
...
- 当你需要把多个图表并排放在一个画面上做比较时，先想到 Figure 和 Axes 的排布。
- 当你需要说明为什么像 train/validation 曲线这样的同类指标应放在同一坐标轴上比较时，回到本节。
- 当你需要重新确认屏幕显示与文件保存的区别，以及为什么只有图像还不够可复现时，本节会成为标准。
