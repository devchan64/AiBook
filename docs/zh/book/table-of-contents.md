...
阅读目录时，也请记住本书的一个基本设计：在同一个 Part 内，重要概念的详细说明会尽量只放在一个 Section 中。后续 Section 只保留当前语境所需的最小连接，而重复出现的核心术语，应当通过 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary.md) 和与之对应的代表 Section 再次确认。
