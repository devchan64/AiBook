...
Part 7의 여러 실습을 끝냈다고 해서 프로젝트가 곧바로 다시 실행 가능한 상태가 되는 것은 아닙니다. 좋은 결과표, 모델 출력, RAG 검색 기록, AI 에이전트 실행 로그가 있어도 다른 사람이 같은 조건에서 다시 돌릴 수 없다면 학습 기록은 금방 흩어집니다.
...
Part 7 앞 절들은 서로 다른 실행 장면을 다룹니다. P7-2에서는 비교 실험, P7-4에서는 학습 로그, P7-9에서는 RAG 검색 근거, P7-11에서는 AI 에이전트 실행 기록, P7-13 앞 절에서는 배포와 운영 회고를 다뤘습니다. 그런데 실제 프로젝트에서는 이 기록들이 한 폴더나 문서 묶음 안에서 다시 실행 가능해야 합니다.
...
- 재현성 점검 파일: [`p7-14-reproducibility-items.csv`](../../../assets/part-07/chapter-14/p7-14-reproducibility-items.csv){ .csv-preview }
...
data_path = Path("docs/assets/part-07/chapter-14/p7-14-reproducibility-items.csv")
...
{'패키지': 'agent-ops', '항목': 'item-12', '범주': 'release_note', '상태': 'missing', '다음 조치': 'P7-11.2 또는 P7-11.3 릴리즈노트 상태를 확인한다'}
```
