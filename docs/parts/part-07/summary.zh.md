...
Part 7 是把前面介绍过的内容放进真实项目执行与解释里来实作的区段。这个 Part 的核心，不是完成大型服务，而是把 Part 4、Part 5、Part 6 讲过的内容真正执行出来，并能解释 `为什么会得到这样的结果`。
...
- Part 4 里讲过的问题设定、baseline、预处理、比较实验，会在 `P7-1` 与 `P7-2` 里重新执行。
- Part 5 里讲过的输入结构、学习结果解释、错误阅读，会在 `P7-3` 与 `P7-4` 里重新确认。
- Part 6 里讲过的 RAG、agent、部署与运营判断，会在 `P7-5`、`P7-6`、`P7-7` 里连同依据、日志、策略、回顾一起处理。
...
1. 在 Part 4 实作里运行问题、样本单位、baseline、比较实验。
2. 在 Part 5 实作里阅读输入结构、学习曲线、错误案例。
3. 在 Part 6 实作里按照检索依据、执行日志、审批策略标准来检查 RAG 与 agent。
4. 在综合运营实作里把部署、日志、失败应对、回顾文档整理成一条流程。
...
这些例子轴已经在 Part 7 里分散但充分地回收，所以当前版本不再需要额外的项目 `补充学习`。相反，更重要的是让各节正文示例直接呈现出 `问题 -> 执行 -> 比较 -> 解释 -> 回顾` 这条流。
...
在合上 Part 7 总结之前，最好再把下一次真实项目可以直接复用的最小执行步骤固定一遍。
...
| 固定问题与输入单位 | `P7-1.1` |
| 转成回顾结构 | `P7-1.2` |
| 比较 baseline 与改进 | `P7-2.2` |
| 把错误案例改写成回顾 | `P7-3.2` |
| 通过评估记录读失败位置 | `P7-4.2`, `P7-5.2` |
| 留下执行日志、权限、失败回顾 | `P7-6.2`, `P7-7.2` |

换句话说，重新启动真实项目时，直接去对照当前卡住阶段对应的代表节就可以了。尤其把练习节也一起看时，回到的就不只是读说明，而是直接回到改值和重写结果的层级。
