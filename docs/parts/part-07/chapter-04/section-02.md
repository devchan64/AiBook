...
- 정확도는 적었지만 OOV와 coverage를 기록하지 않은 경우
- 오답을 남겼지만 핵심 표현이 학습 어휘 밖이었는지 보지 않은 경우
- coverage가 낮은데도 맞은 샘플을 무시해 `낮으면 무조건 오답`처럼 읽고 있는 경우

이때는 새 모델을 붙이기 전에 `토큰화 규칙`, `OOV 목록`, `coverage`, `검토 대상 샘플`을 먼저 문서에 남겨야 합니다.

- 텍스트 프로젝트에서는 정확도만으로 해석이 부족하다는 점을 설명할 수 있는가?
- OOV와 coverage를 함께 기록해야 한다는 점을 말할 수 있는가?
- 낮은 coverage를 즉시 오답이 아니라 재검토 신호로 읽을 수 있는가?
- 다음 개선 방향을 동의어 확장, 표현 정규화, 토큰화 방식 변경, 데이터 추가로 나누어 적을 수 있는가?
