...
이 절의 핵심은 `기준선 비교 -> coverage/OOV -> 반복 패턴` 순서로 실패를 분해하는 데 있습니다. 정확도 숫자를 입력 표현 기록과 함께 읽어야만 지금 고쳐야 할 것이 기준선 한계인지, 표현 불일치인지, 더 큰 구조 문제인지 갈라집니다.

Part 7에서 `토큰화(tokenization)`, `토큰 커버리지(token coverage)`, `평가(evaluation)`의 경계가 흐려지면 이 절과 [개념사전](../../../reference/concept-glossary.md)을 다시 함께 확인해, 입력 표현 기록과 실패 해석이 어디서 이어지는지 분리해 두는 것이 좋습니다.
...
이번 절에서는 P7-4.1의 같은 학습 데이터를 유지한 채 평가 문장을 조금 더 넓혀 봅니다. 핵심은 `낯선 표현`, `혼합 문의`, `coverage는 낮지만 정답은 맞은 문의`를 함께 두어, coverage를 너무 단순하게 읽지 않는 것입니다.
...
이번 예제의 목적은 각 평가 문장의 토큰 목록, OOV 목록, 기존 어휘 포함 비율을 함께 출력하는 것입니다. 이번에는 정확도와 함께 `coverage가 낮은 샘플`, `오류 샘플`, `다시 볼 질문`까지 묶어 남기겠습니다.
...
이번 절도 P7-4.1과 같은 [`p7-4-support-routing-dataset.csv`](../../../assets/part-07/chapter-04/p7-4-support-routing-dataset.csv)를 그대로 읽습니다. 이렇게 두면 같은 문의 데이터 파일 위에서 기본 분류와 coverage/OOV 평가를 연속해서 확인할 수 있습니다.

## 이 절에서 직접 할 일

이 절의 연습은 오답 하나를 `모델이 틀렸다`로 닫지 않고, 기준선 비교와 입력 표현 기록으로 다시 분해하는 일입니다. 실행 전에는 평가 문장의 토큰이 학습 어휘에 얼마나 남는지 먼저 예상하고, 실행 뒤에는 낮은 coverage 샘플 중 무엇이 실제 오답이고 무엇이 재검토 후보인지 나눕니다.
...
실행 뒤에는 다음 형식으로 실패 분해 기록을 남겨 봅니다.
...
텍스트 분류 프로젝트를 읽다 보면 `틀렸으니 모델 구조를 바꾸자`로 너무 빨리 뛰기 쉽습니다. 하지만 이번 예제처럼 범위가 제한된 실습에서도 그 전에 먼저 갈라야 할 질문이 있습니다.
...
즉, 이번 절의 coverage 기록은 단순 보조 통계가 아니라 `어디부터 고칠지`를 고르는 첫 분기점입니다.
...
이 절은 OOV와 coverage 해석이 중심이므로, 실행 뒤에는 표현을 일부러 바꿔 coverage를 흔들어 보는 편이 좋습니다.
...
이 절에서 직접 확인해야 하는 핵심은 `정확도가 몇 점인가`보다 `표현 변화가 검토 우선순위를 어떻게 바꾸는가`입니다.
