...
같은 결과를 회고 문서에 넣을 때는 coverage와 팀 점수를 이미지로 함께 남겨 둡니다. 다음 차트는 [`p7_4_2_coverage_review_chart.py`](../../../assets/part-07/chapter-04/p7_4_2_coverage_review_chart.py)가 같은 CSV를 읽어 다시 계산한 결과입니다.

![평가 문장별 coverage와 정답 여부, 낮은 coverage인 평가-05와 평가-07의 환불팀·배송팀 점수를 비교한 차트](../../../assets/part-07/chapter-04/p7-4-2-coverage-review-chart-ko.png)

왼쪽 그래프의 점선 아래에는 `평가-05`와 `평가-07`이 함께 놓입니다. 하지만 오른쪽 그래프에서는 두 문장이 다르게 보입니다. `평가-05`는 환불팀 점수 `0`, 배송팀 점수 `3`이라 오답으로 기울고, `평가-07`은 환불팀 점수 `7`, 배송팀 점수 `0`이라 정답을 유지합니다. 즉, 낮은 coverage는 위험 신호이지만, 다음 행동은 팀 점수와 핵심 OOV를 함께 본 뒤에 정합니다.

차트를 학습 기록으로 옮길 때는 다음 세 단계를 따릅니다.

1. **신호 찾기:** 점선 아래의 낮은 coverage 샘플 ID를 모두 적습니다.
2. **결과 가르기:** 각 샘플의 팀 점수와 정답 여부를 보고, 즉시 수정 후보와 다음 반복 후보를 나눕니다.
3. **다음 행동 정하기:** 오답인 `평가-05`에는 `캔슬 -> 취소` 정규화를, 정답인 `평가-07`에는 `스케줄 -> 일정` 같은 예방적 검토를 제안합니다.
...
3. 차트에서 점선 아래에 있는 샘플을 골라 `coverage`, 팀 점수, 정답 여부, 다음 행동을 한 줄로 기록해 봅니다.
   관찰할 점: 낮은 coverage라는 공통 신호만으로 `즉시 수정`과 `예방적 검토`를 구분할 수 없는 이유는 무엇인가?
...
| 신호와 행동 | coverage·팀 점수·정답 여부를 함께 써서 즉시 수정과 예방적 검토를 나눴는가? |
