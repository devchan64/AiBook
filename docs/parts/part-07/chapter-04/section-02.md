...
고객 문의를 환불팀과 배송팀으로 나누는 작은 텍스트 분류기를 만들었다고 해도, 정확도 숫자 하나만으로 검토를 끝내기 어렵습니다. 실패한 결과를 `기준선 문제`, `입력 표현 문제`, `데이터 범위 문제`, `구조 문제`로 다시 분해해 봐야 합니다.
...
이 절에서는 `기준선 비교 -> coverage/OOV -> 반복 패턴` 순서로 실패를 분해하는 최소 구조를 다룹니다. 즉, 여기서는 `정확도 숫자를 입력 표현 기록과 함께 읽고, 어디부터 고쳐야 할지 가르는 절차`를 먼저 닫습니다.
