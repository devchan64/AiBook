...
P7-4.1에서는 고객 문의를 환불팀과 배송팀으로 나누는 작은 텍스트 분류기를 만들었습니다. 그런데 실제 운영에서는 정확도 숫자 하나만으로 검토를 끝내기 어렵고, 실패한 결과를 다시 분해해 봐야 합니다.

같은 0.833이라도 어떤 문의는 학습 때 본 단어를 대부분 포함하고 있고, 어떤 문의는 핵심 표현이 거의 처음 등장합니다. 그래서 텍스트 프로젝트에서는 `맞았는가`만이 아니라 `모델이 실제로 읽은 단어가 얼마나 있었는가`를 같이 남겨야 합니다.

## 이 절의 범위

- 토큰화(tokenization)는 프로젝트 성능 해석에 왜 직접 영향을 주는가?
- OOV(out-of-vocabulary) 토큰이 많은 문장은 왜 더 조심해서 읽어야 하는가?
- 정확도 외에 어떤 기록이 있어야 다음 개선으로 넘어갈 수 있는가?

이 절에서는 `기존 어휘 포함 비율(token coverage)`과 `사전에 없는 토큰(OOV token)`을 평가 기록에 남기는 최소 구조를 다룹니다. 즉, 여기서는 `정확도 숫자를 입력 표현 기록과 함께 어떻게 읽을 것인가`를 먼저 닫습니다. 더 넓은 실패 기록 구조는 P7-5.2 검색 품질과 답변 검증, P7-7.2 실패 기록과 개선 계획에서 다시 이어집니다.
...
| 무엇이 낮으면 위험한가? | 기존 어휘 포함 비율과 핵심 OOV |
| 그래서 문서에 무엇을 남기는가? | OOV 목록, coverage, 오류 샘플, 다시 볼 질문 |
...
텍스트 분류 프로젝트를 읽다 보면 `틀렸으니 모델 구조를 바꾸자`로 너무 빨리 뛰기 쉽습니다. 하지만 이번 예제처럼 아주 작은 프로젝트에서는 그 전에 먼저 갈라야 할 질문이 있습니다.
