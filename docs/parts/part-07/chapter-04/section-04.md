...
핵심은 `값을 줄이는 요약`과 `판단에 필요한 정보를 남기는 표현`을 구분하는 데 있습니다. 평균은 전체 크기를 빠르게 비교하게 해 주지만, 상승형과 하강형처럼 회고에서 중요한 흐름 차이는 숨길 수 있습니다.
...
```mermaid
--8<-- "assets/part-07/chapter-04/p7-4-4-pattern-decision-flow-ko.mmd"
```
...
여섯 동작을 같은 축에 그리면 평균이 같아도 구간 순서가 다르다는 점을 바로 확인할 수 있습니다. 다음 차트는 [`p7_4_4_equal_mean_patterns_chart.py`](../../../assets/part-07/chapter-04/p7_4_4_equal_mean_patterns_chart.py)가 같은 CSV의 대표 여섯 행을 다시 읽어 만든 결과입니다.

![평균 2.5 기준선을 공유하지만 상승형, 평탄형, 하강형, 중간 집중형, 양끝 집중형으로 다른 여섯 동작의 작은 선 그래프](../../../assets/part-07/chapter-04/p7-4-4-equal-mean-patterns-chart-ko.png)

각 작은 그래프의 점선은 평균 `2.5`입니다. 모든 선이 같은 점선을 중심으로 끝나지만, `PAT-01`은 뒤로 갈수록 높아지고 `PAT-03`은 낮아집니다. `PAT-04`·`PAT-05`는 가운데가 높아 같은 token으로 묶이며, `PAT-06`은 양끝이 높아 별도 token이 됩니다. 차트가 보여 주는 핵심은 `평균이 같다`는 정보만으로는 다음 조치에 필요한 순서 차이를 말할 수 없다는 점입니다.

학습 기록은 다음 세 단계로 남깁니다.

1. **같은 점 찾기:** 평균선이 같은 동작 ID를 고릅니다.
2. **순서 차이 적기:** 선의 시작·끝·집중 구간을 보고 shape token을 하나 붙입니다.
3. **회고 바꾸기:** 평균만 쓸 때의 문장과 shape token까지 쓸 때의 문장을 각각 한 줄로 적고, 달라진 다음 조치를 표시합니다.
...
> 평균만 보면 `PAT-01`부터 `PAT-06`까지 모두 같은 `avg=2.5`로 묶인다. 하지만 shape token을 붙이면 상승형, 평탄형, 하강형, 중간 집중형, 양끝 집중형으로 갈라진다. 따라서 프로젝트에서는 평균만으로 동작 상태를 비교하면 패턴 차이를 잃을 수 있으므로, 회고 표에는 평균과 shape token을 함께 남기는 편이 적절하다.
...
4. 차트에서 `PAT-01`과 `PAT-03`을 골라 평균, shape token, 다음 점검 질문을 한 줄씩 비교해 봅니다.
   관찰할 점: 평균이 같은 두 동작에 서로 다른 다음 조치를 남겨야 하는 이유는 무엇인가?
...
| 차트 해석 | 같은 평균선 위의 두 동작을 골라 순서 차이와 다음 점검 질문을 구분했는가? |
