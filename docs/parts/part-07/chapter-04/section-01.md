# P7-4.1 텍스트 분류 학습 로그 읽기

> Section ID: `P7-4.1`
> Version: `v2026.07.18`

고객 문의를 어느 팀으로 보내야 하는지 분류하는 모델을 만들었다고 해도, 정확도 숫자 하나만 적어 두면 실제로 무엇이 좋아졌는지 읽기 어렵습니다. Part 5에서 본 학습 루프 관점으로 다시 보면, 프로젝트 문서에는 `baseline`, `loss`, `accuracy`, `epoch별 로그`, `오류 샘플`이 함께 남아 있어야 다음 판단이 가능합니다.
...
- epoch별 로그를 남기면 다음 오류 분석으로 무엇을 넘길 수 있는가?

이 절은 `텍스트 분류기의 학습 결과를 어떤 로그와 그래프로 읽을 것인가`를 닫는 데 집중합니다. 즉, 여기서는 `문장 -> 토큰 -> 벡터` 준비를 이미 했다는 전제 위에서, 그 입력으로 실제 학습을 반복했을 때 `loss`, `accuracy`, `오류 샘플`이 어떻게 남는지를 먼저 정리합니다. 토큰화(tokenization)와 coverage가 오류 해석을 어떻게 흔드는지는 바로 다음 P7-4.2에서 이어서 설명합니다.

## 이 절의 목표

- 텍스트 분류 프로젝트의 학습 로그를 `baseline`, `loss`, `accuracy`, `오류 샘플`로 요약할 수 있습니다.
- accuracy가 그대로여도 loss가 더 내려갈 수 있는 이유를 설명할 수 있습니다.
- epoch 로그를 Part 5의 `step`, `batch`, `epoch`, `update` 관점으로 다시 읽을 수 있습니다.
...
| 오류 샘플 | 다음 절에서 어떤 문장을 다시 봐야 하는지 고정하기 위해 |
...
flowchart TD
  A["문의 문장"]
  B["토큰화<br/>공백 기준 분리"]
  C["벡터화<br/>학습 어휘 기준 단어 수 세기"]
  D["softmax 분류기 학습<br/>epoch마다 loss와 accuracy 기록"]
  E["평가 셋 예측"]
  F["오류 샘플과 다음 질문 기록"]
...
이번 절의 흐름에서 달라지는 지점은 `학습` 단계입니다. P7-4.2에서 다시 다룰 OOV와 coverage 이전에, 먼저 `학습이 실제로 반복되면 어떤 기록이 남는가`를 눈으로 붙잡아야 합니다.
...
이번 예제의 목적은 공백 기준 토큰화와 bag-of-words 벡터를 유지한 채, 아주 작은 softmax 분류기를 실제로 학습해 epoch 로그를 남기는 것입니다. 이 예제는 `문장 -> 토큰 -> 벡터` 준비보다 `반복 학습 결과를 어떻게 읽을 것인가`에 더 초점을 둡니다.
...
- 확인할 개념:
  ...
  - 샘플별 예측과 epoch 로그를 같이 남겨야 다음 절의 오류 분석으로 이어질 수 있다
...
## 이 로그를 Part 5 용어로 다시 읽기

이번 예제는 Part 5에서 본 학습 루프 용어를 프로젝트 문서 위에서 다시 읽기 좋게 단순화해 둔 형태입니다.

| Part 5 용어 | 이번 예제에서 무엇을 뜻하나 |
| --- | --- |
| batch | 학습 문의 12개 전체 |
...
이번 실행은 첫 epoch부터 평가 정확도 `0.857`를 기록했습니다. 그런데 이후 epoch에서도 평가 손실이 `0.624 -> 0.404`로 계속 내려갑니다. 이것은 `맞춘 샘플을 더 확실하게 맞추는 중`이라고 읽는 편이 정확합니다.
...
| 샘플 | 최종 예측 | 왜 다음 절로 넘기나 |
| --- | --- | --- |
| `평가-05` `캔슬 후 송장 번호 남아 있어요` | 배송팀 | accuracy 개선만으로는 해결되지 않는 표현 불일치와 OOV 가능성이 보이기 때문 |

따라서 이 절의 결론은 `정확도 0.857`가 아니라, `기준선보다 나아졌지만 특정 표현에서는 여전히 흔들리므로 토큰화와 coverage를 다음 절에서 더 봐야 한다`입니다.

## 학습 로그에서 바로 적어 둘 문장

프로젝트 회고 문장을 한 문단으로 적으면 다음처럼 정리할 수 있습니다.

> 이번 고객 문의 라우팅 분류 실습에서 baseline 정확도는 0.714였고, 학습된 softmax 분류기는 평가 정확도 0.857을 기록했다. 평가 정확도는 첫 epoch부터 0.857로 유지됐지만 평가 손실은 0.624에서 0.404까지 계속 내려갔다. 따라서 이 로그는 `오답 건수는 그대로지만, 맞춘 문의들에 대한 확신은 더 안정화됐다`고 읽는 편이 적절하다. 마지막 오답인 `평가-05`는 다음 절에서 OOV와 token coverage 관점으로 다시 검토할 필요가 있다.
...
- baseline과 학습 모델을 같은 평가 셋에서 비교했는가?
- epoch별 `loss`와 `accuracy`를 같이 기록했는가?
- accuracy가 그대로여도 loss가 더 내려갈 수 있다는 점을 설명할 수 있는가?
- 마지막 오답 샘플을 다음 절의 토큰화 검토 대상으로 넘길 수 있는가?
