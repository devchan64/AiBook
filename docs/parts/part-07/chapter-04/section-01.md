...
`고객 문의를 환불팀(refund)과 배송팀(delivery) 중 어디로 보낼 것인가`라는 문제를 다시 사용해, 텍스트 분류 프로젝트의 학습 로그를 실제로 만들고 읽는 기준을 정리합니다. 핵심은 모델 구조를 복잡하게 늘리는 것이 아니라, `문장 -> 토큰 -> 벡터 -> 학습 로그 -> 오류 샘플` 흐름이 어떻게 기록되는지 직접 확인하는 데 있습니다.
...
`텍스트 분류기의 학습 결과를 어떤 로그와 그래프로 읽을 것인가`를 닫는 데 집중합니다. 즉, `문장 -> 토큰 -> 벡터` 준비를 이미 했다는 전제 위에서, 그 입력으로 실제 학습을 반복했을 때 `loss`, `accuracy`, `오류 샘플`이 어떻게 남는지를 먼저 정리합니다. 토큰화(tokenization)와 coverage가 오류 해석을 어떻게 흔드는지는 다음 입력 해석 단계에서 별도로 다룹니다.
...
예를 들어 평가 정확도가 몇 epoch째 `0.857`로 그대로면, 빠르게는 `학습이 이미 멈췄다`고 적고 싶어질 수 있습니다. 하지만 더 안전한 다음 판단은 정확도 한 줄로 결론 내리는 것이 아니라, `loss가 계속 내려가는가`, `같은 오답 샘플이 남아 있는가`, `확률 분포가 더 또렷해졌는가`를 같이 보는 것입니다. 그래야 `더 이상 맞는 개수는 늘지 않았지만 확신은 커지고 있다`는 장면과 `정말 더 배울 것이 없어졌다`는 장면을 구분할 수 있습니다.
...
프로젝트는 `고객 문의를 환불팀과 배송팀 중 어디로 보내야 하는가?`라는 질문에서 시작합니다. 같은 문제를 `학습 로그` 관점으로 다시 읽습니다. 그래서 더 중요하게 볼 것은 최종 예측표만이 아니라, 그 예측표가 만들어질 때 epoch마다 어떤 값이 남았는가입니다.
...
프로젝트 흐름에서 달라지는 지점은 `학습` 단계입니다. OOV와 coverage를 따로 따지기 전에, 먼저 `학습이 실제로 반복되면 어떤 기록이 남는가`를 눈으로 붙잡아야 합니다.

## 입력 파일

- 문의 데이터: [`p7-4-support-routing-dataset.csv`](../../../assets/part-07/chapter-04/p7-4-support-routing-dataset.csv){ .csv-preview }
- 학습 로그: [`p7-4-training-log.csv`](../../../assets/part-07/chapter-04/p7-4-training-log.csv){ .csv-preview }
...
즉, `모델 입력`과 `학습 결과 기록`을 분리해 두고 읽습니다. 이렇게 해야 나중에 같은 데이터셋으로 토큰화 규칙만 바꿔도 무엇이 달라졌는지 다시 비교하기 쉽습니다.

## 실행 기록 기준

학습 로그의 핵심은 코드를 길게 설명하는 데 있지 않고, `기준선`, `loss`, `accuracy`, `오류 샘플`을 같은 프로젝트 기록으로 묶는 데 있습니다. 입력 데이터와 학습 로그는 서로 다른 파일이고, 결과 해석에서는 마지막 정확도보다 epoch별 변화와 남은 오답을 먼저 봅니다.

기록은 다음 기준으로 나눕니다.

1. 문의 데이터 파일에서 train/test 분리와 라벨 기준을 확인합니다.
2. 학습 로그 파일에서 `train_loss`, `eval_loss`, `eval_accuracy`, `baseline_accuracy` 열을 찾습니다.
3. Python 예제를 실행한 뒤 baseline 정확도와 마지막 epoch 평가 정확도를 비교합니다.
4. 정확도가 고정된 구간에서도 loss가 내려가는지 확인하고, 마지막 오답 샘플 ID를 따로 적습니다.

## Python 예제

예제는 공백 기준 토큰화와 bag-of-words 벡터를 유지한 채, 기본 softmax 분류기를 실제로 학습해 epoch 로그를 남기는 것입니다. 이 예제는 `문장 -> 토큰 -> 벡터` 준비보다 `반복 학습 결과를 어떻게 읽을 것인가`에 더 초점을 둡니다.
...
샘플별 평가 =
{'평가 샘플': '평가-01', '문장': '환불 진행 상태 확인', '예측 팀': '환불팀', '실제 팀': '환불팀', '정답 여부': '예', '팀별 확률': [0.727, 0.273]}
{'평가 샘플': '평가-02', '문장': '배송 조회 언제 가능', '예측 팀': '배송팀', '실제 팀': '배송팀', '정답 여부': '예', '팀별 확률': [0.252, 0.748]}
{'평가 샘플': '평가-03', '문장': '불량 반품 후 환불 일정', '예측 팀': '환불팀', '실제 팀': '환불팀', '정답 여부': '예', '팀별 확률': [0.822, 0.178]}
{'평가 샘플': '평가-04', '문장': '출고 지연 도착 예정 문의', '예측 팀': '배송팀', '실제 팀': '배송팀', '정답 여부': '예', '팀별 확률': [0.304, 0.696]}
{'평가 샘플': '평가-05', '문장': '캔슬 후 송장 번호 남아 있어요', '예측 팀': '배송팀', '실제 팀': '환불팀', '정답 여부': '아니오', '팀별 확률': [0.342, 0.658]}
{'평가 샘플': '평가-06', '문장': '카드 승인 취소 금액 언제 돌아오나요', '예측 팀': '환불팀', '실제 팀': '환불팀', '정답 여부': '예', '팀별 확률': [0.691, 0.309]}
...
차트를 보고 프로젝트 기록으로 옮길 때는 epoch의 처음과 끝을 다음처럼 같은 행에서 비교합니다. 이 표는 `loss가 내려갔다`를 곧바로 `문제를 해결했다`로 읽지 않게 하는 점검 장치입니다.

| 확인 축 | epoch 1 | epoch 12 | 학습자가 남길 해석 |
| --- | ---: | ---: | --- |
| 평가 손실 | `0.624` | `0.404` | 맞춘 문의의 정답 쪽 확신은 더 커졌다. |
| 평가 정확도 | `0.857` | `0.857` | 맞힌 문의 수는 늘지 않았다. |
| 오류 샘플 | `평가-05` | `평가-05` | 혼합 표현 오류는 다음 입력 해석에서 계속 검토한다. |

따라서 이 그래프의 다음 행동은 epoch를 무조건 더 늘리는 것이 아닙니다. `평가-05`처럼 계속 남는 문장을 골라, 데이터 추가·토큰화·입력 표현 중 무엇을 먼저 바꿀지 정하는 것입니다.

## 이 로그를 학습 루프 용어로 다시 읽기

예제는 학습 루프 용어를 프로젝트 문서 위에서 다시 읽기 좋게 단순화해 둔 형태입니다.

| 학습 루프 용어 | 예제에서 무엇을 뜻하나 |
| --- | --- |
| batch | 학습 문의 12개 전체 |
...
이 코드가 `full-batch`이므로 `한 epoch = 한 step`으로 보입니다. 실제 mini-batch 학습에서는 한 epoch 안에 여러 step이 들어가지만, 그 차이를 줄여 `epoch 로그를 읽는 감각`에 집중했습니다.
...
실행 결과는 첫 epoch부터 평가 정확도 `0.857`를 기록했습니다. 그런데 이후 epoch에서도 평가 손실이 `0.624 -> 0.404`로 계속 내려갑니다. 이것은 `맞춘 샘플을 더 확실하게 맞추는 중`이라는 뜻입니다.
...
이번 로그에서는 최종 정확도만 떼어 보지 않습니다. baseline보다 나아졌는지, loss가 어떤 방향으로 움직였는지, 같은 오답 샘플이 끝까지 남았는지를 함께 봐야 합니다.

| 관찰할 것 | 기록할 질문 |
| --- | --- |
| baseline 비교 | 다수 클래스 기준선보다 학습 모델이 실제로 나아졌는가? |
| loss 변화 | 정확도가 고정돼도 손실은 계속 내려가는가? |
| 오류 샘플 | 마지막까지 남은 오답은 어떤 문장인가? |

## 프로젝트 기록 예시

기록은 다음 형식으로 학습 로그를 압축할 수 있습니다.

| 항목 | 기록 |
| --- | --- |
| 기준선 | baseline 정확도는 `0.714`다. |
| 학습 모델 | 마지막 epoch 평가 정확도는 `0.857`다. |
| loss 해석 | 평가 정확도는 고정됐지만 평가 손실은 `0.624 -> 0.404`로 내려갔다. |
| 오류 샘플 | `평가-05`가 마지막 오답으로 남았다. |
| 다음 질문 | 정확도보다 OOV와 token coverage를 함께 확인한다. |
...
5. epoch 1과 epoch 12의 `eval_loss`, `eval_accuracy`, 마지막 오류 샘플을 한 줄씩 옮겨 적어 봅니다.
   관찰할 점: loss 하락, accuracy 고정, 오류 유지 중 어느 신호가 다음 수정 질문을 가장 직접적으로 여는가?
...
| 곡선과 행동 | loss 하락·accuracy 고정·오류 유지 중 어떤 신호로 다음 실험을 정했는가? |
