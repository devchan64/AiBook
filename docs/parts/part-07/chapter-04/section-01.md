...
> 이번 고객 문의 라우팅 분류 실습에서 baseline 정확도는 0.714였고, 학습된 softmax 분류기는 평가 정확도 0.857을 기록했다. 평가 정확도는 첫 epoch부터 0.857로 유지됐지만 평가 손실은 0.624에서 0.404까지 계속 내려갔다. 따라서 이 로그는 `오답 건수는 그대로지만, 맞춘 문의들에 대한 확신은 더 안정화됐다`고 읽는 편이 적절하다. 마지막 오답인 `평가-05`는 OOV와 token coverage 관점에서 다시 검토할 필요가 있다.
...
- 마지막 오답 샘플을 토큰화 검토 대상으로 분명히 적을 수 있는가?
