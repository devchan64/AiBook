...
이 절의 연습은 학습 코드를 길게 이해하는 것이 아니라, `기준선`, `loss`, `accuracy`, `오류 샘플`을 같은 프로젝트 로그로 묶어 읽는 일입니다. 실행 전에는 입력 데이터와 학습 로그가 서로 다른 파일이라는 점을 확인하고, 실행 뒤에는 마지막 정확도보다 epoch별 변화와 남은 오답을 먼저 적습니다.
