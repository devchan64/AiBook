...
P7-4.1에서는 텍스트 분류의 최소 구조를 만들었고, P7-4.2에서는 OOV(out-of-vocabulary)와 coverage를 같이 읽어야 한다는 점을 확인했습니다. 이제 한 걸음 더 가서, `낯선 표현을 학습 때 본 표현으로 바꾸면 실제로 무엇이 달라지는가`를 직접 실험해 볼 차례입니다.

이번 절은 새 이론을 늘리는 자리가 아니라, 같은 고객 문의를 `정규화 전`과 `정규화 후`로 나누어 다시 실행해 보는 연습 절입니다. 핵심은 `캔슬`, `스케줄`, `하자`처럼 실제 운영에서 흔하지만 학습 어휘와 어긋나는 표현을 다룰 때, 무엇을 바꿨고 무엇이 좋아졌는지를 기록으로 남기는 데 있습니다.
...
이 절에서는 `동의어 또는 운영 표현을 학습 어휘에 가까운 표현으로 치환`하는 작은 정규화 실험에만 집중합니다. 즉, 여기서는 거대한 전처리 파이프라인을 설계하지 않고, `표현 하나를 바꾸면 모델이 읽는 단어와 회고 문장이 어떻게 달라지는가`를 닫습니다. 더 넓은 검색 기반 답변의 근거 경계는 P7-5.1, P7-5.2에서 이어집니다.
...
이번 절도 앞 절과 같은 입력 파일을 그대로 사용합니다. 차이는 코드가 `원문 문장`과 `정규화한 문장`을 둘 다 만들어 비교한다는 점입니다.
...
이번 예제의 목적은 `정규화 전후 비교표`를 바로 얻는 것입니다. 코드가 길어 보이더라도 실제로는 앞 절의 토큰화 코드를 유지한 채, `정규화 규칙 적용` 단계만 하나 더 넣은 형태입니다.
