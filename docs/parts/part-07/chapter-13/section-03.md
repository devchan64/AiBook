...
핵심은 운영 점검 분류와 회고 문장 작성을 통해 `운영 판단을 실행 가능한 세 칸으로 나누는 법`을 익히는 데 있습니다. 더 복잡한 인프라 설계보다 먼저 필요한 것은, 지금 들어온 신호를 `즉시 수정`, `재현 확인`, `다음 반복 개선` 중 어디에 둘지 흔들림 없이 나누는 기준입니다.
...
P7-13.2까지 읽으면 실패 기록은 남길 수 있습니다. 하지만 실제 운영에서는 거기서 한 단계 더 나아가 `그래서 지금 무엇부터 할 것인가`를 빠르게 정해야 합니다.
...
예를 들어 `모바일에서 차트가 약간 넘친다`와 `새 Section 링크가 404다`가 함께 들어오면, 빠르게는 둘 다 `배포 후 문제`로 한 묶음 처리하고 싶어질 수 있습니다. 하지만 더 안전한 다음 판단은 문제 수집 단계에서 멈추지 않고, `독자 흐름이 지금 끊기는가`, `재현이 분명한가`, `바로 경로를 고치면 닫히는가 아니면 추가 근거가 필요한가`를 먼저 나누는 것입니다. 그렇게 적어야 `같은 운영 이슈`처럼 보여도 어떤 것은 즉시 수정, 어떤 것은 재현 확인, 어떤 것은 다음 반복 개선으로 보내야 하는지 분명해집니다.

```mermaid
--8<-- "assets/part-07/chapter-13/p7-13-3-incident-priority-flow-ko.mmd"
```
...
- 파일 경로: [`p7-13-ops-scenarios.csv`](../../../assets/part-07/chapter-13/p7-13-ops-scenarios.csv) · [CSV 미리보기](../../../assets/part-07/chapter-13/p7-13-ops-scenarios.csv){ .csv-preview }
- 한 행의 의미: `운영 중 관찰된 신호 한 건`
- 핵심 열: `reader_impact`, `reproducible`, `scope`, `expected_bucket`, `first_check`

운영 판단 훈련만을 위한 시나리오 CSV를 사용합니다. 이렇게 두면 `실패를 기록하는 일`과 `실패를 지금 어떻게 분류할 것인가`를 분리해 연습할 수 있습니다.

## 연습 흐름

```mermaid
--8<-- "assets/part-07/chapter-13/p7-13-3-ops-review-flow-ko.mmd"
```
...
예제는 운영 시나리오를 막연한 사건 목록이 아니라 `행동 구간이 붙은 운영 판단표`로 다시 읽는 것입니다.
...
data_path = Path("docs/assets/part-07/chapter-13/p7-13-ops-scenarios.csv")
...
읽은 파일 = docs/assets/part-07/chapter-13/p7-13-ops-scenarios.csv
운영 판단 기록 =
...
- 운영 시나리오 파일: [`p7-13-ops-scenarios.csv`](../../../assets/part-07/chapter-13/p7-13-ops-scenarios.csv){ .csv-preview }
