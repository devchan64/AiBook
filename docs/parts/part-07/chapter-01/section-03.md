...
같은 운영 로그를 다른 비교 기준으로 다시 읽으면 `데이터는 같아도 기준선 설계가 바뀌면 회고 앞줄이 달라진다`는 점이 바로 보입니다.
...
핵심은 `기준선 구간`과 `샘플 단위`를 바꿔 같은 로그를 다시 읽었을 때 어떤 비교 설계가 다음 질문을 더 선명하게 만드는지 확인하는 데 있습니다. 지금 필요한 것은 더 복잡한 모델이 아니라, 같은 데이터에서도 무엇을 비교 기준으로 삼느냐에 따라 회고의 첫 문장이 달라진다는 사실입니다.
...
예를 들어 `최근 4일 집중` 설정에서 `ads` 하락폭이 더 크게 보이면, 빠르게는 `이 기준선이 가장 민감하니 항상 이것만 쓰면 된다`고 적고 싶어질 수 있습니다. 하지만 더 안전한 다음 판단은 하락폭 크기만 보고 기준선을 고정하는 것이 아니라, `지금 질문이 전체 건강도 확인인가`, `원인 후보 좁히기인가`, `표본 수가 줄어 해석 강도가 얼마나 약해지는가`를 함께 적는 것입니다. 그렇게 적어야 `더 크게 보이는 설계`와 `현재 질문에 더 맞는 설계`를 구분할 수 있습니다.
...
같은 운영 로그를 두고 `기준선 경계`와 `비교 단위`를 바꿨을 때 무엇이 달라지는지 확인합니다. 핵심은 입력을 새로 늘리는 것이 아니라, 같은 기록 위에서도 비교 설계가 달라지면 해석의 앞줄이 달라진다는 점입니다.
...
예제는 `비교 설계를 바꾸면 회고가 어떻게 바뀌는가`를 바로 확인하는 것입니다.
...
두 채널-일자 설계를 그림으로 나란히 보면, 기준선을 최근으로 옮긴다고 항상 변화량이 커지는 것은 아니라는 점이 분명해집니다. 이 예제에서 ads 전환율 변화는 7일 기준선의 `-3.61%p`보다 최근 4일 집중의 `-2.79%p`에서 작고, 오류율 변화도 `+1.14%p`에서 `+0.91%p`로 작습니다. 대신 최근 구간 표본은 7건에서 4건으로 줄어듭니다.

![ads 채널에서 7일 기준선과 최근 4일 집중 설계의 전환율·오류율 기준선 대비 변화와 표본 수를 비교한 그래프. 최근 4일 집중 설계는 변화량이 항상 더 커지지 않으며, 최근 표본이 4건으로 줄어든다.](../../../assets/part-07/chapter-01/p7-1-3-baseline-design-chart-ko.png)

따라서 이 그래프의 학습 포인트는 `더 큰 막대가 더 좋은 기준선`이 아니라 다음과 같습니다.

1. **질문:** 즉시 대응할 최신 신호를 찾는가, 아니면 더 안정적인 비교를 원하는가?
2. **증거:** 변화량의 방향뿐 아니라 기준선·최근 구간의 표본 수를 함께 적었는가?
3. **판단:** 두 설계 모두 ads를 우선 검토 대상으로 남기지만, 짧은 최근 구간의 수치 하나만으로 원인이나 지속성을 단정하지 않는가?
...
> 전체 합계 기준선에서는 전환율 하락이 약하게 보여 서비스 전체 문제처럼 읽힐 수 있었지만, 같은 로그를 `채널-일자` 단위로 다시 묶자 `ads` 채널의 전환율 하락과 오류율 상승이 훨씬 강하게 드러났다. 기준선을 더 최근 구간으로 당기면 변화폭은 더 커질 수도, 이 예제처럼 작아질 수도 있다. 그러므로 최근 집중 설정은 긴급 신호를 더 크게 만드는 규칙이 아니라, 표본 수와 함께 다른 구간을 다시 읽는 설계다. 따라서 지금 단계의 회고 앞줄은 `서비스 전체 하락`보다 `ads 채널 우선 검토`가 더 적합하다.
...
| 확인할 것 | 스스로 답할 질문 |
| --- | --- |
| 기준선 설계 | 같은 로그를 두 개 이상의 기준선 설계로 다시 비교했는가? |
| 비교 단위 | 전체 합계와 세분화 단위가 다른 결론을 만들 수 있음을 기록했는가? |
| 설계 선택 | 어떤 설계가 현재 질문에 더 적합한지 한 문장으로 적었는가? |
| 다음 질문 | 기준선 재설계 뒤의 다음 질문이 더 구체적으로 좁혀졌는가? |
| 문제 정의 | 같은 원시 데이터를 다른 문제 정의로 바꾸면 출력이 어떻게 달라지는지 확인했는가? |
