...
P7-1.1에서는 한 행이 무엇인지와 질문 문장이 왜 먼저인지 확인했고, P7-1.2에서는 같은 결과를 `사실 -> 해석 -> 다음 질문`으로 남기는 방법을 정리했습니다. 이제 한 걸음 더 가서, `기준선 구간을 어디로 잡는가`, `전체 합계로 볼 것인가 채널-일자 단위로 볼 것인가`를 직접 바꿔 보며 결과가 어떻게 달라지는지 확인할 차례입니다.

이번 절은 새 이론을 늘리는 자리가 아니라, 같은 운영 로그를 다른 비교 기준으로 다시 읽는 연습 절입니다. 핵심은 `데이터는 같아도 기준선 설계가 바뀌면 회고 앞줄이 달라진다`는 점을 손으로 확인하는 데 있습니다.
...
이 절에서는 `기준선 구간`과 `샘플 단위`를 바꿔 같은 로그를 다시 읽는 연습에만 집중합니다. 즉, 여기서는 더 복잡한 모델을 붙이지 않고 `어떤 비교 설계가 다음 질문을 더 선명하게 만드는가`를 닫습니다. 이후 baseline 비교가 예측 프로젝트로 어떻게 넘어가는지는 P7-2.1, P7-2.2에서 다시 이어집니다.
...
이번 절은 앞 절과 같은 입력 파일을 그대로 사용합니다. 차이는 `한 번만 계산하고 끝내지 않고`, 서로 다른 비교 설계를 같은 로그 위에 나란히 돌린다는 점입니다.
