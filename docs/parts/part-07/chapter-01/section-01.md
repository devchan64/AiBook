...
이 절은 표를 읽고 질문을 세우는 출발점에 집중합니다. 즉, 여기서는 `질문 -> 표 확인 -> 요약 -> 관찰 기록`으로 프로젝트 문서를 어떻게 시작할지까지를 먼저 닫습니다. 결과를 해석과 회고까지 연결하는 일은 바로 다음 P7-1.2 결과와 회고에서 다시 회수하고, 이후 다른 프로젝트 장들에서도 같은 기록 구조를 다시 사용합니다.
