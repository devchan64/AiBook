...
프로젝트는 `무엇을 알고 싶은가`, `한 행이 무엇을 뜻하는가`, `무엇과 비교할 것인가`를 먼저 적으며 시작합니다. 이 세 가지가 정해져야 요약값을 다음 분석의 근거로 쓸 수 있습니다.

앞에서는 모델, 검색, AI 에이전트(AI agent), 운영까지 큰 구조를 보았습니다. 하지만 프로젝트 파트의 첫 출발은 더 직접적이어야 합니다. `표를 읽고, 질문을 세우고, 입력 단위를 정하는 일`부터 다시 시작해야 합니다. 바로 그 첫 단계입니다. 목적은 모델을 쓰는 것이 아니라, 표에서 질문을 만들고 어떤 행을 한 건의 입력으로 볼지 정한 뒤 요약 결과를 프로젝트 기록으로 남기는 습관을 만드는 데 있습니다.
...
이 출력에서 먼저 봐야 할 것은 `일자 집계`와 `채널-일자 비교`가 서로 다른 수준의 결론을 준다는 점입니다.

| 비교 단위 | 기준선 7일 전환율 | 최근 7일 전환율 | 해석 |
| --- | ---: | ---: | --- |
| 일자 집계 | 10.51% | 9.43% | 하루를 한 샘플로 보면 하락 신호는 보이지만 원인은 아직 흐릿하다 |
| 2026-06-13 ads | - | 5.82% | 최근 채널-일자 가운데 전환율이 가장 낮다 |
| 2026-06-11 ads | - | 5.90% | 오류율도 2.05%로 높다 |
| 2026-06-14 ads | - | 5.93% | 전환율과 오류율을 함께 다시 볼 대상이다 |
| ads 기준선/최근 | 9.78% | 6.17% | 기준선보다 전환율은 낮고 오류율은 높다 |
| organic 기준선/최근 | 12.17% | 12.07% | 전환율과 오류율이 거의 유지된다 |
| search 기준선/최근 | 8.94% | 8.86% | 전환율과 오류율이 거의 유지된다 |
...
`ads_recent_error_adjustment`는 최근 ads 행마다 같은 오류 건수를 더하거나 뺍니다. 기본값과 `-3`을 비교하면 전환율 순위는 같지만, 오류 신호를 해석하는 방향은 달라집니다.
