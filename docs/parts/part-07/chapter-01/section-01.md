...
이 절은 표를 읽고 질문을 세우는 출발점에 집중합니다. 즉, 여기서는 `질문 -> 표 확인 -> 요약 -> 관찰 기록`으로 프로젝트 문서를 어떻게 시작할지까지를 먼저 닫습니다. 결과를 기준선과 나란히 두는 비교 습관은 뒤의 다른 프로젝트 축에서도 계속 반복되지만, 이 절에서는 우선 `질문과 입력 단위를 어떻게 고정하는가`에만 집중합니다.
