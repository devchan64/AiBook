...
이 절은 작은 표를 읽고 질문을 세우는 출발점에 집중합니다. 즉, 여기서는 `질문 -> 표 확인 -> 요약 -> 관찰 기록`으로 프로젝트 문서를 어떻게 시작할지까지를 먼저 닫습니다. 결과를 해석과 회고까지 연결하는 일은 바로 다음 P7-1.2 결과와 회고에서 다시 회수하고, 이후 다른 프로젝트 장들에서도 같은 기록 구조를 다시 사용합니다.
...
- 작은 표를 요약하는 일이 프로젝트 기록의 출발점이라는 점을 말할 수 있는가?
- 평균, 최대값, 비율만으로도 다음 질문을 만들 수 있다는 점을 설명할 수 있는가?
- 모델을 만들기 전에 데이터의 기본 흐름을 읽는 습관이 필요하다는 점을 말할 수 있는가?
- 작은 데이터 프로젝트를 `모델 없는 준비 단계`가 아니라 `질문과 기준점(baseline)을 세우는 첫 프로젝트`로 설명할 수 있는가?
- 요약값은 숫자 목록이 아니라 뒤 프로젝트가 다시 비교할 기준선이라는 점을 말할 수 있는가?
