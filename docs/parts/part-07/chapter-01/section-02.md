...
P7-1.1에서는 운영 로그를 읽고 평균, 최대값, 비율을 계산해 기본 관찰을 만들었습니다. 이제는 그 숫자를 `좋다`, `나쁘다`로 바로 해석하지 않고, baseline과 나란히 두어 첫 비교를 만드는 단계가 필요합니다. baseline과 첫 비교는 숫자를 다시 적는 절이 아니라, `무엇이 기준선보다 실제로 달라졌는가`, `그 차이 가운데 무엇이 먼저 볼 만한가`를 가르는 자리입니다.
...
이 절에서는 `baseline -> 현재 값 -> 비교 결과 -> 다음 질문`으로 이어지는 최소 비교 구조를 다룹니다. 즉, 여기서는 `계산 기록을 어떤 비교 기준 위에 올려야 다음 반복 판단으로 이어지는가`를 먼저 닫습니다. 이후 프로젝트에서 같은 비교 습관은 P7-2.1 비교표와 오류 사례 읽기, P7-5.2 검색 실패와 근거 누락 기록, P7-7.2 장애 기록과 다음 반복 계획으로 다시 이어집니다.
...
| 현재 값 | 이번 실행의 실제 결과를 고정하기 위해서입니다. |
