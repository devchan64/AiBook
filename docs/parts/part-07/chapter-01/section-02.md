...
이 절에서는 `baseline -> 현재 값 -> 비교 결과 -> 다음 질문`으로 이어지는 최소 비교 구조를 다룹니다. 즉, 여기서는 `계산 기록을 어떤 비교 기준 위에 올려야 다음 반복 판단으로 이어지는가`를 먼저 닫습니다. 같은 비교 습관은 예측 프로젝트, RAG 기록, 운영 회고에서도 반복되지만, 이 절에서는 우선 첫 비교 문서를 어떻게 세우는가에 집중합니다.
