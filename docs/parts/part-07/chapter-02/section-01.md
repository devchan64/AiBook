...
실습용 분류 데이터를 코드 안에 직접 쓰지 않고 [`p7-2-churn-dataset.csv`](../../../assets/part-07/chapter-02/p7-2-churn-dataset.csv){ .csv-preview }에 둡니다. Python 코드는 `split` 열을 기준으로 학습(train)과 평가(test)를 나누는 단계부터 시작합니다. 아래 코드는 `docs/` 폴더가 보이는 책 저장소 루트에서 실행합니다.
...
정확도는 `0.500`에서 `0.667`로 0.167만 올랐지만, 그 변화는 세 샘플의 서로 다른 움직임에서 나왔습니다.
...
![baseline과 raw 1-NN 사이의 평가 샘플 결과 전환](../../../assets/part-07/chapter-02/p7-2-1-prediction-outcome-transition-ko.png)

이 차트에서 `평가-03`, `평가-05`의 두 번의 회수와 `평가-04`의 한 번의 새 오답을 함께 세면, 정확도가 한 건만 늘어난 이유가 보입니다. 따라서 `1-NN이 더 좋다`는 기록은 맞지만, `어떤 고객에게나 더 안전하다`는 결론은 아직 쓸 수 없습니다.
...
| `평가-02`는 둘 다 틀린다 | 모델이 아직 회수하지 못한 이탈 위험 고객이므로 거리와 학습 사례를 다시 읽는다 |
| `평가-04`는 1-NN 새 오답이다 | baseline이 맞힌 유지 고객을 모델이 새로 틀렸으므로, 개선 점수만으로 모델을 채택하지 않는다 |
...
전처리 필요성을 실제 오류 사례로 느끼는 것이 중요하므로, 실행 뒤에는 입력 스케일을 일부러 흔들어 보는 편이 좋습니다. 아래 두 실험은 서로 독립적으로 실행합니다. 두 번째 실험을 시작하기 전에는 `feature_scales`를 기본값으로 되돌립니다.

1. `feature_scales`를 `np.array([1.0, 1.0, 60.0])`으로 바꿔 `usage_minutes_30d`를 시간 단위처럼 줄여 봅니다.
   관찰할 점: `평가-02`의 최근접 이웃은 `학습-02`에서 `학습-08`로, `평가-04`는 `학습-11`에서 `학습-04`로 바뀝니다. 두 행 모두 맞게 예측되어 1-NN 정확도는 `1.000`, 실패 샘플 목록은 빈 목록이 됩니다. 사용 시간의 큰 숫자 차이를 줄이자 다른 두 특징도 최근접 이웃 선택에 반영된 결과입니다.

2. `feature_scales`를 `np.array([1.0, 1.0, 1.0])`으로 되돌린 뒤, `test` 행이 아니라 학습 행 마지막에 다음 한 줄을 추가해 봅니다. `train,학습-13,6,21,4000,1`
   관찰할 점: 학습 라벨 수는 유지(0) 7건, 이탈 위험(1) 6건이므로 baseline은 여전히 0을 예측합니다. 반면 `평가-02`(6, 21, 4050)는 새 학습 행과 가장 가까워져 1-NN이 회수하고, 1-NN 정확도는 `0.833`, 실패 목록에는 `평가-04`만 남습니다. `split=test`로 넣으면 평가 대상만 늘어나므로 이 관찰과 다른 실험이 됩니다.
