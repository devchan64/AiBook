...
| --- | --- | --- |
| `project_run` | 이번 실행에서 baseline과 모델을 비교한 최소 결과를 한 번에 남기기 위해 | 뒤 절에서 실행 단위 비교, 재실험 판단, 개선 여부 확인의 기준점이 된다 |
| `comparison_rows` | 어떤 샘플에서 예측이 달랐는지 숫자 뒤의 실제 사례를 남기기 위해 | 오류 사례 묶음, 회고 메모, 재평가 후보를 정리할 때 근거가 된다 |
...
즉, 프로젝트 문서에는 성과와 한계를 함께 남겨야 합니다. 특히 `baseline_error_ids`처럼 틀린 샘플 ID를 바로 볼 수 있어야 다음 반복에서 `왜 그 샘플을 틀렸는가`를 다시 추적하기 쉽습니다. 이 차이도 먼저 검토 우선순위를 올리는 신호로 읽고, 표본이 작을 때는 원인 설명보다 다음 확인 순서를 더 또렷하게 남기는 편이 안전합니다.
