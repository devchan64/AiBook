...
이 절은 작은 분류 프로젝트의 최소 기록 구조를 잡는 데 집중합니다. 즉, 여기서는 `문제 -> 데이터 분리 -> baseline -> 모델 -> 비교`로 프로젝트 문서를 어떻게 시작할지까지를 먼저 닫습니다. baseline 이후의 개선 비교는 바로 다음 P7-2.2 기준 모델과 개선에서 다시 회수하고, 뒤 프로젝트 장들에서도 같은 비교 기록 구조를 다시 사용합니다.
...
- 작은 데이터 실습이라도 예측값 자체를 직접 읽을 수 있어야 한다는 점을 말할 수 있는가?
