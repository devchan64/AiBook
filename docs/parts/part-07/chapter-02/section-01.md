...
이 절은 분류 프로젝트의 최소 기록 구조를 잡는 데 집중합니다. 즉, 여기서는 `문제 -> 데이터 분리 -> baseline -> 모델 -> 비교`로 프로젝트 문서를 어떻게 시작할지까지를 먼저 닫습니다. baseline 이후의 개선 비교는 바로 다음 P7-2.2 `회고와 다음 질문 만들기`에서 다시 이어집니다.
...
이 감각이 바로 다음 절 P7-2.2에서 다룰 `회고와 다음 질문 만들기`의 핵심입니다.
