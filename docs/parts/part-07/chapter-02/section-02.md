...
이 절에서는 baseline 대비 개선을 어떻게 문서화할지에 집중합니다. 더 다양한 평가 축을 함께 적는 감각은 뒤의 P7-4.2 토큰화와 평가, P7-5.2 검색 품질과 답변 검증에서 다시 이어지며, 교차검증과 자동 탐색 절차 전부는 현재 본편 범위 밖으로 둡니다.
...
여기서 baseline은 단순 분류기 이름만 뜻하지 않습니다. 어떤 프로젝트에서는 다수 라벨 예측이 baseline일 수 있고, 어떤 프로젝트에서는 최근 결과를 평소 기준선과 나란히 둔 비교표 자체가 baseline 역할을 할 수 있습니다. 중요한 것은 `무엇을 기준으로 달라졌다고 말하는가`를 문서에 먼저 고정하는 일입니다.
...
개선 기록도 프로젝트 유형에 따라 형태가 달라질 수 있습니다.

| 프로젝트 질문 | baseline이 놓이는 자리 | 개선 기록의 최소 산출물 |
| --- | --- | --- |
| 분류 모델이 단순 규칙보다 나은가 | 기준 모델(baseline model) | baseline 점수, 모델 점수, 샘플별 예측 비교 |
| 최근 결과가 평소와 다른가 | 비교 기준선(baseline reference) | 최근 구간 요약, 평소 기준선 요약, 차이 표 |
...
이 절의 역할을 더 직접적으로 말하면, 여기서 만든 비교 기록 방식은 뒤의 이미지, 텍스트, RAG, agent, 배포 프로젝트까지 공통으로 이어집니다. 즉, 어떤 프로젝트든 최소한 `기준점`, `달라진 사례`, `review 상태`, `다음 질문`을 같은 형식으로 남길 수 있어야 합니다.

| 공통 개선 기록 항목 | 왜 모든 프로젝트에 필요한가 |
| --- | --- |
| 기준점(baseline or reference) | 무엇과 비교하는지 고정하기 위해서입니다. |
| 달라진 사례 | 실제로 무엇이 변했는지 숫자 밖에서 보기 위해서입니다. |
| review 상태 | 아직 확정할 수 없는 변화나 애매한 사례를 표시하기 위해서입니다. |
...
이 해석은 중요합니다. 프로젝트 문서에서는 `accuracy가 올랐다`보다 `무엇을 이용해 더 잘 구분했는가`가 더 좋은 설명이기 때문입니다. 그리고 그 차이도 먼저 검토 필요 신호로 읽고, 작은 표본에서는 원인 확정보다 다음 반복에서 무엇을 다시 볼지 더 분명히 적는 편이 맞습니다.

같은 원리를 더 압축한 공통 템플릿으로 적으면 다음과 같습니다.
...
여기서 `변화 없음`도 결과라는 점을 익혀 두는 편이 좋습니다. 프로젝트는 항상 dramatic improvement만 기록하는 문서가 아닙니다. 변화가 없다는 사실도 다음 반복의 기준선을 고정하고, 불필요한 과장을 줄이는 검토 메모가 됩니다.
...
이 절까지 오면 Part 7의 첫 두 프로젝트가 연결됩니다.

- P7-1에서는 질문과 요약을 먼저 남겼습니다.
- P7-2에서는 baseline과 모델 비교를 붙였습니다.
...
`Part 7의 이후 프로젝트에서 무엇이 좋아졌다고 쓸 때, 그 문장을 더 조심스럽고 재사용 가능하게 만드는 기준을 마련하는 절이다.`
