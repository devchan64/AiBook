...
P7-2.1에서는 기준점(baseline)과 raw 1-NN 분류기를 나란히 두고 비교했습니다. 이제 같은 프로젝트를 한 단계 더 정리해, 그 비교를 회고와 다음 질문으로 넘겨 보겠습니다.
...
이 절에서는 기준점 대비 개선을 어떻게 문서화할지에 집중합니다. 즉, 여기서는 `같은 기준에서 무엇이 달라졌는가`를 비교 가능한 기록으로 바꾸는 기준을 먼저 닫습니다. 더 다양한 평가 축을 함께 적는 감각은 뒤의 P7-4.2 `실패한 결과를 다시 분해하기`, P7-5.2 `검색 실패와 근거 누락 기록`에서 다시 이어집니다.
