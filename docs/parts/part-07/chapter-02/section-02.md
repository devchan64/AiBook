...
이 절에서는 기준점 대비 개선을 어떻게 문서화할지에 집중합니다. 즉, 여기서는 `같은 기준에서 무엇이 달라졌는가`를 비교 가능한 기록으로 바꾸는 기준을 먼저 닫습니다. 더 다양한 평가 축을 함께 적는 감각은 뒤의 P7-4.2 토큰화와 평가, P7-5.2 검색 품질과 답변 검증에서 다시 이어집니다.
