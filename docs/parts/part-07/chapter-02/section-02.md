...
정확도 `0.667 -> 1.000`은 여섯 평가 샘플 중 두 건의 결과 전환에서 나왔습니다. 아래 차트는 raw 1-NN과 정규화 1-NN을 같은 평가 셋에서 비교해, 정규화가 회수한 샘플과 새로 만든 오답을 함께 셉니다.

![raw 1-NN과 정규화 1-NN의 평가 결과 전환. 평가-02와 평가-04 두 건은 정규화로 회수됐고, 정규화가 새로 만든 오답과 둘 다 오류인 샘플은 없다.](../../../assets/part-07/chapter-02/p7-2-2-normalization-outcome-chart-ko.png)

이 차트의 핵심은 정규화 후 정확도가 1.000이라는 사실만이 아닙니다. `평가-02`, `평가-04`라는 경계 사례 두 건을 회수했고, 이 작은 평가 셋에서는 새 오답이 없었다는 사실입니다. 그러나 여섯 건 모두 맞았다고 해서 다른 고객 구간에서도 같은 결과가 유지된다고 말할 수는 없습니다.

학습 점검은 다음 세 단계로 합니다.

1. **사실:** 회수된 두 샘플과 새 오답 수를 같은 평가 셋에서 확인했는가?
2. **해석:** 사용 시간의 큰 단위가 raw 거리에서 이웃 선택을 왜곡했을 가능성을 설명할 수 있는가?
3. **다음 실험:** 더 많은 경계 사례나 다른 train/test 분리에서도 회수는 유지되고 새 오답은 생기지 않는지 확인할 수 있는가?
