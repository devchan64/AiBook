...
개선(improvement)은 무엇을 기준으로 말해야 하는가? 이 절은 그 질문을 정리합니다. 개선은 점수가 올랐다는 말이 아니라, 같은 기준에서 기준점보다 무엇이 달라졌는지 설명하는 일입니다.
...
이 절의 핵심은 기준점 대비 개선을 `같은 기준에서 무엇이 달라졌는가`라는 기록으로 바꾸는 데 있습니다. 점수 상승만 적는 것이 아니라, 어떤 샘플이 왜 달라졌는지까지 남겨야 개선이 다음 반복의 판단 근거가 됩니다.
...
예를 들어 `평가-02`를 빠르게 읽으면 `raw 1-NN도 결국 틀렸으니 별 차이 없는 모델`이라고 넘기기 쉽습니다. 하지만 실제로는 그렇지 않습니다. baseline은 모든 이탈 위험 고객을 놓치고 있고, raw 1-NN은 이미 `평가-03`, `평가-05` 같은 사례를 잡아내고 있습니다. 따라서 여기서 더 안전한 다음 판단은 `효과가 없었다`고 닫는 것이 아니라, `무엇이 아직 거리 계산을 흔드는가`를 찾아보는 것입니다. 이 절에서 그 흔들림은 사용 시간 축의 과도한 영향으로 드러나며, 그래서 다음 행동이 `정규화 전후를 같은 평가 셋에서 다시 비교한다`로 이어집니다.

```mermaid
flowchart TD
  A["평가-02, 평가-04가 여전히 오답"]
  B["빠른 판단: raw 1-NN도 별 차이 없음"]
  C["baseline과 raw 1-NN을 다시 비교"]
  D["잡아낸 샘플과 놓친 샘플을 함께 확인"]
  E["사용 시간 축이 거리 계산을 과하게 흔듦"]
  F["다음 판단: 정규화 전후를 같은 평가 셋에서 다시 비교"]
...
이번 절에서는 train 데이터의 평균과 표준편차를 이용해 z-score 정규화를 적용해 봅니다. 이번에는 정확도 숫자만 한 줄로 끝내지 않고, `raw 1-NN`, `정규화 후 1-NN`, `샘플별 변화 여부`를 함께 남기겠습니다.
...
이번 절도 P7-2.1과 같은 [`p7-2-churn-dataset.csv`](../../../assets/part-07/chapter-02/p7-2-churn-dataset.csv)를 그대로 읽습니다. 이렇게 두면 `같은 데이터 파일` 위에서 기준점 비교와 전처리 개선을 연속해서 확인할 수 있습니다.
...
```text
학습 평균 = [3.83, 12.17, 3308.33]
학습 표준편차 = [2.29, 8.47, 585.65]
...
{'평가 샘플': '평가-02', '실제 정답': 1, '정규화 전 예측': 0, '정규화 후 예측': 1, '정규화 전 정답 여부': '아니오', '정규화 후 정답 여부': '예', '예측 변화 여부': '예', '정규화 전 최근접 학습 샘플': '학습-01', '정규화 후 최근접 학습 샘플': '학습-08'}
...
{'평가 샘플': '평가-06', '실제 정답': 0, '정규화 전 예측': 0, '정규화 후 예측': 0, '정규화 전 정답 여부': '예', '정규화 후 정답 여부': '예', '예측 변화 여부': '아니오', '정규화 전 최근접 학습 샘플': '학습-05', '정규화 후 최근접 학습 샘플': '학습-05'}
```
...
| --- | --- | --- | --- |
| 평가-02 | 유지(오답), 최근접 `학습-01` | 이탈 위험(정답), 최근접 `학습-08` | raw 거리에서는 사용 시간 4050분이 너무 강하게 작용해 retained 고객 쪽으로 끌렸다 |
...
- 좋은 점: 이번 예제에서는 스케일 차이를 정리하자 최근접 이웃 선택이 더 합리적으로 바뀌었습니다.
...
이 절은 `개선이 실제로 어떤 샘플을 바꿨는가`를 읽는 절이므로, 실행 뒤에는 다음 두 실험이 특히 유익합니다.
...
이 절에서 직접 확인해야 하는 핵심은 `좋아졌다`는 결론이 아니라 `어떤 샘플은 좋아지고 어떤 샘플은 여전히 남는가`입니다.
...
- 전처리나 입력 변경을 했지만 최근접 이웃이나 실패 샘플 변화까지 남기지 않은 경우
...
- 전처리가 예측 경로 자체를 바꿀 수 있다는 점을 설명할 수 있는가?
- 프로젝트 회고 문장은 다음 반복의 출발점이 된다는 점을 말할 수 있는가?
