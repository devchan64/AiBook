...
Part 7은 앞에서 정리한 개념을 `작은 산출물로 다시 확인하는 구간`입니다. 여기서는 더 새로운 이론을 길게 늘리기보다, 이미 배운 개념이 실제 프로젝트 문서 안에서 어떻게 다시 등장하는지 확인합니다.
...
여기서 중요한 점은 `프로젝트를 했다`는 말을 코드 파일 하나로 대신하지 않는 것입니다. Part 7에서는 가능한 한 다음 같은 기록 이름이 실제로 남아야 합니다.
...
즉, Part 7은 `모델을 만들었다`보다 `무슨 기록을 남겼는가`를 더 엄격하게 묻는 파트입니다.
...
Part 5 마지막에서 이미 본 것처럼, 생성형 AI 기능은 `질문 -> 근거 또는 도구 -> 답변 -> 평가 -> 기록` 흐름으로 읽어야 합니다. Part 7은 이 흐름을 실제 프로젝트 산출물로 바꾸는 파트입니다. 즉, 여기서의 핵심은 코드를 더 길게 쓰는 일이 아니라, 무엇을 어떤 형식으로 남겨야 다음 반복이 가능해지는지를 익히는 데 있습니다.

| Part 5에서 먼저 본 기록 | Part 7에서 프로젝트 산출물로 굳는 이름 |
| --- | --- |
...
이 목표를 실제 프로젝트 기록 기준으로 옮기면 다음과 같습니다.
...
이 최소 산출물을 Part 7의 실제 프로젝트 축으로 다시 쓰면 다음처럼 볼 수 있습니다.
...
Part 7에서는 이 예시들을 별도 `보충학습`으로 다시 빼지 않습니다. 분석 프로젝트의 `project_note -> retrospective_note`, 모델 프로젝트의 `baseline -> comparison_rows`, RAG의 `retrieval_records -> answer_status`, agent의 `execution_records -> approval`, 배포의 `incident_records -> improvement_plan`처럼, 각 축의 핵심 예시를 이미 본문 안에서 직접 따라가게 두는 편이 현재 프로젝트 입문 흐름에 더 잘 맞기 때문입니다. 반면 자율주행 경로 계획처럼 route, path, trajectory, control 층위를 이해하는 사례는 현재 판에서는 Part 1의 P1-7.4 보충학습 범위에 남기고, Part 7의 프로젝트 축으로 다시 확장하지 않습니다.
