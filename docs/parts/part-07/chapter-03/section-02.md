...
P7-3.1에서는 작은 이미지 분류기 하나를 학습하고, test 정확도가 `0.75`로 끝나는 장면을 확인했습니다. 여기서 중요한 것은 점수가 낮았다는 사실 자체보다, `어떤 실패가 어떤 구조 계열을 더 의심하게 만드는가`를 프로젝트 문서에 남기는 일입니다.
...
이 절에서는 한 오류 사례를 `다음 반복의 구조 선택 질문`으로 바꾸는 최소 비교 구조만 다룹니다. 즉, 여기서는 `틀린 샘플을 데이터/표현/구조 질문으로 나눈 뒤, 어떤 구조 계열을 먼저 검토할지`를 먼저 닫습니다. 더 다양한 평가 축과 기록 방식은 P7-4.2 실패한 결과를 다시 분해하기, P7-7.2 장애 기록과 다음 반복 계획에서 다시 이어집니다.
...
P7-3.1에서 test 셋의 세 번째 샘플은 `약한 스크래치`가 있는 표면 패치였습니다. 반올림해 보면 다음과 같습니다.
...
| attention 계열 | 멀리 떨어진 요소 관계를 함께 본다 | 작은 패치보다 더 큰 문맥이나 여러 위치 관계가 중요할 때 검토할 만한가 |

## 왜 이 실패가 구조 선택 질문으로 이어지는가

여기서 바로 한 가지를 구분해야 합니다. 현재 예제는 제한된 synthetic 점검 패치로 만든 한 번의 실행 기록이므로, 아래 내용은 `확정된 원인`이 아니라 `다음 검토를 위한 가설 정리`로 읽는 편이 맞습니다.
...
이번 모델은 매우 작은 softmax 분류기입니다. Part 4에서 본 깊은 층(deep layer)이나 합성곱(convolution)을 전혀 쓰지 않았기 때문에, 약한 결함과 조명 그림자를 더 세밀하게 구분하기 어렵습니다. 이번 사례에서는 순서 누적보다 위치 패턴이 먼저 중요하므로 순차 모델보다 CNN 쪽 질문이 더 직접적이고, 작은 8x8 패치에서는 attention 계열을 바로 붙이기보다 공간 구조 보존이 먼저 읽힙니다.
