...
한 파일만 읽고 끝나지 않습니다. `무슨 일이 있었는가`는 평가 기록에서 읽고, `학습에 무엇이 있었는가`는 패치 파일에서 대조하고, `다음에 무엇을 할 것인가`는 조치 파일에서 읽어 회고 문장으로 다시 묶습니다. 아래 코드는 `docs/` 폴더가 보이는 책 저장소 루트에서 실행합니다.
...
3. `p7-3-followup-actions.csv`에서 다음 조치가 데이터, 입력 표현, 모델 구조 중 어느 축에 속하는지 나눕니다. 어느 축인지 바로 판단할 수 없으면 `분류 필요`로 남기고 근거를 더 확인합니다.
...
test_records = [
    {
        ...
        "오분류 여부": int(row["true_label"]) != int(row["pred_label"]),
    }
...
def 확신_해석(확신_차이):
    if 확신_차이 < 0.25:
        return "애매한 경계 사례"
    return "상대적으로 확신한 오분류"


def 조치_축(조치):
    if any(단어 in 조치 for 단어 in ["학습 데이터", "변형 추가", "라벨", "hard negative", "평가 묶음", "조도 구간"]):
        return "데이터"
    if any(단어 in 조치 for 단어 in ["정규화", "전처리", "촬영"]):
        return "입력 표현"
    if "공간 구조" in 조치 or "모델" in 조치:
        return "모델 구조"
    return "분류 필요"
...
for row in test_records:
    if row["샘플"] == target_sample:
        ...
        오류_사례_기록 = {
            ...
            "클래스별 확률": row["클래스별 확률"],
            "확신 차이": row["확신 차이"],
            "오분류 여부": "예" if row["오분류 여부"] else "아니오",
            "회고 초점": 회고_초점,
            "약한 결함 여부": 약한_결함_여부,
...
오분류_샘플 = [row["샘플"] for row in test_records if row["오분류 여부"]]
검토_대상_샘플 = [
    row["샘플"] for row in test_records if row["오류 검토 필요"] == "예"
]

검토_요약 = {
    "전체 평가 샘플 수": len(test_records),
    "전체 오분류 수": len(오분류_샘플),
    "전체 검토 대상 수": len(검토_대상_샘플),
    "오분류이면서 검토 대상 수": len(set(오분류_샘플) & set(검토_대상_샘플)),
...
검토 요약
  ...
  전체 다음 조치 수 = 36
  대표 다음 조치 수 = 3
오류 사례 기록 = {'오류 샘플': '평가-결함-약함', '실제 라벨': '스크래치 경고', '예측 라벨': '정상 표면', '클래스별 확률': [0.73, 0.27], '확신 차이': 0.459, '오분류 여부': '예', '회고 초점': '상대적으로 확신한 오분류', '약한 결함 여부': '예', '학습 데이터 포함 여부': '아니오', '데이터 가설': '학습 데이터에는 약한 스크래치 강도의 사례가 부족하다', '표현 가설': '8x8 입력을 평평하게 펼치면서 미약한 결함 위치 강조가 약해졌다', '모델 가설': '선형 경계만으로는 약한 스크래치와 그림자 변화를 충분히 구분하기 어렵다'}
대표 다음 조치 목록 =
{'다음 조치': '학습 데이터에 약한 스크래치 강도 사례 추가', '축': '데이터', '이유': '현재 학습 데이터가 비교적 명확한 결함 중심이기 때문'}
{'다음 조치': '조명 그림자와 결함이 겹친 변형 추가', '축': '데이터', '이유': '정상 그림자와 결함을 더 분리해 봐야 하기 때문'}
{'다음 조치': '공간 구조를 더 읽는 후속 모델 검토', '축': '모델 구조', '이유': '평평하게 펼친 입력보다 공간 구조가 더 중요할 수 있기 때문'}
```
...
1. 검토 요약  
   오분류 수와 검토 대상 수를 분리해, 틀린 사례와 맞았지만 다시 살펴볼 경계 사례를 섞지 않습니다.
   즉, 긴 오류 분석 문서를 다시 열기 전에 `이번 반복에서 무엇부터 다시 봐야 하는가`를 한 줄로 잡아 주는 표지 역할을 합니다.

2. 오류 사례 기록  
   단순히 `틀렸다`가 아니라, 정답/예측 클래스 이름, 확률, 확신 차이에 따른 회고 초점, 학습 데이터 포함 여부, 해석 가설까지 한 묶음으로 남깁니다.
...
출력을 볼 때는 `오분류 수`와 `검토 대상 수`를 먼저 구분한 뒤, 각 오류가 어떤 질문을 여는지 봅니다. 같은 오답이라도 학습 데이터에 비슷한 패턴이 없어서 틀린 경우와, 입력 표현이 공간 구조를 충분히 살리지 못해서 틀린 경우의 다음 조치는 다릅니다.
...
1. `p7-3-error-review.csv`에서 `평가-결함-약함`의 `class_0_prob`, `class_1_prob`, `confidence_margin`을 각각 `0.60`, `0.40`, `0.20`으로 함께 바꿔 봅니다.
   관찰할 점: `회고 초점`이 `상대적으로 확신한 오분류`에서 `애매한 경계 사례`로 바뀌는가? 정답과 예측은 같아도, 다음 검토에서는 모델 경계와 라벨 기준을 더 먼저 확인해야 합니다.

2. `p7-3-followup-actions.csv`의 첫 세 조치 안에 `조명 정규화 전처리 추가,밝기 차이가 결함처럼 읽히는지 분리하기 위해서`를 한 줄 넣어 봅니다.
   관찰할 점: 출력된 대표 다음 조치의 `축`이 `입력 표현`으로 표시되는가? 조치의 문구만 늘리지 않고 데이터, 입력 표현, 모델 구조 중 어느 층위를 바꾸는지 함께 기록합니다. 어느 표현에도 맞지 않는 새 조치는 `분류 필요`로 남기고, 임의로 데이터 축으로 처리하지 않습니다.
