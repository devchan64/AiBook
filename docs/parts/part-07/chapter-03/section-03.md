...
차트 제목은 세 표현의 평가 정확도가 모두 `0.75`라는 공통 기준을 먼저 고정합니다. 왼쪽 그래프는 그 기준과 다른 축인 오류 수와 낮은 확신 샘플 수만 보여 줍니다. 따라서 `0.75`와 샘플 수를 같은 막대 높이로 오해하지 않고, 정확도는 같아도 추가 검토 범위는 달라진다는 점을 읽을 수 있습니다. 오른쪽 그래프는 평가 샘플별 확신 차이(margin)를 보여 줍니다. 이처럼 그림을 함께 남기면 다음 회의나 회고에서 `정확도는 같은데 왜 다시 봐야 하는가`를 더 짧게 설명할 수 있습니다.
...
같은 정확도에서 무엇이 달라졌는지는 다음 표로 다시 확인할 수 있습니다. `낮은 확신`은 이 예제에서 margin이 `0.25`보다 작은 샘플을 뜻합니다. 따라서 오류 수와 낮은 확신 수는 같은 항목이 아닙니다. 정답이어도 다음 검토 대상으로 남을 수 있습니다.

| 입력 표현 | 평가 입력 shape | 공통 정확도 | 오류 수 / 낮은 확신 수 | 학습자가 남길 판단 |
| --- | ---: | ---: | ---: | --- |
| 64개 픽셀 그대로 | `(4, 64)` | `0.75` | `1 / 1` | 약한 결함 한 건을 우선 검토한다. |
| 열 평균 8개 | `(4, 8)` | `0.75` | `1 / 4` | 행별 위치를 줄인 뒤 모든 평가 샘플의 판단 여유가 낮아졌는지 확인한다. |
| 중심 band 3개 | `(4, 3)` | `0.75` | `1 / 4` | 중심 영역 가설만으로는 최종 판단이 부족한지 확인한다. |
...
4. 세 표현의 정확도가 모두 `0.75`인 상태에서, `오류 수`와 `낮은 확신 수`를 각각 한 문장으로 적어 봅니다.
   - 관찰할 점: 같은 정확도라는 사실만으로는 어떤 표현을 다음 실험의 기준으로 삼을지 결정할 수 없는 이유는 무엇인가?
...
- [ ] 오류 수와 낮은 확신 수가 서로 다른 검토 신호임을 설명할 수 있는가?
