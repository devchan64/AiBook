...
P7-11.2까지 읽으면 독자는 `승인이 없으면 보류`라는 원칙은 이해할 수 있습니다. 하지만 실제 운영에서는 거기서 한 단계 더 나아가 `어디부터 승인 정책이 시작되는가`, `왜 이 단계는 자동으로 돌리면 안 되는가`, `보류 뒤 다음 행동은 무엇인가`까지 직접 써 봐야 합니다.
...
```mermaid
--8<-- "assets/part-07/chapter-11/p7-11-3-permission-case-flow-ko.mmd"
```

즉, AI 에이전트 품질은 도구 수가 아니라 `실행 경계`를 얼마나 분명히 적는가에서도 갈립니다.  역할은 바로 그 경계를 독자가 직접 다시 그어 보는 데 있습니다.

## 입력 파일

- 단계 기록 파일: [`p7-11-permission-log.csv`](../../../assets/part-07/chapter-11/p7-11-permission-log.csv) · [CSV 미리보기](../../../assets/part-07/chapter-11/p7-11-permission-log.csv){ .csv-preview }
- 연습 정책 파일: [`p7-11-approval-cases.csv`](../../../assets/part-07/chapter-11/p7-11-approval-cases.csv) · [CSV 미리보기](../../../assets/part-07/chapter-11/p7-11-approval-cases.csv){ .csv-preview }
...
```mermaid
--8<-- "assets/part-07/chapter-11/p7-11-3-permission-review-flow-ko.mmd"
```
...
- 문제 상황: AI 에이전트 단계들을 승인 정책 기준으로 다시 분류한다.
...
```python
# AI 에이전트 도구 사용 사례를 읽기, 쓰기, 변경 권한과 승인 여부로 분류해 자동 실행, 승인 후 실행, 즉시 보류를 판단하는 예제입니다.
import csv
from pathlib import Path

base_path = Path("docs/assets/part-07/chapter-11/p7-11-permission-log.csv")
case_path = Path("docs/assets/part-07/chapter-11/p7-11-approval-cases.csv")
...
읽은 기존 기록 파일 = docs/assets/part-07/chapter-11/p7-11-permission-log.csv
읽은 연습 정책 파일 = docs/assets/part-07/chapter-11/p7-11-approval-cases.csv
연습 기록 =
...
즉, AI 에이전트 프로젝트에서 중요한 것은 `도구 호출 성공`보다 `어떤 상태에서 멈춰야 하는가`를 미리 정하는 일입니다.
...
- 승인 정책 연습 파일: [`p7-11-approval-cases.csv`](../../../assets/part-07/chapter-11/p7-11-approval-cases.csv){ .csv-preview }
