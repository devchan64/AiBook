...
Part 7은 앞선 다섯 Part에서 복구하고 정리한 개념을 `작은 산출물`로 다시 확인하는 구간입니다. 이 Part의 핵심은 대형 서비스를 완성하는 것이 아니라, 문제 정의, 데이터 준비, 구현, 평가, 회고라는 공통 흐름을 직접 밟아 보는 데 있습니다.
...
| Part 5에서 먼저 남긴 기록 | Part 7에서 더 큰 산출물로 다시 쓰는 이름 | 이름이 커질 때 붙는 역할 |
| --- | --- | --- |
...
> Part 7에서 그 흐름을 작은 프로젝트로 다시 검증했다.
