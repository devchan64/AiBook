...
```mermaid
--8<-- "assets/part-07/chapter-07/p7-7-2-status-split-flow-ko.mmd"
```

## 운영 변경 시나리오 예제

P7-7.1의 같은 장애 티켓 흐름을 이어 받되, 예제에서는 `blocked`와 `failure`가 함께 들어 있는 실행 로그를 사용합니다.
...
- 파일 경로: [`p7-7-permission-log.csv`](../../../assets/part-07/chapter-07/p7-7-permission-log.csv) · [CSV 미리보기](../../../assets/part-07/chapter-07/p7-7-permission-log.csv){ .csv-preview }
...
data_path = Path("docs/assets/part-07/chapter-07/p7-7-permission-log.csv")
...
```text
읽은 파일 = docs/assets/part-07/chapter-07/p7-7-permission-log.csv
실행 기록 =
