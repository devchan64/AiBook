...
- 배포 기록은 남겼지만 어떤 배포를 다시 봐야 하는지 구분하지 않은 경우

이때는 배포 자동화보다 먼저 `브랜치 상태`, `로컬 빌드`, `배포 상태`, `공개 페이지`, `최신 내용 반영`, `깨진 링크`를 분리해 남겨야 합니다.

- 정적 사이트 배포도 하나의 서비스 배포라는 점을 설명할 수 있는가?
- 빌드 성공과 공개 페이지 정상 동작은 같은 말이 아니라는 점을 말할 수 있는가?
- GitHub Pages는 정적 파일 호스팅 서비스라는 점을 설명할 수 있는가?
- 배포 후 상태 확인 체크리스트가 필요하다는 점을 말할 수 있는가?
- 배포 직후의 성공 상태도 재확인 시점과 함께 남기는 편이 안전하다는 점을 설명할 수 있는가?
