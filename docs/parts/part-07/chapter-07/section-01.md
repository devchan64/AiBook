...
| 검토 대상 항목 | 최신 반영, broken link, 404 같은 재점검 대상을 고정하기 위해서입니다. |
...
이 저장소의 문맥에서는 아래 항목을 먼저 기억해 두면 됩니다.
...
여기서 `deployment_check`는 배포 직후 확인표 한 건이고, `monitoring_note`는 잠시 뒤 다시 볼 점을 남기는 짧은 운영 메모로 읽으면 됩니다.
