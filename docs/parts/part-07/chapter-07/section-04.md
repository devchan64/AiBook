...
운영 신호를 세 행동 구간으로 나누는 것만으로는 충분하지 않을 때가 있습니다. 같은 `재현 확인` 후보 안에서도 어떤 신호는 지켜보기만 하면 되고, 어떤 신호는 바로 검토 대기열 앞쪽으로 올려야 합니다. 이번 절은 동작 단위 합성 데이터를 이용해 `watch`, `review`, `action` 등급을 직접 설계하는 연습입니다.
...
이번 예제의 목적은 `값이 크면 무조건 action` 같은 단순 규칙이 왜 운영 판단을 흔들 수 있는지 확인하는 것입니다.
...
- gap 하나만 보고 등급을 확정하지 않았는가?
- 표본 수가 적은 신호를 더 보수적으로 읽었는가?
- 반복성과 최근성이 높은 신호를 단발성 튐과 구분했는가?
- `watch`, `review`, `action`을 진단 확정이 아니라 검토 우선순위로 읽었는가?
- 등급이 달라진 사례마다 첫 점검 위치를 함께 남겼는가?
