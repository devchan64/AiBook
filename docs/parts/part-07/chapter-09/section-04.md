...
- 문서 조각 파일: [`p7-9-rag-documents.csv`](../../../assets/part-07/chapter-09/p7-9-rag-documents.csv){ .csv-preview }
- 한 행의 의미: `검색 가능한 문서 조각 하나`
- 이번 실습 범위: `문서-1`부터 `문서-18`까지의 대표 문서

P7-9.1부터 P7-9.3까지 사용한 같은 문서 집합을 다시 씁니다. 다만 이번 절의 관심은 답변 작성이 아니라, 검색 설정이 후보 목록을 어떻게 바꾸는지입니다.
...
```text
읽은 파일 = docs/assets/part-07/chapter-09/p7-9-rag-documents.csv
...
## 실제 IVF ANN 설정과 지연 시간

앞의 ChromaDB 예제에서 바꾼 `num_probes`는 metadata bucket을 여는 프로젝트 설정이며, ANN 인덱스의 탐색 파라미터는 아닙니다. 이 둘을 같은 것으로 부르면 후보군 필터와 ANN 탐색의 역할이 섞입니다. 따라서 실제 ANN 설정은 별도 실험으로 확인해야 합니다.

[`p7_9_4_faiss_ann_experiment.py`](../../../assets/part-07/chapter-09/p7_9_4_faiss_ann_experiment.py)는 같은 36개 문서를 TF-IDF 벡터로 바꾼 뒤 FAISS `IndexIVFFlat`의 `nlist=4` 인덱스를 만듭니다. `nprobe`만 `1`, `2`, `4`로 바꾸고, `IndexFlatIP` exact top-5와의 포함률 및 1,000회 평균 검색 시간을 기록합니다.

- 실행 스크립트: [`p7_9_4_faiss_ann_experiment.py`](../../../assets/part-07/chapter-09/p7_9_4_faiss_ann_experiment.py)
- 실제 실행 로그: [`p7-9-4-faiss-ann-results.csv`](../../../assets/part-07/chapter-09/p7-9-4-faiss-ann-results.csv) · [CSV 미리보기](../../../assets/part-07/chapter-09/p7-9-4-faiss-ann-results.csv){ .csv-preview }

2026-08-01 CPU 실행에서는 다음 결과를 얻었습니다.

| `nprobe` | exact top-5 포함률 | 평균 검색 시간 | 누락된 exact 후보 | 해석 |
| ---: | ---: | ---: | --- | --- |
| 1 | 0.6 | 0.0056 ms | `문서-3`, `문서-4` | 한 개 cluster만 탐색해 빠르지만 exact 상위 후보 둘을 놓쳤습니다. |
| 2 | 1.0 | 0.0065 ms | 없음 | 두 개 cluster를 탐색하자 이 질문에서는 exact top-5를 모두 회수했습니다. |
| 4 | 1.0 | 0.0063 ms | 없음 | 작은 36문서 집합에서는 전체 cluster 탐색도 시간 차이가 거의 드러나지 않았습니다. |

이 수치는 작은 합성 문서 집합에서 얻은 CPU 스냅샷입니다. 따라서 `nprobe=2`가 일반적으로 최적이라는 결론은 낼 수 없습니다. 대신 `nprobe=1`의 후보 누락은 실제로 확인됐고, 작은 데이터에서는 마이크로초 단위 지연 시간 차이가 실행마다 흔들려 설정을 고를 근거가 되지 않는다는 점을 보여 줍니다. 더 큰 문서 집합과 여러 질문에서 같은 exact 기준선, recall, 지연 시간을 다시 남겨야 합니다.
...
- Meta AI Research, [FAISS GitHub repository](https://github.com/facebookresearch/faiss){: target="_blank" rel="noopener noreferrer" }, 확인일: 2026-08-01.
