...
`같은 운영 시나리오 안에서도 어떤 단계는 자동 실행하고 어떤 단계는 반드시 멈춰야 하는가`를 직접 연습할 차례입니다. 이번 절은 승인 정책을 추상 원칙이 아니라 단계별 판단 기록으로 바꾸는 데 초점을 둡니다.

이번 절은 에이전트 이론을 더 늘리는 자리가 아니라, 승인 정책(approval policy)과 보류 상태(hold state)를 손으로 재분류해 보는 절입니다. 핵심은 `도구를 쓸 수 있다`와 `지금 이 상황에서 자동 실행해도 된다`를 분리해 기록하는 데 있습니다.
...
이 절의 핵심은 승인 정책과 보류 판단을 통해 `실행 경계`를 어디에 그을지 정하는 데 있습니다. 여기서 먼저 갈라야 하는 것은 모델 성능이나 검색 품질이 아니라, 같은 도구 호출이라도 어떤 단계는 자동으로 진행되고 어떤 단계는 반드시 멈춰야 하는가입니다.
...
예를 들어 `캐시 삭제 실행`이 자동 복구처럼 보이면, 빠르게는 `롤백보다 가벼워 보이니 자동 실행 가능`이라고 적고 싶어질 수 있습니다. 하지만 이 절에서 더 안전한 다음 판단은 도구 이름의 무게감으로 결정하는 것이 아니라, `이 단계가 읽기인가 변경인가`, `범위가 테스트 환경인가 프로덕션인가`, `승인 없이 실행했을 때 독자 영향이 어디까지 가는가`를 먼저 적는 것입니다. 그렇게 읽어야 `겉보기에는 작은 변경`과 `운영 정책상 즉시 보류해야 하는 변경`을 구분할 수 있습니다.

```mermaid
--8<-- "assets/part-07/chapter-06/p7-6-3-permission-case-flow-ko.mmd"
```

즉, 에이전트 품질은 도구 수가 아니라 `실행 경계`를 얼마나 분명히 적는가에서도 갈립니다. 이번 절의 역할은 바로 그 경계를 독자가 직접 다시 그어 보는 데 있습니다.
...
이번 절은 기존 권한 기록과 연습용 단계 후보를 함께 읽습니다. 이렇게 두면 이미 남아 있는 실행 기록과 새 정책 연습을 같은 운영 시나리오 안에서 연결해 읽을 수 있습니다.
...
이번 예제의 목적은 승인 정책을 표로만 보지 않고 실제 분류 결과로 다시 읽는 것입니다. 코드가 길어 보이더라도 실제로는 권한 기록 위에 `정책 판정(policy_decision)`과 `더 안전한 다음 행동`을 한 번 더 얹는 구조입니다.
...
| 임시 우회 설정 반영 | 즉시 보류 | 같은 변경이라도 공개 API 범위는 더 보수적으로 다뤄야 한다 |
...
- 조회 단계와 변경 단계를 같은 기준으로 읽지 않았는가?
- `보류`를 기능 실패가 아니라 운영 판단 상태로 기록했는가?
- 범위가 넓은 변경 단계에 더 강한 멈춤 규칙을 붙였는가?
- 승인 정책 뒤에 따라오는 다음 행동을 함께 적었는가?
