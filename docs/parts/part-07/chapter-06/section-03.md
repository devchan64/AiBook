...
P7-6.2까지 읽으면 독자는 보통 이렇게 생각하기 쉽습니다. `문서가 있으면 답하고, 없으면 멈추면 된다.` 하지만 실제 RAG 프로젝트에서는 그 경계가 그렇게 단순하지 않습니다.
...
```mermaid
--8<-- "assets/part-07/chapter-06/p7-6-3-rewrite-risk-flow-ko.mmd"
```
...
- 문서 파일: [`p7-6-rag-documents.csv`](../../../assets/part-07/chapter-06/p7-6-rag-documents.csv) · [CSV 미리보기](../../../assets/part-07/chapter-06/p7-6-rag-documents.csv){ .csv-preview }
- 연습 질문 파일: [`p7-6-boundary-cases.csv`](../../../assets/part-07/chapter-06/p7-6-boundary-cases.csv) · [CSV 미리보기](../../../assets/part-07/chapter-06/p7-6-boundary-cases.csv){ .csv-preview }
- 문서 파일의 한 행 의미: `검색 가능한 문서 조각 하나`
- 연습 질문 파일의 한 행 의미: `질문-근거 경계를 시험하는 질문 한 개`

P7-6.1, P7-6.2와 같은 문서 집합을 그대로 사용합니다. 대신 질문 파일을 별도로 두어, 같은 지식베이스 위에서 질문 문구와 기대 상태만 바꾸며 결과를 비교합니다. 즉, 다른 프로젝트를 새로 여는 것이 아니라 같은 RAG 프로젝트 기록에서 `질문 경계`가 어떻게 품질 판정을 바꾸는지 더 분명히 드러내는 실습입니다.
...
```mermaid
--8<-- "assets/part-07/chapter-06/p7-6-3-question-boundary-flow-ko.mmd"
```
...
예제는 질문 재작성 전후를 바로 비교하는 것입니다. 코드가 길어 보이더라도 실제로는 P7-6.2의 평가 구조를 유지한 채, `expected_state`와 `rewritten_question` 비교만 하나 더 넣은 형태입니다.
...
읽은 문서 파일 = docs/assets/part-07/chapter-06/p7-6-rag-documents.csv
읽은 연습 질문 파일 = docs/assets/part-07/chapter-06/p7-6-boundary-cases.csv
핵심 연습 기록 =
...
- 문서 집합: [`p7-6-rag-documents.csv`](../../../assets/part-07/chapter-06/p7-6-rag-documents.csv){ .csv-preview }
- 경계 사례 질문: [`p7-6-boundary-cases.csv`](../../../assets/part-07/chapter-06/p7-6-boundary-cases.csv){ .csv-preview }
