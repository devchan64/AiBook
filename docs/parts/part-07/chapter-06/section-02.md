...
P7-6.1에서는 에이전트 run이 승인 대기 때문에 blocked 상태로 멈출 수 있다는 점을 봤습니다. 하지만 실제 운영 기록은 여기서 한 단계 더 나아가야 합니다. 같은 run 안에서도 어떤 단계는 `완료`, 어떤 단계는 `blocked`, 어떤 단계는 진짜 `failure`일 수 있기 때문입니다.
