...
여기에 한 가지를 더 붙이면 agent 절의 입구부터 기록 구조가 훨씬 분명해집니다. 목표와 도구 목록만 적는 데서 끝내지 말고, `각 단계에서 무엇을 관찰할 것인가`, `어떤 상태면 review가 필요한가`, `다음 행동을 무엇으로 바꿀 것인가`를 같이 적어 두는 것입니다.
...
| review 상태 | 멈춤, 재시도, 사람 검토가 필요한 경우를 구분하기 위해서입니다. |
...
- 이번 예제에서 모든 단계가 success로 끝났더라도, 그것은 `현재 장난감 도구와 현재 목표` 기준의 결과입니다. 실제 프로젝트에서는 같은 계획도 다른 관찰이 나오면 review 상태로 바뀔 수 있습니다.
...
| 단계 | 관찰 후 상태 | 다음 행동 |
| --- | --- | --- |
| `read_toc` | 구조 확인 완료 | `check_build` |
| `check_build` | 빌드 성공 또는 review 필요 | 계속 진행 또는 중단 |
| `report_status` | 최종 보고 가능 | finish |

이 표가 있으면 agent 프로젝트의 입구에서부터 `질문 -> 도구 -> 관찰 -> review 상태` 구조가 보입니다.
