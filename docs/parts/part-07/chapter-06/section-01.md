...
이 절은 `목표 -> 도구 선택 -> 관찰 -> 다음 행동 -> blocked 또는 보고` 구조를 프로젝트 문서로 닫는 데 집중합니다. 즉, 여기서는 권한 정책의 세부 설계보다, 관찰을 다 했더라도 외부 승인이나 사람 결정이 없으면 run이 blocked 상태로 끝날 수 있다는 점을 먼저 붙잡습니다. 권한과 로그를 운영 문서로 읽는 기준은 바로 다음 P7-6.2에서 이어서 설명합니다.
...
이번 파일은 단순 성공 로그가 아닙니다. 앞 절의 RAG 후보 표처럼, 여기서도 한 단계마다 `왜 이 도구를 불렀는가`, `무엇을 봤는가`, `현재 상태가 계속 진행인지 blocked인지`, `다음 행동이 무엇인가`까지 같이 적혀 있습니다.
...
> 이번 에이전트 실습에서는 조회 도구 3개와 문서화 단계 2개를 통해 장애 강도와 최근 배포 근거를 확보했다. 그러나 롤백 실행은 승인 전이라 실제 변경 단계로 넘어가지 못했고, 마지막 단계는 `승인 응답 대기` 상태에서 blocked로 종료됐다. 따라서 이 run은 `실패`보다 `외부 승인 미충족 때문에 안전하게 멈춘 실행`으로 읽는 편이 적절하다.
