...
목표를 받아, 필요한 도구를 고르고, 도구 결과를 읽고, 다음 행동을 정하는 실행 루프라고 보면 충분합니다. 이 절의 목적은 agent를 똑똑한 존재로 설명하는 것이 아니라, 목표와 도구 결과를 이어 가는 프로젝트 문서 구조를 익히는 데 있습니다.
...
이 절은 단일 agent 프로젝트의 최소 문서 구조를 잡는 데 집중합니다. 권한과 로그를 어떻게 붙여야 하는지는 바로 다음 P7-6.2 권한과 로그 검토에서 다시 회수하고, 다중 agent와 샌드박스 구현 전개는 현재 본편 범위 밖으로 둡니다.
...
여기에 한 가지를 더 붙이면 agent 절의 입구부터 기록 구조가 훨씬 분명해집니다. 목표와 도구 목록만 적는 데서 끝내지 말고, `각 단계에서 무엇을 관찰할 것인가`, `어떤 상태면 검토가 필요한가`, `다음 행동을 무엇으로 바꿀 것인가`를 같이 적어 두는 것입니다.

| 입구에서 같이 남길 기록 | 왜 필요한가 |
| --- | --- |
| 목표(goal) | 무엇을 끝내려는지 고정하기 위해서입니다. |
| 도구(tool) | 어떤 행동 수단이 있는지 분리하기 위해서입니다. |
| 관찰(observation) 기준 | 성공과 실패를 무엇으로 읽을지 미리 정하기 위해서입니다. |
...
이번 예제의 목적은 agent 상태(state)와 도구 결과를 순서대로 기록하는 것입니다. 이번에는 단순히 `tool`과 `result`만 출력하지 않고, 계획 목록, 실행 기록, 최종 보고를 함께 남겨 실제 실행 루프가 어떻게 문서화되는지 보이게 하겠습니다. 계획 목록은 agent가 왜 그 순서를 택했는지 남기는 계획 기록이고, 실행 기록은 각 단계의 관찰과 다음 행동 결정을 이어 주며, 최종 보고는 흩어진 실행 로그를 마지막 사용자 보고로 묶는 역할을 합니다.
...
- 첫 단계의 결과가 없으면 현재 구조를 모릅니다.
- 빌드 확인 단계의 결과가 실패라면 다음 행동은 최종 보고가 아니라 중단 또는 재시도로 바뀌어야 합니다.
- 마지막 보고 단계는 앞 두 단계 관찰을 묶어야 의미가 있습니다.
- 최종 보고는 실행 루프의 끝에서 무엇을 사용자에게 전달할지 정리합니다.
- 이번 예제에서 모든 단계가 success로 끝났더라도, 그것은 `현재 장난감 도구와 현재 목표` 기준의 결과입니다. 실제 프로젝트에서는 같은 계획도 다른 관찰이 나오면 검토 필요 상태로 바뀔 수 있습니다.
...
| 단계 | 관찰 후 상태 | 다음 행동 |
| --- | --- | --- |
| 목차 읽기 | 구조 확인 완료 | 빌드 확인 |
| 빌드 확인 | 빌드 성공 또는 검토 필요 | 계속 진행 또는 중단 |
| 최종 보고 | 최종 보고 가능 | 마무리 |

이 표가 있으면 agent 프로젝트의 입구에서부터 `질문 -> 도구 -> 관찰 -> 검토 상태` 구조가 보입니다.
...
- success로 보이는 단계도 현재 관찰 기준 안에서만 읽어야 한다
...
P7-6.2에서는 같은 agent 프로젝트를 가지고 다음 질문을 봅니다. 특히 `실행할 수 있다`는 판단과 `현재 권한 정책에서 실행해도 된다`는 판단을 분리해 읽는 기준을 이어서 다룹니다.
