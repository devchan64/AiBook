...
실습은 **학습용 이미지 생성 → 입력·목표 데이터셋 구성 → LoRA 학습 → 검증 입력 평가 → 출력 결과 검수 → 외부 입력 45장 추가 평가**로 이어진다. 먼저 학습에서 제외한 19쌍으로 변환과 보존을 비교한 뒤, 학습 자료와 별도로 만든 남성·여성·유아 입력 45장으로 적용 범위를 살핀다. 앞의 평가는 비교할 Mira 목표가 있지만, 뒤의 평가는 정답 목표 없이 입력 장면의 보존과 Mira 특징의 표현을 검수한다.
...
[입력 후보 220장 생성 목록 JSON](../../../assets/part-07/chapter-05/sec-10/p7-5-10-bfs-input-pool-v2.json)
...
[폐기 목록을 적용하는 입력 생성 Python](../../../assets/part-07/chapter-05/sec-10/p7_5_10_generate_reviewed_inputs.py)

```bash
.venv/bin/python docs/assets/part-07/chapter-05/sec-10/p7_5_10_generate_reviewed_inputs.py \
  --spec docs/assets/part-07/chapter-05/sec-10/p7-5-10-bfs-input-pool-v2.json \
  --output-dir docs/assets/part-07/chapter-05/sec-10/input-images \
  --dry-run
```
...
각도는 생성 요청의 범주이며 측정값이 아니다. 같은 인물의 외형 설명을 반복해 생성했지만 세부 외형·의상·배경이 달라질 수 있다. 자세와 표정도 함께 달라지므로 카메라 각도만의 영향을 측정하는 통제 실험으로 해석하지 않는다. 이 45장은 기존 193쌍 학습에 포함되지 않은 새 합성 입력이며, BFS LoRA 편집 결과와 구도·표정·배경 보존 여부는 후속 평가에서 확인한다.

### 같은 LoRA를 새 입력에 적용한다

최종 1600스텝 LoRA를 강도 1.0으로 적용하여 45장을 순차 생성한다. 기존 평가와 같은 시드 62294·20스텝·CFG 4.0·출력 및 참조 VAE 512×512를 사용한다. 남성·여성·유아에 공통 적용하도록 지시문의 `the woman`만 `the person`으로 바꾸며, 목표 얼굴 참조는 추가하지 않는다. 아래 비교표는 완료된 출력부터 표시한다. 생성 상태와 시각 검수 판정은 구분한다.

[새 입력 45장·LoRA 출력 비교표](../../../assets/part-07/chapter-05/sec-12/codex-camera-lora-v1/README.md){ .aibook-markdown-preview }

[45장 실행 조건 JSON](../../../assets/part-07/chapter-05/sec-12/codex-camera-lora-v1/plan.json)

### 캐릭터 일관성과 입력 보존을 나누어 검수한다

추가 평가에서는 각 입력과 출력의 자세·머리 방향·표정·의상·배경을 비교하고, 출력들 사이에서 Mira의 얼굴·헤어·화풍이 일관되게 나타나는지 확인한다. 남성·여성·유아 입력에서도 Mira로 변환하도록 같은 지시를 적용하므로 입력 인물의 원래 정체성 유지가 평가 목표는 아니다. 특히 연령 인상이 크게 바뀌는지, 측면이나 고각도에서 얼굴 특징이 흐트러지는지를 기록한다.

이 45장은 학습 데이터셋 밖에서 별도로 만든 합성 입력이다. 실제 촬영 자료를 대표하는 표본은 아니며, 각도 외에 표정·의상·배경도 달라지므로 각도만의 효과를 단정하지 않는다. 현재 출력은 순차 생성 중이다. 완료 수와 실패 여부는 비교표에서 확인하고, 전체 출력의 시각 검수가 끝난 뒤 캐릭터 일관성과 보존 수준을 결론으로 정리한다.
...
[폐기 목록을 적용하는 목표 생성 Python](../../../assets/part-07/chapter-05/sec-10/p7_5_10_generate_reviewed_targets.py)

```bash
.venv/bin/python docs/assets/part-07/chapter-05/sec-10/p7_5_10_generate_reviewed_targets.py \
  --spec docs/assets/part-07/chapter-05/sec-10/p7-5-10-mira-target-pool-v3.json \
  --output-dir docs/assets/part-07/chapter-05/sec-10/target-images \
  --wait-for-gpu
```
