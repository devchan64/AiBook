...
표에는 입력과 LoRA 적용 결과를 나란히 배치했다. 얼굴·헤어·화풍의 변환과 표정·자세·의상·배경 보존을 구분해 살핀다. 특히 45°·90°에서 머리의 앞뒤 길이가 입력 비율에 머무는지 확인한다. 생성 45장과 파일 무결성을 확인하고 항목별 AI 시각 검수 의견을 표에 기록했다. 사용자 검수와 재검토에서는 **얼굴·헤어는 대체로 양호하고 표정에도 큰 특이점이 없다**고 판단했다. 일부 웃음 강도 차이는 입력 표정 보존의 세부 관찰로 남기며 전체적인 표정 실패로 해석하지 않는다. 반면 009·011·019·026·028·029의 목선·칼라 변경과 010의 가슴 포켓 추가는 **의상 보존 오류**로 기록한다. 책의 상태·몸 비율·배경 화풍에 대한 관찰도 얼굴 일관성과 구분해 비교한다. 누적 3200스텝 결과는 같은 입력과 설정으로 검증하며, 별도의 합성 시트 대신 링크된 검수 MD의 원본 이미지와 관리번호별 의견을 기준으로 확인한다.

### 샘플에서 변환과 보존을 따로 확인한다

아래는 외부 입력 45장 중 검수 관점을 보여주는 네 사례다. 모두 같은 1600스텝 LoRA와 추론 설정을 사용했다. 입력 이미지만 참조했으며, 표의 결과는 미적용 모델과의 비교나 전체 성공률을 나타내지 않는다. 이미지를 확대해 얼굴·헤어와 의상의 목선을 각각 확인한다.

| 관리번호 · 확인할 내용 | 입력 | 1600스텝 LoRA 적용 |
| --- | --- | --- |
| **P712-CAM-008** · 정면 Mira 얼굴과 청록색 단발이 나타나고 컵을 쥔 동작이 이어진다. 몸 비율·미소 강도까지 완전히 같다는 뜻은 아니다. | ![008 입력: 컵을 쥔 정면 인물](../../../assets/part-07/chapter-05/sec-12/codex-camera-inputs-v1/man-level-zero-512.png) | ![008 결과: 정면 Mira 얼굴과 단발](../../../assets/part-07/chapter-05/sec-10/codex-camera-lora-366-v1/man-level-zero-lora.png) |
| **P712-CAM-023** · 다른 입력에서도 공통된 얼굴·헤어 인상을 확인할 수 있다. 모은 손과 도서관의 큰 배치는 이어지며 세부 비율은 별도로 본다. | ![023 입력: 도서관의 정면 인물](../../../assets/part-07/chapter-05/sec-12/codex-camera-inputs-v1/woman-level-zero-512.png) | ![023 결과: 다른 입력에서 나타난 Mira 인상](../../../assets/part-07/chapter-05/sec-10/codex-camera-lora-366-v1/woman-level-zero-lora.png) |
| **P712-CAM-009** · 얼굴·헤어는 변환됐지만 셔츠 칼라가 사라지고 목선이 둥글게 바뀌었다. 옷 색이 같아도 **의상 디자인 보존 오류**다. | ![009 입력: 칼라가 있는 셔츠](../../../assets/part-07/chapter-05/sec-12/codex-camera-inputs-v1/man-level-plus-45-512.png) | ![009 결과: 칼라가 사라진 목선](../../../assets/part-07/chapter-05/sec-10/codex-camera-lora-366-v1/man-level-plus-45-lora.png) |
| **P712-CAM-022** · 웃음·손 흔들기와 기존 V형 중앙 트임이 유지된다. 입력에도 같은 목선 구조가 있으므로 **목선 변경 사례로 분류하지 않는다**. | ![022 입력: 웃음과 기존 V형 중앙 트임](../../../assets/part-07/chapter-05/sec-12/codex-camera-inputs-v1/woman-level-minus-45-512.png) | ![022 결과: 표정·동작·기존 목선 유지](../../../assets/part-07/chapter-05/sec-10/codex-camera-lora-366-v1/woman-level-minus-45-lora.png) |

캐릭터 특징이 잘 나타나는 결과에서도 의상 보존 오류가 함께 생길 수 있다. 따라서 얼굴·헤어의 일관성과 입력 보존을 하나의 성공·실패로 묶지 않고, 같은 관리번호에서 각각 비교한다.
