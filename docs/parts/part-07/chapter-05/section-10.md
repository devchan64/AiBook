...
공개 모델 카드의 재현 조건은 LoRA 강도 1.0, 시드 62294, 추론 20스텝, CFG 4.0, 출력·참조 VAE 처리 크기 512×512다. 공개 가중치는 366쌍 중 학습 347쌍으로 학습한 **1600스텝** 산출물이며 검증 19쌍은 제외했다.

후속 비교에서 정한 **현재 실험 권장값은 누적 3200스텝 · 강도 0.75**다. 위 공개 다운로드에는 3200스텝 가중치가 포함되지 않는다. 권장값의 근거와 Mira 참조를 곁들인 비교시트는 [학습 후 평가](#lora-review)에서 내용보기로 확인한다.
...
### 실제 학습 입력과 목표를 나란히 본다

아래 세 쌍은 현재 데이터셋 JSON에서 `split: train`으로 지정된 학습 예시다. 왼쪽 입력을 편집 지시와 함께 모델에 주고, 오른쪽 Mira 목표에 가까운 출력을 만들도록 학습한다. 목표는 생성된 평가 결과가 아니라 학습에서 맞추려는 정답 이미지다.

| 관리번호 · 학습할 대응 | 입력 · `control_image` | Mira 목표 · `image` |
| --- | --- | --- |
| **P711-IN-002** · 기존 입력: 수채화 표현에서 Mira 목표 화풍으로 변환 | ![P711-IN-002 학습 입력](../../../assets/part-07/chapter-05/sec-10/training-images/input-images/bfs-input-02-watercolor.png) | ![P711-IN-002 Mira 학습 목표](../../../assets/part-07/chapter-05/sec-10/training-images/target-images/p7-5-2-qwen-2511-mira-torso-front-p7-5-4-direct-v1-size-1280x1280-seed-62294-steps-30.png) |
| **P710-PROP-001** · 보강 입력: 여성 사진풍 측면에서 Mira 측면으로 변환 | ![P710-PROP-001 학습 입력](../../../assets/part-07/chapter-05/sec-10/training-images/input-images/bfs-proportion-torso-level-minus-90-woman-long-photo.png) | ![P710-PROP-001 Mira 학습 목표](../../../assets/part-07/chapter-05/sec-10/training-images/target-images/p7-5-2-qwen-2511-mira-torso-multiview-vertical-level-yaw-minus-90-native1280-v1-size-1280x1280-seed-62294-steps-4.png) |
| **P710-PROP-009** · 보강 입력: 다른 얼굴 비율의 남성 사진풍에서 같은 Mira 측면으로 변환 | ![P710-PROP-009 학습 입력](../../../assets/part-07/chapter-05/sec-10/training-images/input-images/bfs-proportion-torso-level-minus-90-man-wide-photo.png) | ![P710-PROP-009 Mira 학습 목표](../../../assets/part-07/chapter-05/sec-10/training-images/target-images/p7-5-2-qwen-2511-mira-torso-multiview-vertical-level-yaw-minus-90-native1280-v1-size-1280x1280-seed-62294-steps-4.png) |

P710-PROP-001과 P710-PROP-009는 같은 5.2 측면 목표 파일을 공유한다. 입력 얼굴이 달라도 같은 Mira를 목표로 삼되, 방향·구도·자세·의상은 대응하도록 구성한 것이다. 목표 파일을 복제하지 않으며, 여러 입력이 하나의 목표를 공유해도 각 입력·목표 대응은 별도의 학습 쌍으로 센다.

학습 지시는 얼굴·헤어를 Mira로 바꾸고 전체 화풍을 맞추면서 표정·의상·장면을 보존하라는 내용이다. 각 쌍의 정확한 지시와 경로·해시는 위 데이터셋 JSON에서 관리번호로 확인한다. 이 학습 예시와 뒤의 외부 입력 45장 평가는 서로 다른 자료다.

## 확정 데이터셋으로 LoRA를 학습한다

현재 확정 데이터셋은 기존 193쌍과 보강 173쌍을 합한 366쌍(학습 347·검증 19)이다. 아래 명령은 **현재 366쌍 데이터셋**으로 새 학습을 준비한다. 배포된 1600스텝 LoRA는 이 중 학습 347쌍으로 학습을 마친 결과이며, 검증 19쌍은 학습에 사용하지 않았다. `purpose`가 `paired_edit_training`이면 각 행의 `control_image`를 입력으로, `image`를 목표로 고정한다. 같은 행에 지정한 입력·목표 대응을 유지한다. 입력·목표와 생성 기록의 해시, 중복 입력, 같은 목표가 학습과 검증에 함께 들어가는 오류를 검사한다.
...
| 검수 기준 | 이번 비교에서 확인한 내용 | 설정 선택에 반영한 판단 |
| --- | --- | --- |
| Mira 얼굴 재현 | 사용자 검수에서 1600스텝은 홍채와 이목구비 사이 거리에 차이가 보이고, 3200스텝은 해당 오류가 거의 보이지 않음 | 누적 3200스텝 선택 |
| 입력의 머리 방향 보존 | 사용자 검수에서 강도 1.0은 0.75보다 머리 방향 오류가 더 많이 보임 | 강도 0.75 선택 |
...
아래에서 필요한 비교시트의 **내용보기**를 누르면 원고 안에서 이미지와 검수 의견을 펼칠 수 있다. 각 사례에는 **입력과 시점·방향이 대응하는 Mira 토르소 참조**를 함께 배치했다. Mira 참조로 얼굴·헤어·화풍을 비교하고, 입력으로 머리 방향·표정·의상·장면의 보존 여부를 비교한다. 토르소 참조는 검수용이며 추론 입력에 추가한 이미지가 아니다.

[검수 요약 · 권장 설정과 먼저 볼 사례](../../../assets/part-07/chapter-05/sec-10/bfs-camera-366-review.md){ .aibook-markdown-preview }

[45개 입력 · 1600스텝과 3200스텝 비교시트 · 강도 1.0](../../../assets/part-07/chapter-05/sec-10/bfs-camera-366-step-review.md){ .aibook-markdown-preview }

[12개 입력 · 미적용과 강도 0.5·0.75·1.0 비교시트 · 두 체크포인트](../../../assets/part-07/chapter-05/sec-10/bfs-camera-366-scale-review.md){ .aibook-markdown-preview }

<details markdown="1">
<summary>검수 조건과 수량 펼치기</summary>

학습에 사용하지 않은 외부 입력 45장은 남성·여성·유아 각각에 높이 3종과 수평 방향 5종을 조합했다. 학습 데이터에서 따로 둔 검증 19쌍과 구분한다. 같은 외부 입력에 1600스텝과 누적 3200스텝 LoRA를 강도 1.0으로 적용해 출력 90장을 비교했다.

강도 비교에서는 이 중 보존 문제 등을 확인할 12개 입력을 골랐다. 미적용 12장과 두 체크포인트의 강도 0.5·0.75 출력 48장을 새로 만들고, 기존 강도 1.0 출력 24장을 재사용해 출력 총 84장을 비교했다. 시드 62294, 추론 20스텝, CFG 4.0, 출력·참조 VAE 처리 크기 512×512를 유지하고 크롭하지 않았다.

12개 입력은 문제를 살펴보기 위해 선택한 부분집합이다. 한 시드의 정성 비교이므로 전체 성능이나 모든 입력에 대한 최적 강도를 확정하지 않는다. 상세 의견·원본 이미지·실험 조건과 해시는 각 비교시트에 남긴다.

</details>

<details markdown="1">
<summary>기존 1600스텝 사례에서 변환과 보존을 구분하는 방법 펼치기</summary>

강도 1.0의 008·023에서는 다른 입력에서도 공통된 Mira 얼굴·헤어 인상을 확인할 수 있다. 그러나 009의 셔츠 칼라가 사라지거나 010에 가슴 포켓이 추가되면, 얼굴이 변환됐더라도 의상 보존 오류다. 반대로 022의 V형 중앙 트임은 입력에도 있으므로 목선 변경 오류로 분류하지 않는다. 위 45개 입력 비교시트에서 해당 관리번호의 입력·참조·출력을 대조한다.

이전의 “얼굴·헤어가 대체로 양호하다”는 판단과 후속 검수의 홍채·이목구비 간 거리 관찰은 검토의 세밀함이 다르다. 현재 설정 선택에는 후속 사용자 검수를 반영하고, 기존 AI의 의상·손·몸 비율 관찰도 항목별 기록으로 유지한다.

</details>

## 평가 이후의 데이터 보강과 추가 학습

출력 검수에서 확인한 개선 과제에 따라 데이터를 보강하거나 저장 상태에서 학습을 이어 갈 수 있다. 이번 비교의 누적 3200스텝 결과는 1600스텝 저장 상태에서 추가 1600스텝을 학습한 것이다. 아래에서는 데이터 보강과 학습 재개 방법을 설명한다.
...
고정한 Musubi 버전은 상태를 복원해도 학습 루프의 스텝 번호를 0부터 센다. 위 예의 새 `1600`은 누적 종료 번호가 아니라 추가 실행량으로, 완료 시 누적 3200스텝을 별도 기록한다. 새 패키지의 `step00000400`은 이 예에서 누적 2000스텝에 해당한다. 데이터 순회 위치까지 완전히 이어지는 것은 아니므로, 중단 없이 3200스텝 실행한 결과와 수치적으로 같다고 단정하지 않는다. 재개 패키지에서는 이전 1600스텝의 저장 상태를 읽어 추가 1600스텝, 누적 3200스텝 실행을 완료했다. 누적 학습량의 증가가 품질 개선을 보장하지는 않으며 같은 외부 입력으로 비교해야 한다.
