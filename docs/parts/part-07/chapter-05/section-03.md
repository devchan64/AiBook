...
P7-5.1에서는 검색 후보와 선택 근거를 남기는 구조를 만들었고, P7-5.2에서는 `근거 기반 답변`, `근거 부족`, `답변 과장 위험`을 구분해 기록했습니다. 이제 한 걸음 더 가서, `같은 문서 집합에서도 질문 문구를 어떻게 쓰느냐에 따라 왜 답변 상태가 달라지는가`를 직접 연습할 차례입니다.

이번 절은 RAG 이론을 더 늘리는 자리가 아니라, 질문 문구와 멈춤 규칙을 손으로 다뤄 보는 연습 절입니다. 핵심은 검색기가 똑같더라도 질문이 문서 범위를 직접 묻는지, 과장된 일반화를 요구하는지, 문서에 없는 개념을 끌고 오는지에 따라 결과가 얼마나 달라지는지 눈으로 확인하는 데 있습니다.
...
이 절에서는 질문 재작성(rewrite)과 멈춤 기준(stop rule) 연습에만 집중합니다. 즉, 여기서는 검색기 자체를 바꾸지 않고, `질문을 덜 과장되게 쓰면 어떤 상태 변화가 생기는가`를 닫습니다. 더 넓은 운영 실패 기록 구조는 P7-7.2에서 다시 이어집니다.
...
이번 절은 P7-5.1, P7-5.2와 같은 문서 집합을 그대로 사용합니다. 대신 질문 파일을 별도로 두어, 같은 지식베이스 위에서 질문 문구와 기대 상태만 바꾸며 결과를 비교합니다.
