...
별도 text-to-pose 확산 후보인 `PoseDiffusion/GUNet`도 검토했다. 이 모델은 텍스트에서 17채널 관절 heatmap을 만들고 OpenPose 골격으로 변환하므로, image-to-pose가 아니라 현재 과제와 맞는다. 공식 가중치는 Apache-2.0으로 표시되고 기본 추론 계약도 `256 x 256`, 27 step이다. 그러나 공식 코드가 요구한 PyTorch 2.4.1과 다른 현 환경에서는 내부 관절 인접행렬의 자료형 및 CFG 배치 크기 2를 별도로 고쳐야 했고, 순차 CPU offload·CFG 단일 배치까지 적용해도 중간 블록에서 8 GB VRAM이 부족했다. 즉 **이 실행 환경에서 8 GB 단일 GPU 계약을 만족하지 못했으므로 관절 품질을 판단할 PNG는 만들지 못했고 후보에서 제외한다.** 임시 소스·가중치·부분 산출물은 삭제했다. `HumanSD`는 사람 골격에 특화돼 있지만, 공식 저장소가 추론 코드·사전학습 가중치 공개를 아직 TODO로 표시하고 가중치의 별도 사용권도 명시하지 않아 공개 가중치 후보로 채택하지 않는다. [PoseDiffusion 공식 코드](https://github.com/MCULab-sohu/PoseDiffusion){: target="_blank" rel="noopener noreferrer"}, [GUNet 모델 카드](https://huggingface.co/sw1125/gunet){: target="_blank" rel="noopener noreferrer"}, [HumanSD 공식 코드](https://github.com/IDEA-Research/HumanSD){: target="_blank" rel="noopener noreferrer"} (확인: 2026-08-07)

생성 모델이 아니라 자연어를 관절 DSL로 컴파일해 가동범위·접지·IK를 검사하는 `Posecode`도 확인했다. 이미지 입력을 쓰지 않고 오른발 `ground-lock`, 왼쪽 hip flex, 양 어깨 벌림을 명시한다. 이 절에서는 가동범위 제한을 배제 사유가 아니라 **입력 일관성 제약**으로 수용한다. `hip_left: flex 135`, `knee_left: extend 0`, `shoulders: abduct 90`, `ground-lock: foot_right`를 가진 같은 DSL을 두 번 변환했을 때 22관절 JSON의 SHA-256이 같았고, 오른발 접지 높이도 `y=0.01 m`로 유지됐다. 이 좌표에서 만든 OpenPose·관절 depth와 표면 silhouette Canny·surface depth는 든 다리가 몸통보다 앞이라는 순서를 분리한다. 이 네 지도는 사람 검수용 후보로 보존한다. 다만 이 정확한 Posecode surface depth와 전신·얼굴 기준 두 장을 Animagine XL IP-Adapter에 함께 준 `512 x 768` 검증도 올린 다리와 양팔을 해부학적으로 복원하지 못했다. Canny+depth는 사람 대신 회색 고체 실루엣을 만들었고, depth 단독은 얼굴은 만들었지만 든 다리와 팔을 원통형으로 붕괴시켰다. 두 PNG·JSON은 폐기했다. 3D 관절에서 검정 타이즈·얼굴 texture를 갖는 절차적 리그 프록시를 직접 렌더한 별도 실험은 든 다리의 앞쪽 z-order와 지지발 접점은 고정했지만, 몸·팔·의상 표면을 캐릭터 수준으로 만들지 못해 역시 폐기했다. 따라서 **관절의 z-order를 고정하는 것과 그 관절을 캐릭터로 작화하는 것은 별도 문제이며, 현재 2D 기준 이미지와 관절만으로는 캐릭터 3D 표면이 생기지 않는다.** 생성 컷의 인체 수용도·캐릭터 일관성 통과 근거도 아직 아니다. 다음 검증은 같은 조합의 강도 반복이 아니라, 리깅·skin·헤어·의상 mesh를 갖춘 사용 허가된 캐릭터 3D 자산을 먼저 확보해 관절에 적용하거나, 통과한 인체 초안의 교차 부위만 mask/inpaint로 보정하는 다른 입력 계약에서만 한다. MagicAnimate는 기준 image appearance와 DensePose 인체 표면을 분리해 받는 BSD-3-Clause 가중치 후보라 입력 surface map 16프레임을 근사 생성해 보았다. 그러나 현재 3D 관절은 DensePose UV surface가 아니며, 공식 코드는 Torch 2.0·Diffusers 0.21·Xformers 0.0.22 계약에서 16-frame latent와 appearance encoder·ControlNet을 GPU에 함께 올린다. 현 Python 3.12·Torch 2.11·Diffusers 0.37에서는 제거된 `diffusers.pipeline_utils` import에서 중단했다. 그러므로 가중치를 내려받아 억지로 실행하지 않고, 현 8 GB 환경의 실험 후보에서 제외한다. `HY-Motion 1.0`은 최소 VRAM이 24–26 GB이고, `UniPose`는 LLaVA Mistral 7B를 함께 요구한다. 따라서 두 생성형 motion 후보는 8 GB 기본 조건에서 제외한다. [Posecode 규격](https://github.com/posecode-dev/posecode/blob/main/spec/SPEC.md){: target="_blank" rel="noopener noreferrer"}, [MagicAnimate 공식 코드](https://github.com/magic-research/magic-animate){: target="_blank" rel="noopener noreferrer"}, [MagicAnimate 모델 카드](https://huggingface.co/zcxu-eric/MagicAnimate){: target="_blank" rel="noopener noreferrer"}, [HY-Motion 모델 카드](https://huggingface.co/tencent/HY-Motion-1.0){: target="_blank" rel="noopener noreferrer"}, [UniPose 공식 코드](https://github.com/VIPL-VISMOD/UniPose){: target="_blank" rel="noopener noreferrer"} (확인: 2026-08-07)
...
우리의 `512 x 768`, 가는 관절선, 수직 다리의 몸통 교차, OpenPose와 depth의 동시 조건은 바로 이 미보장 영역이다. 더 직접적으로, NeurIPS 2024의 Stable-Pose는 기존 pose-guided T2I가 복잡한 자세(특히 측면·후면 인물)에서 성능이 제한된다고 보고한다. [Stable-Pose](https://proceedings.neurips.cc/paper_files/paper/2024/hash/78b9d95f6bb13b080c2a68bdea54cdbb-Abstract-Conference.html){: target="_blank" rel="noopener noreferrer"} 따라서 현재의 판정은 **SD 계열 OpenPose가 단순·비가림 포즈에서는 더 높은 인체 수용도를 낼 가능성은 있으나, 아직 이 절에서 실험·승인한 경로는 아니라는 것**이다. 현재 8 GB·단일 생성·캐릭터 기준·극단 교차 자세에는 충분하지 않다. 단순 포즈는 별도 통제 실험에서 얼굴·양팔·두 다리·접지를 모두 확인한 뒤에만 채택 여부를 판단한다. 후속 경로는 사람 검수 3D 인체 표면/마스크 또는 그에 맞게 학습된 pose 특화 모델을 별도 검증해야 한다.

모델 카드의 입력 계약도 한 번 바꿔 검증했다. text pose ticket을 표준 OpenPose 18관절 순서와 굵은 선으로 다시 그려 `768 x 1152`, 30 step 및 `768 x 1024`, 24 step으로 실행했다. 이는 기존 512 해상도·가는 비표준 선과 다른 가설이다. 두 경우 모두 산출물은 남았지만, 전자는 추가 다리를 만들었고 후자는 얼굴을 머리카락·든 다리에 완전히 가렸다. 고해상도 실행 뒤 GPU 드라이버가 끊기는 현상도 반복됐다. 따라서 **굵은 표준 OpenPose는 부분 수용을 높일 수 있어도 현 컷의 얼굴·사지·접지 동시 통과 해법은 아니다.** PNG·JSON과 임시 입력은 삭제한다.
