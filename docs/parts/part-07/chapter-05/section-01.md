...
이번 프로젝트는 `책 문서 일부를 작은 지식베이스로 두었을 때, 질문에 맞는 문서를 먼저 찾고 그 문장만을 근거로 답할 수 있는가?`라는 질문에서 시작합니다. `검색`과 `답변`을 분리해 기록할 수 있고, 나중에 검색 실패와 환각(hallucination)을 같은 프로젝트 안에서 다룰 수 있으며, Part 5의 RAG 개념을 실제 프로젝트 문서 형식으로 옮기기에도 적합합니다.
...
- 입력: 질문 1개, 문서 조각 3개
- 기대 출력: 가장 관련 높은 문서 1개와 근거 기반 답변
- 확인할 개념:
  - 검색이 먼저다
  - 답변은 검색 결과 위에 작성된다
...
검색_기록.sort(key=lambda x: x["겹친 토큰 수"], reverse=True)
선택_근거 = 검색_기록[0]

근거_답변_기록 = {
    "질문": 질문,
    "선택한 문서 식별자": 선택_근거["문서 식별자"],
    "겹친 토큰 수": 선택_근거["겹친 토큰 수"],
    "선택한 근거 문장": 선택_근거["문장"],
    "답변": (
        f"{선택_근거['문장']} 따라서 질문에 답할 때 "
...
질문 = RAG가 왜 필요한가?
검색 기록 =
{'문서 식별자': '문서-1', '겹친 토큰 수': 1, '문장': 'RAG는 외부 문서를 검색해 모델 입력에 넣는 구조다.'}
{'문서 식별자': '문서-2', '겹친 토큰 수': 0, '문장': '임베딩은 텍스트를 벡터로 바꾸어 유사도 비교를 가능하게 한다.'}
{'문서 식별자': '문서-3', '겹친 토큰 수': 0, '문장': '프롬프트만으로는 최신 문서 근거를 보장할 수 없다.'}
선택 근거 = {'문서 식별자': '문서-1', '겹친 토큰 수': 1, '문장': 'RAG는 외부 문서를 검색해 모델 입력에 넣는 구조다.'}
근거 답변 기록 = {'질문': 'RAG가 왜 필요한가?', '선택한 문서 식별자': '문서-1', '겹친 토큰 수': 1, '선택한 근거 문장': 'RAG는 외부 문서를 검색해 모델 입력에 넣는 구조다.', '답변': 'RAG는 외부 문서를 검색해 모델 입력에 넣는 구조다. 따라서 질문에 답할 때 외부 근거를 함께 붙이기 위해 RAG가 필요하다.'}
```
...
- 문서 식별자를 같이 남기면 나중에 답변 근거를 다시 확인할 수 있다.
- 작은 예제에서 가장 관련 높은 문서가 하나 골라졌더라도, 그것만으로 검색 품질이 충분하다고 단정하지는 않는다.
...
- 문서 식별자를 남기면 나중에 근거를 다시 검토할 수 있다
...
하지만 이 모든 계층을 한 번에 다 넣기보다, `검색 결과를 답변 앞에 둔다`는 기준을 먼저 고정하는 편이 낫습니다.
...
- 답변 문장은 쓰고 있지만 어떤 질문에 어떤 문서가 선택되었는지 남기지 않은 경우
...
- 프로젝트 문서에는 답변뿐 아니라 검색된 근거를 함께 남겨야 한다는 점을 설명할 수 있는가?
- 작은 예제라도 출처 문서 식별자를 남기는 습관이 중요하다는 점을 말할 수 있는가?
