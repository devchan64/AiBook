...
이 프로젝트는 먼저 FLUX.2 Klein 4B로 배경 화풍 기준을 고정하고, 그 기준을 통과한 뒤에 캐릭터 참조 패키지를 만듭니다. 여기서 화풍 참조 셋은 보기 좋은 배경을 모은 폴더가 아닙니다. 인물과 소품을 넣기 전, 선의 역할, 색의 겹침, 시간대의 광원, 장소의 폭, 카메라 구도를 **같은 기준으로 비교할 수 있게 만든 검수 입력**입니다. 한 장이 마음에 들어도 다른 장소와 카메라에서 계약이 무너지면, 그 한 장은 뒤에 만들 캐릭터 참조 패키지의 화풍 기준이 될 수 없습니다.
...
웹툰 컷 생성 전체를 한 번에 해결하려고 하지 않습니다. 이 실험은 8 GB VRAM 환경에서 `배경 화풍 참조 셋 -> 캐릭터 참조 셋 -> 컷씬 -> 화풍 보정`을 네 단계로 나눕니다. 즉 배경의 선·색·광원·카메라 계약을 먼저 만들고 검수한 뒤, 그 텍스트 계약을 바탕으로 캐릭터 기준을 따로 생성·검수합니다. 한 단계의 출력이 다음 단계의 입력이 되려면, 생성된 이미지가 마음에 드는지를 넘어서 어떤 조건을 통과했는지 기록되어야 합니다.

| 단계 | 만드는 것 | 다음 단계로 넘기는 조건 |
| --- | --- | --- |
| P7-5.1 배경 화풍 참조 셋 | 프레임 없는 배경 화풍 원본과 ledger | FLUX.2 Klein 4B가 따라야 할 선·수채화·광원·카메라 기준 |
| P7-5.2 캐릭터 참조 셋 | P7-5.1의 배경 화풍 계약 뒤에 만드는 인물 기준 패키지 | 얼굴·전신·의상·소품·view가 분리 검수된 캐릭터 입력 |
...
이 흐름에서 모델은 배경 후보를 먼저 만들고, 사람은 후보가 계약을 지키는지 판단합니다. `행 승인`은 한 조건에서의 통과이고, `전체 팩 승인`은 모든 필수 행과 보조 근거를 함께 비교한 뒤의 결론입니다. 이 배경 gate를 통과한 뒤에만 캐릭터 후보를 별도 단계에서 만듭니다. 승인된 뒤에도 타일로 합친 비교 이미지는 다음 단계의 모델 입력으로 쓰지 않습니다. 현재 P7-5.1의 인물 화풍 후보 생성은 승인 배경 원본 한 장을 image input으로 넣고, 같은 화풍 텍스트 계약도 prompt로 함께 넣습니다.
...
P7-5.1이 다음 단계에 넘기는 것은 FLUX.2 Klein 4B가 따라야 할 얇은 charcoal 선, 반투명 색층, 프레임 없는 캔버스, 시간대별 배경 광원이라는 **배경 화풍 계약**입니다. 이 계약은 캐릭터보다 앞에서 먼저 확정하는 공통 바탕이지만, 얼굴의 특징·신체 비율·의상 구조까지 승인하지는 않습니다. P7-5.2는 그 뒤에 같은 모델을 사용해 머리카락·피부·눈·의상 색과 인물 표현 규칙을 별도 character contract로 정하고, full body와 얼굴의 일관성을 검수합니다. 배경의 밤·노을·비는 컷신에서 인물에 약한 반사광을 더할 수는 있어도, 승인된 기본색이나 인물의 표현 규칙을 새로 정의하는 근거가 될 수 없습니다.
...
## 배경 화풍 계약 뒤에 인물 후보를 만든다

배경 원본을 먼저 생성·검수해 화풍 계약을 확정한 뒤, 인물 후보를 만듭니다. 이때 인물 후보 생성기는 **승인된 배경 PNG 한 장**을 image input으로 넣고, 배경과 같은 화풍 텍스트 계약도 함께 전달합니다. 프레임 없는 장면, sparse thin charcoal 선, wet-on-wet 번짐, 불규칙한 안료 고임, 반투명 색층, 작은 반사광, natural medium-chroma 안료 같은 조건을 prompt에 넣습니다. 기본값은 거리·카페·옥상·공원·아트리움 각각에 대응하는 승인 배경 원본이며, `--style-reference`로 다른 승인 원본 하나를 명시해 참조 변화만 비교할 수 있습니다. 타일 비교 이미지는 입력으로 쓰지 않습니다.
...
| --- | --- | --- |
| 모델 입력 | P7-5.1 텍스트 화풍 계약·하나의 장면 prompt·승인 배경 PNG 한 장을 함께 넣음 | 타일 비교 이미지를 넣거나, 이 입력만으로 identity를 고정하지 않음 |
...
`--cast`는 성별·나이 정보만 고정한 짧은 생성 프롬프트를 고릅니다. `--compositions`는 거리·카페·옥상·공원·아트리움 장면을 고르는 생성 범위 목록이며, 각 장면은 대응하는 승인 배경 PNG 한 장을 기본 참조로 사용합니다. `--style-reference`에 승인 배경 파일명을 넣으면 여러 장면에 같은 참조를 적용해 비교할 수 있습니다. 후보 PNG에는 `castID-장면ID-실행시각-seed`가 들어갑니다. JSON에는 선택한 성별·나이와 장면 구성, 실제 참조 파일, P7-5.1 텍스트 계약의 선·수채화 색층·안료 질감·프레임 없는 단일 장면이 유지되는지 확인할 검수 항목을 남깁니다.
...
거리·옥상·카페·공원·아트리움에서 화풍 계약을 통과한 기존 다섯 장면은 아래와 같습니다. 모두 프레임 없는 단일 장면, 얇은 charcoal 선, 반투명 수채화 색층과 안료 번짐을 기준으로 승인했습니다. 이들은 배경 PNG를 image input으로 쓰기 전의 텍스트 계약 후보이므로, 새 이미지 참조 경로의 통과 근거로 재사용하지 않습니다. 새 경로의 출력은 참조 원본이 장면을 복제하지 않으면서 선·색·광원 처리에 반영되는지 별도로 사람 검수해야 합니다. 이 표는 인물의 얼굴·나이·성별·동일성을 보증하지 않습니다. 승인 목록과 파일명은 [인물 화풍 기준 manifest](../../../assets/part-07/chapter-05/p7-5-1-approved-cast-style-reference-pack.json)에 남깁니다.
...
| 승인 배경 PNG와 텍스트 화풍 계약을 인물 후보에 함께 전달 | 배경 원본의 선·안료·광원 처리를 실제 참조 입력으로 쓰되, 장면 복제를 별도 검수하려 함 | 장면별 승인 원본 또는 `--style-reference`로 고른 한 원본을 image input으로 기록하는 새 후보 경로를 마련함 | 새 출력이 기존 텍스트 전용 후보의 승인이나 얼굴 identity, 나이, 성별, 시점 일관성을 대신한다는 뜻은 아님 |
...
| 최종 승인 기록 | 아홉 원본의 사람 승인을 ledger와 manifest에 남기고, cast 후보 생성에는 승인 배경 PNG 한 장과 텍스트 화풍 계약을 함께 사용했는가? |
