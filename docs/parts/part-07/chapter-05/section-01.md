...
이 절의 질문은 **공통 T2I 화풍 프롬프트를 유지한 채 장면 변수를 바꾸어, 비교 가능한 화풍 데이터를 어떻게 축적하는가**입니다. 이 절의 산출물은 화풍 계약, 장면별 T2I 원본, 행별 관찰 기록입니다.
...
| --- | --- | --- |
| 화풍 계약 | 선·수채화 색층·프레임 금지 조건 | 관찰할 공통 특징 |
| 장면 행렬 | 장소·시간·카메라 조합 | 스무 장면 행 목록 |
| 행별 생성 | seed와 장면의 구체 구성 | crop하지 않은 후보 원본 |
| 사람 관찰 | 외곽·선·색·장소·시간·카메라 기록 | 일치·이탈과 다음 비교 질문 |
| 원본 표 | 행 사이의 일관성과 관찰 범위 | 원본 파일명·장면 식별자와 관찰 범위 |
...
각 행에는 사람·동물·차량·읽을 수 있는 표지·글자를 넣지 않습니다. 화풍 팩은 배경의 매체 처리와 공간 조건만 관찰하는 자산입니다. 프롬프트에는 `no border frame`, `no panel`, `fill the canvas edge to edge`를 함께 쓰되, 이 단어가 있다고 결과 특성을 단정하지 않습니다. 출력 원본에서 프레임이 보이면 그 이미지는 crop으로 살리지 않고, 프레임 이탈을 포함한 다음 비교 조건을 기록합니다.

## AI 모델은 텍스트 조건과 seed에서 후보 원본을 만든다

후보 한 장은 장면 prompt와 공통 화풍 계약, seed, 해상도, 반복 횟수를 함께 받아 만들어집니다. Qwen Image 구성 요소를 `torch_dtype=torch.bfloat16`으로 읽고 Nunchaku transformer offload 및 `enable_sequential_cpu_offload()`를 켜는 일은 모델 내부 추론 단계가 아니라 실행 준비에 가깝습니다. 준비가 끝난 뒤 `QwenImagePipeline`은 장면 prompt와 공통 화풍 계약을 조건으로 바꾸고, seed에서 시작한 이미지 표현을 정해진 횟수만큼 갱신합니다. 이 단계의 목적은 다른 모델에 일반화되는 배경 화풍을 찾는 것이 아니라 **Qwen Image가 이 계약을 어느 정도 따르는지** 관찰하는 것입니다. 기본 `STEPS=30`과 `true_cfg_scale=4.0`은 이번 후보 생성의 실행 설정이며, 이 숫자 자체가 화풍 특성의 판정값은 아닙니다.
...
| 모델 파이프라인 단계 | 이 절에서 맡는 역할 | 관찰 기록과의 관계 |
| --- | --- | --- |
...
| VAE decode와 PNG 저장 | latent를 이미지로 바꾸고 원본 파일로 남김 | 파일 생성은 관찰할 원본을 남김 |
| 사람 관찰과 기록 | 외곽·선·색·장소·시간·카메라를 기록함 | 입력 역할과 알려진 이탈을 남김 |
...
로컬에서 이미지가 생성된다고 해서 웹툰 컷 파이프라인이 안정적이라는 뜻은 아닙니다. 이 실행은 Nunchaku transformer offload와 `enable_sequential_cpu_offload()`로 GPU 상주량을 줄이고 한 행씩 생성합니다. 따라서 P7-5.1은 모델이 좋은 배경 이미지를 만들 수 있는지 보는 절이 아니라, **다음 단계에서 각 원본을 어떤 화풍 참조로 읽을지 사람이 기록하는 절**입니다.

| 구분 | 이 실험에서 유리한 점 | 조심할 점 |
| --- | --- | --- |
| 실행 구성 | Qwen Image와 양자화 transformer로 로컬 후보를 반복 생성할 수 있음 | offload와 양자화가 화풍·구도 특성을 보장하지 않음 |
| 기능 범위 | text-to-image와 이미지 편집 흐름을 시험할 수 있음 | 기능 지원이 곧 화풍 계약의 반복 관찰을 뜻하지 않음 |
| 공개 가중치 | Apache 2.0 공개 가중치라 실험 조건과 산출물을 기록하기 좋음 | 모델 출력은 prompt를 놓치거나 왜곡할 수 있어 사람 검수 ledger가 필요함 |
| 빠른 후보 생성 | 여러 장면 후보를 반복해 만들 수 있음 | 빠른 생성은 화풍 특성의 근거가 아니며, 이탈은 다음 prompt 구조 비교로 옮김 |

## 실행 코드는 공통 계약과 장면 변수를 분리한다

배경 후보 생성에는 Diffusers의 `QwenImagePipeline`을 사용합니다. 스무 장면 후보를 만드는 기준 실행 코드는 `p7_5_1_regenerate_local_gpu_style_references.py`입니다. 학습 관점에서 이 코드는 세 가지를 구분하게 해 줍니다. 첫째, 모든 행에 같은 화풍 계약을 붙입니다. 둘째, 행마다 장소·시간·카메라·seed만 바꿉니다. 셋째, 생성 기록과 사람의 관찰 기록을 별도로 남깁니다.
...
## 하나의 스크립트로 1~20번 후보와 관찰 기록을 만든다

`p7_5_1_regenerate_local_gpu_style_references.py`는 `SCENES`에 정의한 스무 행을 한 실행 목록으로 읽습니다. 모든 행은 같은 모델·화풍 계약·해상도·30스텝·`true_cfg_scale`을 공유하고, 장소·시간·카메라·seed만 행마다 바뀝니다. 기본 실행은 이 스무 행을 차례로 생성하고, 각 PNG 옆에 같은 stem의 `-result.json`을 저장합니다. result JSON에는 장면 식별자, prompt, seed, 해상도, step, `true_cfg_scale`, 모델·transformer, 출력 PNG와 생성 시간을 남깁니다. 터미널에는 실행 전체의 GPU 메모리 요약과 결과 목록을 출력합니다. 따라서 1~20번은 서로 다른 생성 방법의 결과가 아니라, 하나의 스크립트에서 조건 행만 바꿔 만든 비교 집합입니다.
...
사람은 스무 행의 장소·시간·카메라 변화에서 화풍 계약의 일치와 이탈을 함께 기록합니다. 각 행의 세부 조건과 이미지 자체는 아래의 스무 원본 표에서 한 번만 확인합니다. 재현 비용을 늘린 40·50스텝 후보는 보관하지 않습니다.
...
반대로 장가계는 외곽 프레임은 사라졌지만 절벽의 반복 선이 해칭처럼 남았고, 우천 야간 플랫폼은 비·레일·지붕 선이 화면을 지배했습니다. 이 경우에는 crop이나 부분 보정으로 이탈을 감추지 않습니다. `무엇이 달랐는가`를 다음 생성의 구도·피사체 밀도·광원 조건으로 번역하고, 새 원본과 관찰 기록을 남깁니다.

## 사람 관찰 기록과 원본 표를 분리한다

사람이 기록하는 것은 이미지의 미적 품질 점수 하나가 아닙니다. 각 원본에서 외곽·선·색·장소·시간·카메라와 **로컬 GPU 생성 기록**을 확인하고, 행별 일치·이탈과 다음 비교 질문을 로컬 관찰 기록에 적습니다. 다음 단계에서 사용할 원본은 아래 표의 파일명과 장면 식별자를 보고 작업 목적에 맞게 고릅니다. 별도 목록 파일이 다음 입력을 고정하지 않으므로, `무엇을 관찰했는가`와 `무엇을 다음 생성에 쓸 것인가`를 혼동하지 않습니다.

1~20번 행은 Qwen Image 30스텝으로 만든 로컬 GPU 원본입니다. 이 원본들은 배경 화풍 계약을 비교하기 위한 관찰 대상이며, 내장 이미지 생성 원본은 섞지 않습니다.

## 1~20번 로컬 GPU 참조 원본을 한 번에 확인한다

아래 표는 위 스크립트의 `SCENES` 순서대로 1번부터 20번까지 정리한 로컬 GPU 원본입니다. 모든 이미지는 Qwen Image 30스텝 실행 결과이며, 이미지 파일명과 `scene_id`가 각 원본의 생성 조건을 연결합니다.
...
아래 기록은 한 번의 출력이 보기 좋았는지가 아니라, 같은 로컬 GPU 조건에서 다음 단계가 비교 가능한 입력을 받는지를 기준으로 정리했습니다. 표의 결과는 이 참조 팩에서 관찰한 범위이며, 다른 모델·해상도·prompt에 그대로 일반화하지 않습니다.
...
| Qwen Image 30스텝 기본값 | 1024×1024 아트리움 비교에서 20스텝부터 기본 형태가 읽혔지만 이후 후보의 일관된 운용점을 남기려고 함 | 1번 아트리움, 2번 courtyard, 3번 도심의 30스텝 원본과 prompt 단어 수·시간·메모리 기록을 보관함 | 30스텝이 모든 장면의 필요량을 뜻하는 것은 아님 |
| 공통 화풍 계약과 장면 행의 분리 | 선·수채화·프레임 조건을 바꾸지 않은 채 장소·시간·camera 차이만 비교하려고 함 | 같은 계약 아래 실내·실외, 새벽·낮·석양·밤·우천 야간, 다섯 camera family를 참조 팩에서 대조함 | 공통 prompt 하나가 모든 장면의 공간 구조를 자동으로 고정한다는 뜻은 아님 |
| 중앙 도로 대신 측면 교차로로 도심 조건 변경 | 넓은 도로 요구가 중앙 소실점의 거리 복도로 수렴한 이탈을 장면 구조 문제로 해석함 | 측면 모퉁이에서 비스듬히 보는 도심 원본으로 교체해 낮·wide 행을 비교함 | 금지어를 더 많이 쓰면 모든 원근 오류를 고칠 수 있다는 뜻은 아님 |
| 여객기 실내를 창가 독서실로 대체 | 좌석 모듈·천장·사선 구도를 동시에 안정적으로 관찰하기 어려웠음 | 밤·실내·oblique 조건은 단순한 독서실과 작은 스탠드 조명으로 비교함 | 복잡한 실내 장면이 모델에서 불가능하다는 일반 결론은 아님 |
| 사람 관찰 기록과 원본 표의 분리 | 미적 판단을 자동화하지 않으면서 관찰 근거와 원본 식별 정보를 분리하려 함 | 사람 관찰은 로컬 기록에 남기고, 스무 원본의 파일명·장면 식별자는 본문의 원본 표에서 확인함 | 원본 표가 이미지의 선·색·공간 품질을 자동 채점하거나 사람 관찰을 대신한다는 뜻은 아님 |
...
| 원본 표 기록 | 스무 원본의 자산명·장면 식별자·관찰 범위를 원본 표와 로컬 기록에서 구분했는가? |
