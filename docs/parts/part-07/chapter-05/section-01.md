...
모델 단계의 출력은 `images[0]`의 PNG와 `run.json` 실행 기록까지입니다. `approved`라는 결론, 필수 행 충족, 다음 단계 입력 가능 여부는 모델이 계산하지 않습니다. 그 판단은 생성 뒤 사람 검수와 ledger가 맡습니다. FLUX.2 Klein 4B는 텍스트 기반 이미지 생성과 참조 편집을 지원하는 rectified-flow transformer이며, prompt를 완전히 따르지 못하거나 텍스트를 왜곡할 수 있다는 한계도 모델 카드에 명시되어 있습니다.
...
## 생성 스크립트는 공통 계약과 장면 변수를 분리한다

실제 후보 생성에는 Diffusers의 `Flux2KleinPipeline`을 사용했습니다. 아래 첫 번째 스크립트는 여섯 장면을 같은 실행 조건으로 생성한 배치입니다. `COMMON_CONTRACT`에는 모든 장면에 공통인 full-bleed, 선, 수채화, 금지 조건을 두고, `SCENES`에는 장면마다 달라져야 하는 장소·시간·카메라·seed를 둡니다. 이 분리가 없으면 장면의 차이 때문에 화풍이 달라졌는지, 화풍 계약 자체가 무너졌는지 구별하기 어렵습니다.
...
| `run.json` | 시간·GPU 메모리·출력 파일을 저장 | 생성 성공과 사람 승인을 구분하는 실행 기록 |

이 스크립트에서 바꿔 볼 값은 한 `SCENES` 행의 `prompt` 또는 `seed`입니다. 바꾼 뒤에는 새 PNG가 예뻐 보이는지만 보지 말고, 같은 행의 프레임·선·색·시점 조건이 달라지는지를 ledger에서 다시 판정합니다. `guidance_scale`이나 step을 바꾸는 것은 별도의 비교 실험으로 기록해야 합니다. 아래 코드는 당시 후보 배치의 전체 소스이며, 실행하려면 모델 가중치, CUDA가 가능한 로컬 환경, 충분한 실행 시간이 필요합니다.

<details id="local-style-time-balanced-batch" class="aibook-lazy-source" data-source="/AiBook/assets/part-07/chapter-05/p7_5_1_flux2_time_balanced_style_pack_regeneration.py" data-language="python">
<summary>시간대 균형 화풍 후보 배치 생성 코드 보기</summary>
<div class="aibook-lazy-source__body">펼치면 Python 원문을 불러옵니다.</div>
</details>

후속 재생성은 실패한 장면의 구조를 바꾸는 데 사용했습니다. 예를 들어 도심은 중앙 도로 대신 측면 교차로로, 여객기 실내는 전체 객실 대신 창가의 짧은 좌석 열로 바꿨습니다. 두 번째 스크립트는 이처럼 **무엇을 금지할지**만 늘리지 않고, 장면 구도와 시간대 광원을 바꾼 재생성 기록입니다. 이 코드의 `SCENES`를 첫 배치와 비교하면 공통 계약은 유지하고 scene prompt만 교체한 방식을 확인할 수 있습니다.

<details id="local-style-targeted-chroma-regeneration" class="aibook-lazy-source" data-source="/AiBook/assets/part-07/chapter-05/p7_5_1_flux2_targeted_chroma_regeneration.py" data-language="python">
<summary>결함 원인을 반영한 표적 화풍 재생성 코드 보기</summary>
<div class="aibook-lazy-source__body">펼치면 Python 원문을 불러옵니다.</div>
</details>

이 두 스크립트가 만든 후보와 실행 기록은 재현·비교 근거입니다. P7-5.1의 참조 원본은 로컬 GPU로 생성한 것만 사용할 수 있습니다. 내장 이미지 생성으로 만든 자산은 이 절의 입력·승인·manifest에서 제외했으며, 다른 절의 provenance 기록을 정리한 뒤 파일도 폐기합니다.

현재의 아홉 행은 아래 재생성 스크립트가 직접 만듭니다. `P7_STYLE_SCENE`에 행 ID를 넣으면 한 장면만, 값을 비우면 아홉 행을 생성합니다. 이 차이는 실행 시간을 줄이기 위한 것이며, 한 행을 생성했다고 전체 팩이 승인되는 것은 아닙니다.
...
로컬 GPU에서 후보를 만들 수 있다는 사실은 화풍 팩 승인과 다릅니다. 재생성 후보는 다섯 필수 행을 하나씩 채우고, 보조 후보는 다른 장소·시간·시점에서도 계약이 유지되는지를 확인합니다. 현재는 아트리움 한 행만 사람 승인을 받았고, 나머지 행은 생성·검수 대기입니다.

| 필수 행 | 로컬 GPU 재생성 후보 | 현재 판정 | 이 행이 확인하는 것 |
| --- | --- | --- | --- |
| 실내·새벽·high angle | 아트리움 local-gpu-v5 | 행 승인 | 새벽 광원과 실내 하향 시점 |
| 실내·밤·oblique | 여객기 실내 local GPU 후보 | 생성·검수 대기 | 야간 인공광과 창가 사선 시점 |
| 실외·낮·wide eye-level | 도심 낮 local GPU 후보 | 생성·검수 대기 | 낮 팔레트와 측면 교차로 시점 |
| 실외·해질녘·low angle | 주택가 local GPU 후보 | 생성·검수 대기 | 제한된 석양빛과 curb-height 상향 시점 |
| 실외·우천 야간·overhead high angle | 옥상 광장 local GPU 후보 | 생성·검수 대기 | 젖은 바닥 반사와 하향 야간 시점 |
...
| 베니스 운하 · 석양 · oblique | 베니스 local GPU 후보 | 제한된 apricot 역광과 사선 수면 |
| 공원 연못 · 낮 · eye-level | 공원 local GPU 후보 | 자연 공간의 낮 팔레트와 수면 반사 |
| 열차 승강장 · 우천 야간 · oblique | 승강장 local GPU 후보 | 인공광, 젖은 바닥, 짧은 대각 철로 |
...
이 참조 셋의 상태는 `blocked_pending_local_gpu_regeneration`다. manifest는 아직 비어 있으며, 로컬 GPU 원본의 사람 승인이 끝나기 전에는 다음 생성에 화풍 입력을 제공하지 않는다. manifest 생성 스크립트는 승인된 원본 목록만 갱신한다.

## 승인된 로컬 GPU 원본과 검수 대기 후보를 구분한다

아트리움 local-gpu-v5는 첫 필수 행의 사람 승인 원본입니다. 이 원본은 full-bleed 구조, 계단·난간·화분의 층진 그림자, pale-cyan 반사광, 타일과 벽의 수채화 안료 고임을 함께 확인한 결과입니다. 그러나 한 행의 승인은 전체 팩 승인이 아니므로 manifest에는 아직 넣지 않습니다.

<div class="aibook-style-reference-grid">
  <figure class="aibook-style-reference-grid__item is-approved">
    <img src="../../../assets/part-07/chapter-05/p7-5-1-style-atrium-dawn-high-angle-local-gpu-v5.png" alt="새벽의 실내 아트리움을 위에서 내려다본 local GPU 화풍 원본">
    <figcaption><strong>행 승인</strong><span>실내 아트리움 · 새벽 · high angle · local GPU</span></figcaption>
  </figure>
</div>

나머지 여덟 행은 `review_required` 후보로 저장한 뒤 사람 검수를 거칩니다. 후보 이미지는 승인 전에는 P7-5.2 입력으로 공개하지 않습니다. 전체 팩이 승인된 뒤에는 이름과 역할을 [manifest](../../../assets/part-07/chapter-05/p7-5-1-approved-style-reference-pack.json), 판정과 실행 이력을 [검수 ledger](../../../assets/part-07/chapter-05/p7-5-1-local-style-pack-review.json)에 남깁니다.
...
| 생성 출처 | 참조 원본이 로컬 GPU 실행 기록과 함께 남아 있고, 내장 이미지 생성 자산이 섞이지 않았는가? |
