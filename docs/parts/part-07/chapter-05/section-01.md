...
| 참조 목록 | 행 사이의 일관성과 입력 범위 | 원본 목록과 관찰 범위 |
...
현재 `background-style-v3` 계약은 같은 핵심 조건을 47단어에서 30단어로 압축했다. 1~20번 참조 원본은 이 v3 계약으로 재생성했다. 계약을 바꾸면 기존 원본의 역할을 소급해 바꾸지 않고, 새 후보와 관찰 기록을 manifest에 추가한다.
...
P7-5.1의 참조 원본은 로컬 GPU로 생성한 것만 사용하며, 내장 이미지 생성으로 만든 자산은 현재 참조 목록에 넣지 않습니다.
...
## 사람 관찰 기록과 manifest를 분리한다

사람이 기록하는 것은 이미지의 미적 품질 점수 하나가 아닙니다. 각 원본에서 외곽·선·색·장소·시간·카메라와 **로컬 GPU 생성 기록**을 확인하고, 행별 일치·이탈과 다음 비교 질문을 로컬 관찰 기록에 적습니다. 다음 단계가 실제로 읽는 입력 목록은 [manifest](../../../assets/part-07/chapter-05/p7-5-1-style-reference-pack.json)에 따로 둡니다. 이 분리 덕분에 `무엇을 관찰했는가`와 `무엇을 다음 생성에 참조하는가`가 섞이지 않습니다.

manifest에는 스무 개의 로컬 GPU 원본이 있으며, 1~20번 행은 Qwen Image 30스텝 원본입니다. 이 원본들은 배경 화풍 계약을 비교하기 위한 참조 목록입니다. 내장 이미지 생성 원본은 P7-5.1의 현재 참조 목록에 넣지 않습니다.

## 1~20번 로컬 GPU 참조 원본을 한 번에 확인한다

아래 표는 위 스크립트의 `SCENES` 순서대로 1번부터 20번까지 정리한 로컬 GPU 원본입니다. 모든 이미지는 Qwen Image 30스텝 실행 결과이며, manifest는 이미지 자산명과 `scene_id`로 다음 단계 참조를 연결합니다.
...
스무 장면의 자산 이름과 장면 식별자는 [manifest](../../../assets/part-07/chapter-05/p7-5-1-style-reference-pack.json)에 남기고, 행별 관찰은 커밋하지 않는 로컬 기록으로 분리합니다.
...
| 사람 관찰 기록과 manifest의 분리 | 미적 판단을 자동화하지 않으면서 관찰 근거와 다음 단계 참조 목록을 분리하려 함 | 사람 관찰 뒤 스무 원본을 참조 manifest에 남김 | manifest가 이미지의 선·색·공간 품질을 자동 채점하거나 사람 관찰을 대신한다는 뜻은 아님 |
...
| 참조 목록 기록 | 스무 원본의 자산명·장면 식별자·관찰 범위를 manifest에 남겼는가? |
