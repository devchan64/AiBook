...
```mermaid
--8<-- "assets/part-07/chapter-05/p7-5-5-scene-pipeline-ko.mmd"
```
...
캐릭터는 모두 인물별 마스크·컷아웃에서 시작하지만, 후속 처리 경로는 다음과 같이 나뉜다. A·B·C의 Mira는 P7-5.3 최종 착장을 참조하고, C 조연은 텍스트로 새 외형을 지정한다.
...
컷아웃 아이덴티티 생성기는 신규 컷아웃 경로와 Mira 참조를 기본값으로 고정한다. 기존 포즈 생성기의 경로·이미지 전처리·해시·실행 환경 기록 함수를 재사용하며, 대상별 입력 구성과 프롬프트, 생성 루프는 이 코드에서 관리한다. 로컬 CUDA GPU에서 BF16, sequential CPU offload로 실행했고, 추가 LoRA는 사용하지 않았다.
...
다음 단계는 위 B·C 마네킨을 입력으로 받아 P7-5.3의 착장을 입힌다.
...
C에는 화면 오른쪽을 향하는 아이레벨 쿼터뷰 머리 크롭을 사용한다. 각 장면의 머리 방향에 맞춰 선택한 참조를 아래 입력 표에 고정한다.
...
A의 달리는 자세, B의 도약 자세, C 두 인물의 앉은 자세와 책을 확인한다. C 조연은 책 보강 후에도 펼친 책을 유지했다. 별도의 그림자 추가 단계는 두지 않는다. 이번 검수에서는 그림자 유무를 평가 대상에서 제외하고 인물·착장·소품의 유지 여부를 본다. B에서 기록한 얼굴 아이덴티티 불일치가 DeLight로 해결됐다고 판단하지는 않는다.
...
`--dry-run`을 빼면 세 장면을 순서대로 생성한다. `--targets`로 장면을 선택하고, 한 장면을 선택한 경우 `--prompt`로 조명 설명을 바꾼다. 재실행에는 새 `--run-label`을 사용한다. 위 기본 실행에서는 각 합성 이미지 한 장만 모델에 입력한다. JSON의 `inputs`에는 실제 입력을 기록한다. `prompt_basis`에는 프롬프트 작성에 참고한 최초 씬 경로·해시를 기록한다. 이후 코드 변경으로 현재 코드의 해시는 `text-v2` 생성 당시 기록과 다르지만 위 명령의 입력·프롬프트·생성 조건은 유지한다.
...
- [ ] 캐릭터 분리·외형 적용·BFS·DeLight와 배경 분리·DeLight가 합성으로 합쳐지고, 합성 출력이 텍스트 Relight 입력으로 이어지는가?
...
- [ ] A 직접 적용, B·C 마네킨·착장 적용, C 조연의 텍스트 적용에서 각 출력을 다음 단계의 입력으로 연결했는가?
- [ ] B·C의 착장 1차 결과에서 착장 반영과 포즈·크기 보존을 따로 확인하고, 이 단계에서는 얼굴·헤어 보강이 아직 남아 있음을 구분했는가?
- [ ] C Mira와 조연의 책 보강 결과를 각각 연결하고, DeLight·합성·Relight 결과에서도 두 사람의 펼친 책이 유지됐는지 확인했는가?
- [ ] A·C의 참조 교체 BFS 결과에서 머리 각도와 눈꺼풀·시선을 별도로 검수했는가?
- [ ] B의 BFS 10스텝 결과에서 얼굴 불일치를 확인하고, 후속 DeLight가 이를 해결한 것으로 해석하지 않았는가?
