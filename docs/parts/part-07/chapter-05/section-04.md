...
LoRA on은 off보다 화풍과 착장 경향을 끌어왔지만, 정확한 얼굴·동작·가방을 단독으로 고정하지는 못했다. FaceID·FacePlus도 얼굴 단서를 보조했을 뿐 전신 계약을 통과시키지 못했다.

| character LoRA on/off | FaceID | FacePlus + FaceID |
| --- | --- | --- |
| ![character LoRA on/off 비교](../../../assets/part-07/chapter-05/p7-5-4-character-lora-on-off-contact-sheet.png) | ![FaceID probe](../../../assets/part-07/chapter-05/p7-5-4-faceid-contact-sheet.png) | ![FacePlus와 FaceID 결합 후보](../../../assets/part-07/chapter-05/p7-5-4-faceplus-faceid-contact-sheet.png) |
| 화풍·착장 경향은 보조 | 일러스트 얼굴 embedding이 크게 이탈 | 얼굴 단서는 생겨도 전신 계약 미통과 |

## OpenPose는 동작의 2D 배치를 보조했다
...
## 고각도 guide를 만들 수 있어도 Mira로 전이되지는 않았다
...
## Qwen에서는 세 입력의 역할을 분리해 통과 경로를 만들었다
...
보조 실험의 입력 연결은 아래 도식으로 확인할 수 있다. OpenPose와 depth·Canny는 구조 조건, FaceID·FacePlus·IP-Adapter·LoRA는 캐릭터 조건, mask·VTON은 생성 뒤 국소 보정으로 분리했다.
