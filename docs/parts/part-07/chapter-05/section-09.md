...
StoryDiffusion은 서로 다른 장면을 생성하면서 인물의 특징을 이어 가려는 연구다. 이 저장소의 기존 멀티뷰 실험은 그 목표를 달성하지 못했으며, StoryDiffusion의 연속 장면 생성 결과로 채택하지 않는다.
...
기존 워크플로가 로드한 베이스 모델은 `darkSushiMixMix_225D.safetensors`이며, `SeaArtApplyStory` 등 제3자 ComfyUI 노드를 적용했다. 기존 이미지를 VAE 잠재값으로 변환한 뒤 낮은 denoise로 다시 그린 결과 두 장은, 장면과 행동이 달라지는 연속 이미지 생성의 근거가 되지 못했다.

StoryDiffusion은 별도의 체크포인트 이름만으로 판별하는 모델이 아니라, 사전 학습된 확산 모델에 Consistent Self-Attention을 적용하는 방법이다. 따라서 SD 1.5 기반 모델을 사용했다는 사실만으로 StoryDiffusion 여부를 판정할 수는 없다. 그러나 이 실험은 사용한 포트의 코드 버전과 공식 CSA 구현의 대응을 확인할 근거가 부족하다. 노드 이름만으로 공식 방법을 재현했다고 볼 수 없다. [StoryDiffusion 공식 구현](https://github.com/HVision-NKU/StoryDiffusion){: target="_blank" rel="noopener noreferrer"}

또한 같은 입력을 일반 img2img로 처리한 대조군이 없어, 결과가 원본과 닮은 이유를 CSA 효과로 분리할 수 없다. 기존 실험은 **연속 장면 생성 목표 미달, 공식 방법 재현 여부 미확인**으로 정리한다. 이를 근거로 했던 ‘8GB에서 검증된 StoryDiffusion 멀티뷰 보강’ 결론은 철회한다.
...
- 사용한 베이스 모델과 CSA 적용 구현을 구분하고, 실제 연속 장면 결과와 대조군으로 효과를 확인했는가?
