...
StoryDiffusion은 이 두 번째 문제를 겨냥한 연구다. 중심 질문은 **여러 컷에서 인물과 화풍의 일관성을 높이려는 StoryDiffusion은 무엇을 공유하며, 무엇을 여전히 사람 검수로 남기는가**이다. 이 절을 읽고 나면 독자는 단일 이미지 재생성과 여러 컷 일관성 생성을 구분하고, StoryDiffusion 계열을 다음 탐색 후보로 삼을 근거와 현재 8GB 환경에서 바로 실행 대상으로 삼기 어려운 이유를 함께 설명할 수 있어야 한다.
...
## 4. 현재 8GB 환경에서는 탐색 후보이지 즉시 실행 경로가 아니다

StoryDiffusion 공식 구현은 SD 1.5와 SDXL 기반 확산 모델에 붙일 수 있다고 설명한다. 그러나 공식 README의 low-memory Gradio 경로도 24GB GPU 메모리 장비에서 시험됐으며, 20GB보다 큰 GPU 메모리에서 잘 동작할 것으로 예상한다고 적는다. 그러므로 현재 8GB GPU에서 이 절은 “지금 바로 돌아가는 생성기”가 아니라 **다음 실험 모델을 고르는 기준**이다. [공식 low-memory 안내](https://github.com/HVision-NKU/StoryDiffusion#how-to-use){: target="_blank" rel="noopener noreferrer"}

| 선택지 | 지금 할 일 | 현재 판단 |
| --- | --- | --- |
| 원본 장면을 다시 생성해 얼굴·착장 반영 | 원본 보존과 참조 반영을 분리해 검수 | 선화·pose를 정확히 유지하는 해법은 아님 |
| StoryDiffusion 계열 탐색 | 여러 컷의 프롬프트 묶음, identity·style 검수표를 준비 | 장면 연속성 문제의 후보 |
| 공식 StoryDiffusion 로컬 실행 | GPU 메모리, SDXL 의존성, 묶음 수를 기록하며 사전 점검 | 8GB에서 실행 성공을 가정하지 않음 |

이 경계는 StoryDiffusion을 낮게 평가한다는 뜻이 아니다. 오히려 지금의 실패가 무엇을 요구하는지 정확히 드러낸다. 한 장의 원본 선화를 고정하는 문제와, 여러 컷에서 같은 캐릭터를 유지하는 문제는 서로 연결되지만 동일하지 않다. 전자는 원본 보존 전략을, 후자는 묶음 단위 attention 또는 일관성 모델을 찾아야 한다.
...
- 공식 low-memory 안내가 8GB 실행 성공을 보장하지 않는다는 점을 기록했는가?
