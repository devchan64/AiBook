...
## 8. MoMask 첫 실행에서 교대와 지속 동작을 구분한다

2026년 9월 19일 RTX 5070 Laptop GPU(총 8151MiB)에서 MoMask를 실행했다. batch 1, 20fps, 시드 10107·10108·10109를 사용해 걷기·달리기를 각각 생성했다. 48프레임 기본 지시 6개와 96프레임 평지 지속 동작 지시 6개, 총 12개 모션이다. 96프레임 조건은 길이와 프롬프트를 동시에 바꾸었으므로 어느 변경이 결과에 영향을 주었는지 분리한 실험은 아니다.

[전체 실험 비교표·영상·프레임·실행 기록](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/README.md){ .aibook-markdown-preview } · [MoMask 실행 코드](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/p7_5_15_run_momask.py) · [측면 렌더링·발 높이 진단 코드](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/p7_5_15_review_momask.py)

| 조건 | 관찰 | 판단 범위 |
| --- | --- | --- |
| 48프레임 걷기 | 세 시드에서 좌우 다리의 앞뒤 교대가 나타남 | 교대 구조는 확인. 캐릭터 이미지의 가림·정체성 보존은 별도 검수 |
| 48프레임 달리기 | 초반 부유 후 내려오는 흐름, 골반 높이 범위 약 0.88~1.00m | 양발이 뜨는 프레임만으로 정상 반복 달리기로 판정할 수 없음 |
| 96프레임 평지 걷기 | 초반 긴 정지·부유 뒤 일부 이동 | 지속 보행 지시 충족 미흡 |
| 96프레임 평지 달리기 | 골반 높이 범위 약 0.10~0.13m, 초반 정지 뒤 짧은 달리기 | 높이 변화는 줄었으나 지속 달리기 지시 충족 미흡 |

공식 코드의 BVH 변환 부분은 현재 NumPy와 호환되지 않아 사용하지 않았다. 공식 생성 함수와 원 관절 복원 경로를 사용하고, 제거된 `np.float` 별칭에 호환 처리를 적용했다. IK나 매 프레임 발을 지면으로 옮기는 보정은 적용하지 않았다. 따라서 이 결과는 후처리 없는 3D 모션의 관찰이며 공식 BVH 후처리 결과에 대한 판정은 아니다.

48프레임 실행의 최대 PyTorch 할당 메모리는 약 0.89GiB였다. 이 수치는 프로세스 전체 VRAM이나 화면 표시용 메모리를 모두 포함하지 않는다. 발 높이 진단은 전체 시퀀스에서 추정한 바닥과 2·4·6cm 기준을 사용했다. 지면과 신발 밑창의 실제 접촉 측정이 아니므로 긴 부유 구간을 달리기 성공으로 계산하지 않는다.

## 9. 같은 모션을 캐릭터 영상으로 옮긴 결과

MoMask 뒤에 StableAnimator 기본 모델을 연결해 기존 Mira 전신 참조로 걷기·달리기 영상을 각각 32프레임, 512×512로 생성했다. body-only OpenPose에 고정 측면 투영을 사용했고 얼굴·손의 상세 관절은 넣지 않았다. 25 steps, CFG 3, seed 23123134, tile 16·overlap 4 조건이다. FP16 영상 모델은 GPU에서 실행했고 CPU 오프로딩은 사용하지 않았다. 얼굴 분석의 ONNX CUDA 제공자는 라이브러리 부재로 CPU로 대체됐다.

| 입력 모션 | 캐릭터 영상 관찰 | 실행 비용 |
| --- | --- | --- |
| 48프레임 걷기 seed 10107의 0~31번 | 다리가 겹친 뒤 다시 벌어짐. 첫 프레임의 잔상, 의상·얼굴 변화가 남음 | 270.36초, PyTorch 최대 할당 5.51GiB |
| 96프레임 달리기 seed 10109의 36~67번 | 초반 달리기 형태가 나타나지만 후반 의상·신체가 변하고 하반신이 사라짐 | 260.24초, PyTorch 최대 할당 5.51GiB |

달리기는 원 모션에서 움직이는 중간 구간을 선택했다. 이 선택은 원 시퀀스 전체의 지속 달리기 성공을 뜻하지 않는다. 양발 높이가 추정 바닥보다 4cm 높은 원 프레임 44·52·58·59·65·66을 포즈와 출력에서 대조했다. 입력 포즈에는 다리가 남아 있지만 후반 출력은 이를 보존하지 못했다. 바닥이 드러나지 않는 배경이므로 공중 구간의 정확한 보존도 확인하지 못했다. 따라서 걷기는 동작 전달의 부분 성공, 달리기는 포즈·외형 유지 실패로 기록한다.

[걷기 전체 프레임](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/stableanimator-walk-v1/all-frames.jpg) · [달리기 전체 프레임](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/stableanimator-run-v1/all-frames.jpg) · [공중 후보 포즈·출력 대조](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/stableanimator-run-v1/flight-candidate-comparison.jpg)

전체 64개 출력 프레임을 검수판으로 확인하고 공중 후보는 포즈와 나란히 대조했다. 영상·연속 PNG·조건·실행·검수 JSON은 위 실험 비교표에서 연결한다. 공식 모델 fps 조건 7과 GIF 재생 8fps를 유지했고, 비교용 MP4만 원 모션과 같은 20fps로 저장했다. 최대 메모리는 PyTorch 할당량이며 전체 GPU 사용량이 아니다. 걷기와 달리기는 CPU 스레드 설정도 달라 속도 차이의 원인을 분리할 수 없다.

사용자는 이번 실험을 의미 있는 결과로 평가했다. 실제로 로컬 8GB GPU에서 **텍스트 → 3D 모션 → 측면 포즈 → 참조 캐릭터 프레임**을 연결 실행했고, 걷기에서는 다리의 겹침과 분리가 영상으로 전달되는 모습을 얻었다. 달리기의 실패도 입력 포즈와 출력 프레임을 대조할 수 있어, 영상화 단계에서 신체 구조가 무너지는 구간을 특정할 수 있었다. 따라서 이번 실험의 성과는 완성 영상의 품질뿐 아니라, 실행 가능한 경로와 다음 개선 지점을 함께 확보한 데 있다.

이번 산출물은 `sec-15/2026-09-19-momask-stableanimator-v1/` 한 폴더에 모았다. [실험 자산 목록](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/manifest.json)과 [비교표](../../../assets/part-07/chapter-05/sec-15/2026-09-19-momask-stableanimator-v1/README.md){ .aibook-markdown-preview }에서 12개 원 모션, 캐릭터 영상 2개, 연속 프레임, 실행·검수 JSON 및 코드를 함께 찾을 수 있다.

이번 결과는 캐릭터 하나와 영상 생성 시드 하나의 탐색이다. 8GB에서 연결 실행은 가능했지만 사용자가 지적한 교대·공중 구간 문제를 모두 해결하지는 못했다. 다음 실험은 전신 포즈의 화면 점유율과 바닥 기준을 정한 뒤, 얼굴 관절 유무나 타일 길이를 하나씩 바꿔 영상화 실패 원인을 구분한다. Kimodo는 기반 Llama 가중치 접근 제한이 계속되어 이번 추론에서는 제외했다.
