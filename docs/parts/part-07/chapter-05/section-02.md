...
정면 얼굴을 앵커로 만든 4패널 턴어라운드 시트를 사람 승인했습니다. 시트는 정면·쿼터뷰·측면·후면의 순서로, 눈에 보이는 홍채의 지름과 동공-홍채 비율, 시선과 코의 방향, 콧대·코끝, 단발의 가르마·앞머리·컬·외곽 실루엣을 함께 대조합니다. 현재 승인 자산의 기준 시드는 `62377`이며, 이 승인은 얼굴 회전 identity 범위만 뜻합니다. 생성 스크립트의 이후 기본 seed는 정면 얼굴과 같은 `62294`입니다. pose·camera·표정·전신은 별도 생성과 검수가 필요합니다.
...
| CodeFormer `2x`는 정면 얼굴 패널에만 identity 입력으로 사용 | 보이는 얼굴이 없는 후면까지 복원 대상으로 만들면 새 정보가 생긴 것처럼 보일 수 있음 | 정면 `2x` 패널만 전신 보강의 identity 앵커로 쓰고, 쿼터·측면은 fidelity 비교, 후면은 Lanczos 기록으로 분리함 | 확대·복원이 얼굴을 자동으로 개선하거나 원본보다 정확한 identity를 만든다는 뜻은 아님 |
