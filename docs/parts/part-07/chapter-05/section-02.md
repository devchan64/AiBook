...
| 4 | 소품 기준 | 재킷, 바지, 신발, 가방의 개별 물성·색·형태 계약 | 소품 하나가 독립적으로 읽히는지 |
| 5 | 정면 전신 | 정면 얼굴과 승인 소품 | 전신 비례, 의상 조합, 신발과 가방의 기본 배치 |
...
새 전신 방향 후보는 한 번의 호출 안에서 **방향 얼굴 → 몸통 → 전신 → 비례 보정 → 복장·특징 장치 통일** 순서로 생성합니다. 방향 얼굴은 머리·목·어깨의 방향을 정하고, 그 몸통을 전신으로 확장합니다. 이어 정면 전신 기준으로 머리 대비 몸 비율·어깨 폭·다리 길이·발 크기를 맞춘 뒤, 마지막 단계에서 재킷·바지·신발·가방과 가방 스트랩 같은 특징 장치를 통일합니다. 중간 몸통·전신·비례 보정 출력은 재현 기록이며, 복장 통일 전의 의상과 소품은 기준 자산이 아닙니다. skeleton, OpenPose, depth, Canny 윤곽, 화풍 원본은 이 생성의 입력이 아닙니다.

이 분리는 한 장의 prompt에 방향·비례·복장·소품 구조를 모두 요구하는 것보다 특징 장치를 고정하기 쉽다는 것을 확인한 실험 결과입니다. 예를 들어 후면에서는 비례 보정 출력의 기존 가방과 스트랩을 복사하지 않고, 마지막 단계에서 단일 가방의 위치와 스트랩의 시작점·대각선을 다시 계약할 수 있었습니다. 다만 이 절차가 자동으로 소품을 보장하지는 않습니다. 가방 수, 스트랩의 어깨 시작점과 등 중앙선 통과, 신발의 짝과 형태는 최종 PNG에서 사람 검수로 확인해야 합니다.
...
머리핀을 포함한 이전 정면 얼굴 기준은 폐기했습니다. 현재 정면 얼굴 기준은 머리핀 없이 얼굴형·홍채·머리·표정을 prompt로 정의해 새로 생성·검수했습니다. 화풍 이미지, 기존 얼굴 이미지, 전신 이미지는 참조하지 않습니다. 이 기준은 몸·의상·회전 view·표정 범위를 승인하지 않습니다.
...
이전 방향 얼굴 기준과 character master 이미지는 정리했습니다. 현재 얼굴 기준은 머리 장식 없는 정면 한 장에서 생성해 사람 승인한 좌우 쿼터·좌우 측면·후면입니다. 후보 생성 자체는 회전·정체성·머리 형태의 통과를 뜻하지 않으며, 아래 기준은 사람 검수를 통과한 결과만 등록합니다.

| 정면 | 후면 |
| --- | --- |
| ![승인된 정면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-front-v2.png) | ![승인된 후면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-rear-v1.png) |
| 좌측 전면 쿼터 | 우측 전면 쿼터 |
| ![승인된 좌측 전면 쿼터 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-left-front-quarter-v1.png) | ![승인된 우측 전면 쿼터 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-right-front-quarter-v1.png) |
| 좌측 측면 | 우측 측면 |
| ![승인된 좌측 측면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-profile-left-v1.png) | ![승인된 우측 측면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-profile-right-v1.png) |
