...
같은 문서 집합을 두고도 어떤 질문은 검색 단계에서 막히고, 어떤 질문은 검색은 되었지만 답변 단계에서 과장됩니다. 그래서 이 절에서는 검색 실패와 근거 누락을 분리해 기록하는 일이 중심이 됩니다.

이 절의 핵심은 점수를 더 많이 적는 일이 아닙니다. `지금 막힌 위치가 검색 단계인가, 답변 단계인가`를 분리해 기록하는 일입니다. 그래야 다음 반복에서 검색 규칙을 바꿔야 하는지, 답변 규칙을 바꿔야 하는지 바로 결정할 수 있습니다.
...
이 절은 `실패를 어디에서 봐야 하는가`를 닫는 데 집중합니다. 즉, 여기서는 `질문별 후보 -> 실패 단계 -> 멈춘 이유 -> 다음 수정`을 남기는 기준까지를 먼저 정리합니다.
...
이번 절에서는 [P7-5.1](./section-01.md)과 같은 문서 집합을 그대로 쓰되, 질문 묶음을 더 넓혀 `통과`, `문서 범위 밖`, `검색 실패`, `답변 과장 위험`이 모두 나오게 만듭니다.
...
이번 절은 질문만 읽고 끝나지 않습니다. `질문`, `문서 집합`, `기대 상태`를 함께 놓고, 실제 상위 후보가 그 기대 상태를 설명하는지까지 확인해야 합니다.
...
이번 예제의 목적은 질문별로 `상위 후보`, `판정 상태`, `실패 단계`, `다음 수정`을 한 번에 남기는 것입니다. 이번에는 검색 점수 하나로 끝내지 않고, 경계 사례 파일에 적힌 기대 상태와 현재 검색 결과를 나란히 놓아 `어디에서 막혔는가`를 읽을 수 있게 하겠습니다.
...
이 절은 질문 강도와 검색 규칙을 조금만 바꿔도 상태가 달라지는 장면을 보는 것이 핵심입니다.
