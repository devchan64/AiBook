...
이전 정면 전신과 방향 전신 이미지는 모두 폐기했습니다. 새 정면 전신은 정면 얼굴을 identity와 화풍 기준으로, 일반 핏 회색 마이크로 크롭탑의 밑단-허리선 관계·바지·신발을 복장 기준으로 사용해 생성한 뒤 사람 승인했습니다. 165cm·55kg·7.5등신 조건은 prompt 계약이며, 이 기준은 정면 전신의 비례·기본 복장·신발 형태까지만 승인합니다. [정면 전신 실행·승인 기록](../../../assets/part-07/chapter-05/p7-5-2-fullbody-front-reference-review.json)은 입력·seed·prompt와 사람 판정을 남깁니다. 방향 전신과 표정은 아직 별도 생성·검수가 필요하므로 P7-5.3의 회전 view나 컷신 입력으로 확대 해석하지 않습니다.
