...
- 질문별 기록에서 `판정 상태`와 `실패 단계`를 따로 적었는가?
- `문서 범위 밖`과 `검색 실패`를 같은 한 줄로 뭉개지 않았는가?
- 상위 후보가 있어도 강한 단정 표현은 `답변 과장 위험`으로 멈출 수 있는가?
- 다음 수정이 문서 집합, 검색 규칙, 답변 규칙 중 어디서 시작하는지 적었는가?
- 같은 문서 집합 위에서 질문 표현만 바꿔 상태가 달라질 수 있다는 점을 확인했는가?
