...
정면 전신은 승인 Qwen 얼굴을 identity 입력으로, 승인 body-only 맵을 구조 입력으로 분리하고, 자켓·이너·바지·신발·가방은 prompt 계약으로만 지정합니다. 이 역할 분리는 손·얼굴 점군이 비례 제어와 섞여 발생하는 드리프트를 줄이기 위한 실험 조건입니다. 같은 `seed=62294`, `960×1440`, 세 입력과 105단어 결합 prompt를 고정해 얻어 사람 승인한 30 step v95는 당시의 착장 통합 기준을 입력으로 쓴 이력입니다. 해당 착장 이미지는 이후 폐기했으며, 새 후보는 이를 다시 입력으로 쓰지 않습니다. 이 target의 기본값은 30 step이며, 이 승인은 정면 범위만 뜻합니다.
...
이미지 입력 없이 만든 착장 통합 기준은 자켓·이너·맨살 띠·하이웨이스트 와이드 팬츠·운동화·크로스백을 한 화면에서 대조하려는 중간 실험이었습니다. 그러나 이 이미지는 인물 비례와 착장 조건을 함께 고정해 다음 후보에 불필요한 편향을 줄 수 있어 폐기했습니다. 이후 정면 전신 후보에서는 착장 이미지를 넣지 않고, 필요한 의상·가방 조건을 prompt 계약으로만 명시합니다.
...
현재 Qwen 정면 전신·자켓·가방 기준은 30 step v95입니다. 이 기준은 승인 당시 정면 머리·착장 통합 기준·body-only OpenPose 맵을 분리 입력으로 쓴 결과이며, 착장 통합 기준은 이후 폐기했습니다. 새 후보는 승인 정면 머리와 body-only OpenPose 맵만 이미지 입력으로 쓰고, 흰 크롭 재킷·회색 이너 탑·피부 띠·하이웨이스트 와이드 팬츠·운동화·크로스백을 prompt로 지정합니다. v95의 승인 범위는 정면 전신·착장·가방 재현 대조에 한정하며, 방향 전신 앵커나 다른 방향의 자동 승인을 뜻하지 않습니다.
