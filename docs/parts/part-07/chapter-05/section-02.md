...
| P7-5.2 인물 | 로컬 GPU 전신 master, 실행 기록, 사람 검수 manifest | 단일 전신 master의 identity·화풍 이전만 review-only로 검토 |
...
| 자산군 | 목표 구성 | 역할 | 현재 상태 |
| --- | --- | --- | --- |
| 기준·표정·전신 이미지 | 단일 PNG의 전신·정면·좌우 3/4·측면·후면과 필요한 표정·손 detail | 얼굴·의상·전신·손·소품의 기준 | 화풍 조건 전신 master 한 장만 review-only, 나머지는 미승인 |
...
| 실행·검수 기록 | 원본별 prompt·seed·모델·해상도·사람 판정 | 재현성과 다음 단계 입력 범위 | 현재 master 실행 기록에만 기록 |
...
| 확인 항목 | 관찰 결과 | 판정 |
| --- | --- | --- |
| 전신과 무소품 | 머리부터 신발까지 한 인물로 보이며 가방·소품·프레임이 없음 | 통과 후보 |
| identity 계약 | teal bob, warm light-peach skin, 흰 재킷·charcoal 상의·teal 바지·흰 신발이 유지됨 | 통과 후보 |
| 선과 채색 | 가는 charcoal 윤곽선과 제한된 색은 유지됐지만, 수채화 색번짐은 약하고 평면 웹툰 채색에 가까움 | 보강 필요 |
| 다음 범위 | 단일 정면 master 한 장의 확인만 마침 | turnaround, pose, cutscene에는 사용 금지 |
...
[style-conditioned 실행 기록](../../../assets/part-07/chapter-05/p7-5-2-style-pack-character-master-v1.json)은 source style 원본, prompt, model, seed, 출력과 검수 항목을 함께 기록합니다. 현재 상태는 `review_required_not_character_pack_approved`입니다. 즉 전신 후보 한 장을 관찰한 것일 뿐이며, turnaround나 컷신용 캐릭터 참조 팩은 아직 만들지 않았습니다.
...
이 체크리스트는 참조 셋의 구조 점검 항목을 유지합니다. 다만 모든 원본은 로컬 GPU 실행 기록을 가져야 하며, 현재는 화풍 조건 전신 master 한 장만 review-only로 채워져 있습니다. 19장 기준·16장 train·4장 held-out 구조는 동작과 장면을 포함한 별도 local-only 팩을 만든 뒤에만 다시 판정합니다.
