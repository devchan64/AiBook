...
이 절은 `실패를 어디에서 봐야 하는가`를 닫는 데 집중합니다. 즉, 여기서는 `질문별 후보 -> 실패 단계 -> 멈춘 이유 -> 다음 수정`을 남기는 기준까지를 먼저 정리합니다. 검색기 교체, reranker 구현, 자동 평가 확장은 뒤의 [P7-7.2](../chapter-07/section-02.md) 실패 기록과 개선 계획에서 다시 이어집니다.
