...
P7-5.1에서는 `검색 후보 -> 선택 근거 -> 답변` 흐름을 만들었습니다. 하지만 실제 RAG 프로젝트는 여기서 끝나지 않습니다. 같은 문서 집합을 두고도 어떤 질문은 검색 단계에서 막히고, 어떤 질문은 검색은 되었지만 답변 단계에서 과장되기 때문입니다. 그래서 이 절에서는 검색 실패와 근거 누락을 분리해 기록하는 일이 중심이 됩니다.
...
이 절은 `실패를 어디에서 봐야 하는가`를 닫는 데 집중합니다. 즉, 여기서는 `질문별 후보 -> 실패 단계 -> 멈춘 이유 -> 다음 수정`을 남기는 기준까지를 먼저 정리합니다. 검색기 교체, reranker 구현, 자동 평가 확장은 뒤의 [P7-7.2](../chapter-07/section-02.md) `장애 기록과 다음 반복 계획`에서 다시 이어집니다.
