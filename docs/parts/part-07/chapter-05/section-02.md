...
| 2 | 방향 얼굴 | 정면 얼굴 계약, 거의 돌출되지 않는 광대 윤곽, view별 방향 | 눈·코·입·머리 윤곽이 회전 뒤에도 같은 인물인지 |
...
이전 정면 전신과 방향 전신 이미지는 모두 폐기했습니다. 새 작업은 정면 얼굴과 승인 소품을 입력으로 정면 전신을 먼저 생성·검수한 뒤, 그 정면 전신과 새 방향 얼굴을 이용해 방향 전신을 만듭니다. 따라서 현재는 전신 기준이나 회전 view가 P7-5.3의 입력으로 승인되어 있지 않습니다.
...
이전 방향 얼굴과 표정 기준은 모두 폐기했습니다. 정면 얼굴을 고정 앵커로 두고, 새 방향 얼굴 다섯 장을 한 번 적재한 동일 파이프라인에서 연속 생성해 사람 승인했습니다. 정면 이미지는 다시 그리지 않으며, 새 방향 얼굴은 넓고 평평한 광대와 약간의 볼살을 유지합니다. 이 방식이 자동 일관성을 보장하지는 않으므로, 다음 재생성에서도 view별 얼굴형·눈·코·입·머리 윤곽을 사람 검수합니다.
...
| 정면 | 후면 |
| --- | --- |
| ![정면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-front-v2.png) | ![승인된 후면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-rear-v2.png) |
| 좌측 전면 쿼터 | 우측 전면 쿼터 |
| ![승인된 좌측 전면 쿼터 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-left-front-quarter-v2.png) | ![승인된 우측 전면 쿼터 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-right-front-quarter-v2.png) |
| 좌측 측면 | 우측 측면 |
| ![승인된 좌측 측면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-profile-left-v2.png) | ![승인된 우측 측면 얼굴 기준](../../../assets/part-07/chapter-05/p7-5-2-face-profile-right-v2.png) |
