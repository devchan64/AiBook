...
4단계도 body-only OpenPose를 사용한다. 이 맵은 머리·얼굴의 윤곽이나 이목구비를 지정하지 않고, 양팔을 내린 전신의 관절 관계와 프레이밍만 맡는다. 얼굴형·홍채·헤어·색·음영 같은 identity는 정면 토르소와 3-4단계 착장 결과가 맡는다. FACE_70처럼 턱선·눈·코·입을 모두 포함한 점군은 얼굴 기하를 다시 지정해 토르소의 얼굴형과 경쟁하므로 현재 입력에서 제외한다.
