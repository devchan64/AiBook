...
양자화는 가중치의 비트 수를 줄이고, 오프로딩은 가중치를 둘 위치를 나눈다. 두 방법의 역할을 구분하려면 모델 파일 크기와 실행 중 VRAM 사용량을 따로 확인해야 한다. 아래 Qwen-Image 실행 기록에는 모델 파일, 오프로딩 설정, 입력 크기, step, VRAM 관찰값과 실행 시간이 함께 남아 있다.
...
![Q4_K_S GGUF로 생성한 정면 상체 T2I 결과](../../../assets/part-07/chapter-05/p7-5-6-qwen-image-q4ks-low-vram-front-v1_00002_.png)
...
[Q4_K_S 저VRAM T2I result JSON](../../../assets/part-07/chapter-05/p7-5-6-qwen-image-q4ks-low-vram-front-v1-seed-62294-steps-10-result.json)

[Q4_K_S 저VRAM T2I 실행 Python 원문](../../../assets/part-07/chapter-05/p7_5_6_qwen_image_gguf_low_vram_probe.py)

```bash
.venv/bin/python docs/assets/part-07/chapter-05/p7_5_6_qwen_image_gguf_low_vram_probe.py \
...
이 기록은 8GB GPU에서 오프로딩을 사용한 Qwen-Image T2I 실행 사례다. 인물 identity·pose·camera의 보존 여부는 별도의 참조 입력과 대조 실행으로 확인해야 한다.
...
- Unsloth AI, [Qwen-Image-GGUF](https://huggingface.co/unsloth/Qwen-Image-GGUF){: target="_blank" rel="noopener noreferrer" }, Hugging Face, 확인일: 2026-08-29.
- city96, [ComfyUI-GGUF](https://github.com/city96/ComfyUI-GGUF){: target="_blank" rel="noopener noreferrer" }, GitHub, 확인일: 2026-08-29.
