...
학습용 이미지 후보는 이 절의 `sec-11/` 아래에 통합한다. `input-images/`에는 5.12에서 사용할 참조 입력 후보 220장과 생성 기록을, `target-images/`에는 추가 Mira 목표 후보를 둔다. `target-images/`는 추가 생성하는 목표 후보의 보관 위치다. 입력 220장에 대응하는 기존 Mira 목표 44장은 카탈로그의 `target` 경로로 연결하며 중복 복사하지 않는다. 기존 채택 이미지의 `training-images/`와 구분하고, 후보 카탈로그에서 검수한 항목을 별도 학습 목록으로 선택한다.
...
[참조 입력 후보 220장 카탈로그 JSON](../../../assets/part-07/chapter-05/sec-11/input-images/candidate-catalog.json)
