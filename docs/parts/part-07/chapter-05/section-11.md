...
학습용 이미지 후보는 이 절의 `sec-11/` 아래에 통합한다. `input-images/`에는 5.12에서 사용할 폐기 후 남은 참조 입력 후보 217장과 생성 기록을, `target-images/`에는 추가 Mira 목표 후보를 둔다. `target-images/`는 추가 생성하는 목표 후보의 보관 위치다. 입력 220장에 대응하는 기존 Mira 목표 44장은 카탈로그의 `target` 경로로 연결하며 중복 복사하지 않는다. 기존 채택 이미지의 `training-images/`와 구분하고, 후보 카탈로그에서 검수한 항목을 별도 학습 목록으로 선택한다.
...
[참조 입력 후보 217장 카탈로그 JSON](../../../assets/part-07/chapter-05/sec-11/input-images/candidate-catalog.json)

[입력 후보 217개 비교표 Markdown](../../../assets/part-07/chapter-05/sec-11/input-candidate-comparison.md)

비교표에는 각 입력과 Mira 목표를 나란히 배치하고, 원래 생성 순번에 고정한 `P711-IN-NNN` 관리번호를 부여했다. 현재 기록은 실험용 채택 2개, 1차 제외 의견 24개, 확대 검수 보류 191개다. 보관 수 217개를 학습 사용 가능 수로 해석하지 않으며, 검수 의견은 관리번호로 추적한다.
