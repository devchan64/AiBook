...
Part 7 是把前面介绍过的内容放进真实输入、代码执行、输出比较和错误解释里来实作的区段。这里不会继续拉长新的理论，而是把 Part 4、Part 5、Part 6 已经介绍过的内容真正执行并解释出来。
...
Part 7 按照 `问题与输入定义 -> baseline 比较 -> 结构与失败解释 -> RAG 与 agent 执行记录 -> 部署与运营回顾` 这条流程展开。各节不是分散的示例，而是一组连在一起的实作，展示同一套记录标准怎样随着输入类型和运营阶段的变化而扩展开来。

1. 在 `P7-1.1` 先确认输入单位和第一轮基准线比较。
2. 在复用同一输入文件的 `P7-1.2`，把 `事实 -> 解释 -> 下一问题` 的回顾结构接上去。
3. 在 `P7-1.3` 改变基准线截止日和比较单位，直接看到即使是同一份日志，回顾第一句也会怎样变化。
4. 在 `P7-2.1`、`P7-2.2` 里确认 baseline、比较表、改进记录是怎样连起来的。
5. 在 `P7-2.3` 把多个比较实验放在一起，分开看哪些失败能靠预处理解决，哪些失败会因为数据边界仍然空着而继续留下。
6. 从 `P7-3` 到 `P7-7`，继续跟着同一套记录结构如何扩展到图像、文本、RAG、agent、部署与运营。特别是在 `P7-4.3`、`P7-5.3`、`P7-6.3`、`P7-7.3` 这些练习节里，会进一步做到直接改值并重写回顾句。

先把这条流程抓住，Part 7 就不会再像 `示例很多而分散的 Part`，而会更像 `展示同一记录原则怎样随着输入类型变化而扩大的 Part`。

真正开始时，把下面三行一起抓住会更顺手。

| 先做什么 | 为什么这个顺序更好 |
| --- | --- |
| 先从复用同一输入文件的前几节开始 | 因为可以先看见输入不变、但问题和解释变化的场面。 |
| 先运行带 baseline 比较的节 | 因为它能先建立阅读“变好了”这句话所需的最小感觉。 |
| 后半段把执行记录和失败记录一起读 | 因为这样可以避免 LLM、agent、部署实作只停在答案示例。 |

## 在哪里重新执行 Part 4、5、6

| 实作轴 | 需要从前面 Part 重新找回的感觉 | 在 Part 7 里直接执行的代表位置 |
| --- | --- | --- |
| Part 4 实作 | 问题设定、baseline、预处理、比较实验 | `P7-1.1` 到 `P7-2.3` |
| Part 5 实作 | 输入结构选择、学习结果解释、错误阅读 | `P7-3.1` 到 `P7-4.3` |
| Part 6 实作 | RAG、agent、权限、执行记录 | `P7-5.1` 到 `P7-6.3` |
| 综合运营实作 | 部署确认、故障记录、下一步动作优先级 | `P7-7.1` 到 `P7-7.3` |

这张对应表之所以重要，是因为 Part 7 不是额外拉出一大段新理论的 Part，而是把前面学过的判断标准重新放回真实执行场景里的 Part。只要先把当前卡住的点分成 Part 4 式的 baseline 问题、Part 5 式的结构或学习解释问题、或者 Part 6 式的依据、工具、运营问题，下一步该重开的节也会快得多。

## 实作记录的最小步骤

即使每一节的代码块不同，也最好尽量用相同的节奏去读执行过程。

```text
1. 用一句话读这一节的问题。
2. 先确认它读取的是哪一个输入文件。
3. 运行 Python 示例。
4. 确认输出里出现了什么：执行摘要、比较表、错误案例、失败记录。
5. 在下面的 `怎样阅读结果` 小节里确认这份输出的解释标准。
6. 在最后的 checklist 里确认这一节实际教会了什么。
```
...
Part 7 不会再把这些例子重新抽成单独的 `补充学习`。更重要的是，在各轴正文里直接留下 `怎样阅读真实执行结果`。比起照着敲完就结束的示例，输入一变、输出与解释也跟着变化的实作更重要。

## 这一 Part 不在这里结束的问题

由于 Part 7 处理的是项目入门流程，下面这些问题会交给之后的真实项目去继续。
