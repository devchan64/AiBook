...
这个开篇页首先要建立的计划，是把`问题`、`样本单位`、`基准线`、`比较表`、`错误案例`、`执行记录`、`下一步措施`留在项目文档中。比起实现完成，更优先留下能重新读出为什么出现该结果的记录。
...
- 知道 LLM、RAG、AI agent，但想把执行日志和质量复核的读法整理清楚的读者
...
| 执行记录与权限状态 | 因为要追踪 AI agent 按什么顺序行动、又在哪一步停下。 |
...
| 执行记录与失败记录 | 因为这样能避免 LLM、AI agent、部署实作最后只剩答案示例或成功画面。 |
...
| RAG 与 AI agent 实作 | 依据、权限、执行记录 | `P7-5.1` 到 `P7-6.3` |
...
- baseline 和改进应该怎样比较？
- 在深度学习项目里，什么需要用眼睛确认？
- 在 RAG 和 AI agent 项目里，什么应该算质量标准？
...
| AI agent | 计划、工具调用、审批、blocked 状态复核 | 执行路径和失败处理规则是否可见 |
...
| AI agent | `按什么顺序用了哪些工具？` | 因为在解释成败之前，必须先看见执行路径。 |
...
- baseline 与比较
- 结果与局限记录
- RAG、AI agent、部署项目的基本文档结构

这个 Part 不会把 `实战的全部` 一次讲完，但会明确覆盖 `进入实战前必须亲手运行并重新阅读的场景`。这一 Part 的正文主轴是 `问题与输入定义`、`基准点与比较`、`错误案例与复核`、`RAG 与 AI agent 执行记录`、`部署与失败回顾`。权限、运营、失败应对的大图景，则会继续在正文里的 `P7-6.2` 权限与日志复核、`P7-7.1` 部署确认与状态检查、`P7-7.2` 故障记录与下一轮计划中展开。

特别是，这个 Part 会一直维持下面这些区分。

- `分析项目`：问题、输入单位、摘要值、比较表是中心
- `模型项目`：baseline、预测结果、错误案例、修改优先级是中心
- `RAG 与 AI agent 项目`：依据文档、工具结果、执行记录、失败日志是中心
...
- 在 AI agent 项目里，`审批状态` 与 `下一步动作` 先于成功/失败。
...
换句话说，Part 7 确认的是 `如何开始并回顾一个项目的最小结构`，而不是把所有实务系统都讲完。
...
| 依据是什么？ | 选择依据、检索候选 | 因为 RAG 与 AI agent 的结果之后还要能重新验证。 |
...
| AI agent | 只有成功/失败 | 工具顺序、审批状态、卡住点 |
...
- 能在 RAG 与 AI agent 项目里把质量视角和运营视角一起读出来。
...
如果读完 Part 7 之后，下面这些标准还没有真实地留在记录里，说明这一 Part 的实作主轴还没有确认得足够好。

- 结果数字留下了，但 baseline、错误案例、回顾笔记没有一起留下
- 说明 RAG 或 AI agent 时，依据记录、执行记录、权限记录没有一起留下
...
- 能不能在 RAG 与 AI agent 项目里分开阅读依据、工具、权限记录？
