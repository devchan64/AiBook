# P5-7.3 Adam의 직관: 적응형 업데이트

Section ID: `P5-7.3`
Version: `v2026.07.17`

P5-7.2에서는 같은 gradient라도 learning rate에 따라 실제 update 보폭이 어떻게 달라지는지 보았습니다. 여기서 다음 질문이 바로 이어집니다.
...
이 절에서는 optimizer 이름을 더 많이 늘어놓기보다, Adam이 어떤 문제의식에서 적응형 update를 추가하는지 설명합니다. 즉, 여기서는 `새 모델 구조`보다 이미 계산된 gradient를 Adam이 `어떤 규칙으로 실제 update로 바꾸려 하는가`를 읽습니다. regularization과 일반화의 관점은 P5-8.1, P5-8.2에서 다시 연결하고, adaptive optimization의 수렴 분석은 P5-7.4 보충학습으로 이어집니다.
