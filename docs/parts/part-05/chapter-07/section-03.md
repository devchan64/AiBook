...
처음 읽을 때는 이 보충학습을 `같은 파인튜닝처럼 보이는 선택지들의 차이`를 정리하는 손잡이로만 잡고, 실제 데이터 설계와 운영 판단은 뒤 절로 넘겨 두는 편이 가장 안전합니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| adapter / LoRA / QLoRA 차이 | 어떤 조정 구조가 무엇을 더 적게 바꾸고 덜 무겁게 만드는가? | P5-7.3 |
| 지시 튜닝 연결 | 이 조정 방식으로 실제로 무엇을 가르치고 어떤 응답 습관을 바꿀 것인가? | P5-8.1, P5-8.2 |
| 운영 제약 연결 | 메모리, 저장 비용, 실험 회전 수 제약은 어떤 선택을 밀어 올리는가? | P5-16.1 |
