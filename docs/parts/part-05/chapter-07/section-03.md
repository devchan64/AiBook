...
이 보충학습의 목적은 수식을 길게 따라가는 데 있지 않습니다. `왜 low-rank라는 말이 붙는가`, `작은 조정분이 어디에 붙는가`, `왜 QLoRA가 같이 언급되는가`를 읽을 수 있게 만드는 데 있습니다.
...
이 절의 목적은 LoRA 관련 문서를 읽을 때 이름 때문에 멈추지 않게 하는 데 있습니다. 실제 instruction tuning 흐름은 P5-8.1에서 다시 이어지고, 운영 비용과 제약은 P5-16.1에서 다시 연결합니다.
