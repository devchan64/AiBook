# P5-7.4 보충학습: adaptive optimization의 수렴 분석을 처음 읽는 법
...
- optimizer 논문을 읽을 때 먼저 확인할 조건 목록을 만들 수 있습니다.
...
optimizer 논문을 처음 읽을 때는 정리(theorem)의 결론보다 가정을 먼저 봐야 합니다. 가정이 바뀌면 같은 `수렴`이라는 단어도 뜻이 달라집니다.

| 표현 | 처음 읽는 뜻 | 먼저 확인할 질문 |
| --- | --- | --- |
...
이 절의 사례는 optimizer 선택 사례가 아니라, 논문이나 실험 결과 문장을 읽는 사례입니다. 모든 사례는 같은 순서로 읽습니다.
...
다음 문장을 읽고 `논문 읽기 카드`를 채워 봅니다. 수식 증명을 하지 않아도, 어떤 문장이 실험 주장이고 어떤 문장이 수렴 분석 주장인지 나누고, 그 주장이 넘어가면 안 되는 결론을 적는 것만으로 논문 읽기의 출발점이 잡힙니다.
...
- optimizer 논문을 읽을 때 실험 성능, 수렴 분석, 일반화 성능을 따로 표시할 수 있는가?
