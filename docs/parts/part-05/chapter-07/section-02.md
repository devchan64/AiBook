...
이 절에서는 optimizer 논문을 깊게 비교하기보다, `업데이트 철학의 차이`를 설명합니다. momentum, RMSProp, AdamW의 세부 수식 비교와 일반화 논쟁의 세밀한 정리는 여기서 길게 다루지 않고, regularization과 일반화의 관점은 P5-8.1, P5-8.2에서 다시 연결합니다. 학습률 스케줄러와 AdamW 같은 후속 최적화 실무는 이 책의 현재 본편 범위 밖에 둡니다. 여기서는 `새 모델 구조`보다 이미 계산된 gradient를 `어떤 규칙으로 실제 update로 바꿀 것인가`를 읽습니다.
...
### 사례 1. 손실이 천천히 줄어드는 작은 운영 회귀 문제

작은 수요 예측이나 온도 보정 회귀 실험에서 입력 하나와 오차 하나를 눈으로 따라가며 학습을 보고 싶을 수 있습니다. 사람이 손으로 계산을 따라갈 때는 보통 `지금 오차가 어느 방향으로 났는가`를 보고 바로 조금 되돌리는 식으로 생각합니다. 예를 들어 예측값이 실제값보다 계속 크게 나오면, 현재 가중치를 줄이는 방향으로 한 걸음 움직여 보는 식입니다. 이때 SGD는 현재 gradient를 보고 바로 한 걸음 움직이므로, `이번 step에서 왜 가중치가 이렇게 바뀌었는가`를 비교적 직접 추적할 수 있습니다. 단순한 문제에서는 이런 직선적인 업데이트가 학습 원리를 설명하는 데 특히 유리합니다. 그래서 이 사례에서 확인해야 할 결과는 여러 보정 없이도, 현재 gradient 방향이 다음 가중치 변화에 직접 반영되는가입니다.
...
예를 들어 어떤 층의 파라미터는 gradient가 매우 크고, 다른 층은 아주 작아서 같은 보폭으로는 한쪽은 과하게 튀고 다른 쪽은 거의 안 움직일 수 있습니다. Adam은 이런 차이를 완전히 해결하는 것은 아니지만, 좌표별 변화 크기를 다르게 읽어 초반 업데이트를 조금 더 균형 있게 만들려 합니다. 그래서 `왜 Adam이 초반 실험에서 덜 답답하게 느껴지는가`를 이해할 때는 `더 똑똑한 공식`보다 `파라미터마다 보폭을 조금 다르게 잡으려는 태도`를 떠올리는 편이 더 정확합니다.
...
| 사람이 먼저 보기 쉬운 기준 | SGD 관점으로 다시 읽는 기준 | Adam 관점으로 다시 읽는 기준 |
| --- | --- | --- |
| 오차가 보이면 같은 크기로 바로 되돌리면 된다고 느끼기 쉽다 | 현재 gradient 방향과 learning rate가 이번 step 이동량을 직접 결정한다 | 최근 gradient 흐름과 좌표별 차이를 함께 참고해 보폭을 더 다르게 잡으려 한다 |
| 초반 손실이 빨리 줄면 무조건 더 좋은 optimizer라고 생각하기 쉽다 | 수렴 속도와 최종 일반화는 따로 비교해야 하고, SGD는 기준선 역할을 잘 보여 준다 | 초반 안정성과 실용성은 좋을 수 있지만 최종 결과 우위를 자동 보장하지는 않는다 |
| 모델 구조가 같으면 optimizer는 부수 설정일 뿐이라고 보기 쉽다 | 같은 구조라도 update 규칙이 달라지면 파라미터 경로와 진동 양상이 달라진다 | 같은 gradient라도 누적 정보 때문에 더 매끈하거나 덜 급한 이동이 나타날 수 있다 |
| Adam이 많이 쓰이니 SGD는 이제 덜 중요하다고 느끼기 쉽다 | SGD는 `현재 gradient에 직접 반응하는 기본 보폭` 감각을 가장 분명히 보여 주는 기준점이다 | Adam은 그 기준점 위에 적응형 보정이 무엇을 더하는지 읽게 해 주는 비교 대상이다 |

이 사례들에서 최종적으로 확인해야 할 결과는 분명합니다. SGD와 Adam의 차이는 `어느 이름이 더 최신인가`가 아니라, SGD는 현재 gradient 기준의 직접 업데이트를, Adam은 누적 정보와 좌표별 차이를 더 반영한 적응형 업데이트를 보여 준다는 데 있습니다.
...
## 언제 SGD와 Adam 비교를 먼저 꺼내는가

optimizer의 일반 역할을 이해한 뒤에는 `지금 기본 업데이트 감각이 필요한가, 아니면 적응형 업데이트 감각이 필요한가`를 나누어 읽는 편이 좋습니다.

| 먼저 보이는 문제 장면 | 먼저 떠올릴 optimizer 관점 | 이유 |
| --- | --- | --- |
| gradient 방향과 보폭의 가장 기본 구조를 설명해야 한다 | SGD | 현재 gradient에 직접 반응하는 기본 업데이트 감각을 가장 분명히 보여 줍니다. |
...
| optimizer를 절대 우열로 외우려는 경향이 보인다 | SGD와 Adam을 나란히 둔다 | 속도, 안정성, 일반화를 분리해 읽어야 하기 때문입니다. |

## 체크리스트

- SGD와 Adam이 어떤 업데이트 철학 차이를 가지는지 설명할 수 있는가?
- 왜 Adam이 널리 쓰이면서도 SGD가 여전히 중요한지 말할 수 있는가?
- SGD를 기본적인 gradient-based update가 가장 단순한 형태에서 어떻게 움직이는지 읽게 해 주는 기준점으로 설명할 수 있는가?
- Adam은 누적 정보와 좌표별 조절을 더 반영하는 적응형 optimizer라는 점을 설명할 수 있는가?
- SGD와 Adam을 `기본 보폭 업데이트`와 `적응형 업데이트`의 차이로 설명할 수 있는가?
- optimizer 비교는 속도, 안정성, 일반화까지 함께 보아야 한다는 점을 말할 수 있는가?
- optimizer를 무조건 우열 순위처럼 외우려 할 때, SGD와 Adam을 기준점과 적응형 업데이트 차이로 다시 떠올릴 수 있는가?
