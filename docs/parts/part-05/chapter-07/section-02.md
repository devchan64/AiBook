...
P5-7.1에서는 사전학습(pretraining)을 대규모 텍스트에서 일반 언어 패턴과 표현을 먼저 배우는 단계로 설명했습니다. 그러면 다음 질문이 자연스럽게 생깁니다.
...
이 절에서는 스케일의 방향만 잡고, 다음 토큰 예측으로 이어지는 학습 목표는 P5-8.1에서 다시 구체화합니다. GPU 인프라와 최신 상용 모델 수치 비교는 빠르게 바뀌는 주제이기도 하므로 현재 본편 범위 밖으로 두고, 운영 제약의 큰 그림만 P5-18.1 AI 서비스가 현실에서 만나는 제약에서 다시 연결합니다.
...
작은 모델은 제한된 패턴에만 안정적으로 반응할 수 있지만, 규모가 커질수록 더 많은 표현과 작업 지시에 반응하는 것처럼 보일 수 있습니다.

### 사례 2. 더 긴 입력 처리

더 큰 모델과 더 넓은 문맥 구조는 긴 문서, 긴 코드, 긴 대화에 대한 사용 경험을 개선할 수 있습니다. 하지만 그만큼 비용도 커질 수 있습니다.

### 사례 3. 운영 관점

대규모 모델은 학습뿐 아니라 서비스 추론 비용, 캐시 전략, 모니터링, 안전 점검 부담도 함께 키웁니다.
