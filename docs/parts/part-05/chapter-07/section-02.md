...
momentum, RMSProp, AdamW의 세부 수식 비교와 일반화 논쟁의 세밀한 정리는 여기서 길게 다루지 않습니다. 대신 regularization과 일반화의 관점은 P5-8.1, P5-8.2에서 다시 연결합니다. 학습률 스케줄러와 AdamW 같은 후속 최적화 실무는 이 책의 현재 본편 범위 밖에 둡니다.

이 절에서는 optimizer 논문을 깊게 비교하기보다, `업데이트 철학의 차이`를 설명합니다. regularization과 일반화의 관점은 P5-8.1, P5-8.2에서 다시 연결하고, AdamW와 scheduler 같은 후속 최적화 실무는 이 책의 현재 본편 범위 밖에 둡니다.
