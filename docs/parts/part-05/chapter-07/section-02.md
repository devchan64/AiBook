...
이 질문은 바로 P5-8.1 정규화(regularization)로 이어집니다.
...
- Adam이 많이 쓰여도 SGD 기준점이 여전히 필요한 이유를 말할 수 있는가?
- 이 절 다음에는 optimizer 자체가 아니라 일반화 제약과 regularization 장으로 넘어간다는 흐름을 이해했는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- optimizer를 무조건 우열 순위처럼 외우려 할 때, SGD와 Adam을 기준점과 적응형 업데이트 차이로 다시 떠올립니다.
- 같은 gradient라도 학습 속도와 안정성이 왜 달라지는지 설명해야 할 때, 현재 gradient 반영과 누적 정보 활용의 차이를 다시 봅니다.
- optimizer 비교를 마친 뒤 일반화 제약으로 넘어가야 할 때, update 규칙과 regularization의 역할을 분리하는 관점을 꺼냅니다.
