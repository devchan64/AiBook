...
이 질문은 바로 P5-7.2 SGD, Adam의 직관으로 이어집니다.
...
- 손실, 역전파, optimizer의 역할 차이를 구분할 수 있는가?
- 이 절 다음에는 SGD와 Adam 같은 대표 update 규칙 비교로 넘어간다는 흐름을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- gradient는 이해했는데 실제 파라미터가 어떻게 움직이는지 설명이 비어 있을 때, optimizer 관점을 먼저 떠올립니다.
- 손실, 역전파, 업데이트가 한 덩어리처럼 섞일 때, `틀림 -> gradient -> 실제 수정`의 역할 차이를 다시 구분합니다.
- learning rate가 왜 학습 전략의 핵심 손잡이인지 정리해야 할 때, update 규칙과 보폭을 함께 보는 이 절을 다시 꺼냅니다.
