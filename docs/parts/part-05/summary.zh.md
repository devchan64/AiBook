...
这个总结页的复盘计划，是重新整理`输入变成了什么表示`、`误差以什么信号传递`、`结构选择如何连接到数据形态`、`通向生成问题的出发点是什么`。这样 Transformer 和 LLM 的说明才会被读作计算结构的延伸，而不是名称记忆。
...
这里一个重要原则是，不要把 Part 5 结束成 `下一 Part 的预告片`。CNN、RNN、Attention、Transformer 都会连到后面的 LLM 说明，但这种连接必须遵守 `先确认当前结构，再在下一 Part 里扩展到别的问题`，而不是把解释推成 `后面再看`。

复习时，与其只重读这个总结页就停下来，不如在术语开始模糊的地方回到[概念词汇表](/AiBook/zh/reference/concept-glossary/)，把对应词条和它指向的 `Core Section` 一起再看一遍，会更稳定。
...
换句话说，Part 5 是确认 `结构基础` 的 Part，而不是在这里把 LLM 的整体服务体验一次说完的 Part。
...
- 你能不能把生成与采样解释成不是 `下一 Part 才新出现的话题`，而是 `在这里已经确认结构起点的话题`？
