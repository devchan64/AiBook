...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `RAG 근거 연결 -> tool use 실행 연결 -> agent 목표 흐름`입니다. 여기서는 그 가운데 단계인 `실행 연결`을 맡고 있고, 바로 다음 장에서 이 실행 요청이 함수 호출 구조로 바뀌며 그다음 장에서 여러 단계 흐름으로 이어집니다. 처음 읽을 때는 `무엇을 실제로 조회하거나 실행해야 하는가`를 지금 절의 질문으로 붙잡고, 바로 다음 P5-12.2에서는 그 실행 요청을 어떤 이름과 인자 구조로 넘길지, P5-13에서는 여러 실행과 읽기를 어떤 순서로 이어 갈지, P5-14.2에서는 이 실행 흐름을 어떤 규칙과 기록 구조 안에서 관리할지를 차례로 보면 충분합니다.
