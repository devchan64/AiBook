...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. tool use는 `무엇을 실제로 조회하거나 실행할까`를 먼저 닫는 구조입니다. RAG가 문서를 읽는 구조라면 tool use는 외부 기능으로 실행을 위임하고, P5-12.2에서는 그 요청을 함수 이름과 인자 구조로 어떻게 넘길지 더 봅니다. 그다음 P5-13에서는 이 호출이 한 번으로 끝나지 않을 때의 agent 흐름으로 넘어갑니다.
