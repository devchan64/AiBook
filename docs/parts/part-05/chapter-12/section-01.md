...
즉, 지금 장의 핵심은 `문서를 읽는가`에서 `외부 기능으로 실제 결과를 가져오는가`로 손잡이가 바뀌고, 이 절이 `RAG 근거 연결 -> tool use 실행 연결 -> agent 목표 흐름` 가운데 `실행 연결` 단계를 맡는다는 점입니다. 처음 읽을 때는 `무엇을 실제로 조회하거나 실행해야 하는가`를 지금 절의 질문으로 붙잡고, 바로 다음 P5-12.2에서는 그 실행 요청을 어떤 이름과 인자 구조로 넘길지, P5-13에서는 여러 실행과 읽기를 어떤 순서로 이어 갈지, P5-14.2에서는 이 실행 흐름을 어떤 규칙과 기록 구조 안에서 관리할지를 차례로 보면 충분합니다.
