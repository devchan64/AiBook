...
장기 의존성(long-term dependency) 문제는 P5-12.2에서 이어서 더 집중해서 다루고, attention 이후 구조와의 연결은 다음 장 P5-13.1에서 다시 이어집니다. 즉, 여기서는 `순차 상태를 왜 들고 가야 하는가`와 `기본 RNN만으로는 왜 오래 기억하기 어려웠는가`를 먼저 닫습니다.
