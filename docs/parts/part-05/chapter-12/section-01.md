...
| --- | --- | --- |
| RAG 근거 연결 | 무엇을 근거로 답할까? | P5-10, P5-11 |
| tool use 실행 연결 | 무엇을 실제로 조회하거나 실행할까? | P5-12.1 |
| agent 목표 흐름 | 읽기와 실행을 어떤 순서로 이어 갈까? | P5-13 |
...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `RAG 근거 연결 -> tool use 실행 연결 -> agent 목표 흐름`입니다. 여기서는 그 가운데 단계인 `실행 연결`을 맡고 있고, 바로 다음 장에서 이 실행 요청이 함수 호출 구조로 바뀌며 그다음 장에서 여러 단계 흐름으로 이어집니다.
