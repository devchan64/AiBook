...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. tool use는 `무엇을 실제로 조회하거나 실행할까`를 먼저 닫는 구조이고, RAG가 문서를 읽는 구조라면 tool use는 외부 기능을 불러 결과를 가져오는 구조입니다. 핵심은 `정답을 추측하는가`보다 `필요한 순간 실행을 위임하는가`에 있고, P5-12.2에서는 그 실행 요청을 함수 이름과 인자 구조로 어떻게 넘길지 더 봅니다. 이어서 P5-13 이후에는 이 호출이 한 번으로 끝나지 않을 때 어떤 순서로 이어 가며 다음 행동을 바꿀지로 넘어갑니다. 반면 MCP나 하네스 같은 연결 인터페이스와 실행 관리 환경, 평가와 운영처럼 실행 기록을 품질 판단과 실패 대응으로 바꾸는 단계는 아직 여기서 닫지 않습니다.
