...
처음 읽을 때는 tool use를 Part 5 본류의 `실행 연결 손잡이`로만 잡으면 충분합니다.

즉, 지금 장의 핵심은 `문서를 읽는가`에서 `외부 기능으로 실제 결과를 가져오는가`로 손잡이가 바뀌고, 이 절이 `실행 연결` 단계를 맡는다는 점입니다. 바로 다음 P5-12.2에서는 실행 요청의 이름과 인자 구조를 보고, P5-13에서는 여러 실행과 읽기의 순서를, P5-14.2에서는 이 실행 흐름의 규칙과 기록 구조를 봅니다.
