...
순환 신경망 계열 구조는 현재 입력만 보지 않고, 앞에서 본 정보를 어느 정도 이어받아 순차 데이터(sequence data)를 처리하려는 신경망이다.
...
장기 의존성(long-term dependency) 문제는 P5-12.2에서 이어서 더 집중해서 다루고, attention 이후 구조와의 연결은 그다음 장에서 짧게 이어집니다. 세부 게이트 수식과 BPTT의 상세 전개는 이 책의 현재 본편 범위 밖에 둡니다.
...
여기서는 RNN, LSTM, GRU를 Part 5 본류의 `순차 상태 손잡이`로 먼저 잡고, 그 다음에 장기 의존성과 attention이 어떤 한계와 전환을 보여 주는지 같이 보는 편이 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| RNN / LSTM / GRU | 이전 입력의 흔적을 어떻게 상태로 넘기고 관리할 것인가? | P5-12.1 |
| 장기 의존성 | 그 상태 전달이 왜 먼 단서 앞에서 약해지기 쉬운가? | P5-12.2 |
| attention | 상태를 넘기는 대신 필요한 위치를 직접 다시 참고할 수 있는가? | P5-13.1 |
...
여기서 먼저 읽는 것은 optimizer나 regularization 같은 학습 절차가 아니라, 이전 입력의 흔적을 현재 판단까지 어떻게 이어 가는가라는 상태 전달 구조입니다.

| 지금 이 절에서 읽는 것 | 바로 다음 절이나 뒤 장으로 넘기는 것 |
| --- | --- |
| 이전 입력의 흔적을 어떤 상태(state)로 이어받아 현재 판단에 반영하는가 | 그 상태 전달이 왜 먼 단서 앞에서 약해지기 쉬운가 |
| RNN, LSTM, GRU가 왜 같은 계열 안에서 갈라졌는가 | attention이 상태 전달 대신 어떤 직접 참조 방식을 여는가 |
...
설비 경보음 인식에서 한 순간의 짧은 파형만 잘라 보면, 그 소리가 `압력 경보`의 시작부인지 `온도 복귀` 안내음의 일부인지 분명하지 않을 수 있습니다. 사람도 아주 짧은 경보음 조각만 따로 들으면 비슷한 패턴을 헷갈리기 쉽고, 앞소리와 뒷소리를 함께 들어야 어떤 알림인지 또렷해지는 경우가 많습니다. 예를 들어 짧은 상승음 하나만 보면 두 경보가 비슷하게 들릴 수 있지만, 뒤에 이어지는 반복 리듬이나 하강음 패턴까지 같이 들어야 종류를 더 안정적으로 구분할 수 있습니다. 즉, 현재 시점 소리 하나만으로는 부족하고 앞뒤 시간 맥락이 같이 필요합니다. 순차 상태 구조는 이전에 들은 신호를 이어받아 현재 해석에 반영하는 데 자연스럽습니다.
그래서 이 사례에서 확인해야 할 결과는 현재 소리 조각 하나만으로 섣불리 라벨을 붙이지 않고, 앞뒤 반복 리듬과 하강 패턴이 함께 남아 경보 종류가 실제로 더 안정적으로 갈라지는가입니다.
...
그래서 이 사례에서 확인해야 할 결과는 같은 마지막 값 `80`이라도 직전 상승 추세와 일시 튐이 실제로 다른 상태로 남아, 최종 경보 판단이 서로 다르게 갈라지는가입니다.

세 사례에서 공통으로 확인해야 할 결과는 현재 입력 하나보다 앞 흐름이 함께 남아 있어야 한다는 점입니다. 운영 메모 해석에서는 승인/보류 판단이 뒤의 부정과 앞의 위험 단서를 함께 반영하는지, 설비 경보음 인식에서는 짧은 소리 조각이 앞뒤 연결 속에서 더 안정적으로 해석되는지, 시계열 예측에서는 같은 마지막 값도 직전 추세에 따라 다른 경보로 나뉘는지를 보면 충분합니다.
...
| 장면 | 사람이 먼저 보기 쉬운 결과 | 순차 상태 관점에서 실제로 구분해야 할 것 | 바로 다음에 확인할 것 |
| --- | --- | --- | --- |
| 운영 메모 해석 | 마지막 승인 단어만 비슷하면 같은 조치처럼 보입니다. | 앞에서 누적된 위험/보류 흐름이 마지막 해석을 바꿀 수 있습니다. | 같은 마지막 단어도 앞 문맥에 따라 라벨이 갈라지는지 봅니다. |
| 설비 경보음 인식 | 지금 들리는 짧은 소리 조각만 보면 충분해 보입니다. | 직전 경보 리듬이 함께 남아야 현재 조각 해석이 더 안정됩니다. | 같은 소리 조각이 앞뒤 연결에 따라 다르게 읽히는지 봅니다. |
| 시계열 예측 | 마지막 값이 같으면 같은 경보가 나와야 할 것처럼 보입니다. | 직전 여러 step의 추세가 함께 상태에 남아야 현재 값 의미가 달라집니다. | 같은 마지막 값도 직전 상승/일시 튐 흐름에 따라 경보가 갈라지는지 봅니다. |
...
센서 예제는 입력값과 누적 상태를 같은 시간축에 놓고 보면 더 직접 보입니다.

![같은 마지막 값 80에서도 지속 상승과 일시 튐의 누적 상태가 다르게 남는 그래프](/AiBook/assets/part-05/chapter-12/sequence-state-accumulation-ko.png)

두 시계열은 마지막 입력값이 모두 `80`이지만, 누적 상태는 같은 위치에 도착하지 않습니다. 지속 상승은 이전 step의 상태가 계속 쌓여 마지막에 경보 기준을 넘고, 일시 튐은 중간에 낮은 값이 이어져 마지막 입력이 같아도 상태가 기준 아래에 남습니다. 이 그래프에서 읽을 점은 RNN 계열의 출발 직관이 `마지막 값 하나`가 아니라 `이전 step들이 남긴 상태와 현재 입력을 함께 본다`는 데 있다는 것입니다.
...
| 두 시계열의 마지막 state가 다르다 | 중간 값을 더 올리거나 낮춰 `지속 추세`와 `일시 튐`이 어디서 갈라지는지 본다 | 이 축약 상태 업데이트 식 하나로 LSTM·GRU 내부 게이트 전체를 대체하지 않는다 |
...
- 바로 앞의 P5-11.2 CNN이 `공간 구조`를 다뤘다면, 이제는 `시간 순서와 누적 문맥`이 중요한 데이터를 어떻게 다루는지 봐야 하고
- 순차 모델이 왜 필요한지 가장 직관적으로 보여 주고
- 장기 의존성 문제가 왜 중요한지 설명하며
- Transformer가 무엇을 대체하고 무엇을 개선했는지 비교 기준을 제공하기 때문입니다

여기서 한 번 멈추고, `언제 입력 종류보다 순차 상태 관점부터 먼저 읽어야 하는가`를 짧게 고정해 두면 다음 장기 의존성 절로 넘어갈 때 기준선이 덜 흔들립니다.

| 먼저 떠올릴 질문 | RNN 계열 관점이 먼저 필요한 이유 | 바로 다음 절에서 이어질 것 |
| --- | --- | --- |
| 같은 마지막 입력인데 왜 해석이 달라지는가 | 현재 값 하나보다 직전까지 누적된 상태가 함께 판단을 바꾸기 때문 | 그 상태가 왜 먼 단서 앞에서 약해지는가 |
| 문장, 음성, 시계열에서 왜 앞 흐름을 함께 봐야 하는가 | 순서와 문맥이 없는 해석으로는 현재 입력 의미가 자주 흔들리기 때문 | 장기 기억 유지 문제와 attention 전환 |
| 왜 LSTM과 GRU를 basic RNN 다음에 배우는가 | 순차 상태 아이디어 자체보다 그 상태를 얼마나 오래 안정적으로 유지할지가 곧 핵심 문제가 되기 때문 | 장기 의존성의 구체적 등장 장면 |
