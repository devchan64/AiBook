...
| --- | --- | --- |
| RAG 근거 연결 | 무엇을 근거로 답할까? | P5-10.1, P5-10.2, P5-11.1, P5-11.2 |
| tool use 실행 연결 | 무엇을 실제로 조회하거나 실행할까? | P5-12.1 |
| agent 목표 흐름 | 읽기와 실행을 어떤 순서로 이어 갈까? | P5-13 |

즉, 지금 장의 핵심은 `문서를 읽는가`에서 `외부 기능으로 실제 결과를 가져오는가`로 손잡이가 바뀌는 데 있습니다. 바로 다음 P5-12.2에서는 실행 요청의 이름과 인자 구조를 보고, P5-13에서는 여러 실행과 읽기의 순서를 봅니다.
...
연속 독해에서 Chapter 10~14가 모두 `외부 연결 기술`처럼 한 덩어리로 보일 수 있으므로, 지금 절에서는 최소 차이를 아래처럼 한 번 더 짧게 고정해 두는 편이 좋습니다.
...
| function calling | 실행 요청을 어떻게 검증 가능한 payload로 만들까? | 여러 실행과 읽기를 어떤 목표 순서로 이어 갈까 |
| agent | 여러 실행과 읽기를 어떤 목표 흐름으로 이어 갈까? | 그 흐름을 어떤 연결 형식과 실행 기록 안에 둘까 |

이 표의 목적은 용어를 더 외우게 하는 데 있지 않습니다. 현재 절에서는 `문서를 읽는 단계`, `실제 기능을 호출하는 단계`, `호출을 구조화하는 단계`, `여러 단계를 목표 흐름으로 잇는 단계`가 서로 다른 층위라는 점만 분명히 잡으면 충분합니다.
...
tool use는 `무엇을 실제로 조회하거나 실행할까`를 먼저 닫는 구조입니다. 외부 기능으로 실행을 위임하고, P5-12.2에서는 그 요청을 함수 이름과 인자 구조로 어떻게 넘길지 더 봅니다. 그다음 P5-13에서는 agent 흐름으로 넘어갑니다.
