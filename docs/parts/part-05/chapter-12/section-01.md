...
처음 읽을 때는 도구 이름을 많이 외우기보다 `지금 필요한 것이 문서 읽기인가 실제 실행인가`, `무엇을 조회하거나 계산하거나 실행해야 하는가`, `그 실행 결과를 바로 다음 장에서 어떤 호출 구조로 넘길 것인가`라는 세 질문만 먼저 붙잡으면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| tool use가 문서 읽기를 넘어서 실제 결과를 가져오는 구조라는 점 | P5-12.2에서 그 실행 요청을 어떤 이름과 인자 형식으로 넘길지로 이어집니다. | 이후 API 호출, 계산 도구, 데이터 조회, 실행 기록 설명에서 반복됩니다. |
| RAG와 tool use가 서로 다른 층위라는 점 | 검색과 실행 중 무엇이 먼저 필요한지 판단하는 기준으로 이어집니다. | 이후 agent 설계, 운영 실패 분석, 프로젝트 구조 설명에서 계속 중요합니다. |
| tool use가 `실행 연결`이고 agent가 `목표 흐름 연결`이라는 점 | P5-13에서 여러 읽기와 실행을 어떤 순서로 이어 갈지로 이어집니다. | 이후 MCP, 하네스, 승인 기록, 실행 추적 설명의 바탕이 됩니다. |
...
| --- | --- | --- |
| RAG: 무엇을 근거로 읽을까 | tool use: 무엇을 실제로 조회하거나 실행할까 | agent: 읽기와 실행을 어떤 순서로 이어 갈까 |
| 근거 문서 연결 | 실행 연결 | 목표 흐름 연결 |

즉, 지금 장의 핵심은 `문서를 읽는가`에서 `외부 기능으로 실제 결과를 가져오는가`로 손잡이가 바뀐다는 점입니다.

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| RAG | 어떤 문서를 근거로 붙여 답의 재료를 고를 것인가 |
| tool use | 무엇을 실제로 조회하거나 실행해 결과를 가져올 것인가 |
| agent | 읽기와 실행을 어떤 순서로 이어 갈 것인가 |
...
| 지금 tool use에서 먼저 남길 기록 | 왜 지금 필요한가 | 뒤 절과 Part 6에서 다시 읽는 기록 |
| --- | --- | --- |
| `tool_name`, `arguments`, `result` | 무엇을 실행하려 했고 실제로 어떤 값이 돌아왔는지 남겨야 추측 답변과 실행 결과를 구분할 수 있어서 | P5-12.2의 함수 호출 형식, P5-14.2의 trace/log, Part 6의 `execution_records`로 이어진다 |
| `approved`, 실패 이유 | 실행을 바로 진행해도 되는지와 어디서 멈췄는지 남겨야 권한 문제와 운영 실패를 다시 볼 수 있어서 | P5-14.2의 approval 기록, P5-16.2의 실패 대응, Part 6의 `incident_records`, `next_action`으로 이어진다 |
