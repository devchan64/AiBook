...
| 문장 예제에서 각 줄 `state=`가 음수였다가 다시 양수로 바뀐다 | 단어가 순서대로 들어오며 앞 의미를 뒤 단어가 수정할 수 있다는 뜻 | `지루`나 `않았다` 신호 값을 바꾸면 부정 흐름이 얼마나 강하게 뒤집히는지 볼 수 있습니다 |
...
| 문장 state가 중간에 음수였다가 마지막에 양수로 돌아선다 | `지루`, `않았다` 신호를 바꿔 부정 표현과 뒤집힘이 얼마나 민감한지 본다 | 단어 신호 몇 개만으로 실제 언어 이해 전체를 설명한다고 단정하지 않는다 |
| 두 시계열의 마지막 state가 다르다 | 중간 값을 더 올리거나 낮춰 `지속 추세`와 `일시 튐`이 어디서 갈라지는지 본다 | 이 장난감 상태 업데이트 식 하나로 LSTM·GRU 내부 게이트 전체를 대체하지 않는다 |
...
이 질문은 바로 P5-12.2 장기 의존성(long-term dependency)으로 이어집니다.
...
- 지금 보는 차이가 `입력 종류 차이`인지, 아니면 `현재 입력을 이전 상태와 함께 읽어야 하는가`의 차이인지 구분해 말할 수 있는가?
...
- 입력 종류보다 앞뒤 순서와 누적 문맥이 더 중요해 보일 때, 순차 상태 관점을 먼저 떠올립니다.
- 같은 마지막 입력인데 해석이 왜 달라지는지 설명해야 할 때, 현재 입력과 이전 상태를 함께 읽는 구조를 다시 봅니다.
- LSTM과 GRU를 다른 이름 나열로만 읽고 있을 때, 상태를 더 오래 안정적으로 유지하려는 보강 구조라는 점을 꺼냅니다.
