...
이 절에서는 구현 최적화 자체를 익히기보다, 관련 문서를 읽을 때 이름 때문에 멈추지 않게 하는 데 집중합니다. context window와 실제 사용 제약은 본류인 P5-3.2 context window에서 이미 연결했고, KV cache가 운영 지연 시간과 비용에 미치는 영향은 P5-16.1 서비스 운영 제약에서 다시 회수합니다. multi-head attention과 위치 표현의 구현 수식 증명, 장문맥 전용 아키텍처의 벤치마크 경쟁은 현재 본편 범위 밖으로 둡니다.
