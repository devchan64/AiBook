...
| 사람이 먼저 보기 쉬운 기준 | 출력층 해석으로 다시 읽는 기준 |
| --- | --- |
| 0.91이면 곧바로 `정지 결정`이라고 본다 | 0.91은 먼저 모델 출력이고, 정지는 그 뒤 정책 단계에서 정한다 |
| 출력 숫자와 운영 규칙을 한 덩어리로 본다 | sigmoid 점수 해석과 임계값 정책을 분리해서 본다 |
| 소수값 하나면 충분하다고 느끼기 쉽다 | 같은 점수라도 설비마다 다른 threshold가 붙을 수 있다 |
...
코드를 보기 전에 먼저 세 문제의 출력을 어떻게 읽어야 할지 예상해 보면 좋습니다.
...
이 표의 목적은 코드를 돌리기 전에 `같은 출력 계산처럼 보여도 문제 유형마다 해석 단위가 다르다`는 점을 먼저 붙잡는 데 있습니다.
...
energy_prediction = 12.7

alarm_logits = [2.0, 0.3]
alarm_scores = [sigmoid(x) for x in alarm_logits]

quality_labels = ["normal", "scratch", "contamination"]
quality_logits = [1.2, 2.4, 0.3]
quality_probs = softmax(quality_logits)
top_label = quality_labels[quality_probs.index(max(quality_probs))]

print("regression output =", round(energy_prediction, 3))
print("binary scores =", [round(v, 3) for v in alarm_scores])
print("multiclass scores =", [round(v, 3) for v in quality_probs])
...
| 출력에서 보인 차이 | 먼저 떠올릴 출력 해석 | 이유 |
| --- | --- | --- |
| 숫자 하나가 그대로 남아 있다 | 회귀 출력 | 값 자체를 정답 숫자와 바로 비교해야 하기 때문입니다. |
| 0~1 사이 점수가 따로 나온다 | 이진 분류 출력 | 양성 클래스 쪽 기울기와 정책 임계값을 함께 읽어야 하기 때문입니다. |
| 여러 후보 점수가 함께 나오고 1등이 정해진다 | 다중 분류 출력 | 후보 사이 경쟁 구조와 점수 차이를 같이 읽어야 하기 때문입니다. |

| 운영 판단 기준 | 출력 숫자만 보면 쉬운 판단 | 출력 해석까지 보면 바뀌는 판단 |
| --- | --- | --- |
| `regression output = 12.7` | 숫자 하나가 나왔으니 그냥 큰지 작은지만 보면 된다고 느끼기 쉽다 | 실제로는 정답 사용량과의 거리, 예측 오차 허용 범위를 먼저 비교해야 하는 회귀 출력이라고 읽게 된다 |
| `binary scores = [0.881, 0.574]` | 두 값 중 큰 쪽만 보고 바로 정지/통과를 확정하고 싶어진다 | 0~1 점수는 먼저 모델 출력이고, 그 뒤에 `자동 차단`, `현장 확인`, `기록만` 같은 임계값 정책이 따로 붙는다고 읽게 된다 |
| `multiclass scores = [0.214, 0.713, 0.072]` | `scratch`가 1등이니 바로 끝났다고 느끼기 쉽다 | 다른 후보와의 점수 차이까지 봐야 자동 확정인지, 사람 재검토로 넘길지 판단할 수 있다고 읽게 된다 |
...
출력 뒤 해석을 실제 오판 위험까지 다시 묶으면 차이가 더 또렷해집니다.

| 문제 유형 | 덜 나쁜 오독 | 더 위험한 오독 | 지금 먼저 확인해야 할 것 |
| --- | --- | --- | --- |
| 회귀 | 숫자의 크기만 대략 본다 | 회귀 출력을 확률이나 범주처럼 읽는다 | 정답과의 거리와 허용 오차 |
| 이진 분류 | 양성 쪽 점수라고만 본다 | 모델 점수를 곧바로 정책 결정으로 본다 | threshold와 중간 분기 정책 |
| 다중 분류 | 1등 후보 이름만 먼저 본다 | 1등과 2등이 거의 붙었는데도 자동 확정한다 | 후보 간 점수 차이와 재검토 기준 |
