...
이 질문은 바로 P5-4.1 손실 함수(loss function)로 이어집니다.
...
- 은닉층 활성화와 출력층 활성화가 왜 같은 기준으로 읽히지 않는지 설명할 수 있는가?
- 회귀, 이진 분류, 다중 분류에서 출력값을 각각 어떻게 해석해야 하는지 말할 수 있는가?
- 이 절 다음에는 `정답과 얼마나 다른가`를 읽는 손실 함수 장으로 넘어간다는 흐름을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 같은 활성화 함수라는 이유로 은닉층과 출력층을 같은 방식으로 설명하고 있을 때, 출력 해석 문제를 따로 분리해 떠올립니다.
- 회귀, 이진 분류, 다중 분류의 마지막 출력 의미가 섞일 때, 문제 유형에 맞는 출력 모양과 해석을 다시 봅니다.
- sigmoid나 softmax 출력이 곧바로 서비스 정책이라고 오해하기 쉬울 때, 모델 점수와 운영 결정을 분리하는 관점을 꺼냅니다.
