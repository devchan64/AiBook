...
- sigmoid, tanh, ReLU가 모두 비슷한 이름처럼만 들릴 때, 출력 범위와 반응 방식 차이부터 다시 떠올립니다.
- ReLU가 자주 쓰인다는 사실만 남고 비교 기준이 사라질 때, 역사적 비교 기준으로 세 함수를 함께 다시 봅니다.
- 함수 선택을 마친 뒤 출력층 해석과 손실로 넘어가야 할 때, 이 절을 반응 방식 정리 구간으로 다시 꺼냅니다.
