...
대표 활성화 함수의 차이는 P5-3.2부터 P5-3.5까지 이어서 다루고, 출력층 활성화와 손실 함수의 연결은 P5-3.6과 P5-4.1, P5-4.2에서 다시 봅니다. gradient flow의 더 자세한 맥락은 P5-5.1과 P5-7.1에서 다시 연결합니다. 즉, 이번 절은 활성화 함수가 왜 `선형 계산만 반복되는 구조`를 실제 표현력으로 바꾸는지 먼저 닫습니다.
...
- 오픈체크리스트의 중심축 문장인 "활성화 함수가 비선형성을 넣는 이유를 잡아야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
이 흐름을 연습에서 쓰는 수치로 옮기면, 활성화 전 점수 \(z\)와 활성화 뒤 값 \(a\)의 차이는 다음처럼 보입니다.

![활성화 전후 운영 신호 비교](../../../assets/part-05/chapter-03/activation-case-before-after-ko.svg)

이 그래프에서 음수였던 두 상태는 활성화 뒤 `0`에 가까워지고, 경계 바로 위 신호는 작게 남으며, 강한 경고 신호는 큰 값으로 유지됩니다. 같은 운영 신호라도 다음 층이 실제로 받는 값은 활성화 전후에 달라집니다.
...
앞의 그래프를 다시 보면 이 표의 차이가 더 빨리 보입니다. `z`는 선형 결합이 만든 원래 위치이고, `a`는 활성화 뒤 다음 층이 실제로 받는 위치입니다.
...
이 세 조작은 활성화 뒤 값만 놓고 보면 다음처럼 달라집니다.

![입력 조정에 따른 활성화 전후 변화](../../../assets/part-05/chapter-03/activation-experiment-shift-ko.svg)

편향을 낮추거나 복귀 지연 점수를 줄이는 조작은 경계 신호를 `0`으로 떨어뜨리고, 실패 가중치를 키우는 조작은 이미 강한 신호를 더 크게 남깁니다.
