...
| `recovery_dominant_signal` | `z`는 기준 아래, `a`는 거의 남지 않음 | 복구 지연이 거의 없고 실패도 없으면 다음 층에 거의 안 남을 수 있습니다. |
...
같은 네 상태를 간단한 활성화 관점으로 정리하면 다음처럼 읽을 수 있습니다. 여기서 목표는 특정 함수 이름을 외우는 것이 아니라, 활성화 전 점수 `z`와 다음 층에 실제로 남는 값 `a`가 같은 역할을 하지 않는다는 점을 보는 것입니다.

| 케이스 | z | 활성화 뒤 값 | 다음 층 입장에서 읽히는 상태 | 지금 읽어야 할 핵심 |
| --- | --- | --- | --- | --- |
| `brief_alarm_recovered` | `-0.66` | `0.0` | 잠깐 흔들렸지만 다음 층까지는 거의 안 가는 신호 | 경보가 한 번 있었어도 다른 신호가 약하면 거의 사라질 수 있다 |
| `recovery_dominant_signal` | `-0.82` | `0.0` | 거의 전달되지 않는 신호 | 기준 아래 구간은 다음 층에 거의 남지 않는다 |
| `borderline_warning_signal` | `0.42` | `0.42` | 간신히 살아남는 약한 신호 | 경계 바로 위의 약한 신호는 간신히 살아남는다 |
| `strong_warning_signal` | `4.1` | `4.1` | 다음 층이 계속 활용하기 쉬운 강한 신호 | 충분히 큰 신호는 강하게 남는다 |
...
여기서 한 번 더 학습 관점으로 정리하면, 이 연습의 핵심은 숫자 계산 재현이 아니라 `경계가 어디에 있는가`, `경계 바로 위 신호와 충분히 큰 신호를 왜 다르게 읽어야 하는가`, `거의 사라진 신호를 왜 다음 층이 사실상 못 보는가`를 스스로 설명하는 데 있습니다.

| 입력을 바꿔 볼 축 | 같이 달라질 것으로 예상할 결과 | 지금 떠올릴 질문 |
| --- | --- | --- |
| 편향 \(b\)를 더 낮춘다 | `borderline_warning_signal`까지 거의 사라질 가능성이 커진다 | 경계가 뒤로 밀리면 약한 이상 신호를 얼마나 놓치게 되는가 |
| `재기동 실패 횟수` 가중치를 더 키운다 | 실패가 있는 케이스만 더 빠르게 강한 신호로 살아남는다 | 어떤 입력을 더 민감하게 보도록 모델이 바뀌는가 |
| `압력 복귀 지연 시간`을 줄인다 | 경계 근처 케이스가 다시 0 아래로 내려갈 수 있다 | 약한 경고와 회복 신호를 어디서 갈라 읽을 것인가 |
