...
지금 읽는 층위는 `본류 복습 뒤의 역사 배경 층위`입니다. Chapter 1~17에서 이미 생성 구조, 학습 조정, 서비스 연결을 본 뒤에야 이 절은 더 안정적으로 읽힙니다. 따라서 여기서는 새 개념 본류를 앞에서 다시 여는 것이 아니라, 이미 읽은 생성형 AI 구조가 어떤 직접 계보 위에 서 있는지 뒤에서 정리합니다.

따라서 이 장을 먼저 펼쳤더라도, 낯선 이름을 다 따라가기보다 `본류를 읽고 다시 돌아와 비교 기준을 정리하는 장`으로 받아들이는 편이 안전합니다.

처음 읽을 때는 특히 다음 기준만 먼저 고정해 두면 충분합니다.

| 먼저 끝내야 하는 본류 | 이 배경 축에서 다시 보는 것 |
| --- | --- |
| 토큰, Transformer, GPT, 학습 조정, RAG, agent | 그 본류가 어떤 발전사와 구조 전환 위에 서 있는가 |

재진입 기준을 더 짧게 잡으면 다음처럼 읽어도 충분합니다.

| 지금 상태 | 이 장을 바로 깊게 읽어야 하는가 | 먼저 돌아갈 위치 |
| --- | --- | --- |
| Transformer, GPT, 사전학습 흐름이 아직 섞여 보인다 | 보통 아니다 | P5-3.1, P5-4.1, P5-6.1 |
| RAG, tool use, agent까지 본류를 한 번 다 읽었다 | 그렇다 | 지금 장에서 직접 계보를 다시 정리 |
| 역사 이름은 많은데 구조 전환 이유가 잘 안 잡힌다 | 그렇다 | 지금 장에서 `빈도 -> 표현 -> 순서 -> attention -> Transformer` 순서만 다시 확인 |

즉, 이 장은 새로운 본편을 여는 입구가 아니라 `본류를 한 번 읽은 뒤 구조 전환 이유를 다시 붙잡는 복습형 배경 축`으로 읽는 편이 안전합니다.
...
각 논문의 수식 전개와 모델별 벤치마크 세부 비교는 여기서 다루지 않습니다. 대신 이 절에서 잡은 큰 흐름은 바로 다음 P5-18.2에서 `직접 계보와 주변 근거`로 다시 정리하고, 구조 차이는 P5-3.1, P5-19.1, P5-4.1에서 다시 회수합니다. 최신 상용 모델 계보 전수조사는 현재 판의 본편 범위 밖에 둡니다.

이 절에서는 세부 구조를 모두 외우기보다, Part 5 본류를 읽은 뒤 그 흐름을 다시 정리할 수 있게 해 주는 최소한의 `역사 지도`를 만듭니다.

즉, 이 절은 Chapter 1~17의 본류 설명을 대신하는 새 출발점이 아니라, 그 본류를 다 읽은 뒤 `현재 생성형 AI 구조가 어떤 직접 계보 위에 서 있는가`를 뒤에서 다시 붙여 보는 배경 축입니다. 처음 읽을 때는 이 장을 `새 본류 시작`으로 보기보다 `본류를 더 정확히 해석하게 만드는 역사 정리`로 읽는 편이 안전합니다.

이 배경 축에서는 Python 예제도 `역사를 구현하는 코드`가 아니라 `빈도 -> 표현 -> 순서 -> attention`처럼 계산 관점이 어떻게 넓어졌는지 비교하는 최소 예제로만 둡니다. 따라서 여기서 예제를 더 늘리거나 복잡한 구현 실습으로 확장하지 않고, 한 번의 비교로 구조 전환 이유를 읽게 하는 수준에 머무는 편이 현재 Part 5 흐름에 더 맞습니다.
...
이 절에서 확인해야 할 결과는 앞서 읽은 P5-4.1 GPT, P5-5.1 다음 토큰 예측, P5-6.1 사전학습 같은 본류 설명을 단순 기능 나열이 아니라, 각 구조가 어떤 한계를 메우며 다음 단계로 이어졌는지의 흐름으로 다시 읽을 수 있게 되는가입니다.
