...
P5-18.1에서는 AI 서비스가 품질만으로는 성립하지 않고, 비용, 지연 시간, 사용량 제한, 운영 복잡도 안에서 설계되어야 한다는 점을 보았습니다. 그러면 이제 마지막으로 더 직접적인 질문이 남습니다.
...
- AI 서비스 실패 유형을 초심자 수준에서 설명할 수 있습니다.
...
초심자 기준에서는 다음처럼 한 줄로 다시 묶으면 좋습니다.
...
답변이 틀렸을 때, 검색 문서가 부정확했는지, 모델이 문서를 잘못 요약했는지 trace가 있어야 구분할 수 있습니다.

### 사례 2. 에이전트 도구 호출 실패

파일 읽기 도구가 실패했는데도 에이전트가 다음 단계로 넘어가면 더 큰 오류를 만들 수 있습니다. 이 경우 stop condition과 retry 정책이 중요합니다.

### 사례 3. 느린 응답

답변 내용은 맞지만 20초가 걸리면 사용자 경험은 나쁠 수 있습니다. 이 경우 실패는 `틀린 답`이 아니라 `서비스 품질 실패`로 읽어야 합니다.
...
- Part 6 프로젝트에서 단순 기능 구현이 아니라 실패 대응까지 설계하게 만들며
...
- 이 절은 Part 6 프로젝트에서 실제 설계 판단으로 이어지는 마지막 연결 절입니다.

## 체크리스트

- AI 서비스 실패 유형을 초심자 수준에서 설명할 수 있는가?
