...
모든 주변 분야 논문 열거와 음성, 비전, 추천 모델의 상세 발전사는 여기서 다루지 않습니다. 대신 이 절의 직접 계보 구분은 앞서 읽은 P5-3.1, P5-4.1과 뒤의 P5-19.1에서 Transformer, GPT, BERT를 구조 비교로 다시 읽을 때 쓰입니다. 음성, 비전, 멀티모달 계보 전체를 따로 따라가는 일은 이 책의 현재 본편 범위 밖에 둡니다.
...
| --- | --- | --- |
| Transformer, GPT, 사전학습, RAG까지의 생성 중심 흐름 | 무엇이 LLM의 직접 계보고 무엇이 딥러닝 확산의 주변 근거인가 | P5-3.1, P5-4.1, P5-19.1 |
...
이 구분이 있어야 Part 5에서 다룬 BERT, GPT, pretraining, prompt, RAG, agent를 다시 떠올릴 때 구조와 분위기를 섞지 않게 됩니다.
