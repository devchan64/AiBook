...
| 사람이 먼저 보기 쉬운 기준 | regularization 관점으로 다시 읽는 기준 |
| --- | --- |
| 오차만 줄면 좋은 해답이라고 느끼기 쉽다 | 같은 오차 감소라도 더 과격한 해와 덜 과격한 해를 구분해야 한다 |
| 큰 가중치는 그만큼 중요한 특징이라고 보기 쉽다 | 특정 특징 하나에 지나치게 몰린 의존일 수 있다 |
| 훈련 점수가 더 높으면 더 낫다고 생각하기 쉽다 | 검증 흔들림과 민감도까지 함께 봐야 한다 |
...
네 사례를 같이 놓고 보면 regularization을 `벌점 한 줄`보다 `어떤 해를 덜 선호하게 만드는 기준`으로 읽어야 하는 이유가 더 분명해집니다.

| 장면 | 사람이 먼저 보기 쉬운 결과 | regularization 관점에서 실제로 구분해야 할 것 | 바로 다음에 확인할 것 |
| --- | --- | --- | --- |
| 훈련 성능은 높은데 검증 성능이 떨어짐 | 더 잘 맞췄으니 더 좋은 모델이라고 보기 쉽습니다. | 훈련 적합과 일반화는 다른 질문이며, 검증 붕괴를 줄이는 쪽이 더 좋은 해일 수 있습니다. | 검증 성능 하락이 실제로 덜 심해졌는지 봅니다. |
| 파라미터가 지나치게 커짐 | 큰 가중치는 중요한 특징을 잘 잡았다는 뜻처럼 보일 수 있습니다. | 일부 특징 하나에 과하게 몰린 불안정한 해일 수 있습니다. | 입력이 조금 변해도 예측이 과격하게 뒤집히지 않는지 봅니다. |
| 입력 스케일 조정과 과적합 억제를 같은 것으로 봄 | 둘 다 `정규화`처럼 들리니 같은 일이라고 보기 쉽습니다. | normalization은 값 스케일 정리, regularization은 해의 복잡성 제약이라는 다른 질문입니다. | 스케일 문제와 과적합 문제가 서로 따로 남는지 봅니다. |
| 데이터가 적은 문제 | 샘플이 적으면 더 강하게 맞추면 된다고 보기 쉽습니다. | 데이터가 적을수록 우연한 잡음을 외우기 쉬워 regularization 필요성이 더 커집니다. | 새 입력에서 성능 붕괴가 덜한지 봅니다. |

여기서 regularization의 핵심 비교를 한 장면으로 더 압축하면, `둘 다 훈련 데이터에는 비슷하게 맞지만 한쪽이 더 큰 가중치와 더 복잡한 경로를 쓰는 경우`를 떠올리면 됩니다.
...
| regularization이 덜 불편해하는 쪽 | 아님 | 맞음 |
