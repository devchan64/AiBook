...
- 개념은 알겠는데 순서가 자꾸 섞일 때, forward -> loss -> backward -> update의 공통 학습 루프를 먼저 떠올립니다.
- mode나 regularization이 부가 설정처럼 느껴질 때, 이 요소들이 루프 안에서 어떻게 작동하는지 다시 묶어 봅니다.
- 뒤 장의 구조 설명으로 넘어가기 전에 공통 학습 뼈대를 다시 고정해야 할 때, 이 절을 기준선으로 씁니다.
