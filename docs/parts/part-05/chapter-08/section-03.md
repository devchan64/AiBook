...
분산 학습 루프, mixed precision, gradient accumulation은 이 책의 현재 본편 범위 밖으로 두고, 여기서는 단일 학습 루프의 공통 골격만 먼저 잡습니다. autograd 엔진의 내부 구현도 범위 밖으로 두되, gradient가 optimizer update로 이어지는 관점 자체는 앞선 P5-5.1, P5-5.2와 P5-7.1, P5-7.2에서 이미 회수한 흐름 위에서 다시 묶습니다.
...
이 질문은 P5-9.1 GPU와 병렬 처리로 이어집니다.
