...
inverted dropout 공식과 dropout 비율 조정 경험칙은 여기서 공식을 전개하지 않고 넘깁니다. 대신 학습 모드와 평가 모드의 차이는 P5-6.2에서 다시 연결하고, regularization의 큰 관점은 P5-8.1에서 이미 묶은 흐름 위에서 읽습니다. variational dropout, Monte Carlo dropout 같은 확장 기법은 이 책의 현재 본편 범위 밖에 둡니다.
...
P5-6.2에서 이미 본 것처럼, 드롭아웃은 학습 모드(training mode)와 평가 모드(evaluation mode)에서 다르게 동작합니다. 이유는 간단합니다.
...
| --- | --- |
| 표현을 많이 쓰면 무조건 더 좋을 것 같다 | 특정 경로 하나에 몰린 의존은 오히려 일반화를 해칠 수 있다 |
| 훈련 점수가 빨리 오르면 좋은 학습처럼 보인다 | 검증 간극이 줄어드는지도 같이 봐야 한다 |
| 강한 노드 하나가 있으면 더 효율적일 것 같다 | 일부 노드가 쉬어도 버티는 표현이 더 견고할 수 있다 |
...
세 사례를 같이 놓고 보면 dropout을 `노드 몇 개를 0으로 만드는 기법`보다 `특정 경로 의존을 흔들어 일반화를 압박하는 규칙`으로 읽어야 하는 이유가 더 분명해집니다.

| 장면 | 사람이 먼저 보기 쉬운 결과 | dropout 관점에서 실제로 구분해야 할 것 | 바로 다음에 확인할 것 |
| --- | --- | --- | --- |
| 은닉층이 큰 분류 모델 | 표현을 많이 쓰면 무조건 더 좋다고 보기 쉽습니다. | 특정 지름길 경로에 몰린 의존은 검증 성능을 해칠 수 있습니다. | 훈련-검증 간극이 실제로 줄어드는지 봅니다. |
| 데이터가 상대적으로 적은 경우 | 큰 모델이면 적은 데이터도 더 잘 맞출 수 있다고 보기 쉽습니다. | 샘플이 적을수록 우연한 패턴 암기를 흔드는 장치가 더 중요합니다. | 새 문서에서 기준이 덜 무너지는지 봅니다. |
| 완전연결층 중심 구조 | 큰 마지막 층은 조합을 많이 배우니 더 유리해 보입니다. | 일부 노드가 특징 조합을 통째로 외우지 못하게 흔들어야 할 수 있습니다. | 훈련 점수만 급등하고 검증 점수가 뒤처지는 패턴이 완화되는지 봅니다. |
...
- 앞서 P5-6.2에서 본 학습 모드와 평가 모드의 차이가 왜 실질적으로 필요한지 다시 확인시켜 주기 때문입니다
...
| training/eval mode 차이가 왜 실질적인지 다시 보여 줄 필요가 있다 | dropout이 모드 차이를 가장 직관적으로 드러내는 사례이기 때문입니다. | P5-8.3에서 학습 절차 전체와 다시 연결합니다. |
