...
이 질문은 바로 P5-8.3 학습 루프를 한 번에 다시 묶기로 이어집니다.
...
- 왜 평가 모드에서는 같은 무작위 제거를 계속 유지하지 않는지 말할 수 있는가?
- 이 절 다음에는 손실, optimizer, mode, regularization을 한 번에 다시 묶는 학습 루프 절로 넘어간다는 흐름을 이해했는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 특정 경로나 노드 하나에 모델이 과하게 기대는 느낌이 있을 때, dropout 관점을 먼저 떠올립니다.
- training/eval mode 차이를 가장 직관적으로 다시 보여 줄 사례가 필요할 때, 무작위 경로 제거와 평가 모드 차이를 다시 꺼냅니다.
- 벌점 항만으로 regularization을 설명하기 좁을 때, 구조를 흔들어 일반화를 돕는 방식으로 dropout을 다시 봅니다.
