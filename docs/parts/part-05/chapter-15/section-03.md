...
텍스트 생성에서 샘플링(sampling)은 모델이 그럴듯하다고 본 여러 토큰 후보 중 실제 출력을 하나씩 꺼내는 과정이며, 이 방식은 결과의 다양성과 안정성에 직접 영향을 줍니다. 이미지 디퓨전 모델에서도 sampler라는 말이 쓰이지만, 이는 보이는 이미지 후보 목록에서 하나를 고르는 일이 아니라 노이즈 제거의 다음 잠재 상태로 이동하는 수치적 절차를 가리킵니다.

모델 점수와 실제 출력 선택을 다시 구분해야 할 때는 개념사전의 [샘플링(sampling)](../../../reference/concept-glossary-parts/07-siot.md#sampling) 항목을 기준으로 다시 읽습니다.

샘플링 결과를 기록할 때는 텍스트 생성에서는 `candidate_distribution`, `selection_rule`, `temperature_setting`, `top_k_or_top_p`, `selected_output`, `variation_count`를 분리합니다. 이미지 디퓨전에서는 `seed`, `sampler`, `steps`, `guidance`, `output_review`처럼 복원 경로와 결과 검토를 구분해 기록합니다. 두 경우 모두 학습된 분포와 실제 산출 과정은 구별해야 하지만, 텍스트용 top-k·top-p를 이미지 디퓨전의 동일한 제어값처럼 취급하면 안 됩니다.

## 후보 분포와 실제 출력 선택은 다르다

이 절에서 먼저 붙잡아야 할 핵심은 `생성 모델의 품질은 무엇을 배웠는가뿐 아니라, 그 분포에서 실제 산출물을 만드는 절차에도 크게 달려 있다`는 점입니다. P5-15.2가 모델이 후보들의 상대적 그럴듯함을 어떻게 남기는지 보았다면, P5-15.3은 텍스트에서는 이미 계산된 후보들 중 실제 출력을 어떤 절차로 꺼내는지 봅니다. 이미지 디퓨전에서는 같은 구분을 노이즈 복원 경로와 최종 이미지의 관계로 읽습니다.
...
| 이미지 디퓨전에서 seed·sampler·steps가 복원 경로와 결과 변주에 어떻게 연결되는가 | Stable Diffusion 구성요소와 조건 제어를 어떻게 분리해 기록하는가 |
...
| `stainless mixing tank with side valve and warning beacon` 프롬프트로 이미지를 생성함 | seed·sampler·steps를 고정하면 비슷한 복원 경로를 비교할 수 있다 | seed 또는 sampler를 바꾸면 핵심 조건은 유지하면서 조명, 시점, 배관 배치가 달라질 수 있다 |

즉, `모델이 어떤 후보를 높게 봤는가`와 `그 후보 중 무엇을 실제 출력으로 꺼냈는가`는 같은 문제가 아닙니다.

## 샘플링은 후보 분포에서 실제 출력을 어떻게 꺼내는가: 확인할 판단 기준

이 사례에서는 샘플링을 후보 분포에서 실제 출력을 꺼내는 절차로 설명하고, 다양성과 안정성의 균형이 왜 결과를 바꾸는지 보여 주는지 확인한다.
...
같은 관점은 이미지 생성에도 이어집니다. 다만 이미지 디퓨전에서 변주를 만드는 것은 문장 후보 중 하나를 고르는 장면이 아니라, seed와 sampler가 노이즈 복원 경로를 바꾸는 장면입니다. 이 절에서 붙잡을 핵심은 도메인 이름이 아니라, `학습된 분포와 실제 산출 절차를 구분해야 결과 변주의 원인을 읽을 수 있는가`입니다.
...
| 이미지 생성 | 초기 잠재 노이즈와 복원 경로 | 조건을 고정한 비교가 어려워진다 | 핵심 프롬프트는 유지하면서 어떤 제어값이 장면 변주를 만들었는가 |
...
세 사례를 같이 놓고 보면, 샘플링의 핵심은 `모델이 무엇을 배웠는가`를 다시 설명하는 데 있지 않고, 학습된 분포에서 실제 산출물로 가는 절차를 운영 맥락에 맞게 조절하고 기록하는 데 있다는 점입니다. 텍스트에서는 후보 선택이, 이미지 디퓨전에서는 복원 경로가 그 절차의 대표 장면입니다.
...
언어 모델(language model)은 대개 다음 토큰의 가능성을 계산하고, 이미지 디퓨전 모델은 잠재 노이즈를 여러 단계에 걸쳐 복원합니다. 이때 실제 출력은 계산된 분포와 산출 절차를 거쳐 나타납니다. 텍스트의 temperature·top-k·top-p와 이미지의 seed·sampler·steps는 모두 결과 변주와 관련될 수 있지만, 같은 내부 연산을 가리키는 말은 아닙니다.
...
- 텍스트 생성의 후보 선택과 이미지 디퓨전의 복원 경로를 같은 알고리즘처럼 섞지 않고 설명할 수 있는가?
