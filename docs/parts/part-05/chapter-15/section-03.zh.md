...
这主要是文本生成的直觉：模型从 token 候选中采样。图像 diffusion 模型也会使用 `sampler` 这个词，但它指的是移向下一去噪潜在状态的数值过程，不是从看得见的完整图像候选清单里选一个。两种情况都要区分学到的分布和实际产出物的过程，但文本的 temperature、top-k、top-p 和图像 diffusion 的 seed、sampler、steps 并不是同一种控制值。

## 文本采样和 diffusion 采样

| 视角 | 文本生成 | 图像 diffusion |
| --- | --- | --- |
| 一步中处理的对象 | 下一 token 候选 | 带噪声的潜在表征 |
| 主要控制示例 | temperature、top-k、top-p | seed、sampler、steps、guidance |
| 要比较的结果 | 措辞和结构变化 | 恢复路径和图像变化 |

两者都会改变可见结果，但比较时必须先把这些不同角色分开。

这个区分是避免把文本生成设置直接套到图像生成行为上的最低保护线。
...
| 图像 diffusion 中 seed、sampler、steps 怎样把恢复路径和图像变化连起来 | 怎样分开记录 Stable Diffusion 的组成部分和条件控制 |
...
| 按提示词 `stainless mixing tank with side valve and warning beacon` 生成图像 | 固定 seed、sampler、steps 后可以比较相近的恢复路径 | 改变 seed 或 sampler 后，核心条件可保留，而光线、视角、管路布局会变化 |
