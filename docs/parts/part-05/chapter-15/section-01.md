...
샘플링(sampling)과 출력 품질 변동은 바로 다음 P5-15.2에서 더 구체적으로 다루고, 텍스트 생성의 다음 토큰 예측은 뒤 Part에서 다시 연결합니다. diffusion, GAN, VAE의 세부 알고리즘 비교와 likelihood 수식 전개는 이 책의 현재 본편 범위 밖에 둡니다.
...
| 왜 분류 설명만으로는 챗봇 답변이나 자동완성을 설명하기 어려운가 | 출력이 범주명이 아니라 실제 문장, 이미지, 구성 자체이기 때문 | 출력 선택 절차와 사용자 경험 차이 |
...
| --- | --- | --- |
| `samples`에 `good`, `nice`, `sunny`가 함께 나타난다 | 출력이 정답 하나로 고정되지 않고 후보 분포에서 실제로 펼쳐진다는 뜻 | 반복 횟수를 늘리면 드문 후보도 어느 정도 빈도로 나타나는지 더 또렷하게 보입니다 |
| `counts`에서 `good`이 가장 많고 `sunny`가 가장 적다 | 상대적으로 높은 후보가 더 자주 선택되지만 낮은 후보도 완전히 사라지지 않는다는 뜻 | 가중치를 더 평평하게 하거나 더 뾰족하게 바꾸면 후보 다양성이 어떻게 흔들리는지 볼 수 있습니다 |

- 생성 모델은 항상 하나의 결과만 기계적으로 내지 않을 수 있습니다.
- 더 그럴듯한 후보가 실제로 더 자주 나오지만, 확률이 낮은 후보도 실제 출력 안에 남을 수 있습니다.
- 이 차이를 봐야 생성 문제를 `정답 라벨 하나 선택`이 아니라 `후보 분포에서 새 출력을 구성하는 일`로 읽게 됩니다.

이 예제도 한 번 보고 끝내기보다, 어떤 값을 바꿔 보면 후보 분포 감각이 더 선명해지는지 바로 이어서 확인하는 편이 좋습니다.

| 먼저 보인 출력 신호 | 지금 바로 해 볼 변화 | 아직 이 예제만으로 서두르지 않을 결론 |
| --- | --- | --- |
| `good`이 가장 많이 나온다 | `weights`에서 첫 후보 비중을 낮추거나 높여 가장 그럴듯한 후보가 얼마나 자주 바뀌는지 본다 | 가장 높은 후보가 자주 나온다고 해서 생성이 곧 결정론적이라고 단정하지 않는다 |
| `sunny`도 몇 번은 등장한다 | 샘플 수를 20보다 크게 늘려 낮은 확률 후보가 장기적으로 어떻게 나타나는지 본다 | 드문 후보가 나온다는 사실만으로 품질이 높다고 단정하지 않는다 |
| 세 후보가 실제 목록에 함께 남아 있다 | 후보를 더 추가하거나 제거해 출력 폭이 어떻게 달라지는지 본다 | 장난감 토큰 예제 하나로 이미지·음성·긴 문장 생성 전체를 바로 일반화하지 않는다 |
...
- 라벨 예측보다 새로운 출력 자체를 만들어야 하는 문제를 설명할 때, 생성 모델 관점을 먼저 떠올립니다.
- 분류와 생성 차이가 흐릴 때, 맞는 범주 선택과 자연스러운 후보 분포 구성의 차이를 다시 봅니다.
- 샘플링 장으로 넘어가기 전에 `무엇을 배웠는가`와 `무엇을 실제로 꺼낼 것인가`를 분리해야 할 때, 이 절을 기준선으로 씁니다.
