...
| --- | --- | --- |
| 운영 판단 기준 층위 | 하네스와 실행 기록 | 자동/사람 평가 분담, 비용·지연·실패 대응 |

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| 하네스: 실행을 어떻게 남기고 다시 설명할까 | LLM 평가: 그 실행과 답을 어떤 기준으로 통과 또는 수정 대상으로 볼까 | 자동/사람 평가와 운영: 이 판단을 누구에게 맡기고 실제 서비스 한도 안에서 어떻게 읽을까 |
| 실행 기록 | 품질 판단 기준 | 평가 분업과 운영 판단 |

즉, 지금 장의 핵심은 `기록을 남겼는가`에서 `그 기록과 답을 어떤 기준으로 판정할까`로 손잡이가 바뀐다는 점입니다.

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| harness | 어떤 실행 흔적을 남겼는가 |
| evaluation | 그 흔적과 답을 어떤 품질 기준으로 가를 것인가 |
| operations | 그 판정을 실제 서비스 한도와 복구 경로에 어떻게 연결할 것인가 |
...
특히 바로 앞 P5-14.2에서 남긴 trace, tool call log, replay 정보는 여기서 곧바로 평가 입력으로 다시 읽어야 합니다. 답변 문장만 보면 놓치는 실패도 실행 기록을 함께 보면 `검색이 약했는가`, `도구 호출이 과했는가`, `승인 경로가 비었는가`처럼 다른 평가 축으로 분리할 수 있기 때문입니다.

같은 전환을 가장 짧게 말하면 다음과 같습니다. 앞 절까지는 `무엇을 연결하고 어떻게 실행할까`를 읽었다면, 여기서는 `그 실행을 어떤 기준으로 통과 또는 수정 대상으로 판정할까`를 읽습니다.

처음 읽을 때는 LLM 평가를 Part 5 본류의 `품질 판정 손잡이`로만 잡고, 그 다음에 어떤 운영 층위가 더 붙는지까지 같이 보면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| LLM 평가(evaluation) | 어떤 축에서 통과, 수정, 탈락을 나눌 것인가? | P5-15.1 |
| 자동 평가와 사람 평가 | 그 평가를 누구에게 어떤 비중으로 맡길 것인가? | P5-15.2 |
| 운영과 실패 대응 | 그 판단을 실제 서비스 제약과 복구 경로에 어떻게 연결할 것인가? | P5-16.1, P5-16.2 |

이 경계를 더 또렷하게 보려면 같은 실행 기록을 세 층위에서 나란히 읽어 보는 편이 좋습니다.

| 같은 기록 | P5-14 하네스에서 읽는 질문 | P5-15 평가에서 읽는 질문 | P5-16 운영에서 다시 읽는 질문 |
| --- | --- | --- | --- |
...
| 지금 평가 단계에서 먼저 남길 기록 | 왜 지금 필요한가 | 뒤 절과 Part 6에서 다시 읽는 기록 |
| --- | --- | --- |
| 축별 점수, `answer_status`, 수정 메모 | 어떤 축에서 통과했고 어떤 축에서 탈락했는지 남겨야 사람 검토와 운영 조치를 나눌 수 있어서 | P5-15.2의 자동/사람 평가 분담, Part 6의 `review_summary`, `answer_status`로 이어진다 |
| `review_summary`, 우선 수정 항목 | 무엇을 먼저 고쳐야 하는지 남겨야 운영 절에서 비용·지연·실패 대응보다 앞선 품질 문제를 정리할 수 있어서 | P5-16.1, P5-16.2의 운영 판단과 Part 6의 `improvement_plan`, 회고 문서로 이어진다 |
