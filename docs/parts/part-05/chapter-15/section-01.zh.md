...
在 P5-14.4 和 P5-14.5 里，我们已经看到：Transformer 在并行处理和长上下文直接参考上成为了一个大转折点，而这种结构最终也成了 LLM 与生成式 AI 普及的基础。接下来就会自然出现一个问题。
...
下表不再用代码重新汇总同一组记录，而是把它作为解释性案例直接呈现。候选短语和记录次数已经给出时，Python 很容易只是在重复统计步骤；在本节里，用表格和解读就足够了。
...
这张表里首先要看的，不只是 `0.50` 或 `0.52` 这样的最大数字。温度警报中，冷却水流量、风扇状态、传感器校准一起组成一个候选空间；封边警报中，封边压力、薄膜张力、刀刃磨损则组成另一个候选空间。生成模型视角要读的正是这两个候选空间的差异。
