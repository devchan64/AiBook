...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 자동 평가와 사람 평가는 `무엇을 통과시키고 무엇을 다시 볼까`를 가르는 평가 분업 구조이고, 자동 평가는 빠른 게이트를 만들고 사람 평가는 끝까지 읽어야 하는 후보를 가릅니다. 핵심은 `누가 더 좋은가`가 아니라 `어떤 후보를 어떤 경로로 넘길까`에 있습니다. 그래서 P5-16에서는 이 판단을 비용, 지연 시간, 승인·보류·fallback·stop 같은 운영 경로 안에서 실제 운영 결정으로 더 보고, 뒤 절로 갈수록 이 품질 판단은 운영 한도와 실패 대응 설계로 바뀝니다. 반면 trace와 recovery를 실제 run 기록으로 남기거나, P5-17처럼 이 판단을 하나의 요청 흐름 기록으로 다시 묶고, Part 6처럼 프로젝트 문서와 운영 회고 산출물로 내리는 단계는 아직 여기서 닫지 않습니다.
