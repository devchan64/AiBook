...
즉, 이 절은 평가 자체로 끝나는 절이 아니라 `어떤 후보를 다음 운영 판단으로 넘길까`를 정하는 본류 마지막 구간의 첫 단계입니다. 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `좋은 답 판단 -> 운영 후보 분류 -> 실패 경로 준비`이며, 여기서 세우는 자동 게이트, 사람 검토, 승인 후보 구분은 바로 다음 장의 비용·지연 시간 한도와 `retry`, `fallback`, `stop`, `approval` 같은 운영 경로 판단, 그리고 뒤의 `run record`, `answer_status`, `incident_records` 같은 요청 기록 구조로 이어집니다.
