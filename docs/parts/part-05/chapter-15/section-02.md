...
처음 읽을 때는 이 절을 `무엇을 평가할까`에서 `누가 어떤 평가를 맡을까`로 손잡이가 바뀌는 자리로만 잡아도 충분합니다.
...
Part 5 뒤쪽 본류를 지금 위치에서 가장 짧게 묶으면 다음 세 단계로 읽을 수 있습니다.

| 지금 막 세우는 단계 | 바로 다음 단계 | 그다음 통합 단계 |
| --- | --- | --- |
| 좋은 답을 어떻게 가를까 | 통과하지 못한 경우 어떤 운영 경로로 보낼까 | 이 판단을 요청 흐름 기록으로 어떻게 남길까 |
| 자동 게이트, 사람 검토, 승인 후보 구분 | retry, fallback, stop, approval 구분 | run record, answer_status, incident_records 정리 |

즉, 이 절은 평가 자체로 끝나는 절이 아니라 `어떤 후보를 다음 운영 판단으로 넘길까`를 정하는 본류 마지막 구간의 첫 단계입니다.

처음 읽을 때는 자동 평가와 사람 평가를 Part 5 본류의 `평가 분업 손잡이`로만 잡고, 그 다음에 어떤 운영 층위가 더 붙는지까지 같이 보면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 자동 평가와 사람 평가 | 어떤 후보를 자동 게이트, 사람 검토, 승인 후보로 나눌 것인가? | P5-15.2 |
| 서비스 운영 제약 | 그 후보를 비용과 지연 시간 한도 안에 둘 수 있는가? | P5-16.1 |
| 실패 대응 | retry, fallback, stop, approval 중 어디로 보낼 것인가? | P5-16.2 |

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| evaluation | 어떤 후보를 자동 게이트와 사람 검토로 나눌 것인가 |
| operations | 그 후보를 실제 서비스 한도 안에서 계속 둘 수 있는가 |
| failure handling | 실패 시 retry, fallback, stop, approval 중 어디로 보낼 것인가 |

처음 읽을 때는 아래 세 줄만 바로 구분해도 충분합니다.

| 지금 이 절에서 먼저 못 박을 것 | 아직 여기서 끝내지 않는 것 | 바로 다음 장으로 넘길 것 |
| --- | --- | --- |
| 어떤 후보를 자동 게이트, 사람 검토, 승인 후보로 나눌까 | retry, fallback, stop 같은 복구 경로를 확정하지 않는다 | 운영 한도와 실패 경로 판단으로 넘긴다 |
| 평가 분업 기준을 세운다 | 실제 서비스 통제와 복구 정책을 닫지 않는다 | P5-16에서 운영 경로를 정한다 |
...
| --- | --- | --- |
| 자동 평가와 사람 평가는 `무엇을 통과시키고 무엇을 다시 볼까`를 가르는 평가 분업 구조다. | P5-16에서는 이 판단을 비용, 지연 시간, 실패 경로 안에서 실제 운영 결정을 내리는 문제로 더 봅니다. | P5-17처럼 이 판단을 하나의 요청 흐름 기록으로 다시 묶는 단계는 아직 아닙니다. |
| 자동 평가는 빠른 게이트를 만들고, 사람 평가는 끝까지 읽어야 하는 후보를 가른다. | 바로 다음 절에서는 승인, 보류, fallback, stop 같은 운영 경로가 이 판단 위에서 어떻게 갈리는지 더 봅니다. | trace와 recovery를 실제 run 기록으로 남기는 일은 아직 여기서 끝내지 않습니다. |
| 핵심은 `누가 더 좋은가`가 아니라 `어떤 후보를 어떤 경로로 넘길까`에 있다. | 뒤 절로 갈수록 이 품질 판단이 운영 한도와 실패 대응 설계로 바뀝니다. | Part 6처럼 프로젝트 문서와 운영 회고 산출물로 내리는 단계는 아직 아닙니다. |
