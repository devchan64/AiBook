...
즉, 이 절은 평가 자체로 끝나는 절이 아니라 `어떤 후보를 다음 운영 판단으로 넘길까`를 정하는 단계입니다. 여기서 붙잡을 닫힘 구조는 `좋은 답 판단 -> 운영 후보 분류 -> 실패 경로 준비`이며, 자동 게이트와 사람 검토 구분은 바로 다음 장의 운영 경로 판단과 뒤의 요청 기록 구조로 이어집니다.
...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 자동 평가와 사람 평가는 `누가 더 좋은가`보다 `어떤 후보를 어떤 경로로 넘길까`를 가르는 평가 분업 구조입니다.
