...
P5-15.1에서는 에이전트(agent)가 목표를 작업 흐름으로 이어 가는 실행 구조라는 점을 보았습니다. 그러면 이제 그 내부 흐름을 더 구체적으로 봐야 합니다.
...
이 절은 단일 에이전트 루프의 기본 구조에 집중하고, 도구 연결과 실행 환경은 다음의 P5-16.1 MCP와 도구 연결, P5-16.2 하네스에서 다시 회수합니다. 다중 에이전트와 장기 메모리 설계 전부는 현재 본편 범위 밖으로 둡니다.
...
검색 결과가 부족하면 다시 검색어를 바꾸고, 문서가 충분하면 요약 단계로 넘어갈 수 있습니다. 이것은 관찰 결과에 따라 다음 계획이 바뀌는 사례입니다.

### 사례 2. 코딩 에이전트

테스트가 실패하면 오류 로그를 읽고, 다시 패치를 제안한 뒤 재실행할 수 있습니다. 이 구조는 계획-행동-관찰이 분명한 실무 사례입니다.

### 사례 3. 예약 보조 에이전트

캘린더 조회 결과 빈 시간이 없으면 다른 시간대를 제안하거나 사용자에게 확인을 요청해야 합니다. 즉, 관찰 결과가 흐름을 바꿉니다.
...
이 질문은 P5-16.1 MCP와 도구 연결로 이어집니다.
