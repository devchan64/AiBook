...
| --- | --- | --- |
| 평가 분업 | 어떤 후보를 자동 게이트, 사람 검토, 승인 후보로 나눌 것인가? | P5-15.2 |
| 운영 한도 | 그 후보를 비용과 지연 시간 한도 안에 둘 수 있는가? | P5-16.1 |
| 실패 경로 | retry, fallback, stop, approval 중 어디로 보낼 것인가? | P5-16.2 |

즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `좋은 답 판단 -> 운영 후보 분류 -> 실패 경로 준비`입니다. 여기서는 자동 평가와 사람 평가를 나눠 `운영 후보 분류`를 세우고, 바로 다음 장에서 그 후보를 실제 서비스 한도와 복구 경로 안에 넣습니다.
