...
처음 읽을 때는 지금 읽는 층위를 `평가 분업 층위`로 고정해 두면 덜 흔들립니다. 앞 절이 `무엇을 평가해야 하는가`라는 축을 세웠다면, 여기서는 `그 평가를 누구에게 어떤 비중으로 맡길 것인가`로 질문이 바뀝니다. 아직 비용, 지연 시간, 실패 대응 같은 운영 한도 자체를 다루는 단계까지는 가지 않고, 바로 다음 장에서 그 판단을 실제 서비스 제약과 실패 경로로 넘깁니다.
