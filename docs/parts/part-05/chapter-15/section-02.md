...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 자동 평가와 사람 평가는 `누가 더 좋은가`가 아니라 `어떤 후보를 어떤 경로로 넘길까`를 가르는 평가 분업 구조이고, 이 판단은 P5-16에서 비용, 지연 시간, 승인·보류·fallback·stop 같은 운영 경로로 이어집니다. P5-17과 Part 6의 요청 흐름 기록과 운영 회고 산출물은 그다음 단계입니다.
