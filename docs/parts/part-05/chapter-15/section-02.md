...
즉, 이 절은 평가 자체로 끝나는 절이 아니라 `어떤 후보를 다음 운영 판단으로 넘길까`를 정하는 본류 마지막 구간의 첫 단계입니다. 여기서 세우는 자동 게이트, 사람 검토, 승인 후보 구분은 바로 다음 장의 `retry`, `fallback`, `stop`, `approval` 같은 운영 경로 판단으로 이어지고, 그 뒤에는 `run record`, `answer_status`, `incident_records` 같은 요청 기록 구조로 다시 남습니다.
