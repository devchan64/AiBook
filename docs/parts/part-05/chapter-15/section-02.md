...
| --- | --- | --- |
| 가장 높은 후보 비중을 0.42에서 0.65로 높인다 | argmax와 sampling 결과가 얼마나 더 비슷해지는가 | 반복성이 더 강해지고 변주 폭은 줄어드는가 |
| `weights`를 더 평평하게 만든다 | 낮은 후보가 실제 문장으로 얼마나 더 자주 등장하는가 | `counts` 분포가 넓어지고 평균 길이도 달라지는가 |
| 후보 문장에 더 긴 설명형 표현을 추가한다 | 표현 다양성뿐 아니라 설명 밀도도 함께 바뀌는가 | sampling이 길이 분포까지 흔든다는 점이 더 분명해지는가 |

즉, 이 절의 예제는 `argmax와 sampling이 다르다`는 확인에 머무르지 않고, `후보 분포를 흔들면 사용자 경험이 어떻게 달라지는가`를 직접 보게 해야 더 실험적입니다.
...
- 생성 모델이 무엇을 배웠는지는 알겠는데 실제 결과가 왜 달라지는지 설명이 비어 있을 때, 샘플링 관점을 먼저 떠올립니다.
- argmax와 sampling 차이를 품질과 다양성 균형으로 설명해야 할 때, 후보 점수 계산과 실제 출력 선택 단계를 다시 구분합니다.
- 뒤 Part의 생성 설정을 읽기 전에 `모델 점수`와 `선택 전략`을 분리해 두어야 할 때, 이 절을 기준선으로 씁니다.
