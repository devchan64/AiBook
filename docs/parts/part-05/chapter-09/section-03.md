# P5-9.3 보충학습: LoRA, adapter, QLoRA를 처음 구분하는 법

P5-9.1과 P5-9.2에서는 파인튜닝과 LoRA의 큰 흐름을 보았습니다. 여기서는 본문에서 잠시 넘긴 이름들을 더 차분하게 정리합니다.
...
이 절의 목적은 LoRA 관련 문서를 읽을 때 이름 때문에 멈추지 않게 하는 데 있습니다. 실제 instruction tuning 흐름은 P5-10.1에서 다시 이어지고, 운영 비용과 제약은 P5-18.1에서 다시 연결합니다.
...
한 팀이 고객 문의 분류, 문서 요약, 내부 검색 보조를 모두 같은 기반 모델로 시험한다고 해 봅시다. 전체 파인튜닝을 세 번 하면 저장, 비용, 실험 시간이 크게 늘 수 있습니다. LoRA 계열은 기반 모델은 유지하고 목적별 조정분만 따로 관리하게 해 주므로, 같은 기반 위에서 여러 업무 실험을 반복하기 쉬워집니다.

### 사례 2. 메모리가 빠듯한 환경

노트북급 장비나 제한된 GPU 환경에서 큰 기반 모델 전체를 직접 다루기는 부담이 큽니다. 이때 팀은 `전체 조정은 어렵고, 더 가벼운 방식이 필요하다`는 판단을 하게 됩니다. LoRA와 QLoRA가 함께 언급되는 이유도 이런 현실 제약과 직접 연결됩니다.

### 사례 3. 결과 품질보다 먼저 실험 회전 수가 중요할 때

초기 탐색 단계에서는 `완벽한 최종 모델`보다 `어떤 데이터와 과업 정의가 맞는가`를 빨리 보는 일이 더 중요할 수 있습니다. 이런 단계에서는 큰 비용을 한 번 쓰는 방식보다, 더 작은 조정본을 빠르게 여러 번 비교하는 쪽이 실무적으로 더 유리할 수 있습니다.
