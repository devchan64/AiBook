...
| `12 -> 24` 반복 횟수 추정 | batch가 두 배면 scalar multiply 추정도 바로 두 배가 됩니다. | 작은 예제에서는 숫자가 작으니 큰 차이 아닌 것처럼 보일 수 있습니다. | 실제 모델에서는 batch, feature, layer가 함께 커져 반복 계산이 곧 실험 병목으로 바뀝니다. |
