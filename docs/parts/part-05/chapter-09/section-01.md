...
| --- | --- | --- |
| 모델 조정 축 | 현재 모델 반응을 입력 설계로 어디까지 바꿀 수 있을까? | P5-9.1 |
| 프롬프트의 한계 | 입력 설계만으로는 어디서 막히는가? | P5-9.2 |
| 근거와 실행 연결 | 최신 근거와 실제 실행을 어떤 구조로 더 붙일까? | P5-10, P5-12 |
...
프롬프트는 Part 5 본류의 `첫 번째 실무 손잡이`로 읽고, 그 다음 손잡이가 무엇인지까지 함께 연결해 두면 됩니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 프롬프트 입력 조정 | 입력을 어떻게 써야 원하는 형식과 범위를 더 잘 끌어낼 수 있는가? | P5-9.1, P5-9.2 |
| RAG 근거 연결 | 프롬프트만으로 부족하면 무엇을 근거로 답하게 할 것인가? | P5-10.1, P5-10.2 |
| tool use / agent 실행 구조 | 문서만으로 부족하면 무엇을 실행하고 어떤 순서로 이어 갈 것인가? | P5-12.1, P5-12.2, P5-13.1, P5-13.2 |
