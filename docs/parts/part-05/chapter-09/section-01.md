...
이 절에서는 프롬프트를 `마법의 주문`처럼 설명하지 않고, `모델 행동을 관찰하기 위한 입력 설계 도구`로 정리합니다.
...
그래서 이 절에서 확인해야 할 결과는 예시를 붙였을 때 모델이 단순 내용 생성만이 아니라, 요청한 형식과 반응 패턴까지 실제로 더 가깝게 따르는가입니다.
