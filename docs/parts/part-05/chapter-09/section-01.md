...
- 작은 장난감 모델 단계와 큰 이미지 모델, LLM 단계에서 계산 자원 문제가 왜 다른 크기로 나타나는지 구분해 말할 수 있는가?
- 지금 필요한 질문이 `모델 구조가 무엇인가`인지, 아니면 `이 구조를 감당할 계산 단위와 자원이 무엇인가`인지 나눠 읽고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 모델 구조보다 실행 속도와 실험 회전이 더 큰 병목처럼 보일 때, 계산 자원 관점을 먼저 떠올립니다.
- 모델이 커질수록 학습 가능 여부 자체가 달라지는 이유를 설명해야 할 때, 메모리와 반복 연산 규모 문제를 다시 봅니다.
- GPU를 단순 가속 부품이 아니라 실험 규모를 바꾸는 조건으로 설명해야 할 때, 병렬 처리 관점을 다시 꺼냅니다.
