...
처음 읽을 때는 이 절을 `첫 번째 실무 손잡이 축`으로만 고정해 두면 덜 흔들립니다.

| 지금 이 절에서 읽는 것 | 바로 다음 절이나 뒤 장으로 넘기는 것 |
| --- | --- |
| 입력을 어떻게 설계해야 현재 모델 반응을 더 잘 끌어낼 수 있는가 | 최신 근거와 실제 실행을 어떻게 구조적으로 붙일지 |
| 지시, 맥락, 예시가 출력 형식과 범위를 어떻게 바꾸는가 | Chain-of-thought류 기법 이름과 자동 최적화 기법을 어떻게 더 세밀하게 비교할지 |
...
| --- | --- | --- |
| 프롬프트 입력 조정 | 입력을 어떻게 써야 원하는 형식과 범위를 더 잘 끌어낼 수 있는가? | P5-9 |
| RAG 근거 연결 | 프롬프트만으로 부족하면 무엇을 근거로 답하게 할 것인가? | P5-10 |
| tool use / agent 실행 구조 | 문서만으로 부족하면 무엇을 실행하고 어떤 순서로 이어 갈 것인가? | P5-12, P5-13 |

즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `프롬프트 입력 조정 -> RAG 근거 연결 -> tool use/agent 실행 구조`입니다. 여기서는 그 가운데 이전 출발점인 `입력 조정`을 맡고 있고, 바로 다음 절에서 그 한계를 확인한 뒤 근거 연결과 실행 구조로 넘어갑니다.

| 지금 프롬프트 단계에서 먼저 남길 기록 | 왜 지금 필요한가 | 뒤 절과 Part 6에서 다시 읽는 기록 |
| --- | --- | --- |
| `prompt_name`, `required_slots` | 어떤 입력 설계로 실험했는지 남겨야 형식 안정성과 내용 보존을 다시 비교할 수 있어서 | P5-9.2의 한계 점검, P5-15의 평가 기준, Part 6의 `project_note`, `execution_records`로 이어진다 |
| `missing_slots`, 형식 점검 통계 | 어떤 정보가 자주 빠지고 어디서 출력이 흔들리는지 남겨야 다음에 RAG나 tool use로 넘어갈지 판단할 수 있어서 | P5-10.1의 근거 연결 판단, P5-12.1의 실행 필요 판단, Part 6의 `review_summary`, `improvement_plan`으로 이어진다 |
