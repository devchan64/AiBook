...
| --- | --- | --- |
| 프롬프트: 같은 재료로 답하는 방식을 어떻게 조정할까 | 프롬프트의 한계: 잘 물어도 아직 남는 문제는 무엇일까 | RAG: 그렇다면 무엇을 근거로 답하게 할까 |
| 입력 설계 | 입력 조정의 한계 확인 | 근거 연결 |

즉, 지금 장의 핵심은 `더 잘 물을까`에서 `질문 밖의 근거를 실제로 붙여야 하는가`로 손잡이가 바뀐다는 점입니다.

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| 프롬프트 | 같은 재료로 답하는 방식을 더 잘 조정할 수 있는가 |
| RAG | 답의 재료 자체를 현재 문서로 다시 골라야 하는가 |
| tool use / evaluation | 문서만으로 부족해서 실행 결과나 반복 점검 구조가 필요한가 |
...
| 지금 프롬프트 한계 단계에서 먼저 남길 기록 | 왜 지금 필요한가 | 바로 다음 절과 Part 6에서 다시 읽는 기록 |
| --- | --- | --- |
| `failure_type`, `missing_grounding_reason` | 어떤 문제가 입력 조정 한계인지, 어떤 문제가 근거 연결 부재인지 구분해야 다음 구조 선택이 흔들리지 않아서 | P5-10.1의 RAG 필요 판단, Part 6의 `review_summary`, `improvement_plan`으로 이어진다 |
| 형식 흔들림 통계, 재현 실패 메모 | 프롬프트 개선으로 줄일 문제와 시스템 구조를 바꿔야 할 문제를 나눠야 해서 | P5-15의 평가 기준과 Part 6의 `project_note`, 회고 문서로 이어진다 |
