...
- 값 자체보다 shape 불일치나 축 의미가 더 먼저 문제처럼 보일 때, 배치·텐서 관점을 먼저 떠올립니다.
- 표, 이미지, 문장을 완전히 다른 종류의 입력처럼만 보고 있을 때, 텐서와 shape라는 공통 계산 언어를 다시 꺼냅니다.
- attention이나 Transformer 계산으로 넘어가기 전에 축 이름을 실제로 분리해 읽는 습관이 필요할 때, 이 절을 기준선으로 삼습니다.
