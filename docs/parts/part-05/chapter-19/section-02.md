# P5-19.2 최소 구현과 회고 포인트

P5-19.1에서는 작은 생성형 AI 기능을 `요청 해석 -> 검색 또는 도구 선택 -> 응답 생성 -> 평가와 기록`의 흐름으로 묶었습니다. 그러면 이제 한 단계 더 직접적인 질문이 남습니다.
...
- 더 나은 검색이 필요하면 P5-13의 임베딩과 벡터 검색으로 돌아갑니다.
- 실제 상태 조회가 필요하면 P5-14의 tool use로 갑니다.
- 여러 단계 판단이 필요하면 P5-15의 agent 구조로 갑니다.
- 실패 기록과 안전 장치는 P5-18의 운영 관점으로 다시 읽습니다.
