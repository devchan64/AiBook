...
FAQ 정리나 중복 문의 묶기에서는 `비밀번호를 변경하고 싶어요`와 `비밀번호를 다시 설정하려면 어떻게 하나요?`가 같은 흐름인지 판단해야 합니다. 사람은 표면 단어가 달라 보이면 다른 질문이라고 느끼기 쉽지만, 실제 지원 흐름은 같은 도움말로 닫히는 편이 더 유용합니다. 표면 단어가 조금 달라도 사용자가 하려는 일이 비슷하면 related로 묶는 편이 실무에 더 유용합니다. 반대로 `로그인이 계속 실패합니다`처럼 주변 단어는 비슷하지만 실제 처리는 장애 대응으로 가야 하는 문장을 잘못 묶으면 운영 혼선이 생길 수 있습니다. 이런 관계 판단도 BERT 계열이 잘 맞는 대표 이해 중심 태스크입니다.

### 사례 3. 생성과의 대비

같은 고객 문의를 보고 생성형 모델은 긴 답변 초안을 만들 수 있지만, 운영 첫 단계에서 더 급한 일은 종종 `어느 라벨로 보낼 것인가`입니다. 사람은 챗봇 시대가 되면 모든 문제가 생성으로 풀릴 것처럼 느끼기 쉽지만, 실제 현장에서는 먼저 읽고 분류해야 다음 단계가 열립니다. 예를 들어 환불 문의를 계정 잠금 큐로 잘못 보내면, 아무리 정중한 답변 초안을 붙여도 실제 해결은 더 늦어집니다. 즉, 읽기 중심 모델은 답을 길게 쓰기보다 `어떤 판단을 내릴 것인가`에 집중합니다. 이 대비가 BERT 계열의 역할을 더 선명하게 보여 주며, 생성과 이해의 우선순위가 다를 수 있다는 점을 남깁니다.
