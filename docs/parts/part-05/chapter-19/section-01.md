...
masked language modeling 수식과 BERT 파생 모델들의 세부 구조 비교는 여기서 다루지 않습니다. 대신 BERT 계열이 실제로 어디에 쓰이는지는 바로 다음 P5-19.2 이해 중심 태스크에서 다시 회수합니다. 검색과 임베딩 쪽의 실전 연결은 P5-11.1, P5-11.2에서 다시 다루고, 최신 encoder-only 모델 벤치마크 경쟁은 현재 본편 범위 밖에 둡니다.

대신 BERT 계열이 실제로 어디에 쓰이는지는 바로 다음 P5-19.2 이해 중심 태스크에서 회수합니다. 검색과 임베딩 쪽의 실전 연결은 P5-11.1 벡터 데이터베이스와 P5-11.2 인덱스와 검색 품질에서 다시 다루고, 세부 벤치마크 경쟁은 현재 본편 범위 밖으로 둡니다.
