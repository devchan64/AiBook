...
P5-3.2에서는 attention과 context window를 구분하며, Transformer가 실제로는 입력 길이 제한 안에서 문맥을 읽는 구조라는 점을 보았습니다. 이제 같은 Transformer 계열 안에서도 방향이 갈라집니다.
...
지금 읽는 층위는 `본류 복습 뒤의 비교 배경 층위`입니다. 이 절의 목적은 GPT 중심 본류를 멈추고 새 주제를 길게 여는 데 있지 않고, Chapter 1~17에서 읽은 생성 중심 흐름을 `읽기 중심 Transformer 계열`과 더 정확히 구분하게 만드는 데 있습니다. 따라서 처음 읽을 때도 BERT 자체를 앞쪽 본류로 다시 옮기기보다, 이미 읽은 생성 구조를 비교 기준으로 다시 붙잡는 절로 보는 편이 안전합니다.

즉, 이 장은 `BERT를 새 본편처럼 먼저 끝내는 장`이 아니라 `GPT 본류를 더 정확히 읽기 위해 뒤에서 다시 꺼내 보는 비교 장`으로 두는 편이 현재 Part 5 흐름에 더 맞습니다.

처음 읽을 때는 특히 다음 기준만 먼저 고정해 두면 충분합니다.

| 먼저 끝내야 하는 본류 | 이 비교 배경 축에서 다시 보는 것 |
| --- | --- |
| GPT, next-token prediction, RAG, tool use, agent까지의 생성 중심 흐름 | 읽기 중심 Transformer 계열이 어디에서 다르고 왜 지금도 중요한가 |

재진입 기준을 더 짧게 잡으면 다음처럼 읽어도 충분합니다.

| 지금 상태 | 이 장을 바로 깊게 읽어야 하는가 | 먼저 돌아갈 위치 |
| --- | --- | --- |
| GPT와 BERT 차이가 `생성/이해` 한 줄로만 남아 있다 | 보통 아니다 | P5-4.1, P5-5.1, P5-11.1 |
| 생성 중심 본류를 한 번 끝내고 비교 기준이 필요하다 | 그렇다 | 지금 장에서 encoder/decoder, 읽기/생성 차이를 다시 묶기 |
| 검색, 분류, 임베딩 앞단에서 왜 BERT 계열이 아직 중요한지 헷갈린다 | 그렇다 | 지금 장에서 읽기 중심 과업 위치를 다시 확인 |

즉, 이 장은 BERT를 새 본편처럼 먼저 끝내는 장이 아니라 `GPT 본류를 더 정확히 읽기 위한 비교 복습 장`으로 다시 들어오는 편이 안전합니다.
...
masked language modeling 수식과 BERT 파생 모델들의 세부 구조 비교는 여기서 다루지 않습니다. 대신 BERT 계열이 실제로 어디에 쓰이는지는 바로 다음 P5-19.2 이해 중심 태스크에서 다시 회수합니다. 검색과 임베딩 쪽의 실전 연결은 P5-11.1 벡터 데이터베이스와 P5-11.2 인덱스와 검색 품질에서 다시 다루고, 최신 encoder-only 모델 벤치마크 경쟁은 현재 본편 범위 밖에 둡니다.

BERT는 `LLM 이전의 옛 모델`로 밀려난 것이 아니라, 지금도 분류, 검색, 임베딩 앞단에서 중요한 인코더 기반 흐름입니다. 여기서는 Part 5의 본류를 BERT로 옮기기보다, 앞서 읽은 GPT 계열과 생성형 AI 본류를 더 정확히 비교하기 위한 기준을 세우는 데 집중합니다.

즉, 이 절은 GPT 본류를 멈추고 다른 주제를 새로 여는 장이 아니라, Chapter 1~17에서 읽은 생성 중심 흐름을 `읽기 중심 Transformer 계열과 어떻게 구분할 것인가`를 뒤에서 다시 붙여 보는 비교 축입니다. 처음 읽을 때는 `BERT도 똑같이 깊게 파야 하나`보다 `왜 이 비교가 GPT 본류를 더 정확히 읽게 해 주는가`를 먼저 붙잡는 편이 안전합니다.
...
이 세 축이 잡히면, P5-4.1 GPT 계열의 위치와 P5-4.2 next-token prediction에서 왜 사용자 경험이 크게 달라졌는지 읽기가 쉬워집니다.
...
커리큘럼 관점에서 이 절에서 확인해야 할 결과는 바로 앞의 P5-3.1 Transformer 구조와 P5-4.1 GPT 계열 설명을 기준점으로 삼아, 생성형 AI 시대에도 `읽고 구분하고 연결하는 모델`의 역할이 사라진 것이 아니라 분류, 검색, 임베딩 같은 앞단 과업에서 여전히 별도 축으로 남아 있다는 점을 읽는 것입니다.
...
이 질문은 P5-19.2 이해 중심 태스크로 이어집니다.
