...
고객 지원 팀이 들어오는 문의를 `환불`, `배송`, `계정`, `오류` 같은 라벨로 자동 분류한다고 해 봅시다. 처음에는 사람이 `환불`, `취소`, `반품` 같은 키워드 규칙을 세워 문의를 나눌 수 있습니다. 하지만 실제 문장에는 `돈이 다시 언제 들어오나요`, `결제를 취소했는데 처리 상태를 모르겠어요`처럼 핵심 단어가 달라도 같은 흐름으로 보내야 하는 경우가 많습니다. BERT 계열 표현 모델은 문장 전체를 읽고 이런 표현을 비슷한 업무 의도로 연결하는 데 유리합니다. 그래서 단순 키워드 표에서는 다른 라벨로 흩어지던 문의를 더 안정적으로 같은 처리 큐로 모을 수 있습니다.
