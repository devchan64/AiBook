...
즉, 이 절은 GPT 본류를 멈추고 다른 주제를 새로 여는 장이 아니라, Chapter 1~17에서 읽은 생성 중심 흐름을 `읽기 중심 Transformer 계열과 어떻게 구분할 것인가`를 뒤에서 다시 붙여 보는 비교 축입니다. `BERT도 똑같이 깊게 파야 하나`보다 `왜 이 비교가 GPT 본류를 더 정확히 읽게 해 주는가`를 먼저 붙잡는 편이 안전합니다.
