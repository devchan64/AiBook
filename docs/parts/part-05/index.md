...
5. 합성곱 신경망(CNN), 순환 신경망(RNN), 장단기 메모리(LSTM), 게이트 순환 유닛(GRU)
6. 어텐션(Attention)과 셀프 어텐션(self-attention)
7. 트랜스포머(Transformer)
8. 생성과 샘플링
...
즉, Part 5는 `신경망 기초 -> 학습과 확장 -> 구조 분기 -> 생성`으로 점차 손잡이를 옮기는 파트입니다.
...
| 학습 절차 문제 | 그 구조를 어떻게 안정적으로 학습시킬 것인가? | 손실, 역전파, 옵티마이저(optimizer), 정규화(regularization), 드롭아웃(dropout), 배치 |
...
| 손실 함수와 최적화 | 손실이 왜 필요하고, 기울기(gradient)와 옵티마이저(optimizer)가 어떻게 연결되는가 |
...
- 어텐션(Attention)과 트랜스포머(Transformer)는 LLM 구조 설명의 직접 출발점이 됩니다.
- 생성과 샘플링은 다음 토큰 예측과 디코딩(decoding) 설명으로 이어집니다.
- 표현 학습과 텐서 계산 감각은 임베딩과 대규모 생성 모델 설명을 떠받칩니다.

즉, Part 5에서 구조가 약하면 다음 Part도 다시 제품 이름과 기능 목록 중심 설명으로 미끄러집니다. 반대로 Part 5가 각 구조의 문제 설정과 계산 원리를 자기 파트 안에서 충분히 닫아 주면, 다음 Part는 그 위에 토큰, 사전학습, 검색 증강 생성(RAG), 에이전트(agent) 같은 생성형 AI 본류를 더 안정적으로 얹을 수 있습니다.
