# P5-10.2 정렬(alignment)의 기본 문제

P5-10.1에서는 지시 튜닝(instruction tuning)이 모델을 더 `assistant-like`하게 만드는 조정 단계라는 점을 보았습니다. 하지만 여기서 바로 더 어려운 문제가 나옵니다.
...
대신 정렬 문제가 실제 사용 층에서 어떻게 이어지는지는 P5-11.1 프롬프트 엔지니어링, P5-11.2 프롬프트의 한계, P5-17.1 LLM 평가, P5-17.2 자동 평가와 사람 평가, P5-18.1 AI 서비스가 현실에서 만나는 제약, P5-18.2 운영 중 실패 대응에서 다시 회수합니다. RLHF나 constitutional AI의 세부 설계 전부는 현재 본편 범위 밖으로 둡니다.
...
- 이후 P5-17.1 LLM 평가, P5-17.2 자동 평가와 사람 평가, P5-18.1 AI 서비스가 현실에서 만나는 제약, P5-18.2 운영 중 실패 대응을 읽는 기준을 만들어 주기 때문입니다
...
- 프롬프트 엔지니어링은 어디까지 유효하고, 어디서 한계를 만나는가?

이 질문은 P5-11.1 프롬프트 엔지니어링(prompt engineering)으로 이어집니다.
