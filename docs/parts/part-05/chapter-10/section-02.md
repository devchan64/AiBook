...
| --- | --- | --- |
| RAG 필요 판단 | 왜 답 전에 문서를 붙여야 할까? | P5-10.1 |
| 검색-생성 결합 | 그 문서가 실제로 어디에 붙고 어떻게 답으로 이어질까? | P5-10.2 |
| retrieval 저장소와 탐색 구조 | 그 문서를 어떤 저장 구조와 인덱스로 다시 꺼낼까? | P5-11 |

즉, 지금 장의 핵심은 `문서를 붙여야 하는가`에서 `붙인 문서가 실제 입력 맥락과 최종 답 사이에서 어떻게 작동하는가`로 손잡이가 바뀐다는 점입니다.

즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `RAG 필요 판단 -> 검색-생성 결합 -> retrieval 저장소와 탐색 구조`입니다. 여기서는 그 가운데 단계인 `검색-생성 결합`을 맡고 있고, 바로 다음 장에서 이 결합에 필요한 저장 구조와 검색 품질 문제로 이어집니다.
