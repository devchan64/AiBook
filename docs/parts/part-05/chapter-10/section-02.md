...
처음 읽을 때는 이 절을 `검색-생성 결합 축`으로만 고정해 두면 덜 흔들립니다.

| 지금 이 절에서 읽는 것 | 바로 다음 장이나 뒤 장으로 넘기는 것 |
| --- | --- |
| 검색된 문서가 실제 입력 맥락에 어디에 붙고 어떻게 답으로 이어지는가 | 그 문서를 어떤 저장 구조와 인덱스로 더 잘 찾아올지 |
| 검색 실패와 생성 실패를 왜 따로 구분해야 하는가 | 운영 제약과 평가 절차에서 이 두 실패를 어떻게 계속 추적할지 |

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| RAG의 필요성: 왜 답 전에 문서를 붙여야 할까 | 검색 결과와 생성의 결합: 그 문서가 실제로 어디에 붙고 어떻게 답으로 이어질까 | 벡터 데이터베이스와 인덱스: 그 문서를 어떤 저장 구조와 탐색 구조에서 다시 꺼낼까 |
| 근거 연결 필요 판단 | 검색-생성 결합 구조 | retrieval 저장소와 탐색 구조 |

즉, 지금 장의 핵심은 `문서를 붙여야 하는가`에서 `붙인 문서가 실제 입력 맥락과 최종 답 사이에서 어떻게 작동하는가`로 손잡이가 바뀐다는 점입니다.
