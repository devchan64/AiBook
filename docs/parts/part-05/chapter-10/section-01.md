...
깊은 층에서 표현이 어떻게 점점 추상화되는지는 P5-10.2에서 이어서 다루고, 이미지 도메인 표현은 P5-11.1, P5-11.2에서, 시퀀스와 attention 기반 표현은 P5-12.1, P5-12.2, P5-13.1, P5-13.2에서 다시 연결합니다. self-supervised representation learning의 최신 세부 기법은 이 책의 현재 본편 범위 밖에 둡니다.
...
포장 라인에서 실링이 정상인 파우치와 실링 가장자리가 약하게 뜬 파우치를 나누는 장면을 떠올려 볼 수 있습니다. 사람은 먼저 밝은 선 길이, 들뜬 구간 개수처럼 눈에 잘 띄는 특징을 세어 보려 할 수 있습니다. 하지만 촬영 각도가 조금만 달라지거나 필름이 살짝 접히면 같은 약실링도 밝기와 경계 위치가 크게 달라져 이런 규칙이 자주 흔들립니다. 표현 학습 기반 모델은 실링 선의 굴곡, 필름 주름 방향, 가장자리 주변 질감 조합처럼 여러 패턴을 함께 읽어 분류에 더 유용한 내부 표현을 만들 수 있습니다. 여기서 바뀌는 점은 `사람이 미리 적어 둔 몇 개 규칙`에서 `분류에 실제로 도움이 되는 조합을 모델이 내부에서 형성한다`로 중심이 이동하는 것입니다. 그 결과 사람이 정한 몇 개 규칙으로는 헷갈리던 이미지를 더 안정적으로 같은 품질 범주로 묶을 수 있습니다. 그래서 이 사례에서 확인해야 할 결과는 접힘과 반사광 때문에 규칙이 흔들리던 이미지도 내부 표현 조합을 쓰면 실제로 같은 실링 상태 범주로 더 일관되게 묶이는가입니다.

### 사례 2. 정비 우선순위 추천

설비 유지보수 팀이 어떤 라인을 먼저 점검할지 정한다고 해 봅시다. 사람은 먼저 `최근 7일 경보 수`, `재작업 호출 횟수`, `평균 정지 시간` 같은 통계 열을 만들면 충분하다고 느끼기 쉽습니다. 하지만 실제로는 `온도 경보가 반복된 뒤 압력 불안정이 따라오고, 그다음 재작업 호출이 늘어나는 흐름`처럼 숫자 하나로 잘 안 드러나는 조합이 중요할 수 있습니다. 표현 학습 모델은 라인별 경보 이력과 조치 정보를 함께 보며 고장 묶음과 위험 전이 패턴을 더 압축된 표현으로 만들 수 있습니다. 그래서 단순 경보 수 규칙보다 `지금 이 라인에서 어떤 점검 순서가 더 시급한가`를 더 잘 잡을 수 있고, 정비 우선순위가 단순 빈도순보다 더 맥락에 맞게 달라지는 결과로 이어집니다. 그래서 이 사례에서 확인해야 할 결과는 최근 경보 수만 비슷한 라인보다, 실제 위험 전이 흐름이 비슷한 라인에게 같은 후속 점검 항목이 더 앞쪽에 추천되는가입니다.
...
정비 로그를 비슷한 고장 주제끼리 묶는 업무를 생각해 볼 수 있습니다. 사람은 `온도`, `압력`, `센서` 같은 단어가 몇 번 나오는지 세면 주제를 대충 나눌 수 있다고 느끼기 쉽습니다. 이 기준은 빠르지만, `열교환기 팬 속도가 다시 떨어집니다`와 `냉각 구간 온도 복귀가 지연됩니다`처럼 핵심 단어가 다르면 같은 흐름의 이상도 떨어져 보일 수 있습니다. 표현 학습 기반 임베딩은 문장 전체 문맥을 반영해 이런 로그를 가까운 위치에 놓을 수 있습니다. 그래서 단어 빈도 표보다 `같은 고장을 다른 말로 기록한 로그`를 더 잘 묶을 수 있고, 정비 검색 후보에서도 같은 처리 흐름 문서가 더 가깝게 모이는 차이가 생깁니다. 그래서 이 사례에서 확인해야 할 결과는 표면 단어가 달라도 같은 처리 흐름 로그가 실제로 더 가까운 후보군으로 모이는가입니다.
...
네 사례를 같이 놓고 보면 표현 학습을 `특징을 자동으로 만드는 기술`보다 `한 줄 규칙이나 수작업 축으로는 묻히는 패턴을 새 좌표계로 다시 펼치는 방식`으로 읽어야 하는 이유가 더 분명해집니다.

| 장면 | 사람이 먼저 보기 쉬운 결과 | 표현 학습 관점에서 실제로 구분해야 할 것 | 바로 다음에 확인할 것 |
| --- | --- | --- | --- |
| 이미지 분류 | 밝은 선 길이, 들뜬 구간 개수 같은 규칙이면 충분해 보입니다. | 반사광, 접힘, 질감 조합처럼 수작업 규칙에 잘 안 담기는 패턴이 더 중요할 수 있습니다. | 같은 실링 상태가 내부 표현에서 더 가깝게 묶이는지 봅니다. |
| 정비 우선순위 추천 | 경보 수와 정지 시간 같은 통계 열이면 충분해 보입니다. | 위험 전이 흐름과 고장 묶음은 단일 점수 뒤에 숨어 있을 수 있습니다. | 비슷한 통계값 라인도 표현 좌표에서는 다르게 갈라지는지 봅니다. |
| 텍스트 임베딩 | 단어 빈도만 세도 비슷한 로그를 모을 수 있을 것처럼 보입니다. | 표면 단어가 달라도 같은 처리 흐름 문맥은 더 가까운 표현으로 모일 수 있습니다. | 같은 고장 흐름 로그가 실제로 더 가까운 후보군으로 모이는지 봅니다. |
| 품질 검사 자동화 | 금 길이와 점 개수 같은 몇 개 기준이면 충분해 보입니다. | 위치, 방향, 주변 밝기 변화 조합은 수작업 수치 둘셋으로는 놓치기 쉽습니다. | 규칙표로는 정상처럼 보이던 화면이 표현 좌표에서는 불량 후보로 분리되는지 봅니다. |

## 연습 및 예제

이번 예제의 목표는 사람이 직접 만든 위험 점수 하나와, 모델이 여러 축으로 바꿔 만든 중간 표현을 비교하는 것입니다.

입력:

- 생산 배치 4건의 마이크로 스톱 횟수와 온도 이탈 지속 시간
- 사람이 정의한 단일 위험 점수 규칙
- 두 차원 중간 표현을 만드는 가중치
...
- 2차원 representation
- 각 생산 배치가 새 좌표계에서 어디에 놓이는지
- 단일 점수로는 비슷해 보여도 표현 좌표에서는 달라질 수 있는 배치 비교

문제 상황:

- 표현 학습은 원래 특징을 그대로 쓰지 않고 다른 좌표계로 옮기는 과정이라는 점을 직접 확인해 볼 필요가 있다
- 단일 위험 점수 하나로는 묻히는 차이가 새 표현 좌표에서는 갈라질 수 있다는 점도 같이 봐야 한다

확인할 개념:

- 표현은 원래 입력을 다른 축으로 재배열한 결과로 볼 수 있다
- 새 좌표계에서 배치 위치를 같이 보면 왜 중간 표현이 중요한지 더 잘 보인다
- 같은 hand-crafted risk score라도 representation 좌표가 다르면 다른 패턴으로 읽힐 수 있다

입력(input):

위에 정리한 원본 데이터와 2차원 중간 표현을 만드는 가중치 행렬을 사용합니다.

코드를 보기 전에 먼저 어떤 비교가 벌어질지 예상해 보면 좋습니다.

| 비교 | 먼저 예상해 볼 차이 | 예상 이유 |
| --- | --- | --- |
| `risk_score` | 생산 배치를 한 줄 위험도로만 세울 가능성 | 사람이 고른 규칙이 여러 이상 신호를 하나의 축으로 눌러 만들기 때문입니다. |
| `representation` | 같은 원본이라도 두 축에서 다른 위치로 갈 가능성 | 모델은 스톱 횟수와 온도 이탈 시간을 다른 조합으로 다시 읽을 수 있기 때문입니다. |
| 비슷한 score의 배치 | 표현 좌표에서는 더 분리될 가능성 | 단일 점수에는 숨어 있던 패턴 차이가 두 축에서는 드러날 수 있습니다. |
...
risk_score = data[:, 0] * 6 + data[:, 1] * 0.5
...
for idx, (raw, score, rep) in enumerate(zip(data, risk_score, representation), start=1):
    print(
        f"batch_{idx}: raw={raw.tolist()}, "
...
print("score_gap(batch_1, batch_2) =", score_gap)
print("representation_gap(batch_1, batch_2) =", rep_gap)
```

출력에서는 risk_score 같은 단일 점수와 representation이 동시에 담는 축 정보를 먼저 비교하면 됩니다.

```text
batch_1: raw=[2.0, 80.0], risk_score=52.00, representation=[11.2, 6.7]
batch_2: raw=[6.0, 30.0], risk_score=51.00, representation=[8.4, 1.2]
batch_3: raw=[1.0, 20.0], risk_score=16.00, representation=[3.2, 1.55]
batch_4: raw=[5.0, 75.0], risk_score=67.50, representation=[13.0, 5.5]
score_gap(batch_1, batch_2) = 1.0
representation_gap(batch_1, batch_2) = 6.17
```

- `risk_score`는 사람이 하나의 기준으로 눌러 만든 단일 점수입니다
- `representation`은 같은 입력을 두 개 축으로 다시 펼쳐, `열적 부담`과 `운전 불안정성`처럼 다른 패턴을 동시에 담을 수 있게 합니다
...
| 비교 | 지금 읽어야 할 핵심 |
| --- | --- |
| `risk_score` | 생산 배치는 한 줄 위험 점수로만 줄 세워집니다. |
| `representation` | 같은 배치도 두 축에서 다른 방향 차이를 드러낼 수 있습니다. |
| `batch_1` vs `batch_2` | score 차이만 보면 거의 비슷한 위험처럼 읽히기 쉽지만, representation에서는 `온도 이탈이 긴 배치`와 `마이크로 스톱이 많은 배치`가 꽤 멀리 떨어진 좌표로 남습니다. |

출력 숫자를 읽을 때도 `점수 차이`와 `표현 좌표 차이`를 분리해서 봐야 합니다.

| 비교 | 출력에서 먼저 보이는 것 | 점수만 보면 남기 쉬운 해석 | 표현 학습까지 보면 바뀌는 해석 |
| --- | --- | --- | --- |
| `risk_score` | `batch_1`과 `batch_2`의 점수 차이는 `1.0`뿐입니다. | 두 배치는 거의 같은 위험 상태라고 보기 쉽습니다. | 한 줄 점수는 스톱 횟수와 온도 이탈의 다른 조합을 납작하게 눌러, 서로 다른 운영 패턴을 숨길 수 있습니다. |
| `representation` | 같은 두 배치의 좌표 거리는 `6.17`로 꽤 큽니다. | 점수 차이에 비해 표현 좌표 차이는 과장된 것처럼 느낄 수 있습니다. | 새 좌표계에서는 `열적 부담`과 `운전 불안정성` 같은 다른 축이 살아나, 비슷한 총점 뒤의 패턴 차이가 드러납니다. |
| `batch_1` vs `batch_2` | 둘 다 위험 신호가 있지만 강조되는 축이 다릅니다. | 위험 신호가 있으니 같은 대응이면 충분하다고 보기 쉽습니다. | 표현 좌표가 다르면 후속 점검 우선순위나 필요한 조치가 달라질 수 있습니다. |

즉, 이 예제에서 독자가 붙잡아야 할 질문은 `점수가 더 크냐`만이 아니라, `한 줄 위험 점수로는 묻히는 운영 패턴 차이가 새 좌표계에서 실제로 드러나는가`입니다.
