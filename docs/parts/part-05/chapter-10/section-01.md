...
| --- | --- | --- |
| 프롬프트 입력 조정 | 같은 재료로 답하는 방식을 어떻게 조정할까? | P5-9.1, P5-9.2 |
| RAG 근거 연결 | 답하기 전에 어떤 문서를 먼저 붙일까? | P5-10.1, P5-10.2 |
| tool use 실행 연결 | 문서를 읽는 것만으로 부족하면 무엇을 실제로 조회하거나 실행할까? | P5-12.1, P5-12.2 |
...
RAG는 `무엇을 근거로 답할까`를 먼저 닫는 구조입니다. 답의 재료를 현재 문서로 다시 고르고, P5-10.2에서는 그 문서를 생성 문맥에 어떻게 붙일지 더 봅니다. 그다음 P5-11에서는 이 근거를 저장 구조와 인덱스로 찾는 문제로 넘어갑니다.
