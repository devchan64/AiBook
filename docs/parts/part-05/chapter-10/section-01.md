...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. RAG는 `무엇을 근거로 답할까`를 먼저 닫는 구조입니다. 답의 재료를 현재 문서로 다시 고르고, P5-10.2에서는 그 문서를 생성 문맥에 어떻게 붙일지 더 봅니다. 그다음 P5-11에서는 이 근거를 저장 구조와 인덱스로 찾는 문제로 넘어갑니다.
