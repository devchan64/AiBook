...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. RAG는 `무엇을 근거로 답할까`를 먼저 닫는 구조이고, 프롬프트가 말하는 방식을 조정한다면 RAG는 답의 재료를 현재 문서로 다시 고릅니다. 그래서 P5-10.2에서는 검색된 문서를 실제 생성 문맥에 어떻게 붙이는지 더 보고, P5-11에서는 그 문서를 어떤 저장 구조와 인덱스로 찾을지 더 봅니다. 뒤 절로 갈수록 `근거를 읽는 단계`와 `실행을 호출하는 단계`를 더 분리해 보게 되지만, P5-12의 tool use처럼 외부 기능을 실제로 호출하는 구조, agent처럼 여러 단계를 이어 가며 다음 행동을 바꾸는 흐름, 하네스·평가·운영처럼 실행 기록을 관리하고 판정하는 층위는 아직 여기서 닫지 않습니다.
