...
| --- | --- | --- |
| 프롬프트 입력 조정 | 같은 재료로 답하는 방식을 어떻게 조정할까? | P5-9 |
| RAG 근거 연결 | 답하기 전에 어떤 문서를 먼저 붙일까? | P5-10 |
| tool use 실행 연결 | 문서를 읽는 것만으로 부족하면 무엇을 실제로 조회하거나 실행할까? | P5-12 |

즉, 지금 장의 핵심은 `말을 더 잘 시키는가`에서 `답의 재료를 현재 문서로 다시 고르는가`로 손잡이가 바뀐다는 점입니다.

즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `프롬프트 입력 조정 -> RAG 근거 연결 -> tool use 실행 연결`입니다. 여기서는 그 가운데 단계인 `근거 연결`을 맡고 있고, 뒤 장에서 이 근거를 어떤 저장 구조로 찾고, 그다음에는 어떤 실행까지 붙일지로 이어집니다.

이 흐름을 가장 짧게 다시 붙잡으면, 여기서는 RAG가 `어떤 문서를 먼저 붙여 답의 근거 범위를 좁힐 것인가`를 맡고, 바로 다음 P5-10.2와 P5-11.1, P5-11.2에서는 그 문서를 어떤 결합 흐름과 저장 구조로 찾을지 더 봅니다. 그리고 P5-12.1, P5-12.2와 P5-13.1, P5-13.2로 가면 문서를 읽는 것만으로 부족할 때 무엇을 실제로 조회하거나 실행하고, 그 검색과 실행을 어떤 순서로 이어 가며 언제 멈출지까지 확장됩니다.
