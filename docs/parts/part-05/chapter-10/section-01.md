...
질문이 `출장비 정산 기준이 어떻게 바뀌었나요?`라면, 사람은 먼저 기억나는 공지나 지난번 기준으로 답하려고 하기 쉽습니다. 하지만 내부 정책은 개정이 잦고, 예전 기준과 오늘 기준이 다를 수 있어서 `대충 기억나는 답`은 가장 위험한 방식이 되기 쉽습니다. 예를 들어 작년에는 교통비 한도가 없었지만 올해는 상한이 생겼다면, 기억에 의존한 답은 바로 잘못된 안내가 됩니다. RAG는 먼저 최신 사내 정책 문서를 검색해 현재 기준 문단을 찾고, 그 근거를 붙인 뒤 답하게 만듭니다. 그래서 이 장면에서 중요한 변화는 모델이 더 길게 말하는 것이 아니라, 답변 전에 `지금 유효한 문서`를 먼저 확인하는 절차가 생긴다는 점입니다.

### 사례 2. 제품 매뉴얼 기반 지원

제품 사용법을 묻는 고객 질문에 답하는 지원 챗봇을 생각해 볼 수 있습니다. 사람은 FAQ 몇 개와 자주 쓰는 답변 템플릿만 잘 정리해 두면 기본 질문은 처리할 수 있다고 생각하기 쉽습니다. 하지만 메뉴 이름이나 설정 위치가 버전마다 바뀌면 오래된 답을 그대로 내보내기 쉽습니다. 예를 들어 예전 버전에서는 `고급 설정` 아래 있던 기능이 새 버전에서는 `환경설정`으로 이동했다면, 템플릿 답변은 말투가 자연스러워도 바로 틀릴 수 있습니다. RAG는 최신 매뉴얼과 FAQ에서 관련 문서를 먼저 끌어와 그 내용을 바탕으로 답을 구성하게 만듭니다. 그래서 `기억에 있던 설명`보다 `현재 제품 문서에 있는 설명`이 먼저 오도록 흐름을 바꾸는 데 의미가 있습니다.

### 사례 3. 개발 문서 검색

개발자가 `현재 SDK 버전에서 인증 헤더를 어디에 넣나요?`라고 묻는 장면을 떠올려 볼 수 있습니다. 사람은 모델이 일반적인 API 지식을 많이 알고 있으니 바로 답해도 될 것이라고 생각하기 쉽습니다. 하지만 예전 버전 문법을 기억하고 있으면 얼핏 그럴듯한 답을 낼 수 있어도, 실제 코드에서는 바로 오류로 이어질 수 있습니다. 예를 들어 과거 버전의 `Authorization` 예제를 그대로 답하면, 현재 버전에서 `auth` 객체를 따로 넘겨야 하는 구조와 충돌할 수 있습니다. 이 경우 사람이 먼저 확인해야 하는 것은 모델의 일반 지식이 아니라 `지금 쓰는 버전 문서`입니다. RAG는 현재 API 문서와 예제 페이지를 먼저 찾아 문맥에 붙인 뒤 답을 만들게 해 이런 버전 불일치 위험을 줄입니다. 그래서 개발 문서 사례에서는 생성 능력보다 검색 단계의 정확성이 먼저 중요해집니다.
