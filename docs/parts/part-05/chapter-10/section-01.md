...
- 새 좌표계에서 샘플 위치를 같이 보면 왜 중간 표현이 중요한지 더 잘 보인다
...
이 질문은 바로 P5-10.2 깊은 층의 표현으로 이어집니다.
...
- 지금 보고 있는 차이가 `모델 이름 차이`인지, 아니면 `어떤 내부 표현을 더 잘 만들 수 있는가`의 차이인지 구분해 읽고 있는가?
- 사람이 만든 단일 점수와 모델이 학습한 다차원 표현의 차이를 실제 사례로 설명할 수 있는가?
- 딥러닝의 전환을 계산 자원만이 아니라 표현 학습 능력의 확대와 함께 설명할 수 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 구조 이름은 많은데 왜 성능 차이가 나는지 공통 축이 흐릴 때, 표현 학습 관점을 먼저 떠올립니다.
- 사람이 만든 특징과 모델이 스스로 만든 내부 좌표 차이를 설명해야 할 때, feature engineering 대비를 다시 봅니다.
- CNN, 임베딩, LLM을 하나의 흐름으로 묶어 읽어야 할 때, 모두 유용한 내부 표현을 더 잘 만드는 구조라는 점을 다시 꺼냅니다.
