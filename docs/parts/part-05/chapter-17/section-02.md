...
| --- | --- |
| request flow | 질문, 근거, 실행, 평가를 한 요청 안에서 어떻게 묶을 것인가 |
| minimum implementation | 그 묶음을 어떤 출력과 run record로 눈에 보이게 남길 것인가 |
| project records | 이 최소 기록이 Part 6에서 어떤 회고 산출물로 자랄 것인가 |
