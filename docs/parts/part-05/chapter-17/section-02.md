...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `요청 흐름 설계 -> run record로 남기기 -> 프로젝트 회고로 확장`이며, 여기서는 `run record로 남기기`를 맡습니다. Chapter 17은 Part 5의 마지막 본류 다리이고, 여기서 남기는 기록은 Part 6의 더 큰 프로젝트 산출물로 자라납니다.
