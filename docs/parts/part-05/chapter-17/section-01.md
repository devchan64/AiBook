...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 여기서는 Part 5의 개념들을 `요청 -> 근거/상태 확보 -> 출력 -> 기록`의 한 흐름으로 묶습니다. 바로 다음 절에서는 이 흐름을 최소 run record로 내려 보고, Part 6에서는 더 큰 프로젝트 산출물로 확장합니다.
