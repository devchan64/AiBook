...
요청 흐름 통합은 Part 5 본류의 `요청 구조 손잡이`로 읽고, 바로 다음 절에서 어떤 기록 층위가 더 붙는지 이어서 보면 됩니다.
...
즉, 이 절의 핵심은 `실패 경로를 고른다`에서 `그 판단까지 포함한 요청 하나의 구조를 묶는다`로 손잡이가 바뀌는 데 있습니다. 바로 다음 절에서는 그 구조를 run record와 회고 포인트로 남깁니다.

특히 여기서는 평가와 운영에서 만든 판단이 실제 요청 기록으로 어떻게 내려오는지도 같이 붙잡아야 합니다.

| 앞 절에서 이미 만든 판단 | 지금 요청 흐름 안에 넣어야 하는 것 | 바로 다음 절에서 남길 대표 필드 |
| --- | --- | --- |
| 자동 게이트와 사람 검토 결과 | 이 답을 바로 채택할지, 추가 확인으로 돌릴지 | `answer_status`, `needs_human_review`, `review_summary` |
...
Part 5 뒤쪽 본류는 여기서 `좋은 답 판정`과 `실패 시 운영 경로 선택`을 `질문, 근거/상태, 답변, 평가, 기록을 한 요청 흐름으로 묶는 일`로 닫습니다.
...
여기서는 Part 5의 개념들을 `요청 -> 근거/상태 확보 -> 출력 -> 기록`의 한 흐름으로 묶습니다.
