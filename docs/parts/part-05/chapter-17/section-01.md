...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `실패 경로 판단 -> 기록 가능한 요청 흐름으로 묶기 -> run record로 남기기`이며, 여기서는 `기록 가능한 요청 흐름으로 묶기`를 맡습니다.
...
Part 5 뒤쪽 본류를 여기까지 이어서 가장 짧게 다시 묶으면, 앞 장들에서 세운 `좋은 답 판정`과 `실패 시 운영 경로 선택`이 여기서 `질문, 근거/상태, 답변, 평가, 기록을 한 요청 흐름으로 묶는 일`로 닫힙니다.
...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 여기서는 Part 5의 개념들을 `요청 -> 근거/상태 확보 -> 출력 -> 기록`의 한 흐름으로 묶습니다.
