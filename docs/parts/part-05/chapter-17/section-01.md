...
이 전환을 바로 앞 절과 다음 Part까지 가장 짧게 붙이면 아래처럼 읽는 편이 안전합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| failure handling | 실패가 났을 때 retry, fallback, stop, approval 중 어디로 보낼 것인가 |
| request flow | 그 판단이 질문, 근거/상태, 답변, 평가, 기록으로 어떻게 한 요청 안에 남는가 |
| project records | 이 최소 기록이 Part 6에서 어떤 산출물과 회고 문서로 자라는가 |

Part 5 뒤쪽 본류를 여기까지 이어서 가장 짧게 다시 묶으면 다음과 같습니다.

| 바로 앞 장들에서 이미 세운 것 | 지금 이 절에서 하나로 묶는 것 | Part 6에서 더 크게 자라는 것 |
| --- | --- | --- |
| 좋은 답을 고르는 기준 | 질문, 근거/상태, 출력, 평가를 한 요청 흐름으로 묶기 | 프로젝트 산출물, 배포, 모니터링 |
| 실패 시 어떤 운영 경로로 보낼지 | 그 판단이 남는 최소 run record 구조 | execution_records, review_summary, improvement_plan |

즉, 이 절은 새 개념을 하나 더 붙이는 절이 아니라, 평가와 운영을 거친 판단이 `기록 가능한 요청 흐름`으로 닫히는 Part 5 본류 마지막 통합 단계입니다.

처음 읽을 때는 아래 세 줄만 바로 구분해도 충분합니다.

| 지금 이 절에서 먼저 못 박을 것 | 바로 앞 절에서 이미 끝낸 것 | Part 6으로 넘길 것 |
| --- | --- | --- |
| 질문, 근거/상태, 답변, 평가, 기록을 한 요청 흐름으로 묶는다 | 어떤 후보를 통과·수정·중단할지와 실패 경로를 고르는 일 | `execution_records`, `review_summary`, `improvement_plan` 같은 프로젝트 산출물 |
| 기록 가능한 요청 구조를 만든다 | 운영 복구 경로 자체를 더 늘리지 않는다 | 더 큰 구현·배포·회고 흐름 |
...
| --- | --- | --- |
| 여기서는 Part 5의 개념들을 `요청 -> 근거/상태 확보 -> 출력 -> 기록`의 한 흐름으로 다시 묶는다. | P5-17.2에서는 이 흐름을 더 작은 최소 구현과 회고 포인트로 실제처럼 내려 봅니다. | Part 6처럼 프로젝트 범위, 배포, 모니터링까지 확장하는 단계는 아직 아닙니다. |
| 평가와 운영은 부록이 아니라, 작은 기능을 설계할 때 처음부터 함께 들어가야 하는 구조다. | 바로 다음 절에서는 어떤 출력과 기록을 먼저 확인해야 하는지 더 구체적으로 봅니다. | 실제 상용 API 문법이나 복잡한 멀티에이전트 구조까지는 아직 여기서 늘리지 않습니다. |
| 핵심은 모델 호출 한 번이 아니라 `기록 가능한 요청 흐름`을 만들었다는 점이다. | 뒤 절과 Part 6으로 갈수록 이 흐름이 run record와 프로젝트 산출물 언어로 더 구체화됩니다. | 운영 체계 전체를 문서화하는 일은 아직 여기서 끝내지 않습니다. |
