...
입력된 안내 문장을 더 공손하게 다듬는 기능을 생각해 볼 수 있습니다. 사람은 생성형 AI 기능이면 검색이나 도구가 항상 붙어야 한다고 느끼기 쉽지만, 이 경우 먼저 확인할 것은 `현재 문장 자체`이고 외부 문서나 실시간 시스템 값은 필요하지 않을 수 있습니다. 그래서 핵심 구조는 좋은 지시와 예시를 넣는 프롬프트 쪽에 더 가깝습니다. 즉, 바꿔야 할 대상이 이미 입력 안에 다 들어 있으면 prompt 중심 구조로 충분할 수 있습니다. 이 사례는 모든 생성형 AI 기능에 검색이나 도구가 필요한 것은 아니라는 점을 보여 줍니다.

### 사례 2. 사내 정책 안내

사내 정책 안내 기능에서 사용자가 `육아휴직 신청 순서는 어떻게 되나요?`라고 묻는다고 해 봅시다. 사람은 먼저 모델이 설명을 잘 써 주면 충분하다고 느끼기 쉽지만, 이때 먼저 떠올려야 하는 것은 문장 능력보다 `현재 규정 문서가 어디에 있는가`입니다. 정책은 바뀔 수 있으므로, 프롬프트만으로 답하기보다 관련 규정을 검색해 근거 문장을 붙이는 구조가 먼저 필요합니다. 그래서 이 기능은 prompt만으로 닫히지 않고 retrieval이 중심 구조로 들어옵니다. 이 사례는 `현재 문서가 중요한 질문`에서는 검색 단계가 필수로 붙는다는 점을 보여 줍니다.

### 사례 3. 잔여 휴가 조회

잔여 휴가 조회는 한 단계 더 다릅니다. 정책 문서만 읽어서는 `연차는 어떤 기준으로 계산되는가`는 설명할 수 있어도, `내 계정에 실제로 며칠이 남았는가`는 알 수 없습니다. 사람은 정책을 찾았으니 답도 끝났다고 느끼기 쉽지만, 이 질문을 닫으려면 규정뿐 아니라 인사 시스템 값 조회가 필요하다는 점을 바로 구분해야 합니다. 그래서 이 기능은 retrieval만으로도 부족하고, 실제 상태를 읽는 tool use가 붙어야 닫힙니다. 이 사례는 검색과 도구 사용이 왜 서로 다른 역할을 가지는지 한 번에 보여 줍니다.
