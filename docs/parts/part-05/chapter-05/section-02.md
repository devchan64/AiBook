...
- 계산 그래프를 `연산 의존 관계를 펼쳐 놓은 그림`으로 설명할 수 있는가?
- forward에서 만든 중간값이 backward에서 왜 다시 필요한지 말할 수 있는가?
- 이 절 다음에는 gradient를 실제 업데이트에 쓰는 optimizer와 학습/추론 구분으로 넘어간다는 흐름을 이해했는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 수식이 너무 길어 전체 구조가 한 번에 안 보일 때, 계산 그래프로 연산 블록과 의존 관계를 먼저 펼쳐 봅니다.
- forward에서 만든 중간값이 왜 backward에 다시 필요한지 설명해야 할 때, 그래프 위 경로와 저장값을 다시 확인합니다.
- 자동미분이 마법처럼 느껴질 때, 국소 미분 규칙을 그래프를 따라 조직한 절차라는 관점을 꺼냅니다.
