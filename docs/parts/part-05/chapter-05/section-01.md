...
이 질문은 바로 P5-5.2 계산 그래프(computation graph)로 이어집니다.
...
- 역전파와 optimizer의 역할 차이를 말할 수 있는가?
- 이 절은 역전파의 직관을 닫고, 복잡한 연산 연결은 계산 그래프 절로 넘긴다는 흐름을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 손실이 계산된 뒤 각 가중치가 어떻게 책임을 나눠 가지는지 설명이 막힐 때, 역전파 관점을 먼저 떠올립니다.
- 역전파와 optimizer를 같은 단계처럼 섞고 있을 때, gradient 계산과 실제 업데이트를 다시 분리합니다.
- 연쇄 법칙이 왜 딥러닝에서 반복적으로 등장하는지 묶어 설명해야 할 때, 뒤에서 앞으로 책임을 전파하는 구조를 다시 봅니다.
