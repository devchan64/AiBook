...
통계적 언어 모델의 긴 발전사나 BERT 계열과의 비교사는 여기서 길게 끌지 않습니다. 이 절은 생성형 LLM의 기본 학습 목표를 먼저 붙잡고, 배경 비교는 뒤의 배경 축에서만 짧게 다시 정리합니다.
...
통계적 언어 모델의 상세 발전사와 BERT 계열 비교는 뒤의 배경 축에서 비교 기준으로만 다시 읽습니다.
