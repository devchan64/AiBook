...
통계적 언어 모델의 긴 발전사나 BERT 계열과의 비교사는 여기서 길게 끌지 않습니다. 이 절은 생성형 LLM의 기본 학습 목표를 먼저 붙잡습니다.
...
생성 시 선택 전략의 차이는 바로 다음 P5-5.2 생성 과정의 직관에서 다시 회수합니다. 정렬과 후속 조정 단계는 P5-8.1 지시 튜닝, P5-8.2 정렬의 기본 문제에서 다시 다루며, 손실 수식 자체는 현재 본편 범위 밖으로 둡니다. 통계적 언어 모델 발전사와 BERT 계열 비교는 뒤에서 비교 기준으로만 다시 읽습니다.
