...
뒤 절에서 계산 그래프나 optimizer와 섞여 보일 때는 개념사전의 [역전파(backpropagation)](../../../reference/concept-glossary-parts/08-ieung.md#backpropagation), [그래디언트(gradient)](../../../reference/concept-glossary-parts/01-giyeok.md#gradient), [연쇄 법칙(chain rule)](../../../reference/concept-glossary-parts/08-ieung.md#chain-rule) 항목으로 돌아가 계산 역할부터 다시 구분합니다.
...
실제 딥러닝 구현에서는 샘플별 손실 하나를 그대로 쓰기보다, 배치 평균이나 합으로 줄인 값에 정규화 항을 더한 `objective/cost`를 미분하는 경우가 많습니다. 다만 독자가 gradient 역할을 먼저 붙잡는 단계에서는, 이 전체 목적함수의 핵심 출발점이 `손실이 만들어 내는 어긋남 신호`라고 이해하면 충분합니다.
...
## 손실은 어떻게 gradient 신호가 되는가: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "손실이 gradient 신호로 바뀌는 흐름을 분명히 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
