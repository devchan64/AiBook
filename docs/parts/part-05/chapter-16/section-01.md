...
P5-15.2에서는 자동 평가와 사람 평가가 서로 다른 장단점을 가지며 실제 운영에서는 함께 쓰이는 경우가 많다는 점을 보았습니다. 하지만 평가만 좋아도 서비스가 바로 성립하는 것은 아닙니다. 이 절에서는 품질이 괜찮아 보여도 실제 서비스에서 다시 발목을 잡는 조건을 봅니다.
...
구체적인 클라우드 가격 비교와 GPU 인프라 설계 세부는 여기서 다루지 않습니다. 대신 실제 실패 추적과 대응 기준은 바로 다음 P5-16.2에서 다시 회수합니다. 가격표와 수치 설계 전부는 빠르게 바뀌는 주제이므로 현재 본편 범위 밖에 둡니다.
...
처음 읽을 때는 지금 장의 층위를 다음처럼 잡아 두면 흐름이 덜 끊깁니다.

| 앞 절에서 가져온 것 | 지금 절에서 새로 묻는 것 | 다음 절로 넘길 것 |
| --- | --- | --- |
| 자동 평가, 사람 평가, 실행 기록 | 이 답을 실제 서비스 비용과 속도로 계속 제공할 수 있는가 | 실패 시 retry, fallback, stop을 어디서 고를 것인가 |

즉, P5-15에서 `괜찮은 답인가`를 가렸다면 여기서는 `그 괜찮은 답을 운영 한도 안에서 반복 제공할 수 있는가`를 다시 묻습니다. 이 기준이 서야 평가를 통과한 답이 왜 실제 서비스 후보에서는 다시 탈락할 수 있는지 자연스럽게 이해할 수 있습니다.

처음 읽을 때는 서비스 운영 제약을 Part 5 본류의 `서비스 한도 손잡이`로만 잡고, 그 다음에 어떤 복구 층위가 더 붙는지까지 같이 보면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 서비스 운영 제약 | 어떤 후보를 비용, 속도, 용량 한도 안에 둘 수 있는가? | P5-16.1 |
| 실패 대응 | 운영 한도를 넘기거나 실행이 끊기면 retry, fallback, stop 중 어디로 보낼 것인가? | P5-16.2 |
| 통합 미니 실습과 회고 | 이 운영 판단을 실제 요청 흐름과 기록 필드로 어떻게 남길 것인가? | P5-17.1, P5-17.2 |

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| 평가: 무엇을 통과시키고 무엇을 다시 볼까 | 운영 제약: 그 답을 비용과 속도 안에서 계속 제공할 수 있을까 | 실패 대응: 실제로 문제가 나면 어디서 멈추고 어떤 경로로 복구할까 |
| 품질 판단 | 서비스 한도 판단 | 복구 경로 판단 |

즉, 지금 장의 핵심은 `좋은 답인가`에서 `그 좋은 답을 실제 서비스 안에서 유지할 수 있는가`로 손잡이가 바뀐다는 점입니다.

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| evaluation | 어떤 후보가 품질 기준을 통과했는가 |
| operations | 그 후보를 비용, 속도, 용량 안에서 계속 제공할 수 있는가 |
| failure handling | 운영 한도를 넘기면 어디서 줄이고 어디서 멈출 것인가 |
...
여기서 `더 많은 기능 = 항상 더 좋은 선택`이 아니라는 점을 먼저 기억하면 좋습니다.
...
사례를 읽을 때는 `좋아 보이는가`보다 `운영 제약 안에서 계속 제공 가능한가`를 중심으로 보면 좋습니다.
...
처음 읽을 때는 이 세 사례를 아래처럼 더 짧은 판단 장면으로 줄여 봐도 충분합니다.

| 아주 작은 장면 | 먼저 탈락시키는 기준 | 다음 조정 방향 |
| --- | --- | --- |
| 비밀번호 재설정 질문에도 8초가 걸린다 | latency 초과 | 검색 단계 줄이기, 짧은 답 우선 |
| 변수명 수정에도 전체 테스트를 반복한다 | 비용 과다 | 작업 크기별 검사 강도 분리 |
| 환불 규정 질문에 문서 10개를 모두 붙인다 | 문맥 과밀 | top-k 축소, 예외 조항 우선 |
...
- 바로 앞의 P5-15.1, P5-15.2 평가 절에서 `좋은 답인지`를 점검했다면, 이제는 그 답을 실제로 `빠르고 싸고 안정적으로` 제공할 수 있는지 보게 하고
...
이 질문은 P5-16.2 운영 중 실패 대응으로 이어집니다.
