...
P5-15.2에서는 자동 평가와 사람 평가가 서로 다른 장단점을 가지며 실제 운영에서는 함께 쓰이는 경우가 많다는 점을 보았습니다. 하지만 평가만 좋아도 서비스가 바로 성립하는 것은 아닙니다. 이제 더 현실적인 질문이 나옵니다.
...
이 절은 운영 제약의 큰 종류를 구분하는 데 집중하고, 실제 실패 추적과 대응 기준은 바로 다음 P5-16.2 운영 중 실패 대응에서 다시 회수합니다. 구체적인 가격표와 인프라 수치 설계 전부는 빠르게 바뀌는 주제이므로 현재 본편 범위 밖으로 둡니다.
