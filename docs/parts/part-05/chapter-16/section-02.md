...
Part 5 뒤쪽 본류는 `평가 기준을 세운다 -> 운영 한도와 실패 경로를 정한다 -> 그 판단을 실제 기록으로 남긴다` 흐름으로 읽으면 됩니다.

즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `좋은 답 판단 -> 실패 경로 결정 -> 기록 가능한 요청 흐름`이며, 여기서는 `실패 경로 결정`을 맡습니다.
...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 실패 대응은 평가 다음에 `어디서 멈추고 어디로 복구할까`를 고르는 운영 복구 구조입니다.
