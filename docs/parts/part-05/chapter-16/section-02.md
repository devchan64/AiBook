...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `좋은 답 판단 -> 실패 경로 결정 -> 기록 가능한 요청 흐름`이며, 여기서는 `실패 경로 결정`을 맡습니다. 여기서 정한 `retry`, `fallback`, `stop`, `approval` 같은 복구 경로는 바로 다음 P5-17에서 요청 흐름 기록으로 남습니다.
