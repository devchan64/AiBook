...
핵심은 평가 뒤에 운영이 덧붙는 것이 아니라, 좋은 답 판단이 실제 서비스 경로를 만나면서 비로소 Part 5 본류 마지막 단계로 닫히기 시작한다는 점입니다. 여기서 먼저 못 박을 것은 실패 시 `retry`, `fallback`, `stop`, `approval` 중 어디로 갈지와 운영 복구 경로를 고르는 일이고, 이 판단 자체를 요청 흐름 기록과 프로젝트 산출물 형태로 완전히 정리하는 일은 바로 다음 P5-17로 넘깁니다. 다음 장에서는 이 운영 판단이 실제 `run record`, `trace`, `incident_records` 같은 기록 구조로 남습니다.
...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 실패 대응은 `어디서 멈추고 어디로 복구할까`를 고르는 운영 복구 구조이고, 평가는 `채택할 수 있는가`를 가른다면 실패 대응은 `채택할 수 없을 때 어떤 경로를 탈까`를 가릅니다. 그래서 핵심은 오류를 잡는 것보다 `복구 경로와 다음 조치`를 함께 남기는 데 있고, P5-17에서는 이 판단을 실제 요청 하나의 흐름과 기록 안에 어떻게 남길지 더 봅니다. 반면 Part 6처럼 배포 단위 프로젝트 문서, 장기 운영 회고, 개별 벤더의 온콜 체계 전체는 아직 여기서 닫지 않습니다.
