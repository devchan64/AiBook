...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `좋은 답 판단 -> 실패 경로 결정 -> 기록 가능한 요청 흐름`이며, 여기서는 그 가운데 단계인 `실패 경로 결정`을 맡습니다. 핵심은 평가 뒤에 운영이 덧붙는 것이 아니라, 좋은 답 판단이 실제 서비스 경로를 만나면서 비로소 Part 5 본류 마지막 단계로 닫히기 시작한다는 점이고, 여기서 정한 `retry`, `fallback`, `stop`, `approval` 같은 복구 경로는 바로 다음 P5-17에서 `run record`, `trace`, `incident_records` 같은 기록 구조로 남습니다.
