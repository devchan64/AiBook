...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. agent는 `다음에 무엇을 하고 언제 멈출까`를 다루는 목표 흐름 구조이고, RAG와 tool use가 한 번의 읽기나 실행을 닫는다면 agent는 그 단위들을 순서로 이어 붙입니다. 핵심은 도구 수가 아니라 `현재 상태를 보고 다음 행동을 다시 고르는 반복 구조`에 있습니다. 그래서 P5-13.2에서는 계획, 행동, 관찰이 실제로 어떤 반복 루프를 이루는지 더 보고, P5-14에서는 이 흐름을 어떤 연결 규칙과 실행 환경 안에서 관리할지 더 봅니다. 뒤 절로 갈수록 이 반복 구조는 `trace`, `approval`, `replay` 같은 기록 요구로 이어지지만, MCP처럼 도구 연결을 공통 인터페이스로 정리하는 문제나 평가·운영처럼 남은 기록으로 품질과 실패를 통제하는 단계는 아직 여기서 닫지 않습니다.
