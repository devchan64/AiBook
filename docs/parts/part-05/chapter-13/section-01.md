...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. agent는 `다음에 무엇을 하고 언제 멈출까`를 다루는 목표 흐름 구조입니다. RAG와 tool use가 한 번의 읽기나 실행을 닫는다면 agent는 그 단위들을 순서로 이어 붙이고, P5-13.2에서는 이 흐름이 plan-action-observation 루프로 어떻게 반복되는지 더 봅니다. 그다음 P5-14에서는 이 반복 흐름을 어떤 연결 규칙과 실행 환경 안에서 관리할지로 넘어갑니다.
