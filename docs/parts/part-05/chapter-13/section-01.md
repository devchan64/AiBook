...
| MCP / harness 관리 | 그 흐름을 어떤 연결 규칙과 기록 환경 안에서 관리할 것인가? | P5-14.1, P5-14.2 |

즉, 이 절에서 붙잡아야 할 핵심은 `한 번의 실행 요청`에서 `여러 읽기와 실행의 순서를 어떻게 바꿀 것인가`로 손잡이가 커진다는 점입니다. 바로 다음 절에서 이 흐름은 plan-action-observation 루프로 구체화되고, P5-14에서는 연결 규칙과 실행 관리로 이어집니다.
...
연속 독해에서 이 구간이 쉽게 한 덩어리로 보일 수 있으므로, Chapter 12~14의 최소 차이를 한 번 더 짧게 고정해 두면 안전합니다.

| 지금 읽는 장 | 가장 먼저 붙잡을 질문 | 바로 다음 장으로 넘어갈 때 바뀌는 중심 |
| --- | --- | --- |
| tool use | 무엇을 실제로 조회하거나 실행할까? | 실행 요청을 어떤 이름과 인자 구조로 넘길까 |
| function calling | 그 실행 요청을 어떻게 검증 가능한 payload로 만들까? | 여러 호출을 어떤 목표 순서로 이어 갈까 |
...
이 표의 목적은 장 이름을 더 외우게 하는 데 있지 않습니다. 현재 절에서는 `도구 하나를 부르는 단계`, `그 호출을 구조화하는 단계`, `여러 단계를 목표 흐름으로 묶는 단계`, `그 흐름을 연결 규칙과 실행 기록으로 감싸는 단계`가 서로 다른 층위라는 점만 분명히 잡으면 충분합니다.
...
agent는 `다음에 무엇을 하고 언제 멈출까`를 다루는 목표 흐름 구조입니다. 한 번의 읽기나 실행을 넘어 여러 단위를 순서로 이어 붙이고, P5-13.2에서는 이 흐름이 plan-action-observation 루프로 어떻게 반복되는지 더 봅니다. 그다음 P5-14에서는 이 흐름의 연결 규칙과 실행 관리로 넘어갑니다.
