...
여기서 먼저 남겨야 할 것은 어떤 목표를 어떤 순서로 풀려고 했는지를 보여 주는 `planned_steps`, 중간 관찰 메모와, 왜 멈췄고 왜 사람 검토나 재시도로 넘겼는지를 보여 주는 `next_action`, 종료 이유입니다. 이 기록이 있어야 다음 행동 변경 이유와 흐름 단위 실패를 다시 볼 수 있고, 뒤로 갈수록 P5-13.2의 계획-행동-관찰 루프, P5-16.2의 실패 대응, Part 6의 `execution_records`, `incident_records`, `next_action`, 회고 문서로 다시 읽힙니다.
