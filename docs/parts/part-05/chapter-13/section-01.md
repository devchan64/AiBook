...
Part 5 본류를 이 구간에서 가장 짧게 다시 묶으면 `RAG는 무엇을 근거로 읽을까`, `tool use는 무엇을 실제로 실행할까`, `agent는 그 읽기와 실행을 어떤 순서로 이어 갈까`입니다. 이 구분이 보여야 세 구조가 모두 `외부 연결`처럼 뭉개지지 않고, 지금 장이 새 기능 이름 하나를 더 외우는 절이 아니라 `여러 단계를 이어 가는 판단 구조`를 읽는 절로 잡힙니다.

처음 읽을 때는 agent를 Part 5 본류의 `목표 흐름 손잡이`로만 잡고, 그 다음에 어떤 관리 층위가 더 붙는지까지 같이 보면 흐름이 덜 끊깁니다.

처음 읽을 때는 agent 용어를 넓게 잡기보다 `무엇을 먼저 읽고 무엇을 나중에 실행할지 순서를 정해야 하는가`, `중간 결과를 보고 다음 행동이 바뀌는가`, `그 흐름을 바로 뒤 장과 하네스 층위에서 어떻게 관리하게 되는가`라는 세 질문만 먼저 붙잡으면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| agent가 여러 읽기와 실행을 목표 기준으로 이어 가는 구조라는 점 | P5-13.2에서 계획, 행동, 관찰을 어떤 루프로 읽을지로 이어집니다. | 이후 작업 분해, 재시도, 종료 판단, 회고 기록 설명에서 반복됩니다. |
| tool use와 agent의 차이가 `한 번의 실행`과 `여러 단계 흐름`의 차이라는 점 | 어떤 문제에서 agent까지 필요한지 판단하는 기준으로 이어집니다. | 이후 운영 실패 분석, 에이전트 설계, 프로젝트 구조 설명에서 계속 중요합니다. |
| agent 다음에 MCP와 하네스가 붙는 이유가 흐름 관리와 기록 때문이라는 점 | P5-14에서 연결 규칙과 실행 환경을 왜 따로 다루는지로 이어집니다. | 이후 승인 기록, trace/log, 실행 환경 설명의 바탕이 됩니다. |

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| function calling | 한 번의 실행 요청을 어떤 payload로 넘길 것인가 |
| agent | 여러 호출과 읽기를 어떤 순서로 이어 갈 것인가 |
| MCP / harness | 그 흐름을 어떤 연결 규칙과 기록 환경 안에서 관리할 것인가 |

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 에이전트(agent) | 여러 읽기와 실행을 어떤 순서로 이어 가며 언제 다음 행동을 바꿀 것인가? | P5-13.1, P5-13.2 |
| MCP | 그 도구 연결을 어떤 공통 인터페이스로 정리할 것인가? | P5-14.1 |
| 하네스(harness) | 그 실행 흐름과 기록을 어떤 환경 안에서 통제하고 남길 것인가? | P5-14.2 |
| 평가와 운영 | 남은 기록으로 무엇이 괜찮은 실행이었는지 어떻게 판정하고 실패를 어떻게 다룰 것인가? | P5-15.1, P5-15.2, P5-16.1, P5-16.2 |

| 지금 agent에서 먼저 남길 기록 | 왜 지금 필요한가 | 뒤 절과 Part 6에서 다시 읽는 기록 |
| --- | --- | --- |
| `planned_steps`, 중간 관찰 메모 | 어떤 목표를 어떤 순서로 풀려고 했는지 남겨야 다음 행동 변경 이유를 다시 볼 수 있어서 | P5-13.2의 계획-행동-관찰 루프와 Part 6의 `execution_records`, 회고 문서로 이어진다 |
| `next_action`, 종료 이유 | 왜 멈췄고 왜 사람 검토나 재시도로 넘겼는지 남겨야 흐름 단위 실패를 구분할 수 있어서 | P5-16.2의 실패 대응과 Part 6의 `incident_records`, `next_action`으로 이어진다 |
...
이 표에서 먼저 읽어야 할 점은 agent가 단순히 도구를 더 많이 붙인 버전이 아니라, `다음 단계 선택` 자체를 중심 문제로 바꾼다는 것입니다. 그래서 지금 장은 기능 목록을 늘리는 절이 아니라, 앞 절의 읽기와 실행을 `목표 기준 순서 결정`으로 다시 묶는 절입니다.

바로 앞 장과 연결해 다시 보면, 독자는 다음 순서로 경계를 잡으면 충분합니다.
...
| --- | --- | --- |
| agent는 `다음에 무엇을 하고 언제 멈출까`를 다루는 목표 흐름 구조다. | P5-13.2에서 계획, 행동, 관찰이 실제로 어떤 반복 루프를 이루는지 더 봅니다. | MCP처럼 도구 연결을 공통 인터페이스로 정리하는 문제는 아직 아닙니다. |
| RAG와 tool use가 한 번의 읽기나 실행을 닫는다면, agent는 그 단위들을 순서로 이어 붙인다. | P5-14에서는 이 흐름을 어떤 연결 규칙과 실행 환경 안에서 관리할지 더 봅니다. | 평가와 운영처럼 남은 기록으로 품질을 판정하고 실패를 통제하는 단계는 아직 아닙니다. |
| 핵심은 도구 수가 아니라 `현재 상태를 보고 다음 행동을 다시 고르는 반복 구조`에 있다. | 뒤 절로 갈수록 이 반복 구조가 trace, approval, replay 같은 기록 요구로 이어집니다. | 하네스 이후의 평가/운영 판단까지는 아직 여기서 끝내지 않습니다. |
