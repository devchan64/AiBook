...
이 질문은 바로 P5-13.2 self-attention으로 이어지는 흐름으로 연결됩니다.
...
- 상태를 오래 보존하는 설명만으로는 왜 성능이 막히는지 부족할 때, attention의 직접 참조 관점을 먼저 떠올립니다.
- 모든 입력 위치를 비슷하게 다루면 안 되는 이유를 설명해야 할 때, 현재 질문 기준으로 더 중요한 위치를 다시 보는 구조를 다시 꺼냅니다.
- 장기 의존성 다음에 왜 attention이 자연스러운 전환인지 묶어 설명해야 할 때, `오래 들고 간다`보다 `필요할 때 다시 본다`는 관점을 다시 봅니다.
