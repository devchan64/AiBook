...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `function calling -> agent 목표 흐름 -> MCP/harness 관리`입니다. 여기서는 그 가운데 단계인 `목표 흐름`을 맡고 있고, 바로 뒤 장에서 이 흐름이 계획-행동-관찰 루프로 구체화되며, 다음 장에서 연결 규칙과 실행 기록으로 넘어갑니다. 처음 읽을 때는 `여러 읽기와 실행을 어떤 순서로 이어 가며 언제 다음 행동을 바꿀 것인가`를 지금 절의 질문으로 붙잡고, P5-14.1에서는 그 도구 연결을 어떤 공통 인터페이스로 정리할지, P5-14.2에서는 그 실행 흐름과 기록을 어떤 환경 안에서 통제하고 남길지, P5-15~16에서는 남은 기록으로 무엇이 괜찮은 실행이었는지 판정하고 실패를 어떻게 다룰지를 차례로 보면 충분합니다.
