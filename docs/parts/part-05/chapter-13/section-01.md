...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `function calling -> agent 목표 흐름 -> MCP/harness 관리`입니다. 여기서는 `목표 흐름`을 맡고, 바로 다음 절에서 이 흐름이 plan-action-observation 루프로 구체화된 뒤 P5-14의 연결 규칙과 실행 관리로 넘어갑니다. 처음 읽을 때는 `여러 읽기와 실행을 어떤 순서로 이어 가며 언제 다음 행동을 바꿀 것인가`만 붙잡으면 충분합니다.
