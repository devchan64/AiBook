...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `function calling -> agent 목표 흐름 -> MCP/harness 관리`입니다. 여기서는 그 가운데 단계인 `목표 흐름`을 맡고 있고, 바로 뒤 장에서 이 흐름이 계획-행동-관찰 루프로 구체화된 뒤 연결 규칙, 실행 기록, 평가와 실패 대응으로 이어집니다. 처음 읽을 때는 `여러 읽기와 실행을 어떤 순서로 이어 가며 언제 다음 행동을 바꿀 것인가`를 지금 절의 질문으로 붙잡으면 충분합니다.
