# P5-13.2 인덱스(index)와 검색 품질

P5-13.1에서는 벡터 데이터베이스가 임베딩 벡터와 원문, 메타데이터를 함께 저장하고 검색 단계에서 실무형 저장소 역할을 한다는 점을 보았습니다. 이제 질문은 더 구체적이 됩니다.
...
이 절은 인덱스를 `근사 검색을 위한 구조`로 읽는 데 집중하고, 이후 서비스 안에서 검색 외부 기능이 어떻게 확장되는지는 P5-14.1 도구 사용으로 이어집니다. 엔진별 수학과 분산 검색 아키텍처의 세부 구현은 현재 본편 범위 밖으로 둡니다.
...
문서 수가 적을 때는 문제 없던 검색이, 문서가 급격히 늘면 갑자기 느려질 수 있습니다. 이 시점부터 인덱스 구조와 검색 전략이 중요한 운영 문제가 됩니다.

### 사례 2. 매뉴얼 답변 품질

빠른 검색을 위해 너무 공격적인 근사 설정을 쓰면, 가장 중요한 매뉴얼 조각을 놓칠 수 있습니다. 반대로 너무 정확도만 추구하면 응답 지연 시간이 커질 수 있습니다.

### 사례 3. 개발 문서 도우미

API 문서가 많아질수록 잘못된 top-k 결과가 답변 전체를 흐릴 수 있습니다. 이 경우 검색 품질 평가가 따로 필요합니다.
...
이 질문은 P5-14.1 도구 사용(tool use)으로 이어집니다.
