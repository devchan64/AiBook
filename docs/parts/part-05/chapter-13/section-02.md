...
| --- | --- | --- |
| agent 목표 흐름 | 여러 읽기와 실행을 어떤 목표 흐름으로 이어 갈까? | P5-13.1 |
| plan-action-observation 반복 루프 | 그 목표 흐름이 실제로 어떤 반복 루프로 움직일까? | P5-13.2 |
| MCP / harness 관리 | 이 루프를 어떤 연결 규칙과 기록 환경 안에서 관리할까? | P5-14.1, P5-14.2 |

즉, 지금 장의 핵심은 `여러 단계를 이어 갈까`에서 `그 단계들이 어떤 관찰과 결정 루프로 반복될까`로 손잡이가 바뀌고, 이 절이 `agent 목표 흐름 -> plan-action-observation 반복 루프 -> MCP/harness 관리` 가운데 `반복 루프` 단계를 맡는다는 점입니다.
