...
| 사람이 먼저 보기 쉬운 기준 | 표현 관점으로 다시 읽는 기준 |
| --- | --- |
| 픽셀을 많이 보면 곧바로 상태를 맞힐 수 있다고 느낀다 | 픽셀은 재료일 뿐이고, 먼저 중간 표현이 만들어져야 한다고 본다 |
| 바늘, 경광등, 밸브 같은 단서를 최종 라벨과 거의 같게 본다 | 단서는 중간 표현의 일부이고, 최종 상태 판단은 그 조합 결과라고 구분한다 |
| 비슷한 장면이면 곧바로 헷갈린다고만 본다 | 어떤 내부 축이 더 강하게 살아나는지 보면 왜 갈리는지 더 선명해진다 |
...
| 사람이 먼저 보기 쉬운 기준 | 은닉층 표현으로 다시 읽는 기준 |
| --- | --- |
| 숫자 네 개를 각각 따로 읽는다 | 여러 입력이 합쳐져 만든 중간 상태를 먼저 본다 |
| 경보 하나만 보면 아직 버틴다고 보기 쉽다 | 진동, 온도, 복구 지연이 함께 만든 위험 표현을 본다 |
| 어떤 조합이 문제인지 설명이 흐릴 수 있다 | 어떤 내부 축이 최종 판단에 가까운지 더 분명해진다 |

그래서 이 사례에서 확인해야 할 결과는 원래 숫자 열을 따로 보는 것보다, 여러 신호가 묶인 내부 표현 쪽이 설비 상태를 더 잘 가르는가입니다.

이 두 사례를 한 줄로 묶으면 다음과 같습니다.

`은닉층은 원래 입력을 바로 판단하지 않고, 더 나누기 쉬운 내부 표현으로 다시 적는 층이다.`

이 두 사례를 `원래 입력을 보는 기준`과 `은닉 표현을 보는 기준`으로 다시 묶어 보면 차이가 더 선명해집니다.

| 장면 | 원래 입력만 볼 때 남기 쉬운 해석 | 은닉 표현까지 보면 더 분명해지는 해석 | 최종 판단 전에 먼저 살아나야 하는 내부 축 |
| --- | --- | --- | --- |
| 설비 계기판 경고 상태 판별 | 픽셀이나 윤곽이 비슷하면 곧바로 헷갈린다고만 본다 | 바늘 위치, 경광등, 밸브 방향 같은 중간 표현 조합이 왜 정상 감시와 즉시 점검을 가르는지 더 잘 보인다 | 경고 상태 조합을 요약한 중간 표현 |
| 설비 경고 상태 표현 | 경보 횟수, 진동, 온도 상승, 복구 지연을 각각 따로 읽는다 | 정상 운전 유지 축과 고장 전조 위험 축처럼 문제에 더 가까운 내부 상태가 먼저 보인다 | 안정성/위험도를 다시 적은 내부 축 |

이 표에서 독자가 먼저 붙잡아야 할 결과는, 은닉층의 핵심이 `숫자를 더 계산한다`가 아니라 `최종 판단 전에 먼저 보게 될 내부 축을 다시 만든다`는 점입니다.
...
| 실행 결과에서 보인 차이 | 원래 입력만 보면 남기 쉬운 해석 | 은닉 표현까지 보면 바뀌는 해석 |
| --- | --- | --- |
| `needle_dominant_frame`과 `borderline_watch_frame`는 둘 다 강한 정지 장면은 아니다 | 둘 다 비슷한 약한 이상 장면이라고만 읽기 쉽다 | 하나는 압력계 축 중심, 다른 하나는 여러 축이 얕게 겹친 상태라고 읽는다 |
| `beacon_valve_frame`과 `combined_stop_frame`는 둘 다 개입 필요 쪽으로 높다 | 둘 다 그냥 점수가 높은 프레임이라고만 본다 | 하나는 경광등-밸브 조합이 주도하고, 다른 하나는 세 축이 모두 강하게 살아났다고 구분한다 |
| hidden 세 축이 원래 세 입력과 다르게 움직인다 | 입력 세 개를 조금 섞은 정도라고만 느끼기 쉽다 | 모델이 최종 판단 전에 더 유용한 내부 좌표계로 다시 적었다고 읽는다 |

이 표까지 읽고 나면, representation learning의 핵심이 `입력을 버리는 것`이 아니라 `입력을 더 잘 가르는 내부 좌표계로 다시 적는 것`이라는 점이 더 분명해집니다.
