...
작은 생산 배치 이상 판단 장면으로 옮기면 차이가 더 선명해집니다.

| 입력 장면 | 퍼셉트론 하나로 바로 읽으면 | 층을 나눠 읽으면 |
| --- | --- | --- |
| `x1`만 높다 | 바로 긍정 또는 바로 부정처럼 한 번에 기울기 쉽다 | 첫 층에서 `신호 A`만 켜졌다고 읽고, 다음 층에서 아직 최종 판단이 이르다고 남길 수 있다 |
| `x2`만 높다 | 반대 방향의 단일 규칙으로 바로 기울기 쉽다 | 첫 층에서 `신호 B`만 켜졌다고 읽고, 다음 층에서 다른 조합과 구분해 다룰 수 있다 |
| `x1`, `x2`가 함께 높다 | 한 점수 안에 모두 눌러 담아 왜 다른지 설명이 약해진다 | 앞층에서 두 신호를 따로 만든 뒤, 뒤층에서 `둘이 함께 있을 때만 더 강한 판단`으로 묶을 수 있다 |
...
| 읽기 방식 | 사람이 먼저 보기 쉬운 기준 | 놓치기 쉬운 점 | 다층 구조가 바꾸는 읽기 |
| --- | --- | --- | --- |
| 입력값을 따로 본다 | 스톱 횟수가 많은가, 재작업 호출이 늘었는가 | 신호가 함께 나타날 때만 드러나는 상태를 놓치기 쉽다 | 여러 입력이 함께 만든 중간 조합을 먼저 본다 |
| 규칙 하나로 바로 판단한다 | `온도 이탈 시간이 길면 위험`처럼 단일 규칙을 붙인다 | 같은 규칙이라도 다른 입력 맥락에서 의미가 달라질 수 있다 | 앞층에서 작은 조합을 만들고 뒤층에서 더 큰 상태 판단으로 묶는다 |
| 최종 라벨만 본다 | 정상/개입 필요 같은 마지막 결과만 본다 | 왜 같은 결과가 나왔는지 설명 재료가 부족해진다 | 은닉층이 만든 중간 표현을 함께 읽어 판단 과정을 더 분명히 본다 |
...
| 사람이 먼저 보기 쉬운 기준 | 다층 구조 관점으로 다시 읽는 기준 |
| --- | --- |
| 사진 한 장을 바로 최종 라벨로 읽고 싶어 한다 | 작은 단서를 먼저 만들고, 그 단서를 더 큰 구조로 다시 묶는다고 본다 |
| 밸브, 경광등, 배관 같은 단서를 최종 라벨과 거의 같은 것으로 본다 | 단서는 앞층의 재료이고, 최종 라벨은 뒤층 조합의 결과라고 구분한다 |
| 단서가 몇 개 보이면 곧바로 분류할 수 있다고 느끼기 쉽다 | 어떤 단서 조합이 함께 켜지는지가 더 중요하다고 본다 |
...
이 차이를 학습 책임 관점으로 다시 줄이면 다음과 같습니다.

| 질문 | Part 4의 전통 모델 감각 | P5-2.1의 다층 구조 감각 |
| --- | --- | --- |
| 사람이 먼저 많이 정하는 것은 무엇인가 | 어떤 특징을 넣고 어떤 규칙으로 나눌지 | 입력과 출력만 두고 중간 표현은 모델이 더 많이 맡는다 |
| 모델이 새로 더 떠맡는 것은 무엇인가 | 정해진 특징 위에서 경계나 점수를 조정하는 일 | 중간 표현을 만들고 그 표현을 다시 조합하는 일 |
| 그래서 지금 절에서 먼저 봐야 하는 차이는 무엇인가 | 선형 경계 하나로 설명이 닫히는가 | 중간 표현이 생기면 패턴 설명이 더 잘 닫히는가 |

## 연습 및 예제

이번 예제의 목표는 여러 입력이 같은 층 구조를 지날 때 은닉층(hidden layer) 패턴이 어떻게 갈리고, 그 차이가 최종 출력에 어떻게 연결되는지 직접 확인하는 것입니다.
...
for idx, (x1, x2) in enumerate(batches, start=1):
    # hidden layer
    h1 = step(x1 * 0.9 + x2 * -0.3 - 0.2)
    h2 = step(x1 * -0.5 + x2 * 1.0 - 0.4)

    # output layer
    output = step(h1 * 0.7 + h2 * 0.7 - 0.9)

    print(
        f"batch_{idx}: input=({x1}, {x2}), "
...
코드를 한 번 실행한 뒤에는 값을 더 바꿔 보며 `어느 조합이 은닉층을 통해 최종 판단을 뒤집는가`를 확인하는 편이 좋습니다.

| 다음에 직접 바꿔 볼 값 | 먼저 볼 출력 | 해석 질문 |
| --- | --- | --- |
| `batch_1`의 `x2`를 `0.2`에서 `0.9`로 올린다 | `hidden=(1, 0)`이 `hidden=(1, 1)`로 바뀌는가 | 두 번째 신호가 추가될 때 왜 최종 출력이 뒤집히는가 |
| `batch_4`의 `x1`과 `x2`를 함께 조금씩 올린다 | `(0, 0)`이 어느 지점에서 `(1, 0)`이나 `(0, 1)`로 먼저 바뀌는가 | 은닉층은 모든 신호를 동시에 요구하는가, 아니면 한 축이 먼저 살아나는가 |
| 출력층 편향 `-0.9`를 `-0.5`로 바꾼다 | 같은 hidden 패턴에서도 `output=1`이 늘어나는가 | 중간 조합이 같아도 최종 판단 규칙이 더 느슨해질 수 있다는 점이 보이는가 |
