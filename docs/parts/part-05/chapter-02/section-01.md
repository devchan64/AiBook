...
experiments = [
    ("batch_2 x1 increase", 0.9, 1.0, -0.9),
    ("batch_1 x2 increase", 1.0, 1.0, -0.9),
    ("batch_4 both signals increase", 0.5, 0.6, -0.9),
    ("looser output rule", 0.3, 1.0, -0.5),
]
...
follow-up experiments
batch_2 x1 increase: input=(0.9, 1.0), output_bias=-0.9, hidden=(1, 1), output=1
batch_1 x2 increase: input=(1.0, 1.0), output_bias=-0.9, hidden=(1, 1), output=1
batch_4 both signals increase: input=(0.5, 0.6), output_bias=-0.9, hidden=(1, 0), output=0
looser output rule: input=(0.3, 1.0), output_bias=-0.5, hidden=(0, 1), output=1
```
...
- 후속 실험에서는 입력 하나를 더 올리거나 출력층 편향을 바꿨을 때, `hidden`이 먼저 달라지는지 `output` 규칙이 먼저 달라지는지를 분리해서 볼 수 있습니다.
...
이제 바로 아래 네 가지 후속 실험을 보면, `어느 입력 변화가 은닉층을 바꾸는가`와 `같은 은닉층이라도 출력 규칙을 바꾸면 무엇이 달라지는가`를 나눠 볼 수 있습니다.

| 실험 | 먼저 볼 출력 | 해석 질문 |
| --- | --- | --- |
| `batch_2 x1 increase` | `hidden=(0, 1)`이 `hidden=(1, 1)`로 바뀌는가 | 첫 번째 신호를 더하면 왜 최종 출력까지 같이 뒤집히는가 |
| `batch_1 x2 increase` | 원래 `(1, 0)`이던 패턴이 `(1, 1)`로 확장되는가 | 두 번째 신호가 임계값을 넘는 순간 조합 판단이 어떻게 완성되는가 |
| `batch_4 both signals increase` | `(0, 0)`에서 `(1, 0)`이나 `(0, 1)`이 먼저 생기는가 | 약한 입력이 커질 때 은닉층은 어떤 축부터 반응하는가 |
| `looser output rule` | 같은 `hidden=(0, 1)`인데도 `output=1`이 되는가 | 중간 표현은 그대로여도 최종 판단 기준이 느슨해지면 결과가 바뀔 수 있는가 |

이 네 실험은 두 종류의 변화를 분리해서 보여 줍니다.

- 입력을 바꾸는 실험은 은닉층이 어떤 조합을 새로 만들기 시작하는지 보여 줍니다.
- 출력층 편향을 바꾸는 실험은 중간 표현은 그대로인데 마지막 판단 규칙만 느슨해져도 결과가 바뀔 수 있음을 보여 줍니다.

특히 `batch_4 both signals increase` 결과가 `(1, 0)`에서 멈춘다는 점은 중요합니다. 입력 두 개가 함께 커졌다고 해서 곧바로 `(1, 1)`이 되는 것이 아니라, 각 은닉 노드는 서로 다른 조건에 먼저 반응합니다. 반대로 `looser output rule`에서는 `hidden=(0, 1)`이 그대로인데도 출력층 기준이 완화되면서 최종 출력이 `1`로 바뀝니다. 이 차이를 보면 다층 신경망에서는 `중간 표현을 만드는 규칙`과 `그 표현으로 최종 판단을 내리는 규칙`을 구분해서 읽어야 한다는 점이 더 분명해집니다.
