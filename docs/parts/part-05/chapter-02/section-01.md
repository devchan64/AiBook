...
사용자가 `model`이라는 토큰을 입력하면, 사람은 단어 자체를 보면 이미 뜻이 있다고 느끼기 쉽습니다. 하지만 문자열 자체만으로는 모델이 계산하기 어렵습니다. 예를 들어 `model`과 `models`가 표면상 비슷해 보여도, 모델은 문자 모양이 아니라 다른 토큰들과 어떤 관계를 맺는지 계산 가능한 형태가 먼저 필요합니다. 모델은 이 토큰을 임베딩 벡터로 바꾼 뒤에야 다른 토큰과 얼마나 비슷한지, 어떤 문맥에서 함께 등장하는지 계산할 수 있습니다. 즉, 임베딩은 단어를 보기 좋게 저장하는 장치가 아니라 이후 attention과 비교 계산이 가능한 형태로 바꾸는 출발점입니다.

### 사례 2. 문장 검색

사용자가 `프롬프트의 한계는 무엇인가`라고 묻는데 문서에는 `프롬프트만으로 사실성을 보장할 수 있는가`라고 적혀 있을 수 있습니다. 사람은 보통 같은 단어가 있어야 같은 뜻이라고 느끼기 쉽지만, 표면 단어가 완전히 같지 않으면 단순 문자열 일치는 약해질 수 있습니다. 예를 들어 질문에는 `한계`가 있고 문서에는 `보장`과 `사실성`이 있어도, 실무적으로는 같은 설명 흐름으로 이어질 수 있습니다. 임베딩 공간에서는 두 문장이 비슷한 방향에 놓일 수 있고, 그래서 검색기는 같은 단어보다 `비슷한 의미`를 가진 후보를 더 잘 찾게 됩니다.

### 사례 3. 추천과 유사도 비교

동영상 서비스에서 사용자가 본 강의 목록과 비슷한 시청 패턴을 가진 다른 사용자를 찾거나, 비슷한 강의를 추천하고 싶을 수 있습니다. 사람은 제목 문자열만 비슷하면 비슷한 강의라고 느끼기 쉽지만, 실제로는 시청 행동, 설명 문구, 썸네일 특징이 함께 비슷해야 더 유용한 추천이 나올 수 있습니다. 예를 들어 제목에 `입문`이 붙어 있어도 하나는 수식 중심이고 다른 하나는 실습 중심이면, 문자열만으로는 실제 선호 흐름을 잘 못 잡을 수 있습니다. 이때 제목 문자열만 비교하는 대신 강의, 사용자 행동, 이미지 썸네일 조각 같은 항목을 임베딩 공간에 놓으면 더 다양한 유사도를 계산할 수 있습니다. 즉, 임베딩은 텍스트를 넘어서 여러 대상을 같은 비교 공간에 올려놓는 역할도 합니다.
