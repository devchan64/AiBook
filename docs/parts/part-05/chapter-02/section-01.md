...
이 질문에서 다층 신경망(multilayer neural network)으로 들어갑니다. 다층 신경망은 퍼셉트론 같은 계산 단위를 여러 층으로 쌓아, 단순한 입력 조합을 중간 표현으로 바꾸고 그 표현을 다시 더 복잡한 판단으로 연결하는 구조입니다.
...
은닉층이 실제로 어떤 표현을 만든다고 읽을 수 있는지는 P5-2.2에서 더 이어서 다루고, 활성화 함수 비교는 P5-3.1, P5-3.2에서 다시 연결합니다. 역전파(backpropagation)의 계산 공식은 P5-5.1, P5-5.2에서 다시 봅니다. 보편 근사 정리와 깊이·너비의 이론 비교는 이 책의 현재 본편 범위 밖에 둡니다.
...
예를 들어 방문 수는 높지만 최근 활동일이 오래되었고 문의 횟수도 늘고 있다면, 사람은 처음에는 숫자 네 개를 따로따로 보고 판단하려 하기 쉽습니다. 하지만 실제로는 이 숫자들을 한 번에 묶어 `관심은 있었지만 지금은 이탈 위험이 커진 고객`처럼 읽습니다. 입력값만 직접 비교하면 이런 조합 신호를 놓치기 쉽고, 서로 다른 숫자 패턴이 같은 고객 상태를 뜻할 수도 있습니다. 은닉층은 바로 이런 중간 조합을 내부적으로 만드는 자리로 이해합니다. 다만 중요한 것은 은닉층이 실제로 `활동성`이라는 이름을 알고 있는 것은 아니라는 점입니다. 우리는 직관을 위해 그렇게 설명할 뿐이고, 실제로는 모델이 여러 가중치 조합을 통해 어떤 중간 표현을 내부적으로 만든다고 이해합니다.

## 사례로 보기

### 사례 1. 고객 이탈 예측

고객 이탈 예측을 생각합니다. 사람은 처음에는 `최근 활동일이 오래되었는가`, `문의 횟수가 늘었는가`, `구매 금액이 줄었는가` 같은 규칙을 하나씩 따로 봅니다. 하지만 실제로는 이 세 조건이 함께 나타날 때만 이탈 위험이 커지는 경우가 많습니다. 예를 들어 방문 수는 여전히 높지만 최근 활동일이 멀어지고 문의 횟수까지 늘었다면, 단일 규칙 하나보다 `관심 약화 + 불만 증가`라는 중간 조합으로 읽습니다. 다층 구조는 바로 이런 작은 신호 조합을 한 층에서 만들고, 다음 층에서 더 큰 상태 판단으로 묶는 흐름을 설명합니다. 그래서 이 사례에서 확인해야 할 결과는 개별 규칙 하나보다, 여러 신호가 함께 나타날 때 만들어지는 중간 조합이 실제로 이탈 위험 판단을 더 분명하게 가르는가입니다.

### 사례 2. 이미지 인식의 초기 직관

이미지 인식에서도 비슷한 일이 일어납니다. 사람은 처음에 사진 한 장을 보고 바로 `고양이`, `개`, `자동차`처럼 최종 라벨을 떠올리지만, 실제로는 귀 모양, 눈 위치, 윤곽선 같은 작은 단서들이 먼저 보입니다. 단일 퍼셉트론 하나로는 이런 여러 단서를 한 번에 다루기 어렵지만, 다층 구조에서는 앞층이 작은 모양 조합을 만들고 뒤층이 그 조합을 더 큰 물체 단서로 묶을 수 있습니다. 예를 들어 귀와 눈 주변 패턴이 먼저 잡히고, 그 다음에 얼굴 형태 같은 더 큰 구조가 묶이는 식입니다. 그래서 이 사례에서 확인해야 할 결과는 작은 단서 조합을 거친 뒤에야 최종 라벨 구분이 더 안정적으로 드러나는가입니다.
...
## 실행 가능한 Python 예제로 층 흐름 보기

이번 예제의 목표는 입력 하나만 보는 것이 아니라, 여러 입력이 같은 층 구조를 지나며 어떻게 다른 은닉 표현과 최종 출력을 만드는지 확인하는 것입니다.
...
- 은닉층이 하나 추가되면 입력이 중간 표현을 거쳐 최종 판단으로 가는 흐름을 직접 볼 필요가 있다

확인할 개념:

- 다층 퍼셉트론은 입력을 바로 출력으로 보내지 않고 중간 노드 계산을 한 번 더 거친다
- 샘플별 은닉층 값과 최종 출력을 같이 보면 층의 역할이 더 분명해진다

입력(input):

위에 정리한 세 샘플과 은닉층·출력층 가중치를 사용합니다.
...
출력에서는 각 `input`이 어떤 `hidden` 패턴을 거쳐 같은 `output`에 도달하는지 한 줄씩 비교해 봅니다.

```text
sample_1: input=(1.0, 0.5), hidden=(1, 0), output=1
sample_2: input=(0.2, 1.0), hidden=(0, 1), output=1
sample_3: input=(1.2, 1.1), hidden=(1, 0), output=1
```

- 입력값이 바로 최종 출력으로 가지 않습니다
- 먼저 은닉층의 출력 두 개가 생깁니다
- 같은 최종 출력이라도 그 앞의 은닉층 패턴은 다를 수 있습니다

즉, 다층 신경망은 `입력 -> 중간 표현 -> 최종 판단` 구조를 갖고, 중간 표현이 실제 계산의 핵심 재료가 됩니다.

## 언제 다층 구조 관점으로 먼저 읽는가
...
이 절의 핵심은 층이 많아진다는 사실보다, 중간 표현이 생기기 시작한다는 점을 고정하는 데 있습니다.
...
- 퍼셉트론 하나를 이해한 뒤 왜 바로 다층 구조가 필요한지 설명할 수 있는가?
- 입력층, 은닉층, 출력층의 차이를 `데이터 위치`가 아니라 `계산 역할`로 구분할 수 있는가?
- 다층 구조 설명만으로는 아직 활성화 함수와 비선형성의 역할이 완전히 닫히지 않고, 다음 장으로 넘겨야 한다는 점을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 퍼셉트론 하나로 설명이 닫히지 않고 더 복잡한 패턴 조합이 필요해 보일 때, 다층 구조 관점을 먼저 떠올립니다.
- 입력층, 은닉층, 출력층을 데이터 위치가 아니라 계산 역할로 구분해야 할 때, 층별 역할 정리를 다시 꺼냅니다.
- 딥러닝이 단지 층 수 경쟁처럼 들릴 때, 중간 표현이 생긴다는 점을 기준으로 이 절을 다시 봅니다.
