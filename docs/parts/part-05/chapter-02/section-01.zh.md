...
如果需要重新固定多层结构与中间层的区别，更适合一起回看概念词汇表里的[多层神经网络（multilayer neural network）](/AiBook/zh/reference/concept-glossary-pinyin/m/#multilayer-neural-network)与[隐藏层（hidden layer）](/AiBook/zh/reference/concept-glossary-pinyin/m/#multilayer-neural-network)条目。
...
```python
# 这个例子检查生产批次输入如何经过两个隐藏节点组合再连接到最终输出。
def step(z):
