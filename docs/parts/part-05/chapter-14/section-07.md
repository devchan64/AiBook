# P5-14.7 보충학습: feed-forward network는 왜 위치별 표현 가공을 맡는가
...
용어가 다시 흩어질 때는 개념사전의 [피드포워드 네트워크(feed-forward network)](../../../reference/concept-glossary.md#feed-forward-network) 항목과 P5-14.2의 네 부품 역할 분담을 함께 다시 보면 좋습니다.
...
해설: 좋은 답은 멋진 용어가 아니라 `현재 위치 표현이 어떤 방향으로 바뀌어야 하는가`를 말로 분명히 쓰는 것입니다. 이 감각이 있어야 P5-14.3의 표현 이동 예제를 볼 때 `feed-forward output`이 단순 중간 숫자가 아니라 위치별 의미 가공 결과라는 점을 읽을 수 있습니다.
