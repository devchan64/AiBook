...
- 오픈체크리스트의 중심축 문장인 "feed-forward network가 attention 뒤에서 위치별 표현 가공을 맡는다는 점을 보충해야 합니다. Python 예제는 같은 FFN 가중치를 여러 위치에 공유 적용해도 입력 표현 차이에 따라 hidden/output이 달라지는 점을 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
