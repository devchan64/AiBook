...
解说：这个练习不是实现实际 GPU kernel。P5-14.4 需要学习的是区分 `传递状态的计算`、`重新计算关系的流程`、`可以一次打包的计算`。

标成`等待`的场景，是后面的计算必须先收到前面计算结果才能开始的情况。比如第 3 个 token 等待第 2 个 hidden state，或者前面规则必须经过多个 step 才能到达最后状态，都属于这一类。在这种结构里，一个句子内部的 step 很难随意同时处理。

标成`打包`的场景，是同类计算可以一起放成表或 tensor 的情况。比如把一个句子里所有 token 对的 attention score 做成矩阵，或者对 batch 中多个句子的各位置应用同一组 feed-forward 权重，都属于这一类。GPU 并行处理正适合这种重复计算的打包。

所以这个练习的结论不是`Transformer 一定更快`。重点是：训练时一层内部的关系计算容易组织成大的矩阵和 tensor 运算，而 RNN 式顺序状态传递强烈依赖前一个 step 的结果。有了这个区分，才能把 Transformer 的并行处理优势解释为计算结构变化，而不是单纯的速度印象。
