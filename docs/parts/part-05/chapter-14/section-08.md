# P5-14.8 보충학습: residual connection은 왜 원래 표현의 길을 남기는가
...
여기서 `신호`는 두 가지 감각을 함께 가집니다. 하나는 순전파에서 표현 정보가 지나가는 흐름이고, 다른 하나는 학습 중 오차를 바탕으로 파라미터를 조정하는 흐름입니다. 이 보충학습에서는 수식보다 `원래 표현과 새 계산 결과가 함께 지나갈 길이 있어 깊은 구조를 쌓기 쉬워진다`는 구조 감각을 우선 잡습니다.
...
| 현재 위치 | 새 계산이 강화한 의미 | residual로 함께 남겨야 하는 축 |
| --- | --- | --- |
| `재기동` | 압력 미해소 조건에서는 차단 | 원래 조치 이름 |
| `승인` | 검증 미완료 상태에서는 보류 | 원래 승인 행위 |
| `배포` | rollback 확인 전에는 위험 | 원래 배포 작업 |
...
해설: 좋은 답은 `원래 단어를 그대로 남긴다`에서 멈추지 않습니다. 새 계산이 만든 의미가 뒤 블록에서 다시 쓰일 때, 그 의미가 붙은 원래 대상이나 행위가 추적 가능해야 합니다. 이것이 residual connection을 단순한 덧셈이 아니라 깊은 반복에서 정보 흐름을 남기는 장치로 읽어야 하는 이유입니다.
