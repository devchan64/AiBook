# P5-14.7 补充学习：feed-forward network 为什么负责按位置表示加工？
...
解说：好的答案不是华丽术语，而是清楚写出`当前位置表示应该朝哪个方向改变`。有了这种感觉，阅读 P5-14.3 的表示移动例子时，才能把 `feed-forward output` 读成按位置的意义加工结果，而不是单纯中间数字。
