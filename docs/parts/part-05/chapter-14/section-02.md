...
| --- | --- | --- |
| MCP 연결 형식 | 무엇과 어떤 형식으로 연결했는가? | P5-14.1 |
| harness 실행 기록 | 그 연결을 쓴 실행을 어떤 trace, replay, approval 기록으로 남길 것인가? | P5-14.2 |
| 평가와 운영 판정 | 남긴 기록을 어떤 통과 기준과 서비스 통제로 이어 갈 것인가? | P5-15, P5-16 |
...
즉, 이 절에서 가장 짧게 붙잡아야 할 닫힘 구조는 `MCP 연결 형식 -> harness 실행 기록 -> 평가와 운영 판정`입니다. 여기서는 그 가운데 단계인 `실행 기록`을 맡고 있고, 바로 다음 장에서 이 기록이 품질 기준과 운영 조치의 입력으로 바뀝니다.
