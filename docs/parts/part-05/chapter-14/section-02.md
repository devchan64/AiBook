...
### 사례 2. 긴 교대 인수인계 위험 판단

긴 교대 인수인계 로그를 읽은 뒤 마지막 줄에서 `다음 교대가 지금 바로 라인 3을 재가동해도 되는가?`를 판단하는 장면을 생각해 보겠습니다. 로그 앞부분에는 `02:10 압력 이상 경보 발생`, 중간에는 `02:16 수동 밸브 점검 이상 없음`, 뒤쪽에는 `02:24 라인 안정화 후 생산 재개 검토`가 적혀 있을 수 있습니다. 사람이 급하게 읽으면 마지막 줄의 `생산 재개 검토`나 가장 최근 상태 보고에만 기대어 바로 승인 판단을 내리기 쉽습니다. 하지만 실제로는 현재 판단 위치가 초반 경보와 중간 점검 근거를 함께 다시 불러와야 위험 판단이 덜 왜곡됩니다. 순차 전달만으로 읽으면 이런 로그들을 앞에서 뒤로 넘기며 붙잡아야 하지만, Transformer는 현재 판단 위치가 여러 로그 위치를 직접 참조할 수 있습니다. 그리고 이 여러 위치 비교를 병렬 행렬 연산으로 묶기 때문에, `지금 승인 판단과 관련 있는 로그들`을 한 번에 비교하는 계산 감각이 더 자연스럽습니다.
그래서 이 사례에서 확인해야 할 결과는 현재 판단 위치가 마지막 상태 보고만 따르지 않고, 초반 경보와 중간 점검 근거를 실제로 함께 다시 끌어와 교대 인수인계 위험 판단을 덜 왜곡하는가입니다.

### 사례 3. 긴 설정 파일 검토

긴 설정 파일을 검토할 때는 파일 앞부분의 `pressure_limit=0.8`, `restart_enabled=false`, `timeout_ms=30000` 같은 정의가 뒤쪽 적용 구간 해석에 계속 영향을 줍니다. 사람은 현재 보고 있는 블록 근처만 보면 충분하다고 느끼기 쉽지만, 실제로는 파일 아래쪽의 `apply_restart_profile=true` 같은 줄을 읽을 때 앞 정의를 다시 확인하지 않으면 제한 규칙을 잘못 해석할 수 있습니다. 순차 전달 구조라면 앞 정의를 상태 안에 계속 들고 와야 하고, 설정 블록이 길어질수록 중요한 제한 규칙이 흐려질 수 있습니다. Transformer 계열은 현재 줄이 먼 앞 정의와 제한 규칙을 직접 다시 참고할 수 있어, `지금 보는 줄`과 `앞 정의 줄들`을 더 직접 연결합니다. 여기에 병렬 계산 감각이 붙으면서, 여러 설정 줄 사이 관계를 한 줄씩 순차 확인하기보다 관련 위치들을 함께 계산 대상으로 올리는 쪽이 쉬워집니다.
그래서 이 사례에서 확인해야 할 결과는 현재 줄이 가까운 문맥만 따라가지 않고, 먼 앞쪽 정의와 제한 규칙을 실제로 다시 참조해 설정 해석 일관성을 유지하는가입니다.

세 사례에서 공통으로 확인해야 할 결과는 앞 단서를 상태에만 맡기지 않고 현재 위치가 다시 직접 참고할 수 있어야 한다는 점입니다. 작업 허가 질의응답에서는 금지와 예외 조건을 놓치지 않는지, 교대 인수인계 위험 판단에서는 초반 경보와 중간 점검 근거가 함께 남는지, 긴 설정 파일 검토에서는 정의·제한 규칙이 먼 거리에서도 유지되는지를 보면 충분합니다.
