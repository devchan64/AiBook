...
attention complexity의 상세 빅오 비교는 여기서 전개하지 않습니다. KV cache, sparse attention, long-context를 처음 읽는 법은 P6-3.3 보충학습에서 다시 설명하고, long-context optimization의 최신 벤치마크 경쟁과 구현 세부 비교는 이 책의 현재 본편 범위 밖에 둡니다.
...
| --- | --- | --- |
| 다국어 작업 지시 | 앞부분 금지 조건과 예외 조항이 뒤로 갈수록 흐려질 수 있다 | 현재 번역 위치가 앞 단서를 다시 확인해 안전 의미 반전을 줄인다 |
| 긴 장애 보고서 요약 | 제목이나 마지막 결론에만 기대어 핵심 근거를 놓치기 쉽다 | 현재 요약 문장이 필요한 앞 로그와 중간 점검 근거를 다시 끌어온다 |
| 긴 운영 스크립트 읽기/생성 | 현재 줄 근처 정보만 보다가 먼 앞 정의와 차단 조건을 놓치기 쉽다 | 현재 위치가 앞쪽 정의와 제약을 다시 참조해 일관성을 유지한다 |

### 사례 1. 다국어 작업 지시 번역

긴 작업 지시 문장을 다른 언어로 옮긴다고 해 보겠습니다. 예를 들어 `재기동을 시작하되, 압력 해소가 확인되지 않았으면 밸브를 열지 않는다` 같은 문장은 앞부분의 `재기동을 시작하되`만 빨리 읽으면 곧바로 작업 허가처럼 느껴질 수 있습니다. 하지만 실제 번역에서는 뒤쪽의 금지 조건 `압력 해소가 확인되지 않았으면`이 앞 조치의 해석 범위를 끝까지 바꿉니다. 중간 설명이 길어질수록 앞의 핵심 조건을 다시 확인해야 하고, 이때 순차 전달에만 기대면 중요한 단서가 약해질 수 있습니다. 예를 들어 앞부분의 `열지 않는다`를 놓치면, 뒤 문장은 문법상 자연스러워 보여도 안전 의미는 정반대로 번역될 수 있습니다. 여기서 바뀌는 점은 `가까운 단어 위주로 읽는 방식`에서 `멀리 떨어진 핵심 단서를 현재 위치에서 다시 참조하는 방식`으로 기준이 이동한다는 것입니다. Transformer의 긴 문맥 참조 구조는 현재 번역 위치가 문장 앞쪽 금지 조건을 더 직접 다시 참고하는 쪽으로 이해할 수 있습니다.
그래서 이 사례에서 확인해야 할 결과는 현재 번역 위치가 앞쪽 금지 조건을 실제로 다시 참고해, 문장 끝에서도 안전 의미 반전이 줄어드는가입니다.

### 사례 2. 긴 장애 보고서 요약

긴 장애 보고서를 요약할 때, 초반 핵심 규칙과 중간 로그 근거가 뒤쪽 요약 문장 생성에 그대로 남아 있어야 하는 경우가 많습니다. 사람은 요약을 쓰기 시작하면 이미 읽은 앞부분 내용이 머릿속에 충분히 남아 있을 것이라고 느끼기 쉽습니다. 하지만 실제로는 중간 로그와 부연 설명이 길어질수록 앞부분의 핵심 판단을 다시 집어 오지 않으면 요약 방향이 쉽게 틀어집니다. 예를 들어 서론에 `압력 변동은 배포 직후 시작되었고 수동 밸브 점검에서는 이상이 없었다`가 적혀 있는데, 뒤 요약에서 `설비 상태가 전반적으로 불안정했다`만 남기면 원인 판단이 왜곡됩니다. 여기서 바뀌는 점은 `앞에서 읽은 내용이 저절로 남아 있을 것`이라는 기대에서 벗어나, `현재 요약 위치가 필요한 앞 문장과 중간 근거를 다시 불러와야 한다`는 기준으로 읽게 된다는 것입니다. Transformer는 문서 여러 위치를 함께 참조하며 현재 요약 표현을 갱신하는 구조라, 이런 초반 핵심 근거를 뒤 단계에서 다시 끌어오기에 더 자연스럽습니다.
그래서 이 사례에서 확인해야 할 결과는 현재 요약 문장이 마지막 결론만 남기는 것이 아니라, 초반 핵심 근거와 중간 로그 근거를 실제로 함께 다시 끌어와 원인 판단을 덜 왜곡하는가입니다.

### 사례 3. 운영 스크립트 생성과 분석

긴 운영 스크립트를 읽거나 생성할 때는 먼 앞쪽의 변수 정의, 차단 조건, 단위 규칙이 뒤쪽 토큰 해석에 계속 영향을 줍니다. 사람은 바로 앞 몇 줄만 보면 충분하다고 느끼기 쉽지만, 실제로는 아래 줄을 읽다가 위에서 선언한 `pressure_ok`, `interlock_released`, `timeout_ms` 같은 조건을 다시 확인해야 하는 경우가 자주 생깁니다. 예를 들어 파일 아래쪽에서 재기동 함수를 호출할 때, 위에서 `pressure_ok`가 `False`이면 차단해야 한다는 규칙이나 `timeout_ms`가 밀리초 단위라는 정의를 다시 확인하지 않으면 스크립트는 실행돼도 현장 규칙과 동작 시간이 완전히 달라질 수 있습니다. 여기서 바뀌는 점은 `현재 줄 근처만 보면 된다`는 읽기에서 `먼 앞쪽 정의를 현재 해석에 다시 끌어오는 읽기`로 기준이 이동한다는 것입니다. Transformer 계열은 이런 장면에서 현재 위치가 먼 앞쪽 정의를 더 직접 참조하는 구조로 이해할 수 있습니다.
그래서 이 사례에서 확인해야 할 결과는 현재 줄이 가까운 문맥만 따라가지 않고, 먼 앞쪽 정의와 차단 조건을 실제로 다시 참조해 스크립트 일관성을 유지하는가입니다.

세 사례에서 공통으로 확인해야 할 결과는 앞 단서를 상태에만 맡기지 않고 현재 위치가 다시 직접 참고할 수 있어야 한다는 점입니다. 작업 지시 번역에서는 금지와 예외 조건을 놓치지 않는지, 장애 보고서 요약에서는 상반된 핵심 근거가 함께 남는지, 운영 스크립트 생성과 분석에서는 정의·단위·차단 조건이 먼 거리에서도 유지되는지를 보면 충분합니다.
...
| `sequential_decision`과 `direct_decision`이 갈라진다 | 규칙 단서 수를 줄이거나 늘려 어떤 조건에서 두 구조 판단이 다시 가까워지는지 본다 | 이 축약 비교 예제 하나로 실제 long-context 최적화 성능 전체를 결론내리지 않는다 |
...
앞의 축약 비교 코드는 Transformer 전체를 구현한 것은 아니지만, 비교 기준은 분명합니다.
