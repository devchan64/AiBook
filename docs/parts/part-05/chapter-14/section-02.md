...
| --- | --- |
| 각 부품이 블록 안에서 맡는 역할 | 숫자 예제로 표현이 실제로 어떻게 이동하는가 |
| 관계 읽기와 위치별 가공의 차이 | 병렬 처리와 긴 문맥 계산 감각 |
...
이 도식은 Transformer 블록 하나를 입문 수준에서 압축한 것입니다. 흐름은 `관계 읽기 -> 원래 표현을 더하고 정리 -> 현재 위치 표현 가공 -> 다시 안정적으로 전달`로 읽으면 됩니다.
