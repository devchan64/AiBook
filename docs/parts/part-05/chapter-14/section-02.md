...
처음 읽을 때는 하네스 이름을 제품처럼 잡기보다 `무엇을 어떤 trace와 replay로 남겨야 하는가`, `이 기록이 왜 바로 다음 장의 평가 입력이 되는가`, `MCP와 하네스가 각각 연결과 관리 중 무엇을 맡는가`라는 세 질문만 먼저 붙잡으면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 하네스가 실행을 감싸고 기록을 남기는 관리 층위라는 점 | P5-15에서 이 기록으로 무엇이 좋은 실행인지 어떻게 판정할지로 이어집니다. | 이후 trace/log 검토, replay, 승인 기록, 실패 분석에서 반복됩니다. |
| MCP와 하네스의 차이가 `연결`과 `실행 관리`의 차이라는 점 | 연결 문제와 운영 문제를 어디서 나눠 볼지 기준으로 이어집니다. | 이후 권한 승인, 실행 환경, 운영 정책 설명에서 계속 중요합니다. |
| 좋은 실행 기록이 평가와 운영의 입력이라는 점 | P5-16에서 어떤 기록을 근거로 멈추고 우회하고 개선할지로 이어집니다. | 이후 incident 기록, review summary, 운영 회고에서 바탕이 됩니다. |

바로 다음 장들과 나란히 놓고 보면, 하네스가 맡는 역할과 그 이후 단계의 역할 차이도 더 분명해집니다.

| 층위 | 먼저 남기는 것 | 그다음 장에서 다시 묻는 것 |
| --- | --- | --- |
| P5-14.2 하네스 | trace, tool call log, replay, approval 기록 | 이 기록으로 무엇이 좋은 실행인지 어떻게 판정할 것인가? |
| P5-15 평가 | 품질 축별 통과/수정 판단 | 어떤 후보를 자동 통과, 사람 검토, 재수정 대상으로 둘 것인가? |
| P5-16 운영 | 비용, 지연, 실패 대응 기준 | 어떤 후보를 실제 서비스에 계속 둘 것인가, 어디서 멈추거나 우회할 것인가? |

즉, 하네스는 `실행 흔적을 남기는 층`, 평가는 `그 흔적을 품질 판단으로 바꾸는 층`, 운영은 `그 판단을 실제 서비스 통제와 실패 대응으로 이어 붙이는 층`입니다. 이 차이가 보여야 하네스를 평가 절차나 운영 정책과 같은 말처럼 섞어 읽지 않게 됩니다.

처음 읽을 때는 아래 세 줄만 바로 구분해도 충분합니다.

| 지금 하네스에서 먼저 못 박을 것 | 아직 여기서 하지 않는 것 | 바로 다음 장으로 넘길 것 |
| --- | --- | --- |
| trace, replay, approval 같은 실행 흔적을 남긴다 | 최종 품질 통과 여부를 확정하지 않는다 | 남긴 기록으로 무엇이 좋은 실행인지 평가한다 |
| 실행 관리와 기록을 감싼다 | 비용·정책 차단 같은 서비스 통제를 끝내지 않는다 | P5-15, P5-16에서 판정과 운영 조치로 이어 간다 |

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| MCP: 무엇과 어떻게 연결할까 | 하네스: 그 연결을 써서 실행한 흐름을 어떻게 남기고 통제할까 | 평가와 운영: 남긴 기록으로 무엇이 괜찮은 실행인지 어떻게 판정하고 어디서 멈출까 |
| 연결 인터페이스 | 실행 관리와 기록 | 품질 판정과 서비스 통제 |

즉, 지금 장의 핵심은 `연결을 잘했는가`에서 `그 연결을 쓴 실행을 다시 설명하고 비교할 수 있는가`로 손잡이가 바뀐다는 점입니다.

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| MCP | 무엇과 어떤 형식으로 연결했는가 |
| harness | 그 연결을 쓴 실행을 어떤 trace와 replay 기록으로 남길 것인가 |
| evaluation | 남긴 기록을 어떤 통과 기준으로 판정할 것인가 |

이 전환을 가장 짧게 다시 묶으면 다음과 같습니다.

| 지금 절에서 끝내야 하는 것 | 바로 다음 절에서 새로 묻는 것 | 그다음 운영 절에서 이어지는 것 |
| --- | --- | --- |
| 실행 기록을 빠짐없이 남긴다 | 이 기록으로 무엇이 좋은 답과 좋은 실행인지 판정한다 | 그 판정을 실제 서비스 통제와 실패 대응으로 연결한다 |
...
즉, P5-15에서는 하네스를 `평가 입력`으로 읽고, P5-16에서는 같은 기록을 `운영 통제와 실패 대응 입력`으로 다시 읽습니다. 이 기준이 잡혀야 다음 장들이 부록이 아니라, 앞에서 만든 실행 구조를 실제 서비스 품질 기준으로 바꾸는 단계로 이어집니다.
...
바로 이 지점에서 실행 기록은 다음 장의 평가 입력으로 바뀝니다. 답변 문장만 보면 `괜찮다/이상하다` 정도만 말하기 쉽지만, trace와 approval 기록, replay 정보까지 함께 보면 `검색 근거가 낡았는가`, `승인 경계가 비었는가`, `같은 실행을 다시 비교할 수 있는가`처럼 평가 축을 분리할 수 있습니다. 즉, 여기서 남긴 기록은 단순 보관용 로그가 아니라, 다음 절에서 `무엇을 통과로 보고 무엇을 수정 대상으로 볼 것인가`를 판정하게 만드는 입력 자료입니다.
...
| --- | --- | --- |
| 하네스는 실행을 감싸고 trace, replay, approval 기록을 남기는 구조다. | P5-15에서는 이 기록으로 무엇이 좋은 답과 좋은 실행인지 어떻게 판정할지 더 봅니다. | P5-16처럼 비용, 지연, 실패 대응 기준으로 실제 서비스 통제를 정하는 단계는 아직 아닙니다. |
| MCP가 `무엇과 어떻게 연결할까`를 다룬다면, 하네스는 `그 연결을 써서 실행한 흐름을 어떻게 관리할까`를 다룬다. | 바로 다음 절에서 이 기록이 자동 평가와 사람 평가의 입력으로 어떻게 쓰이는지 더 봅니다. | 연결 인터페이스 자체를 더 세분화하는 문제는 이미 앞 절에서 다뤘고 여기서 더 늘리지 않습니다. |
| 핵심은 실행을 더 많이 시키는 것이 아니라, 실행을 다시 설명하고 비교할 수 있게 남기는 데 있다. | 뒤 절로 갈수록 이 기록이 품질 판정과 운영 복구 판단으로 바뀝니다. | 통합 미니 실습처럼 실제 요청 흐름으로 다시 묶는 일은 P5-17에서 닫습니다. |
