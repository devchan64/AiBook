...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. 하네스는 실행을 감싸고 `trace`, `replay`, `approval` 기록을 남기는 구조이고, MCP가 `무엇과 어떻게 연결할까`를 다룬다면 하네스는 `그 연결을 써서 실행한 흐름을 어떻게 관리할까`를 다룹니다. 핵심은 실행을 더 많이 시키는 것이 아니라, 실행을 다시 설명하고 비교할 수 있게 남기는 데 있습니다. 그래서 P5-15에서는 이 기록으로 무엇이 좋은 답과 좋은 실행인지 판정하는 문제로 넘어가고, 뒤 절로 갈수록 이 기록이 품질 판정과 운영 복구 판단으로 바뀝니다. 반면 P5-16처럼 비용, 지연, 실패 대응 기준으로 실제 서비스 통제를 정하는 단계나, 통합 미니 실습처럼 요청 흐름 전체를 다시 묶는 일은 아직 여기서 닫지 않습니다.
