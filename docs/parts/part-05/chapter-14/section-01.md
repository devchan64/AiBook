...
| --- | --- | --- |
| 계획, 행동, 관찰: 여러 실행을 어떤 루프로 이어 갈까 | MCP: 그 도구와 자원을 어떤 공통 연결 형식으로 드러낼까 | 하네스: 그 연결을 쓴 실행을 어떤 기록과 재현 환경 안에 남길까 |
| 루프 구조 | 연결 인터페이스 | 실행 관리와 기록 |

즉, 지금 장의 핵심은 `어떤 순서로 실행할까`에서 `그 실행이 쓰는 도구와 자원을 어떤 공통 형식으로 노출할까`로 손잡이가 바뀐다는 점입니다.

처음 읽을 때는 이 전환을 아래 세 줄로만 기억해도 충분합니다.

| 여기서 바뀌는 손잡이 | 지금 먼저 확인할 것 |
| --- | --- |
| agent loop | 여러 단계 실행을 어떤 순서로 이어 갈 것인가 |
| MCP | 그 실행이 쓰는 도구와 자원을 어떤 공통 형식으로 드러낼 것인가 |
| harness | 그 연결을 쓴 실행을 어떤 기록과 replay 환경으로 남길 것인가 |

바로 다음 절과 이어서 놓고 보면, MCP가 무엇을 맡고 무엇을 아직 맡지 않는지도 더 분명해집니다.

| 층위 | 먼저 정리하는 것 | 아직 남는 질문 |
| --- | --- | --- |
| P5-14.1 MCP | 도구와 자원을 어떤 공통 연결 형식으로 드러낼 것인가? | 그 연결을 실제 실행에서 어떻게 기록하고 재현할 것인가? |
| P5-14.2 하네스 | 실행, trace, replay, approval을 어떤 환경에서 감쌀 것인가? | 그 기록을 기준으로 무엇이 좋은 실행인지 어떻게 판정할 것인가? |
| P5-15 평가 | 남은 기록과 결과를 어떤 기준으로 괜찮다고 볼 것인가? | 기준 미달일 때 어떤 수정과 운영 조치로 이어 갈 것인가? |

즉, MCP는 `연결을 덜 제각각으로 만드는 층`, 하네스는 `그 연결을 실제 실행 기록으로 남기는 층`, 평가는 `남은 기록을 품질 판단 기준으로 바꾸는 층`입니다. 이 차이가 보여야 MCP를 운영 장치나 평가 절차와 섞어 읽지 않게 됩니다.

처음 읽을 때는 아래 세 줄만 바로 구분해도 충분합니다.

| 지금 MCP에서 먼저 못 박을 것 | 아직 여기서 하지 않는 것 | 바로 다음 절로 넘길 것 |
| --- | --- | --- |
| 어떤 도구와 자원을 어떤 공통 형식으로 노출할까 | trace, replay, approval 같은 실행 관리 | 그 연결을 실제 실행 기록으로 어떻게 감쌀까 |
| 연결 인터페이스를 일정하게 만든다 | 품질 판정 기준을 세우지 않는다 | 하네스에서 실행 흔적을 남긴다 |
...
| --- | --- | --- |
| MCP는 `무엇과 어떻게 연결할까`를 더 일정한 형식으로 정리하는 연결 인터페이스 구조다. | P5-14.2에서는 그 연결을 써서 실행한 흐름을 어떤 환경 안에서 기록하고 재현할지 더 봅니다. | P5-15처럼 남은 기록을 기준으로 무엇이 좋은 실행인지 판정하는 단계는 아직 아닙니다. |
| 핵심은 모델을 더 똑똑하게 만드는 것이 아니라, 도구와 자원을 덜 제각각으로 노출하는 데 있다. | 바로 다음 절에서는 trace, replay, approval 같은 실행 관리 요구가 왜 필요한지 더 봅니다. | P5-16처럼 비용, 실패 대응, 운영 복구 판단까지는 아직 여기서 하지 않습니다. |
| 연결 표준이 생겨도 운영 품질은 자동 해결되지 않는다. | 뒤 절로 갈수록 연결 문제와 실행 관리 문제, 평가 문제를 더 분리해 읽게 됩니다. | 통합 미니 실습처럼 요청 흐름 전체를 기록 구조로 다시 묶는 일은 P5-17에서 닫습니다. |
