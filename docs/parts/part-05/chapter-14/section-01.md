...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. MCP는 `무엇과 어떻게 연결할까`를 일정한 형식으로 정리하는 연결 인터페이스 구조입니다. 핵심은 도구와 자원을 덜 제각각으로 노출하는 데 있고, P5-14.2에서는 그 연결을 쓴 실행 흐름의 기록과 재현으로 넘어갑니다.
