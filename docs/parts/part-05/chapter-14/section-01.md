...
처음 읽을 때는 이 경계만 다시 잡아도 충분합니다. 여기서 먼저 못 박을 것은 어떤 도구와 자원을 어떤 공통 형식으로 노출할지와 연결 인터페이스를 일정하게 만드는 일입니다. 반면 `trace`, `replay`, `approval` 같은 실행 관리와 품질 판정 기준은 아직 여기서 세우지 않고, 바로 다음 절의 하네스에서 그 연결을 실제 실행 기록으로 어떻게 감쌀지로 넘깁니다.
...
처음 읽은 뒤에는 이 경계만 다시 말할 수 있어도 충분합니다. MCP는 `무엇과 어떻게 연결할까`를 더 일정한 형식으로 정리하는 연결 인터페이스 구조이고, 핵심은 모델을 더 똑똑하게 만드는 것이 아니라 도구와 자원을 덜 제각각으로 노출하는 데 있습니다. P5-14.2에서는 그 연결을 써서 실행한 흐름을 어떤 환경 안에서 기록하고 재현할지 더 보고, 뒤 절로 갈수록 연결 문제와 실행 관리 문제, 평가 문제를 더 분리해 읽게 됩니다. 반면 P5-15처럼 남은 기록을 기준으로 무엇이 좋은 실행인지 판정하는 단계, P5-16처럼 비용·실패 대응·운영 복구 판단, P5-17처럼 요청 흐름 전체를 기록 구조로 다시 묶는 일은 아직 여기서 닫지 않습니다.
