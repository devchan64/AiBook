...
이 질문은 바로 P5-5.1 역전파(backpropagation)의 직관으로 이어집니다.
...
- 왜 회귀, 분류, 생성이 같은 손실을 그대로 쓰지 않는지 설명할 수 있는가?
- 회귀는 오차 크기, 분류는 정답 클래스 확신도, 생성은 다음 토큰 확률이라는 핵심 차이를 말할 수 있는가?
- 이 절 다음에는 손실이 앞쪽 층으로 어떻게 전달되는지 보는 역전파 장으로 넘어간다는 흐름을 이해했는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 회귀, 분류, 생성을 같은 출력 오류 감각으로 설명하고 있을 때, 문제 유형별 손실 관점을 먼저 떠올립니다.
- 분류나 생성에서 단순 정오표보다 확신 정도가 중요할 때, 확률 기반 손실 해석을 다시 꺼냅니다.
- 다음 토큰 예측이 왜 손실 구조와 직접 연결되는지 설명해야 할 때, 생성 손실을 반복적 분류 관점으로 다시 봅니다.
