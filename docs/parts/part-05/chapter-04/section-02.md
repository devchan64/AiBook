...
처음 읽을 때는 대화형 LLM을 Part 5 본류의 `사용자 경험 손잡이`로만 잡고, 그 아래에 어떤 층이 다시 풀려 나오는지까지 같이 보면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| GPT 기반 생성 구조 | 텍스트를 어떤 방식으로 이어 생성하는가? | P5-4.1, P5-5.1, P5-6.1 |
| 대화형 LLM 경험 | 왜 사용자는 이것을 답변하는 시스템처럼 느끼는가? | P5-4.2 |
| 지시 튜닝과 정렬 | 그 경험이 어떤 조정 단계로 만들어지는가? | P5-8.1, P5-8.2 |
| 프롬프트와 도구 연결 | 그 조정된 모델을 실제 요청과 실행 구조에 어떻게 붙일 것인가? | P5-9.1, P5-12.1, P5-12.2 |
