...
이 절의 목적은 사용자 경험의 변화가 단순히 모델 파라미터 증가만으로 생긴 것이 아님을 설명하는 데 있습니다.
...
이 도식은 `대화형 경험`이 단일 구조가 아니라 누적된 층이라는 점을 보여 줍니다.

## 사례로 보기

### 사례 1. 일반 자동완성

메일 작성창에 `안녕하세요, 지난 회의에서 논의한 내용은`까지만 적고 다음 문장을 추천받는 상황을 떠올려 보겠습니다. 사람이 이 기능을 쓸 때는 보통 `문장을 자연스럽게 이어 써 주는가`만 먼저 봅니다. 이 단계에서는 질문-답변 형식이나 역할 구분보다, 입력한 문장 뒤에 그럴듯한 표현이 이어지는지가 핵심입니다. 즉, `무엇을 계속 쓸까`가 중심인 경험입니다.

### 사례 2. 챗봇

사내 정책 문서를 열어 둔 채 `이 정책을 세 문장으로 설명해 줘`라고 묻는 상황을 생각해 보겠습니다. 사람이 단순 자동완성기로 기대하는 것은 보통 `다음 문장 후보` 정도이지만, 챗봇에게는 질문 형식 이해, 답변 길이 맞추기, 말투 유지, 위험한 답변 피하기까지 함께 기대합니다. 만약 모델이 길게 아무 말이나 이어 쓰기만 하면 사용자는 `질문을 들은 시스템`이 아니라 `문장을 잇는 도구`로 느낄 것입니다. 여기에 시스템 역할, 대화 이력, 말투 제약, 안전 규칙이 더해지면 비로소 사용자는 모델을 문장 자동완성기보다 대화 상대처럼 느끼게 됩니다. 즉, 대화형 경험은 생성 구조 위에 지시 따르기와 인터페이스 층이 덧씌워진 결과입니다.

### 사례 3. 코파일럿

개발자가 함수 안에 `여기서 사용자 입력을 검증하고 실패하면 에러를 반환`이라고 주석을 적는 상황을 떠올려 보겠습니다. 사람은 코드 자동완성도 결국 다음 몇 글자 추천 정도라고 생각하기 쉽지만, 실제로는 현재 함수 이름, 인자, 반환 형식, 주변 파일 문맥이 함께 작동합니다. 코파일럿은 이런 주변 문맥을 읽고 조건문, 예외 처리, 반환 코드까지 한 묶음으로 제안하므로, 단순 텍스트 자동완성과는 다른 도움으로 느껴집니다. 기본 원리는 생성 구조와 같지만, 편집기 안에서 코드 문맥, 함수 시그니처, 현재 커서 위치가 함께 작동하면서 훨씬 도메인 특화된 경험으로 바뀝니다. 이런 사례는 GPT 계열 생성 구조가 특정 인터페이스와 결합해 사용자 경험을 바꾸는 장면으로 볼 수 있습니다.
