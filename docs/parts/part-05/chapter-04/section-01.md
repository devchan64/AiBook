...
손실의 역할을 뒤 절에서 다시 짧게 확인해야 할 때는 개념사전의 [손실 함수(loss function)](../../../reference/concept-glossary.md#loss-function) 항목으로 돌아옵니다.
...
- 손실 함수와 metric의 차이를 `모델이 배우는 기준`과 `사람이 읽는 기준`으로 구분할 수 있는가?
- 이 절은 손실의 역할을 닫고, 문제 유형별 손실 차이와 역전파는 다음 절과 다음 장으로 넘긴다는 흐름을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 출력은 나왔지만 모델이 무엇을 기준으로 배우는지 설명이 비어 있을 때, 손실 함수 관점을 먼저 떠올립니다.
- 손실과 metric을 같은 숫자처럼 말하고 있을 때, 모델 내부 기준과 사람이 읽는 외부 기준을 다시 구분합니다.
- 예측과 목표의 차이가 어떻게 학습 신호가 되는지 정리해야 할 때, 손실로 숫자를 하나로 모으는 절차를 다시 봅니다.
