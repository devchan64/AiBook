...
- training mode, evaluation mode, regularization은 왜 이 루프 안에서 다시 읽어야 하는가?
...
분산 학습 루프, mixed precision, gradient accumulation은 이 책의 현재 본편 범위 밖으로 두고, 여기서는 단일 학습 루프의 공통 골격만 먼저 잡습니다. autograd 엔진의 내부 구현도 범위 밖으로 두되, gradient가 optimizer update로 이어지는 관점 자체는 앞선 P5-5.1, P5-5.2에서 본 흐름 위에 놓습니다. optimizer의 세부 감각은 P5-7.1, P5-7.2에서 다시 다루고, 여기서는 학습 절차의 순서를 먼저 닫습니다.
