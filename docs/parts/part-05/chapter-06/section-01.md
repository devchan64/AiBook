...
- learning과 inference를 `파라미터를 바꾸는 시간`과 `고정된 파라미터를 쓰는 시간`으로 설명할 수 있는가?
- 결과가 나왔다는 사실만으로 학습이 일어난 것은 아니라는 점을 말할 수 있는가?
- 이 절 다음에는 학습 모드와 평가 모드의 계산 차이를 따로 봐야 한다는 흐름을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 같은 모델 실행이라도 학습과 서비스 처리를 같은 것으로 말하고 있을 때, learning/inference 구분을 먼저 떠올립니다.
- 결과가 나왔다는 사실만으로 학습이 일어났다고 오해하기 쉬울 때, 파라미터 변경 여부를 다시 기준으로 잡습니다.
- 학습 절차와 실행 절차를 나눠 설명해야 할 때, forward는 공통이지만 후속 단계가 다르다는 점을 다시 꺼냅니다.
