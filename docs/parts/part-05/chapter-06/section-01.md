...
대신 이번 절에서 바로 넓히지 않을 질문도 분명합니다. learning과 inference를 왜 구분해야 하는지는 다음 Section인 P5-6.2에서 이어서 설명하고, training mode와 evaluation mode의 차이는 P5-6.3에서 다시 설명합니다. 즉, 이번 절은 `학습 루프의 순서`를 붙잡는 자리이고, 바로 다음 절들은 `그 루프가 언제 어떤 방식으로 실행되는가`를 구분하는 자리입니다.
...
P5-6.1에서는 먼저 `예측 만들기 -> 오차 점수화 -> 책임 되돌리기 -> 값 갱신`이라는 핵심 4단계만 고정합니다. learning과 inference의 차이는 P5-6.2에서, training mode와 evaluation mode의 차이는 P5-6.3에서 다시 읽습니다. regularization도 뒤 장에서 다시 연결합니다.
...
| --- | --- | --- |
| 개념은 알겠는데 순서가 자꾸 섞인다 | forward, loss, backward, update의 공통 골격을 다시 고정할 수 있습니다. | P5-6.2에서 learning/inference 구분으로 이어집니다. |
| 구조 장으로 넘어가기 전에 공통 학습 뼈대를 다시 확인하고 싶다 | CNN, RNN, Transformer도 결국 같은 루프 안에서 학습된다는 점을 정리할 수 있습니다. | P5-6.2, P5-6.3과 뒤 구조 장으로 이어집니다. |
