...
이 절에서는 스케일의 방향만 잡고, 다음 토큰 예측으로 이어지는 학습 목표는 P5-5.1에서 다시 구체화합니다. GPU 인프라와 최신 상용 모델 수치 비교는 빠르게 바뀌는 주제이기도 하므로 현재 본편 범위 밖으로 두고, 운영 제약의 큰 그림만 P5-16.1 AI 서비스가 현실에서 만나는 제약에서 다시 연결합니다.
