# P5-6.2 대화형 LLM으로의 전환

P5-6.1에서는 GPT 계열을 이전 토큰을 바탕으로 다음 토큰을 이어 생성하는 decoder 중심 흐름으로 설명했습니다. 이제 다음 질문이 생깁니다.
...
대화형 전환의 큰 흐름은 여기서 잡고, 지시를 따르도록 조정하는 단계는 P5-10.1 지시 튜닝과 P5-10.2 정렬의 기본 문제에서 다시 설명합니다. 프롬프트 설계는 P5-11.1, 도구 사용은 P5-14.1과 P5-14.2, agent loop는 P5-15.1과 P5-15.2, MCP 연결은 P5-16.1에서 각각 다시 회수합니다.
...
문장 앞부분을 주고 뒤를 이어 쓰게 하는 기능은 생성 구조 자체의 직관을 잘 보여 줍니다.

### 사례 2. 챗봇

사용자는 질문을 하고, 모델은 답변 형식으로 응답합니다. 여기에 역할, 톤, 형식 제약이 더해지면 대화형 경험이 강화됩니다.

### 사례 3. 코파일럿

코드 설명과 주석을 바탕으로 구현을 이어 쓰는 경험은 GPT 계열 생성 구조가 특정 도메인 인터페이스와 결합된 사례입니다.
...
- 이렇게 큰 모델은 먼저 무엇을 배우는가?
- 사전학습(pretraining)은 실제로 어떤 역할을 하는가?
