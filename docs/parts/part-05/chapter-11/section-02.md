...
이 전환을 앞뒤 장과 한 번에 붙여 보면, 바로 앞 장의 벡터 데이터베이스가 `무엇을 어떤 retrieval 저장소에 넣을까`를 맡고, 지금 장의 인덱스는 그 저장소 안에서 후보를 얼마나 빠르고 정확하게 좁힐지를 맡습니다. 그리고 바로 다음 tool use 장에서는 문서 검색을 넘어서 무엇을 실제로 조회하거나 실행할지로 질문이 더 커집니다. 즉, 흐름은 `저장 구조 -> 탐색 구조와 속도-품질 균형 -> 실행 연결`로 이어집니다.
