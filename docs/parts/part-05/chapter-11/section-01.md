...
이 질문은 바로 P5-11.2 합성곱(convolution)과 풀링(pooling)으로 이어집니다.
...
- 손글씨, 얼굴, 산업 비전 사례에서 공통으로 먼저 읽어야 할 것이 지역 단서라는 점을 말할 수 있는가?
- 다음 절에서 convolution을 읽을 때도 먼저 `어느 지역 패턴에 얼마나 반응하는가`를 떠올릴 준비가 되어 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 이미지 문제를 전체 인상으로만 설명하고 있을 때, 지역 패턴 관점을 먼저 떠올립니다.
- 같은 물체가 위치나 배경이 달라도 비슷하게 읽혀야 하는 이유를 설명해야 할 때, 가까운 구조를 반복해서 읽는 CNN 직관을 다시 봅니다.
- 완전연결층과 CNN 차이를 구조 이름이 아니라 입력 구조 가정 차이로 설명해야 할 때, 이 절을 기준선으로 씁니다.
