...
처음 읽을 때는 이 절을 `retrieval 저장소 손잡이 축`으로만 고정해 두면 덜 흔들립니다.

| 지금 이 절에서 읽는 것 | 바로 다음 절이나 뒤 장으로 넘기는 것 |
| --- | --- |
| 임베딩, 원문, 메타데이터를 어떤 retrieval 저장 구조에 함께 담아 둘 것인가 | 그 저장 구조 안에서 후보를 어떤 속도와 품질 균형으로 좁힐 것인가 |
| 왜 의미 기반 검색을 위해 벡터 저장 계층이 필요한가 | 실제 실행 연결과 운영 구조로 어떻게 이어질지 |
...
처음 읽을 때는 벡터 데이터베이스를 Part 5 본류의 `retrieval 저장소 손잡이`로만 잡고, 그 다음에 어떤 구조가 더 붙는지까지 같이 보면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 벡터 데이터베이스 | 검색 후보 문서를 어떤 저장 구조에서 다시 꺼내 올 것인가? | P5-11.1 |
| 인덱스(index) | 그 저장 구조 안에서 후보를 얼마나 빠르고 정확하게 좁힐 것인가? | P5-11.2 |
| 도구 사용(tool use) | 문서 검색을 넘어서 무엇을 실제로 조회하거나 실행할 것인가? | P5-12.1, P5-12.2 |

이 전환을 앞뒤 장과 한 번에 붙여 보면, 바로 앞 장의 RAG 결합이 `찾아온 문서를 생성 전에 어디에 붙일까`를 맡고, 지금 장의 벡터 데이터베이스는 그 문서를 어떤 retrieval 저장 구조에서 다시 꺼낼지를 맡습니다. 그리고 바로 다음 인덱스 장에서는 그 저장 구조 안에서 후보를 얼마나 빠르고 정확하게 좁힐지를 더 봅니다. 즉, 흐름은 `검색 후 생성 흐름 -> retrieval 저장소 -> 탐색 구조와 검색 품질`로 이어집니다.

즉, 지금 장의 핵심은 `문서를 붙이는가`에서 `그 문서를 서비스 안에서 다시 꺼내 쓸 저장 구조가 있는가`로 손잡이가 바뀐다는 점입니다.
