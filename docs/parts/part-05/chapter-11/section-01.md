...
사내 위키에서 사용자가 `퇴사 전에 장비를 어디에 반납하나요?`라고 묻는 장면을 생각해 볼 수 있습니다. 사람이 키워드 검색만 쓰면 `장비 반납`이라는 표현이 그대로 들어간 문서만 먼저 찾게 됩니다. 하지만 실제 문서 제목은 `오프보딩 절차`, `자산 회수 안내`, `퇴사 체크리스트`처럼 다를 수 있습니다. 벡터 데이터베이스는 질문과 문서 조각을 의미 기반 벡터로 저장해, 단어가 완전히 같지 않아도 비슷한 안내 문서를 후보로 꺼내기 쉽게 만듭니다. 그래서 이 사례는 `같은 단어 찾기`보다 `비슷한 뜻 찾기`가 중요한 장면에서 벡터 저장소가 왜 필요한지 보여 줍니다.

### 사례 2. 제품 매뉴얼 검색

제품 매뉴얼에서 사용자가 `설정을 처음 상태로 되돌리고 싶어요`라고 묻는다고 해 봅시다. 문서 쪽에는 `리셋`, `초기화`, `설정 복원`처럼 서로 다른 표현이 섞여 있을 수 있습니다. 사람이 단순 문자열 검색만 쓰면 표현이 조금만 달라져도 중요한 문단을 놓치기 쉽습니다. 벡터 데이터베이스는 이런 문서 조각을 의미가 가까운 위치에 저장해, 표현 차이가 있어도 관련 후보를 모으는 데 유리합니다. 그래서 매뉴얼 검색 사례에서는 `용어 통일이 안 된 문서`를 다룰수록 벡터 저장 구조의 가치가 더 커집니다.

### 사례 3. 개발 문서 지원

개발자가 `요청 제한이 걸렸을 때 잠깐 기다렸다 다시 보내는 옵션이 있나요?`라고 묻는다고 해 봅시다. 사람은 먼저 함수 이름이나 옵션명을 정확히 말해야 검색이 되겠지라고 생각할 수 있습니다. 하지만 질문에는 정확한 함수 이름이 없고, 실제로는 retry나 backoff 관련 API 설명 문단을 찾아야 할 수 있습니다. 키워드 검색만 쓰면 함수명이나 옵션명이 없는 질문에서 관련 문서를 놓칠 가능성이 큽니다. 벡터 데이터베이스는 이런 질문과 문서 조각을 의미 기반으로 가깝게 저장해 관련 API 설명을 더 잘 후보에 올릴 수 있습니다. 그래서 개발 문서 지원에서는 `정확한 이름을 모르는 질문`일수록 벡터 검색 구조가 더 중요해집니다.
