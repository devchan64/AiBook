...
이 절에서는 입력 설계의 기본 손잡이만 다루고, 프롬프트가 어디서 한계에 부딪히는지는 바로 다음 P5-11.2 프롬프트의 한계에서 다시 회수합니다. tool use와 agent 구조는 P5-14.1 도구 사용, P5-14.2 함수 호출, P5-15.1 에이전트에서 각각 다시 다룹니다.
...
`요약해 줘`만 쓰는 것보다 `독자 기준으로 세 줄로 요약해 줘`처럼 길이와 독자 수준을 함께 주면 결과가 더 안정될 수 있습니다.
