...
즉, P5-10.2가 검색된 문서를 생성 전에 어디에 붙일지를 다뤘다면, 지금 절은 그 문서를 임베딩, 원문, 메타데이터와 함께 어떤 retrieval 저장 구조에 둘지를 다룹니다. 이어지는 P5-11.2는 그 저장 구조 안에서 후보를 얼마나 빠르고 정확하게 좁힐지로 넘어갑니다. 이 기준이 잡혀야 벡터 데이터베이스를 또 하나의 유행 기술명이 아니라, `RAG 검색 단계를 운영 가능한 저장소로 바꾸는 층`으로 읽을 수 있습니다.
