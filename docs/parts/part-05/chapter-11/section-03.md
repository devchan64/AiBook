...
이 보충학습에서는 CNN과 ViT를 `이미지를 어떤 계산 단위로 읽기 시작하는가`라는 비교 질문으로 먼저 닫고, 본편의 CNN 직관 뒤에 왜 ViT가 다른 출발점을 갖는지 보강하는 데 집중합니다.

ViT의 self-attention 자체는 P5-13.2와 P5-14.1에서 다시 연결되고, 여기서는 이미지 관점에서 `지역 패턴을 먼저 읽는가`, `패치 관계를 먼저 읽는가`라는 출발 문제 차이까지만 정리합니다.
...
| 단계 | ViT에서 실제로 하는 일 |
| --- | --- |
| 1 | 이미지 한 장을 여러 패치로 자른다 |
| 2 | 각 패치를 숫자 벡터 하나로 바꾼다 |
| 3 | 각 패치가 다른 패치를 얼마나 참고해야 하는지 attention으로 계산한다 |
| 4 | 여러 패치 관계를 합쳐 최종 판단에 쓸 표현을 만든다 |
...
| 둘 다 결국 같은 설비를 맞히면 비슷하다고 생각하기 쉽다 | 출발 계산 단위가 `겹치며 이동하는 지역 창`이라는 점을 먼저 본다 | 출발 계산 단위가 `잘라 둔 패치 토큰`이라는 점을 먼저 본다 |

그래서 이 사례에서 확인해야 할 결과는 둘 다 설비를 맞히는가보다, CNN은 경광등 모서리와 탱크 윤곽 같은 지역 반응이 먼저 뜨고 ViT는 떨어진 패치들이 함께 중요해지는 관계가 먼저 드러나는가입니다.

```mermaid
--8<-- "assets/part-05/chapter-11/cnn-vit-beacon-tank-case-flow-ko.mmd"
```

이 도식은 사례 2를 `같은 설비 이미지 -> 지역 반응 우선 -> 구역 관계 우선`으로 다시 접어, CNN 출력이 재검사 위치 단서로, ViT 출력이 구역 관계 단서로 읽히는 출발 차이를 더 짧게 고정합니다.
...
두 사례를 같이 놓고 보면 비교의 핵심은 `가까운 지역 단서`와 `패치 사이 관계` 중 무엇을 먼저 세우느냐입니다. 그래서 여기서 최종적으로 확인해야 할 결과는 분명합니다. CNN과 ViT의 차이는 `둘 다 이미지를 분류한다`에서 생기는 것이 아니라, CNN은 `어느 지역 반응이 먼저 강하게 뜨는가`, ViT는 `어느 패치 관계가 함께 중요해지는가`라는 첫 질문의 차이에서 시작됩니다.

같은 장면을 다시 한 줄로 묶으면, CNN은 `어느 위치를 먼저 다시 봐야 하는가`를 세우는 데 강하고, ViT는 `어느 구역이 다른 구역과 관계상 어긋나는가`를 세우는 데 강한 출발점을 갖는다고 읽는 편이 안전합니다.

같은 설비 입력 하나를 기준으로 두 구조의 출발 질문을 한 번에 고정하면 다음처럼 볼 수 있습니다.

```mermaid
--8<-- "assets/part-05/chapter-11/cnn-vit-common-input-compare-ko.mmd"
```

이 비교 도식에서 먼저 붙잡아야 할 점은 세 가지입니다.

- 입력은 같은 설비 이미지지만, CNN은 작은 국소 창이 어디서 먼저 강하게 반응하는지부터 따라갑니다.
- ViT는 같은 이미지를 패치 토큰으로 나눈 뒤, 어떤 패치끼리 함께 중요해지는지를 먼저 따집니다.
- 그래서 CNN 쪽 출력은 `다시 볼 위치 후보`처럼 읽히고, ViT 쪽 출력은 `함께 해석할 구역 관계`처럼 읽히기 시작합니다.
...
| 패치 평균 보조값 | `(0, 0)` 패치가 가장 높을 가능성이 큼 | 좌상단 3x3 영역은 대부분 `4`와 `5`로 채워져 있어 단순 평균이 높습니다. |
| 구역 차이 보조 비교 | `blank_code_gap`이 `seal_code_gap`보다 더 클 가능성이 큼 | 코드 영역 평균 `3.56`은 빈 배경 `1.11`과 더 크게 벌어지고, 실링 `2.0`과는 덜 벌어집니다. |

이 표의 목적은 정확한 숫자를 미리 다 계산하는 데 있지 않습니다. 같은 프레임이라도 CNN 쪽 보조 계산은 `어디의 국소 대비가 가장 큰가`를 먼저 보게 만들고, ViT 쪽 보조 계산은 `이미지를 큰 패치 토큰으로 보면 어느 구역 차이가 먼저 눈에 들어오는가`를 붙잡게 하는 데 있습니다.
...
| 패치 단위 보조값 | `(3, 3)` 패치는 평균 `3.56`, `(0, 3)` 패치는 평균 `1.11`로 요약됩니다. | 패치 평균을 실제 ViT attention 결과처럼 보기 쉽습니다. | 여기서는 실제 attention을 계산한 것이 아니라, 이미지를 패치 단위로 바꾸면 구역 관계 질문이 생긴다는 출발점을 확인하는 것입니다. |
...
이 예제에서는 `inspection_frame` 크기를 늘리거나 `patch_size`, `stride`를 바꿔 볼 수 있습니다. 그러면 독자는 단순히 `CNN은 부분, ViT는 패치`라는 문장을 외우는 대신, 같은 입력이 `몇 개의 국소 결함 후보`와 `몇 개의 구역 토큰 관계`로 바뀌는지 직접 비교해 볼 수 있습니다.
...
| ViT는 4개의 큰 패치 토큰으로 시작한다 | `patch_size`를 더 작게 하거나 크게 해 구역 관계 비교가 어떻게 달라지는지 본다 | 토큰 수가 많다고 해서 곧바로 항상 더 좋은 attention 결과가 나온다고 단정하지 않는다 |
| `seal_code_gap` 같은 구역 차이가 계산된다 | 다른 토큰 쌍을 비교해 어떤 관계가 품질 문제와 더 연결되는지 본다 | 평균값 차이 예제 하나로 실제 ViT attention 전부를 설명했다고 단정하지 않는다 |
...
| 추가 질문 | CNN | ViT |
| --- | --- | --- |
| 처음 잘라 보는 단위 | 작은 receptive field | 잘라 놓은 패치 토큰 |
...
| 왜 패치라는 말이 중요해지는가 | 이미지에서도 토큰처럼 취급할 기본 단위가 있어야 attention 구조를 적용할 수 있기 때문 | 패치 토큰, self-attention, image transformer 확장 |
