...
| 학생 | 시험 점수 기여 | 결석 기여 | 추가 과제 기여 | 합산 점수 해석 |
| --- | --- | --- | --- | --- |
| A | 큼 | 작음 | 있음 | 승인 쪽으로 기운다 |
| B | 보통 | 큼 | 없음 | 보류 쪽으로 밀린다 |
...
같은 코드에서 `b = -0.5`로 바꾸면 `z`는 `0.0`이 되고 출력은 `0`이 됩니다. 이 한 줄 변화는 편향이 경계를 얼마나 쉽게 옮길 수 있는지 보여 줍니다. 즉, 연습에서 확인해야 할 결과는 입력값 자체를 바꾸지 않아도 가중치나 편향 조정만으로 최종 판단이 달라질 수 있다는 점입니다.
