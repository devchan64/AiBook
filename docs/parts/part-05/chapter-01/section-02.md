...
앞의 예제는 실제 토크나이저 비교 코드가 아니라, 사람이 보기에는 비슷한 문장도 표기 방식이 달라지면 모델 쪽에서는 다른 길이와 다른 조각 감각으로 들어올 수 있다는 점을 가장 작게 보여 주는 장면입니다. 여기서 읽어야 할 핵심은 문장의 뜻보다 먼저, 입력 표기 방식이 계산 비용과 문맥 사용량에 직접 영향을 줄 수 있다는 점입니다.
