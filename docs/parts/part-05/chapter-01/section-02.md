...
P5-1.1에서는 퍼셉트론(perceptron)을 `입력(input) -> 가중치(weight) -> 합(sum) -> 출력(output)` 흐름으로 보았습니다. 이제 바로 다음 질문이 생깁니다.
...
독자에게는 이것을 아주 엄밀하게 증명할 필요는 없습니다. 다만 `직선 하나로 나뉘는 문제`와 `여러 꺾인 경계가 필요한 문제`를 구분하는 감각은 먼저 잡아야 합니다.
...
즉, 퍼셉트론은 Part 3의 선형 모델 감각과 Part 5의 신경망 감각을 이어 주는 징검다리입니다.
