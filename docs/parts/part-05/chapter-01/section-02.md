...
- 퍼셉트론이 먼저 입력의 선형 결합으로 점수를 만들고, 활성화가 그 점수를 실제 출력 규칙으로 바꾼다는 두 단계를 설명할 수 있는가?
- 퍼셉트론 하나가 입력 공간을 하나의 선형 경계로 나누는 판단기라는 점을 설명할 수 있는가?
- 왜 XOR 같은 예제가 `규칙이 단순한가`보다 `한 직선으로 나눌 수 있는가`를 묻게 만드는지 설명할 수 있는가?
- 여러 층을 쌓는 이유가 단순 선형 판단을 조합해 더 복잡한 표현을 만들기 위해서라는 점을 말할 수 있는가?
- 입력들을 먼저 하나의 점수로 모은 뒤 판단한다는 흐름이 헷갈릴 때, 선형 결합과 활성화의 두 단계를 먼저 떠올릴 수 있는가?
