...
- 퍼셉트론은 여러 입력을 가중합으로 모아 출력을 만드는 가장 단순한 신경망 판단 단위입니다.
- 가중치(weight)는 입력마다 중요도를 다르게 주는 값입니다.
- 편향(bias)은 기준선을 옮기는 값입니다.
- 퍼셉트론의 핵심 계산은 `입력 -> 가중치 -> 합 -> 출력` 흐름입니다.
...
- 왜 Part 5의 시작을 퍼셉트론 같은 가장 작은 계산 단위에서 여는지 말할 수 있는가?
- 가중치와 편향이 각각 입력 중요도 조정과 기준선 이동이라는 역할을 한다는 점을 설명할 수 있는가?
- 퍼셉트론 설명만으로는 아직 선형 판단 단위까지만 닫고, 활성화와 다층 구조는 다음 절과 다음 장으로 넘긴다는 점을 알고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 신경망을 처음 설명해야 하는데 복잡한 층 구조보다 가장 작은 계산 단위부터 잡는 편이 나을 때, 퍼셉트론 관점을 먼저 떠올립니다.
- 여러 입력 신호가 어떻게 하나의 판단으로 모이는지 흐려질 때, 가중치와 편향이 붙은 가중합 계산으로 다시 돌아갑니다.
- 딥러닝을 거대한 모델 이름으로만 읽고 있을 때, 반복되는 기본 문법이 `입력 -> 가중합 -> 출력`이라는 점을 다시 꺼냅니다.
