...
처음 읽을 때는 이 보충학습을 `토큰 수 감각`과 `토크나이저 계열 차이`를 연결하는 손잡이로만 잡고, 구현 세부는 본편 바깥으로 넘겨 두는 편이 가장 안전합니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 토큰 수와 길이 감각 | 같은 문장도 왜 계산 조각 수가 달라질 수 있는가? | P5-1.1, P5-1.2 |
| 토크나이저 계열 차이 | 어떤 분절 방식이 어떤 언어 감각과 비용 차이를 만드는가? | P5-1.3 |
| 검색과 표현 연결 | 이 분절 차이가 청크 경계, 임베딩, 검색 품질에 어떻게 이어지는가? | P5-2.1, P5-10.1, P5-10.2, P5-11.1, P5-11.2 |
