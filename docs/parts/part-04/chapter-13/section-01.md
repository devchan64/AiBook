...
즉, 균등 평균에서는 관련 없는 후보도 같이 섞이지만, attention은 질문과 더 직접 연결된 후보에 비중을 몰아 최종 문맥을 그쪽으로 더 끌어갑니다.

이 예제도 한 번 읽고 끝내기보다, 어떤 값을 바꿔 보면 `선택적 참조` 감각이 더 선명해지는지 바로 이어서 확인하는 편이 좋습니다.
