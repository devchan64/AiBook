...
| --- | --- | --- |
| `return_period weight = 0.762`가 가장 크다 | 질문과 직접 관련된 후보에 attention 비중이 집중된다는 뜻 | `shipping_fee`나 `coupon_policy` 점수를 높이면 어떤 후보가 질문 해석을 더 끌어가는지 달라집니다 |
| `context = 11.211`이 `baseline_uniform_context = 6.0`보다 크다 | 모든 후보를 균등 평균내는 대신, 중요한 후보 쪽으로 문맥값이 더 이동했다는 뜻 | 후보 값 자체를 바꾸면 attention이 어느 방향으로 context를 당기는지 직접 비교할 수 있습니다 |
| `lift_from_baseline = 5.211`이 양수다 | 질문 관련 위치에 비중이 실리면서 최종 문맥 표현이 baseline보다 더 관련 정보 쪽으로 끌렸다는 뜻 | score 분포를 더 평평하게 하거나 더 뾰족하게 바꾸면 lift가 얼마나 줄거나 커지는지 볼 수 있습니다 |

즉, 균등 평균에서는 관련 없는 후보도 같이 섞이지만 attention은 질문과 더 직접 연결된 후보에 비중을 몰아 최종 문맥을 그쪽으로 더 끌어가며, 어떤 값을 바꿔 보면 `선택적 참조` 감각이 어떻게 달라지는지도 바로 다시 확인할 수 있습니다.

| 먼저 보인 출력 신호 | 지금 바로 해 볼 변화 | 아직 이 예제만으로 서두르지 않을 결론 |
| --- | --- | --- |
| `return_period` 비중이 압도적으로 크다 | `shipping_fee` 점수를 높여 질문이 다른 후보 쪽으로 흔들릴 때 weight가 어떻게 재배분되는지 본다 | 큰 weight 하나가 나왔다고 해서 attention이 곧 완전한 이해를 보장한다고 단정하지 않는다 |
| `context`가 baseline보다 크게 올라간다 | 후보 값 자체를 바꿔 attention이 어떤 값을 더 크게 끌어오는지 본다 | context 값 하나만으로 실제 답변 품질 전체를 결론내리지 않는다 |
| `lift_from_baseline`이 양수다 | score 차이를 줄이거나 키워 균등 평균 대비 선택적 참조의 이득이 얼마나 달라지는지 본다 | 장난감 가중 평균 예제 하나로 실제 multi-head attention 전체를 대체하지 않는다 |

앞의 숫자는 실제 단어 임베딩 전체를 계산한 것은 아니지만, baseline 평균이 `문장들이 그냥 같이 있었다`는 사실만 남기는 반면 attention 가중 평균은 `지금 질문이 무엇이냐`를 기준으로 후보 비중을 다시 나눈다는 점은 충분히 드러납니다. 즉, attention은 단순히 정보를 더 많이 모으는 방식이 아니라, `현재 질문에 맞게 어떤 정보를 더 크게 섞을지 다시 정하는 방식`입니다.
...
- attention은 현재 계산에 중요한 위치를 더 크게 참고하는 방식이며, 장기 의존성 문제에 대한 더 직접적인 응답입니다.
- 번역, 요약, 질의응답 같은 순차 문제에서 attention은 `상태를 오래 들고 가기`보다 `필요한 위치를 다시 찾아본다`는 구조 전환을 보여 줍니다.
