...
커널(kernel) 발상과 비선형 경계의 큰 그림은 P4-13.2에서 바로 이어서 다루고, `C`, `gamma` 같은 하이퍼파라미터를 읽는 기준과 검증 비용은 P4-9.1, P4-9.2에서 다시 연결합니다. 최적화 목적 함수, 라그랑주 승수, 듀얼 문제의 엄밀한 전개는 이 책의 현재 본편 범위 밖에 둡니다.

현재 Part 4에서는 이 수학 전개를 별도 `보충학습`으로 다시 만들지 않습니다. 이 책에서 SVM의 학습 목표는 margin, support vector, soft margin, kernel 직관까지로 닫히며, 라그랑주 승수나 듀얼 최적화의 엄밀한 전개는 본편과 보충학습을 오가며 회수하기보다 더 심화된 수학 자료에서 따로 보는 편이 범위상 더 적절합니다.
...
| 앞 절 | 지금 절에서 이어받는 질문 |
| --- | --- |
| P4-11.2 결정 경계 | 경계를 그었다면, 어떤 경계가 더 좋은가? |
| P4-12.2 거리와 스케일 | 경계와 점 사이의 간격을 어떻게 읽을 것인가? |
| P4-9 하이퍼파라미터 | `C` 같은 값이 왜 중요해지는가? |

즉, SVM은 `분류선`을 배우는 절이 아니라, `좋은 분류선의 기준`을 배우는 절입니다.

여기에 한 가지를 더 붙이면 SVM 절이 지금까지 정리한 비교 기록 구조와 자연스럽게 이어집니다. SVM을 후보로 올릴 때는 `margin이 크다`는 말만 남기는 것이 아니라, `어떤 사례가 margin 근처에 남아 있는가`, `baseline이나 다른 후보보다 무엇이 더 안정적으로 보이는가`, `다음에 무엇을 더 조정할 것인가`를 함께 적어 두는 편이 좋습니다. 이때 margin 근처 사례는 우선 검토 우선순위를 올리는 신호로 읽고, 그 사례가 왜 거기에 남았는지 원인 설명까지 자동으로 끝난 것으로 보지는 않는 편이 안전합니다.
...
처음 읽을 때는 앞 모델들과 무엇이 다르게 중요해지는지만 다음처럼 잡아도 충분합니다.
...
즉, SVM은 `경계를 그릴 수 있는가`를 넘어서 `그 경계가 얼마나 여유 있고 안정적인가`를 중심 질문으로 바꿉니다. 이 기준이 잡혀야 뒤의 soft margin, kernel, `C` 같은 설명도 새 옵션 목록이 아니라 `좋은 경계의 기준을 조정하는 장치`로 읽힙니다.
...
여기서는 다음처럼 기억하면 충분합니다.

`margin은 경계선과 가장 가까운 데이터들 사이의 여유 폭이다.`

이 여유 폭이 크면, 경계가 데이터 사이에 더 안정적으로 놓여 있다고 읽을 수 있습니다.

즉, SVM은 단순히 `선을 그릴 수 있는가`를 넘어서 `여러 경계 후보 중 어떤 경계가 더 안정적인가`를 묻습니다. 핵심은 분리 가능 여부가 아니라, 가장 가까운 점들과의 최소 간격을 기준으로 더 여유 있는 경계를 고른다는 데 있습니다.
...
이 도식은 작은 margin과 큰 margin의 차이를 직관적으로 비교합니다. 경계가 한쪽 class에 너무 가까우면 작은 흔들림에도 결과가 쉽게 바뀌지만, 양쪽에서 여유를 둔 경계는 새 데이터에도 덜 예민하게 반응할 가능성이 큽니다.
...
이 도식은 support vector가 왜 특별한지 보여 줍니다. 전체 학습 점이 모두 같은 무게로 경계를 정하는 것이 아니라, 경계에 가장 가까운 일부 점들이 실제로는 분리선 위치를 더 강하게 떠받친다는 뜻입니다.
