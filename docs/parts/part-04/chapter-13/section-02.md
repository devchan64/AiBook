...
Transformer 전체 구성은 P4-14.1, P4-14.2에서 이어서 다루고, context window와 실제 LLM 사용 제약은 Part 5의 P5-4.1, P5-4.2에서 다시 연결합니다. query, key, value의 공식 유도와 multi-head attention 세부 구현은 이 책의 현재 본편 범위 밖에 둡니다.
...
대명사나 참조 관계는 가까운 단어만으로는 충분하지 않을 수 있습니다. self-attention은 문장 안의 관련 토큰을 더 직접 참고하는 직관을 줍니다.

### 사례 2. 문서 요약

중요 문장을 만들 때 문서 앞뒤의 핵심 단어를 함께 참조해야 할 수 있습니다. self-attention은 이런 전역 참조(global reference) 직관과 잘 맞습니다.

### 사례 3. 코드 이해

코드 토큰도 멀리 떨어진 변수 정의나 함수 호출 맥락을 참조해야 할 수 있습니다. self-attention은 이런 관계를 설명하는 데도 유용합니다.
