...
`kernel`이라는 이름은 추상적으로 들리기 쉽습니다. 여기서는 수학적 엄밀성보다, 이 이름이 실제로 무엇을 가리키는지 먼저 잡습니다.
...
즉, kernel 절은 `수식이 복잡해진다`보다 `사례가 다른 공간에서 다시 배치된다`는 관점으로 먼저 읽습니다. 같은 분류 결과처럼 보여도 어떤 사례가 계속 애매하게 남는지, 어떤 사례는 더 또렷하게 갈리는지는 따로 확인해야 합니다.
