...
- self-attention은 같은 시퀀스 안의 토큰들이 서로를 참고해 표현을 다시 계산하는 방식으로, RNN식 순차 상태 전달과 달리 먼 위치 단서를 직접 다시 참조하면서도 토큰 계산을 병렬로 처리할 수 있습니다. Transformer는 이 방식을 핵심 계산 장치로 삼습니다.
