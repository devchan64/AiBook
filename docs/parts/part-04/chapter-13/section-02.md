...
P4-13.1이 `현재 출력이 입력 어디를 더 참고할까`를 묻는 절이었다면, 여기서는 그 질문이 `현재 토큰이 같은 문장 안 다른 토큰을 어떻게 다시 참고할까`로 바뀝니다.
...
즉, baseline 평균에서는 `그것`이 무엇을 가리키는지에 대한 강조가 거의 없지만, self-attention은 현재 토큰이 문장 안 다른 토큰들을 다시 참고해 새 표현을 만들며 이 예제에서는 `반품`보다 `박스`와 `버리지` 쪽 단서를 훨씬 더 크게 반영합니다. `box_plus_not_discarded_weight = 0.694`와 `representation_shift` 변화는 현재 토큰 표현이 `박스/버리지` 문맥 쪽으로 실제로 다시 당겨졌음을 보여 주며, 결국 self-attention을 `지금 이 토큰을 이해하려면 문장 안 어디를 다시 봐야 하는가`를 수치로 정하는 방식으로 읽게 합니다.
...
- self-attention은 같은 시퀀스 안의 토큰들이 서로를 참고해 표현을 다시 계산하는 방식이며, RNN식 순차 상태 전달과 다른 계산 감각을 제공합니다.
- self-attention은 먼 위치 단서를 다시 참조하면서도 토큰 계산을 병렬로 처리할 수 있고, Transformer는 이 방식을 핵심 계산 장치로 삼습니다.
