...
### 직접 특징을 더하는 것과 kernel은 어떻게 이어지는가

여기서 한 번 더 짚어야 할 질문이 있습니다.

- `그럼 새 특징을 직접 만들면 충분한 것 아닌가?`
- `왜 굳이 kernel이라는 말을 따로 써야 하는가?`

입문 수준에서는 먼저 이렇게 잡는 편이 좋습니다.

- 직접 특징 추가: 사람이 `x1 * x2`, `x1^2` 같은 새 특징을 명시적으로 만든다.
- kernel 발상: 그런 더 풍부한 표현 공간에서의 비교 효과를, 원래 공간 계산으로 우회해 다룬다.
