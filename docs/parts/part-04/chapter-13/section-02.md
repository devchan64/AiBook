...
여기서도 기록 구조를 같이 고정해 둡니다. kernel 절은 단순히 `다른 함수 이름`을 배우는 절이 아니라, `같은 사례가 다른 표현 공간에서 어떻게 다시 읽히는가`를 기록하는 절이기 때문입니다. 따라서 어떤 사례가 원래 공간에서는 애매했는지, 새 표현에서는 어떻게 재배치되었는지, 그 결과 review 대상이 줄었는지 또는 새로 생겼는지를 함께 적어 둡니다. 이 변화는 우선 `같은 점수를 주더라도 어떤 실패 양상을 더 잘 가르는가`를 보여 주는 신호로 읽고, 표현 하나만으로 원인 설명까지 끝난 것으로 보지는 않습니다.
...
kernel은 기본값으로 습관처럼 붙이기보다, `선형 경계 자체가 답답한가`를 확인한 뒤 올리는 편이 더 분명합니다.

| 보이는 장면 | kernel 후보를 떠올릴 이유 | 먼저 떠올릴 방향 |
| --- | --- | --- |
| 직선 경계로는 반복해서 같은 사례가 섞여 보인다 | 표현 공간을 바꿔야 할 수 있기 때문 | polynomial 또는 RBF 검토 |
| 특징 간 조합이 중요해 보인다 | 단순 축별 분리보다 상호작용이 중요할 수 있기 때문 | polynomial 계열 감각 |
| 중앙/바깥쪽처럼 곡선 구조가 보인다 | 지역적 거리 구조가 더 중요할 수 있기 때문 | RBF 계열 감각 |
| 선형 SVM은 margin이 좋아도 대표 실패가 남는다 | 경계보다 표현 자체를 다시 봐야 할 수 있기 때문 | kernel 또는 명시적 특징 추가 |
| review 대상이 특정 곡선 구간에 계속 몰린다 | 현재 좌표계가 문제를 단순하게 드러내지 못할 수 있기 때문 | 표현 공간 재설계 검토 |

이 표의 목적은 kernel 이름을 외우게 하는 것이 아니라, `지금 답답한 것이 경계 문제인지 표현 문제인지`를 먼저 가르게 하는 데 있습니다.
...
`kernel`이라는 이름은 추상적으로 들리기 쉽습니다. 이 절에서는 수학적 엄밀성보다, 이 이름이 실제로 무엇을 가리키는지 먼저 잡습니다.

`kernel은 두 입력을 받아, 그 둘이 새 표현 공간에서 얼마나 가깝거나 비슷하게 읽히는지를 계산하는 중심 계산이다.`

즉, kernel이라는 말은 `모든 새 특징 전체`를 뜻하는 것이 아니라, 그 공간에서 비교를 수행하는 핵심 계산부를 가리키는 이름으로 읽으면 됩니다.

그래서 커널을 이해할 때 독자가 실제로 붙잡아야 하는 질문은 다음입니다.

- 이 커널은 두 샘플의 어떤 유사도를 강조하는가?
- 상호작용을 더 보게 하는가?
- 가까운 이웃의 지역 구조를 더 민감하게 보게 하는가?
- 그 결과 경계가 더 곡선적으로 유연해질 수 있는가?

이 질문을 붙잡으면 `kernel 이름 외우기`보다 `kernel이 어떤 비교 방식을 밀어 주는가`를 읽게 됩니다.

### 커널은 왜 직접 특징을 다 만들지 않아도 된다고 말하는가

이름의 감각을 잡고 나면, 다음 질문이 이어집니다.
...
`고차원 전체를 다 적지 않고도, 그 공간에서 비교한 효과를 흉내 내는 계산`으로 이해할 수 있습니다.
...
Module 4를 한 번에 다시 읽을 때는 알고리즘 이름보다 `같은 문제 장면에서 무엇을 먼저 비교하게 만드는가`를 묶어 보는 편이 좋습니다.

먼저 회귀 장면에서는 선형회귀가 출발점이 됩니다.

| 같은 장면 | 먼저 떠올릴 모델 | 먼저 보는 비교축 | 바로 남길 다음 질문 |
| --- | --- | --- | --- |
| 광고비, 계절, 방문 수로 매출 같은 연속값을 예측한다 | 선형회귀 | baseline보다 평균 오차가 실제로 줄었는가 | 직선이 놓친 큰 오차 구간은 어디인가 |

분류 장면에서는 같은 고객 이탈 문제를 두고 세 모델을 다시 비교할 수 있습니다.
...
이 비교의 핵심은 `누가 더 고급 모델인가`가 아닙니다. 같은 고객 이탈 장면에서도 로지스틱 회귀는 `점수와 정책`, k-NN은 `이웃과 거리`, 선형 SVM은 `경계의 여유`, 커널 SVM은 `표현 공간 변화`를 먼저 보게 만듭니다. 즉, Module 4의 목표는 알고리즘을 늘리는 것이 아니라 `문제 구조를 보는 질문`을 바꾸어 쥐는 데 있습니다.
