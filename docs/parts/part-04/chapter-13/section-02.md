...
## 커널의 입문적 의미: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "kernel이 표현 공간을 바꿔 경계를 찾는 직관이라는 점을 잡아야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- 선형 경계가 부족해 보여도 곧바로 선형 사고를 버리는 것이 아니라, `표현 공간을 바꾸면 선형 경계가 다시 의미를 가질 수 있는가`를 먼저 물어야 한다는 점을 설명할 수 있는가
- 커널이 새 공간의 유사도를 원래 공간 계산으로 대신 다루는 아이디어라는 점을 설명할 수 있는가
- polynomial은 상호작용 특징을, RBF는 지역적 유사도 구조를 더 강하게 의식하게 한다는 점을 설명할 수 있는가
