...
이 Part는 머신러닝을 처음 배우는 독자에게는 큰 구조를 잡아 주고, 오래전에 개론을 배웠던 독자에게는 흩어진 기억을 다시 표준 흐름으로 묶어 주기 위한 구간입니다.
...
Part 4는 머신러닝의 공통 구조를 설명하는 파트입니다. 따라서 여기서는 다음 내용을 본편 범위 안에서 설명합니다.
...
Part 4는 이런 오해를 줄입니다. 이 Part는 모델 카탈로그를 제공하는 것이 아니라, 머신러닝을 읽고 비교하고 의심할 수 있는 기본 문해력을 만드는 구간입니다.

## 앞선 Part와의 연결

Part 2에서 복구한 수학과 소프트웨어 도구는 Part 4에서 직접 쓰입니다. NumPy 배열(array)은 입력 `X`와 라벨 `y`를 담는 데 쓰이고, Pandas `DataFrame`은 데이터셋(dataset)을 읽는 데 쓰이며, 평균(mean), 분산(variance), 오차(error)는 평가 지표를 읽는 데 다시 등장합니다. 미분(derivative), 그래디언트(gradient), 손실(loss)은 후반부와 Part 5 딥러닝으로 이어지고, Matplotlib은 데이터 분포, 결정 경계, 손실 곡선, 군집 구조를 확인하는 데 쓰입니다.
...
| --- | --- |
| 배열과 표를 보고 `입력 X`, `라벨 y`, `shape`를 읽을 수 있다 | 어떤 열이 입력 특징이고 어떤 값이 목표값인지 어떻게 정하는가 |
| 평균, 오차, 그래프를 작은 예로 확인할 수 있다 | 점수가 좋아졌다는 말이 실제로 무엇을 뜻하는가 |
| Colab, 로컬 실행, Git 기록의 차이를 안다 | 같은 실험을 다시 실행하고 비교하려면 무엇을 남겨야 하는가 |

Part 3에서 넘어올 때는 `데이터를 모델에 바로 넣는 것`보다 먼저 `무엇이 한 건의 사례인가`를 고정해 두는 감각도 함께 가져와야 합니다.

| Part 3에서 넘겨받는 기준 | Part 4에서 바로 이어지는 질문 |
| --- | --- |
| 한 행(row)이 동작 1건, 사례 1건을 뜻하도록 정리했다 | 이 한 건에서 무엇을 입력 특징으로 쓰고 무엇을 목표값 후보로 둘 것인가 |
| 제어 기록, 센서 기록, 식별 열이 뒤섞이지 않게 구분했다 | 모델 입력에 직접 넣지 말아야 할 열은 무엇인가 |
| 요약 특징과 검토 후보 라벨을 따로 남겼다 | 지금 풀 문제는 분류인가 회귀인가, 라벨 정의는 충분히 일관적인가 |
| 기준선(baseline) 비교와 다음 점검 질문을 함께 남겼다 | 점수가 좋아 보여도 정말 의미 있는 개선인지 무엇과 비교할 것인가 |
