...
장학금 심사를 아주 단순화해 생각해 보겠습니다. 사람은 먼저 `시험 점수가 높다`, `결석이 많다`, `추가 과제를 냈다` 같은 조건을 함께 보고 전체 인상을 만듭니다. 하지만 이 조건들은 서로 방향이 다르고 강도도 다릅니다. 시험 점수와 추가 과제는 승인 쪽으로 밀어 주고, 결석 횟수는 반대 방향으로 작용할 수 있습니다. 선형 결합은 바로 이런 서로 다른 신호를 하나의 점수 축으로 모아, 지금 학생이 승인에 더 가까운지 보류에 더 가까운지 읽기 시작하게 만듭니다. 이 사례는 `여러 조건을 따로 기억하는 것`에서 `하나의 점수 축으로 압축해 비교하는 것`으로 판단 구조가 바뀐다는 점을 보여 줍니다.

### 사례 2. 임계값이 필요한 승인 판단

같은 점수라도 바로 출력으로 쓰지는 않을 수 있습니다. 예를 들어 입장 승인 시스템이 `0보다 크면 통과, 아니면 보류` 같은 기준을 둔다고 해 보겠습니다. 사람은 점수가 0.1이든 3.0이든 둘 다 `통과 쪽`으로 읽고, -0.2와 -5.0은 모두 `보류 쪽`으로 읽을 수 있습니다. 이때 활성화는 연속적인 점수를 실제 판단 규칙으로 바꾸는 단계입니다. 즉, 선형 결합이 `얼마나 승인 쪽인가`를 점수로 모은다면, 활성화는 그 점수를 바탕으로 `그래서 통과인가 아닌가`를 닫는 역할을 합니다. 이 사례는 퍼셉트론이 왜 `합산기`가 아니라 `합산 + 결정` 단위로 설명되는지 보여 줍니다.
