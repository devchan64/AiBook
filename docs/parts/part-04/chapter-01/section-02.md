...
- 퍼셉트론은 먼저 입력의 선형 결합(linear combination)으로 점수를 계산하고, 활성화(activation)는 그 점수를 실제 출력 규칙으로 바꾸는 단계입니다.
- 퍼셉트론 하나는 입력 공간을 하나의 선형 경계로 나누는 판단기로 볼 수 있으므로, XOR 같은 패턴에서는 한계를 보일 수 있습니다.
