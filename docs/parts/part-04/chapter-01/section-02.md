...
여기서는 “규칙”이라는 말보다 “사례를 보고 다음 사례를 판단하는 기준을 만든다”는 해석이 더 정확합니다. 여기서는 아직 알고리즘을 외우지 않습니다. 대신 사람이 직접 기준을 쓰는 방식과, 데이터 사례로부터 기준을 맞춰 보는 방식이 어떻게 다른지 봅니다.
...
따라서 여기서는 표현을 다음처럼 정리합니다. 사람이 직접 규칙을 쓰면 규칙 기반 접근이고, 데이터에서 입력과 출력의 관계를 추정하면 학습 기반 접근이며, 추정된 관계를 새 데이터에 적용하면 모델 실행(inference) 또는 예측(prediction)입니다.
