...
처음 읽을 때는 “규칙”이라는 말보다 “사례를 보고 다음 사례를 판단하는 기준을 만든다”는 정도로 받아들이는 편이 좋습니다. 여기서는 아직 알고리즘을 외우지 않습니다. 대신 사람이 직접 기준을 쓰는 방식과, 데이터 사례로부터 기준을 맞춰 보는 방식이 어떻게 다른지 봅니다.
...
머신러닝은 이런 사례들을 모아 입력과 출력의 관계를 맞춰 보는 방식입니다. 이때 입력은 과일의 색, 모양, 크기 같은 관찰값이고, 출력은 사과, 바나나, 오렌지 같은 라벨입니다. 이 작은 예시는 머신러닝을 “정답 규칙을 발견한다”보다 “사례에서 판단 기준을 조정한다”로 이해하는 데 도움을 줍니다.
...
입문 단계에서는 사례 데이터를 다음 세 칸으로 나누어 보면 좋습니다.
...
Part 3에서 넘어온 표를 떠올리면, 여기서도 `무엇을 바로 학습 입력으로 쓰고 무엇을 분리해야 하는가`를 한 번 더 가르는 편이 안전합니다.
...
초심자 기준에서는 이 세 단계를 아래 한 문장으로 다시 묶어 두면 도움이 됩니다. `무엇을 넣을지 정하고, 그 관계를 맞춰 보고, 처음 보는 사례에서 다시 확인한다.` 이 문장이 서야 `fit`, `predict`, `evaluation`이 API 이름이 아니라 작업 순서로 읽힙니다.
...
한 팀이 고객 문의를 `배송`, `환불`, `계정`, `기타`로 자동 분류하려 한다고 하겠습니다. 처음에는 제목에 특정 단어가 있으면 해당 부서로 보내는 규칙을 직접 만들 수 있습니다.
