...
- 좋은 학습 기반 접근도 결국 `표현`, `학습`, `처음 보는 사례 평가`의 순서를 모두 거쳐야 합니다.
- 모델 예측(prediction)과 서비스의 최종 행동(action)은 같은 단계가 아닐 수 있습니다.
...
- 규칙 기반 접근으로 충분한지, 학습 기반 접근이 필요한지 판단해야 할 때 이 절의 대비를 먼저 떠올립니다.
- `데이터에서 규칙을 배운다`는 표현이 너무 단순해 보여 입력과 출력의 관계 추정이라는 더 정확한 설명이 필요할 때 이 절로 돌아옵니다.
- 모델의 예측과 서비스의 최종 행동, 정책, 사람 검토가 서로 다른 단계라는 점을 다시 구분할 때 이 절이 기준이 됩니다.
