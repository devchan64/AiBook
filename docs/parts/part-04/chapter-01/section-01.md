...
- Part 4에서는 모델 이름보다 데이터 구조, 학습 절차, 평가 기준을 먼저 봅니다.
- Part 2의 수식, Python, NumPy, Pandas, Matplotlib 감각은 Part 4에서 계속 다시 사용됩니다.
...
- LLM이나 생성형 AI 경험만으로 AI 전체를 바로 설명하고 있을 때 AI, 머신러닝, 딥러닝의 포함 관계를 먼저 떠올립니다.
- 서비스 안에 검색, 규칙, 정책, 사람 검토, 모델이 함께 있을 수 있다는 구조를 다시 설명해야 할 때 이 절로 돌아옵니다.
- Part 4를 모델 이름 암기보다 데이터, 학습, 평가 흐름으로 읽어야 한다는 기준을 다시 잡을 때 이 절이 기준이 됩니다.
