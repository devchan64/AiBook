...
## AI, 머신러닝, 딥러닝의 관계에서 닫을 질문

이 절은 먼저 `AI라는 큰 범위 안에서 머신러닝, 딥러닝, 생성형 AI, LLM이 어디에 놓이는가`를 닫습니다. Part 4의 공통 학습 구조는 이어지는 절에서 붙잡고, 신경망과 Transformer의 본격 구조는 뒤 Part에서 다시 연결합니다.

- AI와 머신러닝은 같은 말인가?
- 머신러닝과 딥러닝은 어떻게 다른가?
- 생성형 AI와 LLM은 AI 전체를 대표하는 말인가?
- Part 4에서 왜 딥러닝보다 전통적인 머신러닝을 먼저 보는가?
- Part 2에서 배운 수식과 Python 도구는 여기서 어떻게 다시 등장하는가?

## AI, 머신러닝, 딥러닝의 관계에서 남길 판단 기준
...
- Part 4의 학습 주제가 데이터, 모델, 학습, 평가의 흐름이라는 점을 이해할 수 있습니다.

## 읽기 순서

용어는 포함 관계와 역할 순서로 보면 더 분명해집니다.

1. AI는 가장 넓은 말입니다.
2. 머신러닝은 그 안에서 데이터로부터 관계를 배우는 접근입니다.
3. 딥러닝은 머신러닝 안에서 신경망을 깊게 쌓아 쓰는 흐름입니다.
4. 생성형 AI와 LLM은 현대 AI 경험에서 매우 중요하지만, AI 전체와 같은 말은 아닙니다.
5. Part 4에서는 이 모든 흐름의 바닥에 있는 데이터, 모델, 학습, 평가를 먼저 봅니다.

이 순서는 학술 분류를 완벽하게 외우기 위한 순서가 아닙니다. 이후 문서를 읽을 때 “지금 내가 보고 있는 내용이 데이터 문제인지, 모델 문제인지, 서비스 문제인지”를 구분하기 위한 최소 지도입니다.

## 한 문장으로 먼저 구분하기

용어가 많아지면 포함 관계와 함께 각 말이 답하려는 질문을 먼저 보는 쪽이 더 분명합니다.
...
이 구분은 시험용 정의가 아니라 독서용 지도입니다. 같은 단어라도 논문, 제품 문서, 언론 기사, 서비스 홍보 문구에서는 범위가 조금씩 다르게 쓰일 수 있습니다. 그래서 Part 4에서는 용어 자체보다 데이터, 모델, 학습, 평가라는 반복되는 구조를 중심에 둡니다.
...
따라서 Part 4에서 필요한 관점은 “어느 기술이 더 최신인가”가 아닙니다. 다음처럼 묻는 관점입니다.
...
이 질문은 Part 1에서 본 AI의 넓은 지도와 Part 2에서 본 데이터 표현을 Part 4의 머신러닝 학습으로 연결합니다.

## Part 4에서 머신러닝을 따로 보는 이유

Part 4은 딥러닝이나 LLM으로 바로 가지 않습니다. 먼저 머신러닝의 기본 흐름을 봅니다.
...
작은 머신러닝 예제에서는 “데이터를 준비한다”, “모델을 학습한다”, “새 데이터에 적용한다”, “맞았는지 확인한다”는 흐름이 눈에 보입니다. 이 흐름이 보이면 나중에 딥러닝이나 LLM을 볼 때도 모델 이름보다 먼저 구조를 묻게 됩니다.

## Part 4의 최소 단위

머신러닝을 다시 볼 때는 알고리즘 이름을 많이 외우는 것보다 반복되는 최소 단위를 먼저 고정해야 합니다.
...
이 표는 하나의 질문 묶음을 보여 줍니다. “무엇을 풀려는가”, “무엇을 입력으로 주는가”, “무엇을 맞추려 하는가”, “맞았는지 어떻게 확인하는가”가 머신러닝 문서를 읽을 때 가장 먼저 확인할 질문입니다.
...
이 네 관점은 서로 완전히 대체 관계가 아닙니다. 실제 서비스에서는 규칙 기반 필터와 머신러닝 모델, LLM 기반 검토가 함께 쓰일 수 있습니다. 따라서 “최신 방식이 이전 방식을 모두 없앤다”보다 “문제와 제약에 따라 여러 방식을 조합한다”로 이해해야 합니다.

실제로 글을 읽거나 회의를 할 때는 다음처럼 `지금 어떤 말을 쓰는 것이 가장 정확한가`를 먼저 판정하면 혼동을 줄일 수 있습니다.

| 지금 설명하려는 대상 | 가장 먼저 쓸 말 | 이유 |
| --- | --- | --- |
| 서비스 전체의 지능적 기능 범위 | AI | 검색, 규칙, 모델, 사람 검토까지 함께 포함할 수 있기 때문입니다. |
| 데이터에서 관계를 배워 예측이나 분류를 하는 부분 | 머신러닝(machine learning) | 규칙 작성이 아니라 학습된 모델이 핵심이기 때문입니다. |
| 신경망 기반 표현 학습과 대규모 모델 구조 | 딥러닝(deep learning) | 머신러닝 안에서도 신경망 구조가 중심이기 때문입니다. |
| 텍스트, 이미지, 코드 같은 새 출력을 만드는 경험이나 서비스 | 생성형 AI(generative AI) | 분류보다 생성 결과가 사용자 경험의 중심이기 때문입니다. |
| 언어 입력과 출력을 다루는 대규모 모델 | LLM(large language model) | 생성형 AI 안에서도 언어 모델을 정확히 가리키기 때문입니다. |

## Part 2의 언어가 다시 등장하는 자리

Part 2에서 배운 표현은 Part 4에서 곧바로 다시 나옵니다.

| Part 2에서 본 표현 | Part 4에서 다시 나타나는 방식 |
| --- | --- |
...
Part 4를 시작할 때 가장 먼저 피해야 할 오해는 다음입니다.
...
## AI, 머신러닝, 딥러닝의 관계: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "AI, 머신러닝, 딥러닝의 포함 관계를 다시 잡아야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
- 표, 코드, 도식, 체크리스트 중 어떤 장치가 이 판단을 다시 검토하게 만드는가?

### 사례 1. 챗봇을 써 봤다고 해서 머신러닝 전체를 안 것은 아닌 이유

한 학습자가 AI를 처음 다시 공부하려는데, 이미 챗봇과 이미지 생성 도구를 자주 써 봤다고 하겠습니다. 그래서 `AI는 결국 LLM이나 생성형 AI 아닌가`라고 생각할 수 있습니다.

많은 서비스 경험이 LLM 중심으로 들어오기 때문에 이런 생각이 쉽게 생깁니다. 하지만 서비스 안쪽을 보면 검색, 규칙 기반 필터, 추천 모델, 정책 판단, 사람 검토가 함께 작동할 수 있고, LLM은 그중 하나일 수 있습니다.
...
- 어떤 문맥에서 `AI`, `머신러닝`, `딥러닝`, `생성형 AI`, `LLM` 중 어느 말을 쓰는 편이 가장 정확한지 구분할 수 있는가
- 왜 `AI = LLM`이나 `머신러닝 = 딥러닝`처럼 바로 같은 말로 두면 서비스 구조 설명이 약해지는지 말할 수 있는가
- Part 4가 모델 이름보다 `데이터, 학습, 평가`의 반복 구조를 먼저 보는 이유를 설명할 수 있는가
- AI, 머신러닝, 딥러닝, 생성형 AI, LLM의 포함 관계를 서로 다른 층위로 설명할 수 있는가
- Part 2의 수식, Python, 배열, 표, 그래프 감각이 Part 4의 데이터·학습·평가 흐름에서 어떻게 다시 쓰이는지 설명할 수 있는가
- 서비스 안에서 모델, 규칙, 검색, 정책, 사람 검토가 함께 작동할 수 있다는 점을 사례로 설명할 수 있는가
