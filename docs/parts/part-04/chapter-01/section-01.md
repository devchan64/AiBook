...
이 절의 목적은 AI, 머신러닝, 딥러닝(deep learning), 생성형 AI(generative AI), LLM(large language model)을 한 줄로 암기하는 것이 아닙니다. 각 용어가 어느 범위의 말을 가리키는지 구분하고, Part 4에서 왜 전통적인 머신러닝부터 다시 보는지 이해하는 것입니다.

처음 읽을 때는 용어를 모두 외우려 하지 않아도 됩니다. 여기서는 “AI라는 큰 분야 안에서 머신러닝이 어떤 위치에 있는가”와 “왜 바로 LLM이나 딥러닝으로 가지 않고 데이터, 모델, 학습, 평가를 먼저 보는가”만 잡아도 흐름을 따라갈 수 있습니다.
...
이 절의 용어는 한 번에 모두 이해하려고 하면 어렵습니다. 다음 순서로 읽으면 부담이 줄어듭니다.
...
독자에게 중요한 이유도 여기에 있습니다. 작은 머신러닝 예제에서는 “데이터를 준비한다”, “모델을 학습한다”, “새 데이터에 적용한다”, “맞았는지 확인한다”는 흐름이 눈에 보입니다. 이 흐름이 보이면 나중에 딥러닝이나 LLM을 볼 때도 모델 이름보다 먼저 구조를 묻게 됩니다.
...
처음에는 이 표를 용어 사전처럼 외우기보다 하나의 질문 묶음으로 읽으면 됩니다. “무엇을 풀려는가”, “무엇을 입력으로 주는가”, “무엇을 맞추려 하는가”, “맞았는지 어떻게 확인하는가”가 머신러닝 문서를 읽을 때 가장 먼저 확인할 질문입니다.
...
Part 4을 시작할 때 가장 조심할 오해는 다음입니다.
...
사람이 먼저 떠올리는 기준으로는 자연스럽습니다. 실제로 많은 서비스 경험이 LLM 중심으로 들어오기 때문입니다. 하지만 서비스 안쪽을 보면 검색, 규칙 기반 필터, 추천 모델, 정책 판단, 사람 검토가 함께 작동할 수 있고, LLM은 그중 하나일 수 있습니다.
