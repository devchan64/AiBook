...
여기에 한 가지를 더 붙이면 결정트리 절이 지금까지 정리한 비교 기록 구조와 직접 이어집니다. 결정트리를 후보로 올릴 때는 `첫 분기가 무엇인가`만 남기는 것이 아니라, `어떤 사례가 분기 근처에 남는가`, `baseline이나 다른 후보보다 무엇이 더 읽기 쉬운가`, `다음에 어떤 분기 질문을 더 검토할 것인가`를 함께 적어 둡니다. 같은 점수처럼 보여도 어떤 트리는 특정 leaf 안에 다른 class가 더 많이 섞여 있을 수 있으므로, 분기 뒤에 남는 패턴 차이도 따로 읽습니다.
...
이 표의 핵심은 결정트리를 `읽기 쉬운 모델`이라고만 두지 않고, `질문 흐름이 실제 설명 단위가 되는 문제에서 먼저 써 볼 후보`로 위치시키는 데 있습니다.
...
여기서 `impurity`는 node 안이 얼마나 섞여 있는지를 보는 말입니다. API 문서 기준으로는 분류 트리에서 `criterion`으로 `gini`, `entropy`, `log_loss` 같은 기준을 둘 수 있습니다. 아직 수식을 길게 외우기보다, `한 node 안에 class가 섞여 있으면 impurity가 높고, 한쪽 class로 정리되면 impurity가 낮다`는 감각을 먼저 잡으면 됩니다.
...
즉, 결정트리는 `사람이 직감으로 질문을 쓰는 모델`이 아니라, `데이터를 더 정리해 주는 질문을 찾아 누적하는 모델`입니다.

여기서 한 걸음 더 나가면, `좋은 첫 질문`은 데이터가 조금만 바뀌어도 달라질 수 있다는 점도 함께 봐야 합니다. 예를 들어 고객 `F`의 label이 `stay`가 아니라 `churn`이었다면 `late_payment` 쪽 질문이 더 강한 후보처럼 보일 수 있습니다. 즉 첫 split은 절대적인 규칙이 아니라, 현재 학습 데이터가 어떤 묶음을 만들고 있는지에 기대는 선택입니다.
...
- 확인할 개념:
  ...
  - 결정트리의 질문 흐름은 `현재 데이터가 더 잘 정리되는 방향`에 따라 달라진다.
...
이 비교에서 봐야 할 핵심은 `처음에는 visits가 더 좋아 보였지만, 데이터 한 건의 label을 바꾸자 late_payment가 첫 질문으로 올라왔다`는 점입니다. 이 결과는 결정트리를 불안정한 모델이라고 단정하자는 뜻이 아니라, 분기 구조를 읽을 때 `현재 데이터가 어떤 사례를 어디에 두고 있는가`를 함께 확인해야 한다는 뜻입니다.
...
| 구조를 흔든 사례 | `F`는 첫 split을 바꾸지 않음 | `F`의 label 변경이 첫 split을 바꿈 |
| 먼저 다시 볼 것 | `visits` 기준에서 `C`가 왜 함께 묶였는가 | `late_payment` 기준에서 `F`가 왜 구조를 밀었는가 |

이 표의 목적은 `무엇이 달라졌는가`를 요약하는 데서 끝나지 않습니다. 초심자 기준에서는 `어떤 사례가 질문 선택을 실제로 바꾸었는가`를 이렇게 한 번 적어 두어야, 다음 절에서 깊이와 leaf 크기가 구조를 왜 더 흔들 수 있는지도 자연스럽게 이어집니다.

초심자 기준에서는 이 두 실행을 나란히 두고 아래 질문에 답해 보는 것이 좋습니다.

1. 어떤 사례 하나가 첫 split 선택을 바꿨는가
2. split 점수 차이가 작을 때는 어떤 질문이 더 읽기 쉬운지 사람 해석도 함께 봐야 하는가
3. 이 민감도가 다음 절의 깊이, leaf 크기, 과적합 위험과 어떻게 이어질 것 같은가

가능하면 여기서 한 줄 더 적어 봅니다. `현재 데이터에서는 visits가 첫 질문이었지만, F의 label 하나가 바뀌자 late_payment가 더 좋은 첫 질문이 되었다.` 이런 문장 하나를 직접 써 보면, 결정트리가 `고정 규칙 암기`가 아니라 `현재 데이터가 더 잘 정리되는 질문 선택`이라는 점이 더 분명해집니다.
...
1. 원래 데이터에서 첫 split이 무엇이었는지 적습니다.
2. label 하나를 바꾼 뒤 첫 split이 무엇으로 바뀌었는지 적습니다.
3. 두 경우 모두에서 `가장 애매한 사례`를 1건씩 고릅니다.
4. 마지막으로 `이 트리를 바로 깊게 키우고 싶은가, 아니면 다음 절에서 깊이 제한을 먼저 보고 싶은가`를 한 문장으로 적습니다.
...
| 다음 질문 | `late_payment`를 다음 분기로 둘 것인가 | 깊이를 더 늘리기 전에 leaf를 먼저 점검할 것인가 |

이 연습의 핵심은 정답을 외우는 것이 아니라, `결정트리를 점수 하나`가 아니라 `질문 구조와 사례 묶음`으로 읽는 습관을 만드는 데 있습니다.
...
이 절의 예제와 연습은 Part 4의 공통 목표를 네 가지 방식으로 다시 보여 줍니다.

- 문제 정의: 지금 풀고 있는 것은 `좋은 첫 질문을 고르는 분류 문제`입니다.
- 입력 표현: `visits`, `late_payment` 같은 feature가 실제로 질문 후보가 됩니다.
- 평가 읽기: weighted gini가 낮아질수록 label이 더 정리되는 방향으로 split이 선택됩니다.
- 다음 질문: split 하나로 끝내지 말고, 어떤 사례가 구조를 흔드는지와 깊이를 더 늘릴지까지 이어서 판단해야 합니다.

즉 결정트리 입문에서 정말 배워야 하는 것은 `트리는 조건문처럼 읽힌다`는 사실 하나가 아니라, `질문 후보를 비교하고, 사례 묶음을 읽고, 다음 분기와 다음 제한을 함께 생각하는 흐름`입니다.

| 보인 구조 | 해석 경계 | 다음 질문 |
| --- | --- | --- |
| 첫 split은 데이터를 더 정리하는 질문 후보 비교에서 나온다 | 현재 데이터가 바뀌면 첫 split도 달라질 수 있다 | 깊이 제한과 최소 샘플 수를 주면 구조가 어떻게 바뀌는가 |
| leaf는 분류 결과를 내지만 동시에 사례 묶음도 만든다 | 같은 점수라도 leaf 안의 섞임 정도는 다를 수 있다 | leaf별 대표 사례를 따로 남기면 무엇이 더 보이는가 |
| 트리는 사람이 따라 읽기 쉬운 분기 흐름을 만든다 | 읽기 쉬움이 곧 일반화 보장을 뜻하지는 않는다 | 다음 절에서 과적합과 pruning을 어떻게 연결할 것인가 |
...
- 선을 그리기보다 질문을 순서대로 나누는 설명이 더 자연스러운 데이터일 때, 결정트리의 질문 흐름 관점을 먼저 떠올립니다.
- 어떤 `feature + threshold`가 실제로 묶음을 잘 갈라 주는지 설명해야 할 때, split을 질문 후보로 읽는 방식으로 다시 봅니다.
- leaf가 분류 결과인지 수치 예측인지 섞이기 시작할 때, 트리가 분류와 회귀 모두에 쓰일 수 있다는 점을 함께 떠올립니다.
