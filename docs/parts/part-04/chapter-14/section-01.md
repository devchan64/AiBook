...
즉, 여기서는 `블록 내부 역할`을 먼저 붙잡고, `학습 규모와 계산 효율`은 바로 다음 P4-14.2로 넘기는 편이 가장 안전합니다.
...
즉, 여기서 독자가 먼저 봐야 하는 차이는 `부품 이름이 무엇인가`보다 `같은 토큰 표현이 관계 읽기, 위치별 가공, 안정적 전달을 차례로 거치는가`에 더 가깝습니다.
