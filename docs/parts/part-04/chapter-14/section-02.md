...
문장 앞부분의 주어, 부정 표현, 시제 단서가 뒤쪽 번역에 영향을 줄 때, 사람은 가까운 단어부터 순서대로 옮기면 충분하다고 느끼기 쉽습니다. 하지만 실제로는 앞의 조건을 뒤늦게 다시 확인해야 할 수 있고, 그 과정이 길어질수록 순차 전달만으로는 부담이 커집니다. Transformer는 이런 장면에서 먼 위치를 더 직접 참고하는 감각을 줍니다.

### 사례 2. 긴 문서 요약

문서 초반 핵심 문장이 뒤쪽 요약 문장 생성에 중요할 수 있습니다. 사람은 뒤 문장을 쓰기 시작하면 이미 읽은 앞부분은 대체로 유지될 것이라고 느끼기 쉽지만, 실제로는 앞의 핵심 결론을 다시 확인해야 하는 경우가 많습니다. self-attention은 이런 전역 참조(global reference) 직관과 잘 맞습니다.

### 사례 3. 코드 생성과 분석

코드에서는 먼 앞쪽의 함수 정의나 변수 선언이 뒤쪽 토큰 해석에 영향을 줄 수 있습니다. 사람은 바로 앞 몇 줄만 보면 충분하다고 느끼기 쉽지만, 실제로는 아래 줄을 읽다가 위에서 선언한 이름과 타입을 다시 확인해야 하는 경우가 많습니다. 예를 들어 파일 아래쪽에서 `timeout` 값을 넘길 때, 위에서 그것이 초 단위인지 밀리초 단위인지 선언을 다시 확인하지 않으면 해석이 틀어질 수 있습니다. 긴 문맥 처리 능력은 여기서도 중요합니다. 이 사례는 자연어뿐 아니라 코드에서도 `먼 앞쪽 단서를 다시 직접 참고하는 구조`가 왜 중요한지 보여 줍니다.
