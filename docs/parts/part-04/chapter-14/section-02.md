...
그리고 한 그루 안에서 깊이 제한, leaf 크기, pruning을 모두 조정해 봐도 흔들림이 계속 남는다면, 다음 질문은 자연스럽게 `한 그루를 더 다듬을 것인가`에서 `여러 트리의 합의로 흔들림을 줄일 것인가`로 이동합니다. 이 연결이 다음 절 P4-15.1의 랜덤포레스트로 이어집니다.
...
- train 성능은 계속 오르는데 test가 따라오지 않을 때, 트리가 예외를 외우고 있는지 먼저 떠올립니다.
- 깊이 제한, leaf 크기, pruning 중 무엇을 조정해야 할지 헷갈릴 때, 각각이 복잡도를 줄이는 방식 차이를 다시 봅니다.
- 랜덤포레스트나 부스팅으로 넘어가기 전에 단일 트리의 복잡도 감각을 정리해야 할 때, 이 절을 기준선으로 삼습니다.
