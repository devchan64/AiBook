...
문장 앞부분의 주어, 부정 표현, 시제 단서가 뒤쪽 번역에 영향을 줄 때, 사람은 가까운 단어부터 순서대로 옮기면 충분하다고 느끼기 쉽습니다. 하지만 실제로는 앞의 조건을 뒤늦게 다시 확인해야 할 수 있고, 그 과정이 길어질수록 순차 전달만으로는 부담이 커집니다. 예를 들어 앞부분의 `not`이나 부정 접두를 놓치면, 뒤 문장은 문법은 맞아도 뜻이 반대로 번역될 수 있습니다. Transformer는 이런 장면에서 먼 위치를 더 직접 참고하는 감각을 줍니다. 그래서 이 사례는 긴 문장에서 중요한 앞 단서 하나를 놓치면 결과 뜻 전체가 바뀔 수 있다는 점을 보여 줍니다.

### 사례 2. 긴 문서 요약

문서 초반 핵심 문장이 뒤쪽 요약 문장 생성에 중요할 수 있습니다. 사람은 뒤 문장을 쓰기 시작하면 이미 읽은 앞부분은 대체로 유지될 것이라고 느끼기 쉽지만, 실제로는 앞의 핵심 결론을 다시 확인해야 하는 경우가 많습니다. 예를 들어 서론에 `이번 분기 매출은 늘었지만 이익은 감소했다`가 적혀 있는데 뒤 요약에서 `성과가 좋아졌다`만 남기면 핵심 판단이 왜곡될 수 있습니다. self-attention은 이런 전역 참조(global reference) 직관과 잘 맞습니다. 그래서 이 사례는 문서 초반의 핵심 문장을 다시 집어 오지 못하면 요약이 그럴듯해도 판단 방향이 틀어질 수 있다는 점을 보여 줍니다.
