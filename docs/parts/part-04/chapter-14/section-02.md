...
P4-14.1이 `Transformer 블록 안에 무엇이 들어 있나`를 설명하는 절이었다면, 이 절은 `그 블록 구조가 실제 계산 방식과 학습 규모에서 무엇을 바꾸었나`를 설명하는 절입니다.
...
즉, 이 출력은 순차 상태만으로는 앞 규칙의 핵심 단서가 빠르게 약해지지만, 직접 재참조는 같은 요청 시점에 필요한 규칙 줄을 다시 끌어와 판단을 더 안정적으로 유지한다는 점을 보여 줍니다. 이 차이는 `decay`, 중간 로그 수, 요청 표현을 바꿔 보며 바로 다시 확인할 수 있습니다.
...
- Transformer는 토큰을 순차 상태로만 전달하지 않고 관계를 더 병렬적으로 계산하며, 이 구조는 GPU 병렬 처리와 잘 맞습니다.
- self-attention이 먼 위치를 더 직접 참조하는 감각을 주고, 이 차이가 대규모 생성 모델 확산의 핵심 기반이 됩니다.
