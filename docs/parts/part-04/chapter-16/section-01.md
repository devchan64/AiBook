...
이 절은 입문적으로 `부스팅이 무엇을 하려는 방식인가`를 이해하는 데 초점을 둡니다. 성능과 위험, early stopping과 shrinkage의 역할은 P4-16.2에서 이어서 다루고, 하이퍼파라미터와 검증 비용의 더 넓은 관점은 P4-9.1, P4-9.2에서 다시 연결합니다. XGBoost, LightGBM, CatBoost 같은 구현 차이는 이 책의 현재 본편 범위 밖에 둡니다.
...
그래디언트 부스팅은 `더 안정적인 평균`보다 `남은 오차를 더 세밀하게 줄이는 후보`가 필요할 때 강하게 떠오릅니다.
...
- weak learner는 작은 보정을 담당하는 작은 트리로 이해합니다.
- `learning_rate`와 `n_estimators`는 함께 읽어야 합니다.
- 랜덤포레스트가 병렬적 집계라면, 그래디언트 부스팅은 순차적 보정입니다.

이 절의 핵심은 부스팅 이름을 외우는 일이 아니라, 오차를 어떻게 다음 단계로 넘기는지 읽는 데 있습니다. 중요한 것은 점수 한 칸보다 `어떤 오류 패턴이 줄고 어떤 오류 패턴이 계속 남는가`를 단계별로 읽는 일입니다.

| 같이 봐야 할 것 | 이 절에서 먼저 읽는 질문 | 바로 다음에 이어질 곳 |
| --- | --- | --- |
| residual과 단계별 보정 | 현재 모델이 아직 설명하지 못한 부분이 무엇인가 | P4-16.2 성능과 과적합 위험 |
| `learning_rate`와 `n_estimators` 조합 | 한 번에 얼마나 고치고 몇 단계까지 누적할 것인가 | 튜닝과 early stopping 논의 |
| 랜덤포레스트와의 대비 | 병렬 집계보다 순차 보정이 어떤 장점과 위험을 가지는가 | 뒤의 앙상블 비교와 Part 4 최적화 감각 |
| 남는 오류 사례 | 여러 단계 뒤에도 계속 남는 경계 사례는 무엇인가 | early stopping, 규제, 데이터 표현 재검토 |
...
- 단순 평균보다 남은 오류를 단계적으로 줄이는 접근이 더 필요해 보일 때, 순차 보정으로서의 부스팅을 먼저 떠올립니다.
- residual이 무엇인지 흐려질 때, 현재 모델이 아직 설명하지 못한 남은 부분이라는 정의로 다시 돌아갑니다.
- 랜덤포레스트와의 차이를 정리해야 할 때, 병렬 집계와 순차 보정의 대비를 기준으로 다시 설명합니다.
