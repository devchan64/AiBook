...
이 절은 입문적으로 `강한 성능 후보를 다루는 태도`를 만드는 데 집중합니다. 교차검증과 튜닝 절차는 P4-9.2에서 다시 연결하고, 구현별 시스템 최적화나 대규모 계산 구조는 뒤의 신경망·시스템 Part에서 다시 보게 됩니다. XGBoost, LightGBM, CatBoost의 구현 차이와 histogram binning, distributed training의 세부 구조는 이 책의 현재 본편 범위 밖에 둡니다.
...
이 연습은 Part 4의 목표인 `문제-학습-평가-적용 판단` 흐름을 부스팅 실습 안에서 다시 묶어 줍니다. 문제는 `오차를 얼마나 더 줄일 수 있는가`이지만, 평가는 `residual 감소`와 `과한 보정 신호`를 함께 봐야 하고, 적용 판단은 `어느 단계에서 멈출 것인가`로 이어집니다. 즉, 부스팅의 높은 점수를 체감하는 것과 Part 4 목표를 체감하는 것은 다르며, 목표는 `남은 오류가 줄어드는 장면`과 `멈춤 기준`을 동시에 읽을 때 비로소 살아납니다.
...
| 공통 기록 언어 | Module 5 비교에서 바로 남길 내용 |
| --- | --- |
| 보인 구조 | 단일 트리, 숲, 순차 보정은 같은 표 데이터도 서로 다른 점검 신호로 읽게 만든다 |
| 해석 경계 | 더 높은 점수 후보가 언제나 더 안전하거나 더 해석 가능한 선택을 뜻하지는 않는다 |
| 다음 질문 | 지금 필요한 것이 깊이 제어인지, OOB 점검인지, early stopping인지부터 먼저 가를 것인가 |
...
이 대목에서는 세부 구현보다 다음 기준을 잡는 편이 중요합니다.
