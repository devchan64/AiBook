...
| --- | --- | --- |
| 성능 변화 곡선 | 단계가 늘수록 train과 validation/test가 어떻게 벌어지는가 | early stopping과 튜닝 기준 |
| 과적합 신호 | 어느 지점부터 남은 오차보다 잡음을 더 따라가기 시작하는가 | learning_rate, depth, subsample 재조정 |
| 중단 판단 | 지금 멈추는 편이 나은지 더 가도 되는지 무엇으로 정할 것인가 | Part 6 실험 회고 기록 |
...
- 부스팅 성능이 좋아 보이지만 과적합 위험도 같이 커질 때, 제어 장치 세트를 함께 떠올립니다.
- `learning_rate`, `n_estimators`, 트리 크기를 따로따로 조정하고 있을 때, 세 손잡이를 함께 읽는 관점으로 다시 묶습니다.
- 언제 멈춰야 할지 애매할 때, early stopping과 validation 흐름을 기준으로 중단 판단을 다시 세웁니다.
