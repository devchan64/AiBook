...
## 학습 데이터와 평가 데이터: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "학습 데이터와 평가 데이터를 왜 분리해야 하는지 분명히 해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- 학습 데이터는 모델이 배우는 데 쓰고, 평가 데이터는 모델이 배운 결과를 확인하기 위해 남겨 둔다는 점을 설명할 수 있는가
- 평가 데이터가 새 데이터에서의 동작을 추정하기 위한 대리 장면이라는 점을 설명할 수 있는가
- 데이터가 작으면 평가 점수도 흔들릴 수 있으므로 더 조심해야 한다는 점을 설명할 수 있는가
