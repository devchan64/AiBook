...
학습 데이터(training data)는 모델이 배우는 데 쓰는 데이터입니다. 평가 데이터(evaluation data)는 모델이 배운 결과를 확인하는 데 쓰는 데이터입니다. 독자에게 가장 중요한 첫 관점은 간단합니다. “배운 문제지로만 시험을 보면 실력을 착각할 수 있다”는 것입니다.

## 이 절의 범위

이 절은 데이터를 왜 나누는지 설명합니다. 검증 데이터(validation data)와 테스트 데이터(test data)의 자세한 구분은 P4-4.2에서 다룹니다. 여기서는 먼저 학습에 쓰는 데이터와 평가에 남겨 두는 데이터의 차이를 잡습니다.

과적합(overfitting)과 일반화(generalization)는 P4-5에서 자세히 다룹니다. 정확도(accuracy), 정밀도(precision), 재현율(recall) 같은 평가 지표(metric)는 P4-6에서 다룹니다. 여기서는 “나누어 확인해야 한다”는 이유에 집중합니다.
...
그래서 데이터 분리는 필요한 출발점이지만 충분한 전부는 아닙니다. 시간 변화, 표본 편향(sampling bias), 데이터 수집 방식, 서비스 정책 변화도 함께 봐야 합니다. 표본과 편향의 기본 감각은 Part 2의 확률·통계에서 봤고, 머신러닝에서는 P4-5와 P4-6에서 다시 연결합니다.
...
이런 문제 때문에 교차검증(cross-validation) 같은 방법을 사용하기도 합니다. 다만 이 절에서는 교차검증의 절차를 설명하지 않습니다. 지금은 “데이터가 작을수록 평가 점수도 불안정할 수 있다”는 점만 기억하면 됩니다. 교차검증은 P4-4.2와 P4-8에서 필요한 만큼 다시 만납니다.
...
- 검증 데이터와 테스트 데이터의 세부 구분은 P4-4.2에서 다룰 내용임을 구분할 수 있는가?
