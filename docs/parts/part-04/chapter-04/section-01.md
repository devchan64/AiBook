...
구체적인 손실 함수 종류와 문제별 차이는 P4-4.2에서 이어서 다룹니다.
...
초심자 기준으로는 이 장면을 다음처럼 바꾸어 말할 수 있습니다.
...
Part 3에서 우리는 accuracy, precision, recall, F1, RMSE 같은 평가 지표(metric)를 보았습니다. 초심자는 여기서 손실 함수와 평가 지표를 같은 것으로 이해하기 쉽습니다.
...
이 경우 오차(error)는 0.2억입니다. 손실 함수는 이 오차를 학습에 쓰기 좋은 숫자로 바꿉니다.
...
이 경우도 단순히 틀렸다고 끝나는 것이 아니라, `얼마나 자신 있게 틀렸는가`까지 반영하는 손실 함수가 필요해집니다.
...
초심자는 손실 숫자가 작아지면 모든 것이 해결된 것처럼 느끼기 쉽습니다. 하지만 다음을 함께 기억해야 합니다.
