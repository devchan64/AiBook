...
여기서는 예측 1이 훨씬 덜 틀렸습니다. 사람이 보기에도 `4.9억은 꽤 가깝고 3.5억은 많이 벗어났다`고 느끼기 쉽습니다. 즉, 손실은 보통 예측 1에 더 작은 숫자를 주고, 예측 2에는 더 큰 경고 신호를 주는 편이 자연스럽습니다.
예를 들어 가격 추천 서비스에서 이런 차이는 `조금 보정하면 되는 모델`과 `전반적으로 다시 봐야 하는 모델`을 가르는 수준의 차이일 수 있습니다.
...
둘 다 정답을 맞혔지만, 학습 관점에서는 예측 2가 훨씬 더 낫다고 읽는 편이 자연스럽습니다. 고객센터 자동 분류처럼 잘못 보내면 후속 처리 비용이 큰 문제에서는, `맞히긴 했지만 거의 헷갈린 상태`와 `확신 있게 맞힌 상태`를 같은 것으로 보면 다음 학습 방향이 흐려집니다.
예를 들어 자동 처리 임계치를 둘 때 0.55는 사람 검토로 보내고 0.95는 바로 확정하는 식의 차이가 생길 수 있습니다.

즉, 분류 손실은 단순 정오표보다 확률 분포를 더 세밀하게 봐야 합니다.
...
이 경우도 생성 손실은 예측 2를 더 좋은 상태로 읽는 편이 자연스럽습니다. 사람이 보기에는 둘 다 `apple`을 1위로 올렸으니 비슷해 보일 수 있지만, 실제 생성에서는 정답 토큰에 얼마나 확신을 줬는지가 다음 업데이트 크기를 바꿉니다. 예를 들어 요약 문장이나 코드 토큰을 만들 때 이런 차이가 계속 쌓이면, 문장 전체 품질 차이로 이어질 수 있습니다. 즉, 생성 손실은 `맞혔는가`보다 `정답 토큰을 얼마나 강하게 밀어 주었는가`를 더 세밀하게 읽습니다.
이 사례는 생성이 단순 정오판정보다 `후보들 사이의 분포 모양`을 더 중요하게 본다는 점을 잘 보여 줍니다.
