...
이 절의 목적은 공식을 많이 보여 주는 것이 아니라, `왜 문제마다 손실이 달라지는가`를 설명하는 것입니다.
...
여기서는 예측 1이 훨씬 덜 틀렸습니다. 즉, 손실은 보통 예측 1에 더 작은 숫자를 줘야 자연스럽습니다.
...
둘 다 정답을 맞혔지만, 학습 관점에서는 예측 2가 훨씬 더 낫다고 읽는 편이 자연스럽습니다.
...
이 경우도 생성 손실은 예측 2를 더 좋은 상태로 읽는 편이 자연스럽습니다.
...
이 연결을 초심자가 알면, Part 5에서 토큰(token), 다음 토큰 예측(next-token prediction), cross-entropy, perplexity 같은 말이 अचानक 등장해도 덜 낯설게 느껴집니다.
