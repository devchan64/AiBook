...
이 절은 데이터 분할 이름을 외우는 절이 아니라, 비교와 마지막 확인의 경계를 고정하는 절입니다.

| 지금 절에서 먼저 고정할 것 | 바로 다음에 이어질 질문 | 뒤에서 다시 연결될 곳 |
| --- | --- | --- |
| 검증은 비교, 테스트는 마지막 확인이라는 역할 분리 | 새 데이터에서도 이 비교가 왜 버텨야 하는가 | 일반화는 P4-5, 평가 지표는 P4-6, baseline 비교는 P4-8 |
...
- 실험 중간 비교에 쓸 데이터와 마지막 확인에 남겨 둘 데이터를 구분해야 할 때 검증과 테스트의 역할 분리를 먼저 떠올립니다.
- 테스트 점수를 중간에 반복해서 보며 선택을 바꾸고 있지 않은지 점검할 때 이 절로 돌아옵니다.
- 데이터가 작을수록 검증과 테스트를 안정적으로 확보하기 더 어렵다는 점과, 교차검증이 왜 등장하는지 다시 설명할 때 이 절이 기준이 됩니다.
