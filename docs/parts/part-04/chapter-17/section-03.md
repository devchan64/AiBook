...
이번 예제는 같은 데이터 장면을 볼 때 `무슨 군집 직관을 먼저 떠올릴지`를 짧은 표로 다시 확인하는 실습입니다.

- 문제 상황: 둥근 묶음, 비정형 묶음, 합쳐지는 순서, 연결 구조 중 무엇이 먼저 필요한지 구분한다
- 입력(input): 문제 장면 설명
- 기대 출력(output): 더 먼저 떠올릴 군집 방법
- 확인할 개념:
...
이 예제의 핵심은 알고리즘 이름을 외우는 일이 아니라, `무슨 구조를 보고 싶은가`를 먼저 묻는 일입니다. 같은 클러스터링 문제처럼 보여도 어떤 장면은 중심이 더 중요하고, 어떤 장면은 밀도, 어떤 장면은 합쳐지는 순서, 어떤 장면은 연결 구조가 더 중요할 수 있습니다.
