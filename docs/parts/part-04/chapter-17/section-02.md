...
이 절은 입문적으로 `군집 결과를 과신하지 않는 태도`를 만드는 데 초점을 둡니다. 군집 평가 지표는 P4-6.4에서 다시 읽고, 시각화와 임베딩 공간 왜곡은 P4-18.2에서 다시 연결합니다. 반지도학습과의 연결은 현재 절 안에서 `군집이 왜 라벨 가설의 출발점은 될 수 있지만, 곧바로 정답 라벨을 대신하지는 못하는가` 수준까지 직접 회수하고, 반지도학습의 대표 장면과 연결 기준은 P4-17.4 보충학습에서 다시 묶습니다.
