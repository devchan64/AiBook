...
CUDA 내부 구조와 하드웨어 최적화 세부는 여기서 다루지 않습니다. 대신 다음 절 P4-9.2에서 배치(batch)와 텐서(tensor) 계산으로 더 구체적인 계산 형태를 이어서 봅니다. Transformer가 왜 GPU 병렬 처리와 잘 맞았는지는 P4-14.1, P4-14.2에서 다시 회수하고, TPU/NPU 상세 비교는 이 책의 현재 본편 범위 밖에 둡니다.
