...
GridSearchCV, RandomizedSearchCV의 기본 비교와 검증 비용은 다음 절 P4-9.2에서 바로 이어서 다루고, 고급 탐색 공간 설계와 분산 실험 관리는 P4-9.3 보충학습에서 다시 정리합니다.
...
이 절의 역할은 설정값 이름을 늘리는 일이 아니라, 다음 비교 절차의 손잡이를 만드는 데 있습니다.

| 지금 절에서 먼저 구분할 것 | 바로 다음 절에서 하는 일 | 뒤 알고리즘 절에서 다시 읽을 방식 |
| --- | --- | --- |
| 무엇이 학습되는 파라미터이고 무엇이 미리 넣는 하이퍼파라미터인가 | 어떤 후보 범위를 validation 절차 안에서 비교할 것인가 | 각 모델의 설정값을 `외울 이름`이 아니라 `비교할 손잡이`로 읽는다 |
...
- 지금 보는 값이 학습으로 얻는 파라미터인지, 사람이 먼저 정하는 하이퍼파라미터인지 헷갈릴 때 이 절을 떠올립니다.
- 설정값이 복잡도, 일반화, 계산 비용, 재현성에 어떤 영향을 주는지 다시 설명해야 할 때 이 절로 돌아옵니다.
- `random_state` 같은 값이 성능 향상보다 재현성 관리와 더 관련 깊다는 점을 확인할 때 이 절이 기준이 됩니다.
...
이 절은 하이퍼파라미터가 `무엇인지`를 설명하는 도입 절입니다. 다음 절 P4-9.2에서는 그 값을 실제로 어떻게 탐색할지, 왜 탐색 범위를 넓히면 계산 비용과 검증 비용이 같이 커지는지, 그리고 baseline 이후의 개선을 어떻게 해석해야 하는지를 이어서 다룹니다.

또한 바로 이어지는 알고리즘 절들인 P4-10부터 P4-16까지에서 각 모델의 대표 하이퍼파라미터가 다시 등장합니다.
...
즉, 이 절은 뒤 알고리즘 장에서 설정값을 읽어 낼 공통 언어를 만드는 절입니다.
