...
这一节不会再长篇重复 hyperparameter 本身的定义。`学出来的值` 和 `预先定好的值` 之间的基本区分，会继续通过 P4-9.1 和 [概念词汇表](/AiBook/en/reference/concept-glossary/) 接回；这里则只集中在：这些设置值到底该如何比较、如何管理。
...
- tuning 是通过 validation 流程比较 hyperparameter 候选的工作。
- 计算成本和验证成本不是同一个东西。
- test data 必须保留到最后确认时再用。
- grid search 可以理解成“全比一遍”，random search 可以理解成“策略性地比一部分”。
- baseline 之后的小分数差，必须连同成本一起读。

读完这一节之后，下面这条边界应该很清楚。

| 这一节正在做什么 | 这里还不会直接下什么结论 | 必须一路一起看的东西 |
| --- | --- | --- |
| 在 validation 内部比较候选设置 | 只看一个 `best score` 就立刻下结论说它是好 model | baseline 差异、混淆矩阵中的问题格子、代表错误案例、计算成本 |

## 简短检查

- 你是不是在用 validation 选设置，并把 test 只保留给最后确认？
- 你有没有把计算成本和验证成本分开看，而不是把它们混成同一句话？
- 当看到一点分数上升时，你有没有把代表错误案例和运营成本一起重新检查？

## 什么时候应该先想起这个观念

- 当候选设置必须在 validation 内比较，而 test 必须只留到最后确认时，要先想起 tuning 流程这个视角。
- 当计算成本和验证成本不能混成一个词来讲时，就该回到这一节。
- 当一个很小的分数上升必须通过 baseline 和代表错误案例再确认它是否真的是改进时，这一节会成为标准。
...
走到 P4-9，读者现在已经具备了 `选择 model`、`建立 baseline`、`比较设置值的流程`。从这里开始，后面每一章算法章节都会继续反复使用这一节的视角。
...
也就是说，后面章节里出现的各种设置值，现在都应该被读成不是 `要背的名字`，而是 `要在 validation 流程里比较的候选`。
