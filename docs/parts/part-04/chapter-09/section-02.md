...
이 절의 목적은 텐서 수학의 엄밀한 정의보다, `딥러닝 계산이 어떤 데이터 모양으로 흐르는가`를 설명하는 것입니다. GPU 병렬 처리의 큰 그림은 P4-9.1에서 이미 봤고, Transformer와 attention의 대규모 행렬 계산은 P4-13.2와 P4-14.1, P4-14.2에서 다시 연결합니다. 메모리 레이아웃, stride, 분산 배치 처리 세부는 이 책의 현재 본편 범위 밖에 둡니다.
...
고객 데이터가 100개 있고 각 고객마다 feature가 20개라면, 모델은 보통 한 명씩 읽기보다 여러 명을 한꺼번에 묶어 계산합니다. 예를 들어 한 번의 배치 입력은 `(32, 20)` 같은 텐서로 볼 수 있습니다. 이때 중요한 것은 단순 숫자 외우기가 아니라, `32명의 고객을 동시에`, `각 고객당 20개 특징으로` 처리하고 있다는 점입니다.
...
컬러 이미지 32장을 한 번에 넣는다면 `(32, 3, 224, 224)` 같은 shape가 등장할 수 있습니다. 사람이 보기에는 그냥 사진 32장일 뿐이지만, 계산에서는 `배치`, `채널`, `높이`, `너비`가 축(axis)으로 분리된 텐서가 됩니다. 이 축 구분이 있어야 convolution, pooling, batch 연산이 어떤 방향으로 적용되는지 일관되게 설명할 수 있습니다.

### 사례 3. 문장 모델

문장 데이터를 다룰 때는 `(batch_size, sequence_length, embedding_dim)` 같은 shape가 자주 나옵니다. 예를 들어 16개 문장을 한꺼번에 읽고, 각 문장이 128개 토큰으로 잘리고, 각 토큰이 768차원 벡터로 표현될 수 있습니다. 즉, 텐서는 표 데이터, 이미지, 문장처럼 서로 다른 입력을 같은 계산 언어로 다루게 해 주는 공통 표현입니다.
