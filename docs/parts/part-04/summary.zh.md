...
重新翻开 Part 4 时，最快的方法是抓住代表性 Section。学习类型划分先看 `P4-2.1` 到 `P4-2.3`，validation 和 test 的区分在 `P4-4.2`，overfitting 和 underfitting 在 `P4-5.1`，评估指标在 `P4-6.1`，feature selection 和 preprocessing 在 `P4-7.1` 到 `P4-7.2`，baseline 在 `P4-8.2`，linear regression 在 `P4-10.1`，logistic regression 在 `P4-11.1`，`k-NN` 在 `P4-12.1`，`SVM` 在 `P4-13.1`，decision tree 在 `P4-14.1`，random forest 在 `P4-15.1`，gradient boosting 在 `P4-16.1`，clustering 在 `P4-17.1`，dimensionality reduction 在 `P4-18.1`。同一 Part 里的后续 Section，如果能和这些代表位置以及[概念词汇表](/AiBook/en/reference/concept-glossary/)一起重读，语境就不容易散掉。
...
完成这一 Part 之后，读者应该能把问题定义、数据切分、泛化、评估指标、baseline 和应用边界一起读。
...
4. 读评估指标与业务判断的连接。
5. 把 feature selection、preprocessing、model selection、baseline、tuning 绑在一起看。
...
| baseline | baseline 不是低起点，而是判断改进是否真正有意义的标准 |
...
Part 4 是把 Part 2 里恢复出来的公式、数组、表、图、运行环境感觉，绑成机器学习判断结构的一段。

## 和后面 Parts 的连接

Part 4 固定下来的共同问题，会原样被带到下一 Part 的 deep learning，以及再后面的 LLM 部分。
...
- 能否区分 feature selection、preprocessing、model selection、baseline、tuning 的角色？
...
下一 Part 会进入 deep learning。Part 4 里整理出的共同问题，会原样继续过去。

要带进 Part 5 的三个主要交接标准是：

- 区分 train、validation、test 与 generalization 的感觉
- 按 confusion matrix、error case、baseline 顺序读评估的习惯
- 在算法名字之前先问问题结构与失败成本的态度

> 数据是怎样被表示的？
> 模型到底学到了什么？
> loss 和 optimization 按什么标准工作？
> generalization 要怎么检查？
> 表达能力越大，为什么计算与并行处理的重要性也会跟着变大？

所以，下一 Part 的神经网络，不是替代 Part 4 的新语言，而是把 Part 4 固定下来的共同问题，扩展到更大的 function、更多的 parameter、以及更复杂的 representation learning 上。

在进入下一 Part 前，手里最好留下下面三个交接标准。

| 现在手里应该留下的标准 | 下一 Part 里会立刻再用到的地方 | 如果卡住，该回到 Part 4 的哪里 |
| --- | --- | --- |
| 能区分 train、validation、test 与 generalization 的关系 | loss、optimization、神经网络性能比较 | 数据切分与泛化：P4-4、P4-5 |
| 能按 confusion matrix、error case、baseline 顺序读评估 | deep learning 分类器与生成模型评估解释 | 评估指标与 baseline：P4-6、P4-8 |
| 会先问问题结构与失败成本，而不是算法 | 神经网络结构选择与应用判断 | 整个 Part 4 总结与代表模型 Section |

也就是说，在 Part 4 里，更早的问题不是`拿了几分`，而是`错在哪里`、`为什么错在那里`、`这个差异到底值不值`
