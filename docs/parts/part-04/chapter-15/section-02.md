...
4. 마지막에 이 개념이 이후 토큰 단위 생성 설정으로 어떻게 이어지는지 정리합니다.
...
이 관점은 뒤에서 토큰(token) 단위 생성과 프롬프트(prompt) 실험을 설명할 때 매우 중요해집니다.
...
즉, 이 절은 Part 4의 생성 모델 설명을 실제 출력 선택 문제까지 닫은 뒤, 뒤의 토큰 단위 생성 설명으로 넘겨 주는 연결 절입니다.
...
이 질문은 다음 Part의 첫 장, P5-1.1 토큰(token)은 무엇인가로 이어집니다.
