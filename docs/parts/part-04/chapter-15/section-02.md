...
이 절에서는 생성형 AI를 사용할 때 `모델이 답을 아는가`라는 질문을 넘어, `모델이 답을 어떤 방식으로 꺼내는가`를 설명합니다. 토큰 단위 샘플링, temperature, top-k, top-p의 실사용 감각은 Part 5의 P5-1.2와 P5-2.1, P5-2.2에서 다시 연결합니다. beam search와 diffusion 샘플링의 세부 구현은 이 책의 현재 본편 범위 밖에 둡니다.
