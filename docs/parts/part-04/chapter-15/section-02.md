...
사람은 보통 `문맥상 가장 무난한 한 표현`을 먼저 떠올립니다. 그래서 자동완성도 늘 가장 높은 후보 하나만 고르면 된다고 생각하기 쉽습니다. 하지만 실제 문장에서는 `있습니다`, `열립니다`, `진행됩니다`처럼 여러 후보가 모두 자연스러울 수 있습니다. 예를 들어 회의 공지 메일에서는 `열립니다`가 자연스럽고, 일정 알림에서는 `진행됩니다`가 더 어울릴 수 있습니다. 항상 가장 높은 후보만 고르면 메일 초안이 매번 비슷한 말투로 굳어질 수 있고, 조금 다른 후보를 허용하면 표현 폭이 넓어질 수 있습니다. 즉, 사람이 `정답 한 개`를 먼저 떠올리는 장면에서도 모델은 여러 자연스러운 후보를 두고 선택할 수 있습니다. 샘플링 차이는 이렇게 같은 문장 시작에서도 결과의 반복성과 다양성을 바꾸고, 너무 고정된 자동완성과 지나치게 흔들리는 자동완성 사이 균형을 만들게 합니다.

### 사례 2. 챗봇 답변

사용자가 `정규화와 표준화의 차이를 설명해 줘`라고 물었을 때, 사람은 답이 하나로 정해져 있다고 느끼기 쉽습니다. 그러나 실제로는 짧은 정의 중심 답, 예시를 붙인 설명, 비교 표 중심 답처럼 여러 표현 방식이 모두 가능할 수 있습니다. 사람이 원하는 것은 보통 `틀리지 않는 답`이지만, 실제 사용자 경험에서는 길이, 말투, 예시 유무도 함께 달라집니다. 예를 들어 시험 대비용 답은 짧은 정의가 낫고, 실무 입문자용 답은 `정규화는 과적합 억제, 표준화는 입력 스케일 정리` 같은 예시가 붙는 편이 더 낫습니다. 한 번은 짧은 정의 중심 답이 나오고 다른 한 번은 예시가 붙은 긴 답이 나오는 것은 모델 지식만의 차이가 아니라, 어떤 후보를 선택하도록 두었는지에 따른 출력 선택 방식의 차이이기도 합니다. 즉, 답의 말투와 길이도 샘플링 규칙의 영향을 받으며, 너무 보수적으로 고르면 항상 비슷한 답만 나오고 너무 넓게 허용하면 설명 흐름이 흩어질 수 있습니다.

### 사례 3. 이미지 생성 프롬프트

`a small cafe in the rain` 같은 프롬프트를 보면 사람은 머릿속에 한 장면을 먼저 떠올리기 쉽습니다. 하지만 모델 입장에서는 창가가 강조된 장면, 골목 구도가 강조된 장면, 따뜻한 색감의 장면처럼 여러 결과가 모두 그럴듯할 수 있습니다. 사람이 `한 장면`을 먼저 상상하는 것과 달리, 모델은 여러 시각 요소 조합을 후보로 둘 수 있습니다. 같은 프롬프트여도 한 번은 창가가 강조되고, 다른 한 번은 골목 구도가 강조될 수 있으며, 색감과 조명도 달라질 수 있습니다. 예를 들어 같은 비 오는 카페라도 한 결과는 실내 조명이 따뜻하고, 다른 결과는 거리 반사광이 강조될 수 있습니다. 이 역시 생성 과정에서 어떤 후보를 선택하느냐와 연결된 확률성의 한 예로 읽을 수 있습니다. 이 사례는 샘플링이 텍스트뿐 아니라 이미지 생성에서도 결과 다양성에 직접 영향을 준다는 점을 보여 주고, 같은 모델이라도 출력 분위기가 달라질 수 있음을 설명합니다.
