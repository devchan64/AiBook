...
즉, argmax는 가장 높은 후보 하나로 출력을 빠르게 고정하고, sampling은 높은 후보를 더 자주 택하되 다른 표현과 길이 변화도 함께 실제 출력에 남깁니다. 이 차이는 후보 가중치와 반복 횟수를 바꿔 보며 다양성과 안정성 균형이 어떻게 흔들리는지 바로 다시 확인할 수 있습니다.
...
- 생성 모델은 후보들의 그럴듯함을 계산하고 샘플링은 그중 실제 출력을 고르며, 생성 문제는 정답이 하나만 있는 문제가 아닐 수 있습니다.
- 그래서 다양성과 안정성의 균형이 생성 품질에 중요합니다.
