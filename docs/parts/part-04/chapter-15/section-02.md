...
이 절의 목적은 생성형 AI를 사용할 때 `모델이 답을 아는가`라는 질문을 넘어, `모델이 답을 어떤 방식으로 꺼내는가`를 설명할 수 있게 하는 데 있습니다.
...
초심자 기준에서는 샘플링을 다음처럼 설명하면 충분합니다.
...
초심자 기준에서는 이 절의 핵심을 다음처럼 기억하면 좋습니다.
