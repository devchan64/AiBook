...
랜덤포레스트에서는 이 질문에 대해 가장 먼저 만나는 손잡이 중 하나가 OOB(out-of-bag)입니다.
...
교차검증과 모델 비교의 더 넓은 문맥은 P4-16과 Part 7 프로젝트에서 다시 이어집니다.
...
이 높은 훈련 점수만 보면 모델이 충분히 좋아졌다고 느낄 수 있습니다. 하지만 팀은 이미 결정트리에서 `훈련에 잘 맞는다`와 `새 데이터에도 잘 맞는다`가 다를 수 있다는 점을 배웠습니다. 랜덤포레스트에서는 bootstrap 때문에 각 트리가 보지 않은 샘플이 자연스럽게 생기므로, 그 틈을 이용해 OOB 점수로 내부 점검을 먼저 합니다.
...
예를 들어 train만 높고 OOB가 낮으면 지금 숲이 훈련 데이터에만 낙관적으로 맞고 있을 수 있습니다. 이때는 트리 깊이, 특징 표현, validation 분리를 먼저 다시 점검해야 합니다. 반대로 `OOB와 test가 함께 낮으면 숲 자체보다 입력 표현이나 데이터 품질을 먼저 다시 봐야 한다.` 이런 판단까지 붙어야 OOB가 점수 설명을 넘어 다음 실험 순서를 정하는 기준이 됩니다.
...
즉, OOB 절의 핵심은 `점수가 얼마인가`보다 `이 점수 패턴이면 다음 실험에서 무엇을 먼저 바꾸고, 무엇은 아직 미뤄야 하는가`를 바로 말할 수 있게 되는 데 있습니다.

이 절의 다음 장면은 자연스럽게 그래디언트 부스팅(gradient boosting)입니다. 랜덤포레스트가 `여러 트리를 병렬적으로 모아 흔들림을 줄이는 방식`이었다면, 바로 다음 P4-16의 부스팅은 `이전 오류를 다음 트리가 순차적으로 보정하는 방식`으로 넘어갑니다.
...
- OOB는 빠른 실험의 손잡이로 유용하지만, 최종 배포 판단의 유일한 근거는 아닙니다.
...
| bootstrap과 OOB의 관계 | 이 점수가 왜 내부 점검이고 별도 test를 완전히 대체하지 않는가 | P4-9 검증 절차, Part 6 실험 기록 |
| 다음 조정 손잡이 | 트리 수와 설정을 바꿨을 때 어떤 점검값이 같이 변하는가 | P4-16 부스팅과 후속 앙상블 비교 |
| 반복 오류 사례 | 어떤 샘플이 OOB와 test에서 계속 남는가 | 특징 재설계와 다음 실험 메모 |
