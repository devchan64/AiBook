...
- train 점수는 높지만 일반화 상태가 애매할 때, OOB를 내부 점검 신호로 먼저 떠올립니다.
- OOB를 별도 test와 같은 것으로 읽고 싶어질 때, bootstrap에서 빠진 샘플을 활용한 제한적 검증이라는 점을 다시 봅니다.
- 랜덤포레스트 실험을 비교할 때, train / OOB / test 세 층을 함께 보는 습관이 필요한 순간에 이 절을 다시 꺼냅니다.
