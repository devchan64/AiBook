...
OOB(out-of-bag) 점수의 평가 해석은 P4-15.3에서 이어서 다룹니다.
그래디언트 부스팅과의 비교는 P4-16.1, P4-16.2에서 `병렬적 평균 앙상블`과 `순차적 오차 보정 앙상블`의 대비로 이어서 다룹니다.
Extra Trees와의 세부 비교는 P4-15.4 보충학습에서 이어서 다룹니다.
...
- 단일 트리가 시드나 표본 변화에 따라 쉽게 흔들릴 때, 여러 트리 집계로 분산을 낮추는 관점을 먼저 떠올립니다.
- bootstrap과 `max_features`가 왜 같이 필요한지 설명해야 할 때, 트리들을 서로 덜 닮게 만드는 장치라는 점을 다시 봅니다.
- 최고 한 번의 점수보다 평균적 안정성이 중요한 비교 상황에서, 랜덤포레스트를 안정성 후보로 다시 올립니다.
