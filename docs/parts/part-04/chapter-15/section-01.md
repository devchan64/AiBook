...
P4-14에서는 결정트리(decision tree)가 왜 직관적이면서도 과적합(overfitting)에 쉽게 빠질 수 있는지 보았습니다. 이제 자연스럽게 다음 질문이 나옵니다.
...
여기에 한 가지를 더 붙이면 랜덤포레스트 절이 지금까지 정리한 비교 기록 구조와 자연스럽게 이어집니다. 랜덤포레스트를 후보로 올릴 때는 `트리를 여러 개 쓴다`는 설명만 남기는 것이 아니라, `단일 트리 대비 어떤 오류 사례가 덜 흔들리는가`, `여전히 남는 애매한 사례는 무엇인가`, `다음에 어떤 숲 설정을 더 볼 것인가`를 함께 적어 두는 편이 좋습니다. 같은 평균 점수처럼 보여도 어떤 모델이 특정 오류 유형을 더 자주 반복하는지, 어떤 모델이 시드를 바꿔도 더 안정적으로 유지되는지는 따로 읽어야 합니다.
...
처음 읽을 때는 트리 계열 안에서도 질문이 어떻게 달라지는지만 다음처럼 잡아도 충분합니다.
...
이 표가 있으면 랜덤포레스트 절이 `비교 후보 -> 남는 오류 사례 -> 다음 질문` 구조로 먼저 읽힙니다. 즉, 랜덤포레스트의 장점은 평균 숫자 하나보다 `남는 실패 패턴이 덜 흔들리는가`를 같이 볼 때 더 분명해집니다.

그래서 실무에서 랜덤포레스트를 먼저 고려하는 장면은 대체로 `단일 트리는 너무 흔들리지만, 신경망까지 바로 갈 만큼 데이터 규모나 구조가 크지 않은 표 형식 문제`입니다. 이때 중요한 기대값은 `최고 한 번의 점수`보다 `덜 흔들리는 기본 후보`를 빠르게 세우는 일입니다.
...
scikit-learn 문서는 분류 random forest에서 트리들의 확률 예측을 평균해 결합한다고 설명합니다. 반면 독자에게는 종종 `majority vote`라는 설명이 더 익숙할 수 있습니다. 둘 다 큰 흐름에서는 맞지만, scikit-learn 구현 기준으로는 확률 평균 쪽이 더 정확한 설명입니다.
...
API 문서 기준으로 랜덤포레스트에서 독자가 먼저 알아야 할 손잡이는 다음 정도입니다.
...
- 특징 중요도 해석은 다음 절 P4-15.2의 범위라는 점을 알고 있는가?
