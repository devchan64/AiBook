...
사용자가 `비밀번호를 바꾸려면 어떻게 하나요?`라고 물으면 답은 하나의 문장으로만 정해지지 않습니다. 사람은 FAQ처럼 `정답 문장 하나`가 있을 것이라고 먼저 기대하기 쉽지만, 실제로는 짧게 핵심만 말할 수도 있고, 단계별 안내로 길게 풀 수도 있으며, 주의 문구를 먼저 붙일 수도 있습니다. 즉, 이 장면에서는 하나의 정답 라벨보다 여러 그럴듯한 응답 후보가 존재합니다. 생성 모델은 이런 후보들 중 현재 문맥에 자연스러운 표현을 이어 가는 쪽에 가깝습니다. 그래서 챗봇 사례는 생성 모델이 왜 `가능한 답들의 분포`를 다룬다고 말하는지 보여 줍니다.

### 사례 3. 이미지 생성

`a red house near a lake` 같은 프롬프트를 주면 가능한 이미지는 하나가 아닙니다. 사람은 머릿속에 먼저 한 장면을 떠올리기 쉽지만, 집의 각도, 호수 위치, 하늘 색, 나무 배치가 모두 조금씩 달라도 여전히 자연스러운 그림이 될 수 있습니다. 사람이 여기서 확인해야 하는 것은 `정답 한 장`이 아니라 `자연스러운 시각 패턴 조합이 얼마나 다양하게 나올 수 있는가`입니다. 생성 모델은 이런 시각 패턴 분포를 학습해 여러 결과를 만들 수 있습니다. 그래서 이미지 생성 사례는 생성 모델이 `정답 복원`보다 `가능한 결과 공간 만들기`에 가깝다는 점을 보여 줍니다.
