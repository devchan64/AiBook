...
즉, 이 출력은 생성이 `정답 하나 선택`이 아니라 후보 분포에서 실제 출력을 꺼내는 일이며, 더 그럴듯한 후보가 더 자주 나오더라도 낮은 확률 후보도 결과 안에 실제로 남을 수 있음을 보여 줍니다.
...
- 생성 모델은 데이터 패턴을 학습해 새로운 샘플이나 다음 출력을 만들며, 분류 모델이 범주를 고르는 데 비해 출력 자체를 구성합니다.
- 생성 문제는 여러 그럴듯한 답이 가능하므로 확률적 출력과 자주 연결되고, 핵심은 `맞는 라벨 선택`보다 `자연스러운 후보 분포 구성`에 있습니다.
