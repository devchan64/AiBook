...
- `데이터 분포(data distribution)를 배운다`는 말은 초심자에게 어떻게 설명할 수 있는가?
...
샘플링(sampling)과 출력 품질 변동은 P4-15.2에서 이어서 더 구체적으로 다룹니다.

## 이 절의 목표

- 생성 모델을 `데이터 패턴을 배워 새로운 샘플이나 다음 출력을 만드는 모델`로 설명할 수 있습니다.
- 분류 모델과 생성 모델의 차이를 초심자 수준에서 비교할 수 있습니다.
...
초심자는 `분포(distribution)`라는 말에서 바로 어려움을 느낄 수 있습니다. 이 절에서는 다음처럼 설명하면 충분합니다.
