...
즉, 이 절은 P4-14.2에서 본 강한 문맥 구조를 `무엇을 분류하나`에서 `무엇을 새로 만들 수 있나`로 확장하는 절입니다. 여기서 생성 문제를 현재 파트 안의 독립된 질문으로 닫아 두어야, 다음 절 P4-15.2에서 샘플링과 품질 변동을 `출력 선택 문제`로 바로 이어서 읽을 수 있습니다.
