...
사용자가 `비밀번호를 바꾸려면 어떻게 하나요?`라고 묻는 고객 지원 장면을 생각해 볼 수 있습니다. 사람은 FAQ처럼 `정답 문장 하나`가 있을 것이라고 먼저 기대하기 쉽습니다. 하지만 실제 서비스에서는 사용 채널, 사용자 숙련도, 보안 주의사항에 따라 자연스러운 답 형태가 달라집니다. 예를 들어 모바일 앱 기준 안내와 웹 기준 안내가 모두 후보가 될 수 있고, 어떤 경우에는 경고 문구를 먼저 두는 편이 더 안전할 수 있습니다. 즉, 이 장면에서는 하나의 라벨보다 여러 그럴듯한 응답 구성이 존재합니다. 생성 모델은 이런 후보 분포를 다루며, 현재 문맥에서 더 자연스러운 길이, 말투, 순서를 가진 답을 이어 가는 쪽에 가깝습니다. 그래서 이 사례에서 확인해야 할 결과는 `맞다/틀리다` 하나만이 아니라, 핵심 절차가 빠지지 않았는지와 답변 구조가 상황에 맞게 달라지는가입니다.
