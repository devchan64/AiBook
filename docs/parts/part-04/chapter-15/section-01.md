...
문장 `The weather is` 뒤에 `good`, `nice`, `sunny` 같은 여러 토큰이 가능할 수 있습니다. 생성 모델은 그중 어떤 토큰이 더 자연스러운지 배웁니다.

### 사례 2. 챗봇 응답

사용자 질문에 대해 답변 한 문장만 가능한 것이 아닙니다. 여러 표현이 가능하며, 생성 모델은 그럴듯한 응답을 만들 수 있습니다.

### 사례 3. 이미지 생성

`a red house near a lake` 같은 프롬프트에 대해 가능한 이미지는 하나가 아닙니다. 생성 모델은 자연스러운 시각 패턴을 학습해 다양한 결과를 만들 수 있습니다.
