...
문장 `The weather is` 뒤를 이어 쓴다고 해 봅시다. 사람도 `good`, `nice`, `sunny`처럼 여러 단어를 떠올릴 수 있고, 정답이 하나로 고정되지는 않습니다. 중요한 것은 `무슨 단어가 가능한가`보다 `어떤 단어가 지금 문맥에서 더 자연스러운가`입니다. 생성 모델은 바로 이 여러 가능성의 분포를 학습합니다. 그래서 이 사례는 생성이 분류처럼 정답 하나를 찍는 일과 어떻게 다른지 보여 줍니다.

### 사례 2. 챗봇 응답

사용자가 `비밀번호를 바꾸려면 어떻게 하나요?`라고 물으면 답은 하나의 문장으로만 정해지지 않습니다. 사람은 FAQ처럼 `정답 문장 하나`가 있을 것이라고 먼저 기대하기 쉽지만, 실제로는 짧게 핵심만 말할 수도 있고, 단계별 안내로 길게 풀 수도 있으며, 주의 문구를 먼저 붙일 수도 있습니다. 예를 들어 모바일 앱 기준 안내와 웹 기준 안내가 모두 자연스러운 후보일 수 있고, 경고 문구를 먼저 둘지 마지막에 둘지도 달라질 수 있습니다. 즉, 이 장면에서는 하나의 정답 라벨보다 여러 그럴듯한 응답 후보가 존재합니다. 생성 모델은 이런 후보들 중 현재 문맥에 자연스러운 표현을 이어 가는 쪽에 가깝습니다. 그래서 챗봇 사례는 생성 모델이 왜 `가능한 답들의 분포`를 다룬다고 말하는지 보여 줍니다.
