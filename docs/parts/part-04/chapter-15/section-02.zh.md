...
这一节不会再长篇重复随机森林的基本定义。 `通过多棵树的合意来降低摇摆` 这个核心直觉， 会通过 P4-15.1 和[概念词典](/AiBook/en/reference/concept-glossary/)重新连接。 这里聚焦的只是： 应该怎样解释这片森林把什么看得重要。
...
所以在相关性很强的场景里， 比起只看 `单列排名表`， 更重要的是看 `相似特征组`、 `删除前后比较`、 以及 `不同 importance 方法的对照`。 这也正是本节的目标。 不是让你更强地相信某一个数字， 而是留下一套更保守地阅读这个数字的标准。
