...
- t-SNE, UMAP의 구현 최적화와 전체 파라미터 튜닝 절차
- 차원 축소 기법별 수학적 증명
- trustworthiness 외 추가 품질 지표들의 비교 확장
...
즉, trustworthiness는 `이 그림의 근접 관계를 얼마나 믿을까`를 다시 보는 브레이크 역할을 합니다. 차원 축소 그림을 본 뒤 `붙어 보이는 점들`을 너무 빨리 같은 유형으로 묶고 싶을 때, 바로 이 지표가 던지는 질문을 떠올리면 좋습니다.
