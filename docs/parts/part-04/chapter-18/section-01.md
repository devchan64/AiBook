...
이 질문이 차원 축소(dimensionality reduction)의 출발점입니다. 차원 축소는 많은 특징을 그냥 버리는 기술이 아니라, `원래 표현이 너무 복잡할 때 더 읽기 쉬운 축으로 다시 표현하는 일`에 가깝습니다.
...
- 차원 축소는 특징을 지우는 기술보다 `표현을 다시 만드는 기술`에 가깝습니다.
- PCA는 데이터가 크게 변하는 방향을 첫 번째 새 축으로 잡고, 겹치지 않는 다음 방향을 두 번째 축으로 잡습니다.
- 고유벡터는 새 축의 방향, 고유값은 그 축이 설명하는 변동 크기로 읽습니다.
- 차원을 줄이면 읽기 쉬워지지만, 세부 차이 일부는 약해질 수 있습니다.
