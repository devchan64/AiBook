...
```mermaid
--8<-- "assets/part-04/chapter-02/p4-2-3-mermaid-02-ko.mmd"
```
...
식당 고르기 비유로 보면 더 분명합니다. 이미 좋아하는 식당에 가면 실패할 가능성은 낮습니다. 하지만 새로운 식당을 시도하지 않으면 더 좋은 선택지를 발견하기 어렵습니다. 강화학습도 비슷하게 이미 얻은 지식을 쓰면서도, 새로운 행동을 충분히 시도해야 합니다.
...
강화학습은 Part 5에서 LLM과 생성형 AI를 볼 때 다시 등장합니다. 특히 사람의 선호를 이용해 모델 출력을 조정하는 흐름은 P5-10.1 지시 튜닝과 P5-10.2 정렬의 기본 문제에서 다시 다룹니다.

다만 이 절에서는 LLM 정렬이나 RLHF를 깊게 설명하지 않습니다. 지금 필요한 것은 강화학습을 “보상을 통해 행동 방식을 조정하는 학습”으로 구분하는 일입니다.

강화학습 여부는 `지금 문제를 순차적 의사결정과 보상 흐름으로 읽는가`를 먼저 확인하면 정리됩니다.

| 현재 상태 | 강화학습으로 읽는가 | 이유 |
| --- | --- | --- |
| 행동을 하고 난 뒤 보상이 돌아오며, 다음 선택이 이어진다 | 예 | 한 번의 정답 맞히기보다 순차적 의사결정이 핵심이기 때문입니다. |
| 지금 점수보다 장기 보상을 더 크게 봐야 한다 | 예 | 즉시 보상과 장기 보상의 긴장이 문제 중심에 있기 때문입니다. |
| 이미 입력과 정답 라벨이 함께 있고 한 번의 예측이 중심이다 | 보통 아니오 | 지도학습 문제가 더 직접적이기 때문입니다. |
...
한 서비스가 사용자에게 어떤 쿠폰을 먼저 보여 줄지 자동으로 정하려 한다고 하겠습니다. 사람은 가장 클릭이 잘 나오는 쿠폰만 계속 보여 주면 될 것처럼 생각할 수 있습니다.

하지만 지금 클릭이 높다고 해서 장기 매출이나 재방문까지 좋아지는 것은 아닐 수 있습니다. 어떤 쿠폰은 당장 반응은 적어도 이후 구매로 이어질 수 있고, 어떤 쿠폰은 즉시 클릭은 높지만 장기적으로는 손해일 수 있습니다.
...
```mermaid
--8<-- "assets/part-04/chapter-02/p4-2-3-mermaid-03-ko.mmd"
```

## 언제 이 관점을 먼저 떠올려야 하는가

- 한 번의 입력과 정답 맞히기보다 시간에 따라 행동과 보상을 누적해 읽어야 하는 문제를 만났을 때 강화학습 관점을 먼저 떠올립니다.
- 즉시 보상과 장기 보상이 다를 수 있는 장면, 탐험과 활용의 균형을 설명해야 할 때 이 절로 돌아옵니다.
- 강화학습의 에이전트와 LLM 서비스 문맥의 에이전트를 같은 뜻처럼 섞지 않도록 다시 구분할 때 이 절이 기준이 됩니다.
