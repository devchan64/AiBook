...
비지도학습은 “정답 없이 제멋대로 학습한다”는 뜻이 아닙니다. 더 안전하게 말하면, 사람이 미리 붙인 라벨 없이 데이터 안의 비슷함, 묶음, 축, 밀도, 이상한 점을 찾아보는 접근입니다. 모델이 정답을 맞히는 것이 아니라, 사람이 데이터를 더 잘 이해하거나 다음 작업을 하기 위해 구조 후보를 만드는 경우가 많습니다.
...
초심자 기준에서는 여기서 한 번 더 다음 순서로 읽으면 안전합니다.
...
이 절이 비지도학습을 `라벨 없는 데이터에서 구조 후보를 찾는 접근`으로 설명하는 이유가 여기에 있습니다. 모델이 `이 집단은 VIP다`라는 이름까지 알려 주는 것은 아니고, 사람이 나중에 해석을 붙일 만한 패턴을 먼저 드러냅니다.
