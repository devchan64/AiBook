...
여기서는 지도학습을 “예시와 답이 함께 있는 사례를 보고, 새 사례의 출력을 맞춰 보는 방식”으로 이해합니다. 다만 모델이 해설을 이해하는 것은 아닙니다. 모델은 많은 사례에서 입력과 출력의 관계를 맞추도록 내부 기준을 조정합니다.
...
`X`와 `y`는 수학 기호로만 외우기보다 다음처럼 읽는 편이 더 분명합니다.
...
Part 3의 데이터 정리 흐름을 가져오면, 지도학습으로 넘어가기 전에 아래 네 가지를 먼저 구분해야 합니다.
...
먼저 학습 데이터와 테스트 데이터의 구분을 분명히 잡아야 합니다. 핵심은 “배운 문제를 다시 맞히는 것”과 “처음 보는 문제를 맞히는 것”이 다르다는 점입니다.
