...
- 라벨이 있다는 사실만으로 곧바로 지도학습 문제라고 단정하려 할 때 이 절의 라벨 점검 기준을 먼저 떠올립니다.
- `X`와 `y`를 나눌 때 어떤 열이 입력이고 어떤 값이 목표인지 다시 설명해야 할 때 이 절로 돌아옵니다.
- 모델 예측과 서비스의 최종 결정이 서로 다른 단계라는 점을 지도학습 문맥에서 다시 구분할 때 이 절이 기준이 됩니다.
