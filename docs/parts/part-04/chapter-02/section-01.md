...
처음 읽을 때는 지도학습을 “예시와 답이 함께 있는 사례를 보고, 새 사례의 출력을 맞춰 보는 방식” 정도로 받아들이면 됩니다. 다만 모델이 해설을 이해하는 것은 아닙니다. 모델은 많은 사례에서 입력과 출력의 관계를 맞추도록 내부 기준을 조정합니다.
...
Part 3의 데이터 정리 흐름을 가져오면, 지도학습으로 넘어가기 전에 아래 네 가지를 먼저 구분해 두는 편이 안전합니다.
...
그래서 초심자에게는 `라벨이 있다`와 `바로 지도학습 문제로 쓸 수 있다`를 분리해서 보는 습관이 중요합니다.
