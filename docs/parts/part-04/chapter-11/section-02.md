...
이 절의 목적은 convolution 수식을 엄밀히 증명하는 것이 아니라, `CNN이 지역 패턴을 점수화하고 요약하는 흐름`을 설명하는 것입니다. 지역 패턴에서 계층적 표현으로 이어지는 큰 그림은 P4-10.2와 P4-11.1에서 다시 연결합니다. 최신 vision 구조와 고급 구현 세부는 이 책의 현재 본편 범위 밖에 둡니다.
