...
ViT의 self-attention 자체는 P4-13.2와 P4-14.1에서 다시 연결되고, 여기서는 이미지 관점에서 `지역 패턴을 먼저 읽는가`, `패치 관계를 먼저 읽는가`라는 출발 문제 차이까지만 정리합니다. 최신 비전 모델의 세부 계보와 성능 경쟁은 이 책의 현재 범위 밖에 둡니다.
...
즉, ViT는 처음부터 `3x3 필터가 주변 픽셀을 훑는다`는 방식으로 출발하지 않습니다. 대신 `이미지를 여러 칸으로 잘라 표 조각처럼 놓고, 각 칸을 하나의 입력 단위로 본다`고 생각하면 더 직관적입니다.
...
핵심은 attention이 `입력 단위들 사이의 관계`를 계산하는 구조라는 점입니다. 문장에서는 그 입력 단위가 토큰이고, 이미지에서는 그 역할을 patch가 맡는다고 보면 됩니다.
...
이 표에서 중요한 것은 `CNN은 지역성이 기본 가정으로 더 강하게 들어가고, ViT는 관계 계산을 더 직접적으로 한다`는 차이입니다.
...
이 비교에서 중요한 것은 `CNN은 부분을 본다`, `ViT는 전체를 본다`처럼 단순하게 갈라버리는 일이 아닙니다. 둘 다 결국 이미지 전체 판단으로 가지만, 출발 직관이 `지역 패턴 중심`이냐 `패치 관계 중심`이냐에서 차이가 납니다. 이 절에서 독자가 먼저 붙잡아야 하는 것도 `무슨 이름이 더 최신인가`가 아니라, `이미지 문제를 어떤 계산 단위와 어떤 관계 가정으로 풀기 시작하는가`입니다.
...
이 출력에서 먼저 봐야 할 핵심은 `CNN은 겹치는 지역 창을 더 촘촘히 훑고`, `ViT는 이미지를 잘라 만든 patch token 몇 개를 먼저 만든다`는 출발 단위 차이입니다. 같은 이미지를 보더라도 어디서부터 표현을 만들기 시작하는지가 다르기 때문에, 뒤에서 attention과 patch embedding을 읽을 때도 `무엇을 토큰처럼 취급하는가`를 함께 봐야 합니다.
...
다만 self-attention의 계산식과 Q, K, V 전개는 여기서 처음 다루지 않습니다. 그 핵심 구조는 P4-13.2에서 다시 정리하고, Transformer 전체 흐름은 P4-14.1에서 회수합니다.
