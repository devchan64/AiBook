...
- softmax 전개와 one-vs-rest 비교
- solver별 차이와 regularization 설정
- 음의 로그우도의 미분 전개와 일반적인 최적화 이론

다중 클래스(multinomial) 확장은 P4-11.4에서, solver와 regularization은 P4-11.5에서 이어서 다룹니다. 음의 로그우도의 미분 전개와 일반적인 최적화 이론은 이 책의 현재 범위 밖에 둡니다.
...
여기까지 오면 로지스틱 회귀의 `확률 해석`과 `학습 목적`은 닫힙니다. 다음 보충학습에서는 이 감각이 `둘 중 하나를 고르는 문제`를 넘어 `여러 클래스 중 하나를 고르는 문제`로 어떻게 확장되는지, 즉 다중 클래스(multinomial) 로지스틱 회귀를 읽는 기본 구조를 봅니다.
