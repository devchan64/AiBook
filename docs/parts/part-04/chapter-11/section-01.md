...
| --- | --- | --- |
| `number of 2x2 patches = 9` | 작은 4x4 이미지도 여러 지역 창으로 쪼개어 반복해서 읽는다는 뜻 | 이미지 크기나 창 크기를 바꾸면 지역 패치 수가 어떻게 달라지는지 바로 확인할 수 있습니다 |
| `first patch`와 `second patch`의 점수가 다르다 | 같은 이미지라도 위치가 달라지면 지역 반응이 달라진다는 뜻 | 특정 숫자를 바꾸면 어느 지역 점수가 더 크게 변하는지 비교할 수 있습니다 |
| `best patch position = (2, 2)` | 가장 강한 지역 단서가 이미지 전체가 아니라 특정 위치에서 잡힐 수 있다는 뜻 | 오른쪽 아래 값을 낮추거나 다른 구역 값을 높이면 strongest patch가 다른 위치로 이동합니다 |

즉, 이 출력은 CNN이 이미지 전체를 한 번에 읽기보다 작은 지역 창을 반복해서 훑으며 위치마다 다른 반응 점수를 만들고, 그중 더 강한 지역 단서가 뒤 표현으로 이어질 수 있음을 보여 줍니다. 초기 convolution 직관은 `전체 그림 한 번 보기`보다 `어느 작은 구역이 더 강하게 반응하는가`를 반복 비교하는 쪽에 가깝습니다.

이 예제도 한 번 읽고 끝내기보다, 어떤 값을 바꿔 보면 `지역 반응` 감각이 더 선명해지는지 바로 이어서 확인하는 편이 좋습니다.

| 먼저 보인 출력 신호 | 지금 바로 해 볼 변화 | 아직 이 예제만으로 서두르지 않을 결론 |
| --- | --- | --- |
| 패치 수가 9개다 | 이미지 크기나 창 크기를 바꿔 지역 분할 수가 어떻게 달라지는지 본다 | 패치 수가 많다고 해서 곧바로 더 좋은 특징 추출이라고 단정하지 않는다 |
| 앞쪽 패치와 다음 패치 점수가 다르다 | 특정 위치 숫자를 높이거나 낮춰 어떤 지역 반응이 더 민감하게 바뀌는지 본다 | 단순 합 점수 예제 하나로 실제 convolution 필터 전체 동작을 모두 설명한다고 단정하지 않는다 |
| strongest patch가 오른쪽 아래에 있다 | 오른쪽 아래 대신 다른 구역 값을 더 크게 만들어 strongest location이 어떻게 이동하는지 본다 | 큰 반응 위치 하나만으로 곧바로 전체 이미지 의미 이해가 끝난다고 단정하지 않는다 |
...
- CNN은 이미지의 지역 패턴을 반복적으로 읽는 신경망이며, 이미지에서는 값뿐 아니라 위치와 주변 관계가 중요하므로 완전연결층보다 공간 구조를 더 자연스럽게 다룹니다. 이 구조는 사람이 특징을 직접 세지 않아도 지역 패턴에서 상위 시각 단서가 실제로 쌓여 올라가는지를 확인하는 대표 사례입니다.
