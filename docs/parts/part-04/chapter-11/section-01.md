...
이 절에서 먼저 닫아야 하는 핵심은 `이미지는 가까운 위치가 함께 의미를 만들기 때문에, 지역 패턴을 반복적으로 읽는 구조가 필요하다`는 점입니다. 즉, CNN은 뒤의 모델 역사를 예고하는 이름이 아니라, 이미지 데이터 구조에 맞춰 나온 첫 번째 대표 답으로 읽어야 합니다.
...
즉, 여기서 독자가 먼저 봐야 하는 차이는 `이미지 전체를 한 번에 섞는가`보다 `작은 지역 단서를 반복해서 읽는가`에 더 가깝습니다.
...
이제 그 흐름을 세 장의 자체 도식으로 나누어 보면 더 직관적입니다. 아래 도식의 숫자와 배열은 외부 그림을 옮긴 것이 아니라, 채널 결합과 계층적 표현 개념만 설명하기 위해 새로 단순화한 예시입니다.

먼저 읽는 순서는 다음처럼 잡으면 됩니다.
...
손글씨 글자인식에서는 숫자 이미지를 처음부터 `3`, `8` 같은 라벨로 읽는 것이 아니라, 앞의 작은 이미지 장면에서 본 짧은 획과 고리, 모서리 같은 지역 단서가 먼저 쌓여 숫자 패턴 판단으로 올라간다고 이해하면 됩니다.
