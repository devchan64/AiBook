...
- 완전연결층과 CNN의 차이를 초심자 수준에서 비교할 수 있습니다.
...
이미지를 길게 펼쳐서(flatten) 완전연결층에 넣는 방법도 이론적으로는 가능합니다. 하지만 이 방식은 이미지의 공간 구조를 초심자에게 직관적으로 보존하지 못합니다.
...
이 도식은 CNN이 픽셀 자체를 바로 정답으로 연결하는 것이 아니라, 작은 지역 패턴에서 더 큰 시각적 단서로 올라가는 계층 구조라는 점을 보여 줍니다. 초심자는 `edge -> 부분 구조 -> 객체 단서`의 흐름만 잡아도 CNN이 왜 표현 학습과 잘 연결되는지 이해하기 쉬워집니다.
...
- CNN을 초심자 수준에서 한 문장으로 설명할 수 있는가?
