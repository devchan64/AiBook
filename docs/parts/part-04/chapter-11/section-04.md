...
- softmax의 미분 전개
- 다중 클래스 로그우도의 엄밀한 행렬 수식
- solver별 수치 최적화 차이

solver와 regularization의 구현 관점은 P4-11.5에서 이어서 다룹니다. softmax의 미분 전개와 다중 클래스 로그우도의 엄밀한 행렬 수식은 이 책의 현재 범위 밖에 둡니다.
...
여기까지 오면 로지스틱 회귀의 `다중 클래스 확장`은 닫힙니다. 다음 보충학습에서는 같은 로지스틱 회귀라도 왜 solver, penalty, `C` 같은 설정을 만나게 되는지, 그리고 이 설정들이 왜 단순 구현 옵션이 아니라 비교 조건인지 봅니다.
