...
미분값의 엄밀한 수식 유도와 vanishing gradient의 상세 수학은 여기서 전개하지 않습니다. 대신 출력층 activation 설계는 P4-3.3에서 바로 이어서 보고, gradient와 학습 안정성의 더 넓은 맥락은 P4-5.1, P4-7.1에서 다시 회수합니다. GELU, Swish, Leaky ReLU 같은 후속 함수 비교는 이 책의 현재 본편 범위 밖에 둡니다.

이 절의 목적은 세 함수를 `암기`하는 것이 아니라, `어떤 성질 때문에 다르게 읽히는가`를 설명하는 것입니다. 출력층 activation 설계는 P4-3.3에서 바로 이어서 보고, gradient와 학습 안정성의 더 넓은 맥락은 P4-5.1, P4-7.1에서 다시 연결합니다. GELU, Swish, Leaky ReLU 같은 후속 함수 비교는 이 책의 현재 본편 범위 밖에 둡니다.
...
사람이 이 점수를 읽을 때도 `음수면 거의 꺼진 상태로 볼 것인가`, `중간값도 조금은 살려 둘 것인가`, `양수는 그대로 강하게 통과시킬 것인가` 같은 기준이 달라지는 셈입니다. 즉, 활성화 함수는 같은 점수에 서로 다른 반응 규칙을 붙이는 장치라고 볼 수 있습니다.
...
예를 들어 이 노드가 `위험 신호를 강하게 보이면 다음 층으로 넘기는 역할`을 한다고 해 보면, ReLU는 음수 신호를 바로 끊고 양수만 강하게 넘깁니다. 반면 sigmoid는 음수와 양수 모두 0과 1 사이로 눌러 `확률처럼` 전달하고, tanh는 음수와 양수를 모두 살리되 범위를 압축합니다. 그래서 같은 점수라도 다음 층이 받는 표현의 성격이 달라지고, 결국 학습 감각과 표현 방식도 달라집니다. 즉, 활성화 함수는 단순히 “모양이 다른 수학 함수”가 아니라, `같은 점수를 어떤 방식으로 해석하고 전달할지 정하는 규칙`입니다.
