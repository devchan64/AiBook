...
초심자 기준에서는 이를 아주 단순하게 다음처럼 이해하면 충분합니다.
...
초심자는 `ReLU가 좋다면 sigmoid와 tanh는 이제 필요 없는가?`라는 질문을 자주 합니다. 그렇지는 않습니다.
