...
```mermaid
--8<-- "assets/part-04/chapter-03/churn-model-selection-flow-ko.mmd"
```
...
```mermaid
--8<-- "assets/part-04/chapter-03/model-selection-basic-flow-ko.mmd"
```
...
기준 모델은 P4-8.2에서 더 자세히 다룹니다. 이 절에서는 모델 선택 휴리스틱의 첫 기준점으로만 사용합니다.
...
이 기준들은 서로 충돌합니다. 설명이 쉬운 모델은 성능이 낮을 수 있고, 성능이 좋은 모델은 비용이 크거나 설명이 어려울 수 있습니다. 모델 선택 휴리스틱은 이런 충돌을 미리 드러냅니다.
...
실제로는 `지금 어떤 모델 후보부터 줄이는가`를 먼저 판정하는 것이 핵심입니다.

| 현재 문제 상태 | 먼저 줄일 후보 방향 | 이유 |
| --- | --- | --- |
| 출력이 범주이고 설명 가능성도 중요하다 | 로지스틱 회귀, 결정트리 같은 단순 분류 후보부터 본다 | 기준 모델과 규칙 감각을 빠르게 비교하기 쉽기 때문입니다. |
| 표 형식 데이터에서 비선형 관계가 의심된다 | 트리 계열 후보를 함께 본다 | 특징 상호작용을 더 잘 잡을 가능성이 있기 때문입니다. |
| 라벨보다 구조 탐색이 먼저다 | 군집화나 차원 축소 후보로 방향을 바꾼다 | 예측 모델 비교보다 구조 이해가 현재 목표이기 때문입니다. |
| 기준 모델조차 성능이 너무 약하다 | 모델보다 데이터와 라벨 상태를 먼저 다시 본다 | 문제 정의나 데이터 품질이 먼저 흔들릴 수 있기 때문입니다. |
...
두 번째 오해는 검증 점수 하나만 보고 모델을 고르는 것입니다. 검증 데이터에 지나치게 맞추는 문제도 생길 수 있습니다. 이 문제는 P4-5의 과적합과 일반화에서 다시 다룹니다.
...
- 오픈체크리스트의 중심축 문장인 "휴리스틱이 모델 선택과 평가 비용 판단에 어떻게 개입하는지 설명해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
- 표, 코드, 도식, 체크리스트 중 어떤 장치가 이 판단을 다시 검토하게 만드는가?

### 사례 1. 고객 문의 분류에서 어떤 모델부터 비교할까

고객센터가 문의를 `환불`, `배송 지연`, `계정 문제`, `상품 문의`로 자동 분류하려고 합니다. 현업 담당자는 지금까지 제목에 `환불`, `취소`, `배송` 같은 단어가 있는지 보고 수작업 규칙으로 먼저 나누어 왔습니다.

이 방식은 초반에는 빠르지만, 표현이 조금만 달라져도 놓치는 문의가 늘어납니다. `환불하고 싶어요`는 잡히지만 `결제 취소 가능할까요`처럼 비슷한 뜻을 다른 말로 쓴 문장은 규칙만으로 놓치기 쉽습니다. 그래서 “무조건 가장 복잡한 모델을 쓰자”와 “기존 규칙만 유지하자” 사이에서 출발점을 정해야 합니다.
...
```mermaid
--8<-- "assets/part-04/chapter-03/inquiry-classification-model-selection-flow-ko.mmd"
```

## 체크리스트

- 왜 모델 선택은 `유명한 모델 고르기`가 아니라 `후보 집합 줄이기`인지 설명할 수 있는가
- 어떤 상태에서 모델을 더 복잡하게 바꾸기보다 데이터와 라벨을 먼저 다시 봐야 하는지 말할 수 있는가
- 기준 모델이 왜 복잡한 후보의 성능을 해석하는 비교 기준이 되는지 설명할 수 있는가
- 모델 선택이 모델 이름을 맞히는 일이 아니라 후보를 줄이고 검증하는 과정이라는 점을 설명할 수 있는가
- 문제 유형이 모델 후보를 좁히는 첫 기준이고, 기준 모델은 복잡한 후보를 비교하기 위한 최소 기준이라는 점을 설명할 수 있는가
- 성능, 설명 가능성, 계산 비용, 데이터 상태, 운영성을 함께 봐야 하는 이유를 설명할 수 있는가
