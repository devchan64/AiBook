...
이 절에서는 세 함수를 `암기`하기보다, `어떤 성질 때문에 다르게 읽히는가`를 설명합니다. 출력층 activation 설계는 P4-3.3에서 바로 이어서 보고, gradient와 학습 안정성의 더 넓은 맥락은 P4-5.1, P4-7.1에서 다시 연결합니다. GELU, Swish, Leaky ReLU 같은 후속 함수 비교는 이 책의 현재 본편 범위 밖에 둡니다.
...
- sigmoid, tanh, ReLU는 모두 비선형 변환이지만 출력 범위와 반응 방식이 다르며, sigmoid는 0~1, tanh는 -1~1, ReLU는 음수 차단과 양수 통과라는 직관으로 먼저 읽으면 됩니다.
- ReLU는 현대 딥러닝에서 자주 쓰이지만, sigmoid와 tanh도 역사와 구조 이해에 여전히 중요하며 이런 활성화 함수 차이는 표현 방식과 학습 감각의 차이로 이어집니다.
