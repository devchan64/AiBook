...
- 모델 선택을 `유명한 모델 고르기`처럼 읽고 있지 않은지 점검해야 할 때 후보 축소와 검증 흐름을 먼저 떠올립니다.
- 기준 모델이 왜 복잡한 후보의 성능을 해석하는 비교 기준이 되는지 다시 설명해야 할 때 이 절로 돌아옵니다.
- 성능만이 아니라 설명 가능성, 비용, 데이터 상태, 운영성을 함께 봐야 하는 상황을 정리할 때 이 절이 기준이 됩니다.
