...
이 표는 정답표가 아닙니다. 독자가 처음 방향을 잃지 않도록 후보를 좁히는 지도입니다. 실제 선택은 데이터와 평가 결과로 다시 확인해야 합니다.
...
이 방식은 처음에는 빠르지만, 표현이 조금만 달라져도 놓치는 문의가 늘어납니다. `환불하고 싶어요`는 잡히지만 `결제 취소 가능할까요`처럼 비슷한 뜻을 다른 말로 쓴 문장은 규칙만으로 놓치기 쉽습니다. 그래서 “무조건 가장 복잡한 모델을 쓰자”와 “기존 규칙만 유지하자” 사이에서 출발점을 정해야 합니다.
