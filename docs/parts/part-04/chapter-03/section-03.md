...
이 경우에는 sigmoid가 자주 등장합니다. sigmoid는 출력을 0과 1 사이로 눌러 주기 때문에, 초심자 입장에서는 `확률처럼 읽히는 값`이라는 직관을 주기 쉽습니다.
