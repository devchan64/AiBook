...
- 활성화 함수는 가중합 점수에 비선형 변환을 주는 규칙이며, 선형 결합만 반복하면 깊이가 늘어도 표현력이 충분히 늘지 않을 수 있으므로 딥러닝의 깊이는 활성화 함수와 함께 읽을 때 비로소 의미가 생깁니다.
...
| --- | --- | --- |
| 비선형 변환이 있어야 은닉층 표현이 더 풍부해진다는 점 | 어떤 활성화 함수가 어떤 계산 감각을 가지는가 | P4-3.2 활성화 함수 비교, 뒤의 학습 안정화 절 |
