...
- 계산 그래프는 연산 의존 관계를 펼쳐 놓은 표현이고, 순전파는 그 그래프를 따라 값을 계산하는 단계이며 역전파는 손실에서 시작한 gradient를 그래프를 따라 되돌려 보내는 단계입니다. 계산 그래프를 읽을 때는 복잡한 전체 미분식 대신 각 연산 블록의 국소 규칙을 연결해 gradient 전달을 따라가며, 자동미분은 이 구조를 바탕으로 gradient 계산을 조직합니다.
