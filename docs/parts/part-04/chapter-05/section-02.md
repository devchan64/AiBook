...
이 절의 목적은 그래프 이론 자체를 배우는 것이 아니라, `딥러닝 계산을 그래프처럼 읽는 이유`를 설명하는 것입니다. 실제 학습 단계에서 gradient가 optimizer와 만나 파라미터를 바꾸는 흐름은 P4-7.1, P4-7.2에서 다시 연결합니다. 프레임워크별 구현 차이와 내부 엔진 구현은 이 책의 현재 본편 범위 밖에 둡니다.
...
예를 들어 총매출 셀이 수량, 단가, 할인율 셀을 함께 참조한다고 해 봅시다. 최종 결과가 예상보다 작으면, 사람은 어떤 앞단 값이 영향을 주었는지 참조 관계를 따라 다시 살펴보게 됩니다. 물론 스프레드시트 자체가 역전파를 하는 것은 아니지만, `의존 관계가 있는 계산망`이라는 감각은 비슷합니다.
...
이 전체를 한 줄 수식으로만 보면 읽기 어렵고, 중간에 어디서 값이 크게 바뀌었는지도 보이지 않습니다. 계산 그래프 관점은 이 긴 계산을 `연산 블록의 연결`로 읽게 해 주고, 어느 블록 출력이 다음 블록 입력으로 이어졌는지 추적하기 쉽게 만듭니다.
...
- 계산 그래프는 연쇄 법칙을 작은 국소 규칙들의 조합으로 읽게 해 줍니다.
