...
이 점은 자주 오해됩니다. 일반화는 보통 `아무 환경에서나 무조건 잘 된다`는 뜻이 아닙니다. 먼저 `같은 문제 설정 안에서, 아직 보지 못한 예시에도 어느 정도 버틴다`는 뜻에 가깝습니다.
...
일반화 범위는 `어디까지를 지금 같은 문제의 새 예시로 볼 수 있는가`를 먼저 확인하면 정리됩니다.
...
```mermaid
--8<-- "assets/part-04/chapter-05/p4-5-2-mermaid-01-ko.mmd"
```
...
```mermaid
--8<-- "assets/part-04/chapter-05/p4-5-2-mermaid-03-ko.mmd"
```
...
즉, 사회현상에서도 일반화는 `과거에 잘 맞았다`보다 `새로운 표현과 조건 변화에도 얼마나 버티는가`를 묻는 질문으로 읽어야 더 정확합니다.
...
- 왜 일반화를 `학습 점수 높이기`가 아니라 `같은 문제의 새 예시에 버티는가`로 설명해야 하는지 말할 수 있는가
...
- 머신러닝의 목표를 학습 점수 자체가 아니라 새 데이터에서의 버팀성으로 다시 설명해야 할 때 일반화 관점을 떠올립니다.
- 과적합과 과소적합이 결국 일반화를 약하게 만든다는 연결을 다시 확인해야 할 때 이 절로 돌아옵니다.
- 검증과 테스트가 왜 일반화를 미리 가늠하는 장치인지 정리할 때 이 절이 기준이 됩니다.
