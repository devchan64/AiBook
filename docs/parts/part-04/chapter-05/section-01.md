...
커리큘럼 관점에서도 역전파는 매우 중요한 절입니다. 바로 앞의 P4-4.1, P4-4.2에서 손실 함수를 왜 두는지 배웠다면, 이제 그 손실이 각 층 가중치로 어떻게 전달되는지를 이해해야 합니다. 이유는 단순합니다.
...
| --- | --- | --- |
| 손실이 각 가중치에 미친 영향을 뒤에서 앞으로 계산한다 | 복잡한 연산 연결을 무엇으로 정리하면 좋은가 | 계산된 gradient를 optimizer 규칙으로 적용하는 일 |

- 역전파는 손실이 각 가중치에 미치는 영향을 뒤에서 앞으로 계산하는 절차이며, 학습 전체가 아니라 gradient 계산 단계입니다.
