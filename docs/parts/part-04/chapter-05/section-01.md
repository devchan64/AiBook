...
독자에게는 다음 세 문장을 함께 붙이는 편이 더 안전합니다.
...
복잡한 미분 흐름을 계산 그래프로 보는 관점은 P4-5.2에서 이어서 다루고, 실제 파라미터 갱신과 optimizer의 역할은 P4-7.1, P4-7.2에서 다시 연결합니다. 자동미분(automatic differentiation)의 일반 이론은 이 책의 현재 본편 범위 밖에 둡니다.
...
이 구분은 독자에게 매우 중요합니다. 많은 독자가 `역전파가 곧 업데이트`라고 오해하지만, 실제 업데이트는 아직 일어나지 않았습니다. 역전파는 `바꿔야 할 방향과 민감도`를 계산할 뿐입니다.
...
예를 들어 입력이 2이고 목표값이 5인데 현재 예측이 3이라면, 사람은 `출력이 모자라니 가중치를 키워야겠다`는 방향 감각을 잡을 수 있습니다. 하지만 여기까지는 가중치 하나짜리 아주 단순한 경우라서 가능한 직관입니다. 층이 많아지면 어느 중간 노드가 얼마나 책임이 있는지 사람이 눈으로 따지기 어렵습니다. 역전파는 이런 감각을 단순한 직관이 아니라, 모든 층에 대해 반복 가능한 계산으로 바꿉니다.

실제 다층 네트워크에서는 상황이 더 복잡합니다. 앞단 은닉층이 만든 값이 뒤층 점수에 영향을 주고, 그 뒤층 점수가 다시 손실을 바꾸기 때문입니다. 사람은 최종 오차를 보면 `마지막 가중치만 고치면 되지 않을까`라고 먼저 생각하기 쉽지만, 그렇게 하면 중간 층 책임을 놓치게 됩니다. 역전파는 바로 이 `중간 층도 결과에 책임이 있다`는 사실을 계산 절차로 바꾼 것입니다.
