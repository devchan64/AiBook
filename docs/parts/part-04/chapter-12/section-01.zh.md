...
这正是 k-NN(k-nearest neighbors) 的出发点。更准确地说，k-NN 不太像 `先写出一个公式的模型`，而更像 `先去寻找新输入 주변相似案例的模型`。
...
- 不同 distance function 的差异
- 为什么 scale 会改变结果
- 真正使用 k-NN 时，应该先检查什么

这些内容会在 `P4-12.2 距离与尺度` 和 `P4-12.3 使用 k-NN 时应先检查什么` 里继续处理。
...
所以，k-NN 可以被读成：`它不会单独解释一个新点，而是把这个点和 주변已经知道的案例拿来比较`。
...
不过，`近` 并不自动等于 `对`。只要用来计算近远的 기준改变，neighbor 本身就可能改变。这正是下一节要处理的点。
...
logistic regression 会先问：`怎样用一条公式或一条边界，把整个空间切得比较好？` 相比之下，k-NN 先问的是：`这个点 주변聚着什么样的案例？`

| 模型视角 | 中心问题 |
| --- | --- |
| logistic regression | 画什么 boundary 才能把 class 分得比较好？ |
| k-NN | 这个新点 주변的相似案例属于什么 class？ |
...
但团队还没有找到足够简单、足够稳定的规则，能直接说 `流失客户总是满足这种条件`。不过看已有记录时，又常常发现行为相似的客户，最后结果也比较像。这时，k-NN 不会单独解释新客户，而是会先去找 주변比较相似的老客户，把他们的 label 当作依据。
...
- k-NN 更像 `先参考 주변案例的模型`，而不是 `先写规则的模型`
...
同一个 query `(4.0, 4.2)` 还会继续连到后面两节。`P4-12.2` 会看：如果计算近远的规则改变，哪些 neighbor 会跟着改变；`P4-12.3` 会看：当这个 query 开始摇晃时，究竟该先重新检查什么。

## 本节要记住的视角

- k-NN 是通过把新输入和 주변已知案例比较来做判断的
- `query`、`neighbor`、`label`、`k` 在判断里各自承担不同角色
- `k` 不是单纯的数字，而是改变判断范围的把手
- k-NN 的 training 更接近 `准备比较案例`，而不是 `构造公式`

## 检查清单

- 能不能把 k-NN 解释成 `把附近案例聚起来再判断的方法`？
- 能不能说明 `k` 太小和太大时的差别？
- 是否理解了：在 k-NN 里，prediction time 的比较成本可能会变大？
