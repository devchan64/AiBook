...
所以，本节的目标不是得出一句模糊的结论 `k-NN 要小心用`。更准确地说，是要让读者能自己说出：`prediction 一旦开始摇晃，应该先从哪里重新打开检查`

## 本节要记住的视角

- 在局部相似性很重要的问题里，k-NN 可以是很好的第一比较候选
- 结果开始摇晃时，通常应该先重看 `distance rule`、`k`、`data representation`，再去谈模型名字本身
- review query 不是固定原因的证据，而是告诉你 `哪里必须重查` 的信号
