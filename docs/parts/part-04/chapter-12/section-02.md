...
여기서도 비교 기록 구조를 같이 고정해 둡니다. 거리와 스케일 절은 단순 전처리 절이 아니라, `같은 query가 왜 다른 이웃을 갖게 되었는가`를 기록하는 절이기 때문입니다. 따라서 점수 변화보다 먼저 `어떤 사례가 이웃 목록에서 들어오고 나갔는가`, `어떤 칼럼이 거리를 지배했는가`, `조정 후 review 대상이 줄었는가`를 함께 적어 둡니다. 이 변화도 먼저 `표현을 바꾸자 해석이 어떻게 달라졌는가`를 보여 주는 신호로 읽고, 전처리 하나만으로 원인 설명이 완성됐다고 보지는 않습니다.
...
대표적으로 많이 보는 방식은 표준화(standardization)입니다. 여기서는 다음 기준만 잡으면 됩니다.
...
실무에서는 모델 내부를 수식으로 먼저 보기보다, 데이터 칼럼과 단위를 먼저 봐야 합니다.
...
이 결과는 점수보다 `사례 이동 기록`으로 먼저 읽습니다.
...
즉, 스케일 조정은 `점수 개선`보다 먼저 `해석이 바뀐 사례를 어떻게 기록할 것인가`의 문제이기도 합니다.
