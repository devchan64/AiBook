...
- 오픈체크리스트의 중심축 문장인 "거리와 스케일이 왜 결과를 바꾸는지 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
이 비교를 통해 k-NN은 `가까운 사례를 가져오는 모델`이 아니라 `표현과 입력 변화에 민감한 비교 규칙`으로 읽힙니다. 중요한 것은 k 값을 외우는 것이 아니라, 같은 query라도 표현 방식과 특징값이 조금 달라지면 어떤 이웃이 들어오고 빠지는지 설명할 수 있게 되는 데 있습니다. 반복 변화 실습의 학습효과도 `예측이 바뀌었다`보다 `무엇을 바꾸자 비교 기준이 다시 섞였는가`를 말할 수 있을 때 생깁니다.
