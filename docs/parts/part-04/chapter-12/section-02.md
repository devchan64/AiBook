...
처음 읽을 때는 장기 의존성을 Part 4 본류의 `상태 보존 한계 손잡이`로만 잡고, 그 다음에 attention이 어떤 구조 전환을 가져오는지 같이 보면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| RNN / LSTM / GRU | 순차 상태를 어떻게 넘기고 더 잘 관리할 수 있는가? | P4-12.1 |
| 장기 의존성 | 그 상태 보존이 왜 먼 단서 앞에서 흔들리기 쉬운가? | P4-12.2 |
| attention | 필요한 위치를 현재 시점에서 다시 직접 참고할 수 있는가? | P4-13.1 |
...
이 설명은 바로 앞 절과도 이어집니다. P4-12.1에서 RNN을 `상태를 넘기는 구조`로 봤다면, 여기서는 LSTM과 GRU를 `그 상태를 더 잘 보존하게 만든 구조`로 읽으면 됩니다.
...
출력에서는 gap이 커질수록 `state_support`가 약해지고 `direct_match_score`는 유지되는지부터 보면 됩니다.
...
| --- | --- | --- |
| `gap=1 -> 3 -> 6`으로 갈수록 `state_support`가 `0.518 -> 0.269 -> 0.1`로 줄어든다 | 순차 상태에만 기대면 오래전 핵심 단서가 빠르게 약해진다는 뜻 | decay를 더 강하게 하거나 gap을 더 늘리면 상태 약화가 얼마나 빨라지는지 더 분명해집니다 |
| `state_decision`은 `keeps exclusion`에서 `loses exclusion`으로 바뀐다 | 같은 규칙과 같은 질문이어도 중간 간격이 길어지면 상태 기반 판단이 핵심 예외를 놓칠 수 있다는 뜻 | 중간 설명 줄을 더 추가하거나 제거하면 어느 지점에서 판단이 무너지는지 달라집니다 |
| `direct_match_score = 3`과 `direct_decision = keeps exclusion`이 유지된다 | 필요한 줄을 직접 다시 찾는 방식은 간격이 길어져도 관련 규칙을 다시 끌어올 수 있다는 뜻 | 질문 표현이나 규칙 문장을 바꾸면 직접 재참조가 얼마나 안정적으로 유지되는지 비교할 수 있습니다 |

즉, 같은 규칙과 같은 질문이라도 둘 사이 간격이 길어지면 순차 상태 쪽은 핵심 예외 단서를 잃기 쉬워지고, 직접 다시 찾는 쪽은 같은 규칙 줄을 더 안정적으로 유지합니다.

이 예제도 결과를 한 번 읽고 끝내기보다, 어떤 값을 바꿔 보면 `장기 의존성` 문제가 더 선명해지는지 바로 이어서 확인하는 편이 좋습니다.

| 먼저 보인 출력 신호 | 지금 바로 해 볼 변화 | 아직 이 예제만으로 서두르지 않을 결론 |
| --- | --- | --- |
| gap이 커질수록 `state_support`가 빠르게 줄어든다 | gap을 더 늘리거나 decay를 더 키워 오래전 단서 약화가 얼마나 빨라지는지 본다 | 모든 순차 모델이 항상 같은 속도로 무너진다고 단정하지 않는다 |
| `state_decision`이 예외 규칙을 잃는다 | 중간 줄 수를 줄이거나 규칙 단서를 더 강하게 만들어 어느 지점에서 판단이 유지되는지 본다 | 장난감 상태 예제 하나로 실제 LSTM·GRU 성능 전체를 결론내리지 않는다 |
| direct 쪽은 같은 규칙 줄을 계속 집어온다 | 질문 표현을 바꾸거나 규칙 문장을 더 멀리 보내 재참조가 얼마나 안정적인지 본다 | 직접 재참조가 곧 완전한 이해를 자동으로 보장한다고 단정하지 않는다 |

이 장난감 코드는 attention 자체를 구현한 것은 아니지만, 순차 상태 쪽이 `앞 단서를 상태 안에 계속 남겨 둘 수 있는가`를 보여 주고 direct reference 쪽이 `현재 질문이 필요할 때 앞 단서를 다시 집어 올 수 있는가`를 보여 준다는 연결은 충분히 드러납니다. 즉, 장기 의존성 문제를 오래전 단서의 `보존` 문제로 보면 RNN/LSTM/GRU 쪽과 연결되고, 필요한 단서의 `재참조` 문제로 보면 attention 쪽과 연결됩니다. 이 구분이 잡혀야 다음 절 P4-13.1에서 attention을 `가중치 계산 공식`이 아니라 `필요한 위치를 직접 다시 보는 발상`으로 자연스럽게 읽을 수 있습니다.
...
바로 앞의 P4-12.1에서 `순차 상태를 이어받는 구조`를 보았다면, 이제는 그 구조가 어디서 막히는지를 이해해야 합니다. 단순히 구조 이름을 외우는 대신, `무엇이 문제였기 때문에 다음 구조가 나왔는가`를 보여 주어야 attention과 Transformer가 왜 필요했는지 더 자연스럽게 읽을 수 있습니다.
...
- 장기 의존성은 오래전 정보가 중요한데도 충분히 유지되지 않는 문제이며, 기본 RNN에서는 시간이 길어질수록 오래전 단서가 약해지기 쉽습니다. LSTM과 GRU는 이를 더 잘 다루려는 발전 구조이고, attention은 여기에 더 직접적으로 응답하는 다음 발상입니다.
