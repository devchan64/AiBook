...
이 절에서 먼저 닫아야 하는 핵심은 `순차 상태를 넘기는 구조만으로는 먼 앞 단서를 현재 판단까지 안정적으로 들고 오기 어렵다`는 점입니다. 즉, 장기 의존성은 뒤 구조 이름으로 넘어가기 위한 예고가 아니라, RNN 계열이 실제로 어디에서 막히는지를 보여 주는 현재 장의 핵심 문제로 읽어야 합니다.
...
같은 전환을 `무엇이 부족했는가`와 `그래서 무엇을 바꿨는가`로 다시 보면 다음처럼 읽을 수 있습니다.
...
커리큘럼 관점에서도 이 절은 RNN 장의 핵심 마무리입니다. 바로 앞의 P4-12.1에서 `순차 상태를 이어받는 구조`를 보았다면, 이제는 그 구조가 어디서 막히는지를 이해해야 합니다. 단순히 구조 이름을 외우는 대신, `무엇이 문제였기 때문에 다음 구조가 나왔는가`를 보여 주어야 attention과 Transformer가 왜 필요했는지 더 자연스럽게 읽을 수 있습니다.

즉, 여기서 RNN/LSTM/GRU는 `Transformer 이전의 준비 운동`으로 남아서는 안 됩니다. 현재 장 안에서 이미 `상태 보존` 계열 구조가 무엇을 해결했고 어디서 한계에 부딪혔는지를 닫아 두어야, 다음 장의 attention이 `새 이름 소개`가 아니라 실제 문제 전환으로 읽힙니다.
