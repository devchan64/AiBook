...
P4-12.1에서는 k-NN(k-nearest neighbors)을 `가까운 사례를 보고 판단하는 모델`로 보았습니다. 이제 그 설명에서 가장 중요한 단어 하나를 꺼내야 합니다.
...
이 절은 k-NN의 기본 직관을 다시 처음부터 풀지 않습니다. `가까운 사례를 보고 판단한다`는 핵심 감각은 P4-12.1과 [개념사전](../../../reference/concept-glossary.md)을 기준으로 다시 연결하고, 여기서는 그 가까움이 어떻게 계산되고 왜 전처리와 연결되는지에만 집중합니다.
...
표준화(standardization)와 전처리(preprocessing) 맥락은 P4-7.2에서 다시 연결하고, 거리 기반 판단이 실제 모델에 어떻게 이어지는지는 뒤의 P4-13 SVM과 P4-18 차원 축소에서 다시 봅니다. 반면 Minkowski 일반식 유도와 거리 집중(distance concentration)의 엄밀한 이론은 이 책의 현재 본편 범위 밖에 둡니다.
...
| --- | --- |
| P4-13 SVM | 경계 이전에 거리와 간격을 어떻게 읽는가 |
| P4-14~16 트리/앙상블 | 왜 어떤 모델은 스케일에 덜 민감한가 |
| P4-18 차원 축소 | 가까움과 표현 공간이 함께 바뀔 수 있음 |
| Part 5 임베딩, 벡터 검색 | 텍스트/의미 공간에서도 거리 정의가 핵심이 됨 |

즉, 12.2는 단지 k-NN을 위한 절이 아니라, `거리 기반 사고`를 처음 명시적으로 소개하는 절입니다.

여기서도 비교 기록 구조를 같이 고정해 둡니다. 거리와 스케일 절은 단순 전처리 절이 아니라, `같은 query가 왜 다른 이웃을 갖게 되었는가`를 기록하는 절이기 때문입니다. 따라서 점수 변화보다 `어떤 사례가 이웃 목록에서 들어오고 나갔는가`, `어떤 칼럼이 거리를 지배했는가`, `조정 후 review 대상이 줄었는가`를 함께 적어 둡니다. 이 변화도 `표현을 바꾸자 해석이 어떻게 달라졌는가`를 보여 주는 신호로 읽고, 전처리 하나만으로 원인 설명이 완성됐다고 보지는 않습니다.

이 비교도 같은 기준을 유지해야 의미가 있습니다. 같은 query, 같은 거리 함수, 같은 `k`, 같은 대표 이웃 사례를 두고 스케일 조정 전후를 봐야 `무엇이 표현 변경 때문이고`, `무엇이 거리 규칙 자체의 차이 때문인지`를 덜 섞고 읽을 수 있습니다.

| 같이 남길 기록 | 왜 필요한가 |
| --- | --- |
| 스케일 조정 전 이웃 목록 | 원래 무엇이 거리를 지배했는지 보기 위해서입니다. |
| 스케일 조정 후 이웃 목록 | 표현 변경 뒤 어떤 사례가 새로 가까워졌는지 보기 위해서입니다. |
| 이동한 query 사례 | 전처리 변화가 실제로 어느 사례 해석을 바꿨는지 추적하기 위해서입니다. |
| 다음 검토 질문 | 거리 규칙을 바꿀지, 특징을 더 다듬을지 정하기 위해서입니다. |

이 기록이 있으면 `전처리 후 예측이 바뀌었다`, `이웃이 달라졌다`, `review 대상이 이동했다`를 따로 보지 않고, 같은 query를 기준으로 다시 설명할 수 있습니다.

### 언제 스케일 문제를 먼저 의심해야 하는가

거리 기반 모델에서는 성능이 이상할 때 모델 구조보다 먼저 `무슨 축이 거리를 거의 혼자 결정하고 있는가`를 의심해야 하는 경우가 많습니다.

| 보이는 신호 | 먼저 의심할 것 | 이유 |
| --- | --- | --- |
| 어떤 칼럼만 숫자 범위가 매우 크다 | 스케일 지배 | 큰 숫자 축이 거리를 거의 독점할 수 있기 때문 |
| 스케일 조정 전후 이웃이 크게 바뀐다 | 표현 의존성 | 가까움의 정의가 표현 변화에 민감하다는 뜻이기 때문 |
| 거리 기반 모델인데 전처리 설명이 없다 | 비교 기준 누락 | 결과를 재현하거나 해석하기 어렵기 때문 |
| 작은 범위 칼럼이 중요한데 예측에 잘 반영되지 않는다 | 큰 축에 의한 가림 | 실제 중요한 신호가 거리 계산에서 묻힐 수 있기 때문 |
| query별로 review 대상이 이상하게 몰린다 | 거리 규칙 또는 스케일 왜곡 | 같은 유형의 사례가 특정 축 때문에 잘못 묶일 수 있기 때문 |
...
모든 거리 공식을 외우기보다, `같은 데이터라도 가까움의 기준이 여러 개일 수 있다`는 감각이 중요합니다.
...
대표적으로 많이 보는 방식은 표준화(standardization)입니다. 여기서는 표준화가 무엇을 하는지만 먼저 분명히 잡습니다.
...
문제는 이 칼럼들의 단위가 크게 다르다는 점입니다. 연 소득은 수백만에서 수천만 단위 숫자이고, 연체 횟수는 0회에서 몇 회 수준입니다. 이 상태로 k-NN 거리를 계산하면, 연체 횟수는 실제로 중요해도 소득 차이에 묻혀 버릴 수 있습니다. 그러면 모델은 `누가 비슷하게 늦게 갚았는가`보다 `누가 소득 숫자가 비슷한가`를 더 강하게 보게 됩니다.

이 장면에서 거리와 스케일은 전처리 바깥의 사소한 선택이 아니라, 모델의 판단 규칙 일부가 됩니다. 유클리드 거리와 맨해튼 거리의 차이도 이웃 순서를 바꿀 수 있고, 표준화 전후 비교는 어떤 축이 거리를 지배하고 있는지 드러냅니다. 즉, 데이터는 그대로여도 표현 방식이 바뀌면 `가까운 사람` 자체가 달라질 수 있습니다.

확인 가능한 결과는 원본 거리와 스케일 조정 후 거리를 나란히 볼 때 드러납니다. 스케일 조정 전에는 위험 고객이 더 가깝게 보였는데, 조정 후에는 안전 고객이 더 가깝게 보인다면, 그 차이는 모델이 바뀐 것이 아니라 `가까움의 계산 기준`이 바뀐 결과라고 설명할 수 있습니다. 따라서 이 차이는 우선 `어떤 query를 다시 검토해야 하는가`를 알려 주는 변화 신호이며, 특정 칼럼이 곧 원인이라고 자동으로 단정하는 근거는 아닙니다.

## 사례 및 예시

### 실무에서는 어떤 신호를 보면 스케일 문제를 의심해야 하는가

실무에서는 모델 내부를 수식으로 보기보다, 데이터 칼럼과 단위를 먼저 봐야 합니다.
...
이 결과는 점수보다 `사례 이동 기록`으로 읽습니다.

| 비교 항목 | 조정 전 | 조정 후 | 읽기 |
| --- | --- | --- | --- |
| 가장 가까운 이웃 그룹 | risky | safe | 거리를 지배하던 축이 바뀌었다 |
| review 필요 여부 | 높음 | 다시 확인 필요 | 표현 변경이 판단을 흔들었다 |
| 다음 질문 | 소득 축이 너무 강한가 | 다른 거리 함수도 볼 것인가 | 전처리와 거리 규칙을 함께 검토 |

즉, 스케일 조정은 `점수 개선`보다 `해석이 바뀐 사례를 어떻게 기록할 것인가`의 문제이기도 합니다.

여기서 반드시 잡아야 할 문장은 다음입니다.
...
- 거리 함수가 모델 밖의 장식이 아니라 이웃 순서를 정하는 규칙이라는 점을 설명할 수 있는가?
- 스케일 조정 전후에 어떤 이웃이 들어오고 나갔는지 같은 query 기준으로 비교하고 있는가?
- 스케일 조정 후 성능 변화가 보여도 그것만으로 원인을 확정하지 않고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 같은 query인데 전처리나 거리 함수만 바뀌어 결과가 달라졌을 때, 거리 규칙이 모델 일부라는 관점을 먼저 꺼냅니다.
- 특정 큰 축이 판단을 거의 독점하는 것처럼 보일 때, 스케일 차이가 거리를 지배하고 있는지 다시 봅니다.
- 뒤에서 SVM, 임베딩, 벡터 검색처럼 거리와 표현이 함께 중요한 주제를 읽을 때, 이 절의 전처리-거리 결합 감각을 재사용합니다.
