...
여기서도 비교 기록 구조를 같이 고정해 두는 편이 좋습니다. 거리와 스케일 절은 단순 전처리 절이 아니라, `같은 query가 왜 다른 이웃을 갖게 되었는가`를 기록하는 절이기 때문입니다. 따라서 점수 변화보다 먼저 `어떤 사례가 이웃 목록에서 들어오고 나갔는가`, `어떤 칼럼이 거리를 지배했는가`, `조정 후 review 대상이 줄었는가`를 함께 적어 두는 편이 안전합니다. 이 변화도 먼저 `표현을 바꾸자 해석이 어떻게 달라졌는가`를 보여 주는 신호로 읽고, 전처리 하나만으로 원인 설명이 완성됐다고 보지는 않는 편이 맞습니다.
...
이 결과는 점수보다 `사례 이동 기록`으로 먼저 읽는 편이 좋습니다.
