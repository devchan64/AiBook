...
커리큘럼 관점에서도 이 절은 RNN 장의 핵심 마무리입니다. 바로 앞의 P4-12.1에서 `순차 상태를 이어받는 구조`를 보았다면, 이제는 그 구조가 어디서 막히는지를 이해해야 합니다. 단순히 구조 이름을 외우는 대신, `무엇이 문제였기 때문에 다음 구조가 나왔는가`를 보여 주어야 바로 뒤의 P4-13 attention, P4-14 Transformer, 그리고 Part 5의 P5-3 LLM 구조 설명이 매끄럽습니다.
