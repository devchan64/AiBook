...
- 순차 데이터를 읽을 때는 같은 항목이라도 앞뒤 순서와 누적 문맥에 따라 해석이 달라지는지를 먼저 봐야 하며, RNN은 이전 상태를 이어받아 현재 입력을 처리하려는 구조이고 LSTM과 GRU는 오래 기억하기 어려운 문제를 더 잘 다루려는 발전 구조로서 RNN 계열은 Transformer 이전 순차 모델링의 핵심 기준점입니다.
