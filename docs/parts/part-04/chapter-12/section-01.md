...
즉, 여기서 독자가 먼저 봐야 하는 차이는 `입력 하나를 바로 읽는가`보다 `현재 입력에 앞 흐름을 함께 붙여 읽는가`에 더 가깝습니다.
...
즉, 이 절의 최소 기억점은 `RNN 계열은 현재 입력을 바로 판정하는 구조가 아니라, 이전 단계의 흔적을 상태로 들고 와 현재 입력과 함께 해석하는 구조`라는 한 문장입니다.
...
따라서 이 절의 책임도 `나중에 attention이 나온다`는 예고에 머물지 않습니다. 현재 절 안에서 이미 `순차 데이터는 왜 상태를 이어받는 구조를 요구하는가`, `basic RNN과 LSTM/GRU는 무엇을 해결하려 했는가`를 끝내 설명해야 하고, 다음 절은 그 답이 어디서 막히는지를 보여 주는 단계로만 이어지면 충분합니다.
