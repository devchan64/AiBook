...
- 거리(distance) 함수의 차이
- 스케일(scale)이 결과를 바꾸는 이유
- k-NN을 쓸 때 무엇을 먼저 점검할지에 대한 적용 가이드

그 내용은 `P4-12.2 거리(distance)와 스케일(scale)`, `P4-12.3 k-NN을 사용할 때 무엇을 먼저 점검할까`에서 이어서 다룹니다.
...
이 query `(4.0, 4.2)`는 다음 절들과도 이어집니다. `P4-12.2`에서는 `가까움을 계산하는 규칙`이 바뀌면 어떤 이웃이 달라질 수 있는지 보고, `P4-12.3`에서는 같은 query가 흔들릴 때 무엇을 먼저 다시 점검할지 읽게 됩니다.

## 이 절에서 기억할 관점

- k-NN은 새 입력을 주변의 이미 알려진 사례와 비교해 판단합니다.
- `query`, `neighbor`, `label`, `k`는 판단 안에서 서로 다른 역할을 맡습니다.
- `k`는 단순 숫자가 아니라 판단 범위를 조절하는 손잡이입니다.
- k-NN의 학습은 `식을 만드는 일`보다 `비교 기준 사례를 준비하는 일`에 가깝습니다.

## 짧은 점검

- k-NN을 `가까운 사례를 모아 판단하는 방식`이라고 설명할 수 있는가?
- `k`가 너무 작을 때와 너무 클 때의 차이를 설명할 수 있는가?
- k-NN에서 예측 시 비교 비용이 커질 수 있다는 점을 이해하고 있는가?
