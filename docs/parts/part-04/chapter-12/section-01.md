...
k-NN은 독자에게 중요한 전환을 보여 줍니다. 앞에서 본 선형회귀(linear regression)나 로지스틱 회귀는 `모델이 학습된 식`을 앞세웠습니다. 반면 k-NN은 `새 입력 주변에 어떤 사례가 모여 있는가`를 먼저 봅니다.
...
여기에 한 가지를 더 붙이면 k-NN 절이 지금까지 정리한 비교 기록 구조와 자연스럽게 이어집니다. k-NN을 첫 비교 후보로 쓸 때는 `정답 예측`만 남기는 것이 아니라, `어떤 이웃을 근거로 판단했는가`, `경계 근처라 review가 필요한 query는 무엇인가`, `baseline보다 실제로 무엇이 달라졌는가`를 함께 적어 두는 편이 좋습니다. 이때 경계 근처 query는 우선 검토 우선순위를 올리는 신호로 읽고, 이웃 몇 개가 그렇게 나왔다는 사실만으로 원인 설명까지 끝난 것으로 보지 않는 편이 안전합니다.

이 기록도 같은 비교 프레임 위에 두는 편이 좋습니다. 같은 query, 같은 거리 규칙, 같은 `k`, 같은 대표 이웃 사례를 두고 비교해야 `예측이 왜 바뀌었는가`, `k를 바꾼 영향인가`, `거리 해석이 흔들린 것인가`를 덜 섞고 읽을 수 있습니다.
...
여기서 독자가 먼저 잡아야 할 말은 `non-parametric`입니다. 입문 단계에서는 이를 엄밀하게 정의하기보다 다음처럼 읽으면 충분합니다.
