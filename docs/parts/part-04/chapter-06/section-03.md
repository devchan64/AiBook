...
P4-6.1과 P4-6.2에서는 모델 평가 지표(metric)를 봤습니다. 이제 시선을 조금 바깥으로 돌립니다. 모델이 잘 맞는 것과 서비스가 잘 운영되는 것은 같은 말이 아닙니다. 이 차이를 이해하려면 SRE(site reliability engineering)에서 `metric`이라는 말을 어떻게 쓰는지 보는 것이 도움이 됩니다.
...
독자에게는 `같은 서비스인데 질문이 둘로 나뉜다`는 점이 가장 중요합니다. 챗봇 하나만 놓고 봐도 이렇게 읽을 수 있습니다.
...
Google SRE Book은 SLI, SLO, SLA를 구분해서 정의합니다. 이 구분은 독자에게도 중요합니다. 세 용어는 비슷해 보이지만 질문이 다릅니다.
...
에러 버짓은 독자에게 추상적으로 들리기 쉽습니다. 하지만 실제로는 `지금 공격적으로 바꿔도 되는가, 잠시 안정화에 집중해야 하는가`를 판단하는 기준으로 읽을 수 있습니다.
...
이 도식은 운영 지표를 외울 때 `무엇이 느린가`, `수요가 얼마나 큰가`, `어디서 실패하는가`, `한계에 얼마나 가까운가`라는 네 질문으로 나누어 읽으면 된다는 점을 보여 줍니다.
...
확인 가능한 결과는 두 종류의 숫자를 나란히 놓을 때 드러납니다. 오프라인 평가 점수와 함께 p95 latency, timeout rate, availability를 보면, 왜 `좋은 모델`이 바로 `좋은 서비스`를 뜻하지 않는지 설명할 수 있습니다.
...
- 에러 버짓은 정의만 읽으면 추상적이므로, 성공/실패 기록에서 허용 실패율과 실제 실패율을 직접 계산해 보는 편이 이해에 도움이 된다

입력(input):

- 요청 성공/실패 기록 `requests`
- 목표 SLO `slo_target`
...
- P4-7.1 특징 선택(feature selection), P4-7.2 전처리(preprocessing)로 가면 모델 metric을 바꾸는 입력 설계가 다시 중요해집니다. 이 절은 그와 별도로 운영 metric은 또 다른 층위에 있다는 점을 붙잡아 둡니다.
- P4-8.1 모델 선택(model selection), P4-8.2 기준 모델(baseline), P4-9.1 하이퍼파라미터(hyperparameter), P4-9.2 튜닝(tuning)과 검증 비용에서는 모델 성능 수치를 어떻게 비교할지 보게 됩니다. 이때도 서비스 운영의 latency, availability 문제와는 다른 숫자라는 구분이 도움이 됩니다.
- P4-10장부터 P4-18장까지 알고리즘을 배우면서 분류·회귀·군집화 지표가 계속 다시 등장합니다. 이 절은 그 알고리즘들이 실제 서비스에 올라가면 운영 metric이 별도로 필요하다는 사실을 미리 연결합니다.
- P4-19 강화학습 알고리즘으로 가면 보상(reward), 누적 성과처럼 또 다른 평가 관점이 열립니다. 따라서 이 절은 `문제마다 읽는 숫자가 달라진다`는 감각을 더 넓게 준비하는 역할도 합니다.
