...
这一节说明 `评价指标(metric)`、`准确率(accuracy)`、`精确率(precision)`、`召回率(recall)`、`F1` 的基本作用。下一节会沿着这个抓手继续判断当前语境，而 `到底把哪一类错误当成更重要` 的标准，会通过本节和 [概念词汇表](/AiBook/en/reference/concept-glossary/) 再次接回。
...
| 最先看到的信号 | 这个信号意味着什么 | 立刻该做的下一步 |
| --- | --- | --- |
| FN 多、FP 多这种错误方向 | 说明你得先重新定义，到底哪种失败更痛 | 回去再看 confusion matrix 和代表错误案例，再检查 threshold 或成本标准 |
| accuracy 很高，但 recall 或 precision 有一边明显很低 | 说明总分看起来不错，但重要错误方向可能被藏住了 | 不要只靠 accuracy 下结论，要重新看失败集中在哪个类、哪种成本上 |

- 评价指标(metric)在用数字概括 model 结果的同时，也会暴露出哪种错误被看得更重要。
- accuracy 是有用的起点，但不是所有场景的代表指标。
- precision 问的是：`被说成正类的里面，有多少是真的？`
- recall 问的是：`真正的正类里面，有多少没被漏掉？`
- F1 score 是把 precision 和 recall 一起看的概括指标。
- 即使 accuracy 一样，只要 precision 和 recall 不同，对 model 的解释和选择就可能不同。

这一节与其说是在背分类指标名字，不如说是在固定评价的阅读顺序。

| 这一节先做什么 | 紧接着该做什么 | 后面还要再比较什么 |
| --- | --- | --- |
| 先通过 confusion matrix 和错误案例看清失败方向 | 再去读 precision、recall、F1 是怎样概括这种失败的 | 和 baseline 比较，确认这种变化到底是不是真的改进 |
...
- 当你想只用 accuracy 来选 model 时，要先想起评价指标视角：真正该先问的是哪一类错误更重要。
- 当你需要重新说明 precision、recall、F1 各自概括的是哪种失败方向时，就该回到这一节。
- 当 metric 必须被读成选择标准，而不是单纯记分牌时，这一节会成为标准。
