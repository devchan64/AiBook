...
- 정확도 하나만으로 모델을 고르고 싶은 순간에 어떤 오류를 더 중요하게 보는지 먼저 묻는 평가 지표 관점을 떠올립니다.
- 정밀도, 재현율, F1이 각각 어떤 실패 방향을 요약하는지 다시 설명해야 할 때 이 절로 돌아옵니다.
- 지표를 점수판이 아니라 선택 기준으로 읽어야 할 때 이 절이 기준이 됩니다.
