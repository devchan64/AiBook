...
평가 지표는 모델이 얼마나 잘 맞는지 숫자로 보여 주는 도구입니다. 하지만 독자에게 더 중요한 점은, 지표가 단순히 점수판이 아니라 `무엇을 중요하게 보겠다는 약속`이라는 사실입니다. 같은 모델이라도 어떤 지표를 보느냐에 따라 좋아 보일 수도 있고, 위험해 보일 수도 있습니다.

## 이 절의 범위

이 절은 평가 지표의 역할을 설명하는 도입 절입니다. 여기서는 정확도(accuracy), 정밀도(precision), 재현율(recall), F1 점수(F1 score)를 처음 만나는 수준으로 연결합니다. ROC-AUC, PR-AUC, 로그 손실(log loss), 캘리브레이션(calibration), 실루엣(silhouette) 같은 지표는 P4-6.4 보충학습에서 따로 회수하고, 회귀 지표와 군집화 관점은 P4-6.2에서 이어서 봅니다.

즉, 본편의 책임은 `무엇을 먼저 중요하게 볼 것인가`를 정하는 데 있습니다. 확률 점수의 세밀한 해석, reliability diagram, Brier score, threshold 세부 조정처럼 한 단계 더 섬세한 읽기는 P4-6.4와 뒤의 P4-15.3으로 넘기고, 여기서는 지표가 왜 하나가 아닌지부터 분명히 잡습니다.

또한 여기서 지표를 읽는 기본 태도도 함께 고정합니다. 분류(classification)에서는 먼저 혼동 행렬(confusion matrix)과 대표 오류 사례를 보고, 뒤의 P4-8.2에서는 baseline과 비교해 그 오류 구조 개선이 실제로 의미 있는지 다시 읽습니다. 즉, Part 4의 평가는 `숫자 하나`보다 `어디서 틀렸는가`와 `무엇과 비교해 나아졌는가`를 먼저 보는 흐름으로 이어집니다.
...
정밀도와 재현율을 이해하려면 먼저 `혼동 행렬(confusion matrix)`을 가볍게 봐야 합니다. Google 용어집은 혼동 행렬을 `모델이 맞춘 것과 틀린 것을 표로 요약한 표`로 설명합니다.
...
여기서 평가 지표는 `무엇을 더 아프게 보는가`를 드러내는 기준이 됩니다. 재검사 비용이 큰 현장이라면 정밀도를 더 주의 깊게 봐야 하고, 불량 놓침이 더 위험하다면 재현율을 더 먼저 봐야 합니다. 같은 혼동 행렬도 어떤 오류 비용을 먼저 읽느냐에 따라 판단이 달라집니다.
...
Google 용어집은 F1 점수를 정밀도와 재현율을 함께 쓰는 대표적인 요약 지표로 설명합니다. 이 절에서는 다음 한 문장으로 잡습니다.
...
여기서는 지표 이름을 많이 외우기보다, 분류 평가를 다음 순서로 읽는 편이 더 안전합니다.
...
| --- | --- | --- |
| FN이 많다, FP가 많다 같은 오류 방향 | 어떤 실패를 더 아프게 보는지부터 다시 정해야 한다는 뜻 | 혼동 행렬과 대표 오류 사례를 다시 보고 threshold나 비용 기준을 점검한다 |
| accuracy는 높은데 recall이나 precision 한쪽이 크게 낮다 | 전체 점수는 그럴듯해도 중요한 오류 방향이 가려졌을 수 있다는 뜻 | 정확도 하나로 결론내리지 말고, 어떤 클래스와 어떤 비용에서 실패가 몰렸는지 다시 본다 |
...
이 절은 분류 지표 이름을 외우는 절이라기보다, 평가 순서를 고정하는 절입니다.

| 지금 절에서 먼저 하는 일 | 바로 다음에 해야 할 일 | 뒤에서 다시 비교할 일 |
| --- | --- | --- |
| 혼동 행렬과 오류 사례로 실패 방향을 본다 | 정밀도, 재현율, F1이 그 실패를 어떻게 요약하는지 읽는다 | baseline과 비교해 그 변화가 정말 개선인지 확인한다 |
