...
- 새 이메일 하나를 넣고
- 현재 모델 점수를 계산해
- 차단, 통과, 검토 같은 후속 정책으로 넘깁니다

즉, 실행 단계의 새 이메일이 곧바로 학습 데이터를 추가하지는 않습니다. 사람은 운영 화면에서 `새 메일을 판별했으니 모델도 바로 더 배웠겠지`라고 느낄 수 있지만, 실제로 보는 `스팸 점수`와 `차단 여부`는 inference 결과입니다. 그 뒤에서 파라미터가 자동으로 다시 학습되는 것은 아닙니다.

### 사례 2. 이미지 분류 데모

학습 단계에서는 수천 장의 이미지와 라벨을 사용합니다. 실행 단계에서는 사용자가 올린 한 장의 이미지를 읽고 현재 모델이 가장 그럴듯한 클래스를 반환합니다. 사람은 데모 화면에서 분류 결과가 바로 바뀌는 것을 보면 `방금 이 이미지를 보고 모델이 또 배운 것 아닐까`라고 느끼기 쉽습니다. 하지만 실제로 그 순간 일어나는 일은 현재 파라미터로 결과를 계산하는 inference입니다. 즉, 사람이 데모에서 보는 것은 `지금 이 이미지가 무엇으로 분류되었는가`이고, 그 순간 모델이 새로 똑똑해지는 것은 아닙니다.

이때 사용자는 결과만 보지만, 학습 단계에서는 손실, gradient, optimizer가 모두 뒤에서 작동했습니다. 이 사례는 `학습은 모델을 바꾸는 과정`, `실행은 바뀐 모델을 쓰는 과정`이라는 차이를 보여 줍니다.

### 사례 3. LLM 채팅

채팅 서비스에서도 사용자 입장에서는 단지 답변이 생성되는 것처럼 보이지만, 일반적인 서비스 사용은 학습이 아니라 inference에 가깝습니다. 사람은 대화를 이어 가며 답변이 조금씩 달라지는 것을 보면 `지금 이 대화 자체가 모델을 곧바로 다시 학습시키는 것 아닐까`라고 생각하기 쉽습니다. 하지만 보통 서비스 사용 자체는 파라미터 업데이트가 아니라, 이미 학습된 파라미터로 다음 토큰을 계산하는 현재 모델 실행 단계입니다.

이 구분은 Part 5에서 LLM을 다룰 때 더 중요해집니다.
