...
이 질문은 surprisingly 중요합니다. 초심자는 모델이 언제나 같은 방식으로 작동한다고 생각하기 쉽지만, 딥러닝에서는 `파라미터를 바꾸는 단계`와 `이미 배운 파라미터를 사용하는 단계`를 분리해서 보는 것이 매우 중요합니다.
...
여기서 중요한 점이 하나 더 있습니다. 학습과 실행 모두 순전파(forward pass)를 사용합니다. 그래서 초심자는 둘이 비슷해 보일 수 있습니다.
...
즉, 실행 단계의 새 이메일이 곧바로 학습 데이터를 추가하지는 않습니다.

### 사례 2. 이미지 분류 데모

학습 단계에서는 수천 장의 이미지와 라벨을 사용합니다. 실행 단계에서는 사용자가 올린 한 장의 이미지를 읽고 현재 모델이 가장 그럴듯한 클래스를 반환합니다.

이때 사용자는 결과만 보지만, 학습 단계에서는 손실, gradient, optimizer가 모두 뒤에서 작동했습니다.

### 사례 3. LLM 채팅

채팅 서비스에서도 사용자 입장에서는 단지 답변이 생성되는 것처럼 보이지만, 일반적인 서비스 사용은 학습이 아니라 inference에 가깝습니다. 모델은 이미 학습된 파라미터를 사용해 다음 토큰을 계산합니다.
