...
커리큘럼 관점에서 이 절이 필요한 이유도 분명합니다. 바로 앞의 P4-5.1, P4-5.2에서 gradient가 어떻게 계산되는지 보았다면, 이제 `계산이 되었을 때 언제 실제 업데이트가 일어나고 언제 단순 실행만 하는가`를 구분해야 합니다.
...
| --- | --- | --- |
| 학습은 업데이트까지 포함하고, 실행은 파라미터를 바꾸지 않는다 | 모드에 따라 어떤 층의 동작이 실제로 달라지는가 | optimizer 종류와 정규화 전략을 자세히 비교하는 일 |

- learning은 파라미터를 바꾸는 단계이고 inference는 바꾸지 않고 사용하는 단계이며, 딥러닝 학습은 순전파, 손실 계산, 역전파, 업데이트를 포함합니다.
