...
입문 단계에서는 모델이 낸 점수를 어느 값부터 양성으로 볼지 바꾸면, 오탐과 놓침의 균형도 함께 바뀐다고 이해하면 충분합니다.
...
입문 단계에서는 정답을 틀린 것도 문제지만, 틀린 답을 아주 높은 확률로 말한 것은 더 크게 벌주는 방식이라고 이해하면 충분합니다.
...
입문 단계에서는 비슷한 점수를 받은 사례들을 모아 봤을 때, 그 점수가 실제 발생 빈도와 비슷한가를 묻는 관점으로 잡으면 됩니다.
...
P4-15.3에서 threshold 조정과 calibration 이야기가 다시 나오므로, 여기서는 `확률처럼 보이는 점수도 다시 검토해야 한다`는 감각만 붙잡으면 충분합니다.

## silhouette는 정답 없이 군집 구조를 읽는 내부 기준이다

군집화에서는 정답 라벨이 없을 수 있다고 P4-6.2에서 봤습니다. silhouette score는 이런 장면에서 자주 등장하는 내부 평가 기준입니다.

입문 단계에서는 같은 군집 안에서는 가깝고, 다른 군집과는 멀수록 더 좋은 묶음처럼 읽는 기준이라고 이해하면 충분합니다.
