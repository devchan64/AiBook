...
학습 단계에서는 dropout이 켜져 있어 모델이 일부 경로를 무작위로 쉬게 할 수 있습니다. 사람은 학습 중 정확도와 검증 정확도가 다르면 먼저 `코드가 이상한 것 아닌가`라고 느끼기 쉽습니다. 하지만 이 상태에서 나온 학습 손실과, 평가 모드에서 측정한 검증 성능은 계산 환경 자체가 다르기 때문에 느낌이 달라질 수 있습니다. 이 차이는 버그가 아니라, `학습을 돕는 계산 환경`과 `성능을 재는 계산 환경`의 차이입니다. 이 사례는 숫자가 다르다는 사실보다 `왜 서로 다른 모드에서 잰 값인가`를 먼저 구분해야 한다는 점을 보여 줍니다.

### 사례 2. 실서비스 배포

서비스에서 사용자 요청을 처리할 때마다 dropout이 계속 무작위로 동작한다면, 같은 입력에도 출력이 불필요하게 흔들릴 수 있습니다. 사람은 `학습 때 잘 되던 설정이면 배포 때도 그대로 쓰면 되지 않을까`라고 생각하기 쉽지만, 일반적인 예측 서비스에서는 이런 동작을 원하지 않습니다. 예를 들어 같은 이미지나 같은 요청을 두 번 넣었는데 결과가 쓸데없이 달라지면 사용자는 모델을 불안정하다고 느끼게 됩니다. 따라서 배포에서는 보통 평가 모드에 가까운 설정을 사용합니다.

### 사례 3. 작은 배치 평가

모바일 추론이나 실시간 API처럼 한 번에 한 샘플씩 처리하는 상황에서는, batch normalization이 학습 중 배치 통계 방식으로 계속 동작하면 안정성이 떨어질 수 있습니다. 사람은 배치 크기가 작아져도 `어차피 같은 모델이니 비슷하게 동작하겠지`라고 느끼기 쉽지만, 실제로는 학습 중 통계와 배포 시 통계 조건이 달라지면 결과가 흔들릴 수 있습니다. 이때도 평가 모드 개념이 중요합니다. 이 사례는 모드 전환이 단순한 구현 디테일이 아니라 실제 추론 안정성과 연결된다는 점을 보여 줍니다.
