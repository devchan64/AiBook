...
- 학습 모드와 평가 모드는 같은 모델의 서로 다른 계산 상태이며, dropout과 batch normalization은 이 모드 차이에 특히 민감합니다. 검증, 테스트, 배포에서는 같은 입력에 더 안정적 출력이 나오는지 확인하기 위해 평가 모드가 중요하므로, 딥러닝 모델은 단순 함수라기보다 학습 상태를 가진 계산 시스템으로 읽을 필요가 있습니다.
