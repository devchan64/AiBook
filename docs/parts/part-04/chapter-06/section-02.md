...
- P4-7.1 특징 선택(feature selection)과 P4-7.2 전처리(preprocessing)에서는 `무엇을 입력으로 줄 것인가`가 다시 중요해집니다. 평가 지표는 같아 보여도, 입력 특징과 전처리가 바뀌면 성능 해석도 달라질 수 있다는 점을 연결해서 보게 됩니다.
- P4-8.1 모델 선택(model selection)과 P4-8.2 기준 모델(baseline)에서는 `어떤 모델을 먼저 비교할 것인가`라는 질문이 이어집니다. 이때 지금 절에서 정리한 문제 유형별 평가 기준이 모델 비교의 기준선이 됩니다.
- P4-9.1 하이퍼파라미터(hyperparameter)와 P4-9.2 튜닝(tuning)과 검증 비용에서는 `성능을 조금 더 올리는 시도`가 실제로 어떤 지표 개선으로 이어졌는지 읽는 일이 중요해집니다. 즉, 튜닝은 숫자를 바꾸는 작업이 아니라 평가 기준을 따라 움직이는 작업으로 다시 보게 됩니다.

- P4-10.1 선형회귀(linear regression)의 직관과 P4-10.2 선형회귀의 평가와 한계에서는 회귀(regression) 문제를 실제 알고리즘으로 처음 만나게 됩니다. 이때 MAE, MSE, RMSE, R² 같은 회귀 지표가 왜 필요한지 다시 구체적으로 연결됩니다.
- P4-11.1 로지스틱 회귀(logistic regression)의 직관과 P4-11.2 결정 경계(decision boundary)에서는 분류(classification)의 대표적인 형태를 보게 됩니다. 여기서 정확도, 정밀도, 재현율, F1이 다시 등장하고, 임계값(threshold)과 확률 해석도 더 중요해집니다.
- P4-12.1 k-NN의 직관과 P4-12.2 거리(distance)와 스케일(scale)에서는 같은 분류라도 `가까움`의 정의가 결과를 바꾼다는 점을 보게 됩니다. 즉, 같은 분류 지표라도 거리 정의와 스케일 처리에 따라 해석이 달라질 수 있음을 예상할 수 있습니다.
- P4-13.1 SVM의 직관과 P4-13.2 커널(kernel)의 입문적 의미에서는 분류 경계(boundary)를 더 직접적으로 다루게 됩니다. 따라서 분류 성능을 읽을 때도 단순 정확도보다 경계가 어떤 실수를 만들고 있는지를 다시 떠올리게 됩니다.
- P4-14.1 결정트리(decision tree), P4-14.2 트리의 과적합, P4-15.1 랜덤포레스트(random forest), P4-15.2 특징 중요도(feature importance), P4-16.1 그래디언트 부스팅(gradient boosting), P4-16.2 부스팅의 성능과 위험에서는 분류와 회귀가 모두 다시 등장할 수 있습니다. 즉, 알고리즘 이름이 바뀌어도 `이 문제가 분류인가 회귀인가`를 먼저 확인해야 평가 지표를 올바르게 고를 수 있다는 점을 다시 확인하게 됩니다.

- P4-17.1 클러스터링(clustering)의 직관과 P4-17.2 군집 결과를 해석할 때의 주의점에서는 이 절의 군집화(clustering) 설명이 직접 이어집니다. 특히 `군집 ID는 정답 이름이 아니다`, `군집 결과는 사람이 다시 해석해야 한다`는 관점이 여기서 본격적으로 중요해집니다.
- P4-18.1 차원 축소(dimensionality reduction)와 P4-18.2 시각화와 정보 손실에서는 군집화와 비슷하게 `정답을 맞히는 것`보다 `구조를 어떻게 읽고 해석할 것인가`가 중요해집니다. 따라서 군집화 평가처럼 시각적 구조와 해석의 차이를 조심해서 읽는 감각이 이어집니다.

- P4-19.1 가치 기반 강화학습(value-based reinforcement learning), P4-19.2 정책 기반 강화학습(policy-based reinforcement learning), P4-19.3 강화학습 적용의 주의점으로 가면 또 다른 평가 관점이 열립니다. 강화학습은 분류·회귀·군집화처럼 한 번의 예측 오차만으로 읽기 어렵고, 보상(reward), 누적 성과, 탐험 비용을 함께 봐야 합니다. 따라서 이 절은 `모든 문제를 하나의 점수로 읽을 수는 없다`는 감각을 미리 준비하는 역할도 합니다.
...
- 분류, 회귀, 군집화 문제를 같은 평가 질문으로 읽고 있지 않은지 점검해야 할 때 이 절을 떠올립니다.
- 어떤 문제에서는 precision/recall이, 어떤 문제에서는 MAE/RMSE가, 어떤 문제에서는 내부 구조 기준이 더 중요해지는지 다시 정리할 때 이 절로 돌아옵니다.
- 지표를 고르기 전에 모델 출력과 사용 장면을 먼저 확인해야 할 때 이 절이 기준이 됩니다.
