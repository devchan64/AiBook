...
这一节不会深入处理 AUC 计算证明、对数损失公式展开、校准算法实现，或多个聚类指标的全面比较。这些细致计算先放在这本书当前范围之外，这里只回收读取标准。
...
评价指标会变多，不是因为 model 更复杂了，而是因为我们提出的问题更具体了。

- ROC 和 PR 读的是 threshold 改变时的分类平衡
- log loss 读的是概率输出的自信程度
- calibration 读的是分数和真实频率是否匹配
- silhouette 读的是即使没有答案也能看的 cluster 结构

也就是说，`指标很多` 这句话，其实等于在说 `好结果的含义不只一个`。
...
- 当 accuracy、precision、recall 不够用，必须把 ROC、PR、log loss、calibration、silhouette 这类补充指标叫出来时，就该看这一节。
- 当必须重新怀疑一个看起来像概率的分数的自信程度，或者必须用内部标准去读没有答案 label 的 cluster 结构时，就该回到这一节。
- 当需要避免在同一个 `指标` 名字下面，把不同层位的问题混在一起时，这一节会成为标准。
