...
고객 행동 로그에서 사람은 최근 방문 수, 마지막 결제 간격, 최근 클릭 수만 봐도 상태를 알 수 있다고 느끼기 쉽습니다. 하지만 더 깊은 표현으로 가면 `이탈 직전 패턴`, `반복 구매 리듬`, `프로모션 반응성`처럼 단기 이벤트를 넘는 더 큰 행동 구조를 반영하는 쪽으로 설명할 수 있습니다. 이 사례는 깊은 표현이 단순 최근값 저장이 아니라 장기적 행동 특징 압축으로도 읽힐 수 있음을 보여 주고, 눈앞의 최근값은 평범해 보여도 누적 패턴상 위험한 사용자를 더 일찍 가려내는 차이로 이어질 수 있습니다.
