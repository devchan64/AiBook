...
평가 지표와 잔차 해석은 다음 절인 P4-10.2에서 바로 이어서 다루고, 다중공선성, 가정 검정, 회귀 진단의 기초 읽기는 P4-10.3 보충학습에서 다시 정리합니다. 정규화와 관련 하이퍼파라미터의 더 넓은 관점은 P4-9.1, P4-9.2에서 다시 연결합니다.
...
![산점도 위에서 회귀선, 기울기, 절편이 어떻게 읽히는지 보여 주는 차트](../../../assets/part-04/chapter-10/p4-10-1-regression-line-intuition-en.svg)
...
- 더 복잡한 모델을 보기 전에 선형회귀를 baseline처럼 사용합니다.

## 짧은 점검

- 지금 문제가 연속값 예측인지 먼저 확인했는가?
- 선형회귀를 해석 가능한 첫 비교 모델로 두는 이유를 baseline 관점에서 설명할 수 있는가?
- 기울기 숫자를 바로 중요도나 원인으로 읽지 않고 단위와 맥락을 함께 보고 있는가?

## 언제 이 관점을 먼저 떠올려야 하는가

- 지금 문제가 분류가 아니라 연속값 예측인지부터 다시 확인해야 할 때 선형회귀 관점을 떠올립니다.
- 선형회귀를 왜 해석 가능한 첫 baseline 후보로 두는지, 기울기를 어디까지 의미로 읽어야 하는지 다시 설명할 때 이 절로 돌아옵니다.
- 직선이 완벽하지 않아도 왜 여전히 유용한 출발점이 되는지 정리할 때 이 절이 기준이 됩니다.
...
이 절에서는 선형회귀를 `직선으로 관계를 읽는 모델`로 먼저 보았습니다. 다음 절인 P4-10.2에서는 이 직선이 실제로 얼마나 잘 맞았는지, 어떤 경우에 쉽게 틀어지는지, 잔차(residual)와 오차(error)를 어떻게 읽는지로 넘어갑니다.

즉, P4-10.1이 `모델의 모양`을 보는 절이라면, P4-10.2는 `그 모양이 얼마나 적절했는가`를 검토하는 절입니다.
