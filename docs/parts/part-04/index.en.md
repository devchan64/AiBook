...
Part 2 restored the basics for reading formulas, Python, arrays, tables, graphs, and execution environments. Now Part 4 organizes what those tools are actually for, namely what it means to `learn rules from data`.

Many readers have already used AI services, but here it matters more to regroup that experience into standard concepts. Rather than memorizing model names, we first clarify what counts as a machine learning problem, what data must be collected, what a model learns, what it means for learning to have gone well, and why some models can look accurate while still being risky in practice.

The core purpose of Part 4 is to read machine learning not as a list of models, but as a flow of problem, data, learning, evaluation, and application. Linear regression, logistic regression, decision trees, random forests, boosting, clustering, dimensionality reduction, and reinforcement learning are not separate items to memorize. They are options that appear depending on what problem is being solved, what the input and output are, and what criteria define good or bad performance.

Part 4 follows the same rule as the other parts: within the same Part, a major concept should receive its detailed explanation in one main Section first, and later Sections should keep only the minimum needed for the current context. So `supervised learning` is anchored first in `P4-2.1`, `unsupervised learning` in `P4-2.2`, `reinforcement learning` in `P4-2.3`, the role distinction between `validation` and `test` in `P4-4.2`, `overfitting` and `underfitting` in `P4-5.1`, `metric` in `P4-6.1`, `feature selection` in `P4-7.1`, `preprocessing` in `P4-7.2`, `baseline` in `P4-8.2`, `linear regression` in `P4-10.1`, `logistic regression` in `P4-11.1`, `k-NN` in `P4-12.1`, `SVM` in `P4-13.1`, `decision tree` in `P4-14.1`, `random forest` in `P4-15.1`, `gradient boosting` in `P4-16.1`, `clustering` in `P4-17.1`, and `dimensionality reduction` in `P4-18.1`. When they reappear, read them together with the [concept glossary](../../reference/concept-glossary.md) and the current context.
...
4. Feature selection, preprocessing, model choice, baselines, and tuning
...
- Why should feature selection, preprocessing, baselines, and tuning be checked before model names?
...
- You can explain the role of feature selection, preprocessing, model selection, baselines, and tuning.
- You can distinguish the intuition and use cases of linear regression, logistic regression, decision trees, random forests, and gradient boosting.
...
- Feature selection, preprocessing, baselines, model selection, and tuning
...
Next it turns to representative classical models. It covers linear regression, logistic regression, k-NN, SVM, decision trees, random forests, and gradient boosting, and asks what each does well and where caution begins. The goal here is problem sense rather than formula derivation.
...
The mathematics and software tools restored in Part 2 are used directly in Part 4. NumPy arrays hold input `X` and label `y`, Pandas DataFrames read datasets, and mean, variance, and error reappear when reading evaluation metrics. Derivatives, gradients, and loss continue into the later sections and into Part 5 on deep learning, while Matplotlib is used to inspect data distributions, decision boundaries, loss curves, and cluster structure.

In other words, if Part 2 restored the language of calculation, Part 4 shows what judgments that language supports inside machine learning.

To begin Part 4 after Part 2, it is enough if you still hold the following.

| What comes from Part 2 | The reading question that changes immediately in Part 4 |
| --- | --- |
| You can read `input X`, `label y`, and shape from arrays and tables | How do you decide which columns are input features and which value is the target? |
| You can check mean, error, and graphs with small examples | What does it actually mean when someone says the score improved? |
| You know the difference among Colab, local execution, and Git records | What must be recorded to rerun the same experiment and compare it again? |

When coming from Part 3, you must also bring the sense that `putting data directly into the model` is not the first step. First fix `what counts as one case`.

| What is carried over from Part 3 | The question that continues immediately in Part 4 |
| --- | --- |
| One row was organized to mean one action or one case | In this case, what will be used as input features and what will be a target candidate? |
| Control logs, sensor logs, and identifier columns were kept separate | Which columns should not be put directly into the model input? |
| Summary features and candidate review labels were kept apart | Is the problem now classification or regression, and is the label definition consistent enough? |
| Baseline comparison and the next inspection question were recorded together | Even if the score looks better, what should it be compared against to judge meaningful improvement? |

## Connection To Later Parts

After Part 4, the reader can see deep learning not as a `new fashionable model`, but as a flow that handles the common problems of machine learning with greater representational power and larger computational resources.

Part 4 leads to the following questions.

- Why are neural networks strong at representation learning?
- Why do GPUs and parallel processing matter once parameter counts grow?
- How do gradient descent, loss, and generalization problems reappear in deep learning?

So Part 4 is not a Part about the era before deep learning. It is a Part that builds the common base for reading deep learning and generative AI.
...
In the next Part, the common questions established in Part 4 expand into the context of neural networks.

Problem definition, data splitting, loss, optimization, and generalization remain exactly as in Part 4. Added to them are structural questions such as hidden layers, representation learning, parallel computation, and Transformers.

## Understanding Gained After Finishing This Part
...
- You can distinguish the role of feature selection, preprocessing, baselines, and tuning.
- You can explain at an introductory level the intuition and difference among linear regression, logistic regression, decision trees, random forests, and gradient boosting.
...
- Google, `Machine Learning Glossary`, entries including `machine learning`, `supervised learning`, `unsupervised learning`, and `reinforcement learning`, accessed 2026-07-10. [https://developers.google.com/machine-learning/glossary](https://developers.google.com/machine-learning/glossary){: target="_blank" rel="noopener noreferrer" }
- scikit-learn developers, `User Guide`, scikit-learn documentation, accessed 2026-07-10. [https://scikit-learn.org/stable/user_guide.html](https://scikit-learn.org/stable/user_guide.html){: target="_blank" rel="noopener noreferrer" }
