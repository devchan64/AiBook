...
먼저 한 장면을 떠올려 봅시다. 고객 이탈 예측 표에 `최근 방문 횟수`, `회원 등급`, `최근 문의 수`, `담당자가 검토를 끝낸 뒤 생긴 결과 메모`, `내부 임시 ID`가 함께 있다면, 모델에게 모두 주는 것이 좋은 출발점은 아닙니다. 이 절에서 먼저 할 일은 성능을 계산하기 전에 `예측 시점에 쓸 수 없는 열`, `문제와 관련이 약한 열`, `운영에서 다시 만들기 어려운 열`을 걸러 내는 것입니다.
