...
- optimizer는 gradient를 실제 파라미터 업데이트로 바꾸는 규칙이며, 손실 함수와 역전파가 계산한 값을 실제 이동으로 연결합니다. learning rate는 그 보폭을 정하는 핵심 하이퍼파라미터이고, optimizer 선택은 단순 취향이 아니라 학습 전략의 일부입니다.
