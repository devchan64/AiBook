...
원시 로그를 다루는 경우에는 이 차이가 더 분명해집니다. 처음에는 시간 순서대로 쌓인 값들이 그대로 보이지만, 모델 입력으로는 그 전체 행을 모두 넣기보다 동작 단위로 다시 요약한 특징 후보를 만들게 되는 경우가 많습니다.

즉, 여기서도 순서는 `원시 시계열 -> 동작 1회 요약 행 -> 특징 선택`입니다. 특징 선택은 이 순서가 끝난 뒤에 `요약 행의 어떤 입력 칸을 남길 것인가`를 다시 정하는 단계로 보는 편이 안전합니다.
...
이 코드에서 독자가 먼저 봐야 할 것은 성능 숫자가 아니라 표현의 변환입니다.
