...
사람은 보통 손실이 줄고 있기만 하면 `방향은 맞으니 기다리면 되겠다`고 먼저 생각하기 쉽습니다. 그런데 실제로는 손실이 아주 천천히 줄어들어 실험 한 번에 너무 긴 시간이 걸릴 수 있습니다. 이때는 gradient 자체가 틀린 것이 아니라 학습률이 너무 작거나, 더 적절한 optimizer가 필요할 수 있습니다.

즉, gradient가 틀린 것이 아니라 `업데이트 방식이 지나치게 조심스러운 것`일 수 있습니다.

### 사례 2. 손실이 계속 흔들리는 경우

반대로 손실이 내려가다가 다시 튀고, 방향이 불안정하게 바뀌는 경우도 있습니다. 사람은 이 장면을 보면 `모델이 아예 못 배우는 것 아닌가`라고 느끼기 쉽습니다. 하지만 실제로는 내려가는 방향은 알고 있어도 한 번에 너무 크게 움직여 좋은 지점을 계속 지나치는 경우가 많습니다. 이때는 학습률이 너무 크거나, 현재 optimizer 설정이 문제에 비해 거칠 수 있습니다.

즉, 방향은 알지만 보폭이 너무 큰 상황으로 볼 수 있습니다.

### 사례 3. 큰 모델 학습

딥러닝 모델이 커질수록 사람은 `가중치가 많아졌을 뿐, 업데이트 방식은 같아도 되지 않을까`라고 생각하기 쉽습니다. 하지만 실제로는 파라미터마다 민감도가 다르고, gradient의 스케일도 균일하지 않을 수 있습니다. 이런 상황에서는 단순한 업데이트 규칙만으로는 학습이 어려울 수 있어, 더 정교한 optimizer가 실무적으로 많이 쓰입니다.
