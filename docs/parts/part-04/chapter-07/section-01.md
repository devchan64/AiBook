...
전처리 종류를 입력 문제 기준으로 나누는 감각은 P4-7.3 보충학습에서 다시 붙잡고, 통계 검정 기반 선택과 재귀적 특징 제거의 비교 관점은 P4-7.4 보충학습에서 다시 정리합니다. 차원 축소의 큰 그림과 내부 계산은 P4-18.1, P4-18.2에서 다시 연결합니다.
...
- 예측 시점에 없는 칼럼이나 정답 이후 정보가 입력에 섞이지 않았는지 점검해야 할 때 특징 선택 관점을 먼저 떠올립니다.
- 식별자, 상수 칼럼, 운영에서 불안정한 신호를 왜 먼저 걸러야 하는지 다시 설명할 때 이 절로 돌아옵니다.
- 특징 선택과 전처리를 같은 일처럼 말하고 있지 않은지 확인할 때 이 절이 기준이 됩니다.
...
이 절에서 `무엇을 남길 것인가`를 정했다면, 다음 절 P4-7.2 전처리(preprocessing)에서는 `남긴 특징을 어떤 형태로 바꿀 것인가`를 보게 됩니다. 결측치, 스케일(scale), 범주형 인코딩(encoding)은 바로 그 다음 단계의 문제입니다.
