...
이 절의 목적은 optimizer 논문을 깊게 비교하는 것이 아니라, `업데이트 철학의 차이`를 설명하는 것입니다.
...
Adam은 SGD보다 더 많은 정보를 사용합니다. 초심자 기준에서는 다음 정도로 이해하면 충분합니다.
...
초심자는 `Adam이 편하면 SGD는 이제 중요하지 않은가?`라는 질문을 자주 합니다. 그렇지 않습니다.
