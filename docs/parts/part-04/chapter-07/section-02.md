...
작은 회귀 실험에서 입력 하나와 오차 하나를 눈으로 따라가며 학습을 보고 싶을 수 있습니다. 사람이 손으로 계산을 따라갈 때는 보통 `지금 오차가 어느 방향으로 났는가`를 보고 바로 조금 되돌리는 식으로 생각합니다. 이때 SGD는 현재 gradient를 보고 바로 한 걸음 움직이므로, `이번 step에서 왜 가중치가 이렇게 바뀌었는가`를 비교적 직접 추적할 수 있습니다. 단순한 문제에서는 이런 직선적인 업데이트가 학습 원리를 설명하는 데 특히 유리합니다.
그래서 이 사례에서 확인해야 할 결과는 여러 보정 없이도, 현재 gradient 방향이 다음 가중치 변화에 직접 반영되는가입니다.
...
를 비교하기도 합니다. 예를 들어 Adam은 초반에 빠르게 내려가지만, SGD가 더 오래 학습했을 때 최종 검증 성능이 더 좋아질 수도 있습니다. 이처럼 optimizer는 단순 부수 설정이 아니라 학습 속도와 최종 일반화 성능을 함께 바꾸는 핵심 실험 변수입니다.
