...
먼저 입력 표의 상태를 봅니다. `age`에는 빈 값이 있고, `income`은 다른 숫자보다 훨씬 큰 축으로 움직이며, `city`는 문자열입니다. 이때 전처리는 세 기술 이름을 외우는 일이 아니라, 빈 값은 어떤 규칙으로 채울지, 숫자 축은 어떻게 맞출지, 문자열은 어떤 계산 가능한 표현으로 바꿀지 정하는 작업입니다.
...
## 전처리는 입력을 모델이 읽을 수 있게 바꾸는 준비 작업이다
