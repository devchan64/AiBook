...
개별 인코더와 스케일러의 세부 설정은 P4-9.1, P4-9.2의 하이퍼파라미터와 튜닝 문맥에서 다시 연결하고, 차원 축소의 목적과 수학적 직관은 P4-18.1, P4-18.2에서 다시 다룹니다.
...
| 텍스트/이미지 등 특수 표현 변환 | 비정형 데이터를 수치 표현으로 바꾼다 | 이 절 범위 밖 |
...
| 빈도/집계 기반 변환 | 범주를 다른 통계값으로 바꾼다 | 고급 상황에서 쓰일 수 있다 | 이 절 범위 밖이며 누수 주의가 필요하다 |
...
- 입력 문제를 결측치, 스케일, 범주형 표현, `fit/transform` 규칙 중 어디에서 먼저 다뤄야 하는지 진단해야 할 때 전처리 관점을 떠올립니다.
- 테스트 데이터가 전처리 규칙을 배우는 데 섞이지 않도록 왜 훈련 데이터에서만 `fit`해야 하는지 다시 설명할 때 이 절로 돌아옵니다.
- 전처리를 단순 청소가 아니라 재현 가능한 입력 표현 설계로 읽어야 할 때 이 절이 기준이 됩니다.
...
P4-8 모델 선택(model selection)과 P4-9 하이퍼파라미터(hyperparameter)로 넘어가면, 같은 모델이라도 어떤 전처리와 함께 묶느냐에 따라 성능과 해석이 달라질 수 있다는 점을 더 자주 보게 됩니다. 또한 P4-10 이후의 알고리즘 절에서는 어떤 모델이 스케일과 표현에 더 민감한지가 더 구체적으로 드러납니다.
