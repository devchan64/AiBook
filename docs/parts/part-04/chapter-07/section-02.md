...
즉, 이 절을 지나면 독자는 단순히 데이터를 모아 둔 상태에서 벗어나, `비교 가능한 입력 표현을 만든 뒤 모델 후보를 세우는 흐름`으로 넘어가게 됩니다. 그래서 P4-7.2는 P4-8 모델 선택(model selection), P4-9 하이퍼파라미터 튜닝(hyperparameter tuning), P4-10 이후 알고리즘 절의 공통 바탕이 됩니다.
