...
- 평균, 중앙값, robust scaling, target encoding의 수식 차이
- 개별 라이브러리 문법과 API 세부 옵션
- 필터(filter), 래퍼(wrapper), 임베디드(embedded), 차원 축소(dimensionality reduction)의 비교

필터, 래퍼, 임베디드, 차원 축소 구분은 다음 절 P4-7.4에서 다시 다룹니다.
...
- 전처리 종류는 칼럼 이름이 아니라 `입력 문제의 종류`로 구분합니다.
- 한 칼럼에도 결측치 처리와 인코딩처럼 여러 규칙이 차례로 붙을 수 있습니다.
- 전처리의 첫 순서는 `입력 문제 -> 전처리 종류 -> 모델 민감도 확인`입니다.
- 전처리 종류를 나눈 뒤에도 규칙 학습은 train에서만 해야 합니다.
