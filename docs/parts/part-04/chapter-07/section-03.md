...
- 통계 검정 기반 특징 선택은 보통 필터 접근으로 이해합니다.
- RFE는 모델을 반복 학습하며 특징을 줄이는 래퍼 접근으로 이해합니다.
...
- 원래 칼럼을 고르는 일과 새 축으로 다시 표현하는 일을 구분해야 할 때 이 절을 먼저 떠올립니다.
- 필터, 래퍼, 임베디드, 차원 축소를 같은 방식의 해결책처럼 섞고 있지 않은지 점검할 때 이 절로 돌아옵니다.
- 해석 가능성과 계산 비용을 함께 보며 어떤 방법을 먼저 써야 하는지 정리할 때 이 절이 기준이 됩니다.
