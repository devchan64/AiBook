...
이번 연습은 `현재 보상 + 다음 가치`, `반복 갱신 안정화`, `표에서 함수로 이동`이라는 세 손잡이를 직접 연결해 보는 데 초점을 둡니다.
...
- 할인율 0.9
- 다음 상태에서 기대하는 가치 0.8
- 작은 상태 공간과 큰 상태 공간의 상태 수 비교

기대 출력(output):

- 현재 상태의 벨만식 읽기 결과
- Q-table이 자연스러운지 함수 근사가 자연스러운지에 대한 판단
...
small_grid_states = 25
large_game_states = 1_000_000

print("bellman view =", round(bellman_view, 2))
print("small grid: q-table is still imaginable =", small_grid_states)
print("large game: q-table becomes unwieldy =", large_game_states)
```
...
```text
bellman view = 1.72
small grid: q-table is still imaginable = 25
large game: q-table becomes unwieldy = 1000000
```

이 예제에서 읽어야 할 것은 다음입니다.

1. 현재 가치는 지금 보상만이 아니라 `다음 가치 0.8`까지 합쳐 `1.72`로 읽힌다.
2. 이런 값을 반복해서 고치며 변화폭이 줄어드는지를 보는 질문이 수렴이다.
3. 상태 수가 작을 때는 표 직관이 남지만, 상태 수가 커지면 같은 가치 직관을 함수 근사로 옮겨야 한다.
...
| 관찰 | 성급한 결론 | 더 안전한 해석 |
| --- | --- | --- |
| `reward + gamma * next_value`가 현재 값보다 크다 | 보상만 크면 된다 | 현재 보상과 미래 가치가 함께 현재 판단을 만든다 |
| 반복 갱신 뒤 값 변화가 줄어든다 | 이제 수식은 끝났고 더 볼 것이 없다 | 값이 안정되는지 보는 수렴 질문으로 이어진다 |
| 상태 수가 커졌다 | 가치 기반 강화학습은 이제 쓸 수 없다 | 가치 기반 직관은 유지하되 표 대신 함수 근사를 쓴다 |
