...
| 把实际下一个行动改成 `up` 后，两边的值相同了 | 两个算法差别不重要 | 差别不在名字，而在于`下一个值是从哪里读出来的` |

这张表的目的，不是让人给算法排高下，而是让人养成习惯：即使看到同一组数字，也先读`到底是哪种更新标准在起作用`。
...
- 能不能把 Q-value 解释成`在这个状态下做这个行动，接下来整体会有多划算`的预期分数？
