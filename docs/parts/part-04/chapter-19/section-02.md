...
보상 설계, 탐험 비용, 시뮬레이션과 현실 차이, 실제 적용의 위험은 P4-19.3에서 이어서 정리합니다. PPO, TRPO, A2C, A3C 같은 후속 알고리즘과 연속 제어의 큰 그림은 P4-19.4 보충학습에서 다시 정리합니다.

강화학습 장에서는 `지금 어디까지 이해해야 하는가`도 함께 고정해 둘 필요가 있습니다.
...
즉, 현재 절의 책임은 `정책 기반이 왜 등장했고 어떤 문제 장면에서 자연스러운가`를 닫는 데 있습니다. 알고리즘 이름을 더 많이 외우는 일이나 수학 유도를 길게 따라가는 일보다, `정책을 직접 바꾼다`는 말이 무엇을 뜻하는지와 `어떤 문제 장면에서 가치 기반보다 읽기 쉬운가`를 분명히 잡는 것이 중요합니다.
