...
Part 4에서 강화학습을 배우는 목적은 알고리즘 이름을 늘리는 데 있지 않고, `무엇을 최적화할 것인가`를 잘못 잡으면 전체 시스템이 어디로 빗나가는지 이해하는 데 있습니다. 이 연습은 보상 함수의 작은 가중치 변화가 곧 정책 선호 변화를 만든다는 점을 보여 주므로, 문제 정의와 평가 기준을 먼저 세워야 한다는 Part 4의 목표를 가장 직접적으로 회수합니다. 독자가 실습 뒤에도 목표를 체감하지 못했다면 보통 식이 부족해서가 아니라, `수치가 바뀌면 의도 해석도 바뀐다`는 연결 문장이 약했던 경우가 많습니다.

| 공통 기록 언어 | 이번 연습에서 바로 남길 내용 |
| --- | --- |
| 보인 구조 | 같은 클릭 데이터도 불만 비용 가중치에 따라 더 좋은 행동 판단이 달라졌다 |
| 해석 경계 | 보상 숫자 상승만으로는 실제 목표 달성이나 안전한 배포를 뜻하지 않는다 |
| 다음 질문 | 어떤 부작용 지표와 제한 배포 절차를 같이 묶어야 실제 목표에 가까워지는가 |
