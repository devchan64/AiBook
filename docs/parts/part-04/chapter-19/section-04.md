...
이 보충학습에서도 이름만 나열하지 말고, 어떤 문제 신호가 어떤 분기를 낳았는지 같이 남기는 편이 좋습니다. 같은 강화학습 계열 이름처럼 보여도 실제로 줄이려는 실패 비용과 현실 제약은 다를 수 있으므로, 이름보다 문제 신호를 먼저 적는 편이 안전합니다.
...
- RLHF는 강화학습 언어가 LLM 정렬과 만나는 지점입니다.
