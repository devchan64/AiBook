...
P4-2.3에서는 강화학습(reinforcement learning)을 `행동(action)과 보상(reward)을 통해 정책(policy)을 조정하는 학습`으로 먼저 잡았습니다. 이제 그 안으로 한 단계 더 들어가 보겠습니다.
...
이 절에서도 값 설명만 두지 말고, 어떤 상태-행동 후보와 어떤 실패 비용을 함께 적을지 붙여 두는 편이 좋습니다. 같은 높은 Q-value처럼 보여도 어떤 상태에서는 안전하게 반복되고, 어떤 상태에서는 탐험 비용과 실패 손해가 더 크게 남을 수 있으므로 검토 메모를 같이 남겨야 합니다.
...
- 정책 기반 강화학습은 다음 절(P4-19.2)에서 이어진다는 흐름을 알고 있는가?
