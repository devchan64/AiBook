...
정책 기반 강화학습(policy-based reinforcement learning)은 P4-19.2에서 이어서 다룹니다. 보상 설계, 탐험 비용, 현실 적용의 주의점은 P4-19.3에서 다시 정리합니다. DQN과 함수 근사, 가치 기반에서 정책 기반으로 이어지는 후속 계보는 P4-19.4 보충학습에서 다시 회수합니다.

현재 Part 4에서는 강화학습 수학을 별도 `보충학습`으로 더 쪼개지 않습니다. 이 책에서 강화학습의 학습 목표는 상태, 행동, 보상, 가치, 정책, 탐험, sim-to-real 같은 구조를 읽는 데 있고, 벨만 방정식의 엄밀한 전개나 수렴 증명은 본편과 보충학습을 오가며 회수하기보다 더 심화된 수학 자료에서 따로 보는 편이 범위상 더 적절합니다.
...
- 이 차이는 탐험이 섞인 행동 흐름을 얼마나 직접 반영하느냐와 연결됩니다.
...
- 행동 후보 수가 적고 각 선택의 장기적 좋음을 점수표처럼 비교할 수 있을 때, 가치 기반 관점을 먼저 떠올립니다.
- Q-learning과 SARSA 차이가 흐려질 때, 가장 좋아 보이는 다음 행동과 실제 다음 행동을 기준으로 다시 구분합니다.
- 높은 Q-value를 곧바로 안전한 현실 행동으로 오해하려 할 때, 값 추정과 배포 위험을 분리해 봅니다.
