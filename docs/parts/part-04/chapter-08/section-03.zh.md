...
这一节不会深入讲交叉验证公式推导、统计检验、复杂 benchmark 运营、按算法细分的优化技巧。这些内容会在 P4-9 之后再接回来。
...
| 现在要抓住什么 | 为什么重要 | 接下来会连到什么 |
| --- | --- | --- |
| 按问题类型看的代表 baseline 候选 | 因为分类、回归、时序的出发标准不同。 | 会继续连到 P4-9 里的候选比较与调优。 |
| 用同样指标比较 baseline 和候选 model | 因为解释分数差异必须使用同一个尺子。 | 这样才能分清是真改进还是幻觉。 |
| 检查代表错误场景 | 因为只看数字，无法完整读出改进方向。 | 会继续连到混淆矩阵、大误差区间、失败案例。 |

- baseline 不是 `先输掉的模型`，而是 `必须先比较的标准`。
- 代表性 baseline 方法必须按问题类型不同来设置。
- baseline 解释后必须立刻接案例和例子，这样读者才能跟上真实比较场景。
- 只有确认出现了比 baseline 更好的候选，才更安全地进入调优。

## 简短检查

- 你有没有至少写下一个符合当前问题类型的代表性 baseline 候选？
- 你是不是在用同一个指标比较 baseline 和候选 model？
- 你看的不只是分数差，也包括代表错误场景吗？
- 你有没有避免把一个过不了 baseline 的候选拖进很长的调优？

## 和下一节的连接

如果这里已经固定好了 baseline 候选和设置流程，那么下一节 P4-9 就会进入：那些看起来比 baseline 更好的候选，到底该怎样继续比较和调整。后面的算法章节里，读者也会更具体地看到为什么有些 model 更容易越过某种 baseline，而另一些则不容易。
