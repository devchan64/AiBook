...
이미지를 넣고 분류 점수를 계산한 뒤 손실을 구하고, 역전파와 optimizer step으로 가중치를 갱신하는 흐름은 CNN에서도 그대로 유지됩니다. 사람은 CNN처럼 구조가 바뀌면 학습 방식도 완전히 새로 바뀐다고 느끼기 쉽지만, 실제로는 `forward -> loss -> backward -> update`라는 뼈대는 그대로 남습니다.

### 사례 2. 문장 분류 학습

입력을 텍스트로 바꾸고 구조를 RNN이나 Transformer로 바꿔도, 손실 계산과 backward, optimizer step이라는 루프는 그대로 남습니다. 사람은 텍스트 모델이 되면 완전히 다른 절차를 쓸 것처럼 느끼기 쉽지만, 실제로 바뀌는 것은 입력 구조와 내부 계산이고 학습 루프의 큰 골격은 유지됩니다.

### 사례 3. 과적합 징후

학습 손실은 계속 내려가는데 검증 성능이 나빠진다면, 사람은 먼저 `모델 구조가 나쁘다`고 결론내리기 쉽습니다. 하지만 이런 경우에는 구조 자체보다 regularization이나 mode 설정을 다시 봐야 할 수 있습니다. 이 사례는 학습 루프를 한 장면으로 묶어 보고 있어야 어디를 점검해야 하는지 더 빨리 구분할 수 있다는 점을 보여 줍니다.
