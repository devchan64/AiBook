...
이 절은 교차검증 수식 전개, 통계 검정, 복잡한 벤치마크 운영, 알고리즘별 최적화 기법을 깊게 다루지 않습니다. 그런 내용은 P4-9 이후에서 다시 연결합니다.
...
| 지금 붙잡을 것 | 왜 중요한가 | 다음으로 이어질 것 |
| --- | --- | --- |
| 문제 유형별 대표 baseline 후보 | 분류, 회귀, 시계열은 출발 기준이 다르기 때문입니다. | P4-9에서 후보 비교와 튜닝으로 이어집니다. |
| 같은 지표로 baseline과 후보 모델 비교 | 점수 차이를 해석하려면 같은 잣대가 필요하기 때문입니다. | 실제 개선인지, 착시인지 구분합니다. |
| 대표 오류 장면 확인 | 숫자만으로는 개선 방향을 다 못 읽기 때문입니다. | 혼동 행렬, 큰 오차 구간, 실패 사례로 이어집니다. |

- baseline은 `먼저 지는 모델`이 아니라 `먼저 비교해야 할 기준`입니다.
- 대표 baseline 방법은 문제 유형에 따라 다르게 잡아야 합니다.
- baseline 설명 뒤에는 사례와 예제가 바로 붙어야 독자가 실제 비교 장면을 따라갈 수 있습니다.
- baseline보다 나은 후보가 생겼는지 확인한 뒤에야 튜닝으로 가는 편이 안전합니다.
...
여기서 baseline 후보와 설정 절차를 잡았다면, 다음 절 P4-9에서는 baseline보다 나아 보이는 후보를 어떻게 비교하고 조정할지로 넘어갑니다. 이후 알고리즘 절에서는 각 모델이 왜 어떤 baseline을 넘기 쉬운지, 또는 넘기 어려운지를 더 구체적으로 보게 됩니다.
