...
이 절의 목적은 새 알고리즘을 더 추가하는 데 있지 않습니다. 이미 본 개념들을 `한 장면의 학습 흐름`으로 다시 묶는 데 있습니다.
...
이미지를 넣고 분류 점수를 계산한 뒤 손실을 구하고, 역전파와 optimizer step으로 가중치를 갱신하는 흐름은 CNN에서도 그대로 유지됩니다.

### 사례 2. 문장 분류 학습

입력을 텍스트로 바꾸고 구조를 RNN이나 Transformer로 바꿔도, 손실 계산과 backward, optimizer step이라는 루프는 그대로 남습니다.

### 사례 3. 과적합 징후

학습 손실은 계속 내려가는데 검증 성능이 나빠진다면, 구조 자체보다 regularization이나 mode 설정을 다시 봐야 할 수 있습니다.
...
- 이렇게 학습되는 구조가 이미지에서는 왜 CNN으로, 순서 데이터에서는 왜 RNN으로 갈라지는가?
- 어떤 입력 구조 문제 때문에 다른 신경망 설계가 필요해지는가?

이 질문은 P4-11.1 CNN의 직관부터 시작하는 구조 장들로 이어집니다.
