...
### 用 Python 例子把候选家族整理成表

下面这个例子不是在训练 model，而是一个非常简单的思考工具，用来按问题条件整理“应该先想到哪些候选”。

问题场景:

- 模型选择与其说是在背算法名字，不如说更接近于看着问题条件先缩小候选家族
...
- 候选家族可以从任务类型、解释需求、非线性预期等条件出发来整理
- 如果先把候选家族整理成像表一样，后面的比较和 baseline 设定会更容易
...
这个例子的目的不是自动选择，而是展示 `读取问题条件并减少候选的思考方式`。

读者在这里应该读出的核心有三点。

- 候选家族通常不是从一个 model 开始，而是从两个或三个以上开始。
- `可解释性`、`非线性`、`距离概念` 这样的词，会成为缩小候选的标准。
- 模型选择还不是决定胜者的阶段，而是在建立一份可比较的参赛名单。

## 本节要记住的观念

- 模型选择不是去猜一个唯一正确算法。
- 它会把问题类型、数据条件、可解释性、运营约束一起看。
- 真正的起点不是一个 model，而是一个合理的 shortlist。
- baseline 和后面的算法章节，就是拿这份 shortlist 去比较并继续缩小的阶段。

## 简短检查

- 你是不是先固定了问题类型，再建立 shortlist？
- 在立刻调一个 model 之前，你有没有先留下 2 到 4 个左右可比较的候选？
- 你记下来的不只是候选 model 名字，也包括 baseline 和第一优先看的错误场景吗？

## 什么时候应该先想起这个观念

- 当你不该立刻抓住一个有名 model，而是应先建立合理候选家族时，要先想起模型选择视角。
- 当你需要一边看问题类型、输入表达、可解释性、运营约束，一边缩小 shortlist 时，就该回到这一节。
- 当你需要重新整理为什么 baseline 和第一优先看的错误场景也必须一起记时，这一节会成为标准。

- 现在的问题是分类、回归，还是聚类？
- 你有没有考虑输入表达和数据规模？
- 你有没有把可解释性和运营约束放进标准里？
- 你是否已经准备好建立至少两三个候选并进行比较？
- 你是不是还在没有 baseline 的情况下，只抓着一个复杂 model 不放？

## 和下一节的连接

下一节 P4-8.2 baseline 会继续看：为什么在这里建立好的候选家族里，还必须先放上 `最简单的比较起点`。接着在 P4-9 里，读者会看这些候选要怎样调整和比较；而在 P4-10 之后，则会更具体地看到每个候选到底具有什么性格。
