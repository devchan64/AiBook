...
- regularization은 과적합을 줄이기 위한 제약이나 비용을 추가하는 생각이며, 벌점 공식만이 아니라 더 넓은 설계 철학으로 볼 수 있습니다. 일반화 문제를 이해하려면 이 관점을 함께 봐야 하며, regularization과 normalization은 이름이 비슷해도 하나는 과적합 억제 쪽을, 다른 하나는 값의 스케일과 분포 정리 쪽을 더 직접 다룹니다. optimizer가 업데이트 방법을 조정한다면, regularization은 선호하는 해의 성질을 조정합니다.
