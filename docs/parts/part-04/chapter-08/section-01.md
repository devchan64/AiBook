...
사람은 보통 훈련 손실이 계속 줄고 정확도가 높아지면 `학습이 잘되고 있다`고 먼저 생각합니다. 그런데 검증 데이터에서 성능이 오히려 떨어지기 시작하면, 이 모델은 훈련 데이터 세부 패턴을 너무 잘 외운 것일 수 있습니다. regularization은 바로 이런 `훈련 성능만 좋고 새 데이터에서는 흔들리는` 경향을 줄이려는 장치로 등장합니다.

### 사례 2. 파라미터가 지나치게 커지는 경우

사람이 모델을 손으로 해석할 때는 보통 `오차만 줄면 된다`고 생각하기 쉽습니다. 하지만 어떤 모델은 손실을 줄이기 위해 일부 가중치를 지나치게 크게 키우기도 합니다. 이때 regularization penalty를 주면 `맞추는 것`과 `지나치게 큰 가중치를 피하는 것`을 함께 고려하게 됩니다.

### 사례 3. 데이터가 적은 문제

사람은 데이터가 적으면 `그만큼 더 열심히 맞추면 되지 않을까`라고 생각하기 쉽습니다. 하지만 실제로는 데이터가 적을수록 모델이 우연한 잡음과 예외를 규칙처럼 외워 버릴 위험이 더 커집니다. 이때 정규화는 더 중요해집니다. 즉, data size와 model capacity의 균형 문제와 연결됩니다.
