...
이 경우 모델은 훈련 데이터 세부 패턴을 너무 잘 외운 것일 수 있습니다. regularization은 그런 외우기 경향을 줄이려는 장치로 등장합니다.

### 사례 2. 파라미터가 지나치게 커지는 경우

어떤 모델은 손실을 줄이기 위해 일부 가중치를 지나치게 크게 키우기도 합니다. 이때 regularization penalty를 주면 `맞추는 것`과 `지나치게 큰 가중치를 피하는 것`을 함께 고려하게 됩니다.

### 사례 3. 데이터가 적은 문제

데이터가 충분하지 않을수록 모델은 우연한 잡음에 끌릴 위험이 커집니다. 이때 정규화는 더 중요해집니다. 즉, data size와 model capacity의 균형 문제와 연결됩니다.
