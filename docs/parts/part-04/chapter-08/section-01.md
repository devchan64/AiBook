...
교차검증의 기본 역할은 P4-4.2와 P4-9.2에서 다시 연결하고, 정보 기준, AutoML, 대규모 탐색 시스템은 P4-9.3 보충학습에서 `고급 모델 선택과 자동화` 관점으로 따로 정리합니다.
...
- 하나의 유명한 모델을 바로 붙잡기보다 합리적인 후보군을 먼저 세워야 할 때 모델 선택 관점을 떠올립니다.
- 문제 유형, 입력 표현, 설명 가능성, 운영 제약을 함께 보며 shortlist를 줄여야 할 때 이 절로 돌아옵니다.
- baseline과 먼저 볼 오류 장면까지 같이 메모해야 하는 이유를 다시 정리할 때 이 절이 기준이 됩니다.
...
다음 절 P4-8.2 기준 모델(baseline)에서는 지금 만든 후보군 가운데 왜 `가장 단순한 비교 시작점`이 먼저 필요한지 보게 됩니다. 그리고 P4-9에서는 그 후보들을 어떻게 조정하고 비교할지, P4-10 이후에서는 각 후보가 실제로 어떤 성격을 가지는지 더 구체적으로 보게 됩니다.
