...
이 점은 Part 2와 P4-7의 흐름을 다시 한 번 묶어 줍니다. Part 2에서는 `한 행이 무엇을 뜻하는가`와 `동작 1회 요약 행이 왜 필요한가`를 봤고, P4-7에서는 그 요약 행에서 어떤 입력 칸을 특징으로 남길지 봤습니다. 모델 선택은 그 다음에야 비로소 `이 입력 표현이라면 어떤 모델 계열을 먼저 시험할까`를 묻는 단계입니다.
