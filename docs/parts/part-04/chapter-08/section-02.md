...
드롭아웃은 Part 4 초반부의 여러 개념을 한 번에 다시 묶습니다.
...
- 드롭아웃은 학습 중 일부 경로를 무작위로 쉬게 해 특정 경로에 대한 과한 의존을 줄이고 일반화를 돕는 정규화 기법입니다.
- 평가 모드에서는 같은 무작위 제거를 유지하지 않는 것이 보통이며, 드롭아웃은 정규화, 모드 전환, 일반화 개념을 함께 연결하는 대표 사례입니다.
