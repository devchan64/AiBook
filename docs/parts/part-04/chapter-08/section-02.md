...
## DummyClassifier와 DummyRegressor를 읽는 기준
...
- 오픈체크리스트의 중심축 문장인 "기준 모델이 비교를 위한 최소 출발선이라는 점을 분명히 해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- baseline이 복잡한 모델 전에 세우는 비교 기준이며, 높은 점수도 baseline과 비교하지 않으면 해석하기 어렵다는 점을 설명할 수 있는가
- baseline이 `점수 하나를 더 보는 절`이 아니라 출발점 점수, 대표 오류 사례, 핵심 지표 차이를 함께 남기는 기준이라는 점을 설명할 수 있는가
