...
초심자에게는 이 발상이 다소 역설적으로 들릴 수 있습니다.
...
입문 설명에서 드롭아웃은 종종 `여러 부분 네트워크를 번갈아 학습하는 느낌`으로도 설명됩니다. 엄밀히 완전히 같은 것은 아니지만, 초심자에게는 도움이 되는 직관입니다.
...
은닉층이 크고 파라미터가 많은 모델은 훈련 데이터의 우연한 조합까지 외우기 쉽습니다. 이때 드롭아웃은 특정 은닉 경로 의존을 줄이는 데 도움을 줄 수 있습니다.

### 사례 2. 데이터가 상대적으로 적은 경우

데이터가 적을수록 큰 모델은 과적합 위험이 커집니다. 이때 드롭아웃은 일반화를 돕는 보조 장치로 자주 언급됩니다.

### 사례 3. 완전연결층(fully connected layer) 중심 구조

초기 딥러닝 실무와 교육에서는 특히 완전연결층이 큰 구조에서 드롭아웃이 강한 regularization 도구로 많이 쓰였습니다. 이후 구조와 normalization 기법이 발전했지만, 드롭아웃은 여전히 중요한 기본 개념입니다.
...
이 질문은 바로 P4-9.1 GPU와 병렬 처리로 이어집니다.
