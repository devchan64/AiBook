...
바로 그 점이 핵심입니다. 드롭아웃은 모델이 특정 경로 하나에만 과하게 맞추는 것을 방지합니다.
...
상품 리뷰 분류 모델을 만들었는데 은닉층이 크고 파라미터 수가 많으면, 사람은 보통 `표현을 많이 볼수록 더 잘 맞겠지`라고 기대하기 쉽습니다. 하지만 모델이 훈련 데이터에만 우연히 함께 나온 표현 조합까지 외워 버릴 수 있습니다. 이때 일부 노드를 임시로 끄는 드롭아웃을 넣으면, 특정 은닉 경로 하나에만 과하게 의존하지 못하게 만들 수 있습니다. 즉, 모델은 매 step마다 조금 다른 부분 조합으로 판단하는 연습을 하게 됩니다.

### 사례 2. 데이터가 상대적으로 적은 경우

의료 문서나 내부 품질 보고서처럼 데이터 수가 많지 않은 문제에서는 큰 모델이 훈련 예시의 문장 패턴을 거의 외워 버리기 쉽습니다. 이때 드롭아웃은 `훈련 데이터에서는 맞지만 새 데이터에서는 흔들리는` 상황을 줄이기 위한 보조 장치로 자주 쓰입니다. 물론 이것만으로 해결되지는 않지만, 데이터 부족 상황에서 일반화 성능을 점검할 때 함께 고려할 수 있는 기본 수단입니다.

### 사례 3. 완전연결층(fully connected layer) 중심 구조

초기 이미지 분류기나 탭형 데이터 분류기에서는 마지막 완전연결층이 매우 크게 잡히는 경우가 많았습니다. 이런 구조에서는 일부 노드가 특정 특징 조합을 거의 외워 버려, 훈련 점수는 높은데 검증 점수는 떨어지는 장면이 자주 나왔습니다. 드롭아웃은 이런 구간에서 과한 의존을 줄이는 대표 regularization 도구였고, 지금도 왜 학습 중 일부 연결을 끄는지 설명할 때 가장 기본이 되는 개념입니다.
...
- regularization을 `벌점 공식`으로만 이해하는 것을 막아 주고
- 학습 중 잡음을 일부러 넣어 일반화를 돕는 사고를 소개하며
- 학습 모드와 평가 모드의 차이가 왜 실질적으로 필요한지 다시 확인시켜 주기 때문입니다
