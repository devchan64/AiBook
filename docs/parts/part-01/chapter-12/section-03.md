...
여기서는 프롬프트의 한계와 검토 기준을 다룹니다. RAG(retrieval-augmented generation), 벡터 검색(vector search), 에이전트 도구 사용(tool use), AI 에이전트(AI agent), 하네스(harness)는 자세히 다루지 않습니다. 그런 구조는 바로 다음 장의 P1-13.3과 P1-13.4, 그리고 P1-14.2부터 P1-14.5에서 다시 다룹니다.
...
또한 여기서는 프롬프트 작성법을 더 늘어놓지 않습니다. 평가와 한계를 분리하고, 왜 다음 장의 RAG, 도구 사용, AI 에이전트, 하네스 같은 구조가 필요해지는지를 설명하는 데 집중합니다.
...
- RAG, 에이전트 도구 사용(tool use), AI 에이전트(AI agent), 하네스(harness)가 왜 다음 단계의 주제가 되는지 예고합니다.
...
학습용 문서를 작성할 때는 특히 `Section 경계`가 중요합니다. 예를 들어 12.3에서 RAG의 세부 구조를 길게 설명하면, P1-13의 도메인을 침범합니다. 출력이 좋아 보여도 현재 Section의 중심 질문을 벗어나면 수정해야 합니다.
...
| 계산이나 파일 확인 | 에이전트 도구 사용(tool use) |
| 여러 단계 작업 관리 | AI 에이전트(AI agent) |
...
- RAG, 에이전트 도구 사용(tool use), AI 에이전트(AI agent), 하네스(harness)가 왜 다음 단계의 주제가 되는지 설명할 수 있다.
- `입력 조건`, `출력 평가`, `근거 확인`, `재현성 기록`을 나누어 프롬프트 개선과 별도 검토 작업을 설명할 수 있다.
- 결과가 그럴듯해 보여도 사실성, 근거성, 최신성을 따로 확인해야 함을 설명할 수 있다.
