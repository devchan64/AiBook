...
또한 RAG(retrieval-augmented generation), 도구 사용(tool use), 에이전트(agent)는 다루지 않습니다. RAG는 다음 장의 13.3과 13.4에서, 도구 사용과 에이전트는 14.2와 14.3에서 다시 연결합니다. 이 절의 목표는 LLM에게 요청을 줄 때 가장 먼저 분리해야 하는 기본 요소를 익히는 것입니다.
