...
P1-14에서는 RAG가 실제 서비스 아키텍처와 도구 사용(tool use), AI 에이전트(AI agent) 구조 안에서 어디에 놓이는지 봅니다. P1-13.3에서는 검색 결과가 LLM 입력으로 들어가는 연결 흐름에 집중합니다.
...
- `검색`, `보강`, `생성`, `검토`를 나누어 RAG를 정답 보장 장치가 아니라 외부 근거 후보 연결 구조로 설명할 수 있다.
