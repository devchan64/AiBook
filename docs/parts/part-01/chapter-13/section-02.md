...
이 절은 유사도 검색의 직관을 다룹니다. 벡터 데이터베이스(vector database), 인덱스(index), 근사 최근접 이웃(approximate nearest neighbor, ANN) 같은 구현 구조는 여기서 자세히 다루지 않고, 13.4에서 `왜 빠른 검색을 위해 별도 구조가 필요한가`라는 구현 직관으로 다시 연결합니다.
...
이 절은 유사도 공식을 외우는 절이 아니라, 벡터 검색이 무엇을 비교하는지 이해하는 절입니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.
