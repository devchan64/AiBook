...
여기서 던지는 핵심 질문은 생성형 AI가 완성된 답을 한 번에 꺼내는가, 아니면 조건을 바탕으로 여러 단계를 거쳐 산출물을 구성하는가입니다.

입문 단계에서 유용한 기준선은 다음과 같습니다.

> 생성형 AI는 조건(condition)을 바탕으로 생성 상태를 여러 단계에 걸쳐 바꾸며 산출물을 구성한다. 다만 텍스트는 다음 토큰을 잇고, 디퓨전 이미지 모델은 노이즈를 줄이며 복원한다는 차이가 있다.
...
| --- | --- | --- |
| 다음 출력 생성 | 조건을 바탕으로 생성 상태를 단계적으로 바꾸는 직관 | 생성 과정을 보는 큰 틀 |
| 다음 토큰 예측 | 문맥을 바탕으로 다음 토큰 후보를 계산하는 방식 | 텍스트 생성의 기본 직관 |
| 오디오 샘플 생성 | 이전 파형을 바탕으로 다음 샘플을 잇는 방식 | 음성 생성의 순차 직관 |
| diffusion 복원 | 노이즈를 점진적으로 줄여 이미지를 복원하는 방식 | Stable Diffusion 같은 이미지 생성이 텍스트와 다른 점 |
| sampling | 후보 중 실제 출력을 선택하는 절차 | 같은 입력에도 결과가 달라지는 이유의 일부 |

여기서 유지해야 할 최소 구분은 `텍스트는 다음 토큰`, `음성은 다음 샘플`, `이미지는 잠재 표현의 복원 과정`, `sampling은 선택 또는 다음 상태로의 이동 절차`입니다.

여기서는 다음 직관을 기준선으로 둡니다.

> 조건을 읽는다.
> 다음 출력 후보를 계산하거나 현재 생성 상태를 갱신한다.
> 하나의 출력을 선택하거나 노이즈를 줄여 복원한다.
...
- 이미지 생성은 다음 픽셀을 단순히 이어 붙이는 방식으로만 설명하면 안 되며, Stable Diffusion 같은 latent diffusion 계열에서는 잠재 표현의 노이즈를 점진적으로 복원한다는 점을 구분합니다.
...
Stable Diffusion 같은 이미지 생성 모델이라면 같은 문장을 조건으로 삼아 이미지 표현을 점진적으로 구성할 수 있습니다.

> 조건:
> 차분한 표지, AI 재학습, 노트북, 노트, 신경망 다이어그램
>
> 생성:
> 잠재 노이즈에서 시작해 조건을 참고하며 이미지로 복원
...
- Stable Diffusion이 프롬프트 조건과 잠재 노이즈를 바탕으로 반복 복원해 이미지를 만든다는 큰 흐름을 설명할 수 있다.
- 생성 결과는 사람의 검토와 반복 수정이 필요하다는 점을 설명할 수 있다.
- 텍스트·음성·이미지 각각에서 무엇이 단계적으로 선택되거나 복원되는지 설명할 수 있다.
