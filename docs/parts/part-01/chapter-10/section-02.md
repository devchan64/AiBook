...
| --- | --- |
| 텍스트 생성은 `다음 토큰을 이어 가는 과정`으로 먼저 이해한다. | Part 5의 P5-1.2, P5-8.1 |
| 같은 입력에서도 후보 분포와 선택 방식 때문에 출력이 달라질 수 있다. | Part 4의 P4-15.2, Part 5의 P5-8.2 |
| 이미지 생성은 텍스트처럼 단순 `다음 조각 붙이기`로만 보면 부족하다. | P1-11.3, Part 4의 P4-14, Part 5 관련 생성 서비스 설명 |
