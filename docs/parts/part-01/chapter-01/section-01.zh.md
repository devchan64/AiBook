...
在 Part 1 中，`AI` 是最外层、最宽的类别。后面的 Section 再次出现这个词时，只会保留当前问题所需的最小连接；如果需要重新拆分这个范围，就回到这一节，以及 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary/)。
...
这一节最先应该留下的基准，是 `AI 是宽广领域`，而 `AI 系统是具有输入、目标与输出的结构`。这里之所以把 `影响(impact)` 也放进来，是因为同一种输出在真实环境中可能导致完全不同的后果。如果术语再次变得不稳定，就回到 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary/) 对照看。
...
这一节至少要留下的一句话是：`AI 不是一个产品，而是一大类具有输入、目标和输出的系统。` 之后在 Part 1 里再次遇到 `AI` 这个词时，就先把这一句当作基准线；若需要核对术语，再回到 [Concept Glossary (English)](/AiBook/en/reference/concept-glossary/)。
