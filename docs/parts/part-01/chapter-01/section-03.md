...
각 층위의 내부 구조는 머신러닝 전개를 다루는 Part 3, 딥러닝 구조를 다루는 Part 4, LLM과 생성형 AI를 다루는 Part 5에서 다시 회수합니다. 이 절은 먼저 용어의 `자리`를 구분하는 데 집중합니다.
...
| --- | --- | --- |
| 머신러닝 | Part 3 | 문제, 데이터, 평가를 어떤 구조로 읽는지 가장 먼저 본격적으로 정리하기 때문입니다. |
| 딥러닝 | Part 4 | 표현 학습, 역전파, CNN, Attention, Transformer 같은 계산 구조를 실제로 회수하기 때문입니다. |
| 생성형 AI | Part 5 | 생성 구조, RAG, 도구 사용, agent를 서비스 관점까지 이어서 설명하기 때문입니다. |
| LLM | Part 5 | 토큰, next-token prediction, GPT 계열, 지시 튜닝을 한 흐름으로 본격 설명하기 때문입니다. |
