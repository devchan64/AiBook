...
이 절은 용어의 포함 관계를 완벽하게 외우는 절이 아닙니다. 아래 세 가지 관점을 전체 지도로 삼으면 흐름을 잡기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| AI가 가장 넓고, 머신러닝과 딥러닝은 그 안의 접근이라는 점 | 서로 다른 말을 같은 급으로 비교하는 실수를 줄여 줍니다. | `AI > 머신러닝 > 딥러닝`이라는 큰 흐름을 먼저 구분합니다. |
| 생성형 AI는 `무엇을 만든다`는 출력 성격의 말이라는 점 | 딥러닝, LLM, 서비스 경험을 한 덩어리로 오해하지 않게 해 줍니다. | 생성형 AI가 텍스트나 이미지를 만드는 쪽이라는 감각을 먼저 잡습니다. |
| LLM은 생성형 AI의 대표 사례이지만 AI 전체는 아니라는 점 | 최근 사용 경험만으로 AI 전체를 좁게 보는 일을 막아 줍니다. | `AI = LLM`이 아니라는 점을 먼저 분명히 구분합니다. |

`분야`, `학습 접근`, `모델 방법`, `출력 범주`, `모델 계열`은 초반에 서로 비슷하게 들릴 수 있습니다. 이 절에서는 아래 정도로 짧게 구분해 두고 계속 읽으면 됩니다.
...
이 단계에서는 `AI는 가장 넓은 분야`, `머신러닝은 학습 접근`, `딥러닝은 그중 한 방법`, `생성형 AI는 출력 범주`, `LLM은 그 안의 대표 모델 계열`이라는 구분이 먼저 남아 있으면 됩니다.
...
이 도식의 핵심은 화살표를 엄격한 계보표처럼 외우는 데 있지 않습니다. 여기서는 `AI가 가장 넓은 바깥 범주`, `머신러닝과 딥러닝은 학습 접근 쪽`, `생성형 AI는 출력 범주 쪽`, `LLM은 생성형 AI 안의 대표 언어 모델 계열`이라는 자리만 먼저 잡으면 됩니다.
...
이 용어들을 여기서 한 번에 끝내려 하기보다, `어느 Part에서 본격적으로 다시 읽는가`를 먼저 연결해 두는 편이 더 중요합니다.
