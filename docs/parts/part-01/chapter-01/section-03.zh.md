...
`AI` 是最宽的领域与系统类别；`machine learning` 是通过数据改善性能的学习方法；`deep learning` 是其中更强调神经网络与表征学习的技术路径；`generative AI` 是产生新内容的输出类别；`LLM` 则是其中一类代表性的语言模型家族。也就是说，应把它们读成：`AI 是外层类别`，`machine learning/deep learning 是学习方法`，`generative AI 是输出类别`，`LLM 是语言模型家族`。
...
```mermaid
--8<-- "assets/part-01/chapter-01/ai-to-llm-map-zh.mmd"
```
...
- 你可以说明 `AI`、`machine learning`、`deep learning`、`generative AI`、`LLM` 并不是同一层级的词。
- 你可以说明生成式 AI 比 LLM 更宽，而 LLM 是其中有代表性的语言模型家族。
- 你可以说明真实服务不会止于一个模型，还应与检索、权限、日志、安全过滤等部件一起阅读。
