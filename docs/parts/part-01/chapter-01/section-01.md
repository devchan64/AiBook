...
세부 기술 흐름은 P1-2, P1-3, 그리고 Part 3 이후에서 다시 회수합니다. 이 절은 먼저 `AI가 가장 넓은 바깥 범주`라는 감각을 잡는 데 집중합니다.
...
| 탐색과 휴리스틱 | 후보를 탐색하되 경험적 기준으로 줄임 | P1-7에서 다시 다룸 |
| DSS/BI/DW/OLAP | 데이터를 모아 의사결정에 연결 | AI를 둘러싼 데이터 기반 시스템 배경 |
| 데이터마이닝과 머신러닝 | 데이터에서 패턴과 예측 기준을 찾음 | P1-3 이후와 Part 3에서 다시 다룸 |
| 딥러닝과 생성형 AI | 표현과 가중치를 학습해 복잡한 출력을 만듦 | P1-9 이후와 Part 4, Part 5에서 다시 다룸 |
