...
처음 읽을 때는 `분야`, `시스템`, `입력`, `출력`, `목표`가 모두 비슷한 말처럼 느껴질 수 있습니다. 이 절에서는 다음 정도로만 짧게 구분해 두면 충분합니다.
...
처음 단계에서는 `AI는 넓은 분야`, `그 안의 AI 시스템은 입력과 목표와 출력을 가진 구조` 정도로만 잡아도 충분합니다.
...
여기서 `DSS/BI/DW/OLAP`를 함께 적어 둔 이유는, 데이터 모델링이 나중에 갑자기 등장한 별도 기술이 아니라 데이터를 모아 의사결정에 연결하던 흐름 위에서 자라난다는 배경을 먼저 보여 주기 위해서입니다. 처음 읽을 때는 약어를 모두 외울 필요는 없습니다. `DSS(decision support system)`는 의사결정 지원 시스템, `BI(business intelligence)`는 데이터를 읽어 업무 판단에 쓰는 체계, `DW(data warehouse)`는 데이터를 모아 두는 저장 구조, `OLAP(online analytical processing)`는 모아 둔 데이터를 여러 기준으로 분석하는 방식을 가리킨다고만 잡아도 충분합니다.
...
- AI는 하나의 제품 이름이 아니라, 입력을 받아 판단이나 출력을 만드는 넓은 시스템 범주입니다.
