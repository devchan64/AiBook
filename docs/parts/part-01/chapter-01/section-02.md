...
처음 읽을 때는 `입력`, `출력`, `목표`, `영향`이 서로 비슷하게 들릴 수 있습니다. 이 절에서는 아래처럼 짧게 다시 구분해 두면 충분합니다.
...
처음 단계에서는 `입력은 넣는 정보`, `출력은 나오는 결과`, `목표는 왜 그렇게 만들려는가`, `영향은 그 결과가 어디에 쓰이는가` 정도로만 잡아도 충분합니다.
...
이 도식은 현실의 넓은 질문이 그대로 모델 문제가 되지 않고, 먼저 `문제 정의`, `입력`, `출력`으로 잘려 나간 뒤 실제 결과가 사람이나 환경에 영향을 주는 흐름을 보여 줍니다. 초심자는 이 그림을 볼 때 `현실의 질문 -> 계산 가능한 문제 -> 결과의 영향`이라는 세 단계만 먼저 잡아도 충분합니다.
