...
문제 유형별 알고리즘과 학습 구조는 뒤의 Part 3, Part 4, Part 5에서 다시 회수합니다. 이 절은 먼저 `무엇을 넣고 무엇이 나오는가`라는 문제 정의 감각을 만드는 데 집중합니다.
...
사용자가 제품 매뉴얼 사이트에 질문을 입력한다고 해 보겠습니다. 사람은 겉으로 보이는 검색창 하나만 보고 `이건 검색 문제`라고 생각하기 쉽습니다. 하지만 실제로는 질문 문장을 읽는 과정은 언어 이해에 가깝고, 문서 후보를 찾는 과정은 검색 문제이며, 후보를 다시 정렬하는 과정은 랭킹 문제이고, 마지막에 답변 문장을 쓰면 생성 문제까지 붙습니다. 이 사례는 실제 서비스가 하나의 AI 문제만 다루는 것이 아니라, 여러 문제 유형을 한 흐름으로 묶어 놓는 경우가 많다는 점을 보여 줍니다.
