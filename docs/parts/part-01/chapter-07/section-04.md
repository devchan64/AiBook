...
여기서는 센서(sensor), 인식(perception), 지도 제작(map making), 제어(control), 차량 동역학(vehicle dynamics)을 깊게 다루지 않습니다. 강화학습(reinforcement learning) 중심의 자율주행 접근도 여기서 비교하지 않습니다. 강화학습 자체는 P1-8.3과 Part 4 Chapter 19에서 다시 보지만, 자율주행 안에서의 RL 계열 비교는 이 책 본편 범위 밖으로 둡니다.
...
다음처럼 추상적인 탐색 개념을 실제 시스템 구조와 연결해 보여 줘야 할 때 이 절의 관점을 먼저 떠올리면 됩니다.

- 길찾기, 경로(path), 궤적(trajectory), 제어가 어디서 갈리는지 사례로 설명해야 할 때
- 자율주행을 하나의 거대한 모델이 아니라 여러 계획 층위로 나눠 읽어야 할 때
- 7.1~7.3의 탐색, 휴리스틱, 운영 기준이 실제 시스템에서 어떻게 분해되는지 보여 주고 싶을 때

이때는 먼저 `큰 경로를 정하는 일`, `지금 실행할 짧은 궤적을 고르는 일`, `실제 조향과 가감속으로 바꾸는 일`을 나누면 됩니다. 그러면 도메인 사례를 추가하더라도 핵심은 자율주행 자체가 아니라 계획 층위와 개념 대응이라는 점이 유지됩니다.
