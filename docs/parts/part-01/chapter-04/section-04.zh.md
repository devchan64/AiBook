...
问题定义不是摆在模型前面的附属准备工作，而是决定模型看什么、输出什么，以及怎样判断它是否做得好的核心阶段。

把 Chapter 4 的脉络重新串起来，就是下面这样：

1. 现实问题为了某个目的被缩减后，才变成模型
2. 模型接收输入并产出输出
3. 输入会被转成特征和表示来参与计算
4. 问题定义决定这一切选择的标准
...
如果这句话还补不完整，那问题通常还不是“该选哪个模型”，而是“现实目标本身还没有被整理清楚”。
...
当大家已经开始讨论要用什么模型，但你感觉问题本身其实还没有被定义清楚时，就该先想起这一节的视角。

- 当 `提升客户满意度`、`减少配送问题` 这类目标还没被缩窄成建模任务时
- 当团队一直在“该做分类、打分还是生成回复”之间摇摆时
- 当你需要解释为什么模型指标提高了，但真实业务表现并没有同步变好时

这时先把问题重新按 `现实目标 -> 建模任务 -> 输入/输出/数据 -> 评价 -> 业务表现` 的顺序写一遍。这样就会看见：只要输出定义改变，数据和评价也会一起改变，模型选择讨论才会真正站稳。
