...
Part 1에서 `문제 정의(task definition)`와 `현실 목표를 모델링 과제로 바꾸는 방식`의 기본 기준은 이 절에서 잡습니다. `모델`, `입력/출력/데이터`, `특징/표현/파라미터`의 기본 뜻은 4.1~4.3에서 먼저 잡았고, 여기서는 그 요소들을 어떤 과제로 묶을지와 무엇을 잘했다고 볼지를 정하는 단계에 집중합니다.
...
| --- | --- | --- |
| 현실 목표와 모델링 과제는 같지 않다는 점 | AI가 해결하는 범위를 과장하지 않게 해 줍니다. | `고객 만족 향상`과 `문의 분류`를 서로 다른 층위로 구분합니다. |
...
`현실 목표`, `모델링 과제`, `출력 정의`, `평가`, `업무 성과`는 서로 연결되어 있지만 같은 말이 아닙니다.
...
여기서는 `현실 목표는 바깥 문제`, `모델링 과제는 좁힌 계산 문제`, `출력 정의는 모델에게 시키는 일`, `평가는 모델 점검`, `업무 성과는 실제 결과`라는 자리 구분이 중요합니다.
...
`현실 목표 -> 모델이 풀 수 있는 과제 -> 입력/출력/데이터/평가`라는 순서로 정리할 수 있습니다.

이 순서는 단순한 기획 문장이 아니라, 뒤에서 모델 후보와 데이터 수집 범위를 흔들리지 않게 잡아 주는 기준입니다. 여기서는 `현실 문제를 바로 모델에 넣는 것이 아니라 중간에 과제 정의가 한 번 필요하다`는 점을 분명히 이해하는 것이 핵심입니다.
...
실무에서는 같은 목표를 놓고도 팀마다 다른 출력을 먼저 상상하는 경우가 많습니다. 상담 운영팀은 `문의 유형을 자동으로 붙이자`고 생각할 수 있고, 관리자는 `긴급한 문의를 먼저 올리자`고 생각할 수 있으며, 작성 지원을 원하는 팀은 `답변 초안을 만들자`고 생각할 수 있습니다. 세 방향은 모두 `문의 처리를 더 잘하자`는 같은 목표에서 출발하지만, 실제 모델링 과제는 분류, 점수 예측, 생성으로 달라집니다. 이런 갈라짐은 혼란이 아니라 정상이며, 문제 정의의 역할은 바로 그 차이를 문서로 고정하는 데 있습니다.
...
1. 현실 문제를 목적에 맞게 줄이면 모델이 됩니다.
2. 모델은 입력을 받아 출력을 만듭니다.
3. 입력은 특징과 표현으로 바뀌어 계산됩니다.
4. 문제 정의는 이 모든 선택의 기준을 정합니다.

따라서 모델을 고르기 전에 다음 문장을 완성해야 합니다.
...
다음처럼 어떤 모델을 쓸지 논의하기 전에 아직 문제 자체가 충분히 정의되지 않았다는 느낌이 들 때 이 절의 관점을 먼저 떠올리면 됩니다.

- `고객 만족을 높이자`, `배송 문제를 줄이자` 같은 목표가 모델링 과제로 아직 좁혀지지 않았을 때
- 분류를 할지, 점수를 낼지, 답변을 생성할지에 따라 팀 논의가 자꾸 갈릴 때
- 모델 지표는 좋아졌는데 실제 업무 성과는 나아지지 않는 이유를 설명해야 할 때

이때는 먼저 `현실 목표 -> 모델링 과제 -> 입력/출력/데이터 -> 평가 -> 업무 성과` 순서로 다시 적어 보면 됩니다. 그러면 출력 정의가 바뀌면 데이터와 평가도 함께 바뀐다는 점이 드러나고, 모델 선택을 논의할 기반이 생깁니다.
