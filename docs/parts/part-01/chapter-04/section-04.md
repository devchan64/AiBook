...
이 절은 좋은 알고리즘을 고르기 전에 좋은 문제 정의가 왜 필요한지 설명하는 절입니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 현실 목표와 모델링 과제는 같지 않다는 점 | AI가 해결하는 범위를 과장하지 않게 해 줍니다. | “고객 만족 향상”과 “문의 분류”를 구분할 수 있으면 충분합니다. |
| 같은 목표도 여러 출력 정의로 바뀔 수 있다는 점 | 분류, 점수 예측, 생성이 어떻게 갈라지는지 보여 줍니다. | 무엇을 출력하게 할지 먼저 정해야 한다는 점만 잡으면 충분합니다. |
| 출력 정의가 데이터와 평가 기준까지 함께 바꾼다는 점 | 모델 선택 이전에 문제 설계가 중요하다는 사실을 보여 줍니다. | 출력이 바뀌면 필요한 사례와 잘했다는 기준도 바뀐다는 점만 이해하면 충분합니다. |
