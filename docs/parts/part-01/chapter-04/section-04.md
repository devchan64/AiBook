...
Part 1 안에서는 이 절을 `문제 정의(task definition)`와 `현실 목표를 모델링 과제로 바꾸는 방식`의 대표 상세 설명 위치로 사용합니다. `모델`, `입력/출력/데이터`, `특징/표현/파라미터`의 기본 뜻은 4.1~4.3에서 먼저 잡았고, 여기서는 그 요소들을 어떤 과제로 묶을지와 무엇을 잘했다고 볼지를 정하는 단계에 집중합니다.
...
이 절에서는 그 앞 단계만 다룹니다. 어떤 현실 목표를 어떤 입력과 출력의 문제로 정의하고, 무엇을 잘했다고 평가할지 정하는 단계입니다.
...
이 절에서는 좋은 알고리즘을 고르기 전에 문제 정의가 왜 필요한지 먼저 고정합니다. 아래 세 가지 관점이 그 출발점입니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 현실 목표와 모델링 과제는 같지 않다는 점 | AI가 해결하는 범위를 과장하지 않게 해 줍니다. | `고객 만족 향상`과 `문의 분류`를 서로 다른 층위로 먼저 구분합니다. |
| 같은 목표도 여러 출력 정의로 바뀔 수 있다는 점 | 분류, 점수 예측, 생성이 어떻게 갈라지는지 보여 줍니다. | 무엇을 출력하게 할지 먼저 정해야 한다는 점을 잡습니다. |
| 출력 정의가 데이터와 평가 기준까지 함께 바꾼다는 점 | 모델 선택 이전에 문제 설계가 중요하다는 사실을 보여 줍니다. | 출력이 바뀌면 필요한 사례와 잘했다는 기준도 함께 바뀐다는 점을 먼저 이해합니다. |

`현실 목표`, `모델링 과제`, `출력 정의`, `평가`, `업무 성과`는 서로 연결되어 있지만 같은 말이 아닙니다. 이 절에서는 먼저 아래처럼 짧게 구분합니다.
...
여기서는 `현실 목표는 바깥 문제`, `모델링 과제는 좁힌 계산 문제`, `출력 정의는 모델에게 시키는 일`, `평가는 모델 점검`, `업무 성과는 실제 결과`라는 자리만 먼저 분리해 두면 됩니다.
