...
Google의 머신러닝 용어집은 자연어 이해(NLU, Natural Language Understanding)를 사용자가 말하거나 입력한 것의 의도를 결정하는 자연어 처리의 하위 영역으로 설명합니다. 여기서는 인텐트가 모델 내부 파라미터가 아니라, 입력 표현을 사용해 얻는 업무적 해석이나 출력에 가깝다는 자리만 구분합니다.
