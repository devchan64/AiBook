...
4.1에서는 모델을 현실 문제 전체가 아니라 목적에 맞게 줄여 만든 계산 가능한 모형으로 정리했습니다. 이제 그 모델을 설명할 때 가장 먼저 나누어 볼 세 요소를 봅니다.
...
특징과 표현, 파라미터의 관계는 바로 다음 4.3에서 다시 회수하고, 문제 정의가 모델 선택을 어떻게 바꾸는지는 4.4에서 이어집니다. 입력과 출력을 실제 특징·표현 계산으로 바꾸는 구조는 Part 3의 모델 장과 Part 4의 신경망 장에서 다시 연결합니다. 이 절은 먼저 `무엇을 넣고 무엇을 얻을 것인가`를 분리하는 데 집중합니다.
