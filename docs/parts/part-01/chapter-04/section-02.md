...
이 절은 한 번에 많은 용어를 외우기보다, 하나의 예시를 계속 바꿔 보며 읽는 방식으로 진행합니다. 예시 상황은 `고객 문의를 AI로 도와주고 싶다`는 문제입니다.

Part 1 안에서는 이 절을 `입력(input)`, `출력(output)`, `데이터(data)`, `사례(example)`, `라벨(label)`의 대표 상세 설명 위치로 사용합니다. `모델`과 `시스템`의 기본 구분은 4.1에서 먼저 잡았고, `특징`, `표현`, `파라미터`의 차이는 4.3에서 더 자세히 다룹니다. 여기서는 먼저 `무엇을 넣고 무엇을 얻을 것인가`와 `어떤 사례를 모아야 하는가`를 분리하는 데 집중합니다.
...
특징과 표현, 파라미터의 관계는 바로 다음 4.3에서 다시 회수하고, 문제 정의가 모델 선택을 어떻게 바꾸는지는 4.4에서 이어집니다. 입력과 출력을 실제 특징·표현 계산으로 바꾸는 구조는 Part 4의 머신러닝 장과 Part 5의 신경망 장에서 다시 연결합니다. 이 절은 먼저 `무엇을 넣고 무엇을 얻을 것인가`를 분리하는 데 집중합니다.
...
이 절은 용어를 많이 외우는 절이 아니라, 현실 문제를 계산 가능한 문제로 줄이는 절입니다. 아래 세 가지 관점을 먼저 잡으면 구조를 따라가기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| 입력(input)은 모델이 보는 정보라는 점 | 모델이 무엇을 근거로 판단하는지 보여 줍니다. | 문의 문장, 이미지, 센서값처럼 “넣는 정보”라는 감각을 먼저 잡습니다. |
| 출력(output)은 모델이 만들어야 할 결과라는 점 | 같은 현실 문제도 다른 과제로 바뀔 수 있음을 이해하게 해 줍니다. | 분류, 점수, 문장처럼 “무엇을 내보낼지”가 먼저 정해져야 한다는 점을 먼저 구분합니다. |
| 데이터(data)는 입력과 출력의 사례 모음이라는 점 | 단순 파일 묶음이 아니라 학습 기준이라는 점을 보여 줍니다. | 과거 사례가 모델에게 무엇을 배우게 하는지 먼저 연결합니다. |

`입력`, `출력`, `데이터`, `사례`, `라벨`은 초반에 모두 비슷하게 들릴 수 있습니다. 이 절에서는 아래 정도로 짧게 구분해 두고 계속 읽으면 됩니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 입력 | 모델이 실제로 받는 정보 | 계산의 출발점 |
| 출력 | 모델이 내야 하는 결과 | 모델에게 시키는 일의 정의 |
| 데이터 | 과거 사례의 모음 | 학습과 점검의 재료 |
| 사례 | 데이터 안의 한 건 | 입력과 출력이 짝을 이루는 단위 |
| 라벨 | 사례에 붙인 정답 출력 | 지도학습에서 모델이 맞혀야 할 이름표 |

이 단계에서는 `입력은 넣는 정보`, `출력은 얻고 싶은 결과`, `데이터는 사례 묶음`, `사례는 한 건`, `라벨은 정답 이름`이라는 구분이 먼저 남아 있으면 됩니다.
...
`입력`, `출력`, `데이터`, `사례`, `라벨`은 비슷하게 들릴 수 있습니다. 이 절에서는 다음처럼 짧게 다시 잡아 두고 계속 읽으면 됩니다.
...
모델은 이 반복된 사례를 통해 입력과 출력 사이의 관계를 조정합니다. Google의 머신러닝 입문 자료는 지도학습에서 예시가 특징과 라벨을 포함한다고 설명합니다. 여기서는 아직 특징을 자세히 다루지 않으므로, “모델이 참고할 입력과 원하는 출력이 함께 있는 사례”로 이해하면 됩니다.
