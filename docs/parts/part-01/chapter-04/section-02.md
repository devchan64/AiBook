...
특징과 표현, 파라미터의 관계는 바로 다음 4.3에서 다시 회수하고, 문제 정의가 모델 선택을 어떻게 바꾸는지는 4.4에서 이어집니다. 이 절은 먼저 `무엇을 넣고 무엇을 얻을 것인가`를 분리하는 데 집중합니다.
...
이 절은 용어를 많이 외우는 절이 아니라, 현실 문제를 계산 가능한 문제로 줄이는 절입니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.
