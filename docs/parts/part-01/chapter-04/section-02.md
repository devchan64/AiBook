...
이 절은 용어를 많이 외우는 절이 아니라, 현실 문제를 계산 가능한 문제로 줄이는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 입력(input)은 모델이 보는 정보라는 점 | 모델이 무엇을 근거로 판단하는지 보여 줍니다. | 문의 문장, 이미지, 센서값처럼 “넣는 정보”라는 감각만 잡으면 충분합니다. |
| 출력(output)은 모델이 만들어야 할 결과라는 점 | 같은 현실 문제도 다른 과제로 바뀔 수 있음을 이해하게 해 줍니다. | 분류, 점수, 문장처럼 “무엇을 내보낼지”가 먼저 정해져야 한다는 점만 알면 충분합니다. |
| 데이터(data)는 입력과 출력의 사례 모음이라는 점 | 단순 파일 묶음이 아니라 학습 기준이라는 점을 보여 줍니다. | 과거 사례가 모델에게 무엇을 배우게 하는지 연결되면 충분합니다. |
...
이 도식은 현실 문제를 바로 모델에 넣는 것이 아니라, 먼저 `입력`, `목표 출력`, `과거 사례`로 나눈 뒤 학습과 예측으로 이어지는 구조를 보여 줍니다. 왼쪽의 `real-world problem`은 아직 넓은 현실 문제이고, 그 아래로 내려가며 무엇을 모델이 실제로 보게 할지와 무엇을 맞히게 할지가 좁혀집니다. 초심자는 이 그림을 `현실 문제를 바로 푸는 기계`가 아니라 `현실 문제를 계산 가능한 작은 과제로 잘라 내는 과정`으로 읽으면 충분합니다.
...
처음 읽을 때는 `입력`, `출력`, `데이터`, `사례`, `라벨`이 비슷하게 들릴 수 있습니다. 이 절에서는 다음처럼 짧게 다시 잡아 두면 충분합니다.
...
처음 단계에서는 `입력은 넣는 정보`, `출력은 얻고 싶은 결과`, `데이터는 그런 사례 묶음`, `라벨은 정답 이름` 정도로만 구분해도 충분합니다.
