...
- 구체적인 머신러닝 알고리즘 구조
- 특징(feature), 표현(representation), 파라미터(parameter)의 세부 계산
- 서비스 아키텍처 전체 설계

입력, 출력, 데이터의 관계는 바로 다음 4.2에서 다시 회수하고, 특징·표현·파라미터는 4.3에서 더 구체적으로 이어집니다. 문제 정의가 모델 선택을 어떻게 바꾸는지는 4.4에서 다시 정리합니다. 여기서는 `모델은 계산용 모형`이라는 구분에 집중합니다.
...
다음처럼 `모델`이라는 말을 들었을 때 그것이 현실 전체인지, 계산용 부품인지 다시 분리해야 할 때 이 절의 관점을 먼저 떠올리면 됩니다.

- 모델 성능이 곧바로 업무 해결과 같다고 느껴질 때
- LLM이나 분류 모델 하나를 서비스 전체와 같은 말처럼 부르고 싶어질 때
- 어떤 시스템에서 모델이 맡는 일과 정책, 검토, 실행 절차가 맡는 일을 나눠 설명해야 할 때

이때는 먼저 `모델은 목적에 맞게 줄인 계산용 모형`이고, `시스템은 그 모델을 둘러싼 전체 절차`라는 기준으로 돌아가면 됩니다. 그런 다음 이 모델이 무엇을 입력으로 받고 무엇을 출력하며, 무엇을 아예 맡지 않는지 확인하면 역할 경계가 정리됩니다.
