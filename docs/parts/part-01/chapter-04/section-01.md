...
이 절은 모델의 수학적 정의를 세우는 절이 아닙니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 모델(model)은 현실 전체가 아니라 목적에 맞게 줄인 모형이라는 점 | AI 모델을 사람 같은 존재로 오해하지 않게 해 줍니다. | 지도나 건물 모형처럼 필요한 부분만 남긴 표현이라는 비유가 잡히면 충분합니다. |
| 모델을 볼 때 대상, 목적, 단순화, 한계를 함께 봐야 한다는 점 | 모델이 잘하는 일과 못하는 일을 같이 읽게 해 줍니다. | 무엇을 위해 만들었고 무엇을 빼 버렸는지 묻는 습관만 생기면 충분합니다. |
| 모델은 현실 문제 전체가 아니라 좁은 계산 과제를 맡는다는 점 | 이후 입력, 출력, 데이터 설명으로 자연스럽게 이어집니다. | “고객 불만 해결” 전체가 아니라 “문의 분류” 같은 작은 과제를 맡는다고 이해하면 충분합니다. |
...
처음 읽는 독자에게는 `모델`, `입력`, `출력`, `학습`, `시스템`이 한 덩어리처럼 들릴 수 있습니다. 이 절에서는 아래처럼 짧게 다시 구분해 두면 충분합니다.
