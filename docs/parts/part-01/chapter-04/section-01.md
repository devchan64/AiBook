...
입력, 출력, 데이터의 관계는 바로 다음 4.2에서 다시 회수하고, 특징·표현·파라미터는 4.3에서 더 구체적으로 이어집니다. 이 절은 먼저 `모델은 계산용 모형`이라는 감각을 잡는 데 집중합니다.
...
이 절은 모델의 수학적 정의를 세우는 절이 아닙니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.
