...
9.2 最重要的不是记住几个模型内部的全部细节，而是记住 `问题重构`。

> 目标检测：同时预测图像里的位置和类别。  
> 语音生成：生成按时间排序的音频样本。  
> TTS：语音生成模型与真实语音合成任务相遇的辅助语境。

这些案例说明，深度学习已经从图像分类扩展到了更复杂的输出结构。但它们不应被写成 LLM 的直接祖先。把 `direct lineage` 和 `surrounding evidence` 分开，是这一章的重要目的之一。
