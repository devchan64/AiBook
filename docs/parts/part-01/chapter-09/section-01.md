...
이 네 요소는 따로 떨어진 장점이 아니라 서로 맞물린 조건이었습니다. 대규모 데이터가 없으면 깊은 모델은 충분히 다양한 예시를 보지 못합니다. 깊은 CNN이 없으면 이미지 안의 낮은 수준 단서와 높은 수준 표현을 여러 층으로 조합하기 어렵습니다. GPU가 없으면 큰 모델을 여러 번 반복해 훈련하고 비교하는 일이 현실적인 시간 안에 어렵습니다. 정규화(regularization)와 데이터 증강(data augmentation) 같은 학습 기법이 없으면 큰 모델은 훈련 데이터에만 지나치게 맞춰질 위험이 커집니다.

그래서 AlexNet을 볼 때는 “좋은 CNN 하나가 등장했다”보다 다음처럼 읽는 편이 더 정확합니다.

> 대규모 라벨 데이터
> + 깊은 CNN 구조
> + GPU 기반 계산
> + 과적합을 줄이는 학습 기법
> = 이미지 인식에서 딥러닝이 강한 성능을 보인 전환점
...
이 배경 위에서 뒤의 절은 객체 검출과 음성 생성 사례를 봅니다. 9.2에서도 같은 원칙을 유지합니다. YOLO와 WaveNet 같은 사례는 LLM의 직접 원인이 아니라, 딥러닝 패러다임이 여러 입력과 출력 문제로 확산된 주변 근거로 다룹니다.
...
- 사람이 특징을 직접 설계하던 방식과 모델이 표현을 함께 학습하는 방식을 대비해 설명할 수 있다.
- 이미지 인식 사례를 `LLM의 직접 계보`가 아니라 딥러닝 패러다임 확산의 배경으로 위치시킬 수 있다.
...
- Geoffrey E. Hinton, Simon Osindero, Yee-Whye Teh, [A Fast Learning Algorithm for Deep Belief Nets](https://direct.mit.edu/neco/article/18/7/1527/7065/A-Fast-Learning-Algorithm-for-Deep-Belief-Nets){: target="_blank" rel="noopener noreferrer" }, Neural Computation 18(7), 1527-1554, 2006-07-01, 확인 날짜: 2026-07-19.
