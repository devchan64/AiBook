...
이 절은 “정답이 없다”는 말만으로 비지도학습을 설명하지 않기 위한 절입니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.
...
이 절에서는 표현 학습(representation learning)을 깊게 다루지 않습니다. 표현을 모델이 함께 학습하는 흐름은 Part 1의 딥러닝 장과 Part 4에서 다시 연결하고, 임베딩이 검색과 RAG로 이어지는 흐름은 Part 5에서 다시 봅니다. 여기서는 비지도학습이 “라벨 없이 데이터의 구조가 보이도록 표현을 바꾸는 일”과 연결될 수 있다는 정도만 잡습니다.
