# 5.2 모델 실행(inference)은 무엇을 실행하는가
...
Part 1 안에서는 이 절을 `inference`와 `모델 실행(model execution)`의 대표 상세 설명 위치로 사용합니다. `학습`, `훈련`, `적합`의 기본 구분은 5.1에서 먼저 잡았고, reasoning 등 다른 `추론` 계열 용어와의 충돌은 5.3에서 더 자세히 다룹니다. 여기서는 먼저 `학습된 값을 사용해 새 입력에서 출력을 만드는 단계`와 `서비스 전체 처리`를 분리하는 데 집중합니다.
...
한국어 정서로는 `모델 실행`, `모델 적용`, `출력을 만드는 실행`에 가깝게 이해하면 됩니다. 모델이 생각하거나 판단한다는 느낌을 먼저 붙이기보다, 이미 훈련된 계산 구조를 새 입력에 적용해 결과를 만드는 단계로 보는 것입니다.

> 훈련: 모델을 준비한다.
> inference: 준비된 모델을 실행한다.
> 후처리: 모델 출력을 사람이 쓰는 업무 결과로 연결한다.

이 세 줄은 `모델이 결과를 계산하는 단계`와 `서비스가 그 결과를 사용해 일을 처리하는 단계`를 나누기 위한 기준입니다. 초심자는 먼저 `모델 실행`과 `업무 처리 전체`가 같지 않다는 점만 분명히 이해하면 충분합니다.
...
- inference를 한국어로 `모델 실행`, `모델 적용`, `출력을 만드는 실행`에 가깝게 풀어 읽습니다.
...
이 절은 어려운 계산보다 “지금 모델이 실제로 무엇을 하고 있는가”를 읽는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| inference는 학습된 모델을 새 입력에 적용하는 실행이라는 점 | `추론`이라는 번역이 주는 과한 의미를 줄여 줍니다. | 새 입력을 넣어 출력을 받는 단계라고만 이해하면 충분합니다. |
| training과 inference는 다른 단계라는 점 | 학습 중과 사용 중을 섞지 않게 해 줍니다. | training은 내부 값을 바꾸고, inference는 보통 바꾸지 않는다고 알면 충분합니다. |
| inference 결과는 서비스 전체 처리와 같지 않다는 점 | 모델 출력과 업무 결정의 차이를 이해하게 해 줍니다. | 모델이 점수나 문장을 만들고, 그 뒤에 정책과 사람 검토가 붙을 수 있다는 점만 알면 충분합니다. |

처음 읽을 때는 `inference`, `prediction`, `response generation`, `후처리`가 모두 비슷하게 들릴 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.
...
처음 단계에서는 `inference는 모델 실행`, `prediction은 예측 결과`, `response generation은 생성 결과`, `후처리는 모델 밖 처리` 정도로만 잡아도 충분합니다.
