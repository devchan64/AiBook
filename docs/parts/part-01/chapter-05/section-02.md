...
이 절은 어려운 계산보다 “지금 모델이 실제로 무엇을 하고 있는가”를 읽는 절입니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.
...
이 설명은 LLM의 내부 알고리즘을 자세히 설명하려는 것이 아닙니다. 핵심은 LLM에서도 inference가 “학습된 모델을 새 입력에 적용해 출력을 만드는 실행”이라는 점입니다. 토큰화, 디코딩, 샘플링, attention 계산은 뒤의 LLM과 딥러닝 장에서 다시 다룹니다.
