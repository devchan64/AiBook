...
AI 문서에서는 이 과정을 보통 `inference`라고 부릅니다. 한국어로는 흔히 `추론`이라고 번역하지만, 여기서는 영어 `inference`를 함께 둡니다. 한국어 `추론`이 reasoning, prediction, statistical inference까지 함께 덮을 수 있기 때문입니다. 이 용어 혼동은 5.3에서 따로 다룹니다.

이 절에서는 `inference`를 `학습된 값을 사용해 새 입력에서 출력을 만드는 단계`로 두고, 그것을 서비스 전체 처리와 구분합니다. `학습`, `훈련`, `적합`의 기본 구분은 5.1에서, reasoning 등 다른 `추론` 계열 용어와의 충돌은 5.3에서 이어집니다.

## 한국어로는 어떻게 받아들이면 좋은가

`inference`는 한국어로 흔히 `추론`이라고 번역되지만, 5.2에서는 다음처럼 풀어 읽습니다.

> inference = 학습된 모델을 새 입력에 적용해 출력을 만드는 실행

한국어 정서로는 `모델 실행`, `모델 적용`, `출력을 만드는 실행`에 가깝게 이해합니다. 모델이 생각하거나 판단한다는 느낌을 붙이기보다, 이미 훈련된 계산 구조를 새 입력에 적용해 결과를 만드는 단계로 보는 것입니다.

> 훈련: 모델을 준비한다.
> inference: 준비된 모델을 실행한다.
> 후처리: 모델 출력을 사람이 쓰는 업무 결과로 연결한다.

이 세 줄은 `모델이 결과를 계산하는 단계`와 `서비스가 그 결과를 사용해 일을 처리하는 단계`를 나누기 위한 기준입니다. 여기서는 `모델 실행`과 `업무 처리 전체`가 같지 않다는 점이 중요합니다.
...
| --- | --- | --- |
| inference는 학습된 모델을 새 입력에 적용하는 실행이라는 점 | `추론`이라는 번역이 주는 과한 의미를 줄여 줍니다. | 새 입력을 넣어 출력을 받는 단계라고 이해합니다. |
...
`inference`, `prediction`, `response generation`, `후처리`는 초반에 비슷하게 들릴 수 있습니다. 아래처럼 자리만 짧게 구분해 두고 읽으면 됩니다.
...
여기서는 `inference는 모델 실행`, `prediction은 예측 결과`, `response generation은 생성 결과`, `후처리는 모델 밖 처리`라는 자리 구분을 유지합니다.
...
다음처럼 모델이 답을 내는 순간을 `생각`, `판단`, `서비스 전체 처리`와 섞어 이해하고 있을 때 이 절의 관점을 다시 떠올리면 됩니다.

- 사용자의 질문에 답할 때마다 모델이 곧바로 새로 학습한다고 느껴질 때
- prediction, response generation, 후처리가 모두 inference 자체라고 생각될 때
- 모델 출력과 업무 시스템의 최종 결정이 어디서 갈리는지 설명해야 할 때

이때는 먼저 `inference는 학습된 모델을 새 입력에 적용하는 실행`이라는 기준으로 돌아가면 됩니다. 그 위에서 지금 보고 있는 것이 모델의 출력 단계인지, 그 출력을 정책과 절차에 연결하는 서비스 단계인지 나누면 구조가 정리됩니다.
