...
- 线性回归、决策树、神经网络等算法的具体计算
- 防止过拟合技巧的数学比较
- 大规模数据集的实验设计

这里的重点，是机器学习最基础的思维方式。规则式方法与表征学习的差异会在 P1-3.3 接着讲，具体算法会在 Part 4 和 Part 5 回来处理。
...
从数据中学习模式，并不是把过去案例原样存起来。按照这一节的 supervised learning 例子，更准确的理解是：从 example data 中找出输入与输出之间的关系，然后把这种关系再用到新数据上。

这个转向起点，正是规则式系统的限制。在那些人很难把所有例外与复杂模式都直接写出来的问题上，从数据里找关系会变得有用。但从数据学出来的模型也不是完美的。它的判断会随着数据质量、labels、features、evaluation 以及真实使用环境而变化。

所以，在理解学习式 AI 时，不要只看模型本身。还要一起看：数据是如何被整理出来的，用什么标准贴上 labels，又怎样在新数据上被验证。下一节会接着这条脉络，继续整理规则式方法与表征学习各自处在什么位置。
...
当 `从数据中学习模式` 这句话需要被重新解释成学习结构，而不是流行词时，就应该拿出这一节的视角。

- 当你需要区分：模型到底是在记规则，还是学到了也能用在新输入上的关系
- 当你需要整理：example、feature、label、model、training 与 generalization 分别放在哪里
- 当线上性能开始波动，需要先判断是数据多样性、label 质量还是 overfitting 更值得怀疑

这时，先回到 `example -> features and label -> training -> model -> prediction for new input` 这条流程。然后在此基础上再去分：当前问题更接近 `记忆`、`generalization`，还是 `数据问题`。这样解释会稳定得多。
