...
이 절은 먼저 `규칙 기반 시스템이 어디에서 강하고 어디에서 어려워지는가`를 닫습니다. 데이터에서 패턴을 배우는 흐름은 바로 다음 P1-3.2에서, 표현 학습과의 차이는 P1-3.3에서, 규칙과 모델이 함께 쓰이는 운영 구조는 Part 6과 Part 7에서 이어집니다.
...
위 용어는 초반에 서로 비슷하게 들릴 수 있습니다. 사전식으로 아주 짧게 다시 잡으면 다음과 같습니다.
...
여기에는 물리적 성능의 변화도 크게 작용했습니다. 규칙 기반 시스템은 비교적 적은 데이터와 계산 자원으로도 유용한 시스템을 만들 수 있었지만, 딥러닝은 많은 데이터, 반복 학습, 병렬 처리 능력을 필요로 합니다. 저장 장치가 싸지고 데이터가 많이 쌓였으며, CPU와 GPU, 분산 처리 환경, 오픈소스 학습 도구가 발전하면서 과거에는 연구 아이디어에 가까웠던 신경망 접근이 실제 성능을 내는 공학적 선택지로 강해졌습니다. 이 절에서는 이 변화가 `규칙 기반 시스템의 강점은 유지되지만, 복잡한 패턴 인식의 중심은 점점 학습 기반으로 이동했다`는 정도만 붙잡으면 충분합니다.

| 변화 | AI 접근에 준 영향 |
| --- | --- |
| 데이터 축적과 저장 비용 감소 | 규칙으로 직접 적기 어려운 패턴을 학습할 재료가 늘어났습니다. |
| 병렬 처리와 계산 자원 발전 | 큰 학습 모델을 반복 훈련하고 비교할 수 있는 조건이 좋아졌습니다. |
| 도구와 평가 생태계 확대 | 연구 결과를 재현하고, 비교하고, 개선하는 속도가 빨라졌습니다. |
...
- 규칙 기반 시스템은 사람이 지식을 명시적으로 쓰고 시스템이 그 규칙을 적용해 결론이나 행동을 내리는 방식이며, 설명 가능성·통제 가능성·재현성에서 강하다는 점을 설명할 수 있다.
- 전문가의 암묵지를 규칙으로 꺼내기 어렵고, 예외가 늘어날수록 관리가 복잡해지며, 이미지·음성·언어처럼 복잡한 패턴에는 한계가 있다는 점을 설명할 수 있다.
