...
규칙을 넘어서 데이터에서 패턴을 배우는 흐름은 바로 다음 P1-3.2에서 다시 회수하고, 표현 학습과의 차이는 P1-3.3에서 더 구체적으로 이어집니다. 규칙과 모델이 함께 쓰이는 운영 구조는 Part 6과 Part 7에서 다시 연결합니다.
...
이 절은 규칙 기반 시스템을 평가하는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 규칙 기반 시스템은 `현재 사실`과 `규칙`을 연결해 결론을 낸다는 점 | 시스템의 기본 구조를 먼저 잡게 해 줍니다. | 입력, 규칙, 결과의 세 자리만 먼저 보이면 충분합니다. |
| 강점은 `설명 가능성`과 `통제 가능성`이라는 점 | 왜 지금도 정책과 운영 절차에 남아 있는지 이해하게 해 줍니다. | 왜 이렇게 처리했는지 추적하기 쉽다는 점만 알면 충분합니다. |
| 한계는 `예외와 변화가 많아질수록 규칙 관리가 어려워진다`는 점 | 머신러닝으로 관심이 이동한 이유와 연결됩니다. | 좁은 문제에는 강하지만 복잡한 현실 전체를 다 적기 어렵다는 점만 알면 충분합니다. |

## 규칙 기반 시스템의 기본 구조

규칙 기반 시스템은 현재 사실(fact)이나 입력(input)을 규칙(rule)과 대조해 결론(conclusion), 분류(classification), 행동(action), 처리 절차를 정하는 시스템입니다.
...
처음 읽을 때는 위 용어가 비슷하게 들릴 수 있습니다. 사전식으로 아주 짧게 다시 잡으면 다음과 같습니다.

| 용어 | 아주 짧은 뜻 | 이 절의 예시에서 무엇에 해당하는가 |
| --- | --- | --- |
| 사실 | 지금 입력된 현재 상태 | 요청 금액, 예산 잔액, 요청자와 승인자의 관계 |
| 규칙 | 조건이 맞으면 따를 판단 기준 | `100만 원 초과면 부서장 승인` 같은 문장 |
| 지식 기반 | 사실과 규칙을 모아 둔 자리 | 승인 기준표, 예외 규정, 역할 정보 |
| 추론 엔진 | 맞는 규칙을 골라 적용하는 부분 | 어떤 승인 단계로 보내야 할지 계산하는 절차 |
| 설명 기능 | 왜 그런 결과가 나왔는지 보여 주는 부분 | `예산 충분 + 100만 원 초과 -> 부서장 승인` 설명 |

처음 단계에서는 `사실은 현재 상태`, `규칙은 판단 기준`, `지식 기반은 그것을 모아 둔 곳`, `추론 엔진은 적용 장치`, `설명 기능은 이유를 보여 주는 장치`라고만 기억해도 충분합니다.
...
규칙 기반 시스템의 장점과 한계는 전문가 시스템에만 나타난 것이 아닙니다. 얼굴인식, 차선 추적, 음성 합성처럼 사람이 특징과 절차를 많이 설계하던 영역에서도 비슷한 흐름을 볼 수 있습니다.

예를 들어 얼굴인식 초기 방법은 얼굴 구성 요소의 위치와 거리, 수작업 특징, 템플릿을 많이 사용했고, 차선 추적 프로토타입은 ROI(region of interest), Canny edge, Hough transform 같은 영상 처리 절차를 많이 사용했습니다. 이런 방식은 구조가 단순하고 환경이 통제될 때는 설명하기 쉽고 빠르게 유용해질 수 있습니다.

반면 입력 조건이 다양해지면 한계가 빨리 드러납니다. 조명, 표정, 자세, 배경, 날씨, 노이즈, 예외 발음처럼 변수가 많아질수록 사람이 직접 특징과 절차를 계속 늘려야 하기 때문입니다. 반대로 TTS처럼 표기와 발음 규칙이 비교적 구조화된 영역에서는 규칙 기반 절차가 오랫동안 유용하게 쓰이기도 했습니다.
...
예를 들어 AlexNet 논문은 ImageNet의 120만 개 학습 이미지, 6천만 개 파라미터를 가진 깊은 합성곱 신경망, 효율적인 GPU 구현, 두 개의 GTX 580 GPU를 사용한 학습을 함께 설명합니다. 이 사례는 딥러닝의 부상이 단지 아이디어의 변화만이 아니라, 데이터 규모와 계산 성능이 결합된 공학적 변화와도 연결되어 있음을 보여 줍니다. 다만 GPU와 딥러닝 구조의 세부 내용은 P1-9와 Part 5에서 다시 다룹니다.
...
아래 사례를 보고 `규칙 기반 시스템이 먼저 잘 맞는지`, `규칙만으로는 빠르게 한계가 오는지`, `규칙과 모델을 함께 써야 하는지`를 먼저 가려 보겠습니다.

| 사례 | 먼저 확인할 질문 | 이 절 기준의 1차 판단 |
| --- | --- | --- |
| 사내 결재에서 직급별 승인 한도를 검사한다 | 기준이 명확한 문장으로 이미 정리되어 있는가 | 규칙 기반 시스템이 먼저 잘 맞습니다. |
| 고객 문의 문장을 읽고 숨은 불만 의도를 자동 분류한다 | 같은 뜻이 매우 다양한 표현으로 나타나는가 | 규칙만으로는 한계가 빨리 옵니다. |
| LLM 서비스에서 권한 없는 문서를 검색하지 못하게 막는다 | 허용/차단 기준을 명시적으로 고정해야 하는가 | 규칙 기반이 꼭 필요합니다. |
| 공장 센서 값과 작업 이력으로 고장 가능성을 예측한다 | 사람이 모든 패턴을 규칙으로 적을 수 있는가 | 모델이 더 필요할 가능성이 큽니다. |
| 문서 도우미가 답변을 만들기 전에 주민번호를 가린다 | 무조건 지켜야 하는 안전 조건인가 | 규칙과 모델을 함께 쓰기 쉽습니다. |

이 연습의 핵심은 규칙 기반 시스템을 `옛 방식`으로 보는 것이 아니라, `명시적으로 통제해야 하는 절차`에 특히 강한 도구로 읽는 데 있습니다. 반대로 의미 해석이 모호하고 패턴이 복잡한 문제일수록 모델의 역할이 커집니다.
...
여기서는 서비스 아키텍처를 자세히 설명하지 않습니다. 핵심은 “학습된 모델이 있어도 명시적 규칙이 필요한 영역은 남는다”는 점입니다. RAG, Tool use, Agent 같은 구체적인 구조는 P1-14와 Part 6에서 다시 다룹니다.
...
| 배포 또는 실행 | 승인 상태, 실행 조건, 환경 조건 확인 | 변경 요약 초안 작성 |
