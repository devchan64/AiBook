...
이 절은 `규칙은 밖에 있고 표현은 안에 있다`는 감각을 먼저 잡는 데 집중합니다. 입력, 출력, 데이터, 특징, 파라미터를 더 체계적으로 정리하는 일은 다음 Chapter 4에서 다시 회수합니다.
...
이 차이는 이후 장의 핵심으로 이어집니다. P1-4에서는 현실 문제를 입력, 출력, 데이터, 특징으로 바꾸는 과정을 다루고, Part 3와 Part 4에서는 모델이 어떤 기준으로 파라미터와 표현을 조정하는지 더 자세히 봅니다.
