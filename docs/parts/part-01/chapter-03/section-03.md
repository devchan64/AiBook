...
Part 1 안에서는 이 절을 `표현(representation)`, `벡터(vector)`, `활성값(activation)`, `표현 학습(representation learning)`의 대표 상세 설명 위치로 사용합니다. `예시`, `라벨`, `학습`, `일반화`의 기본 구조는 3.2에서 먼저 잡았고, 여기서는 입력이 모델 안에서 어떤 내부 형태로 바뀌는지와 규칙 기반 접근과의 차이에 필요한 만큼만 다시 연결합니다.
...
이 절은 `규칙은 밖에 있고 표현은 안에 있다`는 감각을 먼저 잡는 데 집중합니다. 입력, 출력, 데이터, 특징, 파라미터를 더 체계적으로 정리하는 일은 바로 다음 Part 1 Chapter 4에서 다시 회수합니다.
...
이 절은 입력을 모델이 어떤 형태로 다루는지 보는 절입니다. 아래 세 가지 관점을 먼저 잡으면 구조를 따라가기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| 규칙은 사람이 `밖에서 적는 기준`이고, 표현은 모델 `안에서 쓰는 내부 형태`라는 점 | 규칙 기반 접근과 학습 기반 접근의 자리가 달라진다는 점이 보입니다. | 규칙은 읽을 수 있고 표현은 계산용 내부 값이라는 점을 먼저 구분합니다. |
| 특징(feature)은 사람이 만들 수도 있고, 모델이 더 깊은 표현으로 바꿀 수도 있다는 점 | 전통적 머신러닝과 딥러닝의 차이로 이어집니다. | 키워드 같은 사람이 만든 단서와 모델이 학습한 내부 표현이 다르다는 점을 먼저 잡습니다. |
| 학습된 표현은 강하지만 `왜 그렇게 봤는지` 읽기 어려울 수 있다는 점 | 모델 해석과 평가가 왜 필요한지 연결됩니다. | 유사성을 넓게 다루는 대신 설명은 어려워질 수 있다는 점을 먼저 연결합니다. |

`규칙`, `특징`, `표현`, `벡터`, `활성값`, `파라미터`는 초반에 모두 비슷한 내부 값처럼 들릴 수 있습니다. 이 절에서는 아래 정도로 짧게 구분해 두고 계속 읽으면 됩니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 규칙 | 사람이 바깥에 적는 판단 기준 | 명시적으로 읽고 수정할 수 있는 기준 |
| 특징 | 사람이 만들거나 먼저 뽑은 입력 단서 | 모델이 보기 쉽게 정리한 재료 |
| 표현 | 모델이 내부에서 다루는 형태 | 입력을 계산 가능한 구조로 바꾼 결과 |
| 벡터 | 숫자 묶음으로 된 표현 | 내부 표현을 담는 흔한 형식 |
| 활성값 | 계산 도중 나온 중간 값 | 층을 지나며 바뀌는 표현의 한 순간 |
| 파라미터 | 학습으로 조정되는 내부 값 | 어떤 표현을 만들지에 영향을 주는 기준 |

이 단계에서는 `규칙은 밖에 있는 기준`, `특징은 사람이 만든 단서`, `표현은 안쪽 형태`, `벡터는 숫자 묶음`, `활성값은 중간 값`, `파라미터는 학습되는 기준`이라는 구분이 먼저 남아 있으면 됩니다.
...
`표현`, `벡터`, `활성값`, `파라미터`는 한꺼번에 나오면 모두 비슷한 내부 값처럼 들릴 수 있습니다. 이 절에서는 다음 정도로 구분해 두고 계속 읽으면 됩니다.
...
이 단계에서는 `표현은 내부 형태`, `벡터는 그 표현을 담는 숫자 묶음`, `활성값은 계산 중간 값`, `파라미터는 학습으로 조정되는 값`이라는 구분이 먼저 남아 있으면 됩니다.
...
이 그림은 입력이 한 번에 이해되는 것이 아니라, 여러 단계를 거치며 다른 수준의 표현으로 바뀔 수 있다는 점을 보여 줍니다. 여기서 `낮은 수준 특징`은 비교적 작은 단서, `중간 수준 표현`은 여러 단서의 조합, `높은 수준 표현`은 더 추상적인 의미 묶음에 가깝게 읽으면 됩니다. 실제 모델 내부가 항상 이렇게 깔끔한 단계 이름으로 저장되는 것은 아니지만, 여기서는 `입력이 지나가며 점점 다른 표현으로 바뀐다`는 감각이 먼저 보이면 됩니다.
