...
- 딥러닝 내부 층과 파라미터 학습의 세부 수식
- 표현 벡터의 차원과 최적화 절차
- 모델 해석 기법의 본격 구현

여기서는 `규칙은 밖에 있고 표현은 안에 있다`는 구분을 정리하는 데 집중합니다. 입력, 출력, 데이터, 특징, 파라미터를 더 체계적으로 정리하는 일은 바로 다음 Part 1 Chapter 4에서 다시 회수합니다.
...
이 차이는 이후 장의 핵심으로 이어집니다. Part 1 Chapter 4에서는 현실 문제를 입력, 출력, 데이터, 특징으로 바꾸는 과정을 다루고, Part 4와 Part 5에서는 모델이 어떤 기준으로 파라미터와 표현을 조정하는지 더 자세히 봅니다.
...
다음처럼 규칙과 모델을 둘 중 하나로만 고르기보다, 입력을 어떤 형태로 다루는지가 더 중요해지는 장면에서 이 절의 관점을 다시 떠올리면 됩니다.

- 같은 뜻의 문장이 계속 다르게 나타나 단어 규칙만으로는 분류가 흔들릴 때
- 특징, 표현, 벡터, 활성값, 파라미터가 모두 비슷한 내부 용어처럼 들릴 때
- 정책은 규칙으로 고정하고 의미 유사성은 모델이 맡게 해야 하는 이유를 설명해야 할 때

이때는 `규칙은 밖에 있고 표현은 안에 있다`는 기준으로 다시 나누면 됩니다. 사람이 직접 통제할 기준인지, 데이터에서 유사성을 학습해야 할 부분인지를 먼저 나누면 시스템 설계와 설명이 모두 쉬워집니다.
