...
이 절은 먼저 `데이터에서 패턴을 배운다는 말이 어떤 구조를 뜻하는가`를 닫습니다. 규칙 기반 접근과 표현 학습의 차이는 P1-3.3에서, 구체 알고리즘은 Part 4와 Part 5에서 이어집니다.
...
## 규칙이 늘어나는 문제를 패턴 학습으로 바꾸기

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "데이터에서 패턴을 배운다는 전환이 무엇을 바꾸는지 직관적으로 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- 데이터에서 패턴을 배운다는 것이 과거 사례를 그대로 외우는 것이 아니라, 입력과 출력의 관계를 찾아 새 데이터에도 적용하려는 시도라는 점을 설명할 수 있다.
- 학습 기반 AI를 이해할 때는 모델만이 아니라 데이터가 어떻게 만들어졌고 어떤 기준으로 학습되었으며 새 데이터에서 어떻게 검증되는지를 함께 봐야 한다는 점을 설명할 수 있다.
