...
이 절은 머신러닝의 가장 기본적인 사고방식을 잡는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 학습은 `규칙을 직접 쓰는 것`이 아니라 `예시에서 관계를 찾는 것`이라는 점 | 규칙 기반 시스템과의 차이가 가장 먼저 보입니다. | 입력 예시와 정답 예시를 보고 모델이 기준을 조정한다는 점만 알면 충분합니다. |
| 예시, 특징, 라벨, 모델은 `역할이 다른 자리`라는 점 | Part 4의 머신러닝 절을 읽기 위한 최소 용어가 됩니다. | 특징은 입력 단서, 라벨은 정답, 모델은 관계를 쓰는 계산 구조라는 점만 알면 충분합니다. |
| 학습은 `암기`가 아니라 `새 데이터에도 쓸 수 있는 패턴`을 찾는 일이라는 점 | 일반화와 과적합 설명으로 자연스럽게 이어집니다. | 본 적 없는 새 입력에도 어느 정도 맞아야 좋은 학습이라는 점만 알면 충분합니다. |
...
처음 읽을 때는 위 용어가 다 비슷한 기술 용어처럼 보일 수 있습니다. 사전식으로 아주 짧게 다시 잡으면 다음과 같습니다.
...
처음 단계에서는 `예시는 사례`, `특징은 단서`, `라벨은 정답`, `모델은 관계를 쓰는 구조`, `파라미터는 학습으로 바뀌는 내부 값` 정도로만 잡아 두면 충분합니다.
