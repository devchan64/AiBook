...
이 절의 목적은 머신러닝 알고리즘을 자세히 설명하는 것이 아닙니다. 선형회귀와 결정트리 같은 알고리즘은 Part 4에서, 신경망과 딥러닝 구조는 Part 5에서 다시 다룹니다. 여기서는 “데이터에서 패턴을 배운다”는 말이 어떤 구조를 뜻하는지, 그리고 왜 이것이 규칙 기반 시스템과 다른지 이해하는 데 집중합니다.

Part 1 안에서는 이 절을 `데이터에서 패턴을 배운다`는 기본 구조, `예시(example)`, `특징(feature)`, `라벨(label)`, `모델(model)`, `학습(training)`, `일반화(generalization)`의 대표 상세 설명 위치로 사용합니다. `표현(representation)`의 내부 구조와 규칙 기반 접근과의 대비는 3.3에서 더 자세히 다루고, 여기서는 패턴 학습의 전체 흐름을 잡는 데 필요한 만큼만 연결합니다.
...
이 절은 머신러닝의 기본 사고방식을 잡는 데 집중합니다. 규칙 기반 접근과 표현 학습의 차이는 바로 다음 P1-3.3에서 다시 회수하고, 구체 알고리즘은 Part 4와 Part 5에서 다시 다룹니다.
...
이 절은 머신러닝의 가장 기본적인 사고방식을 잡는 절입니다. 아래 세 가지 관점을 먼저 잡으면 구조를 따라가기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| 학습은 `규칙을 직접 쓰는 것`이 아니라 `예시에서 관계를 찾는 것`이라는 점 | 규칙 기반 시스템과의 차이가 가장 먼저 보입니다. | 입력 예시와 정답 예시를 보고 모델이 기준을 조정한다는 점을 먼저 구분합니다. |
| 예시, 특징, 라벨, 모델은 `역할이 다른 자리`라는 점 | Part 4의 머신러닝 절을 읽기 위한 최소 용어가 됩니다. | 특징은 입력 단서, 라벨은 정답, 모델은 관계를 쓰는 계산 구조라는 점을 먼저 잡습니다. |
| 학습은 `암기`가 아니라 `새 데이터에도 쓸 수 있는 패턴`을 찾는 일이라는 점 | 일반화와 과적합 설명으로 자연스럽게 이어집니다. | 본 적 없는 새 입력에도 어느 정도 맞아야 좋은 학습이라는 점을 먼저 연결합니다. |

`예시`, `특징`, `라벨`, `모델`, `학습`, `일반화`는 초반에 서로 비슷한 기술 용어처럼 들릴 수 있습니다. 이 절에서는 아래 정도로 짧게 구분해 두고 계속 읽으면 됩니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 예시 | 학습에 쓰는 한 건의 사례 | 입력과 정답이 어떻게 짝을 이루는지 보여 주는 단위 |
| 특징 | 모델이 읽는 입력 단서 | 예시에서 무엇을 근거로 볼지 정하는 값 |
| 라벨 | 사람이 붙인 정답 | 모델이 맞혀야 할 목표 |
| 모델 | 특징과 라벨의 관계를 쓰는 계산 구조 | 새 입력에 대한 예측을 만드는 장치 |
| 학습 | 모델 내부 값을 조정하는 과정 | 과거 예시를 보고 기준을 맞추는 단계 |
| 일반화 | 새 데이터에도 맞는 상태 | 학습이 암기가 아니라는 점을 보여 주는 기준 |

이 단계에서는 `예시는 사례`, `특징은 단서`, `라벨은 정답`, `모델은 관계를 계산하는 구조`, `학습은 기준 조정`, `일반화는 새 입력에도 맞는 상태`라는 구분이 먼저 남아 있으면 됩니다.
...
이 도식은 `사람이 기준을 직접 쓰는 흐름`과 `예시 데이터로 기준을 맞추는 흐름`을 나란히 비교하게 해 줍니다. 여기서는 `규칙 기반은 기준이 바깥에 적혀 있고`, `학습 기반은 기준이 데이터로 조정된다`는 대비가 먼저 보이면 됩니다.
...
위 용어는 초반에 모두 비슷한 기술 용어처럼 보일 수 있습니다. 사전식으로 아주 짧게 다시 잡으면 다음과 같습니다.
...
이 단계에서는 `예시는 사례`, `특징은 단서`, `라벨은 정답`, `모델은 관계를 쓰는 구조`, `파라미터는 학습으로 바뀌는 내부 값`이라는 구분이 먼저 남아 있으면 됩니다.
