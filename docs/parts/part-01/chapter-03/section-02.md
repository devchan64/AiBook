...
이 절은 머신러닝의 기본 사고방식을 잡는 데 집중합니다. 규칙 기반 접근과 표현 학습의 차이는 바로 다음 3.3에서 다시 회수하고, 구체 알고리즘은 Part 3의 P3-10부터 P3-18, Part 4의 딥러닝 장에서 다시 다룹니다.
...
| 예시, 특징, 라벨, 모델은 `역할이 다른 자리`라는 점 | 뒤의 머신러닝 파트를 읽기 위한 최소 용어가 됩니다. | 특징은 입력 단서, 라벨은 정답, 모델은 관계를 쓰는 계산 구조라는 점만 알면 충분합니다. |
