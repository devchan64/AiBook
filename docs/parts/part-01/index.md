...
같은 Part 안에서 중요한 개념의 상세 설명은 가능한 한 한 Section에만 두고, 이후 Section에서는 현재 질문에 필요한 만큼만 다시 연결합니다. 읽다가 용어가 다시 헷갈리면 [개념사전](../../reference/concept-glossary.md)의 `중심 Section`으로 돌아가 대표 설명 위치를 먼저 확인하고, `등장 Section`으로 현재 개념이 어디서 다시 쓰이는지 이어서 추적하면 됩니다.
...
이 Part는 용어가 많고 범위가 넓어서 초반에 막히기 쉽습니다. 모든 용어를 한 번에 외우려 하기보다, 먼저 다음 세 질문을 기준으로 읽는 편이 흐름을 잡기 쉽습니다.

| 먼저 붙잡을 질문 | 이 질문이 필요한 이유 | 이 Part에서 먼저 잡을 기준 |
| --- | --- | --- |
...
이 Part에서는 모든 개념을 매 장마다 같은 밀도로 반복하지 않습니다. 예를 들어 `모델`, `입력`, `출력`, `특징`, `표현`, `파라미터` 같은 핵심 용어는 앞 절에서 자리를 먼저 잡고, 뒤 절에서는 현재 문제 장면에 필요한 연결만 남깁니다. 따라서 같은 용어가 다시 나와도 `정의가 새로 추가되는지`, 아니면 `다른 맥락에 놓이는지`를 구분해 읽는 편이 좋습니다.

개념이 많아 보여도, 아래 다섯 줄이 먼저 남아 있으면 Part 1의 역할은 살아 있습니다.
...
Part 1 안에서 모든 장을 완결된 지식처럼 닫으려 하기보다, 아래처럼 `지금은 어디까지 잡고, 뒤에서 어디서 다시 배우는가`를 연결해 두는 편이 더 중요합니다.
