...
이 절은 먼저 `기호 기반 AI가 지식과 규칙을 어떻게 적고 다루려 했는가`를 닫습니다. 탐색, 지식 표현, 확률 추론의 세부 흐름은 바로 다음 `P1-2.2`에서 이어지고, 규칙 기반 시스템의 장단점 평가는 Part 1 Chapter 3에서 더 구체적으로 이어집니다.
...
전문가 시스템(expert system)은 이런 규칙 기반 접근이 실제 응용으로 발전한 대표 사례입니다. 전문가의 판단을 규칙과 지식으로 정리해, 특정 영역의 진단, 분류, 추천, 의사결정을 보조하려 했습니다. 이 절에서는 먼저 기호 기반 AI와 규칙 기반 접근의 기본 사고방식을 붙잡고, 실제 응용으로 이어질 때의 장점과 한계는 `P1-3.1`에서 다시 정리합니다.
...
## 명시적 규칙이 잘 맞는 문제와 흔들리는 문제

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "기호 기반 AI와 규칙 기반 접근이 무엇을 잘했고 어디서 막혔는지 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- 기호 기반 AI가 지식을 사람이 읽을 수 있는 기호와 규칙으로 표현하고, 그 표현 위에서 추론과 탐색을 수행하려 한 접근이라는 점을 설명할 수 있다.
- 이 접근이 설명 가능성과 통제 가능성이 강하지만, 현실 세계의 모호함과 예외, 대규모 패턴 인식에는 한계를 보였다는 점을 설명할 수 있다.
