...
Part 1 안에서는 이 절을 `기호 기반 AI`, `규칙 기반 접근`, `지식 표현`의 대표 상세 설명 위치로 사용합니다. 뒤 절에서 이 용어들이 다시 나오더라도, 그 절의 중심 질문에 필요한 만큼만 연결하고 상세 정의는 여기와 개념사전으로 다시 돌아오면 됩니다.
...
이 절은 역사 설명처럼 보이지만, 사실은 AI가 문제를 푸는 오래된 기본 방식을 소개하는 절입니다. 아래 세 가지 관점을 전체 지도로 삼으면 흐름을 잡기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| 기호 기반 AI는 `지식을 명시적으로 적는 방식`이라는 점 | 그래야 뒤의 머신러닝과 대비가 됩니다. | 사람이 규칙과 사실을 먼저 적어 둔다는 점을 먼저 구분합니다. |
| 규칙 기반 접근은 `조건에 따라 결론이나 행동을 정하는 방식`이라는 점 | 실제 생활 규칙과 연결해 이해하기 쉽습니다. | 입력 상황과 규칙을 맞춰 결과를 내는 구조를 먼저 봅니다. |
| 이 방식은 `사라진 과거`가 아니라 지금도 일부 시스템에 남아 있다는 점 | 현대 AI와 완전히 끊어진 옛 기술로 오해하지 않게 해 줍니다. | 정책, 권한, 안전 규칙처럼 지금도 쓰인다는 점을 먼저 연결합니다. |

`기호`, `규칙`, `사실`, `지식 표현`, `추론`은 초반에 서로 비슷하게 들릴 수 있습니다. 이 절에서는 아래 정도로 짧게 구분해 두고 계속 읽으면 됩니다.
...
이 단계에서는 `무엇을 이름 붙여 적을지`, `어떤 기준을 규칙으로 둘지`, `현재 무엇이 사실인지`, `그 둘로 어떤 결론을 낼지`라는 구분이 먼저 남아 있으면 됩니다.
...
이 도식은 규칙 기반 시스템을 `현재 사실`, `명시적 규칙`, `적용 절차`, `결론`의 네 부분으로 읽게 해 주는 가장 기본적인 그림입니다. 여기서는 `사실만으로도 안 되고 규칙만으로도 안 되며, 둘을 맞춰 결론을 내는 절차가 따로 있다`는 구조가 먼저 보이면 됩니다.

전문가 시스템(expert system)은 이런 규칙 기반 접근이 실제 응용으로 발전한 대표 사례입니다. 전문가의 판단을 규칙과 지식으로 정리해, 특정 영역의 진단, 분류, 추천, 의사결정을 보조하려 했습니다. 다만 이 절에서는 전문가 시스템을 깊게 다루지 않습니다. 여기서는 기호 기반 AI와 규칙 기반 접근의 기본 사고방식만 잡고, 장점과 한계는 `P1-3.1`에서 다시 정리합니다.
...
라벨, 데이터, 입력, 출력의 자세한 관계는 `P1-4.2`에서 다시 정리합니다. 여기서는 라벨을 `사람이 붙인 명시적 이름표`로 읽는 감각만 먼저 연결해 두면 됩니다.
