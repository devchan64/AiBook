...
탐색, 지식 표현, 확률 추론의 세부 흐름은 바로 다음 2.2에서 다시 회수하고, 규칙 기반 시스템의 장단점 평가는 Chapter 3에서 더 구체적으로 이어집니다.
...
전문가 시스템(expert system)은 이런 규칙 기반 접근이 실제 응용으로 발전한 대표 사례입니다. 전문가의 판단을 규칙과 지식으로 정리해, 특정 영역의 진단, 분류, 추천, 의사결정을 보조하려 했습니다. 다만 이 절에서는 전문가 시스템을 깊게 다루지 않습니다. 여기서는 기호 기반 AI와 규칙 기반 접근의 기본 사고방식만 잡고, 장점과 한계는 P1-3.1에서 다시 정리합니다.
