...
이 절은 역사 설명처럼 보이지만, 사실은 AI가 문제를 푸는 오래된 기본 방식을 소개하는 절입니다. 먼저는 아래 세 가지 관점을 전체 지도처럼 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 기호 기반 AI는 `지식을 명시적으로 적는 방식`이라는 점 | 그래야 뒤의 머신러닝과 대비가 됩니다. | 사람이 규칙과 사실을 먼저 적어 둔다는 점만 잡으면 충분합니다. |
| 규칙 기반 접근은 `조건에 따라 결론이나 행동을 정하는 방식`이라는 점 | 실제 생활 규칙과 연결해 이해하기 쉽습니다. | 입력 상황과 규칙을 맞춰 결과를 내는 구조만 보이면 충분합니다. |
| 이 방식은 `사라진 과거`가 아니라 지금도 일부 시스템에 남아 있다는 점 | 현대 AI와 완전히 끊어진 옛 기술로 오해하지 않게 해 줍니다. | 정책, 권한, 안전 규칙처럼 지금도 쓰인다는 점만 알면 충분합니다. |
