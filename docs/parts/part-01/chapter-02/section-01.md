...
- 기호 기반 AI는 무엇을 하려 했는가?
- 규칙, 지식 표현, 추론, 탐색은 어떤 큰 흐름 안에서 연결되는가?
- 왜 이 접근은 지금도 일부 시스템에서 유효한가?
...
탐색, 지식 표현, 확률 추론의 세부 흐름은 바로 다음 `P1-2.2`에서 다시 회수하고, 규칙 기반 시스템의 장단점 평가는 Part 1 Chapter 3에서 더 구체적으로 이어집니다.
...
| --- | --- | --- |
| 기호 기반 AI는 `지식을 명시적으로 적는 방식`이라는 점 | 그래야 뒤의 머신러닝과 대비가 됩니다. | 사람이 규칙과 사실을 먼저 적어 둔다는 점을 구분합니다. |
| 규칙 기반 접근은 `조건에 따라 결론이나 행동을 정하는 방식`이라는 점 | 실제 생활 규칙과 연결해 이해하기 쉽습니다. | 입력 상황과 규칙을 맞춰 결과를 내는 구조를 봅니다. |
| 이 방식은 `사라진 과거`가 아니라 지금도 일부 시스템에 남아 있다는 점 | 현대 AI와 완전히 끊어진 옛 기술로 오해하지 않게 해 줍니다. | 정책, 권한, 안전 규칙처럼 지금도 쓰인다는 점을 연결합니다. |

`기호`, `규칙`, `사실`, `지식 표현`, `추론`은 초반에 서로 비슷하게 들릴 수 있습니다. 이 절에서는 아래처럼 역할을 나누어 둡니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 기호 | 이름 붙여 구분하는 표식 | 사람과 시스템이 같은 대상을 가리키게 하는 단위 |
| 규칙 | 조건에 따라 결론이나 행동을 정하는 기준 | 명시적 판단 기준 |
| 사실 | 현재 참이라고 두는 정보 | 규칙 적용의 입력 재료 |
| 지식 표현 | 사실, 관계, 규칙을 적는 형식 | 시스템이 무엇을 알고 있다고 볼지 정하는 틀 |
| 추론 | 주어진 사실과 규칙에서 결론을 얻는 과정 | 결과를 만들어 내는 절차 |

이 절에서 먼저 남겨야 할 구분은 `무엇을 이름 붙여 적을지`, `어떤 기준을 규칙으로 둘지`, `현재 무엇이 사실인지`, `그 둘로 어떤 결론을 낼지`입니다.
...
여기서는 symbolic AI를 기본적으로 “기호 기반 AI”라고 부릅니다. “상징주의 AI”, “기호주의 AI”, “상징적 AI”, “심볼릭 인공지능” 같은 표현도 볼 수 있지만, 가장 뜻이 직접적인 표현으로 통일합니다.
...
여기서 초심자가 자주 헷갈리는 지점은 `규칙`, `추론`, `탐색`이 모두 비슷한 처리 과정처럼 보인다는 점입니다. 하지만 셋은 맡는 역할이 다릅니다.

| 요소 | 먼저 갖고 있는 것 | 실제로 하는 일 | 짧은 예 |
| --- | --- | --- | --- |
| 규칙 | 조건과 결론의 연결 기준 | 어떤 상황에서 어떤 판단을 써야 하는지 적어 둠 | `결제 금액이 한도를 넘으면 추가 승인` |
| 추론 | 현재 사실과 규칙 | 지금 사실에 맞는 규칙을 적용해 결론을 만듦 | `한도를 넘었으므로 추가 승인 필요` |
| 탐색 | 가능한 상태나 다음 선택지 | 후보들을 따라가며 목표 상태나 해답을 찾음 | 체스에서 다음 수 후보를 따라가 보기 |

즉 규칙은 `판단 기준`, 추론은 `현재 기준 적용`, 탐색은 `가능한 길 찾기`에 더 가깝습니다. 실제 시스템에서는 셋이 함께 붙는 경우도 많습니다. 예를 들어 게임 AI는 말을 어떻게 움직일 수 있는지 규칙으로 표현하고, 현재 판에서 가능한 수를 탐색하며, 그 결과 어떤 수가 더 낫다고 판단하는 추론을 함께 사용할 수 있습니다.
...
전문가 시스템(expert system)은 이런 규칙 기반 접근이 실제 응용으로 발전한 대표 사례입니다. 전문가의 판단을 규칙과 지식으로 정리해, 특정 영역의 진단, 분류, 추천, 의사결정을 보조하려 했습니다. 다만 이 절에서는 전문가 시스템을 깊게 다루지 않습니다. 이 절에서는 기호 기반 AI와 규칙 기반 접근의 기본 사고방식을 정리하고, 장점과 한계는 `P1-3.1`에서 다시 정리합니다.

아주 작은 예시로 보면 구조가 더 분명해집니다. 온라인 상점의 반품 접수 화면에서 아래 사실이 들어왔다고 가정해 보겠습니다.
...
- 판매자 귀책이면 배송비를 환불한다.
- 단순 변심이면 배송비는 고객이 부담한다.
- 묶음 주문이면 주문 전체가 아니라 품목 단위로 다시 계산한다.

이때 시스템은 `배송비 환불`이라는 결론과 함께 `품목 단위 재계산 필요`라는 처리 절차를 함께 낼 수 있습니다. 이 예시의 핵심은 규칙 기반 접근이 복잡한 학습 없이도 `현재 사실`과 `명시적 규칙`만으로 판단을 만들 수 있다는 점입니다.

동시에 이런 구조는 규칙 충돌이 왜 생기는지도 보여 줍니다. 예를 들어 어떤 팀이 `판매자 귀책이면 배송비 환불` 규칙을 넣고, 다른 팀이 `묶음 주문이면 배송비는 일단 고객 선부담 후 정산` 같은 운영 규칙을 따로 넣었다고 해 보겠습니다. 현재 사실이 `판매자 귀책`이면서 `묶음 주문`이면, 시스템은 두 규칙을 모두 만족해 서로 다른 처리 방향을 동시에 내놓을 수 있습니다. 이때는 `어느 규칙이 우선인가`, `예외 규칙을 따로 둘 것인가`, `사람 검토 단계로 넘길 것인가` 같은 추가 설계가 필요합니다. 초심자 기준에서 중요한 점은 규칙 기반 시스템이 단순히 `규칙만 많이 쓰면 되는 구조`가 아니라, 규칙 사이의 우선순위와 충돌 해결 방식까지 함께 설계해야 하는 구조라는 점입니다.
...
아래 사례를 보고 `규칙 기반 접근으로 먼저 시작하기 좋은지`, `학습 기반 접근이 더 필요한지`, `둘을 함께 써야 할지`를 먼저 판단해 보면 이 절의 경계가 더 분명해집니다.

| 사례 | 먼저 떠올릴 질문 | 이 절 기준의 1차 판단 |
| --- | --- | --- |
| 사내 비용 정산에서 직급별 승인 한도를 넘으면 추가 결재로 보내야 하는가 | 기준이 문서로 명확히 적혀 있는가 | 규칙 기반 접근으로 시작하기 좋습니다. |
| 고객 문의 문장을 읽고 불만 유형을 자동 분류해야 하는가 | 표현이 매우 다양하고 예외 문장이 많은가 | 학습 기반 접근이 더 필요할 수 있습니다. |
| 생성형 AI 답변에 금지 주제 요청이 들어오면 차단해야 하는가 | 정책 규칙이 명확한가, 문맥 판단이 필요한가 | 규칙과 학습 기반 판단을 함께 쓰기 쉽습니다. |
| 공장 설비 센서 값으로 고장을 미리 예측해야 하는가 | 사람이 모든 패턴을 규칙으로 적을 수 있는가 | 학습 기반 접근이 더 적합한 경우가 많습니다. |

이 연습의 핵심은 `규칙 기반`을 낡은 방식으로 보는 것이 아니라, `명시적 기준이 강한 문제`에 특히 잘 맞는 접근으로 읽는 데 있습니다. 반대로 입력이 모호하고 패턴이 복잡할수록 규칙만으로는 한계가 커지고, 데이터에서 패턴을 배우는 방식이 더 중요해질 수 있습니다.

## 사례 및 예시

### 사례 1. 배송비 환불 규정을 사람이 직접 확인하던 고객센터

온라인 쇼핑몰 고객센터에서 상담원이 배송비 환불 여부를 직접 판단한다고 가정해 보겠습니다. 사람은 `파손 여부`, `오배송 여부`, `단순 변심 여부`, `무료 배송 조건 충족 여부` 같은 기준을 하나씩 확인합니다.

처음에는 이 기준이 명확해 보여도, 실제 문의에는 `부분 환불`, `쿠폰 사용`, `여러 상품 묶음 주문`, `판매자 귀책과 고객 귀책이 함께 있는 경우` 같은 예외가 붙습니다. 상담원이 경험으로 처리하면 빠를 수는 있지만, 담당자마다 판단이 달라질 수 있고 나중에 왜 그렇게 처리했는지 추적하기도 어렵습니다.

규칙 기반 접근은 이런 기준을 명시적으로 적어 두고, 현재 주문 정보와 규칙을 대조해 결론을 내리게 합니다. 예를 들어 `판매자 귀책이면 배송비 환불`, `단순 변심이면 고객 부담`, `묶음 주문이면 주문 전체가 아니라 품목별로 다시 계산` 같은 규칙을 지식 기반에 넣을 수 있습니다.

이 사례에서 중요한 점은 규칙 기반 시스템이 사람의 판단 기준을 없애는 것이 아니라, 반복되는 판단 기준을 읽을 수 있는 형태로 옮긴다는 데 있습니다. 동시에 예외가 늘어날수록 규칙 관리가 복잡해진다는 한계도 같이 드러납니다.
...
라벨, 데이터, 입력, 출력의 자세한 관계는 `P1-4.2`에서 다시 정리합니다. 여기서는 라벨을 `사람이 붙인 명시적 이름표`로 읽는 연결만 남겨 둡니다.
...
여기서는 기호 기반 AI와 규칙 기반 접근을 다음처럼 읽습니다.
...
다음처럼 어떤 문제를 보고 `이건 학습 모델이 아니라서 옛 방식 아닌가` 또는 `규칙으로 처리하는 편이 더 자연스럽지 않은가`가 헷갈릴 때 이 절의 관점을 다시 꺼내면 됩니다.

- 정책, 권한, 승인, 안전 규칙처럼 기준이 문서로 명확한 업무를 AI 맥락 안에서 어디에 놓아야 할지 정리해야 할 때
- 규칙 기반 시스템을 보고 머신러닝이 아니니 AI 바깥이라고 느껴질 때
- 생성형 AI 서비스 안에서도 왜 명시적 필터와 정책 규칙이 함께 남는지 설명해야 할 때

이때는 먼저 `사람이 읽을 수 있는 기호와 규칙으로 지식을 적고 그 위에서 판단한다`는 기호 기반 AI의 출발점으로 돌아가야 합니다. 그다음 현재 문제가 `명시적 기준`에 더 가까운지, `복잡한 패턴 학습`에 더 가까운지 나누어 읽으면 경계가 선명해집니다.
