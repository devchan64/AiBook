...
- 지도학습, 비지도학습 알고리즘의 세부 절차
- 신경망 구조와 역전파 수식
- LLM 서비스의 제품 수준 구현 차이

세부 학습 구조는 뒤의 Part 3, Part 4, Part 5, Part 6에서 각각 다시 회수합니다. 여기서는 먼저 `설명 중심이 왜 학습 중심으로 이동했는가`를 큰 역사 흐름으로 정리하는 데 집중합니다.
...
| --- | --- | --- |
| [데이터](../../../reference/concept-glossary.md#data) | 학습과 판단의 재료 | 규칙 대신 무엇에서 기준을 배우는지 보기 위해 |
| [특징](../../../reference/concept-glossary.md#feature) | 사람이 먼저 정리하거나 선택한 단서 | 초기 머신러닝이 어떤 입력 표현에 기대는지 잡기 위해 |
| [표현](../../../reference/concept-glossary.md#representation) | 모델 안에서 다루는 내부 형태 | 딥러닝이 왜 표현 학습으로 설명되는지 보기 위해 |
| [파라미터](../../../reference/concept-glossary.md#parameter) | 학습으로 조정되는 내부 값 | 학습이 실제로 무엇을 바꾸는지 잡기 위해 |
| [머신러닝](../../../reference/concept-glossary.md#machine-learning) | 데이터로 성능을 개선하는 학습 접근 | 규칙 작성에서 모델 학습으로 이동한 이유를 보기 위해 |
| [딥러닝](../../../reference/concept-glossary.md#deep-learning) | 표현 학습을 강하게 쓰는 신경망 기반 접근 | 특징 설계 중심에서 표현 학습 중심으로 이동한 흐름을 보기 위해 |
| [생성형 AI](../../../reference/concept-glossary.md#aigenerative-ai) | 새 콘텐츠를 만들어 내는 모델과 서비스 범주 | 최근 흐름이 어디까지 이어졌는지 잡기 위해 |
...
다음처럼 규칙 기반 접근과 머신러닝, 딥러닝, 생성형 AI가 서로 대체 관계처럼 보일 때 이 절의 흐름을 다시 꺼내면 됩니다.

- 복잡한 입력 문제를 보며 왜 설명의 중심이 규칙 작성에서 데이터 학습으로 옮겨 갔는지 정리해야 할 때
- 특징, 표현, 모델, 파라미터가 각각 어디서 중요해지는지 한 번에 설명하려 할 때
- LLM을 완전히 새로운 단절처럼 보지 말고 이전 AI 흐름 위에 놓아 설명해야 할 때

이때는 `규칙 -> 데이터 학습 -> 표현 학습 -> 생성`의 누적 흐름으로 다시 정리하면 됩니다. 그리고 현재 사례가 `사람이 기준을 직접 쓰는 문제인지`, `데이터에서 패턴을 배워야 하는 문제인지`, `새 콘텐츠를 만들어야 하는 문제인지`를 먼저 나누어 보면 설명 중심이 안정됩니다.
