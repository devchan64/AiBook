...
이 절은 현대 AI의 큰 흐름을 잡는 절입니다. 먼저는 아래 세 가지 관점을 전체 지도처럼 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 규칙만으로 풀기 어려운 문제가 `데이터 기반 학습`으로 이어졌다는 점 | 머신러닝이 왜 중심으로 올라왔는지 이해하게 해 줍니다. | 사람이 모든 기준을 직접 쓰기 어려운 문제가 많아졌다는 점만 알면 충분합니다. |
| 머신러닝, 딥러닝, 생성형 AI는 `서로 끊어진 유행어`가 아니라 이어진 흐름이라는 점 | 용어가 많아도 한 지도 안에서 읽게 해 줍니다. | 데이터 학습 -> 표현 학습 -> 생성으로 확장된 흐름만 잡으면 충분합니다. |
| LLM은 AI 전체가 아니라 `최근의 강한 한 흐름`이라는 점 | 현재의 제품 경험을 전체 역사와 구분하게 해 줍니다. | LLM이 생성형 AI 안의 대표 사례라는 점만 알면 충분합니다. |
...
이 절에서는 `데이터`, `특징`, `표현`, `모델`, `파라미터`가 반복해서 나옵니다. 처음 읽을 때는 다 비슷하게 들릴 수 있으므로, 사전식으로 짧게 구분해 두는 편이 좋습니다.
...
처음 단계에서는 `데이터는 재료`, `특징과 표현은 그 재료를 읽기 좋은 형태로 바꾸는 방법`, `모델은 판단이나 생성을 수행하는 구조`, `파라미터는 학습으로 조정되는 내부 값` 정도로 잡아 두면 충분합니다.
