...
이 절의 목적은 머신러닝, 딥러닝, 생성형 AI를 깊게 설명하는 것이 아닙니다. 그것은 Part 4, Part 5, Part 6에서 따로 다룹니다. 여기서는 “사람이 규칙을 모두 쓰는 방식”에서 “데이터와 경험으로 모델을 학습하는 방식”으로 중심이 이동한 이유를 역사적 관점에서 잡습니다.

Part 1 안에서는 이 절을 `규칙 기반 접근에서 학습 기반 접근으로 중심이 이동한 역사 흐름`, `데이터`, `특징(feature)`, `표현(representation)`, `파라미터(parameter)`의 대표 상세 설명 위치로 사용합니다. `AI/머신러닝/딥러닝/생성형 AI/LLM`의 기본 관계는 1.3에서 먼저 잡았고, 여기서는 그 용어들이 어떤 흐름으로 이어지는지에 필요한 만큼만 다시 연결합니다.
...
세부 학습 구조는 뒤의 Part 4, Part 5, Part 6에서 각각 다시 회수합니다. 이 절은 먼저 `설명 중심이 왜 학습 중심으로 이동했는가`를 큰 역사 흐름으로 정리하는 데 집중합니다.
...
이 절은 현대 AI의 큰 흐름을 잡는 절입니다. 아래 세 가지 관점을 전체 지도로 삼으면 흐름을 붙잡기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| 규칙만으로 풀기 어려운 문제가 `데이터 기반 학습`으로 이어졌다는 점 | 머신러닝이 왜 중심으로 올라왔는지 이해하게 해 줍니다. | 사람이 모든 기준을 직접 쓰기 어려운 문제가 많아졌다는 점을 먼저 구분합니다. |
| 머신러닝, 딥러닝, 생성형 AI는 `서로 끊어진 유행어`가 아니라 이어진 흐름이라는 점 | 용어가 많아도 한 지도 안에서 읽게 해 줍니다. | 데이터 학습 -> 표현 학습 -> 생성으로 확장된 흐름을 먼저 잡습니다. |
| LLM은 AI 전체가 아니라 `최근의 강한 한 흐름`이라는 점 | 현재의 제품 경험을 전체 역사와 구분하게 해 줍니다. | LLM이 생성형 AI 안의 대표 사례라는 점을 먼저 연결합니다. |

`데이터`, `특징`, `표현`, `모델`, `파라미터`는 초반에 서로 비슷하게 들릴 수 있습니다. 이 절에서는 아래 정도로 짧게 다시 구분해 두고 계속 읽으면 됩니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 데이터 | 학습이나 판단의 재료가 되는 사례 모음 | 모델이 무엇을 볼 수 있는지 정하는 바탕 |
| 특징 | 사람이 읽기 좋게 뽑아 낸 입력 단서 | 전통적 머신러닝에서 중요한 설계 대상 |
| 표현 | 모델이 내부에서 다루는 입력 형태 | 딥러닝이 더 많이 학습하려는 대상 |
| 모델 | 입력에서 출력을 만드는 계산 구조 | 학습 결과가 담기는 핵심 단위 |
| 파라미터 | 학습으로 조정되는 내부 값 | 같은 입력에 대한 출력 차이를 만드는 기준 |

이 단계에서는 `데이터는 재료`, `특징은 사람이 만든 단서`, `표현은 모델 안의 형태`, `모델은 계산 구조`, `파라미터는 학습으로 바뀌는 값`이라는 구분이 먼저 남아 있으면 됩니다.
...
이 절에서는 `데이터`, `특징`, `표현`, `모델`, `파라미터`가 반복해서 나옵니다. 다 비슷하게 들릴 수 있으므로, 사전식으로 짧게 구분해 두는 편이 좋습니다.
...
이 그림은 한 기술이 이전 기술을 지워 버렸다는 뜻이 아니라, AI 설명의 중심이 점점 더 많은 층을 쌓아 왔다는 뜻으로 읽어야 합니다. 여기서는 `규칙 -> 불확실성 처리 -> 데이터 학습 -> 표현 학습 -> 생성`이라는 큰 흐름이 먼저 보이면 됩니다.
