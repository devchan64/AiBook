...
Part 1 안에서는 이 절을 `탐색(search)`과 `확률 추론(probabilistic reasoning)`의 대표 상세 설명 위치로 사용합니다. `지식 표현(knowledge representation)`은 2.1에서 기본 뜻을 먼저 소개했고, 여기서는 탐색과 확률 추론 사이에서 어떤 역할을 하는지 비교하는 데 필요한 만큼만 다시 연결합니다.
...
이 절은 세 흐름의 `역할 차이`를 먼저 잡는 데 집중합니다. 데이터에서 패턴을 학습하는 방향으로 설명 중심이 이동하는 이유는 다음 2.3에서 다시 회수합니다.
...
이 절은 용어가 세 갈래로 나뉘어 보여서 초반에 흐름이 흩어지기 쉽습니다. 아래 세 가지를 큰 지도로 삼으면 구조를 붙잡기 쉽습니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 잡을 기준 |
| --- | --- | --- |
| 탐색(search)은 `후보가 많을 때 어디부터 볼지 정하는 문제`라는 점 | 규칙만으로는 해결되지 않는 이유가 보입니다. | 가능한 선택지가 너무 많아 순서가 필요하다는 점을 먼저 구분합니다. |
| 지식 표현은 `무엇을 알고 있다고 적을 것인가`의 문제라는 점 | 규칙, 사실, 관계가 왜 따로 언급되는지 연결됩니다. | 문제를 풀 기준을 컴퓨터가 다룰 수 있게 적는다는 점을 먼저 잡습니다. |
| 확률 추론은 `불완전한 정보에서 얼마나 그럴듯한가`를 다루는 방식이라는 점 | 참/거짓만으로 설명되지 않는 현실 문제와 연결됩니다. | 정보가 애매할 때 확률이 판단 언어가 된다는 점을 먼저 연결합니다. |

`상태`, `행동`, `목표`, `표현`, `확률`은 한꺼번에 나오면 낯설 수 있습니다. 이 절에서는 아래 정도로 짧게 구분해 두고 계속 읽으면 됩니다.
...
이 단계에서는 `탐색은 상태와 행동과 목표를 다루고`, `지식 표현은 무엇을 적어 둘지 정하며`, `확률 추론은 애매한 정보의 그럴듯함을 다룬다`는 구분이 먼저 남아 있으면 됩니다.
...
이 도식은 탐색을 `시작 상태 -> 후보 -> 평가 기준 -> 목표` 흐름으로 읽게 해 줍니다. 여기서는 `문제를 풀려면 후보를 만들고, 그중 어디를 먼저 볼지 기준이 필요하다`는 구조가 먼저 보이면 됩니다.
...
이 그림은 세 흐름이 서로 경쟁하는 것이 아니라, 같은 문제를 풀 때 서로 다른 역할로 연결된다는 점을 보여 줍니다. 여기서는 `지식 표현은 바탕을 적고`, `탐색은 후보를 찾고`, `확률 추론은 애매한 정보를 계산해 결론을 돕는다`는 분업 구조가 먼저 보이면 됩니다.
...
이 사례를 통해 이 절의 세 용어를 따로 외우기보다, `후보를 찾는 일`, `세계를 적는 일`, `애매한 정보를 판단하는 일`이 실제 시스템 안에서 동시에 등장한다고 읽으면 됩니다.
