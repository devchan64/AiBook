...
이 절은 용어가 세 갈래로 나뉘어 보여서 처음 읽을 때 흩어지기 쉽습니다. 먼저 아래 세 가지를 큰 지도처럼 붙잡으면 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 탐색(search)은 `후보가 많을 때 어디부터 볼지 정하는 문제`라는 점 | 규칙만으로는 해결되지 않는 이유가 보입니다. | 가능한 선택지가 너무 많아 순서가 필요하다는 점만 알면 충분합니다. |
| 지식 표현은 `무엇을 알고 있다고 적을 것인가`의 문제라는 점 | 규칙, 사실, 관계가 왜 따로 언급되는지 연결됩니다. | 문제를 풀 기준을 컴퓨터가 다룰 수 있게 적는다는 점만 잡으면 충분합니다. |
| 확률 추론은 `불완전한 정보에서 얼마나 그럴듯한가`를 다루는 방식이라는 점 | 참/거짓만으로 설명되지 않는 현실 문제와 연결됩니다. | 정보가 애매할 때 확률이 판단 언어가 된다는 점만 알면 충분합니다. |
