...
다음처럼 한 문제 안에 탐색, 지식 표현, 확률 추론이 한꺼번에 섞여 보여 무엇을 먼저 설명해야 할지 막힐 때 이 절의 기준을 다시 꺼내면 됩니다.

- 로봇, 경로, 배차, 게임 같은 사례에서 `후보를 찾는 일`과 `세계를 적는 일`이 구분되지 않을 때
- 센서 오차나 불완전한 정보가 붙으면서 규칙 설명만으로는 부족하다고 느껴질 때
- 현대 AI 서비스 안에서도 검색, 지식 그래프, 확률적 예측이 어떤 역할로 남는지 설명해야 할 때

이때는 `무엇을 찾는가`, `무엇을 적어 두어야 하는가`, `무엇이 불확실한가`를 따로 적어 보면 됩니다. 그러면 탐색, 지식 표현, 확률 추론이 경쟁 개념이 아니라 서로 다른 질문에 답하는 도구라는 점이 다시 드러납니다.
