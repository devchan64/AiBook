...
Part 1 안에서는 이 절을 `에이전트(agent)`, `목표(goal)`, `상태(state)`, `행동(action)`, `관찰(observation)`, `종료 조건(stop condition)`의 대표 상세 설명 위치로 사용합니다. 14.2에서는 RAG와 도구 사용을 서비스 안의 두 구조로 구분했고, 여기서는 그 구조들이 `어떻게 여러 단계 작업으로 이어지는가`를 설명합니다. MCP는 14.4에서, 하네스와 평가는 14.5에서 다시 다룹니다.

## 이 절의 범위

이 절은 에이전트(agent)를 `목표를 작업 흐름(workflow)으로 이어 가는 구조`로 설명합니다. MCP(Model Context Protocol)처럼 도구 연결을 표준화하는 방식은 P1-14.4에서 다룹니다. 하네스(harness), 실행 로그(log), 평가(evaluation), 재현성은 P1-14.5에서 다룹니다. 비용(cost), 지연 시간(latency), 운영(operation)은 P1-14.6에서 다룹니다.

처음 읽을 때는 `에이전트`, `목표`, `상태`, `행동`, `관찰`, `종료 조건`이 모두 추상적인 운영 용어처럼 들릴 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 에이전트 | 목표를 여러 단계 작업으로 이어 가는 실행 구조 | Chapter 14 중반의 중심 개념 |
| 목표 | 해결하려는 일 | 루프의 출발점 |
...
처음 단계에서는 `에이전트는 목표를 따라 상태를 보며 행동을 잇는 구조` 정도로만 잡아도 충분합니다.
...
이 절은 에이전트를 과장된 지능체로 설명하는 절이 아니라, 여러 단계 작업 흐름으로 보는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 에이전트(agent)는 목표를 여러 단계 작업으로 이어 가는 실행 구조라는 점 | 챗봇과 에이전트를 구분하게 해 줍니다. | 한 번 답하고 끝나는 대신 상태를 보며 다음 행동을 잇는다고 이해하면 충분합니다. |
| 프롬프트, RAG, 도구 사용은 에이전트 안의 일부가 될 수 있다는 점 | 관련 개념들을 한 지도 안에 배치하게 해 줍니다. | 에이전트가 이 요소들을 조합해 쓸 수 있다고 알면 충분합니다. |
| 에이전트는 무제한 자율성이 아니라 권한과 승인 안에서 동작해야 한다는 점 | 실제 서비스 관점의 안전한 이해를 만들어 줍니다. | 서버와 앱이 범위와 승인 조건을 관리한다고 이해하면 충분합니다. |
...
또한 이 절은 `MCP 연결 방식의 세부 프로토콜`을 설명하는 절도, `실행 추적과 평가 기준`을 자세히 설명하는 절도 아닙니다. 여기서는 에이전트를 서비스 구조의 한 종류로 먼저 잡고, 연결 규약은 14.4, 검증 장치는 14.5에서 다시 좁혀 봅니다.
