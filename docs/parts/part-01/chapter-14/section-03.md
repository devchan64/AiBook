...
| --- | --- | --- |
| 에이전트(agent)는 목표를 여러 단계 작업으로 이어 가는 실행 구조라는 점 | 챗봇과 에이전트를 구분하게 해 줍니다. | 한 번 답하고 끝나는 대신 상태를 보며 다음 행동을 잇는 구조로 이해합니다. |
...
- 에이전트를 단순히 더 똑똑한 챗봇이나 모델 이름처럼 부르고 있을 때
- RAG, 도구 사용, 에이전트의 관계가 단계 구조로 보이지 않을 때
- 실패 지점과 종료 조건을 포함한 실행 흐름으로 설명해야 할 때

이때는 먼저 `목표`, `상태`, `행동`, `관찰`, `종료 조건`을 나누면 됩니다. 그러면 에이전트를 단일 모델 능력이 아니라 여러 단계 작업을 운영하는 구조로 설명할 수 있습니다.
