...
| --- | --- | --- |
| MCP | AI 앱과 외부 시스템 연결 규약 | Chapter 14의 표준화 관점 |
| 호스트 | 사용자가 만나는 앱 또는 실행 환경 | 연결을 소유하는 쪽 |
| 클라이언트 | 특정 MCP 서버와 통신하는 구성요소 | 연결 관리 단위 |
| 서버 | 도구, 리소스, 프롬프트를 제공하는 프로그램 | 외부 능력 제공자 |
| 도구 | 실행 가능한 기능 | 행동 호출의 단위 |
...
P1-14.1에서 AI 서비스는 모델(model), 앱(application), 데이터(data), 도구(tool), 흐름(orchestration)의 조합이라고 했습니다. 문제는 서비스가 커질수록 연결해야 할 외부 시스템도 늘어난다는 점입니다.
...
```mermaid
--8<-- "assets/part-01/chapter-14/mcp-connection-flow-ko.mmd"
```
...
이 구분은 P1-14.2에서 본 RAG와 도구 사용의 차이와 연결됩니다.
...
예를 들어 조직 내부 문서 시스템을 MCP 서버로 제공한다면, 다음처럼 나눌 수 있습니다.
...
P1-14.3의 에이전트 루프를 다시 가져오면 MCP의 위치가 더 분명해집니다.
...
긴 문서 작업 과정을 예로 들면 MCP의 위치를 더 쉽게 볼 수 있습니다.
...
| 작업 | MCP로 연결될 수 있는 대상 |
| --- | --- |
| 문서 파일 읽기 | 파일 시스템 MCP 서버 |
| 이슈와 작업 상태 확인 | 이슈 추적 또는 프로젝트 관리 MCP 서버 |
| 참고 자료 검색 | 검색 또는 문서 저장소 MCP 서버 |
| 검증 작업 실행 | 로컬 실행 환경 또는 명령 실행 도구 |
...
- `연결 규약`, `능력 제공`, `실행 책임`을 나누어 MCP를 도구나 에이전트 그 자체와 구분할 수 있다.
