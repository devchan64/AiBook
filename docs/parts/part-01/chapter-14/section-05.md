...
여기서는 하네스(harness), 추적(trace), 로그(log), 평가(evaluation), 그레이더(grader)의 기본 역할을 설명합니다. 비용(cost), 지연 시간(latency), 운영(operation) 제약은 P1-14.6에서 다시 다룹니다. 여기서는 먼저 `에이전트 실행을 어떻게 관찰 가능하고 비교 가능하게 만들 것인가`를 닫는 데 집중합니다.
...
- 에이전트(agent)와 에이전트 도구 사용(tool use)에는 추적(trace)과 로그(log)가 필요함을 이해합니다.
...
OpenAI Agents SDK 문서는 실행 추적이 모델 호출, 도구 호출, 핸드오프(handoff), AI 가드레일(guardrail), 커스텀 스팬(custom span)을 구조화된 기록으로 남길 수 있음을 설명합니다. 이 문서를 제품 사용법으로 외울 필요는 없습니다. 여기서는 “에이전트 실행은 단계별 관찰이 필요하다”는 근거로 읽으면 됩니다.
...
| AI 가드레일(guardrail) | 어떤 검증이나 차단 조건이 작동했는가? |
...
- `실행을 감싼다`, `단계를 기록한다`, `반복 비교한다`를 기준으로 하네스의 역할을 설명할 수 있다.
