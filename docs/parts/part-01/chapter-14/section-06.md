...
Part 1 안에서는 이 절을 `비용(cost)`, `지연 시간(latency)`, `처리량(throughput)`, `레이트 리밋(rate limit)`, `재시도(retry)`, `배치(batch)`, `캐싱(caching)`, `운영(operation)`의 대표 상세 설명 위치로 사용합니다. 14.5에서는 실행을 감싸고 평가하는 하네스를 봤고, 여기서는 그 실행이 `현실 서비스에서 얼마나 오래, 얼마나 자주, 얼마의 비용으로 유지될 수 있는가`를 정리합니다. 보안과 개인정보는 15장에서 다시 다룹니다.

## 이 절의 범위

이 절은 AI 서비스를 실제로 사용할 때 고려해야 하는 제약을 개론 수준에서 설명합니다. 특정 사업자의 가격표, 특정 모델 추천, 배포 인프라 구성, 클라우드 비용 계산은 깊게 다루지 않습니다. 보안 정책과 개인정보 처리는 바로 다음 P1-15.1 편향·안전성·책임, P1-15.2 저작권과 학습 데이터, P1-15.3 보안과 개인정보에서 다시 다루고, 구체적인 사업자별 가격표와 인프라 설계 비교는 현재 본편 범위 밖에 둡니다.

처음 읽을 때는 `비용`, `지연 시간`, `처리량`, `레이트 리밋`, `재시도`, `배치`, `캐싱`, `운영`이 모두 실무 용어라 한꺼번에 부담스러울 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.
...
처음 단계에서는 `품질 외에 비용과 지연 시간을 함께 봐야 한다` 정도로만 잡아도 충분합니다.
...
이 절은 기술 구조가 서비스로 이어질 때 무엇이 부딪히는지 정리하는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 좋은 답변만으로는 서비스가 완성되지 않는다는 점 | 모델 성능과 운영 가능성을 구분하게 해 줍니다. | 품질 외에 비용과 지연 시간도 함께 봐야 한다고 이해하면 충분합니다. |
| 비용(cost)은 모델 한 번 호출값만이 아니라 전체 흐름에서 생긴다는 점 | RAG, 도구 사용, 재시도까지 서비스 비용으로 읽게 해 줍니다. | 토큰 수 외에도 검색, 평가, 실패 재시도가 비용을 만든다고 알면 충분합니다. |
| 지연 시간(latency)과 실패 대응도 설계의 일부라는 점 | 실제 사용자 경험과 운영 안정성을 함께 보게 해 줍니다. | 느리거나 자주 실패하면 좋은 모델도 서비스로 쓰기 어렵다고 이해하면 충분합니다. |
...
또한 이 절은 `사업자별 가격표 비교`를 하는 절도, `보안과 개인정보 정책`을 정리하는 절도 아닙니다. 여기서는 현실 제약을 설계 조건으로 먼저 잡고, 위험과 책임의 세부 논의는 15장으로 넘깁니다.
