...
여기서 필요한 것은 세부 구현보다 큰 지도입니다.

Part 1에서 `모델(model)`, `앱(application)`, `데이터(data)`, `도구(tool)`, `흐름 조정(orchestration)`의 기본 구분은 여기서 잡습니다. 13장에서는 임베딩, 검색, RAG를 중심으로 `모델이 외부 자료를 어떻게 참고하는가`를 봤고, 여기서는 그 흐름을 한 단계 넓혀 `AI 서비스 전체는 어떤 부품 조합으로 이루어지는가`를 정리합니다. RAG와 도구 사용의 구체적 구분은 14.2로, 여러 단계 실행 구조는 14.3 이후로 넘깁니다.

## 이 절의 범위

여기서는 AI 서비스의 주요 구성요소를 구분합니다. RAG와 도구 사용(tool use)의 구체적 위치는 P1-14.2에서 다룹니다. 에이전트(agent), MCP(Model Context Protocol), 하네스(harness), 비용과 운영 제약은 P1-14.3 이후에서 다룹니다.

`모델`, `앱`, `데이터`, `도구`, `흐름 조정`은 서로 다른 서비스 구성요소입니다. 각 용어의 역할은 다음처럼 구분할 수 있습니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 모델 | 입력을 바탕으로 생성과 판단을 수행하는 계산 부품 | 서비스 핵심 계산 요소 |
| 앱 | 사용자가 요청하고 결과를 받는 표면과 흐름 | 사용자 경험의 입구 |
| 데이터 | 근거와 상태를 제공하는 정보 | 답변과 실행의 재료 |
| 도구 | 외부 시스템 기능을 호출하는 연결 수단 | 조회와 행동의 실행면 |
| 흐름 조정 | 이 요소들을 어떤 순서와 조건으로 묶는가 | 서비스 구조를 만드는 틀 |

여기서는 `모델은 계산`, `앱은 경험`, `데이터는 근거`, `도구는 실행`, `흐름 조정은 연결 방식`이라는 구분을 기준선으로 둡니다.
...
- P1-14.2의 RAG와 도구 사용(tool use) 설명으로 넘어갈 준비를 합니다.

## 세 가지 기준

여기서는 AI 서비스를 모델 하나로만 보는 시각을 풀어 줍니다. 본문을 읽을 때 기준이 되는 세 가지 관점은 다음과 같습니다.

| 기준 | 왜 중요한가 | 이 절에서 필요한 이해 수준 |
| --- | --- | --- |
| AI 서비스는 모델(model) 하나만으로 이루어지지 않는다는 점 | 챗봇 경험을 곧바로 전체 시스템과 같게 보는 오해를 줄여 줍니다. | 앱, 데이터, 도구가 함께 있어야 서비스가 된다고 이해합니다. |
| 모델, 앱, 데이터, 도구는 서로 다른 역할을 맡는다는 점 | 뒤의 RAG, 에이전트, MCP 설명을 읽는 기본 지도입니다. | 모델은 생성, 앱은 흐름 관리, 데이터는 근거, 도구는 외부 행동이라고 구분합니다. |
| 서비스 품질은 모델 능력만이 아니라 전체 연결 방식에 달린다는 점 | 아키텍처 관점을 처음 읽는 독자도 잡게 해 줍니다. | 답변 생성 외에 권한, 검색, 후처리가 함께 중요하다고 이해합니다. |

## 모델은 답변을 만드는 핵심 부품이다

모델(model)은 AI 서비스의 핵심 부품입니다. 사용자의 입력을 받아 텍스트, 코드, 이미지 설명, 구조화된 데이터 같은 출력을 만들 수 있습니다.

여기서는 모델을 다음처럼 단순화해 볼 수 있습니다.
...
첫째, 답변의 근거가 됩니다. RAG에서 검색하는 문서, 조직의 지식 문서, 제품 설명서, 주제별로 나뉜 학습 문서가 여기에 해당합니다.
...
이 관점은 Part 1 Chapter 13의 RAG 설명과 연결됩니다. RAG는 데이터 전체를 모델에 넣는 방식이 아니라, 질문과 관련된 후보를 찾아 입력 맥락(context)에 넣는 방식입니다.

## 도구는 모델 밖의 행동을 실행한다

도구(tool)는 모델 밖의 시스템과 연결되는 실행 수단입니다. OpenAI의 function calling 문서도 큰 언어 모델이 외부 데이터와 시스템에 연결될 수 있음을 설명합니다.

여기서는 도구를 이렇게 이해하면 됩니다.
...
도구 사용은 강력하지만 위험도 있습니다. 잘못된 도구를 호출하거나, 권한 없는 데이터에 접근하거나, 사용자가 승인하지 않은 행동을 실행하면 문제가 됩니다. 그래서 도구 호출은 P1-14.2에서 더 구체적으로 다룹니다.
...
이 절을 읽은 뒤에는 아래 네 가지 구분을 다시 설명할 수 있어야 합니다.

| 남겨야 할 핵심 구분 | 뒤에서 다시 읽는 위치 |
| --- | --- |
| 모델은 생성과 판단을 담당하는 계산 부품이다. | Part 5의 모델 동작과 평가 절 |
| 데이터는 근거 자료와 상태 정보를 제공한다. | P1-14.2의 RAG와 도구 사용, Part 5의 RAG 절 |
| 도구는 모델 밖 시스템을 실제로 실행하는 연결 수단이다. | P1-14.2, P1-14.3, Part 5의 tool use/agent 절 |
| 서비스 품질은 모델 하나가 아니라 연결 방식 전체에 달려 있다. | P1-14.3부터 P1-14.6, Part 6 프로젝트 기록 절 |

## 언제 이 관점을 먼저 떠올려야 하는가

- 모델 성능만 올리면 서비스가 좋아질 것처럼 보일 때
- RAG, 도구 사용, 에이전트를 읽기 전에 서비스 구성요소 지도가 먼저 필요할 때
- 앱, 데이터, 도구, 흐름 조정의 책임이 한 문장 안에서 섞여 보일 때

이때는 먼저 `모델`, `앱`, `데이터`, `도구`, `흐름 조정`을 나누면 됩니다. 그러면 챗봇 경험을 곧바로 전체 서비스 구조와 같게 보는 오해를 줄일 수 있습니다.
