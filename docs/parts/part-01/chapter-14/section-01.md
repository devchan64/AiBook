...
Part 1 안에서는 이 절을 `모델(model)`, `앱(application)`, `데이터(data)`, `도구(tool)`, `흐름 조정(orchestration)`의 대표 상세 설명 위치로 사용합니다. 13장에서는 임베딩, 검색, RAG를 중심으로 `모델이 외부 자료를 어떻게 참고하는가`를 봤고, 여기서는 그 흐름을 한 단계 넓혀 `AI 서비스 전체는 어떤 부품 조합으로 이루어지는가`를 정리합니다. RAG와 도구 사용의 구체적 구분은 14.2로, 여러 단계 실행 구조는 14.3 이후로 넘깁니다.

## 이 절의 범위

이 절은 AI 서비스의 주요 구성요소를 구분합니다. RAG와 도구 사용(tool use)의 구체적 위치는 P1-14.2에서 다룹니다. 에이전트(agent), MCP(Model Context Protocol), 하네스(harness), 비용과 운영 제약은 P1-14.3 이후에서 다룹니다.

처음 읽을 때는 `모델`, `앱`, `데이터`, `도구`, `흐름 조정`이 모두 서비스 안의 비슷한 부품처럼 들릴 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.
...
처음 단계에서는 `모델은 계산`, `앱은 경험`, `데이터는 근거`, `도구는 실행`, `흐름 조정은 연결 방식` 정도로만 잡아도 충분합니다.
...
이 절은 AI 서비스를 모델 하나로 보는 시각을 풀어 주는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| AI 서비스는 모델(model) 하나만으로 이루어지지 않는다는 점 | 챗봇 경험을 곧바로 전체 시스템과 같게 보는 오해를 줄여 줍니다. | 앱, 데이터, 도구가 함께 있어야 서비스가 된다고 이해하면 충분합니다. |
| 모델, 앱, 데이터, 도구는 서로 다른 역할을 맡는다는 점 | 뒤의 RAG, 에이전트, MCP 설명을 읽는 기본 지도입니다. | 모델은 생성, 앱은 흐름 관리, 데이터는 근거, 도구는 외부 행동이라고 구분하면 충분합니다. |
| 서비스 품질은 모델 능력만이 아니라 전체 연결 방식에 달린다는 점 | 아키텍처 관점을 처음 읽는 독자도 잡게 해 줍니다. | 답변 생성 외에 권한, 검색, 후처리가 함께 중요하다고 알면 충분합니다. |
...
또한 이 절은 `RAG와 도구 사용의 세부 비교`를 하는 절도, `에이전트 구현 방식`을 설명하는 절도 아닙니다. 여기서는 서비스 지도를 먼저 잡고, 구조 간 경계는 14.2부터 차례대로 다시 좁혀 봅니다.
...
처음 읽을 때는 아래 네 줄만 분명히 구분하면 충분합니다.
