...
이 절은 AI 서비스를 모델 하나로 보는 시각을 풀어 주는 절입니다. 처음에는 아래 세 가지만 먼저 붙잡으면 됩니다.
...
| 서비스 품질은 모델 능력만이 아니라 전체 연결 방식에 달린다는 점 | 아키텍처 관점을 초심자도 잡게 해 줍니다. | 답변 생성 외에 권한, 검색, 후처리가 함께 중요하다고 알면 충분합니다. |
