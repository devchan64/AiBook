...
여기서는 AI 서비스의 주요 구성요소를 구분합니다. RAG와 에이전트 도구 사용(tool use)의 구체적 위치는 P1-14.2에서 다룹니다. 에이전트(agent), MCP(Model Context Protocol), 하네스(harness), 비용과 운영 제약은 P1-14.3 이후에서 다룹니다.
...
- P1-14.2의 RAG와 에이전트 도구 사용(tool use) 설명으로 넘어갈 준비를 합니다.
...
- `모델`, `앱`, `데이터`, `도구`, `흐름 조정`을 나누어 챗봇 경험과 전체 서비스 구조를 구분할 수 있다.
