...
Part 1 안에서는 이 절을 `RAG(retrieval-augmented generation)`, `도구 사용(tool use)`, `검색(retrieval)`, `도구 호출(tool call)`, `승인(approval)`의 대표 상세 설명 위치로 사용합니다. 13.3에서는 RAG를 `검색 결과를 생성 입력에 붙이는 구조`로 먼저 봤고, 14.1에서는 서비스 전체를 모델, 앱, 데이터, 도구의 조합으로 넓혔습니다. 여기서는 그 둘을 연결해 `RAG와 도구 사용은 서비스 안에서 어디가 다른가`를 분명히 구분합니다.

## 이 절의 범위

이 절은 RAG와 도구 사용의 위치를 비교합니다. 구체적인 에이전트(agent) 구조는 P1-14.3에서 다룹니다. MCP(Model Context Protocol)처럼 도구 연결을 표준화하는 방식은 P1-14.4에서 다룹니다. 하네스(harness), 평가, 실행 로그는 P1-14.5에서 다룹니다.

처음 읽을 때는 `RAG`, `도구 사용`, `검색`, `도구 호출`, `승인`이 모두 모델 밖을 다루는 비슷한 절차처럼 들릴 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.
...
처음 단계에서는 `RAG는 읽기`, `도구 사용은 실행`, `승인은 행동 전 검토` 정도로만 잡아도 충분합니다.
...
이 절은 둘 다 “모델 밖을 쓴다”는 이유로 섞이기 쉬운 두 구조를 분리하는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| RAG는 자료를 찾아 맥락에 붙이는 구조라는 점 | 검색과 생성의 결합이 무엇인지 명확히 보여 줍니다. | 문서를 읽어 답변 근거로 붙인다고 이해하면 충분합니다. |
| 도구 사용(tool use)은 외부 기능을 실제로 호출하는 구조라는 점 | 문서를 참고하는 것과 행동을 실행하는 것을 구분하게 해 줍니다. | 검색 API, 계산기, DB 조회처럼 바깥 기능을 부른다고 알면 충분합니다. |
| 둘은 함께 쓸 수 있지만 역할과 위험이 다르다는 점 | 에이전트 설명으로 넘어가기 전 경계를 세워 줍니다. | 읽기와 실행은 다르고, 검토 책임도 다르다고 이해하면 충분합니다. |
...
> 요청:
> 벡터 검색 설명의 근거를 확인하고, 관련 문단을 보강해 줘.
...
또한 이 절은 `에이전트가 여러 단계를 어떻게 이어 가는가`를 설명하는 절도, `MCP가 도구 연결을 어떻게 표준화하는가`를 설명하는 절도 아닙니다. 여기서는 `읽기 구조`와 `실행 구조`를 먼저 분리하고, 다단계 오케스트레이션은 14.3 이후에서 다시 다룹니다.
