...
Part 1에서 `RAG(retrieval-augmented generation)`, `도구 사용(tool use)`, `검색(retrieval)`, `도구 호출(tool call)`, `승인(approval)`의 기본 구분은 여기서 잡습니다. 13.3에서는 RAG를 `검색 결과를 생성 입력에 붙이는 구조`로 먼저 봤고, 14.1에서는 서비스 전체를 모델, 앱, 데이터, 도구의 조합으로 넓혔습니다. 여기서는 그 둘을 연결해 `RAG와 도구 사용은 서비스 안에서 어디가 다른가`를 분명히 구분합니다.
...
- 검색과 도구 호출이 모두 비슷한 외부 연결처럼 보여 차이가 흐려질 때
- RAG와 tool use를 같은 구조로 설명하려 할 때
- 이후 에이전트 설명으로 넘어가기 전에 읽기와 실행의 책임을 먼저 가르고 싶을 때

이때는 먼저 `자료를 읽는가`, `외부 기능을 실행하는가`, `사전 승인과 검증이 필요한가`를 나누면 됩니다. 그러면 RAG와 도구 사용을 같은 범주의 연결로 뭉뚱그리지 않고 설명할 수 있습니다.
