...
在同一个 Part 内，重要概念的详细说明应尽量只留在一个主 Section。后续 Section 只重新接回当前问题所需的内容。如果阅读过程中某个术语再次变得不稳定，请先回到 [概念词汇表](../../reference/concept-glossary.md)，先看 `Core Section`，再沿着 `Appears In` 列表追踪它在后续语境中的回归位置。
