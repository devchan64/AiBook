...
이 절의 질문은 다음입니다. AI 시스템은 어디에서 확률을 사용하고, 그 숫자는 어떻게 조심해서 읽어야 할까요?

AI가 확률(probability)을 사용한다는 말은 “아무렇게나 답한다”는 뜻이 아닙니다. 정보가 부족하거나 후보가 여러 개일 때, 가능한 결과의 그럴듯함을 숫자로 비교한다는 뜻에 가깝습니다.

Part 1 안에서는 이 절을 `확률 숫자가 실제 AI 시스템에서 어떻게 쓰이는가`를 설명하는 대표 상세 설명 위치로 사용합니다. 6.2에서는 `불확실성`, `확률`, `확률적 과정`의 용어 기준을 먼저 잡았고, 여기서는 그 숫자가 분류, 예측, 생성, 의사결정에서 어떤 역할을 하는지와 왜 그대로 정답이나 책임 있는 결정으로 읽으면 안 되는지를 정리합니다.
...
이 절은 확률 계산 공식을 배우는 절이 아니라, AI 시스템이 숫자를 어디에 쓰는지 읽는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 분류에서 확률 점수는 후보의 그럴듯함을 비교하는 재료라는 점 | 높은 점수를 곧바로 정답으로 읽는 실수를 줄여 줍니다. | 0.68이 “가장 가능성 높음”이지 “절대 참”은 아니라는 점만 알면 충분합니다. |
| 예측과 생성에서도 불확실성이나 후보 선택이 함께 따라온다는 점 | 확률이 분류에만 쓰인다고 좁게 보지 않게 해 줍니다. | 값 하나만 보는 게 아니라 범위와 선택 과정도 본다고 이해하면 충분합니다. |
| 확률 출력은 최종 업무 결정과 같지 않다는 점 | 정책, 임계값, 사람 검토가 왜 필요한지 보여 줍니다. | 모델 점수 뒤에 서비스 판단이 한 번 더 붙는다고 이해하면 충분합니다. |

처음 읽을 때는 `확률 추정값`, `임계값`, `보정`, `생성 설정`, `의사결정`이 모두 비슷한 숫자 문제처럼 들릴 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.

| 용어 | 아주 짧은 뜻 | 이 절에서의 역할 |
| --- | --- | --- |
| 확률 추정값 | 후보가 얼마나 그럴듯한지 나타내는 숫자 | 분류와 예측에서 비교 기준 |
| 임계값 | 점수를 보고 행동을 갈라놓는 기준선 | 자동 처리, 보류, 사람 검토를 나누는 기준 |
| 보정 | 점수를 실제 신뢰 수준처럼 읽을 수 있는지 확인하는 일 | `0.80`을 어느 정도 믿을지 점검 |
| 생성 설정 | 생성 후보 선택 방식을 조절하는 값 | temperature 같은 출력 변동성 조절 기준 |
| 의사결정 | 모델 숫자와 비용·정책을 함께 보고 행동을 정하는 일 | 확률을 실제 서비스 행동으로 연결하는 단계 |

처음 단계에서는 `확률 숫자는 후보 비교`, `임계값은 행동 분기`, `보정은 숫자 해석 점검`, `생성 설정은 선택 방식 조절`, `의사결정은 서비스 행동 결정` 정도로만 잡아도 충분합니다.
...
초심자는 이 네 줄을 `확률 숫자 -> 곧바로 정답`으로 읽지 않는 기준으로 삼으면 충분합니다. 핵심은 모델이 숫자를 만들고, 시스템이 그 숫자를 해석하고, 필요한 경우 사람이 마지막 판단을 맡는다는 분업 구조를 보는 데 있습니다.
