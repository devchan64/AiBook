...
| 보정 | 점수를 실제 확률처럼 읽을 수 있는지 확인하는 일 | `0.80`을 어느 정도 믿을지 점검 |
...
모델이 `0.80`이라고 출력했다고 해서, 실제로 항상 80%의 의미를 갖는 것은 아닙니다. scikit-learn은 어떤 모델은 클래스 확률을 좋지 않게 추정할 수 있으며, 잘 보정된 분류기(well-calibrated classifier)에서는 `predict_proba` 출력을 실제 비율에 가까운 확률처럼 해석할 수 있다고 설명합니다.
...
- 보정(calibration)이 모델의 확률 출력을 실제 비율에 가까운 확률처럼 읽을 수 있는지 확인하는 문제라는 점을 설명할 수 있다.
...
- `확률 숫자 -> 곧바로 정답`으로 읽지 않고 서비스 문맥에서 해석할 수 있다.
