...
이 절의 질문은 단순합니다. 왜 어떤 문제는 명시적 규칙만으로 처리하기 어려운지 묻습니다.
...
Part 1 안에서는 이 절을 `불완전한 정보(incomplete information)`, `부분 관측(partial observability)`, `잡음(noise)`, `예외(exception)`, `후보 폭증`이 왜 규칙만으로 처리하기 어려운지 설명하는 대표 상세 설명 위치로 사용합니다. `확률`, `불확실성`, `stochastic`의 개념 구분은 6.2에서 더 자세히 다루고, `탐색`과 `휴리스틱`의 구체 구조는 P1-7에서 다시 회수합니다. 여기서는 먼저 `왜 여러 접근이 함께 필요해지는가`를 문제 조건 중심으로 잡는 데 집중합니다.
...
이 절은 규칙을 버리자는 절이 아니라, 규칙만으로 닫히지 않는 문제를 설명하는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 규칙은 여전히 필요하다는 점 | 이후 확률과 학습 설명을 규칙의 대체가 아니라 보완으로 읽게 해 줍니다. | 정책, 금지 조건, 절차에는 규칙이 필요하다는 점만 알면 충분합니다. |
| 현실 문제에는 정보 부족, 예외, 잡음이 자주 섞인다는 점 | 왜 명시적 규칙이 금방 부족해지는지 보여 줍니다. | 같은 문장이나 같은 장면도 상황에 따라 해석이 달라질 수 있다고 이해하면 충분합니다. |
| 그래서 불확실성, 확률, 휴리스틱, 학습이 이어서 등장한다는 점 | P1-6과 P1-7의 흐름을 한 번에 잡게 해 줍니다. | 규칙 다음에 왜 확률적 사고가 필요한지 감만 잡히면 충분합니다. |

처음 읽을 때는 `정보 부족`, `부분 관측`, `잡음`, `예외`, `후보 폭증`이 모두 비슷한 문제처럼 들릴 수 있습니다. 이 절에서는 아래 정도로만 짧게 구분해 두면 충분합니다.
...
처음 단계에서는 `정보가 없거나`, `보여도 흔들리거나`, `예외가 많거나`, `후보가 너무 많으면` 규칙만으로 닫히기 어렵다는 감각만 잡아도 충분합니다.
...
초심자는 이 도식을 `한 문제를 푸는 데 규칙 하나만 있는 것이 아니라, 상황에 따라 탐색, 확률, 학습, 사람 검토가 함께 붙을 수 있다`는 구조로 읽으면 충분합니다. 여기서 핵심은 무엇이 최종 정답인지를 외우는 것이 아니라, 어떤 문제 조건이 어떤 접근을 더 필요하게 만드는지 연결하는 데 있습니다.
