...
다음처럼 규칙은 분명히 필요한데도 왜 현실 문제에서는 다른 접근이 함께 붙는지 다시 설명해야 할 때 이 절의 관점을 떠올리면 됩니다.

- 정책과 절차는 명확한데 입력이 흔들리거나 정보가 빠져 규칙만으로 닫히지 않을 때
- 예외가 계속 늘어나며 규칙 우선순위와 충돌 관리가 복잡해질 때
- 왜 확률, 탐색, 휴리스틱, 학습이 규칙 다음에 이어서 등장하는지 연결해서 설명해야 할 때

이때는 먼저 `정보 부족`, `부분 관측`, `잡음`, `예외`, `후보 폭증` 가운데 무엇이 현재 문제의 핵심 조건인지 나누면 됩니다. 그러면 왜 규칙만으로는 부족하고 어떤 보완 접근이 필요한지 더 선명하게 읽을 수 있습니다.
