...
后面读到启发式、搜索、机器学习评估、LLM 生成设置时，这个视角还会继续回来。关键是一直看清分工：模型先产生数字，系统再解释这些数字，必要时人还会承担最后判断。
...
当你开始把模型给出的概率式数字，直接读成答案本身或最终行动时，就应该先想起这一节。

- 当你看到分类分数 `0.68`，就想把它自动当成可信概率时
- 当你需要说明阈值到底只是技术设置，还是同时牵涉服务成本与责任时
- 当你需要把生成设置、概率输出、人工复核和最终决策分开整理时

这时先重新拆成：`概率数字是在比较候选`、`阈值是在分叉行为`、`校准是在检查分数解释`、`决策属于服务层判断`。
