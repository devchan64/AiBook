...
Part 1에서 `개인 학습(personal learning)`, `문서화(documentation)`, `근거 확인(evidence review)`, `작업 가설(working hypothesis)`의 기본 구분은 여기서 잡습니다. 15장에서는 AI의 사회적 위험, 저작권, 보안을 봤고, 여기서는 그 기준을 적용해 `개인이 AI를 어떻게 안전하게 학습 도구로 사용할 것인가`를 정리합니다. 업무 적용은 16.2에서, 프로젝트 검증은 16.3에서 이어집니다.
...
- AI를 개인 학습의 답안지처럼 쓰고 구조화 도구로 보지 않을 때
- 문서화를 단순 기록으로만 여기고 검증 장치로 보지 않을 때
- 개인적 직관과 검증된 설명을 한 문단 안에서 섞어 쓰려 할 때

이때는 먼저 `질문 구조화`, `문서화`, `근거 확인`, `작업 가설 보존`을 나누면 됩니다. 그러면 AI를 만능 답안지가 아니라 학습 흐름을 정리하는 도구로 다시 볼 수 있습니다.
