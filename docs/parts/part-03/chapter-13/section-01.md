...
커널(kernel) 발상과 비선형 경계의 큰 그림은 P3-13.2에서 바로 이어서 다루고, `C`, `gamma` 같은 하이퍼파라미터를 읽는 기준과 검증 비용은 P3-9.1, P3-9.2에서 다시 연결합니다. 최적화 목적 함수, 라그랑주 승수, 듀얼 문제의 엄밀한 전개는 이 책의 현재 본편 범위 밖에 둡니다.

현재 Part 3에서는 이 수학 전개를 별도 `보충학습`으로 다시 만들지 않습니다. 이 책에서 SVM의 학습 목표는 margin, support vector, soft margin, kernel 직관까지로 닫히며, 라그랑주 승수나 듀얼 최적화의 엄밀한 전개는 본편과 보충학습을 오가며 회수하기보다 더 심화된 수학 자료에서 따로 보는 편이 범위상 더 적절합니다.
...
| --- | --- | --- |
| margin과 support vector | 여러 경계 중 무엇이 더 여유 있고 안정적인가 | P3-13.2 kernel과 비선형 경계 |
| soft margin과 오류 허용 | 완벽한 분리보다 어떤 균형을 택해야 하는가 | P3-9 하이퍼파라미터, `C` 해석 |
