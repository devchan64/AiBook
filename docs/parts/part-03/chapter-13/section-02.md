...
`gamma`, `degree`, `coef0` 같은 설정값을 읽는 기준과 검증 비용은 P3-9.1, P3-9.2에서 다시 연결합니다. 양의 정부호 조건, RKHS, 듀얼 최적화의 수학 전개는 이 책의 현재 본편 범위 밖에 두고, 필요하면 더 심화된 수학 문서에서 따로 보는 편이 맞습니다.
...
이 도식은 원래 좌표 공간에서 직선 경계가 답답하게 느껴지는 장면을 보여 줍니다. 핵심은 데이터가 `어렵다`기보다, 지금 쓰는 표현 방식이 선형 분리에 불리할 수 있다는 점입니다.
...
- P3-11의 로지스틱 회귀: 선형 점수와 결정 경계를 배운다.
- P3-13.1의 SVM: 그 선형 경계 중 더 안정적인 경계를 고른다.
- P3-13.2의 커널: 선형 경계가 작동하는 공간 자체를 다시 설계한다.
