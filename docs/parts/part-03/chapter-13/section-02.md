...
초심자에게 `kernel`이라는 이름은 추상적으로 들리기 쉽습니다. 입문 단계에서는 수학적 엄밀성보다, 이 이름이 실제로 무엇을 가리키는지 먼저 잡는 편이 좋습니다.
...
즉, 커널은 `비선형 문제를 직접 붙잡는 기술`이기보다, `선형적으로 읽히는 표현을 다시 확보하는 기술`로 소개하는 편이 초심자에게 더 안전합니다.
...
여기서 초심자가 자주 하는 오해도 함께 정리할 필요가 있습니다.
...
이 순서를 지키는 이유는, 커널이 강력한 만큼 `무엇이 실제로 좋아졌는가`를 흐리게 만들 수 있기 때문입니다. 초심자에게 중요한 것은 커널을 빨리 쓰는 것이 아니라, `왜 선형이 부족했고 왜 커널이 그 부족함을 메웠는가`를 말할 수 있게 되는 것입니다.
