...
如果测量单位、传感器版本、计算规则发生了变化，那么同一个列名也可能已经不是同一个特征，所以在 Part 3 里，应该先检查特征定义是否一致，再去看数字。这一节与其说是在讲列名管理技巧，不如说更接近于：应该怎样识别 [特征定义身份(feature-definition identity)](/AiBook/zh/reference/concept-glossary-pinyin/t/#glossary-feature-definition-identity)。
