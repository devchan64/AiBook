...
같은 문제는 모델 입력에서도 드러납니다. 아래 예제는 같은 k-NN(k-nearest neighbors) 모델을 쓰되, 스케일 조정 없이 읽을 때와 `StandardScaler`로 각 열을 같은 비교 눈금으로 맞춘 뒤 읽을 때 최근접 이웃이 어떻게 달라지는지 보여 줍니다.
