...
“[汇总表(summary table)](/AiBook/zh/reference/concept-glossary-pinyin/d/#data-modeling)里有一列”这件事，和“它就是[特征(feature)](/AiBook/zh/reference/concept-glossary-pinyin/f/#glossary-feature)”这个判断，并不是同一句话。汇总表里当然会有用来描述样本结构的特征，但也可能同时包含用于比较的列、结果候选列，以及用于识别和保留上下文的列。所以，这一节最先要抓住的区分是：`汇总表中的列` 和 `应该被当成模型输入来读取的特征`，并不会自动重合。
...
列的作用并不会由名称永久决定。从记录时间得到的时段，或预测前已确定的运行模式，都可以成为候选输入特征。反过来，如果在动作开始前预测，即使是 `mid_flow_mean` 也还是尚不存在的值。下表的`是`表示在该动作测量结束后使用时，它可以作为候选输入。选择输入不仅要看目的，也要确认实际可用的时点。
...
这也解释了为什么有些地方写着`视情况而定`。基准线本身及其差值原本是为了比较说明而构造的列，因此是否把它们直接作为输入特征，还要根据之后定义的预测问题重新判断。
...
把 `视情况而定` 变成一次具体选择。假设在动作结束后立即预测未来七天内是否发生故障，并且此时已知当前动作均值和仅用历史动作计算的基准。那么 `delta_from_baseline = mid_flow_mean − baseline_mid_flow_mean` 可以成为输入候选。反之，如果基准包含当前动作之后的测量值，同名差值列也不能作为输入。

请分别标记每列是否 `用于比较`、`是输入候选`、`在预测时可获得`。前一种差值列可以同时满足三项。列角色并不是互斥的标签：不能仅因它用于比较就排除它，也不能仅因它是数字就纳入输入。

## 检查清单

- 你是否区分了标识、上下文、输入和结果列？
- 你是否选了一个上下文列，确认它在预测时是否实际可用？
