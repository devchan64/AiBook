...
특징(feature)을 설계하다 보면 초심자가 놓치기 쉬운 함정이 하나 더 있습니다. `열 이름이 같으니 같은 특징이겠지`라고 생각하는 순간입니다. 하지만 현실 데이터에서는 같은 `flow_mean`이라는 이름 아래에서도 센서 버전이 바뀌었거나, 단위가 바뀌었거나, 계산 규칙이 달라졌을 수 있습니다. 이런 변화가 있으면 숫자는 있어도 더 이상 같은 특징이라고 보기 어렵습니다.

핵심은 단순합니다. 특징은 열 이름이 아니라, `무엇을 어떤 규칙과 단위로 측정한 값인가`까지 포함해 같은 특징인지 판단해야 합니다.

## 왜 이 질문이 Part 3에 필요한가

이 문제를 너무 늦게 보면 초심자는 `학습 성능이 이상하다`, `기준선이 흔들린다` 같은 뒤 증상만 보게 됩니다. 하지만 실제로는 특징 의미가 이미 앞단에서 달라졌을 수 있습니다.
...
Part 4에서는 분할, 평가, 일반화를 읽습니다. 그런데 같은 열 이름 아래 뜻이 섞여 있으면, 그 뒤 절차도 오해되기 쉽습니다.

| Part 4에서 뒤에 보게 될 것 | Part 3에서 먼저 막아야 하는 문제 |
| --- | --- |
| 분할(split) | 서로 다른 특징 정의가 학습/평가에 섞일 수 있다 |
| baseline | 다수 클래스나 기준 모델보다 먼저 특징 뜻이 바뀌었을 수 있다 |
| generalization | 실제 일반화 실패가 아니라 정의 불일치일 수 있다 |

즉 Part 4의 성능 문제처럼 보이는 것 중 일부는, Part 3에서 특징 정의를 섞은 결과일 수 있습니다.

## Part 5와는 어떻게 이어지는가

시계열과 표현 학습 쪽에서도 이 문제는 그대로 남습니다.

| Part 5에서 남는 질문 | 왜 Part 3에서 먼저 봐야 하는가 |
| --- | --- |
| 시계열 패턴이 달라졌다 | 실제 패턴 변화가 아니라 센서 정의 변화일 수 있기 때문 |
| 모델이 특정 시기만 다르게 읽는다 | 입력 의미가 시기별로 달라졌을 수 있기 때문 |
| 표현 학습이 자동으로 해결해 주는가 | 입력 뜻이 바뀐 문제는 자동으로 사라지지 않기 때문 |

즉 Part 5의 더 복잡한 입력 표현도, 앞단의 특징 정의 일치가 깨지면 그대로 흔들립니다.
