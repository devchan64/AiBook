...
이 메모를 남겨 두어야 `모델이 이상하다`고 보기 전에 먼저 `같은 특징 정의가 섞였는가`를 점검할 수 있습니다. 핵심은 현재 특징 표가 같은 뜻의 열끼리만 비교 가능한 구조인지 확인하는 데 있습니다.
...
여기서 마지막으로 확인할 것은 세 가지입니다. 단위와 계산 규칙이 메모되어 있는지, 버전 변경이나 센서 변경을 구분했는지, 기준선과 분할에 섞이면 안 되는 정의 차이를 표시했는지입니다. 이 세 조건이 함께 서야 특징 표는 단순 숫자 모음이 아니라, 비교 가능한 정의가 붙은 구조로 남습니다.
