...
자동으로 실행되는 동작 1회를 샘플로 본다고 하겠습니다. 같은 길이와 순서로 맞춘 시점별 센서값 전체를 벡터나 시퀀스로 남길 수도 있습니다. 여기서는 사람이 동작을 비교하기 쉽도록 중요한 면을 드러내는 요약값을 선택합니다. 예를 들어 다음 값들은 자주 좋은 출발점이 됩니다.
...
`segment_variability`는 구간 평균들 사이의 차이입니다. 한 구간 안에서 센서가 얼마나 흔들렸는지는 원시 시점값으로 별도 계산해야 합니다. 매 구간 평균이 모두 2여도, 각 구간의 값이 2, 2인 동작과 0, 4인 동작은 구간 안 변동성이 다릅니다. 따라서 구간 평균의 표준편차가 0이라는 이유로 동작이 흔들리지 않았다고 결론 내릴 수 없습니다.
...
- 평균이 가린 차이를 드러내는 특징을 하나 계산했는가?
- 구간 평균 사이의 변동과 구간 안 측정값의 변동을 구분했는가?
