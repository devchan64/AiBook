...
이 절은 평가 지표의 역할을 설명하는 도입 절입니다. 여기서는 정확도(accuracy), 정밀도(precision), 재현율(recall), F1 점수(F1 score)를 처음 만나는 수준으로 연결합니다. ROC-AUC, PR-AUC, 로그 손실(log loss), 캘리브레이션(calibration), 실루엣(silhouette) 같은 지표는 P3-6.4 보충학습에서 따로 회수하고, 회귀 지표와 군집화 관점은 P3-6.2에서 이어서 봅니다.

즉, 본편의 책임은 `무엇을 먼저 중요하게 볼 것인가`를 정하는 데 있습니다. 확률 점수의 세밀한 해석, reliability diagram, Brier score, threshold 세부 조정처럼 한 단계 더 섬세한 읽기는 P3-6.4와 뒤의 P3-15.3으로 넘기고, 여기서는 지표가 왜 하나가 아닌지부터 분명히 잡습니다.

또한 여기서 지표를 읽는 기본 태도도 함께 고정합니다. 분류(classification)에서는 먼저 혼동 행렬(confusion matrix)과 대표 오류 사례를 보고, 뒤의 P3-8.2에서는 baseline과 비교해 그 오류 구조 개선이 실제로 의미 있는지 다시 읽습니다. 즉, Part 3의 평가는 `숫자 하나`보다 `어디서 틀렸는가`와 `무엇과 비교해 나아졌는가`를 먼저 보는 흐름으로 이어집니다.
...
scikit-learn 문서는 평가 지표(metrics and scoring)를 `예측의 품질을 수치화하는 도구`로 다룹니다. 동시에 어떤 지표를 쓸지는 결국 `예측을 무엇에 쓰는가`와 연결된다고 설명합니다. 여기서는 다음처럼 정리하면 충분합니다.
...
Google 용어집은 F1 점수를 정밀도와 재현율을 함께 쓰는 대표적인 요약 지표로 설명합니다. 여기서는 다음 한 문장으로 이해하면 충분합니다.
...
처음 읽을 때는 지표 이름을 많이 외우기보다, 분류 평가를 다음 순서로 읽는 편이 더 안전합니다.
...
| 4 | baseline과 비교 | 뒤의 P3-8.2에서 이 점수 변화가 정말 의미 있는 개선인지 다시 확인해야 하기 때문입니다. |
