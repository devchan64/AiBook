...
기준선이라는 말을 들으면 초심자는 종종 모델 평가나 성능 비교를 먼저 떠올립니다. 실제로 머신러닝에서는 `baseline model`이라는 말도 자주 쓰기 때문입니다. 하지만 이 Part에서 말하는 기준선은 그보다 앞선 단계에 있습니다. 여기서 기준선은 모델 성능 비교 이전에, `지금 상태를 무엇과 비교할 것인가`를 정하는 데이터 표현 구조의 일부입니다.
