...
특징을 설계한다는 말은 요약 표 안의 숫자를 그대로 쓰는 일이 아니라, 비교하고 싶은 구조를 숫자 표현으로 다시 고르는 일입니다. 그래서 먼저 어떤 구조를 남길지 정한 뒤에야 평균, 기울기, 변동성 같은 특징 후보가 의미를 가집니다. 여기서 한 번 더 갈라지는 판단이 있습니다. 같은 구조를 평균, 차이, 기울기, 토큰, 비율처럼 다른 표현으로 바꾸는 일은 변수변환(variable transformation)이고, 그렇게 바꾼 표현 중 실제로 남길 항목을 고르는 일은 특징 선택(feature selection)입니다.
...
이 표는 앞에서 말한 두 갈래를 다시 확인하게 합니다. 평균, 차이, 기울기, 변동성으로 바꾸는 단계가 변수변환이고, 그 가운데 어떤 값을 현재 질문에 남길지 고르는 단계가 특징 선택입니다. 숫자 특징은 구조를 요약하는 첫 단계이고, 토큰화된 표현은 그 구조를 좀 더 사람이 읽기 쉬운 중간 표현으로 바꾸는 다음 단계입니다.
