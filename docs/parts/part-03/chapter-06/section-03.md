...
특징(feature)과 [중간 표현(intermediate representation)](../../../reference/concept-glossary.md#glossary-intermediate-representation)을 함께 두면, 곧 이런 질문이 나옵니다. `지금 사람이 입력 구조를 이렇게 정하는 일과, 모델이 나중에 내부 패턴을 배우는 일은 무엇이 다른가?` 이 차이가 흐려지면 Part 3의 특징 설계가 낡은 전처리처럼 보이거나, 반대로 모델이 문제 구조를 대신 정해 줄 것처럼 오해하기 쉽습니다.
