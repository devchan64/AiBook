...
평균, 기울기, 변동성 같은 특징은 좋은 출발점이 됩니다. 하지만 어떤 경우에는 숫자 몇 개만으로는 구간별 구조를 충분히 설명하기 어렵습니다. 예를 들어 초반에는 천천히 오르고, 중반에는 평평하게 유지되다가, 후반에는 빠르게 떨어지는 패턴이 있다고 하겠습니다. 이런 구조를 숫자 두세 개로만 남기면 사람이 다시 읽을 때도 아쉽고, 모델이 비교할 때도 중요한 모양 차이를 놓칠 수 있습니다. 그래서 Part 3에서는 숫자 특징만으로 부족할 때 `중간 표현`을 하나 더 둘 수 있다는 점을 함께 봅니다.
...
다음 질문에 답할 수 있으면, 이 절의 중간 표현 사용 기준은 잡힌 것입니다.

- 숫자 특징만으로 부족하다고 느끼는 장면은 언제인가
- `UP, FLAT, DOWN` 같은 표현이 사람에게 무엇을 더 빨리 보여 주는가
- 토큰화를 쓰면 무엇이 더 잘 보이고 무엇이 사라지는가
- 숫자 특징과 토큰화 표현을 왜 동시에 가질 수도 있는가

이 절에서 기억해야 할 핵심은 다음과 같습니다. `세그먼트 표현과 토큰화는 원시 곡선을 사람이 비교할 수 있는 중간 표현으로 바꾸는 방법이지만, 정보를 완전히 보존하지는 않는다.` 다음 장에서는 이렇게 남긴 특징과 중간 표현을 기준선과 연결해, `무엇이 달라졌는가`를 상태 수준에서 읽는 법으로 넘어갑니다.

## 언제 이 관점을 먼저 떠올려야 하는가

- 숫자 특징 몇 개만으로는 구조 차이가 잘 드러나지 않을 때 세그먼트 표현과 토큰화된 중간 표현을 먼저 떠올립니다.
- `UP, FLAT, DOWN` 같은 기호열이 무엇을 더 잘 보이게 하고 무엇을 잃게 하는지 설명해야 할 때 이 절로 돌아옵니다.
- 숫자 특징과 중간 표현을 경쟁 관계가 아니라 서로 다른 목적의 도구로 써야 할 때 이 절이 기준이 됩니다.
