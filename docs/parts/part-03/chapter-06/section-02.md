...
이 절은 분류, 회귀, 군집화에서 평가 기준이 왜 달라지는지 독자 기준으로 정리하는 도입 절입니다. 여기서는 문제 유형마다 먼저 떠올려야 할 질문과 대표 지표를 연결합니다. 아직 ROC 곡선(ROC curve), PR 곡선(PR curve), 로그 손실(log loss), 캘리브레이션(calibration), 실루엣 계수(silhouette coefficient)의 세부 계산은 깊게 다루지 않습니다.
