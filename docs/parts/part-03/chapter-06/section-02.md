...
| 상위 프레임 | 이 절에서의 대응 |
| --- | --- |
| 원시 구조 보존 | 원시 곡선을 그대로 두어 가장 많은 정보를 유지 |
| 중간 표현 설계 | 세그먼트 표현, `UP`/`FLAT`/`DOWN` 같은 기호열 |
| 요약 특징 설계 | 평균, 기울기, 변동성처럼 비교용 숫자 특징 |

이 프레임을 잡아 두면 토큰화는 독립 기법처럼 보이기보다, 원시 로그를 그대로 둘지 너무 강하게 요약할지 사이에서 `구조를 어느 해상도로 남길지` 정하는 선택으로 읽힙니다.
