...
| --- | --- | --- |
| 분류 점수는 알겠는데 threshold를 바꾸면 뭐가 달라지는가? | ROC, PR | P3-6.2, P3-15.3 |
| 정확도는 괜찮은데 확률 점수를 그대로 믿어도 되는가? | log loss, calibration | P3-6.2, P3-15.3 |
| 군집을 만들었는데 정답 라벨 없이도 구조를 읽을 수 있는가? | silhouette | P3-17.1, P3-17.2 |
...
처음에는 정확도와 재현율만으로 모델을 비교했지만, 운영 단계에서는 더 복잡한 문제가 생깁니다. 어떤 모델은 양성과 음성을 잘 가르는 것처럼 보여도 threshold를 조금만 바꾸면 오탐이 급격히 늘고, 어떤 모델은 확률 점수 `0.8`을 많이 내놓지만 실제로는 그 정도로 위험하지 않을 수 있습니다. 또 고객 유형을 군집으로 나눠 보면 위험 거래가 특정 묶음에 모이는지 따로 보고 싶어집니다.
