...
열 역할은 이름만으로 영구히 결정되지 않습니다. 기록 시각에서 얻은 시간대나 예측 전에 정해진 운전 모드는 입력 특징 후보가 될 수 있습니다. 반대로 `mid_flow_mean`도 동작 시작 전에 예측한다면 아직 없는 값입니다. 아래 표의 `예`는 해당 동작의 측정이 끝난 뒤 사용할 때의 후보라는 뜻입니다. 입력 채택에는 목적뿐 아니라 실제 사용 가능 시점도 확인해야 합니다.
...
표의 `경우에 따라 다름`을 실제 선택으로 바꿔 보겠습니다. 동작 종료 직후 다음 7일의 고장을 예측하고, 현재 동작의 평균과 과거 동작만으로 만든 기준선을 그 시점에 모두 알 수 있다고 가정합니다. 이때 `delta_from_baseline = mid_flow_mean − baseline_mid_flow_mean`은 입력 후보가 될 수 있습니다. 반대로 현재 동작 이후의 측정까지 넣어 기준선을 계산했다면 같은 이름의 차이 열도 입력으로 사용할 수 없습니다.

따라서 같은 열에 `비교에 사용`, `입력 후보`, `예측 시점에 사용 가능`을 각각 표시해 보세요. 앞의 차이 열은 세 항목이 모두 해당할 수 있습니다. 열 역할은 서로 배타적인 이름표가 아니며, 비교에 쓴 열이라는 이유만으로 입력에서 제외하거나 숫자라는 이유만으로 입력에 넣지 않습니다.

## 체크리스트

- 식별·문맥·입력·결과 열을 구분했는가?
- 문맥 열 하나를 골라 예측 시점에 실제로 알 수 있는지 확인했는가?
