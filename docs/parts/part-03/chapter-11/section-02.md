...
초평면(hyperplane) 직관은 P4-1.2에서 다시 연결하고, kernel 방법과 비선형 경계는 P3-13.1, P3-13.2에서 다시 다룹니다. `C`, `gamma`, threshold 조정 같은 설정과 계산 비용은 P3-9.1, P3-9.2에서 다시 볼 수 있습니다. 다중 클래스 경계 분할과 plot 구현 세부는 이 책의 현재 본편 범위 밖에 둡니다.
...
| --- | --- | --- |
| threshold 위치 | 어디서 class가 갈리고, 경계가 얼마나 보수적인가 | P3-6 분류 지표, P3-15.3 threshold 조정 |
| confusion matrix의 문제 칸 | 이 경계가 FP와 FN 중 어떤 실수를 더 만들고 있는가 | P3-6 평가 지표 |
| 경계 근처 대표 사례와 baseline 비교 | 애매한 입력이 왜 반대 class로 넘어가고, 단순 기준보다 실제로 나은가 | P3-8 baseline, 뒤 분류 알고리즘 비교 |
