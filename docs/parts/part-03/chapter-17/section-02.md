...
이 절은 입문적으로 `군집 결과를 과신하지 않는 태도`를 만드는 데 집중합니다. 군집 평가 지표는 P3-6.4에서 다시 읽고, 시각화와 임베딩 공간 왜곡은 P3-18.2에서 다시 연결합니다. 반지도학습과의 연결은 이 책의 현재 본편 범위 밖에 둡니다.
...
이 도식은 군집 결과를 업무 판단으로 연결할 때 필요한 안전한 중간 단계를 보여 줍니다. 군집이 나왔다고 바로 정책에 쓰는 것이 아니라, 각 그룹을 설명하고 도메인 데이터와 대조하고 후속 결과로 검증한 뒤에야 실제 활용을 검토해야 합니다.
