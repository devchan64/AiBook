...
Part 3이 끝나면 남는 것은 `아무 표나`가 아닙니다. 샘플 단위가 고정되어 있고, 입력으로 줄 특징과 나중에 맞히고 싶은 결과 후보가 구분되어 있으며, 아직 비교 리포트로 남겨 둘 범위도 함께 정리된 구조입니다. 한 줄로 줄이면 `원천데이터 -> 비교 가능한 표 -> 보수적 해석 -> 문제 구조 마감`의 연쇄가 남아 있어야 합니다.
