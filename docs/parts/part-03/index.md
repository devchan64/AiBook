...
| 원시 로그 재구성 | 수치형 값과 범주형 상태를 비교 가능한 표로 어떻게 다시 묶는가 | 요약 표, 비교 표, 누락/샘플 붕괴 구분, 수치형/범주형 탐색의 출발점 |
| 특징 설계 | 어떤 변수변환과 특징 선택으로 비교와 학습이 가능한가 | 특징 열, 중간 표현, 열 역할 구분, 단위와 구조 역할 구분, 특징 정의 일치 점검 |
