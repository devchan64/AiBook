...
이렇게 Part 3을 길게 두는 이유는 Part 4와 Part 5가 모두 데이터 과학의 다음 층위를 다루기 때문입니다. Part 4는 이렇게 정리된 구조 위에서 입력 `X`, 목표 `y`, 분할(split), 평가(evaluation), 일반화(generalization)를 읽고, Part 5는 같은 구조를 바탕으로 더 긴 시계열 입력, 표현 학습, 신경망 구조를 읽습니다. 따라서 Part 3은 독립된 준비 운동이 아니라, 뒤 Part의 데이터 과학 설명을 이해하기 위해 먼저 닫아 두어야 할 전제를 맡습니다.
...
Part 3에서는 먼저 데이터 모델링이 무엇을 달성하려는지와 어떤 순서로 진행되는지부터 고정합니다. 그 다음 저장된 기록을 왜 곧바로 데이터셋처럼 읽으면 안 되는지 확인하고, 한 행과 한 샘플의 뜻을 정한 뒤, 원시 로그를 비교 가능한 표로 다시 묶습니다. 이어서 특징과 중간 표현을 설계하고, 어떤 열이 식별용인지, 어떤 열이 비교용인지, 어떤 열이 목표 후보인지 같은 열 역할도 분리합니다. 이때 특징은 숫자 크기만으로 읽지 않고, 단위와 구조 역할이 다른 열을 구분해 함께 읽는 기초도 잡습니다. 그 다음 최근 구간과 기준선을 비교하는 구조를 세우고, 적은 표본과 흔들리는 반복성 앞에서 어디까지 해석할 수 있는지 경계를 둡니다. 마지막 Chapter에서는 여러 비교 열을 변화 크기, 반복성, 해석 신뢰도, 운영 중요도 같은 축으로 다시 묶어 검토 우선순위 후보를 만들고, 비교 리포트, 검토 후보 큐, 목표 라벨 후보 표가 달라도 같은 샘플 식별 기준과 최소 근거를 유지하며 사건을 계속 추적합니다. 또한 뒤 Part로 넘어가기 전에 `입력과 결과가 구분되었는지`, `표 입력과 시계열 입력이 어디서 갈라지는지`, `미래 정보와 운영 시점 재현 가능성이 닫혔는지`, `cutoff와 horizon이 고정되었는지`를 Chapter 9 안에서 함께 닫습니다.
...
Part 3에서 반복해서 다루는 질문 축도 미리 적어 둘 필요가 있습니다. 저장된 기록을 어떤 샘플로 다시 볼 것인가, 지금 모은 샘플이 전체 운영 범위를 얼마나 대표하는가, 같은 평균인데 패턴이 다른가, 최근 구간이 평소와 달라졌는가, 어떤 열이 feature 후보이고 어떤 열이 목표 후보인가, 숫자 크기가 아니라 단위와 역할은 어떻게 읽어야 하는가, 값이 빠지거나 구간이 비었을 때 이 샘플은 아직 같은 비교 단위인가, 같은 열 이름이라도 단위·버전·계산 규칙이 같아 정말 같은 특징인가, 반복성이 줄어들고 있는가, 사람이 먼저 검토해야 할 건은 무엇인가, 여러 비교 열을 어떤 우선순위 후보 축으로 묶을 것인가, 서로 다른 산출물 사이에서 같은 사건을 무엇으로 추적할 것인가, 목표 라벨 후보가 같은 사건과 비슷한 조건에서 비슷하게 붙는가, 목표 라벨은 보통 언제 정답으로 확정되는가, 0 라벨을 붙일 만큼 충분히 끝까지 관측했는가, 여러 후속 사건이 있을 때 어떤 하나를 target으로 접는가, 여러 target 후보가 있으면 무엇을 먼저 대표 목표로 세우는가, 겹치는 입력 창이 많을 때 창 수가 실제 사건 수보다 얼마나 부풀어 보이는가, 같은 target 이름을 계속 써도 판정 기준과 상태 체계가 정말 같은가, 지금 쓰는 target이 실제 목표 자체인가 아니면 대신 쓰는 대리 target인가, 확정 라벨이 어떤 사례에만 붙었는가, 현재의 규칙과 조치가 이후 데이터 자체를 바꿨는가, 같은 target 아래에서 어떤 오류가 더 아픈가, 모델 점수를 순위용으로 쓰는가 확률처럼 읽는가, binary target이 실제로는 몇 갈래 행동으로 연결되는가, 범주가 여러 개 보일 때 정답이 정확히 하나인지 여러 라벨이 동시에 가능한가, 숫자 target이 보일 때 원래는 연속값을 맞히는 문제인지 숫자를 잘라 만든 class인지, 실제 목표가 상위 몇 건 선별이나 목록 순서 최적화인지, 보수적 해석 문장을 어느 수준까지 운영 출력으로 바꿀 수 있는가, 목표 라벨 후보가 너무 드물거나 특정 시기·조건에 몰려 있지는 않은가, 이 문제가 무작위 분할보다 시간 순서 분할을 먼저 요구하는가, 같은 개체를 train/test 양쪽에 두지 말아야 하는가 같은 질문입니다. Part 3의 각 Chapter는 결국 이 질문들을 더 정확하게 말하기 위한 구조 하나를 맡습니다.

## 짧은 점검

- 왜 Part 3은 `모델을 고르는 순서`가 아니라 `문제를 표현하는 순서`로 시작하는지 설명할 수 있는가
- 현재 9개 Chapter 흐름에서 `원시 로그 재구성`, `문제 유형 구분`, `뒤 Part 연결`이 서로 어떻게 이어지는지 말할 수 있는가
- Part 3의 끝에서 무엇을 비교 리포트로 남기고, 무엇을 Part 4의 표 입력으로 넘기고, 무엇을 Part 5의 시계열 입력으로 넘기는지 큰 흐름으로 설명할 수 있는가
...
Part 3이 끝나면 다음 질문을 자연스럽게 Part 4로 넘길 수 있습니다.
...
즉 Part 4는 단순히 모델 이름을 배우는 Part가 아니라, Part 3에서 만든 구조 위에 `학습 가능한 데이터 과학 절차`를 얹는 Part로 읽어야 합니다. 입력과 목표를 나누고, 학습/검증/평가 분할을 하고, 점수와 일반화를 해석하는 일은 모두 이미 정리된 샘플 구조가 있을 때만 뜻을 가집니다.

같은 흐름은 Part 5에도 이어집니다. 어떤 센서 구간을 한 입력 시퀀스로 묶을지, 요약 벡터를 먼저 쓸지 원시 시계열 구간을 직접 쓸지, 비교 구조를 입력에 어떻게 반영할지 같은 질문은 신경망 구조를 고르기 전에 먼저 정리되어 있어야 합니다. Part 5의 RNN, CNN, Attention, Transformer 설명은 바로 이 입력 구조 전제 위에서 읽힙니다.

따라서 Part 5도 `딥러닝 모델 파트`로만 읽기보다, Part 3에서 잡은 샘플 경계와 입력 구조를 더 긴 시퀀스와 표현 학습으로 확장하는 데이터 과학의 다음 단계로 읽는 편이 정확합니다.
