...
一听到“[基准线(baseline)](/AiBook/zh/reference/concept-glossary-pinyin/b/#glossary-baseline)”这个词，人们往往会先想到模型评估或性能比较。确实，在机器学习里也常常会听到 [基准模型(baseline model)](/AiBook/zh/reference/concept-glossary-pinyin/b/#glossary-baseline-model) 这个说法。但这一 Part 里说的基准线，处在更前面的阶段。这里的基准线，是在模型性能比较之前，决定 `当前状态应该和什么相比` 的数据表示结构的一部分。如果前一章已经通过特征和中间表示把结构留下来了，那么现在就要决定：应该把这些结构拿去和什么比较，变化才会显现。
...
请自行核算表中的 type-A。若差值定义为 `近期 − 基准`，则 `2.10 − 2.45 = −0.35`。负号表示近期值更低。反过来相减会得到 +0.35，因此除了列名，还要记录计算方向。

把同一个近期值 2.10 与[基准 2.00](/AiBook/zh/reference/concept-glossary-pinyin/b/#baseline) 比较，差值为 +0.10。原表显示下降，新比较却显示上升。在决定采用哪一个比较之前，应先确认两个基准各自代表的时段和条件。不能为了得到想要的正负号而选择基准。若同时报告两种比较，应分别标明时段和条件，让不同的比较含义可见。

## 检查清单

- 你是否检查了近期值与基准值的条件、期间和单位是否一致？
- 你是否用原来的两个值核对了差值的正负号？
