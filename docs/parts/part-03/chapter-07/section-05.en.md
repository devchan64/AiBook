...
If the baseline-maintenance method is chosen poorly, the meaning of both the [comparison report](/AiBook/en/reference/concept-glossary-alpha/o/#output-structure) and the current comparison sentence shakes together. Even for the same change, if the reference itself changes, the weight of the current judgment such as `needs review`, `caution`, or `normal/abnormal candidate` also changes. If this section is reread not as a matter of taste between fixed baselines and recent-usual baselines, but as the problem of [reference maintenance strategy](/AiBook/en/reference/concept-glossary-alpha/b/#baseline), then it becomes clearer that baseline maintenance is not a contest with one right answer. It is a choice of what reference-maintenance method fits the comparison question.
