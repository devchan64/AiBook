...
기준선 운영 방식이 잘못 잡히면 Part 3의 비교 리포트만 흔들리는 것이 아닙니다. `검토 필요`, `주의`, `정상/이상 후보` 같은 후속 라벨 후보도 쉽게 흔들리고, 뒤 Part로 넘길 목표 후보와 시계열 입력 해석도 함께 약해집니다.

## 일반화된 상위 프레임으로 다시 보면

이 절은 고정 기준선과 최근 평소 기준선의 취향 차이가 아니라, `참조 기준을 유지할 것인가 이동시킬 것인가(reference maintenance strategy)`의 문제로 다시 볼 수 있습니다.

| 상위 프레임 | 이 절에서의 대응 |
| --- | --- |
| 고정 참조 | 특정 기준 시점 대비 얼마나 달라졌는지 읽음 |
| 이동 참조 | 최근 평소 흐름 안에서 지금만 달라졌는지 읽음 |
| 질문 적합성 | 어떤 변화를 보고 싶은지가 유지 방식 선택을 좌우 |

이 프레임을 쓰면 기준선 운영 방식은 정답 경쟁이 아니라, 비교 질문에 맞는 참조 유지 방식을 고르는 선택으로 정리됩니다.
