...
기준선 선택은 가장 간단하게 보면 다음 세 단계로 요약할 수 있습니다.

1. 먼저 `무엇을 무엇과 비교하려는가`를 문장으로 정한다.
2. 그 질문에 맞는 `비교 가능한 기준 구간`만 남긴다.
3. 그 후보들 가운데 지금 샘플과 가장 같은 조건의 묶음을 고른다.
...
이 절에서는 기준선 후보를 고르는 자리까지만 다룹니다. 기준선을 오래 유지할지, 최근 평소 구간에 맞춰 갱신할지는 다음 절에서 따로 봅니다.

## 일반화된 상위 프레임으로 다시 보면

이 절은 특정 현장 규칙 모음이 아니라, `비교 가능한 참조 집단을 어떻게 고를 것인가(selecting a comparable reference group)`의 문제로 다시 볼 수 있습니다.

| 상위 프레임 | 이 절에서의 대응 |
| --- | --- |
| 비교 질문 정하기 | 무엇을 무엇과 비교할지 먼저 문장으로 정함 |
| 조건 일치 확인 | 샘플 단위, 공정, 표본 수, 운영 상태를 맞춤 |
| 후보 선별 | 현재 샘플과 가장 같은 기준선 묶음을 남김 |

이 프레임을 쓰면 기준선 선택은 `과거 평균 하나 고르기`가 아니라, 현재 샘플과 비교 가능한 참조 집단을 조건별로 좁혀 가는 과정으로 읽힙니다.
