...
초심자는 전처리를 평균 채우기, 스케일러 적용, 인코더 선택처럼 개별 기술 이름으로 먼저 외우기 쉽습니다. 하지만 실무에서는 반대로 `무슨 입력 문제가 생겼는가`에서 출발하는 편이 더 자연스럽습니다. 앞에서 본 결측치, 스케일, 범주형 표현 문제도 결국 이 현장 질문의 다른 모습입니다.
