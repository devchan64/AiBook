...
다음은 같은 운전 조건의 과거 동작 200건을 기준선으로 둔 별도 가상 예시입니다. 유량 단위는 L/min이며, 각 동작의 후반 평균을 비교합니다. 하강 건수는 동작 안에서 정한 하강 규칙을 만족한 횟수입니다. 앞 절의 표와는 다른 관찰 묶음입니다.

| 유형 | 최근 건수 | 기준선 평균 | 최근 평균 | 기준선 표준편차 | 최근 표준편차 | 최근 하강 건수 |
| --- | --- | --- | --- | --- | --- | --- |
| type-A | 20 | 2.8 | 2.2 | 0.2 | 0.4 | 14 |
| type-B | 3 | 2.8 | 1.9 | 0.2 | 0.5 | 1 |

이 표를 문장으로 옮기면 다음과 같습니다.

- type-A 최근 20건의 후반 평균은 기준선보다 0.6 L/min 낮았다.
- type-A의 하강 규칙 충족은 14/20건이다. 반복 여부는 평균이나 표준편차가 아니라 이 건수와 발생 순서로 확인한다.
...
이제 두 행을 운영 문장으로 바꾸면 차이가 더 분명해집니다. `type-A`는 최근 20건 중 하강 규칙을 충족한 동작이 14건이므로 반복 변화 후보로 볼 수 있습니다. 평균 저하와 변동성 증가만으로 반복을 확정할 수는 없습니다. 반면 `type-B`는 차이값이 더 커 보여도 최근 3건뿐이므로, 같은 강도의 문장으로 말하기보다 `표본이 적어 추가 관찰 필요`처럼 한 단계 낮춘 표현이 더 적절합니다. 바로 이런 이유 때문에 비교표 읽기와 운영 문장 쓰기는 같은 절 안에서 함께 다룹니다. 차이값은 설명 후보를 좁혀 주지만, 비교표 하나만으로 원인까지 자동으로 말해 주지는 않습니다.
...
- type-A의 평균 차이와 하강 비율을 계산해 검토 문장으로 썼는가?
- type-B의 표본 수와 반복 근거를 A와 구분해 서술했는가?
