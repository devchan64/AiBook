...
[기준선(baseline)](../../../reference/concept-glossary-parts/01-giyeok.md#glossary-baseline) [비교표(comparison table)](../../../reference/concept-glossary-parts/05-mieum.md#glossary-output-structure)가 만들어지면 이제 숫자가 많이 보이기 시작합니다. 최근 평균, 기준선 평균, 차이값, 비율 차이, 최근 변동성, 기준선 변동성 같은 열이 한꺼번에 나타날 수 있습니다. 이때는 가장 눈에 띄는 차이값 하나만 보고 곧바로 결론을 내리곤 합니다. 하지만 비교표는 읽는 순서가 중요합니다. Part 3에서는 이 비교표를 자동 진단 결과표보다 `사람 검토 문장을 만들기 위한 표`로 읽어야 합니다.
