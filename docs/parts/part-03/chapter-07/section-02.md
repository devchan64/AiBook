...
이 읽기 순서를 더 짧게 줄이면 다음처럼 볼 수 있습니다.

| 먼저 볼 열 | 바로 확인하려는 것 |
| --- | --- |
| `recent_count` | 최근 구간 해석 강도를 얼마나 높일 수 있는가 |
| 기준선 열 | 무엇과 비교 중인가 |
| `diff` | 방향과 크기가 얼마나 다른가 |
| 변동성/패턴 열 | 평균 밖 구조 차이도 함께 있는가 |

이 표는 `어떤 숫자가 먼저 눈에 띄는가`보다 `어떤 맥락을 먼저 확인해야 과잉 해석이 줄어드는가`를 정해 줍니다.

## 짧은 점검

다음 질문에 답할 수 있으면, 이 절의 비교표 읽기 순서는 잡힌 것입니다.

- `diff`가 커 보여도 왜 `recent_count`를 먼저 봐야 하는가
- 기준선 정의를 보지 않고 차이값부터 읽으면 무엇을 놓치기 쉬운가
- 평균 차이와 변동성 차이를 함께 봐야 하는 이유는 무엇인가
- `자동 진단` 대신 `사람 검토 문장`으로 낮춰 쓰는 이유를 설명할 수 있는가
...
- 비교표에서 차이값 하나만 보고 곧바로 결론을 내리려 할 때 비교 맥락부터 읽는 순서를 먼저 떠올립니다.
- `recent_count`, 기준선 정의, 평균 차이, 변동성, 패턴 열을 어떤 순서로 봐야 과잉 해석이 줄어드는지 점검할 때 이 절로 돌아옵니다.
- 자동 진단이 아니라 사람 검토 문장으로 낮춰 써야 하는 이유를 다시 확인할 때 이 절이 기준이 됩니다.
