...
기준선을 세운다는 말은 현재 값을 하나 더 적어 두는 일이 아니라, 현재 구조가 어떤 평소 구조와 만나야 변화가 보이는지 정하는 일입니다. 같은 최근 값이라도 무엇을 기준선으로 두느냐에 따라 해석 문장이 달라지므로, 비교 구조 자체를 먼저 설계해야 합니다.
...
즉 지금의 기준선은 `모델을 하나 세워 두는 일`이 아니라, `최근 값을 비교할 평소 구조를 함께 준비하는 일`에 가깝습니다.

즉 기준선을 도입하는 순간, 데이터셋은 단순 기록표가 아니라 `비교가 가능한 표`로 바뀝니다. 이 점이 중요합니다. 여기서 중심은 기준선 용어를 다시 길게 정의하는 데 있지 않고, 지금 어떤 비교 구조를 설계해야 변화가 보이는지를 정하는 데 있습니다.
