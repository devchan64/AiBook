...
[기준선(baseline)](../../../reference/concept-glossary-parts/01-giyeok.md#glossary-baseline)이라는 말을 들으면 모델 평가나 성능 비교를 먼저 떠올리곤 합니다. 실제로 머신러닝에서는 [기준 모델(baseline model)](../../../reference/concept-glossary-parts/01-giyeok.md#glossary-baseline-model)이라는 말도 자주 쓰기 때문입니다. 하지만 이 Part에서 말하는 기준선은 그보다 앞선 단계에 있습니다. 여기서 기준선은 모델 성능 비교 이전에, `지금 상태를 무엇과 비교할 것인가`를 정하는 데이터 표현 구조의 일부입니다. 앞 장에서 특징과 중간 표현으로 구조를 남겼다면, 이제는 그 구조를 무엇과 비교해야 변화가 보이는지 정해야 합니다.
...
표의 type-A를 직접 검산해 보세요. 차이를 `최근 − 기준선`으로 정의하면 `2.10 − 2.45 = −0.35`입니다. 음수는 최근 값이 기준선보다 낮다는 뜻입니다. 순서를 반대로 빼면 +0.35가 되므로, 열 이름만 적지 말고 계산 방향도 함께 남겨야 합니다.

같은 최근 값 2.10을 [기준선 2.00](../../../reference/concept-glossary-parts/01-giyeok.md#baseline)과 비교하면 차이는 +0.10입니다. 원래 표에서는 낮아졌고 새 비교에서는 높아졌습니다. 어느 계산이 맞는지를 정하는 문제보다 먼저, 두 기준선이 어떤 기간과 조건을 나타내는지 확인해야 합니다. 원하는 부호가 나오도록 기준선을 고르면 안 됩니다. 두 기준을 함께 보고한다면 각각의 기간과 조건을 붙여 서로 다른 비교임을 드러냅니다.

## 체크리스트

- 최근 값과 기준선의 조건·기간·단위가 같은지 확인했는가?
- 차이값의 부호를 원래 두 값으로 검산했는가?
