...
통계 검정 기반 선택과 재귀적 특징 제거의 비교 관점은 P3-7.3 보충학습에서 다시 정리하고, 차원 축소의 큰 그림과 내부 계산은 P3-18.1, P3-18.2에서 다시 연결합니다.
...
| 진행 구간마다 반복되는 모양 | `pattern_token` | 비슷한 패턴을 짧게 비교하기 위해 |

이때 중요한 것은 특정 이름을 외우는 일이 아니라, `원시 값 그대로`보다 `문제와 더 직접 연결되는 읽기 단위`로 바꾸고 있는가입니다. 즉, 특징 선택은 칼럼 제거만이 아니라 `원시 로그 -> 동작 단위 요약 -> 모델 입력 칸`으로 표현을 다시 설계하는 작업이기도 합니다.
