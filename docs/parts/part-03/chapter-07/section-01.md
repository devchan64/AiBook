...
이 절은 독자 기준에서 `좋은 특징을 어떻게 고를 것인가`를 다룹니다. 복잡한 선택 알고리즘을 깊게 설명하기보다, 실제 업무에서 먼저 확인해야 하는 판단 기준을 잡는 데 목적이 있습니다.
...
- 독자 기준에서 먼저 버릴 특징과 먼저 남길 특징을 구분하는 기본 질문을 사용할 수 있습니다.
...
특징을 하나씩 보는 것도 중요하지만, 머신러닝은 보통 특징 하나만으로 판단하지 않습니다. 여러 특징이 함께 만들어 내는 입력 공간을 봅니다. 이를 입문 수준에서는 `특징 공간(feature space)`이라고 이해하면 충분합니다.
...
물론 모두가 항상 불필요한 것은 아닙니다. 다만 독자는 `비슷한 뜻을 다른 단위로 반복하고 있지 않은가`를 먼저 확인하는 편이 좋습니다.
