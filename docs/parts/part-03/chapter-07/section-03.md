...
이 단계를 먼저 거쳐야 `무엇을 예측 문제로 만들 수 있는가`와 `무엇은 비교 리포트로 남겨야 하는가`를 정할 수 있습니다. Part 3에서 붙잡아야 하는 기준선의 뜻도 여기까지입니다. 즉 현재 Section의 기준선은 `평소 구조를 함께 두고 지금 상태의 차이를 읽는 기준`입니다.
