...
Chapter 9를 읽다 보면 자연스럽게 다음 질문이 남습니다. `검토 후보 큐까지는 알겠는데, 그다음에 Part 4에서 쓸 목표 라벨은 어디서 오는가?` 현실 데이터에서는 처음부터 깔끔한 `정답 라벨`이 주어지는 경우가 드뭅니다. 오히려 비교 리포트와 검토 큐를 먼저 만들고, 사람이 남긴 검토 결과와 운영 메모가 쌓이면서 뒤늦게 목표 라벨 후보가 생기는 경우가 많습니다. 이 절에서는 바로 그 축적 과정을 정리합니다.

핵심은 단순합니다. 목표 라벨 후보는 대개 `처음부터 주어진 정답`이 아니라, `검토 과정에서 반복적으로 남은 운영 판단을 더 안정된 열로 바꾼 결과`입니다.
...
즉 Part 4로 가장 먼저 넘어가기 쉬운 것은 대개 `운영 검토 필요 여부`, `반복 경고 여부`, `간단한 상태 구분` 같은 열입니다. 반면 세부 원인 분류는 더 나중에야 안정될 수 있습니다.

## 왜 이 절이 Part 4 직전 이해에 중요한가

Part 4에서는 `target`이 자연스럽게 등장하지만, 초심자는 종종 `그 target은 누가 만들어 주는가`에서 멈춥니다. 이 절의 답은 단순합니다. 많은 경우 target은 데이터셋 안에 원래 들어 있던 것이 아니라, 비교 리포트와 검토 큐를 운영하면서 축적된 판단을 다시 열로 바꾼 결과입니다.
