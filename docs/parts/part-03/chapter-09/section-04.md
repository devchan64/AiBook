...
| B | 미확인 | 0 | 기록만 남김 |
| C | 1 | 1 | 후반 급락 반복, 추가 확인 권장 |

B의 `기록만 남김`은 후반 급락 반복이 없다는 진술이 아닙니다. 따라서 `late_drop_repeated`는 미확인으로 남깁니다. `needs_manual_review=0`도 추가 검토 요청이 없다는 뜻으로만 해석합니다. 메모를 열로 옮길 때 언급되지 않은 속성을 자동으로 0으로 채우면 미확인 상태가 음성 라벨로 바뀝니다.
...
- 메모에서 직접 확인한 사실과 미언급 정보를 구분했는가?
- 반복 여부가 적혀 있지 않을 때 0으로 확정하지 않는 이유를 설명했는가?
