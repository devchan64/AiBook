...
목표 라벨 후보는 대개 `처음부터 주어진 정답`이 아니라, `검토 과정에서 반복적으로 남은 운영 판단을 더 안정된 열로 바꾼 결과`입니다.
...
Part 4에서는 `target`이 자연스럽게 등장하지만, 종종 `그 target은 누가 만들어 주는가`에서 멈춥니다. 이 절의 답은 단순합니다. 많은 경우 target은 데이터셋 안에 원래 들어 있던 것이 아니라, 비교 리포트와 검토 큐를 운영하면서 축적된 판단을 다시 열로 바꾼 결과입니다.
