...
| target 종류 | 뜻 |
| --- | --- |
| [실제 목표(actual target)](../../../reference/concept-glossary-parts/07-siot.md#glossary-actual-target) | 정말 알고 싶고 최종적으로 줄이고 싶은 결과 |
| [대리 타깃(proxy target)](../../../reference/concept-glossary-parts/03-digeut.md#glossary-proxy-target) | 실제 목표를 바로 못 보거나 너무 늦게 봐서 대신 쓰는 열 |
...
actual target과 proxy target의 차이는 이름 차이로 끝나지 않습니다. 무엇을 실제로 맞히고 있는지가 달라지면, 지금 이 문제를 [비교 리포트(comparison report)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure)로 둘지, [검토 후보(review candidate)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure)를 고르는 문제로 둘지, [예측(prediction)](../../../reference/concept-glossary-parts/08-ieung.md#prediction) 문제로 올릴지도 함께 달라지기 때문입니다.
...
| 실제 목표도 약하고 대리 열도 불안정하다 | 비교 리포트 또는 [검토 후보 큐(review queue)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure) | 아직 어떤 값을 결과 열로 둘지 자체가 덜 닫혀 있기 때문 |
...
입력의 하강률이 기준을 넘으면 1이라는 규칙으로 임시 라벨을 만들었다면, 그 입력으로 학습한 모델은 우선 그 규칙을 재현하는 셈입니다. 임시 라벨에 대한 높은 점수가 실제 고장 예측을 검증하지는 않습니다. 실제 고장을 목표로 삼으려면 독립적으로 확인한 고장 기록과 관측 기간이 필요합니다.

## 체크리스트

- 임시 규칙 라벨과 실제 확인된 결과를 구분했는가?
- 규칙을 잘 재현한 점수만으로 실제 고장 예측을 검증할 수 없는 이유를 설명했는가?
