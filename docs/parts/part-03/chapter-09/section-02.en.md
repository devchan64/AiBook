...
Pushing every real problem into a [label prediction](/AiBook/en/reference/concept-glossary-alpha/s/#supervised-learning-label-prediction) problem is not good data modeling. In some cases, a [comparison report](/AiBook/en/reference/concept-glossary-alpha/o/#glossary-output-structure) is more honest and fits the current data state better. This is especially true when cause labels are weak, or when what the decision-maker actually wants is not `correct classification` but `choosing what to inspect first right now`. Here, you also need to organize the possibility that some problems are more correctly left as comparison reports all the way through instead of being raised upward.
...
| Output | Priority table, comparison sentence, [review queue](/AiBook/en/reference/concept-glossary-alpha/o/#glossary-output-structure) | Target label, predicted value, evaluation result |
