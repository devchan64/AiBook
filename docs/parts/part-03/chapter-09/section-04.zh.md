# P3-9.4 复核备忘录如何变成候选目标标签

> Section ID: `P3-9.4`
> Version: `v2026.09.15`

即使[复核候选队列(review queue)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure)和[比较报告(comparison report)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure)先出现，目标标签候选(target candidate)通常也不会被立即给出。最开始留下来的，往往不是整洁的`正确标签`，而是各种各样的复核结果和复核备注。因此，把目标标签候选读成`一开始就给定的答案`并不准确，更准确的读法是：它是把复核过程中反复留下来的判断，转成更稳定列之后得到的结果。
...
| --- | --- | --- | --- |
| A | 1 | 1 | 后段急降重复出现，需要再次确认 |
| B | 未确认 | 0 | 仅保留记录 |
| C | 1 | 1 | 后段急降重复出现，建议进一步确认 |

B 的`仅保留记录`并没有说明不存在后段急降重复。因此，`late_drop_repeated` 应保留为未确认。`needs_manual_review=0` 也只能解释为没有追加复核请求。把备忘录转换成列时，如果自动把未提及的属性填成 0，就会将未知状态变成阴性标签。
...
- 你是否区分了备忘录中直接确认的事实与未提及的信息？
- 你能否解释为什么未记载是否重复时，不应直接确认为 0？
