...
Chapter 9를 읽다 보면 자주 다시 멈추는 지점이 있습니다. 비교 리포트에도 `event_id`가 있고, 검토 후보 큐에도 `event_id`가 있고, 목표 라벨 후보 표에도 다시 `event_id`가 보입니다. `왜 같은 열이 계속 따라다니는가?`, `어떤 열은 남기고 어떤 열은 사라지는가?` 이 연결이 흐리면 세 산출물이 서로 다른 표라는 점은 이해해도, 왜 같은 사건을 계속 추적해야 하는지는 놓치기 쉽습니다.

비교 리포트, 검토 후보 큐, 목표 라벨 후보 표는 서로 다른 역할을 맡지만, 가능하면 `같은 샘플 1건`을 같은 식별 기준으로 계속 추적해야 합니다. 그래야 `왜 이 사건이 올라왔는가`, `어떤 검토 결과가 나중에 어떤 목표 라벨 후보로 바뀌었는가`를 다시 연결할 수 있습니다.
