...
여기서 중요한 것은 feature와 target을 기계적으로 나누는 것이 아니라, 그 구분이 어떤 질문을 위해 필요한지 이해하는 일입니다. feature는 `입력으로 주고 싶은 정보`이고, target은 `나중에 맞히고 싶은 결과`입니다. 이 설명이 서야 독자는 Part 4에서 `X`와 `y`를 나누는 이유를 단순 문법이 아니라 문제 설정의 결과로 읽을 수 있습니다.
...
- 이 열이 feature인 이유를 한 문장으로 설명할 수 있는가
- 이 열이 target 후보라면 실제로 나중에 알고 싶은 결과인가
- 운영 문맥 열과 식별 열을 feature와 섞어 쓰고 있지 않은가
- 비교 리포트로 남길 문제를 억지로 target으로 만들고 있지 않은가

즉 Part 4가 이어받는 것은 단순한 CSV 파일이 아니라, `샘플 단위가 고정된 표`, `설명 가능한 feature`, `후보 target`, `비교 리포트로 남길 범위`까지 정리된 전제입니다. 다음 절에서는 이 전제 위에서 split, baseline, evaluation으로 넘어가기 전에 무엇을 다시 점검해야 하는지 정리하겠습니다.
