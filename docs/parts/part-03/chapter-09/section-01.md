# 3.17 경고, 검토 후보, 라벨 예측을 어떻게 구분하는가

> Section ID: `P3-9.1`
> Version: `v2026.07.06`

현실 데이터를 보면 `센서 로그가 있고 결과도 조금은 있으니, 바로 분류기부터 만들면 되지 않을까?`라는 반응이 먼저 나옵니다. 하지만 운영 데이터에서는 이 생각이 너무 빠를 때가 많습니다. 어떤 문제는 정말로 예측 문제로 만들 수 있지만, 어떤 문제는 아직 `검토 후보를 잘 골라 내는 문제`로 남겨 두는 편이 더 정직하며 현재 데이터 상태에도 더 잘 맞습니다.
...
이 절에서 기억해야 할 핵심은 다음과 같습니다. `경고는 변화 신호이고, 검토 후보는 운영 우선순위이며, 라벨 예측은 그보다 더 강한 문제 설정이다.` 다음 절에서는 바로 이 구분을 바탕으로, Part 4가 실제로 이어받을 특징(feature)과 목표 라벨(target) 전제를 정리합니다.
