...
GridSearchCV, RandomizedSearchCV의 기본 비교와 검증 비용은 다음 절 P3-9.2에서 바로 이어서 다루고, 고급 탐색 공간 설계와 분산 실험 관리는 P3-9.3 보충학습에서 다시 정리합니다.
...
P3-8.2까지 오면 독자는 이런 오해를 하기 쉽습니다.
...
또한 바로 이어지는 알고리즘 절들인 P3-10부터 P3-16까지에서 각 모델의 대표 하이퍼파라미터가 다시 등장합니다.
