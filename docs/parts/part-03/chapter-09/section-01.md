...
세 문제는 아래에서 위로 갈수록 더 강한 근거를 요구하는 단계입니다. 그래서 현재 데이터 상태에서 어디까지 말할 수 있는지가 더 분명해집니다.
...
핵심은 `경고는 변화 신호이고, 검토 후보는 운영 우선순위이며, 라벨 예측은 그보다 더 강한 문제 설정이다`는 점입니다. 지금 문제를 어디까지 올릴지는 `더 고급인가`가 아니라 `현재 데이터가 어디까지 정직하게 지지하는가`로 판단해야 합니다.
