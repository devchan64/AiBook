...
초심자가 여기서 자주 헷갈리는 실제 문장을 바꾸면 다음과 같습니다.
...
이 질문이 중요한 이유는, 초심자가 하이퍼파라미터를 단순한 옵션 표로만 보면 `필요할 때 하나씩 만지면 되는 값`처럼 오해하기 쉽기 때문입니다. 실제로는 이 값들이 비교의 공정성, 일반화, 계산 비용을 함께 흔듭니다.
...
즉, 초심자가 지금 이 절에서 가져가야 할 감각은 다음입니다.
