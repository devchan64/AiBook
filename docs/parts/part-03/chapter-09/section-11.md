...
이 표를 보면 `A`와 `B`는 둘 다 `review_needed = 1`이지만, `final_status`와 `status_type`은 다릅니다. 만약 지금 대표 target을 `review_needed`로 두면 `A`와 `B`는 같은 결과로 묶입니다. 반대로 대표 target을 최종 상태인 `final_status`로 두면 A의 `pending`은 아직 정답이 확정되지 않은 상태이고, B의 `normal`만 확정 결과입니다. `pending`을 정상·이상과 나란한 최종 상태 라벨로 학습시키지 않습니다. 즉 어떤 열을 대표 target으로 고정하느냐에 따라 같은 사건도 같은 정답이 되기도 하고 다른 정답이 되기도 합니다.
...
- pending 상태와 최종 결과 범주를 구분했는가?
- 기준 버전이 다른 라벨을 합치기 전에 필요한 확인을 적었는가?
