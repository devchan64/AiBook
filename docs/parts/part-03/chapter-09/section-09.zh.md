...
如果用`输入下降率超过标准就标为 1`的规则制作临时标签，那么用这些输入训练的模型，首先是在复现这条规则。对临时标签取得高分，并不能验证真实故障预测。若要以真实故障为目标，就需要独立确认的故障记录和观察期间。

## 检查清单

- 你是否区分了临时规则标签与实际确认的结果？
- 你能否解释为什么仅凭良好复现规则的分数，无法验证真实故障预测？
