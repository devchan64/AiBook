...
同一个事件列表，也会因为目的不同而变成[比较报告(comparison report)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure)、[复核候选队列(review queue)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure)、或者目标标签候选(target candidate)表。有的表把比较句子和差值放在前面，有的表把复核优先级放在前面，还有的表把输入列和结果候选列的区分放在前面。这里的目标标签候选，即使还没有凝固成确认答案，也应该被读成：在把输入列和结果列分开的问题结构里，先被立起来的一列结果候选。
...
| --- | --- | --- | --- | --- | --- |
| A | 2.6 | 2.2 | -0.4 | high | 后段均值明显低于基准线 |
| B | 2.5 | 2.4 | -0.1 | low | 与基准线没有较大差异 |
| C | 2.7 | 2.1 | -0.6 | high | 均值比基准线低 0.6 |
...
这些分数是用于说明格式的虚构值，并不是概率。假设实际队列只保留 `review_needed=1` 的案例，再按分数降序排列。B 记录在完整候选表中，但不进入复核队列。
...
可以根据目的同时使用三张表，也可以只制作其中一部分。如果已通过独立检查收集到标签，也可以不经过复核队列就构造目标标签表。无论走哪条路径，样本标识都必须与标签依据相连接。
...
- 你是否在报告、复核队列和目标表中找到同一个 A 事件，并比较其作用？
- 你能否解释 review_needed 为 0 的 B 为什么被排除在复核队列之外？
