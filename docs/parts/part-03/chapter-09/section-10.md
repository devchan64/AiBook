...
[목표 라벨 후보(target candidate)](../../../reference/concept-glossary-parts/05-mieum.md#glossary-target-candidate)를 정할 때는 `언제 결과가 확정되는가`와 `0 라벨을 붙일 만큼 충분히 관측했는가`를 구분해야 합니다. 이 둘을 섞으면 최근 사건이 과하게 0으로 보이거나, 아직 임시 상태인 값을 확정 라벨처럼 읽기 쉽습니다. 라벨 확정 지연과 관측 미완료 음성은 다른 문제이므로, 이 둘을 먼저 분리해 두어야 합니다.
