...
같은 사건 목록도 목적에 따라 [비교 리포트(comparison report)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure), [검토 후보 큐(review queue)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure), 목표 라벨 후보(target candidate) 표로 달라집니다. 어떤 표는 비교 문장과 차이값을 앞세우고, 어떤 표는 검토 우선순위를 앞세우며, 어떤 표는 입력 열과 결과 후보 열 구분을 앞세웁니다. 여기서 목표 라벨 후보는 아직 확정 정답으로 굳지 않았더라도, 입력 열과 결과 열을 가르는 문제 구조 안에서 결과 후보로 세워 보는 열로 읽습니다.
...
| C | 2.7 | 2.1 | -0.6 | high | 평균이 기준선보다 0.6 낮다 |
...
점수는 형식 설명을 위한 가상 값이고 확률이 아닙니다. 실제 큐는 `review_needed=1`인 사례만 남긴 뒤 점수 내림차순으로 정렬한 것으로 가정합니다. B는 후보 전체 표에 기록되지만 검토 큐에는 들어가지 않습니다.
...
세 표는 목적에 따라 함께 쓰거나 일부만 만들 수 있습니다. 라벨이 별도 검사에서 이미 수집된다면 검토 큐를 거치지 않고 목표 라벨 표를 구성할 수도 있습니다. 어떤 경로든 샘플 식별자와 라벨 근거가 연결되어 있어야 합니다.
...
- 같은 A 사건을 리포트·검토 큐·타깃 표에서 찾아 역할을 비교했는가?
- review_needed가 0인 B가 검토 큐에서 제외되는 이유를 설명했는가?
