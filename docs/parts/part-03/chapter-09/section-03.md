# P3-9.3 보충학습: 고급 모델 선택, 튜닝 자동화, 실험 추적을 큰 그림으로 읽기

P3-8과 P3-9에서는 모델 후보를 세우고, baseline을 두고, 하이퍼파라미터를 검증 절차 안에서 비교하는 기본 흐름을 먼저 잡았습니다. 그다음에는 보통 다음 이름들이 등장합니다.
...
P3-9.2에서 본 것처럼 grid search는 설명하기 쉽지만 조합 수가 빠르게 커집니다. 그래서 다음 요구가 생깁니다.
