...
여기까지 오면 자연스럽게 다음 질문을 떠올립니다. 시간 순서 분할(time split)은 언제 필요한가, 같은 개체를 train/test 양쪽에 두면 왜 안 되는가, 데이터 누출(data leakage)은 무엇인가, 상위 몇 건 선별은 왜 분류와 다른가. 모두 중요한 질문입니다. 하지만 지금 단계에서 필요한 일은 이 항목들을 자세히 펼치는 것보다, 현재 문제 구조가 어디까지 닫혀 있어야 하는지 마지막으로 확인하는 것입니다.
...
이 절에서는 이름을 모두 외우는 것보다, 현재 데이터 구조가 시간 경계와 개체 경계, 정보 경계를 제대로 닫고 있는지 확인하는 편이 더 중요합니다. 각 항목의 자세한 평가 절차는 이후 학습 문제 설명에서 다시 다루게 됩니다.
...
따라서 여기서 중요한 일은 각각을 다 가르치는 것이 아니라, 현재 문제 구조 안에서 어떤 경계가 이미 닫혀 있어야 하는지 확인하는 것입니다.
