...
즉 Part 3에서는 다음 정도의 경계가 닫혀 있으면 충분합니다.
...
이 절에서는 이름을 모두 외우는 것보다, 현재 데이터 구조가 시간 경계와 개체 경계, 정보 경계, 산출물 형식을 제대로 닫고 있는지 확인하는 편이 더 중요합니다. 지금 단계에서 필요한 것은 세부 절차를 길게 펼치는 일이 아니라, 현재 구조가 무엇을 예측하고 무엇을 아직 예측하면 안 되는지 스스로 모순 없이 말할 수 있게 만드는 일입니다.

이 절은 항목 이름 모음이 아니라, 현재 문제 구조를 닫기 위한 마지막 확인표로 읽어야 합니다.

더 넓게 보면 여기서 확인하는 것은 `분할 설계`, `정보 경계 점검`, `산출물 형식 선택`이 현재 문제 구조 안에서 서로 모순 없이 닫혀 있는가입니다.


따라서 여기서 중요한 일은 각각의 기법을 다 가르치는 것이 아니라, 현재 문제 구조 안에서 어떤 경계가 이미 닫혀 있어야 하는지 확인하는 것입니다.
