# 3.18 어떤 운영 문제는 왜 비교 리포트로 남겨야 하는가

> Section ID: `P3-9.2`
> Version: `v2026.07.06`

모든 운영 문제를 예측 문제로 밀어 넣는 것은 좋은 데이터 모델링이 아닙니다. 어떤 경우에는 비교 리포트가 더 정직하며 현재 데이터 상태에도 더 잘 맞습니다. 특히 원인 라벨이 약하거나, 운영자가 실제로 보고 싶은 것이 `정답 분류`보다 `지금 먼저 볼 대상을 고르는 일`일 때 그렇습니다.

이 지점에서는 `입력 -> 정답 라벨 -> 분류기` 구조가 먼저 떠오르기 때문에, 현실 문제도 모두 그 틀에 넣어야 할 것처럼 느껴질 수 있습니다. 하지만 운영 데이터에서는 `정답을 맞히는 일`보다 `무엇을 먼저 보여 줄 것인가`가 더 중요한 경우가 많습니다. 이런 문제를 억지로 분류 문제로 바꾸면, 오히려 라벨 품질이 약한 상태에서 과장된 자동화를 만들기 쉽습니다.
...
따라서 이 절에서의 결론은 단순합니다. `비교 리포트는 임시 대안이 아니라, 그 자체로 올바른 문제 설정일 수 있다.` 다음 장에서는 바로 이 구분을 바탕으로, Part 4로 넘길 feature, target, split 전제가 어디까지 정리되어야 하는지 마무리합니다.
