...
분류 문제라면 여기에 대표 오류 사례를 한 번 더 붙여 보는 편이 안전합니다. baseline보다 점수가 조금 올랐더라도, 실제로는 같은 소수 클래스나 같은 혼동 구간을 계속 놓치고 있을 수 있기 때문입니다.
...
운영 문장으로 줄이면 다음과 같습니다. `후보 모델이 0.910이고 튜닝 후 0.912라면 바로 배포 결론을 내리지 않는다. 다음 조치는 어떤 조합이 같은 validation 분할에만 과하게 맞았는지, 그리고 0.002 개선이 추론 시간과 운영 복잡도 증가를 감수할 값인지 함께 다시 보는 일이다.` 즉, 튜닝 절의 핵심은 `최고 점수`를 외우는 것이 아니라 `작은 개선이 실제로도 남는 개선인가`를 끝까지 확인하는 데 있습니다.
