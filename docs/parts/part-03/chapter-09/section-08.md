...
| [정책 규칙(policy rule)](/AiBook/reference/concept-glossary-parts/09-jieut.md#glossary-policy-rule) | 그 출력을 어떤 기준으로 행동으로 바꾸는가 |
...
| [정책 규칙(policy rule)](/AiBook/reference/concept-glossary-parts/09-jieut.md#glossary-policy-rule) | `0.8 이상이면 검토`, `상위 10%만 본다` |
