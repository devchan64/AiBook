...
같은 [타깃(target)](../../../reference/concept-glossary-parts/12-tieut.md#target) 이름 아래에서도 어떤 실수가 더 아픈지는 문제마다 다를 수 있습니다. `review_needed`를 맞히는 문제라고 해도 위험 사례를 놓치는 것이 더 위험한지, 괜히 검토에 올리는 것이 더 부담스러운지는 운영 맥락에 따라 달라집니다. 같은 target이라도 놓치는 실수와 괜히 잡는 실수의 비용이 다를 수 있으므로, 이 [오류 비용(error cost)](../../../reference/concept-glossary-parts/08-ieung.md#glossary-error-cost) 차이를 먼저 적어 두어야 지금 어떤 판단을 더 줄이려는지 분명해집니다.

| 실수 종류 | 운영에서 생길 수 있는 일 |
| --- | --- |
| [거짓 음성(false negative)](../../../reference/concept-glossary-parts/01-giyeok.md#glossary-false-negative) | 위험 사례를 놓쳐 더 큰 이상으로 번질 수 있다 |
| [거짓 양성(false positive)](../../../reference/concept-glossary-parts/01-giyeok.md#glossary-false-positive) | 사람이 괜히 시간을 써서 검토 부담이 커질 수 있다 |
...
오류 비용은 설명 문단에만 두지 말고 `false_negative_cost`, `false_positive_cost`, `review_capacity`, `threshold_policy_note`, `cost_owner`처럼 정책과 연결되는 필드로도 남깁니다. 이렇게 하면 target 이름이 같아도 어떤 실수를 더 줄이려는 문제인지가 점수 해석과 검토 큐 설계까지 이어집니다.

## 왜 오류 비용이 target 해석을 바꾸는가

같은 `review_needed` target이라도 모든 예측값을 같은 방식으로 읽는 것은 아닙니다. 어떤 문제에서는 놓침(false negative)이 더 아파서 조금 더 많이 [검토 후보 큐(review queue)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure)에 올리더라도 위험 사례를 덜 놓치는 편이 낫고, 어떤 문제에서는 과검출(false positive)이 더 아파서 검토 큐를 더 좁게 유지하는 편이 낫습니다. 이때 바뀌는 것은 단순히 [임계값(threshold)](../../../reference/concept-glossary-parts/08-ieung.md#glossary-threshold) 숫자 하나가 아니라, `이 target을 어떤 판단 구조로 해석할 것인가`입니다.
...
놓침 비용이 큰 문제라면 `B`도 검토 큐에 넣는 편이 더 자연스럽습니다. 반대로 과검출 비용이 큰 문제라면 `B`는 아직 보류하고 `A`만 보는 편이 더 자연스러울 수 있습니다. 즉 같은 [점수(score)](../../../reference/concept-glossary-parts/05-mieum.md#glossary-score)와 같은 target 이름이 있어도, 오류 비용 구조가 다르면 review queue 우선순위와 threshold 해석도 함께 달라집니다.
...
비용표를 적용하기 전에 양성(1)이 무엇인지 적습니다. 예를 들어 `7일 안 고장 발생`이 1이면 놓침은 실제 고장을 음성으로 예측한 경우입니다. 이 절의 비용 수치는 계산을 위한 가정이며 실제 비용의 추정값이 아닙니다. 검토 인원과 처리 가능한 건수가 달라지면 같은 점수에도 실행 가능한 임계값이 달라지므로, 예제에서 가장 싼 규칙을 운영 최적값으로 그대로 옮기지 않습니다.

## 체크리스트

- 양성의 뜻을 정하고 오탐·놓침 비용을 계산했는가?
- 예제 비용과 실제 운영 비용·용량이 다르면 판단이 어떻게 달라질지 설명했는가?
