...
[목표 라벨 후보(target candidate)](../../../reference/concept-glossary.md#glossary-target-candidate)은 `열이 있는가`만으로 충분하지 않고, `같은 사건에 비슷한 조건이면 비슷한 라벨이 붙는가`도 함께 봐야 합니다.

## 라벨 일관성은 왜 함께 점검해야 하는가

이 질문은 현재 라벨 후보를 바로 목표 라벨(target label)로 올려도 되는지 판단하는 데 필요합니다. Part 3에서 계속 다뤄 온 것은 `무엇을 예측 문제로 올릴 수 있는가`였고, 여기서는 그 판단을 `라벨 의미가 반복되는가`까지 확장해 점검합니다.
...
이 메모를 남겨 두면 이후 더 복잡한 입력 구조를 다룰 때도 `열이 있다`는 사실보다 `그 열이 같은 뜻으로 반복되는가`를 먼저 점검할 수 있습니다. 지금 이 절의 초점은 뒤 단계를 예고하는 데 있지 않고, 현재 목표 라벨 후보가 아직 비교 리포트 단계에 머물러야 하는지 판단하는 데 있습니다.
...
여기서 중요한 것은 특정 운영팀의 메모 습관이 아니라, `라벨 의미 안정성(label meaning stability)`을 확인하는 일입니다.


즉 `라벨 열이 있다`는 사실은 출발점일 뿐이고, 실제로는 그 열이 같은 뜻으로 반복되는지까지 봐야 합니다.

목표 라벨 후보를 읽을 때는 세 가지를 함께 확인해야 합니다. 현재 라벨 후보가 같은 뜻으로 비교적 반복되는가, 기준이 바뀐 시점을 메모할 수 있는가, 그리고 불안정한 라벨을 바로 학습 문제로 올리지 않고 있는가입니다. 이런 점검이 있어야 목표 라벨 후보 표는 단순한 열 목록이 아니라, `라벨 의미의 안정성`까지 포함한 구조가 됩니다.

목표 라벨 후보는 열이 있다는 사실만으로 충분하지 않고, 같은 사건과 비슷한 조건에서 비슷한 판단이 반복되는지도 함께 확인해야 합니다.
