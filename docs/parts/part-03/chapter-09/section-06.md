...
이 메모를 남겨 두어야 뒤에서 학습 입력이나 긴 시퀀스를 다룰 때도 `열이 있다`는 사실보다 `그 열이 같은 뜻으로 반복되는가`를 먼저 점검할 수 있습니다. 즉 Part 4·5의 학습 설명을 미리 펼치는 데 있지 않고, 현재 목표 라벨 후보가 아직 비교 리포트 단계에 머물러야 하는지까지 판단하는 데 있습니다.
