...
이 절은 Part 3 안에서 `행`, `샘플`, `구간`의 층위를 한 번에 비교하는 보강 위치를 맡습니다. 앞 절의 정의를 다시 반복하기보다, 지금까지 나온 표 구조를 한 화면에서 겹치지 않게 정렬하는 데 집중합니다.
...
이 절의 핵심 문장은 다음과 같습니다. `행은 표의 줄이고, 샘플은 비교의 기본 단위이며, 최근 구간은 샘플 여러 건을 다시 묶은 비교 단위다.` 이 구분이 잡혀야 뒤에서 특징, 기준선, 검토 문장을 서로 다른 층위에 놓고 읽을 수 있습니다.
