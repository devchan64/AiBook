...
把这四点留在表中时，不要只把它们放在样本表之外的说明里。要保留 `sampled_at`、`shift`、`load_mode`、`machine_id`、`maintenance_phase` 等能读出代表性范围的列，以便以后重新统计按条件的空白。不足的区间不要只写成结论句；如果留在 `coverage_note` 或单独 复核备注 中，Part 4 读取评估分数时就能再次确认这个结果几乎没有覆盖哪些条件。
...
代表性并不意味着所有条件下的案例数都相同。如果目标运行场景中白天占 80%、夜间占 20%，就要结合这个比例和采集过程来判断。反过来，如果需要分别检查两个时段的性能，还应单独确认夜间案例是否足够。各条件都超过最低数量，并不能证明具有代表性。
...
输入(input)：保存在 [p3_4_5_sample_coverage.csv](/AiBook/assets/part-03/chapter-04/p3_4_5_sample_coverage.csv){ .csv-preview } 里的动作样本表，以及最低观察标准 `minimum_count`。这张表里包含 `shift`、`load_mode`、`machine_id`、`maintenance_phase`。
...
这段代码只统计在 CSV 中至少出现过一次的条件。目标运行场景中存在、但尚未采集到的条件，需要与另外的清单对照并标为 0 条。此外，即使分别存在`夜间`和`高负载`案例，也可能没有`夜间且高负载`的组合，因此还要检查所需的条件组合。
...
- 你是否比较了采集表中各条件的比例与目标运行分布？
- 你是否找出了数量为 0 的条件，以及单独分类统计会遗漏的组合？
