...
看到这个信号时，下一步不是继续增加特征，而是先把表格倒回检查一次。看到重复标签，就先写下标签贴到哪个对象的识别符；看到一行解释不了的特征，就先写下计算该特征的分组 标准；如果训练和评估中出现同一次动作，就把分割列重新贴到样本单位，而不是行。这样，警告信号才会连接到修正表结构的工作，而不是停在简单 检查项。
...
输入(input)：保存在 [p3_4_4_sample_unit_warning_log.csv](/AiBook/assets/part-03/chapter-04/p3_4_4_sample_unit_warning_log.csv){ .csv-preview } 里的原始日志表，以及重复警告标准 `repeat_warning_threshold`。这张表里按 `event_id` 存放了时点流量值，并把属于动作级的 `review_needed` 重复保存到了多行中。
...
只要这四个问题中有一个暴露出与当前分析目的不一致之处，就应在继续构造特征之前重新检查样本单位。
...
- 你是否在自己的表中找到了问题与样本边界不一致的信号？
- 你能否追溯到原始事件，解释特征或标签重复的原因？
