...
## 행 단위 집계가 경고 수를 부풀리는 경우 {#_5}

문제 상황: 시점별 표에서 같은 라벨이 반복되고 동작 단위 특징이 따로 나타날 때, 샘플 단위를 잘못 잡았다는 신호를 어떻게 읽는지 확인합니다.

입력(input): [p3_4_4_sample_unit_warning_log.csv](../../../assets/part-03/chapter-04/p3_4_4_sample_unit_warning_log.csv){ .csv-preview }에 저장된 원시 로그 표와 반복 경고 기준 `repeat_warning_threshold`. 이 표에는 `event_id`별 시점 유량과 동작 단위에 붙은 `review_needed`가 반복 저장되어 있습니다.
...
이 네 질문 중 하나라도 현재 분석 목적과 어긋나면, 특징을 더 만들기 전에 샘플 단위를 다시 보는 편이 낫습니다.

## 반복 라벨과 비교 오류에서 샘플 단위로 돌아가기 {#_7}
...
- 자신의 표에서 질문과 샘플 경계가 맞지 않는 신호를 하나 찾았는가?
- 특징이나 라벨이 중복되는 이유를 원래 사건까지 거슬러 설명했는가?
