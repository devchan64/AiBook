...
Chapter 5부터는 원시 로그를 요약 표와 집계 표로 다시 묶는 구체적 작업으로 넘어갑니다. 그런데 그 전에 `왜 다시 묶어야 하는가`에 대한 진단 신호가 없으면, 요약 표 만들기가 단순 변형처럼 보일 수 있습니다. 그래서 Chapter 4 마지막에서는 샘플 단위를 잘못 잡았을 때 나타나는 징후를 먼저 모아 둘 필요가 있습니다.
