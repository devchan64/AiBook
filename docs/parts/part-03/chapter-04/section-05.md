...
샘플 단위를 동작 1회나 최근 구간 1개처럼 정하고 나면, 초심자가 한 번 더 놓치기 쉬운 질문이 남습니다. `지금 모은 샘플이 전체 운영 상황을 얼마나 대표하는가?` 표가 잘 정리되어 있어도, 그 표가 특정 공정 모드나 특정 기간, 특정 장비 상태에서만 모인 사례라면 뒤에서 읽게 될 평가(evaluation)와 일반화(generalization)는 쉽게 흔들립니다.

핵심은 단순합니다. 샘플 단위를 잘 정한 것과, 그 샘플 묶음이 전체 상황을 고르게 대표하는 것은 같은 말이 아닙니다.

## 왜 이 질문이 Part 3에 필요한가

대표성 문제를 너무 늦게 보면 초심자는 보통 뒤 Part에서만 증상을 보게 됩니다.
...
Part 4에서는 일반화와 평가를 읽게 됩니다. 그런데 대표성 메모가 없으면 초심자는 평가 점수를 너무 빨리 일반화처럼 읽기 쉽습니다.

| Part 4에서 뒤에 보게 될 것 | Part 3에서 먼저 막아야 하는 오해 |
| --- | --- |
| evaluation | 특정 구간 점수를 전체 운영 성능처럼 읽기 쉽다 |
| generalization | 실제로는 보지 못한 조건이 많은데도 넓게 일반화한다고 착각하기 쉽다 |
| split | train/test가 나뉘어도 둘 다 같은 조건 범위만 담을 수 있다 |

즉 Part 4의 일반화 문제 일부는, Part 3에서 대표성 범위를 메모하지 않은 채 넘어간 결과일 수 있습니다.

## Part 5와는 어떻게 이어지는가

시계열과 딥러닝 쪽에서도 대표성 문제는 그대로 남습니다.

| Part 5에서 남는 질문 | 왜 Part 3에서 먼저 봐야 하는가 |
| --- | --- |
| 특정 패턴을 잘 잡는다 | 실제로는 특정 모드만 많이 본 결과일 수 있기 때문 |
| 시계열 구조를 잘 배웠다 | 야간, 고부하, 전환 구간을 거의 못 봤을 수 있기 때문 |
| 표현 학습이 해결해 준다 | 입력이 대표하지 못한 조건을 자동으로 만들어 주지는 않기 때문 |

즉 더 복잡한 모델로 가더라도, 대표하지 못한 운영 조건은 그대로 공백으로 남을 수 있습니다.
