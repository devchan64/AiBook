...
그래서 `요약`을 단순한 축소로 이해하면 곤란합니다. 동작 1회 요약 행은 원시 시계열 여러 행을 사람이 비교하고 모델이 다루기 쉬운 한 행으로 바꾼 결과입니다. 비교를 쉽게 만들어 주지만, 원시 시계열의 모든 맥락을 대체하지는 않습니다.
...
초심자가 여기서 자주 놓치는 점은 `세 표 중 어느 것이 지금 내 작업의 출발점인가`입니다. 아래처럼 간단히 확인하면 표를 덜 섞어 읽게 됩니다.
...
이 예제를 본 뒤에는 아래처럼 짧게 자문해 보면 좋습니다.
...
이 절에서 기억할 문장은 단순합니다. `표의 이름보다 먼저, 한 행이 무엇을 뜻하는지 확인해야 한다.` 그 질문이 정해져야 뒤에서 특징도, 기준선도, 라벨도 안정적으로 읽을 수 있습니다. 다음 절에서는 바로 이 요약 표 위에서 `같은 평균인데 다른 패턴`이 어떻게 생기는지 살펴봅니다.
