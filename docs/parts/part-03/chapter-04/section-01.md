...
Chapter 3에서 `아직 학습 문제로 올릴 단계가 아니다`를 확인했다면, 이번 장에서는 이제 실제로 무엇을 샘플 1건으로 셀지 정해야 합니다.

| 이전 Chapter에서 남긴 것 | 이번 Chapter에서 추가로 정하는 것 | 다음 Chapter로 넘길 구조 |
| --- | --- | --- |
| 원천데이터를 아직 학습 문제로 읽지 말아야 한다는 경고 | 한 행, 한 샘플, 최근 구간 가운데 무엇을 비교 단위로 둘지 | 원시 로그를 비교 가능한 표로 다시 묶기 위한 샘플 단위 |
