...
운영 문장으로는 더 짧게 적을 수 있습니다. `train은 거의 완벽한데 validation/test가 흔들리면 지금 부스팅은 패턴보다 잡음을 더 배우고 있을 가능성이 크다. 다음 조치는 learning_rate, 단계 수, 트리 깊이를 함께 줄여 보고 early stopping 기준을 다시 확인하는 일이다.` 즉, 부스팅 절의 핵심은 `점수가 높다`에서 끝나지 않고 `어느 지점부터 과한 보정이 시작됐는가`, `그 과한 보정을 무엇으로 먼저 줄일 것인가`까지 이어져야 합니다.
