...
이 절의 핵심은 트리가 깊어진다는 사실보다, 그 깊이가 어떤 실패를 만드는지 같이 읽는 데 있습니다.

| 같이 봐야 할 것 | 이 절에서 먼저 읽는 질문 | 뒤에서 다시 이어질 곳 |
| --- | --- | --- |
| train-test gap | 더 깊어진 트리가 실제로 일반화까지 좋아졌는가 | P3-5 일반화, P3-9 튜닝 |
| 복잡도 손잡이 | `max_depth`, `min_samples_leaf`, `ccp_alpha` 중 무엇이 어떤 식으로 과적합을 줄이는가 | P3-9 하이퍼파라미터 |
| 대표 실패 구간과 다음 모델 | 어떤 가지가 예외를 외우고 있고, 이를 bagging이나 boosting이 어떻게 완화할 수 있는가 | P3-15 랜덤포레스트, P3-16 그래디언트 부스팅 |
