...
一个好的[数据问题](/AiBook/zh/reference/concept-glossary-pinyin/s/#glossary-data-question)，首先应该让人看见 `什么算一条案例`、`什么和什么比较`、`最终想知道什么`。只有在模型名或技术名之前，先把这种问题结构立起来，后面的样本单位、表结构、特征、基准线、输出结构才会一起跟着定下来。尤其是，先挑出人要优先看的对象的问题，应该能自然通向 [review queue](/AiBook/zh/reference/concept-glossary-pinyin/s/#glossary-output-structure)；而先定义以后要预测什么结果的问题，则应该通向 target candidate。相反，不好的问题往往只有模型名字，却把样本单位和比较基准留空了。
