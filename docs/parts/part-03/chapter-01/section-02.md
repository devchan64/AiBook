...
이 Part에서는 몇 개의 핵심 용어가 반복해서 등장합니다. `샘플(sample)`, `특징(feature)`, `기준선(baseline)`, `출력 구조(output structure)`가 그것입니다. 초심자는 보통 이 용어들을 각각 따로 외우려 하지만, 실제로는 네 가지가 하나의 연결 구조를 이룹니다. 무엇을 한 건의 샘플로 볼지 정해야 특징을 만들 수 있고, 특징이 있어야 무엇을 기준선과 비교할지 정할 수 있으며, 그 비교가 있어야 어떤 출력 구조를 만들 것인지도 결정할 수 있습니다.
...
즉 이 네 요소는 따로 외울 용어 목록이 아니라, 앞에서 뒤로 이어지는 설계 순서입니다. 샘플이 흔들리면 특징도 흔들리고, 특징이 흔들리면 기준선 비교도 흔들리고, 비교가 흔들리면 출력 구조도 흔들립니다. 뒤에서 Part 3 전체를 더 자세히 풀더라도, 이 기본 연결은 계속 유지됩니다.

초심자 기준에서는 이 네 요소를 정의로만 외우기보다, 실제로는 아래 순서로 질문한다고 생각하는 편이 더 쉽습니다.
...
이 네 질문은 각각 샘플, 특징, 기준선, 출력 구조에 대응합니다. 그래서 용어가 흐릿해도 질문 순서를 붙잡으면 현재 어떤 설계 단계에 있는지 다시 찾기 쉽습니다.
...
여기서 한 번 더 중요한 차이를 짚어 둘 필요가 있습니다. 출력 구조는 아직 `정답 라벨이 확정된 학습 데이터`를 뜻하지 않을 수도 있습니다. 초심자에게는 `검토 필요`, `정상 범위` 같은 출력과 `yes/no` 같은 지도학습 라벨이 같은 것처럼 보일 수 있지만, 실제로는 다를 수 있습니다.
...
이 구분을 먼저 두면 뒤에서 `출력 구조`를 말할 때도 초심자가 곧바로 `라벨이 이미 완성되었다`고 오해하지 않게 됩니다.
