...
여기서는 Part 3 안에서 `데이터 모델링의 진행 순서`를 가장 압축해서 보여 주는 대표 설명 위치를 맡습니다. 뒤 Chapter에서는 이 여섯 단계를 하나씩 펼치되, 같은 용어를 다시 길게 정의하기보다 현재 단계에서 왜 중요한지만 연결합니다.
...
이 순서가 추상적으로만 보인다면, 초심자가 실제로 자주 밟는 `잘못된 시작 순서`와 비교해 보는 편이 이해가 빠릅니다.

| 시작 방식 | 처음에는 왜 그럴듯해 보이는가 | 실제로 곧 생기는 문제 |
| --- | --- | --- |
| 모델 이름부터 고른다 | 분류, 회귀, 이상 탐지 같은 익숙한 말이 먼저 떠오르기 때문이다 | 아직 한 행이 한 시점인지 한 동작인지 정해지지 않아 `입력 X` 자체가 흔들린다 |
...
다음 질문에 막힘 없이 답할 수 있으면, Part 3을 읽는 순서 감각이 잡힌 것입니다.
...
이 절의 핵심 문장은 다음과 같습니다. `데이터 모델링은 질문, 샘플, 표, 특징, 비교, 출력 구조를 순서 있게 설계하는 과정이다.` 다음 Chapter부터는 이 순서를 실제 장면과 표 구조로 하나씩 펼쳐서 봅니다.
