...
이 절은 Part 3 안에서 `데이터 모델링의 진행 순서`를 가장 압축해서 보여 주는 대표 설명 위치입니다. 뒤 Chapter에서는 이 여섯 단계를 하나씩 펼치되, 같은 용어를 다시 길게 정의하기보다 현재 단계에서 왜 중요한지만 연결합니다.
...
같은 이유로 Part 3을 읽을 때는 `데이터과학을 배우는 중`이라는 감각보다 `학습 가능한 데이터 문제를 설계하는 중`이라는 감각을 유지하는 편이 좋습니다. 이 관점을 잡아 두면 뒤에서 나올 샘플 설계, 요약 표, 특징 설계, 기준선 비교도 흩어진 기법이 아니라 하나의 문제 설정 절차로 읽히기 시작합니다.
