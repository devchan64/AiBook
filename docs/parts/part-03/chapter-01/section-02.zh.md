...
一旦理解了数据建模到底想达成什么，接下来的问题就会立刻出现：实际工作应该按什么顺序推进？在样本单位还没定下来之前，特征没法先做；如果没有比较基准，连 [输出结构(output structure)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure) 也会跟着晃动。所以，数据建模最好被读成一种前段顺序：把后面会用到的结构，从前往后逐步固定下来。
...
这个顺序不是定下一次就不再回头的直线流程。例如，如果后段经常缺失，就应先重新检查问题或采集范围，而不是只修改特征。记录前面的决定，并不是为了禁止修改，而是为了知道应该回到哪一个判断。
...
- 你是否按从问题到输出表的顺序列出了必要决定？
- 某个区段的记录缺失时，你能否说明应该回到哪一步？
