...
처음 이 구분이 낯설다면, 눈앞의 표를 보고 바로 아래 세 질문만 먼저 던져도 충분합니다.
...
이 세 질문은 초심자에게 `저장 구조를 문제 표현 구조로 다시 만들어야 하는가`를 빠르게 판단하게 해 줍니다. 첫 질문은 행의 뜻을 묻고, 둘째 질문은 비교 가능성을 묻고, 셋째 질문은 원시 로그를 언제 다시 열어야 하는지 묻습니다. 데이터 모델링은 이 세 질문에 답할 수 있게 표를 다시 설계하는 일로 이해하면 훨씬 덜 추상적으로 읽힙니다.
