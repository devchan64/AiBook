...
여기서는 Part 3 안에서 `데이터 모델링`이라는 가장 바깥 개념의 대표 설명 위치를 맡습니다. 뒤 절에서 `샘플`, `특징`, `기준선`, `비교 리포트` 같은 말이 다시 나오더라도, 그 절의 중심 질문에 필요한 만큼만 연결하고 큰 정의 축은 여기와 [개념사전](../../../reference/concept-glossary.md)에 둡니다.
...
다음 네 가지를 스스로 말해 볼 수 있으면 이 절의 출발점이 잡힌 것입니다.

- 원천데이터 표의 한 행이 `시점 기록`인지 `샘플 1건`인지 구분할 수 있는가
- 모델링 후 표의 한 행이 무엇을 비교하기 위한 단위인지 설명할 수 있는가
- 특징 열과 비교 열이 왜 따로 필요한지 말할 수 있는가
- 출력 구조가 `자동 확정`이 아니라 `사람 검토 후보`일 수도 있음을 설명할 수 있는가

이 절의 핵심 문장은 다음과 같습니다. `데이터 모델링은 원천데이터를 질문에 답할 수 있는 샘플, 특징, 비교, 출력 구조로 다시 설계하는 일이다.` 다음 절에서는 바로 이 일을 어떤 순서로 진행하는지 한 번에 정리합니다.

## 언제 이 관점을 먼저 떠올려야 하는가

- 원천데이터를 보기 좋게 정리하는 일과, 질문에 답할 수 있는 비교 구조를 설계하는 일을 구분해야 할 때 데이터 모델링 관점을 먼저 떠올립니다.
- 모델 이름보다 먼저 샘플, 특징, 기준선, 출력 구조를 정해야 하는 이유를 설명해야 할 때 이 절로 돌아옵니다.
- 지금 손에 있는 표가 `무엇을 한 건으로 보고 무엇을 결과로 낼지`까지 답할 수 있는지 점검할 때 다시 확인합니다.
