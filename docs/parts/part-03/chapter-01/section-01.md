...
이 절은 Part 3 안에서 `데이터 모델링`이라는 가장 바깥 개념의 대표 설명 위치로 사용합니다. 뒤 절에서 `샘플`, `특징`, `기준선`, `비교 리포트` 같은 말이 다시 나오더라도, 그 절의 중심 질문에 필요한 만큼만 연결하고 큰 정의 축은 여기와 [개념사전](../../../reference/concept-glossary.md)으로 다시 돌아오면 됩니다.
