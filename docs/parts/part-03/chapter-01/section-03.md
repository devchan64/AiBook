...
Part 3 초반을 읽을 때 가장 자주 생기는 막힘은 이것입니다. `질문을 먼저 정하라`는 말은 알겠는데, 실제로 무엇을 질문이라고 써야 하는지 모르겠다는 점입니다. 많은 경우 질문 대신 바로 `분류를 할까`, `시계열 딥러닝을 쓸까`, `이상 탐지를 할까` 같은 모델 이름이나 기술 이름을 떠올립니다. 그래서 이 절에서는 모델 이름보다 먼저 써야 하는 `데이터 질문`의 모양을 고정합니다.

좋은 데이터 질문은 `무엇을 한 건으로 볼지`, `무엇을 무엇과 비교할지`, `최종적으로 무엇을 알고 싶은지`가 드러나야 합니다. 반대로 나쁜 질문은 모델 이름은 있는데 샘플 단위와 비교 기준이 비어 있습니다.
...
Chapter 2와 Chapter 3에서는 저장 구조와 데이터셋 후보, 질문에 맞는 표 재설계를 설명합니다. 그런데 그 전에 질문 자체가 흐리면 `무슨 표를 다시 만들어야 하는가`도 계속 추상적으로 남습니다. 그래서 `질문을 먼저 정한다`는 원칙과 `저장 구조를 다시 읽는다`는 설명 사이에, 실제 질문 문장을 고쳐 쓰는 다리를 하나 두는 편이 더 안전합니다.
