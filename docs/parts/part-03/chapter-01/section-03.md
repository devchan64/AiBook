...
좋은 [데이터 질문(data question)](../../../reference/concept-glossary-parts/03-digeut.md#data-modeling)은 `무엇을 한 건으로 볼지`, `무엇을 무엇과 비교할지`, `최종적으로 무엇을 알고 싶은지`가 먼저 드러나야 합니다. 모델 이름이나 기술 이름보다 이런 질문 구조가 먼저 서 있어야, 뒤의 샘플 단위, 표 구조, 특징, 기준선, 출력 구조도 함께 정해집니다. 특히 사람이 먼저 볼 대상을 고르는 질문은 [검토 후보 큐(review queue)](../../../reference/concept-glossary-parts/05-mieum.md#output-structure), 나중에 맞힐 결과를 세우는 질문은 목표 라벨 후보(target candidate)로 이어진다는 점이 함께 보여야 합니다. 반대로 나쁜 질문은 모델 이름은 있는데 샘플 단위와 비교 기준이 비어 있습니다.
...
| 결과를 예측하고 싶다 | 어떤 결과를 나중에 맞히고 싶은지 정하고 싶다 | 동작 종료 시점의 특징으로 다음 7일 안의 고장 발생 여부를 예측할 수 있는지 보고 싶다 |
| 딥러닝을 써 보고 싶다 | 원시 시계열을 그대로 쓸지 정하고 싶다 | 동작 1회 요약 벡터와 같은 동작의 시계열 중 어떤 입력 구조가 더 자연스러운지 먼저 정하고 싶다 |
...
`설비가 이상한가?`를 읽고, 한 건의 단위와 비교 대상, 출력 형식을 직접 넣어 보세요. 설비 한 대의 완료된 동작을 비교한다는 가정이라면 `동작 1회를 한 건으로 보고, 같은 운전 모드의 과거 동작과 후반 평균을 비교해 사람이 검토할 후보를 표시한다`라고 쓸 수 있습니다. 여기에는 동작별 한 행, 운전 모드, 후반 평균, 기준선, 검토 후보 표시가 필요합니다. 고장 여부를 예측한다고 쓰지 않았으므로, 이 질문만으로 미래 고장 라벨이 준비된 것은 아닙니다.

질문을 `매일 끝날 때 그날의 전체 운전 상태를 요약한다`로 바꾸면 무엇이 달라질까요? 첫 질문의 한 행은 동작 1회이고, 두 번째 결과표의 한 행은 하루입니다. 일별 표에는 동작 건수와 집계 기간도 필요합니다. 질문 문장을 고쳤을 때 표의 한 행과 필요한 열이 함께 달라지는지 확인하면, 표현만 바꾼 것인지 실제 설계가 바뀐 것인지 구분할 수 있습니다.

## 체크리스트

- 모델 이름 없이 샘플 단위·알고 싶은 결과·비교 기준을 담은 질문을 썼는가?
- 같은 동작의 요약 벡터와 시계열을 비교할 때 고정한 것과 바꾼 것을 구분했는가?
