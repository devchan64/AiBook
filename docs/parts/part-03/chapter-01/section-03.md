...
Chapter 2와 Chapter 3에서는 저장 구조와 데이터셋 후보, 질문에 맞는 표 재설계를 설명합니다. 그런데 그 전에 질문 자체가 흐리면 `무슨 표를 다시 만들어야 하는가`도 계속 추상적으로 남습니다. 그래서 `질문을 먼저 정한다`는 원칙과 `저장 구조를 다시 읽는다`는 설명 사이에는 실제 질문 문장을 고쳐 쓰는 다리가 하나 필요합니다.
