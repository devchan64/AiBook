...
보상 설계, 탐험 비용, 시뮬레이션과 현실 차이, 실제 적용의 위험은 P3-19.3에서 이어서 정리합니다. PPO, TRPO, A2C, A3C 같은 후속 알고리즘과 연속 제어의 큰 그림은 P3-19.4 보충학습에서 다시 정리합니다.

강화학습 장을 처음 읽을 때는 `지금 어디까지 이해하면 충분한가`를 같이 고정해 두는 편이 안전합니다.

| 지금 이 절에서 닫아야 하는 것 | 바로 다음 절로 넘기는 것 | 보충학습이나 현재 책 범위 밖에 두는 것 |
| --- | --- | --- |
| 정책 기반 강화학습이 왜 `행동 점수표`보다 `행동 방식 자체`를 직접 조정하는 관점으로 읽히는가 | 보상 설계, 탐험 비용, sim-to-real 같은 현실 적용 제약 | PPO, TRPO, A2C, A3C 세부 구현과 policy gradient 엄밀한 수학 유도 |
| 가치 기반과 정책 기반이 어떤 문제 장면에서 다른 손잡이를 주는가 | 정책을 실제 환경에 붙일 때 어떤 위험 점검이 먼저 필요한가 | 연속 제어 심화 수식과 확률비 추정 증명 |

즉, 현재 절의 책임은 `정책 기반이 왜 등장했고 어떤 문제 장면에서 자연스러운가`를 닫는 데 있습니다. 알고리즘 이름을 더 많이 외우는 일이나 수학 유도를 길게 따라가는 일보다, `정책을 직접 바꾼다`는 말이 무엇을 뜻하는지와 `어떤 문제 장면에서 가치 기반보다 읽기 쉬운가`를 분명히 잡으면 충분합니다.
...
여기서는 REINFORCE를 다음처럼 읽으면 충분합니다.
...
독자에게는 로봇 팔 사례가 특히 중요합니다. `왼쪽`과 `오른쪽`처럼 행동 수가 적은 문제에서는 가치 기반 접근이 직관적일 수 있지만, `얼마나`, `어느 속도로`, `어느 각도로`까지 함께 정해야 하면 정책을 직접 출력하는 쪽이 더 자연스럽게 읽힐 수 있습니다.

반대로 모든 문제에 정책 기반 강화학습이 더 낫다고 읽으면 안 됩니다. 행동 후보가 적고, 점수표를 통해 명확히 비교할 수 있는 문제라면 가치 기반 접근이 더 단순하고 설명력이 좋을 수 있습니다.

## 실행 가능한 Python 예제로 정책 조정 보기

이번 예제는 `보상이 좋았던 행동의 확률을 높인다`는 정책 기반 강화학습의 핵심 감각을 숫자로 직접 확인하는 데 초점을 둡니다.
...
이 절의 핵심은 정책 기반 이름을 더 외우는 일이 아니라, 정책을 직접 바꾼다는 말이 무엇을 뜻하는지 고정하는 데 있습니다.

| 같이 봐야 할 것 | 이 절에서 먼저 읽는 질문 | 바로 다음에 이어질 곳 |
| --- | --- | --- |
| 가치 기반과의 차이 | 점수표를 먼저 배울 것인가, 행동 방식을 바로 바꿀 것인가 | P3-19.1 가치 기반 강화학습 |
| policy gradient와 actor-critic | 어떤 방식으로 정책을 고치고, 누가 평가 신호를 주는가 | P3-19.4 후속 알고리즘 지도 |
| 연속 행동과 큰 상태 공간 | 왜 정책 자체를 다루는 편이 더 자연스러운가 | P3-19.3 현실 적용과 제어 문제 |
