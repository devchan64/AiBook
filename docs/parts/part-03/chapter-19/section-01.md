...
초심자에게는 이 차이가 중요합니다. 가치 기반 강화학습은 행동 규칙을 바로 외우기보다, `행동의 예상 점수판`을 먼저 만든다고 이해하면 읽기 쉬워집니다.
...
이 점 때문에 Q-learning은 보통 `오프-정책(off-policy)` 알고리즘으로 소개됩니다. 초심자 기준에서는 이렇게 읽으면 충분합니다.
...
이 점 때문에 SARSA는 보통 `온-정책(on-policy)` 알고리즘으로 설명됩니다. 초심자 기준에서는 이렇게 읽으면 충분합니다.
