...
현재 Part 3에서는 강화학습 수학을 별도 `보충학습`으로 더 쪼개지 않습니다. 이 책에서 강화학습의 학습 목표는 상태, 행동, 보상, 가치, 정책, 탐험, sim-to-real 같은 구조를 읽는 데 있고, 벨만 방정식의 엄밀한 전개나 수렴 증명은 본편과 보충학습을 오가며 회수하기보다 더 심화된 수학 자료에서 따로 보는 편이 범위상 더 적절합니다.
...
| Q-learning과 SARSA 차이 | 가장 좋아 보이는 다음 행동과 실제 다음 행동 중 무엇을 기준으로 삼는가 | P3-19.2 정책 기반 강화학습 |
| 탐험이 섞인 현실 흐름 | 이 갱신 방식이 얼마나 낙관적이거나 보수적인가 | P3-19.3 적용 위험과 탐험 비용 |
