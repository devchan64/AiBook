# P3-19.4 보충학습: DQN, PPO, RLHF를 강화학습 큰 흐름 안에서 읽기

P3-19.1부터 P3-19.3까지 읽고 나면 강화학습을 더 공부할 때 곧 여러 이름을 만나게 됩니다.
...
이 이름들은 서로 다른 시대와 문제의식에서 나왔지만, 입문 단계에서는 한꺼번에 밀려들기 쉽습니다. 이 절의 목적은 각 알고리즘 구현을 배우는 것이 아니라, `왜 이런 이름들이 분기되었는가`를 큰 흐름으로 정리하는 데 있습니다.
...
P3-19.1에서 본 Q-learning은 직관이 좋지만, 상태와 행동이 커지면 표(table)로 값을 다 적기 어려워집니다. DQN은 이 지점에서 등장합니다.

핵심은 다음 한 문장입니다.

`Q-value를 표가 아니라 함수 근사기로 대신 표현해 더 큰 상태 공간을 다루려는 흐름이 DQN이다.`

그래서 DQN은 완전히 새로운 철학이라기보다, 가치 기반 강화학습을 더 큰 문제로 확장한 대표 사례로 이해하면 됩니다.

## PPO와 actor-critic 계열은 왜 널리 쓰이나

P3-19.2에서 본 정책 기반 강화학습은 행동 방식을 직접 조정한다는 장점이 있지만, 학습이 흔들리기 쉽습니다. PPO, TRPO, A2C, A3C 같은 이름은 대체로 이 문제와 연결됩니다.
...
P3-19.3에서 본 것처럼 현실에서는 탐험 자체가 위험하거나 비쌀 수 있습니다. 그래서 다음 두 갈래가 따로 커졌습니다.
...
| 지금 Part 3에서 잡는 것 | Part 5에서 다시 보는 것 |
| --- | --- |
