...
即使已经有了 [比较表(comparison table)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure)，也不代表所有差异都应该用同样的强度去解读。尤其在运营数据里，样本量往往较小，而且同样的变化是否重复出现也未必明确。因此在解读阶段，不仅要看`哪里变了`，还要一起看`这个差异可以被多大程度地信任`。
...
- 样本少且重复证据弱：降低对持续状态变化这一判断的确信程度。
- 样本少但多次出现：列为人工复核候选，而不是自动确认。
- 样本数量和重复证据都充足：可以考虑更强的警告或后续分析。

这里需要同时保留`这时候还不能说得太重`这一规则，才能调节解读强度。读者需要能马上看出，为什么同一个 `diff` 会对应不同的运营句子。这样后面再解释告警、[复核候选队列(review queue)](/AiBook/zh/reference/concept-glossary-pinyin/s/#output-structure) 和 [评估(evaluation)](/AiBook/zh/reference/concept-glossary-pinyin/p/#evaluation-design) 时，理解才不会变得摇晃。
...
这张表是一个说明性示例，假定比较条件和波动幅度相近。20 条并不是普遍适用的充分数量标准。从同一事件切出的 20 个重叠窗口，与 20 个不同事件提供的证据不同。这里 `repeat_ratio` 是呈现相同方向的事件数除以总事件数，并不保证在时间顺序上连续重复。
...
样本少，并不意味着实际措施也必须减弱。即使只有一次观测，只要超过另行定义的允许限值，也可能需要立即确认。`对持续变化有多大把握`与`确认有多紧迫`是两个不同的判断。
...
- 你是否分别确认了样本数量、独立性和重复证据？
- 你是否用不同语句表达了解释的确信程度与措施的紧迫性？
