...
`확정 라벨이 검토된 사례에만 남아 있다면, 그 라벨은 전체를 대표한다고 바로 읽으면 안 된다.`
...
| 상위 프레임 | 이 절에서의 대응 |
| --- | --- |
| 라벨이 붙은 집합 | 사람이 실제로 다시 본 사례 |
| 라벨이 비어 있는 집합 | 정상인지 미확인인지 더 구분이 필요한 사례 |
| 해석 메모 | 검토 경로, 라벨 없음의 뜻, 편중 가능성 |

이 프레임을 잡아 두면 확정 라벨 표는 `전체 사건의 정답표`가 아니라, 검토 경로를 거친 일부 사건의 확인 결과일 수 있다는 점이 더 분명해집니다.
