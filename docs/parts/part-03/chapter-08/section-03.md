...
표본이 적다는 이유만으로 조치까지 늦춰야 하는 것은 아닙니다. 변화가 지속된다는 확신은 낮게 두더라도, 한 번의 관측이 별도 안전 한계를 넘었다면 정해진 운영 절차에 따라 확인할 수 있습니다. 이때 `반복 변화 근거는 부족함`과 `즉시 확인 필요`를 서로 다른 칸에 씁니다.

## 체크리스트

- 관측 사실·불확실성·다음 확인 순서로 검토 문장을 썼는가?
- 표본이 적다는 표현이 필요한 조치를 자동으로 미루지 않는지 확인했는가?
