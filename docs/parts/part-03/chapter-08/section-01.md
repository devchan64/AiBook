...
초심자에게는 이 세 문제를 `한 단계씩 올라가는 사다리`처럼 읽는 것도 도움이 됩니다.
...
이 절에서 기억해야 할 핵심은 다음과 같습니다. `경고는 변화 신호이고, 검토 후보는 운영 우선순위이며, 라벨 예측은 그보다 더 강한 문제 설정이다.` 다음 절에서는 바로 이 구분을 바탕으로, Part 4가 실제로 이어받을 특징(feature)과 목표 라벨(target) 전제를 정리하겠습니다.
