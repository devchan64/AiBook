# 3.15 표본 수가 적을 때 무엇을 말하지 말아야 하는가
...
이 두 예제는 다음 순서로 읽으면 표본 수와 반복성이 해석 강도를 어떻게 바꾸는지 더 분명해집니다.
...
이 절에서 먼저 남겨야 할 문장은 다음과 같습니다. `표본 수가 적을수록 차이는 더 조심스럽게 읽어야 한다.` 그리고 `반복되는 변화는 단발성 변화와 다른 의미를 가질 수 있다.` 다음 절에서는 이 감각을 바탕으로, 왜 변화 신호와 원인 확정을 같은 문장으로 말하면 안 되는지 더 분명히 다룹니다.
