...
검토 우선순위는 차이값 하나가 아니라, 변화 크기, 반복성, 해석 신뢰도, 운영 중요도를 함께 묶어 만든 후보 판단입니다. 따라서 이 절의 핵심은 `여러 비교 열을 어떻게 한 줄 점수로 구현할까`보다 `여러 비교 열이 어떤 질문 묶음으로 다시 압축되는가`를 먼저 분명히 하는 데 있습니다.
