...
이 표에 설명용 규칙을 실제로 적용해 보세요. `safety_related=yes`를 먼저 두고, 그 안에서는 `diff_mean`의 절댓값이 큰 순서로 정렬하며, 두 값이 모두 같으면 `event_id` 오름차순으로 정한다고 합시다. 이 규칙에서는 A → C → B입니다. C는 차이가 더 작아도 안전 관련 조건 때문에 B보다 먼저 옵니다. 이 순서는 고장 확률의 순위가 아니라, 지금 정한 검토 순서입니다.

우선순위 1순위를 차이의 절댓값으로 바꾸면 A → B → C가 됩니다. A와 B의 차이 크기가 같아서 동률 규칙으로 A가 먼저 옵니다. 원자료가 같아도 정책이 바뀌면 순서가 달라지므로, 순위와 함께 규칙 버전을 남겨야 합니다. 두 규칙은 계산 예시이며 실제 운영에 맞는 기준은 별도로 정해야 합니다.
