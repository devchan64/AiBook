...
이 절은 우선순위 점수 구현이 아니라, `다기준 판단(multi-criteria judgment)`을 어떤 축으로 압축할 것인가의 문제로 다시 볼 수 있습니다.
...
이 프레임을 잡아 두면 `열이 많다`는 복잡함보다 `서로 다른 질문을 몇 개의 판단 축으로 묶는다`는 구조가 먼저 보입니다.
...
- 왜 `diff`가 큰 사례를 바로 가장 위의 검토 후보로 올리면 안 되는지 설명할 수 있는가
- 여러 비교 열을 변화 크기, 반복성, 해석 신뢰도, 운영 중요도로 다시 묶는 이유를 말할 수 있는가
- 같은 차이값이라도 최근 건수와 반복성이 다르면 왜 다른 우선순위 후보가 되는지 설명할 수 있는가

## 언제 이 관점을 먼저 떠올려야 하는가

- 비교표의 열이 많아 무엇을 먼저 보고 검토 후보로 줄여야 할지 막힐 때 이 절의 네 판단 축을 먼저 떠올립니다.
- `priority_score` 같은 열이 갑자기 나온 것처럼 느껴질 때, 그 앞단에 어떤 비교 축 묶음이 있었어야 하는지 다시 확인할 때 이 절로 돌아옵니다.
- Chapter 9의 검토 후보 큐가 단순 점수표가 아니라 해석 결과를 압축한 산출물이라는 점을 설명해야 할 때 이 절이 기준이 됩니다.

## 출처와 참고 자료

- NIST/SEMATECH e-Handbook of Statistical Methods, `What are Variables Control Charts?`. 관리도는 여러 종류의 신호를 함께 읽는 decision aid로 설명되므로, 여러 비교 열을 몇 개의 판단 축으로 묶어 읽는 발상의 일반 근거가 됩니다. [https://www.itl.nist.gov/div898/handbook/pmc/section3/pmc32.htm](https://www.itl.nist.gov/div898/handbook/pmc/section3/pmc32.htm){: target="_blank" rel="noopener noreferrer" } / 확인일: 2026-07-08
