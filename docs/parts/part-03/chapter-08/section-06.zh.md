...
这里重要的点是：`选择性附着的确认标签，可以成为解读依据，但在把它当作代表所有事件的完整答案集合之前，必须先写出复核路径和可能的偏向。` 因此，确认标签表首先不该被读成`全部事件的答案表`，而应被读成通过[复核候选队列（review queue）](/AiBook/zh/reference/concept-glossary-pinyin/s/#glossary-output-structure)等复核路径后，对部分事件得到的确认结果。
