...
baseline은 이길 상대가 아니라, 점수를 읽는 자이다.

처음 읽을 때는 baseline 비교 순서를 아래처럼 짧게 고정해 두는 편이 좋습니다.
...
벤치마크와 리더보드의 운영 관점, 통계 검정 기반 모델 비교의 큰 그림은 P3-9.3 보충학습에서 다시 정리하고, 실제 하이퍼파라미터 비교 절차는 P3-9.2에서 바로 이어서 봅니다.
...
독자가 이 절에서 가장 먼저 붙잡아야 할 질문은 다음 세 가지입니다.
...
이 기준은 Part 6의 회고 문서 언어로 다시 쓰면 더 분명해집니다.
