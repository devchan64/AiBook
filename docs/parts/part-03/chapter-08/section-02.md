...
여기서 한 걸음 더 나가면 `모델링 사다리` 관점도 보입니다. 처음에는 비교 리포트와 규칙 기반 검토 큐(review queue)로 시작할 수 있고, 나중에 라벨이 쌓이면 `review_needed` 예측으로 넘어갈 수 있으며, 그보다 더 안정된 라벨이 생기면 원인 분류나 더 복잡한 모델을 검토할 수 있습니다. 즉 좋은 데이터 모델링은 처음부터 가장 복잡한 문제를 세우는 일이 아니라, 현재 데이터 상태에 맞는 문제를 정직하게 고르는 일입니다.
