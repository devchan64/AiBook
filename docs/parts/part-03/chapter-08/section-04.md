...
| --- | ---: | ---: | --- | --- |
| A | -0.35 | 20 | high | 검토 우선순위를 높이고 원인 확정은 보류한다 |
| B | -0.35 | 3 | low | 차이는 보이지만 표본이 적어추가 관찰이 필요하다 |
...
`priority_score`의 0.82와 0.41은 출력 형식을 보여 주기 위한 가상 값입니다. 앞 문장에서 수학적으로 계산된 값이나 고장 확률이 아닙니다. 실제 점수로 쓰려면 어떤 열에 어떤 가중치·조건을 적용했는지 명시해야 합니다. 계산 규칙이 없다면 `높음/낮음` 같은 판단 등급까지만 남기는 편이 재현 가능합니다.
...
- 구간 식별자와 event_count를 근거로 해석 강도를 설명했는가?
- 가상 우선순위 점수를 확률로 읽지 않고 출력 열의 용도를 적었는가?
