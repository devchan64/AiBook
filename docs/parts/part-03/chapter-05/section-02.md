...
초심자 기준에서는 아래처럼 `이 표현이 왜 필요한가`를 한 번 더 나눠 보면 도움이 됩니다.
...
그래서 이 표현은 원시 로그를 대체하기보다, 원시 로그와 요약 표 사이에 놓이는 `중간 표현`으로 읽는 편이 안전합니다. 원시 로그는 가장 많은 정보를 갖고 있고, 요약 표는 비교가 쉽고, 토큰화된 표현은 그 중간에서 구조를 더 눈에 띄게 드러냅니다. 이 관계를 이해하면 초심자도 `왜 어떤 문제는 평균만으로 부족하고, 왜 어떤 문제는 원시 로그 전체를 매번 볼 필요가 없는가`를 더 자연스럽게 받아들일 수 있습니다.
...
이 절에서 기억해야 할 핵심은 다음과 같습니다. `세그먼트 표현과 토큰화는 원시 곡선을 사람이 비교할 수 있는 중간 표현으로 바꾸는 방법이지만, 정보를 완전히 보존하지는 않는다.` 다음 장에서는 바로 이런 비교 구조를 최근 구간과 기준선 비교에 연결해, `무엇이 달라졌는가`를 상태 수준에서 읽는 법으로 넘어가겠습니다.
