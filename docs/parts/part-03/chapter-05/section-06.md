...
창을 실제 표로 만들 때는 파생된 각 입력 창에도 원천 사건 식별자를 남겨야 합니다. `source_event_id`, `window_id`, `window_start`, `window_end`, `stride`, `source_event_weight` 같은 열이 있으면 입력 창 수와 원천 사건 수를 따로 셀 수 있습니다. 이 열이 없으면 237개의 창이 36개의 사건에서 나왔다는 사실이 사라지고, 뒤에서 대표성이나 평가 점수를 읽을 때 근거 단위가 실제보다 커 보일 수 있습니다.
