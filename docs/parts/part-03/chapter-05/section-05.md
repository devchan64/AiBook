...
이 판단을 먼저 해 두어야 뒤에서 빈칸 처리나 시계열 입력을 다룰 때도 `채울 수 있는 값`과 `샘플 구조를 이미 무너뜨린 누락`을 섞지 않게 됩니다. 즉 이 절의 중심은 처리 기법 예고가 아니라, 현재 샘플이 아직 같은 비교 단위인지와 빠짐 자체를 구조 정보로 남길지 먼저 구분하는 데 있습니다.
