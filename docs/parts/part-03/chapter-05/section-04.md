...
일정 간격 재샘플링은 측정 간격을 맞추는 일입니다. 40초와 80초의 기록을 모두 1초 간격으로 바꿔도 점 개수는 같아지지 않습니다. 고정 개수가 필요하다면 공통 시간 범위로 자르거나, 진행률 구간 수를 맞추거나, 짧은 입력의 빈자리를 채우는 별도 규칙이 필요합니다. 빈자리를 0으로 채웠다면 실제 측정값 0과 구별할 표시도 남겨야 합니다.

동작 종료 전 예측이라면 실제 종료 시각을 알아야 계산되는 최종 진행률이나 종료 직전 구간은 아직 사용할 수 없습니다. 완료된 동작을 비교하는 창과 진행 중 예측에 쓰는 창을 구분해야 합니다.
...
- 80초 동작의 마지막 25%가 어디서 시작하는지 계산했는가?
- 동작 종료 전 예측에 최종 동작 길이를 쓰면 왜 문제가 되는지 설명했는가?
