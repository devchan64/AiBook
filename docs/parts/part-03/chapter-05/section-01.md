...
이 절에서 기억할 문장은 단순합니다. `표의 이름보다 먼저, 한 행이 무엇을 뜻하는지 확인해야 한다.` 그 질문이 정해져야 뒤에서 특징도, 기준선도, 라벨도 안정적으로 읽을 수 있습니다. 다음 절에서는 바로 이 요약 표 위에서 `같은 평균인데 다른 패턴`이 어떻게 생기는지 다룹니다.
