...
여기서는 `원시 로그 -> 요약 표 -> 집계 표` 변환 절차를 가장 먼저 고정합니다. 뒤 절에서 요약 표가 어떤 패턴을 남겨야 하는지 더 자세히 다루더라도, 표 변환의 큰 흐름 자체는 여기와 [개념사전](../../../reference/concept-glossary.md)에 둡니다.
...
- 원시 로그만으로는 왜 동작 간 직접 비교가 어려운가
- 요약 표가 생기면 왜 동작 1회 비교가 쉬워지는가
- 집계 표는 왜 개별 동작보다 `최근 상태`를 읽는 데 더 가깝다고 말하는가
- 이상한 변화가 보였을 때 왜 다시 집계 표에서 요약 표, 원시 로그 순서로 내려가야 하는가
...
- 원시 로그를 바로 비교하려 들기보다 요약 표와 집계 표로 다시 표현해야 할 때 표 변환 관점을 먼저 떠올립니다.
- 지금 손에 든 표가 원시 로그인지, 동작 단위 요약 표인지, 최근 상태 집계 표인지 구분해야 할 때 이 절로 돌아옵니다.
- 이상한 변화가 보였을 때 어떤 표에서 어떤 표로 다시 내려가 확인해야 하는지 판단할 때 이 절이 기준이 됩니다.
