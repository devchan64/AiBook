...
여기서는 `원시 로그 -> 요약 표 -> 집계 표` 변환 절차를 먼저 고정합니다. 뒤 절에서 요약 표가 어떤 패턴을 남겨야 하는지 더 자세히 다루더라도, 이 흐름은 [기준선(baseline)](../../../reference/concept-glossary.md#glossary-baseline) 비교와 [중간 표현(intermediate representation)](../../../reference/concept-glossary.md#glossary-intermediate-representation) 설계가 어디에서 붙는지 읽는 기준점이 됩니다.
