...
이 도식은 거리 함수가 모델 바깥의 장식이 아니라, 실제 이웃 순서를 정하는 판단 규칙이라는 점을 보여 줍니다. 같은 query라도 거리 규칙이 바뀌면 가까운 이웃이 달라지고, 그 결과 예측 자체도 달라질 수 있습니다.
...
이 도식은 스케일 조정의 목적을 아주 단순하게 보여 줍니다. 조정 전에는 큰 숫자 축 하나가 거리를 거의 혼자 결정할 수 있지만, 조정 후에는 여러 특징이 함께 비교 기준으로 반영될 수 있습니다.
...
독자 기준으로 여기서 반드시 잡아야 할 문장은 다음입니다.
