...
확인 가능한 결과는 원본 거리와 스케일 조정 후 거리를 나란히 볼 때 드러납니다. 스케일 조정 전에는 위험 고객이 더 가깝게 보였는데, 조정 후에는 안전 고객이 더 가깝게 보인다면, 그 차이는 모델이 바뀐 것이 아니라 `가까움의 계산 기준`이 바뀐 결과라고 설명할 수 있습니다.
