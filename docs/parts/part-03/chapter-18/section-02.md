...
P3-18.1에서는 차원 축소(dimensionality reduction)가 많은 특징을 더 적은 축으로 다시 표현하는 일이라는 점을 보았습니다. 이제 바로 다음 질문이 중요합니다.

차원을 줄여서 그림으로 보기 쉬워졌다면, 그 그림을 어디까지 믿어도 될까?

차원 축소로 만든 2차원 또는 3차원 그림은 구조를 살펴보는 데 유용하지만, 원래 고차원 데이터의 모든 관계를 그대로 보존하지는 않는다.

즉, 시각화는 매우 강한 도구이지만, 동시에 강한 착시를 만들 수도 있습니다.
...
이 절은 입문적으로 `그림은 유용하지만 과신하면 안 된다`는 해석 원칙을 만드는 데 집중합니다.
...
차원 축소를 배우고 나면 초심자는 보통 이런 기대를 갖습니다.
...
즉, 차원 축소는 본질적으로 `압축(compression)`에 가깝습니다.
...
즉, 시각화는 `사람 친화적 표현`이지, 자동으로 `완전한 구조 복제`는 아닙니다.
...
즉, 그림의 거리와 원래 거리 사이에는 차이가 생길 수 있습니다.
...
즉, 차원 축소는 보통 `큰 그림(big picture)`에는 도움이 되지만, 세밀한 판단에서는 주의가 필요합니다.
...
즉, 그림 속 군집은 `실제 구조의 힌트`일 수는 있지만, 그 자체로 충분한 증거는 아닙니다.
...
즉, 차원 축소 그림은 `확정 도구`가 아니라 `탐색 도구`로 읽어야 합니다.
...
이 도식은 차원 축소 시각화를 `탐색 도구`로 읽어야 한다는 점을 보여 줍니다. 패턴을 찾고 설명하는 데는 강하지만, 그림 하나만으로 최종 결론이나 증명을 대신할 수는 없습니다.

## 안전한 읽기 순서

차원 축소 결과를 볼 때는 다음 순서가 안전합니다.
...
- 차원 축소 시각화는 고차원 구조를 쉽게 보게 해 주는 강력한 도구입니다.
- 하지만 더 적은 축으로 압축하는 과정에서 정보 손실과 왜곡이 생길 수 있습니다.
- 2D 그림의 거리와 원래 고차원 거리 사이에는 차이가 생길 수 있습니다.
- 그림 속 덩어리는 구조 가설을 제안할 수 있지만, 그 자체로 정답이나 증거는 아닙니다.
- 차원 축소 결과는 탐색과 설명의 도구로 읽고, 원래 데이터와 후속 검토로 다시 확인해야 합니다.

## 체크리스트

- 차원 축소 그림이 원래 구조의 완전한 복사본이 아니라는 점을 설명할 수 있는가?
- 2D에서 가까워 보인다고 원래도 반드시 가깝다고 단정하면 왜 위험한지 말할 수 있는가?
- 차원을 줄이면 큰 흐름은 남길 수 있지만 일부 세부 정보는 잃을 수 있다는 점을 이해했는가?
- 차원 축소 그림을 최종 결론보다 가설 생성 도구로 읽는 태도를 가질 수 있는가?
- 군집 그림과 차원 축소 그림을 함께 볼 때 착시가 더 강해질 수 있다는 점을 알고 있는가?
