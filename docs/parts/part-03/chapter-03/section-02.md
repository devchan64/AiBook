# 3.6 데이터셋은 주어진 표가 아니라 다시 만든 구조다

> Section ID: `P3-3.2`
> Version: `v2026.07.06`

앞 절에서 본 것처럼 원천데이터를 처음 보면 이미 데이터셋이 주어진 것처럼 받아들이곤 합니다. 하지만 데이터셋은 보통 처음부터 완성된 형태로 주어지지 않습니다. 데이터셋을 만든다는 말은 단지 파일을 하나 여는 일이 아니라, `무엇을 한 행으로 둘지`, `어떤 열을 남길지`, `무엇과 비교할지`를 다시 정하는 일입니다.
