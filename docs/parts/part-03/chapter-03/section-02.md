...
이 절은 Part 3 안에서 `질문에 맞는 데이터셋 재설계`를 가장 직접적으로 설명하는 대표 위치입니다. 뒤에서 동작 단위 표, 비교 리포트, Part 4 인계 구조를 다시 다루더라도, 데이터셋을 `주어진 파일`이 아니라 `질문에 맞게 다시 만든 구조`로 읽는 큰 축은 여기와 [개념사전](../../../reference/concept-glossary.md)으로 다시 돌아오면 됩니다.
