...
여기서는 Part 3 안에서 `질문에 맞는 데이터셋 재설계`를 가장 직접적으로 설명하는 대표 위치를 맡습니다. 뒤에서 동작 단위 표, 비교 리포트, Part 4 인계 구조를 다시 다루더라도, 데이터셋을 `주어진 파일`이 아니라 `질문에 맞게 다시 만든 구조`로 읽는 큰 축은 여기와 [개념사전](../../../reference/concept-glossary.md)에 둡니다.
...
| 다시 만드는 표 | 왜 따로 필요한가 |
| --- | --- |
| 동작 단위 특징 표 | 동작 1회를 샘플로 비교하려면 시점별 로그를 요약해야 해서 |
| 비교 리포트 표 | 최근 상태와 기준선 차이를 바로 읽으려면 비교 열이 더 필요해서 |
| 예측 문제 후보 표 | 나중에 목표 라벨과 입력 열을 분리하려면 출력 구조를 다시 정리해야 해서 |

즉 `표를 여러 번 다시 만든다`는 말은 같은 일을 반복한다는 뜻이 아니라, 질문이 바뀔 때마다 필요한 구조를 분리한다는 뜻입니다.
...
다음 질문에 막힘 없이 답할 수 있으면, 이 절의 재설계 관점은 잡힌 것입니다.

- 같은 원천데이터에서 동작 단위 표와 비교 리포트 표가 왜 동시에 필요할 수 있는가
- `파일이 있다`와 `비교 가능한 샘플 표가 있다`를 왜 다른 말로 보아야 하는가
- 표를 다시 만드는 일이 왜 단순 복사가 아니라 질문 재설계인가
- 이 절의 4단계를 쓰면 지금 표가 저장 구조인지 데이터셋 설계 결과인지 어떻게 구분할 수 있는가
...
- 같은 원천데이터에서도 비교용 표, 리포트용 표, 예측 문제 후보 표가 따로 필요할 수 있음을 설명해야 할 때 데이터셋 재설계 관점을 먼저 떠올립니다.
- `파일이 있다`와 `비교 가능한 샘플 표가 있다`를 구분해야 할 때 이 절의 표 역할 구분으로 돌아옵니다.
- 지금 표를 저장 구조로 보고 있는지, 질문에 맞게 다시 만든 데이터셋 구조로 보고 있는지 점검할 때 다시 확인합니다.
