...
在一个虚构案例中，已完成动作 A 的后段均值为 2.4 L/min，相同运行条件下的历史基准为 2.8 L/min。请为 `报告每次动作的后段均值是否低于平常水平` 填写一行。

| event_id | late_flow_mean | baseline_late_flow_mean | delta_from_baseline | report_sentence |
| --- | ---: | ---: | ---: | --- |
| A | 2.4 | 2.8 | -0.4 | 后段均值比相同条件下的基准低 0.4 L/min |

差值为 `当前值 − 基准 = 2.4 − 2.8 = −0.4`。标识、测量值、比较值和报告句在同一行承担不同角色。如果 B 有测量值，却没有对应条件的基准，该怎么填？将基准和差值留空，并记录 `需要取得比较基准`。把基准填成 0，会制造出与一个未观测值的比较。这张表也还没有填写未来故障标签的依据。

## 检查清单

- 你是否在第一张表中区分了标识、观测值和比较基准列？
- 对于暂时无法填写的列，你是否标明了所需资料，而不是填入 0？
