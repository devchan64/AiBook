...
| 当前要回答的问题 | 先构造的表 | 原因 |
| --- | --- | --- |
| 这一次动作是否比其他动作更不稳定？ | 动作级表 | 比较对象是一次动作，因此要先把逐时点日志合成一条动作记录 |
| 最近 20 次动作是否表现出不同于平时 200 次的趋势？ | 近期区间比较表 | 问题比较多次动作组成的集合，需要汇总和基准线比较列 |
| 异常迹象从哪个时点开始？ | 测量值表 + 动作级表 | 先在动作级表选出异常动作，再回到原始时点日志确定变化开始的位置 |
| 之后能否构造用于监督学习的输入表？ | 动作级特征表或候选预测问题表 | 必须在同一套样本单位上分开输入列与候选目标标签 |

因此，问题是在问一次动作、近期区间的变化，还是变化开始的时点，会决定首先构造哪张表。重新设计数据集之所以容易让人觉得困难，与其说是表太多，不如说是容易忽略：`问题变了，作为依据的表也要变。`
...
- 你是否画出了同一份日志针对两个不同问题所需的表？
- 你是否区分了异常被观察到的开始时点与已经确认异常原因的说法？
