...
회귀 진단의 기초 읽기, 유의성 검정, 다중공선성의 해석은 P3-10.3 보충학습에서 다시 정리하고, 정규화와 더 넓은 과적합 제어 관점은 P4-8.1에서 다시 연결합니다. feature engineering의 큰 흐름은 P3-7.1, P3-7.2와 P3-18.1, P3-18.2에서 다시 볼 수 있습니다.
...
확인 가능한 결과는 잔차와 지표를 함께 볼 때 드러납니다. 평균 오차는 작아 보여도 특정 구간에서 잔차가 한쪽으로 크게 몰리거나 큰 실패가 반복되면, 선형회귀가 그 구간 구조를 충분히 설명하지 못하고 있다는 신호로 읽어야 합니다.
...
| --- | --- | --- |
| baseline 오차 | 직선 모델이 단순 평균 예측보다 실제로 나은가 | P3-8 baseline 비교 |
| 평균 오차와 큰 실패 구간 | 전체적으로 얼마나 틀리고, 어디서 유난히 크게 틀리는가 | P3-6 회귀 지표 |
| 대표 오류 사례 | 어떤 입력 조건에서 같은 방식으로 반복 실패하는가 | P3-18 feature engineering, 뒤 회귀 모델 비교 |
...
선형회귀를 지나면, 이제 `직선으로 연속값을 예측하는 모델`에서 `직선을 분류 경계처럼 읽는 모델`로 넘어갈 준비가 됩니다. 다음 P3-11의 로지스틱 회귀(logistic regression)는 이 연결을 가장 자연스럽게 보여 줍니다.
