...
평가 지표와 잔차 해석은 다음 절인 P3-10.2에서 바로 이어서 다루고, 다중공선성, 가정 검정, 회귀 진단의 기초 읽기는 P3-10.3 보충학습에서 다시 정리합니다. 정규화의 더 넓은 관점은 P4-8.1에서 다시 연결합니다.
...
이 도식은 실제 데이터가 직선 위에 완벽히 놓이지 않아도 선형회귀가 여전히 유용한 이유를 보여 줍니다. 목표는 모든 점을 정확히 지나는 선이 아니라, 전체 방향을 가장 무난하게 요약하는 선을 만드는 데 있습니다.
