...
이 도식은 한 변수 선형회귀를 `입력 하나가 기울기와 절편을 거쳐 예측값으로 이어지는 구조`로 보여 줍니다. 초심자는 여기서 선형회귀가 데이터를 외우는 모델이 아니라, 입력 변화가 출력 변화를 어떻게 밀어 올리거나 끌어내리는지 수치로 읽는 모델이라는 점을 먼저 잡으면 됩니다.
