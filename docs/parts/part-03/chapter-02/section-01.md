...
이 입력은 1초 간격이므로 마지막 두 유량 값의 차이가 1초당 변화율과 같습니다. 간격이 다르면 시간 차로 나누어야 합니다.
...
- 원시 로그도 데이터셋일 수 있지만 현재 질문에는 준비가 덜 된 이유를 설명했는가?
- 한 동작을 비교하는 표를 만들 때 필요한 식별자와 묶음 규칙을 적었는가?
...
- [Google Machine Learning Glossary](https://developers.google.com/machine-learning/glossary){ target="_blank" rel="noopener noreferrer" }. dataset과 labeled/unlabeled example의 구분를 확인했다. 확인일: 2026-09-15.
