...
다음 질문에 답할 수 있으면 이 절의 구분은 잡힌 것입니다.
...
이 절에서 먼저 잡아야 할 한 문장은 다음과 같습니다. `저장된 기록은 아직 데이터셋이 아닐 수 있으며, 데이터셋은 질문에 맞게 다시 표현한 구조다.` 이 구분이 생기면 데이터 모델링을 단순한 DB 스키마 설명으로 축소하지 않게 되고, 뒤에서 나올 샘플, 특징, 기준선, 출력 구조도 왜 `데이터셋 후보를 만드는 단계`에 속하는지 더 직접적으로 읽을 수 있습니다. 다음 절에서는 이 흐름을 구성하는 `샘플`, `특징`, `기준선`, `출력 구조`가 데이터셋 후보 안에서 어떤 자리를 차지하는지 한 번에 놓고 보겠습니다.
