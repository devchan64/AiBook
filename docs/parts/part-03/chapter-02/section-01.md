...
| 상위 프레임 | 이 절에서의 대응 |
| --- | --- |
| 원천 기록 보존 | 시점별 기록, 센서 측정, 제어 설정 |
| 분석 단위 재설정 | 동작 1회, 최근 구간 같은 비교 단위 |
| 파생 표현 생성 | 평균, 하강률, 비교 열 같은 데이터셋 후보 |

이 프레임을 잡아 두면 저장된 기록이 곧바로 데이터셋이 아니라는 말은 `형식이 다르다`는 뜻보다 `질문에 답할 단위와 파생 표현이 아직 정해지지 않았다`는 뜻이라는 점이 더 분명해집니다.
