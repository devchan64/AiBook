...
여기서는 `저장된 기록`과 `데이터셋 후보`를 구분하는 데 집중합니다. [데이터셋(dataset)](../../../reference/concept-glossary-parts/01-giyeok.md#glossary-dataset)은 학습이나 평가를 위해 같은 질문에 답하도록 묶어 둔 샘플과 변수의 집합이라고 먼저 잡으면 됩니다. 뒤 절에서는 `샘플`, `특징`, `기준선`, `출력 구조`를 한 번에 펼쳐 보겠지만, 여기서는 아직 그 용어를 길게 정의하기보다 왜 저장 구조만으로는 그 단계에 바로 들어갈 수 없는지를 먼저 고정합니다.
