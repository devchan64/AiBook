...
여기서 먼저 붙잡아야 하는 관점은 [데이터 형식 정합성(format consistency)](../../../reference/concept-glossary-parts/14-hieut.md#glossary-format-consistency)과 [데이터 품질의 첫 점검(data quality check)](../../../reference/concept-glossary-parts/03-digeut.md#glossary-data-quality-check)입니다. 형식 정합성은 같은 대상을 가리키는 키가 같은 형식으로 적혀 있는지, 시간 열이 실제 순서를 읽을 수 있는 형태인지, 같은 의미의 값이 열마다 다른 단위나 문자열 규칙으로 섞여 있지 않은지를 먼저 보는 일입니다. 첫 품질 점검은 그다음 단계로, 빠진 값, 끊긴 순서, 중복 행, 묶이지 않는 고아 행처럼 비교 구조를 바로 무너뜨리는 문제가 있는지를 초기에 확인하는 일입니다.
