...
즉 이 네 요소는 따로 외울 용어 목록이 아니라, 데이터셋 후보를 만드는 앞에서 뒤로 이어지는 설계 순서입니다. 샘플이 흔들리면 특징도 흔들리고, 특징이 흔들리면 기준선 비교도 흔들리고, 비교가 흔들리면 출력 구조도 흔들립니다. 뒤에서 Part 3 전체를 더 자세히 풀더라도, 이 기본 연결은 계속 유지됩니다.

이 네 요소는 정의 목록보다 설계 순서로 읽을 때 더 분명해집니다. 실제로는 아래 순서로 질문이 이어집니다.
