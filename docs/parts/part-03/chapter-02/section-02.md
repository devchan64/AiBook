# 3.4 샘플, 특징, 기준선, 출력 구조를 한눈에 보기

> Section ID: `P3-2.2`
> Version: `v2026.07.06`

이 Part에서는 몇 개의 핵심 용어가 반복해서 등장합니다. `샘플(sample)`, `특징(feature)`, `기준선(baseline)`, `출력 구조(output structure)`가 그것입니다. 이 용어들은 각각 따로 외우는 목록보다, 하나의 연결 구조로 읽어야 더 정확합니다. 무엇을 한 건의 샘플로 볼지 정해야 특징을 만들 수 있고, 특징이 있어야 무엇을 기준선과 비교할지 정할 수 있으며, 그 비교가 있어야 어떤 출력 구조를 만들 것인지도 결정할 수 있습니다.
...
이 네 질문은 각각 샘플, 특징, 기준선, 출력 구조에 대응합니다. 그래서 용어가 흐릿더라도 질문 순서를 따라가면 현재 어떤 설계 단계에 있는지 다시 확인할 수 있습니다.
...
여기서 먼저 정리해야 할 관계는 다음과 같습니다. `샘플을 정해야 특징이 생기고, 특징을 정해야 비교 구조가 생기고, 비교 구조가 생겨야 출력 구조가 정리된다.` 이 흐름은 뒤 절에서 계속 반복됩니다.
