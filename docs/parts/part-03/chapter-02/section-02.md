...
초심자에게는 이 지점이 가장 헷갈릴 수 있습니다. `표가 이미 있는데 왜 데이터셋이 아니지?`라는 질문이 자연스럽기 때문입니다. 이때는 `파일이 있다`와 `비교 가능한 샘플 표가 있다`를 분리해서 생각하면 됩니다.
...
처음 표를 다시 만들 때는 아래처럼 아주 짧은 순서로 움직여도 충분합니다.
...
이 네 줄을 적고 나면 `지금 하는 일이 저장 구조 확인인지, 데이터셋 설계인지`가 훨씬 분명해집니다.
