...
在 P2-1.1，我们把数学看成阅读 AI 计算的语言。接下来看看这种语言在实际学习中起什么作用。学习 AI 时，公式（formula）、代码（code）、数据（data）并不是彼此分开的东西。它们以三种不同的形式呈现同一个计算。

1. 公式会压缩计算结构。
2. 代码会执行计算步骤。
3. 数据提供参与计算的值以及供检查的结果。
...
例如，句子 `"我重新学习 AI"` 可以变成 token、数字 ID 和向量；图片可以继续变成像素（pixel）、数值数组（array）、张量（tensor）；表格数据则会基于行和列进入数组、矩阵或 DataFrame 的形式。
...
- 公式会表达模型输出、损失和更新方向
- 代码会按真实顺序执行这些计算
- 结果通过性能指标（metric）、损失（loss）和预测（prediction）来查看

这就是为什么 Part 2 会把数学和 Python 放在一起看。只看公式时，执行不会显现；只看代码时，计算意图可能变得模糊；只看数据时，又很难看见模型到底学到了什么。

## 数据个数与平均值的分母

四名学生的分数 `[55, 65, 80, 90]`，总和为 `290`，个数为 `4`，所以平均值是 `72.5 分`。对于这组数据，`sum(scores) / 4` 和 `sum(scores) / len(scores)` 的结果相同。

如果再加入第五名学生的分数 `100`，会怎样？分数总和变为 `390`，学生人数变为 `5`。

| 计算方法 | 代码表达式 | 用五名学生的分数计算的结果 |
| --- | --- | --- |
| 继续除以最初的人数 4 | `sum(scores) / 4` | `390 / 4 = 97.5` |
| 除以当前的学生人数 | `sum(scores) / len(scores)` | `390 / 5 = 78` |

五名学生的平均分是 `78 分`。`97.5` 是把五个分数相加，却仍除以四个人得到的值。代码执行了除法，但结果并不是这组数据的平均值。

平均值公式中的分母 `n` 是参与求和的数值个数。代码中的 `len(scores)` 从当前分数列表中取得这个个数。增加数据时，总和与个数都必须随之改变，才能在代码中保持公式的含义。
