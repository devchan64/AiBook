...
Part 2 안에서는 이 절을 `수식(formula)`, `코드(code)`, `데이터(data)`, `출력(output)`, `shape`를 함께 읽는 대표 상세 설명 위치로 사용합니다. 1.1에서 수학을 계산 언어로 보는 이유를 먼저 잡았다면, 여기서는 그 계산 언어가 `문서, 코드 셀, 실제 값` 안에서 어떻게 같은 계산을 가리키는지 정리합니다.

## 이 절의 범위

이 절은 수식 표기법 전체를 설명하지 않습니다. 변수(variable), 함수(function), 시그마(sigma), 극한(limit)은 Part 2 Chapter 2에서 다시 다룹니다. 벡터(vector), 행렬(matrix), 배열(array)은 Part 2 Chapter 3과 Part 2 Chapter 11에서 더 자세히 봅니다.
...
이 절은 Part 2 전체의 `수식과 Python을 왜 같이 보는가` 손잡이입니다. 개별 기호와 배열 계산 세부는 뒤 절에서 회수됩니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 수식-코드-데이터 연결 | 같은 계산이 왜 문서, 코드, 데이터에서 다른 모양으로 보이는가? | P2-1.2 |
| 수식 표기 읽기 | 시그마, 변수, 함수 표기는 무엇을 압축해서 보여 주는가? | Part 2 Chapter 2 |
| 벡터와 배열 계산 | 데이터의 모양이 바뀌면 코드와 계산 방식이 어떻게 달라지는가? | Part 2 Chapter 3, Part 2 Chapter 11 |
...
이 절은 수학과 프로그래밍이 만나는 지점을 보여 주는 절입니다. 아래 세 가지 관점이 먼저 잡혀야 합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 같은 계산도 `수식`, `코드`, `데이터`로 다르게 보일 수 있다는 점 | 그래야 표현이 달라도 같은 내용을 읽고 있다는 감각이 생깁니다. | 평균 하나를 세 방식으로 설명합니다. |
| 수식은 `계산 구조를 압축`하고 코드는 `실행 순서를 펼친다`는 점 | 문서와 예제 코드가 왜 서로 다르게 생기는지 이해하게 해 줍니다. | 무엇을 더하고 나누는지, 코드에서 어떤 순서로 실행하는지 읽습니다. |
| 결과는 숫자만 보는 것이 아니라 `문맥에서 다시 해석`해야 한다는 점 | 머신러닝 지표와 손실을 읽을 때 바로 연결됩니다. | 평균 2.5가 무엇을 뜻하는지 한 문장으로 설명합니다. |
...
평균이 2.5라면, 값들의 중심이 대략 2.5 근처에 있다는 뜻입니다.
...
또한 이 절은 `NumPy와 배열 API 사용법 전체`를 설명하는 절도, `손실 함수와 gradient 계산의 세부 구현`을 다루는 절도 아닙니다. 여기서는 수식, 코드, 데이터가 만나는 자리를 먼저 잡고, 배열 계산과 모델 수식은 뒤 Chapter에서 다시 좁혀 봅니다.
