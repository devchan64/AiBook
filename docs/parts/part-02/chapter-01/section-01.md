...
| 개별 수학 주제 | 시그마, 벡터, 미분, 확률, 최적화가 각각 무엇을 설명하는가? | `P2-2.1`, `P2-3.1`, `P2-4.1`, `P2-5.1`, `P2-6.1` |
...
이 장의 핵심은 `AI에 무엇이 있나`에서 `그 AI 설명을 어떤 계산 언어로 읽을까`로 질문이 바뀐다는 점입니다.
...
- 수학을 AI 문해력 도구가 아니라 시험 과목처럼만 다시 보고 있을 때
- 손실, 확률, 벡터, 최적화가 왜 한 파트에 같이 나오는지 연결이 끊길 때
- 수식을 읽지 못해 모델, 데이터, 학습 설명이 한꺼번에 막히기 시작할 때
