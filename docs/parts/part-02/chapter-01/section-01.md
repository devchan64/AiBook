...
즉, 지금 수학을 다시 보는 이유는 `문제를 풀기 위한 시험 준비`보다 `Part 3에서 X, y, loss, metric 같은 표현을 읽기 위한 문해력 복구`에 가깝습니다. 이 기준을 먼저 잡아 두면 뒤에서 벡터, 미분, 확률, 최적화가 왜 한 파트에 묶이는지도 덜 낯설게 읽힙니다.
...
처음 읽을 때는 이 절을 Part 2 전체의 `수학을 왜 다시 읽는가` 손잡이로만 잡고, 개별 개념 세부는 뒤 절에서 회수된다고 생각하면 흐름이 덜 끊깁니다.
...
| 개별 수학 주제 | 시그마, 벡터, 미분, 확률, 최적화가 각각 무엇을 설명하는가? | P2-2, P2-3, P2-4, P2-5, P2-6 |
| 머신러닝 본편 연결 | 이 계산 언어가 X, y, loss, metric, gradient를 읽는 데 어디서 다시 쓰이는가? | Part 3 전반 |

이 전환을 Part 1과 Part 3까지 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 Part | 지금 장 | 바로 다음에 더 붙는 Part/장 |
| --- | --- | --- |
| Part 1: AI 전체 지도를 본다 | Part 2 수학 입구: 그 지도를 읽는 계산 언어를 복구한다 | Part 3: X, y, loss, metric, gradient를 실제 모델 판단에 붙인다 |
...
수학이라는 제목만 보고 겁먹기 쉽지만, 이 절에서 먼저 필요한 것은 증명이 아니라 역할 구분입니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 수학은 `정답 시험`이 아니라 `계산 언어`라는 점 | 그래야 수식을 모두 풀지 못해도 문서를 읽는 출발점을 잡을 수 있습니다. | 수식이 계산 구조를 줄여 적는 도구라는 점만 먼저 잡으면 충분합니다. |
| 수학은 `데이터의 모양`과 `학습의 방향`을 설명한다는 점 | 벡터, 행렬, 손실, 그래디언트가 왜 같은 파트에 나오는지 연결됩니다. | 무엇을 표현하고 무엇을 줄이거나 바꾸는지 보이면 충분합니다. |
| 수식은 결국 `코드와 결과`로 다시 확인해야 한다는 점 | AI 학습에서는 수식만 읽고 끝나지 않기 때문입니다. | 작은 코드가 어떤 계산을 확인하는지 읽을 수 있으면 충분합니다. |
...
예를 들어 예측값과 실제값의 차이를 제곱해서 더하는 식은, 모델이 얼마나 틀렸는지를 한 숫자로 모으는 계산 구조를 짧게 적은 것입니다. 이때 중요한 것은 증명을 바로 따라가는 일이 아니라, `입력은 무엇인가`, `비교 대상은 무엇인가`, `최종적으로 줄이려는 값은 무엇인가`를 먼저 푸는 일입니다.
