...
P2-13장에서는 Matplotlib으로 그래프를 만들고, 그 출력 이미지를 문서에 연결했습니다. 여기서 바로 문제가 생깁니다. 문서, 코드, 이미지, 조사 메모가 함께 바뀌면 나중에 “무엇을 왜 바꿨는지”를 기억하기 어렵습니다.
...
처음 읽을 때는 바로 다음 Part 3에서 어떤 기록이 늘어나는지만 먼저 잡아도 충분합니다.
...
이 문서 프로젝트는 단순한 완성 문서가 아니라 학습 과정의 결과입니다. 원고, 조사 메모, 예제 코드, 이미지, 배포 설정이 함께 바뀝니다. 특히 Part 3부터는 같은 데이터라도 전처리, baseline, threshold, 평가표가 함께 바뀌기 시작하므로, Git은 `최종 답 보관함`보다 `실험 비교 기록`에 더 가깝게 읽는 편이 정확합니다.
