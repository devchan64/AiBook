...
| branch workflow | 그 기록을 어떤 작업선과 배포 기준으로 나눌 것인가 |
...
| --- | --- | --- |
| 어떤 코드, 표, 그래프 변경을 한 단위 기록으로 묶을지 정한다 | 그 기록을 어떤 브랜치에 두고 언제 배포 기준으로 넘길지 정한다 | baseline 비교, 전처리 수정, metric 해석을 같은 실험 맥락으로 다시 설명한다 |
...
이 책은 단순한 완성 문서가 아니라 학습 과정의 결과입니다. 원고, 근거 메모, 예제 코드, 이미지, 배포 설정이 함께 바뀝니다. 특히 Part 3부터는 같은 데이터라도 전처리, baseline, threshold, 평가표가 함께 바뀌기 시작하므로, Git은 `최종 답 보관함`보다 `실험 비교 기록`에 더 가깝게 읽는 편이 정확합니다.
...
이때 Git은 단순 저장소보다 `변경 이유를 묶는 기록`으로 작동합니다. 원고 Markdown, 이미지 생성 코드, 출력 이미지, 목차 수정이 모두 `P2-13.3 그래프 설명 보강`이라는 하나의 목적에 속한다면, 그 목적을 커밋 메시지와 함께 남길 수 있습니다.
