...
- 파일을 저장하는 것과 작업 이력을 남기는 것이 왜 다른지 설명해야 할 때 Git 관점을 먼저 떠올립니다.
- 무엇을 바꿨는지, 왜 바꿨는지, 어떤 단위로 묶어 기록할지 판단해야 할 때 이 절의 변경 이력 감각으로 돌아옵니다.
- 문서·코드·그래프를 함께 다루는 학습 프로젝트에서 재현 가능한 기록을 남기려 할 때 다시 확인합니다.
