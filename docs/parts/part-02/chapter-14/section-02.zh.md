...
本节不讨论分支的内部实现、复杂合并策略、冲突解决或 rebase。也不深入讨论 GitHub Pages 的具体设置。
...
- 分支是分离工作流的有名字历史。
- 在文档项目里，写作分支和部署分支可以分开运作。
- 提交应该是有意义的变更组合，而不只是文件组合。
- 只有当正文、代码、图片、调查笔记和部署目录彼此对齐时，文档可复现性才会成立。
- 反映到部署分支可能直接导致公开部署，因此需要单独判断。

这一节不是为了多背 Git 命令，而是为了决定：前面几节里的计算与解释，应该按什么标准留下来。

| 前面章节做出的东西 | 本节负责什么 | 本节还不做什么 |
| --- | --- | --- |
| 数组计算、表格确认、图表解读 | 决定哪些变更算作一个提交单位，以及应留在哪个分支上 | 复杂合并策略、冲突解决、高级协作流程 |

## 简短回看表

| 卡住的场景 | 先回到哪里 |
| --- | --- |
| Git 为什么需要仍然模糊 | `P2-14.1` |
| 可复现性与依赖关系之间的连接模糊 | `P2-7.5`, `P2-10.3` |
| 为什么笔记本/图表/正文要一起移动不够清楚 | Chapter 10, Chapter 13 |
...
- 当需要分开管理“写作中的变更”和“可部署的变更”时，先想起分支和提交单位的视角。
- 当你要判断一个提交该包含哪些文件、以及在什么时点应该移入部署分支时，回到本节。
- 当你需要重新确认“文档可复现性不只包括正文，还包括目录、资源和构建状态”时，本节会再次成为标准。
