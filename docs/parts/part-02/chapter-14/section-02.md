...
여기서는 `브랜치(branch)`, `배포(deployment)`, `작업 흐름(workflow)`, `정적 배포(static deployment)`, `문서 재현성(document reproducibility)`의 기본 구분을 설명합니다. `Git`과 커밋 단위 자체의 대표 설명은 P2-14.1과 [개념사전](../../../reference/concept-glossary.md)에 두고, 여기서는 그 기록을 어떤 작업선과 공개 기준으로 나눌지 설명합니다.
...
- 작성 중인 변경과 배포 가능한 변경을 분리해 관리해야 할 때 브랜치와 커밋 단위 관점을 먼저 떠올립니다.
- 한 커밋에 어떤 파일을 넣어야 하는지, 어떤 시점에 배포 브랜치로 옮겨야 하는지 판단할 때 이 절로 돌아옵니다.
- 문서 재현성이 본문만이 아니라 목차, 자산, 빌드 상태까지 포함한다는 점을 다시 확인해야 할 때 이 절이 기준이 됩니다.
