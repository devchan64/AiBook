...
여기서는 `브랜치(branch)`, `배포(deployment)`, `작업 흐름(workflow)`, `정적 배포(static deployment)`, `문서 재현성(document reproducibility)`의 기본 구분을 설명합니다. `Git`과 커밋 단위 자체의 대표 설명은 P2-14.1과 [개념사전의 브랜치 항목](../../../reference/concept-glossary-parts/06-bieup.md#branch)에 두고, 여기서는 그 기록을 어떤 작업선과 공개 기준으로 나눌지 설명합니다.
