...
Part 2 Chapter 14에서는 `브랜치(branch)`, `배포(deployment)`, `작업 흐름(workflow)`, `정적 배포(static deployment)`, `문서 재현성(document reproducibility)`의 대표 설명 위치를 이 절에 둡니다. 앞 절이 변경 기록 단위를 설명했다면, 그 기록을 어떤 작업선과 공개 기준으로 나눌지는 여기 설명을 기준으로 다시 연결합니다.
...
이 표의 핵심은 Git 장을 `명령어 목록`으로 읽지 않는 데 있습니다. 여기서는 저장소를 더 복잡하게 다루는 법보다, `어떤 변경을 아직 쓰는 중으로 둘 것인가`, `어떤 변경만 공개 기준으로 올릴 것인가`를 구분하는 감각을 먼저 잡아야 합니다.
