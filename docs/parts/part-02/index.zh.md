...
Part 2 是在重新学习机器学习与深度学习之前，恢复数学、Python、数据工具与文档管理感的一段。这里不会深入证明数学，也不会要求把所有 Python 语法全部背下来。相反，它要建立的是：让你在 Part 3 中阅读模型训练、数据切分、评估、过拟合与泛化时，能够跟上说明，并用小段代码亲自确认的基础。

在同一个 Part 内，Part 2 也尽量把主要概念的详细说明先放在一个代表 Section 中。之后的章节只保留当前语境所需要的最小连接。像公式阅读、向量与矩阵、导数、概率、优化、数组与表格式数据这些会反复再遇到的概念，应先读代表 Section，再在重新出现时结合 [概念词汇表](/AiBook/en/reference/concept-glossary/) 回头确认。

这里最核心的目的，是 `让 AI 文档与示例代码里反复出现的计算语言重新变得可读`。公式、数组、表、图、运行环境、Git 历史并不是彼此分离的话题。它们是在读取模型计算、检查数据集、可视化结果、留下学习记录时一起被使用的工具。

读这一 Part 时，要不断确认诸如 `这个公式在计算什么`、`这段代码放进了什么值，又在验证什么结果`、`这个工具为什么现在需要` 这样的问题。数学、Python、NumPy、Pandas、Git 会连成一组共同准备，服务于 Part 3 的机器学习正文阅读。
...
当人们在重新学习 AI 时会卡住，原因往往不只是数学本身难，而是 \(x\)、\(\sum\)、\(\frac{1}{n}\)、向量、矩阵、导数、梯度、概率、平均值、损失这些表达，会同时出现在代码、数据与模型说明中。
...
| 导数、chain rule、概率、优化 | `P2-4.1`, `P2-4.6`, `P2-5.1`, `P2-6.1` | 为了读懂损失、梯度、反向传播、不确定性和学习方向 |

如果分别学习 Python、NumPy、Pandas、Matplotlib、Git，它们也会散开。但在机器学习语境里，它们会连成一条工作流：用 Python 执行小计算，用 NumPy 处理向量和矩阵，用 Pandas 读取表格式数据，用 Matplotlib 观察数据和学习流程，再用 Git 留下文档、代码和实验记录的变更历史。

Part 2 恢复的正是这种连接。目标不是专家级数学证明，也不是软件熟练度，而是让你在阅读 Part 3 机器学习说明时，能够跟得上 `这个计算到底在做什么`。
...
| 如果卡住，就先短暂绕回补充学习 | 像不同操作系统安装、终端、class、传统数据结构这类超出当前范围的内容，只在补充学习里按需要回收 |
...
| 如果在这一节卡住，应该短暂回到哪里？ | 让你不必从头把 Part 2 全部重读一遍 |
...
- 能通过行(row)、列(column)、索引(index)阅读 Pandas DataFrame，并形成数据集准备的直觉。
...
这并不是回避解释，而是范围控制。Part 2 的责任是 `让计算语言重新变得可读`，而高级工具用法与更严格的数学证明留给后续学习或单独参考资料。对于搜索空间中组合数迅速增长的直觉，先通过 P1-7.1 的表格与例子抓住，这里不再另开一个组合论 Section 重新展开。
...
| --- | --- | --- |
| 公式、平均、误差、梯度 | 因为要读懂 Part 3 中损失和评估的说明 | Part 3 |
...
后半部分处理数据结构与数据工具。它建立 array、table、tree、graph 的直觉，再看如何通过 Jupyter 与 Colab notebook 同时留下代码与说明。它用 NumPy 计算向量与矩阵，用 Pandas 处理表格式数据，用 Matplotlib 检查数字的形状。
...
要读懂这种说明，需要几个基础同时到位。你需要能够把 \(X\) 和 \(y\) 读成数组与表，把损失读成平均与求和，能通过图去检查学习过程，还要能追踪：示例代码在什么环境里跑，结果图和原稿在哪个 commit 里一起发生了变化。

Part 2 不会把每一项都挖得过深。相反，它只准备最小语言与实践直觉，让你在 Part 3 阅读模型与数据流时不会直接被卡住。

## 与前一 Part 的连接

如果 Part 1 建立了 AI 的总体地形和共享术语，那么 Part 2 就是把那些术语第一次转换成实际计算语言的阶段。

- Part 1 中 data、model、learning、model execution 的区分，会再次以公式与代码的形式出现。
- Part 1 中像 prompt、embedding、RAG 这样的词，最终也都建立在 arrays、vectors、tables、runtime environments 之上。

换句话说，Part 2 不是丢掉地图再进入新科目的 Part，而是把那张地图搬到实际计算记号与执行工具上的 Part。

## 与后续 Part 的连接

当 Part 2 结束后，下面这些问题会自然连到 Part 3 的机器学习。

- 输入 \(X\) 和标签 \(y\) 实际上应该怎样读？
- 损失与评估指标在不同问题里会怎样变化？
- 为什么数据切分与泛化很重要？

所以 Part 2 不是把工具与基础学完的 Part，而是搭建起读取机器学习说明所需共同地板的 Part。
...
因为 Part 2 专注于基础恢复，所以它有意把以下问题留给 Part 3 及之后。

- 损失与优化在真实模型训练中是怎样连接起来的？
- 为什么需要 train、validation、test 的切分？
- 为什么神经网络和深度学习结构需要更大的计算资源？

这些问题会在 Part 3、Part 4、Part 5 的实际正文里再回收。

## 与下一 Part 连接的视角

在下一 Part 中，最重要的是 `读取问题、切分数据、设定评估标准的共同结构`。

Part 2 中恢复的公式、数组、表、图、Git 直觉，会在 Part 3 中再次绑成 `problem definition -> data split -> learning -> evaluation` 这条流程。

在进入 Part 3 之前，要用 `能不能现在就试一次` 的标准检查如下内容。

| Part 3 之前可以马上做的小检查 | 为什么重要 |
| --- | --- |
| 看一个小表并说出哪一列是 feature，哪一列是 label | 因为这会成为机器学习问题定义的起点 |
| 把 `X` 和 `y` 写成一个非常小的数组或表 | 为了确认公式与代码指向的是同一个对象 |
| 直接用 Python 输出看一次平均值、误差或差值 | 因为这是跟上评估指标与损失说明的最小直觉 |
| 改变数组的 `shape` 和 `axis` 并说明差别 | 为了不把数据方向和计算方向混淆 |
| 想象一个代码与记录一起变化的 Git commit | 因为实验与文档的可复现性必须一起留下 |
...
- 能说明公式里的变量、函数、sigma、平均值、误差怎样连到具体计算步骤。
- 能说出向量与矩阵在数据和模型计算中出现在哪里。
- 能用入门水平解释导数、梯度、损失函数、梯度下降。
- 能把概率、分布、平均值、方差、样本、估计区分成数据判断语言。
...
- 能用 Matplotlib 检查函数形状、散点图、直方图、损失曲线。
- 能从管理文档、代码、图片、实验记录的变更历史角度说明 Git commit 与 branch。
- 在进入 Part 3 之前，能说出 `X`、`y`、sample、feature、fit、predict 的基本含义。

## 简短检查

- 能说明公式、数组、表、图、Git 都会汇入 Part 3 的同一条学习流程吗？
- 能看到 `一个现在卡住的表达` 后，选出自己应回到哪一章吗？
- 能各用一句话解释 `X`、`y`、shape、mean、error、runtime environment 吗？

## 来源与参考资料

本文是整理 Part 2 目的与学习路径的内部概览，没有直接引用外部资料。
