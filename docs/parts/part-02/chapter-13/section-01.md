...
- 표만으로는 변화, 관계, 분포, 이상값이 잘 보이지 않아 시각적으로 다시 확인해야 할 때 그래프 관점을 먼저 떠올립니다.
- 어떤 질문에는 표보다 그래프가 더 직접 답해 준다는 점을 설명해야 할 때 이 절의 비교 기준으로 돌아옵니다.
- 그래프를 볼 때 축, 점, 범위, 해석 과잉 여부를 함께 점검해야 할 때 다시 확인합니다.
