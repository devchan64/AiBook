...
| --- | --- | --- |
| loss가 줄어드는지, 값이 튀는지, 관계가 보이는지 | 어떤 그래프를 봤는지, 무엇이 예상과 달랐는지, 어떤 코드를 바꿨는지 | Part 3에서는 결과를 본 뒤 다시 비교하고 설명해야 하기 때문 |
| 분포가 한쪽으로 몰리는지 | 어떤 전처리 뒤에 분포가 달라졌는지 | 그래프 해석이 실험 판단과 연결되기 때문 |
| 두 변수 관계가 기대와 비슷한지 | 그 관계를 확인한 데이터 버전과 커밋 맥락 | 나중에 같은 결과를 다시 재현해야 하기 때문 |
...
| --- | --- | --- |
| Pandas: 어떤 사례와 변수를 표로 읽을까 | Matplotlib: 그 표에서 바로 안 보이는 변화와 관계를 어떤 모양으로 확인할까 | Git: 그 그래프를 보고 어떤 판단을 남기고 어느 변경을 기록할까 |
