...
| --- | --- | --- |
| 그래프가 표에서 바로 안 보이는 변화와 관계를 드러내는 도구라는 점 | Part 2 Chapter 14에서 그 그래프를 보고 어떤 변경 이유와 해석을 기록할지로 이어집니다. | 이후 학습 곡선, 오류 분포, 변수 관계 해석에서 반복됩니다. |
...
이 절은 Matplotlib의 모든 기능을 설명하지 않습니다. 기본 차트 선택은 다음 P2-13.2에서, 여러 그래프 배치와 저장은 P2-13.3에서 다시 다룹니다. 다만 색상, 스타일, 출판용 레이아웃, 고급 시각화 설계는 현재 본편 범위 밖에 둡니다.
