...
이 절은 Matplotlib의 모든 기능을 설명하지 않습니다. 기본 차트 선택은 다음 P2-13.2에서, 여러 그래프 배치와 저장은 P2-13.3에서 다시 다룹니다. 다만 색상, 스타일, 출판용 레이아웃, 고급 시각화 설계는 이 책의 현재 본편 범위 밖에 둡니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 왜 표만으로 충분하지 않은가? | 숫자 목록만으로는 보이지 않는 모양과 경향이 있기 때문입니다. |
| 그래프는 무엇을 잘 보여 주는가? | 변화, 비교, 분포 같은 질문을 더 빨리 읽게 해 줍니다. |
| 무엇을 먼저 정해야 하나? | 차트 종류보다 먼저 어떤 질문을 할지 정해야 합니다. |
...
확인 가능한 결과는 같은 데이터를 선 그래프로 그렸을 때 더 분명해집니다. 손실이 대체로 내려가는지, 특정 epoch에서 흔들리는지, 값이 어디쯤 몰려 있는지가 표보다 빠르게 보인다면 그래프를 그릴 이유가 충분합니다.
