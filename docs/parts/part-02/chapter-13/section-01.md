...
Part 2 Chapter 13에서는 `그래프(plot)`, `Figure`, `Axes`, `분포(distribution)`, `이상값(outlier)`의 대표 설명 위치를 이 절에 둡니다. 뒤 절에서 기본 차트 선택과 저장 흐름을 다시 보더라도, 그래프가 표에서 무엇을 드러내는지는 여기 기준으로 다시 연결합니다.
