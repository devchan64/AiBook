...
지금 읽는 층위는 `시각적 확인 층위`입니다. 앞 장의 Pandas가 `그 숫자 묶음이 어떤 사례와 변수의 표로 읽혀야 하는가`를 다뤘다면, 여기서는 그 표에서 바로 안 보이는 변화와 관계를 어떤 모양으로 확인할지 읽습니다. 바로 다음 Git 장에서는 이 그래프를 보고 무엇을 바꾸고 어떤 해석을 기록으로 남길지로 질문이 다시 이동합니다.

처음 읽을 때는 차트 종류를 많이 외우기보다 `지금 보고 싶은 것이 변화인가 관계인가 분포인가`, `표에서 안 보이던 모양이 그래프로 드러났는가`, `바로 다음 장에서 무엇을 판단과 기록으로 남길 것인가`라는 세 질문만 먼저 붙잡으면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 그래프가 표에서 바로 안 보이는 변화와 관계를 드러내는 도구라는 점 | P2-14에서 그 그래프를 보고 어떤 변경 이유와 해석을 기록할지로 이어집니다. | 이후 학습 곡선, 오류 분포, 변수 관계 해석에서 반복됩니다. |
| 그래프를 그리는 일과 해석을 남기는 일이 연결되어 있다는 점 | 어떤 차트를 봤고 무엇이 예상과 달랐는지 기록하는 기준으로 이어집니다. | 이후 실험 비교, 리포트 작성, 프로젝트 회고에서 중요합니다. |
| Matplotlib이 `계산 -> 표 -> 그래프 -> 기록` 흐름의 마지막 확인 도구라는 점 | Git 장을 개발자 부록이 아니라 해석 기록 도구로 읽는 기준으로 이어집니다. | Part 3 이후 모델 비교와 실험 결과 설명의 출발점이 됩니다. |

즉, Part 2 후반 흐름을 가장 짧게 묶으면 `배열로 계산하고 -> 표로 확인하고 -> 그래프로 모양을 보고 -> 다음 장에서 그 변경과 해석을 기록으로 남긴다`입니다. 그래서 Matplotlib은 별도 시각화 취미가 아니라, 바로 다음 Git 장과 Part 3의 실험 해석을 준비하는 마지막 확인 도구로 읽는 편이 안전합니다.

지금 이 절에서 특히 붙잡아야 할 것은 `그래프를 그렸다`에서 끝나지 않는다는 점입니다.

| 지금 보는 것 | 바로 다음에 남겨야 하는 것 | 왜 이어서 필요한가 |
| --- | --- | --- |
| loss가 줄어드는지, 값이 튀는지, 관계가 보이는지 | 어떤 그래프를 봤는지, 무엇이 예상과 달랐는지 | Part 3에서는 결과를 본 뒤 다시 비교하고 설명해야 하기 때문 |
| 분포가 한쪽으로 몰리는지 | 어떤 전처리 뒤에 분포가 달라졌는지 | 그래프 해석이 실험 판단과 연결되기 때문 |
| 두 변수 관계가 기대와 비슷한지 | 그 관계를 확인한 데이터 버전과 기록 맥락 | 나중에 같은 결과를 다시 재현해야 하기 때문 |

즉, Matplotlib 절의 역할은 `모양을 빨리 본다`에서 끝나지 않고, 바로 다음 Git 절에서 `무엇을 보고 어떤 판단을 남길 것인가`로 이어지는 준비까지 포함합니다.

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| Pandas: 어떤 사례와 변수를 표로 읽을까 | Matplotlib: 그 표에서 바로 안 보이는 변화와 관계를 어떤 모양으로 확인할까 | Git: 그 그래프를 보고 어떤 판단과 변경 이유를 남길까 |
| 표 해석 구조 | 시각적 확인 | 기록과 재현성 |

즉, 지금 장의 핵심은 `표를 읽는가`에서 `그 표의 변화와 관계를 눈으로 확인하는가`로 손잡이가 바뀐다는 점입니다.
...
확인할 개념: 표는 값 자체를, 그래프는 변화의 모양을 더 빨리 드러내며 학습 손실처럼 순서가 있는 값은 선 그래프로 읽는 편이 자연스럽습니다.
...
| --- | --- | --- |
| NumPy 배열과 Pandas 표로 확인한 숫자 | 숫자의 모양과 변화, 관계를 그래프로 빠르게 읽는다 | 어떤 그래프를 보고 무엇을 바꿨는지 Git 기록과 배포 기준으로 남긴다 |
