...
여기서 한 걸음만 더 가 보면, 숫자 요약이 같아 보여도 그래프 모양은 다를 수 있습니다. 예를 들어 두 동작 기록의 평균이 모두 `1.5`여도, 한쪽은 초반부터 끝까지 거의 평평하고 다른 한쪽은 중간에 크게 올랐다가 다시 내려올 수 있습니다. 표의 평균만 보면 둘이 비슷해 보이지만, 선 그래프로 그리면 `같은 평균, 다른 패턴`이 바로 드러납니다.
...
즉, 그래프는 숫자를 예쁘게 다시 쓰는 도구가 아니라, 표의 요약값 하나로는 놓치기 쉬운 움직임의 모양을 다시 보게 만드는 도구입니다.
