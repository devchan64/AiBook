...
여기서는 `선 그래프(line plot)`, `산점도(scatter plot)`, `히스토그램(histogram)`, `손실 곡선(loss curve)`의 기본 구분을 설명합니다. `그래프(plot)` 자체의 역할과 `Figure`, `Axes`의 대표 설명은 P2-13.1과 [개념사전의 라인 플롯 항목](../../../reference/concept-glossary-parts/04-rieul.md#line-plot)에 두고, 여기서는 어떤 질문에 어떤 기본 차트를 먼저 고를지 설명합니다.
