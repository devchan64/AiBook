...
Part 2 Chapter 13에서는 `선 그래프(line plot)`, `산점도(scatter plot)`, `히스토그램(histogram)`, `손실 곡선(loss curve)`의 대표 설명 위치를 이 절에 둡니다. 앞 절이 그래프가 무엇을 드러내는지 설명했다면, 어떤 질문에 어떤 기본 차트를 먼저 고를지는 여기 기준으로 다시 연결합니다.
