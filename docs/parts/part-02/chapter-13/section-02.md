...
여기서는 `선 그래프(line plot)`, `산점도(scatter plot)`, `히스토그램(histogram)`, `손실 곡선(loss curve)`의 기본 구분을 설명합니다. `그래프(plot)` 자체의 역할과 `Figure`, `Axes`의 대표 설명은 P2-13.1과 P2-13.2에 두고, 여기서는 어떤 질문에 어떤 기본 차트를 먼저 고를지 설명합니다.
...
```python
# 선, 산점도, 막대그래프와 수식 곡선을 그려 질문에 맞는 차트 모양을 확인하는 예제입니다.
...
## 기본 차트와 수식의 모양 확인: 확인할 판단 기준

이 사례에서는 기본 차트로 수식과 데이터의 모양을 확인하게 하는지 확인한다.
