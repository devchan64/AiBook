...
| --- | --- | --- |
| 변화, 관계, 분포 중 무엇을 보고 싶은지에 따라 차트를 고르는 기준 | Git에서 이 해석 변경과 그림 선택을 어떤 기록으로 남길까 | 손실 곡선, 변수 관계, 오류 분포를 보고 다음 실험이나 설명을 바꿀 때 |
...
- 선 그래프, 산점도, 히스토그램 같은 기본 차트가 각각 어떤 질문에 답하는지 구분해야 할 때 이 절을 먼저 떠올립니다.
- 수식의 모양, 점 하나의 의미, 분포의 생김새를 실제 그림으로 확인해야 할 때 기본 차트 예시로 돌아옵니다.
- 그래프를 만들며 축 라벨과 제목을 왜 붙이는지 설명하거나, 손실 곡선처럼 학습 흐름을 읽으려 할 때 다시 확인합니다.
