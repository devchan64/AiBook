...
This section explains the basic distinctions in the compare-and-save flow, including `savefig`, `legend`, and `accuracy`. The representative explanation of `plot`, `Figure`, and `Axes` stays in P2-13.1, and the standards for choosing basic chart types stay in P2-13.2 and P2-13.2. Here, we focus on how to compare several plots together and leave the results as files.
