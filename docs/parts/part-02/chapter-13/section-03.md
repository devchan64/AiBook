...
Part 2 Chapter 13에서는 `savefig`, `범례(legend)`, `정확도(accuracy)`를 포함한 비교·저장 흐름의 대표 설명 위치를 이 절에 둡니다. 앞 절들이 그래프 해석과 기본 차트 선택을 다뤘다면, 여러 그래프를 함께 비교하고 결과를 파일로 남기는 기준은 여기 설명을 기준으로 다시 연결합니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 여러 그래프를 왜 같이 그리나? | 비교 질문을 더 분명하게 만들기 위해서입니다. |
| 같은 축과 다른 축은 언제 나누나? | 직접 비교가 중요하면 같이, 해석이 섞이면 나눕니다. |
| 저장은 왜 중요한가? | 결과뿐 아니라 나중에 다시 설명할 기록을 남기기 때문입니다. |
