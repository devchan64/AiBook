...
- 여러 그래프를 한 화면에서 나란히 놓고 비교해야 할 때 Figure와 Axes 배치를 먼저 떠올립니다.
- train/validation 곡선처럼 같은 종류의 지표를 한 축에서 비교해야 할 이유를 설명할 때 이 절로 돌아옵니다.
- 화면 표시와 파일 저장의 차이, 그리고 그래프 이미지 만으로는 재현성이 부족하다는 점을 다시 확인해야 할 때 이 절이 기준이 됩니다.
