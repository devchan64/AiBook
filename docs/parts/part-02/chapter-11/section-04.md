...
- 슬라이싱 결과가 원본을 공유하는지, 새 배열을 만드는지 헷갈릴 때 shape와 원본 공유 감각을 다시 떠올립니다.
- `(3,)`, `(3, 1)`, `(1, 3)`처럼 모양이 비슷해 보여도 계산 결과가 달라지는 이유를 점검할 때 이 보충 절이 기준이 됩니다.
- 값 수정이 원본까지 바꿔 버리는 문제나 broadcasting 전 준비 모양을 확인해야 할 때 다시 확인합니다.
