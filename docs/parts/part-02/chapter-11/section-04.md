...
Part 2 Chapter 11에서는 `불리언 마스크(boolean mask)`, `팬시 인덱싱(fancy indexing)`, `np.newaxis`, `원본 공유(shared view)`를 묶어 보는 대표 설명 위치를 이 절에 둡니다. 앞 절들의 인덱싱, 슬라이싱, 브로드캐스팅이 읽히더라도, 선택 방식과 shape 변화가 원본 공유와 어떻게 연결되는지는 여기 기준으로 다시 연결합니다.
...
이 보충학습을 읽는 시점도 분명합니다. P2-11.2에서 슬라이싱과 축은 알겠는데 `이 선택이 원본을 같이 바꾸는가`가 헷갈리거나, P2-11.3에서 broadcasting은 읽히는데 `왜 갑자기 (3, 1)`과 `(1, 3)`이 나오는지 멈춘다면 이 절로 잠깐 우회하면 됩니다. 반대로 이 절을 읽고 나면 다시 P2-11.2와 P2-11.3으로 돌아가 슬라이싱, 축, broadcasting을 `shape`와 `원본 공유`까지 함께 보는 편이 더 안정적입니다.

## 먼저 잡을 세 가지

| 질문 | 먼저 붙잡을 기준 |
| --- | --- |
| 무엇을 골랐는가? | 위치 범위를 자른 것인지, 특정 위치만 따로 모은 것인지 먼저 봅니다. |
| shape가 어떻게 바뀌는가? | `(3,)`, `(3, 1)`, `(1, 3)`은 같은 값이어도 계산 방식이 달라집니다. |
| 원본을 함께 보는가? | 뒤에서 값을 바꿀 가능성이 있으면 복사 여부를 먼저 점검합니다. |
...
확인할 개념: 위치를 따로 모으거나 조건으로 걸러 낸 결과는 새 배열처럼 읽는 편이 안전하다는 점을 봅니다.
...
이 절을 읽은 뒤에는 다음 본문으로 다시 연결해 읽는 편이 좋습니다.
...
- NumPy에서는 `어떻게 골랐는가`에 따라 원본 공유 여부가 달라질 수 있습니다.
- 기본 슬라이싱은 구간 보기, 팬시 인덱싱은 위치 모으기, 불리언 마스크는 조건 걸러내기로 먼저 읽으면 안전합니다.
