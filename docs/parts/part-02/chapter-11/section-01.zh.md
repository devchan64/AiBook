...
本节说明 `NumPy`、`shape`、`ndim`、`dtype` 的基本区分。这一章的重点不是背很多 NumPy 语法，而是学会在 AI 实践里把 vector、matrix、数据成组的形状读出来。以后再次遇到 array、axis、broadcasting 时，也可以把 [概念词汇表](/AiBook/en/reference/concept-glossary/) 当作回返点。
