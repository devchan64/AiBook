...
이 절에서는 고급 브로드캐스팅, `np.newaxis`, `reshape`의 세부 활용, stride, 메모리 뷰(view), 성능 벤치마크, GPU 텐서 연산은 깊게 다루지 않습니다. `np.newaxis`와 shape 변화는 P2-11.4 보충학습에서 다시 다루고, 여기서는 “shape이 맞으면 반복 계산을 배열 단위로 읽을 수 있다”는 감각을 만드는 데 집중합니다. 만약 broadcasting 예제는 읽히는데 `(3,)`, `(3, 1)`, `(1, 3)` 차이나 슬라이싱 뒤 원본 공유가 계속 헷갈린다면, 이 절을 잠깐 멈추고 P2-11.4로 이동한 뒤 다시 돌아옵니다.
...
| 무엇을 먼저 조심해야 하나 | 편한 문법과 shape 오류 가능성을 함께 보게 해 줍니다. | 편해 보여도 `shape`가 맞는지부터 확인해야 함을 이해합니다. |
...
- 반복문 없이 배열 전체에 같은 계산을 적용하거나, shape가 다른 배열을 함께 계산하려 할 때 브로드캐스팅과 벡터화 관점을 먼저 떠올립니다.
- `(4, 3) + (3,)`는 되고 `(4, 3) + (4,)`는 왜 곧바로 안 되는지 설명해야 할 때 이 절의 shape 규칙으로 돌아옵니다.
- 평균 빼기, 정규화, 특징별 계산처럼 축 기준 통계와 자동 확장을 함께 써야 할 때 다시 확인합니다.
