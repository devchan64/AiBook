...
이 절에서는 고급 브로드캐스팅, `np.newaxis`, `reshape`의 세부 활용, stride, 메모리 뷰(view), 성능 벤치마크, GPU 텐서 연산은 깊게 다루지 않습니다. `np.newaxis`와 shape 변화는 P2-11.4 보충학습에서 다시 다루고, 여기서는 “shape이 맞으면 반복 계산을 배열 단위로 읽을 수 있다”는 감각을 만드는 데 집중합니다.
