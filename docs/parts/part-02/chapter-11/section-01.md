...
여기서는 `넘파이(NumPy)`, `shape`, `ndim`, `dtype`의 기본 구분을 설명합니다. 이번 장은 NumPy 문법을 많이 외우는 장이 아니라, AI 실습에서 벡터, 행렬, 데이터 묶음이 코드에서 어떤 모양으로 나타나는지 읽기 위한 장입니다. 이후 배열, 축, 브로드캐스팅을 다시 만날 때는 [개념사전의 배열 항목](../../../reference/concept-glossary-parts/06-bieup.md#array)도 함께 확인합니다.
