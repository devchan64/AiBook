...
| Pandas | 그 숫자 묶음을 사례와 변수의 표로 읽는다 |
| Matplotlib | 그 표에서 바로 안 보이는 변화와 관계를 모양으로 확인한다 |
| Git | 그 계산과 해석을 변경 이유와 함께 기록으로 남긴다 |
...
| --- | --- | --- |
| 벡터, 행렬, shape처럼 계산 가능한 숫자 모양을 만든다 | 그 모양을 사례와 변수의 표, 변화와 관계의 그래프로 읽는다 | 어떤 계산과 해석을 했는지 Git으로 다시 설명할 수 있게 남긴다 |
