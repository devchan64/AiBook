...
여기서는 `인덱싱(indexing)`, `슬라이싱(slicing)`, `축(axis)`의 기본 구분을 설명합니다. `NumPy`와 배열 기본 속성은 P2-11.1과 [개념사전의 인덱싱 항목](../../../reference/concept-glossary-parts/08-ieung.md#indexing)에서 다시 확인할 수 있고, 여기서는 그 배열에서 무엇을 고르고 어느 방향으로 읽는지에 집중합니다.
