...
- 배열의 일부를 고르거나, 행과 열 중 어느 방향으로 계산하는지 설명해야 할 때 인덱싱·슬라이싱·축 관점을 먼저 떠올립니다.
- `data[i, j]`, `data[:, j]`, `sum(axis=0)` 같은 표현이 왜 그런 결과를 내는지 막힐 때 이 절로 돌아옵니다.
- 데이터셋을 행은 샘플, 열은 특징으로 읽는 기본 감각을 다시 잡아야 할 때 다시 확인합니다.
