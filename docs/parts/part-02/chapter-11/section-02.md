...
여기서는 `인덱싱(indexing)`, `슬라이싱(slicing)`, `축(axis)`의 기본 구분을 설명합니다. `NumPy`와 배열 기본 속성은 P2-11.1과 P2-11.2에서 다시 확인할 수 있고, 여기서는 그 배열에서 무엇을 고르고 어느 방향으로 읽는지에 집중합니다.
...
```python
# NumPy 배열에서 위치, 구간, 축을 기준으로 값을 선택하고 집계하는 예제입니다.
import numpy as np
...
## 인덱싱, 슬라이싱, 축: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "인덱싱, 슬라이싱, 축(axis)이 배열 읽기의 핵심이라는 점을 정리해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
