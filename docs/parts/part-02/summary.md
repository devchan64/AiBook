...
이 마무리 페이지의 재점검 계획은 `작은 계산을 실행할 수 있는가`, `배열과 표의 방향을 말할 수 있는가`, `결과를 그래프와 기록으로 남길 수 있는가`를 확인하는 것입니다. 막히는 지점은 새 설명을 덧붙이기보다 해당 대표 Section으로 돌아가 다시 실행하거나 다시 읽을 위치로 표시합니다.
