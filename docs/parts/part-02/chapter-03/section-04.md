...
Part 2 안에서는 이 절을 `Colab`, `로컬 PC`, `코드 셀(code cell)`, `터미널(terminal)`, `import`와 설치 명령의 차이를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 3.1부터 3.3까지 수식과 선형대수 구조를 읽었다면, 여기서는 그 구조를 실제 코드로 옮길 실행 자리를 먼저 정리합니다.
...
또한 이 절은 `Python 설치 전체 절차`를 끝내는 절도, `가상환경 관리법`을 자세히 설명하는 절도 아닙니다. 여기서는 실행 위치를 먼저 구분하고, 설치와 환경 관리 세부는 Part 2 Chapter 7과 보충학습에서 다시 좁혀 봅니다.
