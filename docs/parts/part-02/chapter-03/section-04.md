...
- 내적은 두 벡터를 한 숫자로 압축해 `함께 움직이는 정도`를 보는 계산입니다.
- 길이(norm)는 벡터의 크기를 보고, 유사도는 닮음을 보고, 거리는 떨어짐을 봅니다.
- `가깝다`와 `비슷하다`는 같은 질문이 아닙니다.
- k-NN은 주로 거리 질문에, 임베딩과 벡터 검색은 유사도 질문에 더 가깝습니다.
- 벡터 비교 문제를 만나면 먼저 `크기까지 중요한가, 방향이 더 중요한가`를 물어야 합니다.
