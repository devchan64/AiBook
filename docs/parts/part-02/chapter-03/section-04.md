...
따라서 여기서는 Colab 자체를 깊게 배우거나 로컬 설치법을 자세히 다루기보다, 이후 Python/NumPy 실습을 따라가기 위해 “코드 셀에서 실행하는 명령”, “개인 PC 터미널에서 실행하는 명령”, “파이썬 코드 안에 쓰는 문장”을 구분하는 데 집중합니다.

여기서는 `Colab`, `로컬 PC`, `코드 셀(code cell)`, `터미널(terminal)`, `import`와 설치 명령의 차이를 다시 정리합니다. 3.1부터 3.3까지 수식과 선형대수 구조를 읽었다면, 이제 그 구조를 실제 코드로 옮길 실행 자리를 먼저 정리합니다.

개인 PC에 파이썬을 설치하고 가상환경을 관리하는 방법은 `P2-7.1`, `P2-7.6`, `P2-7.7`, `P2-7.8`에서 다시 다룹니다. 여기서는 Colab과 로컬 PC의 실행 위치 차이를 먼저 잡습니다.
...
여기서는 Python 설치와 개발환경 전체보다 다음 NumPy 실습을 읽기 전에 실행 위치를 구분하는 데 집중합니다. Colab 셀, 로컬 터미널, Python 코드의 자리를 구분해 두면, 뒤에서 `왜 같은 NumPy 준비 작업인데 명령이 다르게 보이는가`를 구조적으로 따라갈 수 있습니다.
...
| `%pip`, `python -m pip`, `import`의 역할 차이 | `P2-3.5`에서 실제 NumPy 배열과 shape를 실행해 봅니다. | `P2-7.1`, `P2-7.6`, `P2-7.7`, `P2-7.8`의 가상환경, 패키지 설치, 터미널 보충학습으로 이어집니다. |
| Colab을 빠른 실습 공간으로 쓰는 이유 | 뒤 절에서 작은 예제를 바로 실행해 결과를 확인합니다. | Part 3 이후의 Pandas, Matplotlib, 모델 실습을 따라갈 때도 같은 실행 감각이 필요합니다. |

## 이 절의 범위

여기서는 다음 Python/NumPy 실습을 준비하기 위해 Colab과 로컬 PC의 실행 위치를 구분하고, 필요한 경우 NumPy를 설치하는 명령이 환경별로 어떻게 달라지는지만 다룹니다.

Colab의 파일 관리, Google Drive 연동, GPU/TPU 런타임, 노트북 공유 권한, 장시간 실행 제한은 다루지 않습니다. 로컬 PC의 파이썬 설치, 가상환경(virtual environment), 패키지 관리 세부 절차도 여기서 깊게 다루지 않습니다. 로컬 설치와 터미널, 가상환경 절차는 `P2-7.1`, `P2-7.6`, `P2-7.7`, `P2-7.8`에서 다시 정리합니다.
...
| --- | --- | --- |
| Colab | 브라우저 기반 Python 실습 공간 | 설치 없이 실습을 시작하는 대표 환경 |
| 로컬 PC | 내 컴퓨터의 Python 실행 환경 | 터미널과 파일 기반 실행을 이해하는 기준 |
| 코드 셀 | 노트북 안에서 코드를 실행하는 칸 | `%pip`, `print`, 예제 코드를 넣는 자리 |
| 터미널 | 운영체제 명령을 입력하는 창 | `python -m pip` 같은 명령을 넣는 자리 |
| import | 설치된 패키지를 코드에서 불러오는 문장 | 설치와 실행을 구분하는 핵심 표식 |
...
Colab은 파이썬 설치 전에 코드를 먼저 실행해 볼 수 있는 브라우저 기반 환경입니다. 로컬 PC는 내 컴퓨터에 설치된 파이썬으로 실행하는 환경입니다. 두 환경은 모두 실습에 쓰이지만, 명령을 쓰는 위치가 다릅니다.

> 코드 셀에서 실행하는가?
> 터미널에서 실행하는가?
> 파이썬 코드 안에서 import하는가?

이 세 가지를 구분하면 다음 절의 NumPy 실습에서 설치, import, 실행 흐름을 바로 구분할 수 있습니다.
...
- 설치 명령, 코드 셀, Python import 문장을 같은 자리에서 실행하려 할 때
- Colab과 로컬 PC의 차이를 코드 내용 차이로 오해하고 있을 때
- 뒤 NumPy 실습 전에 지금 내가 어느 실행 위치에 있는지 다시 구분해야 할 때
