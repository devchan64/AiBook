...
NumPy 是帮助我们用代码确认线性代数的工具。真正重要的，不是语法本身，而是建立“把公式、shape、输出一起读”的习惯。例如，我们应该一起读成：`公式: xW`、`shape: (2,) @ (2, 2)`、`输出: (2,)`、`含义: 接收 2 个输入，生成 2 个输出`。

在阅读矩阵乘法时，一定要把 `*` 和 `@` 分开。`*` 是逐位置乘法，`@` 是矩阵乘法。只要这个区分先稳住，AI 代码里很多混乱就会少很多。
...
- 当在 NumPy 代码里总是忘记“先看 shape、再看值”时
- 当开始混淆 `*` 和 `@` 到底各自在计算什么时
- 当需要重新确认：公式、数组形状、输出结果其实是在展示同一种计算结构时
