...
여기서는 벡터 공간의 엄밀한 수학 정의보다 AI 문서에서 벡터를 위치처럼 읽는 데 집중합니다. 위치, 차원, 가까움의 직관을 잡아 두면, 뒤에서 임베딩 검색이나 RAG를 볼 때 `왜 가까운 벡터를 찾는가`를 계산 언어로 다시 읽을 수 있습니다.
...
벡터 공간(vector space)을 엄밀하게 정의하려면 여러 공리(axiom)를 다뤄야 합니다. 여기서는 그 전체를 증명하지 않습니다. 대신 입문자가 먼저 알아야 할 두 가지 계산만 봅니다.
...
다만 여기서는 위상수학(topology)을 공부하지 않습니다. 지금은 다음처럼 구분합니다.
...
여기서는 위치와 가까움의 직관을 먼저 잡고, 벡터 공간 공리의 증명이나 거리식과 코사인 유사도 계산법의 본격적인 전개는 다루지 않습니다. shape와 배열 계산은 P2-3.5에서, 더 구체적인 검색 활용은 Part 6에서 다시 회수합니다.
...
그래서 여기서는 고차원 공간을 정확히 그림으로 상상하려고 하지 않습니다. 대신 다음 직관을 유지합니다.
...
벡터 공간(vector space)은 여기서 “벡터들이 놓이고 비교되는 계산 가능한 자리”로 이해합니다. 엄밀한 수학 정의는 뒤로 미루고, 먼저 AI 데이터 표현의 관점에서 봅니다.
...
- 벡터를 값 목록으로는 읽지만 위치와 가까움의 관점으로는 연결되지 않을 때
- 임베딩과 유사도 검색이 왜 벡터 공간 언어를 쓰는지 감이 흐려질 때
- 같은 공간과 같은 차원에서만 비교가 가능하다는 조건을 다시 확인해야 할 때
