...
Part 2 안에서는 이 절을 `벡터 공간(vector space)`, `차원(dimension)`, `위치(position)`, `거리(distance)`, `유사도(similarity)`를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 3.1에서 벡터를 값의 목록으로 읽었다면, 여기서는 그 벡터가 같은 공간 안에서 어떻게 비교되는지 정리합니다.
...
| 같은 공간에서만 비교할 수 있다는 감각 | P2-3.5에서 배열의 shape와 계산 규칙을 코드로 확인합니다. | Part 3의 특징 공간, 클러스터링, 거리 기반 직관을 읽을 때 다시 씁니다. |
...
즉 \([2,\ 3]\)은 “가로로 2, 세로로 3만큼 이동한 위치”처럼 읽을 수 있습니다. 이 설명은 수학적으로 모든 벡터의 의미를 다 말하지는 않지만, AI 입문에서는 벡터를 위치처럼 읽는 기본 직관을 제공합니다.
...
벡터 공간(vector space)은 엄밀하게 정의하면 여러 조건을 만족하는 수학적 구조입니다. 여기서는 다음처럼 읽습니다.
...
다만 이 절에서는 위상수학(topology)을 공부하지 않습니다. 지금은 다음처럼 구분합니다.
...
또한 이 절은 `벡터 공간 공리의 증명`을 다루는 절도, `거리식과 코사인 유사도 계산법`을 본격적으로 연습하는 절도 아닙니다. 여기서는 위치와 가까움의 직관을 먼저 잡고, shape와 배열 계산은 P2-3.5에서, 더 구체적인 검색 활용은 Part 6에서 다시 회수합니다.
...
대상은 벡터로 표현되고, 그 벡터들은 같은 공간에 놓입니다. 가까운 벡터는 비슷함의 후보가 되지만, 그 해석은 문제와 기준에 따라 검증해야 합니다.

이 관점은 P1-13.1부터 P1-13.4까지의 임베딩, 유사도 검색, RAG 흐름에서 다시 등장하고, Part 3의 P3-17 클러스터링, Part 6의 `P6-11.1` 벡터 데이터베이스와 `P6-11.2` 인덱스와 검색 품질 설명에서도 이어집니다.
