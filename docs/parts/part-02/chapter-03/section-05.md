...
실행 위치가 잘못되면 다음과 같은 문제가 생깁니다.

| 혼동 장면 | 왜 막히는가 | 수정 방법 |
| --- | --- | --- |
| `%pip install numpy`를 `.py` 파일에 적는다 | 설치 명령과 Python 코드를 섞었기 때문 | 코드 셀에서 실행하거나 터미널의 `python -m pip` 명령으로 바꾼다 |
| `import numpy as np`를 터미널에 그대로 실행하려 한다 | Python 문장을 셸 명령처럼 본 것 | Python 실행기나 `.py` 파일에서 실행한다 |
| Colab 예제를 로컬에서 그대로 복사한다 | 실행 위치가 바뀌었는데 문법을 안 바꿨기 때문 | 노트북 전용 설치 명령을 로컬 터미널 명령과 구분한다 |
