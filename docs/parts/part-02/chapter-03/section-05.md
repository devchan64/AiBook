...
처음 읽을 때는 NumPy 문법을 넓게 외우기보다 `수식 하나를 배열 하나로 만들고, shape를 확인하고, 출력으로 다시 검증한다`는 순서만 먼저 익히면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 수식과 NumPy 배열이 대응된다는 감각 | 다음 장에서 Pandas 표 데이터가 배열 계산과 어떻게 이어지는지 봅니다. | Part 3에서 `X`, `y`, train/test 분할 결과를 shape로 읽을 때 다시 씁니다. |
| `*`와 `@`를 shape와 함께 구분하는 습관 | 뒤 장의 데이터 처리와 시각화 실습에서도 출력 모양을 함께 확인합니다. | Part 4의 선형층, Part 5의 임베딩·로짓 계산을 읽을 때 다시 등장합니다. |
| 배치 계산을 행렬 단위로 읽는 시선 | P2-4, P2-5의 데이터 테이블과 배열 변환으로 이어집니다. | 머신러닝 입력 행렬과 미니배치(batch) 설명을 따라갈 때 바탕이 됩니다. |
