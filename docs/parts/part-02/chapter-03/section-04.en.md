...
When comparing vectors, distinguish these questions: how similar are they, how far apart are they, and how do we distinguish a vector with a large norm from one pointing in a similar direction? These questions lead to dot product, norm, distance, and similarity.
...
## Comparing Norm and Direction

| Comparison target | Coordinate features | Comparison criterion |
| --- | --- | --- |
...
| `q = [1, 1]`, `c = [10, 10]` | very far away, but the same direction | do distance and cosine similarity give different answers? |

## Dot Product: Products and Sums of Components
...
The Euclidean norm, or 2-norm, is given by:
...
This can be read immediately through the 3-4-5 triangle. By contrast, `[30, 40]` has the same direction but a norm 10 times larger. Because of that difference, we must separate `vectors with similar direction but different size`.
...
| `[30, 40]` | the same direction | `50` |
| `[4, -3]` | different direction | `5` |

## Criteria for Comparing Distance and Direction
...
Both k-NN and embedding retrieval select candidates using a vector-comparison criterion. For embedding retrieval too, check whether the criterion is distance, cosine similarity, or another measure.
...
- `a` has the same direction as `q`, and is also longer.
- `b` has similar length but is missing one axis.

The Euclidean distance from `q` to `a` is approximately `1.414`, while the distance to `b` is `1`. Distance selects `b` as closer, while `a` points in the same direction as `q`.
...
- Can you explain why the comparison criterion used in k-NN or embedding retrieval needs to be checked?
