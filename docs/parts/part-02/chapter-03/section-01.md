...
AI 데이터는 숫자 하나, 값의 목록, 행과 열을 가진 표로 표현할 수 있습니다. 각각 스칼라(scalar), 벡터(vector), 행렬(matrix)에 대응합니다.

## 숫자·목록·표

| 기준 | 왜 중요한가 |
| --- | --- |
| 스칼라는 숫자 하나다 | 손실, 확률, 학습률처럼 단일 값을 읽는 기본 단위가 되기 때문입니다. |
| 벡터는 순서 있는 값의 목록이다 | 한 대상을 여러 특징으로 표현할 때 가장 자주 쓰이는 모양이기 때문입니다. |
| 행렬은 여러 벡터를 함께 담는 표다 | 여러 샘플과 여러 특징을 한꺼번에 계산할 때 행과 열 구조가 필요하기 때문입니다. |
...
NumPy에서 숫자 하나, 값 세 개, 2행 3열의 배열을 만들고 `shape`를 출력합니다.
...
AI 계산에서 같은 배열 모양도 다음처럼 다른 역할을 맡습니다.
...
이 행렬의 shape은 `(3, 2)`입니다. 학생은 3명이고, 각 학생의 특징은 공부 시간과 퀴즈 점수 두 가지입니다. 두 번째 학생의 퀴즈 점수는 2행 2열의 값 `6`입니다. 학생 한 명을 더 추가하면 `(4, 2)`, 특징 한 가지를 더 추가하면 `(3, 3)`이 됩니다.
