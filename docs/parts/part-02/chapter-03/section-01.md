...
Part 2 안에서는 이 절을 `스칼라(scalar)`, `벡터(vector)`, `행렬(matrix)`, `shape`, `배열(array)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. Chapter 2에서 수식 한 줄의 문법과 반복 표기를 읽었다면, 여기서는 그 수식이 실제 데이터 모양으로 펼쳐질 때 무엇을 보고 있는지 정리합니다.

지금 읽는 층위는 `수학 이론 전체`가 아니라 `AI 데이터의 기본 모양을 읽는 입문 층위`입니다. 여기서 스칼라, 벡터, 행렬, shape을 구분해 두면, 뒤에서 `입력 X의 모양이 무엇인가`, `샘플이 몇 행인가`, `특징이 몇 열인가` 같은 질문을 훨씬 덜 막히고 읽을 수 있습니다.
...
| 표 데이터를 숫자 배열로 보는 시선 | P2-3.5에서 NumPy 배열 계산으로 이어집니다. | Part 3에서 데이터프레임을 모델 입력용 배열로 바꾸는 흐름을 읽을 때 다시 씁니다. |

## 이 절의 범위

이 절은 스칼라(scalar), 벡터(vector), 행렬(matrix)의 기본 의미와 데이터 모양(shape)을 다룹니다. 벡터 공간(vector space), 위치와 방향의 직관은 P2-3.2에서, 행렬 곱(matrix multiplication)은 P2-3.3에서, NumPy 배열 계산은 P2-3.5에서 다시 다룹니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 스칼라는 무엇인가? | 숫자 하나를 다룰 때의 기본 단위입니다. |
| 벡터는 무엇인가? | 여러 값을 순서 있게 한 줄로 모아 놓은 표현입니다. |
| 행렬은 왜 필요한가? | 여러 벡터나 여러 샘플을 행과 열 구조로 함께 다루기 위해 필요합니다. |
...
또한 이 절은 `벡터 공간의 공리`를 엄밀하게 설명하는 절도, `행렬 곱 계산법`을 본격적으로 연습하는 절도 아닙니다. 여기서는 데이터 모양을 읽는 손잡이를 먼저 잡고, 위치 직관은 P2-3.2에서, 행렬 곱과 선형 변환은 P2-3.3에서 다시 좁혀 봅니다.
...
스칼라, 벡터, 행렬은 선형대수의 용어이지만, AI 입문에서는 먼저 데이터 모양을 읽는 언어로 이해합니다.
