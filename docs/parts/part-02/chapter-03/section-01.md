...
여기서는 스칼라(scalar), 벡터(vector), 행렬(matrix)의 기본 의미와 데이터 모양(shape)을 다룹니다. 벡터 공간(vector space), 위치와 방향의 직관은 P2-3.2에서, 행렬 곱(matrix multiplication)은 P2-3.3에서, NumPy 배열 계산은 P2-3.5에서 다시 다룹니다.
...
- 숫자 하나, 값의 목록, 행과 열 구조가 같은 종류의 데이터처럼 섞여 보일 때
- `X`의 모양, 샘플 수, 특징 수를 읽는 순간마다 shape 감각이 자꾸 끊길 때
- 임베딩, 특징 벡터, 배치 입력이 왜 같은 선형대수 언어로 설명되는지 다시 붙잡아야 할 때
