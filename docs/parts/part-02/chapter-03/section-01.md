...
스칼라, 벡터, 행렬은 선형대수의 용어이지만, AI 입문에서는 먼저 데이터 모양을 읽는 언어로 이해합니다. 스칼라는 숫자 하나이고, 벡터는 순서가 있는 숫자 목록이며, 행렬은 행과 열을 가진 숫자 배열입니다.

AI 모델은 데이터를 숫자 배열로 바꾸어 계산합니다. 따라서 선형대수를 배운다는 것은 단순히 수학 기호를 배우는 것이 아니라, 모델이 데이터를 어떤 모양으로 받아들이고 처리하는지 읽는 준비입니다.

이 절의 흐름을 한 문장으로 줄이면, AI는 현실의 대상을 숫자 하나로 요약하고, 여러 숫자로 표현하고, 여러 표현을 모은 뒤, 그 표현들이 놓인 위치와 관계를 계산한다고 할 수 있습니다.
