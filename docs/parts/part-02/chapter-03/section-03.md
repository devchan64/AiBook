...
처음 읽을 때는 모든 계산 과정을 외우기보다 `행렬 곱은 같은 입력에서 여러 가중합을 반복해 새 출력 벡터를 만든다`는 한 문장을 먼저 붙잡으면 충분합니다.
...
즉 행렬 곱을 읽을 때는 계산 규칙만 외우기보다 `입력을 다른 표현 공간으로 바꾸는 재사용 가능한 계산`으로 보는 편이 훨씬 유용합니다. 이 감각이 있어야 신경망 층(layer), 선형 변환(linear transformation), 가중치 행렬(weight matrix) 설명도 자연스럽게 이어집니다.
