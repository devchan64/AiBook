...
| --- | --- | --- |
| 행렬 곱이 위치별 곱이 아니라는 구분 | P2-3.4에서 NumPy로 배열 shape와 곱셈을 직접 확인합니다. | Part 3에서 특징 행렬 `X`와 가중치가 만나 예측 점수를 만드는 흐름을 읽을 때 다시 씁니다. |
| 가중합을 여러 출력에 재사용하는 감각 | 뒤 절에서 코드로 shape 오류와 계산 결과를 확인합니다. | Part 4에서 선형층(layer), attention 계산, 표현 변환 설명으로 이어집니다. |
| 입력 차원과 출력 차원이 shape에 반영된다는 점 | P2-6의 학습 절에서 가중치가 어떻게 조정되는지로 넘어갑니다. | Part 5에서 임베딩 투영, 로짓(logit), 출력 벡터 설명을 읽을 때 다시 등장합니다. |
...
다만 이 절에서는 가중치가 어떻게 학습되는지는 다루지 않습니다. 학습(training)과 최적화(optimization)의 기본 흐름은 P2-6.1부터 P2-6.3에서, 딥러닝 문맥의 역전파(backpropagation)와 옵티마이저(optimizer)는 P4-5.1, P4-7.1, P4-7.2에서 다시 다룹니다. 여기서는 “입력에 가중치를 곱해 더하면 새 값이 만들어진다”는 계산 형태만 봅니다.
