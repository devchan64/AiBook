...
Part 2 안에서는 이 절을 `행렬 곱(matrix multiplication)`, `가중합(weighted sum)`, `가중치 행렬(weight matrix)`, `선형 변환(linear transformation)`, `입출력 차원(input/output dimension)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 3.1에서 데이터 모양을 읽고 3.2에서 벡터를 위치처럼 읽었다면, 여기서는 그 입력을 새 표현으로 바꾸는 계산 구조를 정리합니다.

지금 읽는 층위는 `행렬 계산 공식을 완전히 숙달하는 수학 훈련`이 아니라 `AI 모델이 입력을 새 표현으로 바꾸는 계산을 읽는 입문 층위`입니다. 여기서 가중합, shape, 선형 변환의 직관을 잡아 두면, 뒤에서 신경망 층이나 분류기 설명을 볼 때 `왜 행렬을 곱하는가`를 구조적으로 이해할 수 있습니다.
...
| 입력 차원과 출력 차원이 shape에 반영된다는 점 | Part 2 Chapter 6의 학습 절에서 가중치가 어떻게 조정되는지로 넘어갑니다. | Part 5에서 임베딩 투영, 로짓(logit), 출력 벡터 설명을 읽을 때 다시 등장합니다. |

## 이 절의 범위

이 절은 행렬 곱(matrix multiplication), 가중합(weighted sum), 선형 변환(linear transformation)의 입문적 의미를 다룹니다. 행렬식(determinant), 역행렬(inverse matrix), 고유값(eigenvalue), 고유벡터(eigenvector)는 다루지 않습니다. NumPy로 행렬 곱을 직접 실행하는 실습은 P2-3.5로 넘깁니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 행렬 곱은 무엇을 하는가? | 행과 열을 비교해 여러 값을 새로운 값으로 다시 조합합니다. |
| 왜 shape을 먼저 봐야 하는가? | 안쪽 차원이 맞지 않으면 계산 자체가 성립하지 않기 때문입니다. |
| AI에서 왜 자주 쓰이는가? | 입력 특징을 가중치와 결합해 새로운 표현으로 바꾸는 기본 계산이기 때문입니다. |
...
다만 이 절에서는 가중치가 어떻게 학습되는지는 다루지 않습니다. 학습(training)과 최적화(optimization)의 기본 흐름은 P2-6.1부터 P2-6.3에서, 딥러닝 문맥의 역전파(backpropagation)와 옵티마이저(optimizer)는 P5-5.1, P5-7.1, P5-7.2에서 다시 다룹니다. 여기서는 “입력에 가중치를 곱해 더하면 새 값이 만들어진다”는 계산 형태만 봅니다.
...
또한 이 절은 `행렬식, 역행렬, 고유값` 같은 선형대수 주제를 다루는 절도, `NumPy 문법`을 자세히 익히는 절도 아닙니다. 여기서는 가중합 재사용 구조를 먼저 읽고, 실제 배열 실행과 shape 확인은 P2-3.5에서 다시 확인합니다.
