...
NumPy는 선형대수를 코드로 확인하게 해 주는 도구입니다. 중요한 것은 문법 자체보다 수식, shape, 출력이 서로 맞는지 함께 보는 습관입니다. 예를 들어 `수식: xW`, `shape: (2,) @ (2, 2)`, `출력: (2,)`, `의미: 입력 2개를 받아 출력 2개를 만든다`처럼 함께 읽어야 합니다.
...
- NumPy 코드에서 값보다 shape를 먼저 봐야 한다는 기준이 자꾸 빠질 때
- `*`와 `@`가 각각 무엇을 계산하는지 헷갈릴 때
- 수식, 배열 모양, 출력 결과가 같은 계산 구조를 보여 준다는 점을 다시 확인해야 할 때
