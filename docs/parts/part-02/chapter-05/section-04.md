...
이 절에서는 작은 데이터 목록을 놓고 평균, 편차, 분산, 표본 평균 변화를 순서대로 확인합니다.

이 절의 목적은 통계 공식을 많이 외우는 것이 아닙니다. 숫자와 코드가 앞 절의 개념을 어떻게 드러내는지 확인하는 것입니다.

Part 2 안에서는 이 절을 `작은 데이터`, `평균(mean)`, `중위값(median)`, `분산(variance)`, `표본 평균(sample mean)`을 코드로 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 5.2와 5.3에서 개념을 읽었다면, 여기서는 그 개념이 실제 숫자와 출력에서 어떻게 보이는지 정리합니다.

지금 읽는 층위는 `통계 라이브러리 사용법을 넓게 익히는 절`이 아니라 `확률과 통계의 기본 개념을 작은 데이터와 코드로 다시 확인하는 입문 층위`입니다. 여기서 평균, 분산, 표본 평균의 변화를 눈으로 확인해 두면, 뒤에서 데이터셋 통계나 평가 숫자를 볼 때 개념과 출력이 따로 놀지 않고 연결됩니다.
...
| 극단값이 평균과 중위값을 다르게 흔든다는 점 | 뒤 절에서 추정의 신뢰도와 통계 보강 질문으로 넘어갑니다. | 데이터 전처리와 이상치 해석 문맥에서 다시 등장합니다. |
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 이 절의 코드는 무엇을 보여 주는가? | 통계 개념이 작은 숫자 데이터에서 어떤 계산과 출력으로 나타나는지 보여 줍니다. |
| 평균과 중위값은 왜 같이 보는가? | 둘 다 중심을 말하지만, 데이터가 치우쳤을 때 다르게 보일 수 있기 때문입니다. |
| 분산은 무엇을 더해 주는가? | 값들이 평균 주변에 얼마나 퍼졌는지 보여 줍니다. |
...
여기서는 중위값을 깊게 다루지 않습니다. 중위값, 표준편차, 신뢰구간처럼 평균 바깥의 요약값은 P2-5.5 보충학습에서 다시 정리합니다. 다만 평균을 볼 때는 다음 질문을 함께 기억합니다.
...
처음 읽는 독자 단계에서는 `ddof`를 외우기보다 다음 질문을 기억하는 편이 더 중요합니다.
...
또한 이 절은 `통계 라이브러리 전체 사용법`을 배우는 절도, `신뢰구간과 가설검정`을 직접 계산하는 절도 아닙니다. 여기서는 앞 절의 개념을 작은 데이터와 출력으로 확인하고, 더 넓은 통계 보강 개념은 P2-5.5에서 다시 회수합니다.
