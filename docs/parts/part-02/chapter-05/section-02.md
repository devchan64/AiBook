...
정규분포와 확률밀도함수는 이 책의 현재 본편 범위 밖에 두고, 표준편차·공분산·상관계수는 P2-5.5 보충학습에서 다시 다룹니다. 여기서는 분포, 평균, 분산을 “AI 문서를 읽기 위한 최소 통계 언어”로 복구합니다.
...
여기서는 표준편차를 깊게 다루지 않습니다. 표준편차와 상관 개념은 P2-5.5 보충학습에서 다시 다룹니다. 다만 이름을 봤을 때는 분산이 평균에서 떨어진 정도를 제곱해서 요약한 값이고, 표준편차는 그 분산에 제곱근을 취해 원래 값의 단위에 더 가깝게 돌린 값이라고 먼저 이해하면 됩니다.
...
- 분포는 값들의 전체 모양입니다.
- 평균은 중심을 하나의 숫자로 압축한 값입니다.
- 분산은 평균 주변의 퍼짐을 따로 드러냅니다.
- 평균이 같아도 분산이 다르면 데이터의 성격은 다를 수 있습니다.
- 뒤 Part에서 `mean`, `distribution`, `variance`가 나오면 먼저 `지금 중심을 보는가, 퍼짐을 보는가, 전체 모양을 보는가`를 물어야 합니다.

분포, 평균, 분산은 데이터 묶음을 읽는 세 가지 기본 질문입니다. 분포는 값들이 어떤 모양으로 놓여 있는가를, 평균은 중심을 어디로 볼 것인가를, 분산은 그 중심 주변에 얼마나 퍼져 있는가를 묻습니다.
...
- 평균 숫자 하나는 보이는데 데이터가 어디에 몰리고 얼마나 퍼졌는지 놓치고 있을 때 먼저 떠올립니다.
- 데이터 분포와 확률분포를 같은 말처럼 읽어 실제 관측값과 수학적 모형을 구분해야 할 때 먼저 떠올립니다.
- 평균 손실, 분포 이동, 데이터셋 통계 같은 표현이 나왔을 때 모두 같은 통계 지도 안에서 읽어야 할 때 먼저 떠올립니다.
