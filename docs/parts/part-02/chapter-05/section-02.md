...
Part 2 안에서는 이 절을 `분포(distribution)`, `평균(mean)`, `분산(variance)`, `중심(center)`, `퍼짐(spread)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 5.1에서 불확실성을 확률 숫자로 읽었다면, 여기서는 값의 묶음을 어떤 모양과 요약값으로 읽어야 하는지 정리합니다.

지금 읽는 층위는 `통계 공식을 세세하게 계산하는 절`이 아니라 `데이터 묶음의 모양과 중심과 퍼짐을 읽는 입문 층위`입니다. 여기서 분포, 평균, 분산의 역할을 구분해 두면, 뒤에서 평균 손실(mean loss), 데이터 분포(data distribution), 분포 이동(distribution shift) 같은 표현이 나와도 모두 같은 지도 안에서 읽을 수 있습니다.
...
| 평균만으로는 부족하고 퍼짐도 봐야 한다는 점 | 뒤 절에서 표본 변동과 추정 오차 질문으로 넘어갑니다. | 평균 손실, 배치 통계, 정규화 같은 모델 학습 설명으로 이어집니다. |
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 분포는 무엇인가? | 값들이 어디에 몰리고 얼마나 퍼져 있는지 보여 주는 모양입니다. |
| 평균은 무엇을 말해 주는가? | 전체를 하나의 중심값으로 요약합니다. |
| 분산은 왜 필요한가? | 평균만으로는 보이지 않는 퍼짐의 크기를 알려 주기 때문입니다. |
...
여기서는 각 값이 평균에서 얼마나 떨어져 있는지, 그리고 그 떨어진 정도가 전반적으로 큰지 작은지를 묻는다고 이해하면 됩니다.
...
왜 차이를 제곱하는지는 나중에 더 깊게 다룰 수 있습니다. 지금은 아래 흐름으로 이해하면 됩니다.
...
이 절에서는 표준편차를 깊게 다루지 않습니다. 표준편차와 상관 개념은 P2-5.5 보충학습에서 다시 다룹니다. 다만 이름을 봤을 때는 아래 연결만 먼저 잡으면 됩니다.
...
또한 이 절은 `정규분포`를 자세히 다루는 절도, `공분산과 상관계수`를 본격적으로 계산하는 절도 아닙니다. 여기서는 데이터 묶음의 모양과 중심과 퍼짐을 먼저 읽고, 표본과 추정은 P2-5.3에서, 더 넓은 통계 용어는 P2-5.5에서 다시 회수합니다.
