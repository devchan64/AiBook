...
样本、估计、误差，就是“如何用部分去谈整体”。总体是我们想知道的整体，样本是实际观察到的部分，估计是用样本去猜整体的值，误差则是估计值与真实值之间的差。

在 AI 中，这个视角非常重要。我们手里的数据集不是现实的全部，而只是样本。模型评估也不是对现实表现的完整证明，而是通过样本做出来的估计。
...
- 当你把数据集当成现实整体来接受，需要重新把总体与样本分开时，先想起这个视角。
- 当你把某一次评估分数解释成绝对性能，需要重新接上估计与误差的视角时，先想起这个视角。
- 当你把抽样波动与抽样偏差当成同一个问题处理，需要另外检查收集方式的偏斜时，先想起这个视角。
