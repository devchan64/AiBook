...
| 피어슨 상관계수(Pearson correlation coefficient) | 두 변수의 선형 관계의 방향과 강도 |
...
![직선 관계의 상관계수 1과 곡선 관계의 상관계수 0을 비교한 산점도](../../../assets/part-02/chapter-05/linear-vs-curved-correlation-ko.svg)

점은 본문의 관측값 세 쌍이고, 점선은 주어진 관계식입니다. 왼쪽은 `y=2x+4`라는 증가 직선 위에 놓여 상관계수가 1입니다. 오른쪽은 `y=x²`의 곡선 위에 놓이지만, 음의 x와 양의 x에서 편차 곱이 상쇄되어 상관계수는 0입니다. 이 값들은 세 관측값의 관계를 요약하며, 작은 표본의 상관계수만으로 모집단의 관계를 확정할 수는 없습니다.
...
신뢰구간이 좁다고 수집 편향까지 작다는 뜻은 아닙니다. 예를 들어 빠른 응답이 나오는 기기의 기록만 수집하면, 표본 수를 늘려 그 집단 평균의 구간을 좁혀도 전체 기기의 평균을 빗나갈 수 있습니다. 위 95% 해석은 목표 모집단을 반영하는 표본추출과 계산 가정이 맞을 때 적용됩니다. P2-5.3의 표본 대표성 점검은 신뢰구간을 계산한 뒤에도 필요합니다.
