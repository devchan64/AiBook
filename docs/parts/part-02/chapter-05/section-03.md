...
Part 2 안에서는 이 절을 `모집단(population)`, `표본(sample)`, `추정(estimation)`, `오차(error)`, `표본 편향(sampling bias)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 5.2에서 데이터 묶음의 모양과 중심을 읽었다면, 여기서는 우리가 가진 데이터가 전체가 아니라 일부일 때 어떤 한계가 생기는지 정리합니다.
...
지금 읽는 층위는 `통계 추론 공식을 정교하게 계산하는 절`이 아니라 `일부 데이터로 전체를 말할 때 어떤 한계와 오차가 생기는지 읽는 입문 층위`입니다. 여기서 모집단, 표본, 추정, 오차의 관계를 잡아 두면, 뒤에서 일반화(generalization), 평가 점수, 데이터셋 편향을 볼 때 숫자를 훨씬 조심스럽고 정확하게 읽을 수 있습니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 모집단과 표본의 차이 | P2-5.4와 P2-5.5에서 추정의 신뢰도와 통계 보강 개념으로 이어집니다. | Part 3의 일반화, 평가 지표, 데이터 분할 설명에서 다시 씁니다. |
| 추정값과 실제 값 사이의 오차 | 뒤 절에서 변동성과 보강 통계 개념을 더 봅니다. | 테스트 성능을 현실 성능의 추정값으로 읽을 때 다시 등장합니다. |
| 표본 편향과 표본추출 변동의 차이 | 이어지는 데이터셋 점검 질문으로 넘어갑니다. | Part 6의 프로젝트 데이터셋 점검과 모델 해석에서 바탕이 됩니다. |
...
표준오차, 신뢰구간, 가설검정, 중심극한정리는 P2-5.5 보충학습에서 입문 수준으로 다시 다룹니다. 편향-분산 트레이드오프는 Part 3의 P3-5 일반화(generalization)와 P3-6 평가 지표(metric) 맥락에서 다시 등장합니다. 여기서는 “표본으로 전체를 추정할 때 오차가 생긴다”는 관점을 먼저 잡습니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 모집단과 표본은 무엇이 다른가? | 모집단은 전체이고, 표본은 실제로 손에 들어온 일부입니다. |
| 추정은 무엇을 하는가? | 일부를 보고 전체의 성질을 조심스럽게 말해 보는 일입니다. |
| 왜 머신러닝과 연결되는가? | 우리가 가진 데이터셋도 현실 전체가 아니라 일부 표본이기 때문입니다. |
...
AI 문맥에서는 `parameter`라는 단어가 모델 파라미터(model parameter)로도 자주 쓰입니다. 그래서 이 절에서는 통계의 모수(parameter)와 모델의 파라미터(parameter)를 구분해야 합니다. 통계의 `parameter`는 모집단의 실제 특성을 뜻하고, 모델의 `parameter`는 모델이 학습으로 조정하는 숫자를 뜻합니다.
...
테스트 성능도 현실 전체 성능의 추정값입니다. 테스트 데이터가 충분히 대표적이지 않으면 평가 결과는 불안정해지거나 현실 성능을 빗나갈 수 있습니다.
