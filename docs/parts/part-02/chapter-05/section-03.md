...
| 추정값과 실제 값 사이의 오차 | `P2-5.4`와 `P2-5.5`에서 변동성과 보강 통계 개념을 더 봅니다. | 테스트 성능을 현실 성능의 추정값으로 읽을 때 다시 등장합니다. |
| 표본 편향과 표본추출 변동의 차이 | 이어지는 데이터셋 점검 질문으로 넘어갑니다. | Part 6의 프로젝트 데이터셋 점검과 모델 해석에서 바탕이 됩니다. |

## 이 절의 범위

여기서는 표본(sample), 모집단(population), 추정(estimation), 오차(error)의 기본 관계를 다룹니다.
...
표준오차, 신뢰구간, 가설검정, 중심극한정리는 P2-5.5 보충학습에서 입문 수준으로 다시 다룹니다. 편향-분산 트레이드오프는 Part 3의 P3-5 일반화(generalization)와 P3-6 평가 지표(metric) 맥락에서 다시 등장합니다. 여기서는 “표본으로 전체를 추정할 때 오차가 생긴다”는 관점을 먼저 둡니다.
...
표본, 추정, 오차는 “일부로 전체를 말하는 법”입니다. 모집단은 알고 싶은 전체이고, 표본은 실제로 관측한 일부이며, 추정은 표본으로 전체의 값을 짐작하는 일이고, 오차는 추정값과 실제 값 사이의 차이입니다.
...
- 데이터셋을 현실 전체처럼 받아들이고 있어 모집단과 표본을 다시 분리해야 할 때 먼저 떠올립니다.
- 평가 점수 하나를 절대 성능처럼 해석하고 있어 추정과 오차의 관점을 다시 붙여야 할 때 먼저 떠올립니다.
- 표본추출 변동과 표본 편향을 같은 문제처럼 다루고 있어 수집 방식의 치우침을 따로 점검해야 할 때 먼저 떠올립니다.
