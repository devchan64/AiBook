...
처음 읽을 때는 세부 공식보다 `우리가 가진 데이터는 전체가 아니라 일부`, `그래서 추정에는 오차가 있다`는 두 문장만 먼저 붙잡으면 충분합니다.
...
서비스 팀이 전체 사용자 행동을 알고 싶지만, 실제로는 최근 1만 명의 로그만 모았다고 해 보겠습니다. 사람은 손에 들어온 데이터가 많아 보이면 곧바로 `전체 사용자도 이럴 것`이라고 느끼기 쉽습니다.
...
이 사례는 왜 AI 데이터셋도 `현실 전체`가 아니라 `수집된 표본`으로 읽어야 하는지를 보여 줍니다. 훈련 데이터와 테스트 데이터도 결국 표본이므로, 모델 성능 숫자 역시 현실 성능의 완전한 증명이 아니라 조심스럽게 읽어야 할 추정값입니다.
