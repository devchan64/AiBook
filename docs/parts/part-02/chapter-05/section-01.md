...
Part 2 안에서는 이 절을 `확률(probability)`, `불확실성(uncertainty)`, `사건(event)`, `결과(outcome)`, `표본공간(sample space)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. Chapter 4에서 변화와 학습 방향을 읽었다면, 여기서는 확실하지 않은 상황을 숫자로 표현하는 확률의 입구를 다시 정리합니다.

지금 읽는 층위는 `확률 공식을 본격적으로 계산하는 절`이 아니라 `불확실성을 숫자로 표현하는 언어를 다시 잡는 입문 층위`입니다. 여기서 사건(event), 결과(outcome), 표본공간(sample space), 가능성 점수의 감각을 잡아 두면, 뒤에서 분포, 추정, 모델 예측 확률을 볼 때 왜 확률 언어가 계속 반복되는지 덜 끊기고 읽을 수 있습니다.
...
| 확률이 의사결정 자체가 아니라 판단 재료라는 점 | 뒤 절에서 베이즈 갱신과 통계 해석으로 넘어갑니다. | Part 5의 안전성, 정렬, 확률적 출력 해석으로 다시 등장합니다. |
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 불확실성은 무엇인가? | 아직 모르는 상태를 말합니다. |
| 확률은 무엇인가? | 그 모르는 상태를 숫자로 표현하고 비교하는 방법입니다. |
| 왜 AI에서 필요한가? | 예측과 판단이 늘 확실하지 않기 때문에 결과를 수치로 표현해야 하기 때문입니다. |
...
또한 이 절은 `조건부확률 공식 계산`을 본격적으로 연습하는 절도, `베이즈 규칙`을 자세히 계산하는 절도 아닙니다. 여기서는 불확실성을 숫자로 읽는 감각을 먼저 잡고, 분포와 통계 요약은 P2-5.2와 P2-5.3에서 다시 좁혀 봅니다.
