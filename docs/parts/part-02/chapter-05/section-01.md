...
| --- | --- | --- |
| 불확실성을 확률 숫자로 표현하는 감각 | P2-5.2에서 분포와 평균·분산으로 확장합니다. | Part 3의 예측 점수, 확률 해석, calibration 설명에서 다시 씁니다. |
| 사건, 결과, 표본공간의 기본 구분 | P2-5.3에서 표본과 추정 질문으로 이어집니다. | 데이터 수집과 평가에서 `무엇을 관측했는가`를 읽을 때 바탕이 됩니다. |
| 확률이 의사결정 자체가 아니라 판단 재료라는 점 | `P2-5.2`, `P2-5.3`, `P2-5.5`에서 베이즈 갱신과 통계 해석으로 넘어갑니다. | Part 5의 안전성, 정렬, 확률적 출력 해석으로 다시 등장합니다. |
