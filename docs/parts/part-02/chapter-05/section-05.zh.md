...
| 皮尔逊相关系数(Pearson correlation coefficient) | 两个变量线性关系的方向与强度 |
...
![比较直线关系的相关系数 1 与曲线关系的相关系数 0 的散点图](/AiBook/assets/part-02/chapter-05/linear-vs-curved-correlation-zh.svg)

点表示正文中的三对观测值，虚线表示给定的关系式。左图的点位于递增直线 `y=2x+4` 上，相关系数为 1。右图的点位于 `y=x²` 曲线上，但负 x 与正 x 对应的偏差乘积相互抵消，因此相关系数为 0。这些值概括的是三个观测值的关系，不能仅凭小样本的相关系数就确定总体关系。
...
置信区间窄，并不意味着收集偏差也小。例如，如果只收集响应快的设备记录，增加样本量可以缩窄该群体均值的区间，却仍可能偏离全部设备的均值。上述 95% 的解释，适用于抽样反映目标总体且计算假设成立的情况。计算置信区间之后，仍需要执行P2-5.3 的样本代表性检查。
