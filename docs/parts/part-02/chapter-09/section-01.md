...
지금 읽는 층위는 `새 Python 문법을 더 배우는 절`이 아니라 `앞 절에서 배운 문법들을 데이터 조직 관점으로 다시 묶는 층위`입니다. Chapter 8이 값, 묶음, 반복, 함수라는 실행 문장을 익히는 구간이었다면, 여기서는 그 문장들이 결국 어떤 데이터 모양을 전제하고 있었는지 다시 봅니다. 이 손잡이를 먼저 잡아 두면, 리스트와 딕셔너리를 `문법 항목`이 아니라 `서로 다른 자료구조 선택`으로 읽기 쉬워집니다.

처음 읽을 때는 자료구조 이름을 많이 외우기보다 `지금 데이터에서 중요한 것은 순서인가, 이름표인가, 관계인가`, `자주 하는 연산은 무엇인가`, `같은 데이터라도 목적이 바뀌면 구조가 왜 바뀌는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
이 절은 자료구조 이름을 외우는 절이 아니라, `데이터를 어떤 모양으로 담을 것인가`를 묻는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 자료구조(data structure)는 `데이터를 조직하는 방식`이라는 점 | 문법 이름 암기보다 목적 중심으로 읽게 해 줍니다. | 순서, 이름표, 계층, 관계 중 무엇이 중요한지 묻는 절이라는 점만 알면 충분합니다. |
| 같은 데이터도 `자주 하는 연산`에 따라 다른 구조가 필요하다는 점 | 리스트와 딕셔너리 이후에 왜 더 많은 구조를 배우는지 이해하게 해 줍니다. | 순서대로 볼지, 이름으로 찾을지에 따라 구조가 달라진다는 점만 알면 충분합니다. |
| 추상 자료형과 구현은 `같은 것`이 아니라는 점 | 뒤에서 스택, 큐, 그래프를 더 안전하게 읽게 해 줍니다. | 동작 규칙과 실제 저장 방식은 구분해야 한다는 점만 알면 충분합니다. |
