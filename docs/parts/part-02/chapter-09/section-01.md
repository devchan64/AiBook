...
이 질문이 자료구조 선택(data structure)의 출발점입니다. 자료구조는 단순히 문법 이름이 아닙니다. 데이터를 어떻게 조직하고, 어떤 연산을 자주 할 것인지에 따라 코드의 모양과 계산 방식이 달라집니다.

여기서는 `자료구조 선택(data structure)`, `추상 자료형(abstract data type)`, `선형 구조(linear structure)`, `비선형 구조(non-linear structure)`의 기본 구분을 설명합니다. 뒤 절에서 배열, 표, 트리, 그래프를 나누어 다시 보더라도, 자료구조를 왜 먼저 질문으로 읽어야 하는지는 여기 기준으로 다시 연결합니다. 이후 구조 이름이 반복될 때는 [개념사전의 자료구조 항목](../../../reference/concept-glossary-parts/09-jieut.md#data-structure)도 함께 확인합니다.
...
| 용어 | 이 절에서 먼저 잡을 뜻 |
| --- | --- |
| 자료구조 선택(data structure) | 데이터를 어떤 모양으로 조직하고 다룰지 정하는 방식입니다. |
...
```python
# 이 예제는 리스트, 딕셔너리, 중첩 구조가 데이터를 담고 찾는 방식을 확인합니다.
scores = [82, 75, 91]
```
...
## 핵심 기준: 자료구조 선택(data structure)는 왜 필요한가

- 자료구조 선택(data structure)를 데이터를 조직하는 방식으로 설명할 수 있습니다.
...
| --- | --- | --- |
| 자료구조 선택(data structure)는 `데이터를 조직하는 방식`이라는 점 | 문법 이름 암기보다 목적 중심으로 읽게 해 줍니다. | 순서, 이름표, 계층, 관계 중 무엇이 중요한지 묻는 절입니다. |
...
## 자료구조 선택는 왜 필요한가를 실제 학습 장면에서 보기

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "자료구조가 계산 방식에 영향을 준다는 점을 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- 자료구조 선택(data structure)를 데이터를 조직하는 방식으로 설명할 수 있다.
