...
Part 2 Chapter 9에서는 `자료구조(data structure)`, `추상 자료형(abstract data type)`, `선형 구조(linear structure)`, `비선형 구조(non-linear structure)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 배열, 표, 트리, 그래프를 나누어 다시 보더라도, 자료구조를 왜 먼저 질문으로 읽어야 하는지는 여기 기준으로 다시 연결합니다.
...
이 절은 자료구조 이름을 외우는 절이 아니라, `데이터를 어떤 모양으로 담을 것인가`를 묻는 절입니다. 아래 세 가지 관점이 뒤 절의 배열, 표, 트리, 그래프 설명을 읽는 기준이 됩니다.
