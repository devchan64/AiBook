...
这里说明 `graph`、`node`、`edge`、`direction`、`weight` 的基本区分。把 `data structure` 读成“一个问题”的代表性说明放在 P2-9.1，四种结构的比较放在 P2-9.2 和[数据结构词条](/AiBook/zh/reference/concept-glossary-pinyin/d/#data-structure)中，而这里专注于：我们该用什么问题来阅读关系数据。
