...
问题场景：想用同一份学生材料比较按顺序访问、按 key 查找、沿关系追踪。
输入(input)：`scores`、`score_by_name`、`relation_graph`。
期望输出(output)：第一个分数、Kim 的分数、Kim 的直接连接对象。
要确认的概念：问题是位置、key 查找还是关系，决定了结构会怎样变化。
