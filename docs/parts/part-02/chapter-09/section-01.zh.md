...
数据结构不只是装数据的容器。它是为了让某些问题更容易回答而组织数据的方法。
...
当需要表达层级和关系时，树和图会变得必要。

在 P2-9.2 中，数组、表、树、图会被放到更大的地图里比较。在 P2-9.3 中，图会从 node 和 edge 的角度单独再看。在 P2-9.4 中，传统数据结构名称会作为补充学习再整理一次。
...
- 当你需要解释的不是“把数据放在哪里”，而是“按什么标准组织它”时，应先想起数据结构视角。
- 当你需要区分同一份数据也会因为顺序、键、关系不同而被完全不同地处理时，应回看本节的比较。
- 当你越过 Python 语法，开始判断真实场景里该选什么结构时，应先检查数据结构和操作之间的连接。
