...
P2-9.2에서는 배열(array), 표(table), 트리(tree), 그래프(graph)를 서로 다른 데이터 관점으로 비교했습니다. 그중 그래프는 처음 읽는 독자에게 특히 낯설 수 있습니다.
...
이 절에서는 그래프 탐색 알고리즘, 최단 경로, 중심성, 그래프 신경망, 그래프 데이터베이스 구현을 깊게 다루지 않습니다. 자료구조 이름과 읽는 기준은 P2-9.4 보충학습에서 다시 정리하고, 그래프 감각이 검색 구조로 다시 등장하는 장면은 Part 1의 13.4에서 이어집니다. 그래프 신경망과 그래프 데이터베이스의 구현은 이 책의 현재 본편 범위 밖에 둡니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 그래프는 무엇을 표현하는가? | 대상과 대상 사이의 연결 관계를 표현합니다. |
| 표와 그래프는 왜 다르게 느껴지는가? | 표는 기록을 보기 좋고, 그래프는 연결을 따라가기 좋기 때문입니다. |
| 방향과 가중치는 왜 중요한가? | 연결이 어느 쪽으로 향하는지, 얼마나 강한지까지 의미가 달라지기 때문입니다. |
...
그래프 감각은 Part 1의 13.4 벡터 검색 구현, Part 5의 P5-13 벡터 데이터베이스와 인덱스 설명, 그리고 Part 6의 워크플로우·검색 프로젝트 문맥에서 다시 등장합니다.
