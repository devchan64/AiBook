...
Part 2 Chapter 9에서는 `그래프(graph)`, `노드(node)`, `엣지(edge)`, `방향(direction)`, `가중치(weight)`의 상세 설명을 이 절에 대표로 둡니다. P2-9.2가 네 구조를 넓게 비교했다면, 관계 데이터를 어떤 질문으로 읽는지는 이 절 기준으로 다시 연결합니다.
...
이 절에서는 그래프 탐색 알고리즘, 최단 경로, 중심성, 그래프 신경망, 그래프 데이터베이스 구현을 깊게 다루지 않습니다. 자료구조 이름과 읽는 기준은 P2-9.4 보충학습에서 다시 정리하고, 그래프 감각이 검색 구조로 다시 등장하는 장면은 P1-13.4에서 이어집니다. 그래프 신경망과 그래프 데이터베이스의 구현은 현재 본편 범위 밖에 둡니다.
