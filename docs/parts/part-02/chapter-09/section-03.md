...
지금 읽는 층위는 `그래프 알고리즘을 배우는 절`이 아니라 `관계 데이터를 어떤 구조로 읽어야 하는지 감각을 만드는 층위`입니다. 앞 절이 배열, 표, 트리, 그래프를 넓게 비교했다면, 여기서는 그중에서도 특히 `연결을 따라가는 질문`이 필요한 장면만 따로 붙잡습니다. 이 손잡이를 먼저 잡아 두면, 그래프를 통계 차트와 혼동하지 않고 이후 검색, 추천, 지식 연결 이야기와도 자연스럽게 이어갈 수 있습니다.

처음 읽을 때는 세부 이론보다 `무엇이 노드이고 무엇이 엣지인가`, `지금 질문이 표보다 관계 추적에 더 가까운가`, `방향과 가중치가 붙으면 무엇이 달라지는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
문제 상황: 검색 후보나 추천 후보 사이의 관계 점수에서 기준 이상만 고르고 싶습니다.
입력(input): 질의와 문서 사이 유사도 점수를 담은 딕셔너리입니다.
기대 출력(output): 점수가 `0.7` 이상인 문서와 점수만 출력됩니다.
확인할 개념: 가중치가 붙은 그래프에서 점수 기준으로 연결을 해석할 수 있음을 확인합니다.
...
이 절에서는 각 분야의 구현을 깊게 설명하지 않습니다. 벡터 검색과 인덱스는 Part 1의 13.4와 Part 5의 P5-13에서 다시 보고, 워크플로우와 검색 프로젝트 연결은 Part 6에서 다시 봅니다. 지금은 그래프가 “관계가 중요한 데이터”를 표현하는 방식이라는 감각을 잡는 것이 중요합니다.
