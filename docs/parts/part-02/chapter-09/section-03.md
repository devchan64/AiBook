...
이 절에서는 그래프 탐색 알고리즘, 최단 경로, 중심성, 그래프 신경망, 그래프 데이터베이스 구현을 깊게 다루지 않습니다. 자료구조 이름과 읽는 기준은 P2-9.4 보충학습에서 다시 정리하고, 그래프 감각이 검색 구조로 다시 등장하는 장면은 P1-13.4에서 이어집니다. 그래프 신경망과 그래프 데이터베이스의 구현은 이 책의 현재 본편 범위 밖에 둡니다.
