...
- 데이터의 핵심이 항목 자체보다 항목 사이의 연결이라는 점을 설명해야 할 때 그래프 관점을 먼저 떠올립니다.
- 추천, 경로 탐색, 지식 연결처럼 이웃과 관계를 따라가는 문제를 만나면 이 절의 노드·엣지 설명으로 돌아옵니다.
- 표로는 잘 드러나지 않는 연결 구조를 표현하거나 작은 예시를 Python으로 직접 확인하려 할 때 다시 확인합니다.
