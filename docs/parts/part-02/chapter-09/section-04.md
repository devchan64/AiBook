...
P2-9.1에서 자료구조가 왜 필요한지 읽고도 배열, 스택, 큐, 그래프 이름이 한꺼번에 낯설다면 이 절로 돌아오면 됩니다. 여기서 먼저 회수해야 하는 것은 구현 공식이 아니라, `이 이름이 어떤 질문을 쉽게 만드는가`입니다.
...
이 절에서는 시간 복잡도(time complexity), 메모리 배치(memory layout), 포인터(pointer), 해시 충돌(hash collision), 그래프 탐색 알고리즘(graph traversal algorithm)을 깊게 다루지 않습니다. 이 책 본편에서는 자료구조 이름을 읽는 감각까지를 우선 범위로 두고, 구현 세부와 복잡도 분석은 이후 알고리즘·시스템 학습에서 다시 다룰 주제로 남겨 둡니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 왜 전통 자료구조를 다시 보나? | Python 문법 뒤에 있는 더 일반적인 사고를 회복하기 위해서입니다. |
| 자료구조 이름은 무엇을 대표하는가? | 저장 방식보다 어떤 질문을 쉽게 만들지 대표합니다. |
| Python 자료형과 어떤 관련이 있는가? | 리스트와 딕셔너리도 더 오래된 자료구조 생각과 이어져 있습니다. |
...
현실의 해시 테이블은 같은 위치가 나오는 충돌(collision)을 처리해야 하고, 데이터가 늘어나면 내부 크기도 조정해야 합니다. 이 절에서는 그런 구현 세부를 다루지 않습니다. 이 책 본편에서는 충돌 처리와 리사이징을 따로 확장하지 않고, 중요한 것은 딕셔너리처럼 키로 값을 찾는 구조 뒤에 해시 테이블 감각이 있다는 점입니다.
...
| --- | --- | --- |
| 순서, 키 조회, 관계 추적이 서로 다른 질문이라는 점 | 지금 보는 데이터는 어떤 구조 질문을 먼저 만드는가 | 자료구조 큰 그림은 P2-9.1, 배열과 표 감각은 P2-11~P2-12 |
| 배열, 스택, 큐, 트리, 그래프 이름을 문제 장면과 연결하는 최소 감각 | AI 실습에서 왜 같은 데이터도 다른 구조 이름으로 읽히는가 | 이후 NumPy, Pandas, 검색/관계 설명은 Part 3 이후 본편 |

즉, `이건 순서 문제인가`, `키 조회 문제인가`, `관계 추적 문제인가`를 구분할 수 있으면 이 보충학습의 핵심은 이미 회수한 것이므로 본편으로 돌아오면 됩니다.
