...
- 전통 자료구조 이름은 구현 이전에 `질문의 언어`로 먼저 읽는 편이 좋습니다.
- 배열, 스택, 큐는 선형 구조 질문과, 트리와 그래프는 관계 구조 질문과 연결됩니다.
- Python 리스트와 딕셔너리를 이미 배웠다면, 전통 자료구조는 그 뒤의 더 일반적인 감각을 붙이는 절입니다.
- AI 실습에서도 순서, 키 기반 검색, 관계 추적은 계속 다시 나옵니다.
...
즉, `이건 순서 문제인가`, `키 조회 문제인가`, `관계 추적 문제인가`를 구분할 수 있으면 이 보충학습의 핵심을 잡은 것입니다.
...
- Python의 편의 문법 뒤에 있는 전통 자료구조 감각을 따로 정리해야 할 때 이 보충 절을 다시 읽습니다.
- 배열, 연결 리스트, 스택, 큐, 트리, 그래프, 해시 기반 구조가 각각 어떤 질문을 해결하려고 만들어졌는지 구분해야 할 때 떠올립니다.
- 구현 세부를 깊게 파기보다 구조별 역할과 선택 기준을 빠르게 복구해야 할 때 이 절이 기준선이 됩니다.
