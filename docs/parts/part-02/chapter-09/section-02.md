...
여기서는 `배열(array)`, `표(table)`, `트리(tree)`, `그래프(graph)`를 한 번에 비교하는 기본 설명을 제공합니다. 뒤 절에서 그래프를 따로 더 보더라도, 네 구조가 서로 어떤 질문에 답하는지는 여기 기준으로 다시 연결합니다.
...
이 절은 자료구조 이름을 한 번에 외우는 대신, 어떤 질문이 어떤 구조를 자연스럽게 부르는지 비교합니다. 앞 절에서 자료구조가 왜 필요한지 큰 그림을 봤다면, 여기서는 그 큰 그림을 배열, 표, 트리, 그래프라는 네 가지 대표 시야로 펼쳐 봅니다. 이 구분을 먼저 잡아 두면, 뒤에서 NumPy와 Pandas를 다시 만날 때도 도구 이름보다 지금 어떤 구조 질문을 하고 있는지를 먼저 떠올리기 쉬워집니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 배열, 표, 트리, 그래프가 서로 다른 질문에 답하는 구조라는 점 | P2-9.3에서 관계 표현이 특히 중요한 그래프를 왜 따로 다시 보는지로 이어집니다. | 이후 NumPy, Pandas, 문서 구조, 관계 데이터 설명에서 반복됩니다. |
| 같은 데이터도 위치, 비교, 계층, 관계 중 무엇을 보느냐에 따라 다른 구조로 읽힌다는 점 | 어떤 구조가 현재 작업에 더 자연스러운지 판단하는 기준으로 이어집니다. | 이후 데이터셋 설계, 전처리, 검색, 추천, 지식 그래프 설명에서 다시 쓰입니다. |
| Chapter 9가 이름 암기보다 구조 선택 질문을 훈련하는 장이라는 점 | P2-9.4 보충학습의 전통 용어 목록을 더 안전하게 읽는 기준으로 이어집니다. | 이후 자료구조 용어와 실제 실습 장면을 섞지 않고 읽는 바탕이 됩니다. |

| 용어 | 이 절에서 먼저 잡을 뜻 |
| --- | --- |
| 배열(array) | 위치와 축을 기준으로 값을 읽는 구조입니다. |
| 표(table) | 행과 열의 교차로 값을 정리하는 구조입니다. |
...
이 절에서는 그래프 탐색 알고리즘, 트리 구현, 데이터베이스 정규화, pandas 세부 사용법, NumPy 내부 메모리 배치를 깊게 다루지 않습니다. 그래프는 P2-9.3에서 따로 보고, 전통적인 자료구조 이름은 P2-9.4 보충학습에서 다시 정리합니다. 여기서는 네 구조를 구현보다 질문 기준으로 비교하는 데 집중합니다.
...
이 절에서 가장 먼저 붙잡아야 할 기준은 `자료구조 이름`보다 `지금 어떤 질문을 하고 있는가`입니다.

| 지금 하는 질문 | 먼저 떠올릴 구조 |
| --- | --- |
| 어느 위치의 값인가 | 배열(array) |
| 어떤 행과 열의 값인가 | 표(table) |
| 상위와 하위가 어떻게 나뉘는가 | 트리(tree) |
| 무엇과 무엇이 연결되는가 | 그래프(graph) |

즉 같은 데이터라도 `위치`, `행열`, `계층`, `관계` 중 무엇을 보느냐에 따라 더 자연스러운 구조가 달라집니다.

## 세 가지 기준

| 기준 | 왜 중요한가 | 이 절에서 필요한 이해 수준 |
| --- | --- | --- |
| 같은 데이터를 여러 구조로 보는 이유 | 던지는 질문이 달라지면 잘 보이는 구조도 달라지기 때문입니다. | 같은 정보도 위치, 비교, 계층, 관계 중 무엇을 보느냐에 따라 다른 구조로 읽힙니다. |
| 배열, 표, 트리, 그래프의 차이 | 네 구조를 이름이 아니라 읽기 기준으로 구분하게 해 줍니다. | 위치, 행과 열, 계층, 연결이라는 서로 다른 관점을 구분하면 충분합니다. |
| 지금 먼저 기억할 것 | 뒤에서 도구 이름보다 구조 질문을 먼저 떠올리게 해 줍니다. | 자료구조 이름보다 어떤 질문에 강한지 먼저 기억합니다. |
...
여기서 중요한 것은 “정답 구조”를 외우는 일이 아닙니다. 어떤 질문을 던지는지에 따라 데이터가 다르게 보인다는 점을 익히는 일입니다.
...
| AI 실습 장면 | 먼저 읽을 구조 감각 |
| --- | --- |
| 임베딩 벡터, 이미지 픽셀 | 배열 |
| CSV, 학습 데이터셋, 실험 결과표 | 표 |
| 목차, 폴더, 분류 체계 | 트리 |
| 링크, 추천, 지식 관계 | 그래프 |

## 배열(array): 위치와 축으로 읽는 데이터

배열(array)은 값을 위치(index)로 다루는 구조입니다. NumPy 문서에서 `ndarray`는 같은 타입과 크기의 항목을 담는 다차원 컨테이너로 설명됩니다. 여기서는 배열을 다음처럼 이해합니다.

배열은 숫자들이 위치와 축을 가진 채 놓인 구조입니다.

1차원 배열은 한 줄의 숫자입니다.

문제 상황: 위치로 값을 읽는 배열 감각을 가장 작은 숫자 목록으로 확인하고 싶습니다.
입력(input): 숫자 목록 `embedding`.
기대 출력(output): 첫 번째 값과 세 번째 값이 출력됩니다.
확인할 개념: 배열에서는 값 자체뿐 아니라 어느 위치의 값인가가 중요합니다.
...
문제 상황: 2차원 배열처럼 보이는 숫자 격자에서 특정 위치 값을 꺼내는 예를 보고 싶습니다.
입력(input): 2행 3열 숫자 구조 `image_patch`.
기대 출력(output): 지정한 행과 열 위치의 값 두 개.
확인할 개념: 2차원 배열은 행과 열 위치를 함께 읽어 값을 찾습니다.
...
문제 상황: 숫자만 모인 구조가 계산에 유리한 장면을 평균 계산으로 보고 싶습니다.
입력(input): 점수 목록 `scores`.
기대 출력(output): 평균값 `average`.
확인할 개념: 배열 감각은 숫자 계산 중심 작업에서 특히 유용합니다.
...
문제 상황: 사례별 속성을 표처럼 정리한 데이터를 순회하는 예를 보고 싶습니다.
입력(input): 학생 레코드가 들어 있는 리스트 `students`.
기대 출력(output): 학생 이름과 점수가 차례대로 출력됩니다.
확인할 개념: 표 구조는 한 행이 한 사례, 각 키가 열 역할을 하는 방식으로 읽을 수 있습니다.
...
문제 상황: 한 사례의 여러 속성을 함께 보고 조건 필터링하는 예를 보고 싶습니다.
입력(input): 학생 레코드 리스트 `students`.
기대 출력(output): 합격한 학생 이름만 모은 `passed_students`.
확인할 개념: 표 감각은 여러 속성을 함께 보고 비교하거나 필터링할 때 유리합니다.
...
문제 상황: 사람 사이 연결을 그래프처럼 표현하는 가장 단순한 예를 보고 싶습니다.
입력(input): 친구 관계 딕셔너리 `friends`.
기대 출력(output): `"Kim"`과 직접 연결된 사람 목록이 출력됩니다.
확인할 개념: 그래프는 대상과 대상의 연결을 저장하고 따라가는 구조입니다.
...
문제 상황: 그래프에서 한 노드와 직접 연결된 이웃을 확인하는 예를 다시 보고 싶습니다.
입력(input): `friends["Kim"]`.
기대 출력(output): `"Kim"`과 연결된 친구 이름들.
확인할 개념: 그래프 읽기의 첫 질문은 직접 연결된 이웃이 누구인가입니다.
...
문제 상황: 하나의 원천 데이터에서 배열, 계층형 묶음, 그래프 표현을 동시에 만드는 예를 보고 싶습니다.
입력(input): 학생 레코드 리스트 `students`.
기대 출력(output): 점수 배열 `scores`, 반별 이름 묶음 `names_by_class`, 친구 그래프 `friends`.
확인할 개념: 같은 데이터도 질문에 따라 여러 구조로 다시 표현할 수 있습니다.
...
- 배열, 표, 트리, 그래프는 서로 다른 질문에 답하는 구조입니다.
- 배열은 위치와 축, 표는 행과 열, 트리는 계층, 그래프는 연결을 먼저 봅니다.
- 같은 데이터도 어떤 질문을 던지느냐에 따라 다른 구조로 읽힐 수 있습니다.
- 뒤에서 NumPy와 Pandas를 만나면 도구 이름보다 먼저 `지금 배열 질문인가, 표 질문인가`를 떠올리면 됩니다.
...
AI 실습에서는 이 네 가지 감각이 계속 섞여 나옵니다. 따라서 처음부터 구현을 외우기보다, 지금 다루는 데이터가 어떤 질문을 요구하는지 먼저 봅니다.

## 짧은 점검

- 배열, 표, 트리, 그래프를 한 문장씩 구분할 수 있는가?
- 같은 데이터를 왜 다른 구조로도 읽을 수 있는지 설명할 수 있는가?
- CSV와 임베딩 벡터가 왜 같은 구조 질문이 아닌지 말할 수 있는가?
...
- 데이터를 숫자 배열로 볼지, 행과 열의 표로 볼지, 계층 구조로 볼지, 관계망으로 볼지 선택해야 할 때 이 비교를 먼저 떠올립니다.
- 같은 대상을 두고도 질문이 달라지면 적합한 구조가 달라진다는 점을 설명해야 할 때 이 절의 네 가지 직관이 기준이 됩니다.
- AI 실습에서 토큰, 임베딩, 데이터셋, 문서 구조, 지식 연결을 서로 다른 구조 감각으로 분리해 읽어야 할 때 다시 확인합니다.
