...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 왜 같은 데이터를 여러 구조로 보나? | 던지는 질문이 달라지면 잘 보이는 구조도 달라지기 때문입니다. |
| 배열, 표, 트리, 그래프는 무엇이 다른가? | 위치, 행과 열, 계층, 연결이라는 서로 다른 읽기 기준을 가집니다. |
| 무엇을 먼저 기억하면 되는가? | 자료구조 이름보다 어떤 질문에 강한지 먼저 기억하면 됩니다. |
...
배열은 “숫자 계산을 위한 구조”에 가깝습니다. 그래서 P2-11의 NumPy 배열, P2-3장의 벡터·행렬, 그리고 Part 4 이후의 텐서(tensor) 설명에서 다시 등장합니다.
...
### 사례 1. 같은 학급 데이터를 네 가지 구조로 읽어 보면 무엇이 달라지는가

한 학급의 학생 이름, 점수, 소속 반, 친구 관계가 한꺼번에 들어 있는 데이터가 있다고 하겠습니다. 사람은 처음에 이 데이터를 하나의 표로만 보고 싶어 할 수 있습니다.

하지만 질문이 바뀌면 잘 보이는 구조도 바뀝니다. 점수만 계산하고 싶을 때는 배열이 자연스럽고, 학생별 속성을 비교할 때는 표가 편하며, 학교-반-학생처럼 소속 체계를 볼 때는 트리가 맞고, 친구 관계를 따라갈 때는 그래프가 더 직접적입니다.

이 절은 바로 이 전환을 보여 줍니다. 배열, 표, 트리, 그래프는 경쟁하는 정답 구조가 아니라, 서로 다른 질문에 강한 관점입니다. 따라서 같은 원천 데이터를 여러 방식으로 다시 읽을 수 있어야 이후 AI 데이터셋, 임베딩, 문서 구조, 링크 관계도 더 잘 해석할 수 있습니다.

확인 가능한 결과는 질문별로 코드가 달라지는지입니다. 점수 목록만 뽑아 평균을 낼 때와, 반별 이름을 묶을 때와, 친구 연결을 볼 때 서로 다른 표현이 더 간단해진다면 구조를 바꾼 의미가 분명해집니다.
