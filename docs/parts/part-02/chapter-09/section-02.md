...
P2-9.1에서는 자료구조 선택(data structure)가 데이터를 어떤 모양으로 조직하느냐의 문제라고 봤습니다. 이제 AI 실습에서 자주 만나는 네 가지 모양을 넓게 비교합니다.
...
## 핵심 기준: 배열(array), 표(table), 트리(tree), 그래프(graph) 직관
...
## 배열, 표, 트리, 그래프 직관: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "배열, 표, 트리, 그래프의 차이를 깊지 않게 정리해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
