...
Part 2 Chapter 9에서는 `배열(array)`, `표(table)`, `트리(tree)`, `그래프(graph)`를 한 번에 비교하는 대표 설명을 이 절에 둡니다. 뒤 절에서 그래프를 따로 더 보더라도, 네 구조가 서로 어떤 질문에 답하는지는 여기 기준으로 다시 연결합니다.
...
지금 읽는 층위는 `자료구조 이름을 한 번에 외우는 절`이 아니라 `어떤 질문이 어떤 구조를 자연스럽게 부르는지 비교하는 층위`입니다. 앞 절에서 자료구조가 왜 필요한지 큰 그림을 봤다면, 여기서는 그 큰 그림을 배열, 표, 트리, 그래프라는 네 가지 대표 시야로 펼쳐 봅니다. 이 구분을 먼저 잡아 두면, 뒤에서 NumPy와 Pandas를 다시 만날 때도 `도구 이름`보다 `지금 내가 어떤 구조 질문을 하고 있는가`를 먼저 떠올리기 쉬워집니다.
...
데이터의 이름보다 질문을 먼저 적어 보는 편이 좋습니다.
