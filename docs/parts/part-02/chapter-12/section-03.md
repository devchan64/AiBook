...
여기서는 `데이터셋(dataset)`, `특징(feature)`, `타깃(target)`, `검증(validation)`, `데이터 누수(data leakage)`의 기본 구분을 설명합니다. `DataFrame`과 표 선택의 대표 설명은 P2-12.1, P2-12.2와 [개념사전의 데이터셋 항목](../../../reference/concept-glossary-parts/03-digeut.md#dataset)에 두고, 여기서는 그 표를 학습용 입력과 정답으로 어떻게 다시 구성할지 설명합니다.
...
## 학습용 데이터셋 준비의 직관: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "학습용 데이터셋 준비가 원시 표를 그대로 쓰는 일이 아니라는 점을 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
- 데이터셋 준비는 표를 그대로 쓰는 일이 아니라 문제 정의에 맞게 `X`와 `y`를 나누는 일이라는 점을 설명할 수 있는가?
