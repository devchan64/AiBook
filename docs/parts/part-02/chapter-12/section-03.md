...
Part 2 Chapter 12에서는 `데이터셋(dataset)`, `특징(feature)`, `타깃(target)`, `검증(validation)`, `데이터 누수(data leakage)`의 대표 설명 위치를 이 절에 둡니다. 앞 절들이 표 구조와 표 선택을 다뤘다면, 그 표를 학습용 입력과 정답으로 재구성하는 기준은 여기 설명을 기준으로 다시 연결합니다.

이 절의 위치를 Part 2 후반 흐름 안에서 가장 짧게 다시 묶으면 다음과 같습니다. Chapter 11이 `계산 가능한 배열 모양`을 만들었다면, 지금 Chapter 12는 `그 배열로 넘기기 전 표에서 어떤 열을 남기고 어떤 열을 뺄지 결정하는 층위`를 맡습니다. 그리고 이렇게 정리한 입력과 정답 후보는 바로 다음 Chapter 13에서 그래프로 다시 확인하고, Chapter 14에서 변경 이유와 실험 조건을 기록으로 남기게 됩니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 표를 그대로 학습시키면 안 되는 이유는 무엇인가? | 질문과 무관한 열, 정답 열, 식별자 열이 섞여 있을 수 있기 때문입니다. |
| `X`와 `y`는 왜 나누는가? | 입력과 맞히려는 대상을 분리해야 학습 구조가 선명해집니다. |
| 왜 먼저 데이터를 나누는가? | 나중 평가에 쓸 정보를 미리 학습에 새어 넣지 않기 위해서입니다. |
...
처음 읽는 독자가 자주 겪는 혼동은 `눈에 보이는 모든 열이 특징(feature)인 것처럼 느껴지는 것`입니다. 하지만 식별자(identifier)와 정답(target)은 특징과 역할이 다릅니다.
