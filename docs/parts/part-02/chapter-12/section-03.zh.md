...
本节说明 `数据集(dataset)`、`特征(feature)`、`目标(target)`、`验证(validation)`、`数据泄漏(data leakage)` 的基本区分。`DataFrame` 与表选择的代表性说明放在 P2-12.1、P2-12.2 与[数据集词条](/AiBook/zh/reference/concept-glossary-pinyin/d/#dataset)中，这里关注的是：如何把那张表重新组织成学习输入与答案。
