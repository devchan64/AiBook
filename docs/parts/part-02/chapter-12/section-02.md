...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 열 선택과 행 필터는 무엇이 다른가? | 하나는 무엇을 읽을지 고르고, 다른 하나는 무엇을 남길지 고릅니다. |
| `loc`와 `iloc`는 왜 나뉘는가? | 라벨 기준과 위치 기준을 구분하기 위해서입니다. |
| `groupby`는 무엇을 하는가? | 같은 범주끼리 묶은 뒤 요약값을 계산합니다. |
...
이런 불리언 결과를 종종 `mask`라고 부릅니다. 입문 단계에서는 필터링을 `각 행에 질문을 던져서, 맞다(True)고 대답한 행만 남기는 일`로 이해하면 됩니다.
...
Pandas 공식 문서는 `groupby`를 데이터를 어떤 기준으로 나눈 뒤, 각 그룹에 함수를 적용해 결합하는 흐름으로 설명합니다. 입문 단계에서는 `groupby`를 같은 값을 가진 행들끼리 먼저 묶고, 그 묶음마다 집계를 하는 방식으로 이해하면 충분합니다.
...
| --- | --- | --- |
| 표에서 어떤 행과 열을 남기고 어떤 요약값으로 줄일지 정하는 기준 | Matplotlib에서 이 표와 요약을 어떤 모양의 그래프로 확인할까 | 데이터셋을 고르고, 열을 나누고, 범주별 요약을 만든 뒤 모델 입력을 준비할 때 |
