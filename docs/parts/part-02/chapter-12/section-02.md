...
Part 2 Chapter 12에서는 `Series`, `필터링(filtering)`, `집계(aggregation)`, `groupby`, `loc`, `iloc`의 대표 설명 위치를 이 절에 둡니다. 앞 절이 표 구조 자체를 설명했다면, 그 표에서 무엇을 읽고 무엇을 남기고 무엇을 요약할지는 여기 기준으로 다시 연결합니다.
