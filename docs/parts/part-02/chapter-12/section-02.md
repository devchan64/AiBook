...
- 표에서 어떤 행과 열을 남기고, 어떤 기준으로 묶어 요약할지 결정해야 할 때 선택·필터링·집계 관점을 먼저 떠올립니다.
- `loc`, `iloc`, 불리언 필터, `groupby`가 각각 어떤 질문에 답하는지 정리해야 할 때 이 절로 돌아옵니다.
- 원본 표 전체를 보는 단계에서 필요한 부분만 추려 분석 가능한 형태로 바꿔야 할 때 다시 확인합니다.
