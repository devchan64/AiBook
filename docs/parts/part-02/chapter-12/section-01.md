...
지금 읽는 층위는 `사례와 변수의 표 해석 층위`입니다. 앞 장의 NumPy가 `그 값을 실제 계산에 쓸 수 있는 벡터와 행렬 모양으로 어떻게 다룰까`를 다뤘다면, 여기서는 그 숫자 묶음이 어떤 사례와 변수의 표로 읽혀야 하는지 살핍니다. 바로 다음 장에서는 이 표에서 바로 안 보이는 변화와 관계를 그래프로 어떻게 확인할지로 질문이 다시 이동합니다.
...
| Pandas | 그 숫자 묶음이 어떤 사례와 변수인지 표로 읽는다 |
| Matplotlib | 표에서 바로 안 보이는 변화와 관계를 모양으로 본다 |
| Git | 어떤 표와 그래프를 보고 무엇을 바꿨는지 기록으로 남긴다 |

이 표의 핵심은 Pandas가 새 도구 하나를 더 외우는 장이 아니라, NumPy에서 만든 계산 가능한 모양을 `사례와 변수`가 있는 표로 다시 읽게 만드는 중간 단계라는 점입니다.

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| NumPy: 계산 가능한 숫자 모양을 어떻게 만들까 | Pandas DataFrame: 그 숫자 묶음을 어떤 사례와 변수의 표로 읽을까 | Matplotlib: 그 표에서 바로 안 보이는 변화와 관계를 어떤 모양으로 확인할까 |
| 계산 가능한 배열 | 표 해석 구조 | 시각적 확인 |

즉, 지금 장의 핵심은 `숫자를 계산할 수 있는가`에서 `그 숫자가 무엇을 뜻하는 사례와 변수인가`로 손잡이가 바뀐다는 점입니다.

이 흐름을 한 번 더 짧게 눌러 두면 다음과 같습니다.

| 바로 지금 장에서 끝내야 하는 것 | 바로 뒤 장에서 이어질 것 | 마지막에 기록으로 남길 것 |
| --- | --- | --- |
| 한 행을 한 사례로, 한 열을 한 변수로 읽는 표 감각을 만든다 | 필요한 열과 행을 고르고, 그 결과를 그래프로 확인한다 | 어떤 표를 보고 어떤 열을 골랐는지 Git으로 다시 설명할 수 있게 남긴다 |
...
다만 여기서 한 가지를 더 조심해야 합니다. DataFrame에서 `한 행 = 완성된 사례 1건`이라고 바로 단정할 수는 없습니다. 원시 로그처럼 시간 순서대로 쌓인 표에서는 한 행이 동작 전체가 아니라 `동작 중 한 시점의 기록`일 수도 있습니다.
...
이런 표에서는 한 행이 한 동작이 아니라 `한 시점`을 뜻하고, `event_id` 같은 식별자 열이 여러 행을 하나의 동작으로 묶어 주는 기준이 됩니다. 즉, DataFrame을 읽을 때는 `한 행이 곧 한 최종 사례인가`, 아니면 `여러 행이 함께 한 사례를 이루는가`를 먼저 구분해야 합니다.
