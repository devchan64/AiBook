...
- 데이터를 단순한 숫자 배열이 아니라 행과 열 이름이 있는 표로 읽어야 할 때 DataFrame 관점을 먼저 떠올립니다.
- 한 행이 무엇을 뜻하고 한 열이 어떤 속성을 담는지 설명해야 할 때 이 절의 표 해석 기준으로 돌아옵니다.
- 데이터셋을 열어 보자마자 `shape`, `columns`, `index`, `dtypes`, `head()`부터 확인해야 하는 이유를 정리할 때 다시 확인합니다.
