...
여기서는 `운영체제별 터미널 진입 절차`와 `기본 위치 확인 명령`을 보충 설명합니다.

여기서는 Python 문법을 설명하지 않습니다. Python 설치도 깊게 다루지 않습니다. Python 설치는 P2-7.7에서 따로 봅니다. 이 보충학습은 “명령을 어디에 입력하고, 현재 위치를 어떻게 확인하며, 실습 폴더로 어떻게 이동하는가”를 연습합니다.

이 보충학습은 Windows, macOS, Linux에서 터미널을 열고 현재 위치를 확인하고 실습 폴더로 이동하는 절차를 정리합니다. 실행 환경 개념 자체는 P2-7.1과 P2-7.2에서 다룹니다.
...
셸 스크립트(shell script), 파이프(pipe), 리다이렉션(redirection), 관리자 권한, 환경 변수(environment variable)는 여기서 깊게 다루지 않습니다. 이 표현들은 P2-7.8 보충학습에서 따로 회수하고, Part 6에서는 프로젝트 문맥에서 권한과 로그를 다시 연결합니다.
...
| --- | --- | --- |
| 운영체제가 다르면 터미널 앱, 기본 셸, 경로 표기가 조금씩 다르다 | 다른 운영체제 예제를 그대로 복사하면 경로와 명령이 어긋날 수 있다 | 같은 목적이라도 명령 표기가 조금 다를 수 있다고 이해한다 |
| 그래도 공통으로 먼저 보는 것은 현재 위치와 파일 목록이다 | 운영체제가 달라도 실습 전 점검 순서는 크게 다르지 않다 | 위치 확인이 가장 먼저라는 기준을 유지한다 |
| 터미널 단축키는 일반 앱과 다르게 동작할 수 있다 | 복사·붙여넣기와 실행 중단을 혼동하면 작업이 끊길 수 있다 | `Ctrl+C`류 동작을 환경별로 조심해야 한다는 점을 기억한다 |
...
Windows에서 만나는 명령 입력 창은 여러 가지입니다.
...
터미널 단축키는 모두 외울 필요가 없습니다. 우선은 “열기, 복사·붙여넣기, 탭, 명령 보완, 실행 중단” 정도를 기준으로 삼습니다.
...
단축키는 속도를 높이는 도구입니다. 하지만 초반에는 속도보다 현재 위치와 실행하려는 명령을 확인하는 일이 더 중요합니다.
...
여기서는 Python 설치 자체를 해결하지 않습니다. “어떤 명령이 내 환경에서 Python을 가리키는가”를 확인하는 정도로만 봅니다. 설치가 필요해지는 시점은 P2-7.7에서 따로 다룹니다.
...
이 보충학습은 운영체제별 절차를 잠깐 복구하는 자리입니다. 아래 기준을 확인한 뒤 본편으로 다시 돌아갑니다.

| 여기서 해결한 질문 | 돌아갈 본편 질문 | 다시 이어서 읽을 곳 |
| --- | --- | --- |
| Windows, macOS, Linux에서 터미널을 어디서 열고 현재 위치를 어떻게 확인하는가 | 코드가 실제로 어디서 실행되고 어떤 폴더를 기준으로 움직이는가 | 실행 환경의 큰 그림은 P2-7.1, 작업 폴더와 명령 입력 자리는 P2-7.2 |

`터미널을 열 수 있다`, `현재 위치를 확인할 수 있다`, `실습 폴더로 이동할 수 있다`까지 되면 본편으로 이어집니다. Python 설치 판단이 다음으로 막히면 그때 P2-7.7로 넘어갑니다.
...
여기서 줄이려는 차이는 바로 그 운영체제별 표기 차이입니다. 핵심은 모든 명령을 외우는 것이 아니라 `현재 위치 확인`, `파일 목록 확인`, `프로젝트 폴더 이동`이라는 공통 목적을 읽는 것입니다. Windows에서는 `Get-Location`, `Set-Location`, `Get-ChildItem`을, macOS/Linux에서는 `pwd`, `cd`, `ls`를 그 목적에 맞게 대응시키면 됩니다.

확인 가능한 결과는 현재 위치를 보여 주는 명령으로 바로 알 수 있습니다. 운영체제가 달라도 결국 `나는 지금 어느 폴더에 있는가`를 확인할 수 있으면, 경로 표기 차이 때문에 생기는 초기 혼란을 크게 줄일 수 있습니다.
...
- 같은 명령을 복사했는데 Windows와 macOS에서 다르게 보여 운영체제 차이를 먼저 나눠야 할 때 떠올립니다.
- 터미널을 어디서 열고 현재 위치를 어떻게 확인할지부터 막혀 본편 실습 전에 운영체제별 최소 절차를 다시 봐야 할 때 떠올립니다.
- 복사·붙여넣기나 `Ctrl + C` 동작이 일반 앱과 달라 터미널 단축키를 따로 확인해야 할 때 떠올립니다.
