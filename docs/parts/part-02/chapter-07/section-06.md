...
이 보충학습은 운영체제별 절차를 잠깐 복구하는 자리입니다. 아래처럼 본편으로 다시 돌아오면 충분합니다.
...
이 절에서 `터미널을 열 수 있다`, `현재 위치를 확인할 수 있다`, `실습 폴더로 이동할 수 있다`까지 되면 더 오래 머물지 말고 본편으로 돌아오면 됩니다. Python 설치 판단이 다음으로 막히면 그때 P2-7.7로 넘어가면 충분합니다.
