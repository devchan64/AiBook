...
Part 2 Chapter 7에서는 `운영체제별 터미널 진입 절차`와 `기본 위치 확인 명령`의 대표 보충 설명을 이 절에 둡니다. 본편 P2-7.2가 터미널의 개념을 설명하는 자리라면, 이 절은 Windows, macOS, Linux에서 그 개념을 실제 손동작으로 옮기는 보충학습 기준점입니다.
...
P2-7.1이나 P2-7.2를 읽다가 `Windows에서는 어디를 눌러야 하지`, `macOS에서는 왜 경로가 다르지` 같은 질문이 생겼다면 바로 이 절로 돌아오면 됩니다. 여기서 해결해야 하는 것은 운영체제별 절차이지, 실행 환경 개념 자체가 아닙니다.
...
이 절은 Windows, macOS, Linux에서 터미널을 처음 열고 실습 전에 확인할 최소 절차를 다룹니다. 터미널과 셸의 개념 정의는 P2-7.2에서, Python 설치 판단은 P2-7.7에서, 셸 기호와 환경 변수는 P2-7.8에서, 로컬 환경 점검 순서는 P2-7.9에서 다시 다룹니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 운영체제가 다르면 무엇이 달라지는가? | 터미널 앱, 기본 셸, 경로 표기, 단축키 감각이 조금씩 달라집니다. |
| 그래도 공통으로 보는 것은 무엇인가? | 현재 위치, Python 명령, 복사한 명령의 의미는 모두 먼저 확인해야 합니다. |
| 왜 단축키 설명이 필요한가? | 일반 앱에서 익숙한 복사·붙여넣기가 터미널에서는 다르게 동작할 수 있기 때문입니다. |
...
Windows에서 처음 읽는 독자가 만나는 명령 입력 창은 여러 가지입니다.
...
처음 읽는 독자는 문서의 명령을 복사해서 붙여넣는 경우가 많습니다. 이것 자체가 나쁜 습관은 아닙니다. Ubuntu 문서도 숙련된 사용자도 종종 명령을 복사해 붙여넣는다고 설명합니다.
...
이 절에서 `터미널을 열 수 있다`, `현재 위치를 확인할 수 있다`, `실습 폴더로 이동할 수 있다`까지 되면 더 오래 머물지 말고 본편으로 돌아갑니다. Python 설치 판단이 다음으로 막히면 그때 P2-7.7로 넘어갑니다.
...
이 절은 바로 그 차이를 줄이기 위해 있습니다. 핵심은 모든 명령을 외우는 것이 아니라 `현재 위치 확인`, `파일 목록 확인`, `프로젝트 폴더 이동`이라는 공통 목적을 읽는 것입니다. Windows에서는 `Get-Location`, `Set-Location`, `Get-ChildItem`을, macOS/Linux에서는 `pwd`, `cd`, `ls`를 그 목적에 맞게 대응시키면 됩니다.
