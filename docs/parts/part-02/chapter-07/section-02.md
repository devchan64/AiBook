...
처음 읽을 때는 모든 명령을 외우기보다 `어디서 입력하는가`, `누가 해석하는가`, `지금 어디에 있는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
처음 읽는 독자에게 흔한 오류는 코드 문법보다 위치 문제에서 시작합니다. 파일은 `downloads/`에 있는데 터미널은 `home/`에 있을 수 있습니다. 프로젝트 폴더는 `project-name/`에 있는데 터미널은 그 상위 폴더에 있을 수 있습니다.
...
입문 단계에서는 두 용어를 다음처럼 구분하면 충분합니다.
