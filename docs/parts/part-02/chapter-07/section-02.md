...
Part 2 Chapter 7에서는 `터미널(terminal)`, `셸(shell)`, `작업 폴더(working directory)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 Python 실행, 가상환경 활성화, 패키지 설치 명령이 이어져도, 명령을 어디서 입력하고 어느 위치를 기준으로 해석하는지는 여기 설명을 기준으로 다시 연결합니다.

지금 읽는 층위는 `운영체제별 터미널 활용 전체를 배우는 절`이 아니라 `로컬 PC에서 명령을 읽고 실행하는 최소 언어를 만드는 입문 층위`입니다. 여기서 터미널, 셸, 작업 폴더의 역할을 구분해 두면, 뒤에서 Python 파일 실행, 가상환경, 패키지 설치를 볼 때 문제를 코드 오류와 위치 오류로 먼저 나눠 볼 수 있습니다.
...
이 절은 터미널 사용법 전체를 가르치지 않습니다. 운영체제별 터미널을 여는 방법, 화면 구성, 기본 단축키, Windows와 macOS/Linux 명령 차이는 P2-7.6 보충학습으로 넘깁니다. 셸 스크립트(shell script), 파이프(pipe), 리다이렉션(redirection), 권한(permission), 환경 변수(environment variable)는 P2-7.8 보충학습에서 따로 회수합니다. 또한 Python 코드 실행 방식 자체는 P2-7.3, 가상환경 활성화와 패키지 설치는 P2-7.4에서 다시 다룹니다.
