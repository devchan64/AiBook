...
여기서는 터미널 사용법 전체를 가르치지 않습니다. 운영체제별 터미널을 여는 방법, 화면 구성, 기본 단축키, Windows와 macOS/Linux 명령 차이는 P2-7.6 보충학습으로 넘깁니다. 셸 스크립트(shell script), 파이프(pipe), 리다이렉션(redirection), 권한(permission), 환경 변수(environment variable)는 P2-7.8 보충학습에서 따로 회수합니다. 또한 Python 코드 실행 방식 자체는 P2-7.3, 가상환경 활성화와 패키지 설치는 P2-7.4에서 다시 다룹니다.
...
- 파일은 있는데 `python example.py`가 실패해서 코드 문제와 위치 문제를 먼저 나눠야 할 때 떠올립니다.
- 터미널과 셸이 같은 말처럼 섞여 보여 명령을 누가 해석하는지 다시 구분해야 할 때 떠올립니다.
- 상대 경로와 절대 경로, 현재 작업 폴더가 명령 해석에 어떤 영향을 주는지 다시 점검해야 할 때 떠올립니다.
