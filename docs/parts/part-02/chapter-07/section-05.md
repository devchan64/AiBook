...
처음 읽을 때는 모든 도구를 외우기보다 `이 코드는 무엇에 의존하는가`, `그 환경을 다시 만들 수 있는가`, `버전을 기록했는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
이 절은 의존성과 재현성의 입문 개념을 다룹니다. 패키지 배포, lock 파일, dependency resolver, 컨테이너(container), CI 환경은 깊게 다루지 않습니다. 로컬 환경에서 어떤 Python과 패키지를 실제로 보고 있는지 점검하는 순서는 P2-7.9 보충학습에서 다시 묶습니다.
...
이 파트의 실습에서는 처음부터 복잡한 패키지 배포 구조를 다루지 않습니다. 따라서 우선은 `requirements.txt`를 “실습 환경을 다시 만들기 위한 설치 목록”으로 이해하면 충분합니다.
