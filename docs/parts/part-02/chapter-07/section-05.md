...
Part 2 Chapter 7에서는 `의존성(dependency)`, `재현성(reproducibility)`, `requirements.txt`, `버전 고정(version pinning)`의 상세 설명을 이 절에 대표로 둡니다. 뒤에서 예제 프로젝트나 협업 환경을 다시 보더라도, 코드 외에 무엇을 함께 기록해야 하는지는 여기 설명을 기준으로 다시 연결합니다.

지금 읽는 층위는 `Python 패키지 배포 체계를 깊게 배우는 절`이 아니라 `같은 코드를 나중에 다시 실행하려면 무엇을 함께 기록해야 하는지 이해하는 입문 층위`입니다. 여기서 의존성, requirements 파일, 버전 기록의 역할을 잡아 두면, 뒤에서 팀 협업이나 장기 실습을 할 때 `코드만 있으면 충분한가`를 더 정확하게 판단할 수 있습니다.
...
| `requirements.txt`와 버전 기록의 역할 | 운영체제별 설치와 Python 버전 점검 질문으로 넘어갑니다. | Part 3 이후 예제 재실행, 프로젝트 전달, 협업 환경 안내에서 반복됩니다. |
...
이 절은 의존성과 재현성의 입문 개념을 다룹니다. 패키지 배포, lock 파일, dependency resolver, 컨테이너(container), CI 환경은 깊게 다루지 않습니다. 로컬 환경에서 어떤 Python과 패키지를 실제로 보고 있는지 점검하는 순서는 P2-7.9 보충학습에서 다시 묶습니다. 설치 절차 자체는 P2-7.4, 운영체제별 Python 준비는 P2-7.7에서 다시 다룹니다.
...
운영체제별 터미널 사용법은 P2-7.6에서, Python 설치는 P2-7.7에서, 자주 막히는 로컬 환경 점검 순서는 P2-7.9에서 보충학습으로 다룹니다. 특히 `requirements.txt`는 읽히는데도 로컬 PC에서 `import`가 계속 실패한다면, 이 절을 다 읽은 뒤 P2-7.9로 잠깐 이동해 `지금 어느 Python 환경을 보고 있는가`를 먼저 점검하고 돌아오는 편이 좋습니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 의존성은 무엇인가? | 내 코드가 기대고 있는 외부 패키지와 실행 조건입니다. |
| 재현성은 왜 중요한가? | 나중에 같은 결과를 다시 얻지 못하면 학습과 협업이 어려워지기 때문입니다. |
| requirements 파일은 무엇을 남기는가? | 필요한 패키지 목록과 버전 범위를 기록합니다. |
...
문제 상황: 코드가 어떤 외부 패키지에 의존하는지 가장 단순한 예로 확인합니다.
입력(input): NumPy 배열을 만들고 평균을 출력하는 코드입니다.
기대 출력(output): NumPy가 설치되어 있으면 평균값 `2.0`이 출력됩니다.
...
이 구조를 처음부터 모두 외울 필요는 없습니다. 다만 “내가 설치한 하나의 패키지 뒤에 여러 패키지가 함께 따라올 수 있다”는 감각은 필요합니다.

## 예시: 같은 코드인데 어떤 곳에서는 실패한다

다음 상황을 생각해 봅니다. 책의 예제를 내려받아 평균을 계산하는 Python 파일을 실행했습니다.

문제 상황: 같은 평균 계산 코드가 Colab에서는 되지만 로컬 PC에서는 패키지 부족으로 실패할 수 있음을 봅니다.
입력(input): NumPy 배열로 평균을 계산하는 짧은 코드입니다.
기대 출력(output): NumPy가 준비된 환경에서는 평균이 출력되지만, 없는 환경에서는 import 단계에서 실패할 수 있습니다.
...
문제 상황: 다른 사람에게 전달할 작은 실습 코드가 어떤 패키지를 필요로 하는지 확인합니다.
입력(input): `pandas`로 CSV를 읽고 평균을 계산하는 스크립트입니다.
기대 출력(output): 필요한 환경이 준비되면 점수 평균이 출력됩니다.
...
이 파트의 실습에서는 복잡한 패키지 배포 구조를 다루지 않습니다. 따라서 `requirements.txt`를 “실습 환경을 다시 만들기 위한 설치 목록”으로 이해합니다.
...
문제 상황: Colab 노트북을 다시 열었을 때 필요한 패키지를 먼저 다시 준비할 수 있게 합니다.
입력(input): NumPy, pandas, matplotlib 설치를 위한 `%pip` 명령입니다.
기대 출력(output): 현재 Colab 런타임에 세 패키지가 설치됩니다.
...
사람은 쉽게 `노트북 파일이 망가졌나`라고 생각할 수 있습니다. 하지만 실제로는 Colab 런타임이 초기화되었거나, 패키지 버전이 바뀌었거나, 데이터 파일 위치가 달라졌을 가능성이 더 큽니다. 즉 코드 자체보다 실행 조건이 재현되지 않은 문제입니다.

이 절에서 말하는 `의존성`, `재현성`, `requirements.txt`, `버전 고정`은 이런 실패를 줄이기 위한 장치입니다. 코드를 다시 실행할 수 있으려면 어떤 패키지와 어떤 버전을 기대했는지 함께 남겨야 합니다.
