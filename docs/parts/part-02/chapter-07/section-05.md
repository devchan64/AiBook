...
여기서는 Python 패키지 배포 체계를 깊게 배우기보다, 같은 코드를 나중에 다시 실행하려면 무엇을 함께 기록해야 하는지 이해하는 데 집중합니다. 여기서 의존성, requirements 파일, 버전 기록의 역할을 잡아 두면, 뒤에서 팀 협업이나 장기 실습을 할 때 `코드만 있으면 충분한가`를 더 정확하게 판단할 수 있습니다.
...
여기서는 의존성과 재현성의 입문 개념을 다룹니다. 패키지 배포, lock 파일, dependency resolver, 컨테이너(container), CI 환경은 깊게 다루지 않습니다. 로컬 환경에서 어떤 Python과 패키지를 실제로 보고 있는지 점검하는 순서는 P2-7.9 보충학습에서 다시 묶습니다. 설치 절차 자체는 P2-7.4, 운영체제별 Python 준비는 P2-7.7에서 다시 다룹니다.
...
| --- | --- | --- |
| 의존성은 코드가 기대고 있는 외부 패키지와 실행 조건이다 | 코드만 보고는 실행되지 않는 이유를 설명해 준다 | `import`가 보이면 어떤 외부 패키지가 필요한지 연결할 수 있다 |
| 재현성은 같은 코드를 나중에 다시 실행할 수 있게 조건을 남기는 일이다 | 학습과 협업은 한 번 실행되고 끝나지 않기 때문이다 | 코드 외에 환경 조건도 함께 남겨야 한다고 이해한다 |
| requirements 파일은 필요한 패키지 목록과 버전 범위를 기록한다 | 다른 사람이 환경을 다시 만들 때 출발점이 된다 | `requirements.txt`의 역할을 한 문장으로 설명할 수 있다 |
...
하지만 버전을 고정한다고 모든 문제가 사라지는 것은 아닙니다. 운영체제, Python 버전, 하드웨어, 패키지 배포 상태가 영향을 줄 수 있습니다. 여기서는 이를 `재현성을 높이는 출발점`으로 이해합니다.
...
- 한 달 뒤 다시 연 노트북이 갑자기 안 돼서 코드보다 실행 조건을 먼저 되살려야 할 때 떠올립니다.
- `내 컴퓨터에서는 된다`가 왜 충분하지 않은지 설명해야 해서 의존성과 재현성 관점을 다시 붙잡아야 할 때 떠올립니다.
- `requirements.txt`, 버전 고정, `pip freeze`가 왜 필요한지 환경 재구성 관점으로 다시 묶어야 할 때 떠올립니다.
