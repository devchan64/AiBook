...
假设一个学习者到现在为止一直只在 Colab 里跟着示例做练习。但现在出现了一个情况：必须把一个包含多个 `.py` 文件的项目下载下来，并在本地文件夹里运行。这个时候，学习者首先会问 `现在真的必须安装吗？`、`不能直接上传到 Colab 吗？`

在前期，Colab 可能确实已经足够。但如果必须保留项目文件夹结构、在本地读取数据文件、划分虚拟环境、并直接管理包版本，那么本地 Python 安装就会变得必要。

这一节的作用，不是把安装按钮逐个列出来，而是让读者判断 `什么时候才算到了需要安装的时点`。也就是说，它给出的标准是：`浏览器 runtime 还够不够？` 和 `我是不是已经必须在自己的电脑上直接管理执行环境？`

可确认的结果可以在安装后立刻通过命令看到。如果 `python --version`、`python3 --version`、`py --version` 之中有一个确实能运行本地解释器，并且之后还能继续建立虚拟环境、安装包，那么就说明已经准备好从 Colab 过渡到本地环境了。
