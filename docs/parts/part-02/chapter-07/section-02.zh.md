...
这三个问题看起来很简单，但后面在 Python 脚本执行、虚拟环境激活、包安装、读取数据文件时会不断重复。
...
- 当文件明明存在，但 `python example.py` 失败，需要先把问题分成代码问题和位置问题时，先想起这个视角。
- 当 terminal 和 shell 混在一起，看起来像一个词时，需要重新区分到底是谁在解释命令，先想起这个视角。
- 当你需要重新确认相对路径、绝对路径、当前工作文件夹会怎样影响命令解释时，先想起这个视角。
