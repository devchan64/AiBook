...
Part 2 Chapter 7에서는 `가상환경(virtual environment)`, `패키지(package)`, `pip`, `import`의 관계를 이 절에 대표로 둡니다. 뒤 절에서 의존성 목록이나 환경 점검 순서를 다시 보더라도, 설치와 사용을 어느 Python 공간에 연결해야 하는지는 여기 설명을 기준으로 다시 읽습니다.
...
이 절은 가상환경(virtual environment)과 패키지(package)의 개념을 설명합니다. 운영체제별 가상환경 활성화 명령, Python 설치 방법, PATH 설정, 권한 문제는 깊게 다루지 않습니다. 이런 환경 점검 순서는 P2-7.9 보충학습에서 다시 묶습니다. 의존성 목록 저장과 버전 기록의 상세 설명은 P2-7.5에서 다시 다룹니다.
