...
여기서는 가상환경(virtual environment)과 패키지(package)의 개념을 설명합니다. 운영체제별 가상환경 활성화 명령, Python 설치 방법, PATH 설정, 권한 문제는 깊게 다루지 않습니다. 이런 환경 점검 순서는 P2-7.9 보충학습에서 다시 묶습니다. 의존성 목록 저장과 버전 기록의 상세 설명은 P2-7.5에서 다시 다룹니다.
...
- Python은 실행되는데 NumPy 같은 패키지가 없다고 나와 실행 공간과 패키지 준비 상태를 함께 봐야 할 때 떠올립니다.
- `pip install`은 했는데 `import`가 실패해서 설치와 사용이 서로 다른 단계라는 점을 다시 붙잡아야 할 때 떠올립니다.
- Colab 런타임과 로컬 `.venv`를 같은 공간처럼 착각하고 있어 어떤 환경에 설치했는지 다시 구분해야 할 때 떠올립니다.
