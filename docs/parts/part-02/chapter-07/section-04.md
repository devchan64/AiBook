...
처음에는 Python이 하나만 있으면 충분해 보입니다. 곧바로 `Python을 설치하고`, `필요한 패키지를 설치하고`, `코드를 실행하면 끝난다`고 생각하기 쉽습니다.
...
입문 단계에서는 가상환경을 `프로젝트별 Python 실행 공간`으로 이해하면 충분합니다.
