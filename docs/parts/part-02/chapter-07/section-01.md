...
Part 2 Chapter 7에서는 `로컬 환경(local environment)`과 `실행 환경(runtime)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 `터미널`, `인터프리터`, `가상환경`, `의존성`이 이어서 나오더라도, 이 두 용어의 큰 그림은 여기 설명을 기준으로 다시 연결합니다.
...
이 절은 Python 설치법을 단계별로 안내하지 않습니다. Python 설치는 P2-7.7의 보충학습으로 넘깁니다. 운영체제별 설치 화면, 권한 설정, IDE 설정, 패키지 충돌 해결도 이 절에서는 깊게 다루지 않습니다. 또한 터미널 명령의 문법, 인터프리터 실행 방식, 가상환경 생성 절차, 의존성 기록 방식의 상세 설명은 각각 P2-7.2, P2-7.3, P2-7.4, P2-7.5에서 다시 회수합니다.
...
이 절은 설치 절차를 배우는 절이 아니라, 코드가 도는 자리를 구분하는 절입니다. 아래 세 가지 관점이 뒤 절의 설치, 터미널, 가상환경 설명을 묶는 기준이 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 코드는 `어딘가에서 실행`되어야 한다는 점 | 그래야 파일에 적힌 글자와 실제 실행을 구분할 수 있습니다. | 코드만 쓰면 끝이 아니라 실행하는 프로그램과 자리가 필요합니다. |
| Python 인터프리터는 `Python 코드를 읽고 실행하는 프로그램`이라는 점 | 터미널, 스크립트, 노트북 실행이 한 흐름으로 묶입니다. | `print("hello")`를 누가 읽는지 이 구분으로 설명할 수 있습니다. |
| Colab과 로컬 PC는 둘 다 실행 환경이지만 `관리 주체`가 다르다는 점 | 이후 가상환경과 의존성 이야기가 왜 필요한지 연결됩니다. | Colab은 서비스가, 로컬은 사용자가 더 많이 관리합니다. |
...
이 절에서는 로컬 환경을 `내 컴퓨터에서 Python 코드가 실행되는 조건`으로 다룹니다.
...
처음 읽은 뒤에는 아래 경계를 다시 말할 수 있어야 합니다.
