...
Python 설치는 Chapter 7 앞부분에서 바로 다루지 않습니다. 처음부터 설치 화면과 운영체제별 차이를 만나면, 코드가 실행되는 기본 개념보다 설치 문제에 먼저 막힐 수 있습니다. 이 책의 초반 실습은 Colab 같은 브라우저 실행 환경으로도 어느 정도 대체할 수 있습니다. 따라서 먼저 Colab으로 코드 실행 감각을 만들고, Chapter 7 후반부의 보충학습에서 로컬 PC에 Python을 설치해야 하는 시점과 확인 방법을 따로 다룹니다.
