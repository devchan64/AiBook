...
지금 읽는 층위는 `Python 설치 화면을 하나씩 따라가는 절`이 아니라 `계산을 실제로 어디서 실행하고 누가 관리하는지 구분하는 입문 층위`입니다. 여기서 로컬 환경, 실행 환경, Colab, 로컬 PC의 관계를 먼저 잡아 두면, 뒤에서 터미널, 인터프리터, 가상환경을 볼 때 `새 도구를 또 하나 배우는가`가 아니라 `같은 실행 자리를 더 세밀하게 나누는가`로 읽을 수 있습니다.

처음 읽을 때는 운영체제별 절차를 다 외우기보다 `코드는 어디서 실행되는가`, `그 실행 자리를 내가 직접 관리하는가`, `뒤 절들이 그 자리를 어떻게 더 세분화하는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
이후 Chapter 7~10을 읽을 때는 다음 세 층을 나눠 두면 덜 흔들립니다.
...
이 흐름을 Chapter 7~10 기준으로 더 짧게 묶으면 다음과 같습니다.
...
이 절은 Python 설치법을 단계별로 안내하지 않습니다. Python 설치는 P2-7.7의 보충학습으로 넘깁니다. 운영체제별 설치 화면, 권한 설정, IDE 설정, 패키지 충돌 해결도 이 절에서는 깊게 다루지 않습니다.
...
> 7.4 가상환경과 패키지
> 7.5 의존성과 재현성
> 7.6 보충학습: Windows, macOS, Linux 터미널 사용법
> 7.7 보충학습: Python 설치는 언제 필요한가

Python 설치는 P2-7 앞부분에서 바로 다루지 않습니다. 처음부터 설치 화면과 운영체제별 차이를 만나면, 코드가 실행되는 기본 개념보다 설치 문제에 먼저 막힐 수 있습니다. 이 책의 초반 실습은 Colab 같은 브라우저 실행 환경으로도 어느 정도 대체할 수 있습니다. 따라서 먼저 Colab으로 코드 실행 감각을 만들고, P2-7.7 보충학습에서 로컬 PC에 Python을 설치해야 하는 시점과 확인 방법을 따로 다룹니다.
...
처음 읽을 때는 아래 기준으로만 갈라 읽어도 충분합니다.
...
즉, 운영체제별 절차나 설치 판단이 막히는 경우에만 잠깐 보충학습으로 빠지고, 그렇지 않으면 본편 흐름을 계속 읽는 편이 더 안전합니다.
...
이 절은 설치 절차를 배우는 절이 아니라, 코드가 도는 자리를 구분하는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 코드는 `어딘가에서 실행`되어야 한다는 점 | 그래야 파일에 적힌 글자와 실제 실행을 구분할 수 있습니다. | 코드만 쓰면 끝이 아니라 실행하는 프로그램과 자리가 필요하다는 점만 알면 충분합니다. |
| Python 인터프리터는 `Python 코드를 읽고 실행하는 프로그램`이라는 점 | 터미널, 스크립트, 노트북 실행이 한 흐름으로 묶입니다. | `print("hello")`를 누가 읽는지 설명할 수 있으면 충분합니다. |
| Colab과 로컬 PC는 둘 다 실행 환경이지만 `관리 주체`가 다르다는 점 | 이후 가상환경과 의존성 이야기가 왜 필요한지 연결됩니다. | Colab은 서비스가, 로컬은 사용자가 더 많이 관리한다는 점만 알면 충분합니다. |
...
| --- | --- | --- |
| 코드는 어디서 실행되고 누가 읽는가 | 값, 변수, 자료 구조를 어떤 문장으로 적는가 | 노트북 셀 순서, 결과 기록, 공유 문서 정리 |
