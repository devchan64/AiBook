...
여기서는 Python 설치법을 단계별로 안내하지 않습니다. Python 설치는 P2-7.7의 보충학습으로 넘깁니다. 운영체제별 설치 화면, 권한 설정, IDE 설정, 패키지 충돌 해결도 깊게 다루지 않습니다. 또한 터미널 명령의 문법, 인터프리터 실행 방식, 가상환경 생성 절차, 의존성 기록 방식의 상세 설명은 각각 P2-7.2, P2-7.3, P2-7.4, P2-7.5에서 다시 회수합니다.
...
P2-3.4에서 Colab과 로컬 PC 실행을 이미 봤지만, 여기서는 중복을 허용합니다. 같은 내용을 실습 관점이 아니라 “프로그래밍을 시작하기 위한 언어”로 다시 설명합니다.
...
- Colab에서는 되는데 로컬 PC에서는 안 되는 문제를 코드 자체보다 실행 자리 문제로 먼저 나눠야 할 때 떠올립니다.
- 터미널, 인터프리터, 가상환경, 패키지라는 말이 한꺼번에 나오는데 어느 층위의 이야기인지 다시 구분해야 할 때 떠올립니다.
- Chapter 7 이후의 터미널, 인터프리터, 가상환경 절이 모두 같은 실행 환경 지도를 세분화하는 것임을 다시 붙잡아야 할 때 떠올립니다.
