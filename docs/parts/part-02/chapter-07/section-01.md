...
1. 7.1 실행 환경의 큰 그림
2. 7.2 터미널과 작업 폴더
3. 7.3 Python 인터프리터와 스크립트
4. 7.4 가상환경과 패키지
5. 7.5 의존성과 재현성
6. 7.6 보충학습: Windows, macOS, Linux 터미널 사용법
7. 7.7 보충학습: Python 설치는 언제 필요한가
...
입문 단계에서 기억할 핵심은 Python이 처음부터 사람이 읽고 쓰기 쉬운 고수준 언어이면서, 실제 시스템 작업에도 연결되는 실행 도구였다는 점입니다.
...
입문 단계에서는 Python 코드와 Python 인터프리터를 다음처럼 구분하면 충분합니다.
...
입문 단계에서는 로컬 환경을 `내 컴퓨터에서 Python 코드가 실행되는 조건`으로 이해하면 충분합니다.
...
입문 단계에서는 가상환경을 `프로젝트별 Python 실행 공간`으로 이해하면 됩니다.
...
실습을 시작할 때는 네 가지를 먼저 확인합니다.

1. 어디에서 실행하는가?
2. 무엇이 실행하는가?
3. 어떤 패키지가 필요한가?
4. 그 패키지는 어디에 설치되어 있는가?
