...
Part 2 Chapter 7에서는 `Python 설치가 필요한 시점`, `설치 뒤 첫 확인 명령`, `설치와 가상환경의 구분`에 대한 대표 보충 설명을 이 절에 둡니다. 본편이 실행 환경의 구조를 설명했다면, 이 절은 설치가 지금 필요한지 판단하는 기준을 따로 정리하는 보충학습 기준점입니다.

P2-7.1을 읽다가 이 질문이 가장 먼저 떠올랐다면, 그 흐름은 정상입니다. 이 절은 바로 그 질문에 답하기 위해 분리되어 있습니다. 실행 환경의 큰 그림을 먼저 본 뒤, 설치가 지금 필요한지 나중에 필요한지 판단하게 만드는 것이 목적입니다.
...
이 절은 Python 설치의 필요성과 확인 기준을 다룹니다. 운영체제별 화면을 따라 누르는 상세 절차는 다루지 않으며, 설치 뒤 환경 불일치 점검은 P2-7.9에서 다시 회수합니다.
...
이 절은 운영체제별 설치 화면을 길게 따라 하지 않습니다. 특정 버전 설치, PATH 문제 해결, Windows Store와 python.org 설치 방식 비교, Homebrew, pyenv, conda, Docker, WSL 설정은 깊게 다루지 않습니다. PATH와 설치 뒤 자주 막히는 점검 순서는 P2-7.9 보충학습에서 다시 묶고, 세부 설치 방식 비교는 실제 프로젝트 환경을 만들 때 다시 다루는 편이 안전합니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 처음부터 로컬 설치가 꼭 필요한가? | 작은 실습은 Colab만으로도 시작할 수 있습니다. |
| 로컬 설치는 무엇을 준비하는 일인가? | 내 컴퓨터에서 Python 인터프리터를 실행할 기반을 만드는 일입니다. |
| 설치 뒤 가장 먼저 무엇을 확인하는가? | 버전과 실행 명령이 정상적으로 동작하는지 확인합니다. |
...
Windows에서 자주 생기는 문제는 “설치는 했는데 터미널에서 명령을 찾지 못한다”입니다. 이 경우 Python 코드의 문제가 아니라 설치 경로 또는 PATH 설정 문제일 수 있습니다. 이 절에서는 PATH를 직접 수정하는 절차까지 다루지 않습니다. PATH와 환경 변수(environment variable)의 읽는 법은 P2-7.8 보충학습에서 다시 보고, 설치 뒤 점검 순서는 P2-7.9 보충학습에서 다시 묶습니다. 여기서는 공식 문서의 Troubleshooting을 확인하거나 설치 방식을 다시 점검하는 수준까지만 잡습니다.
...
이 절에서 `지금은 Colab으로 충분한가`, `로컬 설치가 필요한 시점인가`, `설치 뒤 어떤 명령으로 확인할 것인가`까지 정리되면 본편으로 돌아오면 됩니다. 운영체제별 설치 세부 절차나 PATH 문제를 여기서 끝까지 붙들기보다, 다음 본편 질문으로 다시 이어 가는 편이 더 안전합니다.
