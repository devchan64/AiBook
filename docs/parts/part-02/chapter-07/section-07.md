...
여기서는 `Python 설치가 필요한 시점`, `설치 뒤 첫 확인 명령`, `설치와 가상환경의 구분`을 설명합니다.

이 질문은 실행 환경의 큰 그림을 본 뒤 자연스럽게 이어집니다. 여기서는 설치가 지금 필요한지, 나중에 필요한지 판단하는 기준을 정리합니다.

처음부터 반드시 설치해야 하는 것은 아닙니다. 이 파트의 앞쪽 실습은 Colab 같은 브라우저 실행 환경으로도 따라갈 수 있습니다. 하지만 어느 시점부터는 로컬 PC(local PC)에 Python을 설치하고, 가상환경(virtual environment)을 만들고, 패키지(package)를 직접 관리해야 하는 이유가 생깁니다.

여기서는 설치 버튼을 하나씩 따라 누르기보다, “언제 설치가 필요한지”, “설치 뒤 무엇을 확인해야 하는지”, “설치와 가상환경을 어떻게 구분해야 하는지”를 판단하는 데 집중합니다. 운영체제별 화면은 계속 바뀔 수 있고, Python 배포 방식도 시간이 지나며 바뀔 수 있습니다.

| 용어 | 이 절에서 먼저 잡을 뜻 |
| --- | --- |
| 로컬 설치(local installation) | 내 컴퓨터에서 Python 인터프리터를 직접 실행할 수 있게 준비하는 일입니다. |
| Colab 시작 단계 | 설치 없이 브라우저에서 실습 감각을 먼저 만드는 단계입니다. |
| `python --version` / `python3 --version` / `py --version` | 내 환경에서 어떤 Python 명령이 실제로 연결되는지 확인하는 첫 점검 명령입니다. |
| 설치와 가상환경의 차이 | 설치는 Python 자체를 준비하는 일이고, 가상환경은 설치된 Python 위에 프로젝트별 공간을 나누는 일입니다. |
| 공식 설치 문서 | 운영체제별 최신 설치 방식과 주의점을 확인하는 기준 자료입니다. |
...
여기서는 Python 설치의 필요성과 확인 기준을 다룹니다. 운영체제별 화면을 따라 누르는 상세 절차는 다루지 않으며, 설치 뒤 환경 불일치 점검은 P2-7.9에서 다시 회수합니다.
...
여기서는 운영체제별 설치 화면을 길게 따라 하지 않습니다. 특정 버전 설치, PATH 문제 해결, Windows Store와 python.org 설치 방식 비교, Homebrew, pyenv, conda, Docker, WSL 설정은 깊게 다루지 않습니다. PATH와 설치 뒤 자주 막히는 점검 순서는 P2-7.9 보충학습에서 다시 묶고, 세부 설치 방식 비교는 실제 프로젝트 환경을 만들 때 다시 다룹니다.
...
| --- | --- | --- |
| 처음부터 로컬 설치가 꼭 필요한 것은 아니다 | 작은 실습은 Colab만으로도 시작할 수 있기 때문이다 | Colab과 로컬 설치의 역할 차이를 구분한다 |
| 로컬 설치는 내 컴퓨터에서 Python 인터프리터를 실행할 기반을 만드는 일이다 | 설치와 가상환경, 패키지 준비를 한 덩어리로 보면 판단이 흐려진다 | 설치가 무엇을 준비하는 일인지 설명할 수 있다 |
| 설치 뒤 가장 먼저 확인할 것은 버전과 실행 명령이다 | 설치 성공과 명령 연결 성공은 같은 일이 아니다 | `python --version`류 점검의 목적을 설명할 수 있다 |
...
Colab은 브라우저에서 실행되므로 Python 설치 문제를 뒤로 미룰 수 있습니다. 이것이 큰 장점인 이유는 설치, PATH, 운영체제 권한, 터미널 차이를 만나기 전에 “코드가 실행된다”는 감각을 먼저 만들 수 있기 때문입니다.
...
따라서 여기서는 먼저 Colab으로 실행 감각을 만들고, 이후 로컬 PC에서 프로젝트 단위 실습을 할 때 Python 설치를 본격적으로 생각합니다.
...
여기서 막히면 Python 코드를 고치기 전에 설치와 명령 연결 문제를 먼저 의심해야 합니다.
...
Windows에서 자주 생기는 문제는 “설치는 했는데 터미널에서 명령을 찾지 못한다”입니다. 이 경우 Python 코드의 문제가 아니라 설치 경로 또는 PATH 설정 문제일 수 있습니다. 여기서는 PATH를 직접 수정하는 절차까지 다루지 않습니다. PATH와 환경 변수(environment variable)의 읽는 법은 P2-7.8 보충학습에서 다시 보고, 설치 뒤 점검 순서는 P2-7.9 보충학습에서 다시 묶습니다. 여기서는 공식 문서의 Troubleshooting을 확인하거나 설치 방식을 다시 점검하는 수준까지만 잡습니다.
...
여기서는 4번의 운영체제별 활성화 명령을 외우게 하지 않습니다. 활성화 명령은 Windows, macOS, Linux, 셸 종류에 따라 다르게 보일 수 있습니다. 지금은 “설치한 Python 하나를 모든 프로젝트가 같이 쓰지 않고, 프로젝트별 환경을 만든다”는 방향을 기억합니다.
...
Colab으로도 시작할 수 있습니다. 하지만 프로젝트 폴더, 데이터 파일, 가상환경, 패키지 버전, 재현성을 직접 관리해야 할 때는 로컬 설치가 필요해집니다.
...
이 보충학습을 읽고 나면 설치 절차를 모두 끝냈는지보다, 어느 본편 질문으로 돌아가야 하는지가 더 중요합니다.

| 여기서 해결한 질문 | 돌아갈 본편 질문 | 다시 이어서 읽을 곳 |
| --- | --- | --- |
| 지금 로컬 Python 설치가 필요한가, 설치 뒤 무엇을 먼저 확인해야 하는가 | 실행 환경, 인터프리터, 가상환경, 패키지 관리가 왜 분리되어야 하는가 | 실행 환경의 큰 그림은 P2-7.1, 인터프리터와 스크립트는 P2-7.3, 가상환경과 패키지는 P2-7.4 |

여기서는 `지금은 Colab으로 충분한가`, `로컬 설치가 필요한 시점인가`, `설치 뒤 어떤 명령으로 확인할 것인가`를 정리합니다. 운영체제별 설치 세부 절차와 PATH 문제는 실제 프로젝트 환경을 다룰 때 다시 연결합니다.

## 사례로 보기

### 사례 1. 언제까지 Colab으로 하고, 언제부터 로컬 설치가 필요한가

한 학습자가 지금까지는 Colab에서만 예제를 따라왔다고 하겠습니다. 그런데 이제는 `.py` 파일 여러 개가 있는 프로젝트를 내려받아 로컬 폴더에서 실행해야 하는 상황이 생겼습니다. 이때 사람은 `지금 꼭 설치해야 하나`, `그냥 Colab에 올리면 안 되나`를 먼저 묻게 됩니다.

초반에는 Colab이 충분했을 수 있습니다. 하지만 프로젝트 폴더 구조를 유지해야 하고, 데이터 파일을 로컬에서 읽고, 가상환경을 나누고, 패키지 버전을 직접 관리해야 한다면 로컬 Python 설치가 필요해집니다.

이 절의 역할은 설치 버튼을 나열하는 것이 아니라 `설치가 필요한 시점`을 판단하게 만드는 데 있습니다. 즉 `브라우저 런타임으로도 충분한가`와 `내 컴퓨터에서 실행 환경을 직접 관리해야 하는가`를 구분하는 기준을 주는 것입니다.

확인 가능한 결과는 설치 뒤 명령으로 바로 볼 수 있습니다. `python --version`, `python3 --version`, `py --version` 중 하나로 로컬 인터프리터가 실제로 실행되고, 이후 가상환경과 패키지 설치가 이어진다면 Colab에서 로컬 환경으로 넘어갈 준비가 된 것입니다.
...
- 처음부터 로컬 설치가 꼭 필요한지 판단이 서지 않아 Colab과 로컬 설치의 역할을 다시 나눠야 할 때 떠올립니다.
- 설치 뒤 무엇부터 확인해야 하는지 모르겠어서 `python --version`, `python3 --version`, `py --version` 같은 점검 순서를 다시 붙잡아야 할 때 떠올립니다.
- Python 설치와 가상환경 생성을 같은 일처럼 받아들이고 있어 준비 단계와 프로젝트 분리 단계를 다시 구분해야 할 때 떠올립니다.
