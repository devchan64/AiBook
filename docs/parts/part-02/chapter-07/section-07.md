...
Windows에서 자주 생기는 문제는 “설치는 했는데 터미널에서 명령을 찾지 못한다”입니다. 이 경우 Python 코드의 문제가 아니라 설치 경로 또는 PATH 설정 문제일 수 있습니다. 이 절에서는 PATH를 직접 수정하는 절차까지 다루지 않습니다. PATH와 환경 변수(environment variable)의 읽는 법은 P2-7.8 보충학습에서 다시 보고, 여기서는 공식 문서의 Troubleshooting을 확인하거나 설치 방식을 다시 점검하는 수준까지만 잡습니다.
