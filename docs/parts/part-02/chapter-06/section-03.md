...
| --- | --- | --- |
| 경사하강법이 손실을 줄이는 반복 이동이라는 점 | Part 4의 `P4-5.1`, `P4-5.2`에서 역전파가 이 방향 정보를 어떻게 계산하는지 봅니다. | P4-5의 역전파, P4-7의 옵티마이저 설명에서 다시 씁니다. |
...
여기서는 경사하강법(gradient descent)의 직관을 다룹니다. P2-4.5에서 그래디언트(gradient), 편미분(partial derivative), 경사하강법의 이름을 보충학습으로 다뤘다면, 이제 최적화 장의 흐름 안에서 경사하강법이 어떤 역할을 하는지 정리합니다.
...
| 그래디언트 반대 방향으로 움직여야 한다 | 그래디언트는 손실이 커지는 방향 정보이므로 목표와 반대로 읽어야 한다 | 왜 `descent`가 되는지 설명할 수 있으면 충분하다 |
...
공통 장면으로 읽으면 `\(\theta\)`는 직선의 `a`, `b`이고, `\(J(\theta)\)`는 학생 네 명 전체에서 계산한 평균 손실입니다. 식은 결국 `현재 직선이 데이터에 얼마나 안 맞는지 보고, 더 잘 맞는 직선 쪽으로 조금 이동한다`는 말을 압축해 적은 것입니다.
...
독자가 여기서 이해해야 할 핵심은 `정답 직선을 사람이 직접 적어 넣는 것`과 `손실을 보고 직선을 조금씩 조정하는 것`의 차이입니다. 전자는 규칙을 직접 쓰는 쪽에 가깝고, 후자는 데이터와 손실을 기준으로 학습하는 쪽에 가깝습니다.
...
- 손실을 줄인다는 말은 이해했지만 실제로 파라미터를 어떻게 바꾸는지 연결이 끊길 때 먼저 떠올립니다.
- 그래디언트 방향과 그 반대 방향 이동이 왜 필요한지 다시 분리해서 설명해야 할 때 먼저 떠올립니다.
- 학습률이 너무 크거나 작은 상황을 해석할 때 `발걸음 크기` 감각으로 다시 읽어야 할 때 먼저 떠올립니다.
