...
Part 2 안에서는 이 절을 `경사하강법(gradient descent)`, `그래디언트(gradient)`, `학습률(learning rate)`, `업데이트(update)`, `반복(iteration)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 6.2에서 손실과 목적 함수를 읽었다면, 여기서는 그 값을 줄이기 위해 파라미터를 어떻게 조금씩 움직이는지 정리합니다.

지금 읽는 층위는 `경사하강법 공식을 엄밀하게 유도하는 절`이 아니라 `손실을 줄이기 위해 값을 조금씩 바꾸는 반복 방법을 읽는 입문 층위`입니다. 여기서 그래디언트 방향, 반대 방향 이동, 학습률의 역할을 잡아 두면, 뒤에서 역전파(backpropagation)와 옵티마이저(optimizer)가 등장해도 왜 그런 계산이 필요한지 먼저 이해할 수 있습니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 경사하강법이 손실을 줄이는 반복 이동이라는 점 | 뒤의 Part 4에서 역전파가 이 방향 정보를 어떻게 계산하는지 봅니다. | P4-5의 역전파, P4-7의 옵티마이저 설명에서 다시 씁니다. |
| 그래디언트 반대 방향으로 움직이는 이유 | 학습률과 시작 위치가 결과에 어떤 영향을 주는지로 이어집니다. | 튜닝, 학습 안정성, 수렴 해석 문맥에서 바탕이 됩니다. |
| 학습률이 발걸음 크기라는 감각 | 이후 Adam, SGD 같은 옵티마이저 비교 질문으로 넘어갑니다. | Part 5의 미세조정과 운영 튜닝 문맥에서도 다시 등장합니다. |
...
이 절은 경사하강법(gradient descent)의 직관을 다룹니다. P2-4.5에서 그래디언트(gradient), 편미분(partial derivative), 경사하강법의 이름을 보충학습으로 다뤘다면, 여기서는 최적화 장의 흐름 안에서 경사하강법이 어떤 역할을 하는지 정리합니다.
...
편미분과 그래디언트의 복습은 P2-4.5에서 이미 다뤘고, 역전파는 P4-5.1에서, 옵티마이저와 학습률은 P4-7.1과 P4-7.2에서 다시 다룹니다. Adam, RMSProp 같은 세부 옵티마이저 비교는 P4-7.2에서 이어집니다. 여기서는 “왜 조금씩 움직이는가”와 “무엇을 조심해야 하는가”에 집중합니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 경사하강법은 무엇을 반복하는가? | 손실이 줄어드는 쪽으로 작은 이동을 반복합니다. |
| 왜 조금만 움직이는가? | 너무 크게 움직이면 오히려 좋은 지점을 지나칠 수 있기 때문입니다. |
| 학습률은 무엇인가? | 한 번의 이동 폭을 정하는 조절 손잡이입니다. |
...
경사하강법은 “AI 모델 내부의 숫자를 바꾸는 방법”으로만 생각하면 멀게 느껴집니다. 업무 관점에서는 목표가 있고, 현재 설정으로 결과를 낸 뒤, 결과가 얼마나 나쁜지 숫자로 보고, 그 숫자가 줄어드는 방향으로 설정을 조금 바꾸고, 다시 확인한다고 이해하면 됩니다.
...
이 절의 목표는 이 식을 유도하는 것이 아닙니다. 식이 말하는 구조를 읽는 것입니다. 즉 현재 파라미터에서 그래디언트 반대 방향으로 학습률만큼 조금 이동한다고 읽으면 됩니다.
...
이 사례는 경사하강법을 `수식을 외우는 절`이 아니라 `반응을 보고 조금씩 기준을 조정하는 반복 절차`로 읽게 해 줍니다. 동시에 학습률이 너무 크면 추천이 흔들리고, 너무 작으면 개선 속도가 지나치게 느려질 수 있다는 점도 함께 보여 줍니다.
...
경사하강법은 손실을 줄이기 위해 파라미터를 조금씩 바꾸는 반복 방법입니다. 손실을 계산하고, 그래디언트로 방향을 읽고, 그래디언트 반대 방향으로 조금 이동한 뒤, 다시 손실을 계산합니다.

이 흐름은 Part 4의 딥러닝에서도 계속 다시 등장합니다. 다만 딥러닝에서는 파라미터가 매우 많고, 그래디언트를 계산하는 일이 복잡합니다. 그래서 P4-5.1, P4-5.2의 역전파(backpropagation)와 P4-7.1, P4-7.2의 옵티마이저(optimizer)가 등장합니다.

지금은 손실 함수는 무엇을 줄일지 정하고, 그래디언트는 어느 방향으로 변하는지 알려 주며, 경사하강법은 그 방향 정보를 이용해 조금씩 값을 바꾼다고 기억합니다.

또한 이 절은 `SGD, Adam, RMSProp` 같은 옵티마이저 비교를 다루는 절도, `역전파 계산식`을 자세히 설명하는 절도 아닙니다. 여기서는 반복 이동의 직관을 먼저 읽고, 더 세부적인 최적화 도구는 뒤 Part에서 다시 좁혀 봅니다.

여기까지가 Part 2의 `계산 언어` 축입니다. 바로 다음 Chapter 7부터는 이 수식과 계산을 독자가 실제로 어디서 실행하고 어떤 환경에서 읽는지로 넘어갑니다. 즉, 이제부터는 더 어려운 수학을 추가하는 대신 `이 계산을 어떤 자리에서 돌리고 확인할 것인가`를 읽는 층위로 이동합니다.
