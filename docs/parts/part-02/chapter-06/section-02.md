...
| 방식 | 계산 | 직관 |
| --- | --- | --- |
| 절댓값 오차(absolute error) | \(\lvert y - \hat{y}\rvert\) | 얼마나 떨어졌는지 본다 |
| 제곱 오차(squared error) | \((y - \hat{y})^2\) | 큰 오차를 더 크게 벌준다 |
| 로그 손실(log loss) | 정답 클래스 확률의 음의 로그 | 확률을 틀리게 자신한 경우를 강하게 벌준다 |

## 정답에 부여한 확률과 로그 손실

분류 모델은 정답이 될 가능성을 확률로 예측합니다. 정답이 ‘스팸’인 메일에 스팸 확률 0.9를 부여한 경우와 0.1을 부여한 경우를 비교해 보겠습니다. 이처럼 정답 클래스가 하나인 분류에서 개별 로그 손실은 `−ln(정답 클래스의 예측 확률)`입니다. `ln`은 자연로그이며, 0과 1 사이 확률의 로그는 음수이므로 앞에 마이너스를 붙입니다.

| 정답 스팸에 부여한 확률 | 개별 로그 손실 | 해석 |
| --- | --- | --- |
| 0.9 | `−ln(0.9) ≈ 0.105` | 정답에 높은 확률 |
| 0.5 | `−ln(0.5) ≈ 0.693` | 두 클래스에 같은 확률 |
| 0.1 | `−ln(0.1) ≈ 2.303` | 정답에 낮은 확률 |

정답에 부여한 확률이 1에 가까우면 손실은 0에 가까워지고, 0에 가까우면 손실은 매우 커집니다. 단순히 맞음·틀림을 세는 정확도와 달리, 로그 손실은 정답에 얼마나 높은 확률을 부여했는지도 반영합니다. 예를 들어 스팸 판정 기준이 0.5라면 0.6과 0.9는 모두 정답으로 판정되지만 로그 손실은 서로 다릅니다.
...
시그마(sigma)는 각 샘플의 제곱 오차를 모두 더하는 계산을 압축합니다. 여기서 n은 샘플 수이고 i는 샘플의 번호입니다. 제곱 오차를 모두 더한 뒤 n으로 나누어 샘플당 평균 손실을 얻습니다. 평균을 구하는 MSE 자체도 손실 함수라고 부르므로, 개별 손실인지 평균 손실인지는 식과 문맥으로 구분합니다.
...
벌점의 강도를 일반적으로 `λ`(람다)로 쓰면 목적 함수는 `평균 손실 + λa²`입니다. `λ≥0`은 두 항의 비중을 정하는 설정값입니다. `λ=0`이면 평균 손실만 비교하고, λ가 커지면 큰 기울기에 더 많은 비용을 부여합니다. 이 사례에서 λ는 학습으로 찾는 `a, b`와 구분하여 미리 정합니다.

| λ | 후보 1: a=10, b=45 | 후보 2: a=12, b=40 | 두 후보 중 작은 목적 함수값 |
| --- | --- | --- | --- |
| 0 | 12.5 | 7.5 | 후보 2 |
| 0.1 | 22.5 | 21.9 | 후보 2 |
| 0.2 | 32.5 | 36.3 | 후보 1 |

벌점은 `a가 어떤 한도를 절대 넘지 못한다`는 제약과 다릅니다. 큰 a에도 비용을 붙여 비교할 뿐 금지하지 않습니다. λ가 크다고 새 데이터의 성능이 반드시 좋아지는 것도 아닙니다. 너무 강하면 데이터의 변화까지 충분히 따라가지 못할 수 있으므로 검증 데이터에서 확인해야 합니다.

도식의 평균 손실과 벌점은 서로 다른 항이며, 두 항을 더한 값이 이 예에서 최소화하는 목적 함수입니다.
...
1. `λ=0.05`일 때 두 직선 후보의 목적 함수값을 계산하고 더 작은 쪽을 고르세요.
2. 정답이 스팸인 메일에 스팸 확률 0.6을 부여했다면 로그 손실은 얼마인가요? 확률 0.9와 정확도 판정은 같아도 손실은 어떻게 다른가요? 스팸 판정 기준은 0.5로 둡니다.
3. 배송 사례에서 ‘절대 오차가 4분을 초과한 주문 수’를 세면 A와 B는 각각 몇 건인가요? 기준이 6분일 때와 선택이 달라지나요?

**풀이:** ① 후보 1은 `12.5+0.05×100=17.5`, 후보 2는 `7.5+0.05×144=14.7`로 후보 2가 작습니다. ② `−ln(0.6)≈0.511`입니다. 두 확률 모두 스팸으로 맞히지만 0.9의 손실 약 0.105가 더 작습니다. ③ A는 3건, B는 1건이므로 이 건수를 최소화하면 B가 낫습니다. 6분 기준에서는 A가 0건, B가 1건이므로 A가 낫습니다. 평가 기준의 이름뿐 아니라 임계값도 선택에 영향을 줍니다.
...
- scikit-learn developers, [log_loss](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.log_loss.html){: target="_blank" rel="noopener noreferrer" }, scikit-learn API Reference, 확인 날짜: 2026-09-15. 로그 손실이 로지스틱 손실 또는 교차 엔트로피 손실로 불리며 예측 확률에 적용된다는 설명 확인에 사용했다.
