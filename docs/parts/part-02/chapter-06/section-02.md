...
Part 2 안에서는 이 절을 `손실 함수(loss function)`, `목적 함수(objective function)`, `오차(error)`, `평균 손실(mean loss)`, `평가 지표(metric)`를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 6.1에서 더 나은 값을 찾는 문제를 읽었다면, 여기서는 학습이 실제로 무엇을 기준으로 움직이는지 정리합니다.
...
또한 이 절은 `손실 함수 목록`을 전부 외우는 절도, `정규화가 포함된 목적 함수 설계`를 자세히 다루는 절도 아닙니다. 여기서는 학습 기준의 역할을 먼저 읽고, 실제로 그 값을 줄이는 반복 구조는 P2-6.3에서 다시 회수합니다.
