...
여기서는 최적화(optimization)를 AI 학습으로 들어가기 전 필요한 사고방식으로 소개합니다. 엄밀한 공식보다 “무엇을 찾는 문제인가”를 먼저 봅니다.
...
손실 함수와 목적 함수는 P2-6.2에서, 경사하강법의 직관은 P2-6.3에서 다시 다룹니다. 문제 유형별 손실은 P4-4.1과 P4-4.2에서, 역전파와 옵티마이저는 P4-5.1, P4-7.1, P4-7.2에서 이어집니다. 반면 볼록 최적화와 선형계획법의 세부 알고리즘은 이 책의 현재 본편 범위 밖에 둡니다. 여기서는 최적화라는 말을 들었을 때 떠올릴 핵심 장면을 정리합니다.
...
AI 학습은 이 흐름을 모델 파라미터에 적용한 것으로 볼 수 있습니다. 모델이 처음부터 좋은 답을 아는 것이 아니라, 기준을 이용해 값을 조정해 갑니다. 이 관점이 뒤의 손실 함수와 경사하강법 설명의 바탕이 됩니다.
...
- 학습을 단순 계산이 아니라 `더 나은 값 찾기` 문제로 다시 읽어야 할 때 먼저 떠올립니다.
- 후보, 기준, 제약이 섞여 보여 무엇을 비교하고 무엇을 지켜야 하는지 분리해야 할 때 먼저 떠올립니다.
- 최적이라는 말을 곧바로 완벽한 현실 해답으로 받아들이고 있어 기준과 조건을 다시 물어야 할 때 먼저 떠올립니다.
