...
처음 읽을 때는 모든 팁보다 `문서에 보이는 순서와 실제 실행 순서가 같은가`, `환경과 데이터 준비가 앞에서 다시 드러나는가`, `이 노트북이 나중에 .py 파일이나 프로젝트 기록으로 이어질 수 있는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
Part 2의 다음 장으로 넘어가기 전에는 이 절에서 다음 정도만 점검해도 충분합니다.
