...
이 세 이름은 서로 겹쳐 보입니다. Colab도 Jupyter Notebook 형식을 사용하고, Jupyter도 내 컴퓨터에서 실행할 수 있으며, 로컬 실행도 노트북이 아니라 `.py` 스크립트로 할 수 있습니다. 그래서 “어디에서 실행되는가”, “어디에 저장되는가”, “무엇을 공유하는가”를 나누어 보는 편이 좋습니다.

Part 2 Chapter 10에서는 `Jupyter`, `Colab`, `로컬 실행(local execution)`, `런타임(runtime)`, `.ipynb 파일`의 관계를 이 절에 대표로 둡니다. 앞 절이 노트북 문서 형식을 설명했다면, 실제로 그 문서가 어디서 실행되고 무엇이 공유되는지는 여기 기준으로 다시 연결합니다.
...
| 용어 | 이 절에서 먼저 잡을 뜻 |
| --- | --- |
| Jupyter | 오픈소스 노트북 도구와 생태계입니다. |
| Colab | Jupyter 형식을 기반으로 Google이 제공하는 hosted 노트북 서비스입니다. |
| 로컬 실행(local execution) | 내 컴퓨터의 Python, 파일, 패키지 환경에서 직접 실행하는 방식입니다. |
| 런타임(runtime) | 코드를 실제로 실행하는 현재 Python 환경과 상태입니다. |
| `.ipynb` 파일 | 노트북 내용이 저장된 문서 파일이며, 실행 중인 상태와는 다를 수 있습니다. |
...
이 절에서는 세부 메뉴를 익히는 것보다 다음 구분이 먼저 서야 합니다.
