...
여기서 먼저 해결할 질문은 이것입니다. `같은 노트북 파일처럼 보여도 실제로 어디에서 실행되는지에 따라 왜 결과와 파일 접근이 달라지는가`입니다.
...
이 절은 도구를 하나로 고정하기보다, 실습 문서를 읽을 때 “이 명령은 어디에서 실행해야 하는가”를 판단하는 기준을 만드는 데 초점을 둡니다.

이 절은 실행 위치와 런타임 조건을 구분하는 데 초점을 둡니다. 앞 절의 노트북이 코드와 출력을 한 문서 안에 어떻게 남길지를 다뤘다면, 여기서는 그 문서가 실제로 어디에서 실행되고 어떤 파일과 런타임 조건에 기대는지 읽습니다. 바로 다음 절에서는 이 차이를 재실행 가능한 기록 습관으로 어떻게 정리할지로 질문이 다시 이동합니다.

이 절 다음 흐름도 단순합니다.

- `P2-10.3`에서는 지금 본 실행 차이를 재실행 가능한 기록 습관으로 다시 묶습니다.
- Chapter 7의 환경 절과 연결하면 런타임, 파일 접근, 패키지 설치 문제를 같은 실행 위치 질문으로 읽을 수 있습니다.
- Part 6 프로젝트 구간에서는 실험 기록과 재현성 문맥에서 이 차이가 다시 중요해집니다.
...
같은 `.ipynb` 파일이라도 어디에서 실행하느냐에 따라 런타임과 파일 접근이 달라지고, Colab은 hosted 서비스이며 로컬 Jupyter와 로컬 스크립트는 내가 환경을 더 직접 관리합니다. 또한 노트북 파일과 실행 중인 런타임은 다릅니다. 이 구분이 서면 Chapter 10은 `노트북 서비스 비교`보다 `실행 기록이 놓이는 자리와 조건을 읽는 장`으로 읽힙니다.

즉, 지금 장의 핵심은 `무엇을 한 문서에 남길까`에서 `그 문서가 실제로 어디에서 어떤 조건으로 실행될까`로 질문이 이동한다는 점입니다. P2-10.3에서는 이 실행 차이를 재실행 가능한 기록 습관으로 다시 정리합니다.
...
- 같은 `.ipynb` 파일이라도 어디서 실행하느냐에 따라 결과와 제약이 달라진다는 점을 설명해야 할 때 이 절을 먼저 떠올립니다.
- Colab, Jupyter, 로컬 스크립트 가운데 무엇을 써야 할지 판단하려 할 때 파일과 런타임을 분리해서 다시 봅니다.
- 경로 오류, 설치 상태 차이, 공유 후 재실행 실패처럼 실행 환경 문제가 의심될 때 이 비교가 기준이 됩니다.
