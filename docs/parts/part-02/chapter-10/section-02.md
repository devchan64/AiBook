...
이 절은 Jupyter와 Colab의 세부 사용법을 설명하는 매뉴얼이 아닙니다. 설치법, 메뉴 위치, 단축키, GPU 설정, Google Drive 연동 세부는 다루지 않습니다. 여기서는 그런 기능 이름을 외우기보다 `실행 위치`, `런타임`, `파일 접근`의 차이를 먼저 잡고, 대규모 실험 추적 같은 확장 주제는 뒤의 P3-9.3 보충학습과 Part 6 프로젝트 기록 문맥에서 다시 연결합니다.
...
지금 읽는 층위는 `실행 위치와 런타임 조건 층위`입니다. 앞 절의 노트북이 `코드와 출력을 한 문서 안에 어떻게 남길까`를 다뤘다면, 여기서는 그 문서가 실제로 어디에서 실행되고 어떤 파일과 런타임 조건에 기대는지 읽습니다. 바로 다음 절에서는 이 차이를 재실행 가능한 기록 습관으로 어떻게 정리할지로 질문이 다시 이동합니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| Jupyter와 Colab은 같은가? | Colab은 Jupyter 스타일 노트북을 제공하는 하나의 서비스입니다. |
| 무엇을 기준으로 환경을 고르나? | 시작 장벽, 재현성, 파일 접근, 반복 실행 편의성을 봅니다. |
| 공유할 때 무엇을 조심해야 하나? | 파일만 공유되는지, 실행 환경까지 같은지 따로 확인해야 합니다. |

이 절에서는 세부 메뉴를 익히는 것보다 다음 구분이 먼저 서면 충분합니다.

| 지금 여기서 먼저 잡을 최소선 | 바로 다음 장으로 넘겨도 되는 것 |
| --- | --- |
| 같은 `.ipynb` 파일이라도 어디에서 실행하느냐에 따라 런타임과 파일 접근이 달라진다는 점 | Jupyter 설치 절차와 Colab 세부 메뉴 |
| Colab은 hosted 서비스이고, 로컬 Jupyter와 로컬 스크립트는 내가 환경을 더 직접 관리한다는 점 | GPU, Drive 연동, 서버 설정 같은 확장 기능 |
| 노트북 파일과 실행 중인 런타임은 다르다는 점 | 재실행 가능한 노트북 정리 습관의 상세 절차 |

여기까지 구분되면 Chapter 10은 `노트북 서비스 비교`가 아니라 `실행 기록이 놓이는 자리와 조건을 읽는 장`으로 읽힙니다. 그다음 P2-10.3에서 이 실행 차이를 실제 재현성 습관으로 연결하면 됩니다.

이 전환을 앞뒤 장과 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 장 | 지금 장 | 바로 다음에 더 붙는 장 |
| --- | --- | --- |
| 노트북: 코드, 설명, 출력을 한 문서에 어떻게 남길까 | Jupyter, Colab, 로컬 실행: 그 기록이 실제로 어디에서 돌고 무엇이 공유될까 | 재실행 가능한 기록: 이 실행 차이를 어떻게 다시 실행 가능한 습관으로 정리할까 |
| 기록 형식 | 실행 위치와 런타임 조건 | 재현성 습관 |

즉, 지금 장의 핵심은 `무엇을 한 문서에 남길까`에서 `그 문서가 실제로 어디에서 어떤 조건으로 실행될까`로 손잡이가 바뀐다는 점입니다.

Chapter 7~10 전체를 기준으로 보면 지금 장의 역할은 아래처럼 정리할 수 있습니다.

| 앞에서 이미 붙잡은 것 | 지금 장에서 확인할 것 | 바로 다음 절에서 이어질 것 |
| --- | --- | --- |
| Chapter 7: 어디서 실행하나 | 같은 노트북 파일이라도 어디서 실행하느냐에 따라 런타임과 파일 접근이 달라진다는 점 | P2-10.3에서 이 차이를 재실행 가능한 기록 습관으로 정리합니다. |
| Chapter 8-9: 무엇을 어떤 값과 묶음으로 적나 | 그 코드와 출력이 어떤 실행 환경 위에 놓이는지 | 이후 Part 3에서 실험 결과를 다시 실행하고 비교하는 기준이 됩니다. |

이 표의 핵심은 Chapter 10이 `노트북 서비스 소개`가 아니라, 앞에서 만든 코드와 데이터 묶음을 어떤 실행 조건에서 다시 확인하고 남길지 정리하는 장이라는 점입니다.
...
처음 읽는 독자가 자주 헷갈리는 지점은 파일과 실행 상태를 같은 것으로 보는 것입니다.
...
문제 상황: 노트북에서 검증한 계산을 나중에 반복 재사용할 함수로 옮기고 싶습니다.
입력(input): 숫자 목록을 받아 평균을 반환하는 함수 정의입니다.
기대 출력(output): 출력은 없지만 재사용 가능한 함수 형태가 만들어집니다.
확인할 개념: 반복 실행과 재사용이 필요해지면 노트북 코드가 `.py` 함수로 이동할 수 있음을 봅니다.
...
문제 상황: 다른 사람이 Colab 노트북을 다시 실행할 때 필요한 패키지를 먼저 설치하게 하고 싶습니다.
입력(input): `numpy` 설치를 위한 `%pip` 명령입니다.
기대 출력(output): 현재 커널에 `numpy`가 설치됩니다.
확인할 개념: 공유 노트북은 필요한 준비 과정을 앞쪽 셀에 남겨야 재실행이 쉬워집니다.
...
문제 상황: 설치 뒤 실제 코드가 어떤 패키지를 쓰는지 분명히 보여 주고 싶습니다.
입력(input): `numpy`를 `np` 이름으로 import하는 코드입니다.
기대 출력(output): 출력은 없지만 뒤 셀에서 `np`를 사용할 준비가 됩니다.
확인할 개념: 설치 셀과 import 셀을 분리해 두면 실행 환경 의존성이 더 잘 드러납니다.
...
문제 상황: 파일을 읽는 코드가 환경에 따라 왜 달라지는지 가장 단순한 경로 예시로 보고 싶습니다.
입력(input): 로컬 프로젝트 기준 파일 경로 문자열입니다.
기대 출력(output): 출력은 없지만 코드가 어떤 파일을 찾으려는지 드러납니다.
확인할 개념: 파일 문제는 코드 문법보다 실행 환경과 경로 차이에서 생길 수 있다는 점을 봅니다.
...
학습자가 집에서는 로컬 Jupyter로 `data/scores.csv`를 잘 읽었는데, 다음 날 Colab에서 같은 코드를 실행하자 `파일을 찾을 수 없다`는 오류가 났다고 해 보겠습니다. 사람 입장에서는 `어제 되던 코드`라서 코드 자체가 잘못된 것처럼 느껴질 수 있습니다.
...
이 사례는 `Jupyter`, `Colab`, `로컬 실행`을 기능 이름이 아니라 실행 환경의 차이로 읽게 해 줍니다. 같은 `.ipynb` 문서라도 어디에서 실행되는지에 따라 파일 경로, 설치 상태, 유지되는 런타임이 달라질 수 있습니다.

그래서 실습 문서를 읽을 때는 먼저 `이 코드는 어디에서 실행되는가`, `이 파일은 어디에 있는가`, `다른 사람이 다시 실행해도 같은 파일을 볼 수 있는가`를 확인해야 합니다. 환경 구분이 선행되어야 오류 원인도 더 빨리 좁힐 수 있습니다.
...
이 절은 노트북 사용법 전체를 익히는 절이 아니라, 기록 층의 경계를 잡는 절입니다.

| 지금 장의 손잡이 | 바로 다음 장에서 더 보는 것 | 아직 여기서 하지 않는 것 |
| --- | --- | --- |
| 같은 코드라도 실행 위치와 런타임에 따라 파일, 패키지, 공유 조건이 달라진다 | NumPy, Pandas, Matplotlib, Git 실습에서 어떤 실행 기록을 남겨야 하는가 | 실험 추적 도구, 대규모 협업 기록 체계, 배포 자동화 문서화 |
