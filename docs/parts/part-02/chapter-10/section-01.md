...
Part 2 Chapter 10에서는 `노트북(notebook)`, `코드 셀(code cell)`, `마크다운 셀(markdown cell)`, `출력(output)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 Jupyter, Colab, 로컬 실행과 재실행 습관을 다시 보더라도, 노트북이 무엇을 한 문서에 남기는지는 여기 기준으로 다시 연결합니다.
...
이 절은 노트북을 설치하거나 Colab 사용법을 자세히 설명하는 절이 아닙니다. Colab과 로컬 PC 실행 환경의 차이는 P2-3.4와 Part 2 Chapter 7에서 이미 봤고, Jupyter와 Colab의 차이는 P2-10.2에서 따로 다룹니다.
...
이 절에서는 Jupyter 서버 구조, 커널 프로토콜, Colab 런타임 정책, 노트북 배포 자동화는 깊게 다루지 않습니다. 다만 `.ipynb` 파일과 실행 중인 런타임(runtime)의 구분은 바로 다음 P2-10.2와 P2-10.3에서 다시 연결합니다. 서버와 커널 내부 구조 자체는 현재 본편 범위 밖에 둡니다.
