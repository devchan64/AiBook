...
- Part 3로 넘어가기 전에 무엇을 최소한 다시 말할 수 있어야 하는지 점검해야 할 때 이 절을 먼저 떠올립니다.
- 모델 이름보다 데이터, 입력과 출력, 학습과 평가 흐름을 먼저 읽겠다는 기준을 다시 세워야 할 때 이 절로 돌아옵니다.
- `X`, `y`, sample, feature, fit, predict, 손실, 평가 지표를 헷갈리지 않고 구분하는지 확인할 때 다시 확인합니다.
