...
지금 읽는 층위는 `다음 Part 진입 전 재진입 기준 층위`입니다. 이 절의 목적은 Part 2를 완전히 닫아 버리는 데 있지 않고, Part 3에서 막혔을 때 어떤 개념을 어디로 돌아와 다시 볼지 기준을 남기는 데 있습니다. 따라서 여기서는 새로운 내용을 더 배우기보다 `무엇을 들고 넘어가고`, `무엇은 다시 찾아오면 되는가`를 구분하는 편이 더 중요합니다.

즉, Part 2는 완벽히 끝내고 봉인하는 Part가 아니라, Part 3를 읽다가 막히는 지점마다 다시 찾아와 기준을 복구하는 재방문 기준점으로 읽는 편이 더 정확합니다.
...
조금 더 실전형으로 줄이면 아래 네 가지만 다시 말할 수 있어도 충분합니다.
...
이 표의 목적은 Git 명령을 더 외우게 하는 데 있지 않습니다. 현재 절에서는 `계산을 했다`와 `무엇을 바꿔 어떤 결과가 달라졌는지 다시 설명할 수 있다`가 같은 말이 아니라는 점만 분명히 잡으면 충분합니다. 그래서 Part 2의 Git은 개발자 부록이 아니라, Part 3 실험 비교를 다시 읽게 만드는 기록 손잡이로 이해하는 편이 더 안전합니다.
...
| 코드를 어디에서 실행해야 하는지 모른다 | P2-3.4, Part 2 Chapter 7, Chapter 10을 다시 본다 |
...
즉 Part 2의 점검은 시험이 아니라 지도 확인에 가깝습니다. 모델 이름을 많이 아는 것보다, 데이터를 읽고 학습 흐름을 따라갈 준비가 되었는지가 더 중요합니다.
