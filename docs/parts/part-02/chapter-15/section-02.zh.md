...
这一节与其说是彻底关闭 Part 2 的结尾，不如说是一个“再次返回时的基准点”：当你在 Part 3 卡住时，应该回到哪些概念，把标准重新恢复出来。这里不会继续学习很多新内容，而是区分 `现在应该带着走的东西` 和 `之后可以回来再查的东西`。当你重新寻找概念时，请把代表 Section 和[公式词条](/AiBook/zh/reference/concept-glossary-pinyin/g.zh/#formula)一起当作基准点。
...
- 能说明 Part 2 的目标不是完整学完数学和编程，而是恢复阅读机器学习所需的最小基础吗？
- 能说明在 Part 3 应该先看数据形状、学习流程和评价标准，而不是先看模型名字吗？
- 能说明 `X`、`y`、`fit`、`predict` 会连接到 Part 2 学过的数组、表格和函数执行流程吗？
...
- 能说明机器学习应该被读成“公式、代码、数据、评价一起联动”的学习流程吗？
- 你是否记得，如果某个概念仍然模糊，可以回到 Part 2 再检查一次？
