...
Sigma 是压缩重复加法的记法。在 AI 中，因为数据很多，同一个计算经常被反复执行，然后结果再通过总和或平均值聚合起来。

换句话说，当数据很多时，同一个计算会被重复执行，结果会被求和或求平均；这在公式里表现成 sigma，在代码里表现成循环或数组计算。

看到 sigma 时，不要先把它看成复杂数学，而要把它读成 `到底在加什么、加多少次？`

这个视角会在 P2-5.2 阅读平均值和方差时、在 P2-6.2 计算整份数据的损失时、以及在 Part 4 小批次学习（mini-batch learning）语境里处理样本组合时不断重复出现。正因为如此，sigma 更适合被看成 AI 计算里用来阅读“重复与聚合”的第一路标，而不是一个孤立的数学符号。
...
- 当你一看到 sigma 就想立刻背公式，而不是先把重复结构展开时
- 当平均值、损失、批次计算其实共享同一种聚合结构这件事开始变模糊时
- 当你需要重新抓回“循环、数组计算、sigma 记法只是同一个计算的不同表达”时
