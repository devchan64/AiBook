...
这里要把对数(logarithm)和指数(exponential)重新接成阅读 `逻辑回归(logistic regression)`、`对数损失(log loss)`、`softmax` 所需的最小计算语言。需要快速回看术语时，也可以一起查看[log loss 词条](/AiBook/zh/reference/concept-glossary-pinyin/l/#log-loss)。
