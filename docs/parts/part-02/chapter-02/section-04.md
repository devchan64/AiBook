...
여기서는 로그와 지수의 엄밀한 성질 전체를 다루지 않습니다. 밑(base) 바꾸기 공식의 증명, 복소수 로그, 연속 복리의 세부 전개 같은 내용은 범위 밖에 둡니다.
...
- 지수는 `같은 양 증가`가 아니라 `같은 비율 증가`를 읽는 언어입니다.
- 로그는 지수 변화를 다시 원래 축으로 읽는 반대 방향 함수입니다.
- 작은 확률을 계속 곱하면 값이 빠르게 작아지므로, 로그는 곱셈을 더하기로 바꿔 읽게 해 줍니다.
- `sigmoid`, `softmax`, `log loss`는 모두 점수와 확률 사이를 오갈 때 이 감각을 다시 호출합니다.
- 뒤 Part에서 `exp`, `log`, `softmax`, `log loss`가 나오면 먼저 `점수를 어떻게 비율과 손실로 바꾸는가`를 떠올리면 됩니다.
