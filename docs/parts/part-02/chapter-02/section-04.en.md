...
Here we reconnect logarithms and exponentials as the minimum calculation language needed to read `logistic regression`, `log loss`, and `softmax`. When you want to recheck the terms quickly, also refer to the [log loss glossary entry](/AiBook/en/reference/concept-glossary-alpha/l/#log-loss).
