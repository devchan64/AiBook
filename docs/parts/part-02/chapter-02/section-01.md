...
| 벡터와 행렬 표기 | 값 하나가 아니라 배열과 축으로 바뀌면 기호 읽기가 어떻게 달라지는가? | `P2-3.1`, `P2-11.1` |
...
이 절의 핵심은 `수식이 코드와 어떻게 연결되는가`에서 `그 수식 한 줄을 어떤 최소 기호 문법으로 읽을까`로 질문이 바뀐다는 점입니다.
...
- 수식 한 줄을 봐도 무엇이 값이고 무엇이 관계인지 분리되지 않을 때
- `y = f(x)`를 오래전 함수 문제로만 보고 모델 실행 구조로 읽지 못할 때
- 변수 이름만 보고 값의 의미, 타입, shape 확인을 건너뛰려 할 때
