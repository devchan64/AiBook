...
| 벡터와 행렬 표기 | 값 하나가 아니라 배열과 축으로 바뀌면 기호 읽기가 어떻게 달라지는가? | P2-3, P2-11 |
| 머신러닝 본편 연결 | `y = f(x)`, `loss`, `prediction`, `target` 같은 표현을 어디서 다시 읽게 되는가? | Part 3 전반 |

이 전환을 Part 2 앞뒤와 Part 3까지 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 절 | 지금 절 | 바로 다음에 더 붙는 절/Part |
| --- | --- | --- |
| 수식, 코드, 데이터는 같은 계산의 다른 얼굴이다 | 변수, 함수, 식은 그 계산을 읽는 최소 기호다 | 시그마, 벡터, 손실 표기와 Part 3 모델 수식 읽기 |
