...
Part 2 안에서는 이 절을 `변수(variable)`, `함수(function)`, `식(expression)`, `입력(input)`, `출력(output)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 1.2에서 수식, 코드, 데이터가 같은 계산의 다른 얼굴이라는 감각을 잡았다면, 여기서는 그 계산을 읽는 최소 기호 문법을 정리합니다.
...
지금 읽는 층위는 `기호 읽기 복구 층위`입니다. 앞 절의 수식-코드-데이터 연결이 `같은 계산이 서로 다른 표현으로 보인다`는 감각을 잡았다면, 여기서는 그 표현 안에서 가장 자주 나오는 최소 기호인 변수, 함수, 식을 다시 읽습니다. 바로 뒤의 시그마, 벡터, 미분, 손실 절에서는 이 기호 읽기가 반복 계산, 데이터 모양, 학습 방향을 읽는 더 구체적인 표기로 확장됩니다.

이 절은 Part 2 전체의 `수식 한 줄을 읽는 최소 문법` 손잡이입니다. 개별 수학 개념 세부는 뒤 절에서 회수됩니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 변수, 함수, 식 | 수식 한 줄 안에서 값, 관계, 계산을 어떻게 구분할 것인가? | P2-2.1 |
| 시그마와 반복 계산 | 이런 기호들이 반복과 집계 안에서는 어떻게 보이는가? | P2-2.2 |
| 벡터와 행렬 표기 | 값 하나가 아니라 배열과 축으로 바뀌면 기호 읽기가 어떻게 달라지는가? | Part 2 Chapter 3, Part 2 Chapter 11 |
| 머신러닝 본편 연결 | `y = f(x)`, `loss`, `prediction`, `target` 같은 표현을 어디서 다시 읽게 되는가? | Part 3 데이터 모델링과 Part 4 머신러닝 전반 |

이 전환을 Part 2 앞뒤와 Part 3까지 한 번에 붙이면 다음과 같습니다.

| 바로 앞 절 | 지금 절 | 바로 다음에 더 붙는 절/Part |
| --- | --- | --- |
| 수식, 코드, 데이터는 같은 계산의 다른 얼굴이다 | 변수, 함수, 식은 그 계산을 읽는 최소 기호다 | 시그마, 벡터, 손실 표기와 Part 3 데이터 모델 읽기, Part 4 모델 수식 읽기 |
| 계산 표현의 연결 | 계산 표현의 최소 문법 | 반복, 배열, 모델 관계 해석 |

즉, 지금 절의 핵심은 `수식이 코드와 어떻게 연결되는가`에서 `그 수식 한 줄을 어떤 최소 기호 문법으로 읽을까`로 손잡이가 바뀐다는 점입니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 변수는 무엇인가? | 값 자체가 아니라 값을 가리키는 이름입니다. |
| 함수는 무엇인가? | 어떤 입력을 받아 다른 출력으로 바꾸는 관계입니다. |
| 식은 무엇인가? | 계산이나 관계를 짧게 적은 표현입니다. |
...
또한 이 절은 `시그마와 반복 계산`을 설명하는 절도, `벡터와 배열 기호`를 깊게 다루는 절도 아닙니다. 여기서는 수식 한 줄의 최소 기호 문법을 먼저 잡고, 반복 표기는 2.2, 데이터 모양은 Part 2 Chapter 3과 Chapter 11에서 다시 좁혀 봅니다.
