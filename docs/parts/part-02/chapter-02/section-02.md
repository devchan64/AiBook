...
Part 2 안에서는 이 절을 `시그마(sigma)`, `인덱스(index)`, `평균(mean)`, `반복 계산(repeated computation)`, `손실 집계(loss aggregation)`를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 2.1에서 변수, 함수, 식의 최소 문법을 읽었다면, 여기서는 그 문법이 여러 데이터에 대해 반복될 때 어떤 압축 표기로 바뀌는지 정리합니다.

## 이 절의 범위

이 절은 시그마(sigma)와 반복 계산을 다룹니다. 극한(limit)은 P2-2.3에서 다룹니다. 벡터(vector), 행렬(matrix), 행렬 곱(matrix multiplication)은 Part 2 Chapter 3에서 다룹니다. 손실 함수(loss function)와 최적화(optimization)는 Part 2 Chapter 6에서 다시 다룹니다.
...
지금 읽는 층위는 `반복 계산 표기 층위`입니다. 앞 절의 변수, 함수, 식이 수식 한 줄의 최소 문법을 다뤘다면, 여기서는 그 문법이 여러 데이터에 대해 반복될 때 어떤 압축 표기로 바뀌는지 읽습니다. 바로 뒤의 극한, 벡터, 손실 절에서는 이 반복 구조가 변화율, 배열 계산, 전체 손실 집계로 더 구체적으로 이어집니다.

이 절은 Part 2 전체의 `무엇을 몇 번 더하는가` 손잡이입니다. 통계 공식과 손실 함수 세부는 뒤 절에서 회수됩니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 시그마와 반복 덧셈 | 같은 계산을 여러 데이터에 대해 어떻게 압축해서 쓸 것인가? | P2-2.2 |
| 극한과 변화 직관 | 반복 합만이 아니라 값의 변화는 어떤 표기로 읽을 것인가? | P2-2.3 |
| 벡터·배열·배치 계산 | 이 반복 구조가 배열 계산과 샘플 묶음 처리에서는 어떻게 나타나는가? | Part 2 Chapter 3, Part 2 Chapter 11, Part 4 미니배치 문맥 |
| 손실·최적화 연결 | 데이터 여러 개의 손실을 모으고 평균내는 구조를 어디서 다시 읽는가? | Part 2 Chapter 6, Part 3 데이터 모델링, Part 4 머신러닝 전반 |

이 전환을 Part 2 앞뒤와 Part 3까지 한 번에 붙이면 다음과 같습니다.

| 바로 앞 절 | 지금 절 | 바로 다음에 더 붙는 절/Part |
| --- | --- | --- |
| 변수, 함수, 식은 수식의 최소 문법이다 | 시그마는 그 문법이 반복될 때의 압축 표기다 | 통계, 벡터, 손실, 배치 계산과 Part 3 데이터 모델 읽기, Part 4 모델 학습 읽기 |
| 기호의 최소 문법 | 반복과 집계의 문법 | 배열 계산과 학습 집계 |

즉, 지금 절의 핵심은 `수식 한 줄을 어떻게 읽을까`에서 `그 수식이 여러 데이터에 대해 반복될 때 어떻게 읽을까`로 손잡이가 바뀐다는 점입니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 시그마는 무엇을 뜻하는가? | 같은 모양의 덧셈을 여러 번 반복하라는 압축 표기입니다. |
| 인덱스는 왜 필요한가? | 몇 번째 값을 더하는지 구분하기 위해 필요합니다. |
| 왜 AI 문서에 자주 나오는가? | 데이터 여러 개를 한꺼번에 모아 합, 평균, 손실을 계산하기 때문입니다. |
...
또한 이 절은 `극한과 변화율`을 설명하는 절도, `벡터와 행렬 계산 전체`를 다루는 절도 아닙니다. 여기서는 반복 덧셈과 집계 표기를 먼저 잡고, 변화의 직관은 2.3, 배열과 벡터 계산은 Part 2 Chapter 3과 Chapter 11에서 다시 좁혀 봅니다.
...
즉 데이터가 여러 개이면 같은 계산을 반복하고, 그 결과를 합치거나 평균내며, 수식에서는 시그마 표기로, 코드에서는 반복문이나 배열 계산으로 나타납니다.

시그마가 보이면 복잡한 수학으로 먼저 보지 말고, “무엇을 몇 번 더하는가?”라고 읽습니다.

이 관점은 P2-5.2에서 평균과 분산을 읽을 때, P2-6.2에서 손실을 데이터 전체에 대해 계산할 때, Part 4의 미니배치 학습 문맥에서 샘플 묶음을 처리할 때 계속 다시 등장합니다. 그래서 시그마는 독립된 수학 기호라기보다, AI 계산에서 반복과 집계를 읽는 첫 표지로 볼 수 있습니다.
