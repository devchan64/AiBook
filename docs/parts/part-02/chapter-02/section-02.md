...
| 벡터·배열·배치 계산 | 이 반복 구조가 배열 계산과 샘플 묶음 처리에서는 어떻게 나타나는가? | `P2-3.1`, `P2-11.1`, Part 4 미니배치 문맥 |
| 손실·최적화 연결 | 데이터 여러 개의 손실을 모으고 평균내는 구조를 어디서 다시 읽는가? | `P2-6.1`, `P2-6.2`, Part 3 데이터 모델링, Part 4 머신러닝 전반 |
...
이 절의 핵심은 `수식 한 줄을 어떻게 읽을까`에서 `그 수식이 여러 데이터에 대해 반복될 때 어떻게 읽을까`로 질문이 바뀐다는 점입니다.
...
- 시그마 기호를 보면 바로 공식을 외우려 하고 반복 구조를 펼쳐 읽지 않을 때
- 평균, 손실, 배치 계산이 모두 같은 집계 구조라는 점이 흐려질 때
- 반복문, 배열 계산, 시그마 표기가 같은 계산을 다른 표현으로 보여 준다는 점을 다시 잡아야 할 때
