...
| 벡터·배열·배치 계산 | 이 반복 구조가 배열 계산과 샘플 묶음 처리에서는 어떻게 나타나는가? | P2-3, P2-11, Part 4 미니배치 문맥 |
| 손실·최적화 연결 | 데이터 여러 개의 손실을 모으고 평균내는 구조를 어디서 다시 읽는가? | P2-6, Part 3 전반 |

이 전환을 Part 2 앞뒤와 Part 3까지 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 절 | 지금 절 | 바로 다음에 더 붙는 절/Part |
| --- | --- | --- |
| 변수, 함수, 식은 수식의 최소 문법이다 | 시그마는 그 문법이 반복될 때의 압축 표기다 | 통계, 벡터, 손실, 배치 계산과 Part 3 모델 학습 읽기 |
