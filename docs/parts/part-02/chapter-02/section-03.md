...
Part 2 안에서는 이 절을 `극한(limit)`, `수렴(convergence)`, `변화율(rate of change)`, `대입(substitution)`, `가까워짐(approach)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 2.2에서 반복과 집계의 표기를 읽었다면, 여기서는 값이 아주 조금 바뀔 때 함수가 어떻게 반응하는지 읽는 표기를 정리합니다.
...
이 절은 Part 2 전체의 `가까워질 때 어떻게 변하는가` 손잡이입니다. 미분 공식과 최적화 계산 세부는 뒤 절에서 회수됩니다.
...
또한 이 절은 `미분 공식 계산법`을 본격적으로 다루는 절도, `연속성의 엄밀한 증명`을 설명하는 절도 아닙니다. 여기서는 작은 변화와 가까워짐의 감각을 먼저 잡고, 미분과 그래디언트는 Part 2 Chapter 4에서 다시 좁혀 봅니다.
