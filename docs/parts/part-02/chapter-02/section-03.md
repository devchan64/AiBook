...
이 절은 극한(limit)을 수식 표기 독해 수준에서 다룹니다. 엄밀한 \(\epsilon\)-\(\delta\) 정의, 연속성(continuity)의 증명, 미분 공식의 계산은 다루지 않습니다. 변화율(rate of change), 미분(derivative), 그래디언트(gradient)는 Part 2 Chapter 4에서 다시 다룹니다.
...
지금 읽는 층위는 `작은 변화 읽기 층위`입니다. 앞 절의 시그마가 여러 데이터를 반복하고 모으는 표기였다면, 여기서는 값 하나를 아주 조금 움직일 때 함수가 어떻게 반응하는지 읽는 표기로 넘어갑니다. 바로 뒤의 미분, 그래디언트, 최적화 절에서는 이 작은 변화 관점이 실제 기울기와 손실 감소 방향 설명으로 더 구체화됩니다.

처음 읽을 때는 이 절을 Part 2 전체의 `가까워질 때 어떻게 변하는가` 손잡이로만 잡고, 미분 공식과 최적화 계산 세부는 뒤 절에서 회수된다고 생각하면 흐름이 덜 끊깁니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 극한과 가까워짐 | 값을 정확히 넣는 대신, 가까워질 때의 경향을 어떻게 읽을 것인가? | P2-2.3 |
| 미분과 변화율 | 아주 작은 변화에서 함수가 얼마나 바뀌는지 어떻게 수치화할 것인가? | P2-4.1, P2-4.2 |
| 그래디언트와 최적화 | 이 변화 방향을 손실 감소와 파라미터 조정에 어떻게 연결할 것인가? | P2-4.5, P2-6.1, P2-6.3 |
| 머신러닝 본편 연결 | `손실을 줄이는 방향`이라는 표현을 어디서 다시 읽게 되는가? | Part 3 데이터 모델링과 Part 4 머신러닝 전반 |

이 전환을 Part 2 앞뒤와 Part 3까지 한 번에 붙여 보면 다음처럼 읽는 편이 가장 안전합니다.

| 바로 앞 절 | 지금 절 | 바로 다음에 더 붙는 절/Part |
| --- | --- | --- |
| 시그마는 반복과 집계의 압축 표기다 | 극한은 작은 변화의 경향을 읽는 표기다 | 미분, 그래디언트, 최적화와 Part 3 데이터 모델 설명, Part 4 손실 감소 설명 |
| 반복과 집계 | 변화와 경향 | 기울기와 학습 방향 |

즉, 지금 절의 핵심은 `여러 값을 어떻게 모을까`에서 `값을 조금 바꾸면 함수가 어떻게 달라질까`로 손잡이가 바뀐다는 점입니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 극한은 무엇을 보는가? | 특정 점 자체보다, 그 점에 가까워질 때의 경향을 봅니다. |
| 대입과 무엇이 다른가? | 바로 값을 넣는 것이 아니라 주변에서 어떻게 변하는지 봅니다. |
| 왜 AI와 연결되는가? | 작은 변화가 결과를 얼마나 바꾸는지 읽는 미분의 준비가 되기 때문입니다. |
...
이 절에서는 수렴을 깊게 증명하지 않습니다. 다만 극한 표기를 볼 때 “무엇이 어디로 수렴하는가?”라고 질문하면 이해에 도움이 됩니다.
...
이 표기는 P2-4.1, P2-4.2에서 미분과 변화율을 다시 읽을 때 다시 등장합니다. 지금은 “작은 변화를 보는 표기”로만 기억해도 충분합니다.
...
입력이 어떤 값에 가까워질 때 함수값이 어디로 가는지 보고, 그 변화 비율을 읽으며, 그 감각이 미분과 그래디언트로 이어진다고 이해하면 충분합니다.
