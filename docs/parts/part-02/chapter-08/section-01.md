...
여기서는 `값(value)`, `변수(variable)`, `타입(type)`의 기본 구분을 설명합니다. 이 세 단어는 Python에만 갇힌 표현이 아니라, 대부분의 프로그래밍 언어와 데이터 처리 도구가 공통으로 쓰는 최소 단위입니다. 이후 Part 2에서 이 개념들을 다시 만날 때는 P2-8.1도 함께 기준점으로 삼습니다.
...
- 오픈체크리스트의 중심축 문장인 "값, 변수, 타입을 계산과 데이터 처리 관점에서 다시 붙잡아야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
