...
여기서는 리스트(list), 딕셔너리(dictionary), 반복(loop), 함수(function)를 깊게 다루지 않습니다. 그 내용은 P2-8.2부터 P2-8.5까지 이어집니다. 또한 메서드 호출 모양과 참조/복사 문제는 각각 P2-8.6, P2-8.7 보충학습에서 다시 회수합니다.
...
- 화면에는 숫자처럼 보이는데 계산이 이상하게 동작해 값, 변수, 타입을 다시 나눠 봐야 할 때 떠올립니다.
- `=`을 수학 등호처럼 읽고 있어 Python의 이름 붙이기와 재할당 감각을 다시 붙잡아야 할 때 떠올립니다.
- 이후 리스트, 딕셔너리, 배열 예제를 읽기 전에 지금 다루는 대상이 어떤 값이고 어떤 타입인지 먼저 확인해야 할 때 떠올립니다.
