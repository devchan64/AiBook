...
Part 2 Chapter 8에서는 `값(value)`, `변수(variable)`, `타입(type)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 리스트, 딕셔너리, 반복, 함수가 이어지더라도, 코드의 가장 작은 단위가 무엇인지에 대한 설명은 여기 기준으로 다시 연결합니다.
...
이 절은 리스트(list), 딕셔너리(dictionary), 반복(loop), 함수(function)를 깊게 다루지 않습니다. 그 내용은 P2-8.2부터 P2-8.5까지 이어집니다. 또한 메서드 호출 모양과 참조/복사 문제는 각각 P2-8.6, P2-8.7 보충학습에서 다시 회수합니다.
...
이 절은 Python 문법을 외우는 절이라기보다, 코드가 데이터를 어떻게 다루는지 보는 절입니다. 아래 세 가지 관점이 뒤 절의 리스트, 딕셔너리, 반복 설명을 읽는 기준이 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 값(value)은 계산이 실제로 다루는 대상이라는 점 | 숫자, 문자열, 참거짓이 왜 다르게 동작하는지 여기서 시작합니다. | 코드 안의 `3`, `"AI"`, `True`가 모두 값입니다. |
| 변수(variable)는 값을 다시 가리키기 위한 이름이라는 점 | 수학의 등호와 Python의 `=`을 구분하는 데 필요합니다. | `score = 82`는 등식이 아니라 이름 붙이기입니다. |
| 타입(type)은 값으로 `무슨 연산이 가능한지`를 정한다는 점 | 문자열과 숫자가 왜 섞여서 오류가 나는지 설명해 줍니다. | 같은 `+`라도 값의 종류에 따라 다르게 동작합니다. |
