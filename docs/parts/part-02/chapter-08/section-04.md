...
여기서는 `반복(loop)`, `이터러블(iterable)`, `이터레이터(iterator)`의 기본 구분을 설명합니다. `리스트(list)`와 `딕셔너리(dictionary)`의 대표 설명은 P2-8.2, P2-8.3과 P2-8.4에 두고, 여기서는 그 묶음에서 값을 하나씩 꺼내 처리 흐름으로 바꾸는 데 집중합니다.
...
## 핵심 기준: 반복(loop): 이터러블(iterable)을 하나씩 처리하기
...
이 예제의 기준 소스는 그대로 두고, 읽을 때는 `scores` 전체가 한 번에 출력되는지보다 반복 안의 `score`가 매번 어떤 값으로 바뀌는지를 봅니다. 출력이 네 줄로 나뉘는 이유는 리스트가 네 개로 쪼개졌기 때문이 아니라, 같은 처리 `print(score)`가 항목마다 한 번씩 적용되었기 때문입니다.
