...
반복(loop)은 Python 문법 하나를 더 배우는 단계가 아니라, `여러 값을 같은 기준으로 처리한다`는 데이터 작업의 기본 흐름을 회복하는 단계입니다. 리스트와 딕셔너리를 이미 봤더라도, 반복이 붙는 순간 코드는 `항목을 본다`, `새 묶음을 만든다`, `값을 누적한다`는 다른 구조로 읽혀야 합니다.

이 절은 그 전환을 분명하게 만드는 데 목적이 있습니다. 그래서 문법 암기보다 `묶음에서 무엇을 꺼내 무엇을 만들고 있는가`를 먼저 읽도록 구성합니다.

## 먼저 볼 세 가지

이 절은 반복문 문법보다 `묶음에서 값을 하나씩 꺼내 처리하는 감각`을 먼저 잡는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 반복(loop)은 `여러 값에 같은 처리를 적용하는 방식`이라는 점 | for 문을 외우기보다 목적을 먼저 보게 해 줍니다. | 리스트에서 값을 하나씩 꺼내 출력하는 예시를 읽을 수 있으면 충분합니다. |
| Python의 반복은 `몇 번`보다 `무엇에서 하나씩 꺼내는가`로 읽는 편이 자연스럽다는 점 | 항목 중심 반복과 이터러블 개념이 연결됩니다. | `for score in scores`가 `scores`에서 값을 하나씩 꺼낸다는 점만 알면 충분합니다. |
| 반복의 결과는 `출력`, `새 묶음 만들기`, `누적`으로 자주 나타난다는 점 | 이후 컴프리헨션과 데이터 전처리 예시를 읽기 쉬워집니다. | 반복이 값 보기에서 끝나지 않고 새 리스트나 딕셔너리를 만들 수 있다는 점만 알면 충분합니다. |
...
이 관점은 처음 읽는 독자에게 중요합니다. `for item in items`를 볼 때 `items`가 반드시 리스트라고 가정하면 안 됩니다. `items`는 리스트일 수도 있고, 딕셔너리의 `.items()` 결과일 수도 있고, 파일일 수도 있고, 나중에 보게 될 generator일 수도 있습니다. 이름은 다르지만 공통 질문은 같습니다.
...
### 사례 1. 여러 샘플 중 통과한 것만 골라내려면 무엇을 반복하는가

한 학습자가 텍스트 샘플 여러 개를 보고, 라벨이 있는 샘플만 따로 모으려 한다고 하겠습니다. 사람은 처음에 샘플 하나를 보고 판단하는 일은 할 수 있지만, 샘플 수가 수십 개를 넘기면 같은 기준을 반복 적용해야 합니다.

이때 반복을 `몇 번 도는 문법`으로만 보면 구조를 놓치기 쉽습니다. 실제로 중요한 것은 `무엇에서 무엇을 하나씩 꺼내는가`, `그 결과로 무엇을 만드는가`입니다. 샘플 목록에서 샘플을 하나씩 꺼내고, 조건을 확인하고, 새 리스트나 딕셔너리에 결과를 모으는 흐름이 핵심입니다.

이 절이 반복을 이터러블, 항목 반복, 누적, 필터링, 키별 집계 패턴으로 나누는 이유도 여기에 있습니다. 실습에서 반복은 단순히 코드를 길게 쓰는 장치가 아니라, 여러 데이터를 같은 기준으로 처리하는 기본 구조입니다.
