...
Part 2 Chapter 8에서는 `반복(loop)`, `이터러블(iterable)`, `이터레이터(iterator)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 함수 안에 반복이 들어가더라도, 묶음에서 값을 하나씩 꺼내는 기본 흐름 설명은 여기 기준으로 다시 연결합니다.
...
지금 읽는 층위는 `반복 문법을 종류별로 외우는 절`이 아니라 `묶음 자료구조를 실제 처리 흐름으로 바꾸는 층위`입니다. 앞의 리스트와 딕셔너리 절이 `어떤 구조로 값을 담는가`를 다뤘다면, 여기서는 그 구조에서 값을 하나씩 꺼내 무엇을 만들고 있는지를 읽습니다. 이 전환을 먼저 잡아 두면, 뒤의 함수 절에서도 `반복을 감싼 재사용`이라는 관점으로 자연스럽게 넘어갈 수 있습니다.
...
이 절은 반복문 문법보다 `묶음에서 값을 하나씩 꺼내 처리하는 감각`을 먼저 잡는 절입니다. 아래 세 가지 관점이 뒤 절의 함수 재사용과 전처리 예시를 읽는 기준이 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 반복(loop)은 `여러 값에 같은 처리를 적용하는 방식`이라는 점 | for 문을 외우기보다 목적을 먼저 보게 해 줍니다. | 리스트에서 값을 하나씩 꺼내 출력하는 구조입니다. |
| Python의 반복은 `몇 번`보다 `무엇에서 하나씩 꺼내는가`로 읽는 편이 자연스럽다는 점 | 항목 중심 반복과 이터러블 개념이 연결됩니다. | `for score in scores`는 `scores`에서 값을 하나씩 꺼내는 구조입니다. |
| 반복의 결과는 `출력`, `새 묶음 만들기`, `누적`으로 자주 나타난다는 점 | 이후 컴프리헨션과 데이터 전처리 예시를 읽기 쉬워집니다. | 반복은 값 보기에서 끝나지 않고 새 리스트나 딕셔너리를 만들 수 있습니다. |
