...
여기서는 반복 문법을 종류별로 외우기보다, 묶음 자료구조를 실제 처리 흐름으로 바꿉니다. 앞의 리스트와 딕셔너리 절이 `어떤 구조로 값을 담는가`를 다뤘다면, 여기서는 그 구조에서 값을 하나씩 꺼내 무엇을 만들고 있는지를 읽습니다. 이 전환을 먼저 잡아 두면, 뒤의 함수 절에서도 `반복을 감싼 재사용`이라는 관점으로 자연스럽게 넘어갈 수 있습니다.

## 먼저 가져갈 것

이 절은 반복 패턴 입문과 반복 추상화 보충 설명이 함께 들어 있습니다. 처음 읽을 때는 아래 `필수`만 먼저 잡아도 충분합니다.

| 구분 | 먼저 가져갈 내용 |
| --- | --- |
| 필수 | `for item in items`가 묶음에서 값을 하나씩 꺼내는 구조라는 점 |
| 필수 | `enumerate()`는 위치와 값을 함께 볼 때, `.items()`는 딕셔너리의 키와 값을 함께 볼 때 쓴다는 점 |
| 필수 | `zip()`은 둘 이상의 묶음을 나란히 읽을 때 쓴다는 점 |
| 필수 | 반복 결과가 출력, 새 묶음 만들기, 누적으로 자주 나타난다는 점 |
| 확장 | 이터러블(iterable)과 이터레이터(iterator)를 더 엄밀히 나누는 설명 |
| 확장 | PEP 234와 반복 인터페이스의 역사 |
| 확장 | generator 같은 뒤 학습 주제로 이어지는 반복 추상화 감각 |

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| `for item in items`가 묶음에서 항목을 하나씩 꺼내는 구조라는 점 | P2-8.5에서 이런 반복 처리를 함수로 어떻게 묶어 재사용하는지로 이어집니다. | 이후 전처리, 평가, 파일 처리, 모델 출력 정리에 계속 쓰입니다. |
| 리스트와 딕셔너리가 반복 안에서 서로 다른 방식으로 순회된다는 점 | `enumerate()`, `.items()`, `zip()`을 언제 쓰는지가 더 분명해집니다. | 이후 DataFrame 행 처리, 설정값 순회, 결과 집계에서 반복됩니다. |
| 반복 결과가 출력, 새 묶음, 누적값으로 자주 나타난다는 점 | 컴프리헨션과 누적 패턴을 같은 틀에서 읽는 기준으로 이어집니다. | 이후 NumPy 이전 전처리, Python 기초 실습, 프로젝트 코드 읽기에서 바탕이 됩니다. |

| 용어 | 이 절에서 먼저 잡을 뜻 |
| --- | --- |
| 반복(loop) | 여러 값에 같은 기준의 처리를 차례대로 적용하는 흐름입니다. |
| 이터러블(iterable) | 값을 하나씩 내보낼 수 있어 반복 대상이 될 수 있는 구조입니다. |
...
이 절에서 가장 먼저 붙잡아야 할 기준은 `반복은 여러 값에 같은 처리를 차례대로 적용하는 흐름`이라는 점입니다.

| 반복 장면 | 먼저 물을 질문 |
| --- | --- |
| `for score in scores` | 무엇을 하나씩 꺼내는가 |
| `enumerate(scores)` | 위치도 같이 필요한가 |
| `metrics.items()` | 이름과 값을 함께 봐야 하는가 |
| `zip(a, b)` | 둘 이상의 묶음을 나란히 읽는가 |

즉 반복 문법을 외우기보다 `묶음에서 무엇을 꺼내 무엇을 만들고 있는가`를 먼저 읽는 편이 중요합니다.
...
여기서는 그 전환을 분명하게 다룹니다. 그래서 문법 암기보다 `묶음에서 무엇을 꺼내 무엇을 만들고 있는가`를 먼저 읽도록 구성합니다.

## 세 가지 기준

여기서는 반복문 문법보다 `묶음에서 값을 하나씩 꺼내 처리하는 감각`을 먼저 잡습니다. 아래 세 가지 기준이 뒤 절의 함수 재사용과 전처리 예시를 읽는 바탕이 됩니다.

| 기준 | 왜 중요한가 | 이 절에서 필요한 이해 수준 |
| --- | --- | --- |
| 반복은 여러 값에 같은 처리를 적용하는 방식이다 | for 문을 외우기보다 목적을 먼저 보게 해 준다 | 리스트에서 값을 하나씩 꺼내 출력하는 구조라고 이해한다 |
| Python의 반복은 몇 번보다 무엇에서 하나씩 꺼내는가로 읽는 쪽이 자연스럽다 | 항목 중심 반복과 이터러블 개념이 연결된다 | `for score in scores`는 `scores`에서 값을 하나씩 꺼내는 구조라고 설명할 수 있다 |
| 반복의 결과는 출력, 새 묶음 만들기, 누적으로 자주 나타난다 | 이후 컴프리헨션과 데이터 전처리 예시를 읽기 쉬워진다 | 반복이 새 리스트나 딕셔너리를 만들 수 있다고 이해한다 |
...
문제 상황: 리스트에서 값을 하나씩 꺼내 같은 처리를 적용하는 가장 기본 반복을 보고 싶습니다.
...
문제 상황: 같은 반복을 위치 번호를 이용해 쓰면 어떤 모양인지 비교하고 싶습니다.
입력(input): 점수 리스트 `scores`.
기대 출력(output): 인덱스로 접근한 점수들이 차례대로 출력됩니다.
...
| --- | --- |
| 출력 | `print(score)` |
| 새 리스트 만들기 | `results.append(...)` |
| 누적값 만들기 | `total += score` |
| 조건 분리 | `if ...:`로 일부만 고르기 |

### 왜 이터러블이라고 부르는가

여기부터는 `확장` 성격이 조금 더 강합니다. 앞의 항목 중심 반복, `enumerate()`, `.items()`, `zip()` 감각이 먼저 잡혔다면 이 단락은 한 번에 모두 소화하지 않아도 됩니다.
...
이 관점은 중요합니다. `for item in items`를 볼 때 `items`가 반드시 리스트라고 가정하면 안 됩니다. `items`는 리스트일 수도 있고, 딕셔너리의 `.items()` 결과일 수도 있고, 파일일 수도 있고, 나중에 보게 될 generator일 수도 있습니다. 이름은 다르지만 공통 질문은 같습니다.
...
문제 상황: 항목 반복의 기본형을 한 번 더 고정해서 보고 싶습니다.
입력(input): 점수 리스트 `scores`.
기대 출력(output): 각 점수가 순서대로 출력됩니다.
...
문제 상황: 몇 번째 항목인지도 함께 확인해야 하는 반복을 보고 싶습니다.
입력(input): `enumerate(scores)`.
기대 출력(output): 위치 번호와 점수가 함께 출력됩니다.
...
- 반복은 여러 값에 같은 처리를 적용하는 흐름입니다.
- Python의 `for`는 보통 `몇 번`보다 `무엇에서 하나씩 꺼내는가`로 읽는 편이 자연스럽습니다.
- `enumerate()`, `.items()`, `zip()`은 반복 대상의 모양에 따라 읽는 방식을 바꾸는 도구입니다.
- 반복 결과는 출력, 새 묶음 만들기, 누적값 만들기로 자주 나타납니다.
- 뒤에서 함수(function)를 보면 `반복을 감싼 재사용`이라는 관점으로 이어집니다.

## 짧은 점검

- `for item in items`를 읽을 수 있는가?
- `enumerate()`, `.items()`, `zip()`을 언제 쓰는지 말할 수 있는가?
- 반복 결과가 출력인지, 새 묶음인지, 누적인지 구분할 수 있는가?

### 딕셔너리의 키와 값을 함께 본다

딕셔너리에서는 `.items()`를 자주 씁니다.

문제 상황: 딕셔너리의 키와 값을 동시에 확인하는 반복을 보고 싶습니다.
입력(input): 지표 딕셔너리 `metrics`.
기대 출력(output): `accuracy`, `loss`와 그 값이 출력됩니다.
...
문제 상황: 입력 텍스트와 라벨을 같은 위치끼리 묶어 함께 보고 싶습니다.
입력(input): `texts`, `labels` 두 리스트.
기대 출력(output): 텍스트와 라벨이 짝지어 출력됩니다.
...
문제 상황: 점수 목록에서 유효한 값만 새 리스트로 모으는 필터링 반복을 보고 싶습니다.
입력(input): 0이 섞인 점수 리스트 `scores`.
기대 출력(output): 0을 제외한 점수만 들어 있는 `valid_scores`.
...
문제 상황: 점수들을 0과 1 사이 값처럼 바꾸어 새 리스트로 만드는 변환 반복을 보고 싶습니다.
입력(input): 점수 리스트 `scores`.
기대 출력(output): 각 점수를 100으로 나눈 `normalized_scores`.
...
문제 상황: 짧은 반복 규칙으로 새 리스트를 한 줄에 만드는 예를 보고 싶습니다.
입력(input): `range(5)`.
기대 출력(output): 제곱값 목록 `squares`.
...
문제 상황: 조건에 맞는 값만 컴프리헨션으로 골라 새 리스트를 만드는 예를 보고 싶습니다.
입력(input): 0이 섞인 점수 리스트 `scores`.
기대 출력(output): 0이 아닌 값만 모은 `valid_scores`.
...
문제 상황: 컴프리헨션이 하기 전 일반 `for` 문 구조를 먼저 보고 싶습니다.
입력(input): 점수 리스트 `scores`.
...
문제 상황: 바로 앞의 반복을 컴프리헨션으로 줄였을 때 모양이 어떻게 바뀌는지 비교하고 싶습니다.
입력(input): 점수 리스트 `scores`.
기대 출력(output): 같은 정규화 결과를 만드는 한 줄 표현.
...
문제 상황: 반복으로 새 딕셔너리를 만드는 컴프리헨션 예를 보고 싶습니다.
입력(input): 라벨 리스트 `labels`와 `enumerate(labels)`.
기대 출력(output): 라벨 이름을 키로, 위치를 값으로 둔 `label_to_id`.
...
문제 상황: 중간 계산과 조건이 함께 있을 때는 일반 `for` 문이 더 읽기 쉬운 경우를 보고 싶습니다.
입력(input): `items`의 `score`를 검사하고 정규화하는 반복.
기대 출력(output): 조건을 통과한 값만 `results`에 추가하는 코드.
...
문제 상황: 점수 합계를 반복으로 하나씩 쌓아 가는 예를 보고 싶습니다.
입력(input): 점수 리스트 `scores`와 초기값 `total = 0`.
기대 출력(output): 모든 점수를 더한 합계.
...
문제 상황: 점수 목록을 통과와 실패 두 묶음으로 나누는 반복을 보고 싶습니다.
입력(input): 점수 리스트 `scores`와 기준 `60`.
기대 출력(output): `passed`, `failed` 두 리스트.
...
문제 상황: 라벨이 몇 번씩 나오는지 키별로 세는 반복을 보고 싶습니다.
입력(input): 라벨 리스트 `labels`.
기대 출력(output): 각 라벨의 개수가 들어 있는 `label_counts`.
...
문제 상황: 리스트와 딕셔너리, 조건 반복이 함께 쓰이는 작은 데이터 처리 예를 보고 싶습니다.
입력(input): 학생 딕셔너리들이 들어 있는 리스트 `students`.
기대 출력(output): 통과한 학생 이름만 담긴 `passed_students`.
...
문제 상황: 원본을 직접 지우기보다 새 결과 리스트를 만드는 안전한 필터링 방식을 보고 싶습니다.
입력(input): 0이 섞인 점수 리스트 `scores`.
기대 출력(output): 0을 제외한 새 리스트 `filtered_scores`.
...
문제 상황: 문장 목록에서 길이 목록을 만드는 반복을 공통 질문으로 읽어 보고 싶습니다.
입력(input): 문장 리스트 `texts`.
기대 출력(output): 각 문장 길이가 담긴 `lengths`.
...
- 리스트나 딕셔너리를 봤지만 이제 실제로 하나씩 처리하는 흐름으로 넘어가야 할 때 떠올립니다.
- `for item in items`, `enumerate()`, `.items()`, `zip()`이 각각 무엇을 하나씩 꺼내는지 다시 구분해야 할 때 떠올립니다.
- 반복 결과가 출력인지, 새 리스트인지, 누적값인지 헷갈려 처리 결과의 모양을 먼저 나눠 봐야 할 때 떠올립니다.
