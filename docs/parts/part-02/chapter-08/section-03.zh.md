...
在这个例子中，首先要看的不是 `student` 里有多个值，而是 `"name"` 和 `"score"` 这两个键分别作为取出值的标准。即使是一条数据，也不是像列表那样数第几个位置，而是像表中的识别列或设置名一样，用有意义的名称来查询。
...
这里会说明 `字典（dictionary）` 与 `键（key）` 的基本区分。关于 `列表（list）` 与按顺序访问的代表性说明放在 P2-8.2，关于 `值（value）` 与 `类型（type）` 的代表性说明放在 P2-8.1 和 P2-8.3。这里集中处理的是：`应该按什么标准去查找值？`
