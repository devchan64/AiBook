...
这里会说明 `字典（dictionary）` 与 `键（key）` 的基本区分。关于 `列表（list）` 与按顺序访问的代表性说明放在 P2-8.2，关于 `值（value）` 与 `类型（type）` 的代表性说明放在 P2-8.1 和 [概念词典](/AiBook/en/reference/concept-glossary/)。这里集中处理的是：`应该按什么标准去查找值？`
