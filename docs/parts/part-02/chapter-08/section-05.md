...
- 값과 여러 값 묶음을 반복으로 처리했다면, 이제 그 처리에 이름을 붙여 다시 쓴다.
- 함수는 `입력 -> 처리 -> 출력` 경계를 분명하게 만든다.
- `print`는 보여 주는 것이고, `return`은 다음 계산으로 넘기는 것이다.
- 작은 함수는 뒤에서 만날 라이브러리 함수와 API 함수 읽기의 기준이 된다.
...
여기서는 람다(lambda), 데코레이터(decorator), 클래스 메서드(method), 제너레이터(generator), 타입 힌트(type hint), 예외 처리(exception handling)를 깊게 다루지 않습니다. 값에 붙어 보이는 메서드 호출 모양은 P2-8.6 보충학습에서 다시 회수합니다.
...
앞 절의 흐름으로 다시 묶으면, `값과 변수`를 다루는 데서 시작해 `리스트와 딕셔너리`로 여러 값을 모으고, `반복문`으로 같은 처리를 여러 번 적용한 뒤, 이제 그 처리 자체를 `함수`라는 이름 붙은 단위로 정리하는 셈입니다.
...
- 같은 처리 코드를 여러 셀이나 여러 줄에 반복해서 쓰고 있어 작은 재사용 단위로 분리해야 할 때 떠올립니다.
- 수학의 함수, Python 함수, 라이브러리 함수가 모두 입력-처리-출력 계약으로 이어진다는 점을 다시 붙잡아야 할 때 떠올립니다.
- `print`와 `return`을 혼동해 화면 출력과 다음 계산에 넘길 결과를 다시 구분해야 할 때 떠올립니다.
