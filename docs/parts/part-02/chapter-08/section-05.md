...
이 첫 코드는 아직 함수가 아닙니다. 먼저 `score`라는 입력값이 조건문을 거쳐 `result`라는 출력 후보로 바뀌는 흐름을 봅니다. 바로 다음 함수 예제는 이 흐름을 쪼개는 것이 아니라, 같은 입력-처리-출력 관계에 이름을 붙여 다시 쓰는 단계입니다.
...
## 함수와 작은 재사용: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "함수가 작은 재사용 단위라는 점을 수학 함수와 연결해 보여 주어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
