...
Part 2 Chapter 8에서는 `함수(function)`, `매개변수(parameter)`, `인자(argument)`, `반환값(return value)`의 상세 설명을 이 절에 대표로 둡니다. 뒤에서 라이브러리 함수나 메서드를 다시 만나더라도, 입력-처리-출력 계약을 읽는 가장 기본 설명은 여기 기준으로 다시 연결합니다.
...
이 절은 람다(lambda), 데코레이터(decorator), 클래스 메서드(method), 제너레이터(generator), 타입 힌트(type hint), 예외 처리(exception handling)를 깊게 다루지 않습니다. 값에 붙어 보이는 메서드 호출 모양은 P2-8.6 보충학습에서 다시 회수합니다.
...
이 절은 함수 문법보다 `반복되는 처리를 이름 붙여 다시 쓰는 방법`을 먼저 보는 절입니다. 아래 세 가지 관점이 뒤에서 라이브러리 함수와 메서드를 읽는 기준이 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 함수(function)는 `입력, 처리, 출력`을 한 단위로 묶는다는 점 | 수학의 함수와 코드의 함수를 같은 큰 틀에서 읽게 해 줍니다. | 값을 받아 결과를 돌려주는 이름 붙은 코드입니다. |
| 같은 처리가 반복되면 함수로 `분리`하는 편이 읽기 쉽다는 점 | 작은 데이터 처리 코드가 왜 재사용 구조로 바뀌는지 이해하게 해 줍니다. | 점수 판정이나 정규화 같은 계산을 함수로 묶어 읽습니다. |
| `print`와 `return`은 역할이 다르다는 점 | 코드 실행 결과와 계산 결과를 구분하는 데 중요합니다. | 화면에 보여 주는 것과 다음 계산에 돌려주는 것은 다릅니다. |
