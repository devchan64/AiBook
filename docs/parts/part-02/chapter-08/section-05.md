...
지금 읽는 층위는 `함수 문법을 고급 기능까지 배우는 절`이 아니라 `앞 절의 반복과 자료구조 처리를 작은 재사용 단위로 묶는 층위`입니다. 리스트, 딕셔너리, 반복을 각각 따로 읽었다면, 여기서는 그 처리 흐름에 이름을 붙여 다시 쓸 수 있게 만드는 지점으로 넘어갑니다. 이 손잡이를 먼저 잡아 두면, 뒤에서 라이브러리 함수나 모델 API를 볼 때도 `문법`보다 `입력-처리-출력 계약`을 먼저 읽기 쉬워집니다.

처음 읽을 때는 모든 문법보다 `무엇이 반복되어 함수로 묶이는가`, `입력과 출력이 어디서 끊기는가`, `print`와 `return`이 어떤 역할 차이를 만드는가`라는 세 질문만 먼저 붙잡으면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 함수가 반복되는 처리를 이름 붙은 재사용 단위로 바꾼다는 점 | 이후 보충학습과 라이브러리 API에서 입력과 출력 계약을 어떻게 읽을지로 이어집니다. | 이후 모든 Python 예제, 데이터 전처리, 모델 호출 코드에서 반복됩니다. |
| 입력, 처리, 출력의 경계를 함수가 분리해 준다는 점 | `parameter`, `argument`, `return`을 왜 따로 구분하는지가 더 분명해집니다. | 이후 모델 함수, 평가 함수, 유틸리티 함수 설계에서 계속 중요합니다. |
| `print`와 `return`의 역할이 다르다는 점 | 눈으로 보는 출력과 다음 계산에 넘기는 결과를 어떻게 구분할지로 이어집니다. | 이후 디버깅, 재사용 코드 작성, 라이브러리 함수 읽기에서 다시 쓰입니다. |
...
이 절은 함수 문법보다 `반복되는 처리를 이름 붙여 다시 쓰는 방법`을 먼저 보는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 함수(function)는 `입력, 처리, 출력`을 한 단위로 묶는다는 점 | 수학의 함수와 코드의 함수를 같은 큰 틀에서 읽게 해 줍니다. | 값을 받아 결과를 돌려주는 이름 붙은 코드라는 점만 알면 충분합니다. |
| 같은 처리가 반복되면 함수로 `분리`하는 편이 읽기 쉽다는 점 | 작은 데이터 처리 코드가 왜 재사용 구조로 바뀌는지 이해하게 해 줍니다. | 점수 판정이나 정규화 같은 계산을 함수로 묶는 예시를 읽을 수 있으면 충분합니다. |
| `print`와 `return`은 역할이 다르다는 점 | 코드 실행 결과와 계산 결과를 구분하는 데 중요합니다. | 화면에 보여 주는 것과 다음 계산에 돌려주는 것이 다르다는 점만 알면 충분합니다. |
...
이 절에서는 고급 함수형 프로그래밍(functional programming)을 다루지 않습니다. 이 책 본편에서 함수형 프로그래밍 자체를 따로 확장하지는 않고, 여기서 필요한 범위는 `함수를 값처럼 전달할 수 있다`는 감각까지로 둡니다. 다만 Python에서는 함수가 값처럼 전달될 수 있다는 점을 알아 두면, 라이브러리 API를 읽을 때 덜 낯설어집니다.
...
한 학습자가 점수 정규화 코드를 노트북 여러 셀에 복사해 넣었다고 하겠습니다. 처음에는 빨리 진행되는 것 같지만, 나중에 정규화 기준을 바꿔야 할 때는 모든 셀을 다시 찾아 고쳐야 합니다.
