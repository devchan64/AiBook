...
리스트는 같은 목적이나 맥락의 값들을 순서대로 모아 두는 자료구조 선택(data structure)입니다. 점수 목록, 문장 목록, 파일 이름 목록, 모델 출력 목록처럼 “여러 개를 순서대로 보고 싶다”는 상황에서 자주 만납니다.

여기서는 `리스트(list)`와 `인덱스(index)`의 기본 구분을 설명합니다. `값(value)`, `변수(variable)`, `타입(type)`의 대표 설명은 P2-8.1과 P2-8.2에 두고, 여기서는 값이 여러 개가 되었을 때 왜 순서 있는 묶음이 먼저 필요한지에 집중합니다.
...
이 예제에서 먼저 볼 것은 점수의 크고 작음이 아니라, 네 값이 하나의 이름 `scores` 아래 순서를 가진 묶음으로 출력된다는 점입니다. 바로 아래 `type(scores)` 출력은 이 묶음이 숫자 네 개가 흩어진 상태가 아니라 리스트 값 하나로 다뤄진다는 사실을 확인해 줍니다.

```python
# 이 예제는 리스트가 여러 값을 순서대로 묶고 처리하는 방식을 확인합니다.
scores = [82, 75, 91, 68]
...
복사 여부와 `원본과 복사본이 언제 함께 바뀌는가`라는 질문은 바로 뒤의 P2-8.7 보충학습에서 다시 정리합니다.
...
문제 상황: `+`와 `extend()`가 모두 리스트를 붙이지만 원본을 바꾸는 방식이 다른지 비교하고 싶습니다.
입력(input): `front`, `back`, `scores`.
기대 출력(output): 새로 만든 `combined`, 그대로 남은 `front`, 수정된 `scores`.
확인할 개념: `+`는 붙인 새 리스트를 만들고, `extend()`는 기존 리스트 자체를 수정합니다.
...
## 리스트: 순서가 있는 값 묶음: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "리스트를 순서가 있는 값 묶음으로 이해시키고 반복과 연결해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
