...
Part 2 Chapter 8에서는 `리스트(list)`와 `인덱스(index)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 반복, 함수, NumPy 배열을 다시 만나더라도, 순서 있는 값 묶음을 읽는 가장 기본 설명은 여기 기준으로 다시 연결합니다.
...
이 절은 Python 문법보다 `여러 값을 어떻게 한꺼번에 다룰 것인가`를 먼저 보는 절입니다. 아래 세 가지 관점이 이후 딕셔너리, 반복, 배열 설명의 기준이 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 리스트(list)는 `순서가 있는 값 묶음`이라는 점 | 딕셔너리와의 차이를 잡는 출발점이 됩니다. | 몇 번째 값인지가 중요한 구조입니다. |
| 인덱스(index)는 `위치 번호`라는 점 | 이후 슬라이스, 배열, DataFrame 읽기와 연결됩니다. | 첫 번째 값이 `0`부터 시작한다는 규칙을 기준으로 읽습니다. |
| 리스트는 `바뀔 수 있는 값`이라는 점 | 전처리와 예제 코드에서 값이 왜 같이 바뀌는지 설명해 줍니다. | 같은 리스트를 두 이름이 함께 볼 수 있습니다. |
