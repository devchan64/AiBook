...
- 列表是有顺序的值集合。
- 索引是位置编号，而且在 Python 中通常从 0 开始。
- 列表是可变值（mutable value）。
- 把列表赋给另一个变量，并不会自动得到副本。
- 以后遇到 NumPy 数组时，必须先想起 `列表和数组不是同一个词`。

列表是有顺序的值集合。当值有多个，并且顺序或位置本身有意义时，首先考虑列表。

这个视角是为阅读 Python 列表准备的标准。以后在数组（array）、向量（vector）、表格行（row）、模型输出列表中，位置和顺序还会再次出现，但那些结构都还要分别确认各自的目的和计算规则。

阅读列表时，可以使用下面这些问题。

1. 这个列表里放的是什么目的或语境的值？
2. 顺序重要吗？
3. 需要通过位置（index）把值取出来吗？
4. 它是一种先空着、之后再把值加进去的结构吗？
5. 这是一个允许修改原列表的场景吗？
...
- 当同一目的的值有多个，却被分散成 `score1`、`score2` 这样的单独变量，需要重新按有顺序的集合来阅读时，就想起这个视角。
- 当问题变成“第几个值”“要不要只切出一段”“是否要在末尾追加新值”这种位置中心问题时，就想起这个视角。
- 当你把列表、数组或其他结构混成同一个词，需要重新先区分“这里说的是以顺序为中心的通用集合”时，就想起这个视角。
