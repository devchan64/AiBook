...
在这个例子中，首先要看的不是分数高低，而是四个值会作为一个名为 `scores` 的有序集合被输出。下面的 `type(scores)` 输出会确认，这组值不是四个零散数字，而是作为一个列表值来处理。
...
这里会说明 `列表（list）` 和 `索引（index）` 的基本区分。`值（value）`、`变量（variable）`、`类型（type）` 的代表性说明放在 P2-8.1 和 P2-8.2 中，而这里集中处理的是：当值变成多个之后，为什么首先需要一种有顺序的集合结构。
