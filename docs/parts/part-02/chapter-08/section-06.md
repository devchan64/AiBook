...
- Python에서는 많은 값이 객체(object)로 다뤄집니다.
- 클래스(class)는 그런 객체를 만들기 위한 정의입니다.
- 메서드(method)는 객체에 붙어 호출되는 동작입니다.
- `function(value)`와 `value.method()`는 비슷해 보여도 읽는 방향이 다릅니다.
- 뒤에서 `model.fit()`, `model.predict()`, `dataset.map()`이 나오면 먼저 `이 객체가 어떤 상태와 동작을 함께 갖는가`를 떠올리면 됩니다.
...
- `model.fit()`나 `text.lower()`처럼 점(`.`)이 붙은 호출이 왜 함수와 다르게 보이는지 다시 읽어야 할 때 떠올립니다.
- 객체가 값과 동작을 함께 가진다는 관점으로 문자열, 리스트, 모델 객체를 한 틀에서 묶어 봐야 할 때 떠올립니다.
- 딕셔너리와 클래스 기반 객체 중 무엇이 더 자연스러운지 데이터와 동작의 결합 정도로 다시 판단해야 할 때 떠올립니다.
