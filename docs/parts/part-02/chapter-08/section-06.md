...
이 보충학습은 아래 기준까지만 회수하면 본편으로 돌아오면 충분합니다.
...
그래서 클래스와 객체를 처음 만날 때는 `객체가 값과 동작을 함께 가진다`, `메서드는 그 객체에 붙어 호출된다`는 정도만 잡아도 충분합니다. 문자열의 `text.strip()`와 모델의 `model.fit()`은 복잡도는 다르지만 같은 호출 모양을 공유합니다.
