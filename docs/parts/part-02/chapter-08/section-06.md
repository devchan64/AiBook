...
Part 2 Chapter 8에서는 `클래스(class)`, `객체(object)`, `메서드(method)`를 처음 읽는 보충 설명을 이 절에 둡니다. 본편이 값, 자료구조, 반복, 함수의 최소 문법을 다뤘다면, 이 절은 `value.method()`와 `model.fit()` 같은 모양을 읽기 위한 보충 기준점입니다.
...
P2-8.1에서 값, 변수, 타입을 읽다가 `왜 문자열은 .lower()를 쓰고, 리스트는 .append()를 쓰지` 같은 질문이 생겼다면 이 절로 돌아오면 됩니다. 여기서 회수하는 질문은 Python 설계 전체가 아니라, `객체가 자기 타입에 맞는 동작을 메서드 형태로 제공한다`는 최소 감각입니다.
...
이 절은 Python 객체와 클래스의 입문용 감각만 다룹니다. 값·타입·함수의 기본 설명은 P2-8.1과 P2-8.5를 기준으로 다시 연결하고, 참조와 복사처럼 객체 공유가 만드는 효과는 P2-8.7에서 따로 회수합니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 객체는 무엇인가? | 값과 동작을 함께 가진 하나의 다루는 대상입니다. |
| 클래스는 무엇인가? | 그런 객체를 만들기 위한 설계도 같은 정의입니다. |
| 왜 AI 라이브러리에서 자주 보이는가? | 모델, 데이터셋, 설정을 하나의 묶음으로 다루기 좋기 때문입니다. |
...
이 보충학습은 아래 기준까지만 회수하면 본편으로 돌아갈 수 있습니다.

| 여기서 회수할 것 | 돌아갈 본편 질문 | 다시 이어서 읽을 곳 |
| --- | --- | --- |
| `function(value)`와 `value.method()`의 호출 중심 차이 | 값과 타입에 따라 왜 다른 동작을 쓰는가 | 값, 변수, 타입은 P2-8.1, 리스트와 딕셔너리는 P2-8.2~P2-8.3 |
| `model.fit()` 같은 표현이 객체 메서드 호출이라는 점 | 라이브러리 코드에서 점(`.`) 앞의 대상을 어떻게 읽을까 | 함수는 P2-8.5, 이후 라이브러리 예제는 Part 3 본편 |

즉, 점(`.`) 앞의 대상이 무엇인지 읽을 수 있고, 메서드가 객체에 붙은 동작이라는 점만 잡히면 더 오래 머물지 말고 본편으로 돌아오는 편이 안전합니다.
