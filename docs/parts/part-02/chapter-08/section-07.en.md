...
This Section does not go deeply into Python memory-management implementation, reference counts, the internal behavior of the `copy` module, or copy rules for user-defined objects. Those implementation details stay outside the current scope of the main text, and we only recover the sense for reading `when the original and the copy change together`.
...
- Python Software Foundation, [The Python Tutorial - More on Lists](https://docs.python.org/3/tutorial/datastructures.html){: target="_blank" rel="noopener noreferrer" }, checked 2026-06-29.
- Python Software Foundation, [Standard Library - `copy`](https://docs.python.org/3/library/copy.html){: target="_blank" rel="noopener noreferrer" }, checked 2026-06-29.
