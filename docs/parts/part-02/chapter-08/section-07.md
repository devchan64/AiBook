...
```python
# 이 예제는 참조와 복사가 리스트 같은 변경 가능한 값에 어떤 차이를 만드는지 확인합니다.
scores = [82, 75, 91]
other_scores = scores
...
## 참조와 복사 구분이 실제로 갈리는 장면

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "참조와 복사 차이를 값이 함께 바뀌는 문제로 설명해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
