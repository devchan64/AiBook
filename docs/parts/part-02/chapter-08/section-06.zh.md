...
这里提供一个基础补充说明，用来阅读 `class`、`object`、`method`。这个补充学习整理的是：怎样去读 `value.method()`、`model.fit()` 这样的调用形状。`值(value)`、`类型(type)`、`字典(dictionary)` 的代表性说明仍然放在 P2-8.1、P2-8.3 和[类词条](/AiBook/zh/reference/concept-glossary-pinyin/c.zh/#class)里，而这里是在这些内容之上，再加上类和对象的视角。
