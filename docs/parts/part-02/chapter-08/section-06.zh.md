...
在这个例子中，首先要看的不是清理字符串的技巧本身，而是 `text.strip()` 和 `text.lower()` 的调用形态。同一个值 `text` 放在前面，点号后面接的是这个值提供的动作名称。以后看到 `model.fit()` 或 `dataset.map()` 时，也可以先想起这个结构。

在 P2-8.5 里，我们把函数(function)看成小型复用单元。函数接收输入、进行处理、返回结果。但在阅读 Python 代码时，我们经常会遇到一种看起来和函数调用相似、却又稍微不同的表达。

这里提供一个基础补充说明，用来阅读 `class`、`object`、`method`。这个补充学习整理的是：怎样去读 `value.method()`、`model.fit()` 这样的调用形状。`值(value)`、`类型(type)`、`字典(dictionary)` 的代表性说明仍然放在 P2-8.1、P2-8.3 和P2-8.6里，而这里是在这些内容之上，再加上类和对象的视角。
