...
Part 2 Chapter 8에서는 `딕셔너리(dictionary)`와 `키(key)`의 상세 설명을 이 절에 대표로 둡니다. 뒤 절에서 반복 안의 `.items()`나 함수 인자 묶음을 다시 만나더라도, 이름으로 값을 찾는 구조의 기본 설명은 여기 기준으로 다시 연결합니다.
...
이 절은 문법보다 `무엇을 기준으로 값을 찾을 것인가`를 먼저 보는 절입니다. 아래 세 가지 관점이 뒤 절의 반복과 키 누락 처리 설명을 읽는 기준이 됩니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 딕셔너리(dictionary)는 `키로 값을 찾는 구조`라는 점 | 리스트와의 차이를 가장 빠르게 잡게 해 줍니다. | 위치가 아니라 이름표나 식별자로 찾는 구조입니다. |
| 키(key)와 값(value)은 `찾는 기준`과 `연결된 내용`이라는 점 | 설정값, 라벨 맵, ID 조회를 같은 구조로 읽게 해 줍니다. | `"score"`로 점수를 찾고 `0`으로 라벨 이름을 찾는 구조입니다. |
| 딕셔너리는 먼저 `매핑(mapping)`으로 이해하는 편이 안전하다는 점 | 구현 용어인 해시맵에 바로 갇히지 않게 해 줍니다. | 코드 읽기 단계에서는 키로 값을 연결하는 구조로 읽습니다. |
