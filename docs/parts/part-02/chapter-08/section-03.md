...
여기서는 `딕셔너리(dictionary)`와 `키(key)`의 기본 구분을 설명합니다. `리스트(list)`와 순서 기반 접근의 대표 설명은 P2-8.2에, `값(value)`과 `타입(type)`의 대표 설명은 P2-8.1과 [개념사전](../../../reference/concept-glossary.md)에 두고, 여기서는 “어떤 기준으로 값을 찾을 것인가”에 집중합니다.
...
- 몇 번째 값인가보다 어떤 이름이나 식별자로 찾는가가 중요한 데이터를 만나 딕셔너리 구조를 먼저 떠올려야 할 때 떠올립니다.
- 리스트와 딕셔너리의 차이를 문법이 아니라 위치와 키의 차이로 다시 설명해야 할 때 떠올립니다.
- 라벨 맵, 설정값, 샘플 ID, 컬럼 설명처럼 매핑 구조를 읽는 예제가 나왔을 때 떠올립니다.
