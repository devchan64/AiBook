...
여기서는 `딕셔너리(dictionary)`와 `키(key)`의 기본 구분을 설명합니다. `리스트(list)`와 순서 기반 접근의 대표 설명은 P2-8.2에, `값(value)`과 `타입(type)`의 대표 설명은 P2-8.1과 [개념사전의 딕셔너리 항목](../../../reference/concept-glossary-parts/03-digeut.md#dictionary)에 두고, 여기서는 “어떤 기준으로 값을 찾을 것인가”에 집중합니다.
