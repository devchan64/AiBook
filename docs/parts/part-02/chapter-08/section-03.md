...
지금 읽는 층위는 `딕셔너리 문법을 따로 외우는 절`이 아니라 `값 묶음을 읽는 기준을 순서에서 이름으로 바꾸는 층위`입니다. 바로 앞 절의 리스트가 `몇 번째 값인가`를 중심으로 읽혔다면, 여기서는 `어떤 이름이나 식별자로 찾는가`가 중심이 됩니다. 이 차이를 먼저 잡아 두면, 뒤의 반복 절에서 리스트와 딕셔너리를 같은 loop 안에서 돌리더라도 `무엇을 기준으로 꺼내고 있는가`를 구분하기 쉬워집니다.

처음 읽을 때는 구현 세부보다 `이 데이터는 위치보다 이름이 중요한가`, `찾는 기준이 키인가`, `리스트로 두면 오히려 더 헷갈리지 않는가`라는 세 질문만 먼저 붙잡으면 충분합니다.
...
이 절은 문법보다 `무엇을 기준으로 값을 찾을 것인가`를 먼저 보는 절입니다. 먼저는 아래 세 가지 관점만 보면 충분합니다.

| 먼저 볼 것 | 왜 중요한가 | 이 절에서 먼저 이해할 수준 |
| --- | --- | --- |
| 딕셔너리(dictionary)는 `키로 값을 찾는 구조`라는 점 | 리스트와의 차이를 가장 빠르게 잡게 해 줍니다. | 위치가 아니라 이름표나 식별자로 찾는다는 점만 알면 충분합니다. |
| 키(key)와 값(value)은 `찾는 기준`과 `연결된 내용`이라는 점 | 설정값, 라벨 맵, ID 조회를 같은 구조로 읽게 해 줍니다. | `"score"`로 점수를 찾고 `0`으로 라벨 이름을 찾는 정도만 이해하면 충분합니다. |
| 딕셔너리는 먼저 `매핑(mapping)`으로 이해하는 편이 안전하다는 점 | 구현 용어인 해시맵에 바로 갇히지 않게 해 줍니다. | 코드 읽기 단계에서는 키로 값을 연결하는 구조라는 점만 잡으면 충분합니다. |
...
확인 가능한 결과는 예측 라벨 숫자를 넣었을 때 바로 이름이 나오는지입니다. `label_map[prediction]`으로 원하는 문자열을 얻을 수 있다면, 이 문제는 리스트보다 딕셔너리로 읽는 편이 더 자연스럽습니다.
