...
这里会说明 `循环（loop）`、`iterable`、`iterator` 的基本区分。`列表（list）` 和 `字典（dictionary）` 的代表性说明放在 P2-8.2、P2-8.3 和 [循环词条](/AiBook/zh/reference/concept-glossary-pinyin/l.zh/#loop)，而这里集中处理的是：如何从这些集合里把值一个个拿出来，转成处理流程。
