...
这里会说明 `循环（loop）`、`iterable`、`iterator` 的基本区分。`列表（list）` 和 `字典（dictionary）` 的代表性说明放在 P2-8.2、P2-8.3 和 [概念词典](/AiBook/en/reference/concept-glossary/)，而这里集中处理的是：如何从这些集合里把值一个个拿出来，转成处理流程。
...
- 循环是把同一种处理作用到多个值上的流程。
- 在 Python 中，`for` 通常比起“循环几次”，更自然地读成“从什么里面一个个拿出来”。
- `enumerate()`、`.items()`、`zip()` 是会根据循环目标形状改变阅读方式的工具。
- 循环结果经常会表现成输出、生成新集合、生成累积值。
- 以后看到函数（function）时，会继续连到 `把循环包起来重复利用` 这个视角。

循环是一种从集合里一个个取出值并处理的方式。在 Python 中，如果把它读成 `for` 接收一个可迭代目标，并逐个消耗值流` 的结构，那么很多不同的循环模式都能理解。

如果把它一般化，循环就是把数据集合变成结果集合、累积值、被分离的集合或输出的过程。比起语法，先确认输入集合和结果形状。
...
4. 结果是新列表、新字典，还是累积值？
5. 是否正在直接修改原对象？
6. 如果是生成新数据结构的短规则，它是不是写成了 comprehension？
...
- 当已经看过列表和字典，现在要真正进入“一个个处理它们”的流程时，就想起这个视角。
- 当需要重新区分 `for item in items`、`enumerate()`、`.items()`、`zip()` 各自到底一个个拿出了什么时，就想起这个视角。
- 当你分不清循环结果到底是输出、新列表还是累积值，需要先把结果形状分开来看时，就想起这个视角。
