...
Here, we explain the basic distinction between a `list` and an `index`. The representative explanation of `value`, `variable`, and `type` is placed in P2-8.1 and in the [list glossary entry](/AiBook/en/reference/concept-glossary-alpha/l.en/#list), and here we focus on why, once values become several, an ordered group is needed first.
