...
Here, we explain the basic distinction between a `dictionary` and a `key`. The representative explanation of `list` and order-based access is placed in P2-8.2, and the representative explanation of `value` and `type` is placed in P2-8.1 and in P2-8.3. Here, we focus on `by which criterion should a value be found?`
