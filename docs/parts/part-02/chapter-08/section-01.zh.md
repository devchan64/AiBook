...
这里会说明 `值（value）`、`变量（variable）`、`类型（type）` 的基本区分。这三个词并不只局限在 Python 里，而是大多数编程语言与数据处理工具共享的最小单位。以后在 Part 2 中再次遇到这些概念时，也把 [概念词典](/AiBook/en/reference/concept-glossary/) 一起当作参照点。
