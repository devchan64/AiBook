...
Part 2 안에서는 이 절을 `미분(differentiation)`, `변화 비교(change comparison)`, `순간 변화율(instantaneous rate of change)`, `접선(tangent line)`, `기억 복구`를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. Chapter 3에서 데이터 모양과 벡터 계산을 읽었다면, 여기서는 값이 변할 때 무엇을 질문해야 하는지 미분의 입구부터 다시 정리합니다.
...
| --- | --- | --- |
| 미분이 공식보다 변화 비교의 질문이라는 점 | P2-4.2에서 변화율과 기울기를 더 직접 봅니다. | Part 2 Chapter 6에서 손실이 줄어드는 방향을 읽을 때 다시 씁니다. |
...
| 순간 변화율 | 한 점 근처에서의 변화 속도 | 뒤 절의 변화율 설명으로 가는 손잡이 |
...
또한 이 절은 `미분 공식 계산법`을 본격적으로 연습하는 절도, `그래디언트`를 자세히 설명하는 절도 아닙니다. 여기서는 변화 질문을 먼저 다시 세우고, 변화율과 기울기는 P2-4.2에서, 미분과 그래디언트의 연결은 P2-4.3에서 다시 좁혀 봅니다.
