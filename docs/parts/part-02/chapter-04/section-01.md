...
- 미분은 공식을 외우는 절차보다 `어떤 변화가 어떤 결과 차이로 이어지는가`를 묻는 질문으로 먼저 읽습니다.
...
1. 값이 변합니다.
2. 얼마나 변했는지 비교합니다.
3. 입력 변화에 대한 출력 변화를 봅니다.
4. 그것을 기울기와 변화율로 읽습니다.
5. 아주 작은 구간의 변화로 가면 미분으로 이어집니다.

여기서는 기억을 복구했고, 다음 절에서는 그 기억을 계산 가능한 형태로 정리합니다.
...
- 접선, 미분계수, 순간 속도라는 단어는 떠오르는데 서로 어떤 질문으로 묶이는지 흐려질 때 먼저 떠올립니다.
- 손실이나 그래디언트 설명을 읽기 전에 `값이 얼마나 변하는가`라는 질문 자체를 다시 복구해야 할 때 먼저 떠올립니다.
- 공식을 기억하고 있어도 왜 두 점을 가깝게 만드는지, 왜 접선 기울기를 보는지 설명이 끊길 때 먼저 떠올립니다.
