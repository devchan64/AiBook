...
Part 2 안에서는 이 절을 `미분(derivative)`, `도함수(derivative function)`, `편미분(partial derivative)`, `그래디언트(gradient)`, `나블라(nabla)`를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 4.2에서 변화율과 기울기를 읽었다면, 여기서는 한 변수의 순간 변화율이 여러 변수의 변화 정보로 어떻게 확장되는지 정리합니다.
...
| --- | --- | --- |
| 미분계수, 도함수, 편미분의 역할 구분 | 뒤 절과 보충학습에서 표기와 계산 감각을 더 천천히 복구합니다. | Part 2 Chapter 6에서 손실 함수와 파라미터 업데이트를 읽을 때 다시 씁니다. |
...
| 나블라 | 그래디언트를 적을 때 쓰는 기호 | 수식 표기를 덜 낯설게 하는 손잡이 |
...
여기서 \(\partial\) 기호는 여러 변수 중 하나에 대한 변화율을 본다는 뜻으로 이해합니다. 엄밀한 계산 규칙은 나중에 다루더라도, 지금은 다음 두 가지를 먼저 잡습니다.
...
또한 이 절은 `연쇄 법칙(chain rule)`이나 `야코비안, 헤시안`을 자세히 다루는 절도, `경사하강법 업데이트 식`을 설명하는 절도 아닙니다. 여기서는 그래디언트가 무엇인지 먼저 읽고, 학습에서 왜 필요한지는 P2-4.4에서, 더 느린 복구 흐름은 P2-4.5에서 다시 좁혀 봅니다.
...
1. 손실(loss)은 파라미터 `w_1`의 영향을 받습니다.
2. 손실은 파라미터 `w_2`의 영향도 받습니다.
3. 손실은 파라미터 `w_3`의 영향도 받습니다.
4. 같은 식으로 더 많은 파라미터의 영향도 받습니다.

학습 중에는 다음 질문을 반복합니다.

1. `w_1`을 조금 바꾸면 손실이 줄어드는가, 늘어나는가?
2. `w_2`를 조금 바꾸면 손실이 얼마나 민감하게 변하는가?
...
이 절에서는 위 표현들을 모두 자세히 설명하지 않습니다. 지금은 “그래디언트가 여러 변화율을 묶은 방향 정보”라는 중심을 먼저 잡습니다.
...
1. 미분(derivative)은 한 입력에 대한 순간 변화율입니다.
2. 편미분(partial derivative)은 여러 입력 중 하나를 따로 바꿨을 때의 변화율입니다.
3. 그래디언트(gradient)는 여러 편미분을 모은 벡터입니다.
4. AI 학습에서 그래디언트는 손실이 각 파라미터 방향으로 어떻게 변하는지 알려 주는 정보입니다.
