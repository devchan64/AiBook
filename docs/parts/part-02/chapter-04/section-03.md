...
이제 그 흐름을 미분(derivative)과 그래디언트(gradient)로 연결합니다. 여기서는 미분 계산 공식을 많이 외우기보다 AI 문서에서 `derivative`, `gradient`, `partial derivative`라는 말을 만났을 때 무엇을 가리키는지 구분하는 데 집중합니다.
...
| --- | --- | --- |
| 미분계수, 도함수, 편미분의 역할 구분 | `P2-4.4`와 `P2-4.5`에서 표기와 계산 감각을 더 천천히 복구합니다. | `P2-6.1`, `P2-6.2`, `P2-6.3`에서 손실 함수와 파라미터 업데이트를 읽을 때 다시 씁니다. |
...
- `derivative`, `partial derivative`, `gradient`가 한 문단 안에 함께 나오는데 각각 무엇을 가리키는지 헷갈릴 때 먼저 떠올립니다.
- 한 변수 미분 기억만 남아 있어 다변수 손실 함수와 파라미터 업데이트 설명이 갑자기 멀어질 때 먼저 떠올립니다.
- 그래디언트를 단순한 숫자 묶음이 아니라 방향별 변화율 벡터로 다시 읽어야 할 때 먼저 떠올립니다.
