...
독자가 여기서 가장 자주 놓치는 지점은 `기울기`를 그래프 문제로만 읽고 끝내는 것입니다. 하지만 학습 문맥에서는 기울기가 `지금 이 값을 조금 바꾸면 결과가 어느 쪽으로 얼마나 움직일까`를 묻는 준비 단계입니다.
...
즉 `그래프에서 기울기를 읽는다`는 말은 뒤에서 `손실을 줄이려면 어느 방향으로 움직여야 하는가`를 읽는 말로 바뀝니다.
