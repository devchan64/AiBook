...
Part 2 안에서는 이 절을 `변화율(rate of change)`, `기울기(slope)`, `평균 변화율(average rate of change)`, `순간 변화율(instantaneous rate of change)`, `곡선(curve)`을 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 4.1에서 미분을 질문으로 다시 꺼냈다면, 여기서는 그 질문을 구간과 한 점의 변화 비교로 더 구체화합니다.

지금 읽는 층위는 `도함수 공식을 바로 계산하는 절`이 아니라 `구간의 변화율에서 한 점 근처의 변화율로 시야를 좁히는 입문 층위`입니다. 여기서 변화율, 기울기, 평균 변화율, 순간 변화율의 연결을 잡아 두면, 뒤에서 손실(loss)을 줄이는 방향과 그래디언트(gradient)를 읽을 때 왜 `조금 바꾸면 얼마나 달라지는가`를 먼저 묻는지 바로 연결됩니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 변화율이 입력 변화 대비 출력 변화라는 점 | 다음 절에서 순간 변화율과 미분 표기를 더 직접 봅니다. | Part 2 Chapter 6에서 손실이 얼마나 줄어드는지 읽을 때 다시 씁니다. |
| 평균 변화율과 순간 변화율의 차이 | 뒤 절에서 도함수 표기와 접선 의미로 이어집니다. | Part 4의 gradient, 역전파, 최적화 설명으로 이어집니다. |
...
| 기울기 | 변화율을 그래프에서 읽는 값 | 시각적 해석의 손잡이 |
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 변화율은 무엇을 비교하는가? | 입력이 얼마나 바뀌었을 때 출력이 얼마나 바뀌었는지 비교합니다. |
| 기울기는 무엇을 보여 주는가? | 변화율을 그림 위에서 읽게 해 주는 시각적 표현입니다. |
| 왜 미분으로 이어지는가? | 구간 전체의 변화율에서 한 점 근처의 더 작은 변화율로 좁혀 가기 때문입니다. |
...
반면 미분방정식(differential equation)은 함수와 그 함수의 도함수들이 함께 들어 있는 방정식을 말합니다. 이 절의 목표는 미분방정식을 푸는 것이 아닙니다. 다만 “차수”라는 기억이 있다면, 우선은 `몇 번 변화율을 보느냐`라는 정도로 이해합니다.
...
또한 이 절은 `도함수 공식 계산`을 본격적으로 연습하는 절도, `경사하강법`을 자세히 설명하는 절도 아닙니다. 여기서는 변화율과 기울기의 구분을 먼저 읽고, 미분과 그래디언트의 연결은 P2-4.3과 P2-4.4에서 다시 회수합니다.

이 절에서는 여기까지를 먼저 잡습니다. 미분 계산법은 다음 절에서 다룹니다.
