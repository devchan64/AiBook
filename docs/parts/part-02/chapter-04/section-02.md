...
지금 읽는 층위는 `도함수 공식을 바로 계산하는 절`이 아니라 `구간의 변화율에서 한 점 근처의 변화율로 시야를 좁히는 입문 층위`입니다. 여기서 변화율, 기울기, 평균 변화율, 순간 변화율의 연결을 잡아 두면, 뒤에서 손실(loss)을 줄이는 방향과 그래디언트(gradient)를 읽을 때 왜 `조금 바꾸면 얼마나 달라지는가`를 먼저 묻는지 자연스럽게 이어집니다.

처음 읽을 때는 기호보다 `입력 변화 대비 출력 변화`, `구간 전체`, `한 점 근처`라는 세 표현의 차이만 먼저 붙잡으면 충분합니다.
...
확인 가능한 결과는 주차별 광고비와 가입자 수를 표로 놓고, `광고비 증가분 대비 가입자 증가분`을 계산해 보는 것입니다. 구간마다 이 값이 달라진다면 곡선처럼 반응한다고 볼 수 있고, 바로 그 지점에서 평균 변화율과 순간 변화율을 구분해야 할 이유가 생깁니다.
