...
여기서 먼저 해결할 질문은 이것입니다. `출력이 얼마나 변했는지를 넘어서, 입력이 얼마나 바뀌었을 때 그 변화가 생겼는지 왜 함께 봐야 하는가`입니다.
...
- `P2-4.3`에서는 이 변화율 질문을 미분, 편미분, 그래디언트 표기로 더 직접 읽습니다.
- `P2-6.1`부터 `P2-6.3`까지는 손실을 얼마나 줄일 수 있는지와 파라미터를 어느 방향으로 바꿀지를 이 변화율 언어로 다시 읽습니다.
- Part 4에서는 같은 질문이 `gradient`, `update`, `optimizer` 언어로 반복됩니다.
