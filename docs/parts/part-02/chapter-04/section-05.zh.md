...
这里提供的是一节补充学习，用来减轻从高中微分记忆跨到梯度时产生的陌生感。`梯度(gradient)` 的代表性说明放在 P2-4.3，`梯度下降(gradient descent)` 的代表性说明放在 P2-6.3，而这一节只更慢地整理填平它们之间间隔所需要的背景。想快速重新确认术语时，也可以一起看[概念词汇表](/AiBook/en/reference/concept-glossary/)。
