...
이 절은 고등학교 미분 기억에서 그래디언트로 넘어갈 때 생기는 낯섦을 줄이기 위한 보충학습입니다. 분량은 조금 길지만, 목적은 하나입니다. AI 학습 문서에서 반복해서 만나는 `gradient`, `gradient descent`, `backpropagation`이라는 말을 처음부터 계산 공식으로 밀어붙이지 않고, 무엇을 보려는 개념인지 먼저 잡는 것입니다.

Part 2 안에서는 이 절을 `보충학습`, `방향도함수(directional derivative)`, `경사하강법(gradient descent)`, `역전파(backpropagation)`, `벡터해석(vector calculus)`으로 이어지는 느린 복구 절로 사용합니다. 4.3과 4.4가 본문에서 개념 연결을 먼저 잡는 절이라면, 여기서는 그 간격을 더 천천히 메우는 역할을 맡습니다.

한 변수 함수의 미분은 한 방향의 변화율을 읽는 일이고, 여러 변수 함수의 그래디언트는 여러 방향의 변화율 묶음입니다. 경사하강법은 그 묶음을 참고해 손실을 줄이는 반복 방법입니다.

## 먼저 정의부터 잡기

이 보충학습에서는 엄밀한 대학 수학 정의보다, AI 학습 문서를 읽기 위한 작업용 정의를 먼저 둡니다. 이후 설명과 차트는 이 정의를 조금씩 풀어 가는 방식으로 이어집니다.
...
미분은 한 방향에서 얼마나 변하는지 보고, 편미분은 여러 방향 중 하나씩 따로 봅니다. 방향도함수는 내가 고른 방향으로 변화를 보는 방식이고, 그래디언트는 그런 변화율들을 한 묶음으로 모은 벡터입니다. 경사하강법은 그 묶음을 참고해 손실이 줄어드는 쪽으로 조금씩 이동하는 방법이며, 역전파는 그 묶음을 딥러닝 모델 안에서 효율적으로 계산하는 절차입니다.
...
그래디언트는 현재 위치의 방향 정보이고, 경사하강법은 그 방향 정보를 이용한 반복 이동 방법입니다. 역전파는 그 방향 정보를 효율적으로 구하는 계산 절차입니다.

이 구분을 먼저 잡아 두면 뒤의 설명이 덜 헷갈립니다.

또한 이 절은 `편미분 공식 연습장`도 아니고, `딥러닝 최적화 전부`를 끝내는 절도 아닙니다. 여기서는 고등학교 미분 감각에서 다변수 미분으로 넘어가는 간격만 메우고, 실제 업데이트 식과 계산 절차는 뒤 Chapter와 뒤 Part에서 다시 회수합니다.
...
| 경사하강법 | 손실을 줄이는 쪽으로 반복 이동하는 방법 | 그래디언트 활용의 대표 사례 |
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 왜 이 절이 따로 필요한가? | 고등학교 미분과 AI 학습의 다변수 미분 사이에 설명 간격이 크기 때문입니다. |
| 여기서 가장 먼저 구분할 것은 무엇인가? | 그래디언트, 경사하강법, 역전파는 같은 말이 아니라 서로 다른 역할입니다. |
| 어디까지 이해하면 충분한가? | 계산 공식을 다 외우기보다, 각 용어가 무엇을 보려는지 설명할 수 있으면 됩니다. |
...
미분은 한 방향의 변화율이고, 편미분은 여러 방향 중 하나를 따로 본 변화율입니다. 그래디언트는 여러 방향의 변화율을 모은 벡터이며, 경사하강법은 그 벡터를 이용해 손실을 낮추는 반복 방법입니다. 역전파는 그 벡터를 딥러닝 모델 안에서 계산하는 절차입니다.
