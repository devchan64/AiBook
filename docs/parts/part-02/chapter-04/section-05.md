...
여기서 `조금`이라는 말이 중요합니다. 너무 크게 움직이면 낮은 곳을 지나쳐 버릴 수 있고, 너무 작게 움직이면 오래 걸릴 수 있습니다. 이 이동 폭은 P4-7.1에서 학습률(learning rate)이라는 이름으로 다시 만납니다.
...
편미분이 경사하강법으로 이어지는 계산은 P2-6.3에서, 손실이 각 가중치에 어떻게 전달되는지는 P4-5.1과 P4-5.2에서, 실제 업데이트 규칙은 P4-7.1과 P4-7.2에서 다시 만납니다. 지금은 용어가 서로 어떻게 연결되는지, 그리고 왜 AI 학습에서 반복해서 등장하는지를 잡는 것이 목표입니다.
