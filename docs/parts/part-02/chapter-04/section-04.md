...
| --- | --- | --- |
| 손실만으로는 방향을 알 수 없고 미분이 필요하다는 점 | `P2-4.5`와 `P2-6.3`에서 경사하강법의 실제 업데이트를 봅니다. | `P2-6.1`, `P2-6.2`, `P2-6.3`의 학습 절차와 손실 감소 설명에서 다시 씁니다. |
...
여기서는 학습이 무엇을 조정하는지, 손실이 무엇을 말해 주는지, 미분이 손실을 줄이는 데 어떤 정보를 주는지, 역전파가 왜 나중에 등장하는지를 중심으로 읽습니다.
...
| --- | --- | --- |
| 학습은 파라미터를 조정하는 과정이다 | 모델은 현재 결과를 보고 내부 숫자를 조금씩 바꾸며 개선되기 때문입니다. | 학습을 `예측 -> 손실 계산 -> 파라미터 조정`의 반복으로 이해합니다. |
...
- 손실 함수는 이해했는데 왜 미분과 그래디언트가 학습 설명에 바로 붙는지 납득이 안 될 때 먼저 떠올립니다.
- `얼마나 틀렸는가`와 `어느 방향으로 바꿔야 하는가`를 구분해서 설명해야 할 때 먼저 떠올립니다.
- 역전파를 학습 전체와 같은 말처럼 받아들이고 있어 역할을 다시 나눠야 할 때 먼저 떠올립니다.
