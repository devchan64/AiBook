...
지금 읽는 층위는 `최적화 공식을 자세히 전개하는 절`이 아니라 `왜 학습에서 미분과 그래디언트가 필요한지 연결하는 입문 층위`입니다. 여기서 손실, 파라미터, 조정 방향의 관계를 잡아 두면, 뒤에서 경사하강법(gradient descent), 역전파(backpropagation), 옵티마이저(optimizer)를 볼 때 계산 절차보다 목적이 먼저 읽힙니다.

처음 읽을 때는 업데이트 식을 외우기보다 `손실은 나쁨의 크기`, `미분은 바꿀 방향`, `학습은 반복 조정`이라는 세 문장만 먼저 붙잡으면 충분합니다.

| 지금 이 절에서 잡을 것 | 바로 다음에 이어질 질문 | 이후 다시 쓰이는 위치 |
| --- | --- | --- |
| 손실만으로는 방향을 알 수 없고 미분이 필요하다는 점 | 보충학습과 뒤 장에서 경사하강법의 실제 업데이트를 봅니다. | P2-6의 학습 절차와 손실 감소 설명에서 다시 씁니다. |
| 파라미터 조정이 학습의 핵심이라는 점 | 다음 단계에서 학습률과 반복 업데이트 질문으로 넘어갑니다. | Part 4의 옵티마이저와 학습 안정성 설명으로 이어집니다. |
| 그래디언트가 여러 파라미터 방향의 정보를 준다는 점 | 이어지는 역전파 예고를 통해 계산 절차로 연결됩니다. | 딥러닝 전체에서 `gradient`, `backpropagation` 용어를 읽는 기본 틀이 됩니다. |
...
만약 `도함수 부호는 알겠는데 그래디언트와 경사하강법이 왜 같이 나오는지`가 아직 낯설다면 바로 뒤의 P2-4.5 보충학습으로 잠깐 돌아가는 편이 좋습니다. 그 절은 고등학교 미분 감각에서 다변수 미분과 그래디언트로 넘어가는 간격을 메우기 위해 따로 둔 절입니다.
...
| 질문 | 먼저 잡을 관점 |
| --- | --- |
| 학습은 무엇을 하는가? | 모델 내부 숫자를 조금씩 바꿔 더 나은 결과를 찾습니다. |
| 손실만 보면 왜 부족한가? | 얼마나 나쁜지는 알 수 있어도 어디로 바꿀지는 알기 어렵기 때문입니다. |
| 미분은 왜 필요한가? | 손실을 줄일 방향 정보를 주기 때문입니다. |
...
집값 예측 모델을 운영한다고 하겠습니다. 검증 결과에서 어떤 집의 실제 가격은 5억 원인데 모델은 4억 원으로 예측했습니다. 사람은 곧바로 `1억 원 틀렸다`는 사실을 확인할 수 있습니다.

하지만 그 숫자만으로는 모델을 어떻게 고쳐야 하는지 알 수 없습니다. 면적을 더 크게 반영해야 하는지, 위치 정보를 더 민감하게 읽어야 하는지, 특정 파라미터를 올려야 하는지 내려야 하는지가 드러나지 않기 때문입니다. 손실은 `얼마나 틀렸는가`를 말해 주지만 `어디로 움직일 것인가`는 말해 주지 않습니다.
