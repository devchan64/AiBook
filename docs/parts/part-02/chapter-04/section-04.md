...
Part 2 안에서는 이 절을 `학습(training)`, `손실(loss)`, `파라미터(parameter)`, `조정 방향(update direction)`, `역전파(backpropagation)`를 다시 읽기 위한 대표 상세 설명 위치로 사용합니다. 4.3에서 그래디언트를 읽었다면, 여기서는 그 변화 정보가 왜 실제 학습 절차에 필요해지는지 정리합니다.
...
| --- | --- | --- |
| 손실만으로는 방향을 알 수 없고 미분이 필요하다는 점 | 보충학습과 뒤 장에서 경사하강법의 실제 업데이트를 봅니다. | Part 2 Chapter 6의 학습 절차와 손실 감소 설명에서 다시 씁니다. |
...
`도함수 부호는 알겠는데 그래디언트와 경사하강법이 왜 같이 나오는지`가 아직 낯설다면 바로 뒤의 P2-4.5 보충학습이 그 간격을 메웁니다. 그 절은 고등학교 미분 감각에서 다변수 미분과 그래디언트로 넘어가는 연결을 더 천천히 복구하기 위해 따로 둔 절입니다.
...
학습은 파라미터를 조정해 손실을 줄이는 과정입니다. 손실은 현재 모델이 얼마나 좋지 않은지 알려 주는 숫자이고, 미분은 한 파라미터를 조금 바꿨을 때 손실이 어떻게 변하는지 보여 줍니다. 그래디언트는 여러 파라미터에 대한 변화 정보를 모은 벡터이며, 역전파는 복잡한 모델에서 그 그래디언트를 효율적으로 계산하는 절차입니다.

미분은 AI 학습에서 수학적 장식이 아닙니다. 모델이 스스로 더 나은 방향으로 움직이게 하려면, 현재 값 주변에서 손실이 어떻게 변하는지 읽어야 합니다. 그 읽기 방법이 미분과 그래디언트입니다.

또한 이 절은 `옵티마이저 종류 비교`를 다루는 절도, `역전파 계산 절차`를 상세히 설명하는 절도 아닙니다. 여기서는 손실과 방향 정보의 연결을 먼저 읽고, 실제 최적화 절차와 계산 구조는 Part 2 Chapter 6과 뒤 Part에서 다시 회수합니다.
