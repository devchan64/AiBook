...
- 합성함수는 `출력이 다음 입력으로 들어가는 계산 파이프라인`입니다.
- 연쇄 법칙은 `앞단 변화가 뒤단 결과까지 얼마나 전달되는가`를 읽는 규칙입니다.
- `x -> y`와 `y -> z`를 따로 본 뒤 이어 붙여야 `x -> z`를 읽을 수 있습니다.
- 역전파는 연쇄 법칙 그 자체가 아니라, 그 규칙을 여러 층에 적용해 gradient를 계산하는 절차입니다.
- 뒤 Part에서 `backpropagation`이 나오면 먼저 `손실의 변화가 여러 단계를 거쳐 전달된다`는 그림을 떠올리면 됩니다.
