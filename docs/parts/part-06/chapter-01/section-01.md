...
이 생략 항목 가운데 토크나이저 계열의 비교와 한국어를 포함한 분절 감각은 같은 장의 P6-1.3 보충학습에서 다시 회수합니다. 멀티모달 토큰의 내부 설계는 이 책의 현재 본편 범위를 넘어가므로, 여기서는 `텍스트를 계산 단위로 쪼갠다`는 공통 감각까지만 잡습니다.
...
| 토큰화(tokenization) | 그 조각은 실제로 어떻게 잘리고 왜 언어마다 체감이 다른가? | P6-1.2, P6-1.3 |
