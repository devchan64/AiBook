...
이 질문은 P6-1.2 토큰화(tokenization)의 실제 영향으로 이어집니다.
...
- 토큰을 `단어`와 같은 말로만 쓰지 않고, `모델이 실제로 읽고 쓰는 계산 단위`로 설명할 수 있는가?
- 같은 문장 길이 감각과 실제 토큰 길이 감각이 다를 수 있다는 점을 사례로 말할 수 있는가?
- 다음 절을 읽을 때도 먼저 `이 계산 단위가 실제로 어떻게 잘리는가`를 떠올릴 준비가 되어 있는가?

## 언제 토큰 관점을 먼저 떠올려야 하는가
