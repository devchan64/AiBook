...
토큰화 종류 차이를 읽을 때 초심자가 자주 하는 실수는, 세 계열 이름을 떠올린 뒤에도 정작 지금 문제를 어느 장면으로 읽어야 하는지 바로 못 고르는 일입니다. 이때는 `어느 계열이 더 좋으냐`보다 `지금 어떤 실패가 먼저 보이는가`를 먼저 고르는 편이 안전합니다.
...
이 표의 목적은 토크나이저 계열을 다시 설명하는 데 있지 않습니다. 초심자가 실제 운영 장면을 봤을 때 `지금 드러난 차이가 비용 문제인지`, `청크 문제인지`, `혼합 표기 길이 문제인지`를 먼저 분기하게 만드는 데 있습니다.
...
`환불 정책을 알려줘`, `환불 관련 정책을 간단히 설명해 줘`, `환불이 가능한 조건만 짧게 정리해 줘`는 같은 의도처럼 보이지만, 토크나이저에 따라 조각 경계와 토큰 수가 다르게 늘 수 있습니다. 이때 중요한 것은 `같은 뜻인가`보다 `어느 표현이 반복해서 더 많은 입력 예산을 먹는가`입니다.

### 사례 2. 정책 청크가 자꾸 어색하게 끊길 때

`연차는 3일 전 신청`과 `긴급 병가는 사후 보고 가능`이 서로 다른 조각으로 찢어지면, 검색 결과는 원칙만 가져오고 중요한 예외를 놓칩니다. 여기서 토크나이저 차이는 `문단 모양`보다 `실제 분절 경계`가 더 중요하다는 점에서 드러납니다.

### 사례 3. 혼합 표기 문서에서 비용이 튈 때

`SDK v2.1`, `Authorization`, 긴 URL, 코드 조각이 섞인 문장은 겉보기보다 토큰 수가 더 빨리 늘 수 있습니다. 이런 구간은 공백 기준 단어 감각보다 실제 문자열 처리 방식이 얼마나 어긋나는지가 더 중요합니다.
