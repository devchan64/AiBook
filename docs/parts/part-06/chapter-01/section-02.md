# P6-1.2 토큰화(tokenization)의 실제 영향

> Section ID: `P6-1.2`
> Version: `v2026.07.12`

P6-1.1에서는 토큰(token)을 모델이 텍스트를 계산하기 위해 사용하는 기본 단위라고 설명했습니다. 이제는 텍스트를 어떤 규칙으로 토큰으로 나누느냐가 실제로 왜 중요한지 바로 이어서 봐야 합니다. 토큰화(tokenization)는 텍스트를 모델 계산 단위로 바꾸는 절차이며, 이 절차가 입력 길이, 비용, 문맥 사용량, 검색 품질, 생성 결과 해석에 실제 영향을 줍니다.

Part 6에서 `토큰화(tokenization)`, `같은 문장도 토큰 수가 달라지는 이유`, `토큰화가 비용·검색·생성 해석에 미치는 영향`에 대한 첫 상세 설명은 이 절에서 잡습니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 분절 절차의 기본 뜻은 이 절과 개념사전을 기준으로 다시 연결합니다.

## 이 절의 범위

이 절은 다음 질문을 정리합니다.

- 토큰화는 무엇을 하는 절차인가?
- 왜 같은 문장도 토큰 수가 다르게 나올 수 있는가?
- 토큰화가 비용, 문맥 길이, 검색, 생성에 어떤 실제 영향을 주는가?
- 왜 어떤 언어와 표기 환경에서는 토큰 감각이 더 생소할 수 있는가?

이 절에서는 다음 내용을 깊게 다루지 않습니다.

- BPE, WordPiece, SentencePiece의 세부 알고리즘 비교
- 멀티링구얼 토크나이저의 학습 절차
- tokenizer vocabulary 설계 최적화

이 항목들은 같은 장의 P6-1.3, P6-1.4 보충학습에서 `대표 토크나이저 계열이 무엇인가`, `무엇을 다르게 자르는가`라는 관점으로 다시 묶어 설명합니다. 다만 vocabulary 최적화의 세부 구현과 학습 절차 전부를 따라가지는 않고, 처음 읽는 독자가 토큰화 차이를 읽는 데 필요한 수준까지만 다룹니다.

이 절에서는 토큰화를 단순 내부 구현이 아니라, 비용과 문맥 길이, 검색 품질에 직접 연결되는 실제 사용 문제로 읽습니다.

## 이 절의 목표

- 토큰화를 `원문 텍스트를 모델 계산 단위로 바꾸는 절차`로 설명할 수 있습니다.
- 같은 텍스트라도 토큰 수가 달라질 수 있는 이유를 말할 수 있습니다.
- 토큰화가 비용, context window, RAG 품질에 영향을 준다는 점을 설명할 수 있습니다.
- 이후 장의 임베딩, 프롬프트, RAG 설명을 더 구체적으로 읽을 수 있습니다.

## 토큰화는 무엇을 하나

토큰화(tokenization)는 원문 문자열(raw text)을 모델이 다룰 수 있는 토큰 시퀀스(sequence)로 바꾸는 절차입니다.

이 절차는 단순히 `띄어쓰기 기준으로 자르기`보다 더 복잡할 수 있습니다.

- 자주 나오는 조각은 하나로 묶일 수 있고
- 드문 표현은 더 잘게 쪼개질 수 있으며
- 숫자, 기호, 공백, 문장부호도 별도 처리될 수 있습니다

즉, 토큰화는 단순 분리 작업이 아니라 `모델이 언어를 어떤 계산 단위로 볼 것인가`를 결정하는 전처리 단계입니다.

## 왜 같은 문장도 토큰 수가 달라질 수 있나

같은 의미의 문장이라도 표기 방식이 조금만 달라져도 토큰 수가 달라질 수 있습니다.

예를 들어:

- 줄임말 사용
- 숫자 표기 방식
- 특수문자 포함 여부
- 영어와 한국어 혼용

같은 내용이어도 모델 입장에서는 다른 토큰 조합으로 읽힐 수 있습니다.

이 때문에 사용자는 `문장이 짧아 보이는데 왜 토큰 수가 많지?`라는 경험을 자주 하게 됩니다.

## 실제 영향 1. 비용과 문맥 길이

LLM 서비스에서는 토큰화가 곧 비용과 연결됩니다.

- 같은 길이처럼 보이는 두 입력도 토큰 수가 다를 수 있고
- 출력 역시 어떤 표현을 쓰느냐에 따라 토큰 수가 달라질 수 있습니다

또한 context window는 문장 수가 아니라 토큰 수 기준으로 차기 때문에, 긴 문서를 다룰 때는 토큰화 감각이 필수입니다.

`토큰화는 단지 텍스트를 자르는 일이 아니라, 실제로 얼마만큼의 입력 공간과 비용을 쓰는지 결정하는 절차다.`

## 실제 영향 2. 검색과 RAG

토큰화는 RAG(retrieval-augmented generation)에도 영향을 줍니다.

- 문서를 어떤 길이의 청크(chunk)로 나눌지
- 검색 결과를 얼마나 많이 넣을지
- 검색된 문맥이 잘리지 않고 핵심을 유지하는지

이 모두가 토큰 길이 감각과 연결됩니다.

즉, 토큰화는 모델 내부 문제이면서 동시에 서비스 설계 문제이기도 합니다.

## 실제 영향 3. 생성 결과 해석

생성은 보통 다음 토큰 예측(next-token prediction)으로 설명됩니다. 그러므로 토큰화 방식은 생성 과정 자체를 바꿀 수 있습니다.

예를 들어:

- 어떤 표현은 하나의 토큰처럼 잘 이어질 수 있고
- 어떤 표현은 잘게 나뉘어 더 길고 복잡한 생성 경로를 만들 수 있습니다

이 차이는 응답 길이, 반복, 문장 스타일에도 간접적으로 영향을 줄 수 있습니다.

## 왜 어떤 언어와 표기 환경에서는 더 생소하게 느껴질 수 있나

사람이 느끼는 단어 경계와 모델이 계산하는 토큰 경계가 비교적 비슷해 보이는 환경도 있지만, 그렇지 않은 경우도 많습니다. 조사나 어미처럼 단어 뒤에 기능 요소가 붙는 언어, 복합어가 자주 나타나는 표현, 띄어쓰기 변형이 많은 문장, 숫자·기호·영문 표기가 자주 섞이는 입력에서는 토큰 감각이 더 낯설게 느껴질 수 있습니다.

다음처럼 정리할 수 있습니다.

`사람이 느끼는 단어 경계와 모델이 계산하는 토큰 경계가 크게 어긋나는 환경에서는 토큰 감각이 더 직관적이지 않게 느껴질 수 있다.`

이 때문에 어떤 언어권이든, 표기와 형태가 복잡하게 섞인 입력을 자주 다루는 사용자는 토큰 수를 직관적으로 예측하기 더 어려울 수 있습니다.

## 아주 단순하게 그리면

```mermaid
--8<-- "assets/part-06/chapter-01/p6-c01-s02-diagram-01-ko.mmd"
```

이 도식에서 확인해야 할 결과는 같은 문장이라도 토큰 경계가 달라지면 비용 계산, 청크 분할, 응답 길이 제한 같은 서비스 동작 기준도 함께 달라진다는 점입니다.

도식을 바로 읽을 때는 세 칸만 먼저 보면 충분합니다.

| 먼저 볼 것 | 왜 먼저 보는가 |
| --- | --- |
| 입력 표현이 어디서 갈리는가 | 같은 뜻이라도 토큰 수가 달라지는 출발점이기 때문입니다. |
| 그 차이가 비용인지 청크인지 출력 길이인지 | 토큰화 영향이 어느 운영 판단으로 이어지는지 바로 보이기 때문입니다. |
| 마지막 확인 문장이 무엇인지 | 사례를 읽기 전에 무엇을 검증해야 하는지 고정할 수 있기 때문입니다. |

## 사례 및 예시

아래 도식은 이 절의 세 사례를 `문장이 어떻게 보이는가`보다 `토큰 경계가 바뀌면 어떤 실무 판단이 함께 바뀌는가`라는 공통 질문으로 다시 묶은 것입니다.

```mermaid
--8<-- "assets/part-06/chapter-01/p6-c01-s02-diagram-02-ko.mmd"
```

이 도식에서 확인해야 할 점은 토큰화가 단순 분리 작업으로 끝나지 않는다는 것입니다. 같은 입력도 토큰 경계가 달라지면 비용 계산, 청크 설계, 출력 길이 제한처럼 실제 서비스 판단 기준이 함께 달라집니다.

### 사례 1. 짧은 문장인데 비용이 예상보다 큼

운영팀이 알림 문구를 자동 생성하는 도구에 `회의는 내일 10:00 AM, Zoom 링크는 mail@example.com으로 보냈어요` 같은 짧은 문장을 넣는 장면을 생각해 보겠습니다. 사람은 보통 화면에 보이는 글자 수가 적으면 `짧으니 비용도 적겠지`라고 먼저 짐작합니다. 하지만 숫자, 특수문자, 영어 표기, 이메일 주소가 섞이면 실제 토큰 수는 기대보다 더 늘 수 있습니다. 예를 들어 운영팀이 `짧은 문의 100건이면 충분히 예산 안에 들어가겠다`고 계산했는데, 이런 표기가 많은 문의가 몰리면 예상보다 빨리 비용 한도에 닿을 수 있습니다. 여기서 바뀌는 점은 `눈에 보이는 글자 길이`를 먼저 보던 기준에서 `실제 토큰 수와 그에 따른 비용`을 먼저 보는 기준으로 이동한다는 것입니다. 모델은 글자 모양이 아니라 토큰 조각 단위로 계산하므로, 짧아 보이는 문장도 내부 기준에서는 더 비쌀 수 있습니다. 그래서 이 사례에서 확인해야 할 결과는 화면 길이보다 실제 토큰 수와 비용이 더 빨리 커지는가입니다.

이 사례가 중요한 이유는 운영 현장에서는 `짧은 공지`, `짧은 문의` 같은 사람이 보는 길이 감각으로 예산을 먼저 잡기 쉽기 때문입니다. 하지만 모델이 계산하는 기준은 글자 수가 아니라 토큰 조각 수입니다. 숫자 표기, 이메일, URL, 영어 약어처럼 사람이 보기에는 한 덩어리처럼 보이는 표현이 실제로는 여러 조각으로 잘리면 비용 추정이 빠르게 어긋날 수 있습니다. 그래서 토큰화 절에서 바로 비용 이야기가 붙는 이유는, 분절 방식이 곧 과금과 입력 길이 관리로 이어지기 때문입니다.

같은 짧은 공지도 토큰화 기준으로 보면 이렇게 달라질 수 있습니다.

| 입력 장면 | 사람 기준 첫인상 | 토큰화 관점에서 다시 봐야 하는 것 |
| --- | --- | --- |
| `회의는 내일 오전 10시입니다` | 짧고 단순한 알림 | 비교적 단순한 표기라 토큰 수가 안정적인가 |
| `회의는 내일 10:00 AM입니다` | 여전히 짧은 알림 | 숫자, 콜론, 영문 표기가 더 잘게 갈리는가 |
| `Zoom 링크는 mail@example.com으로 보냈어요` | 한 문장 추가일 뿐 | 이메일 주소와 특수문자가 토큰 수를 예상보다 키우는가 |

이 표에서 초심자가 넘어가야 할 오해는 `짧은 문장은 대체로 다 비용이 비슷하다`는 감각입니다. 토큰화 관점은 바로 이런 표기 차이가 실제 입력 비용을 얼마나 흔드는지 먼저 보게 만듭니다.

### 사례 2. 문서 검색 청크 크기 조정

RAG에서 사내 규정 문서를 조각내어 넣는다고 해 봅시다. 사람이 문서를 나눌 때는 보통 `문단 두세 개면 적당하겠지`처럼 문장 수나 눈대중 길이로 먼저 판단합니다. 그런데 너무 길게 자르면 한 조각이 context window를 빨리 소모하고, 너무 짧게 자르면 `원칙` 문단과 `예외` 문단이 서로 다른 조각으로 갈라져 검색 때 따로 떨어질 수 있습니다. 예를 들어 `연차는 3일 전 신청`이라는 원칙과 `긴급 병가는 사후 보고 가능`이라는 예외가 분리되면, 검색은 원칙만 가져오고 중요한 예외를 놓칠 수 있습니다. 이때 실제 판단 기준은 단순 문장 개수보다 `한 조각이 몇 토큰을 차지하는가`, 그리고 중요한 문맥을 함께 묶어 둘 수 있는가에 더 가깝습니다. 여기서 바뀌는 점은 `문단 수나 눈대중 길이`를 먼저 보던 기준에서 `토큰 길이와 문맥 보존`을 먼저 보는 기준으로 이동한다는 것입니다. 그래서 이 사례에서 확인해야 할 결과는 청크를 나눈 뒤 원칙과 예외가 함께 검색되는가, 아니면 따로 떨어져 누락되는가입니다.

이 장면도 실제 설계에서 아주 자주 부딪힙니다. 사람이 보기 좋은 청크와 검색에 유리한 청크가 항상 같지는 않기 때문입니다. 문단 둘로 예쁘게 나뉜다고 해도, 핵심 조건과 예외 조항이 서로 다른 조각에 찢어지면 검색기는 `원칙만 맞는 답`을 더 쉽게 만들 수 있습니다. 반대로 너무 크게 자르면 토큰 예산을 빨리 먹어 버려 한 번에 넣을 수 있는 문서 수가 줄어듭니다. 그래서 청크 크기 조정은 예쁜 문단 나누기가 아니라, `핵심 문맥을 어떤 토큰 묶음으로 보존할 것인가`를 고르는 일에 가깝습니다.

같은 규정 문서도 청크 기준에 따라 운영 판단이 달라집니다.

| 청크 기준 | 사람 눈에는 어떻게 보이나 | 실제로 다시 확인해야 하는 것 |
| --- | --- | --- |
| 문단 수 기준으로 자름 | 보기 좋은 문단 묶음 | 원칙과 예외가 같은 조각 안에 남는가 |
| 토큰 수만 줄이려 너무 짧게 자름 | 짧아서 안전해 보임 | 핵심 조건이 찢어져 검색 근거가 약해지지 않는가 |
| 조건절과 예외까지 함께 묶음 | 약간 길어 보여도 덩어리가 큼 | 토큰 예산 안에서 근거 문맥을 더 잘 보존하는가 |

이 사례에서 초심자가 붙잡아야 할 기준은 `몇 문단인가`보다 `검색기에 필요한 의미 단위가 함께 남는가`입니다. 토큰화가 청크 설계와 바로 연결되는 이유가 바로 여기에 있습니다.

### 사례 3. 응답 길이 제한

같은 질문이라도 답변 형식이 달라지면 토큰 사용량이 크게 달라질 수 있습니다. 사람은 짧은 문단 하나면 될 답에 표, 목록, 코드 블록까지 붙이면 더 친절해졌다고 느끼기 쉽습니다. 하지만 서비스 운영에서는 `친절한 형식`이 곧 더 긴 출력 예산을 뜻합니다. 예를 들어 배송 지연 안내 한 문단이면 끝날 답에 표와 주의사항 목록을 계속 붙이면, 마지막 환불 조건 문장이 잘린 채 응답이 끊길 수 있습니다. 여기서 바뀌는 점은 `보기 좋은 형식인가`를 먼저 보던 기준에서 `중요한 마지막 문장까지 남길 출력 예산이 있는가`를 먼저 보는 기준으로 이동한다는 것입니다. 그래서 운영자는 `몇 줄이 보기 좋은가`보다 `중요한 마지막 문장까지 남길 만큼 최대 몇 토큰을 허용할 것인가`를 먼저 정해야 합니다. 그래서 이 사례에서 확인해야 할 결과는 형식을 늘렸을 때 마지막 핵심 문장이 실제로 잘리지 않고 남는가입니다.

세 사례를 토큰 경계 관점으로 다시 정리하면 다음과 같습니다.

| 상황 | 토큰 경계가 바뀌면 무엇이 달라지는가 | 먼저 조정할 운영 항목 |
| --- | --- | --- |
| 짧은 문장 비용 계산 | 같은 길이처럼 보여도 실제 토큰 수와 비용이 달라짐 | 비용 추정식, 예산 경보 기준 |
| RAG 청크 분할 | 원칙과 예외, 조건과 결과가 다른 청크로 갈라질 수 있음 | 청크 크기, 겹침(overlap), 분할 기준 |
| 응답 길이 제한 | 마지막 핵심 문장이 출력 한도 때문에 잘릴 수 있음 | 최대 출력 토큰, 답변 형식, 요약 우선순위 |

이 세 사례는 아래처럼 더 짧은 운영 장면으로도 다시 묶을 수 있습니다.

| 아주 작은 장면 | 먼저 볼 질문 | 바로 조정할 값 |
| --- | --- | --- |
| 알림 문구 100건을 자동 생성한다 | 짧아 보이는 문장이 실제로도 싼가? | 입력 토큰 수, 건당 비용 |
| 규정 문서를 두 청크로 나눈다 | 원칙과 예외가 같이 남는가? | chunk size, overlap |
| 배송 지연 답변에 표를 붙인다 | 마지막 환불 안내가 잘리지 않는가? | max output tokens |

## 연습 및 예제

이번 예제의 목표는 `같은 뜻의 문장도 토크나이저 규칙이 다르면 토큰 수가 달라질 수 있다`는 점을 눈으로 확인하는 것입니다. 실제 서비스용 토크나이저 전체를 구현하지는 않지만, `긴 조각부터 찾는 longest-match` 방식으로도 토큰화 차이의 핵심은 충분히 볼 수 있습니다. 여기에 간단한 비용 추정과 RAG 청크 길이 비교를 함께 붙여, 토큰화가 실제 서비스 설계로 이어지는 장면까지 봅니다.

입력:

- 비슷한 의미의 세 문장
- 서로 다른 두 개의 토크나이저 사전

여기서 토크나이저 A는 `오전10시`처럼 붙은 표현을 하나의 조각으로 잘 읽는 쪽이고, 토크나이저 B는 `오전`, `10`, `시`처럼 더 잘게 나누는 쪽이라고 가정합니다.

출력:

- 각 토크나이저가 만든 토큰 조각
- 각 토크나이저에서의 토큰 개수
- 설명용 비용 추정
- 같은 문장을 RAG 청크로 볼 때의 길이 변화
- 같은 규정 청크가 토큰 예산 안에 들어오는지 여부

문제 상황:

- 같은 문장도 토크나이저에 따라 토큰 수와 예산 사용량이 달라질 수 있다

입력(input):

위에 정리한 두 토크나이저 어휘 집합, 예시 문장, 규정 청크를 사용합니다.

확인할 개념:

- 같은 문장이라도 토크나이저가 다르면 토큰 수, 청크 길이, 예산 통과 여부가 함께 달라질 수 있다

```python
tokenizer_a_vocab = [
    "회의", "는", "내일", "오전10시", "10", "시", "10:00", "AM", "에", "시작", "합니다", ".",
    "연차", "3일", "전", "신청", "긴급병가", "사후보고", "가", "가능합니다",
]

tokenizer_b_vocab = [
    "회의", "는", "내일", "오전", "10", "시", "10:00", "AM", "에", "시작", "합니다", ".",
    "연", "차", "3", "일", "전", "신", "청", "긴", "급", "병", "가", "사", "후", "보", "고", "능",
]

def longest_match_tokenize(text, vocab):
    pieces = []
    i = 0
    vocab = sorted(vocab, key=len, reverse=True)

    while i < len(text):
        if text[i].isspace():
            i += 1
            continue

        matched = None
        for piece in vocab:
            if text.startswith(piece, i):
                matched = piece
                break

        if matched is None:
            matched = text[i]

        pieces.append(matched)
        i += len(matched)

    return pieces

examples = [
    "회의는 내일 오전10시에 시작합니다.",
    "회의는 내일 오전 10시에 시작합니다.",
    "회의는 내일 10:00 AM에 시작합니다.",
]

rag_sentence = "연차는 3일 전 신청합니다. 긴급병가는 사후보고가 가능합니다."
rag_budget = 13

for text in examples:
    pieces_a = longest_match_tokenize(text, tokenizer_a_vocab)
    pieces_b = longest_match_tokenize(text, tokenizer_b_vocab)

    cost_a = len(pieces_a) * 0.4
    cost_b = len(pieces_b) * 0.4
    print("text =", text)
    print("A pieces =", pieces_a, "count =", len(pieces_a), "cost =", cost_a)
    print("B pieces =", pieces_b, "count =", len(pieces_b), "cost =", cost_b)
    print("---")

rag_a = longest_match_tokenize(rag_sentence, tokenizer_a_vocab)
rag_b = longest_match_tokenize(rag_sentence, tokenizer_b_vocab)
policy_sentence = "연차는 3일 전 신청합니다."
exception_sentence = "긴급병가는 사후보고가 가능합니다."
policy_a = longest_match_tokenize(policy_sentence, tokenizer_a_vocab)
policy_b = longest_match_tokenize(policy_sentence, tokenizer_b_vocab)
exception_a = longest_match_tokenize(exception_sentence, tokenizer_a_vocab)
exception_b = longest_match_tokenize(exception_sentence, tokenizer_b_vocab)

print("[RAG chunk check]")
print("A chunk =", rag_a, "count =", len(rag_a))
print("B chunk =", rag_b, "count =", len(rag_b))
print("A policy+exception fits budget =", len(rag_a) <= rag_budget, "budget =", rag_budget)
print("B policy+exception fits budget =", len(rag_b) <= rag_budget, "budget =", rag_budget)
print("A policy_count =", len(policy_a), "exception_count =", len(exception_a))
print("B policy_count =", len(policy_b), "exception_count =", len(exception_b))
```

실행 결과 예시는 다음처럼 읽을 수 있습니다.

```text
text = 회의는 내일 오전10시에 시작합니다.
A pieces = ['회의', '는', '내일', '오전10시', '에', '시작', '합니다', '.'] count = 8 cost = 3.2
B pieces = ['회의', '는', '내일', '오전', '10', '시', '에', '시작', '합니다', '.'] count = 10 cost = 4.0
---
text = 회의는 내일 오전 10시에 시작합니다.
A pieces = ['회의', '는', '내일', '오', '전', '10', '시', '에', '시작', '합니다', '.'] count = 11 cost = 4.4
B pieces = ['회의', '는', '내일', '오전', '10', '시', '에', '시작', '합니다', '.'] count = 10 cost = 4.0
---
text = 회의는 내일 10:00 AM에 시작합니다.
A pieces = ['회의', '는', '내일', '10:00', 'AM', '에', '시작', '합니다', '.'] count = 9 cost = 3.6
B pieces = ['회의', '는', '내일', '10:00', 'AM', '에', '시작', '합니다', '.'] count = 9 cost = 3.6
---
[RAG chunk check]
A chunk = ['연차', '는', '3일', '전', '신청', '합니다', '.', '긴급병가', '는', '사후보고', '가', '가능합니다', '.'] count = 13
B chunk = ['연', '차', '는', '3', '일', '전', '신', '청', '합니다', '.', '긴', '급', '병', '가', '는', '사', '후', '보', '고', '가', '가', '능', '합니다', '.'] count = 24
A policy+exception fits budget = True budget = 13
B policy+exception fits budget = False budget = 13
A policy_count = 7 exception_count = 6
B policy_count = 10 exception_count = 14
---
```

이 예제에서 확인해야 할 핵심은 다음입니다.

- 같은 뜻의 문장도 `오전10시`처럼 붙여 쓰느냐 `오전 10시`처럼 띄어 쓰느냐에 따라 토큰 수가 달라질 수 있습니다.
- 토크나이저 A는 `오전10시`를 하나의 조각으로 읽지만, 토크나이저 B는 `오전`, `10`, `시`로 더 잘게 쪼갭니다.
- 결국 `어떤 규칙으로 자르느냐`가 비용, context window 사용량, 청크 길이 판단에 바로 연결됩니다.
- 같은 일정 안내 문장도 8토큰과 11토큰으로 갈릴 수 있으므로, 비용과 최대 입력 길이 판단이 실제로 달라집니다.
- RAG에서는 같은 규정 두 문장을 묶더라도 토크나이저 A는 13토큰 안에 담지만, 토크나이저 B는 24토큰으로 늘어나 같은 청크 예산을 넘길 수 있습니다.
- 그래서 토큰화 차이는 `원칙과 예외를 한 청크에 같이 남길 수 있는가` 같은 검색 품질 판단에도 직접 연결됩니다.

## 이 예제를 표기 차이 관점으로 다시 보면

앞의 예제는 실제 서비스용 토크나이저 전체를 비교한 것은 아니지만, 사람이 보기에는 거의 같은 입력도 `어떤 조각을 사전에 넣어 두었는가`, `어디까지를 한 조각으로 볼 것인가`에 따라 길이와 계산 비용이 달라질 수 있다는 점을 가장 작게 보여 줍니다. 여기서 읽어야 할 핵심은 문장의 뜻보다 먼저, 입력 표기 방식과 토크나이저 규칙이 계산 비용과 문맥 사용량에 직접 영향을 줄 수 있다는 점입니다.

자연어 처리의 역사에서는 텍스트를 어떤 단위로 나눌지가 오래된 주제였습니다. 초기에는 단어 사전 기반 접근이 강했고, 이후에는 희귀 단어 처리와 다국어 처리를 위해 subword 단위가 널리 쓰이게 되었습니다.

LLM 시대에는 이 문제가 더 실무적으로 중요해졌습니다.

- 모델 입력 길이가 토큰 기준으로 제한되고
- 비용이 토큰 기준으로 계산되며
- 다음 토큰 예측이 생성 구조의 핵심이 되었기 때문입니다

커리큘럼 관점에서 이 절은 단순 전처리 설명이 아닙니다. 뒤에서 나올 임베딩, context window, prompt, RAG, tool use를 모두 더 정확하게 이해하기 위한 기초 절입니다.

여기서 한 번 멈추고, `언제 토큰 개수만 아는 것으로는 부족하고 토큰화 절차 자체를 먼저 떠올려야 하는가`를 짧게 고정해 두면 뒤 임베딩과 RAG 절로 넘어갈 때 분절 방식의 영향이 더 선명해집니다.

| 먼저 떠올릴 질문 | 토큰화 관점이 먼저 필요한 이유 | 뒤 절에서 이어질 것 |
| --- | --- | --- |
| 왜 같은 뜻의 문장도 토큰 수와 비용이 다르게 나오나 | 어떤 조각으로 자르느냐가 계산 단위 수를 직접 바꾸기 때문 | 토크나이저 계열 비교 |
| 왜 RAG 청크가 어색하게 갈리거나 검색 품질이 흔들리나 | 문단 모양보다 토큰 경계가 문맥 보존을 더 직접 좌우하기 때문 | 임베딩과 검색 입력 단위 |
| 왜 마지막 핵심 문장이 출력 한도에서 잘리나 | 답 형식과 표기 방식에 따라 출력 토큰 사용량이 달라지기 때문 | max output, generation setting 판단 |

## 체크리스트

- 토큰화는 원문 텍스트를 모델 계산 단위로 바꾸는 절차입니다.
- 같은 텍스트도 표기 방식에 따라 토큰 수가 달라질 수 있습니다.
- 토큰화는 비용, 문맥 길이, 검색, 생성 결과 해석에 실제 영향을 줍니다.
- 이 절은 Part 6의 임베딩, 프롬프트, RAG 설명을 위한 기초입니다.

- 토큰화를 `띄어쓰기 분리` 정도가 아니라 `모델이 언어를 어떤 계산 조각으로 볼지 정하는 절차`로 설명할 수 있는가?
- 비용, 문맥 길이, 검색 품질이 모두 같은 토큰화 경계 변화에서 함께 흔들릴 수 있다는 점을 말할 수 있는가?
- 뒤 장을 읽을 때도 먼저 `어떤 조각 단위가 임베딩과 검색의 입력이 되는가`를 떠올릴 준비가 되어 있는가?

- 토큰화가 무엇을 하는 절차인지 설명할 수 있는가?
- 왜 같은 문장도 토큰 수가 달라질 수 있는지 말할 수 있는가?
- 토큰화가 비용과 context window에 왜 중요한지 설명할 수 있는가?
- RAG와 생성 품질에도 영향이 있다는 점을 말할 수 있는가?

## 출처와 참고 자료

- Daniel Jurafsky, James H. Martin, `Speech and Language Processing` draft materials, 확인 날짜: 2026-06-29.
- Rico Sennrich, Barry Haddow, Alexandra Birch, `Neural Machine Translation of Rare Words with Subword Units`, ACL 2016, 확인 날짜: 2026-06-29.
- OpenAI API Docs, 토큰 관련 설명, 확인 날짜: 2026-06-29. [https://platform.openai.com/docs](https://platform.openai.com/docs){: target="_blank" rel="noopener noreferrer" }
