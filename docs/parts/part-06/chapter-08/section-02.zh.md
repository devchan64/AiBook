# P6-9.2 减轻全量微调负担的 LoRA

> Section ID: `P6-9.2`
> Version: `v2026.07.26`

在 P6-9.1 中，我们看到微调(fine-tuning)是把经过预训练的模型进一步调整为更适合特定目的的过程。但这里马上会出现一个现实问题。
...
因此，核心不是 `把模型做小`，而是 `用更少的调整成本适应同一个大的基础模型`。本节会先抓住为什么全量微调会变得过重，以及在保持大基础模型的同时只添加小适应量的想法。LoRA 名称的直觉和 low-rank 规模感，会在 P6-10.4 继续看；adapter·LoRA·QLoRA 的细部约束区分，会在 P6-10.5 继续看。
...
- 避免把前面的 P6-9.1 微调马上理解成 `重新训练整个模型`
- 让我们在 LLM 服务实务中同时思考成本和结构选择
- 为后续 P6-10.1 instruction tuning、P6-10.2 alignment、P6-18.1 服务运营约束章节中阅读 `模型调整成本` 建立基础
...
P6-9.1 中看到的微调方向本身很自然。问题在于 LLM 太大。
...
- P6-9.1 的问题：`是否要让模型更适合我们的目的？`
- P6-9.2 的问题：`能否不重新改变整个模型，也完成这个调整？`
...
输入文件是 [P6-9.2 目的适应组合](/AiBook/assets/part-06/chapter-08/p6-8-2-adaptation-portfolio.csv){ .csv-preview }。一行表示一个团队正在审查的目的适应任务。核心列是 `team`、`task`、`monthly_experiments`、`expected_change`。这里不预测各业务的质量分数，只看在同一个基础模型上必须反复做多个调整实验时，存储和版本管理负担会以什么结构增长。
