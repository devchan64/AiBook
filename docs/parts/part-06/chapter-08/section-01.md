...
이 중간 위치는 이렇게 잡을 수 있습니다. 사전학습은 넓은 언어 기반을 만들고, 파인튜닝은 그 기반을 우리 과업 기준, 문체, 출력 습관에 더 가깝게 조정합니다. 아직 `assistant처럼 보이게 만드는 마지막 조정`까지 가지 않습니다. 먼저 `범용 기반만으로는 부족한 우리 목적의 차이`를 얼마나 안정적으로 반영할 것인가를 읽고, 그다음 같은 조정 축이 `사람의 요청 방식에 맞는 응답`으로 이어집니다.

따라서 여기서 붙잡을 질문은 `우리 과업과 형식에 더 맞게 무엇을 조정할까?`입니다. 지시 튜닝은 그다음에 사람의 요청 방식에 더 잘 반응하도록 좁히고, 정렬은 안전성과 정책 경계를 더 맞춥니다.
...
여기서 중요한 것은 `파인튜닝이 더 강한 방법`이라고 외우는 일이 아니라, 먼저 `무엇을 바꾸고 싶은가`를 `입력 지시`, `모델 반응 습관`, `외부 정보 연결`로 나눠 읽는 일입니다.
