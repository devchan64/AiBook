...
프롬프트 조정은 P6-9.1, P6-9.2에서, 근거 연결은 P6-10.1, P6-10.2와 P6-11.1, P6-11.2에서, 실행 연결은 P6-12.1, P6-12.2와 P6-13.1, P6-13.2에서 다시 구체화합니다. 파인튜닝 축 안의 세부 이름 구분은 바로 다음 P6-8.4에서 다시 보강합니다. 운영 판단의 큰 축은 P6-15.1, P6-15.2와 P6-16.1, P6-16.2에서 다시 회수합니다. 이 절은 후속 장의 세부 기술을 대신 설명하기보다 `무엇을 어디서 먼저 손봐야 하는가`를 구분하는 선택 지도로 읽는 편이 좋습니다. 다시 말해 뒤의 Module 4~6을 `기술 목록`으로 읽지 않고, `형식 문제는 어디서`, `근거 문제는 어디서`, `실행 문제는 어디서`, `운영 문제는 어디서 먼저 다루는가`를 가르는 입구가 바로 여기입니다.

## 왜 정렬 다음에 바로 선택 지도를 읽는가

초심자는 여기서 자주 `지시 튜닝`과 `정렬`까지 읽었으면 이제 모델 품질 문제도 거의 다 설명된 것처럼 느낍니다. 하지만 실제 제품에서는 거기서부터가 시작입니다. 답이 이상할 때마다 `모델이 별로다`라고 뭉뚱그리면, 형식 문제에 RAG를 붙이거나, 최신성 문제를 프롬프트만 길게 써서 버티거나, 계산 문제를 파인튜닝으로 해결하려고 드는 식의 엇나간 대응이 반복되기 쉽습니다.

그래서 이 절은 Chapter 8의 옆길이 아니라, 바로 앞의 `응답 습관 조정`과 `허용 가능한 행동 기준`을 실제 개선 작업으로 번역하는 다리 역할을 합니다.

| 바로 앞 절까지에서 이미 잡은 것 | 이 절에서 새로 잡는 것 | 바로 뒤 장으로 넘기는 질문 |
| --- | --- | --- |
...
여기서 붙잡아야 할 핵심은 `모든 문제를 한 방법으로 고친다`가 아니라 `실패 원인에 따라 형식, 근거, 실행, 지속 적응을 구분해 선택한다`는 기준입니다. 이 기준이 서야 바로 뒤의 장들도 `새 기술 소개`가 아니라 `실패 원인별 보강 경로`로 읽힙니다.
...
그래서 이 절의 닫힘은 `기술 이름보다 실패 원인 분류가 먼저`라는 기준을 실제 선택 습관으로 바꾸는 데 있습니다.
