...
개별 모델의 비용 비교표와 파인튜닝 파이프라인 전체는 여기서 다루지 않고, 각각의 선택 기준만 남깁니다. 프롬프트 조정은 P6-9.1, P6-9.2에서, 근거 연결은 P6-10.1, P6-10.2와 P6-11.1, P6-11.2에서, 실행 연결은 P6-12.1, P6-12.2와 P6-13.1, P6-13.2에서 다시 구체화합니다. 조직별 품질 운영 프로세스의 세부 설계는 현재 본편 범위 밖에 두고, 운영 판단의 큰 축은 P6-15.1, P6-15.2와 P6-16.1, P6-16.2에서 다시 회수합니다. 이 절은 후속 장의 세부 기술을 대신 설명하기보다 `무엇을 어디서 먼저 손봐야 하는가`를 구분하는 선택 지도로 읽는 편이 좋습니다.
...
이 사례에서는 해결 수단을 크게 붙일수록 좋아지는 것이 아닙니다. 최신 문서를 더 붙이거나 계산 도구를 연결해도, 세 줄로 정리해야 한다는 요구 자체가 안정되지 않으면 사용자는 여전히 `말을 안 듣는다`고 느낍니다. 그래서 먼저 확인해야 할 것은 지식 부족이 아니라 `출력 틀`이 제대로 고정되는가입니다.
...
이 장면에서는 `말을 더 잘하게 만드는 일`과 `지금 기준으로 맞게 말하게 만드는 일`을 분리해야 합니다. 문장을 부드럽게 고치거나 요약 형식을 맞춰도, 근거 문서가 지난 분기 기준이면 핵심 실패는 그대로 남습니다. 그래서 여기서는 먼저 `무슨 말을 했는가`보다 `어떤 최신 문서를 보고 말했는가`를 물어야 합니다.
