...
P6-16장까지 오면 LLM, RAG, 도구 사용, 에이전트, 평가, 실패 대응을 각각 따로 설명할 수는 있게 됩니다. 하지만 실제 기능을 만들려면 한 질문이 남습니다.
...
검색과 근거 연결은 P6-10장과 P6-11장에서, 도구 선택과 실행 연결은 P6-12장과 P6-13장에서, 실행 기록과 재현성은 P6-14.2에서, 운영 제약과 실패 대응은 P6-16장에서 이미 따로 읽었습니다. 이 절은 그 조각들을 `요청 하나가 실제로 닫히는 구조`로 다시 묶는 자리입니다.

앞 절이 `문제가 났을 때 어떤 경로로 멈추고 다시 시도할까`를 다뤘다면, 여기서는 질문, 근거, 실행, 평가, 기록이 한 요청 안에서 어떤 순서로 함께 움직이는지 읽습니다. 바로 다음 절에서는 그 구조가 어떤 요청 실행 기록과 회고 포인트로 남아야 하는지로 질문이 이동합니다.
...
즉, 여기서 중요한 전환은 `실패 경로를 고른다`에서 `그 판단까지 포함한 요청 하나의 구조를 묶는다`로 관점이 바뀌는 데 있습니다. 질문, 근거, 실행, 평가를 한 요청 단위로 묶는 기준을 여기서 세우고, 요청 실행 기록의 기본 뜻과 대표 기록 형태는 바로 다음 P6-17.2에서 다시 연결합니다.
...
초심자 기준에서는 여기서 한 번 더 내려와, 같은 질문이 요청 안에서 언제 `답변 초안`, `사람 검토`, `상태 조회 필요`, `근거 부족`으로 갈리는지 보는 편이 더 안전합니다. 요청 흐름 통합의 핵심은 구성요소 이름을 많이 붙이는 데 있지 않고, 평가와 운영 판단이 실제 요청 상태값으로 언제 바뀌는지 보이게 만드는 데 있습니다.
...
작은 생성형 AI 기능 설계를 처음 읽을 때 초심자가 가장 자주 놓치는 것은 `AI 기능이니까 일단 검색이나 도구를 많이 붙여야 한다`고 생각하는 점입니다. 하지만 실제로는 `지금 질문을 닫는 데 먼저 없는 것이 무엇인가`를 가르는 편이 더 중요합니다.
...
이 절에서 먼저 익혀야 하는 기준은 단순합니다. 작은 생성형 AI 기능 설계는 `모델 호출 방식을 고르는 일`보다 `현재 입력`, `최신 근거`, `현재 상태`, `확답 가능한지 여부` 중 무엇이 먼저 비어 있는지를 가르고 그에 맞는 구조와 상태값을 붙이는 일에 가깝습니다.
