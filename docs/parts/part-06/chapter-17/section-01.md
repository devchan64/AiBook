# P6-18.1 비용·지연·사용량으로 다시 거르는 운영 제약
...
## 비용·지연·사용량으로 다시 거르는 운영 제약: 확인할 판단 기준

이 사례에서는 품질 점수만으로 배포 후보를 고르지 않고 비용, 지연 시간, 처리량, 사용량 제한을 함께 보게 하는지 확인한다.
...
- 바로 앞의 P6-17.1, P6-17.2 평가 절에서 `좋은 답인지`를 점검했다면, 이제는 그 답을 실제로 `빠르고 싸고 안정적으로` 제공할 수 있는지 보게 하고
