...
즉, 이 절의 핵심은 `요청 흐름을 설계한다`에서 `그 흐름을 실제 요청 실행 기록으로 남긴다`로 관점이 바뀌는 데 있습니다. 뒤의 프로젝트 문서와 회고는 그 기록을 더 큰 개선 산출물로 키우는 단계입니다.
...
이번 예제는 `질문 -> 검색 -> 답변 초안 -> 평가 -> 기록`을 한 번에 확인하는 데 목적이 있습니다. 이번에는 질문 두 개만 보는 대신, `다중 근거가 잡히는 경우`, `근거가 하나만 잡히는 경우`, `아예 검색 실패가 나는 경우`를 함께 넣어 최소 기능도 여러 실패 유형으로 갈라진다는 점을 확인하겠습니다. 특히 각 질문이 끝난 뒤 요청 실행 기록 하나로 남도록 만들어, 뒤의 회고나 운영 판단에서 무엇을 고쳐야 하는지 바로 다시 읽을 수 있게 하겠습니다.
