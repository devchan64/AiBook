...
대신 이 절에서는 Part 6 전체에서 본 프롬프트, RAG, 도구 사용, 에이전트, 평가, 권한 문제를 운영 관점으로 다시 묶습니다. 조직 운영 체계와 대규모 관측성 스택의 세부 설계는 현재 본편 범위 밖으로 두고, `어디를 추적해야 실패 원인을 구분할 수 있는가`까지를 이 절의 회수 범위로 삼습니다.

이 절에서는 실패 대응을 단순한 오류 메시지 처리로 축소하지 않고, LLM 서비스 특유의 다단계 실패 구조로 설명합니다.

지금 읽는 층위는 `운영 복구 층위`입니다. 앞 절의 자동 평가와 사람 평가는 `무엇이 좋은 답인가`를 가르는 기준을 다뤘다면, 여기서는 `문제가 났을 때 어떤 경로로 멈추고, 다시 시도하고, 사람에게 넘길 것인가`로 질문이 바뀝니다. 아직 조직 운영 체계 전체를 설계하는 단계까지는 가지 않고, 뒤의 P6-17에서는 이 운영 판단을 하나의 작은 기능 흐름과 회고 기록으로 다시 묶습니다.
...
즉, 이 절의 핵심은 `좋은 답 판단`에서 `실패 경로 결정`으로 관점이 바뀌는 데 있습니다. 바로 뒤의 P6-17에서는 그 판단을 요청 흐름과 실행 기록으로 남깁니다.

이 절은 Part 6에서 재시도(retry), 대체 경로(fallback), 중단(stop), 승인(approval)으로 이어지는 실패 대응을 대표로 설명하는 Section입니다. 답변 실패와 시스템 실패를 같은 말로 뭉개지 않고 경로별로 나눠 읽는 기준을 여기서 세웁니다.
...
즉, 운영 관점에서는 `틀린 답`보다 `틀린 답이 만들어진 경로`가 더 중요합니다. 같은 오답처럼 보여도 검색 후보가 비어 있었는지, 후보는 맞았는데 예외 문장을 빠뜨렸는지에 따라 다음 조치는 완전히 달라집니다. 그래서 trace를 남긴다는 말은 로그를 많이 쌓는 것이 아니라, 어느 단계까지는 정상이고 어디서부터 어긋났는지를 다시 말할 수 있게 만드는 일입니다.
...
이 사례가 중요한 이유는 `도구 실패` 자체보다 `실패를 성공처럼 처리하는 거짓 진행`이 더 위험하기 때문입니다. 읽기에 실패했으면 그다음 계획도 실패를 반영해 바뀌어야 합니다. 그래야 이후 패치, 요약, 상태 기록이 모두 같은 전제를 공유하게 됩니다.
...
| 상황 | 먼저 떠올릴 관점 | 왜 중요한가 |
| --- | --- | --- |
| 결과가 틀렸다는 사실만으로는 어디가 잘못됐는지 설명되지 않을 때 | 실패를 모델·검색·도구·권한·성능 층위로 나눠 봐야 한다는 점 | 같은 틀린 답도 검색 누락인지 생성 오류인지 실행 실패인지에 따라 바로 다음 조치가 달라집니다. |
| 오류가 난 뒤에도 계속 진행해 거짓 성공 흐름이 생길 위험이 있을 때 | retry, fallback, stop, approval 경로를 미리 정해야 한다는 점 | 실패를 무시하고 다음 단계로 가면 잘못된 전제 위에 더 큰 오류가 쌓일 수 있습니다. |
| 다음 장의 작은 기능 흐름 통합이 왜 필요한지 감이 오지 않을 때 | 실패 대응 판단도 결국 요청 하나의 실행 기록 안에 남아야 한다는 점 | 이 절의 복구 경로는 추상 정책이 아니라, 실제 요청 흐름에 기록으로 남아야 반복 개선이 가능합니다. |

| 지금 이 절에서 정리한 것 | 바로 다음에 붙는 질문 | 이 본류 단계가 닫는 역할 |
| --- | --- | --- |
| 실패를 분류하고 retry, fallback, human review 같은 복구 경로를 정한다 | 이 운영 판단을 실제 요청 하나의 흐름과 기록으로 어떻게 남길까 | Part 6 후반이 `나쁜 경우를 어떻게 통제할까`를 운영 구조로 닫는 단계 |

- AI 서비스 실패는 모델 오류와 시스템 오류를 함께 포함합니다.
- trace, retry, fallback, approval은 핵심 대응 장치입니다.
- 실패 대응은 출력 뒤 수습이 아니라 실행 구조 안에 들어가야 합니다.
- 이 절은 통합 미니 실습으로 이어지는 운영 관점의 연결 절입니다.

실패 대응은 평가 다음에 `어디서 멈추고 어디로 복구할까`를 고르는 운영 복구 구조입니다.
