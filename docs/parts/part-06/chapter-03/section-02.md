...
| --- | --- | --- |
| 모델이 이전 토큰을 본다는데 왜 긴 문서를 그대로 다 넣지 못할까? | attention이 작동하기 전에 먼저 입력 범위 제한이 있기 때문입니다. | 입력 선택, 요약, 분할 전략 |
| 왜 좋은 모델이어도 서비스에서 답이 흔들리거나 비싸질까? | 문맥 길이는 비용, 지연 시간, 핵심 단서 보존과 직접 연결되기 때문입니다. | RAG, 운영 제약, 문맥 관리 |
| 검색 결과를 많이 넣는 것이 항상 더 좋은가? | 많이 넣는 것보다 예산 안에서 핵심 상태를 남기는 것이 더 중요하기 때문입니다. | retrieval 조각 선택과 재정렬 |
