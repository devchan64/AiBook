...
Part 6에서 `attention과 context window의 관계`, `입력 범위 제한`, `길이 제한이 서비스 구조 문제로 바뀌는 이유`에 대한 첫 상세 설명은 이 절에서 잡습니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 문맥 범위 제약의 기본 감각은 이 절과 개념사전을 기준으로 다시 연결합니다.
...
따라서 여기서 먼저 붙잡아야 할 것은 `attention이 모든 것을 본다`가 아니라 `입력 범위가 먼저 제한되고 attention은 그 안에서만 작동한다`는 점입니다.
...
이 항목들은 같은 장의 P6-3.4, P6-3.5 보충학습에서 `왜 반복 생성과 긴 문맥이 따로 어렵고 어떤 보강 장치가 붙는가`라는 수준으로 다시 설명합니다. 긴 문맥이 실제 서비스 구조와 RAG 설계에 미치는 영향은 P6-10.1, P6-10.2, P6-16.1에서도 다시 이어집니다.
...
커리큘럼 관점에서 이 절에서 확인해야 할 결과는 바로 앞의 P6-3.1 Transformer 구조를 실제 사용 제약과 연결해, 이후 RAG, prompt 설계, tool use, agent loop에서 왜 입력 선택이 중요해지는지 설명할 수 있게 되는가입니다. 이 절이 필요한 이유는 다음과 같습니다.
...
이 절에서 초심자가 자주 하는 실수는, `긴 문맥이 필요하다`는 말을 들으면 곧바로 더 많이 넣는 쪽으로만 생각하는 일입니다. 하지만 실제 서비스 장면에서는 먼저 `지금 실패가 윈도우 안에 무엇이 안 남아서 생긴 것인가`, `아니면 이미 남아 있는 범위 안에서 무엇이 더 중요했는가`를 가르는 편이 안전합니다.
...
이 표의 목적은 context window와 attention을 다시 정의하는 데 있지 않습니다. 초심자가 실제 실패 장면을 봤을 때 `먼저 윈도우 안에 무엇이 남았는가`를 볼지, `남은 범위 안에서 무엇이 더 중요해졌는가`를 볼지 분기하게 만드는 데 있습니다.
...
이 사례가 중요한 이유는 많은 초심자가 context window를 `크면 좋은 것` 정도로만 받아들이기 때문입니다. 하지만 실제 판단은 크기 자체보다 `무엇이 남고 무엇이 잘리는가`에 더 가깝습니다. 긴 보고서를 통째로 넣는다고 해서 자동으로 더 정확해지는 것이 아니라, 오히려 부록과 주변 설명이 핵심 결론을 밀어낼 수 있습니다. 그래서 context window를 이해할 때는 `얼마나 담을 수 있나`보다 `그 제한 안에서 어떤 절을 먼저 남길 것인가`를 더 먼저 봐야 합니다.

같은 긴 문서도 입력 선택 방식에 따라 결과가 달라집니다.

| 입력 방식 | 초심자가 먼저 기대하기 쉬운 것 | 실제로 다시 봐야 하는 것 |
| --- | --- | --- |
...
이 표에서 초심자가 넘어가야 할 오해는 `더 많이 넣으면 핵심도 같이 남겠지`라는 기대입니다. context window 사례의 핵심은 많이 넣기보다 핵심을 남기는 선택이 실제로 더 중요하다는 점입니다.

### 사례 2. 코드 도우미

큰 코드베이스에서 버그를 고칠 때 사용자는 `저장소 전체를 보고 원인을 찾아 달라`고 기대할 수 있습니다. 처음에는 전체를 다 보여 주면 더 잘 고칠 것 같다고 느끼기 쉽습니다. 하지만 실제로는 모든 파일을 한 번에 넣기 어렵기 때문에, 현재 파일, 관련 함수, 최근 에러 로그, 실패한 테스트 결과를 우선 선택해야 합니다. 예를 들어 로그인 오류를 고치는데 디자인 자산 파일과 오래된 문서까지 함께 넣는다면, 정작 인증 미들웨어와 세션 설정 파일이 잘리고 핵심 원인 후보를 놓칠 수 있습니다. 즉, 문맥 관리란 단순 길이 제한이 아니라 `무엇을 먼저 보여 줄지`를 정하는 선택 문제이기도 합니다. 여기서 바뀌는 점은 `전체를 더 많이 보여 주는가`를 보던 기준에서 `현재 오류와 직접 연결된 문맥이 우선 남는가`를 보는 기준으로 이동한다는 것입니다. 그래서 이 사례에서 확인해야 할 결과는 정보량을 늘리는 것보다 현재 질문과 직접 연결된 파일을 남긴 쪽이 실제 원인 후보를 더 잘 보존하는가입니다.

이 장면도 실무 코드 도우미에서 바로 체감됩니다. 초심자는 저장소를 더 넓게 보여 주면 더 똑똑하게 고칠 것처럼 느끼기 쉽지만, 실제로는 관련 없는 정보가 많아질수록 핵심 파일과 에러 로그가 밀릴 수 있습니다. context window는 `전부 다 보여 줄 수 없다`는 제약일 뿐 아니라, `지금 문제와 직접 연결된 문맥을 선별해야 한다`는 설계 기준이기도 합니다. 그래서 코드 도우미에서 중요한 것은 저장소를 얼마나 넓게 열었는가보다, 현재 버그와 가장 직접 연결된 파일과 로그가 살아남았는가입니다.
...
이 사례에서 초심자가 꼭 붙잡아야 할 기준은 `전체를 보여 주는가`가 아니라 `현재 오류를 설명하는 단서가 오래 남는가`입니다. context window 관점은 바로 이 선별 기준을 세우기 위해 필요합니다.
...
이 세 사례를 현재 실무 판단 메모로 다시 줄이면 다음처럼 읽을 수 있습니다.

| 장면 | 많이 넣는 쪽으로만 생각하면 생기기 쉬운 오해 | 먼저 붙잡아야 할 판단 기준 |
| --- | --- | --- |
| 긴 문서 요약 | 페이지 수를 더 넣으면 자동으로 더 정확해진다 | 결론과 핵심 절이 실제로 윈도우 안에 남는가 |
| 코드 도우미 | 저장소 범위를 넓히면 원인 후보도 자동으로 더 잘 보인다 | 현재 오류와 직접 연결된 파일·로그가 먼저 보존되는가 |
| 대화형 챗봇 | 대화는 가능한 한 전부 남겨 두는 것이 항상 안전하다 | 반복 대화보다 주문번호·예외 조건 같은 상태가 오래 유지되는가 |
...
- 왜 문맥 길이가 비용과 서비스 설계에 영향을 주는지 설명할 수 있어야 합니다.
- 왜 이 절이 RAG와 연결되는지 말할 수 있어야 합니다.
