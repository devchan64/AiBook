...
이 절에서 확인해야 할 결과는 바로 앞의 P6-3.1 Transformer 구조를 실제 사용 제약과 연결해, 이후 RAG, prompt 설계, tool use, agent loop에서 왜 입력 선택이 중요해지는지 설명할 수 있게 되는가입니다. 이 절이 필요한 이유는 다음과 같습니다.
...
이 절에서 자주 하는 실수는, `긴 문맥이 필요하다`는 말을 들으면 곧바로 더 많이 넣는 쪽으로만 생각하는 일입니다. 하지만 실제 서비스 장면에서는 먼저 `지금 실패가 윈도우 안에 무엇이 안 남아서 생긴 것인가`, `아니면 이미 남아 있는 범위 안에서 무엇이 더 중요했는가`를 가르는 편이 안전합니다.
...
이 사례가 중요한 이유는 context window를 `크면 좋은 것` 정도로만 받아들이기 쉽기 때문입니다. 하지만 실제 판단은 크기 자체보다 `무엇이 남고 무엇이 잘리는가`에 더 가깝습니다. 긴 보고서를 통째로 넣는다고 해서 자동으로 더 정확해지는 것이 아니라, 오히려 부록과 주변 설명이 핵심 결론을 밀어낼 수 있습니다. 그래서 context window를 이해할 때는 `얼마나 담을 수 있나`보다 `그 제한 안에서 어떤 절을 먼저 남길 것인가`를 더 먼저 봐야 합니다.
...
이 표가 바로잡는 오해는 `더 많이 넣으면 핵심도 같이 남겠지`라는 기대입니다. context window 사례의 핵심은 많이 넣기보다 핵심을 남기는 선택이 실제로 더 중요하다는 점입니다.
...
이 장면도 실무 코드 도우미에서 바로 체감됩니다. 저장소를 더 넓게 보여 주면 더 똑똑하게 고칠 것처럼 느끼기 쉽지만, 실제로는 관련 없는 정보가 많아질수록 핵심 파일과 에러 로그가 밀릴 수 있습니다. context window는 `전부 다 보여 줄 수 없다`는 제약일 뿐 아니라, `지금 문제와 직접 연결된 문맥을 선별해야 한다`는 설계 기준이기도 합니다. 그래서 코드 도우미에서 중요한 것은 저장소를 얼마나 넓게 열었는가보다, 현재 버그와 가장 직접 연결된 파일과 로그가 살아남았는가입니다.
...
이 사례에서 중요한 기준은 `전체를 보여 주는가`가 아니라 `현재 오류를 설명하는 단서가 오래 남는가`입니다. context window 관점은 바로 이 선별 기준을 세우기 위해 필요합니다.
