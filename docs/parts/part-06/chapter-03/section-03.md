...
P6-3.1에서는 Transformer 블록의 큰 구조를 보았고, P6-3.2에서는 attention이 context window 안에서만 작동한다는 점을 보았습니다. 그런데 여기서 초심자가 자주 다시 막히는 이름이 두 개 있습니다.
...
이 절에서는 `모델이 문맥을 읽을 때 무엇을 보강하는가`를 먼저 닫습니다.

| 지금 이 보충학습에서 읽는 것 | 바로 다음 보충학습이나 뒤 장으로 넘기는 것 |
| --- | --- |
...
- multi-head attention을 `관계를 여러 시선으로 나누어 읽는 장치`로 설명할 수 있습니다.
- 위치 표현을 `순서를 따로 알려 주는 장치`로 설명할 수 있습니다.
- 두 이름이 모두 Transformer 안에서 등장하지만 같은 역할을 하는 것은 아니라는 점을 말할 수 있습니다.
- 다음 P6-3.4의 KV cache와 P6-3.5의 sparse attention, long-context를 읽을 준비를 할 수 있습니다.

## 왜 이 둘을 먼저 같이 보나

처음 읽으면 `multi-head attention`과 `위치 표현`이 모두 Transformer의 내부 부품 이름처럼 보입니다. 하지만 초심자 기준에서는 둘을 다음처럼 구분하는 편이 가장 안전합니다.
...
이 단계에서 초심자가 먼저 남기면 좋은 기준은 다음입니다.
...
이 차이는 `둘 다 중요하다`는 말만으로는 잘 남지 않을 수 있습니다. 초심자에게는 `하나가 빠지면 어떤 종류의 오해가 남는가`까지 같이 보는 편이 더 직접적입니다.
...
이 표의 목적은 두 용어를 더 빨리 외우게 하는 데 있지 않습니다. 초심자가 실제 문장을 읽을 때 `지금 모델이 여러 관계를 갈라 읽지 못하는 문제인가`, `아니면 순서를 따로 모르기 쉬운 문제인가`를 먼저 구분하게 만드는 데 있습니다.

## 사례 및 예시

### 사례 1. 여러 관계가 한 문장에 겹치는 경우

긴 문장을 읽을 때 사람은 주어-행동 관계, 대상 변화, 전달 대상, 시간 순서를 한꺼번에 봅니다. 하지만 이를 한 종류의 관련도만으로 읽으려 하면, 무엇이 누구에게 연결되는지와 어떤 순서로 일어나는지가 쉽게 섞입니다. 여기서 multi-head attention을 `여러 시선`으로 읽는 기준이 필요해집니다. 이 사례에서 확인해야 할 결과는 한 문장 안에서도 서로 다른 관계 축을 나눠 읽어야 할 필요가 실제로 보이는가입니다.

### 사례 2. 단어는 비슷한데 순서가 달라지는 경우

`고양이가 개를 쫓는다`와 `개가 고양이를 쫓는다`는 등장 단어가 비슷하지만 뜻은 다릅니다. 사람은 단어 자체보다 순서를 함께 읽기 때문에 둘을 구분합니다. 위치 표현이 필요한 이유도 여기에 있습니다. 이 사례에서 확인해야 할 결과는 같은 단어 집합이라도 순서 정보가 빠지면 의미가 흔들린다는 점이 분명히 보이는가입니다.

### 사례 3. 둘을 섞어 오해하기 쉬운 경우

초심자는 종종 `head가 여러 개면 순서도 자동으로 잘 읽겠지`라고 느끼기 쉽습니다. 하지만 여러 시선과 순서 정보는 같은 문제가 아닙니다. 예를 들어 문장 안의 관계를 여러 각도에서 봐도, 그 관계가 어느 순서에서 발생했는지를 따로 알지 못하면 해석은 여전히 흔들릴 수 있습니다. 이 사례에서 확인해야 할 결과는 `관계 읽기`와 `순서 읽기`를 별도의 보강 문제로 구분할 수 있는가입니다.

## 연습 및 예제

아래 표를 보고 각 질문이 multi-head attention, 위치 표현, 또는 둘 다와 더 직접 연결되는지 먼저 판단해 보십시오.

| 질문 | 먼저 더 직접 연결되는 것 | 이유를 적어 볼 핵심 |
| --- | --- | --- |
| `누가 무엇을 누구에게 했는가`가 한 문장 안에서 여러 겹으로 얽힐 때 | multi-head attention | 관계 축이 여러 개 겹치기 때문 |
| 같은 단어가 비슷하게 들어가지만 문장 뜻이 순서 때문에 달라질 때 | 위치 표현 | 앞뒤 순서를 알아야 하기 때문 |
| 긴 문장에서 주체 관계와 행동 순서를 함께 놓치지 않아야 할 때 | 둘 다 | 관계와 순서가 동시에 중요하기 때문 |

이 표의 목적은 정답 암기가 아니라, 두 이름을 같은 부품 이름처럼 넘기지 않고 `어떤 질문에 답하는 장치인가`로 다시 읽는 데 있습니다.
