...
종종 `head가 여러 개면 순서도 자동으로 잘 읽겠지`라고 느끼기 쉽습니다. 하지만 여러 시선과 순서 정보는 같은 문제가 아닙니다. 예를 들어 문장 안의 관계를 여러 각도에서 봐도, 그 관계가 어느 순서에서 발생했는지를 따로 알지 못하면 해석은 여전히 흔들릴 수 있습니다. 이 사례에서 확인해야 할 결과는 `관계 읽기`와 `순서 읽기`를 별도의 보강 문제로 구분할 수 있는가입니다.
