...
긴 문장을 읽을 때 사람은 주어-행동 관계, 대상 변화, 전달 대상, 시간 순서를 한꺼번에 봅니다. 하지만 이를 한 종류의 관련도만으로 읽으려 하면, 무엇이 누구에게 연결되는지와 어떤 순서로 일어나는지가 쉽게 섞입니다.
...
이 사례가 이 절을 지지하는 이유는 multi-head attention을 `부품 이름`이 아니라 `서로 다른 관계 축을 여러 시선으로 나누어 읽게 하는 보강`으로 읽게 해 주기 때문입니다. 여기서 확인해야 할 결과는 한 문장 안에서도 서로 다른 관계 축을 나눠 읽어야 할 필요가 실제로 보이는가입니다.

### 사례 2. 단어는 비슷한데 순서가 달라지는 경우

`고양이가 개를 쫓는다`와 `개가 고양이를 쫓는다`는 등장 단어가 비슷하지만 뜻은 다릅니다. 사람은 단어 자체보다 순서를 함께 읽기 때문에 둘을 구분합니다.

이 사례가 이 절을 지지하는 이유는 위치 표현이 attention과 별도로 필요한 이유를 가장 짧게 보여 주기 때문입니다. 같은 토큰들이 있어도 `어떤 토큰이 앞에 있었는가`를 따로 알지 못하면, 주체와 대상이 뒤집힌 문장을 안정적으로 구분하기 어렵습니다.

여기서 확인해야 할 결과는 같은 단어 집합이라도 순서 정보가 빠지면 의미가 흔들린다는 점이 분명히 보이는가입니다.

### 사례 3. 둘을 섞어 오해하기 쉬운 경우

종종 `head가 여러 개면 순서도 자동으로 잘 읽겠지`라고 느끼기 쉽습니다. 하지만 여러 시선과 순서 정보는 같은 문제가 아닙니다. 예를 들어 문장 안의 관계를 여러 각도에서 봐도, 그 관계가 어느 순서에서 발생했는지를 따로 알지 못하면 해석은 여전히 흔들릴 수 있습니다.

반대로 순서 정보가 있어도 모든 관계를 한 기준으로만 보면, 주체 관계와 전달 대상 관계가 섞일 수 있습니다. 그래서 이 절에서는 `multi-head attention은 관계 축을 나누는 쪽`, `위치 표현은 순서 정보를 공급하는 쪽`으로 분리해 읽어야 합니다.

이 사례가 이 절을 지지하는 이유는 두 이름이 같은 Transformer 내부 부품처럼 보이더라도, 실제로는 서로 다른 빈칸을 메운다는 점을 보여 주기 때문입니다. 여기서 확인해야 할 결과는 `관계 읽기`와 `순서 읽기`를 별도의 보강 문제로 구분할 수 있는가입니다.
