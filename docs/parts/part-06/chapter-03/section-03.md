...
| --- | --- | --- |
| 표현 학습 | 무엇을 가깝게 배치하도록 배웠는가 | P6-3.3, P6-7.1, P6-8.1 |
| 빠른 후보 탐색 | 그 표현 공간에서 어떻게 더 빨리 좁힐 것인가 | P6-3.4, P6-13.1, P6-13.2 |
...
## 보충학습: 가까운 표현과 먼 표현을 배우는 임베딩 학습: 확인할 판단 기준

이 사례에서는 임베딩 학습을 가까운 표현과 먼 표현을 조정하는 문제로 보충하는지 확인한다.
...
이 사례에서 닫을 판단은 `같은 단어`가 아니라 `같은 해결 흐름`입니다. 환불 문의끼리는 가까워져야 하고, 주소 변경 문의는 멀어져야 하므로 검색 속도보다 먼저 표현 배치 품질을 봐야 합니다.
