...
이 절에서는 `긴 문맥 계산과 유지 문제`를 먼저 닫습니다. 다만 닫는 방식은 `긴 입력 문제`를 한 단어로 뭉개지 않고, 계산 부담과 단서 유지라는 두 질문을 갈라 놓는 데 있습니다.
...
이 사례를 더 풀면 이렇습니다. 로그를 읽을 때 사람도 보통 모든 줄을 모든 줄과 다 비교하지는 않습니다. 대신 다음처럼 먼저 좁혀 갑니다.

- 같은 시각대의 연속 로그를 먼저 묶어 봅니다.
- 같은 세션 ID나 같은 에러 코드를 다시 찾습니다.
- 상태가 바뀌는 지점만 멀리 떨어진 위치까지 다시 확인합니다.

즉, 핵심은 `아무것도 안 보는가`가 아니라 `무엇을 촘촘히 보고 무엇은 드물게 다시 볼 것인가`입니다. sparse attention도 이런 감각과 닿아 있습니다. 여기서 붙잡아야 할 기준은 `긴 입력을 잘 읽으려면 무조건 전부 전부 비교해야 한다`가 아니라, `필요한 연결을 남기면서 덜 중요한 비교를 줄일 수 있는가`입니다.
...
그래서 long-context 사례에서 먼저 붙잡아야 할 기준은 `몇 페이지를 넣었는가`보다 `앞 단서가 뒤 판단까지 실제로 이어지는가`입니다. 여기서 long-context는 `긴 입력 수용`과 `핵심 단서 유지`를 함께 보는 이름으로 읽어야 합니다.

### 사례 3. 길이는 길어졌는데 핵심 단서는 약해지는 경우

context window 숫자가 커지면 자동으로 long-context 문제도 해결된다고 느끼기 쉽습니다. 하지만 실제로는 입력 길이가 길어져도 핵심 단서가 뒤 작업까지 제대로 유지되지 않으면 장문맥 설계 문제는 여전히 남아 있습니다. 여기서 확인해야 할 결과는 `길이 숫자 증가`와 `핵심 단서 유지`를 같은 말로 보지 않는가입니다.
...
즉, 여기서 가장 크게 경계할 오해는 `길이 숫자만 커지면 장문맥 문제는 끝난다`는 생각입니다. long-context에서는 `많이 담았다`보다 `중요한 것이 남았다`가 더 중요한 확인 기준입니다.

## 실패 장면으로 다시 보면

지금까지의 구분을 더 단단하게 만들려면, 각 이름이 막으려는 실패 장면을 같이 보는 편이 좋습니다.
