...
```mermaid
--8<-- "assets/part-06/chapter-03/p6-c03-s05-diagram-01-ko.mmd"
```
...
그래서 long-context 사례에서 먼저 붙잡아야 할 기준은 `몇 페이지를 넣었는가`보다 `앞 단서가 뒤 판단까지 실제로 이어지는가`입니다. 확인해야 할 결과는 `문서 길이가 길어져도 초반 정의와 뒤 예외가 실제로 함께 살아남는가`입니다.
