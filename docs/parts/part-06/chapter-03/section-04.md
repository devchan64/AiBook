...
이 표를 다시 읽으면 다음 오해를 걷어 내는 용도에 가깝습니다.

- `KV cache를 켜면 답의 뜻이 바뀌는가?`
- `KV cache를 켜면 갑자기 더 영리한 추론을 하는가?`
- `KV cache를 끄면 답이 틀리고 켜면 답이 맞는가?`

이 질문들에 먼저 남길 답은 대체로 `그것이 핵심은 아니다`입니다. KV cache가 먼저 바꾸는 것은 `같은 답을 내기까지 드는 반복 계산의 양`입니다. `이름이 어려우니 모델 내부 의미 계산 자체를 바꾸는 기술이겠지`라고 느끼기 쉬운데, 여기서 더 안전한 기준은 `답의 내용`과 `답을 만들기까지의 반복 작업량`을 분리해 보는 것입니다.
