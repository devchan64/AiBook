# P6-6.2 긴 답변보다 판단값을 먼저 내는 이해 중심 태스크
...
이 비교 기준은 검색 파이프라인 안에서 구조가 어떻게 갈라지는지를 볼 때 P6-13.1 벡터 데이터베이스와 P6-13.2 인덱스와 검색 품질에서 다시 회수할 수 있습니다.
...
이 한 줄이 잡히면 세부 태스크 이름을 모두 외우지 않아도 P6-5.1, P6-7.1의 GPT 및 다음 토큰 예측 설명과 P6-12.1, P6-12.2의 RAG 설명을 읽는 데 큰 문제는 없습니다.

## 긴 답변보다 판단값을 먼저 내는 이해 중심 태스크: 확인할 판단 기준

이 사례에서는 긴 답변 생성보다 분류, 문장쌍 판단, 검색과 랭킹, 임베딩 재사용 같은 판단값 중심 태스크를 먼저 내는 구조로 설명하는지 확인한다.
