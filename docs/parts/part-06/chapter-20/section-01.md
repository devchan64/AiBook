...
문서 분류를 처음 보면 `라벨에 가까운 단어가 들어 있느냐`부터 세기 쉽습니다. 하지만 실제 문의 운영에서는 표면 단어보다 `어느 처리 흐름으로 보내야 하는가`가 더 중요합니다. `환불` 단어가 없어도 환불 상태 문의일 수 있고, `취소` 단어가 보여도 실제 병목은 계정 인증일 수 있습니다. 이 장면은 BERT 계열이 왜 `문장 전체가 가리키는 처리 의도`를 더 안정적으로 붙잡는 읽기 중심 구조로 설명되는지 보여 줍니다. 그래서 이 사례에서 확인해야 할 결과는 단어 표면이 달라도 같은 문의가 같은 처리 큐로 더 안정적으로 모이는가입니다.
