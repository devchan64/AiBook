...
이 Section에서는 첫 번째와 두 번째 질문에 집중합니다. 루프가 실제로 어떻게 계획, 행동, 관찰로 움직이는지는 P6-14.2에서 더 자세히 보고, 연결 형식과 실행 기록은 P6-15의 MCP와 하네스에서 다시 다룹니다.

| 현재 관점 | 중심 질문 | 이어지는 문제 |
| --- | --- | --- |
| function calling | 한 번의 실행 요청을 어떤 검증 가능한 구조로 넘길 것인가? | 목표 흐름과 상태 관리 |
| agent | 여러 호출과 읽기를 어떤 순서로 이어 갈 것인가? | 계획, 행동, 관찰 루프 |
| MCP / harness | 그 흐름을 어떤 연결 규칙과 기록 환경 안에 둘 것인가? | 평가와 운영 판단 |
