...
먼저 닫을 문제는 에이전트의 최소 실행 구조를 `목표를 여러 단계로 이어 가는 흐름`으로 읽고, 단일 호출과 무엇이 달라지는지 붙잡는 것입니다.

이 흐름이 실제로 어떤 반복 구조로 움직이는지는 계획, 행동, 관찰의 문제입니다. 도구 연결 표준과 실행 환경은 루프가 어떤 연결 규칙과 기록 환경 안에서 돌아가는지의 문제로 따로 남습니다.
...
agent 용어를 넓게 잡기보다 `무엇을 먼저 읽고 무엇을 나중에 실행할지 순서를 정해야 하는가`, `중간 결과를 보고 다음 행동이 바뀌는가`, `그 흐름을 연결 규칙과 하네스 층위에서 어떻게 관리하게 되는가`라는 세 질문으로 먼저 읽으면 됩니다.

후반 실행 구조를 같은 틀로 다시 보면, 선택 기준은 아래처럼 정리할 수 있습니다.

| 부족한 것 | 붙이는 구조 | 아직 남는 문제 |
| --- | --- | --- |
| 한 번의 호출만으로는 목표를 끝낼 수 없고, 중간 결과를 보고 다음 행동을 바꿔야 한다 | 여러 읽기와 실행을 이어 붙이는 목표 흐름 구조 | 그 흐름이 실제로 어떤 반복 루프로 움직이는지, 어떤 연결 형식과 실행 기록 안에 둘지는 아직 남아 있다 |

| 현재 관점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| function calling | 한 번의 실행 요청을 어떤 검증 가능한 구조로 넘길 것인가? | 목표 흐름과 상태 관리 |
| agent 목표 흐름 | 여러 호출과 읽기를 어떤 순서로 이어 갈 것인가? | 계획, 행동, 관찰 루프 |
| MCP / harness 관리 | 그 흐름을 어떤 연결 규칙과 기록 환경 안에서 관리할 것인가? | 평가와 운영 판단 |

여기서는 `에이전트(agent)`를 `더 똑똑한 챗봇`이라는 인상보다 `여러 읽기와 실행을 목표 흐름으로 이어 가는 작업 구조`로 읽는 기준을 잡습니다.

붙잡아야 할 핵심은 `한 번의 실행 요청`에서 `여러 읽기와 실행의 순서를 어떻게 바꿀 것인가`로 관점이 커진다는 점입니다. 이 흐름은 계획-행동-관찰 루프로 구체화되고, 연결 규칙과 실행 관리 문제로 이어집니다.

이 단계에서 먼저 남겨야 할 기록은 어떤 목표를 어떤 순서로 풀려고 했는지를 보여 주는 단계 계획과 중간 관찰 메모, 그리고 왜 멈췄고 왜 사람 검토나 재시도로 넘겼는지를 보여 주는 다음 단계와 종료 이유입니다. 이 기록이 있어야 다음 행동 변경 이유와 흐름 단위 실패를 다시 볼 수 있고, 뒤로 갈수록 P6-14.2의 계획-행동-관찰 루프, P6-17.2의 실패 대응, 회고 문서로 다시 읽힙니다.
...
먼저 붙잡아야 할 점은 `설명을 한 번 돌려주는 일`과 `여러 단계 작업을 끝까지 이어 가는 일`이 같은 문제가 아니라는 것입니다. 그래서 agent가 필요한 장면은 보통 다음처럼 `중간 결과를 보고 다음 행동을 다시 골라야 하는가`로 먼저 드러납니다.
