...
MCP의 연결 관점은 여기서 잡고, 실행을 감싸는 운영 장치의 역할은 바로 다음 P6-14.2 하네스에서 다시 회수합니다. 인증과 권한이 실제 실패 대응과 만나는 지점은 P6-16.2 운영 중 실패 대응에서 다시 이어지며, 프로토콜 포맷 세부는 현재 본편 범위 밖으로 둡니다.
...
이 절은 Part 6에서 `모델 컨텍스트 프로토콜(Model Context Protocol, MCP)`을 대표로 설명하는 Section입니다. `모델 능력`과 `도구 연결 형식`을 섞지 않도록, 공통 연결 인터페이스라는 기준선을 여기서 세웁니다.
...
즉, 초심자 기준에서는 `둘 다 문서 관련 도구`라는 공통점보다 `무엇을 알고 시작하느냐`가 더 중요한 구분입니다. 이미 경로를 알고 있으면 읽기 자원이고, 아직 위치를 모르면 검색 자원입니다. MCP 관점은 이 차이를 도구마다 제각각 외우지 않고, 같은 연결 형식 안에서 읽기 가능 자원과 검색 가능 자원으로 나눠 다루게 해 줍니다.
...
이 사례에서 초심자가 특히 놓치기 쉬운 점은 `도구가 많다`와 `도구가 이어진다`가 다른 문제라는 것입니다. 검색기, 읽기 도구, 테스트 실행기가 각각 잘 동작해도, 한 도구의 결과를 다음 도구가 바로 이해하지 못하면 전체 작업은 자주 멈춥니다. 그래서 MCP를 읽을 때는 개별 도구 성능보다 `다음 단계로 넘길 형식이 공통으로 맞춰지는가`를 먼저 보는 편이 안전합니다.
