...
Part 6에서 `모델 컨텍스트 프로토콜(Model Context Protocol, MCP)`, `도구와 데이터 연결의 공통 인터페이스 관점`, `모델 자체와 연결 계층의 구분`에 대한 첫 상세 설명은 이 절에서 잡습니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 표준 연결 계층의 기본 뜻은 이 절과 개념사전을 기준으로 다시 연결합니다.
...
MCP의 연결 관점은 여기서 잡고, 실행을 감싸는 운영 장치의 역할은 바로 다음 P6-14.2 하네스에서 다시 회수합니다. 인증과 권한이 실제 실패 대응과 만나는 지점은 P6-16.2 운영 중 실패 대응에서 다시 이어집니다. 즉, 여기서는 `도구와 자원을 어떤 공통 형식으로 연결할 것인가`를 먼저 닫습니다.

여기서는 MCP를 `도구 연결을 덜 제각각으로 만들려는 표준화 관점`으로 읽습니다.

앞 절의 에이전트 루프가 `여러 읽기와 실행을 어떤 반복 구조로 이어 갈까`를 다뤘다면, 여기서는 그 루프가 쓰는 도구와 자원을 어떤 공통 형식으로 드러내야 다음 실행과 기록이 덜 흔들리는지 읽습니다. 바로 다음의 하네스 절에서는 이 연결을 쓴 실행을 어떤 기록과 재현 환경 안에 남길지로 질문이 다시 이동합니다.

후반 실행 구조를 같은 질문으로 다시 고정하면, 지금 장의 역할은 아래처럼 압축할 수 있습니다.

| 지금 부족한 것 | 이번 장이 붙이는 구조 | 아직 남는 문제 | 바로 이어지는 위치 |
| --- | --- | --- | --- |
| 여러 도구와 자원이 제각각 연결되면, 같은 목표 흐름도 입력 형식과 호출 규칙 때문에 자주 흔들린다 | 도구와 자원을 공통 형식으로 드러내는 연결 인터페이스 관점 | 실제 실행을 어떤 기록과 재현 환경 안에 둘지, 실패를 어디서 멈추고 복구할지는 아직 남아 있다 | P6-14.2, P6-16.2 |

| 지금 단계의 관점 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| agent loop | 여러 실행을 어떤 루프로 이어 갈 것인가? | P6-13 |
| MCP 연결 인터페이스 | 그 도구와 자원을 어떤 공통 연결 형식으로 드러낼 것인가? | P6-14.1 |
| harness 실행 관리 | 그 연결을 쓴 실행을 어떤 기록과 재현 환경 안에 남길 것인가? | P6-14.2 |

여기서는 `모델 컨텍스트 프로토콜(Model Context Protocol, MCP)`을 `모델 능력`과 `도구 연결 형식`을 섞지 않게 하는 공통 연결 인터페이스로 읽는 기준을 잡습니다.

즉, 지금 장의 핵심은 `어떤 순서로 실행할까`에서 `그 실행이 쓰는 도구와 자원을 어떤 공통 형식으로 노출할까`로 관점이 바뀌는 데 있습니다. MCP는 `연결을 덜 제각각으로 만드는 층`, 하네스는 `그 연결을 실제 실행 기록으로 남기는 층`, 평가는 `남은 기록을 품질 판단 기준으로 바꾸는 층`입니다. 이 차이가 보여야 MCP를 운영 장치나 평가 절차와 섞어 읽지 않게 됩니다.
...
| --- | --- | --- |
| 도구 설명과 자원 설명 | 어떤 도구와 자원이 어떤 이름·입력 형식으로 연결되는지 남겨야 호출 실패와 연결 불일치를 줄일 수 있어서 | P6-14.2의 trace/replay와 도구 연결 메모로 이어집니다 |
| 권한 경계와 승인 조건 | 어떤 호출이 바로 실행 가능하고 어떤 호출이 승인을 거쳐야 하는지 남겨야 운영 실패를 줄일 수 있어서 | P6-14.2의 승인 기록, P6-16.2의 실패 대응으로 이어집니다 |

초심자 기준에서는 `공통 형식`이라는 말이 아직 추상적으로 느껴질 수 있습니다. 이때는 프로토콜 이름을 먼저 외우기보다, 같은 목표를 처리할 때 도구마다 입력 형식이 제각각이면 어디서 먼저 흔들리는지를 떠올리는 편이 더 안전합니다.
...
- 바로 앞의 P6-12.1, P6-12.2 도구 사용 구조와 P6-13.1, P6-13.2 에이전트 실행 구조를 `연결 계층` 관점에서 다시 읽게 하고
- agent와 tool use를 시스템 연결 문제까지 확장해 읽게 하며
- 다음 절의 P6-14.2 하네스와 뒤의 운영 실패 대응으로 넘어갈 준비를 시키기 때문입니다
...
이 표를 먼저 붙잡고 아래 내용을 읽으면, MCP를 `프로토콜 이름`보다 `도구와 자원을 덜 제각각으로 연결하게 만드는 공통 형식`으로 더 직접 읽을 수 있습니다.
...
MCP를 처음 읽을 때 초심자가 가장 자주 놓치는 것은 `도구가 잘 안 붙는다`는 문제를 곧바로 모델 성능 부족으로 읽는 점입니다. 하지만 이 절에서 먼저 봐야 하는 것은 모델이 똑똑하냐보다 `도구와 자원이 어떤 공통 형식으로 노출되어 있는가`입니다.
...
이 절에서 먼저 익혀야 하는 기준은 단순합니다. MCP는 `모델을 더 똑똑하게 만드는 기능`이 아니라, `도구 설명`, `자원 접근`, `요청/응답 형식`을 덜 제각각으로 만들어 연결 불안정을 줄이는 공통 인터페이스 관점입니다.
...
이 표의 핵심은 MCP가 `연결을 일정하게 만드는 층`이지, 실행을 기록하거나 품질을 판정하는 층이 아니라는 점입니다. 바로 다음 절의 하네스는 같은 연결을 실제 실행 trace와 replay로 남기고, 그 뒤의 평가와 운영은 그 기록을 품질 판단과 통제 조치로 읽습니다.
...
| 상황 | 먼저 떠올릴 관점 | 왜 중요한가 |
| --- | --- | --- |
| 도구와 자원이 늘면서 연결 방식이 제각각 흔들리기 시작할 때 | 공통 연결 형식이 먼저 필요하다는 점 | 도구마다 이름, 입력, 반환 방식이 달라지면 agent 흐름보다 연결 불일치가 먼저 문제를 만듭니다. |
| 모델 성능 문제와 도구 연결 문제를 자꾸 섞어 보게 될 때 | MCP는 모델 능력이 아니라 연결 인터페이스 층위라는 점 | 어떤 실패가 모델 한계인지, 어떤 실패가 연결 설계 문제인지 구분해야 수정 방향이 정확해집니다. |
| 다음 절의 하네스가 왜 별도 개념인지 감이 오지 않을 때 | MCP는 연결, 하네스는 실행 관리라는 점 | 연결을 일정하게 만드는 일과 실행을 기록·재현하는 일은 다른 층위이므로 먼저 분리해야 합니다. |
