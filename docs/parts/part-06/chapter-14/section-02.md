...
agent가 `여러 읽기와 실행을 어떤 목표 흐름으로 이어 갈까`를 다뤘다면, 계획-행동-관찰은 그 흐름이 실제로 어떤 반복으로 움직이고 어디서 멈추거나 사람 검토로 넘어가는지 다룹니다. 목표 흐름을 안다고 해도, 중간 결과에 따라 언제 계속하고 언제 멈출지를 정하지 않으면 실행이 흔들립니다. 그래서 필요한 구조는 계획-행동-관찰-결정으로 움직이는 반복 루프 구조입니다.

P6-14.1이 여러 읽기와 실행을 어떤 목표 흐름으로 이어 갈지 봤다면, 여기서는 그 목표 흐름이 실제로 어떤 반복 루프로 움직일지 봅니다. 그다음 P6-15에서는 이 루프를 어떤 연결 규칙과 실행 기록 환경 안에서 관리할지로 질문이 이동합니다.
