...
초심자 기준에서는 하네스를 도구 이름처럼 외우기보다, 어떤 기록이 없으면 어떤 실패를 다시 못 좁히는지 먼저 붙잡는 편이 더 안전합니다.
...
하네스를 처음 읽을 때 초심자가 가장 자주 놓치는 것은 `로그를 많이 남긴다`는 사실만 기억하고, 그 로그가 실제로 `재현`, `원인 분리`, `운영 조치`로 이어질 수 있어야 한다는 점까지는 바로 연결하지 못하는 점입니다. 하지만 하네스의 핵심은 기록의 양이 아니라, 같은 실패를 다시 설명하고 다음 조치를 결정할 수 있을 정도로 실행을 남기는 데 있습니다.
