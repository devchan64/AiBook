...
먼저 닫을 문제는 단일 에이전트 루프의 기본 구조를 `계획-행동-관찰 반복`으로 읽고, 어디서 계속 진행하고 어디서 멈추는지 붙잡는 것입니다.

도구 연결 규칙과 실행 환경은 루프가 어떤 도구와 자원을 쓰고, 그 실행을 어떤 기록 환경에 남기는지의 문제입니다. 계획-행동-관찰 루프는 먼저 관찰 결과가 다음 분기와 종료 판단을 어떻게 바꾸는지에 집중합니다.

에이전트는 추상적인 개념으로만 두지 않고, `반복 루프(loop)`로 읽어야 합니다.

agent가 `여러 읽기와 실행을 어떤 목표 흐름으로 이어 갈까`를 다뤘다면, 계획-행동-관찰은 그 흐름이 실제로 어떤 반복으로 움직이고 어디서 멈추거나 사람 검토로 넘어가는지 다룹니다. 연결 규칙과 기록 환경은 이 루프가 쓰는 도구와 실행 흔적을 어떻게 관리할지의 문제입니다.

후반 실행 구조를 같은 질문으로 다시 고정하면, 역할은 아래처럼 압축할 수 있습니다.

| 부족한 것 | 붙이는 구조 | 아직 남는 문제 |
| --- | --- | --- |
| 목표 흐름을 안다고 해도, 중간 결과에 따라 언제 계속하고 언제 멈출지를 정하지 않으면 실행이 흔들린다 | 계획-행동-관찰-결정으로 움직이는 반복 루프 구조 | 이 루프를 어떤 연결 형식과 실행 기록 안에 둘지, 어떤 평가와 운영 판단으로 읽을지는 아직 남아 있다 |

| 현재 관점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| agent 목표 흐름 | 여러 읽기와 실행을 어떤 목표 흐름으로 이어 갈까? | 반복 루프와 종료 판단 |
| plan-action-observation 반복 루프 | 그 목표 흐름이 실제로 어떤 반복 루프로 움직일까? | 연결 규칙과 기록 환경 |
| MCP / harness 관리 | 이 루프를 어떤 연결 규칙과 기록 환경 안에서 관리할까? | 평가와 운영 판단 |

여기서는 `계획(plan)`, `행동(action)`, `관찰(observation)` 루프를 통해 에이전트를 한 번의 응답이 아니라 반복 구조로 읽는 기준을 잡습니다.

핵심은 `여러 단계를 이어 갈까`에서 `그 단계들이 어떤 관찰과 결정 루프로 반복될까`로 관점이 바뀌는 데 있습니다. 그다음에는 이 루프를 어떤 연결 규칙과 기록 환경 안에서 관리할지로 질문이 이동합니다.

이 단계에서 먼저 남겨야 할 기록은 어느 단계에서 판단이 바뀌었는지를 보여 주는 계획, 행동, 관찰 기록과, 언제 멈췄고 왜 사람에게 넘겼는지를 보여 주는 종료 이유와 다음 단계입니다. 이 기록이 있어야 루프 실패와 재시도 이유를 다시 좁힐 수 있고, 뒤로 갈수록 P6-15.2의 trace/log, P6-17.2의 실패 대응에서 다시 읽힙니다.
