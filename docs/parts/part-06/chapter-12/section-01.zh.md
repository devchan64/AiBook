# P6-12.1 同时保存嵌入、原文和元数据的向量数据库

> Section ID: `P6-12.1`
> Version: `v2026.07.26`

在 P6-11.2 中，我们看到检索结果会在生成前附加到输入上下文。现在问题转向：这种检索实际运行在什么样的存储结构之上。
...
如果 P6-11.2 看的是取回文档在回答前附加到哪里，那么这里看的就是为了让这些文档可检索，要把它们放进什么存储结构。接着 P6-12.2 会看用什么索引和检索质量标准缩小已保存的候选。超出文档检索、进入实际查询或执行的事情，会在后面的工具使用部分另外讨论。

## 区分向量、原文和元数据的保存

理解向量数据库时，要先分开看保存的值。嵌入是为了找到相似文档而准备的数值表示，文档块(chunk)是生成阶段实际重新阅读的原文，元数据(metadata)则是来源、版本、日期、类别等在选择和验证候选时使用的信息。三者要一起读，才能自然连接：为什么 RAG 中普通关键词搜索不够，以及 P6-12.2 的索引和检索质量问题为什么要另外接上。
...
- 帮助我们准备阅读 P6-12.2 的索引和检索质量问题
- 把前面的 P6-11.1 和 P6-11.2 RAG 流程重新连接到实际存储层

这里建立的视角会继续进入后续章节。

- P6-12.2 索引和检索质量：同时阅读检索速度和候选质量的标准
- P6-13.1 工具使用和 P6-14.1 AI agent 结构：观察检索型功能在整个系统中位于哪里
- P6-16.1 LLM 评价、P6-17.1 服务运营约束、P6-18.1 把小型生成式 AI 功能串成一个流程：把检索型和工具连接型功能带入实际设计与运营判断的可复用标准

## 检查清单

- 能否把向量数据库说明为`一起处理嵌入、原文、元数据的检索存储结构`，而不是`只含有向量的存储`？
- 能否说明字符串搜索和语义搜索为什么不同，为什么要分开？
- 是否准备好把 P6-12.2 读成`怎样更快、更准确地探索已保存候选`的问题，而不是存储本身的说明？
