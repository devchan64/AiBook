...
여기서는 인덱스를 단순한 내부 기술명으로 넘기지 않고, `빠른 검색을 위해 근사(approximation)를 허용하는 구조`로 읽습니다. P6-12.1이 검색 후보를 어떤 저장 구조에 담아 다시 꺼내 쓸지 봤다면, 여기서는 그 후보를 어떤 속도와 품질 균형으로 좁힐지 봅니다. 문서 검색을 넘어 실제 조회나 실행이 필요한지는 P6-13의 도구 사용 구간에서 이어서 봅니다.
...
- 오픈체크리스트의 중심축 문장인 "벡터 DB를 붙였다는 사실과 검색 품질이 보장된다는 판단을 섞지 않고, 후보 순위, 누락, 오래된 문서, 필터 문제를 점검해야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
