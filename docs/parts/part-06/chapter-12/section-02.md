...
후반 실행 구조로 넘어가기 직전, 역할을 먼저 고정할 수 있습니다. 임베딩, 원문, 메타데이터를 저장해 두어도 후보를 너무 느리게 찾으면 실제 서비스 검색으로 쓰기 어렵습니다. 그래서 인덱스는 속도와 검색 품질을 함께 조정하는 탐색 구조로 붙습니다. 다만 문서 검색을 넘어 무엇을 실제로 조회하거나 실행할지, 그 실행 요청을 어떻게 구조화할지는 P6-13의 도구 사용 구간에서 이어서 봅니다.

P6-12.1이 검색 후보를 어떤 저장 구조에 담아 다시 꺼내 쓸지 봤다면, 여기서는 그 후보를 어떤 속도와 품질 균형으로 좁힐지 봅니다. 그다음 P6-13에서는 문서 검색을 넘어서 무엇을 실제로 조회하거나 실행할지로 질문이 커집니다.
