...
도구 사용의 기본 구조는 여기서 잡고, 구조화된 호출 형식은 바로 다음 P6-12.2 함수 호출에서 다시 회수합니다. 여러 단계 반복 구조는 P6-13.1 에이전트와 P6-13.2 계획, 행동, 관찰에서 다시 다루며, 권한과 운영 문제는 P6-14.2 하네스와 P6-16.2 운영 중 실패 대응에서 이어집니다.
...
이 절은 Part 6에서 `도구 사용(tool use)`을 대표로 설명하는 Section입니다. `모델이 스스로 실행한다`는 인상을 `애플리케이션이 외부 기능을 연결해 실제 결과를 가져오는 구조`로 바꾸는 기준선을 여기서 세웁니다.
...
여기서 먼저 남겨야 할 것은 무엇을 실행하려 했고 실제로 어떤 값이 돌아왔는지를 보여 주는 호출 이름, 인자, 실행 결과와, 실행을 바로 진행해도 되는지와 어디서 멈췄는지를 보여 주는 승인 여부와 실패 이유입니다. 이 기록이 있어야 추측 답변과 실행 결과를 구분하고, 권한 문제와 운영 실패를 다시 볼 수 있습니다. 뒤로 갈수록 이 기록은 P6-12.2의 함수 호출 형식, P6-14.2의 trace/log와 승인 기록, P6-16.2의 실패 대응에서 다시 읽힙니다.
...
이 사례가 중요한 이유는 수치 작업에서 사용자가 실제로 기대하는 것은 `설득력 있는 설명`보다 `틀리지 않는 숫자`이기 때문입니다. 설명이 자연스러워도 계산이 틀리면 할인 금액, 세금, 청구액처럼 뒤 숫자들이 함께 무너집니다. 그래서 계산 문제에서는 모델의 언어 능력과 계산 정확도를 분리해서 봐야 합니다. 초심자는 흔히 말이 또렷하면 계산도 맞을 것이라고 느끼지만, 도구 사용 관점에서는 오히려 `말은 잘해도 숫자는 별도 장치로 확인해야 한다`가 기본값에 가깝습니다.
...
이 표에서 초심자가 넘어가야 할 오해는 `설명이 좋아 보이면 계산도 거의 맞다`는 기대입니다. 계산기 도구 사례의 핵심은 설명과 계산을 분리하고, 계산 쪽은 별도 실행 결과를 먼저 붙잡는 데 있습니다.
...
이 장면이 실제 운영에서 중요한 이유는 상태 질문이 문서 질문처럼 보일 때가 많기 때문입니다. `회의실을 예약할 수 있나요?`는 규칙 설명으로 닫힐 수 있지만, `내일 오후 비어 있나요?`는 현재 시스템 값을 조회하지 않으면 닫히지 않습니다. 초심자는 이 둘을 같은 종류의 질문으로 묶기 쉽지만, 도구 사용 관점에서는 `설명 문제`와 `상태 조회 문제`를 먼저 갈라야 합니다. 그래야 문서 검색으로 충분한 요청과 실제 시스템 접근이 필요한 요청을 분리할 수 있습니다.
...
이 사례에서 초심자가 붙잡아야 할 기준은 `관련 정보를 알고 있는가`보다 `지금 필요한 정보가 문서 안에 있는가, 시스템 상태 안에 있는가`입니다. 일정 조회 사례는 바로 이 분기에서 도구 사용 필요가 결정된다는 점을 보여 줍니다.
...
| 상황 | 먼저 떠올릴 관점 | 왜 중요한가 |
| --- | --- | --- |
| 문서를 읽는 것만으로는 닫히지 않고 실제 조회나 계산이 필요한 요청을 만났을 때 | 답변보다 실행 연결이 먼저 필요하다는 점 | 최신 값 조회, 계산, 예약 같은 요청은 근거 문서만으로는 닫히지 않고 외부 기능 호출이 필요합니다. |
| 모델이 말을 잘해도 결과를 검증해야 하는 작업을 다룰 때 | 추측 답변보다 실제 실행 결과를 가져와야 한다는 점 | 계산값, 상태 조회, 파일 수정은 그럴듯한 설명보다 검증 가능한 결과가 먼저입니다. |
| RAG와 agent 사이에서 현재 문제가 어느 층위인지 헷갈릴 때 | `읽기`와 `실행`을 구분하는 중간 층위라는 점 | 이 절을 기준으로 잡아 두면 다음 절의 함수 호출과 뒤 장의 agent 구조를 다른 층위로 읽기 쉬워집니다. |
...
tool use는 `무엇을 실제로 조회하거나 실행할까`를 먼저 닫는 구조입니다. 외부 기능으로 실행을 위임하고, P6-12.2에서는 그 요청을 함수 이름과 인자 구조로 어떻게 넘길지 더 봅니다. 그다음 P6-13에서는 agent 흐름으로 넘어갑니다.
