...
후반 실행 구조로 넘어가기 전, 선택 기준을 먼저 고정해야 합니다. 지금 부족한 것은 모델이 외부 문서를 오래 기억하지 못하고, 표현이 달라지면 관련 문단을 바로 다시 찾기 어렵다는 점입니다. 그래서 필요한 구조는 임베딩, 원문, 메타데이터를 함께 두는 검색 저장 구조입니다. 다만 어떤 속도와 품질 균형으로 후보를 좁힐지, 검색을 넘어 실제 조회·실행까지 갈지는 P6-12.2와 뒤의 도구 사용 구간에서 이어서 봅니다.

P6-11.2가 찾아온 문서를 답변 전에 어디에 붙일지 봤다면, 여기서는 그 문서를 검색 가능하게 어떤 저장 구조에 담아 둘지 봅니다. 그다음 P6-12.2에서는 저장된 후보를 어떤 인덱스와 검색 품질 기준으로 좁힐지 봅니다.
