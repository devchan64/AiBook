# P6-12.1 벡터 데이터베이스(vector database)
...
먼저 닫을 문제는 `왜 텍스트만이 아니라 임베딩, 원문, 메타데이터를 함께 저장하는가`입니다. 속도와 근사 검색의 균형은 후보를 어떻게 좁힐지의 문제이고, 문서 검색을 넘어 실제 조회나 실행으로 가는 일은 별도 실행 구조의 문제입니다.

벡터 데이터베이스는 `새로운 종류의 마법 저장소`가 아니라, 임베딩 검색을 서비스 구조 안에서 다루기 쉽게 만든 시스템으로 읽는 편이 맞습니다.

이 절은 `검색 저장 구조`를 이해하는 절로 읽으면 됩니다.

후반 실행 구조로 넘어가기 전, 선택 기준을 먼저 고정하면 다음과 같습니다.

| 지금 부족한 것 | 붙이는 구조 | 아직 남는 문제 |
| --- | --- | --- |
| 모델이 외부 문서를 오래 기억하지 못하고, 표현이 달라지면 관련 문단을 바로 다시 찾기 어렵다 | 임베딩, 원문, 메타데이터를 함께 두는 검색 저장 구조 | 어떤 속도와 품질 균형으로 후보를 좁힐지, 검색을 넘어 실제 조회·실행까지 갈지는 아직 남아 있다 |

| 지금 단계의 관점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| 검색 결과와 생성 결합 | 찾아온 문서를 답변 전에 어디에 붙일 것인가? | 문서 준비와 저장 구조 |
| 검색 저장소 | 임베딩, 원문, 메타데이터를 어떤 저장 구조에 함께 담아 둘 것인가? | 후보 압축과 검색 품질 |
| 인덱스와 검색 품질 | 그 저장 구조 안에서 후보를 어떤 속도와 품질 균형으로 좁힐 것인가? | 도구 호출과 외부 실행 |

여기서는 `벡터 데이터베이스(vector database)`를 `벡터만 모아 둔 저장소`가 아니라 `임베딩, 원문, 메타데이터를 함께 다루는 RAG 검색 저장 구조`로 읽는 기준을 잡습니다.

검색된 문서를 생성 전에 붙이려면, 그 전에 문서가 검색 가능한 형태로 저장되어 있어야 합니다. 여기서 중요한 것은 문서를 임베딩, 원문, 메타데이터와 함께 어떤 검색 저장 구조에 둘지입니다. 그다음 문제는 저장 구조 안에서 후보를 얼마나 빠르고 정확하게 좁힐지입니다. 이 기준이 잡혀야 벡터 데이터베이스를 또 하나의 유행 기술명이 아니라, `RAG 검색 단계를 운영 가능한 저장 구조로 바꾸는 계층`으로 읽을 수 있습니다.
...
이 표를 먼저 붙잡고 아래 내용을 읽으면, 벡터 데이터베이스를 `벡터만 저장하는 곳`보다 `검색 뒤에 바로 다시 쓸 원문과 메타데이터까지 함께 돌려주는 저장 구조`로 더 직접 읽을 수 있습니다.
