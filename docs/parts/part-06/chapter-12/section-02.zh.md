# P6-12.2 在搜索速度和候选质量之间取舍的索引

> Section ID: `P6-12.2`
> Version: `v2026.07.26`

在 P6-12.1 中，我们看到向量数据库会把嵌入向量、原文和元数据一起保存，并在检索阶段承担实务型存储层角色。现在问题变得更具体：为什么快速找到相似向量很难，又必须放弃或调整什么？
...
这里不把索引当作简单的内部技术名略过，而是把它读成`为了更快搜索而允许近似(approximation)的结构`。P6-12.1 看的是候选要放进什么存储结构才能再次取回，本节看的是这些候选要以什么速度和质量平衡被缩小。文档检索是否要走向实际查询或执行，会在 P6-13 的工具使用部分继续讨论。
