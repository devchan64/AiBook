...
정렬은 단순 도덕 구호가 아니라 `LLM을 실제 서비스에 쓰기 위해 반드시 등장하는 설계 문제`입니다. 먼저 붙잡아야 할 것은 `잘 따르는가`와 `허용 가능한가`가 서로 다른 질문이라는 점입니다.
...
![후보 유형별 평균 평가 축](../../../assets/part-06/chapter-08/alignment-axis-average-ko.png)
...
먼저 붙잡아야 할 점은 `지시를 따르는가`와 `어디까지 허용할 것인가`가 같은 문제가 아니라는 것입니다. 그래서 alignment는 유창한 답변을 더 만드는 기술 하나가 아니라, 여러 업무 장면에서 어떤 응답을 통과시키고 어디서 멈출지를 고르는 기준으로 읽는 편이 좋습니다.
