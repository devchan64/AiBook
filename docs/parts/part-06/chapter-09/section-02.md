...
이 장면도 장난감 예시가 아닙니다. 실제 보고서 자동화에서는 숫자 한 칸이 틀리면 문장 한 줄이 틀리는 수준에서 끝나지 않고, 다음 회의의 판단과 조치까지 흔들릴 수 있습니다. 특히 매출, 재고, 광고비, 전환율처럼 서로 연결된 지표는 한 항목이 틀리면 뒤의 해석 전체가 연쇄적으로 어긋납니다. 그래서 운영자는 `표가 예쁘게 나왔는가`보다 먼저 `이 수치가 어디서 왔고, 어떤 계산을 거쳤고, 다시 계산해도 같은가`를 확인해야 합니다.
