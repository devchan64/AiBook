...
이번 절은 `입력 조정`과 `시스템 구조를 바꿔야 하는 문제`를 나누는 기준까지를 먼저 닫습니다. 프롬프트만으로 부족한 최신성·근거 문제는 바로 다음 P6-10.1 RAG의 필요성과 P6-10.2 검색 결과와 생성의 결합에서 다시 회수하고, 평가 구조는 P6-15.1과 P6-15.2에서, 에이전트 프레임워크의 큰 그림은 P6-13.1과 P6-13.2에서 각각 다시 다룹니다.

이 절에서는 프롬프트 엔지니어링을 과대평가하지 않고, 앞 절의 `어떻게 물을까`에서 `잘 물어도 아직 남는 문제는 무엇인가`로 질문을 옮깁니다. 바로 다음 P6-10.1에서는 이 질문이 `그렇다면 무엇을 근거로 답하게 할까`로 넘어가며, RAG는 프롬프트 확장판이 아니라 근거 연결 구조라는 점을 잡습니다.
...
여기서 먼저 바뀌어야 하는 인상은 `프롬프트를 더 세게 쓰면 해결된다`가 아니라 `입력 조정과 시스템 구조 보장은 다른 층`이라는 이해입니다. 지금 장의 중심도 `더 잘 물을까`에서 `질문 밖의 근거를 실제로 붙여야 하는가`로 이동합니다.
...
| 먼저 보인 한계 | 먼저 떠올릴 질문 | 왜 이 질문이 먼저 필요한가 |
| --- | --- | --- |
| 형식은 어느 정도 맞지만 최신 사실이 자꾸 불안하다 | 지금 부족한 것은 프롬프트가 아니라 최신 문서 연결인가? | 최신성 문제를 입력 문장 문제로만 읽으면, 구조를 바꿔야 할 신호를 놓치기 쉽기 때문입니다. |
| 답이 그럴듯하지만 출처를 믿기 어렵다 | 근거를 요구하는 것과 실제 근거를 붙이는 것은 다른가? | `출처처럼 보이는 답`과 `문서에 묶인 답`을 같은 말로 보면 검증된 답처럼 오해하기 쉽기 때문입니다. |
| 계산, 저장, 조회 같은 행동이 필요한데 답변 문장만 좋아진다 | 지금 막히는 곳은 말투가 아니라 실행 구조인가? | 행동 성공은 프롬프트 강화보다 tool use/function calling 구조가 먼저 필요한 신호이기 때문입니다. |
| 같은 실패가 반복되는데도 원인을 비교하기 어렵다 | 지금 필요한 것은 프롬프트 보강이 아니라 평가·기록 구조인가? | 반복 판정과 회귀 감지는 응답 문장보다 evaluation/harness 층의 기준이 먼저 필요하기 때문입니다. |

이 표를 먼저 붙잡고 아래 내용을 읽으면, 프롬프트의 한계를 `더 강한 문장 요령`보다 `입력 조정으로 줄일 문제와 구조를 바꿔야 할 문제를 가르는 기준`으로 더 직접 읽을 수 있습니다.
...
이 비교의 핵심은 `더 조심스럽게 말하는 답`과 `현재 문서에 묶인 답`이 서로 다른 층이라는 점입니다. 초심자가 여기서 꼭 넘어가야 할 오해는 `신중한 표현이 최신성 문제도 어느 정도 해결해 준다`는 기대입니다. 실제로는 최신성 문제를 줄이는 첫 작업이 프롬프트 미세조정이 아니라, 현재 문서 회수와 근거 표기 구조를 붙이는 일입니다.
...
이 사례에서 초심자가 꼭 붙잡아야 할 기준은 `정확히 계산해 달라`는 지시가 곧 `정확함이 보장됐다`는 뜻이 아니라는 점입니다. 계산 문제는 말의 세기가 아니라 검산 구조의 유무로 갈립니다. 그래서 실제로는 프롬프트를 길게 다듬기 전에 `계산기를 붙였는가`, `중간 합계가 로그로 남는가`, `원본 수치와 결과 수치를 다시 맞춰 볼 수 있는가`를 먼저 확인해야 합니다.
...
- 바로 앞의 P6-9.1 프롬프트 엔지니어링을 `입력 설계의 첫 번째 제어 지점`으로 두되 만능 해법으로 오해하지 않게 하고
- 구조를 추가해야 하는 시점을 판단하게 하며
- 다음 장의 P6-10.1, P6-10.2 RAG와 뒤의 P6-12.1 도구 사용, P6-13.1 에이전트로 왜 확장되는지 자연스럽게 연결하기 때문입니다

## 바로 적용해 보면

초심자가 실제 실패 장면에서 가장 자주 막히는 이유는 `답이 이상하다`는 한 문장 안에 여러 원인이 한꺼번에 섞여 있기 때문입니다. 이 절을 읽은 뒤에는 먼저 `무엇이 빠져서 이 실패가 났는가`를 한 번 갈라 적는 습관이 필요합니다.
...
이 기준을 붙잡고 다음 장으로 넘어가면 RAG를 `프롬프트 확장판`으로 오해하지 않게 됩니다. `말을 더 잘 시키는 일`과 `답의 출발점을 외부 근거에 묶는 일`이 다른 층이라는 감각이 여기서 먼저 고정돼야, P6-10의 검색·근거 결합도 훨씬 덜 압축되어 읽힙니다.
