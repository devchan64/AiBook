# P6-10.1 요청 형식에 맞는 응답 습관을 만드는 지시 튜닝

> Section ID: `P6-10.1`
> Version: `v2026.07.31`

P6-9.2에서는 LoRA 같은 효율적 조정 방식이 왜 실무에서 중요한지 보았습니다. 그런데 모델을 조금 더 싸게 조정할 수 있다고 해서, 곧바로 사람 지시를 잘 따르는 답변이 만들어지는 것은 아닙니다.
...
- P6-10.2의 alignment 문제를 `왜 필요한가`라는 질문과 연결해 주기 때문입니다
...
이 사례에서는 지시 튜닝이 사용자 지시 형식에 맞는 응답 습관을 만든다는 점을 보여 주는지 확인한다.
...
- P6-10.2를 `잘 따르는 것`과 `허용 가능한 행동`을 나누는 문제로 읽을 준비가 되었는가?
