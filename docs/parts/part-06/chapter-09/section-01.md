...
이번 절은 `입력 설계`가 현재 모델 반응을 관찰하고 조정하는 첫 번째 실무 손잡이라는 점까지를 먼저 닫습니다. Chain-of-thought, self-consistency, automatic prompt optimization 같은 이름은 P6-9.3 보충학습에서 다시 정리하고, 프롬프트가 어디서 한계에 부딪히는지는 바로 다음 P6-9.2 프롬프트의 한계에서 다시 회수합니다. tool use와 agent 구조는 P6-12.1 도구 사용, P6-12.2 함수 호출, P6-13.1 에이전트에서 각각 다시 다룹니다.

프롬프트는 `마법의 주문`이 아니라, 모델 행동을 관찰하고 조정하기 위한 입력 설계 도구로 읽는 편이 안전합니다.

현재 Part 6에서 Chapter 9는 더 앞쪽으로 당겨 놓기보다, 지금 위치에서 `모델 조정 축`을 닫고 `외부 근거와 실행 구조`로 넘어가기 직전의 전환 장을 맡는 편이 맞습니다. 즉, 사전학습, 파인튜닝, 정렬을 본 뒤에 `사용자가 지금 당장 쥘 수 있는 첫 번째 손잡이`로 프롬프트를 읽고, 바로 다음 절에서 그 한계를 확인한 뒤 RAG와 도구 사용으로 넘어가는 흐름이 Part 6 본류에서는 가장 자연스럽습니다.

이 절은 `첫 번째 실무 손잡이 축`으로 읽으면 됩니다.
...
이 절은 Part 6에서 `프롬프트 엔지니어링(prompt engineering)`을 대표로 설명하는 Section입니다. `잘 쓰는 문장 요령`이라는 인상을 `현재 모델 반응을 관찰하고 조정하는 첫 번째 실무 손잡이`로 바꾸는 기준선을 여기서 세웁니다.
...
이 요약이 보이면 뒤 절의 핵심도 같이 잡힙니다. 프롬프트는 `모델 capability를 더 잘 끌어내는 첫 손잡이`이지만, `limit를 넘어서는 구조 보장`까지 맡는 손잡이는 아닙니다.
...
프롬프트는 Part 6 본류의 `첫 번째 실무 손잡이`로 읽고, 그 다음 손잡이가 무엇인지까지 함께 연결해 두면 됩니다.
...
| --- | --- | --- |
| 왜 같은 모델인데 답변 길이, 형식, 말투가 요청마다 흔들릴까? | 지금 바꿀 수 있는 가장 안쪽 손잡이가 입력 설계이기 때문입니다. | 프롬프트 한계 확인, 형식 실험 |
