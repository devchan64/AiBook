# P6-9.1 建立符合请求格式回应习惯的指令调优

> Section ID: `P6-9.1`
> Version: `v2026.07.26`

在 P6-8.2 中，我们看到 LoRA 这样的高效调整方式为什么在实务中重要。但能够以更低成本稍微调整模型，并不意味着马上就会产生很好遵循人类指令的回答。
...
- 把 P6-9.2 的 alignment 问题连接到 `为什么需要它` 这个问题
...
- 是否准备好把 P6-9.2 读作区分 `很好遵循` 和 `可接受行为` 的问题？
