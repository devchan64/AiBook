# P6-10.3 형식·근거·실행 부족으로 나누어 보는 LLM 실패

> Section ID: `P6-10.3`
> Version: `v2026.07.31`

P6-10.2까지 오면 `어떤 답이 더 assistant처럼 보이는가`, `어떤 답이 더 허용 가능하고 안전한가`까지는 읽게 됩니다. 그런데 실제 기능 개선 단계로 가면 질문이 한 번 더 바뀝니다. 이제는 좋은 답의 기준을 아는 데서 멈추지 않고, 실패한 답에서 무엇이 먼저 부족했는지 진단해야 하기 때문입니다.
...
이 절은 후속 장의 세부 기술을 대신 설명하지 않습니다. 여기서 닫을 문제는 `무엇을 어디서 먼저 손봐야 하는가`입니다. 프롬프트 자체의 설계는 P6-10에서, RAG와 벡터 검색은 P6-11과 P6-12에서, 도구 사용과 AI 에이전트 구조는 뒤 Module에서 이어 봅니다. P6-10.4와 P6-10.5는 이 진단 중 파인튜닝 축으로 판단이 갔을 때 필요한 효율적 조정 보충학습입니다.
...
## 형식·근거·실행 부족으로 나누어 보는 LLM 실패: 확인할 판단 기준

이 사례에서는 LLM 실패를 형식, 근거, 실행, 지속 적응 부족으로 나누어 프롬프트, RAG, 도구 사용, 평가·운영 장으로 잇는지 확인한다.
