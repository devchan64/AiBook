# P6-20.1 한계와 구조 전환의 흐름으로 읽는 LLM 발전사
...
## 한계와 구조 전환의 흐름으로 읽는 LLM 발전사: 확인할 판단 기준

이 사례에서는 통계적 언어 모델에서 GPT형 인터페이스까지의 변화를 `어떤 한계를 줄였는가` 기준으로 설명하는지 확인한다.
...
| --- | --- |
| 언어 모델은 `다음 표현을 예측한다`는 문제의식에서 출발했다 | P6-7.1 다음 토큰 예측 |
...
| 사전학습이 모델 사용 방식을 바꾸었다 | P6-8.1 사전학습 |
