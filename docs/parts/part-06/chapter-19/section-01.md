...
이 사례가 중요한 이유는 분류 문제를 처음 보면 많은 초심자가 `라벨에 가까운 단어가 들어 있느냐`부터 세기 쉽기 때문입니다. 하지만 실제 문의 운영에서는 표면 단어보다 `어느 처리 흐름으로 보내야 하는가`가 더 중요합니다. `환불`이라는 단어가 없어도 돈을 돌려받는 과정이 핵심이면 환불 큐로 가야 하고, 반대로 `취소`라는 단어가 있어도 실제로는 계정 인증 문제라 다른 큐로 가야 할 수 있습니다. 그래서 읽기 중심 모델을 도입하는 이유는 키워드 사전을 더 길게 만드는 데 있지 않고, 문장 전체가 가리키는 업무 의도를 더 안정적으로 붙잡기 위해서입니다.
...
이 표에서 초심자가 넘어가야 할 오해는 `라벨 이름과 비슷한 단어가 보이면 거의 충분하다`는 생각입니다. 문서 분류 사례에서 읽기 중심 모델이 필요한 이유는 단어 표면보다 처리 의도를 더 직접 붙잡기 위해서입니다.
...
이 장면도 실제 운영에서 바로 체감됩니다. 검색 랭킹은 많은 팀이 처음에는 제목 키워드 일치로 시작하지만, 사용자가 쓰는 표현과 문서 작성 표현이 조금만 달라져도 상단 결과가 쉽게 흔들립니다. `장비 반납`을 찾는데 문서 쪽 표현은 `자산 회수`일 수 있고, `퇴사`를 찾는데 제목은 `오프보딩 절차`일 수 있습니다. 이때 중요한 것은 단어를 몇 개 더 사전에 넣느냐보다, 질문 전체가 문서의 어느 절차와 연결되는지를 읽는 능력입니다.
...
이 사례에서 초심자가 꼭 붙잡아야 할 기준은 `같은 단어가 얼마나 겹치는가`와 `실제로 같은 절차를 말하는가`를 분리해서 보는 일입니다. 검색 랭킹에서 BERT 계열이 필요한 이유는 바로 이 둘이 자주 어긋나기 때문입니다.
