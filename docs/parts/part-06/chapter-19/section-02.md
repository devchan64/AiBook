...
GLUE 같은 벤치마크의 세부 점수와 cross-encoder, bi-encoder의 구현 차이는 여기서 다루지 않습니다. 대신 검색 파이프라인 안에서 이런 구조가 어떻게 갈라지는지는 P6-11.1 벡터 데이터베이스와 P6-11.2 인덱스와 검색 품질에서 다시 회수합니다. reranker와 dense retrieval의 세부 설계는 현재 본편 범위 밖에 둡니다.
