...
이 절에서는 태스크 이름을 많이 나열하기보다, `입력을 읽고 판단하는 흐름`을 한 묶음으로 이해합니다. 지금 읽는 층위는 `본류 복습 뒤의 이해 과업 배경 층위`입니다. 앞 절이 BERT 계열의 위치를 비교 기준으로 잡았다면, 여기서는 그 비교를 실제 작업 묶음으로 좁혀 `왜 읽기 중심 모델이 분류, 검색, 임베딩과 잘 맞는가`를 다시 정리합니다. 즉, 이 장은 생성형 AI 본류를 새로 바꾸는 단계가 아니라, 앞서 읽은 GPT 중심 설명을 `생성 출력`과 `판단 출력`의 차이로 더 선명하게 다시 읽게 만드는 배경 정리 단계에 가깝습니다.
