...
이 절에서는 태스크 이름을 많이 나열하기보다, `입력을 읽고 판단하는 흐름`을 한 묶음으로 이해합니다. 앞 절이 BERT 계열의 위치를 비교 기준으로 잡았다면, 여기서는 그 비교를 실제 작업 묶음으로 좁혀 `왜 읽기 중심 모델이 분류, 검색, 임베딩과 잘 맞는가`를 다시 정리합니다. 초심자에게 여기서 더 필요한 것은 장의 위치 설명보다 `라벨`, `점수`, `순위`, `벡터`가 왜 한 종류의 출력으로 묶이는가를 먼저 구분하는 기준입니다.
