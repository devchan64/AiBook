...
在 P6-10.2 中，我们看到对齐(alignment)不只是让回答变亲切的问题，而是有用性、安全性、事实性、服务政策一起牵涉进来的设计问题。接下来要看用户最先能直接握在手里的工具。
...
这里首先要留下的是实验备忘和格式检查统计：尝试了什么输入设计，答案里哪些项目经常缺失，格式在哪里摇摆。有了这个记录，P6-11.2 才能重新检查提示限制，也能不动摇地转到 P6-12.1 的依据连接判断或 P6-14.1 的执行需要判断。越往后，这个记录会再次被读成 P6-16 的评估标准，以及 Part 6 的回顾备忘、执行记录、改善计划。
...
- 准备好把 P6-11.2 读成寻找 `只靠输入调整无法确认的失败` 的阶段了吗？
