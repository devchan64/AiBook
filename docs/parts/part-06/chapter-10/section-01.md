...
먼저 붙잡아야 할 점은 프롬프트가 `현재 모델 반응을 관찰하고 조정하는 입력 설계`라는 사실입니다. Chain-of-thought, self-consistency, automatic prompt optimization은 프롬프트 층에서 중간 단계나 후보 비교를 더 다루는 전략이고, 프롬프트의 한계는 최신 근거와 실제 실행 구조가 필요한 지점에서 드러납니다.

프롬프트는 `마법의 주문`이 아니라, 모델 행동을 관찰하고 조정하기 위한 입력 설계 도구로 읽는 편이 안전합니다.

프롬프트 장은 사전학습, 파인튜닝, 정렬을 본 뒤에 `사용자가 지금 당장 조정할 수 있는 입력 설계`를 읽고, 그 한계를 확인한 뒤 RAG와 도구 사용으로 넘어가는 전환 구간입니다.

| 지금 단계의 초점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| 모델 조정 축 | 현재 모델 반응을 입력 설계로 어디까지 바꿀 수 있을까? | 모델 가중치 조정 |
| 프롬프트의 한계 | 입력 설계만으로는 어디서 막히는가? | 근거 연결, 실행 연결, 평가 구조 |
| 근거와 실행 연결 | 최신 근거와 실제 실행을 어떤 구조로 더 붙일까? | RAG, tool use, agent 구조 |
...
| --- | --- | --- |
| 프롬프트 입력 조정 | 입력을 어떻게 써야 원하는 형식과 범위를 더 잘 끌어낼 수 있는가? | P6-10.1, P6-10.2 |
| RAG 근거 연결 | 프롬프트만으로 부족하면 무엇을 근거로 답하게 할 것인가? | P6-11.1, P6-11.2 |
| tool use / agent 실행 구조 | 문서만으로 부족하면 무엇을 실행하고 어떤 순서로 이어 갈 것인가? | P6-13.1, P6-13.2, P6-14.1, P6-14.2 |

가장 짧게 붙잡아야 할 구조는 `프롬프트 입력 조정 -> RAG 근거 연결 -> tool use/agent 실행 구조`입니다. 프롬프트는 이 흐름에서 `입력 조정`을 맡고, 그 한계가 보이면 근거 연결과 실행 구조를 따로 붙여야 합니다.
