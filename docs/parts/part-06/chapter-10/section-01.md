...
RAG의 실제 결합 흐름은 바로 다음 P6-10.2 검색 결과와 생성의 결합에서 다시 회수하고, 검색 저장 구조와 인덱스 문제는 P6-11.1 벡터 데이터베이스, P6-11.2 인덱스와 검색 품질에서 이어집니다. 장기 메모리 시스템 전체 설계는 현재 본편 범위 밖으로 둡니다.

이 절에서는 RAG를 유행어가 아니라 `외부 근거 연결 구조`로 설명하고, 왜 LLM 서비스에서 매우 자주 등장하는지 바로 연결해 봅니다.

지금 읽는 층위는 `서비스 근거 연결 층위`입니다. 앞 절의 프롬프트와 지시 조정이 `모델이 어떻게 답하게 할까`를 다뤘다면, 여기서는 `무엇을 근거로 답하게 할까`로 질문이 바뀝니다. 아직 실행 구조까지는 가지 않고, 뒤의 P6-12부터 `문서를 읽는 것`을 넘어 `기능을 호출하는 것`으로 넘어갑니다.
...
RAG는 Part 6 본류의 `근거 연결 손잡이`이고, 그다음에는 무엇이 더 붙는지까지 이어집니다.

| 지금 단계의 손잡이 | 바로 다음에 이어질 질문 | 뒤에서 본격적으로 다시 읽는 위치 |
| --- | --- | --- |
| 프롬프트 입력 조정 | 같은 재료로 답하는 방식을 어떻게 조정할까? | P6-9.1, P6-9.2 |
| RAG 근거 연결 | 답하기 전에 어떤 문서를 먼저 붙일까? | P6-10.1, P6-10.2 |
| tool use 실행 연결 | 문서를 읽는 것만으로 부족하면 무엇을 실제로 조회하거나 실행할까? | P6-12.1, P6-12.2 |
...
- 임베딩과 벡터 검색이 P6-11.1 벡터 데이터베이스, P6-11.2 인덱스와 검색 품질에서 어떻게 다시 확장되고, 외부 기능 연결이 P6-12.1 도구 사용으로 어떻게 이어지는지 연결하며
