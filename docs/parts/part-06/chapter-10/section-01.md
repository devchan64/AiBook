...
| 문장을 더 정교하게 써도 계산·조회·실행 성공까지는 불안하다 | 지금 막히는 곳은 입력 설계가 아니라 실행 구조인가? | 프롬프트 층에서 먼저 풀 수 있는 문제와 tool/RAG로 넘어가야 하는 문제를 초반부터 분리해야 하기 때문입니다. |

이 표를 기준으로 삼으면, 프롬프트 엔지니어링을 `좋은 문장 요령`보다 `입력 설계에서 먼저 풀 문제와 구조를 바꿔야 할 문제를 가르는 첫 제어 지점`으로 더 직접 읽을 수 있습니다.
