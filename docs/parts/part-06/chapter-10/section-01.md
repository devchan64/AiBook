...
P6-9.2에서는 프롬프트만으로는 최신성, 근거 보장, 실행 가능성 같은 문제를 해결하기 어렵다는 점을 보았습니다. 그러면 답변 문장 자체보다, 답변에 들어갈 재료를 먼저 어떻게 바꿀지가 중요해집니다.

모델 기억에만 의존하지 않고, 외부 근거를 함께 쓰려면 어떻게 해야 하는가?

Part 6에서 `검색 증강 생성(retrieval-augmented generation, RAG)`, `외부 근거를 붙여 생성하는 구조`, `모델 기억과 검색 근거의 역할 분리`에 대한 첫 상세 설명은 이 절에서 잡습니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 근거 결합 생성의 기본 뜻은 이 절과 개념사전을 기준으로 다시 연결합니다.
...
이번 절은 `답의 출발점을 모델 기억에서 외부 문서 근거로 바꾸는 구조`까지를 먼저 닫습니다. RAG의 실제 결합 흐름은 바로 다음 P6-10.2 검색 결과와 생성의 결합에서 다시 회수하고, 검색 저장 구조와 인덱스 문제는 P6-11.1 벡터 데이터베이스, P6-11.2 인덱스와 검색 품질에서 이어집니다.

이 절에서는 RAG를 유행어가 아니라 `외부 근거 연결 구조`로 설명하고, 왜 LLM 서비스에서 매우 자주 등장하는지 바로 연결해 봅니다. 앞 절의 프롬프트와 지시 조정이 `모델이 어떻게 답하게 할까`를 다뤘다면, 여기서는 `무엇을 근거로 답하게 할까`로 질문이 바뀝니다. 아직 실행 구조까지는 가지 않고, 뒤의 P6-12부터 `문서를 읽는 것`을 넘어 `기능을 호출하는 것`으로 넘어갑니다.
...
후반 검색·실행 구조로 넘어가기 전에, 지금 장의 선택 기준을 먼저 고정하면 다음과 같습니다.

| 지금 부족한 것 | 이번 장이 붙이는 구조 | 아직 남는 문제 | 바로 이어지는 위치 |
| --- | --- | --- | --- |
| 모델 기억만으로는 최신 문서, 내부 문서, 현재 버전 근거를 안정적으로 답에 반영하기 어렵다 | 답변 전에 관련 문서를 먼저 찾고 붙이는 근거 연결 구조 | 붙인 문서가 실제 입력에서 어디에 결합되는지, 그 문서를 어떤 저장 구조와 탐색 방식으로 다시 꺼낼지는 아직 남아 있다 | P6-10.2, P6-11.1, P6-11.2 |
...
초심자가 여기서 가장 먼저 던져야 할 질문도 이 순서를 따라갑니다. `답이 어색한가`보다 먼저 `같은 재료를 더 잘 말하면 되는가`, 아니면 `답하기 전에 재료 자체를 현재 문서로 다시 가져와야 하는가`를 가르는 편이 맞습니다.

따라서 이 절에서 먼저 붙잡아야 할 변화는 `질문을 더 잘 쓰는가`보다 `답하기 전에 어떤 문서를 먼저 연결하는가`입니다.
...
이 절은 Part 6에서 `검색 증강 생성(retrieval-augmented generation, RAG)`을 대표로 설명하는 Section입니다. `프롬프트에 문서를 길게 붙이는 요령`이라는 인상을 `답의 출발점을 외부 근거 문서로 바꾸는 구조`로 바꾸는 기준선을 여기서 세웁니다.

즉, 지금 장의 핵심은 `말을 더 잘 시키는가`에서 `답의 재료를 현재 문서로 다시 고르는가`로 넘어가는 데 있습니다. 여기서는 어떤 문서를 먼저 붙일지를 보고, 뒤 절들에서는 그 문서를 어떤 결합 흐름과 저장 구조로 찾을지, 그리고 나중에 문서를 읽는 것만으로 부족할 때 무엇을 실제로 조회하거나 실행할지로 확장됩니다.
...
| 먼저 구분할 장면 | 이 절에서 먼저 잡을 판단 | 왜 여기서 먼저 갈라야 하는가 |
| --- | --- | --- |
...
| --- | --- | --- |
| 사전학습용 데이터 준비 | 모델이 넓은 언어 패턴을 배우게 하는가 | P6-6.1, P6-6.2 |
...
질문이 `출장비 정산 기준이 어떻게 바뀌었나요?`라면, 초심자는 먼저 기억나는 공지나 지난번 기준으로 답하려고 하기 쉽습니다. 내부 정책도 결국 사람이 읽는 문서이니, 모델이 일반 지식처럼 알고 있을 것이라고 기대하기 쉽기 때문입니다. 하지만 내부 정책은 자주 개정되고, 예전 기준과 오늘 기준이 다를 수 있어서 기억 의존 방식은 바로 잘못된 안내로 이어지기 쉽습니다. 예를 들어 작년에는 교통비 상한이 없었는데 올해부터 상한이 생겼다면, 말투가 자연스러워도 답 자체는 틀립니다. 더 위험한 점은 이런 답이 조직 안에서 `공식 안내처럼 보이는 문장`으로 복제되기 쉽다는 데 있습니다.

RAG는 최신 사내 정책 문서를 먼저 검색해 현재 효력이 있는 조항을 찾고, 그 문단을 문맥에 붙인 뒤 답을 만들게 합니다. 여기서 구조적으로 바뀌는 점은 `기억나는 답을 꺼낸다`에서 `현재 유효한 문서를 먼저 확인한다`로 출발점이 이동하는 것입니다. 초심자가 여기서 넘어가야 할 오해는 `자연스럽게 설명하면 일단 충분하다`는 기대입니다. 그래서 이 사례에서 확인해야 할 결과는 자연스러운 설명보다 먼저, 실제 최신 정책 문단이 답의 근거로 붙는가, 그리고 그 문단만 보고도 현재 효력 기준을 다시 확인할 수 있는가입니다.

### 사례 2. 제품 매뉴얼 기반 지원

제품 사용법을 묻는 고객 지원 챗봇을 생각해 볼 수 있습니다. 초심자는 FAQ 몇 개와 자주 쓰는 답변 템플릿만 잘 정리해 두면 기본 질문은 충분히 처리된다고 느끼기 쉽습니다. 고객 질문도 반복되는 편이니, 한 번 만든 답변을 계속 재사용해도 큰 문제는 없을 것처럼 보이기 때문입니다. 하지만 메뉴 이름과 설정 위치는 버전이 바뀔 때마다 달라질 수 있어서, 템플릿은 자연스러워도 내용은 곧바로 낡을 수 있습니다. 예를 들어 예전 버전의 `고급 설정` 메뉴가 현재 버전에서는 `환경설정`으로 옮겨졌다면, 기억 기반 답변은 고객을 잘못된 화면으로 보내게 됩니다. 이때 사용자는 `답은 친절했는데 왜 실제 화면과 다르지?`라는 실패를 겪게 됩니다.

RAG는 최신 매뉴얼과 FAQ에서 관련 문서를 먼저 찾아 현재 버전 설명을 붙인 뒤 답을 구성하게 만듭니다. 그 결과 답변 품질은 말투보다 먼저 `현재 문서와의 정합성`으로 관리할 수 있게 됩니다. 여기서 초심자가 넘어가야 할 오해는 `자주 묻는 질문이면 기억 기반 템플릿으로도 충분하다`는 감각입니다. 그래서 이 사례에서 확인해야 할 결과는 템플릿이 자연스러운가보다, 현재 버전 메뉴와 절차가 실제 문서 기준으로 맞는가, 그리고 답의 각 단계가 실제 화면 경로와도 대응되는가입니다.

### 사례 3. 개발 문서 검색

개발자가 `현재 SDK 버전에서 인증 헤더를 어디에 넣나요?`라고 묻는 장면을 떠올려 볼 수 있습니다. 초심자는 모델이 일반적인 API 지식을 많이 알고 있으니 바로 답해도 될 것이라고 생각하기 쉽습니다. 문법 질문은 검색보다 기억된 예제가 더 빠를 것처럼 보이기 때문입니다. 하지만 예전 버전 문법을 기억하고 있으면 얼핏 그럴듯한 답이라도 실제 코드에서는 즉시 오류로 이어질 수 있습니다. 예를 들어 과거 버전의 `Authorization` 예제를 그대로 답했는데 현재 버전은 `auth` 객체를 따로 넘기게 바뀌었다면, 복사해 넣은 코드가 바로 실패합니다. 이때 실패는 단순 오답이 아니라 디버깅 시간, 신뢰 하락, 잘못된 샘플 코드 확산으로 이어집니다.

이때 먼저 확인해야 하는 것은 모델의 일반 지식이 아니라 `지금 쓰는 버전 문서`입니다. RAG는 현재 API 문서와 예제 페이지를 먼저 찾아 문맥에 붙인 뒤 답을 만들게 해 이런 버전 불일치 위험을 줄입니다. 여기서 핵심은 생성 문장의 유창함보다 검색 단계가 현재 문서를 정확히 집어오는가에 있습니다. 초심자가 넘어가야 할 오해는 `그럴듯한 코드면 일단 복사해서 써 볼 수 있다`는 태도입니다. 그래서 이 사례에서 확인해야 할 결과는 답이 그럴듯한가보다, 실제 현재 SDK 문서와 코드 예제가 함께 근거로 붙는가, 그리고 그 근거만 따라가도 같은 코드를 재현할 수 있는가입니다.
...
RAG를 처음 읽을 때 초심자가 가장 자주 헷갈리는 지점은 `답이 틀렸다`는 사실만 보고도 곧바로 프롬프트를 더 길게 고치려는 점입니다. 하지만 이 절의 세 사례에서 먼저 봐야 하는 것은 문장 표현보다 `답하기 전에 현재 문서를 실제로 붙였는가`입니다.
...
이 절에서 먼저 익혀야 하는 기준은 단순합니다. RAG는 `질문을 더 잘 쓰는 요령`이 아니라, `답하기 전에 무엇을 근거로 붙일지`를 시스템 단계에서 먼저 고정하는 구조입니다.
...
| --- | --- | --- |
| 왜 답변 형식은 괜찮아 보여도 최신 정책은 자주 틀릴까? | 입력 조정 문제가 아니라 최신 근거 문서를 먼저 붙여야 하는 문제이기 때문입니다. | 검색 결과와 생성 결합 |
| 왜 파인튜닝보다 문서 연결을 먼저 검토해야 할 때가 있을까? | 지금 필요한 것이 반응 성향 조정보다 현재 문서 근거이기 때문입니다. | RAG와 파인튜닝의 역할 분리 |
| 왜 `프롬프트에 문서를 더 길게 붙인다`는 말로는 부족할까? | RAG는 문서가 있느냐보다 답의 출발점을 외부 근거로 바꾸는 구조이기 때문입니다. | retrieval 저장소와 인덱스 구조 |
