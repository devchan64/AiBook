...
이 전환은 `같은 재료로 답하는 방식을 어떻게 조정할까`에서 시작해, `잘 물어도 아직 남는 문제는 무엇일까`를 거쳐, `그렇다면 무엇을 근거로 답하게 할까`로 넘어갑니다. 프롬프트 입력 조정은 모델 내부 조정이 아니고, RAG 근거 연결은 검색 품질과 생성 품질을 함께 보게 만드는 별도 구조입니다.
