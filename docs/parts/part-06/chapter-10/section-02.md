...
프롬프트 엔지니어링을 과대평가하지 않으려면, `어떻게 물을까`에서 `잘 물어도 아직 남는 문제는 무엇인가`로 질문을 옮겨야 합니다. 이 질문은 곧 `그렇다면 무엇을 근거로 답하게 할까`로 이어지며, RAG는 프롬프트 확장판이 아니라 근거 연결 구조라는 점을 잡게 해 줍니다.

즉, 여기서 붙잡아야 할 전환은 `말하는 방식 조정`에서 `답의 근거 연결`로 중심이 옮겨 간다는 점입니다. 이 구분이 보이면 P6-11의 RAG도 프롬프트를 더 길게 쓰는 요령이 아니라, 답의 출발점을 바꾸는 구조로 읽힙니다.

이 전환은 다음처럼 읽는 편이 가장 안전합니다.

| 지금 단계의 초점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| 프롬프트 입력 조정 | 같은 재료로 답하는 방식을 어떻게 조정할까? | 모델 내부 조정 |
| 프롬프트 한계 확인 | 잘 물어도 아직 남는 문제는 무엇일까? | 최신 근거, 실행, 반복 평가 |
| RAG 근거 연결 | 그렇다면 무엇을 근거로 답하게 할까? | 검색 품질과 생성 품질의 분리 |
...
이 표를 기준으로 삼으면, 프롬프트의 한계를 `더 강한 문장 요령`보다 `입력 조정으로 줄일 문제와 구조를 바꿔야 할 문제를 가르는 기준`으로 더 직접 읽을 수 있습니다.
