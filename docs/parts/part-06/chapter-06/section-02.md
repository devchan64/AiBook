...
Part 6에서 `스케일(scale)`, `데이터·모델·계산량의 동시 확대`, `성능 향상과 비용 증가의 동반 관계`에 대한 첫 상세 설명은 이 절에서 잡습니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 규모 확대의 기본 뜻은 이 절과 개념사전을 기준으로 다시 연결합니다.
...
이 절은 Part 6에서 `스케일(scale)`을 대표로 설명하는 Section입니다. `큰 모델이 더 낫다`는 단순 인상을 `데이터, 파라미터, 계산량이 함께 커지며 성능 가능성과 비용 부담이 같이 커지는 구조`로 바꾸는 기준선을 여기서 세웁니다.
...
| 먼저 떠올릴 질문 | 스케일 관점이 먼저 필요한 이유 | 바로 다음 절이나 뒤 장에서 이어질 것 |
| --- | --- | --- |
| 왜 더 큰 모델이 더 잘하는 것처럼 보이면서도 운영은 더 어려워질까? | 데이터, 모델, 계산량이 함께 커지며 성능 가능성과 비용 부담이 동시에 커지기 때문입니다. | 파인튜닝, 운영 제약, 비용 판단 |
| 긴 입력을 더 잘 다루는 것이 왜 공짜가 아닐까? | 더 넓은 문맥 처리 능력은 보통 더 큰 context, 계산, 지연 시간 증가와 함께 오기 때문입니다. | context 관리, 서비스 설계 |
| 데이터가 많아지면 왜 품질 문제가 자동으로 해결되지 않을까? | 스케일은 양을 키우지만 오래된 정보, 편향, 저작권, 사실 오류 같은 질 문제는 별도로 남기 때문입니다. | 평가, 검증, 정책 책임 |
