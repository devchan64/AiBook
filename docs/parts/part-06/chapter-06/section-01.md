...
이 질문부터는 계산 구조만으로는 충분하지 않습니다. 이제는 `무엇을 먼저 배우고`, `얼마나 큰 규모로 배우고`, `그 위에 어떤 후속 조정을 얹는가`를 함께 읽어야 합니다. Chapter 6은 바로 그 학습 축의 출발점입니다.

P6-4.2에서는 대화형 LLM이 단순 자동완성 모델 위에 지시 따르기, 안전 조정, 인터페이스 층이 더해진 사용자 경험이라는 점을 보았습니다. 여기서는 그보다 한 단계 아래로 내려가, 그런 조정이 얹히기 전에 모델이 먼저 어떤 기반을 가져야 하는지부터 다시 봅니다.
...
이 절에서는 사전학습을 `지식을 통째로 저장하는 마법 같은 과정`으로 보지 않고, LLM이 왜 먼저 대규모 일반 패턴을 배우는지 설명합니다. 앞 절들이 `어떻게 생성하는가`를 설명했다면, 여기서는 그 생성 구조가 이후 파인튜닝과 지시 튜닝으로 이어지기 전에 어떤 기반을 먼저 필요로 하는지의 첫 단계를 잡습니다.

따라서 이 절에서 먼저 붙잡아야 할 것은 `이미 똑똑한 모델`이라는 인상보다 `먼저 넓은 언어 기반을 배우고 나중에 목적별 조정을 얹는 구조`입니다.

## 왜 생성 설명 다음에 학습 축을 다시 읽는가

생성 구조를 이해한 직후 초심자가 가장 자주 하는 오해는 `그럼 이제 모델은 구조만 같으면 비슷하게 반응하겠네`라고 느끼는 일입니다. 하지만 실제로는 그렇지 않습니다. 같은 Transformer 구조를 써도 무엇을 먼저 얼마나 넓게 배웠는지에 따라 기본 반응 범위가 크게 달라지고, 그 위에 어떤 조정을 더 얹었는지에 따라 사용자 경험도 다시 달라집니다.

그래서 Chapter 6은 `생성 원리 설명이 끝난 뒤 갑자기 딴이야기`가 아닙니다. 오히려 `왜 생성 원리만으로는 실제 모델 차이가 안 닫히는가`에 답하는 첫 장입니다.

이 장의 흐름은 다음처럼 읽으면 됩니다.

| 지금 답하려는 질문 | 이 절에서 먼저 잡는 것 | 바로 다음 절에서 이어지는 것 |
| --- | --- | --- |
...
즉, 이 절이 닫혀야 다음 절의 `스케일(scale)`도 단순한 크기 경쟁이 아니라 `왜 넓은 기반을 그렇게 큰 규모로 만들려 하는가`의 문제로 읽힙니다.
...
이 기준이 보여야 P6-6.2의 스케일 설명도 자연스럽게 이어집니다. `왜 굳이 그렇게 큰 데이터와 계산을 쓰는가`라는 질문은, 바로 지금 보고 있는 `넓은 기반을 먼저 만드는 단계`가 실제로 얼마나 넓고 무거운 준비 단계인지와 이어지기 때문입니다.
...
| --- | --- | --- |
| 요약 예시는 조금 있는데 문장 연결과 핵심 압축 자체가 자주 흔들린다 | 넓은 기반 학습 층위 | 지금 부족한 것은 과업 규칙보다 언어 기반 자체인가 |
| 라벨 정의는 분명한데 표현이 다른 같은 문의를 자주 놓친다 | 기반 위 과업 조정 층위 | 사전학습 표현 위에서 파인튜닝을 더 해야 하는가 |
| 기본 설명은 되는데 `세 문장으로`, `차분하게`, `단계별로` 같은 요청 형식을 자주 어긴다 | 요청 형식 조정 층위 | 지금 막히는 것은 기반 학습보다 instruction tuning 쪽 문제인가 |

이 표의 목적은 원인을 한 번에 확정하는 데 있지 않습니다. 초심자가 `학습이 더 필요하다`는 한 문장으로 뭉개지 않고, 지금 보고 있는 현상이 `넓은 기반`, `과업 조정`, `요청 형식 조정` 중 어디에서 먼저 드러나는지 짧게 가르는 데 있습니다.
