...
사전학습 목표 자체는 여기서 잡고, 규모와 계산 제약은 바로 다음 P6-6.2 데이터와 스케일에서 다시 이어집니다. 지시 튜닝과 후속 정렬 조정은 P6-8.1 지시 튜닝과 P6-8.2 정렬의 기본 문제에서 다시 회수하며, 분산 학습 인프라의 구현 세부는 현재 본편 범위 밖으로 둡니다.
