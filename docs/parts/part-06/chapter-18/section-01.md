...
이 절은 그 흐름을 사건 이름 나열이 아니라 `어떤 한계를 줄이려다 다음 구조가 나왔는가`라는 기준으로 다시 묶습니다. 초심자에게 여기서 더 필요한 것은 장의 위치 설명보다 `왜 n-gram 다음에 임베딩이 나오고, 왜 RNN 다음에 attention과 Transformer가 나왔는가`를 먼저 읽는 기준입니다.
...
발전사를 처음 읽을 때 초심자가 가장 자주 놓치는 것은 사건 이름을 순서대로 외우는 데 집중하고, `왜 다음 구조가 필요해졌는가`를 놓치는 점입니다. 하지만 이 절에서 먼저 붙잡아야 하는 것은 연도 암기보다 `무슨 한계가 다음 전환을 밀어냈는가`입니다.
...
- 다음 절에서는 이 큰 흐름 안에서 무엇이 직접 계보고 무엇이 주변 근거인지 더 좁혀 볼 수 있어야 합니다.
