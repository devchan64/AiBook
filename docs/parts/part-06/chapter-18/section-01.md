...
| --- | --- | --- |
| 입력 문장 자체만 바꾸면 되는 요청처럼 보임 | 필요한 정보가 이미 입력 안에 다 있는가 | 이미 닫히는 작업에 retrieval이나 tool을 붙이면 답이 산만해지고 흐름만 무거워질 수 있기 때문입니다. |
| 최신 규정이나 근거 문서가 중요해 보임 | 현재 문서를 먼저 찾아야 하는가 | 말투보다 최신 근거 확보가 먼저 실패를 막는 핵심이기 때문입니다. |
| 규정 설명이 아니라 현재 잔액·상태·승인 여부를 묻고 있음 | 실제 시스템 값을 조회해야 하는가 | 문서 설명만으로는 현재 상태 답을 닫을 수 없기 때문입니다. |
| 문서는 찾았지만 예외가 많거나 근거가 불충분해 확답이 불안함 | 사람 검토나 추가 질문으로 넘겨야 하는가 | 자연스러운 일반론으로 덮으면 `답은 했지만 닫히지 않은 요청`을 숨기게 되기 때문입니다. |
