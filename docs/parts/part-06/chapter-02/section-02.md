...
이 절의 중심은 `가까운 후보`와 `맞는 근거`를 구분하는 데 있습니다. 거리(distance)와 유사도(similarity)는 후보를 먼저 좁히는 비교 기준이지, 그 후보가 곧 정답이거나 최신 근거라는 보장은 아닙니다.
...
이 절에서는 의미와 거리를 `임베딩 벡터를 어떤 비교 기준으로 가까운 후보로 읽을 것인가`라는 질문으로 먼저 닫고, 검색과 추천의 기본 비교 기준을 붙잡는 데 집중합니다. 임베딩 학습 흐름은 P6-2.3에서, 빠른 후보 탐색은 P6-2.4에서, 검색 시스템 안의 실제 사용은 뒤의 P6-11에서 다시 붙잡습니다.

이 절에서는 수식을 외우기보다, `임베딩 공간의 거리`를 실제 비교와 검색의 언어로 읽습니다.

앞 절의 임베딩 설명이 `표현을 벡터로 바꾸는가`를 다뤘다면, 여기서는 그렇게 만든 벡터를 어떤 기준으로 서로 가깝거나 멀다고 읽을지 다룹니다. 바로 뒤의 Transformer, RAG, 벡터 데이터베이스 절에서는 이 비교 기준이 실제 생성 모델 내부 계산과 검색 후보 선정에 어떻게 쓰이는지로 질문이 더 커집니다.

따라서 이 절에서 먼저 붙잡아야 할 것은 `벡터를 만들었다`가 아니라 `그 벡터를 어떤 기준으로 비교해야 하는가`입니다.
...
즉, 지금 장의 핵심은 `벡터를 만든다`에서 `그 벡터를 어떤 후보 비교 기준으로 읽는다`로 넘어가는 데 있습니다.
...
이 절은 구조적으로 작은 절처럼 보여도, 바로 앞의 P6-2.1 임베딩을 `벡터를 만든다`에서 `벡터를 비교한다`로 확장하고, 뒤에서 나올 검색과 외부 지식 연결을 이해하는 핵심 기초입니다.
...
사용자가 도움말 창에 `프롬프트만으로 거짓말을 막을 수 있나요?`라고 묻는 장면을 떠올려 보겠습니다. 초심자는 보통 질문 속 단어가 문서 제목에 그대로 있어야 찾기 쉽다고 생각해 `거짓말`이나 `막다` 같은 단어를 먼저 찾습니다. 하지만 실제 지식베이스에는 `프롬프트의 한계와 사실성 보강 방법`처럼 더 기술적인 표현으로 정리된 문서만 있을 수 있습니다. 이때 키워드만 맞추면 관련 문서를 놓치고, 사용자는 `문서가 없나 보다`라고 오해할 수 있습니다.

여기서 바뀌는 점은 단어 일치 여부를 넘어서, 두 문장이 실제로 같은 문제 장면을 가리키는지까지 비교하게 된다는 것입니다. 유사도 검색은 `거짓말을 막는다`와 `사실성을 보강한다`를 꽤 가까운 문제로 보고 그 문서를 후보로 올립니다. 여기서 바로잡아야 할 오해는 `같은 단어가 없으면 같은 질문도 아니다`라는 감각입니다. 그래서 이 사례에서 확인해야 할 결과는 질문 단어가 그대로 없더라도 같은 문제를 다루는 문서가 실제 후보로 올라오는가, 그리고 후보만 봐도 왜 그 문서가 같은 장면으로 묶였는지 설명할 수 있는가입니다.

이 사례를 단계로 닫으면 다음과 같습니다.

| 사례 단계 | 이 장면에서의 판단 | 의미와 거리 관점에서 확인할 결과 |
| --- | --- | --- |
| 사람이 먼저 보는 기준 | 같은 단어가 있어야 같은 질문이라고 본다 | `거짓말`, `막다` 같은 키워드에 먼저 끌린다 |
| 기준의 한계 | 같은 문제라도 문서에서는 다른 표현을 쓸 수 있다 | `사실성 보강` 문서가 키워드 기준에서는 밀릴 수 있다 |
| 거리 비교가 바꾸는 판단 | 단어 일치보다 같은 문제 장면을 가까운 후보로 본다 | `프롬프트만으로 충분한가` 계열 문서가 후보가 된다 |
| 확인할 결과 | 가까운 후보는 정답 확정이 아니라 먼저 열어 볼 후보이다 | 후보 본문과 근거를 다시 확인해야 한다 |

### 사례 2. 문서 검색

사내 정책 문서 수백 개 중에서 `출장비 정산 마감일이 언제인가요?`를 묻는 상황을 생각해 보겠습니다. 초심자가 먼저 쓰는 기준은 대개 제목입니다. `출장비`, `정산`, `마감일`이 제목에 모두 들어 있으면 바로 답이 나올 것처럼 느껴집니다. 하지만 실제 문서 구조에서는 제목이 `출장 운영 가이드`이고, 본문 중간 표에만 `매월 5영업일 이내 제출`과 `해외 출장 예외`가 들어 있을 수 있습니다. 제목만 맞는 문서 하나를 열어도 정작 핵심 문단을 놓치면 답은 여전히 늦거나 틀릴 수 있습니다.

여기서 바뀌는 점은 문서 제목 하나를 고르는 것에서 끝나지 않고, 질문과 가장 가까운 문단 몇 개를 먼저 모으는 쪽으로 탐색 단위가 내려간다는 것입니다. 유사도 검색은 질문과 가까운 문단 몇 개를 먼저 후보로 모으고, 그다음 LLM이 그 문단을 읽어 자연어 답변을 정리하게 만듭니다. 여기서 바로잡아야 할 오해는 `제목이 맞으면 문서도 곧 답이다`라는 기대입니다. 그래서 이 사례에서 확인해야 할 결과는 제목만 맞는 문서 하나가 아니라, 실제 마감일이 적힌 핵심 문단이 후보에 포함되는가, 그리고 예외 조항이 있는 문단도 함께 올라오는가입니다.

이 사례를 단계로 닫으면 다음과 같습니다.

| 사례 단계 | 이 장면에서의 판단 | 의미와 거리 관점에서 확인할 결과 |
| --- | --- | --- |
| 사람이 먼저 보는 기준 | 제목이 맞으면 문서도 답이라고 본다 | `출장비`, `정산`, `마감일` 제목을 먼저 찾는다 |
| 기준의 한계 | 답은 제목이 아니라 본문 중간 문단에 있을 수 있다 | 마감일과 예외 조항이 다른 문단에 숨어 있을 수 있다 |
| 거리 비교가 바꾸는 판단 | 문서 전체보다 질문과 가까운 문단 후보를 먼저 모은다 | 마감일 문단과 예외 문단이 top-k 후보에 들어와야 한다 |
| 확인할 결과 | 가까운 문단을 열어 실제 근거인지 확인한다 | 제목 일치와 답 근거 확정을 분리한다 |

### 사례 3. 추천 시스템

사용자가 입문용 선형대수 강의를 끝까지 봤고, 다음 강의를 추천해야 하는 상황을 생각해 보겠습니다. 초심자는 보통 `입문용` 태그가 같으면 비슷한 강의일 것이라고 먼저 묶습니다. 하지만 실제로는 하나는 칠판 수식 설명 위주이고, 다른 하나는 NumPy 실습 위주라서 학습 리듬이 꽤 다를 수 있습니다. 예를 들어 수식 설명 영상을 끝까지 본 사용자가 바로 코드 실습 중심 강의로 넘어가면 중도 이탈이 늘 수 있습니다. 즉, 같은 태그는 비슷한 소비 경험을 보장하지 않습니다.

여기서 바뀌는 점은 태그 하나를 맞추는 것보다, 실제로 어떤 강의를 어떤 흐름으로 소비했는지를 함께 보게 된다는 것입니다. 시청 행동과 강의 특징을 함께 반영한 벡터 공간에서 가까운 항목을 찾으면, 단순 태그보다 실제 학습 흐름이 비슷한 후보를 더 자연스럽게 고를 수 있습니다. 여기서 바로잡아야 할 오해는 `라벨이 같으면 경험도 같다`는 감각입니다. 그래서 이 사례에서 확인해야 할 결과는 같은 태그보다 실제 학습 리듬이 비슷한 후보가 더 앞쪽에 모이는가, 그리고 그 후보 선택이 다음 학습 이탈 가능성까지 줄이는가입니다.

이 사례를 단계로 닫으면 다음과 같습니다.

| 사례 단계 | 이 장면에서의 판단 | 의미와 거리 관점에서 확인할 결과 |
| --- | --- | --- |
| 사람이 먼저 보는 기준 | 같은 태그면 비슷한 강의라고 본다 | `입문용` 라벨에 먼저 끌린다 |
| 기준의 한계 | 같은 태그라도 수식 중심과 실습 중심은 경험이 다르다 | 라벨만으로 학습 리듬을 설명하기 어렵다 |
| 거리 비교가 바꾸는 판단 | 시청 행동과 강의 특징이 가까운 후보를 먼저 찾는다 | 실제 소비 흐름이 비슷한 항목이 앞쪽에 모인다 |
| 확인할 결과 | 추천 후보는 최종 선택이 아니라 후속 필터의 입력이다 | 난이도, 목표, 최신성 같은 조건을 다시 본다 |
...
이 절을 읽은 뒤에는 아직 거리 함수를 자세히 몰라도, 아래처럼 `가까운 후보를 고르는 일`과 `최종 정답을 확정하는 일`을 먼저 갈라보는 연습을 할 수 있습니다.
...
| --- | --- | --- |
| 거리 기준 1등과 최종 선택 비교 | 둘이 같지 않을 수 있다 | 후보 선정과 최종 확정이 서로 다른 단계라는 점을 먼저 잡아야 하기 때문입니다. |
| `doc_A`와 `doc_C` 비교 | 둘 다 가까운 후보로 올라오지만 최신성 정보가 판단을 바꿀 수 있다 | 벡터 거리 하나만으로는 실무 답변을 닫지 못하기 때문입니다. |
| `doc_B` 비교 | 거리가 멀면 우선순위가 뒤로 밀린다 | 유사도 비교가 무엇을 후보에서 먼저 제외하는지 읽기 위해서입니다. |
...
rank 3: doc_B, distance = 1.0, updated_at = 2025-12, note = 다른 주제
...
같은 결과를 더 짧은 작업 순서로 다시 줄이면 다음처럼 볼 수 있습니다.

| 지금 보이는 신호 | 아직 하고 있는 일 | 바로 다음에 붙어야 하는 일 |
| --- | --- | --- |
| 거리 기준 1등이 보인다 | 가까운 후보를 먼저 세우는 단계 | 본문, 최신성, 예외 조항을 실제로 열어 확인 |
| top-k 안에 비슷한 후보가 여러 개 있다 | 검토할 후보 묶음을 좁히는 단계 | 후보끼리 무엇이 더 최신이고 더 직접 근거인지 비교 |
| 멀리 있는 후보가 뒤로 밀린다 | 우선순위를 낮추는 단계 | 정말 후보군 밖으로 빼도 되는지 최소 확인 |

이 표의 목적은 절차를 더 많이 외우게 하는 데 있지 않습니다. 초심자가 `가까운 후보가 나왔다`는 순간 바로 답을 확정하지 않고, 지금이 `후보 선정`, `후보 검토`, `근거 확정` 중 어느 단계인지 먼저 가르게 만드는 데 있습니다.
...
| 장면 | 먼저 답해야 하는 질문 |
| --- | --- |
| 거리 기준 1등 후보가 보인다 | 이 후보는 1차 후보인가, 최종 정답인가 |
| 2등 후보가 더 최신이다 | top-k 후보를 실제로 다시 검토했는가 |
| 질문과 비슷한 문서가 올라왔다 | 관련성 외에 사실 일치와 예외 조건도 확인했는가 |

즉, 이 절의 닫힘은 `가깝다`를 감각적으로 이해하는 데만 있지 않고, `가까운 후보`, `검토할 후보 묶음`, `최종 근거`를 각각 다른 단계로 읽을 수 있게 되는 데 있습니다.

이 축약된 예제는 `가까운 벡터를 찾는다`는 말이 실제 서비스에서는 `답을 바로 확정한다`가 아니라 `먼저 검토할 후보를 순서대로 좁힌다`는 뜻임을 다시 보여 줍니다. 예를 들어 `doc_A`가 가장 가깝게 나왔더라도, 실제 문단을 열어 보면 지난 분기 기준만 담겨 있고 `doc_C`에 최신 예외 조건이 들어 있을 수 있습니다. 그래서 이후 검색, RAG, 추천 절을 읽을 때도 핵심은 거리 계산 그 자체보다 `무엇을 후보로 올리고, 그다음 어떤 단계로 검토하는가`에 있습니다.

### 연습 2. 후보 선정과 최종 확정을 다시 가르면

다음 장면마다 `거리 계산`, `문서 본문 검토`, `최신성 확인`, `추가 검색` 중 무엇을 먼저 해야 하는지 고르고 이유를 한 문장으로 적어 보세요.

1. top-1 문서는 질문과 가장 가깝지만 작년 정책만 담고 있다.
2. top-3 후보는 모두 비슷하게 가까운데, 하나만 최신 공지 링크를 포함하고 있다.
3. top-k 후보 전체가 질문과 거리가 멀고 핵심 단어도 거의 겹치지 않는다.
4. top-1 후보는 그럴듯하지만 예외 조항 언급이 전혀 없다.

해설: 1번은 최신성 확인이 먼저입니다. 가까운 후보라도 작년 정책만 담고 있으면 최종 근거가 될 수 없습니다. 2번은 문서 본문 검토와 최신성 확인을 함께 해야 합니다. top-k가 모두 가깝다면 거리 순서만 보지 말고 실제 공지 링크와 근거 문단을 열어 봐야 합니다. 3번은 추가 검색이 먼저입니다. 후보 전체가 멀고 핵심 단어도 겹치지 않으면 현재 검색 질의나 인덱스가 맞지 않을 수 있습니다. 4번은 문서 본문 검토가 먼저입니다. top-1 후보가 그럴듯해도 예외 조항이 없으면 답을 확정하기 어렵습니다. 네 장면 모두 핵심은 `가까운 후보 선정`과 `최종 근거 확정`을 분리해서 읽는 것입니다.
