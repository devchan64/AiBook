...
## 언제 의미와 거리 비교 관점을 먼저 떠올려야 하는가

| 먼저 떠올릴 질문 | 거리 비교 관점이 먼저 필요한 이유 | 바로 다음 절이나 다음 장에서 이어질 것 |
| --- | --- | --- |
| 비슷한 질문과 문서가 왜 함께 올라오는가? | 벡터 공간에서는 먼저 가까운 후보를 좁혀 보는 단계가 있기 때문입니다. | RAG에서 검색 후보를 고르는 흐름 |
| 가장 가까운 후보가 왜 항상 정답은 아닐까? | 가까움은 우선순위 신호일 뿐이고, 최신성·사실성·맥락 적합성 검토가 더 필요합니다. | 검색 뒤 검증과 생성 단계를 분리해서 읽기 |
| 검색과 추천을 왜 비슷한 언어로 설명할 수 있을까? | 둘 다 `가까운 후보를 찾는 문제`로 일부를 공통화할 수 있기 때문입니다. | 벡터 DB, 필터링, 재정렬로 이어지는 구조 |
...
- 거리와 유사도를 `후보 비교 기준`이라는 말로 설명할 수 있는가?
- 가장 가까운 벡터와 최종 정답을 분리해서 생각할 수 있는가?
- 다음 절과 다음 장을 `후보 선택`과 `최종 검토`의 구분 속에서 읽을 준비가 되었는가?
