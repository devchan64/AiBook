...
이 절은 그 질문에 답합니다. 의미와 거리라는 표현은, 임베딩 공간에서 비슷한 쓰임의 표현을 서로 가까운 후보로 비교하려는 계산 관점을 뜻합니다.
...
이 절에서는 의미와 거리를 `임베딩 벡터를 어떤 비교 기준으로 가까운 후보로 읽을 것인가`라는 질문으로 먼저 닫고, 검색과 추천의 기본 비교 손잡이를 붙잡는 데 집중합니다.
...
여기서는 거리와 유사도를 `후보 비교 손잡이`로 먼저 읽고, 직관 보강은 같은 장의 P6-2.3에서, 검색 시스템 안의 실제 사용은 뒤의 P6-11에서 다시 붙잡습니다.
...
의미와 거리는 Part 6 본류의 `후보 비교 손잡이`로 먼저 잡고, 그 다음에 어떤 시스템 층위가 더 붙는지까지 함께 봐야 합니다.
