...
이번 연습의 목표는 `가까운 후보를 먼저 고른다`는 감각과 `가까움이 곧 정답은 아니다`라는 점을 코드 없이 구분해 보는 것입니다. 아래 수치 예시는 이미 계산된 거리값과 문서 메타데이터를 함께 보여 줍니다.
...
해설: 거리 기준 1등은 `doc_A`이고, 우선순위가 가장 낮은 후보는 `doc_B`입니다. 하지만 이 결과는 최종 답 확정이 아니라 1차 후보 선정입니다. P6-2.2의 중심은 `가깝다`를 정답으로 읽지 않고, 먼저 검토할 후보 순서로 읽는 데 있습니다.
