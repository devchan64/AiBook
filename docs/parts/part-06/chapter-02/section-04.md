...
이 절은 바로 그 질문을 다룹니다. 초심자 기준에서는 `좋은 벡터를 만드는 문제`와 `그 벡터를 빨리 찾는 문제`를 섞지 않는 것만으로도 절반은 정리됩니다.
...
하지만 이 장면에서 확인해야 할 결과는 `작을 때 버틴다`와 `커졌을 때도 버틴다`가 같은 말이 아니라는 점입니다.

### 사례 2. 문서 수가 커지자 갑자기 느려질 때

정책 문서와 FAQ가 수십만 개로 늘어나면, 이전에는 괜찮던 비교 방식이 갑자기 병목이 될 수 있습니다. 이때 초심자가 먼저 넘겨야 할 오해는 `임베딩 품질이 나빠져서 느린가 보다`라는 생각입니다.

실제로는 후보가 맞게 올라와도 비교 비용이 너무 커서 늦어질 수 있습니다. 이 장면은 표현 품질보다 탐색 속도가 먼저 문제인 경우입니다.

### 사례 3. 속도는 빨라졌는데 중요한 문서가 빠질 때

ANN 설정을 더 공격적으로 조정해 속도는 빨라졌는데, 정작 최신 예외 조항이 든 문서가 상위 후보에서 자꾸 빠질 수 있습니다. 이 경우 초심자가 넘어가야 할 오해는 `속도만 좋아졌으니 무조건 개선`이라는 감각입니다.
