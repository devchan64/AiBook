...
이 절에서는 `임베딩(embedding)`, `토큰 ID와 임베딩 벡터의 차이`, `LLM 내부 계산과 검색이 공유하는 표현 기반`을 처음 자세히 설명합니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 벡터 표현의 기본 뜻은 이 절과 개념사전을 기준으로 다시 연결합니다.
...
이 절에서는 임베딩을 `토큰이나 문장을 계산 가능한 벡터 표현으로 바꾸는 기반`으로 먼저 닫고, 토큰 ID와 벡터 표현이 왜 다른지를 붙잡는 데 집중합니다.

대신 이번 절에서 바로 더 넓히지 않을 질문도 분명합니다. 임베딩 계열과 학습 감각을 어떻게 구분해 읽어야 하는지는 같은 장의 P6-2.3 보충학습에서 다시 회수합니다. 이후 검색과 연결되는 실제 사용 맥락은 P6-2.4 ANN 검색, P6-11.1 벡터 데이터베이스와 P6-11.2 인덱스와 검색 품질에서 이어집니다.

여기서는 임베딩을 `표현 기반`으로 먼저 읽고, 계열별 학습 배경은 같은 장의 P6-2.3에서, 검색 연결의 큰 그림은 P6-2.4와 뒤의 P6-11에서 다시 붙잡습니다.
