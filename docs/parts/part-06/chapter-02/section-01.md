...
대신 이번 절에서 바로 더 넓히지 않을 질문도 분명합니다. 임베딩 계열을 어떻게 구분해 읽어야 하는지는 같은 장의 P6-2.3 보충학습에서 다시 회수합니다. 이후 검색과 연결되는 실제 사용 맥락은 P6-11.1 벡터 데이터베이스와 P6-11.2 인덱스와 검색 품질에서 이어집니다.

여기서는 임베딩을 `표현 기반`으로 먼저 읽고, 계열별 학습 배경과 검색 연결의 큰 그림은 같은 장의 P6-2.3과 뒤의 P6-11에서 다시 붙잡습니다.
