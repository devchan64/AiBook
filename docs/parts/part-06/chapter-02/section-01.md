...
토큰 자체가 정확히 무엇이고, 단어와 어떻게 다르며, 모델 안에서 어떻게 계산되는지는 P6-2.2에서 이어집니다. 여기서는 `답변이 예상보다 비싸다`, `긴 문서 끝부분이 빠진다`, `검색 답이 예외를 놓친다` 같은 장면을 볼 때 먼저 사람 기준 길이 감각을 내려놓고 모델이 세는 계산 단위를 떠올리면 됩니다.
