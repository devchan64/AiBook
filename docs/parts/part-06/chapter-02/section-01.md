...
이 표에서 초심자가 넘어가야 할 오해는 `철자가 같으면 의미도 계산상 거의 같다`는 기대입니다. 임베딩은 표면 철자보다 문맥 속 관계를 더 직접적인 비교 기준으로 만듭니다.
...
이 사례에서 초심자가 꼭 붙잡아야 할 기준은 `같은 단어가 있나`와 `같은 문제를 말하나`를 분리해서 보는 일입니다. 문장 검색에서 임베딩이 중요한 이유는 이 두 가지가 실제로 자주 어긋나기 때문입니다.
...
| 지금 보는 값 | 초심자가 먼저 하기 쉬운 오해 | 임베딩 관점에서 먼저 바꿔 물을 질문 |
| --- | --- | --- |
...
초심자가 여기서 자주 섞는 것도 바로 이 세 층위입니다.
