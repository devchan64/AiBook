...
이 절의 중심은 `번호를 붙이는 일`과 `비교 가능한 좌표로 바꾸는 일`을 구분하는 데 있습니다. 토큰 ID는 사전 안의 항목을 가리키는 번호이고, 임베딩 벡터는 그 항목을 다른 표현과 비교하고 계산할 수 있게 만든 좌표 표현입니다.
...
이 절에서는 임베딩을 `토큰이나 문장을 계산 가능한 벡터 표현으로 바꾸는 기반`으로 먼저 닫고, 토큰 ID와 벡터 표현이 왜 다른지를 붙잡는 데 집중합니다. 계열별 학습 배경은 P6-2.3 보충학습에서, 검색 연결의 큰 그림은 P6-2.4와 뒤의 P6-11에서 다시 붙잡습니다.

이 절에서는 임베딩을 `마법 같은 의미 저장소`가 아니라, 모델이 텍스트를 계산 가능한 공간에 놓는 표현 방식으로 이해합니다. 따라서 여기서 먼저 붙잡아야 할 것은 `토큰 번호`가 아니라 `계산 가능한 벡터 표현`입니다.

| 지금 단계의 초점 | 앞 절에서 가져온 것 | 뒤 절로 넘길 것 |
| --- | --- | --- |
...
바로 앞의 P6-1.1, P6-1.2에서 `토큰이 무엇인가`와 `길이와 비용이 왜 중요한가`를 잡았다면, 이제 그 토큰이 어떻게 벡터가 되어 모델 계산과 검색으로 이어지는지를 알아야 합니다.
...
이 사례가 중요한 이유는 많은 초심자가 같은 철자를 보면 뜻도 거의 같을 것이라고 느끼기 때문입니다. 하지만 모델은 단어를 사전 뜻풀이처럼 읽기보다, 어떤 문맥에서 무엇과 함께 쓰였는지를 바탕으로 계산 가능한 좌표에 먼저 놓습니다. 그래서 `model`처럼 같은 철자라도 주변 단서가 다르면 다른 관계 안에 놓일 수 있고, 반대로 철자가 달라도 같은 설명 흐름에 자주 함께 나오면 더 가까운 좌표로 모일 수 있습니다. 임베딩 관점은 바로 이 `표면 철자`와 `문맥 관계`의 차이를 보여 주기 위해 필요합니다.
...
하지만 뒤 절의 의미와 거리, 뒤쪽 RAG와 벡터 검색 설명을 읽으려면 이 세 층위를 먼저 나눠 볼 수 있어야 합니다.

## 연습 및 예제

이번 연습의 목표는 `토큰 ID는 단지 번호이고, 실제 비교는 임베딩 벡터 위에서 일어난다`는 점을 코드 없이 손으로 구분해 보는 것입니다. 여기서는 Python을 실행하는 대신, 이미 계산된 작은 수치 예시를 보고 `번호`, `좌표`, `후보 순서`를 분리해서 읽습니다.
...
앞의 예시는 임베딩을 학습하는 절차가 아니라, `번호를 붙이는 일`과 `비교 가능한 수치 표현으로 바꾸는 일`이 다르다는 점을 가장 짧게 보여 주는 장면입니다. 이 예시를 읽고 나면 마지막으로 다음 세 질문에 바로 답할 수 있으면 충분합니다. 질문마다 먼저 스스로 답한 뒤, 바로 아래 해설과 비교합니다.
