...
이 예제를 읽고 나면 마지막으로 다음 세 질문에 바로 답할 수 있으면 충분합니다. 질문마다 먼저 스스로 답한 뒤, 바로 아래 해설과 비교합니다.
...
즉, 이 절의 닫힘은 `임베딩이 벡터다`를 외우는 데 있지 않고, `번호`, `좌표`, `후보 비교`를 각각 다른 층위로 읽을 수 있게 되는 데 있습니다.
...
- ID 숫자만 보면 어느 문서가 질의와 더 가까운지 말할 수 있는가?
- 거리값까지 보면 어느 후보가 더 가까운가?

해설: ID 숫자만으로는 의미 비교를 할 수 없습니다. `1042`와 `3881`은 사전이나 저장소 안의 항목 번호일 뿐입니다. 거리값을 보면 `hallucination_doc`의 거리 `0.001`이 `prompt_limit_doc`의 `0.006`보다 작으므로, 이 장난감 예제에서는 `hallucination_doc`이 질의 벡터에 더 가깝습니다. 이 차이가 바로 `번호`와 `비교 가능한 좌표`의 차이입니다.

### 연습 2. 가까운 후보를 정답으로 오해하지 않기
...
해설: 1순위 후보는 정답 확정이 아닙니다. 임베딩 거리는 `먼저 볼 만한 후보`를 좁혀 주는 값입니다. 실제 검색 서비스나 RAG에서는 이 후보가 질문의 근거로 적절한지, 최신인지, 충분한 문맥을 포함하는지 다시 확인해야 합니다. 임베딩 관점은 정답 판정기가 아니라 후보 비교를 가능하게 하는 표현 기반입니다.
