...
이 도식은 아래 순서로 보면 부담이 덜합니다.

| 먼저 볼 것 | 왜 중요한가 |
| --- | --- |
| 토큰 ID와 벡터가 어디서 갈리는가 | 번호와 계산 표현을 섞어 읽지 않게 해 주기 때문입니다. |
| 벡터로 바뀐 뒤 어떤 계산이 가능해지는가 | attention과 검색이 같은 기반 위에 있다는 점이 보이기 때문입니다. |
| 최종 답이 아니라 출발점이라는 점 | 임베딩을 `답을 아는 기능`처럼 오해하지 않게 하기 때문입니다. |
...
사례로 들어가기 전에는 도식에서 아래 세 질문만 먼저 붙잡으면 됩니다.

| 질문 | 도식에서 확인할 것 |
| --- | --- |
| 무엇을 비교 좌표로 바꾸는가 | 토큰인지 문장인지 입력 단위를 먼저 봅니다. |
| 왜 바로 문자열 비교로 끝나지 않는가 | 같은 철자와 같은 의미가 다를 수 있음을 봅니다. |
| 다음 계산은 어디로 이어지는가 | 검색, 추천, 내부 attention 같은 후속 계산으로 연결되는지 봅니다. |
