...
아래 연습은 실제 tokenizer 결과를 맞히는 문제가 아닙니다. 설명용 tokenizer가 만든 작은 출력표를 보고, `원문 문자열 -> 토큰 문자열 -> 토큰 ID` 변환에서 무엇이 달라졌는지 직접 추적하는 중간 판독 연습입니다. 질문마다 먼저 스스로 답한 뒤, 바로 아래 해설과 비교합니다.
...
네 연습을 마치면 아래 한 문장으로 정리할 수 있어야 합니다.
