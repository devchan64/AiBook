...
## 언제 보충학습 관점을 따로 떠올려야 하는가

| 먼저 떠올릴 질문 | 보충학습 관점이 필요한 이유 | 바로 다음에 연결될 것 |
| --- | --- | --- |
| 비슷한 문장이 왜 실제로 가까운 벡터가 되도록 학습될까? | 임베딩 직관만으로는 부족하고, 어떤 목표로 표현을 학습하는지 따로 봐야 합니다. | contrastive learning과 표현 학습의 역할 |
...
- 이후 검색 파트를 읽을 때 표현 품질과 검색 속도를 구분해서 볼 준비가 되었는가?
