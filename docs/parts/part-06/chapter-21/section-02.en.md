...
For a Diffusers pipeline such as those in P7-5.1 through P7-5.3, P7-5.5, and P7-5.16, use this order.
...
For example, the FLUX runs in P7-5.1 through P7-5.3 and P7-5.5 enable sequential offload after loading the weights and generate one scene at a time. The SDXL ControlNet comparisons in P7-5.16 also enable sequential offload after attaching their additional components. This lets the offload hooks cover the complete pipeline used for the run. Model support and component compatibility still differ, so a successful call does not prove that every adapter combination works the same way.
...
| --- | --- |
| P7-5.1–P7-5.3, P7-5.5, and P7-5.16 image experiments | Read sequential CPU offload as a device for execution feasibility, and record model file, dtype, reference input, resolution, and human-review ledger together |
