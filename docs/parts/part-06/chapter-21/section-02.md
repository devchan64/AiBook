...
P7-5.1~P7-5.3, P7-5.5와 P7-5.11처럼 Diffusers pipeline을 쓰는 경우에는 다음 순서를 지킵니다.
...
예를 들어 P7-5.1~P7-5.3와 P7-5.5의 FLUX 실행은 가중치를 읽은 뒤 순차 offload를 켜고 한 장면씩 생성합니다. P7-5.11의 SDXL 비교는 ControlNet과 IP-Adapter를 먼저 연결한 뒤 순차 offload를 켭니다. 이렇게 해야 offload hook이 실제 실행에 쓰일 전체 pipeline을 대상으로 동작합니다. 다만 모델마다 지원 구성요소와 호환성이 다르므로, 호출이 성공했다는 사실만으로 모든 adapter 조합이 같은 방식으로 작동한다고 단정하지 않습니다.
...
P6-21.3은 이 절의 실행 기준을 공개 디퓨전 모델의 실제 비교 기록으로 이어 갑니다. 그 절에서는 seed, steps, scheduler, guidance, 조건 입력 중 하나만 바꾸고, 모델 파일·dtype·실행 환경·평가 기준은 고정합니다.

따라서 오픈웨이트 모델을 직접 다룬다는 것은 `내 컴퓨터에서 한번 실행해 본다`보다 넓은 판단입니다. Part 6 안에서 모델 공개 범위를 확인하고, 실행 조건과 메모리 배치를 기록하고, 출력 품질·실행 부담·재현성을 따로 판단하는 일까지 포함합니다.
...
- P6-21.3의 한 변수 비교에서 실행 조건과 품질 검수 항목을 같은 기록에 남길 수 있는가?
