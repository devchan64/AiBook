...
P7-5.1~P7-5.3, P7-5.5와 P7-5.16처럼 Diffusers pipeline을 쓰는 경우에는 다음 순서를 지킵니다.
...
예를 들어 P7-5.1~P7-5.3와 P7-5.5의 FLUX 실행은 가중치를 읽은 뒤 순차 offload를 켜고 한 장면씩 생성합니다. P7-5.16의 SDXL ControlNet 비교도 추가 구성요소를 연결한 뒤 순차 offload를 켭니다. 이렇게 해야 offload hook이 실제 실행에 쓰일 전체 pipeline을 대상으로 동작합니다. 다만 모델마다 지원 구성요소와 호환성이 다르므로, 호출이 성공했다는 사실만으로 모든 adapter 조합이 같은 방식으로 작동한다고 단정하지 않습니다.
