...
이때 양자화(quantization)가 자주 등장한다. 양자화는 가중치를 더 작은 숫자 표현으로 바꾸어 메모리 사용량과 실행 부담을 줄이는 방식이다. 대신 품질, 속도, 안정성은 모델과 런타임에 따라 달라질 수 있다. 그래서 Part 7의 로컬 LLM 실습은 오픈웨이트 개념의 자연스러운 후속 실험이 된다.
