...
여기서는 function calling을 단순 제품 기능명이 아니라, `도구 사용을 안정적으로 연결하기 위한 구조화 방식`으로 읽습니다. tool use가 `무엇을 실행할까`를 다뤘다면, function calling은 그 실행 판단을 어떤 이름과 인자 구조로 바꿔야 시스템이 검증하고 이어서 처리할 수 있는지 다룹니다. 여러 호출을 어떤 순서로 이어 갈지는 P6-14의 agent 구조에서 이어서 봅니다.
