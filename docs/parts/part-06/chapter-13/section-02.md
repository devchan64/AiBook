...
후반 실행 구조를 같은 질문으로 다시 고정하면, 지금 부족한 것은 도구를 써야 한다는 판단만으로는 시스템이 바로 실행할 수 없고 어떤 필드가 빠졌는지도 확인하기 어렵다는 점입니다. 그래서 필요한 구조는 함수 이름과 인자를 분리한 구조화된 실행 요청입니다. 여러 호출을 어떤 순서로 이어 갈지, 중간 관찰에 따라 어떻게 재계획할지는 P6-14의 agent 구조에서 이어서 봅니다.

tool use가 어떤 외부 기능이 필요한가를 다뤘다면, function calling은 그 기능 요청을 어떤 이름과 인자 구조로 넘길 것인가를 다룹니다. agent는 이런 구조화된 호출을 어떤 순서로 이어 갈 것인가로 질문을 넓힙니다.
