# P6-14.2 자연어 요청을 이름과 인자로 나누는 함수 호출
...
P6-14.1에서는 에이전트 도구 사용(tool use)이 모델과 외부 기능을 연결하는 구조라는 점을 보았습니다. 그러면 이제 더 구체적인 질문이 나옵니다.
...
이 사례에서는 자연어 요청을 함수 이름과 인자로 구조화하고, 실행 전 누락 필드를 확인할 수 있어야 한다는 점을 보여 주는지 확인한다.
