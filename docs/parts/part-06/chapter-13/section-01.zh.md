# P6-14.1 把查询、计算、执行交给模型外部的工具使用

> Section ID: `P6-14.1`
> Version: `v2026.07.31`

记录工具使用请求时，要分开 `model_request`、`tool_name`、`tool_input`、`tool_output`、`execution_status`、`model_summary`。留下这个区分后，模型生成说明的部分和工具实际执行 查询、计算、执行的部分就会分开。

在 P6-13.2 中，我们看到向量检索中的索引(index)会在检索速度和候选质量之间形成平衡。但检索只是连接外部世界的一种方式。现在会出现一个更宽的问题。
...
这里先把`只需要读取文档的请求`和`只有实际调用外部函数才会结束的请求`分开。把一个执行请求拆成名称和参数，是 P6-14.2 的主题；把多个执行串起来，是 P6-14 之后单独处理的主题。

工具使用并不表示`模型突然获得了执行能力`。它表示`应用把模型和外部函数连接起来`。如果 RAG 是把外部文档作为依据接到回答上，工具使用则再往前一步，实际调用外部函数并把结果带回来。如何把调用名称和参数变成可验证的形状，是 P6-14.2 的主题；如何连续执行多个调用，是 P6-14 的主题。
...
这张表的核心是：`读取文档`、`执行函数`、`持续推进多个步骤`属于不同层位(level)。RAG 之上可以放工具使用，AI agent 又可以把两者绑成一个目标流程。

到这里为止，我们仍然在读`一个请求应该接上哪个外部函数`。例如，`总结我们的内部退款政策`更接近先找文档依据的 RAG 问题。`用今天的汇率把 300 美元换算成韩元`更接近需要当前值查询和计算工具的工具使用问题。`找一个明天可用的会议室并预订`同时连接查询和执行，所以会在 AI agent 结构中再讨论。

在本节，我们先完成从`应该读什么`到`实际应该查询、计算或执行什么`的移动。如何把这个执行请求稳定成名称和参数结构，会接到 P6-14.2 的 function calling；如何串联多个执行，会接到 P6-14 的 AI agent 结构。
...
这些问题会延续到后面的 AI agent 和 harness 结构。
...
- 它把文档读取和实际查询、计算、执行分开。
- 它为 P6-14.2 中把 function calling 读成结构化请求格式做准备。
- 它为 P6-14 中把 agent 读成多步骤执行流程做准备。

这里建立的视角会延续到后面的 Section。

- P6-14.2 function calling：如何稳定工具名称和参数
- P6-15.1 和 P6-15.2 AI agent：如何随时间连接多个工具调用
