...
| --- | --- | --- |
| function calling | 한 번의 실행 요청을 어떤 payload로 넘길 것인가? | P6-12.2 |
| agent 목표 흐름 | 여러 호출과 읽기를 어떤 순서로 이어 갈 것인가? | P6-13.1, P6-13.2 |
| MCP / harness 관리 | 그 흐름을 어떤 연결 규칙과 기록 환경 안에서 관리할 것인가? | P6-14.1, P6-14.2 |

즉, 이 절에서 붙잡아야 할 핵심은 `한 번의 실행 요청`에서 `여러 읽기와 실행의 순서를 어떻게 바꿀 것인가`로 손잡이가 커진다는 점입니다. 바로 다음 절에서 이 흐름은 plan-action-observation 루프로 구체화되고, P6-14에서는 연결 규칙과 실행 관리로 이어집니다.

여기서 먼저 남겨야 할 것은 어떤 목표를 어떤 순서로 풀려고 했는지를 보여 주는 `planned_steps`, 중간 관찰 메모와, 왜 멈췄고 왜 사람 검토나 재시도로 넘겼는지를 보여 주는 `next_action`, 종료 이유입니다. 이 기록이 있어야 다음 행동 변경 이유와 흐름 단위 실패를 다시 볼 수 있고, 뒤로 갈수록 P6-13.2의 계획-행동-관찰 루프, P6-16.2의 실패 대응, Part 6의 `execution_records`, `incident_records`, `next_action`, 회고 문서로 다시 읽힙니다.
