...
에이전트의 최소 실행 구조는 여기와 다음 P6-13.2 계획, 행동, 관찰에서 먼저 회수합니다. 도구 연결 표준과 실행 환경 문제는 P6-14.1 MCP와 도구 연결, P6-14.2 하네스에서 다시 다루고, 다중 에이전트 설계 전개는 현재 본편 범위 밖으로 둡니다.

이 절에서는 agent를 `목표를 작업 흐름으로 이어 가는 실행 구조`로 설명합니다.

지금 읽는 층위는 `목표 흐름 층위`입니다. 앞 장의 tool use가 `무엇을 한 번 조회하거나 실행할까`를 다뤘다면, 여기서는 여러 도구 호출과 문서 읽기 결과를 어떤 순서로 이어 붙이고 언제 멈추거나 다시 시도할지를 다룹니다.
...
이 절은 Part 6에서 `에이전트(agent)`를 대표로 설명하는 Section입니다. `더 똑똑한 챗봇`이라는 인상을 `여러 읽기와 실행을 목표 흐름으로 이어 가는 작업 구조`로 바꾸는 기준선을 여기서 세웁니다.
...
여기서 먼저 남겨야 할 것은 어떤 목표를 어떤 순서로 풀려고 했는지를 보여 주는 단계 계획과 중간 관찰 메모, 그리고 왜 멈췄고 왜 사람 검토나 재시도로 넘겼는지를 보여 주는 다음 단계와 종료 이유입니다. 이 기록이 있어야 다음 행동 변경 이유와 흐름 단위 실패를 다시 볼 수 있고, 뒤로 갈수록 P6-13.2의 계획-행동-관찰 루프, P6-16.2의 실패 대응, 회고 문서로 다시 읽힙니다.
...
즉, 코딩 에이전트의 핵심은 코드를 많이 쓰는 데 있지 않고 `실행 결과를 다시 읽고 계획을 바꾸는가`에 있습니다. 사람 개발자도 테스트가 깨지면 처음 생각을 접고 다른 원인을 다시 찾듯이, 에이전트도 같은 식으로 관찰 결과를 다음 단계에 반영해야 합니다. 이 연결이 없으면 그 시스템은 코드 생성기일 수는 있어도 작업형 에이전트라고 부르기 어렵습니다.
...
이 장면에서도 중요한 것은 검색 횟수가 아니라 판단 갱신입니다. 결과가 한 개 나왔다는 사실만으로는 충분하지 않고, `이 문서가 최신인가`, `근거 수준이 충분한가`, `핵심 조건이 빠지지 않았는가`를 점검한 뒤 다음 검색이나 추가 읽기로 이어져야 합니다. 그래서 조사 에이전트는 단순 검색 기능이 아니라 `근거 점검을 바탕으로 재탐색하는 구조`로 읽어야 합니다.
