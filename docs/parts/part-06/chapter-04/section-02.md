...
이 표에서 초심자가 넘어가야 할 오해는 `생성 모델이면 곧바로 챗봇처럼 동작해야 한다`는 기대입니다. 일반 자동완성은 같은 생성 위에 있어도, 평가 기준이 훨씬 더 `이어쓰기 경험`에 가까운 층에 머뭅니다.
...
이 사례에서 초심자가 꼭 붙잡아야 할 기준은 `자연스러운 문장`과 `질문에 맞는 응답 구조`를 분리해서 보는 일입니다. 챗봇은 이어쓰기 도구가 아니라, 의도·형식·안전 제약까지 함께 반영해야 하는 생성 경험이라는 점이 여기서 드러납니다.
...
| --- | --- | --- |
| 앞문장 뒤에 자연스러운 한 문장만 잘 붙으면 충분해 보인다 | 이어쓰기 경험 층 | 지금 기준은 질문 해결보다 후속 문장이 매끄럽게 붙는가인가 |
| `세 문장으로 설명해 줘`를 자주 어기거나 역할 톤이 흔들린다 | 지시·형식 조정층 | 생성 자체보다 instruction tuning, 역할 제약, 안전 조정이 더 앞의 문제인가 |
| 다음 한 줄 추천은 괜찮지만 함수 전체 맥락과 예외 처리는 자주 놓친다 | 문맥 결합이 더 필요한 제품 경험 층 | 더 긴 문맥과 도구·편집기 인터페이스 결합이 함께 필요한가 |

이 표의 목적은 원인을 한 번에 확정하는 데 있지 않습니다. 초심자가 `대화형 LLM이 이상하다`는 한 문장으로 뭉개지 않고, 지금 보고 있는 현상이 `이어쓰기 경험`, `조정층`, `제품 인터페이스 결합` 중 어디에서 먼저 드러나는지 짧게 가르는 데 있습니다.
...
| --- | --- | --- |
| 왜 같은 생성 모델인데 챗봇은 `질문을 이해해 답한다`고 느껴질까? | 생성 구조만으로는 부족하고, 지시 따르기와 대화 형식 조정이 함께 붙기 때문입니다. | instruction tuning과 alignment |
| 자동완성은 되는데 형식 제약이나 안전 조건을 자주 어기는 이유는 무엇일까? | 자연스러운 이어쓰기와 요청 형식 준수는 같은 문제가 아니기 때문입니다. | 프롬프트 설계, 안전 조정, 정책 층 |
| 모델, 시스템 프롬프트, 도구 실행, 앱 인터페이스를 왜 구분해야 할까? | 사용자가 보는 경험은 여러 층의 결합이지만, 각각 맡는 역할이 다르기 때문입니다. | tool use, agent loop, MCP 연결 |
