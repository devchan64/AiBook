...
이 질문을 풀 때 기준이 되는 이름이 `multi-head attention`과 `위치 표현(position encoding)`입니다.

여기서는 이 둘을 `문맥을 읽는 방식 보강`이라는 한 묶음으로 설명합니다. 반복 생성 속도, 긴 문맥 계산 부담, long-context 설계는 문맥 읽기 자체와 다른 층위의 문제입니다.
...
이 구분이 잡혀야 KV cache와 sparse attention, long-context를 `문맥 읽기 자체`가 아니라 `반복 생성 속도`, `계산 부담`, `긴 입력 유지` 문제로 따로 읽을 수 있습니다.
...
RoPE, ALiBi 같은 이름은 이 위치 정보를 다루는 서로 다른 방식입니다. 처음에는 이름을 다 외우기보다, `Transformer가 순서를 저절로 아는 것은 아니다`라는 사실을 먼저 잡습니다.

## 위치 정보와 attention이 함께 바꾸는 것
...
```mermaid
--8<-- "assets/part-06/chapter-04/p6-c04-s03-attention-position-flow-ko.mmd"
```
...
## 보충학습: attention head와 위치 표현: 확인할 판단 기준

이 사례 절은 다음 질문으로 중심축을 확인한다.

- 오픈체크리스트의 중심축 문장인 "attention head와 위치 표현을 여러 관계 축과 순서 정보의 보충학습으로 다루어야 합니다."를 본문 사례에서 어느 대목으로 확인할 수 있는가?
...
반대로 순서 정보가 있어도 모든 관계를 한 기준으로만 보면, 주체 관계와 전달 대상 관계가 섞일 수 있습니다. 그래서 `multi-head attention은 관계 축을 나누는 쪽`, `위치 표현은 순서 정보를 공급하는 쪽`으로 분리해 읽어야 합니다. 여기서 확인할 결과는 `관계 읽기`와 `순서 읽기`를 별도의 보강 문제로 구분할 수 있는가입니다.
...
![multi-head attention과 위치 표현의 역할 분리](../../../assets/part-06/chapter-04/attention-position-role-split-ko.png)
...
- 문맥 읽기 구조 위에서 `반복 생성 속도`, `계산 부담`, `긴 입력 유지` 문제가 별도 층위로 등장한다는 점을 설명할 수 있어야 합니다.
