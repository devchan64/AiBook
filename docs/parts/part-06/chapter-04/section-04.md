...
context window 제약 자체는 본류인 P6-4.2에서 이미 설명했고, KV cache가 운영 지연 시간과 비용에 미치는 영향은 P6-18.1에서 다시 회수합니다. 긴 문맥 자체를 더 잘 유지하는 문제와 sparse attention은 별도의 장문맥 설계 문제입니다.
...
## 보충학습: KV cache와 반복 생성: 확인할 판단 기준

이 사례에서는 KV cache를 반복 생성에서 이미 계산한 key/value 재사용 문제로 보충하는지 확인한다.
