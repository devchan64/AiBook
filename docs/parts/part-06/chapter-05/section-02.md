...
그렇다면 실제 생성은 어떤 흐름으로 진행되는가? 이 절은 그 과정을 독자 기준에서 직관적으로 설명합니다.

이 절에서는 `다음 토큰 예측`의 기본 정의를 다시 길게 반복하지 않습니다. 생성 목표의 기본 뜻은 P6-5.1과 개념사전을 기준으로 다시 붙잡고, 여기서는 그 목표가 실제 생성 절차로 어떻게 보이는지에만 집중합니다.
...
따라서 여기서 먼저 붙잡아야 할 것은 `모델이 답을 안다`보다 `후보들 가운데 무엇을 어떤 규칙으로 고르느냐에 따라 결과가 달라진다`는 점입니다.
...
이 절은 Part 6에서 `샘플링(sampling)`과 `temperature`를 대표로 설명하는 Section입니다. `모델이 무엇을 학습했는가`와 `생성 시 어떤 후보를 어떻게 고르는가`를 구분하는 기준선을 여기서 세웁니다.
...
세 사례를 여기까지 묶으면, 생성 설정은 `창의성 버튼`이 아니라 `어느 정도까지 구조 흔들림을 허용할 것인가`를 정하는 선택으로 읽으면 충분합니다.

## 바로 적용해 보면

이 절을 읽은 뒤에는 아직 beam search나 top-p 같은 세부 공식을 다 몰라도, 아래처럼 `지금 보는 장면이 모델이 모르는 문제인가, 아니면 이미 있는 후보 중 무엇을 뽑느냐의 문제인가`를 먼저 가르는 연습을 할 수 있습니다.

| 지금 보이는 장면 | 초심자가 먼저 하기 쉬운 오해 | 먼저 바꿔 물을 질문 |
| --- | --- | --- |
| 같은 문의인데 답변 말투와 안내 순서가 매번 조금씩 흔들린다 | 모델이 정책을 몰라서 그런가 보다 하고 지식 부족으로만 보기 쉽다 | 후보는 알고 있는데 sampling 폭이나 temperature 때문에 구조 흔들림이 커진 것인가 |
| 마케팅 시안이 너무 비슷해서 비교할 재료가 없다 | 모델이 창의력이 부족하다고만 느끼기 쉽다 | 더 다양한 후보를 허용하는 선택 규칙이 필요한가 |
| 같은 코드 수정 요청인데 예외 처리 방식이 실행마다 달라진다 | 모델이 코드를 이해하지 못한다고만 느끼기 쉽다 | 지금 필요한 것은 지식 추가보다 더 보수적인 선택 규칙인가 |

이 표에서 중요한 것은 `temperature를 올릴까 내릴까`를 기계적으로 외우는 일이 아니라, 먼저 `무엇을 학습했는가`와 `그중 무엇을 실제로 뽑고 있는가`를 다른 문제로 읽는 일입니다.
...
그래서 이 절의 닫힘은 `생성은 후보 분포에서 실제로 무엇을 뽑을지 정하는 과정`이라는 말을 실제 서비스 장면을 읽는 기준으로 바꾸는 데 있습니다.

같은 장면을 더 짧은 실무 판단으로 다시 줄이면 다음처럼 읽을 수 있습니다.

| 지금 먼저 보이는 신호 | 먼저 의심할 층위 | 바로 다음에 붙여 볼 질문 |
| --- | --- | --- |
| 같은 문의인데 말투와 안내 순서가 조금씩 흔들린다 | 출력 선택 규칙 층위 | 후보는 이미 알고 있는데 sampling 폭이나 temperature 때문에 흔들림이 커진 것인가 |
| 마케팅 시안이 너무 비슷해서 비교 재료가 없다 | 후보 다양성 부족 문제 | 더 넓은 후보를 허용하는 선택 규칙이 필요한가 |
| 같은 코드 수정 요청인데 실행마다 구조가 달라진다 | 더 보수적인 선택 규칙 필요 | 지식 부족보다 greedy에 가까운 안정적 선택이 먼저 필요한가 |

이 표의 목적은 원인을 한 번에 확정하는 데 있지 않습니다. 초심자가 `생성이 이상하다`는 한 문장으로 뭉개지 않고, 지금 문제가 `지식 부족`보다 `선택 규칙`, `후보 다양성`, `구조 흔들림` 중 어디에서 먼저 드러나는지 짧게 가르는 데 있습니다.
...
| --- | --- | --- |
| 왜 같은 질문에도 답이 조금씩 달라질 수 있을까? | 학습 목표와 별개로, 생성 시 후보 분포에서 실제로 무엇을 고르는 규칙이 따로 있기 때문입니다. | sampling과 temperature 설명 |
| temperature를 왜 모델 지식 자체와 구분해야 할까? | 보통 서비스 문맥의 temperature는 학습된 파라미터가 아니라 출력 선택 성향을 바꾸는 설정값이기 때문입니다. | 생성 설정, 재현성, 평가 |
| 언제 다양성이 유리하고 언제 흔들림이 문제일까? | 고객 응답, 마케팅 초안, 코드 생성처럼 과업마다 안정성과 다양성의 우선순위가 다르기 때문입니다. | 프롬프트 설계, 운영 정책, 평가 기준 |
