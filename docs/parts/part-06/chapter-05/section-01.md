...
여기서는 생성형 LLM의 기본 학습 목표를 먼저 붙잡습니다. 즉, 아직 검색이나 도구를 붙이는 단계가 아니라 `LLM이 무엇을 반복 학습해 긴 생성으로 이어지는가`를 먼저 닫는 자리입니다. 뒤의 P6-8장은 이 구조 위에 `어떻게 응답 습관을 조정하는가`를, P6-10장은 `언제 외부 근거를 붙이는가`를 이어서 다룹니다.
...
따라서 여기서 먼저 붙잡아야 할 것은 `자동완성처럼 보인다`는 인상보다 `현재 문맥에서 다음 토큰 분포를 반복 계산한다`는 학습 목표입니다.
...
| --- | --- | --- |
| 같은 앞문장 뒤에 사과형, 설명형, 안내형 후보가 모두 자연스럽다 | 다음 토큰 후보 분포 | 현재 문맥에서 후보가 여러 갈래로 열려 있는가 |
| 첫 요약 문장을 잘못 잡자 뒤 문장도 같은 방향으로 밀린다 | 순차 생성의 누적 효과 | 첫 선택이 뒤 문장들의 강조 순서와 초점을 계속 밀고 있는가 |
| 코드에서 괄호나 변수명 하나가 틀린 뒤 전체 블록이 무너진다 | 현재 토큰 선택의 연쇄 효과 | 지금 토큰 하나가 뒤 문법과 분기 조건까지 함께 바꾸는가 |

이 표의 목적은 원인을 한 번에 확정하는 데 있지 않습니다. 초심자가 `다음 토큰 예측`을 한 문장 정의로만 외우지 않고, 지금 보고 있는 현상이 `후보 분포`, `누적 생성`, `연쇄 오류` 중 어디에서 먼저 드러나는지 짧게 가르는 데 있습니다.
