...
학습 축은 `왜 같은 생성 구조라도 기본 반응 범위가 다른가`라는 질문에서 시작합니다. 여기서 먼저 잡을 것은 특정 과업 전에 넓은 언어 기반을 배우는 사전학습입니다. 그다음에야 `그 기반을 왜 그렇게 큰 규모로 만드는가`, `그 기반을 우리 목적과 사용자 요청 형식에 어떻게 더 맞추는가`라는 질문으로 넘어갈 수 있습니다.

즉, 사전학습 기준이 잡혀야 `스케일(scale)`도 단순한 크기 경쟁이 아니라 `왜 넓은 기반을 그렇게 큰 규모로 만들려 하는가`의 문제로 읽힙니다. 실제 사용자 반응과 정책 기준을 맞추는 문제는 파인튜닝, 지시 튜닝, 정렬에서 다시 좁혀 읽습니다.

즉, 여기서 잠시 학습 축을 길게 읽는 이유는 `생성할 수 있다`와 `사용자에게 쓸모 있게 반응한다` 사이에 실제로 어떤 추가 학습 단계가 끼어 있는지 분리해 두기 위해서입니다.

이 기준을 먼저 잡아 두어야 뒤의 P6-8 파인튜닝과 P6-9 지시 튜닝·정렬 절도 `새 기능 추가`가 아니라, 같은 생성 구조를 실제 사용자 반응으로 조정하는 후속 단계로 읽힙니다.

`모델이 이미 똑똑하다`는 인상은 `대규모 일반 패턴을 먼저 배우고 이후 목적별 조정이 덧붙는 구조`로 바꾸어 읽어야 합니다.

따라서 이 구간은 생성 원리를 반복하는 장이 아닙니다. Transformer와 next-token prediction으로 생성 구조를 잡은 뒤, 같은 구조가 어떤 학습과 조정 단계를 거쳐 실제 사용자 반응으로 좁혀지는지 읽는 축입니다. 여기서 `언어 기반 -> 목적 조정 -> 사용자 반응 조정`의 순서가 잡히면, 뒤의 파인튜닝과 지시 튜닝도 새 기능 추가가 아니라 기반 위의 후속 조정으로 보이기 시작합니다.
