...
이 절은 Part 6에서 `파인튜닝(fine-tuning)`을 대표로 설명하는 Section입니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 파인튜닝의 기본 뜻은 이 절과 개념사전을 기준으로 다시 연결합니다. `모델을 새로 만드는 일`이라는 인상을 `사전학습된 기반 모델을 우리 목적에 맞게 추가 조정하는 단계`로 바꾸는 기준선을 여기서 세웁니다.
...
| --- | --- | --- |
| 왜 일반 모델은 대략 맞아도 우리 업무 형식은 자주 흔들릴까? | 넓은 기반 능력과 내부 라벨·문체·출력 구조 적합성은 다른 층위이기 때문입니다. | LoRA, instruction tuning, 업무 맞춤 조정 |
| 프롬프트만 바꾸는 것과 모델 반응 습관을 바꾸는 것은 왜 다를까? | 프롬프트는 입력을 바꾸고, 파인튜닝은 모델 내부 반응 경향 자체를 더 목적에 맞게 조정하기 때문입니다. | 프롬프트 한계, 후속 조정 방식 구분 |
| 최신 정보 문제와 형식 일관성 문제를 왜 같은 해결책으로 보면 안 될까? | 파인튜닝은 최신성보다 과업 적합성과 출력 습관 조정에 더 가깝기 때문입니다. | RAG와 외부 근거 연결의 역할 |
