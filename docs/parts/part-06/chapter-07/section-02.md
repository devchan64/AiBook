...
초심자 기준에서는 여기서 `모델이 여러 개`라는 말과 `조정본이 여러 개`라는 말을 분리해 읽는 것이 중요합니다. 전자는 본체 전체를 업무 수만큼 따로 들고 가는 그림이고, 후자는 본체는 하나로 유지한 채 목적별 차이만 작은 파일처럼 따로 관리하는 그림입니다. 그래서 LoRA를 이해할 때는 정확한 수식보다 먼저 `무엇이 공통 자산이고 무엇이 업무별 차이인가`를 나눠 보는 감각이 필요합니다.
...
이 사례는 `최고 성능`과 `현실적으로 가능한 성능 개선`을 구분해서 읽어야 합니다. GPU 예산이 빠듯한 팀에게는 논문 최고 점수보다 `이번 주 안에 실험 1회라도 돌릴 수 있는가`, `실패 후 설정을 바꿔 다시 돌릴 여지가 있는가`가 더 절실한 기준입니다. 그래서 LoRA의 장점은 단순히 가볍다는 말보다 `실험 자체를 시작 가능한 일로 바꿔 준다`는 데 있습니다.
