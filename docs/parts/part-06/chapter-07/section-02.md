...
이번 절은 `큰 기반 모델 위에 작은 조정분만 더해 비용을 줄이는 발상`까지를 먼저 닫습니다. `왜 low-rank라는 이름이 붙는가`, `어디에 작은 조정분이 붙는가`, `adapter, LoRA, QLoRA를 실제 제약 장면에서 어떻게 구분하는가` 같은 더 깊은 배경은 Chapter 8의 선택 지도 뒤 P6-8.4 보충학습에서 다시 회수하고, instruction tuning이나 도메인 맞춤 조정과의 연결은 바로 다음 P6-8.1 지시 튜닝에서 이어집니다.

즉, `adapter / LoRA / QLoRA` 세부 구분이 사라진 것이 아니라, Chapter 7 본선을 끊지 않도록 `P6-8.4`로 위치를 옮겨 다시 읽게 한 상태입니다.

이 절에서는 LoRA를 `작은 모델`로 오해하지 않고, `큰 기반 모델 위에 작은 조정 층을 더하는 방식`으로 이해합니다.

따라서 여기서 먼저 붙잡아야 할 것은 `모델을 작게 만든다`가 아니라 `같은 큰 기반 모델을 더 적은 조정 비용으로 적응시킨다`는 점입니다.
...
이 절을 읽은 뒤에는 아직 low-rank 수식이나 QLoRA 세부를 다 몰라도, 아래처럼 `지금 필요한 것이 모델 전체를 다시 크게 조정하는 일인가, 아니면 작은 적응분으로 실험을 빠르게 돌리는 일인가`를 먼저 가르는 연습을 할 수 있습니다.

| 지금 보이는 장면 | 초심자가 먼저 하기 쉬운 오해 | 먼저 바꿔 물을 질문 |
| --- | --- | --- |
| 같은 기반 모델로 여러 업무용 조정본을 만들어야 한다 | 업무가 다르면 큰 모델도 매번 통째로 따로 가져야 한다고 느끼기 쉽다 | 지금 필요한 것은 본체 복사보다 작은 조정본 분리 관리인가 |
| GPU 예산이 빠듯해 첫 실험 자체가 무겁다 | 최고 성능이 중요하니 전체 파인튜닝부터 해야 한다고 느끼기 쉽다 | 지금 먼저 필요한 것은 최고의 점수보다 실험을 시작하고 반복할 수 있는가 |
| 일주일 안에 여러 조정 가설을 비교해야 한다 | 한 번 크게 조정하면 오히려 더 빨리 끝날 것이라고 느끼기 쉽다 | 지금 중요한 것은 단일 최고 실험보다 가설 회전 수와 비교 속도인가 |

이 표에서 중요한 것은 `LoRA가 더 가볍다`를 외우는 일이 아니라, 먼저 `무엇이 병목인가`를 `메모리`, `저장`, `버전 관리`, `실험 회전 속도`로 나눠 읽는 일입니다.
...
그래서 이 절의 닫힘은 `LoRA는 큰 기반 모델을 더 가볍게 적응시키는 운영 방식`이라는 문장을 실제 선택 기준으로 바꾸는 데 있습니다.
...
- 바로 다음 장에서 지시 튜닝과 정렬을 읽은 뒤, 선택 지도와 보충절에서 효율적 조정 이름 구분을 다시 볼 준비가 되었는가?

| 먼저 떠올릴 질문 | 효율적 조정 관점이 먼저 필요한 이유 | 바로 다음 절이나 뒤 장에서 이어질 것 |
| --- | --- | --- |
| 파인튜닝은 필요해 보이는데 왜 바로 전체 모델을 다시 학습하지 않을까? | 큰 기반 모델 전체를 다시 조정하는 비용과 저장 부담이 너무 클 수 있기 때문입니다. | 지시 튜닝을 거친 뒤 P6-8.3 선택 지도와 P6-8.4 효율적 조정 구분 |
| 같은 기반 모델로 여러 업무를 시험할 때 무엇이 먼저 병목이 될까? | 품질 이전에 버전 관리, 메모리, 실험 회전 수가 먼저 한계가 될 수 있기 때문입니다. | 운영 제약과 실험 전략 |
| LoRA를 왜 `작은 모델`이 아니라 `작은 조정분`으로 봐야 할까? | 핵심은 본체를 줄이는 것이 아니라 본체를 유지한 채 적응 비용을 줄이는 데 있기 때문입니다. | P6-8.4의 low-rank, adapter, QLoRA 구분 |
