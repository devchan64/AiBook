...
앞 절의 파인튜닝과 LoRA가 `모델을 어떻게 조정할 것인가`를 다뤘다면, 여기서는 그 조정 방식 안에서 adapter, LoRA, QLoRA가 어떤 문제를 줄이려는지 더 세분화해 봅니다. 뒤의 지시 튜닝 절에서는 무엇을 가르칠지로, 운영 절에서는 메모리와 비용 제약으로 질문이 다시 이동합니다.
