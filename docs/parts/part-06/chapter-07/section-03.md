...
P6-7.1과 P6-7.2에서는 `왜 목적 맞춤 조정이 필요한가`, `왜 전체를 다시 조정하는 방식이 너무 무거울 수 있는가`를 보았습니다. 하지만 여기까지 읽고 나면 초심자는 곧바로 다음 벽에 부딪힙니다. 문서와 강의마다 adapter, LoRA, QLoRA 같은 이름이 한꺼번에 나오기 시작하기 때문입니다.

이 보충학습의 역할은 바로 그 지점에서 멈추지 않게 하는 것입니다. 이 절은 수식을 길게 따라가기보다, `왜 low-rank라는 말이 붙는가`, `작은 조정분이 어디에 붙는가`, `왜 QLoRA가 같이 언급되는가`를 읽을 수 있게 만드는 데 초점을 둡니다.

여기서는 LoRA의 기본 정의를 다시 길게 반복하지 않습니다. 효율적 조정과 LoRA의 대표 설명 위치는 P6-7.2와 개념사전으로 다시 붙잡고, 이 절은 이름이 비슷한 조정 방식들을 `이름 암기`가 아니라 `제약 장면별 선택 기준`으로 처음 구분하는 보강 절로 읽으면 충분합니다.
...
여기서는 이름이 비슷한 조정 방식들을 먼저 구분합니다. 앞 절의 파인튜닝과 LoRA가 `모델을 어떻게 조정할 것인가`를 다뤘다면, 여기서는 그 조정 방식 안에서 adapter, LoRA, QLoRA가 각각 어떤 제약을 먼저 줄이려는지 더 세분화해 봅니다. 뒤의 지시 튜닝 절에서는 `무엇을 가르칠 것인가`로, 운영 절에서는 `메모리와 비용 제약이 실제 선택을 어떻게 밀어 올리는가`로 질문이 다시 이동합니다.

이 보충학습에서 먼저 붙잡아야 할 것은 `이름이 비슷하다`가 아니라 `각 방식이 무엇을 덜 무겁게 만들려고 하는가`입니다. 실제 데이터 설계와 운영 판단은 뒤 절로 넘겨 두는 편이 안정적입니다.

## 왜 P6-7.2 다음에 이 이름 구분이 필요한가

P6-7.2까지 읽으면 `큰 기반 모델을 더 가볍게 적응시키는 방식이 필요하다`는 큰 축은 잡힙니다. 그런데 실제 자료를 보기 시작하면 곧 `그 가벼운 방식이 정확히 무엇인가`에서 다시 흐려지기 쉽습니다. 같은 문단 안에 adapter, LoRA, QLoRA가 함께 나오면 초심자는 이 셋을 `비슷한 신기술 이름 몇 개` 정도로 받아들이기 쉽기 때문입니다.
...
그래서 이 절은 본선에서 벗어난 옆길이 아니라, P6-7.2의 `효율적 조정` 설명이 뒤에서 무너지지 않게 붙잡아 주는 정리 구간입니다.

| 바로 앞 절에서 이미 잡은 것 | 이 보충 절에서 새로 잡는 것 | 바로 뒤 절로 넘기는 질문 |
| --- | --- | --- |
| 큰 기반 모델을 더 가볍게 적응시키는 흐름이 필요하다 | adapter, LoRA, QLoRA가 무엇을 줄이려는 선택지인지 구분한다 | 이 조정 방식으로 실제 응답 습관과 지시 따르기를 무엇으로 가르칠 것인가 |
| 전체 파인튜닝과 LoRA의 큰 차이 | 구조 차이, 변화분 학습, 메모리 제약을 서로 다른 질문으로 나눈다 | 운영 제약이 커질 때 어떤 방식이 더 현실적인가 |

즉, 이 절의 닫힘은 `이름을 더 많이 아는 것`이 아니라 `문서를 읽다가 어떤 제약 장면에서 어떤 이름이 나오는지 덜 헷갈리는 것`입니다.
