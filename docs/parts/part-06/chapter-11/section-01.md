...
후반 검색·실행 구조로 넘어가기 전에 선택 기준을 먼저 고정하면, 지금 부족한 것은 모델 기억만으로 최신 문서, 내부 문서, 현재 버전 근거를 안정적으로 반영하기 어렵다는 점입니다. 그래서 먼저 붙이는 구조는 답변 전에 관련 문서를 찾고 입력 맥락에 붙이는 근거 연결 구조입니다. 다만 붙인 문서가 실제 입력에서 어디에 결합되는지, 그 문서를 어떤 저장 구조와 탐색 방식으로 다시 꺼낼지는 P6-11.2와 P6-12에서 이어서 봅니다.
...
따라서 핵심 변화는 `질문을 더 잘 쓰는가`보다 `답하기 전에 어떤 문서를 먼저 연결하는가`입니다. 프롬프트 입력 조정은 같은 재료로 답하는 방식을 조정하는 문제이고, RAG 근거 연결은 답하기 전에 어떤 문서를 붙일지 정하는 문제입니다. 문서를 읽는 것만으로 부족해 실제 조회나 실행이 필요해지면 tool use와 function calling이 다시 별도 층위로 이어집니다.
