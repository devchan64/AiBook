...
즉, 지금 장의 핵심은 `기록을 남겼는가`에서 `그 기록과 답을 어떤 기준으로 판정할까`로 관점이 바뀌는 데 있습니다. 여기서의 판정은 아직 `좋은 답인가`를 가르는 단계이지, `실제 서비스 운영 후보로 바로 남길 수 있는가`를 끝내는 단계는 아닙니다. 그 다음 질문은 P6-16에서 다시 가릅니다.
...
![LLM 평가 축별 통과 점검](../../../assets/part-06/chapter-15/llm-eval-axis-check-ko.png)
