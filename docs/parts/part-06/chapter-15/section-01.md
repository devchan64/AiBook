...
에이전트 루프가 `여러 읽기와 실행을 어떤 반복 구조로 이어 갈까`를 다뤘다면, MCP 관점은 그 루프가 쓰는 도구와 자원을 어떤 공통 형식으로 드러내야 다음 실행과 기록이 덜 흔들리는지 다룹니다. 실행 기록과 재현 환경은 이 연결을 실제 run 안에서 어떻게 남길지의 문제입니다.

후반 실행 구조를 같은 질문으로 다시 고정하면, 역할은 아래처럼 압축할 수 있습니다.

| 부족한 것 | 붙이는 구조 | 아직 남는 문제 |
| --- | --- | --- |
| 여러 도구와 자원이 제각각 연결되면, 같은 목표 흐름도 입력 형식과 호출 규칙 때문에 자주 흔들린다 | 도구와 자원을 공통 형식으로 드러내는 연결 인터페이스 관점 | 실제 실행을 어떤 기록과 재현 환경 안에 둘지, 실패를 어디서 멈추고 복구할지는 아직 남아 있다 |

| 현재 관점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| agent loop | 여러 실행을 어떤 루프로 이어 갈 것인가? | 도구와 자원의 연결 형식 |
| MCP 연결 인터페이스 | 그 도구와 자원을 어떤 공통 연결 형식으로 드러낼 것인가? | 실행 기록과 재현 환경 |
| harness 실행 관리 | 그 연결을 쓴 실행을 어떤 기록과 재현 환경 안에 남길 것인가? | 평가와 운영 판단 |

여기서는 `모델 컨텍스트 프로토콜(Model Context Protocol, MCP)`을 `모델 능력`과 `도구 연결 형식`을 섞지 않게 하는 공통 연결 인터페이스로 읽는 기준을 잡습니다.

핵심은 `어떤 순서로 실행할까`에서 `그 실행이 쓰는 도구와 자원을 어떤 공통 형식으로 노출할까`로 관점이 바뀌는 데 있습니다. MCP는 `연결을 덜 제각각으로 만드는 층`, 하네스는 `그 연결을 실제 실행 기록으로 남기는 층`, 평가는 `남은 기록을 품질 판단 기준으로 바꾸는 층`입니다. 이 차이가 보여야 MCP를 운영 장치나 평가 절차와 섞어 읽지 않게 됩니다.

MCP, 하네스, 평가, 운영의 최소 차이는 아래 표처럼 다시 고정할 수 있습니다.

| 현재 층위 | 가장 먼저 붙잡을 질문 | 이어지는 중심 |
| --- | --- | --- |
