...
후반 실행 구조를 같은 질문으로 다시 고정하면, 지금 부족한 것은 여러 도구와 자원이 제각각 연결될 때 같은 목표 흐름도 입력 형식과 호출 규칙 때문에 자주 흔들린다는 점입니다. 그래서 필요한 관점은 도구와 자원을 공통 형식으로 드러내는 연결 인터페이스입니다. 실제 실행을 어떤 기록과 재현 환경 안에 둘지, 실패를 어디서 멈추고 복구할지는 P6-15.2와 운영 구간에서 이어서 봅니다.

P6-14.2가 여러 실행을 어떤 루프로 이어 갈지 봤다면, 여기서는 그 루프가 쓰는 도구와 자원을 어떤 공통 연결 형식으로 드러낼지 봅니다. P6-15.2의 하네스는 그 연결을 쓴 실행을 어떤 기록과 재현 환경 안에 남길지 다룹니다.
