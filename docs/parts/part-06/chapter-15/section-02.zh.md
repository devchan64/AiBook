# P6-16.2 包裹执行记录和可复现环境的 Harness

> Section ID: `P6-16.2`
> Version: `v2026.07.31`

harness 记录要分为 `input_snapshot`、`tool_call_log`、`environment_state`、`approval_state`、`output_snapshot`、`replay_note`。这样就不是只看最终回答，而能再次确认同一次执行为什么按那样的 流程 进行。

在 P6-16.1 中，我们看到 MCP 是一种让模型、外部工具和数据之间的连接更一致的接口视角。但即使连接格式已经整理好，如果执行流程没有留下记录，之后仍然很难再次解释失败原因或改进效果。现在要看的，是包裹 AI agent 执行、留下日志和评估输入，并让流程能够再次运行的结构。
