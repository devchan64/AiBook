...
P6-15.1에서는 MCP가 모델과 외부 도구, 데이터 사이의 연결을 더 일관되게 만드는 인터페이스 관점이라는 점을 보았습니다. 하지만 연결만으로는 충분하지 않습니다. 이제는 에이전트 실행을 안정적으로 감싸고, 로그와 평가를 남기며, 반복 가능하게 관리하는 구조를 봐야 합니다.
...
앞 절까지가 연결과 실행 구조를 만드는 쪽이었다면, harness는 그 실행에서 남긴 `실행 추적(trace)`, `로그(log)`, `재현 실행 정보(replay)`가 왜 평가 기준의 입력이 되는지 다룹니다. 좋은 실행 기록은 운영 부록이 아니라 `무엇을 기준으로 괜찮다고 판정할까`를 떠받치는 입력입니다.

하네스는 제품 이름보다 `무엇을 어떤 실행 추적(trace)과 재현 실행 정보(replay)로 남겨야 하는가`, `이 기록이 왜 평가 입력이 되는가`, `MCP와 하네스가 각각 연결과 관리 중 무엇을 맡는가`라는 세 질문으로 읽으면 됩니다.

후반 실행 구조를 같은 질문으로 다시 고정하면, 역할은 아래처럼 압축할 수 있습니다.

| 부족한 것 | 붙이는 구조 | 아직 남는 문제 |
| --- | --- | --- |
| 연결과 루프만으로는 같은 실패를 다시 설명하거나 수정 전후를 비교하기 어렵다 | 실행 추적(trace), 재현 실행 정보(replay), 승인 기록(approval)으로 실행을 감싸는 관리 구조 | 남긴 기록을 어떤 품질 기준으로 통과시킬지, 어떤 운영 통제와 실패 대응으로 이어 갈지는 아직 남아 있다 |

| 현재 관점 | 이어지는 질문 | 다른 층위로 남겨 둘 것 |
| --- | --- | --- |
| MCP 연결 형식 | 무엇과 어떤 형식으로 연결했는가? | 실행 기록과 승인 경계 |
| harness 실행 기록 | 그 연결을 쓴 실행을 어떤 실행 추적(trace), 재현 실행 정보(replay), 승인 기록(approval)으로 남길 것인가? | 평가 기준과 운영 통제 |
| 평가와 운영 판정 | 남긴 기록을 어떤 통과 기준과 서비스 통제로 이어 갈 것인가? | 실패 대응과 개선 루프 |

여기서는 `하네스(harness)`를 `도구 하나`라는 인상보다 `실행을 감싸고 기록과 평가 입력을 남기는 운영 장치`로 읽는 기준을 잡습니다.

하네스는 `실행 흔적을 남기는 층`, 평가는 `그 흔적을 품질 판단으로 바꾸는 층`, 운영은 `그 판단을 실제 서비스 통제와 실패 대응으로 이어 붙이는 층`입니다. 핵심은 `연결을 잘했는가`에서 `그 연결을 쓴 실행을 다시 설명하고 비교할 수 있는가`로 관점이 바뀌는 데 있습니다. 이 차이가 보여야 하네스를 평가 절차나 운영 정책과 같은 말처럼 섞어 읽지 않게 됩니다.
...
![하네스 실행 원인 분리](../../../assets/part-06/chapter-14/harness-run-issue-split-ko.png)
