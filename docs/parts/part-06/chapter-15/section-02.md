...
Part 6에서 `자동 평가(automatic evaluation)`, `사람 평가(human evaluation)`, `평가 분업 구조`에 대한 첫 상세 설명은 이 절에서 잡습니다. 뒤 절에서는 현재 맥락에 필요한 최소 설명만 남기고, 평가 축의 기본 뜻은 P6-15.1과 이 절, 개념사전을 기준으로 다시 연결합니다.
