...
InvokeAI와 ComfyUI는 설치·버튼·노드 목록을 배우기 위한 대상이 아닙니다. 이미지 생성의 파이프라인을 눈에 보이게 해, 에이전트의 제어 흐름과 같은 말로 섞지 않기 위한 사례입니다. 실제 공개 디퓨전 모델의 실행과 비교 기록은 P6-21.3에서 다룹니다.
...
이 흐름은 P5-15.4~P5-15.5의 `조건과 현재 노이즈 상태 -> 반복 노이즈 예측·복원 -> 결과`를 실행 환경의 언어로 다시 적은 것입니다. 여기서는 각 모델의 정확한 내부 구현이나 특정 노드의 모든 인자를 설명하지 않습니다.
...
- 이 절에서는 workflow의 종류, 연결·변환·기록의 역할을 비교합니다.
- P6-21.3에서는 공개 디퓨전 모델의 입력 또는 설정 하나를 바꾸고 결과 차이와 실패 신호를 기록합니다.
...
- 실제 공개 모델의 조건 비교는 P6-21.3에서 한다는 점을 설명할 수 있는가?
