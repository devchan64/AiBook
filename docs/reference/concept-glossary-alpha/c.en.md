# Concept Glossary: C

This page lists English glossary entries for this letter. Entry bodies are assembled from term source files.

--8<-- "reference/concept-glossary-terms/causal-inference.en.md"

--8<-- "reference/concept-glossary-terms/correlated-features.en.md"

--8<-- "reference/concept-glossary-terms/caching.en.md"

--8<-- "reference/concept-glossary-terms/calibration.en.md"

--8<-- "reference/concept-glossary-terms/catboost.en.md"

--8<-- "reference/concept-glossary-terms/category.en.md"

--8<-- "reference/concept-glossary-terms/candidate-distribution.en.md"

--8<-- "reference/concept-glossary-terms/ccp-alpha.en.md"

--8<-- "reference/concept-glossary-terms/chain-of-thought-cot.en.md"

--8<-- "reference/concept-glossary-terms/chain-rule.en.md"

--8<-- "reference/concept-glossary-terms/chunk.en.md"

--8<-- "reference/concept-glossary-terms/class.en.md"

--8<-- "reference/concept-glossary-terms/classification.en.md"

--8<-- "reference/concept-glossary-terms/client.en.md"

--8<-- "reference/concept-glossary-terms/cluster.en.md"

--8<-- "reference/concept-glossary-terms/cluster-label.en.md"

--8<-- "reference/concept-glossary-terms/clustering.en.md"

--8<-- "reference/concept-glossary-terms/cnn-convolutional-neural-network.en.md"

--8<-- "reference/concept-glossary-terms/code-cell.en.md"

--8<-- "reference/concept-glossary-terms/column.en.md"

--8<-- "reference/concept-glossary-terms/column-role-separation.en.md"

--8<-- "reference/concept-glossary-terms/combinatorial-explosion.en.md"

--8<-- "reference/concept-glossary-terms/commit.en.md"

--8<-- "reference/concept-glossary-terms/comparability.en.md"

--8<-- "reference/concept-glossary-terms/cross-validation.en.md"

--8<-- "reference/concept-glossary-terms/cross-entropy.en.md"

--8<-- "reference/concept-glossary-terms/comparison-report.en.md"

--8<-- "reference/concept-glossary-terms/comparison-result.en.md"

--8<-- "reference/concept-glossary-terms/comparison-table.en.md"

--8<-- "reference/concept-glossary-terms/comparison-plot.en.md"

--8<-- "reference/concept-glossary-terms/computation-graph.en.md"

--8<-- "reference/concept-glossary-terms/computational-limit.en.md"

--8<-- "reference/concept-glossary-terms/component.en.md"

--8<-- "reference/concept-glossary-terms/covariance-matrix.en.md"

--8<-- "reference/concept-glossary-terms/connectivity.en.md"

--8<-- "reference/concept-glossary-terms/confabulation.en.md"

--8<-- "reference/concept-glossary-terms/confidential-information.en.md"

--8<-- "reference/concept-glossary-terms/confusion-matrix.en.md"

--8<-- "reference/concept-glossary-terms/generalization.en.md"

--8<-- "reference/concept-glossary-terms/constraint.en.md"

--8<-- "reference/concept-glossary-terms/context.en.md"

--8<-- "reference/concept-glossary-terms/context-window.en.md"

--8<-- "reference/concept-glossary-terms/contextual-representation.en.md"

--8<-- "reference/concept-glossary-terms/control.en.md"

--8<-- "reference/concept-glossary-terms/convergence.en.md"

--8<-- "reference/concept-glossary-terms/conversational-llm.en.md"

--8<-- "reference/concept-glossary-terms/convolution.en.md"

--8<-- "reference/concept-glossary-terms/copy.en.md"

--8<-- "reference/concept-glossary-terms/copyright.en.md"

--8<-- "reference/concept-glossary-terms/corpus.en.md"

--8<-- "reference/concept-glossary-terms/cosine-similarity.en.md"

--8<-- "reference/concept-glossary-terms/centroid.en.md"

--8<-- "reference/concept-glossary-terms/cost.en.md"

--8<-- "reference/concept-glossary-terms/credential.en.md"

--8<-- "reference/concept-glossary-terms/contrastive-learning.en.md"
