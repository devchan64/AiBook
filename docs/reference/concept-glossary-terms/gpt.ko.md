...
- 왜 중요한가: 오늘날 LLM 사용 경험의 직접 계보를 이루며, 사전학습 뒤 prompt와 in-context learning으로 여러 과업에 연결되는 흐름을 보여 주기 때문입니다. 이 개념이 있어야 챗봇, 코드 생성, 요약, 도구 호출 같은 현대 LLM 사용이 왜 `다음 토큰 생성`이라는 공통 기반 위에서 돌아가는지 이해할 수 있습니다. 또한 인코더 중심 모델과 달리 왜 GPT 계열이 대화형 생성과 장문 작성 맥락에서 더 자주 등장하는지도 함께 읽게 됩니다.
