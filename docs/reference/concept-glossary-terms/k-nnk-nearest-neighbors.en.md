<a id="k-nnk-nearest-neighbors"></a>
<a id="glossary-k-nn"></a>
<a id="glossary-k-nnk-nearest-neighbors"></a>

## k-NN, k-nearest neighbors

- Meaning: k-NN is a model that finds the `k` nearest stored examples to a new input and predicts from their majority vote or average. It relies directly on similarity to nearby cases rather than learning a complex global formula.
- Why it matters: k-NN gives a direct case-based intuition for machine learning. It shows why distance, feature value ranges, and the chosen number of neighbors can strongly change model behavior.
- Related concepts: `nearest neighbor`, `distance`, `scale`
- Core Section: `P4-12.1`
- Appears in: `P3-6.5`, `P4-12.2`, `P4-12.3`
