...
- 왜 중요한가: 음성 생성과 언어 모델링을 무리하게 같은 모델로 묶지 않으면서도, `이전 출력에 조건부로 다음 출력을 만든다`는 공통 구조를 이해하게 해 주기 때문입니다. 이 개념이 있어야 순차 생성이 한 번에 전체 결과를 찍는 것이 아니라, 이전 맥락을 바탕으로 다음 단위를 반복해 만드는 방식일 수 있음을 읽게 됩니다.
