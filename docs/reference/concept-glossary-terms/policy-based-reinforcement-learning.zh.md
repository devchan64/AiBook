...
- 含义: 策略型强化学习不是先给状态或行动建立价值记分板，而是直接调整选择行动的 policy。它更接近`直接改变行动方式`，而不是`先算分数再选择`。把 policy parameters 朝 expected reward 增大的方向更新，是这种方法的代表性下位方式。
...
- 相关概念: `reinforcement learning`, `policy`, `expected reward`
- 核心 Section: `P4-19.2`
