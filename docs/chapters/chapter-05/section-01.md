...
한국어에서는 `learning`도 `training`도 모두 `학습`으로 번역되는 경우가 많습니다. 그래서 AI 문서를 읽을 때 두 층위가 쉽게 섞입니다. 이 책에서는 혼동을 줄이기 위해 다음처럼 구분해서 읽겠습니다.
...
따라서 이 절의 제목인 “학습은 무엇을 바꾸는가”에서의 학습은 입문자를 위한 넓은 표현입니다. 본문에서 내부 값의 조정을 말할 때는 더 정확히 `훈련(training)` 또는 `학습 절차`라고 부르겠습니다.
...
| 학습 절차나 모델 설정값 | learning rate, max depth, temperature | 하이퍼파라미터 또는 설정값 |
