...
영어권 사전과 중국어권 사전의 표현을 비교하면, 인공지능은 대체로 컴퓨터 시스템(computer system), 기계(machine), 알고리즘(algorithm)이 인간 지능(human intelligence)과 관련된 일부 기능을 수행하거나 모의(simulate)하는 능력 또는 그 분야로 설명됩니다. 반복해서 등장하는 기능은 언어(language), 이미지(image), 문제 해결(problem-solving), 학습(learning), 예측(prediction), 추천(recommendation), 결정(decision)입니다.
