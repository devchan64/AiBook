...
- **P6-4.1 Transformer 在 LLM 中怎样通向下一个候选打分**：把 Transformer 重新读成上下文反映、表示更新与下一个候选打分的流程。
- **P6-4.2 attention 在 context window 内能看到什么**：观察模型在有限范围内计算关系这一点。
- **P6-4.3 补充学习：multi-head attention 与位置表示分别补强什么**：区分顺序信息与多个关系轴。
- **P6-4.4 补充学习：KV cache 在反复生成中不再重算什么**：观察反复生成的重算负担。
- **P6-4.5 补充学习：sparse attention 与 long context 分别想减少什么、保留什么**：区分连接数量控制与长上下文保持。
...
- **P6-10.3 补充学习：CoT、self-consistency、automatic prompt optimization 进一步调整什么**：区分 prompt 层补强策略及其边界。
...
- **P6-16.2 自动评价与人工评价如何划分重复检查和语境判断**：观察重复检查与语境判断的分工。
