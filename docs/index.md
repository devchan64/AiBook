...
- 영어: Part 1을 먼저 공개했고 나머지 Part는 순차적으로 추가
- 중국어(간체): 소개, 목차, Part 1 개요·마무리 페이지를 먼저 공개했고 Section 본문은 순차적으로 추가

즉, 현재 시점에서 영어와 중국어(간체) 페이지는 더 이상 단순 `TBD` 상태는 아니지만, 한국어 기본 본문에 비해 아직 미완성입니다.
