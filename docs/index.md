...
어떤 개념이 다시 나올 때는 같은 설명을 매번 길게 반복하기보다, 그 Part에서 처음 자세히 설명한 대표 Section를 기준으로 읽는 편이 좋습니다. 읽다가 용어가 헷갈리면 개념사전으로 돌아가 표제어와 `중심 Section`을 함께 확인하고, 현재 절과의 연결이 궁금하면 `등장 Section`까지 이어서 보는 편이 좋습니다.

처음부터 모든 절을 같은 밀도로 읽을 필요는 없습니다. 소개 페이지와 각 Part 시작 페이지에서 지금 단계의 질문을 먼저 잡고, 본문 Section에서 대표 설명 위치를 읽고, 이후 절에서는 현재 맥락에서 무엇이 달라지는지 확인하는 순서로 읽으면 부담이 줄어듭니다.
