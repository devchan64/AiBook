...
Pushing every operational problem into a prediction problem is not good data modeling. In some cases, a comparison report is more honest and fits the current data state better. This is especially true when cause labels are weak, or when what the operator actually wants is not `correct classification` but `choosing what to inspect first right now`. In this final judgment on problem type in Part 3, you also need to organize the possibility that some problems are more correctly left as comparison reports all the way through instead of being raised upward.

At this point, the structure `input -> correct label -> automatic decision` comes to mind first, so real problems can start to feel as if they must all fit that frame. But in operational data, `what should be shown first` is often more important than `what should be matched correctly`. If such a problem is forced into a classification problem, it becomes easy to create exaggerated automation while label quality is still weak.
...
- Human follow-up checking is part of the operating procedure.
...
This table shows that a comparison report is not `a temporary artifact used only because prediction is not possible`. It is a different problem setup from the start. A comparison report helps the operator read the state and decide the next action. A prediction problem is a structure for automatically matching a relatively stable target label.
...
| Review priority can be set, but confirmed labels are weak | Comparison report or review queue | The operator wants to know what to look at first, while classification answers are still weak |
