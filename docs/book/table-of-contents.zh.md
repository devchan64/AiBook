...
- **P7-1.3 练习重新设计基准线**：直接比较同一份运营日志在更换基准线分界日和比较单位后，回顾第一句话会怎样变化，并练习为什么整体汇总与 `channel-day` 单位会导向不同结论。
...
- **P7-2.3 练习比较实验**：把 baseline、raw 1-NN、部分缩放、标准化放在同一个评估集上并排比较，直接区分哪些失败会被预处理修复，哪些失败仍然因为边界样本或特征不足而留下。
...
- **P7-5.3 练习问题与依据的边界**：在同一组文档上重写问题，直接比较`基于依据的回答`、`依据不足`、`过度延伸风险`三种状态怎样分开。
...
- **P7-6.3 练习审批策略**：在同一运维场景上重新划分哪些步骤可以自动运行、哪些必须立即保留，并直接重写更安全的下一步行动。
...
- **P7-7.3 运营检查模拟**：直接把运营中观测到的信号分成 `立即修复`、`先确认能否复现`、`留到下一轮改进`，练习判断什么问题要先修、什么问题要先重新确认。
