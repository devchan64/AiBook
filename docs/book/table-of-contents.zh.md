...
阅读目录时，也请记住本书的一个基本设计：在同一个 Part 内，重要概念的详细说明会尽量只放在一个 Section 中。后续 Section 只保留当前语境所需的最小连接，而重复出现的核心术语，应当通过 [概念词汇表](/AiBook/en/reference/concept-glossary/) 和与之对应的代表 Section 再次确认。
...
- **P6-3.3 补充学习：第一次阅读位置表示、multi-head attention、KV cache、sparse attention、long-context 的方法**：在不打断主线的前提下补上实现细节。
...
把前面学到的内容用小型产出重新验证。项目文档不仅记录成功结果，也一起记录失败、限制与下一步改进点。

### Module 1. 数据与传统机器学习

#### Project 1. 数据分析迷你项目 `Project Practice`

- **P7-1.1 数据分析迷你项目目标**：用小数据集完成提问、探索与总结。
- **P7-1.2 结果与回顾**：记录分析结果、限制与下一轮问题。

#### Project 2. 传统机器学习预测模型 `Project Practice`

- **P7-2.1 传统机器学习预测模型目标**：把数据切分、学习与评估流程串进一个模型中确认。
- **P7-2.2 基准模型与改进**：比较 baseline 与改进模型。

### Module 2. 深度学习与文本

#### Project 3. 图像分类模型 `Project Practice`

- **P7-3.1 图像分类模型目标**：用简单图像分类确认深度学习训练流程。
- **P7-3.2 错误案例分析**：查看误分类案例并记录局限。

#### Project 4. 文本分类模型 `Project Practice`

- **P7-4.1 文本分类模型目标**：确认把句子分类到标签的整体流程。
- **P7-4.2 分词与评估**：一起观察文本预处理与评估指标。

### Module 3. LLM 服务项目

#### Project 5. 基于文档的 RAG 聊天机器人 `Project Practice`

- **P7-5.1 基于文档的 RAG 聊天机器人目标**：实现一条先检索书中文档再给出有依据回答的流程。
- **P7-5.2 检索质量与回答验证**：记录检索失败、证据缺失与幻觉现象。

#### Project 6. 使用工具的 agent `Project Practice`

- **P7-6.1 使用工具的 agent 目标**：确认文件读取、补丁、构建等受限工具流程。
- **P7-6.2 权限与日志检查**：检查 agent 执行中的权限、审批与记录。

### Module 4. 部署与运营

#### Project 7. 部署与监控 `Service Structure`

- **P7-7.1 部署与监控目标**：建立静态文档部署与基础状态检查流程。
- **P7-7.2 故障记录与改进计划**：把故障、成本与质量问题留成回顾文档。
