...
원천데이터를 바로 모델에 넣기보다, 문제를 표현 가능한 샘플과 표 구조로 다시 만드는 과정을 다룹니다. 이때 데이터 모델링의 앞단에는 `DSS/BI/DW/OLAP`처럼 데이터를 모아 의사결정에 연결하던 데이터 기반 시스템 흐름이 있다는 점도 함께 짚습니다. 저장 구조, 샘플 단위, 기준선 비교, 특징 설계, 해석 경계를 먼저 세운 뒤 머신러닝 Part로 handoff합니다.

### Module 1. 역할과 문제 장면

#### Chapter 1. 데이터 모델링은 데이터 생애주기 어디에 놓이는가 `모델링 관점`

- **P3-1.1 저장 구조와 문제 표현 구조는 어떻게 다른가**: 저장을 위한 구조와 모델 입력을 위한 구조를 구분합니다.
- **P3-1.2 샘플, 특징, 기준선, 출력 구조를 한눈에 보기**: 데이터 모델링이 무엇을 정하는 작업인지 큰 그림을 먼저 잡습니다.

#### Chapter 2. 데이터셋은 처음부터 주어지지 않는다 `모델링 관점`

- **P3-2.1 원천데이터를 처음 보면 왜 바로 모델을 떠올리게 되는가**: 원천데이터와 문제 정의 사이의 성급한 점프를 경계합니다.
- **P3-2.2 데이터셋은 주어진 표가 아니라 다시 만든 구조다**: 데이터셋을 만드는 판단이 모델링의 일부임을 봅니다.

### Module 2. 샘플과 표 구조

#### Chapter 3. 샘플 단위를 어떻게 정하는가 `모델링 관점`

- **P3-3.1 행 하나가 뜻하는 것은 무엇인가**: 한 행이 한 시점인지, 한 동작인지, 한 사례인지 먼저 정합니다.
- **P3-3.2 측정값과 샘플을 혼동하면 왜 문제가 생기는가**: 센서값, 로그 조각, 표의 행을 같은 것으로 읽지 않는 기준을 세웁니다.

#### Chapter 4. 원시 로그를 요약 표로 바꾸는 법 `모델링 관점`

- **P3-4.1 원시 로그, 요약 표, 집계 표의 차이**: 원천 시계열과 분석용 표 구조의 층위를 구분합니다.
- **P3-4.2 같은 평균, 다른 패턴을 어떻게 드러내는가**: 평균 하나로 가려지는 차이를 표와 요약 구조로 드러내는 법을 봅니다.

### Module 3. 특징과 비교 구조

#### Chapter 5. 특징과 중간 표현을 설계하는 법 `모델링 관점`

- **P3-5.1 평균, 기울기, 변동성은 왜 특징이 되는가**: 요약값이 모델 입력으로 바뀌는 이유를 봅니다.
- **P3-5.2 세그먼트 표현과 토큰화된 표현은 어디까지 쓸 수 있는가**: 연속 신호를 나누거나 상징화하는 표현 방식을 비교합니다.

#### Chapter 6. 기준선 비교 구조를 읽는 법 `모델링 관점`

- **P3-6.1 기준선은 왜 데이터 모델링의 일부인가**: baseline을 평가 후반이 아니라 표현 설계의 일부로 다룹니다.
- **P3-6.2 최근 구간과 기준선 비교표는 어떻게 읽는가**: 현재 구간과 평소 기준을 비교하는 표의 의미를 정리합니다.

### Module 4. 해석 경계

#### Chapter 7. 표본 수와 반복성은 어떻게 읽는가 `학습 원리`

- **P3-7.1 표본 수가 적을 때 무엇을 말하지 말아야 하는가**: 표본 부족 상태에서 과도한 결론을 내리지 않는 기준을 세웁니다.
- **P3-7.2 변화 신호와 통계적 보수성을 왜 함께 봐야 하는가**: 경고 신호와 해석의 보수성을 함께 다루는 이유를 봅니다.

### Module 5. 문제 유형과 handoff

#### Chapter 8. 어떤 문제를 예측 문제로 만들 것인가 `모델링 관점`

- **P3-8.1 경고, 검토 후보, 라벨 예측을 어떻게 구분하는가**: 모든 문제를 바로 분류 문제로 만들지 않는 기준을 세웁니다.
- **P3-8.2 어떤 운영 문제는 왜 비교 리포트로 남겨야 하는가**: 예측 대신 비교와 검토 큐가 더 맞는 경우를 봅니다.

#### Chapter 9. 머신러닝 Part로 무엇을 넘길 것인가 `모델링 관점`

- **P3-9.1 Part 4가 이어받을 feature와 target의 전제**: 머신러닝 Part가 받는 입력 구조를 정리합니다.
- **P3-9.2 split, baseline, evaluation으로 넘어가기 전 확인할 것**: 데이터 모델링 이후 무엇이 준비되어야 하는지 체크합니다.
...
- **P4-7.3 보충학습: 필터(filter), 래퍼(wrapper), 차원 축소(dimensionality reduction)를 처음 구분하는 법**: 특징 선택과 차원 축소를 섞지 않기 위한 기준을 보충합니다.

#### Chapter 8. 모델 선택 휴리스틱 `실무 휴리스틱`

- **P4-8.1 모델 선택(model selection)**: 문제와 데이터에 맞는 모델 후보를 좁히는 기준을 봅니다.
- **P4-8.2 기준 모델(baseline)**: 복잡한 모델 전에 비교 기준을 세우는 이유를 봅니다.
