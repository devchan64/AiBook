...
- **P6-1.1 토큰(token)은 무엇인가**: 텍스트가 모델 입력 단위로 나뉘는 방식을 봅니다.
- **P6-1.2 토큰화(tokenization)의 실제 영향**: 비용, 길이, 언어별 차이를 봅니다.
- **P6-1.3 보충학습: 대표 토크나이저 계열은 무엇인가**: BPE, WordPiece, SentencePiece가 무엇의 이름인지부터 먼저 소개합니다.
- **P6-1.4 보충학습: BPE, WordPiece, SentencePiece를 처음 구분하는 법**: 소개된 계열을 실제 분절 차이와 비용·검색 감각으로 구분합니다.
