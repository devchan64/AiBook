...
- **P5-3.2 ReLU, sigmoid, tanh**: 자주 쓰이는 활성화 함수의 차이를 봅니다.
- **P5-3.3 출력층(output layer)과 활성화의 선택**: 문제 유형에 따라 마지막 출력의 해석이 왜 달라지는지 봅니다.
...
- **P5-7.2 SGD, Adam의 직관**: 대표 옵티마이저를 깊지 않게 비교합니다.
...
- **P6-3.3 보충학습: 위치 표현, multi-head attention, KV cache, sparse attention, long-context를 처음 읽는 법**: 구현 세부를 본류를 끊지 않는 범위에서 보충합니다.
...
- **P7-1.1 데이터 분석 미니 프로젝트 목표**: 작은 데이터셋으로 질문, 탐색, 요약을 수행합니다.
- **P7-1.2 결과와 회고**: 분석 결과, 한계, 다음 질문을 기록합니다.

#### Project 2. 전통 머신러닝 예측 모델 `프로젝트 실습`

- **P7-2.1 전통 머신러닝 예측 모델 목표**: 데이터 분리, 학습, 평가 흐름을 하나의 모델로 확인합니다.
- **P7-2.2 기준 모델과 개선**: baseline과 개선 모델을 비교합니다.

### Module 2. 딥러닝과 텍스트

#### Project 3. 이미지 분류 모델 `프로젝트 실습`

- **P7-3.1 이미지 분류 모델 목표**: 간단한 이미지 분류로 딥러닝 학습 흐름을 확인합니다.
- **P7-3.2 오류 사례 분석**: 잘못 분류된 사례를 보고 한계를 기록합니다.

#### Project 4. 텍스트 분류 모델 `프로젝트 실습`

- **P7-4.1 텍스트 분류 모델 목표**: 문장을 라벨로 분류하는 흐름을 확인합니다.
- **P7-4.2 토큰화와 평가**: 텍스트 전처리와 평가 지표를 함께 봅니다.

### Module 3. LLM 서비스 프로젝트

#### Project 5. 문서 기반 RAG 챗봇 `프로젝트 실습`

- **P7-5.1 문서 기반 RAG 챗봇 목표**: 책 문서를 검색해 근거가 있는 답을 만드는 흐름을 구현합니다.
- **P7-5.2 검색 품질과 답변 검증**: 검색 실패, 근거 누락, 환각을 기록합니다.

#### Project 6. 도구를 사용하는 에이전트(agent) `프로젝트 실습`

- **P7-6.1 도구를 사용하는 에이전트 목표**: 파일 읽기, 패치, 빌드처럼 제한된 도구 흐름을 확인합니다.
- **P7-6.2 권한과 로그 검토**: agent 실행의 권한, 승인, 기록을 점검합니다.

### Module 4. 배포와 운영

#### Project 7. 배포와 모니터링 `서비스 구조`

- **P7-7.1 배포와 모니터링 목표**: 정적 문서 배포와 기본 상태 확인 흐름을 잡습니다.
- **P7-7.2 실패 기록과 개선 계획**: 장애, 비용, 품질 문제를 회고 문서로 남깁니다.
