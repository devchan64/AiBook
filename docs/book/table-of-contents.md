...
목차를 읽을 때는 같은 Part 안에서 중요한 개념의 상세 설명이 가능한 한 한 Section에만 놓이도록 구성한다는 점도 함께 기억하면 좋습니다. 이후 절에서는 현재 맥락에 필요한 최소 연결만 남기고, 반복해서 등장하는 핵심 용어는 [개념사전](../reference/concept-glossary.md)과 대표 Section를 함께 참고하도록 설계합니다.
...
이 도식은 단순한 나열이 아니라 앞 Part가 뒤 Part의 읽기 조건이 되는 학습 의존 관계를 보여 줍니다. 예를 들어 Part 1에서 용어와 전체 지도를 먼저 잡아 두어야 Part 3의 데이터 모델링이나 Part 5의 LLM 설명을 덜 막연하게 읽을 수 있습니다. 처음 읽는 독자는 이 그림을 `무엇을 먼저 이해해야 다음 Part가 덜 어려워지는가`를 보여 주는 안내도로 보면 충분합니다.

## 목차 표기

이 책은 공개 설명에서는 `Part > Module > Chapter > Section` 구조로 읽습니다. 실제 소스트리는 `Part > Chapter > Section` 디렉터리 구조를 사용하고, `Module`은 여러 Chapter를 읽기 흐름으로 묶는 편집 단위입니다. Part 7도 프로젝트 성격을 강조해 `Project`라고 부르지만, 디렉터리 이름은 `chapter-XX` 형식을 유지합니다.
...
- **P2-7.8 보충학습: 셸 스크립트(shell script), 파이프(pipe), 리다이렉션(redirection), 환경 변수(environment variable)를 처음 읽는 법**: 터미널 실습에서 자주 만나는 문법을 보충합니다.
- **P2-7.9 보충학습: 자주 막히는 로컬 Python 환경 문제를 점검하는 법**: 설치와 실행이 어긋날 때 무엇부터 확인해야 하는지 점검 순서로 정리합니다.
...
- **P2-8.7 보충학습: 참조(reference), 얕은 복사(shallow copy), 깊은 복사(deep copy)를 처음 구분하는 법**: 값이 함께 바뀌는 혼동을 줄이기 위한 최소 기준을 보충합니다.
...
- **P3-5.6 겹치는 입력 창이 많을 때 왜 샘플 수가 실제보다 커 보일 수 있는가**: 입력 창 수와 원천 사건 수를 구분하고, 창 중첩이 표 구조를 어떻게 부풀려 보이게 하는지 정리합니다.
- **P3-5.7 같은 샘플 뒤의 여러 후속 사건은 표 구조에서 어떻게 접어야 하는가**: `any`, `first`, `worst`, `count` 같은 접기 규칙이 결과 표의 뜻을 어떻게 바꾸는지 정리합니다.
...
- **P3-6.6 같은 열 이름이라도 측정 방식이나 단위가 바뀌면 왜 같은 특징이 아닐 수 있는가**: 단위, 센서 버전, 계산 규칙, 운영 정의가 바뀐 열을 같은 특징처럼 섞지 않는 기준을 정리합니다.
...
- **P3-8.4 보수적 해석 문장은 어떻게 경고 열과 검토 큐 기준으로 바뀌는가**: 해석 문장이 `warning_level`, `review_needed`, `priority_score` 같은 구조화된 운영 출력으로 이어지는 다리를 정리합니다.
- **P3-8.5 여러 비교 열은 어떻게 하나의 검토 우선순위 후보로 묶는가**: 차이값 하나가 아니라 변화 크기, 반복성, 해석 신뢰도, 운영 중요도를 함께 묶어 검토 후보를 만드는 축을 정리합니다.
- **P3-8.6 확정 라벨이 검토된 사례에만 남아 있다면 해석에서 무엇을 함께 적어야 하는가**: 선택적으로 붙은 라벨을 전체 사건 대표처럼 읽지 않기 위해 검토 경로와 편중을 함께 적는 법을 정리합니다.
- **P3-8.7 현재의 검토 규칙과 조치가 나중 데이터 자체를 바꾼다면 해석은 왜 달라져야 하는가**: 개입 후 데이터와 자연 경과를 같은 뜻으로 읽지 않기 위한 해석 경계를 정리합니다.
...
- **P3-9.3 비교 리포트, 검토 후보 큐, 목표 라벨 후보 표는 어떻게 다른가**: 같은 데이터가 세 산출물로 어떻게 갈라지는지 봅니다.
...
- **P3-9.6 같은 사건도 사람이나 시기에 따라 다른 라벨이 붙으면 어떻게 해야 하는가**: 목표 라벨 후보 열이 있어도 라벨 의미가 일관되는지 먼저 점검해야 하는 이유를 정리합니다.
...
- **P3-9.10 늦게 확정되는 라벨과 닫히지 않은 음성 라벨은 어떻게 구분하는가**: 라벨 확정 지연과 관측 미완료 음성을 분리해 적는 법을 정리합니다.
- **P3-9.11 target 후보가 여러 개이거나 기준이 바뀔 때 무엇을 먼저 고정해야 하는가**: 대표 target과 정의 버전을 먼저 고정하는 이유를 정리합니다.
- **P3-9.12 같은 target 이름이라도 어떤 오류가 더 아픈지 왜 먼저 적어야 하는가**: false negative와 false positive의 운영 비용 차이를 먼저 적는 이유를 정리합니다.
- **P3-9.13 시간 순서 분할, 개체 분리, 데이터 누출, ranking 문제는 왜 여기서 예고만 하는가**: 뒤 Part 본편에서 다룰 평가·문제 유형 항목의 이름과 필요성만 잡고 넘깁니다.
...
- **P4-6.4 보충학습: ROC, PR, 로그 손실(log loss), 캘리브레이션(calibration), 실루엣(silhouette)을 처음 읽는 법**: 자주 보이지만 낯선 평가 지표를 큰 그림으로 구분합니다.
...
- **P4-7.3 보충학습: 결측치, 스케일, 인코딩을 어떤 입력 문제로 구분하는가**: 전처리 종류를 기술 이름보다 입력 문제 기준으로 나누는 감각을 보충합니다.
- **P4-7.4 보충학습: 필터(filter), 래퍼(wrapper), 차원 축소(dimensionality reduction)를 처음 구분하는 법**: 특징 선택과 차원 축소를 섞지 않기 위한 기준을 보충합니다.
...
- **P4-9.3 보충학습: 고급 모델 선택, 튜닝 자동화, 실험 추적을 큰 그림으로 읽기**: 튜닝 자동화와 실험 관리 도구의 위치를 보충합니다.
...
- **P4-15.4 보충학습: Extra Trees와 랜덤포레스트를 처음 비교하는 법**: 같은 숲 계열 안에서 split 무작위화, bootstrap 기본값, OOB 가능 조건이 어떻게 달라지는지 비교합니다.
...
- **P4-17.3 보충학습: 계층적 군집화와 스펙트럴 클러스터링을 처음 구분하는 법**: 중심, 밀도, 순서, 연결이라는 서로 다른 군집 직관을 다시 묶어 봅니다.
...
- **P4-19.4 보충학습: DQN, PPO, RLHF를 강화학습 큰 흐름 안에서 읽기**: 대표 강화학습 계열을 큰 흐름 안에서 다시 정리합니다.
- **P4-19.5 보충학습: 벨만 방정식, 수렴, 함수 근사를 처음 읽는 법**: 가치 기반 강화학습의 재귀적 가치식과 큰 상태 공간 확장 배경을 처음 연결합니다.
- **P4-19.6 보충학습: policy gradient와 likelihood ratio trick을 처음 읽는 법**: 정책 기반 수식이 왜 로그 확률 기울기 형태로 적히는지 처음 연결합니다.
...
- **P5-13.3 보충학습: query, key, value와 multi-head attention**: Attention 내부 구성요소를 처음 구분하는 기준을 보충합니다.
...
- **P5-15.1 생성 모델(generative model)은 무엇을 배우는가**: 데이터를 분류하는 모델과 생성하는 모델의 차이를 봅니다.
- **P5-15.2 생성과 샘플링(sampling)**: 확률적 출력과 품질 변동의 직관을 봅니다.


## Part 6. LLM과 생성형 AI

Transformer 이후의 흐름을 이해하고, LLM을 실제 서비스와 연결합니다. 읽는 본류를 먼저 세우고, LLM 발전사와 BERT 계열은 뒤쪽 배경 Module에서 비교 관점으로 정리합니다.

### Module 1. 입력 단위와 표현

#### Chapter 1. 토큰과 토큰화 `LLM 핵심`

- **P6-1.1 왜 토큰이 필요한가**: 길이 제한, 비용, 잘림, 입력 분할 문제를 왜 다른 계산 단위로 읽어야 하는지 봅니다.
- **P6-1.2 토큰(token)은 무엇인가**: 모델이 입력과 출력을 세는 기본 계산 단위를 잡습니다.
- **P6-1.3 토큰은 어떻게 사용하나**: 길이, 비용, context window, 문서 분할을 토큰 기준으로 읽는 법을 봅니다.
- **P6-1.4 토큰화(tokenization)는 무엇인가**: 문자열이 어떻게 모델 계산용 토큰 시퀀스로 바뀌는지 봅니다.
- **P6-1.5 토큰화는 무엇을 바꾸는가**: 같은 문장도 토큰 수와 경계가 달라지며 비용, 청크, 출력 길이가 흔들릴 수 있음을 봅니다.
- **P6-1.6 대표 토크나이저 계열은 무엇인가**: BPE, WordPiece, SentencePiece가 무엇의 이름인지 소개합니다.
- **P6-1.7 토큰화 종류 차이는 언제 드러나는가**: 계열 차이가 비용, 청크 경계, 혼합 표기 길이 감각에서 언제 보이기 시작하는지 봅니다.
- **P6-1.8 토큰 관점은 어디에 활용되는가**: 프롬프트 길이, RAG 청크 설계, 비용 예산, 긴 문서 입력, 출력 형식 설계에 어떻게 쓰이는지 봅니다.

#### Chapter 2. 임베딩 `LLM 핵심`

- **P6-2.1 임베딩(embedding)의 직관**: 토큰과 문장을 벡터로 표현하는 이유를 봅니다.
- **P6-2.2 의미와 거리**: 비슷한 표현이 가까워진다는 직관과 한계를 봅니다.
- **P6-2.3 보충학습: 임베딩 학습과 ANN 검색을 큰 그림으로 읽기**: 임베딩과 검색 인프라가 어디에서 다시 만나는지 미리 잡습니다.

### Module 2. LLM 핵심 구조

#### Chapter 3. Transformer 구조 복습 `딥러닝 구조`

- **P6-3.1 Transformer를 LLM 관점에서 다시 읽기**: Part 5의 Transformer를 생성형 언어 모델 관점에서 다시 봅니다.
- **P6-3.2 attention과 context window**: 문맥을 처리하는 구조적 조건을 봅니다.
- **P6-3.3 보충학습: 위치 표현과 multi-head attention을 처음 읽는 법**: 문맥을 읽는 방식 보강을 먼저 구분합니다.
- **P6-3.4 보충학습: KV cache를 처음 읽는 법**: 반복 생성에서 왜 같은 계산을 다시 하지 않으려 하는지 보충합니다.
- **P6-3.5 보충학습: sparse attention과 long-context를 처음 읽는 법**: 긴 문맥 계산과 장문맥 유지 문제를 본류를 끊지 않는 범위에서 보충합니다.

#### Chapter 4. GPT 계열 `LLM 핵심`

- **P6-4.1 GPT 계열의 위치**: 디코더 기반 생성 모델의 흐름을 봅니다.
- **P6-4.2 대화형 LLM으로의 전환**: 모델 사용 경험이 바뀐 지점을 봅니다.

#### Chapter 5. 다음 토큰 예측과 생성 `LLM 핵심`

- **P6-5.1 다음 토큰 예측(next-token prediction)**: 생성형 언어 모델의 기본 학습 목표를 봅니다.
- **P6-5.2 생성 과정의 직관**: 한 번에 답을 꺼내는 것이 아니라 순차적으로 생성하는 흐름을 봅니다.

### Module 3. 학습과 조정

#### Chapter 6. 사전학습 `학습 원리`

- **P6-6.1 사전학습(pretraining)**: 대규모 데이터에서 일반 패턴을 배우는 과정을 봅니다.
- **P6-6.2 데이터와 스케일**: 규모가 모델 성능에 주는 영향을 조심스럽게 봅니다.

#### Chapter 7. 파인튜닝 `학습 원리`

- **P6-7.1 파인튜닝(fine-tuning)**: 특정 목적에 맞게 모델을 조정하는 방식을 봅니다.
- **P6-7.2 LoRA와 효율적 조정**: 전체 모델을 다 바꾸지 않는 조정 방법을 소개합니다.
- **P6-7.3 보충학습: LoRA, adapter, QLoRA를 처음 구분하는 법**: 효율적 조정 기법의 관계를 보충합니다.

#### Chapter 8. 지시 튜닝과 정렬 `LLM 핵심`

- **P6-8.1 지시 튜닝(instruction tuning)**: 사용자의 지시를 따르도록 학습시키는 흐름을 봅니다.
- **P6-8.2 정렬(alignment)의 기본 문제**: 유용성과 안전성을 함께 맞추는 문제를 봅니다.
- **P6-8.3 프롬프트, 파인튜닝, RAG, 도구 사용을 언제 고를까**: 조정 수단의 선택 기준을 한자리에 묶습니다.

### Module 4. 프롬프트와 근거 연결

#### Chapter 9. 프롬프트 엔지니어링 `LLM 핵심`

- **P6-9.1 프롬프트 엔지니어링(prompt engineering)**: 입력을 설계해 모델 행동을 관찰하고 조정하는 방식을 봅니다.
- **P6-9.2 프롬프트의 한계**: 프롬프트가 보장하지 못하는 지점을 봅니다.
- **P6-9.3 보충학습: Chain-of-thought, self-consistency, automatic prompt optimization을 처음 읽는 법**: 프롬프트 확장 기법을 큰 그림으로 구분합니다.

#### Chapter 10. RAG `서비스 구조`

- **P6-10.1 RAG의 필요성**: 모델 기억에만 의존하지 않고 외부 근거를 연결하는 이유를 봅니다.
- **P6-10.2 검색 결과와 생성의 결합**: 검색된 문서를 입력으로 넣는 흐름을 봅니다.

#### Chapter 11. 벡터 데이터베이스 `서비스 구조`

- **P6-11.1 벡터 데이터베이스(vector database)**: 임베딩을 저장하고 찾는 시스템을 봅니다.
- **P6-11.2 인덱스(index)와 검색 품질**: 빠른 검색과 정확한 검색의 균형을 봅니다.

### Module 5. 실행 구조

#### Chapter 12. 도구 사용(tool use) `서비스 구조`

- **P6-12.1 도구 사용(tool use)**: LLM이 외부 기능을 호출하는 방식을 봅니다.
- **P6-12.2 함수 호출(function calling)**: 도구 사용의 구조화된 입력과 출력을 봅니다.

#### Chapter 13. 에이전트(agent) `서비스 구조`

- **P6-13.1 에이전트(agent)**: 목표를 여러 단계 작업으로 이어 가는 구조를 봅니다.
- **P6-13.2 계획, 행동, 관찰**: agent loop의 기본 흐름을 봅니다.

#### Chapter 14. MCP(Model Context Protocol)와 하네스(harness) `서비스 구조`

- **P6-14.1 MCP와 도구 연결**: 외부 도구와 데이터 연결 표준화 흐름을 봅니다.
- **P6-14.2 하네스(harness)**: 실행, 로그, 평가, 재현성을 감싸는 환경을 봅니다.

### Module 6. 평가, 운영, 통합 실습

#### Chapter 15. LLM 평가 `LLM 핵심`

- **P6-15.1 LLM 평가(evaluation)**: 답변 품질을 어떻게 확인할지 봅니다.
- **P6-15.2 자동 평가와 사람 평가**: 평가 방식의 장단점을 봅니다.

#### Chapter 16. AI 서비스가 현실에서 만나는 제약 `서비스 구조`

- **P6-16.1 AI 서비스가 현실에서 만나는 제약**: 비용, 지연 시간, 사용량 제한을 실제 운영 관점에서 봅니다.
- **P6-16.2 운영 중 실패 대응**: 장애, 품질 저하, 보안 문제에 대응하는 기준을 봅니다.

#### Chapter 17. 작은 생성형 AI 기능을 한 흐름으로 묶기 `프로젝트 실습`

- **P6-17.1 작은 생성형 AI 기능을 한 흐름으로 묶기**: 입력, 검색, 생성, 도구 호출이 하나의 기능 안에서 어떻게 이어지는지 확인합니다.
- **P6-17.2 최소 구현과 회고 포인트**: 무엇을 직접 구현하고 무엇을 회고해야 하는지 정리합니다.

### Module 7. 배경 지도: LLM 발전사

#### Chapter 18. LLM 발전사 `역사와 패러다임`

- **P6-18.1 LLM 발전사의 큰 흐름**: 통계적 언어 모델에서 사전학습 LLM까지 연결합니다.
- **P6-18.2 직접 계보와 주변 근거**: LLM의 직접 발전사와 딥러닝 확산 사례를 구분합니다.

### Module 8. 배경 비교: BERT 계열

#### Chapter 19. BERT 계열 `LLM 핵심`

- **P6-19.1 BERT 계열의 위치**: 인코더 기반 사전학습 모델의 역할을 봅니다.
- **P6-19.2 이해 중심 태스크**: 분류, 검색, 임베딩과 연결되는 흐름을 봅니다.


## Part 7. 프로젝트

Part 7은 앞에서 배운 데이터 모델링, 머신러닝, 딥러닝, LLM 구조를 실제 프로젝트 단위로 다시 실행해 보는 구간입니다. 여기서는 새로운 이론을 길게 추가하기보다, 같은 문제를 어떤 질문으로 닫고 어떤 입력 단위와 출력 단위로 정의하며 어떤 baseline과 구조를 선택해야 하는지를 코드, 로그, 비교표, 도식으로 직접 확인합니다. 각 장은 `문제 정의 -> 실행 -> 비교 -> 해석 -> 다음 반복`의 흐름을 따라가며, 장난감 예제를 빠르게 통과하는 데서 멈추지 않고 실제로 검토할 만한 결과, 실패, 제약, 운영 기록까지 함께 다룹니다.

### Module 1. 프로젝트 문제 정의와 기준선

#### Chapter 1. 프로젝트 질문과 baseline `프로젝트 실습`

- **P7-1.1 프로젝트 질문과 입력 정의**: 같은 데이터라도 무엇을 예측하거나 구분하려는지, 입력 샘플을 어디까지 한 건으로 볼 것인지에 따라 전혀 다른 프로젝트가 되며, 그 차이가 실제 데이터 준비와 실험 설계까지 바꾼다는 점을 확인합니다.
- **P7-1.2 baseline과 첫 비교**: 가장 단순한 기준선을 먼저 실행해 보고, 이후의 모델 성능을 해석할 때 왜 `얼마나 좋아 보이는가`보다 `무엇과 비교해 좋아졌는가`가 먼저여야 하는지 실제 비교표와 실패 사례까지 포함해 확인합니다.
- **P7-1.3 기준선 재설계 연습**: 같은 운영 로그라도 기준선 경계일과 비교 단위를 바꾸면 회고 앞줄이 어떻게 달라지는지 직접 비교하면서, 전체 합계와 채널-일자 단위가 왜 다른 결론을 만들 수 있는지 연습합니다.

#### Chapter 2. 결과 해석과 회고 `프로젝트 실습`

- **P7-2.1 비교표와 오류 사례 읽기**: 점수 하나만 보는 대신 예측 결과 비교표와 대표 오류 사례를 함께 읽어, 모델이 어디서 맞고 어디서 틀리는지를 구체적으로 해석합니다.
- **P7-2.2 회고와 다음 질문 만들기**: 실행 결과를 `사실`, `해석`, `다음 질문`으로 나누어 정리하면서, 한 번의 실험 결과를 다음 반복의 더 나은 질문과 더 나은 실험 설계로 넘기는 방법을 연습합니다.
- **P7-2.3 비교 실험 연습**: baseline, raw 1-NN, 부분 스케일 조정, 정규화를 같은 평가 셋에서 나란히 놓고, 어떤 실패는 전처리로 해결되고 어떤 실패는 더 많은 경계 사례나 특징이 필요해 남는지 직접 구분합니다.

### Module 2. 구조 선택이 드러나는 모델 프로젝트

#### Chapter 3. 입력 구조와 모델 구조 선택 `프로젝트 실습`

- **P7-3.1 입력 구조는 프로젝트를 어떻게 바꾸는가**: 표 데이터, 이미지, 시퀀스 입력이 데이터 준비 코드, 샘플 확인 방식, 모델 입출력 정의를 어떻게 바꾸는지 나란히 비교합니다.
- **P7-3.2 CNN, 순차 모델, attention 계열 비교**: 같은 목표라도 입력 구조와 참조해야 할 관계가 다르면 왜 다른 계산 구조를 선택해야 하는지, 선택 이유를 설명 가능한 기준과 실제 구현 부담까지 포함해 정리합니다.

#### Chapter 4. 학습 결과 검토 `프로젝트 실습`

- **P7-4.1 loss, metric, 오류 사례 함께 읽기**: 학습 곡선, 평가 지표, 오류 샘플을 함께 봐야만 `학습은 되었지만 실제로 무엇이 좋아졌는가`를 제대로 판단할 수 있다는 점을 확인합니다.
- **P7-4.2 실패한 결과를 다시 분해하기**: 기대보다 나쁜 결과가 나왔을 때 이를 막연한 실패로 남기지 않고, 구조 문제인지 데이터 문제인지 baseline 설정 문제인지로 나누어 다음 실험 방향과 수정 우선순위를 정합니다.
- **P7-4.3 표현 정규화 연습**: 같은 문의를 정규화 전후로 다시 실행하며, 동의어 치환이 coverage, 예측, 회고 우선순위를 어떻게 바꾸는지 직접 비교합니다.

### Module 3. LLM 서비스 프로젝트

#### Chapter 5. RAG 프로젝트 검증 `프로젝트 실습`

- **P7-5.1 검색, 근거, 답변 흐름 실습**: 검색 후보가 어떻게 모이고, 어떤 근거가 선택되며, 그 근거 위에서 답변이 어떻게 만들어지는지를 하나의 파이프라인으로 직접 확인합니다.
- **P7-5.2 검색 실패와 근거 누락 기록**: 답변이 부정확할 때 이를 막연히 `LLM이 틀렸다`로 묶지 않고, 검색 실패인지 근거 부족인지 답변 구성 문제인지로 나누어 기록하며, 서비스 품질 검토에 필요한 로그 관점까지 함께 정리합니다.
- **P7-5.3 질문-근거 경계 연습**: 같은 문서 집합 위에서 질문 문구를 다시 쓰며, `근거 부족`, `답변 과장 위험`, `근거 기반 답변` 상태가 어떻게 갈리는지 직접 비교합니다.

#### Chapter 6. 에이전트 실행 기록 검토 `프로젝트 실습`

- **P7-6.1 계획, 도구 호출, 승인 흐름 실습**: 에이전트가 어떤 계획을 세우고 어떤 도구를 어떤 순서로 호출하며, 어디서 승인과 제약을 만나는지를 실행 경로 중심으로 읽습니다.
- **P7-6.2 권한, 로그, blocked 상태 검토**: 실행 로그를 permission, blocked, next action 기준으로 다시 정리하면서, 에이전트 프로젝트에서 왜 결과만큼 실행 기록과 실패 처리 규칙 자체가 중요해지는지 확인합니다.
- **P7-6.3 승인 정책 연습**: 같은 운영 시나리오 위에서 자동 실행 가능 단계와 즉시 보류 단계를 다시 나누고, 더 안전한 다음 행동을 직접 써 보며 승인 정책의 역할을 확인합니다.

### Module 4. 배포와 운영 회고

#### Chapter 7. 배포 점검과 운영 회고 `프로젝트 실습`

- **P7-7.1 배포 확인과 상태 점검**: 빌드 성공 여부만 보는 데서 끝나지 않고, 배포 후 공개 상태와 기본 동작이 실제로 유지되는지를 작은 점검 코드와 흐름도로 검토합니다.
- **P7-7.2 장애 기록과 다음 반복 계획**: 실패 사건을 단순한 사고 기록이 아니라, 원인 추정과 우선순위, 다음 조치, 재발 방지 관점이 함께 있는 운영 회고로 남기는 방법을 정리합니다.
- **P7-7.3 운영 점검 시뮬레이션**: 운영 중 관찰된 신호를 `즉시 수정`, `재현 확인`, `다음 반복 개선`으로 직접 분류하면서, 실제로 어떤 문제를 먼저 고치고 어떤 문제는 다시 확인해야 하는지 판단하는 연습을 합니다.
