...
이 책은 공개 설명에서는 `Part > Module > Chapter > Section` 구조로 읽습니다. 다만 실제 소스트리는 `Part > Chapter > Section` 디렉터리 구조를 사용하며, `Module`은 여러 Chapter를 읽기 흐름으로 묶는 편집 단위입니다. 즉, Module 번호는 목차 설명용이고 파일 경로에는 별도 Module 디렉터리가 생기지 않습니다. Part 6도 프로젝트 성격을 강조해 `Project`라고 부르지만, 실제 소스트리 디렉터리는 `chapter-XX` 형식을 유지합니다.
...
- **P1-11.1 통계적 언어 모델(statistical language model)과 임베딩(embedding)**: LLM 이전의 언어 모델링 흐름을 봅니다.
- **P1-11.2 RNN, Seq2Seq, Attention**: 순차 데이터와 번역 연구가 만든 전환점을 봅니다.
- **P1-11.3 Transformer와 사전학습 LLM(pretrained LLM)**: 현재 LLM의 직접 계보를 잡습니다.

### Module 5. LLM 사용 경험의 핵심 요소

#### Chapter 12. 프롬프트와 LLM 사용 경험 `LLM 핵심`

- **P1-12.1 프롬프트(prompt)는 무엇을 지정하는가**: 지시, 조건, 맥락을 입력으로 주는 방식을 봅니다.
- **P1-12.2 지시(instruction), 맥락(context), 예시(example)**: 프롬프트를 구성하는 기본 요소를 정리합니다.
- **P1-12.3 프롬프트(prompt)의 한계(limit)와 평가(evaluation)**: 프롬프트가 보장하지 못하는 것과 검증 기준을 봅니다.

#### Chapter 13. 임베딩과 벡터 검색 `LLM 핵심`

- **P1-13.1 텍스트(text)를 벡터(vector)로 표현한다는 것**: 의미를 수치 공간에 놓는 직관을 잡습니다.
- **P1-13.2 유사도 검색(similarity search)의 직관**: 가까운 벡터를 찾는 방식과 한계를 봅니다.
- **P1-13.3 RAG(retrieval-augmented generation)로 이어지는 흐름**: 검색 결과를 LLM 입력으로 연결하는 구조를 봅니다.
...
- **P1-14.2 RAG(retrieval-augmented generation)와 도구 사용(tool use)의 위치**: 외부 자료와 도구 호출이 왜 필요한지 봅니다.
...
- **P3-6.3 보충학습: 사이트 신뢰성 엔지니어링에서 매트릭스(metrics)를 읽는 법**: 머신러닝 평가 지표와 운영 지표가 어떻게 다르고, SLI/SLO, 에러 버짓(error budget), 경보(alert) 같은 운영 관점이 무엇을 추가하는지 보충학습으로 정리합니다.
- **P3-6.4 보충학습: ROC, PR, 로그 손실(log loss), 캘리브레이션(calibration), 실루엣(silhouette)을 처음 읽는 법**: 자주 보이지만 낯선 평가 지표를 큰 그림으로 구분합니다.
...
- **P3-7.3 보충학습: 필터(filter), 래퍼(wrapper), 차원 축소(dimensionality reduction)를 처음 구분하는 법**: 특징 선택과 차원 축소를 섞지 않기 위한 기준을 보충합니다.
...
- **P3-9.3 보충학습: 고급 모델 선택, 튜닝 자동화, 실험 추적을 큰 그림으로 읽기**: 튜닝 자동화와 실험 관리 도구의 위치를 보충합니다.
...
- **P3-10.3 보충학습: 회귀 진단(regression diagnostics)을 처음 읽는 법**: 잔차 패턴과 가정 점검을 처음 볼 때 필요한 기준을 보충합니다.
...
- **P3-19.4 보충학습: DQN, PPO, RLHF를 강화학습 큰 흐름 안에서 읽기**: 대표 강화학습 계열을 큰 흐름 안에서 다시 정리합니다.


## Part 4. 딥러닝

신경망이 데이터를 어떻게 표현하고 학습하는지 이해합니다. 이 Part는 현재 소스트리의 15개 Chapter를 유지하되, 읽는 흐름을 8개의 Module로 다시 묶습니다. 즉, Chapter 번호는 실제 파일 구조를 따르고, Module은 학습 순서를 더 분명하게 보여 주기 위한 상위 묶음입니다.
...
- **P4-8.3 학습 루프를 한 번에 다시 묶기**: forward, loss, backward, optimizer step이 하나의 반복으로 묶이는 흐름을 정리합니다.
...
Transformer 이후의 흐름을 이해하고, LLM을 실제 서비스와 연결합니다. 이 Part도 실제 소스트리의 19개 Chapter 번호를 유지하면서, 읽는 본류와 배경 축이 드러나도록 8개의 Module로 다시 묶습니다. Chapter 3과 Chapter 5는 소스트리상 앞쪽 번호를 갖고 있지만, 현재 학습 흐름에서는 `배경 지도`로 뒤쪽 Module에 배치합니다.
...
- **P5-1.3 보충학습: BPE, WordPiece, SentencePiece를 처음 구분하는 법**: 토큰화 계열을 너무 깊게 들어가지 않고 큰 그림으로 구분합니다.

#### Chapter 2. 임베딩 `LLM 핵심`

- **P5-2.1 임베딩(embedding)의 직관**: 토큰과 문장을 벡터로 표현하는 이유를 봅니다.
- **P5-2.2 의미와 거리**: 비슷한 표현이 가까워진다는 직관과 한계를 봅니다.
- **P5-2.3 보충학습: 임베딩 학습과 ANN 검색을 큰 그림으로 읽기**: 임베딩과 검색 인프라가 어디에서 다시 만나는지 미리 잡습니다.
...
- **P5-4.1 GPT 계열의 위치**: 디코더 기반 생성 모델의 흐름을 봅니다.
- **P5-4.2 대화형 LLM으로의 전환**: 모델 사용 경험이 바뀐 지점을 봅니다.

#### Chapter 5. 다음 토큰 예측과 생성 `LLM 핵심`

- **P5-5.1 다음 토큰 예측(next-token prediction)**: 생성형 언어 모델의 기본 학습 목표를 봅니다.
- **P5-5.2 생성 과정의 직관**: 한 번에 답을 꺼내는 것이 아니라 순차적으로 생성하는 흐름을 봅니다.

### Module 3. 학습과 조정

#### Chapter 6. 사전학습 `학습 원리`

- **P5-6.1 사전학습(pretraining)**: 대규모 데이터에서 일반 패턴을 배우는 과정을 봅니다.
- **P5-6.2 데이터와 스케일**: 규모가 모델 성능에 주는 영향을 조심스럽게 봅니다.

#### Chapter 7. 파인튜닝 `학습 원리`

- **P5-7.1 파인튜닝(fine-tuning)**: 특정 목적에 맞게 모델을 조정하는 방식을 봅니다.
- **P5-7.2 LoRA와 효율적 조정**: 전체 모델을 다 바꾸지 않는 조정 방법을 소개합니다.
- **P5-7.3 보충학습: LoRA, adapter, QLoRA를 처음 구분하는 법**: 효율적 조정 기법의 관계를 보충합니다.

#### Chapter 8. 지시 튜닝과 정렬 `LLM 핵심`

- **P5-8.1 지시 튜닝(instruction tuning)**: 사용자의 지시를 따르도록 학습시키는 흐름을 봅니다.
- **P5-8.2 정렬(alignment)의 기본 문제**: 유용성과 안전성을 함께 맞추는 문제를 봅니다.
- **P5-8.3 프롬프트, 파인튜닝, RAG, 도구 사용을 언제 고를까**: 조정 수단의 선택 기준을 한자리에 묶습니다.

### Module 4. 프롬프트와 근거 연결

#### Chapter 9. 프롬프트 엔지니어링 `LLM 핵심`

- **P5-9.1 프롬프트 엔지니어링(prompt engineering)**: 입력을 설계해 모델 행동을 관찰하고 조정하는 방식을 봅니다.
- **P5-9.2 프롬프트의 한계**: 프롬프트가 보장하지 못하는 지점을 봅니다.

#### Chapter 10. RAG `서비스 구조`

- **P5-10.1 RAG의 필요성**: 모델 기억에만 의존하지 않고 외부 근거를 연결하는 이유를 봅니다.
- **P5-10.2 검색 결과와 생성의 결합**: 검색된 문서를 입력으로 넣는 흐름을 봅니다.

#### Chapter 11. 벡터 데이터베이스 `서비스 구조`

- **P5-11.1 벡터 데이터베이스(vector database)**: 임베딩을 저장하고 찾는 시스템을 봅니다.
- **P5-11.2 인덱스(index)와 검색 품질**: 빠른 검색과 정확한 검색의 균형을 봅니다.

### Module 5. 실행 구조

#### Chapter 12. 도구 사용(tool use) `서비스 구조`

- **P5-12.1 도구 사용(tool use)**: LLM이 외부 기능을 호출하는 방식을 봅니다.
- **P5-12.2 함수 호출(function calling)**: 도구 사용의 구조화된 입력과 출력을 봅니다.

#### Chapter 13. 에이전트(agent) `서비스 구조`

- **P5-13.1 에이전트(agent)**: 목표를 여러 단계 작업으로 이어 가는 구조를 봅니다.
- **P5-13.2 계획, 행동, 관찰**: agent loop의 기본 흐름을 봅니다.

#### Chapter 14. MCP(Model Context Protocol)와 하네스(harness) `서비스 구조`

- **P5-14.1 MCP와 도구 연결**: 외부 도구와 데이터 연결 표준화 흐름을 봅니다.
- **P5-14.2 하네스(harness)**: 실행, 로그, 평가, 재현성을 감싸는 환경을 봅니다.

### Module 6. 평가, 운영, 통합 실습

#### Chapter 15. LLM 평가 `LLM 핵심`

- **P5-15.1 LLM 평가(evaluation)**: 답변 품질을 어떻게 확인할지 봅니다.
- **P5-15.2 자동 평가와 사람 평가**: 평가 방식의 장단점을 봅니다.

#### Chapter 16. AI 서비스가 현실에서 만나는 제약 `서비스 구조`

- **P5-16.1 AI 서비스가 현실에서 만나는 제약**: 비용, 지연 시간, 사용량 제한을 봅니다.
- **P5-16.2 운영 중 실패 대응**: 장애, 품질 저하, 보안 문제에 대응하는 기준을 봅니다.

#### Chapter 17. 작은 생성형 AI 기능을 한 흐름으로 묶기 `프로젝트 실습`

- **P5-17.1 작은 생성형 AI 기능을 한 흐름으로 묶기**: 입력, 생성, 근거 연결, 실행 구조를 하나의 최소 기능으로 다시 묶습니다.
- **P5-17.2 최소 구현과 회고 포인트**: 무엇을 직접 구현하고 무엇을 회고해야 하는지 정리합니다.

### Module 7. 배경 지도: LLM 발전사

#### Chapter 18. LLM 발전사 `역사와 패러다임`

- **P5-18.1 LLM 발전사의 큰 흐름**: 통계적 언어 모델에서 사전학습 LLM까지 연결합니다.
- **P5-18.2 직접 계보와 주변 근거**: LLM의 직접 발전사와 딥러닝 확산 사례를 구분합니다.

### Module 8. 배경 비교: BERT 계열

#### Chapter 19. BERT 계열 `LLM 핵심`

- **P5-19.1 BERT 계열의 위치**: 인코더 기반 사전학습 모델의 역할을 봅니다.
- **P5-19.2 이해 중심 태스크**: 분류, 검색, 임베딩과 연결되는 흐름을 봅니다.
