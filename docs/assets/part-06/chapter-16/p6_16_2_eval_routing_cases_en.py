...
CSV_PATH = OUT_DIR / "p6_16_2_eval_routing_cases_en.csv"
...
HUMAN_REVIEW_RUBRIC = [
    "Does the source signal actually support the answer's conclusion?",
    "Can the user immediately understand the next action?",
    "Is the tone or overconfidence risky in context?",
    "Did the answer omit an important condition or exception while shortening?",
]
...
def grade_fix_note(grades: dict[str, float]) -> str:
    ...
    return "Fix the low automatic-grader items first: " + ", ".join(failed_with_axes)
...
def first_human_review_focus() -> str:
    return "The automatic graders passed. A person now reads context, usefulness, and tone to the end."
