...
def grade_fix_note(grades: dict[str, float]) -> str:
    ...
    if not failed:
        return "-"
    failed_with_axes = [
        f"{name}({GRADER_AXIS_MAP[name]})" for name in failed
    ]
    return "자동 채점기가 낮게 준 항목을 먼저 수정한다: " + ", ".join(
        failed_with_axes
    )
...
def build_llm_grader_prompt(row: dict[str, str], grades: dict[str, float]) -> str:
    return f"""You are an automatic grader for an LLM answer.
...
The reason may quote Korean phrases only when it names evidence from the candidate answer or required signal.
...
def parse_llm_grader_json(raw_text: str) -> dict[str, object]:
    ...
    if not match:
        return {
            "score": 0.0,
            "reason": "LLM grader did not return JSON.",
        }
    try:
        ...
        return {
            "score": 0.0,
            "reason": "LLM grader returned invalid JSON.",
        }
    ...
    return {
        "score": score,
        "reason": reason[:180] if reason else "No reason returned.",
    }


def fallback_reason_from_code_graders(
    row: dict[str, str], grades: dict[str, float]
) -> str:
    ...
    return "LLM judge did not return a reason, so the code grader observations remain the primary record."
...
def run_llm_grader(row: dict[str, str], ollama_available: bool) -> dict[str, object]:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a strict automatic grader. Return only JSON.",
            },
...
def llm_grader_alignment_note(
    grades: dict[str, float], llm_result: dict[str, object]
) -> str:
    if not llm_result["available"]:
        return "LLM 판정기는 이번 실행에서 사용하지 않았다."
    failed_code_graders = [name for name, grade in grades.items() if grade < 1.0]
    llm_score = llm_result["score"]
    if llm_score == 1.0 and failed_code_graders:
        return (
            "LLM 판정기가 코드 채점기의 실패 신호를 놓쳤다. "
            "라우팅은 코드 채점기와 사람 검토 패킷을 기준으로 다시 본다."
        )
    if llm_score != 1.0 and not failed_code_graders:
        return (
            "코드 채점기는 통과했지만 LLM 판정기가 품질 의심을 남겼다. "
            "사람 검토에서 근거와 유용성을 다시 읽는다."
        )
    return "LLM 판정기와 코드 채점기가 대체로 같은 방향의 신호를 냈다."


def first_human_review_focus() -> str:
    return "자동 채점기는 통과했다. 이제 맥락, 유용성, 말투를 사람이 끝까지 읽는다."
