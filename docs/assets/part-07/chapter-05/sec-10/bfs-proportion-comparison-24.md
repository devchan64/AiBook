...
수평 좌우 90도에서 생성한 첫 24개 입력 후보와 대응하는 Mira 목표를 비교한다. 생성·파일 해시 확인은 완료했다. 사용자는 24개에 특이사항이 없다고 판단했으며, 추가 크로스체크에서는 아래 재검토 사항을 확인했다. 이후 사용자가 **입력 24개를 모두 활용하고 필요한 입력에만 맞춤 Mira 목표를 추가 생성**하기로 결정했다. 아래 차이는 입력 폐기 사유가 아니라 새 목표에서 보존할 입력 조건으로 관리한다. 이후 생성되는 156개는 이 표에 포함하지 않는다.
...
입력 `P710-PROP-001`–`024`는 사용자 채택 상태다. 001·002·005·006·013·015·016·017·019·022·024는 아래 기존 Mira 목표를 재사용한다. 나머지 13개만 같은 번호의 `P710-TGT-NNN` 목표를 추가 생성한다. 생성 목록의 `training_input`·`training_input_sha256`이 채택된 입력을 가리킨다.
...
새 목표 저장 위치는 `target-images/proportion-matched-v1/`다. 현재 156개 입력 생성이 사용하는 GPU가 비면 재사용 11개를 제외한 목표 13개 생성을 시작한다. 최종 학습 쌍은 새 목표의 Mira 정체성과 입력 조건 보존을 확인한 뒤 구성하며, 기존 193쌍과 함께 [통합 217개 데이터셋](p7-5-10-paired-dataset.json)에서 관리한다. 새 목표 생성·검수 전에는 학습 준비를 통과하지 못한다.
