...
기존 실험의 평문 생성 목록은 당시 조건의 원본 기록으로 보존한다. 새 조합 목록의 `source_spec`에 원본 경로와 해시를 연결하고, 공용 생성기가 펼친 프롬프트뿐 아니라 ID·시드·참조·설정 등 전체 내용이 원본과 같은지 검사한다. 기존 조합을 바꾸면 이 검사가 실패한다. 다른 조건을 실험하려면 원본 검증용 목록을 고치는 대신 독립된 새 후보 목록·ID·출력 폴더를 만든다.

완료된 생성 결과의 프롬프트와 파일 해시는 변경하지 않았다. 기존 평문 JSON은 실행 이력 보존용이며, 현재 실행 설정의 `spec`은 조건 조합 JSON을 가리킨다.
