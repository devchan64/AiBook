...
기존 평문 생성 목록과 중복된 실행·폐기 파일은 조건 조합 JSON으로 통합했다. `output_dir`은 저장 위치, `excluded_items`는 폐기 ID·사유·기존 해시를 보존한다. `provenance`에는 원래 파일의 해시와 펼친 조건을 정규화한 기준 해시를 기록한다. 공용 생성기는 프롬프트·ID·시드·참조·설정이 기준 해시와 일치하는지 검사하므로 평문 사본을 유지할 필요가 없다. 다른 조건을 실험하려면 독립된 새 후보 목록·ID·출력 폴더를 만든다.
