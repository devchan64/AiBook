...
import p7_5_10_generate_supplements as generator

BASE = Path(__file__).resolve().parent


def main(policy_name='p7-5-10-input-generation-exclusions.json',
         spec_name='p7-5-10-bfs-input-pool-v2.json', output_name='input-images'):
    ...
    for ref in spec['references'].values():
        assert generator.sha256(generator.asset_path(ref['path'])) == ref['sha256']
    ...
    if args.dry_run:
        return
    # 이관된 완료 후보는 원본 기록 해시로 확인하고 재생성하지 않는다.
    catalog_path = args.output_dir / 'candidate-catalog.json'
    if catalog_path.is_file():
        saved = json.loads(catalog_path.read_text())
        rows = {row['id']: row for row in saved['items']}
        if set(rows) == {item['id'] for item in spec['items']}:
            for row in rows.values():
                assert generator.sha256(generator.asset_path(row['image'])) == row['sha256']
                assert generator.sha256(generator.asset_path(row['record'])) == row['record_sha256']
            print(json.dumps({'status': 'already_generated', 'count': len(rows)}))
            return
