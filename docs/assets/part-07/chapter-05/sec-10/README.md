...
현재 데이터셋 관리는 [통합 217개 목록](p7-5-10-paired-dataset.json)을 사용한다. 기존 193쌍과 목표 재사용 11쌍은 준비 완료이며, 추가 13개만 목표 생성·검수 대기다. sec-12의 193쌍 JSON은 이전 학습·평가 재현 기록으로만 유지한다.

통합 목록의 `generation_rules`에는 기존 입력·목표와 보강 입력·목표의 규칙 ID 4개를 등록한다. 각 행은 규칙 ID·리비전·조건 ID와 입력·목표 결과 ID를 연결한다. 결과 ID는 `sha256:<전체 이미지 해시>`이며 같은 이미지를 재사용하면 같은 ID를 유지한다. 기존 `P711-IN-*`, `P710-PROP-*` 관리번호는 검수용으로 유지하고, 아직 생성하지 않은 목표의 결과 ID는 `null`이다. 실행 중인 생성 조건과 과거 결과 기록은 ID 부여를 위해 변경하지 않는다.
