...
This runner uses direct Diffusers with official Qwen-Image-Edit-2511 BF16
weights and sequential CPU offload. It starts no ComfyUI server or HTTP API.
"""
...
def prompt_for(yaw: str, vertical: str) -> str:
    return f"<sks> {YAW_VIEWS[yaw]['prompt']} {VERTICAL_VIEWS[vertical]['prompt']} {DISTANCE}"
...
def main() -> None:
    ...
    for yaw, vertical in jobs:
        ...
        stem = (
            f"p7-5-2-qwen-2511-mira-head-multiview-vertical-{vertical}-yaw-{yaw}-"
        ...
        record = {
            "status": "generated",
            "experiment_id": "p7-5-2-mira-head-multiview",
            "stage": "head_multiview_camera",
    ...
    batch_result = output_dir / (
        f"p7-5-2-qwen-2511-mira-head-multiview-{args.run_label}-"
