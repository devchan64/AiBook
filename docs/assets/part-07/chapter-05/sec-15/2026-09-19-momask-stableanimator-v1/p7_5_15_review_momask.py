...
def main():
 ...
 for sample in r['samples']:
  ...
  span=max(float(np.ptp(j[:,:,2]))+.5,1.8);center=float((j[:,:,2].min()+j[:,:,2].max())/2)
  def draw(ax,t):
   for chain,c in CHAINS:ax.plot(j[t,chain,2],j[t,chain,1],'-o',c=c,lw=1.6,ms=2)
   ax.axhline(floor,c='#888',lw=.7);ax.set(xlim=(center-span/2,center+span/2),ylim=(floor-.15,max(2.1,float(j[:,:,1].max())+.15)),aspect='equal');ax.axis('off');ax.set_title(f'{t:02d} / {t/20:.2f}s',fontsize=9)
 ...
 (a.folder/'diagnostics.json').write_text(json.dumps({'method':'Raw joints; floor=1st percentile of all four foot joint heights over entire clip. Threshold sensitivity only, not a physical contact ground truth. Fixed side projection z/y; no camera tracking.','samples':summary},indent=2)+'\n')
