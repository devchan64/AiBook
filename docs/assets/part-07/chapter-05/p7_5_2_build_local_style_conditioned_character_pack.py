"""Build the approved limited-scope character pack conditioned on the local style pack."""
...
PREFIX = "p7-5-2-local-style-conditioned-character-pack-v1"
SOURCES = {
...
def main() -> None:
    ...
    entries = [
        ("front", SOURCES["front"], "style-conditioned text-to-image"),
    ...
    sheet = Image.new("RGB", (thumbnail_size[0] * 3, (thumbnail_size[1] + 28) * 2), "white")
    draw = ImageDraw.Draw(sheet)
    for index, (view_id, image_path, _) in enumerate(entries):
        x = (index % 3) * thumbnail_size[0]
        y = (index // 3) * (thumbnail_size[1] + 28)
    ...
    manifest = {
        ...
        "scope": "local-only character identity reference conditioned on p7-5-2-local-style-pack-v1; neutral full-body turnaround views only",
        "excluded_scope": [
            "true three-quarter-front view",
        ...
        "style_source_pack": "p7-5-2-local-style-pack-v1.json",
        "character_master": {
        ...
        "style_contract": {
            ...
            "palette": "low-saturation teal, warm gray, off-white, charcoal, and warm light skin",
            "shading": "soft flat color blocks with subtle single-edge shadows",
            "background": "plain off-white studio background",
        },
        "character_contract": {
