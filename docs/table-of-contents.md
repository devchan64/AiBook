...
- **P7-5.10 보충학습: 캐릭터 일관성을 위한 LoRA 학습과 평가**: 학습용 이미지 생성, 입력·목표 데이터셋 구성, LoRA 학습, 평가 이미지 생성과 출력 결과 검수를 순서대로 진행합니다.
- **P7-5.14 보충학습: 텍스트 모션을 애니메이션 포즈 입력으로 렌더링하기**: 텍스트→MoMask 3D 관절 모션→SCAIL 포즈·마스크 입력의 순서를 분리하고, 계획한 OpenPose 경로와 실제 보존 자료의 차이를 확인합니다.
- **P7-5.15 포즈 시퀀스와 참조 이미지로 캐릭터 애니메이션 생성하기**: 5.14의 33프레임 포즈·마스크를 그대로 사용해, 전신·근접 캐릭터 참조가 동작과 세부 외형에 미치는 범위를 나누어 검수합니다.
- **P7-5.16 보충학습: Qwen-Image-Edit-2511과 AnyPose로 참조 포즈 옮기기**: 캐릭터 기준 이미지와 포즈 안내 이미지의 역할을 나누고, AnyPose 적용 전후의 자세·구도·캐릭터·장면 보존을 검수하는 실행 초안을 세웁니다.
