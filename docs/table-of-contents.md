...
- **P7-5.2 캐릭터 멀티플 뷰 생성: identity 기준과 카메라 앵글 분리하기**: 정면 얼굴·체스트 기준이 identity·헤어·화풍을 맡고, 다중 앵글 LoRA와 짧은 카메라 명령이 한 축의 카메라 변환을 맡도록 분리해 비교합니다.
- **P7-5.3 캐릭터 identity와 추가 페인팅으로 특징 완성하기**: 얼굴 identity를 기준으로 전신 비례·기본 의상·재킷을 이전 결과 위에 단계적으로 더해, 캐릭터 특징을 완성하는 편집 경로를 비교합니다.
- **P7-5.4 스토리보드 장면과 카메라판 만들기**: 공통 화풍 계약의 T2I 장면을 만들고, 그 장면을 방위·높이·거리 조건이 다른 카메라판으로 바꿉니다.
- **P7-5.5 스토리보드 장면에 캐릭터를 합성하는 경로**: 카메라판을 포즈 기준으로 분리하고, 캐릭터 identity·배경·조명을 단계별로 통합합니다.
- **P7-5.11 화풍·연속성 보정: 국소 편집과 참조 역할을 분리해 고치기**: mask inpaint·VTON이 새 카메라의 가림 관계를 재구성하지 못한 한계를 비교하고, 구조 guide·얼굴·완성 착장을 분리한 Qwen reference 편집 경로를 검증합니다.
- **P7-5.12 착장 추출과 Try-On LoRA: 별도 착장 이식 경로를 검수하기**: 착장을 흰 배경 기준물로 분리하고 James·FoxBaze Try-On의 입력 계약과 신발·포즈 보존 한계를 비교합니다.
- **P7-5.13 OpenPose 맵으로 캐릭터 동작과 구도 변경을 시험하기**: OpenPose가 전달하는 2D 관절 배치와 전달하지 못하는 카메라 원근·3D 가림을 분리하고, 동작·고각도 guide 조건의 결과를 비교합니다.
- **P7-5.14 depth·Canny 구조 조건으로 원근과 복장 계약을 분리해 읽기**: depth·Canny가 원근·실루엣을 보조해도 재킷·가방의 복장 계약을 보장하지 못한 결과를 비교합니다.
- **P7-5.15 FaceID 조건으로 얼굴 identity와 전신 구도의 경계 읽기**: FaceID 단독과 FullFace 결합에서 얼굴 단서의 증가, 전신 frame, 착장 유지가 서로 다른 계약임을 비교합니다.
- **P7-5.8 텍스트 모션으로 12개 OpenPose 키프레임 준비하기**: 텍스트→3D 관절 모션→2D OpenPose의 순서를 분리하고, 8GB GPU에서 보행 48포즈를 12개 구조 키프레임으로 추출하는 실험 조건을 준비합니다.
- **P7-5.9 StoryDiffusion으로 여러 컷의 일관성 읽기**: 여러 컷의 인물·복장·화풍을 함께 유지하려는 Consistent Self-Attention의 역할을 단일 이미지 재생성과 구분하고, 8GB 환경에서의 탐색·검수 경계를 정리합니다.
- **P7-5.10 Qwen-Image Q4 GGUF로 8GB에서 실행 기록 남기기**: Q4 모델 파일 크기와 VRAM 상주를 구분하고, CPU 오프로딩으로 완료한 8GB T2I 실행의 조건·시간·한계를 결과 JSON과 함께 읽습니다.
