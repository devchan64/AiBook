...
- **P7-5.1 화풍 참조 셋 생성: 로컬 GPU로 프레임 없는 기준 만들기**: 캐릭터보다 먼저, 프레임 없는 원본·선화·수채화 색층·장소·시간·카메라 폭을 사람이 검수하는 화풍 참조 셋을 만듭니다.
- **P7-5.2 캐릭터 착장·전신 참조 셋 생성: 역할과 승인 범위 정하기**: 화풍과 얼굴 기준을 분리한 뒤, 착장·가방·전신·body-only OpenPose의 승인 범위를 기록한 참조 셋을 만듭니다.
- **P7-5.3 컷신 생성: 참조 셋으로 pose·camera·장면 만들기**: 두 참조 셋을 입력으로 pose·camera·장소가 다른 전체 컷 후보를 만들고 생성 단계 자체를 판정합니다.
- **P7-5.4 화풍·연속성 보정: 컷신의 구조와 디테일을 분리해 고치기**: 고각도 전신에서 ControlNet·mask inpaint·VTON이 화풍·인물·소품·구조를 함께 지키지 못한 실패를 비교하고, 구조 guide·얼굴·완성 착장을 분리한 제한된 후속 경로를 검증합니다.
- **P7-5.5 보충학습: FLUX 다중참조로 참조 묶음 만들기**: FLUX.2 Klein 4B의 다중참조 기능으로 화풍·인물성·pose·장면·국소 detail 참조를 묶는 방법과, 여러 참조가 충돌할 때 무엇을 검수해야 하는지 정리합니다.
- **P7-5.6 보충학습: 3D 선화와 depth+선화로 구조 입력 읽기**: 3D 선화가 주는 윤곽·관절·실루엣과 depth가 더하는 앞뒤 가림·거리 정보를 구분하고, 구조 입력을 character·style 조건과 분리해 검수합니다.
- **P7-5.7 얼굴 정면과 카메라 회전: identity와 시점 역할 분리하기**: 승인 정면 얼굴이 identity·헤어·화풍을 맡고, 다중 앵글 LoRA와 짧은 카메라 명령이 회전만 맡도록 분리해 검수합니다.
