...
- **P7-5.4 컷신 생성: 참조 셋으로 pose·camera·장면 만들기**: 두 참조 셋을 입력으로 pose·camera·장소가 다른 전체 컷 후보를 만들고 생성 단계 자체를 판정합니다.
- **P7-5.11 화풍·연속성 보정: 컷신의 구조와 디테일을 분리해 고치기**: 고각도 전신에서 ControlNet·mask inpaint·VTON이 화풍·인물·소품·구조를 함께 지키지 못한 실패를 비교하고, 구조 guide·얼굴·완성 착장을 분리한 제한된 후속 경로를 검증합니다.
