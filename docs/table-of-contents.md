...
- **P5-14.3 residual과 normalization은 깊은 반복을 어떻게 안정화하는가**: 깊은 블록 반복에서 정보 흐름과 값 범위를 안정화하는 장치를 봅니다.
- **P5-14.4 RNN의 상태 전달과 Transformer의 관계 계산은 병렬 처리에서 어떻게 갈라지는가**: 순차 상태 전달과 토큰 관계 계산의 차이가 GPU 병렬 처리와 대규모 학습으로 이어지는 방식을 봅니다.
- **P5-14.5 긴 문맥에서 순차 상태와 직접 재참조는 어떻게 갈라지는가**: 긴 문맥에서 필요한 앞 위치를 다시 참고하는 감각을 작은 실험과 함께 확인합니다.
- **P5-14.6 보충학습: feed-forward network는 왜 위치별 표현 가공을 맡는가**: attention으로 문맥이 섞인 표현을 각 위치 안에서 다시 가공하는 역할을 따로 정리합니다.
- **P5-14.7 보충학습: residual connection은 왜 원래 표현의 길을 남기는가**: 새 계산 결과와 원래 입력 표현이 함께 다음 단계로 넘어가는 이유를 따로 정리합니다.
- **P5-14.8 보충학습: layer normalization은 왜 값의 기준선을 맞추는가**: 한 위치 표현 안의 값 범위를 다음 계산이 다루기 쉬운 기준선으로 맞추는 이유를 따로 정리합니다.
...
- **P5-15.2 생성 모델(generative model)은 왜 정답 하나 대신 후보 분포를 남기는가**: 생성 모델을 하나의 정답이 아니라 후보 공간과 상대적 그럴듯함으로 읽습니다.
...
- **P6-1.1 생성형 AI 출력은 왜 검토해야 할 산출물인가**: 생성 결과가 분류값이나 수치 예측이 아니라 검토해야 할 산출물이라는 점을 잡습니다.
- **P6-1.2 왜 LLM을 중심 사례로 읽는가**: LLM을 생성형 AI 전체가 아니라 대표 경로로 읽는 이유를 봅니다.
...
- **P6-2.1 왜 LLM 입력과 출력을 토큰 단위로 읽는가**: 입력과 출력이 비용, 길이 제한, 문맥 보존에서 토큰 단위로 제한되는 이유를 봅니다.
- **P6-2.2 토큰과 token ID는 모델 입력에서 어떻게 쓰이나**: 문자열, 토큰, token ID 순서열을 한 흐름으로 구분합니다.
- **P6-2.3 토큰화(tokenization)는 무엇을 바꾸는가**: 토큰화 절차가 길이, 비용, 청크, 출력 해석을 어떻게 바꾸는지 봅니다.
- **P6-2.4 토큰 관점은 어떤 운영 판단으로 이어지는가**: 프롬프트 길이, RAG 청크, 비용 예산, 출력 형식 설계에 토큰 기준을 적용합니다.
- **P6-2.5 보충학습: 토크나이저 계열 차이는 언제 드러나는가**: BPE, WordPiece, SentencePiece 차이를 운영 관찰값으로 읽습니다.

#### Chapter 3. 임베딩 `LLM 핵심`

- **P6-3.1 임베딩은 토큰 ID를 어떻게 비교 가능한 좌표로 바꾸는가**: token ID가 계산 가능한 벡터 표현으로 바뀌는 이유를 봅니다.
- **P6-3.2 가까운 벡터는 왜 정답이 아니라 후보인가**: 비슷한 표현이 가까운 후보가 되는 직관과 한계를 봅니다.
- **P6-3.3 보충학습: 임베딩 학습은 무엇을 가깝게 두도록 배우는가**: 무엇을 가깝게, 무엇을 멀게 두도록 배우는지 봅니다.
- **P6-3.4 보충학습: ANN 검색은 속도와 후보 누락을 어떻게 맞바꾸는가**: 가까운 후보를 빠르게 좁힐 때 생기는 속도와 후보 누락의 절충을 봅니다.

### Module 3. 다음 후보를 만드는 LLM 구조

#### Chapter 4. Transformer 구조 복습 `딥러닝 구조`

- **P6-4.1 Transformer는 LLM에서 어떻게 다음 후보 점수로 이어지는가**: 문맥 반영, 표현 갱신, 다음 후보 점수 흐름으로 Transformer를 다시 봅니다.
- **P6-4.2 attention은 context window 안에서만 무엇을 볼 수 있는가**: 모델이 제한된 범위 안에서 관계를 계산한다는 점을 봅니다.
- **P6-4.3 보충학습: multi-head attention과 위치 표현은 무엇을 나눠 보강하는가**: 순서 정보와 여러 관계 축을 구분합니다.
- **P6-4.4 보충학습: KV cache는 반복 생성에서 무엇을 다시 계산하지 않는가**: 반복 생성의 재계산 부담을 봅니다.
- **P6-4.5 보충학습: sparse attention과 long-context는 무엇을 줄이고 남기려 하는가**: 연결 수 조절과 장문맥 유지의 차이를 봅니다.

#### Chapter 5. GPT 계열 `LLM 핵심`

- **P6-5.1 GPT 계열은 어떻게 디코더 기반 누적 생성 구조가 되는가**: decoder 중심 자기회귀 생성 계열의 위치를 봅니다.
- **P6-5.2 대화형 LLM 경험은 생성 구조 위에 무엇이 더해져 생기는가**: instruction following과 제품 인터페이스가 사용 경험을 어떻게 바꾸는지 봅니다.

#### Chapter 6. 다음 토큰 예측과 생성 `LLM 핵심`

- **P6-6.1 다음 토큰 예측은 어떻게 긴 생성의 출발점이 되는가**: 다음 토큰 분포라는 국소 목표가 긴 생성으로 이어지는 이유를 봅니다.
- **P6-6.2 출력 선택 규칙은 왜 답변의 안정성과 다양성을 바꾸는가**: greedy, sampling, temperature가 안정성과 다양성을 어떻게 바꾸는지 봅니다.

### Module 4. 응답 습관을 만드는 학습과 조정

#### Chapter 7. 사전학습 `학습 원리`

- **P6-7.1 사전학습은 왜 넓은 언어 기반을 먼저 만드는가**: 대규모 텍스트에서 일반 언어 기반을 만드는 과정을 봅니다.
- **P6-7.2 스케일은 왜 능력과 부담을 함께 키우는가**: 규모가 능력과 운영 부담을 동시에 키우는 축임을 봅니다.

#### Chapter 8. 파인튜닝 `학습 원리`

- **P6-8.1 파인튜닝은 언제 목적 맞춤 조정이 필요한가**: 기반 모델을 특정 목적과 도메인에 맞게 조정하는 흐름을 봅니다.
- **P6-8.2 LoRA는 왜 전체 파인튜닝 부담을 줄이려 하는가**: 조정 범위를 줄여 비용과 회전 속도를 바꾸는 감각을 봅니다.

#### Chapter 9. 지시 튜닝과 정렬 `LLM 핵심`

- **P6-9.1 지시 튜닝은 어떻게 요청 형식에 맞는 응답 습관을 만드는가**: 사용자 지시를 따르는 응답 습관을 봅니다.
- **P6-9.2 정렬은 왜 잘 따르는 답과 허용 가능한 답을 나누는가**: 유용성, 안전성, 사실성, 거절 기준을 함께 맞추는 문제를 봅니다.
- **P6-9.3 LLM 실패는 먼저 무엇이 부족한지 나누어 본다**: 형식, 근거, 실행, 지속 적응 중 무엇이 부족한지 먼저 진단하는 기준을 봅니다.
- **P6-9.4 보충학습: LoRA의 low-rank는 무엇을 작게 표현하는가**: LoRA 이름과 low-rank 조정분의 규모 감각을 봅니다.
- **P6-9.5 보충학습: adapter, LoRA, QLoRA는 어떤 제약에서 갈리는가**: 비용, 구조, 메모리 제약으로 효율적 조정 이름을 구분합니다.

### Module 5. 프롬프트와 근거 보강

#### Chapter 10. 프롬프트 엔지니어링 `LLM 핵심`

- **P6-10.1 프롬프트 엔지니어링은 무엇을 입력에서 조정하는가**: 입력 지시, 맥락, 예시, 출력 형식으로 생성 방향을 유도하는 방법을 봅니다.
- **P6-10.2 프롬프트 한계는 언제 시스템 구조로 넘어가는가**: 최신성, 근거성, 계산, 실행, 재현성 문제를 프롬프트만으로 해결할 수 없는 이유를 봅니다.
- **P6-10.3 보충학습: CoT와 self-consistency는 답변 경로를 어떻게 더 보게 하는가**: 중간 reasoning과 여러 후보 합의를 답변 경로 관찰 전략으로 구분합니다.
- **P6-10.4 보충학습: automatic prompt optimization은 프롬프트 실험을 어떻게 반복 개선하는가**: 프롬프트 후보, 평가 기준, 검증 세트의 관계를 봅니다.

#### Chapter 11. RAG `서비스 구조`

- **P6-11.1 RAG는 왜 답변 전에 외부 근거를 붙이는가**: 모델 내부 기억이 아니라 외부 근거를 입력 문맥으로 붙이는 구조를 봅니다.
- **P6-11.2 검색 결과는 어떻게 생성 입력과 답변으로 이어지는가**: 검색 실패와 생성 실패를 구분합니다.

#### Chapter 12. 벡터 데이터베이스 `서비스 구조`

- **P6-12.1 벡터 데이터베이스는 왜 원문과 메타데이터까지 함께 저장하는가**: 임베딩, payload, metadata를 함께 저장하고 검색하는 구조를 봅니다.
- **P6-12.2 인덱스는 왜 검색 속도와 후보 품질을 함께 흔드는가**: 검색 품질과 latency의 trade-off를 봅니다.

### Module 6. 도구와 에이전트 실행 구조

#### Chapter 13. 도구 사용(tool use) `서비스 구조`

- **P6-13.1 도구 사용은 언제 실제 조회·계산·실행이 필요한가**: 조회, 계산, 파일 처리, 상태 확인이 필요한 장면을 봅니다.
- **P6-13.2 함수 호출은 왜 실행 요청을 이름과 인자로 나누는가**: 자연어 요청을 함수 이름과 인자로 구조화하는 방식을 봅니다.

#### Chapter 14. 에이전트(agent) `서비스 구조`

- **P6-14.1 에이전트는 언제 다음 행동을 다시 고르는 목표 흐름이 되는가**: 목표를 여러 단계의 계획, 도구 호출, 관찰, 상태 판단으로 이어 가는 구조를 봅니다.
- **P6-14.2 계획·행동·관찰은 어떻게 계속·종료·사람 검토를 가르는가**: agent loop의 계속 탐색, 종료, 사람 검토 분기를 봅니다.

#### Chapter 15. MCP와 하네스 `서비스 구조`

- **P6-15.1 MCP는 왜 도구와 자원을 공통 형식으로 연결하려 하는가**: 도구와 리소스를 공통 형식으로 연결하는 인터페이스 층을 봅니다.
- **P6-15.2 하네스는 왜 실행을 기록하고 재현 가능한 환경으로 감싸는가**: 실행, 로그, 평가 입력, 승인, 재현성을 감싸는 환경을 봅니다.

### Module 7. 검토 가능한 서비스 상태

#### Chapter 16. LLM 평가 `LLM 핵심`

- **P6-16.1 LLM 평가는 왜 자연스러운 답과 품질 기준을 나눠 보는가**: 자연스러운 답변과 품질 기준을 분리해 봅니다.
- **P6-16.2 자동 평가와 사람 평가는 어떻게 반복 검사와 맥락 판단을 나누는가**: 반복 검사와 맥락 판단의 분업을 봅니다.

#### Chapter 17. 서비스 운영 제약 `서비스 구조`

- **P6-17.1 서비스 운영 제약은 왜 평가 통과 후보를 다시 걸러내는가**: 비용, 지연 시간, 처리량, 사용량 제한을 봅니다.
- **P6-17.2 운영 중 실패 대응은 왜 오류를 복구 경로로 나눠야 하는가**: hallucination, timeout, 권한 오류, format mismatch를 복구 경로로 나누어 봅니다.

#### Chapter 18. 작은 생성형 AI 기능을 한 흐름으로 묶기 `프로젝트 실습`

- **P6-18.1 작은 생성형 AI 기능은 어떻게 요청 흐름으로 묶이는가**: 질문, 근거, 답변, 상태 판단, 기록을 한 흐름으로 묶습니다.
- **P6-18.2 최소 구현은 왜 응답보다 실행 기록을 먼저 남겨야 하는가**: Part 7로 넘기기 전에 응답, 근거, 상태, 회고 기록이 함께 남아야 하는 이유를 닫습니다.

### Module 8. 배경 지도와 비교

#### Chapter 19. LLM 발전사 `역사와 패러다임`

- **P6-19.1 LLM 발전사는 왜 한계와 구조 전환의 흐름으로 읽어야 하는가**: 통계적 언어 모델에서 GPT형 인터페이스까지의 변화를 봅니다.
- **P6-19.2 직접 계보와 주변 근거는 왜 분리해 읽어야 하는가**: LLM 직접 계보와 딥러닝 확산 주변 근거를 구분합니다.

#### Chapter 20. BERT 계열 `LLM 핵심`

- **P6-20.1 BERT 계열은 왜 읽기 중심 Transformer 비교축인가**: GPT 생성 계열과 BERT 이해 계열을 비교합니다.
- **P6-20.2 이해 중심 태스크는 왜 긴 답변보다 판단값을 먼저 내는가**: 분류, 문장쌍 판단, 검색과 랭킹, 임베딩 재사용을 봅니다.
...
- **P7-7.3 운영 점검 시뮬레이션**: 운영 신호를 `즉시 수정`, `재현 확인`, `다음 반복 개선`으로 나누고 대응 순서를 판단합니다.
