...
- **P6-1.3 토큰은 모델 입력에서 어떻게 쓰이나**: 토큰이 낱개 이름이 아니라 모델이 읽는 순서열의 자리로 쓰인다는 점을 봅니다.
...
- **P6-2.3 보충학습: 임베딩 학습을 큰 그림으로 읽기**: 무엇을 가깝게, 무엇을 멀게 두도록 배우는지 먼저 잡습니다.
- **P6-2.4 보충학습: ANN 검색을 큰 그림으로 읽기**: 가까운 후보를 왜 근사적으로라도 더 빨리 좁혀야 하는지 봅니다.
...
- **P6-3.3 보충학습: 위치 표현과 multi-head attention은 문맥 읽기에 무엇을 더하는가**: 관계 읽기와 순서 정보 공급이 각각 무엇을 보강하는지 구분합니다.
- **P6-3.4 보충학습: KV cache는 반복 생성에서 무엇을 다시 계산하지 않는가**: 긴 대화와 긴 생성에서 왜 앞부분 재계산을 줄이려 하는지 구분합니다.
- **P6-3.5 보충학습: sparse attention과 long-context는 장문맥에서 무엇이 다른가**: 연결 수 조절과 장문맥 유지가 같은 문제가 아니라는 점을 구분합니다.
...
- **P6-8.4 보충학습: adapter, LoRA, QLoRA는 어떤 제약에서 갈리는가**: 파인튜닝 축을 고른 뒤 효율적 조정 이름들을 비용·구조·메모리 제약으로 다시 구분합니다.
...
- **P6-9.3 보충학습: Chain-of-thought, self-consistency, automatic prompt optimization은 프롬프트 층에서 무엇을 더하는가**: 프롬프트 전략이 reasoning 경로, 후보 비교, 반복 개선에서 무엇을 더 바꾸고 무엇은 여전히 못 바꾸는지 구분합니다.
...
- **P6-16.1 서비스 운영 제약**: 비용, 지연 시간, 사용량 제한을 실제 운영 관점에서 봅니다.
