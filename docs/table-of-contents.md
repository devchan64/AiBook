...
- **P6-1.1 생성형 AI는 무엇을 생성하는가**: 생성 결과가 분류값이나 수치 예측이 아니라 검토해야 할 산출물이라는 점을 잡습니다.
...
- **P6-2.1 왜 텍스트 생성을 토큰 단위로 읽는가**: 생성 산출물이 조각 단위로 만들어지고 제한되는 이유를 봅니다.
...
- **P6-2.4 토큰 관점은 어디에 활용되는가**: 프롬프트 길이, RAG 청크, 비용 예산, 출력 형식 설계에 토큰 기준을 적용합니다.
- **P6-2.5 보충학습: 토크나이저 계열 차이는 언제 드러나는가**: BPE, WordPiece, SentencePiece 차이를 운영 관찰값으로 읽습니다.

#### Chapter 3. 임베딩 `LLM 핵심`

- **P6-3.1 임베딩(embedding)의 직관**: token ID가 계산 가능한 벡터 표현으로 바뀌는 이유를 봅니다.
- **P6-3.2 의미와 거리**: 비슷한 표현이 가까워진다는 직관과 한계를 봅니다.
- **P6-3.3 보충학습: 임베딩 학습을 큰 그림으로 읽기**: 무엇을 가깝게, 무엇을 멀게 두도록 배우는지 봅니다.
- **P6-3.4 보충학습: ANN 검색을 큰 그림으로 읽기**: 가까운 후보를 빠르게 좁히는 검색 구조를 봅니다.

### Module 3. 다음 후보를 만드는 LLM 구조

#### Chapter 4. Transformer 구조 복습 `딥러닝 구조`

- **P6-4.1 Transformer를 LLM 관점에서 다시 읽기**: 문맥 반영, 표현 갱신, 다음 후보 점수 흐름으로 Transformer를 다시 봅니다.
- **P6-4.2 attention과 context window**: 모델이 제한된 범위 안에서 관계를 계산한다는 점을 봅니다.
- **P6-4.3 보충학습: 위치 표현과 multi-head attention은 문맥 읽기에 무엇을 더하는가**: 순서 정보와 여러 관계 축을 구분합니다.
- **P6-4.4 보충학습: KV cache는 반복 생성에서 무엇을 다시 계산하지 않는가**: 반복 생성의 재계산 부담을 봅니다.
- **P6-4.5 보충학습: sparse attention과 long-context는 장문맥에서 무엇이 다른가**: 연결 수 조절과 장문맥 유지의 차이를 봅니다.

#### Chapter 5. GPT 계열 `LLM 핵심`

- **P6-5.1 GPT 계열의 위치**: decoder 중심 자기회귀 생성 계열의 위치를 봅니다.
- **P6-5.2 대화형 LLM으로의 전환**: instruction following과 제품 인터페이스가 사용 경험을 어떻게 바꾸는지 봅니다.

#### Chapter 6. 다음 토큰 예측과 생성 `LLM 핵심`

- **P6-6.1 다음 토큰 예측(next-token prediction)**: 다음 토큰 분포라는 국소 목표가 긴 생성으로 이어지는 이유를 봅니다.
- **P6-6.2 생성 과정의 직관**: greedy, sampling, temperature가 안정성과 다양성을 어떻게 바꾸는지 봅니다.

### Module 4. 응답 습관을 만드는 학습과 조정

#### Chapter 7. 사전학습 `학습 원리`

- **P6-7.1 사전학습(pretraining)**: 대규모 텍스트에서 일반 언어 기반을 만드는 과정을 봅니다.
- **P6-7.2 데이터와 스케일**: 규모가 능력과 운영 부담을 동시에 키우는 축임을 봅니다.

#### Chapter 8. 파인튜닝 `학습 원리`

- **P6-8.1 파인튜닝(fine-tuning)**: 기반 모델을 특정 목적과 도메인에 맞게 조정하는 흐름을 봅니다.
- **P6-8.2 LoRA와 효율적 조정**: 조정 범위를 줄여 비용과 회전 속도를 바꾸는 감각을 봅니다.

#### Chapter 9. 지시 튜닝과 정렬 `LLM 핵심`

- **P6-9.1 지시 튜닝(instruction tuning)**: 사용자 지시를 따르는 응답 습관을 봅니다.
- **P6-9.2 정렬(alignment)의 기본 문제**: 유용성, 안전성, 사실성, 거절 기준을 함께 맞추는 문제를 봅니다.
...
- **P6-10.1 프롬프트 엔지니어링(prompt engineering)**: 입력 지시, 맥락, 예시, 출력 형식으로 생성 방향을 유도하는 방법을 봅니다.
- **P6-10.2 프롬프트의 한계**: 최신성, 근거성, 계산, 실행, 재현성 문제를 프롬프트만으로 해결할 수 없는 이유를 봅니다.
- **P6-10.3 보충학습: Chain-of-thought, self-consistency, automatic prompt optimization은 프롬프트 층에서 무엇을 더하는가**: 프롬프트 층의 보강 전략과 한계를 구분합니다.

#### Chapter 11. RAG `서비스 구조`

- **P6-11.1 RAG의 필요성**: 모델 내부 기억이 아니라 외부 근거를 입력 문맥으로 붙이는 구조를 봅니다.
- **P6-11.2 검색 결과와 생성의 결합**: 검색 실패와 생성 실패를 구분합니다.

#### Chapter 12. 벡터 데이터베이스 `서비스 구조`

- **P6-12.1 벡터 데이터베이스(vector database)**: 임베딩, payload, metadata를 함께 저장하고 검색하는 구조를 봅니다.
- **P6-12.2 인덱스(index)와 검색 품질**: 검색 품질과 latency의 trade-off를 봅니다.

### Module 6. 도구와 에이전트 실행 구조

#### Chapter 13. 도구 사용(tool use) `서비스 구조`

- **P6-13.1 도구 사용(tool use)**: 조회, 계산, 파일 처리, 상태 확인이 필요한 장면을 봅니다.
- **P6-13.2 함수 호출(function calling)**: 자연어 요청을 함수 이름과 인자로 구조화하는 방식을 봅니다.

#### Chapter 14. 에이전트(agent) `서비스 구조`

- **P6-14.1 에이전트(agent)**: 목표를 여러 단계의 계획, 도구 호출, 관찰, 상태 판단으로 이어 가는 구조를 봅니다.
- **P6-14.2 계획, 행동, 관찰**: agent loop의 계속 탐색, 종료, 사람 검토 분기를 봅니다.

#### Chapter 15. MCP와 하네스 `서비스 구조`

- **P6-15.1 MCP와 도구 연결**: 도구와 리소스를 공통 형식으로 연결하는 인터페이스 층을 봅니다.
- **P6-15.2 하네스(harness)**: 실행, 로그, 평가, 승인, 재현성을 감싸는 환경을 봅니다.

### Module 7. 검토 가능한 서비스 상태

#### Chapter 16. LLM 평가 `LLM 핵심`

- **P6-16.1 LLM 평가(evaluation)**: 자연스러운 답변과 품질 기준을 분리해 봅니다.
- **P6-16.2 자동 평가와 사람 평가**: 반복 검사와 맥락 판단의 분업을 봅니다.

#### Chapter 17. 서비스 운영 제약 `서비스 구조`

- **P6-17.1 서비스 운영 제약**: 비용, 지연 시간, 처리량, 사용량 제한을 봅니다.
- **P6-17.2 운영 중 실패 대응**: hallucination, timeout, 권한 오류, format mismatch를 복구 경로로 나누어 봅니다.

#### Chapter 18. 작은 생성형 AI 기능을 한 흐름으로 묶기 `프로젝트 실습`

- **P6-18.1 작은 생성형 AI 기능을 한 흐름으로 묶기**: 질문, 근거, 답변, 상태 판단, 기록을 한 흐름으로 묶습니다.
- **P6-18.2 최소 구현과 회고 포인트**: Part 7로 넘기기 전에 기록과 회고가 왜 필요한지 닫습니다.

### Module 8. 배경 지도와 비교

#### Chapter 19. LLM 발전사 `역사와 패러다임`

- **P6-19.1 LLM 발전사의 큰 흐름**: 통계적 언어 모델에서 GPT형 인터페이스까지의 변화를 봅니다.
- **P6-19.2 직접 계보와 주변 근거**: LLM 직접 계보와 딥러닝 확산 주변 근거를 구분합니다.

#### Chapter 20. BERT 계열 `LLM 핵심`

- **P6-20.1 BERT 계열의 위치**: GPT 생성 계열과 BERT 이해 계열을 비교합니다.
- **P6-20.2 이해 중심 태스크**: 분류, 문장쌍 판단, 검색과 랭킹, 임베딩 재사용을 봅니다.
