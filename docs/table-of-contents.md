...
- **P7-5.2 캐릭터 identity와 추가 페인팅으로 특징 완성하기**: 얼굴 identity를 기준으로 전신 비례·기본 의상·재킷을 이전 결과 위에 단계적으로 더해, 캐릭터 특징을 완성하는 편집 경로를 비교합니다.
...
- **P7-5.7 캐릭터 멀티플 뷰 생성: identity 기준과 카메라 앵글 분리하기**: 정면 얼굴·체스트 기준이 identity·헤어·화풍을 맡고, 다중 앵글 LoRA와 짧은 카메라 명령이 한 축의 카메라 변환을 맡도록 분리해 비교합니다.
