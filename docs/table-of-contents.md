...
- **P7-5.1 로컬 GPU로 프레임 없는 웹툰 화풍 참조 팩 만들기**: 캐릭터보다 먼저, 프레임 없는 원본·선화·수채화 색층·장소·시간·카메라 폭을 사람이 검수하는 로컬 GPU 화풍 팩 gate를 만듭니다.
- **P7-5.2 캐릭터 참조 팩과 SD 1.5 LoRA로 기준 만들기**: 자체 제작·권리 확인 참조 팩을 학습과 held-out으로 나누고, 캐릭터 LoRA가 다른 view에서도 인물성을 유지하는지 점검합니다.
- **P7-5.3 SD 1.5와 StoryDiffusion으로 다중 컷 반복성 점검하기**: SD 1.5 기반 최소 조건에서 세 prompt의 캐릭터 반복성, VRAM, 생성 시간을 분리해 확인합니다.
- **P7-5.4 SD 1.5 LoRA, ControlNet, inpaint로 웹툰 컷 검증하기**: 승인한 캐릭터 기준과 컷별 구조 입력을 결합해 네 컷의 구조·인물성·국소 보정을 분리해 판정합니다.
