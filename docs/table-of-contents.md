...
#### Chapter 6. RAG 근거 검증 `프로젝트 실습`

- **P7-6.1 검색, 근거, 답변 흐름 실습**: 검색 후보, 선택 근거, 최종 답변이 한 파이프라인 안에서 어떻게 이어지는지 다룹니다.
- **P7-6.2 검색 실패와 근거 누락 기록**: 부정확한 답변을 검색 실패, 근거 부족, 답변 구성 문제로 나누어 기록하는 방법을 정리합니다.
- **P7-6.3 질문-근거 경계 연습**: 같은 문서 집합에서 질문 문구를 바꾸며, `근거 부족`, `답변 과장 위험`, `근거 기반 답변` 상태가 어떻게 갈리는지 비교합니다.
- **P7-6.4 ANN 검색 설정 실습**: 실제 ANN 라이브러리나 인덱스 설정을 사용해 후보 수, 검색 지연, top-k 포함률, 버전 필터가 RAG 근거 품질을 어떻게 바꾸는지 프로젝트 기록으로 남깁니다.
- **P7-6.5 검색 평가셋으로 근거 품질을 다시 점검하기**: 검색 점수와 답변 가능 상태를 분리하고, mismatch를 다음 검색 보강 질문으로 바꾸는 법을 연습합니다.

#### Chapter 7. AI 에이전트 실행과 승인 정책 `프로젝트 실습`

- **P7-7.1 계획, 도구 호출, 승인 흐름 실습**: AI 에이전트의 계획, 도구 호출 순서, 승인과 제약 지점을 실행 경로 중심으로 다룹니다.
- **P7-7.2 권한, 로그, blocked 상태 검토**: 실행 로그를 permission, blocked, next action 기준으로 정리하고, 실행 기록과 실패 처리 규칙의 역할을 봅니다.
- **P7-7.3 승인 정책 연습**: 같은 운영 시나리오에서 자동 실행 가능 단계와 즉시 보류 단계를 나누고, 더 안전한 다음 행동을 적어 봅니다.

### Module 4. 운영과 반복 개선

#### Chapter 8. 배포와 운영 회고 `프로젝트 실습`

- **P7-8.1 배포 확인과 상태 점검**: 빌드 성공 여부를 넘어, 배포 후 공개 상태와 기본 동작을 점검합니다.
- **P7-8.2 장애 기록과 다음 반복 계획**: 실패 사건을 원인 추정, 우선순위, 다음 조치, 재발 방지까지 포함한 운영 회고로 남기는 방법을 정리합니다.
- **P7-8.3 운영 점검 시뮬레이션**: 운영 신호를 `즉시 수정`, `재현 확인`, `다음 반복 개선`으로 나누고 대응 순서를 판단합니다.
- **P7-8.4 경고 등급 설계 연습**: 표본 수, 반복성, 최근성을 함께 읽어 `watch`, `review`, `action` 등급을 설계합니다.
- **P7-8.5 재현 가능한 프로젝트 패키지 만들기**: 질문, 입력, 코드, 환경, 실행 명령, 결과, 한계, 다음 조치를 묶어 다른 사람이 같은 실행을 다시 확인할 수 있는지 점검합니다.

### Module 5. 유행 모델 확장 실습

#### Chapter 9. 로컬 LLM, 비전, 음성·오디오 모델 `프로젝트 실습`

- **P7-9.1 로컬 LLM으로 양자화와 context 길이 비교하기**: `llama.cpp`, Qwen, Gemma 계열을 기준으로 모델 파일, 양자화, context 길이가 실행 부담과 답변 안정성을 어떻게 바꾸는지 비교합니다.
- **P7-9.2 SAM 2로 이미지 입력 prompt와 segmentation 결과 비교하기**: 같은 이미지에서 point prompt, box prompt, 수정 prompt를 바꾸며 mask 실패를 입력 구조와 표현 학습 관점으로 읽습니다.
- **P7-9.3 Whisper와 MusicGen으로 음성·오디오 모델 입출력 비교하기**: Whisper 전사와 MusicGen 생성 조건을 비교하며 시퀀스, encoder-decoder, 생성 평가를 실제 입출력 기록으로 확인합니다.
