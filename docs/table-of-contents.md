...
- **P7-5.8 보충학습: OpenPose로 동작과 구도 조건 비교하기**: OpenPose가 전달하는 2D 관절 배치와 전달하지 못하는 카메라 원근·3D 가림을 분리하고, 동작·고각도 guide 조건의 결과를 비교합니다.
- **P7-5.9 보충학습: Mira의 표정 바꾸기**: 5.2의 정면 머리를 Qwen-Image-Edit-2511로 편집한 39개 표정을 10개 묶음으로 비교하고, 눈·입의 움직임과 얼굴 외형의 변화를 구분합니다.
- **P7-5.10 텍스트 모션으로 12개 OpenPose 키프레임 준비하기**: 텍스트→3D 관절 모션→2D OpenPose의 순서를 분리하고, 8GB GPU에서 보행 48포즈를 12개 구조 키프레임으로 추출하는 실험 조건을 준비합니다.
