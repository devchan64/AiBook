...
- **P1-5.3 추론 관련 용어의 구분**: inference와 reasoning을 구분합니다.
...
- **P1-7.4 보충학습: 길찾기에서 자율주행 경로 계획으로 이어지는 흐름**: 그래프 길찾기를 자율주행의 경로 계획과 연결합니다.
...
- **P1-12.3 프롬프트(prompt)의 한계(limit)와 평가(evaluation)**: 프롬프트가 보장하지 못하는 것과 초반 점검 기준을 잡습니다.
...
- **P1-13.3 RAG(retrieval-augmented generation)로 이어지는 흐름**: 왜 검색 결과를 LLM 입력에 연결해야 하는지 큰 흐름을 잡습니다.
...
- **P1-14.2 RAG(retrieval-augmented generation)와 에이전트 도구 사용(tool use)의 위치**: 외부 자료와 도구 호출이 왜 필요한지 큰 그림으로 봅니다.
- **P1-14.3 AI 에이전트(AI agent): 목표를 작업 흐름으로 이어 가는 구조**: 모델이 여러 단계 작업과 도구 호출을 이어 가는 구조를 큰 그림으로 봅니다.
- **P1-14.4 MCP(Model Context Protocol)와 도구 연결 표준화**: AI 에이전트가 외부 도구와 데이터에 연결되는 인터페이스 흐름을 먼저 잡습니다.
- **P1-14.5 하네스(harness)와 평가 실행 환경**: 실행을 감싸고 로그, 테스트, 평가, 재현성을 관리하는 장치를 미리 봅니다.
- **P1-14.6 AI 서비스가 현실에서 만나는 제약**: 비용, 지연 시간, 사용량 제한, 실패 대응처럼 실제 서비스에서 필요한 조건을 개괄합니다.

### Module 6. 사회적 쟁점과 적용

#### Chapter 15. AI 윤리, 저작권, 보안 `법과 정책`

- **P1-15.1 편향(bias), 안전성(safety), AI 책임성(accountability)**: AI 결과의 사회적 책임을 봅니다.
...
- **P1-17.1 전망은 어떤 근거로 다룰 것인가**: 예측성 내용을 다룰 때 필요한 근거 기준을 둡니다.
...
- **P2-1.2 수식을 코드로 옮겨 데이터 계산하기**: 평균 수식을 Python 코드로 옮기고, 데이터의 모양과 계산 결과의 의미를 확인합니다.
...
- **P2-4.5 그래디언트 보충학습: 고등학교 미분에서 다변수 미분으로**: 방향에 따른 변화율과 그래디언트 벡터장을 비교하고, 접선 방향의 순간 변화율과 실제 변화량을 구분합니다.
...
- **P2-5.4 확률과 통계를 작은 데이터로 확인하기**: 작은 점수 배열로 평균, 분산, 표본과 추정을 계산합니다.
- **P2-5.5 보충학습: 표준편차, 상관, 신뢰구간을 처음 읽는 법**: 표준편차와 표준오차를 구분하고, 상관과 신뢰구간을 해석합니다.
...
- **P2-7.8 보충학습: 셸 실행 흐름 읽기**: 표준 입출력, 파이프, 리디렉션과 셸 스크립트의 실행 흐름을 읽습니다.
- **P2-7.9 보충학습: 로컬 Python 환경 문제 점검**: 인터프리터, 작업 폴더와 가상환경을 확인해 로컬 Python 오류를 좁힙니다.
...
- **P2-9.1 자료구조(data structure) 선택은 왜 필요한가**: 데이터를 어떤 모양으로 담느냐가 계산 방식에 영향을 준다는 점을 봅니다.
...
- **P3-2.1 저장된 기록은 왜 분석 목적에 맞게 다시 구성해야 하는가**: 원시 기록도 데이터셋이지만 분석 목적에 맞는 샘플·열 정의가 더 필요할 수 있음을 봅니다.
...
- **P3-4.3 시점 기록, 동작 샘플, 구간 집계는 어떻게 다른가**: 시점 기록, 동작 1회, 최근 구간 집계를 한 화면에서 다시 나누어 보며 층위 혼동을 줄입니다.
...
- **P3-5.7 여러 후속 사건을 접는 규칙**: 여러 후속 사건을 한 샘플에 묶는 규칙을 정합니다.
...
- **P3-6.3 사람이 만든 특징과 모델이 학습하는 표현은 어떻게 구분되는가**: 특징 설계와 표현 학습의 연결과 경계를 정리합니다.
...
- **P3-6.6 같은 열 이름과 다른 특징**: 열 이름이 같아도 계산과 관찰 조건이 다르면 특징이 달라짐을 확인합니다.
...
- **P3-7.3 기준선은 무엇과 비교하는 기준인가**: 데이터 비교 기준과 모델 평가 기준의 차이를 정리합니다.
- **P3-7.4 기준선은 어떤 구간과 조건으로 잡아야 하는가**: 샘플 단위와 공정 조건을 맞추고 표본 수를 확인해 기준선을 고르는 기준을 정리합니다.
...
- **P3-9.4 검토 메모는 어떻게 목표 라벨 후보로 바뀌는가**: 검토 메모와 운영 판단이 반복되며 어떻게 더 안정된 target 후보 열로 축적되는지 정리합니다.
...
- **P3-9.7 예측 시점에 쓸 입력과 이후 결과는 어떻게 구분하는가**: 입력/결과 구분, 누수 방지, 운영 시점 재현성, cutoff와 horizon을 한 번에 정리합니다.
- **P3-9.8 예측 점수는 어떤 규칙을 거쳐 행동으로 이어지는가**: 예측 단위, 모델 출력, 운영 정책, 실제 행동을 분리해 정리합니다.
- **P3-9.9 실제 목표와 대리 타깃은 어떻게 구분하는가**: 지금 쓰는 target이 실제 목표인지, 대신 쓰는 대리 열인지 구분합니다.
- **P3-9.10 라벨 확정 지연과 관측 미완료는 어떻게 구분하는가**: 라벨 확정 지연과 관측 미완료 상태를 분리해 적는 법을 정리합니다.
- **P3-9.11 타깃 후보와 변하는 기준**: 대표 target과 정의 버전을 먼저 고정하는 이유를 정리합니다.
- **P3-9.12 오탐과 놓침의 비용은 판단 기준을 어떻게 바꾸는가**: false negative와 false positive의 운영 비용 차이를 먼저 적는 이유를 정리합니다.
...
- **P4-6.3 보충학습: 사이트 신뢰성 엔지니어링에서 지표(metrics)를 읽는 법**: 머신러닝 평가 지표와 운영 지표가 어떻게 다른지 보충합니다.
...
- **P4-7.4 보충학습: 특징 선택 방식 구분**: 특징 선택 방법을 통계 검정과 모델 활용 방식에 따라 구분합니다.
...
- **P5-7.5 보충학습: 대표 optimizer 계열**: momentum, AdaGrad, RMSProp, Adam이 기억하고 조절하는 값을 비교합니다.
- **P5-7.6 보충학습: 학습률 조절 전략**: warmup과 decay로 학습률을 시간에 따라 조절하는 이유를 설명합니다.
- **P5-7.7 보충학습: optimizer state와 개별 update**: 파라미터, gradient, update와 optimizer state를 구분합니다.
- **P5-7.8 보충학습: gradient clipping과 불안정한 update**: gradient clipping의 역할과 학습률 조절의 차이를 비교합니다.
...
- **P5-11.1 합성곱 신경망(CNN)의 직관**: 이미지에서 지역 패턴을 보는 구조를 봅니다.
...
- **P5-13.1 어텐션(Attention)의 직관**: 필요한 위치를 더 강하게 보는 방식을 봅니다.
- **P5-13.2 셀프 어텐션(self-attention)으로 이어지는 흐름**: Transformer로 넘어가기 전 연결을 잡습니다.
...
- **P6-1.4 생성은 하나의 반복 방식이 아니다**: LLM의 다음 토큰 선택과 이미지 디퓨전의 반복 복원을 비교해 생성 방식을 구분합니다.
...
#### Chapter 6. BERT 계열 `LLM 핵심`

- **P6-6.1 읽기 중심 Transformer 비교축으로 보는 BERT 계열**: GPT 생성 계열과 BERT 이해 계열을 비교합니다.
- **P6-6.2 긴 답변보다 판단값을 먼저 내는 이해 중심 태스크**: 분류, 문장쌍 판단, 검색과 랭킹, 임베딩 재사용을 봅니다.

#### Chapter 7. 다음 토큰 예측과 생성 `LLM 핵심`

- **P6-7.1 긴 생성의 출발점이 되는 다음 토큰 예측**: 다음 토큰 분포라는 국소 목표가 긴 생성으로 이어지는 이유를 봅니다.
- **P6-7.2 답변의 안정성과 다양성을 바꾸는 출력 선택 규칙**: greedy, sampling, temperature가 안정성과 다양성을 어떻게 바꾸는지 봅니다.

### Module 4. 응답 습관을 만드는 학습과 조정

#### Chapter 8. 사전학습 `학습 원리`

- **P6-8.1 넓은 언어 기반을 먼저 만드는 사전학습**: 대규모 텍스트에서 일반 언어 기반을 만드는 과정을 봅니다.
- **P6-8.2 능력과 운영 부담을 함께 키우는 스케일**: 규모가 능력과 운영 부담을 동시에 키우는 축임을 봅니다.

#### Chapter 9. 파인튜닝 `학습 원리`

- **P6-9.1 목적 맞춤 조정이 필요한 파인튜닝**: 기반 모델을 특정 목적과 도메인에 맞게 조정하는 흐름을 봅니다.
- **P6-9.2 전체 파인튜닝 부담을 줄이는 LoRA**: 조정 범위를 줄여 비용과 회전 속도를 바꾸는 감각을 봅니다.

#### Chapter 10. 지시 튜닝과 정렬 `LLM 핵심`

- **P6-10.1 요청 형식에 맞는 응답 습관을 만드는 지시 튜닝**: 사용자 지시를 따르는 응답 습관을 봅니다.
- **P6-10.2 잘 따르는 답과 허용 가능한 답을 나누는 정렬**: 유용성, 안전성, 사실성, 거절 기준을 함께 맞추는 문제를 봅니다.
- **P6-10.3 형식·근거·실행 부족으로 나누어 보는 LLM 실패**: 형식, 근거, 실행, 지속 적응 중 무엇이 부족한지 먼저 진단하는 기준을 봅니다.
- **P6-10.4 보충학습: LoRA의 low-rank가 작게 표현하는 조정분**: LoRA 이름과 low-rank 조정분의 규모 감각을 봅니다.
- **P6-10.5 보충학습: 효율적 조정 방식의 제약**: 비용, 구조, 메모리 제약으로 효율적 조정 이름을 구분합니다.

### Module 5. 프롬프트와 근거 보강

#### Chapter 11. 프롬프트 엔지니어링 `LLM 핵심`

- **P6-11.1 입력 지시·맥락·예시를 조정하는 프롬프트 엔지니어링**: 입력 지시, 맥락, 예시, 출력 형식으로 생성 방향을 유도하는 방법을 봅니다.
- **P6-11.2 최신성·근거성·실행을 프롬프트 밖으로 넘기는 한계**: 최신성, 근거성, 계산, 실행, 재현성 문제를 프롬프트만으로 해결할 수 없는 이유를 봅니다.
- **P6-11.3 보충학습: 답변 경로 관찰과 비교**: 중간 reasoning과 여러 후보 합의를 답변 경로 관찰 전략으로 구분합니다.
- **P6-11.4 보충학습: 프롬프트 후보 반복 개선**: 프롬프트 후보, 평가 기준, 검증 세트의 관계를 봅니다.

#### Chapter 12. RAG `서비스 구조`

- **P6-12.1 모델 기억 대신 외부 근거를 붙이는 RAG**: 모델 내부 기억이 아니라 외부 근거를 입력 문맥으로 붙이는 구조를 봅니다.
- **P6-12.2 검색 실패와 생성 실패를 가르는 RAG 흐름**: 검색 실패와 생성 실패를 구분합니다.

#### Chapter 13. 벡터 데이터베이스 `서비스 구조`

- **P6-13.1 임베딩·원문·메타데이터를 함께 저장하는 벡터 데이터베이스**: 임베딩, payload, metadata를 함께 저장하고 검색하는 구조를 봅니다.
- **P6-13.2 검색 속도와 후보 품질을 맞바꾸는 인덱스**: 검색 품질과 latency의 trade-off를 봅니다.

### Module 6. 도구와 AI 에이전트 실행 구조

#### Chapter 14. 도구 사용(tool use) `서비스 구조`

- **P6-14.1 조회·계산·실행을 모델 밖으로 넘기는 도구 사용**: 조회, 계산, 파일 처리, 상태 확인이 필요한 장면을 봅니다.
- **P6-14.2 자연어 요청을 이름과 인자로 나누는 함수 호출**: 자연어 요청을 함수 이름과 인자로 구조화하는 방식을 봅니다.

#### Chapter 14. AI 에이전트(AI agent) `서비스 구조`

- **P6-15.1 중간 결과에 따라 다음 작업을 바꾸는 AI 에이전트**: 목표를 한 번의 답변으로 닫지 않고, 검색 결과나 도구 실행 결과에 따라 다음 작업을 바꾸는 구조를 봅니다.
- **P6-15.2 계속·멈춤·사람 검토로 갈라지는 AI 에이전트 루프**: AI agent loop가 관찰 결과를 보고 계속 탐색, 종료, 사람 검토로 갈라지는 기준을 봅니다.

#### Chapter 16. MCP와 하네스 `서비스 구조`

- **P6-16.1 도구와 자원을 공통 형식으로 연결하는 MCP**: 도구와 리소스를 공통 형식으로 연결하는 인터페이스 층을 봅니다.
- **P6-16.2 실행 기록과 재현 환경을 감싸는 하네스**: 실행, 로그, 평가 입력, 승인, 재현성을 감싸는 환경을 봅니다.
- **P6-16.3 보충학습: 에이전트 워크플로와 이미지 생성 워크플로는 무엇이 다른가**: 제어 흐름, 데이터 변환 흐름, 실행 기록의 역할을 비교합니다.

### Module 7. 검토 가능한 서비스 상태

#### Chapter 17. LLM 평가 `LLM 핵심`

- **P6-17.1 자연스러운 답과 품질 기준을 나누는 LLM 평가**: 자연스러운 답변과 품질 기준을 분리해 봅니다.
- **P6-17.2 자동 평가와 사람 평가의 분업**: 반복 검사와 맥락 판단의 분업을 봅니다.

#### Chapter 18. 서비스 운영 제약 `서비스 구조`

- **P6-18.1 비용·지연·사용량으로 다시 거르는 운영 제약**: 비용, 지연 시간, 처리량, 사용량 제한을 봅니다.
- **P6-18.2 오류를 복구 경로로 나누는 운영 중 실패 대응**: hallucination, timeout, 권한 오류, format mismatch를 복구 경로로 나누어 봅니다.

#### Chapter 19. 작은 생성형 AI 기능을 한 흐름으로 묶기 `프로젝트 실습`

- **P6-19.1 질문·근거·답변·기록을 묶는 작은 생성형 AI 기능**: 질문, 근거, 답변, 상태 판단, 기록을 한 흐름으로 묶습니다.
- **P6-19.2 응답보다 근거·상태·회고를 먼저 남기는 최소 구현**: Part 7로 넘기기 전에 응답, 근거, 상태, 회고 기록이 함께 남아야 하는 이유를 닫습니다.

### Module 8. 배경 지도와 비교

#### Chapter 20. LLM 발전사 `역사와 패러다임`

- **P6-20.1 한계와 구조 전환의 흐름으로 읽는 LLM 발전사**: 통계적 언어 모델에서 GPT형 인터페이스까지의 변화를 봅니다.
- **P6-20.2 직접 계보와 주변 근거를 분리해 읽는 기준**: LLM 직접 계보와 딥러닝 확산 주변 근거를 구분합니다.

#### Chapter 21. 오픈웨이트 모델 `서비스 구조`

- **P6-21.1 오픈웨이트 모델은 무엇을 공개하는가**: 가중치 공개, 오픈소스, 공개 API를 구분하고 라이선스, 실행 환경, 운영 책임을 함께 봅니다.
- **P6-21.2 로컬 실행 환경과 메모리 배치**: 오픈웨이트 모델을 직접 실행할 때 GPU VRAM, CPU RAM, dtype, 양자화, CPU offloading을 구분하고 실행 가능성과 품질 판정을 분리합니다.
- **P6-21.3 공개 디퓨전 모델을 조건 하나씩 바꾸어 평가하기**: seed, steps, scheduler, guidance, 조건 입력을 분리해 실제 모델의 출력·실행 조건·재현성을 비교합니다.
...
- **P7-5.1 공통 T2I 프롬프트로 화풍 데이터 축적하기**: 공통 화풍 계약을 재사용하고 장소·시간·카메라만 바꾼 T2I 원본을 비교해, 다음 단계에 쓸 화풍 데이터를 축적합니다.
- **P7-5.2 Mira 정면 머리 기준 만들기**: Mira의 정면 얼굴·헤어 기준을 생성하고, 카메라 각도가 다른 참조 묶음으로 확장합니다.
- **P7-5.3 캐릭터 identity와 추가 페인팅으로 특징 완성하기**: 얼굴 identity를 기준으로 전신 비례·기본 의상·재킷을 이전 결과 위에 단계적으로 더해, 캐릭터 특징을 완성하는 편집 경로를 비교합니다.
- **P7-5.4 라인아트 구도에서 스토리보드 장면까지**: 라인아트 구도에 Mira를 이식하고 주변 인물·동물을 추가하며, 단계별로 유지되거나 달라진 특징을 비교합니다.
- **P7-5.5 스토리보드 장면에서 캐릭터를 분리하고 적용하는 경로**: 캐릭터와 배경을 분리해 외형과 조명을 처리한 뒤, 합성과 리라이트로 한 장면을 만듭니다.
- **P7-5.6 보충학습: Q4 GGUF와 GPU 메모리**: Q4 모델 파일 크기와 VRAM 상주를 구분하고, CPU 오프로딩으로 완료한 8GB T2I 실행의 조건·시간·한계를 결과 JSON과 함께 읽습니다.
- **P7-5.7 보충학습: 착장 추출과 Try-On LoRA**: 착장 추출과 FoxBaze의 입력 조건을 살펴보고, FoxBaze의 입력·결과 이미지에서 의상·신발·포즈와 그림자의 변화를 확인합니다.
- **P7-5.9 보충학습: Mira의 표정 바꾸기**: 5.2의 정면 머리를 Qwen-Image-Edit-2511로 편집한 39개 표정을 10개 묶음으로 비교하고, 눈·입의 움직임과 얼굴 외형의 변화를 구분합니다.
- **P7-5.11 Mira 얼굴·헤어 LoRA 학습 준비하기**: 5.2의 얼굴·각도와 5.9의 표정을 학습 후보로 선정하고, Qwen-Image-Edit-2511의 새 장면에서 인물 보존을 비교할 데이터·학습·평가 조건을 설계합니다.
- **P7-5.12 얼굴·헤어·화풍을 바꾸는 BFS LoRA 설계하기**: 변환 전 입력과 Mira 목표의 대응 쌍을 생성·검수하고, 얼굴·헤어·화풍 변환과 장면 보존을 평가할 조건을 설계합니다.
- **P7-5.15 텍스트 모션으로 12개 OpenPose 키프레임 준비하기**: 텍스트→3D 관절 모션→2D OpenPose의 순서를 분리하고, 8GB GPU에서 보행 48포즈를 12개 구조 키프레임으로 추출하는 실험 조건을 준비합니다.
- **P7-5.16 보충학습: OpenPose로 동작과 구도 조건 비교하기**: OpenPose가 전달하는 2D 관절 배치와 전달하지 못하는 카메라 원근·3D 가림을 분리하고, 동작·고각도 guide 조건의 결과를 비교합니다.
