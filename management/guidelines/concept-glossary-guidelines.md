...
- 일반 사전적 의미가 먼저 떠오르거나 상위 개념의 하위 설명에 가까운 단어는 단독 표제어로 새로 세우지 않는다. `질의`, `환경`, `목표`, `도구`, `리소스`, `평가`, `검토`, `작업 흐름`, `테이블`, `열`, `축 라벨`, `범례`, `막대 그래프`, `선 그래프`, `산점도`, `히스토그램`, `코드 셀`, `마크다운 셀`, `import 문`, `ndim`, `작업 폴더`, `상대 경로`, `절대 경로`, `대화형 모드`, `파이프`, `리다이렉션`, `반환값`, `기본값`, `메서드`, `속성`, `얕은 복사`, `깊은 복사`, `스크립트`, `셸 스크립트`, `환경 변수`, `패스 환경 변수`, `대입(assignment)`, `변경 가능한 값`, `중첩 리스트`, `키`, `이터러블`, `이터레이터`, `컴프리헨션`, `반복 인덱스`, `가까워짐`, `접선`, `곡선`, `미분계수`, `도함수`, `추상 자료형`, `선형 구조`, `비선형 구조`, `키 기반 구조`, `계층`, `노드`, `엣지`, `인접 리스트`, `방향 그래프`, `무방향 그래프`, `엣지 가중치`, `연결 리스트`, `스택`, `큐`, `해시 테이블`, `집합`, `장기 빈도`, `믿음의 정도`, `사전 믿음`, `사후 믿음`, `중심`, `분포 치우침`, `퍼짐`, `통계량`, `실제 값`, `표본추출 변동`, `표본 평균`, `모집단 분산`, `표본 분산`, `표준오차`, `후보 분포`, `실행 순서`, `런타임 상태`, `재실행 가능한 기록`, `인덱싱`, `슬라이싱`, `불리언 마스크`, `원본 공유`, `필터링`, `집계`, `글로벌 플래너`, `로컬 플래너`, `웨이포인트`, `픽셀`, `원시 오디오 파형`, `바운딩 박스`, `패키지`, `테스트`, `로그`, `서버`, `클라이언트`, `프로토콜`, `수식`, `브랜치`, `재시도`, `스트리밍`, `지수 백오프`, `지터` 같은 표현은 `서비스 운영`, `버전 관리`, `데이터셋`, `의존성`, `시각화`, `계산 언어`, `노트북`, `실행 환경`, `배열`, `복사`, `함수`, `객체`, `자료구조`, `그래프`, `미분`, `극한`, `확률`, `베이즈 규칙`, `분포`, `평균`, `분산`, `표준편차`, `추정`, `표본`, `생성 조건`, `계획`, `모션 플래닝`, `객체 검출`, `이미지 인식`, `음성 생성`, `문서 재현성`, `모델 컨텍스트 프로토콜` 같은 상위 표제 안으로 먼저 흡수한다.
...
- 다음 표현은 원칙적으로 개념사전 항목으로 새로 만들지 않는다.
  ...
  - `코드 셀`, `마크다운 셀`, `import 문`, `ndim`, `작업 폴더`, `상대 경로`, `절대 경로`, `대화형 모드`, `파이프`, `리다이렉션`, `반환값`, `기본값`, `메서드`, `속성`, `얕은 복사`, `깊은 복사`, `스크립트`, `셸 스크립트`, `환경 변수`, `패스 환경 변수`, `실행 순서`, `런타임 상태`, `재실행 가능한 기록`, `대입(assignment)`, `변경 가능한 값`, `중첩 리스트`, `키`, `이터러블`, `이터레이터`, `컴프리헨션`, `인덱싱`, `슬라이싱`, `불리언 마스크`, `원본 공유`, `필터링`, `집계`, `반복 인덱스`, `가까워짐`, `접선`, `곡선`, `미분계수`, `도함수`, `추상 자료형`, `선형 구조`, `비선형 구조`, `키 기반 구조`, `계층`, `노드`, `엣지`, `인접 리스트`, `방향 그래프`, `무방향 그래프`, `엣지 가중치`, `연결 리스트`, `스택`, `큐`, `해시 테이블`, `집합`, `글로벌 플래너`, `로컬 플래너`, `웨이포인트`, `픽셀`, `원시 오디오 파형`, `바운딩 박스`처럼 노트북·Python·배열·파일 시스템·함수·객체·자료구조·그래프·수학 표기·응용 예시 설명의 하위 구성요소나 속성명에 가까운 표현
