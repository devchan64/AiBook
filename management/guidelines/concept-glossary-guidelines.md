...
- 표제어 판정은 표면형이 아니라 `역할`로 한다. 후보 표현은 먼저 `표준 개념`, `상위 표제의 하위 설명`, `원고 운영·형식 이름`, `국소 예제 표현`, `임시 표현` 중 어디에 속하는지 분류한다.
- 단독 표제어는 `표준 개념`으로 분류되고 문서 전체의 독립 기준점 역할을 할 때만 둔다. 나머지는 상위 표제의 하위 설명, 본문 문맥, 검토 리포트로 돌린다.
...
- 문맥 한정 표제는 `반복 참조`, `상위 표제 흡수 불가`, `영어 기준 용어 안정성`, `slug 범위 안정성`을 모두 만족할 때만 예외적으로 둔다.
...
다음 역할이 주가 되면 새 표제어로 만들지 않는다. 이미 존재하는 항목도 같은 기준으로 삭제 후보나 상위 표제 흡수 후보로 돌린다.

- 절차·상태 이름: 원고 작성, 검토, 실행, 배포, 평가 과정에서 생긴 작업 단위다.
- 구현·도구 이름: 특정 도구, 라이브러리, API, 명령, 함수, 세부 구현 방식에 묶여 있다.
- 구성·속성 이름: 표, 코드, 자료구조, 수식, 시스템 구성요소, 결과물의 하위 요소나 속성이다.
- 설계·입력 필드: 특정 예제나 관리 문서에서 값을 나누기 위해 붙인 칸 이름이다.
- 국소·임시 표현: 한 Section의 예제, 변수명, 파일명, 옵션 설명, 개인적 비유, 작업 가설, 임시 번역어에 가깝다.

제외 판정은 단어 자체가 아니라 현재 문서에서 맡는 역할을 기준으로 한다. 같은 단어라도 표준 개념의 기준점이면 등재를 검토할 수 있고, 작업·형식·상태·하위 요소 이름이면 상위 표제나 본문 설명으로 흡수한다.
...
4. 안정성: 한국어 표제어, 영어 기준 용어, slug가 같은 범위를 가리키며 단독 일반어로 과도하게 열리지 않는가?
...
1. 역할을 먼저 분류한다.
2. 상위 표제나 본문 설명으로 흡수할 수 있으면 새 표제를 만들지 않는다.
3. 같은 뜻의 더 구체적이고 안정적인 표제가 있으면 중복 표제를 제거한다.
4. 단독 일반어가 필요한 경우에는 문맥 한정 표제로 바꾸고, 영어 기준 용어와 slug도 함께 검토한다.
...
2. 후보의 역할을 `표준 개념`, `상위 표제의 하위 설명`, `원고 운영·형식 이름`, `국소 예제 표현`, `임시 표현`으로 분류한다.
3. 상위 표제, 관련 개념 연결, 본문 안 설명으로 처리할 수 있으면 새 표제를 만들지 않는다.
4. 등재 기준과 표준화 테스트를 적용한다. 하나라도 불분명하면 `보류`한다.
5. 일반어와 전문 문맥이 겹치면 영어 기준 개념, 역할, 사용 층위를 먼저 분리하고 필요한 경우에만 문맥 한정 표제로 좁힌다.
6. 영어권 표준 용어 또는 널리 쓰이는 학술·기술 용어를 외부 레퍼런스로 확인한다.
7. 영어 기준 용어로 slug를 정하고, 필요한 언어별 단어 파일과 색인 include를 만든다.
8. 한국어 대표 표제어와 영어 병기, 영어 표제어, 중국어 표제어를 언어별로 정한다.
9. 대표 설명 위치를 정하고 `중심 Section`을 하나만 적는다.
10. 관련 언어별 보조 인덱스를 갱신한다. 세부 기준은 `management/glossary-indexes/README.md`를 따른다.
11. 본문 설명이 부족하면 개념사전만 늘리지 말고 해당 Section 보강을 먼저 검토한다.
