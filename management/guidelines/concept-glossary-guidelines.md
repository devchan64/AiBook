...
5. 일반어와 AI 전문 문맥이 겹치는 표현은 일반 의미, 해당 본문에서의 의미, 허용할 축약 표현을 먼저 분리한다. 단독 표제어가 오해를 만들면 `AI agent`, `reinforcement learning agent`처럼 문맥이 붙은 표제어로 나눈다.
6. 영어권 표준 용어 또는 널리 쓰이는 학술·기술 용어를 외부 레퍼런스로 확인한다.
7. 영어 기준 용어로 파일 slug를 정하고, 필요한 언어별 단어 파일을 만든다.
8. 한국어 대표 표제어와 영어 병기, 영어 표제어, 중국어 표제어를 언어별로 정한다.
9. 대표 설명 위치를 정하고 `중심 Section`을 하나만 적는다.
10. 관련 언어별 색인 파일에 단어 파일 include를 추가한다.
11. 관련 언어별 보조 인덱스를 갱신한다. 세부 기준은 `management/glossary-indexes/README.md`를 따른다.
12. 본문 설명이 부족하면 개념사전만 늘리지 말고 해당 Section 보강을 먼저 검토한다.
...
4. 해당 표현을 표제 관리에서 제외할지, 표제로 통일 관리할지 먼저 결정한다. 제외 대상이면 더 큰 대표 항목의 하위 설명이나 본문 문맥으로 흡수하고, 통일 관리 대상이면 일반화된 학술·기술 표현에서 대표 표제어를 다시 정한다.
