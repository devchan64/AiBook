...
3. 일반화된 표현이 독자에게 다른 대상을 떠올리게 할 수 있는지 확인한다. 오해 가능성이 있으면 항목의 `뜻` 첫 문장이나 `왜 중요한가`에서 그 경계를 분명히 쓴다.
4. 영어권 표준 용어 또는 널리 쓰이는 학술·기술 용어를 외부 레퍼런스로 확인한다.
5. 영어 기준 용어로 파일 slug를 정하고, 필요한 언어별 단어 파일을 만든다.
6. 한국어 대표 표제어와 영어 병기, 영어 표제어, 중국어 표제어를 언어별로 정한다.
7. 대표 설명 위치를 정하고 `중심 Section`을 하나만 적는다.
8. 관련 언어별 색인 파일에 단어 파일 include를 추가한다.
