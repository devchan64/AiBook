...
- 일반화된 표현이 오해를 만들 수 있으면 표면형만으로 묶지 말고 영어 기준 개념, 역할, 사용 층위를 먼저 분리한다.
- 일반 사전적 의미가 먼저 떠오르거나 상위 개념의 하위 설명에 가까운 단어는 단독 표제어로 새로 세우지 않는다.
- 표제어가 꼭 필요한지는 `문맥을 붙이면 정확해지는가`가 아니라 `문서 전체에서 독립 기준점 역할을 하는가`로 판단한다.
- 문맥 한정 표제는 다음 조건을 모두 만족할 때만 예외적으로 둔다.
  - 여러 Part 또는 여러 Section에서 같은 의미로 반복 참조된다.
  - 상위 표제의 하위 설명으로 넣으면 개념 연결이나 용어 통일이 흐려진다.
  - 영어 기준 용어가 학술·기술 문맥에서 안정적으로 확인된다.
  - slug와 표제어가 단독 일반어로 다시 읽히지 않도록 충분히 좁혀져 있다.
- 표제는 문맥 한정 표현으로 좁혔지만 파일 slug가 단독 일반어로 남아 있으면 링크와 인덱스에서 다시 일반 의미로 읽힐 수 있다. 영어 기준 용어 자체가 안정적으로 좁혀졌고 링크 영향이 감당 가능할 때만 slug 변경을 검토한다.
- 영어 일반어가 AI 문맥에서 전문화되어 쓰이는 경우에는 일반 의미, 기술 일반 의미, AI 문맥 의미를 분리한다. 단독 표제어가 오해를 만들면 문맥과 층위를 드러낸 표제어로 나누고, 그렇지 않으면 상위 표제 안에서 설명한다.
- 표제 관리는 삭제할 표현과 통일 관리할 표현을 먼저 구분한다. 제외 대상은 상위 표제나 본문 문맥으로 돌리고, 통일 관리 대상만 영어 기준 용어와 일반화된 학술·기술 표현으로 대표 표제어를 정한다.
...
개념사전 항목은 `문서 전체의 용어 기준점`이 되는 표현만 등재한다. 반복 등장, 오해 가능성, 후속 Part 연결 가능성이 있어도 곧바로 등재하지 않고, 먼저 흡수 가능성, 표준 용어성, 독립 기준점 여부를 확인한다.

### 등재 조건

표제어는 필수 조건을 모두 만족하고, 등재 사유 중 하나 이상이 분명할 때만 만든다.
...
다음 성격이 주가 되면 새 표제어로 만들지 않는다. 이미 존재하는 항목도 같은 기준으로 삭제 후보나 상위 표제 흡수 후보로 돌린다.

- 운영 단위: 실행, 검토, 승인, 배포, 작업 흐름처럼 절차나 상태를 관리하기 위한 이름이다.
- 구현 단위: 특정 도구, 라이브러리, API, 명령, 함수, 세부 구현 방식의 이름이다.
- 구성 단위: 표, 차트, 코드, 자료구조, 수식, 시스템 안의 하위 요소나 속성 이름이다.
- 설계 필드: 특정 원고나 예제에서 입력, 라벨, 기준, 버전, 결과를 나누기 위해 붙인 칸 이름이다.
- 국소 표현: 한 Section의 예제 실행, 변수명, 파일명, 옵션 설명에만 필요한 표현이다.
- 임시 표현: 표준 용어로 확인되지 않은 개인적 표현, 비유, 작업 가설, 임시 번역어다.

제외 판정은 단어 자체보다 역할을 기준으로 한다. 같은 단어라도 문서 전체에서 표준 개념의 기준점이면 등재를 검토할 수 있고, 특정 원고의 조작·형식·상태 이름이면 상위 표제나 본문 설명으로 흡수한다.

### 표준화 테스트

표제어 후보는 다음 질문에 모두 답한 뒤 등재한다.

1. 이 표현이 없어지면 문서 전체의 개념 연결이 실제로 흐려지는가?
2. 상위 표제의 하위 설명으로 넣으면 같은 의미를 충분히 관리할 수 없는가?
3. 영어 기준 용어가 교과서, 논문, 공식 문서, 교육 자료에서 같은 범위로 확인되는가?
4. 한국어 표제어와 영어 기준 용어, slug가 단독 일반어로 과도하게 열리지 않는가?
5. 특정 도구·절차·형식·예제의 이름이 아니라 후속 Part에서도 재사용할 개념 기준점인가?
...
4. 제외 판정 기준 중 하나가 주된 역할이면 대표 표제가 아니라 본문 보조 설명이나 검토 리포트에 둔다.
5. 표준화 테스트 중 하나라도 분명하지 않으면 `보류`를 기본값으로 두고, 반복 등장 위치와 대표 설명 위치가 실제로 생긴 뒤 다시 검토한다.
