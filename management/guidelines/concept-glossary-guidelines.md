...
- 일반 사전적 의미가 먼저 떠오르거나 상위 개념의 하위 설명에 가까운 단어는 단독 표제어로 새로 세우지 않는다. 표제 제외 여부는 개별 예시 목록이 아니라 아래 범주로 판정한다.
  - 운영·절차 범주: 실행 기록, 승인 단계, 재시도 방식, 배포·문서 운영의 상태명은 `서비스 운영`, `문서 재현성`, `버전 관리` 같은 상위 표제 안으로 흡수한다.
  - 도구·구현 범주: 특정 라이브러리, 패키지, API, 셸 명령, 함수·메서드, 알고리즘 구현명은 해당 도구를 설명하는 상위 개념 안에서 다룬다.
  - 구성요소 범주: 표·차트·노트북·자료구조·신경망·수학 표기의 하위 요소나 속성명은 상위 구조 표제 안에서 설명한다.
  - 분석 설계 범주: 특정 Section의 입력 경계, 라벨 검토 상태, 기준 구간, 보고서 필드처럼 원고 안의 설계 칸에 가까운 표현은 `입력`, `샘플`, `타깃`, `라벨`, `기준선` 같은 상위 표제 안으로 흡수한다.
  - 단독 일반어 범주: `질의`, `환경`, `시스템`, `구조`, `평가`, `검토`처럼 일반 사전 의미가 먼저 열리는 단어는 독립 표제로 두지 않고, 필요한 경우에만 문맥 한정 표제로 바꾼다.
...
개념사전 항목은 `문서 전체의 용어 기준점`이 되는 표현만 등재한다. 반복 등장, 오해 가능성, 후속 Part 연결 가능성이 있어도 곧바로 등재하지 않고, 먼저 흡수 가능성과 표준 용어성을 확인한다.

### 등재 가능 범주

- 표준 개념: 교과서, 논문, 공식 문서, 널리 쓰이는 교육 자료에서 안정적으로 확인되는 학술·기술 개념이다.
- 문맥 분리 개념: 한국어 표면형 하나가 서로 다른 영어 기준 개념을 덮어 본문 이해를 흔드는 경우다.
- 대표 연결점: 여러 Section에서 정의 반복이 생기며, 상위 표제에 넣으면 개념 연결이 흐려지는 경우다.
- 대표 모델·방법 계열: 특정 제품명이 아니라 분야에서 구조 차이나 역사적 전환점을 설명하는 표준 모델 계열이나 방법 계열이다.

### 표제 제외 범주

- 운영·절차·상태명: 실행 기록, 승인 단계, 검토 상태, 재시도 방식, 배포·문서 운영의 칸 이름.
- 도구·구현명: 특정 라이브러리, 패키지, API, 셸 명령, 함수·메서드, 세부 알고리즘 구현명.
- 구성요소·속성명: 표·차트·노트북·자료구조·신경망·수학 표기의 하위 요소나 속성.
- 분석 설계 필드: 입력 경계, 라벨 후보, 기준 구간, 버전 메모처럼 특정 Section의 설계 칸에 가까운 표현.
- 단발성 표현: 한 Section의 예제 실행을 위해 잠깐 등장하는 조작어, 옵션명, 변수명, 파일명.
- 임시 표현: 표준 용어로 확인되지 않은 개인적 표현, 비유, 작업 가설, 임시 번역어.

### 판정 순서

1. 기존 상위 표제나 본문 설명으로 흡수할 수 있으면 새 표제를 만들지 않는다.
2. 같은 뜻의 더 구체적인 표제가 있으면 중복 표제를 제거한다.
3. 단독 일반어가 필요한 경우에는 문맥 한정 표제로 바꾸고, 영어 기준 용어와 slug도 함께 검토한다.
4. 표준 개념으로 확인되지 않으면 대표 표제가 아니라 본문 보조 설명이나 검토 리포트에 둔다.
5. 판단이 애매하면 `보류`를 기본값으로 두고, 반복 등장 위치와 대표 설명 위치가 실제로 생긴 뒤 다시 검토한다.
