...
층위 오염이 의심되면 새 표제어를 만들기 전에 먼저 역할 분류표로 후보를 다시 분류한다. 표준 개념인지, 문맥 한정 개념인지, 상위 표제의 하위 설명인지, 작업·형식 이름인지, 구현·도구·사례 의존 표현인지 확인한 뒤 `삭제`, `상위 표제 흡수`, `대표 표제 통일`, `보류` 중 하나로 처리한다.
...
개념사전 표제 정리는 단어별 원고만 고치는 작업이 아니다. 표제어를 삭제하거나 상위 표제로 흡수했다면, `docs/parts/` 본문에서 해당 앵커로 직접 연결된 링크도 같은 판단에 맞춰 정리해야 한다.
...
3. 새 표제를 만든 경우에는 한·영·중 단어별 원고, 언어별 공개 색인 include, `management/concept-glossary-integrated-index.md`를 함께 맞춘다.
...
- 영어판 개념사전 항목이 아직 충분히 번역되지 않았더라도 한국어 자음별 개념사전으로 되돌려 링크하지 않는다. 다만 같은 slug의 한국어 단어 파일이 없는 영어 항목은 보강 대상으로 남기지 않고 삭제한다.
...
- 중국어판 개념사전 항목이 아직 충분히 번역되지 않았더라도 한국어 자음별 개념사전으로 되돌려 링크하지 않는다. 다만 같은 slug의 한국어 단어 파일이 없는 중국어 항목은 보강 대상으로 남기지 않고 삭제한다.
