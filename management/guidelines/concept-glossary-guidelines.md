...
- 다음 표현은 원칙적으로 개념사전 항목으로 새로 만들지 않는다.
  ...
  - `코드 셀`, `마크다운 셀`, `import 문`, `ndim`, `작업 폴더`, `상대 경로`, `절대 경로`, `대화형 모드`, `파이프`, `리다이렉션`, `반환값`, `기본값`, `메서드`, `속성`, `얕은 복사`, `깊은 복사`, `스크립트`, `셸 스크립트`, `환경 변수`, `패스 환경 변수`, `대입(assignment)`, `변경 가능한 값`, `중첩 리스트`, `키`, `이터러블`, `이터레이터`, `컴프리헨션`, `반복 인덱스`, `가까워짐`, `접선`, `곡선`, `미분계수`, `도함수`, `추상 자료형`, `선형 구조`, `비선형 구조`, `키 기반 구조`, `계층`, `노드`, `엣지`, `인접 리스트`, `방향 그래프`, `무방향 그래프`, `엣지 가중치`, `연결 리스트`, `스택`, `큐`, `해시 테이블`, `집합`처럼 노트북·Python·배열·파일 시스템·함수·객체·자료구조·그래프·수학 표기 설명의 하위 구성요소나 속성명에 가까운 표현
