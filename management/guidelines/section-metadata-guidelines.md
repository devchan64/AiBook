...
이 문서는 책 본문 문서의 `Section ID`, 제목 앞 인덱스, `Version`, 릴리즈노트 연결 방식을 관리한다. 릴리즈노트 파일 자체의 위치와 항목 형식은 `../release-notes/sections/README.md`를 따른다.
...
- 번역본을 만들거나 원문과 번역본의 버전 대응을 확인할 때
...
- 같은 날의 여러 수정 사항은 대응 릴리즈노트의 같은 `### vYYYY.MM.DD` 항목에 통합해 기록한다.
- 본문을 수정했다면 가능한 한 같은 작업에서 릴리즈노트 파일도 함께 갱신한다.
- 번역본이 생기면 릴리즈노트에 `원문 기준 버전`, `언어별 추가 수정`, `번역 반영 상태`를 구분해서 남긴다.
...
5. 변경 이유와 본문 반영 내용은 릴리즈노트의 최신 버전 항목에 남긴다.
6. 같은 작업에서 Part 체크포인트 노트를 함께 보면, 작업 로그를 누적하지 말고 목차 기준 중심 주제와 구조 판단만 유지한다.

### 번역본 대응 메모

- 번역본이 아직 없어도 `번역 동기화 메모`는 비워 두지 않는다.
- 최소한 `영향 없음`, `확인 필요`, `향후 번역 반영 필요` 중 하나로 남긴다.
- 한 언어에서만 문장을 추가 수정했다면, 릴리즈노트에 `원문 기준 버전과 차이`를 따로 적어 이후 동기화 충돌을 막는다.
