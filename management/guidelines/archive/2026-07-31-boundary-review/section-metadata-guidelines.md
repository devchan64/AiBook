...
이 문서는 책 본문 문서의 `Section ID`, 제목 앞 인덱스, `Version`, 릴리즈노트 연결 방식을 관리한다. 릴리즈노트 파일의 위치와 항목 형식은 `../release-notes/sections/README.md`를 따른다.
...
- 릴리즈노트 예외 문서를 별도로 구분해 불필요한 관리 파일 생성을 막는다.
...
- 예외 문서는 본문 메타데이터만 갱신하고 릴리즈노트 파일을 따로 두지 않는다.
...
릴리즈노트 예외 문서를 수정할 때는 위 4, 5단계를 적용하지 않는다. 이 경우에는 본문 메타데이터와 해당 문서 전용 가이드 정합성만 확인한다.
...
- Part 시작 페이지와 Part 마무리 페이지도 기본 메타데이터 관리 대상에 포함한다.
- Part 개요/마무리 페이지와 공개 진입 문서도 제목 아래 메타데이터 줄 이름은 `Section ID`를 사용한다.
- 이때 값은 Section 번호 대신 다음 식별자를 사용한다.
  - Part 개요/마무리 페이지: `P6-index`, `P6-summary`
  - 소개 페이지: `BOOK-index`
...
- 릴리즈노트 예외 문서는 같은 적용 범위 안에서도 버전 코드는 갱신하되 릴리즈노트는 추가하지 않는다.
