...
- 변경 이유: Part 3 전체 검수에서 한국어 개정과 달리 영어·중국어가 이전 원고에 머문 번역 누락을 확인했습니다.
- 본문 반영: 현재 한국어의 사례·표·연습·해석 경계·출처를 영어와 중국어에 동기화했습니다. 도입부의 고장 발생 기간과 후속 확인 시점도 구분했습니다.
- 번역 동기화 메모: 원문과 번역의 Version을 함께 갱신하고 내부 링크·기존 앵커·도식 대응을 확인했습니다.
- 번역 반영 상태: 영어·중국어 간체 반영 완료.
- 관련 자산: 새 자산 없음. 기존 Section 링크 사용.
- 원문 기준 버전: `v2026.09.20`

### v2026.09.19
- 변경 이유: 설명 개선안에 따라 Part 3의 상위 목적과 데이터의 품질·표현·지표·라벨을 배우는 이유를 시작 페이지에서 연결한다.
- 본문 반영: 가상 센서 사례에서 기록·샘플·표현·해석의 선택을 설명하고, 여섯 질문으로 Chapter 흐름을 정리했다. 지표 계산에 앞선 이름 나열을 걷어내고 상세 계산은 P3-1.1로 연결했다. 원시 기록도 데이터셋이라는 설명을 유지하며 시계열·이미지·문서 입력과 요약의 선택성을 명시했다. Part 4 인계와 자기점검 질문을 정리했다.
- 검수: 개선안의 P3-index 완료 기준, 로컬 링크, Section ID·Version과 릴리즈노트 대응을 확인했다. 빌드·화면 렌더링 검수와 독자 검수는 수행하지 않았다.
- 번역 동기화 메모: 한국어 한 Section씩 검수하는 단계로, 영어·중국어 간체판에는 목적·사례·여섯 질문·체크리스트 재구성을 후속 반영해야 한다.
- 번역 반영 상태: 한국어 패치 반영. 영어·중국어 간체판은 `v2026.09.15` 유지, 동기화 대기.
- 관련 자산: 없음.
