...
- 변경 이유: 대표 사례는 Part 전체에서 일관되게 쓰이지만, 입구에서는 `같은 사례가 어떤 표 단계로 진화하는가`를 한눈에 붙잡는 공용 기준표가 없어 각 Chapter를 새 표처럼 읽을 여지가 있었다.
...
- 변경 이유: Part 3의 spine이 표준 개념 재배열인지 저자 편의적 흐름인지 입구에서 바로 드러나지 않아, 오버뷰의 커리큘럼 근거가 한 번 더 짧게 필요했다.
- 본문 반영: `이 Part의 묶음 / 대응 표준 개념 / 대표 근거 축` 기준표를 추가해, 원천데이터 재구성, 특징·기준선 설계, 해석·산출물 경계가 data wrangling, sample design, feature engineering, problem framing 같은 표준 개념 묶음을 다시 배열한 구조라는 점을 입구에서 바로 확인할 수 있게 정리했다.

- 변경 이유: Part 시작 페이지가 9개 Chapter의 spine을 다시 길게 펼쳐, 오버뷰보다 세부 설계 설명에 더 가까운 구간이 남아 있었다.
- 본문 반영: `읽는 순서`를 `역할 고정 -> 비교 구조 재구성 -> 해석과 경계 마감`의 세 묶음으로 압축해, Chapter 나열보다 Part 3의 큰 흐름이 먼저 읽히도록 정리했다.

- 변경 이유: Part 3의 전체 커리큘럼 관점과 공통 개념 구분을 설명하는 페이지인데도 `출처와 참고 자료`가 없어, 데이터과학 관점의 일반화 근거가 오버뷰 말미에서 바로 회수되지 않았다.
- 본문 반영: NASEM의 `Data Science for Undergraduates`, Google ML Glossary, W3C PROV를 `출처와 참고 자료`에 추가해, Part 3을 문제 구조 복구 구간으로 두는 판단과 샘플/입력/결과 경계의 중요성을 근거와 함께 정리했다.
