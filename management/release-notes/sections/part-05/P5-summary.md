...
### v2026.07.31
- 본문 반영: Part 마무리 페이지 `Version`을 `v2026.07.31`로 갱신하고, 표현 변화·오차 전달·구조 선택·생성 문제 출발점을 다시 묶는 재점검 계획을 추가했다.
- 표현 정리: 구조적 기반을 마감하는 표현을 설명과 출발점 확인 중심 표현으로 바꿨다.
- 번역 동기화 메모: 영어판과 중국어 간체판에 같은 Section ID, Version, 보강 문단을 반영했다.
- 번역 반영 상태: 영문/중문 번역 반영 완료.
- 관련 자산: 없음
- 원문 기준 버전: `docs/parts/part-05/summary.md` `v2026.07.31`
...
### v2026.07.26
- 링크 정합성: 영어판 요약의 개념사전 복귀 링크를 영어 공개 진입점 `/AiBook/en/reference/concept-glossary/`로, 중국어 간체판 요약의 개념사전 복귀 링크를 중국어 공개 진입점 `/AiBook/zh/reference/concept-glossary/`로 정리했다.
- 메타데이터 정합성: 한국어·영어·중국어 간체판의 `Version`을 `v2026.07.26`으로 맞췄다.
- 표기 정리: 영어판 소제목의 대소문자를 현재 번역 표기 기준에 맞춰 추가 정리했다.
- 번역 반영 상태: 영문/중문 번역 반영 완료
