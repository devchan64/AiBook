...
### v2026.07.18
- 추가 반영: Part 6 Chapter 1의 `P6-1.3` 제목과 설명을 `토큰은 모델 입력에서 어떻게 쓰이나`로 갱신해, 비용·문맥 판단보다 P6-1.2의 토큰 정의에서 P6-1.4의 토큰화 절차로 이어지는 순서열 중심 흐름이 드러나게 했다.
- 추가 반영: Part 7 Chapter 4 목차에 `P7-4.4 같은 평균, 다른 패턴 비교 연습`을 추가해, 표현 정규화 연습 뒤에 평균과 shape token의 차이를 독립 연습으로 다시 읽을 수 있게 했다.
- 추가 반영: Part 7 Chapter 7 목차에 `P7-7.4 경고 등급 설계 연습`을 추가해, 운영 신호 분류 뒤에 표본 수, 반복성, 최근성 기반 경고 등급 설계 연습이 이어지도록 독자용 목차를 갱신했다.

- 추가 정리: Part 6 `P6-16.1`의 독자용 목차 제목을 실제 원고 제목인 `서비스 운영 제약`과 맞춰, Chapter 제목 반복 대신 Section 중심 질문이 보이도록 정리했다.
...
- 추가 보강: Part 7 소개 문단의 핵심 설명도 `프로젝트 종류 나열`보다 `Part 4 -> Part 5 -> Part 6 -> 통합 운영` 재실행 순서가 먼저 보이도록 다시 다듬었다.
...
- 추가 반영: Part 7 소개 문단의 `읽는 편이 더 맞습니다`, `다시 손으로 확인`, `다시 읽고`, `다시 확인합니다` 같은 흐름 메타 표현을 줄이고, `실행`, `해석`, `점검`이 직접 보이도록 문장을 다시 다듬었다.
- 추가 반영: 목차 상단의 `목차를 읽을 때는`, `처음 읽는 독자는` 같은 독서 가이드형 메타 안내를 제거하고, 구조 설명만 남기도록 정리했다.
- 추가 반영: Part 7 소개 첫 문장을 `Part 4, Part 5, Part 6에서 배운 판단 기준` 중심 표현보다 `앞서 소개한 내용을 실습하는 공간`으로 다시 정리해, 이 Part의 역할이 새 이론 설명이 아니라 실습이라는 점이 더 직접 보이게 맞췄다.
