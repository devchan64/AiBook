...
이번 개편의 1차 적용에서는 원본 원고 손상을 피하기 위해 파일 경로와 Section ID를 유지하고, 공개 목차와 Part 시작/마무리 페이지의 구조 축을 먼저 조정했다.
...
이후 2차 적용에서 P5-6/P5-8의 파일 이동과 Section 번호 재부여까지 진행했다. 따라서 아래 적용 검토 메모는 1차 적용의 보류 판단을 대체하는 최신 상태 기록이다.

## 2026-07-16 적용 검토 메모

공개 목차 기준의 큰 개편은 적용된 것으로 판단한다. `mkdocs.yml`, `docs/book/table-of-contents.md`, Part 5 시작 페이지, Part 5 마무리 페이지는 모두 다음 여섯 모듈 축으로 정리되어 있다.
...
특히 다음 항목은 더 이상 열린 결정으로 보지 않는다.

- Chapter P5-5의 공개 제목은 `손실에서 gradient로`로 확정했다.
- P5-5.1과 P5-5.2는 각각 `손실이 gradient 신호가 되는 흐름`과 `계산 그래프/자동미분`으로 역할을 분리했다.
- P5-3은 Module 1로, P5-9는 Module 4로, P5-10~P5-14는 Module 5로, P5-15는 Module 6으로 배치했다.
- Chapter P5-6의 공개 제목은 `학습 루프와 모델 실행`으로 조정했다.
- 기존 P5-8.3 학습 루프 요약은 `P5-6.1 학습 루프: forward, loss, backward, optimizer step`으로 이동했다.
- 기존 P5-6.1 학습과 모델 실행은 `P5-6.2`로, 기존 P5-6.2 학습 모드와 평가 모드는 `P5-6.3`으로 이동했다.
- 기존 P5-6.3 안정화 개념 지도는 `P5-8.3`으로, 기존 P5-6.4 큰 초기화 스케일 실험은 `P5-8.4`로 이동했다.
- 이동된 Section의 대응 자산도 새 Chapter 기준으로 옮겼다. 학습 루프 자산은 `docs/assets/part-05/chapter-06/`, 안정화와 deep-scale 자산은 `docs/assets/part-05/chapter-08/` 아래에 둔다.

다만 다음 항목은 후속 검토 대상이다.

- 영어판·중국어 간체판 공개 목차는 아직 새 P5-6/P5-8 Section ID 이동을 반영하지 않았다. 번역 작업 때 같은 순서로 동기화해야 한다.
- P5-6.1이 Chapter 앞쪽으로 이동했으므로, 독자가 regularization과 mode 전환까지 아직 배우기 전에도 학습 루프 요약을 부담 없이 읽을 수 있는지 후속 검토가 필요하다.
- P5-8.3/P5-8.4는 안정화 보충학습으로 이동했으므로, Chapter P5-8의 중심 질문이 `일반화 제약`에서 `학습 안정화와 일반화 제약`으로 충분히 넓게 보이는지 계속 점검한다.

따라서 이 노트는 현재부터 `미적용 제안서`가 아니라 `적용 완료 항목과 후속 검토 항목을 추적하는 개편 기록`으로 취급한다. 이후 큰 폭 재배치를 다시 진행할 때는 이 문서의 권장 개편안을 그대로 재실행하기보다, 위 후속 검토 항목이 실제 독자 이해를 방해하는지 먼저 확인한다.
