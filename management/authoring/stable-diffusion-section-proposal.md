# Stable Diffusion 설명 Section 및 파이프라인 연결 노트
...
현재 원고에는 디퓨전 모델의 입문 직관과 Stable Diffusion 기반 실습이 이미 있다. 따라서 원고의 목적은 모델 이름이나 이미지 제작 기법을 더 나열하는 일이 아니라, 두 층위를 연결하는 **Stable Diffusion의 최소 계산 구조**를 독자가 설명할 수 있게 하는 데 둔다. 이 판단에 따라 P5-15.4를 만들고, P6-16.3 보충학습에는 이미지 생성 파이프라인과 에이전트 제어 흐름을 비교하는 자리를 잡았다. Part 7은 여전히 수정 중이므로 이 노트에서는 그 제목·순서·본문을 바꾸지 않고, 나중에 이론과 실습을 연결할 때의 참고 지점으로만 둔다.
...
| `P5-15.1`~`P5-15.3` | 생성물 중심 출력, 후보 분포, 텍스트 샘플링 선택을 Part 6 전의 딥러닝 준비 관점으로 설명한다. | 이미지 디퓨전의 반복 복원은 P5-15.4에서 따로 다룬다. |
| `P5-15.4` | 텍스트 조건, 잠재 노이즈, 반복 복원, 이미지 변환과 Stable Diffusion의 최소 구성요소를 설명한다. | 설치·노드 조작·이미지 제작 요령은 다루지 않는다. |
| `P6-1.1`~`P6-1.3` | 생성형 AI를 LLM 중심 사례로 읽는 이유와 후보 생성·선택의 반복을 설명한다. | Part 6의 중심축은 LLM이며, 이미지 생성 모델의 구조를 본편으로 확장하지 않는다. |
| `P6-16.3` | 보충학습으로 agent의 제어 흐름, 이미지 생성의 데이터 변환 흐름, harness의 기록 흐름을 비교한다. | InvokeAI·ComfyUI는 파이프라인 관찰·기록 사례이며, 도구 사용법은 다루지 않는다. |
...
P5-15.4는 **Part 5, Chapter 15의 P5-15.3 뒤**에 반영했다. Part 5의 생성 모델·샘플링 입구를 이미지 생성 사례로 한 번 구체화하고, Part 6은 기존대로 LLM 중심 본편으로 유지한다.

- Section ID: `P5-15.4`
- 제목: `Stable Diffusion은 텍스트 조건에서 이미지를 어떻게 복원하는가`
...
P5-15가 “Part 6으로 넘어가기 전의 최소 준비”라는 경계를 넘지 않도록, 구현 코드·Web UI 사용법·프롬프트 작성 요령·학습 튜닝은 넣지 않는다.

P6-16.3은 P6-16.1의 MCP와 P6-16.2의 harness 중심 설명을 바꾸지 않는 보충학습으로 둔다. 이 절은 `workflow`라는 말이 agent에서는 다음 행동을 정하는 제어 흐름이고 이미지 생성에서는 조건과 중간 표현을 잇는 데이터 변환 흐름이라는 점만 비교한다.
...
6. **P6의 파이프라인 보충학습과 Part 7을 읽을 때 참고한다.**
   P6-16.3에서는 ComfyUI의 노드 그래프와 InvokeAI의 Canvas 프로젝트를 파이프라인 관찰·기록 사례로만 비교한다. 실제 설치와 실행은 다루지 않는다.

7. **향후 Part 7을 읽을 때 참고한다.**
...
- `management/authoring/part-05-open-checklist.md`의 Chapter 15 체크포인트에 `P5-15.4` 중심축을 반영했다.
- `management/authoring/part-06-open-checklist.md`의 Chapter 15 체크포인트에 `P6-16.3` 보충학습 중심축을 반영했다.
- `docs/table-of-contents.md`, `mkdocs.yml`, Section 본문 메타데이터, 릴리즈노트, 다국어 대응을 두 Section에 맞춰 반영했다.
