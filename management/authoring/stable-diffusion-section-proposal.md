...
| `P6-15.3` | 보충학습으로 agent의 제어 흐름, 이미지 생성의 데이터 변환 흐름, harness의 기록 흐름을 비교한다. | InvokeAI·ComfyUI는 파이프라인 관찰·기록 사례이며, 도구 사용법은 다루지 않는다. |
...
P6-15.3은 P6-15.1의 MCP와 P6-15.2의 harness 중심 설명을 바꾸지 않는 보충학습으로 둔다. 이 절은 `workflow`라는 말이 agent에서는 다음 행동을 정하는 제어 흐름이고 이미지 생성에서는 조건과 중간 표현을 잇는 데이터 변환 흐름이라는 점만 비교한다.
...
6. **P6의 파이프라인 보충학습과 Part 7을 읽을 때 참고한다.**
   P6-15.3에서는 ComfyUI의 노드 그래프와 InvokeAI의 Canvas 프로젝트를 파이프라인 관찰·기록 사례로만 비교한다. 실제 설치와 실행은 다루지 않는다.
...
- `management/authoring/part-06-open-checklist.md`의 Chapter 15 체크포인트에 `P6-15.3` 보충학습 중심축을 반영했다.
