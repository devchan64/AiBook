...
- 各种 encoder 和 scaler 的全部超参数
- 稀疏矩阵(sparse matrix)优化的实现细节
- 高级非线性变换和降维的数学

各种 encoder 和 scaler 的细部设置，会在 P4-9.1、P4-9.2 的超参数和调优语境里再接回来；降维的目的和数学直觉，则会在 P4-18.1、P4-18.2 再处理。
...
| 文本/图像等特殊表达变换 | 把非结构化数据改成数值表达 | 不在本节范围内 |
...
| 频率/统计型变换 | 把类别改成别的统计值 | 高级场景里可能会用 | 超出本节范围，而且要小心泄漏 |
