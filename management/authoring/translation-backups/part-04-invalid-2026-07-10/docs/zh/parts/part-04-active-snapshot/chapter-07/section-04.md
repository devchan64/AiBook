...
- 各种统计检验公式的推导
- PCA、t-SNE、UMAP 的计算过程
- 大规模特征选择自动化系统

各种降维算法的直觉和局限，会在 P4-18.1、P4-18.2 再详细处理。
...
- 基于统计检验的特征选择，通常理解成 filter 方法。
- RFE 理解成一种 wrapper 方法: 它会反复训练 model，并逐步减少特征。
- 降维看起来和特征选择相似，但它是把原始特征重新表达成新轴的另一类问题。
...
- 当你需要区分“挑原始列”和“重新表达成新轴”时，要先想起这一节。
- 当你需要检查自己是不是把 filter、wrapper、embedded、降维混成同一种解法时，就该回到这一节。
- 当你要同时考虑可解释性和计算成本，并整理出应先用哪种方法时，这一节会成为标准。
