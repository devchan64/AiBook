...
- hyperparameter 是人在学习开始前先定好的配置值。
- parameter 是从数据里学出来的，hyperparameter 是在实验设计时先放进去的。
- 即使是同一个算法，也会因为 hyperparameter 而在复杂度、泛化、计算成本上不同。
- 频繁改动 hyperparameter，也就意味着实验成本和验证成本一起增加。
- `random_state` 经常作为一种特殊设置出现，它更多和可复现性相关，而不是和 model 复杂度直接相关。

这一节的作用，不是增加设置名字，而是为下一步比较流程建立手柄。

| 现在这一节先要分清什么 | 紧接着下一节会做什么 | 在后面的算法章节里会怎样再读它 |
| --- | --- | --- |
| 什么是会被学习出来的 parameter，什么是预先放进去的 hyperparameter | 在 validation 流程里要比较什么候选范围 | 把每个 model 的设置值读成不是 `要背的名字`，而是 `要比较的手柄` |

## 简短检查

- 你现在看到的这个值，能不能分清它是训练里学出来的 parameter，还是人先定好的 hyperparameter？
- 当你改动一个设置时，你看到的不只是性能，还有复杂度、泛化、计算成本一起变化吗？
- 你有没有把 `random_state` 这种主要服务于可复现性的值单独分出来？

## 什么时候应该先想起这个观念

- 当你搞不清楚当前看到的值到底是 model 学出来的 parameter，还是人先定好的 hyperparameter 时，要先想起这一节。
- 当你需要重新说明某个设置会怎样影响复杂度、泛化、计算成本、可复现性时，就该回到这一节。
- 当你需要确认 `random_state` 这种值更多是在管理复现性，而不是直接提升性能时，这一节会成为标准。
...
这一节是解释 hyperparameter `是什么` 的导入章节。下一节 P4-9.2 会继续看：这些值到底要怎样实际搜索，为什么搜索范围一变大，计算成本和验证成本也会一起变大，以及 baseline 之后的改进到底应该怎样解释。

同时，在紧接着到来的算法章节 P4-10 到 P4-16 里，各 model 的代表 hyperparameter 还会再次出现。

- P4-10 线性回归(linear regression)
- P4-11 逻辑回归(logistic regression)
- P4-12 k-NN
- P4-13 SVM
- P4-14 决策树(decision tree)
- P4-15 随机森林(random forest)

也就是说，这一节是在建立一种共同语言，让读者在后面的算法章节里能读懂这些设置值。
