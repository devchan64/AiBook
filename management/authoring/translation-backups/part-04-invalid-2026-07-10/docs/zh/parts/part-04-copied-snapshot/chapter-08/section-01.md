...
这一节会说明 `模型选择(model selection)`、`候选模型(model candidate)`、以及 `根据问题形态和约束来缩小候选的判断`。后面的章节会沿着这个抓手继续当前语境，而到底应该先打开哪一个“模型抽屉”的标准，也会通过这一节和 [概念词汇表](../../../reference/concept-glossary.md) 再接回来。
