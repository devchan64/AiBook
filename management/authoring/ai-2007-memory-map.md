...
사용자의 “휴리스틱은 모든 현상을 함수로 설명할 수 있다는 수학적 사고이지 않았을까”라는 추정은 다음처럼 보정합니다.

AI 탐색에서 휴리스틱은 실제로 함수(function)처럼 쓰이는 경우가 많습니다. 예를 들어 어떤 상태가 목표에 얼마나 가까운지 추정하는 평가 함수(evaluation function)나 휴리스틱 함수(heuristic function)를 두고, 그 값이 더 좋은 후보를 먼저 탐색합니다. 이 점에서 “현상을 수치화하고 함수로 다룬다”는 직관은 일부 맞습니다.

하지만 휴리스틱 전체를 “모든 현상을 함수로 설명할 수 있다는 수학적 사고”로 정의하면 너무 넓습니다. 그것은 휴리스틱이라기보다 수학적 모델링(mathematical modeling) 또는 함수적 모델링(functional modeling)에 가깝습니다. 휴리스틱은 현상 전체를 완전하게 설명하려는 함수가 아니라, 제한된 시간과 계산 자원 안에서 더 그럴듯한 선택을 하게 돕는 불완전한 판단 기준입니다.

| 구분 | 중심 생각 | AI에서의 역할 |
| --- | --- | --- |
| 수학적 모델링 | 현상이나 관계를 변수와 함수로 표현 | 문제를 계산 가능한 형태로 바꿈 |
| 휴리스틱 함수 | 후보 상태의 그럴듯함을 값으로 추정 | 탐색 순서를 정하거나 후보를 줄임 |
| 확률 모델 | 불확실성을 확률 분포로 표현 | 관측과 추론을 수학적으로 연결 |
| 학습 모델 | 데이터로부터 함수나 파라미터를 조정 | 사람이 쓰지 않은 판단 기준을 학습 |

따라서 책에서는 이 추정을 다음 문장으로 일반화합니다.

> 휴리스틱은 모든 현상을 완전한 함수로 설명하려는 시도라기보다, 복잡한 문제 공간에서 선택을 줄이기 위해 일부 판단 기준을 함수처럼 수치화한 실용적 방법이다.

사용자의 “휴리스틱은 확률통계로 설명하고자 했다는 흐름으로 볼 수 있나?”라는 질문은 다음처럼 다룹니다.

휴리스틱을 곧바로 확률통계(probability and statistics)로 보는 것은 정확하지 않습니다. 휴리스틱은 원래 정답을 보장하지 않는 경험적 규칙(rule of thumb), 탐색 기준, 판단 단축법에 가깝습니다. 반면 확률통계는 불확실성(uncertainty)을 확률 분포, 조건부 확률, 추정, 검정 같은 수학적 언어로 표현합니다.

다만 AI 역사에서는 두 흐름이 자주 만납니다. 이유는 둘 다 “모든 경우를 완전하게 알거나 계산할 수 없다”는 문제에서 출발하기 때문입니다.

| 관점 | 핵심 질문 | 휴리스틱과의 관계 |
| --- | --- | --- |
| 휴리스틱 | 모든 후보를 볼 수 없을 때 무엇을 먼저 볼 것인가 | 탐색과 판단을 줄이는 경험적 기준 |
| 확률통계 | 불확실한 정보에서 얼마나 그럴듯한가 | 판단의 불확실성을 수학적으로 표현 |
| 데이터마이닝 | 많은 데이터에서 어떤 패턴이 반복되는가 | 경험적 패턴을 발견하고 규칙 후보를 만듦 |
| 머신러닝 | 데이터로부터 판단 기준을 어떻게 학습할 것인가 | 사람이 만든 휴리스틱 일부를 데이터 기반 모델로 대체하거나 보강 |

그래서 책에서는 다음 표현을 사용합니다.

> 휴리스틱은 확률통계 그 자체는 아니지만, 불완전한 정보와 계산 한계를 다루려는 AI의 흐름 속에서 확률통계, 데이터마이닝, 머신러닝과 점점 가까워졌다.

이 표현은 사용자의 직관을 살리면서도 표준 용어의 경계를 유지합니다.

현대 AI 설명에서는 이 흐름을 하나의 단어로만 부르지 않고, 다음 용어들로 나누어 설명합니다.

| 현대 용어 | 의미 | 사용자의 직관과의 연결 |
| --- | --- | --- |
| 함수 근사(function approximation) | 복잡한 관계를 정확한 공식 대신 근사 함수로 표현 | “현상을 함수로 설명하려 한다”는 직관과 가장 가까움 |
| 목적 함수(objective function) | 모델이 줄이거나 키우려는 기준을 수식으로 둠 | 판단 기준을 수학적으로 놓고 최적화한다는 관점 |
| 최적화(optimization) | 목적 함수가 더 좋아지도록 파라미터를 조정 | AI가 더 나은 답을 찾는 계산 과정 |
| 확률적 추론(probabilistic reasoning) | 불확실한 정보에서 가능한 결론의 그럴듯함을 계산 | 휴리스틱이 다루던 불확실성을 더 엄밀하게 다룸 |
| 통계적 학습(statistical learning) | 데이터에서 일반화 가능한 패턴을 추정 | 데이터 기반 판단으로 넘어가는 핵심 표현 |
| 귀납 편향(inductive bias) | 학습 모델이 어떤 패턴을 더 선호하도록 만드는 전제 | 사람이 넣은 휴리스틱이 모델 구조나 학습 방식에 남는 지점 |
| 표현 학습(representation learning) | 데이터의 유용한 내부 표현을 모델이 학습 | 딥러닝 이후 신경망이 강해진 핵심 설명 |

이 중 휴리스틱과 직접 가장 가까운 현대 표현은 `휴리스틱 함수(heuristic function)`, `평가 함수(evaluation function)`, `귀납 편향(inductive bias)`입니다. 확률통계와 가까운 표현은 `확률적 추론(probabilistic reasoning)`과 `통계적 학습(statistical learning)`입니다. 딥러닝까지 오면 이 흐름은 `함수 근사(function approximation)`, `최적화(optimization)`, `표현 학습(representation learning)`으로 다시 설명됩니다.

따라서 본문에서 사용할 일반화는 다음처럼 둡니다.

> 현대 AI는 휴리스틱을 완전히 버린 것이 아니라, 사람이 직접 쓰던 경험적 판단 기준을 확률 모델, 통계적 학습, 함수 근사, 표현 학습으로 더 체계화하고 데이터 기반으로 확장해 왔다.
...
| 시기적 층위(historical level) | 판단을 만드는 방식 | 대표 표현 |
| --- | --- | --- |
...
> 현대 독자가 AI라는 말을 들을 때는 단순히 “사람처럼 행동하는 기계”만 떠올리는 것이 아니라, 규칙을 사람이 직접 쓰던 방식에서 데이터로부터 판단 기준을 학습하는 방식으로 이동한 역사적 층위(historical level)까지 함께 읽게 된다.
...
| 빅데이터가 곧 AI다 | 데이터 규모와 AI 모델링은 다른 층위(level) | 빅데이터는 대규모 데이터 처리 조건이고, AI는 그 데이터를 사용해 문제를 푸는 모델과 시스템을 포함한다 |
| 데이터마이닝과 머신러닝은 같다 | 겹치지만 목적과 역사적 맥락이 다름 | 데이터마이닝은 데이터에서 지식과 패턴을 발견하는 과정이고, 머신러닝은 경험 데이터로 성능을 개선하는 모델 학습에 초점을 둔다 |
| DSS/BI가 곧 AI다 | 의사결정 지원 시스템과 AI 모델은 층위(level)가 다름 | DSS/BI는 데이터를 의사결정에 연결하는 시스템이고, AI는 그 안에서 예측·추천·자동화를 맡을 수 있는 모델 계층이다 |
| AI라는 단어 자체가 데이터 기반 판단을 뜻한다 | 초기 AI에는 규칙, 논리, 탐색 중심 접근도 포함됨 | 현대 AI 이해에는 규칙 기반에서 데이터 기반으로 확장된 역사적 층위(historical level)가 함께 읽힌다 |
