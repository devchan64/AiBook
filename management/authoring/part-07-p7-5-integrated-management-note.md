...
- 통합 범위: `P7-5.1`~`P7-5.7`, `P7-5.14`~`P7-5.16`
- 대응 본문: `docs/parts/part-07/chapter-05/section-01.md`~`section-07.md`, `section-14.md`~`section-16.md`
- 통합일: 2026-08-14
- 문서 성격: 원고 릴리즈 이력의 대체 관리노트. 독자용 본문이나 `mkdocs.yml`의 nav에 연결하지 않는다.

## 1. 통합 원칙

P7-5의 기록은 `화풍 기준 → 인물 기준 → 장면·구조 → 보정 → 다중참조 → 3D 모션·포즈 입력 → 캐릭터 애니메이션 → 얼굴·카메라 회전`의 순서로 읽는다. 같은 prompt의 단어 수, seed, step, CFG, mask border만 바꾼 반복은 개별 이력으로 남기지 않고 다음 세 가지로 압축한다.
...
### P7-5.10 — 3200스텝 검수 후 보존 품질 개선

- 사용자 방향 검수(2026-09-20): 강도 1.0에서는 0.75보다 머리 방향 오류가 더 많이 보인다. 3200스텝의 얼굴 재현과 0.75의 방향 보존을 함께 고려한 권장 설정이다.
- 사용자 얼굴 검수(2026-09-20): 1600스텝에는 홍채·이목구비 간 거리 차이가 보이고 3200스텝에는 해당 오류가 거의 보이지 않는다. 이 판단을 3200스텝·강도 0.75 권장의 근거로 반영하며, AI의 의상·방향·몸 비율 관찰과 구분한다.

- 2026-09-19의 45장 비교에서 칼라·포켓·웃음의 일부 개선과 새로운 의상·방향·몸 비율 변화가 함께 관찰됐다. 누적 스텝 증가를 전체 품질 개선으로 해석하지 않는다.
- [개선 실험 계획](part-07-p7-5-10-preservation-improvement-plan.md)에 기존 결과 재사용, 대표 12개의 강도 비교, 학습 입력·목표 대응 점검, 조건부 재학습 및 독립 평가 범위를 정리했다. 2026-09-20 신규 60장 생성·무결성 확인과 기존 24개 출력의 비교 검수를 마쳤다. 낮은 강도에서 일부 의상 변경은 완화되지만 시선·손·세부 의상 차이가 남아, AI는 0.5를 비교 후보로 제안했으며 이후 사용자 판단에 따라 **누적 3200스텝·강도 0.75를 현재 권장 설정**으로 기록한다. 학습 쌍 대응 점검은 남은 작업이다. 추가 학습은 미실행이다.
...
### P7-5.14 — 텍스트 모션·포즈 입력 렌더링

- MoMask는 시간 순서가 있는 3D 관절 시퀀스를 만들며, 애니메이션 리그와 동일한 산출물이 아니다. 48→12 OpenPose 경로는 준비 계획으로만 남고 실제 OpenPose JSON은 생성하지 않았다.
- 실제 보존 입력은 MoMask v4 전진 모션의 28~59번을 고정 카메라에서 SCAIL-Pose renderer로 렌더링한 33개 포즈·마스크다.

### P7-5.15 — 포즈 시퀀스·참조 이미지 캐릭터 애니메이션

- P7-5.14의 포즈·마스크 33장을 바꾸지 않고 SCAIL-2 영상 조건으로 재사용한다.
- 전신 참조에 근접 참조와 같은 인물 마스크를 추가한 비교는 동작 식별과 얼굴·손·의상 세부 보존을 별도로 판단한다.

### P7-5.16 — Qwen-Image-Edit-2511·AnyPose 참조 포즈 전이 초안

- 실행 전 초안은 캐릭터 기준 이미지와 포즈 안내 이미지를 각각 고정하고, AnyPose 적용 전후 및 배경 보존 지시만 달리하는 A·B·C 비교를 계획한다.
- 큰 자세, 카메라·구도, 캐릭터 보존, 장면 보존을 독립 관찰 축으로 기록한다. 결과·실행 JSON·공개 자산은 아직 없으며, 초안을 실행 결과로 읽지 않는다.
...
- 이전 P7-5.16의 OpenPose 맵 준비 코드와 전용 결과 자산은 폐기했다. 현재 P7-5.16은 Qwen-Image-Edit-2511·AnyPose 실행 전 초안이며, 실행 자산은 아직 없다.
...
- 기존 `management/release-notes/sections/part-07/`의 P7-5.1~P7-5.10, P7-5.14~P7-5.15 릴리즈노트는 Section별 이력으로 유지한다. 이 문서는 해당 릴리즈노트를 대체하지 않고, 이번 세션의 공통 실험 결론·중복 제거 기준·다음 gate만 요약한다.
...
## 7. P7-5.14·P7-5.15 모션 입력과 개별 프레임 품질 추가 조사 — 2026-09-19

### 현재 판정과 입력의 한계

사용자는 SCAIL의 CFG 참조 조건 수정 결과에서 안개 같은 흐림이 많이 줄었다고 평가했지만, 개별 프레임의 품질은 여전히 부족하다고 평가했다. 최종 후보는 흐림 개선의 근거였으나, 이후 사용자 지시에 따라 SCAIL-v1 영상·프레임·상세 기록을 폐기하고 `2026-09-19-scail-v1/README.md`에 결과 요약만 남겼다. 제작 품질 통과로 해석하지 않는다. 이번 조사는 모델 실행이나 신규 다운로드가 아니다.

로컬 `preparation.json`에서 원본 참조의 290×1180 영역을 80×326으로 축소해 512×512 배경에 붙였음을 확인했다. 원본 이미지 크기와 모델이 실제 받는 캐릭터 정보량은 다르다. 포즈에는 손가락 관절이 없고 머리 기준점은 고정된 측면 대용 좌표다. 따라서 선명도, 얼굴·복장 동일성, 손발 형태·가림 정확도를 별도로 평가해야 한다. 작은 참조와 불완전한 포즈가 품질을 제한할 수 있다는 것은 입력에 근거한 가설이며, 원인별 대조 실험으로 검증해야 한다.

### 추가 후보와 적용 범위

| 후보 | 공식 자료에서 확인한 내용 | 현 실험에 대한 판단 |
| --- | --- | --- |
| SeedVR2 영상 복원 | 시간축을 함께 처리하는 생성형 영상 복원. ComfyUI 구현은 GGUF, BlockSwap, VAE 타일링으로 8 GB 이하 설정을 안내한다. | 기존 최종 PNG 시퀀스를 재사용하는 첫 후처리 후보. 손가락 개수·좌우 팔다리·캐릭터 동일성을 복구한다고 보장할 수 없다. 8 GB에서 목표 해상도·프레임 배치 실행은 미검증이다. |
| SCAIL-2 + DPO LoRA | 공식 문서는 손 왜곡과 눈·입 디테일 개선을 설명하며, pose-driven은 704p에서 더 좋은 결과를 보고한다. 올바른 참조·구동 마스크가 필요하다. | 생성 단계의 형태·디테일 개선 후보. 양자화·offload·DPO 호환성과 8 GB 실행을 먼저 확인해야 한다. 다중 참조는 공식적으로 실험적이며 품질 저하 가능성도 명시되어 있다. |
| FlashVSR v1.1 | 스트리밍 영상 초해상도. 공식 구현은 4배 확대에 최적화되었다고 안내한다. | 보조 후보. 512×512를 2048×2048로 처리하는 비용과 8 GB 실행을 확인해야 한다. 공식 A100 속도 수치를 현재 GPU의 속도로 해석하지 않는다. |

SeedVR2 공식 문서는 심한 열화·큰 동작에서 실패할 수 있고, 이미 비교적 깨끗한 AI 생성 영상에는 디테일을 과하게 만들거나 과도하게 선명하게 할 수 있다고 밝힌다. 업스케일 결과가 원본보다 또렷해 보여도 참조와 다른 얼굴·의상 무늬를 만들면 실패다. 프레임마다 독립적으로 얼굴을 재생성하는 방법은 시간축 일관성이 검증되지 않아 첫 후보로 삼지 않는다.

### 접근 조건과 다음 실험안

2026-09-19 익명 Hugging Face API 조회에서 `ByteDance-Seed/SeedVR2-3B`, `zai-org/SCAIL-2`, `vantagewithai/SCAIL-2-GGUF-ComfyUI`, `JunhaoZhuang/FlashVSR-v1.1`은 `gated=false`였다. `facebook/sam3`는 `gated=manual`이므로 사용자 지시에 따라 승인 요청 경로는 제외한다. SCAIL-2의 SAM3 자동 전처리를 그대로 채택하지 않고, 기존 포즈와 별도 작성한 마스크를 입력할 수 있는지 검증해야 한다. 모델 저장소의 공개 상태만으로 전체 의존성의 실행 가능성이 증명되지는 않는다.

1. **기존 최종 결과의 복원 대조:** 손실 없는 33개 PNG를 입력으로 SeedVR2의 3B 양자화·offload 경로를 검토한다. 2배 출력과 같은 크기의 일반 보간 대조군을 비교한다. 메모리 절감을 위해 시간축 배치를 줄일 때 경계와 깜박임도 검사한다.
2. **생성 단계 개선:** 복원으로 손발 형태나 동일성이 해결되지 않으면 SCAIL-2 + DPO를 검토한다. 동일한 이동 경로를 유지하고 참조 정보량·704p 생성·DPO 효과를 한꺼번에 바꾸지 않고 구분한다. 저해상도 참조를 단순 확대하는 것은 원본 디테일 보강으로 간주하지 않는다.
3. **판정:** 모든 프레임을 원본 배율과 얼굴·손·발 확대에서 검수한다. 얼굴·의상 동일성, 손발 연결과 좌우 교차, 접지, 선명도, 시간축 깜박임을 분리한다. 영상 재생이 자연스럽거나 출력 픽셀 수가 커진 것만으로 통과시키지 않는다.

### 조사 출처

확인일: 2026-09-19. 아래 개선 설명은 저자·구현자의 보고이며 현재 로컬 실험에서 검증한 결과가 아니다.

- [SeedVR 공식 구현 및 한계](https://github.com/ByteDance-Seed/SeedVR)
- [SeedVR2 논문](https://arxiv.org/abs/2506.05301)
- [SeedVR2 ComfyUI 구현: 메모리 설정과 시간축 처리](https://github.com/numz/ComfyUI-SeedVR2_VideoUpscaler)
- [SCAIL-2 공식 구현: DPO, 해상도, 마스크, 다중 참조](https://github.com/zai-org/SCAIL-2/tree/wan-scail2)
- [SCAIL-2 공개 가중치](https://huggingface.co/zai-org/SCAIL-2)
- [FlashVSR 공식 구현](https://github.com/OpenImagingLab/FlashVSR)
- [FlashVSR v1.1 공개 가중치](https://huggingface.co/JunhaoZhuang/FlashVSR-v1.1)

2026-09-19 후속 결정: 사용자가 SCAIL-2 시도를 선택했다. 앞의 SeedVR2 우선 순서는 조사 당시 제안이며 현재 실행 순서가 아니다. SCAIL-2 입력과 결과는 `docs/assets/part-07/chapter-05/sec-15/2026-09-19-scail2-v1/`에 모은다.

SCAIL-2 기본 Q4 실행 완료: 동일 512×512·33프레임 입력, 40 steps·shift 3, DPO 미적용, 약 38.3분. 이동 걷기와 큰 의상 특징은 유지되지만 손·신발 경계의 부드러움, 얼굴·머리·셔츠 형태 변화가 남아 있다. 개별 프레임 제작 품질 통과로 채택하지 않았으며 사용자 검수는 대기한다. 이 결과로 704p나 DPO 효과를 판정하지 않는다. 상세 실행·검수 기록은 SCAIL-2 자산 폴더에 둔다.

2026-09-20 후속 결정: 사용자가 SCAIL-2 기본 결과도 요약 후 폐기하도록 지시했다. 영상·프레임·상세 로그는 삭제하고 기존 README에 결론만 남겼다. 다음 실험은 `2026-09-20-scail2-dpo-v1`의 공식 DPO LoRA strength 1.0이다. 동일 참조·마스크·포즈·시드·512p 조건에서 DPO만 추가하며, 기존 시각 결과가 폐기됐으므로 직접 나란히 비교하는 검수는 불가능하다.

SCAIL-2 DPO 실험 완료: 공식 DPO strength 1.0의 800개 tensor → 400개 patch 적용을 확인했다. 512×512·33프레임·20fps, 약 39.0분. 이동 걷기는 유지되지만 손·신발 디테일 부족과 얼굴·머리·셔츠 형태 변화가 남았다. 이 조건에서 DPO만으로 프레임 품질이 해결됐다고 판정하지 않는다. 사용자 검수는 대기하며, 결과·근거는 `2026-09-20-scail2-dpo-v1`에 모았다.

2026-09-20 후속 결정: 사용자 요청에 따라 512p DPO 실험은 기존 README의 결과 요약만 남기고 폐기했다. 다음은 `2026-09-20-scail2-704-v1`에서 DPO 1.0·입력·프레임 수·시드를 유지한 704×704 생성이다. 원본 입력 파일은 동일하며 내부 리사이즈되므로 참조 디테일을 새로 보강하는 실험은 아니다.

SCAIL-2 DPO 704p 완료: 동일 입력 파일·DPO 1.0 조건에서 704×704·33프레임·20fps를 생성했다. 약 74.7분, PyTorch peak allocated 4.44 GiB / reserved 5.68 GiB. 이동 걷기와 큰 캐릭터 특징은 유지되지만 원본 크기의 얼굴·손·신발 경계가 여전히 부드러워 해상도 증가만으로 개별 프레임 품질이 충분해졌다고 판정하지 않았다. 원본 입력은 512p 그대로이며 사용자 검수는 대기한다. 결과·검수 기록은 `2026-09-20-scail2-704-v1`에 보관한다.

2026-09-20 사용자 검수: 704p 결과에 대해 “품질은 향상되었지만 아직 사용할수 있는 수준은 아니다.”라고 평가했다. 최종 판정은 **품질 향상 확인·실사용 불가**이며, 개선을 부정하거나 제작용으로 승인된 결과로 해석하지 않는다. 이번 평가만으로 추가 폐기나 신규 실험은 실행하지 않는다.

2026-09-20 후속 결정: 704p 결과는 사용자 평가인 **품질 향상 확인·실사용 불가**를 요약으로 남기고 요청대로 폐기했다. 다음은 `2026-09-20-scail2-native-ref-v1`에서 기존 704p DPO 조건을 유지한 채 원본 290×1180 crop → 110×448 인물 영역으로 직접 준비한 참조를 사용한다. 기존 80×326 축소 경로를 제거하는 입력 디테일 가설이며 포즈와 마스크는 유지한다.

원본 기반 704p 참조 실험 완료: 동일 704p DPO 조건에서 원본 crop을 직접 리사이즈한 참조로 33프레임을 생성했다. 약 73.8분, peak allocated 4.44 GiB / reserved 5.68 GiB. 걷기와 큰 특징은 유지되지만 얼굴·손·신발 세부 표현의 부드러움이 남았다. 직전 영상은 폐기되어 개선 정도를 정량 비교하지 않았으며 사용자 검수는 대기한다. 결과·근거는 `2026-09-20-scail2-native-ref-v1`에 보관한다.


### 동작 식별 이후의 개선 방향 — 2026-09-20

사용자는 원본 기반 704p 참조 결과를 “동작은 확인할 수 있을 정도”로 평가했다. 현재 결과를 보존하고, 후속 실험은 동작 식별과 외형 품질을 따로 판정한다. 추가 생성·다운로드는 이번 조사에서 실행하지 않았다.

1. **우선 제안: SeedVR2 영상 복원.** 기존 손실 없는 PNG 33장을 재사용해 원본, 일반 보간 확대, 생성형 영상 복원을 비교한다. 704→1024를 작은 첫 시험안으로 삼되 메모리 실측 후 배치·타일을 정한다. 3B GGUF와 CPU offload·VAE 타일링 경로를 검토한다. 8 GB 설정이 구현 문서에 공개돼 있지만 현재 환경의 실행 속도·메모리는 미검증이다. 단일 프레임 선명도와 영상 깜박임을 함께 평가하며, 새 얼굴·의상 무늬가 생기면 디테일 향상으로 간주하지 않는다. 잘못된 관절·손가락 형태를 복원한다고 보장하지 않는다.
2. **동일성 보강: SCAIL-2 근접 참조 추가.** 기존 전신 참조와 같은 원본에서 얼굴·상반신 근접 crop 하나를 별도 참조와 동일 identity 색상 마스크로 추가하는 대조안이다. 전신 화면 내 110픽셀 폭에 압축된 얼굴 정보와 별도로 세부 외형을 전달한다. 공식 다중 참조는 미최적화·품질 저하 가능성이 명시된 실험 기능이므로 복원과 동시에 바꾸지 않는다.
3. **형태 오류가 남을 때: 세부 제어 입력 보강.** 현재 HumanML22 렌더링에는 손가락 포즈가 없고 머리 좌표도 근사다. 정교한 손·발 방향 제어 또는 3D에서 가림과 좌우를 지정하는 별도 입력 실험을 검토한다. 추출기로 존재하지 않는 정보를 복구할 수 있다고 가정하지 않는다. 이는 로컬 입력 분석에 따른 가설이며 실행 검증 전이다.

참고(2026-09-20 확인): [SeedVR 공식 한계](https://github.com/ByteDance-Seed/SeedVR)는 약하게 열화된 AI 생성 영상에서 디테일 과생성과 과도한 선명화를 경고한다. [SeedVR2 ComfyUI](https://github.com/numz/ComfyUI-SeedVR2_VideoUpscaler)는 8 GB 이하 GGUF·BlockSwap·타일링 및 3B Q8 설정 예를 제공한다. [SCAIL-2 공식 다중 참조](https://github.com/zai-org/SCAIL-2/tree/wan-scail2#experimental-functions-multi-reference)는 근접 참조와 마스크를 지원하지만 품질 저하 가능성도 명시한다.

2026-09-20 SeedVR2 v1 완료: 사용자 선택 1에 따라 현재 SCAIL-2 native-ref 704p 원본을 보존하고 3B Q8_0 복원을 실행했다. 공개 가중치만 사용했고 704→1024, 33프레임, batch 5·overlap 1·LAB·추가 noise 0, 32 blocks CPU offload·VAE 타일 512/64 조건이다. CLI 148.39초, wrapper 156.71초. 초기에는 선의 강조를 개선으로 해석했으나 사용자가 “디테일 개선되지 않았다.”고 평가하여 디테일 개선 없음으로 정정했다. 인물 뒤에 복제 잔상도 확인되어 이번 설정은 미채택이다. 전체 접촉 시트와 다섯 표본 확대를 확인했고 특히 024 원본/복원 직접 비교에서 추가 잔상이 뚜렷하다. 디테일에 관한 사용자 판정은 개선 없음이며, 시간축 재생 품질은 별도로 통과 판정하지 않았다. 잔상 발생 원인은 미분리이며 overlap 0 대조는 잔상 진단 후보일 뿐 디테일 개선 대안으로 검증되지 않았다. 실행·해시·비교·검수 자료는 [SeedVR2 v1 자산 기록](../../docs/assets/part-07/chapter-05/sec-15/2026-09-20-seedvr2-v1/README.md)에 모았다.

2026-09-20 후속 결정: 사용자 요청에 따라 SeedVR2 v1은 실패 요약 README만 남기고 결과·상세 기록을 폐기했다. 가중치는 공용 캐시에 유지한다. 다음 실험 `2026-09-20-scail2-multiref-v1`을 시작했다. 보존된 native-ref 704p 조건에서 같은 원본의 얼굴·상반신·손 크롭을 두 번째 참조와 동일 identity 마스크로 추가한다. 해상도·시드·DPO·포즈·생성 설정은 유지하며 출력 품질은 실행 완료 후 판정한다.
...
2026-09-20 결과 정리: 사용자 지시로 현재 scail2-multiref-v1 영상·33 PNG와 실행 입력·근거만 보존한다. MoMask v4·native-ref 등 나머지 결과와 과거 결과를 포함한 비교판은 삭제했고 각 README 요약에 결론을 남겼다. 2026-09-21 원고 분리에서 모션·포즈 입력의 결론은 P7-5.14로, 캐릭터 영상 결과는 P7-5.15로 옮겼다. 과거의 보존·비교 가능 설명은 당시 이력이다. 근접 실험의 심볼릭 링크 입력은 실제 파일로 이관했으며 공용 모델 가중치는 유지한다.
