...
- 로컬 GPU 배경 참조 셋과 `p7-5-1-style-reference-pack.json`은 화풍 입력으로 쓰되, 각 이미지의 장소·시간·카메라·선·채색 역할과 관찰 한계를 함께 기록한다.
...
- 장면 계약과 캐릭터 계약을 분리한다. RGB는 색·질감·조명을 맡고, 카메라판은 방위·높이·거리 조건을, 인물 마스크와 컷아웃은 교체 영역·포즈·프레이밍을 맡는다.
