...
| Deep Voice | 음성 합성 | 전통적 TTS 파이프라인의 여러 구성요소를 신경망 기반으로 전환 | 보조 사례: 주요 비교축이 아니라 TTS 맥락 확인용 |
...
2. 객체 검출과 음성 생성에서도 신경망은 수작업 파이프라인이나 직접 설계한 특징 일부를 학습 가능한 구조로 바꾸었습니다. TTS는 보조 맥락으로만 둡니다.
...
| Deep Voice가 LLM의 조상이다 | 음성 합성 계열이며 직접 조상도 주요 비교축도 아님 | Deep Voice는 필요한 경우 TTS 보조 사례로만 둔다 |
