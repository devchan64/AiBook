...
| 1.3 AI, ML, DL, GenAI, LLM 관계 | 용어 층위 구분 | 유지 가능 | 즉시 수정 없음 | 생성형 AI와 LLM 관계는 NIST와 LLM survey를 핵심 근거로 유지 |
...
1.3의 용어 층위 구분은 근거와 잘 맞는다. 특히 생성형 AI를 딥러닝의 단순 하위 단계처럼 단정하지 않고, 출력 성격과 서비스 범주로 설명한 점은 안전하다. `AI > ML > DL > GenAI > LLM` 줄은 강한 단순화라고 본문에서 이미 제한하고 있으므로 유지 가능하다.
