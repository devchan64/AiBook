...
| --- | --- | --- | --- |
| Module 1. 역할과 문제 장면 | 데이터 모델링의 위치와 데이터셋 환상을 먼저 정리 | Chapter 1-2 | problem framing, data lifecycle, dataset construction |
| Module 2. 샘플과 표 구조 | 한 행, 한 샘플, 원시 로그, 요약 표의 차이를 고정 | Chapter 3-4 | sample unit, data representation, data wrangling |
| Module 3. 특징과 비교 구조 | feature와 baseline comparison 구조를 읽게 함 | Chapter 5-6 | feature engineering, representation, baseline comparison |
| Module 4. 해석 경계 | 보수적 해석과 불확실성 경계를 고정 | Chapter 7 | sampling, inference, uncertainty |
| Module 5. 문제 유형과 handoff | 경고, 검토, 예측 문제를 나누고 Part 4로 넘김 | Chapter 8-9 | problem framing, operational labels, classification/regression handoff |

## 모듈별 상세 목록

### Module 1. 역할과 문제 장면

- 목표: 데이터 모델링을 DB 설계나 전처리 하위 절로 축소하지 않게 하고, 눈앞의 표를 곧바로 데이터셋처럼 읽는 습관을 먼저 흔든다.
- 포함 Chapter:
...
- 목표: `한 행이 곧 샘플 1건`이라는 자동 가정을 깨고, 원시 로그와 분석용 표의 차이를 함께 고정한다.
- 포함 Chapter:
...
- 목표: feature를 열 선택이 아니라 표현 설계로 이해하게 하고, 기준선 비교표를 낯선 표가 아니게 만든다.
- 포함 Chapter:
...
- 목표: 비교 리포트, 검토 큐, 예측 문제의 경계를 정리하고 Part 4가 이어받을 전제를 고정한다.
- 포함 Chapter:
...
1. Module 1. 역할과 문제 장면
2. Module 2. 샘플과 표 구조
3. Module 3. 특징과 비교 구조
4. Module 4. 해석 경계
5. Module 5. 문제 유형과 handoff
...
- Module 1은 전체 지도와 문제 장면을 먼저 줘서 초반 속도를 늦추고 용어가 공중에 뜨지 않게 한다.
- Module 2는 `한 행 = 샘플 1건` 오해와 `원시 로그 = 데이터셋` 오해를 함께 정리한다.
- Module 3은 feature와 baseline comparison이 모두 `설계된 비교 구조` 위에서 읽힌다는 점을 보여 준다.
- Module 4는 그 뒤에야 보수적 해석을 말해도 무엇을 두고 보수적으로 말하는지 이해할 수 있다.
- Module 5는 마지막에 문제 유형과 handoff를 함께 정리해 모델 이름이 너무 빨리 튀어나오지 않게 한다.
...
| 모듈 | docs 경로 |
| --- | --- |
| Module 1 | `chapter-01`, `chapter-02` |
| Module 2 | `chapter-03`, `chapter-04` |
| Module 3 | `chapter-05`, `chapter-06` |
| Module 4 | `chapter-07` |
| Module 5 | `chapter-08`, `chapter-09` |

## 현재 결론

Part 3 데이터 모델링은 `5개 모듈, 9개 Chapter, 18개 Section`으로 운영하는 편이 더 안정적이다. 이 구조는 외부 커리큘럼의 반복 항목을 흡수하면서도, 현재 책이 사용하는 `자동으로 실행되는 동작 1회와 그 원천 시계열` 사례 흐름을 유지하고 초심자 부담을 더 고르게 나눌 수 있다.
