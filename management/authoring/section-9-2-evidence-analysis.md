...
- 9.1의 이미지 인식(image recognition)과 표현 학습(representation learning)을 이어 받아, 딥러닝 패러다임이 객체 검출(object detection)과 음성 생성(speech generation)으로 확산된 흐름을 설명합니다.
- YOLO와 WaveNet을 LLM의 직접 계보가 아니라 딥러닝 확산의 주변 근거로 정리합니다. Deep Voice는 TTS 보조 사례로만 둡니다.
...
| Deep Voice는 TTS 파이프라인 구성요소를 신경망 기반으로 전환했다 | Deep Voice 논문의 five major building blocks와 neural networks 설명 | 보조 사례로만 유지 |
...
| 9.2 | YOLO와 WaveNet 중심의 딥러닝 패러다임 확산 사례. TTS는 보조 맥락 |
...
- “Deep Voice가 완전한 end-to-end TTS를 완성했다”라고 쓰지 않았습니다. 전통적 파이프라인 구조를 유지하면서 구성요소를 신경망으로 바꿔 간 보조 사례로만 설명했습니다.
...
- Part 4에서 CNN, autoregressive model, dilated convolution을 기술적으로 확장합니다. TTS 파이프라인은 필요할 때 별도 보조 사례로 다룹니다.
