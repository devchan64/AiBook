...
5.5의 중심을 단방향 파이프라인 구성으로 좁혀, 위 재생성 비교 이미지·JSON과 B·C 직접 아이덴티티 비교는 본문에서 제외했다. 재생성 사실과 검수 기록, 자산 파일은 유지하며 현재 파이프라인의 입력으로 취급하지 않는다. 단계별 재현값과 채택 결과의 한계는 원고에 남긴다.

## 기본 파이프라인 도식

`p7-5-5-scene-pipeline-ko.mmd`는 현행 원고의 입력·출력 연결을 요약한 자체 제작 도식이다. 외부 도식을 복제하거나 변형하지 않았다. 캐릭터·배경의 두 흐름과 합성·텍스트 Relight의 연결을 설명하며 성능이나 픽셀 보존을 보장하는 근거로 사용하지 않는다.
