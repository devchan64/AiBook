...
P5-8.1에서는 챕터 8의 첫 축인 `일반화 제약`을 열었습니다. 이제 그 제약이 손실 함수 옆의 벌점만이 아니라, 신경망 내부 경로를 흔드는 방식으로도 나타날 수 있는지로 시야를 넓힙니다. 여기서 다음 질문이 자연스럽게 이어집니다.

가중치에 벌점을 주는 것 말고, 신경망 구조 자체를 흔들어 과적합을 줄이는 방법도 있는가?

이 질문에 답하는 대표적 방법이 드롭아웃(dropout)입니다. 즉, 이 절은 챕터 8의 `일반화 제약` 축을 구조 수준의 regularization 사례로 구체화하는 자리입니다.
