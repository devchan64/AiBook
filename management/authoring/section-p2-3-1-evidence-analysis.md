...
- 스칼라에서 벡터, 행렬, 위치 비교로 사고가 확장되는 흐름을 별도 문단으로 추가했습니다.
...
- 같은 shape의 벡터와 행렬은 위치별 덧셈을 할 수 있고, 스칼라배는 각 값에 같은 숫자를 곱하는 계산이라는 가벼운 수학적 접근을 추가했습니다.
...
- 행렬의 위치별 덧셈과 스칼라배는 다루되, 행렬 곱과 선형 변환은 P2-3.3으로 넘겼습니다.
