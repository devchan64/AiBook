...
P7-5는 이 노트의 이미지·guide·결과 JSON에 승인·미승인·보류 상태를 부여하지 않는다. 이후 비교는 입력 역할, 실행 조건, 관찰된 일치·이탈, 알려진 한계와 다음 질문을 기록한다. 아래의 `승인`, `후보`, `통과`, `탈락`, `사람 검수` 표현은 당시 실험의 이력이며, 현재 원고 작성이나 다음 입력 선택을 막는 상태 gate가 아니다.

## 목적

Qwen-Image-Edit-2509에서 `구도`, `화풍`, `캐릭터 identity·착장`을 서로 다른 입력 역할로 둘 때 어떤 계약이 유지·충돌하는지 확인한다. 결과를 하나의 품질 상태로 승격하지 않고, 다음 단계에서는 이 노트에 기록한 입력 역할과 한계에 맞추어 참고한다.

## 현재 증거

| 조건 | 입력 역할 | 확인된 관찰 | 관찰 메모 |
| --- | --- | --- | --- |
| 고각도 보행, 3입력, seed `62294/62295` | guide / 얼굴 / 완성 착장 | 두 seed에서 고각도 보행·얼굴·착장·가방 strap이 함께 남았다. | 고정 guide·보행 범위에서 네 입력 역할이 함께 관찰된 사례다. |
| 고각도 보행, style clause, seed `62294` | 같은 3입력 / prompt의 화풍 문장만 추가 | 선화와 색 번짐이 기존 고각도 결과보다 P7-5.1의 수채화 기준에 가까워 보인다. | 동일 seed baseline과의 계약별 비교가 더 필요하다. |
| P7-5.3 Scene C, 3입력, seed `62294` | storyboard / 얼굴 / 완성 착장 | 공중 split leap·청록 단발·재킷·바지·가방은 대체로 남았지만, 자갈 바닥이 사진 질감으로 남았다. | 화풍 이탈이 관찰됐으며, 이 입력 조합의 제한으로 기록한다. |
| P7-5.3 Scene C, 4입력, seed `62294` | storyboard / 얼굴 / 완성 착장 / 화풍 이미지 | 화풍 입력을 더하자 실사 인물·검은 머리·검은 의상으로 바뀌었다. | identity·outfit·style 이탈이 함께 관찰됐다. |
| P7-5.3 Scene C, guide-first B1, seed `62294` | 수채화화 guide / 얼굴 / 완성 착장 | 중간 guide의 배경은 수채화화됐지만 익명 인물의 검은 머리·검은 의상이 남았다. 최종 3입력도 그 인물을 교체하지 않고 유지했다. | identity·outfit 이탈이 명확해 추가 seed 비교를 하지 않았다. |
| P7-5.3 Scene C, identity-free 구조 guide B2, seed `62294/62295` | depth 실루엣 기반 구조 guide / 얼굴 / 완성 착장 | 두 seed 모두 청록 단발, 흰 재킷, 청록 와이드 바지, 흰 운동화, 남색 crossbody bag·strap, split leap와 분리 그림자를 함께 만들었다. | 제한된 구조 guide 조건에서 교차 seed 관찰을 확보했다. |
| P7-5.3 Scene A, identity-free 구조 guide B2, seed `62294` | depth 기반 협곡 구조 guide / 얼굴 / 완성 착장 | 캐릭터·착장·수채화는 유지했지만, 양쪽 협곡 벽과 로우앵글 공간 구조가 한쪽 절벽과 넓은 여백으로 단순화됐다. | structure 이탈로 추가 seed 비교를 하지 않았다. |
| P7-5.3 Scene B, identity-free 구조 guide B2, seed `62294` | depth 기반 평지 구조 guide / 얼굴 / 완성 착장 | 캐릭터·착장·수채화·평지와 낮은 수평선은 남았지만, 원래의 곧은 양쪽 다리 split leap가 한쪽 다리를 굽힌 도약으로 바뀌었다. | 구도 일부만 남고 캐릭터·소품·화풍 이탈이 관찰됐다. |
...
| --- | --- | --- | --- |
| A | 양쪽 협곡 벽·로우앵글 공간 구조 | 양쪽 협곡 벽과 split leap가 다시 나타났다. | 구조는 개선됐지만 청록 헤어가 검은색으로 바뀌어 identity 계약 미통과. |
...
| --- | --- | --- | --- | --- | --- |
| A | 탈락 | 탈락 | 탈락 | 탈락 | 협곡·도약·청록 단발·가방은 남았으나, 넓은 로우앵글 대신 밀집된 근거리 협곡이 됐다. 양쪽 황금 홍채와 회색 이너 셔츠가 확인되지 않고, 선화 중심이라 수채화 계약도 탈락했다. |
...
Scene A에서 승인 relative depth·character sheet·style image를 사용한 해상도/step 매트릭스를 진행했으나, 해상도를 올릴수록 품질 저하가 더 뚜렷해져 전체 27조건을 중단했다.

- `720×720`: 색 wash와 기본 착장은 비교적 남았지만 가방·외부 strap이 사라지고, 넓고 완만하게 높은 협곡 구도도 유지되지 않았다.
- `1024×1024`: 가방 또는 strap이 일부 되살아나는 조건은 있었지만, 배경의 색 wash가 빠지고 선화·빈 협곡 면이 우세해졌다.
- `1280×1280`: 협곡이 밀집된 근거리 선화로 과장되고, 수채화·이너 셔츠·얼굴 특징이 더 흔들렸다.

따라서 이 현상은 저해상도 세부 부족을 step/해상도로 보완하지 못한 사례다. raw relative depth를 일반 Edit 이미지로 넣는 현재 경로는 native ControlNet 제약이 아니므로, 높은 해상도에서 depth·character sheet·별도 style image의 역할 경쟁이 더 크게 드러난다. 이는 원본 storyboard RGB를 사용했다는 뜻이 아니다. 실행 JSON의 `structure_source`와 경로는 `storyboard-depth`였으며, **고해상도에서 구조 드리프트가 더 커졌다**는 관찰이다.

high-angle role comparison과의 품질 차이는 해상도 단독 비교가 아니다. high-angle 기준선은 의미가 있는 illustrated guide·얼굴 reference·별도 outfit reference를 3역할로 분리하고 40 step을 사용한다. 반면 이 매트릭스는 합성 character sheet·raw depth·다른 장면의 style image를 결합했다. 다음 실험은 해상도/step을 더 바꾸기 전에 이 입력 역할 차이를 하나씩 통제해야 한다.

### A씬 종횡비 정렬 대조 (2026-08-18)

승인 A relative depth(`1152×1152`)를 직접 Edit 참조로 고정하고, seed `62294`, 40 step, `true_cfg_scale=4.0`에서 캐릭터 참조와 출력의 비율만 바꿨다. 첫 조건은 원본 전신 `960×1440`을 왜곡·자르지 않고 `768×1152`로 축소한 뒤 좌우 `192 px`의 off-white 여백을 넣어 `1152×1152`로 만들었다. 두 번째 조건은 원본 전신 2:3과 `768×1152` 출력으로 정렬했다.

| 조건 | 구조·프레이밍 | 의상·소품 | 결론 |
| --- | --- | --- | --- |
| `1024×1024` 출력 + 1:1 패딩 전신 | 협곡·전신 split은 남았지만 밀집된 선화 협곡이 됐다. | 바지 폭이 불안정하고 한쪽 신발만 단순하게 남았으며 가방·외부 strap이 없다. | 네 계약 탈락. |
| `768×1152` 출력 + 원본 2:3 전신 | 바닥 여백과 로우앵글 읽기는 더 분명해졌다. | 바지는 여전히 가는 실루엣으로 수렴했고 신발·가방·외부 strap이 사라졌다. | 네 계약 탈락. |
