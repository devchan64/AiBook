...
| 再把损失和反向传播接上 | 只有看见这个结构按什么标准、怎样被调整，学习才会显出来。 |
...
复习时，与其只重读这个总结页就停下来，不如在术语开始模糊的地方回到[英文概念词汇表](/AiBook/en/reference/concept-glossary/)，把对应词条和它指向的 `Core Section` 一起再看一遍，会更稳定。
...
1. 先看感知机与多层神经网络。
2. 再把激活函数、输出层、损失、反向传播连起来。
3. 再看学习与模型执行的区分。
4. 再看优化器、正则化、dropout。
5. 再看 GPU、batch、tensor 计算与表征学习。
6. 再看 CNN、RNN、LSTM、GRU。
7. 再看 Attention 与 self-attention。
8. 再看 Transformer。
9. 再看生成模型与采样。
10. 最后整理它如何连到下一 Part 的 LLM 与生成式 AI。
...
| 用损失和反向传播去调整 | 为什么 next-token prediction 会连到生成？ |
...
按当前目录来看，这个流程更自然的重读方式是 `神经网络起点 -> 输出、损失、反向传播 -> 学习循环与稳定化 -> 计算环境与表征学习 -> 结构分叉 -> 生成`。
...
| 反向传播 | 反向传播不是让整个神经网络神奇学会的名字，而是高效传递误差贡献的计算过程。 |
...
| 学习过程 | 损失、反向传播、优化器、正则化、dropout、batch | 它说明即使是同一种结构，学习稳定性、泛化、计算成本也会不同。 |
...
| 能把损失、反向传播、优化器、正则化与结构分开说明 | 服务设置、推理成本、长上下文运营优化 |
