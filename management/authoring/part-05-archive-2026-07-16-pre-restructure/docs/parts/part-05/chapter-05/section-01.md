...
- 자동미분(automatic differentiation)은 역전파를 코드에서 어떻게 가능하게 하는가?
...
- 자동미분 엔진의 내부 구현과 엄밀한 일반 이론

복잡한 미분 흐름을 계산 그래프로 보는 관점은 P5-5.2에서 이어서 다루고, 실제 파라미터 갱신과 optimizer의 역할은 P5-7.1, P5-7.2에서 다시 연결합니다. 자동미분(automatic differentiation) 자체는 역전파를 이해하는 데 필요한 개념입니다. 다만 프레임워크 내부 엔진이 계산 그래프를 저장하고, 미분 규칙을 최적화하고, 메모리를 관리하는 세부 구현까지는 여기서 다루지 않습니다.

이 절에서 필요한 수준은 분명합니다. `PyTorch나 TensorFlow 같은 도구가 순전파 계산 과정을 기록해 두었다가, 손실에서 거꾸로 따라가며 gradient 계산을 자동으로 조직한다`는 점을 알아야 합니다. 손으로 모든 미분식을 전개하지 않아도 역전파가 실행되는 이유가 바로 자동미분입니다.
